...
public class Receiver extends BroadcastReceiver
{
    ...
    private String loadRules(Context context)
    {
        ...
        try {
            ...
            Log.e(TAG, "Failed to open a4a_rules.jon when populating ANGLE whitelist: ", ioe);
        }
        ...
        try {
            ...
            for (int i = 0; i < rules.length(); i++) {
                ...
                for (int j = 0; j < apps.length(); j++) {
                    ...
                    if (app == null) {
                        Log.v(TAG, "Skipping Application entry with no AppName");
                        continue;
                    }
                    if (packageNames.isEmpty()) {
                        packageNames += app.optString("AppName");
                    } else {
                        packageNames += "," + app.optString("AppName");
                    }
                }
