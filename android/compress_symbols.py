...
def main():
  ...
  parser.add_argument('--strip',
                      ...
                      metavar='PATH')
  parser.add_argument('--unstrippedsofile',
                      required=True,
  ...
  nm_cmd = subprocess.Popen([args.nm, args.unstrippedsofile, '--format=posix',
                             '--defined-only'],
  ...
  awk_cmd = subprocess.Popen(['awk', '{ if ($2 == "T" || $2 == "t" ||' +
                              ' $2 == "D") print $1 }'],
                             stdin=nm_cmd.stdout,
                             stdout=subprocess.PIPE)
  ...
  sort_cmd = subprocess.Popen(['comm', '-13', args.output + '.dynsyms',
                               args.output + '.funcsyms'],
                              stdin=awk_cmd.stdout,
                              stdout=keep_symbols)
  ...
  objcopy_cmd = [args.objcopy, '--rename-section',
                 '.debug_frame=saved_debug_frame',
                 args.output + '.debug', args.output + ".mini_debuginfo"]
  subprocess.check_call(objcopy_cmd)

  objcopy_cmd = [args.objcopy, '-S', '--remove-section', '.gdb_index',
                 '--remove-section', '.comment',
                 '--keep-symbols=' + args.output + '.keep_symbols',
                 args.output + '.mini_debuginfo']
  subprocess.check_call(objcopy_cmd)

  objcopy_cmd = [args.objcopy, '--rename-section',
                 '.saved_debug_frame=.debug_frame',
                 args.output + ".mini_debuginfo"]
  subprocess.check_call(objcopy_cmd)
  ...
  objcopy_cmd = [args.objcopy, '--add-section',
                 '.gnu_debugdata=' + args.output + '.mini_debuginfo.xz',
                 args.output]
  subprocess.check_call(objcopy_cmd)

  rm_cmd = ['rm', '-f', args.output + '.dynsyms', args.output + '.funcsyms',
