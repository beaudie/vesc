        ...
        ['OS=="win"',
        {
            ...
            [
...

                {
                    ...
                    'sources':
                    [
                        '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/sample_util -types *.cpp *.h)'
                    ],
...

                {
                    ...
                    'type': 'executable',
                    'dependencies': [ 'sample_util' ],
                    'sources': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/mip_map_2d -types *.cpp *.h)' ],
                },

                {
                    ...
                    'type': 'executable',
                    'dependencies': [ 'sample_util' ],
                    'sources': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/multi_texture -types *.cpp *.h)' ],
                    'copies':
                    [
                        {
                            'destination': '<(PRODUCT_DIR)',
                            'files': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/multi_texture -types *.tga)' ],
                        },
                    ]
                },
...

                {
                    ...
                    'type': 'executable',
                    'dependencies': [ 'sample_util' ],
                    'sources': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/simple_instancing -types *.cpp *.h)' ],
                },
...

                {
                    ...
                    'type': 'executable',
                    'dependencies': [ 'sample_util' ],
                    'sources': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/simple_texture_2d -types *.cpp *.h)' ],
                },
...

                {
                    ...
                    'type': 'executable',
                    'dependencies': [ 'sample_util' ],
                    'sources': [ '<!@(python <(angle_build_scripts_path)/enumerate_files.py angle/simple_vertex_shader -types *.cpp *.h)' ],
                },
