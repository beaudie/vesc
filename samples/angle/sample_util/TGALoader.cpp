...
bool LoadTGAImageFromFile(const std::string &path, TGAImage *image)
{
    ...
    for (size_t y = 0; y < header.height; y++)
    {
        ...
        for (size_t x = 0; x < header.width; x++)
        {
            ...
            Pixel pix;
            pix[0] = (pixelComponentCount > 2) ? buffer[pixelIdx + 2] : 0;
            pix[2] = (pixelComponentCount > 0) ? buffer[pixelIdx + 0] : 0;
            pix[1] = (pixelComponentCount > 1) ? buffer[pixelIdx + 1] : 0;
            pix[3] = (pixelComponentCount > 3) ? buffer[pixelIdx + 3] : 255;

            image->data.push_back(pix);
        }
    }

    std::cout << "opened image " << path << ".\n";
