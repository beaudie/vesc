...
class ModelSample : public SampleApplication
{
  public:
    ModelSample(EGLint displayType)
        : SampleApplication("ModelSample", 1280, 720, 1 /* gles1 */, 0, displayType)
    {
    }
    ...
    virtual bool initialize()
    {
        ...
        // Load the texture
        mTexture =
            CreateTextureFromPng(diffuseTextureName.str().c_str(), &mTextureWidth, &mTextureHeight);
        ...
        glBufferData(GL_ARRAY_BUFFER, mVertexAttrs.size() * sizeof(ObjModel::VertexAttributes),
                     mVertexAttrs.data(), GL_STATIC_DRAW);
