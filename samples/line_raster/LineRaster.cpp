...
class LineRasterSample : public SampleApplication
{
  public:
    ...
    bool initialize() override
    {
        ...
        const std::string lineFS2 = R"(precision mediump float;
varying vec2 pos;
uniform vec4 color;
void main()
{
    vec2 b = ((pos * 0.5) + 0.5) * 300.0;
    ...
    void draw() override
    {
        ...
        GLfloat time = 0;// static_cast<GLfloat>(mTimer->getElapsedTime()) * 0.1f;
        mat[0]       = cosf(time);
        mat[1]       = -sinf(time);
        mat[2]       = -mat[1];
        mat[3]       = mat[0];
