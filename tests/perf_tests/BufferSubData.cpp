...
template <class T>
GLsizeiptr GetSignedNormalizedData(GLsizeiptr numElements, GLfloat *floatData, std::vector<uint8_t> *data)
{
...
template <class T>
GLsizeiptr GetUnsignedNormalizedData(GLsizeiptr numElements, GLfloat *floatData, std::vector<uint8_t> *data)
{
    GLsizeiptr triDataSize = sizeof(T) * numElements;
    data->resize(triDataSize);
...
GLsizeiptr GetVertexData(GLenum type, GLint componentCount, GLboolean normalized, std::vector<uint8_t> *data)
{
    ...
    else if (normalized == GL_TRUE)
    {
        ...
        switch (type)
        {
          case GL_BYTE: triDataSize = GetSignedNormalizedData<GLbyte>(numElements, floatData, data); break;
          case GL_SHORT: triDataSize = GetSignedNormalizedData<GLshort>(numElements, floatData, data); break;
          case GL_INT: triDataSize = GetSignedNormalizedData<GLint>(numElements, floatData, data); break;
          case GL_UNSIGNED_BYTE: triDataSize = GetUnsignedNormalizedData<GLubyte>(numElements, floatData, data); break;
          case GL_UNSIGNED_SHORT: triDataSize = GetUnsignedNormalizedData<GLushort>(numElements, floatData, data); break;
          case GL_UNSIGNED_INT: triDataSize = GetUnsignedNormalizedData<GLuint>(numElements, floatData, data); break;
          default: assert(0);
        }
    }
    else
...
bool BufferSubDataBenchmark::initializeBenchmark()
{
    ...
    (
        ...
        void main()
        {
            gl_Position = vPosition * vec4(uScale) - vec4(uOffset);
        }
    );
    ...
    mUpdateData = new char[mParams.updateSize];
