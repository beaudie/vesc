...
public:
    ...
    virtual const eglw::EGLAttrib*  getPlatformAttributes(void) const { return &m_platformAttributes[0]; }
...
private:
    ...
    std::vector<eglw::EGLAttrib> m_platformAttributes;
};
...
public:
	...
	virtual eglu::NativePixmap*	createPixmap			(eglu::NativeDisplay* nativeDisplay, eglw::EGLDisplay display, eglw::EGLConfig config, const eglw::EGLAttrib* attribList, int width, int height) const;
};
...
public:
    ...
    eglw::EGLNativePixmapType			getLegacyNative(void) { return m_bitmap; }
...
public:
    ...
    eglw::EGLNativeWindowType				getLegacyNative(void) { return m_window.getHandle(); }
...
NativeDisplay::NativeDisplay (void)
	...
	, m_deviceContext		((HDC)((eglw::EGLNativeDisplayType)0))
	, m_library				("libEGL.dll")
{
    m_platformAttributes.push_back(EGL_PLATFORM_ANGLE_TYPE_ANGLE);
    m_platformAttributes.push_back(EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE);
    m_platformAttributes.push_back(EGL_NONE);
    m_platformAttributes.push_back(EGL_NONE);
}
...
eglu::NativePixmap* NativePixmapFactory::createPixmap (eglu::NativeDisplay* nativeDisplay, eglw::EGLDisplay display, eglw::EGLConfig config, const eglw::EGLAttrib* attribList, int width, int height) const
{
    ...
    DE_ASSERT(display != ((eglw::EGLDisplay)0));
...
eglu::NativeDisplay* Win32EGLNativeDisplayFactory::createDisplay(const eglw::EGLAttrib* attribList) const
{
	DE_UNREF(attribList);
