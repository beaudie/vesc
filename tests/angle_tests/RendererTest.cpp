...
{
  protected:
    RendererTest() : ANGLETest(GetGlesMajorVersion<T>(), GetRequestedRenderer<T>())
    {
        setWindowWidth(128);
        setWindowHeight(128);
    }
...
TYPED_TEST(RendererTest, RequestedRendererCreated)
{
    ...
    if (fixtureType.requestedRenderer == EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE || fixtureType.requestedRenderer == EGL_PLATFORM_ANGLE_TYPE_D3D11_WARP_ANGLE)
    {
    ...
    if (fixtureType.requestedRenderer == EGL_PLATFORM_ANGLE_TYPE_D3D9_ANGLE)
    {
        ASSERT_NE(rendererString.find(std::string("direct3d9")), std::string::npos);
    }

    // Ensure that the renderer uses WARP, if we requested it.
    if (fixtureType.requestedRenderer == EGL_PLATFORM_ANGLE_TYPE_D3D11_WARP_ANGLE)
    {
    ...
    if (fixtureType.glesMajorVersion == 3)
    {
    ...
    if (fixtureType.glesMajorVersion == 2)
    {
