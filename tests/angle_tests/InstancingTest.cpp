  ...
  protected:
    ...
    void runDrawArraysTest(GLuint program, GLint first, GLsizei count, GLsizei instanceCount, float *offset)
    {
        ...
        for (unsigned int quadIndex = 0; quadIndex < 4; ++quadIndex)
        {
            ...
            float quadx = ((mInstances[baseOffset + 0] + offset[baseOffset + 0]) * 0.5f + 0.5f) * getWindowWidth();
            float quady = ((mInstances[baseOffset + 1] + offset[baseOffset + 1]) * 0.5f + 0.5f) * getWindowHeight();
