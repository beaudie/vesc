...
static std::string GetTestWindowName()
{
    return "ANGLE_TEST";
}
...
bool ANGLETest::InitTestWindow()
{
    ...
    if (!mOSWindow->initialize(GetTestWindowName(), 128, 128))
    {
        return false;
    }

    // Note- our only Windows-specific line of code
    SetWindowLong(mOSWindow->getNativeWindow(), GWL_STYLE, 0);
