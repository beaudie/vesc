...
// NOTE:  Beware the lifespan of the rp_begin when holding  the return.  If the rp_begin isn't a
// "safe" copy, "IMAGELESS"
//        attachments won't persist past the API entry point exit.
static std::pair<uint32_t, const VkImageView *> GetFramebufferAttachments(
    ...
    const FRAMEBUFFER_STATE &fb_state)
{
    ...
    if (fb_state.createInfo.flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT)
    {
        const auto *framebuffer_attachments =
            LvlFindInChain<VkRenderPassAttachmentBeginInfo>(rp_begin.pNext);
...
std::vector<ImageViewPointer> GetAttachmentViewsImpl(const VkRenderPassBeginInfo &rp_begin,
                                                     const FRAMEBUFFER_STATE &fb_state,
                                                     const Get &get_fn)
{
    ...
    const FRAMEBUFFER_STATE &fb_state) const
{
    ...
    return GetAttachmentViewsImpl<std::shared_ptr<const IMAGE_VIEW_STATE>>(rp_begin, fb_state,
                                                                           get_fn);
}
...
VkFormatFeatureFlags2KHR ValidationStateTracker::GetExternalFormatFeaturesANDROID(
    const CreateInfo *create_info) const
{
    VkFormatFeatureFlags2KHR format_features = 0;
    const VkExternalFormatANDROID *ext_fmt_android =
        LvlFindInChain<VkExternalFormatANDROID>(create_info->pNext);
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    auto ahb_format_props2 =
        LvlFindInChain<VkAndroidHardwareBufferFormatProperties2ANDROID>(pProperties->pNext);
    if (ahb_format_props2)
    {
        ahb_ext_formats_map.insert(ahb_format_props2->externalFormat,
                                   ahb_format_props2->formatFeatures);
    }
    else
    {
        auto ahb_format_props =
            LvlFindInChain<VkAndroidHardwareBufferFormatPropertiesANDROID>(pProperties->pNext);
        if (ahb_format_props)
        {
            ahb_ext_formats_map.insert(
                ahb_format_props->externalFormat,
...
VkFormatFeatureFlags2KHR GetImageFormatFeatures(VkPhysicalDevice physical_device,
                                                bool has_format_feature2,
                                                bool has_drm_modifiers,
                                                VkDevice device,
                                                VkImage image,
                                                VkFormat format,
                                                VkImageTiling tiling)
{
    ...
    // Add feature support according to Image Format Features
    // (vkspec.html#resources-image-format-features) if format is AHB external format then the
    // features are already set
    if (has_format_feature2)
    {
        auto fmt_drm_props = LvlInitStruct<VkDrmFormatModifierPropertiesList2EXT>();
        auto fmt_props_3 =
            LvlInitStruct<VkFormatProperties3KHR>(has_drm_modifiers ? &fmt_drm_props : nullptr);
        ...
        if (tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT)
        {
            ...
            {
                if (fmt_drm_props.pDrmFormatModifierProperties[i].drmFormatModifier ==
                    drm_format_props.drmFormatModifier)
                {
                    format_features = fmt_drm_props.pDrmFormatModifierProperties[i]
    ...
    else if (tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT)
    {
        ...
        VkDrmFormatModifierPropertiesListEXT drm_properties_list =
            LvlInitStruct<VkDrmFormatModifierPropertiesListEXT>();
        ...
        for (uint32_t i = 0; i < drm_properties_list.drmFormatModifierCount; i++)
        {
            ...
            {
                format_features = drm_properties_list.pDrmFormatModifierProperties[i]
                                      .drmFormatModifierTilingFeatures;
                break;
            }
        }
...
std::shared_ptr<IMAGE_STATE> ValidationStateTracker::CreateImageState(
    VkImage img,
    ...
    VkFormatFeatureFlags2KHR features)
{
    ...
    else if (pCreateInfo->flags & VK_IMAGE_CREATE_DISJOINT_BIT)
    {
        ...
        switch (plane_count)
        {
            case 3:
                state =
                    std::make_shared<IMAGE_STATE_MULTIPLANAR<3>>(this, img, pCreateInfo, features);
                break;
            case 2:
                state =
                    std::make_shared<IMAGE_STATE_MULTIPLANAR<2>>(this, img, pCreateInfo, features);
                break;
            case 1:
                state =
                    std::make_shared<IMAGE_STATE_MULTIPLANAR<1>>(this, img, pCreateInfo, features);
                break;
            default:
                // Not supported
                assert(false);
...
std::shared_ptr<IMAGE_STATE> ValidationStateTracker::CreateImageState(
    ...
    VkFormatFeatureFlags2KHR features)
{
    return std::make_shared<IMAGE_STATE_NO_BINDING>(this, img, pCreateInfo, swapchain,
                                                    swapchain_index, features);
}

void ValidationStateTracker::PostCallRecordCreateImage(VkDevice device,
                                                       const VkImageCreateInfo *pCreateInfo,
                                                       ...
                                                       VkResult result)
{
    ...
    {
        format_features =
            GetImageFormatFeatures(physical_device, has_format_feature2,
...
void ValidationStateTracker::PreCallRecordDestroyImage(VkDevice device,
                                                       VkImage image,
                                                       const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordCmdClearColorImage(VkCommandBuffer commandBuffer,
                                                             VkImage image,
...
void ValidationStateTracker::PreCallRecordCmdClearDepthStencilImage(
...
void ValidationStateTracker::PreCallRecordCmdCopyImage(VkCommandBuffer commandBuffer,
                                                       VkImage srcImage,
                                                       ...
                                                       const VkImageCopy *pRegions)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYIMAGE, Get<IMAGE_STATE>(srcImage),
                                Get<IMAGE_STATE>(dstImage));
}

void ValidationStateTracker::PreCallRecordCmdCopyImage2KHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdCopyImage2(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdResolveImage(VkCommandBuffer commandBuffer,
                                                          VkImage srcImage,
                                                          VkImageLayout srcImageLayout,
                                                          VkImage dstImage,
                                                          VkImageLayout dstImageLayout,
                                                          ...
                                                          const VkImageResolve *pRegions)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_RESOLVEIMAGE, Get<IMAGE_STATE>(srcImage),
                                Get<IMAGE_STATE>(dstImage));
}

void ValidationStateTracker::PreCallRecordCmdResolveImage2KHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdResolveImage2(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBlitImage(VkCommandBuffer commandBuffer,
                                                       VkImage srcImage,
                                                       VkImageLayout srcImageLayout,
                                                       VkImage dstImage,
                                                       VkImageLayout dstImageLayout,
                                                       ...
                                                       VkFilter filter)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_BLITIMAGE, Get<IMAGE_STATE>(srcImage),
                                Get<IMAGE_STATE>(dstImage));
}

void ValidationStateTracker::PreCallRecordCmdBlitImage2KHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBlitImage2(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCreateBuffer(VkDevice device,
                                                        ...
                                                        VkResult result)
{
    ...
    if (pCreateInfo->flags & VK_BUFFER_CREATE_SPARSE_BINDING_BIT)
    {
        ...
        {
            buffer_state =
                std::make_shared<BUFFER_STATE_SPARSE<false>>(this, *pBuffer, pCreateInfo);
        }
    }
    else
    {
    ...
    if (pCreateInfo)
    {
        const auto *opaque_capture_address =
            LvlFindInChain<VkBufferOpaqueCaptureAddressCreateInfo>(pCreateInfo->pNext);
        if (opaque_capture_address && (opaque_capture_address->opaqueCaptureAddress != 0))
        {
            ...
            buffer_address_map_.split_and_merge_insert(
                {address_range, {buffer_state}},
                [](auto &current_buffer_list, const auto &new_buffer) {
                    assert(!current_buffer_list.empty());
                    const auto buffer_found_it = std::find(
                        current_buffer_list.begin(), current_buffer_list.end(), new_buffer[0]);
...
void ValidationStateTracker::PostCallRecordCreateBufferView(
    VkDevice device,
    ...
    VkResult result)
{
    ...
    if (has_format_feature2)
    {
        ...
        auto fmt_props_2 = LvlInitStruct<VkFormatProperties2>(&fmt_props_3);
        DispatchGetPhysicalDeviceFormatProperties2(physical_device, pCreateInfo->format,
                                                   &fmt_props_2);
    ...
    {
        VkFormatProperties format_properties;
        DispatchGetPhysicalDeviceFormatProperties(physical_device, pCreateInfo->format,
                                                  &format_properties);
...
void ValidationStateTracker::PostCallRecordCreateImageView(VkDevice device,
                                                           ...
                                                           VkResult result)
{
    ...
    {
        format_features = GetImageFormatFeatures(
            physical_device, has_format_feature2,
    ...
    // filter_cubic_props is used in CmdDraw validation. But it takes a lot of performance if it
    // does in CmdDraw.
    ...
    if (IsExtEnabled(device_extensions.vk_ext_filter_cubic))
    {
        ...
        auto image_format_info =
            LvlInitStruct<VkPhysicalDeviceImageFormatInfo2>(&imageview_format_info);
        ...
        image_format_info.usage =
            usage_create_info ? usage_create_info->usage : image_state->createInfo.usage;
        ...
        DispatchGetPhysicalDeviceImageFormatProperties2(physical_device, &image_format_info,
                                                        &image_format_properties);
    }

    Add(std::make_shared<IMAGE_VIEW_STATE>(image_state, *pView, pCreateInfo, format_features,
                                           filter_cubic_props));
}

void ValidationStateTracker::PreCallRecordCmdCopyBuffer(VkCommandBuffer commandBuffer,
                                                        VkBuffer srcBuffer,
                                                        VkBuffer dstBuffer,
                                                        uint32_t regionCount,
                                                        const VkBufferCopy *pRegions)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYBUFFER, Get<BUFFER_STATE>(srcBuffer),
                                Get<BUFFER_STATE>(dstBuffer));
}

void ValidationStateTracker::PreCallRecordCmdCopyBuffer2KHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdCopyBuffer2(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordDestroyImageView(VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyBuffer(VkDevice device,
                                                        VkBuffer buffer,
                                                        const VkAllocationCallbacks *pAllocator)
{
    ...
    if (buffer_state)
    {
        ...
        if (buffer_state->deviceAddress != 0)
        {
            ...
            buffer_address_map_.erase_range_or_touch(address_range, [&buffer_state](auto &buffers) {
                ...
                const auto buffer_found_it =
                    std::find(buffers.begin(), buffers.end(), buffer_state);
...
void ValidationStateTracker::PreCallRecordDestroyBufferView(VkDevice device,
...
void ValidationStateTracker::PreCallRecordCmdFillBuffer(VkCommandBuffer commandBuffer,
                                                        VkBuffer dstBuffer,
...
void ValidationStateTracker::PreCallRecordCmdCopyImageToBuffer(VkCommandBuffer commandBuffer,
                                                               VkImage srcImage,
                                                               VkImageLayout srcImageLayout,
                                                               VkBuffer dstBuffer,
                                                               uint32_t regionCount,
                                                               const VkBufferImageCopy *pRegions)
{
    ...
    cb_state->RecordTransferCmd(CMD_COPYIMAGETOBUFFER, Get<IMAGE_STATE>(srcImage),
                                Get<BUFFER_STATE>(dstBuffer));
}

void ValidationStateTracker::PreCallRecordCmdCopyImageToBuffer2KHR(
    VkCommandBuffer commandBuffer,
    const VkCopyImageToBufferInfo2KHR *pCopyImageToBufferInfo)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYIMAGETOBUFFER2KHR,
                                Get<IMAGE_STATE>(pCopyImageToBufferInfo->srcImage),
                                Get<BUFFER_STATE>(pCopyImageToBufferInfo->dstBuffer));
}

void ValidationStateTracker::PreCallRecordCmdCopyImageToBuffer2(
    VkCommandBuffer commandBuffer,
    const VkCopyImageToBufferInfo2 *pCopyImageToBufferInfo)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYIMAGETOBUFFER2,
                                Get<IMAGE_STATE>(pCopyImageToBufferInfo->srcImage),
                                Get<BUFFER_STATE>(pCopyImageToBufferInfo->dstBuffer));
}

void ValidationStateTracker::PreCallRecordCmdCopyBufferToImage(VkCommandBuffer commandBuffer,
                                                               VkBuffer srcBuffer,
                                                               VkImage dstImage,
                                                               VkImageLayout dstImageLayout,
                                                               uint32_t regionCount,
                                                               const VkBufferImageCopy *pRegions)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYBUFFERTOIMAGE, Get<BUFFER_STATE>(srcBuffer),
                                Get<IMAGE_STATE>(dstImage));
}

void ValidationStateTracker::PreCallRecordCmdCopyBufferToImage2KHR(
    VkCommandBuffer commandBuffer,
    const VkCopyBufferToImageInfo2KHR *pCopyBufferToImageInfo)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYBUFFERTOIMAGE2KHR,
                                Get<BUFFER_STATE>(pCopyBufferToImageInfo->srcBuffer),
                                Get<IMAGE_STATE>(pCopyBufferToImageInfo->dstImage));
}

void ValidationStateTracker::PreCallRecordCmdCopyBufferToImage2(
    VkCommandBuffer commandBuffer,
    const VkCopyBufferToImageInfo2 *pCopyBufferToImageInfo)
{
    ...
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordTransferCmd(CMD_COPYBUFFERTOIMAGE2,
                                Get<BUFFER_STATE>(pCopyBufferToImageInfo->srcBuffer),
                                Get<IMAGE_STATE>(pCopyBufferToImageInfo->dstImage));
}

// Gets union of all features defined by Potential Format Features
// except, does not handle the external format case for AHB as that only can be used for sampled
// images
VkFormatFeatureFlags2KHR ValidationStateTracker::GetPotentialFormatFeatures(VkFormat format) const
{
    ...
    if (format != VK_FORMAT_UNDEFINED)
    {
        if (has_format_feature2)
        {
            auto fmt_drm_props = LvlInitStruct<VkDrmFormatModifierPropertiesList2EXT>();
            auto fmt_props_3   = LvlInitStruct<VkFormatProperties3KHR>(
                IsExtEnabled(device_extensions.vk_ext_image_drm_format_modifier) ? &fmt_drm_props
                                                                                   : nullptr);
            auto fmt_props_2 = LvlInitStruct<VkFormatProperties2>(&fmt_props_3);
...
void ValidationStateTracker::PostCallRecordCreateDevice(VkPhysicalDevice gpu,
                                                        ...
                                                        VkResult result)
{
    ...
    ValidationObject *validation_data =
        GetValidationObject(device_object->object_dispatch, this->container_type);
...
std::shared_ptr<QUEUE_STATE> ValidationStateTracker::CreateQueue(
    VkQueue q,
    uint32_t index,
...
void ValidationStateTracker::CreateDevice(const VkDeviceCreateInfo *pCreateInfo)
{
    ...
    const auto *vulkan_13_features =
        LvlFindInChain<VkPhysicalDeviceVulkan13Features>(pCreateInfo->pNext);
    ...
    else
    {
        ...
        const auto *image_robustness_features =
            LvlFindInChain<VkPhysicalDeviceImageRobustnessFeatures>(pCreateInfo->pNext);
        if (image_robustness_features)
        {
            enabled_features.core13.robustImageAccess =
                image_robustness_features->robustImageAccess;
        }

        const auto *inline_uniform_block_features =
            LvlFindInChain<VkPhysicalDeviceInlineUniformBlockFeatures>(pCreateInfo->pNext);
        if (inline_uniform_block_features)
        {
            enabled_features.core13.inlineUniformBlock =
                inline_uniform_block_features->inlineUniformBlock;
        ...
        const auto *pipeline_creation_cache_control_features =
            LvlFindInChain<VkPhysicalDevicePipelineCreationCacheControlFeatures>(
                pCreateInfo->pNext);
        ...
        const auto *private_data_features =
            LvlFindInChain<VkPhysicalDevicePrivateDataFeatures>(pCreateInfo->pNext);
        ...
        const auto *demote_to_helper_invocation_features =
            LvlFindInChain<VkPhysicalDeviceShaderDemoteToHelperInvocationFeatures>(
                pCreateInfo->pNext);
        ...
        {
            enabled_features.core13.shaderTerminateInvocation =
        ...
        const auto *synchronization2_features =
            LvlFindInChain<VkPhysicalDeviceSynchronization2Features>(pCreateInfo->pNext);
        ...
        {
            enabled_features.core13.textureCompressionASTC_HDR =
                texture_compression_astchdr_features->textureCompressionASTC_HDR;
        }

        const auto *initialize_workgroup_memory_features =
            LvlFindInChain<VkPhysicalDeviceZeroInitializeWorkgroupMemoryFeatures>(
                pCreateInfo->pNext);
        ...
        const auto *dynamic_rendering_features =
            LvlFindInChain<VkPhysicalDeviceDynamicRenderingFeatures>(pCreateInfo->pNext);
        ...
        {
            enabled_features.core13.shaderIntegerDotProduct =
                shader_integer_dot_product_features->shaderIntegerDotProduct;
        }

        const auto *maintenance4_features =
            LvlFindInChain<VkPhysicalDeviceMaintenance4FeaturesKHR>(pCreateInfo->pNext);
    ...
    const auto *vulkan_12_features =
        LvlFindInChain<VkPhysicalDeviceVulkan12Features>(pCreateInfo->pNext);
    ...
    else
    {
        ...
        // These structs are only allowed in pNext chain if there is no
        // VkPhysicalDeviceVulkan12Features

        const auto *eight_bit_storage_features =
            LvlFindInChain<VkPhysicalDevice8BitStorageFeatures>(pCreateInfo->pNext);
        if (eight_bit_storage_features)
        {
            enabled_features.core12.storageBuffer8BitAccess =
            ...
            enabled_features.core12.storagePushConstant8 =
                eight_bit_storage_features->storagePushConstant8;
        }

        const auto *float16_int8_features =
            LvlFindInChain<VkPhysicalDeviceShaderFloat16Int8Features>(pCreateInfo->pNext);
        ...
        const auto *descriptor_indexing_features =
            LvlFindInChain<VkPhysicalDeviceDescriptorIndexingFeatures>(pCreateInfo->pNext);
        if (descriptor_indexing_features)
        {
            ...
            enabled_features.core12.runtimeDescriptorArray =
                descriptor_indexing_features->runtimeDescriptorArray;
        }

        const auto *scalar_block_layout_features =
            LvlFindInChain<VkPhysicalDeviceScalarBlockLayoutFeatures>(pCreateInfo->pNext);
        if (scalar_block_layout_features)
        {
            enabled_features.core12.scalarBlockLayout =
        ...
        {
            enabled_features.core12.imagelessFramebuffer =
        ...
        {
            enabled_features.core12.shaderSubgroupExtendedTypes =
        ...
        const auto *host_query_reset_features =
            LvlFindInChain<VkPhysicalDeviceHostQueryResetFeatures>(pCreateInfo->pNext);
        ...
        const auto *timeline_semaphore_features =
            LvlFindInChain<VkPhysicalDeviceTimelineSemaphoreFeatures>(pCreateInfo->pNext);
        ...
        const auto *buffer_device_address =
            LvlFindInChain<VkPhysicalDeviceBufferDeviceAddressFeatures>(pCreateInfo->pNext);
        if (buffer_device_address)
        {
            ...
            enabled_features.core12.bufferDeviceAddressMultiDevice =
                buffer_device_address->bufferDeviceAddressMultiDevice;
        }

        const auto *atomic_int64_features =
            LvlFindInChain<VkPhysicalDeviceShaderAtomicInt64Features>(pCreateInfo->pNext);
        if (atomic_int64_features)
        {
            enabled_features.core12.shaderBufferInt64Atomics =
                atomic_int64_features->shaderBufferInt64Atomics;
            enabled_features.core12.shaderSharedInt64Atomics =
                atomic_int64_features->shaderSharedInt64Atomics;
        }

        const auto *memory_model_features =
            LvlFindInChain<VkPhysicalDeviceVulkanMemoryModelFeatures>(pCreateInfo->pNext);
        if (memory_model_features)
        {
            enabled_features.core12.vulkanMemoryModel = memory_model_features->vulkanMemoryModel;
            enabled_features.core12.vulkanMemoryModelDeviceScope =
    ...
    const auto *vulkan_11_features =
        LvlFindInChain<VkPhysicalDeviceVulkan11Features>(pCreateInfo->pNext);
    ...
    else
    {
        ...
        const auto *sixteen_bit_storage_features =
            LvlFindInChain<VkPhysicalDevice16BitStorageFeatures>(pCreateInfo->pNext);
        if (sixteen_bit_storage_features)
        {
            enabled_features.core11.storageBuffer16BitAccess =
            ...
            enabled_features.core11.storagePushConstant16 =
                sixteen_bit_storage_features->storagePushConstant16;
            enabled_features.core11.storageInputOutput16 =
                sixteen_bit_storage_features->storageInputOutput16;
        }

        const auto *multiview_features =
            LvlFindInChain<VkPhysicalDeviceMultiviewFeatures>(pCreateInfo->pNext);
        if (multiview_features)
        {
            enabled_features.core11.multiview = multiview_features->multiview;
            enabled_features.core11.multiviewGeometryShader =
                multiview_features->multiviewGeometryShader;
            enabled_features.core11.multiviewTessellationShader =
                multiview_features->multiviewTessellationShader;
        }

        const auto *variable_pointers_features =
            LvlFindInChain<VkPhysicalDeviceVariablePointersFeatures>(pCreateInfo->pNext);
        if (variable_pointers_features)
        {
            enabled_features.core11.variablePointersStorageBuffer =
        ...
        const auto *ycbcr_conversion_features =
            LvlFindInChain<VkPhysicalDeviceSamplerYcbcrConversionFeatures>(pCreateInfo->pNext);
        if (ycbcr_conversion_features)
        {
            enabled_features.core11.samplerYcbcrConversion =
        ...
        {
            enabled_features.core11.shaderDrawParameters =
                shader_draw_parameters_features->shaderDrawParameters;
        }
    }
    ...
    else
    {
        ...
        device_group_create_info.physicalDeviceCount =
            1;  // see previous VkDeviceGroupDeviceCreateInfo link
    ...
    {
        const auto *exclusive_scissor_features =
            LvlFindInChain<VkPhysicalDeviceExclusiveScissorFeaturesNV>(pCreateInfo->pNext);
        ...
        const auto *mesh_shader_features_NV =
            LvlFindInChain<VkPhysicalDeviceMeshShaderFeaturesNV>(pCreateInfo->pNext);
        ...
        const auto *mesh_shader_features =
            LvlFindInChain<VkPhysicalDeviceMeshShaderFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *descriptor_buffer_features =
            LvlFindInChain<VkPhysicalDeviceDescriptorBufferFeaturesEXT>(pCreateInfo->pNext);
        if (descriptor_buffer_features)
        {
            enabled_features.descriptor_buffer_features = *descriptor_buffer_features;
        }

        const auto *transform_feedback_features =
            LvlFindInChain<VkPhysicalDeviceTransformFeedbackFeaturesEXT>(pCreateInfo->pNext);
        if (transform_feedback_features)
        {
            enabled_features.transform_feedback_features = *transform_feedback_features;
        }

        const auto *vtx_attrib_div_features =
            LvlFindInChain<VkPhysicalDeviceVertexAttributeDivisorFeaturesEXT>(pCreateInfo->pNext);
        ...
        {
            enabled_features.buffer_device_address_ext_features =
                *buffer_device_address_ext_features;
        }

        const auto *cooperative_matrix_features =
            LvlFindInChain<VkPhysicalDeviceCooperativeMatrixFeaturesNV>(pCreateInfo->pNext);
        ...
        {
            enabled_features.compute_shader_derivatives_features =
        ...
        {
            enabled_features.fragment_shader_barycentric_features =
        ...
        {
            enabled_features.fragment_shader_interlock_features =
        ...
        const auto *pipeline_exe_props_features =
            LvlFindInChain<VkPhysicalDevicePipelineExecutablePropertiesFeaturesKHR>(
                pCreateInfo->pNext);
        ...
        const auto *dedicated_allocation_image_aliasing_features =
            LvlFindInChain<VkPhysicalDeviceDedicatedAllocationImageAliasingFeaturesNV>(
                pCreateInfo->pNext);
        if (dedicated_allocation_image_aliasing_features)
        {
            enabled_features.dedicated_allocation_image_aliasing_features =
                *dedicated_allocation_image_aliasing_features;
        }

        const auto *performance_query_features =
            LvlFindInChain<VkPhysicalDevicePerformanceQueryFeaturesKHR>(pCreateInfo->pNext);
        if (performance_query_features)
        {
            enabled_features.performance_query_features = *performance_query_features;
        }

        const auto *device_coherent_memory_features =
            LvlFindInChain<VkPhysicalDeviceCoherentMemoryFeaturesAMD>(pCreateInfo->pNext);
        if (device_coherent_memory_features)
        {
            enabled_features.device_coherent_memory_features = *device_coherent_memory_features;
        }

        const auto *ray_query_features =
            LvlFindInChain<VkPhysicalDeviceRayQueryFeaturesKHR>(pCreateInfo->pNext);
        ...
        const auto *robustness2_features =
            LvlFindInChain<VkPhysicalDeviceRobustness2FeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *fragment_density_map_offset_features =
            LvlFindInChain<VkPhysicalDeviceFragmentDensityMapOffsetFeaturesQCOM>(
                pCreateInfo->pNext);
        if (fragment_density_map_offset_features)
        {
            enabled_features.fragment_density_map_offset_features =
                *fragment_density_map_offset_features;
        }

        const auto *astc_decode_features =
            LvlFindInChain<VkPhysicalDeviceASTCDecodeFeaturesEXT>(pCreateInfo->pNext);
        if (astc_decode_features)
        {
            enabled_features.astc_decode_features = *astc_decode_features;
        }

        const auto *custom_border_color_features =
            LvlFindInChain<VkPhysicalDeviceCustomBorderColorFeaturesEXT>(pCreateInfo->pNext);
        ...
        {
            enabled_features.fragment_shading_rate_enums_features =
                *fragment_shading_rate_enums_features;
        }
        ...
        const auto *depth_clip_enable_features =
            LvlFindInChain<VkPhysicalDeviceDepthClipEnableFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *depth_clip_control_features =
            LvlFindInChain<VkPhysicalDeviceDepthClipControlFeaturesEXT>(pCreateInfo->pNext);
        if (depth_clip_control_features)
        {
            enabled_features.depth_clip_control_features = *depth_clip_control_features;
        }

        const auto *line_rasterization_features =
            LvlFindInChain<VkPhysicalDeviceLineRasterizationFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *portability_features =
            LvlFindInChain<VkPhysicalDevicePortabilitySubsetFeaturesKHR>(pCreateInfo->pNext);
        ...
        const auto *shader_integer_functions2_features =
            LvlFindInChain<VkPhysicalDeviceShaderIntegerFunctions2FeaturesINTEL>(
                pCreateInfo->pNext);
        if (shader_integer_functions2_features)
        {
            enabled_features.shader_integer_functions2_features =
                *shader_integer_functions2_features;
        }

        const auto *shader_sm_builtins_features =
            LvlFindInChain<VkPhysicalDeviceShaderSMBuiltinsFeaturesNV>(pCreateInfo->pNext);
        ...
        const auto *shader_atomic_float_features =
            LvlFindInChain<VkPhysicalDeviceShaderAtomicFloatFeaturesEXT>(pCreateInfo->pNext);
        ...
        {
            enabled_features.shader_image_atomic_int64_features =
                *shader_image_atomic_int64_features;
        }

        const auto *shader_clock_features =
            LvlFindInChain<VkPhysicalDeviceShaderClockFeaturesKHR>(pCreateInfo->pNext);
        ...
        const auto *workgroup_memory_explicit_layout_features =
            LvlFindInChain<VkPhysicalDeviceWorkgroupMemoryExplicitLayoutFeaturesKHR>(
                pCreateInfo->pNext);
        if (workgroup_memory_explicit_layout_features)
        {
            enabled_features.workgroup_memory_explicit_layout_features =
                *workgroup_memory_explicit_layout_features;
        }

        const auto *provoking_vertex_features =
            LvlFindInChain<VkPhysicalDeviceProvokingVertexFeaturesEXT>(pCreateInfo->pNext);
        ...
        {
            enabled_features.vertex_input_dynamic_state_features =
        ...
        {
            enabled_features.inherited_viewport_scissor_features =
                *inherited_viewport_scissor_features;
        }

        const auto *multi_draw_features =
            LvlFindInChain<VkPhysicalDeviceMultiDrawFeaturesEXT>(pCreateInfo->pNext);
        if (multi_draw_features)
        {
            enabled_features.multi_draw_features = *multi_draw_features;
        }

        const auto *color_write_features =
            LvlFindInChain<VkPhysicalDeviceColorWriteEnableFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *present_id_features =
            LvlFindInChain<VkPhysicalDevicePresentIdFeaturesKHR>(pCreateInfo->pNext);
        if (present_id_features)
        {
            enabled_features.present_id_features = *present_id_features;
        }

        const auto *present_wait_features =
            LvlFindInChain<VkPhysicalDevicePresentWaitFeaturesKHR>(pCreateInfo->pNext);
        ...
        const auto *primitive_topology_list_restart_features =
            LvlFindInChain<VkPhysicalDevicePrimitiveTopologyListRestartFeaturesEXT>(
                pCreateInfo->pNext);
        if (primitive_topology_list_restart_features)
        {
            enabled_features.primitive_topology_list_restart_features =
                *primitive_topology_list_restart_features;
        }

        const auto *rgba10x6_formats_features =
            LvlFindInChain<VkPhysicalDeviceRGBA10X6FormatsFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto *image_view_min_lod_features =
            LvlFindInChain<VkPhysicalDeviceImageViewMinLodFeaturesEXT>(pCreateInfo->pNext);
        ...
        {
            enabled_features.primitives_generated_query_features =
                *primitives_generated_query_features;
        }

        const auto image_2d_view_of_3d_features =
            LvlFindInChain<VkPhysicalDeviceImage2DViewOf3DFeaturesEXT>(pCreateInfo->pNext);
        ...
        const auto shader_subgroup_uniform_control_flow_features =
            LvlFindInChain<VkPhysicalDeviceShaderSubgroupUniformControlFlowFeaturesKHR>(
                pCreateInfo->pNext);
        if (shader_subgroup_uniform_control_flow_features)
        {
            enabled_features.shader_subgroup_uniform_control_flow_features =
                *shader_subgroup_uniform_control_flow_features;
        }
        ...
        const auto multisampled_render_to_single_sampled_features =
            LvlFindInChain<VkPhysicalDeviceMultisampledRenderToSingleSampledFeaturesEXT>(
                pCreateInfo->pNext);
        if (multisampled_render_to_single_sampled_features)
        {
            enabled_features.multisampled_render_to_single_sampled_features =
                *multisampled_render_to_single_sampled_features;
        }
        ...
        const auto attachment_feedback_loop_layout =
            LvlFindInChain<VkPhysicalDeviceAttachmentFeedbackLoopLayoutFeaturesEXT>(
                pCreateInfo->pNext);
        if (attachment_feedback_loop_layout)
        {
            enabled_features.attachment_feedback_loop_layout_features =
        ...
        {
            enabled_features.pipeline_protected_access_features =
        ...
        const auto pipeline_library_group_handles_features =
            LvlFindInChain<VkPhysicalDevicePipelineLibraryGroupHandlesFeaturesEXT>(
                pCreateInfo->pNext);
        if (pipeline_library_group_handles_features)
        {
            enabled_features.pipeline_library_group_handles_features =
                *pipeline_library_group_handles_features;
        }

        if (const auto slice_feature =
                LvlFindInChain<VkPhysicalDeviceImageSlicedViewOf3DFeaturesEXT>(pCreateInfo->pNext);
            slice_feature)
        {
            enabled_features.sliced_3d_features = *slice_feature;
        }

        if (const auto shader_object_feature =
                LvlFindInChain<VkPhysicalDeviceShaderObjectFeaturesEXT>(pCreateInfo->pNext);
...

    {
        ...
        instance_dispatch_table.EnumerateDeviceExtensionProperties(physical_device, NULL, &n_props,
                                                                   NULL);
        ...
        has_format_feature2 =
            ...
            phys_dev_extensions.find(VK_KHR_FORMAT_FEATURE_FLAGS_2_EXTENSION_NAME) !=
                phys_dev_extensions.end();

        // feature is required if 1.3 or extension is supported
        has_robust_image_access =
            (api_version >= VK_API_VERSION_1_3 ||
             IsExtEnabled(instance_extensions.vk_khr_get_physical_device_properties2)) &&
            phys_dev_extensions.find(VK_EXT_IMAGE_ROBUSTNESS_EXTENSION_NAME) !=
                phys_dev_extensions.end();
    }
    ...
    // Vulkan 1.2 / 1.3 can get properties from single struct, otherwise need to add to it per
    // extension
    if (dev_ext.vk_feature_version_1_2 || dev_ext.vk_feature_version_1_3)
    {
        GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_feature_version_1_2,
                                       &phys_dev_props_core11);
        GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_feature_version_1_2,
                                       &phys_dev_props_core12);
        if (dev_ext.vk_feature_version_1_3)
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_feature_version_1_3,
                                           &phys_dev_props_core13);
    }
    else
    {
        ...
        if (dev_ext.vk_khr_multiview)
        {
            ...
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_multiview,
                                           &multiview_props);
        ...
        if (dev_ext.vk_khr_maintenance3)
        {
            ...
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_maintenance3,
                                           &maintenance3_props);
            phys_dev_props_core11.maxPerSetDescriptors = maintenance3_props.maxPerSetDescriptors;
            phys_dev_props_core11.maxMemoryAllocationSize =
        ...
        if (api_version >= VK_API_VERSION_1_1)
        {
            ...
            auto protected_memory_prop =
                LvlInitStruct<VkPhysicalDeviceProtectedMemoryProperties>(&subgroup_prop);
            ...
            phys_dev_props_core11.subgroupQuadOperationsInAllStages =
                subgroup_prop.quadOperationsInAllStages;
        ...
        {
            auto descriptor_indexing_prop =
                LvlInitStruct<VkPhysicalDeviceDescriptorIndexingProperties>();
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_descriptor_indexing,
            ...
            phys_dev_props_core12.robustBufferAccessUpdateAfterBind =
                descriptor_indexing_prop.robustBufferAccessUpdateAfterBind;
            phys_dev_props_core12.quadDivergentImplicitLod =
                descriptor_indexing_prop.quadDivergentImplicitLod;
        ...
        {
            auto depth_stencil_resolve_props =
                LvlInitStruct<VkPhysicalDeviceDepthStencilResolveProperties>();
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_depth_stencil_resolve,
            ...
            phys_dev_props_core12.independentResolveNone =
                depth_stencil_resolve_props.independentResolveNone;
            phys_dev_props_core12.independentResolve =
                depth_stencil_resolve_props.independentResolve;
        }

        if (dev_ext.vk_khr_timeline_semaphore)
        {
            auto timeline_semaphore_props =
                LvlInitStruct<VkPhysicalDeviceTimelineSemaphoreProperties>();
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_timeline_semaphore,
        ...
        {
            auto sampler_filter_minmax_props =
                LvlInitStruct<VkPhysicalDeviceSamplerFilterMinmaxProperties>();
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_sampler_filter_minmax,
            ...
            phys_dev_props_core12.filterMinmaxImageComponentMapping =
        ...
        if (dev_ext.vk_khr_shader_float_controls)
        {
            ...
            GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_shader_float_controls,
                                           &float_controls_props);
            phys_dev_props_core12.denormBehaviorIndependence =
                float_controls_props.denormBehaviorIndependence;
            phys_dev_props_core12.roundingModeIndependence =
                float_controls_props.roundingModeIndependence;
            ...
            phys_dev_props_core12.shaderDenormPreserveFloat16 =
                float_controls_props.shaderDenormPreserveFloat16;
            phys_dev_props_core12.shaderDenormPreserveFloat32 =
                float_controls_props.shaderDenormPreserveFloat32;
            phys_dev_props_core12.shaderDenormPreserveFloat64 =
                float_controls_props.shaderDenormPreserveFloat64;
            phys_dev_props_core12.shaderDenormFlushToZeroFloat16 =
                float_controls_props.shaderDenormFlushToZeroFloat16;
            phys_dev_props_core12.shaderDenormFlushToZeroFloat32 =
                float_controls_props.shaderDenormFlushToZeroFloat32;
            phys_dev_props_core12.shaderDenormFlushToZeroFloat64 =
                float_controls_props.shaderDenormFlushToZeroFloat64;
            phys_dev_props_core12.shaderRoundingModeRTEFloat16 =
                float_controls_props.shaderRoundingModeRTEFloat16;
            phys_dev_props_core12.shaderRoundingModeRTEFloat32 =
                float_controls_props.shaderRoundingModeRTEFloat32;
            phys_dev_props_core12.shaderRoundingModeRTEFloat64 =
                float_controls_props.shaderRoundingModeRTEFloat64;
            phys_dev_props_core12.shaderRoundingModeRTZFloat16 =
                float_controls_props.shaderRoundingModeRTZFloat16;
            phys_dev_props_core12.shaderRoundingModeRTZFloat32 =
                float_controls_props.shaderRoundingModeRTZFloat32;
            phys_dev_props_core12.shaderRoundingModeRTZFloat64 =
                float_controls_props.shaderRoundingModeRTZFloat64;
        }
    }

    // Extensions with properties to extract to DeviceExtensionProperties
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_push_descriptor,
                                   &phys_dev_props->push_descriptor_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_nv_shading_rate_image,
                                   &phys_dev_props->shading_rate_image_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_nv_mesh_shader,
                                   &phys_dev_props->mesh_shader_props_nv);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_mesh_shader,
    ...
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_transform_feedback,
                                   &phys_dev_props->transform_feedback_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_nv_ray_tracing,
                                   &phys_dev_props->ray_tracing_props_nv);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_ray_tracing_pipeline,
                                   &phys_dev_props->ray_tracing_props_khr);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_acceleration_structure,
    ...
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_performance_query,
                                   &phys_dev_props->performance_query_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_sample_locations,
                                   &phys_dev_props->sample_locations_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_custom_border_color,
                                   &phys_dev_props->custom_border_color_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_multiview,
                                   &phys_dev_props->multiview_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_khr_portability_subset,
    ...
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_provoking_vertex,
                                   &phys_dev_props->provoking_vertex_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_multi_draw,
                                   &phys_dev_props->multi_draw_props);
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_ext_discard_rectangles,
    ...
    GetPhysicalDeviceExtProperties(physical_device, dev_ext.vk_qcom_image_processing,
    ...
    if (IsExtEnabled(dev_ext.vk_nv_cooperative_matrix))
    {
        ...
        auto cooperative_matrix_props =
            LvlInitStruct<VkPhysicalDeviceCooperativeMatrixPropertiesNV>();
        ...
        instance_dispatch_table.GetPhysicalDeviceCooperativeMatrixPropertiesNV(
            physical_device, &num_cooperative_matrix_properties, NULL);
        cooperative_matrix_properties.resize(num_cooperative_matrix_properties,
                                             LvlInitStruct<VkCooperativeMatrixPropertiesNV>());

        instance_dispatch_table.GetPhysicalDeviceCooperativeMatrixPropertiesNV(
            physical_device, &num_cooperative_matrix_properties,
    ...
    if (pCreateInfo->pQueueCreateInfos != nullptr)
    {
        ...
        instance_dispatch_table.GetPhysicalDeviceQueueFamilyProperties(
            physical_device, &num_queue_families, nullptr);
        std::vector<VkQueueFamilyProperties> queue_family_properties_list(num_queue_families);
        instance_dispatch_table.GetPhysicalDeviceQueueFamilyProperties(
            physical_device, &num_queue_families, queue_family_properties_list.data());

        for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; ++i)
        {
            const VkDeviceQueueCreateInfo &queue_create_info = pCreateInfo->pQueueCreateInfos[i];
            queue_family_index_set.insert(queue_create_info.queueFamilyIndex);
            device_queue_info_list.push_back({i, queue_create_info.queueFamilyIndex,
        ...
        for (const auto &queue_info : device_queue_info_list)
        {
            for (uint32_t i = 0; i < queue_info.queue_count; i++)
            {
                ...
                // vkGetDeviceQueue2() was added in vulkan 1.1, and there was never a KHR version of
                // it.
    ...
    // Query queue family extension properties
    {
        uint32_t queue_family_count =
            (uint32_t)physical_device_state->queue_family_properties.size();
        auto &ext_props = queue_family_ext_props;
        ext_props.resize(queue_family_count);

        std::vector<VkQueueFamilyProperties2> props(queue_family_count,
                                                    LvlInitStruct<VkQueueFamilyProperties2>());

        if (dev_ext.vk_khr_video_queue)
        {
            for (uint32_t i = 0; i < queue_family_count; ++i)
            {
                ext_props[i].query_result_status_props =
                    LvlInitStruct<VkQueueFamilyQueryResultStatusPropertiesKHR>();
                ext_props[i].video_props = LvlInitStruct<VkQueueFamilyVideoPropertiesKHR>(
                    &ext_props[i].query_result_status_props);
        ...
        if (api_version >= VK_API_VERSION_1_1)
        {
            DispatchGetPhysicalDeviceQueueFamilyProperties2(physical_device, &queue_family_count,
                                                            props.data());
        }
        else if (IsExtEnabled(instance_extensions.vk_khr_get_physical_device_properties2))
        {
            DispatchGetPhysicalDeviceQueueFamilyProperties2KHR(physical_device, &queue_family_count,
                                                               props.data());
        }
    }
}

void ValidationStateTracker::PreCallRecordDestroyDevice(VkDevice device,
                                                        const VkAllocationCallbacks *pAllocator)
{
    if (!device)
        return;
...
void ValidationStateTracker::PreCallRecordQueueSubmit(VkQueue queue,
                                                      ...
                                                      VkFence fence)
{
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++)
    {
        ...
        auto *timeline_semaphore_submit =
            LvlFindInChain<VkTimelineSemaphoreSubmitInfo>(submit->pNext);
        for (uint32_t i = 0; i < submit->waitSemaphoreCount; ++i)
        {
            uint64_t value{0};
            if (timeline_semaphore_submit &&
                timeline_semaphore_submit->pWaitSemaphoreValues != nullptr &&
        ...
        for (uint32_t i = 0; i < submit->signalSemaphoreCount; ++i)
        {
            uint64_t value{0};
            if (timeline_semaphore_submit &&
                timeline_semaphore_submit->pSignalSemaphoreValues != nullptr &&
                (i < timeline_semaphore_submit->signalSemaphoreValueCount))
            {
                value = timeline_semaphore_submit->pSignalSemaphoreValues[i];
            }
            submission.AddSignalSemaphore(Get<SEMAPHORE_STATE>(submit->pSignalSemaphores[i]),
                                          value);
        }
...
void ValidationStateTracker::RecordQueueSubmit2(VkQueue queue,
                                                uint32_t submitCount,
                                                ...
                                                VkFence fence)
{
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++)
    {
        ...
        {
            submission.AddCommandBuffer(
                GetWrite<CMD_BUFFER_STATE>(submit->pCommandBufferInfos[i].commandBuffer));
        }
...
void ValidationStateTracker::PreCallRecordQueueSubmit2KHR(VkQueue queue,
                                                          uint32_t submitCount,
                                                          const VkSubmitInfo2KHR *pSubmits,
                                                          VkFence fence)
{
...
void ValidationStateTracker::PreCallRecordQueueSubmit2(VkQueue queue,
                                                       uint32_t submitCount,
...
void ValidationStateTracker::PostCallRecordAllocateMemory(VkDevice device,
                                                          ...
                                                          VkResult result)
{
    ...
    if (const auto import_memory_fd_info =
            LvlFindInChain<VkImportMemoryFdInfoKHR>(pAllocateInfo->pNext))
    {
    ...
    Add(CreateDeviceMemoryState(*pMemory, pAllocateInfo, fake_address, memory_type, memory_heap,
                                std::move(dedicated_binding), physical_device_count));
    return;
}

void ValidationStateTracker::PreCallRecordFreeMemory(VkDevice device,
                                                     VkDeviceMemory mem,
                                                     const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordQueueBindSparse(VkQueue queue,
    ...
    for (uint32_t bind_idx = 0; bind_idx < bindInfoCount; ++bind_idx)
    {
        ...
        for (uint32_t j = 0; j < bind_info.bufferBindCount; j++)
        {
            for (uint32_t k = 0; k < bind_info.pBufferBinds[j].bindCount; k++)
            {
                ...
                if (buffer_state)
                {
                    buffer_state->BindMemory(buffer_state.get(), mem_state,
                                             sparse_binding.memoryOffset,
        ...
        for (uint32_t j = 0; j < bind_info.imageOpaqueBindCount; j++)
        {
            for (uint32_t k = 0; k < bind_info.pImageOpaqueBinds[j].bindCount; k++)
            {
                ...
                if (image_state)
                {
                    // An Android special image cannot get VkSubresourceLayout until the image binds
                    // a memory. See: VUID-vkGetImageSubresourceLayout-image-01895
                    if (!image_state->fragment_encoder)
                    {
                        image_state->fragment_encoder =
                            std::make_unique<const subresource_adapter::ImageRangeEncoder>(
                                *image_state);
                    }
                    image_state->BindMemory(image_state.get(), mem_state,
                                            sparse_binding.memoryOffset,
                                            sparse_binding.resourceOffset, sparse_binding.size);
                }
            }
        ...
        for (uint32_t j = 0; j < bind_info.imageBindCount; j++)
        {
            for (uint32_t k = 0; k < bind_info.pImageBinds[j].bindCount; k++)
            {
                ...
                // TODO: This size is broken for non-opaque bindings, need to update to comprehend
                // full sparse binding data
                VkDeviceSize size = sparse_binding.extent.depth * sparse_binding.extent.height *
                                    sparse_binding.extent.width * 4;
                VkDeviceSize offset =
                    sparse_binding.offset.z * sparse_binding.offset.y * sparse_binding.offset.x * 4;
                ...
                if (image_state)
                {
                    ...
                    image_state->BindMemory(image_state.get(), mem_state,
                                            sparse_binding.memoryOffset, offset, size);
                }
            }
        }
        ...
        for (uint32_t i = 0; i < bind_info.waitSemaphoreCount; ++i)
        {
            ...
            submission.AddWaitSemaphore(Get<SEMAPHORE_STATE>(bind_info.pWaitSemaphores[i]),
                                        payload);
        }
        for (uint32_t i = 0; i < bind_info.signalSemaphoreCount; ++i)
        {
            ...
            submission.AddSignalSemaphore(Get<SEMAPHORE_STATE>(bind_info.pSignalSemaphores[i]),
                                          payload);
        }
...
void ValidationStateTracker::PostCallRecordCreateSemaphore(VkDevice device,
...
void ValidationStateTracker::RecordImportSemaphoreState(
...
void ValidationStateTracker::PreCallRecordSignalSemaphore(VkDevice device,
...
void ValidationStateTracker::PreCallRecordSignalSemaphoreKHR(
    VkDevice device,
    const VkSemaphoreSignalInfo *pSignalInfo)
{
...
void ValidationStateTracker::PostCallRecordSignalSemaphore(VkDevice device,
                                                           const VkSemaphoreSignalInfo *pSignalInfo,
                                                           VkResult result)
{
    if (result != VK_SUCCESS)
...
void ValidationStateTracker::PostCallRecordSignalSemaphoreKHR(
    VkDevice device,
    const VkSemaphoreSignalInfo *pSignalInfo,
    VkResult result)
{
...
void ValidationStateTracker::RecordMappedMemory(VkDeviceMemory mem,
                                                VkDeviceSize offset,
                                                VkDeviceSize size,
                                                void **ppData)
{
...
void ValidationStateTracker::PostCallRecordWaitForFences(VkDevice device,
                                                         uint32_t fenceCount,
...
void ValidationStateTracker::PreRecordWaitSemaphores(VkDevice device,
...
void ValidationStateTracker::PreCallRecordWaitSemaphores(VkDevice device,
                                                         const VkSemaphoreWaitInfo *pWaitInfo,
                                                         uint64_t timeout)
{
...
void ValidationStateTracker::PreCallRecordWaitSemaphoresKHR(VkDevice device,
                                                            const VkSemaphoreWaitInfo *pWaitInfo,
                                                            uint64_t timeout)
{
    PreRecordWaitSemaphores(device, pWaitInfo, timeout);
...
void ValidationStateTracker::PostRecordWaitSemaphores(VkDevice device,
                                                      const VkSemaphoreWaitInfo *pWaitInfo,
                                                      uint64_t timeout,
                                                      VkResult result)
{
    ...
    // Same logic as vkWaitForFences(). If some semaphores are not signaled, we will get their
    // status when the application calls vkGetSemaphoreCounterValue() on each of them.
...
void ValidationStateTracker::PostCallRecordWaitSemaphores(VkDevice device,
                                                          const VkSemaphoreWaitInfo *pWaitInfo,
                                                          uint64_t timeout,
                                                          VkResult result)
{
...
void ValidationStateTracker::PostCallRecordWaitSemaphoresKHR(VkDevice device,
                                                             const VkSemaphoreWaitInfo *pWaitInfo,
                                                             uint64_t timeout,
                                                             VkResult result)
{
...
void ValidationStateTracker::RecordGetSemaphoreCounterValue(VkDevice device,
                                                            VkSemaphore semaphore,
                                                            uint64_t *pValue,
                                                            VkResult result)
{
...
void ValidationStateTracker::PostCallRecordGetSemaphoreCounterValue(VkDevice device,
                                                                    VkSemaphore semaphore,
                                                                    uint64_t *pValue,
                                                                    VkResult result)
{
...
void ValidationStateTracker::PostCallRecordGetSemaphoreCounterValueKHR(VkDevice device,
                                                                       VkSemaphore semaphore,
                                                                       uint64_t *pValue,
                                                                       VkResult result)
{
...
void ValidationStateTracker::PostCallRecordGetFenceStatus(VkDevice device,
                                                          VkFence fence,
                                                          VkResult result)
{
...
void ValidationStateTracker::RecordGetDeviceQueueState(uint32_t queue_family_index,
                                                       VkDeviceQueueCreateFlags flags,
                                                       VkQueue queue)
{
    if (Get<QUEUE_STATE>(queue) == nullptr)
    {
        ...
        Add(CreateQueue(queue, queue_family_index, flags,
                        queue_family_properties_list[queue_family_index]));
    }
}

void ValidationStateTracker::PostCallRecordGetDeviceQueue(VkDevice device,
...
void ValidationStateTracker::PostCallRecordGetDeviceQueue2(VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyFence(VkDevice device,
                                                       VkFence fence,
                                                       const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordDestroySemaphore(VkDevice device,
                                                           VkSemaphore semaphore,
                                                           const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordDestroyEvent(VkDevice device,
                                                       VkEvent event,
                                                       const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordDestroyQueryPool(VkDevice device,
...
void ValidationStateTracker::UpdateBindBufferMemoryState(VkBuffer buffer,
                                                         VkDeviceMemory mem,
                                                         VkDeviceSize memoryOffset)
{
    ...
    if (buffer_state)
    {
        ...
        if (mem_state)
        {
            buffer_state->BindMemory(buffer_state.get(), mem_state, memoryOffset, 0u,
                                     buffer_state->requirements.size);
        }
    }
}

void ValidationStateTracker::PostCallRecordBindBufferMemory(VkDevice device,
                                                            VkBuffer buffer,
                                                            VkDeviceMemory mem,
                                                            VkDeviceSize memoryOffset,
                                                            VkResult result)
...
void ValidationStateTracker::PostCallRecordBindBufferMemory2(
    VkDevice device,
    uint32_t bindInfoCount,
    ...
    VkResult result)
{
    ...
    {
        UpdateBindBufferMemoryState(pBindInfos[i].buffer, pBindInfos[i].memory,
                                    pBindInfos[i].memoryOffset);
    }
}

void ValidationStateTracker::PostCallRecordBindBufferMemory2KHR(
    VkDevice device,
    uint32_t bindInfoCount,
    const VkBindBufferMemoryInfo *pBindInfos,
    VkResult result)
...
void ValidationStateTracker::PostCallRecordGetBufferMemoryRequirements(
    VkDevice device,
    VkBuffer buffer,
...
void ValidationStateTracker::PostCallRecordGetBufferMemoryRequirements2(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordGetBufferMemoryRequirements2KHR(
    VkDevice device,
    const VkBufferMemoryRequirementsInfo2 *pInfo,
    VkMemoryRequirements2 *pMemoryRequirements)
{
...
void ValidationStateTracker::RecordGetImageMemoryRequirementsState(
    VkImage image,
    const VkImageMemoryRequirementsInfo2 *pInfo)
{
    const VkImagePlaneMemoryRequirementsInfo *plane_info =
        (pInfo == nullptr) ? nullptr
                           : LvlFindInChain<VkImagePlaneMemoryRequirementsInfo>(pInfo->pNext);
...
void ValidationStateTracker::PostCallRecordGetImageMemoryRequirements(
    VkDevice device,
    VkImage image,
    VkMemoryRequirements *pMemoryRequirements)
{
...
void ValidationStateTracker::PostCallRecordGetImageMemoryRequirements2(
    VkDevice device,
    const VkImageMemoryRequirementsInfo2 *pInfo,
    VkMemoryRequirements2 *pMemoryRequirements)
{
...
void ValidationStateTracker::PostCallRecordGetImageMemoryRequirements2KHR(
    VkDevice device,
    const VkImageMemoryRequirementsInfo2 *pInfo,
    VkMemoryRequirements2 *pMemoryRequirements)
{
...
void ValidationStateTracker::PreCallRecordDestroyShaderModule(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyPipeline(VkDevice device,
                                                          VkPipeline pipeline,
                                                          const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PreCallRecordDestroyPipelineLayout(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroySampler(VkDevice device,
                                                         VkSampler sampler,
                                                         const VkAllocationCallbacks *pAllocator)
{
    if (!sampler)
...
void ValidationStateTracker::PreCallRecordDestroyDescriptorSetLayout(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyDescriptorPool(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordFreeCommandBuffers(VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateCommandPool(
    VkDevice device,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    auto queue_flags =
        physical_device_state->queue_family_properties[pCreateInfo->queueFamilyIndex].queueFlags;
    Add(std::make_shared<COMMAND_POOL_STATE>(this, *pCommandPool, pCreateInfo, queue_flags));
}

void ValidationStateTracker::PostCallRecordCreateQueryPool(VkDevice device,
                                                           ...
                                                           VkResult result)
{
    ...
    if (pCreateInfo->queryType == VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR)
    {
        ...
        const QUEUE_FAMILY_PERF_COUNTERS &counters =
            *physical_device_state->perf_counters[perf->queueFamilyIndex];
        ...
        DispatchGetPhysicalDeviceQueueFamilyPerformanceQueryPassesKHR(
            physical_device_state->PhysDev(), perf, &n_perf_pass);
    }
...
void ValidationStateTracker::PreCallRecordDestroyCommandPool(
    VkDevice device,
    VkCommandPool commandPool,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PostCallRecordResetCommandPool(VkDevice device,
                                                            VkCommandPool commandPool,
...
void ValidationStateTracker::PostCallRecordResetFences(VkDevice device,
                                                       uint32_t fenceCount,
                                                       const VkFence *pFences,
                                                       VkResult result)
{
...
void ValidationStateTracker::PreCallRecordDestroyFramebuffer(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyRenderPass(VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateFence(VkDevice device,
...
{
    return std::make_shared<PIPELINE_STATE>(this, pCreateInfo, create_index, std::move(render_pass),
                                            std::move(layout), csm_states);
}

bool ValidationStateTracker::PreCallValidateCreateGraphicsPipelines(
    VkDevice device,
    ...
    void *cgpl_state_data) const
{
    ...
    create_graphics_pipeline_api_state *cgpl_state =
        reinterpret_cast<create_graphics_pipeline_api_state *>(cgpl_state_data);
    cgpl_state->pCreateInfos = pCreateInfos;  // GPU validation can alter this, so we have to set a
                                              // default value for the Chassis
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        else if (enabled_features.core13.dynamicRendering)
        {
            auto dynamic_rendering =
                LvlFindInChain<VkPipelineRenderingCreateInfo>(create_info.pNext);
            render_pass = std::make_shared<RENDER_PASS_STATE>(dynamic_rendering);
        }
        else
        {
            const bool is_graphics_lib = GetGraphicsLibType(create_info) !=
                                         static_cast<VkGraphicsPipelineLibraryFlagsEXT>(0);
            const bool has_link_info =
                LvlFindInChain<VkPipelineLibraryCreateInfoKHR>(create_info.pNext) != nullptr;
        ...
        auto csm_states =
            (cgpl_state->shader_states.size() > i) ? &cgpl_state->shader_states[i] : nullptr;
        cgpl_state->pipe_state.push_back(CreateGraphicsPipelineState(
            &create_info, i, std::move(render_pass), std::move(layout_state), csm_states));
    }
    return skip;
}

void ValidationStateTracker::PostCallRecordCreateGraphicsPipelines(
    VkDevice device,
    VkPipelineCache pipelineCache,
    uint32_t count,
    const VkGraphicsPipelineCreateInfo *pCreateInfos,
...
bool ValidationStateTracker::PreCallValidateCreateComputePipelines(
    VkDevice device,
    VkPipelineCache pipelineCache,
    uint32_t count,
    ...
    void *ccpl_state_data) const
{
    ...
    ccpl_state->pCreateInfos = pCreateInfos;  // GPU validation can alter this, so we have to set a
                                              // default value for the Chassis
    ccpl_state->pipe_state.reserve(count);
    for (uint32_t i = 0; i < count; i++)
    {
        // Create and initialize internal tracking data structure
        ccpl_state->pipe_state.push_back(CreateComputePipelineState(
            &pCreateInfos[i], i, Get<PIPELINE_LAYOUT_STATE>(pCreateInfos[i].layout)));
    }
    return false;
}

void ValidationStateTracker::PostCallRecordCreateComputePipelines(
    VkDevice device,
    VkPipelineCache pipelineCache,
    uint32_t count,
    const VkComputePipelineCreateInfo *pCreateInfos,
    ...
    void *ccpl_state_data)
{
    create_compute_pipeline_api_state *ccpl_state =
        reinterpret_cast<create_compute_pipeline_api_state *>(ccpl_state_data);
    ...
    void *crtpl_state_data) const
{
    ...
    {
        // Create and initialize internal tracking data structure
        crtpl_state->pipe_state.push_back(CreateRayTracingPipelineState(
            &pCreateInfos[i], i, Get<PIPELINE_LAYOUT_STATE>(pCreateInfos[i].layout)));
    }
    return false;
}
...
{
    auto crtpl_state =
        reinterpret_cast<create_ray_tracing_pipeline_khr_api_state *>(crtpl_state_data);
    crtpl_state->pipe_state.reserve(count);
    for (uint32_t i = 0; i < count; i++)
    {
        // Create and initialize internal tracking data structure
    ...
    void *crtpl_state_data)
{
    auto *crtpl_state =
        reinterpret_cast<create_ray_tracing_pipeline_khr_api_state *>(crtpl_state_data);
    const bool operation_is_deferred =
    ...
    else
    {
        ...
        cleanup_fn.emplace_back(
            [this, pipeline_states](const std::vector<VkPipeline> &pipelines) mutable {
...
void ValidationStateTracker::PostCallRecordCreateSampler(VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateDescriptorSetLayout(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordGetDescriptorSetLayoutSizeEXT(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreatePipelineLayout(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateDescriptorPool(
    VkDevice device,
    const VkDescriptorPoolCreateInfo *pCreateInfo,
    const VkAllocationCallbacks *pAllocator,
...
void ValidationStateTracker::PostCallRecordResetDescriptorPool(VkDevice device,
                                                               VkDescriptorPool descriptorPool,
...
bool ValidationStateTracker::PreCallValidateAllocateDescriptorSets(
    VkDevice device,
...
// Allocation state was good and call down chain was made so update state based on allocating
// descriptor sets
void ValidationStateTracker::PostCallRecordAllocateDescriptorSets(
    VkDevice device,
    const VkDescriptorSetAllocateInfo *pAllocateInfo,
    VkDescriptorSet *pDescriptorSets,
    VkResult result,
...
void ValidationStateTracker::PreCallRecordFreeDescriptorSets(VkDevice device,
                                                             VkDescriptorPool descriptorPool,
                                                             uint32_t count,
...
void ValidationStateTracker::PreCallRecordUpdateDescriptorSets(
    VkDevice device,
...
{
    cvdescriptorset::PerformUpdateDescriptorSets(this, descriptorWriteCount, pDescriptorWrites,
                                                 descriptorCopyCount, pDescriptorCopies);
}

void ValidationStateTracker::PostCallRecordAllocateCommandBuffers(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordBeginCommandBuffer(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordEndCommandBuffer(VkCommandBuffer commandBuffer,
                                                            VkResult result)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    if (!cb_state)
...
void ValidationStateTracker::PostCallRecordResetCommandBuffer(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBindPipeline(VkCommandBuffer commandBuffer,
                                                          ...
                                                          VkPipeline pipeline)
{
    ...
    if (VK_PIPELINE_BIND_POINT_GRAPHICS == pipelineBindPoint)
    {
        ...
        cb_state->static_status =
            MakeStaticStateMask(dynamic_state ? dynamic_state->ptr() : nullptr);
        cb_state->status |= cb_state->static_status;
        cb_state->dynamic_status = ~CBDynamicFlags(0);
        cb_state->dynamic_status &= ~cb_state->static_status;

        // Used to calculate CMD_BUFFER_STATE::usedViewportScissorCount upon draw command with this
        // graphics pipeline. If rasterization disabled (no viewport/scissors used), or the actual
        // number of viewports/scissors is dynamic (unknown at this time), then these are set to 0
        // to disable this checking.
        auto has_dynamic_viewport_count =
            cb_state->dynamic_status[CB_DYNAMIC_VIEWPORT_WITH_COUNT_SET];
        auto has_dynamic_scissor_count =
            cb_state->dynamic_status[CB_DYNAMIC_SCISSOR_WITH_COUNT_SET];
        cb_state->pipelineStaticViewportCount = has_dynamic_viewport_count || !rasterization_enabled
                                                    ? 0
                                                    : viewport_state->viewportCount;
        cb_state->pipelineStaticScissorCount =
            has_dynamic_scissor_count || !rasterization_enabled ? 0 : viewport_state->scissorCount;

        // Trash dynamic viewport/scissor state if pipeline defines static state and enabled
        // rasterization. akeley98 NOTE: There's a bit of an ambiguity in the spec, whether binding
        // such a pipeline overwrites the entire viewport (scissor) array, or only the subsection
        // defined by the viewport (scissor) count. I am taking the latter interpretation based on
        // the implementation details of NVIDIA's Vulkan driver.
        if (!has_dynamic_viewport_count)
        {
            ...
            {
                cb_state->trashedViewportMask |=
                    (uint32_t(1) << viewport_state->viewportCount) - 1u;
...
void ValidationStateTracker::PostCallRecordCmdBindPipeline(VkCommandBuffer commandBuffer,
                                                           VkPipelineBindPoint pipelineBindPoint,
                                                           VkPipeline pipeline)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    ...
    if (enabled_features.core.variableMultisampleRate == VK_FALSE)
    {
        if (const auto *multisample_state = pipe_state->MultisampleState(); multisample_state)
        {
            if (const auto &render_pass = cb_state->activeRenderPass; render_pass)
            {
                ...
                // if render pass uses no attachment, all bound pipelines in the same subpass must
                // have the same pMultisampleState->rasterizationSamples. To check that, record
                // pMultisampleState->rasterizationSamples of the first bound pipeline.
                if (!render_pass->UsesDynamicRendering() &&
                    !render_pass->UsesColorAttachment(subpass) &&
                    !render_pass->UsesDepthStencilAttachment(subpass))
                {
                    ...
                    {
                        cb_state->SetActiveSubpassRasterizationSampleCount(
...
void ValidationStateTracker::PostCallRecordCmdSetViewport(VkCommandBuffer commandBuffer,
                                                          ...
                                                          const VkViewport *pViewports)
{
    ...
    cb_state->dynamicViewports.resize(
        std::max(size_t(firstViewport + viewportCount), cb_state->dynamicViewports.size()));
...
void ValidationStateTracker::PostCallRecordCmdSetExclusiveScissorNV(
    VkCommandBuffer commandBuffer,
    ...
    const VkRect2D *pExclusiveScissors)
{
    ...
    // cb_state->exclusiveScissorMask |= ((1u << exclusiveScissorCount) - 1u) <<
    // firstExclusiveScissor;
}

void ValidationStateTracker::PreCallRecordCmdBindShadingRateImageNV(VkCommandBuffer commandBuffer,
                                                                    VkImageView imageView,
                                                                    VkImageLayout imageLayout)
{
    if (disabled[command_buffer_state])
...
void ValidationStateTracker::PostCallRecordCmdSetViewportShadingRatePaletteNV(
    VkCommandBuffer commandBuffer,
    uint32_t firstViewport,
    uint32_t viewportCount,
...
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETVIEWPORTSHADINGRATEPALETTENV,
                             CB_DYNAMIC_VIEWPORT_SHADING_RATE_PALETTE_NV_SET);
...
void ValidationStateTracker::PostCallRecordCreateAccelerationStructureNV(
    VkDevice device,
    ...
    VkResult result)
{
    ...
    std::shared_ptr<ACCELERATION_STRUCTURE_STATE> state =
        std::make_shared<ACCELERATION_STRUCTURE_STATE_LINEAR>(device, *pAccelerationStructure,
                                                              pCreateInfo);
    Add(std::move(state));
}

void ValidationStateTracker::PostCallRecordCreateAccelerationStructureKHR(
    VkDevice device,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    auto buffer_state = Get<BUFFER_STATE>(pCreateInfo->buffer);
    Add(std::make_shared<ACCELERATION_STRUCTURE_STATE_KHR>(*pAccelerationStructure, pCreateInfo,
    ...
    VkResult result)
{
    ...
    {
        auto dst_as_state =
            Get<ACCELERATION_STRUCTURE_STATE_KHR>(pInfos[i].dstAccelerationStructure);
...
void ValidationStateTracker::RecordDeviceAccelerationStructureBuildInfo(
    ...
    const VkAccelerationStructureBuildGeometryInfoKHR &info)
{
    ...
    for (uint32_t i = 0; i < info.geometryCount; i++)
    {
        ...
        switch (geom.geometryType)
        {
            ...
            {
                auto vertex_buffers =
                    GetBuffersByAddress(geom.geometry.triangles.vertexData.deviceAddress);
                if (!vertex_buffers.empty())
                {
                    cb_state.AddChildren(vertex_buffers);
                }
                auto index_buffers =
                    GetBuffersByAddress(geom.geometry.triangles.indexData.deviceAddress);
                if (!index_buffers.empty())
                {
                    cb_state.AddChildren(index_buffers);
                }
                auto transform_buffers =
                    GetBuffersByAddress(geom.geometry.triangles.transformData.deviceAddress);
                if (!transform_buffers.empty())
                {
                    cb_state.AddChildren(transform_buffers);
                }
                const auto *motion_data =
                    LvlFindInChain<VkAccelerationStructureGeometryMotionTrianglesDataNV>(
                        info.pNext);
                if (motion_data)
                {
                    auto motion_buffers =
                        GetBuffersByAddress(motion_data->vertexData.deviceAddress);
            ...
            {
                // NOTE: if arrayOfPointers is true, we don't track the pointers in the array. That
                // would require that data buffer be mapped to the CPU so that we could walk through
                // it. We can't easily ensure that's true.
    ...
    VkResult result)
{
    ...
    for (uint32_t i = 0; i < bindInfoCount; i++)
    {
        ...
        if (as_state)
        {
            ...
            if (mem_state)
            {
                as_state->BindMemory(as_state.get(), mem_state, info.memoryOffset, 0u,
                                     as_state->memory_requirements.size);
            }

            // GPU validation of top level acceleration structure building needs acceleration
            // structure handles.
            // XXX TODO: Query device address for KHR extension
            if (enabled[gpu_validation])
            {
                DispatchGetAccelerationStructureHandleNV(device, info.accelerationStructure, 8,
                                                         &as_state->opaque_handle);
            }
        }
    }
}
...
void ValidationStateTracker::PostCallRecordCmdCopyAccelerationStructureNV(
    VkCommandBuffer commandBuffer,
    VkAccelerationStructureNV dst,
    VkAccelerationStructureNV src,
    VkCopyAccelerationStructureModeNV mode)
{
    ...
    if (cb_state)
    {
        ...
        if (!disabled[command_buffer_state])
        {
            cb_state->RecordTransferCmd(CMD_COPYACCELERATIONSTRUCTURENV, src_as_state,
                                        dst_as_state);
        }
...
void ValidationStateTracker::PreCallRecordDestroyAccelerationStructureKHR(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyAccelerationStructureNV(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCmdSetViewportWScalingNV(
    VkCommandBuffer commandBuffer,
    uint32_t firstViewport,
    uint32_t viewportCount,
...
void ValidationStateTracker::PostCallRecordCmdSetLineWidth(VkCommandBuffer commandBuffer,
                                                           float lineWidth)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdSetLineStippleEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthBias(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetScissor(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetBlendConstants(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthBounds(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetStencilCompareMask(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetStencilWriteMask(VkCommandBuffer commandBuffer,
                                                                  VkStencilFaceFlags faceMask,
                                                                  uint32_t writeMask)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdSetStencilReference(VkCommandBuffer commandBuffer,
                                                                  VkStencilFaceFlags faceMask,
                                                                  uint32_t reference)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PreCallRecordCmdBindDescriptorSets(
    VkCommandBuffer commandBuffer,
    VkPipelineBindPoint pipelineBindPoint,
    ...
    const uint32_t *pDynamicOffsets)
{
    ...
    cb_state->UpdateLastBoundDescriptorSets(pipelineBindPoint, *pipeline_layout, firstSet, setCount,
                                            pDescriptorSets, no_push_desc, dynamicOffsetCount,
                                            pDynamicOffsets);
}

void ValidationStateTracker::PreCallRecordCmdPushDescriptorSetKHR(
    VkCommandBuffer commandBuffer,
    VkPipelineBindPoint pipelineBindPoint,
    VkPipelineLayout layout,
    uint32_t set,
    ...
    const VkWriteDescriptorSet *pDescriptorWrites)
{
    auto cb_state        = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    auto pipeline_layout = Get<PIPELINE_LAYOUT_STATE>(layout);
    cb_state->PushDescriptorSetState(pipelineBindPoint, *pipeline_layout, set, descriptorWriteCount,
                                     pDescriptorWrites);
}

void ValidationStateTracker::PreCallRecordCmdBindDescriptorBuffersEXT(
    VkCommandBuffer commandBuffer,
    ...
    const VkDescriptorBufferBindingInfoEXT *pBindingInfos)
{
    ...
    std::copy(pBindingInfos, pBindingInfos + bufferCount,
              cb_state->descriptor_buffer_binding_info.data());
}
    ...
    const VkDeviceSize *pOffsets)
{
    ...
    cb_state->UpdateLastBoundDescriptorBuffers(pipelineBindPoint, *pipeline_layout, firstSet,
                                               setCount, pBufferIndices, pOffsets);
}

void ValidationStateTracker::PostCallRecordCmdPushConstants(VkCommandBuffer commandBuffer,
                                                            VkPipelineLayout layout,
    ...
    if (cb_state)
    {
        ...
        while (flags)
        {
            if (flags & 1)
            {
                ...
                {
                    std::memset(it->second.data() + offset, PC_Byte_Updated,
...
void ValidationStateTracker::PreCallRecordCmdBindIndexBuffer(VkCommandBuffer commandBuffer,
                                                             VkBuffer buffer,
                                                             VkDeviceSize offset,
                                                             VkIndexType indexType)
{
    ...
    cb_state->index_buffer_binding =
        IndexBufferBinding(Get<BUFFER_STATE>(buffer), offset, indexType);
...
void ValidationStateTracker::PreCallRecordCmdBindVertexBuffers(VkCommandBuffer commandBuffer,
                                                               ...
                                                               const VkDeviceSize *pOffsets)
{
    ...
    for (uint32_t i = 0; i < bindingCount; ++i)
    {
        auto &vertex_buffer_binding =
            cb_state->current_vertex_buffer_binding_info.vertex_buffer_bindings[i + firstBinding];
...
void ValidationStateTracker::PostCallRecordCmdUpdateBuffer(VkCommandBuffer commandBuffer,
                                                           VkBuffer dstBuffer,
                                                           VkDeviceSize dstOffset,
...
void ValidationStateTracker::PreCallRecordCmdSetEvent(VkCommandBuffer commandBuffer,
                                                      VkEvent event,
...
void ValidationStateTracker::PreCallRecordCmdSetEvent2KHR(
    VkCommandBuffer commandBuffer,
    VkEvent event,
...
void ValidationStateTracker::PreCallRecordCmdSetEvent2(VkCommandBuffer commandBuffer,
                                                       VkEvent event,
...
void ValidationStateTracker::PreCallRecordCmdResetEvent(VkCommandBuffer commandBuffer,
                                                        VkEvent event,
                                                        VkPipelineStageFlags stageMask)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PreCallRecordCmdResetEvent2KHR(VkCommandBuffer commandBuffer,
                                                            VkEvent event,
...
void ValidationStateTracker::PreCallRecordCmdResetEvent2(VkCommandBuffer commandBuffer,
                                                         VkEvent event,
    ...
    const VkImageMemoryBarrier *pImageMemoryBarriers)
{
    ...
    cb_state->RecordBarriers(memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount,
                             pBufferMemoryBarriers, imageMemoryBarrierCount, pImageMemoryBarriers);
}

void ValidationStateTracker::PreCallRecordCmdWaitEvents2KHR(
    VkCommandBuffer commandBuffer,
    uint32_t eventCount,
    const VkEvent *pEvents,
...
void ValidationStateTracker::PreCallRecordCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                                         uint32_t eventCount,
                                                         const VkEvent *pEvents,
...
void ValidationStateTracker::PreCallRecordCmdPipelineBarrier2KHR(
    VkCommandBuffer commandBuffer,
    const VkDependencyInfoKHR *pDependencyInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PreCallRecordCmdPipelineBarrier2(
    VkCommandBuffer commandBuffer,
    const VkDependencyInfo *pDependencyInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdBeginQuery(VkCommandBuffer commandBuffer,
                                                         VkQueryPool queryPool,
                                                         uint32_t slot,
                                                         VkFlags flags)
{
...
void ValidationStateTracker::PostCallRecordCmdEndQuery(VkCommandBuffer commandBuffer,
                                                       VkQueryPool queryPool,
                                                       uint32_t slot)
{
    if (disabled[query_validation])
...
void ValidationStateTracker::PostCallRecordCmdResetQueryPool(VkCommandBuffer commandBuffer,
                                                             VkQueryPool queryPool,
                                                             uint32_t firstQuery,
                                                             uint32_t queryCount)
{
...
void ValidationStateTracker::PostCallRecordCmdCopyQueryPoolResults(VkCommandBuffer commandBuffer,
                                                                   VkQueryPool queryPool,
                                                                   uint32_t firstQuery,
                                                                   uint32_t queryCount,
                                                                   VkBuffer dstBuffer,
...
{
    if (disabled[query_validation] || disabled[command_buffer_state])
        return;
...
void ValidationStateTracker::PostCallRecordCmdWriteTimestamp(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdWriteTimestamp2KHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdWriteTimestamp2(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCreateVideoSessionKHR(
    VkDevice device,
    ...
    VkResult result)
{
    ...
    Add(std::make_shared<VIDEO_SESSION_STATE>(this, *pVideoSession, pCreateInfo,
                                              std::move(profile_desc)));
}
...
void ValidationStateTracker::PostCallRecordBindVideoSessionMemoryKHR(
    VkDevice device,
    VkVideoSessionKHR videoSession,
...
void ValidationStateTracker::PreCallRecordDestroyVideoSessionKHR(
    VkDevice device,
    VkVideoSessionKHR videoSession,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PostCallRecordCreateVideoSessionParametersKHR(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordUpdateVideoSessionParametersKHR(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyVideoSessionParametersKHR(
    VkDevice device,
    VkVideoSessionParametersKHR videoSessionParameters,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PostCallRecordCreateFramebuffer(
    VkDevice device,
    ...
    VkResult result)
{
    ...
    Add(std::make_shared<FRAMEBUFFER_STATE>(*pFramebuffer, pCreateInfo,
                                            Get<RENDER_PASS_STATE>(pCreateInfo->renderPass),
                                            std::move(views)));
}

void ValidationStateTracker::PostCallRecordCreateRenderPass(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateRenderPass2KHR(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateRenderPass2(
    VkDevice device,
    const VkRenderPassCreateInfo2 *pCreateInfo,
    const VkAllocationCallbacks *pAllocator,
    VkRenderPass *pRenderPass,
...
void ValidationStateTracker::PreCallRecordCmdBeginRenderPass(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBeginRenderPass2KHR(
    VkCommandBuffer commandBuffer,
    const VkRenderPassBeginInfo *pRenderPassBegin,
    const VkSubpassBeginInfo *pSubpassBeginInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->BeginRenderPass(CMD_BEGINRENDERPASS2KHR, pRenderPassBegin,
                              pSubpassBeginInfo->contents);
}

void ValidationStateTracker::PreCallRecordCmdBeginVideoCodingKHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdBeginTransformFeedbackEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdEndTransformFeedbackEXT(
    VkCommandBuffer commandBuffer,
    uint32_t firstCounterBuffer,
    uint32_t counterBufferCount,
    const VkBuffer *pCounterBuffers,
...
void ValidationStateTracker::PostCallRecordCmdEndConditionalRenderingEXT(
    VkCommandBuffer commandBuffer)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PreCallRecordCmdBeginRenderingKHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBeginRendering(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdBeginRenderPass2(
    VkCommandBuffer commandBuffer,
    const VkRenderPassBeginInfo *pRenderPassBegin,
    const VkSubpassBeginInfo *pSubpassBeginInfo)
{
...
void ValidationStateTracker::PostCallRecordCmdNextSubpass(VkCommandBuffer commandBuffer,
                                                          VkSubpassContents contents)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdNextSubpass2KHR(
    VkCommandBuffer commandBuffer,
    const VkSubpassBeginInfo *pSubpassBeginInfo,
...
void ValidationStateTracker::PostCallRecordCmdNextSubpass2(
    VkCommandBuffer commandBuffer,
    const VkSubpassBeginInfo *pSubpassBeginInfo,
    const VkSubpassEndInfo *pSubpassEndInfo)
{
...
void ValidationStateTracker::PostCallRecordCmdEndRenderPass2KHR(
    VkCommandBuffer commandBuffer,
    const VkSubpassEndInfo *pSubpassEndInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdEndRenderPass2(
    VkCommandBuffer commandBuffer,
    const VkSubpassEndInfo *pSubpassEndInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdEndVideoCodingKHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdExecuteCommands(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordMapMemory(VkDevice device,
                                                     VkDeviceMemory mem,
                                                     VkDeviceSize offset,
                                                     VkDeviceSize size,
                                                     VkFlags flags,
...
void ValidationStateTracker::PostCallRecordMapMemory2KHR(VkDevice device,
                                                         ...
                                                         VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordMappedMemory(pMemoryMapInfo->memory, pMemoryMapInfo->offset, pMemoryMapInfo->size,
                       ppData);
}
...
void ValidationStateTracker::PreCallRecordUnmapMemory2KHR(
    VkDevice device,
...
void ValidationStateTracker::UpdateBindImageMemoryState(const VkBindImageMemoryInfo &bindInfo)
{
    ...
    if (image_state)
    {
        ...
        image_state->fragment_encoder =
            std::unique_ptr<const subresource_adapter::ImageRangeEncoder>(
                new subresource_adapter::ImageRangeEncoder(*image_state));
        const auto swapchain_info =
            LvlFindInChain<VkBindImageMemorySwapchainInfoKHR>(bindInfo.pNext);
        if (swapchain_info)
        {
        ...
        else
        {
            ...
            if (mem_info)
            {
                ...
                image_state->BindMemory(
                    ...
                    image_state
                        ->requirements[static_cast<decltype(image_state->requirements)::size_type>(
                            plane_index)]
                        .size);
            }
        }
...
void ValidationStateTracker::PostCallRecordBindImageMemory(VkDevice device,
                                                           VkImage image,
                                                           VkDeviceMemory mem,
                                                           VkDeviceSize memoryOffset,
                                                           VkResult result)
...
void ValidationStateTracker::PostCallRecordBindImageMemory2(VkDevice device,
                                                            uint32_t bindInfoCount,
...
void ValidationStateTracker::PostCallRecordBindImageMemory2KHR(
    VkDevice device,
    uint32_t bindInfoCount,
    const VkBindImageMemoryInfo *pBindInfos,
    VkResult result)
...
void ValidationStateTracker::PostCallRecordImportSemaphoreFdKHR(
    VkDevice device,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordImportSemaphoreState(pImportSemaphoreFdInfo->semaphore,
                               pImportSemaphoreFdInfo->handleType, pImportSemaphoreFdInfo->flags);
}

void ValidationStateTracker::RecordGetExternalSemaphoreState(
    VkSemaphore semaphore,
    VkExternalSemaphoreHandleTypeFlagBits handle_type)
{
    auto semaphore_state = Get<SEMAPHORE_STATE>(semaphore);
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordImportSemaphoreState(pImportSemaphoreWin32HandleInfo->semaphore,
                               pImportSemaphoreWin32HandleInfo->handleType,
                               pImportSemaphoreWin32HandleInfo->flags);
}

void ValidationStateTracker::PostCallRecordGetSemaphoreWin32HandleKHR(
    VkDevice device,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordGetExternalSemaphoreState(pGetWin32HandleInfo->semaphore,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordImportFenceState(pImportFenceWin32HandleInfo->fence,
                           pImportFenceWin32HandleInfo->handleType,
                           pImportFenceWin32HandleInfo->flags);
}

void ValidationStateTracker::PostCallRecordGetFenceWin32HandleKHR(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordGetSemaphoreFdKHR(
    VkDevice device,
...
void ValidationStateTracker::RecordImportFenceState(VkFence fence,
...
#ifdef VK_USE_PLATFORM_WIN32_KHR
void ValidationStateTracker::PostCallRecordGetMemoryWin32HandleKHR(
    VkDevice device,
    ...
    VkResult result)
{
    ...
    if (const auto memory_state = Get<DEVICE_MEMORY_STATE>(pGetWin32HandleInfo->memory))
    {
        ...
        // closed handle value for a new handle. The validation layer does not track handle close
        // operation which is performed by 'CloseHandle' system call.
        win32_handle_map_.insert_or_assign(*pHandle, alloc_info);
    }
}
#endif

void ValidationStateTracker::PostCallRecordGetMemoryFdKHR(VkDevice device,
...
void ValidationStateTracker::PostCallRecordImportFenceFdKHR(
    VkDevice device,
    ...
    VkResult result)
{
    if (VK_SUCCESS != result)
        return;
    RecordImportFenceState(pImportFenceFdInfo->fence, pImportFenceFdInfo->handleType,
                           pImportFenceFdInfo->flags);
}

void ValidationStateTracker::RecordGetExternalFenceState(
    VkFence fence,
    VkExternalFenceHandleTypeFlagBits handle_type)
{
    auto fence_state = Get<FENCE_STATE>(fence);
...
void ValidationStateTracker::PostCallRecordGetFenceFdKHR(VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateEvent(VkDevice device,
...
void ValidationStateTracker::RecordCreateSwapchainState(
    VkResult result,
    const VkSwapchainCreateInfoKHR *pCreateInfo,
    VkSwapchainKHR *pSwapchain,
    std::shared_ptr<SURFACE_STATE> &&surface_state,
    SWAPCHAIN_NODE *old_swapchain_state)
{
    if (VK_SUCCESS == result)
    {
        ...
        auto swapchain_present_modes_ci =
        ...
        if (swapchain_present_modes_ci)
        {
            ...
            std::copy(swapchain_present_modes_ci->pPresentModes,
                      swapchain_present_modes_ci->pPresentModes + present_mode_count,
...
void ValidationStateTracker::PostCallRecordCreateSwapchainKHR(
    VkDevice device,
    const VkSwapchainCreateInfoKHR *pCreateInfo,
    const VkAllocationCallbacks *pAllocator,
    VkSwapchainKHR *pSwapchain,
    VkResult result)
{
    ...
    RecordCreateSwapchainState(result, pCreateInfo, pSwapchain, std::move(surface_state),
                               old_swapchain_state.get());
}

void ValidationStateTracker::PreCallRecordDestroySwapchainKHR(
    VkDevice device,
    VkSwapchainKHR swapchain,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::PostCallRecordCreateDisplayModeKHR(
...
void ValidationStateTracker::PostCallRecordQueuePresentKHR(VkQueue queue,
                                                           const VkPresentInfoKHR *pPresentInfo,
                                                           VkResult result)
{
    // spec: If vkQueuePresentKHR fails to enqueue the corresponding set of queue operations, it may
    // return VK_ERROR_OUT_OF_HOST_MEMORY or VK_ERROR_OUT_OF_DEVICE_MEMORY. If it does, the
    // implementation must ensure that the state and contents of any resources or synchronization
    ...
    // guarantee, the implementation must return VK_ERROR_DEVICE_LOST.
    //
    ...
    //
    // NOTE: This is the only queue submit-like call that has its state updated in PostCallRecord().
    // In part that is because of these non-fatal error cases. Also we need a place to handle the
    // swapchain image bookkeeping, which really should be happening once all the wait semaphores
    // have completed. Since most of the PostCall queue submit race conditions are related to
    // timeline semaphores, and acquire sempaphores are always binary, this seems ok-ish.
    ...
    {
        // Note: this is imperfect, in that we can get confused about what did or didn't succeed--
        // but if the app does that, it's confused itself just as much.
        ...
        if (swapchain_data)
        {
            uint64_t present_id = (present_id_info && i < present_id_info->swapchainCount)
...
void ValidationStateTracker::PostCallRecordCreateSharedSwapchainsKHR(
    VkDevice device,
    ...
    VkResult result)
{
    if (pCreateInfos)
    {
        for (uint32_t i = 0; i < swapchainCount; i++)
        {
            ...
            RecordCreateSwapchainState(result, &pCreateInfos[i], &pSwapchains[i],
                                       std::move(surface_state), old_swapchain_state.get());
        }
    }
}

void ValidationStateTracker::RecordAcquireNextImageState(VkDevice device,
                                                         VkSwapchainKHR swapchain,
                                                         uint64_t timeout,
                                                         VkSemaphore semaphore,
                                                         VkFence fence,
                                                         ...
                                                         const char *func_name)
{
    ...
    if (fence_state)
    {
        // Treat as inflight since it is valid to wait on this fence, even in cases where it is
        // technically a temporary import
    ...
    if (semaphore_state)
    {
        // Treat as signaled since it is valid to wait on this semaphore, even in cases where it is
        // technically a temporary import
...
void ValidationStateTracker::PostCallRecordAcquireNextImageKHR(VkDevice device,
                                                               VkSwapchainKHR swapchain,
                                                               uint64_t timeout,
                                                               VkSemaphore semaphore,
                                                               VkFence fence,
...
void ValidationStateTracker::PostCallRecordAcquireNextImage2KHR(
    VkDevice device,
    ...
    VkResult result)
{
    if ((VK_SUCCESS != result) && (VK_SUBOPTIMAL_KHR != result))
        return;
    RecordAcquireNextImageState(device, pAcquireInfo->swapchain, pAcquireInfo->timeout,
                                pAcquireInfo->semaphore, pAcquireInfo->fence, pImageIndex,
                                "vkAcquireNextImage2KHR");
}

std::shared_ptr<PHYSICAL_DEVICE_STATE> ValidationStateTracker::CreatePhysicalDeviceState(
                                                          ...
                                                          VkResult result)
{
    ...
    auto export_metal_object_info =
        LvlFindInChain<VkExportMetalObjectCreateInfoEXT>(pCreateInfo->pNext);
    while (export_metal_object_info)
    {
        export_metal_flags.push_back(export_metal_object_info->exportObjectType);
        export_metal_object_info =
            LvlFindInChain<VkExportMetalObjectCreateInfoEXT>(export_metal_object_info->pNext);
    }
#endif  // VK_USE_PLATFORM_METAL_EXT
}

// Common function to update state for GetPhysicalDeviceQueueFamilyProperties & 2KHR version
static void StateUpdateCommonGetPhysicalDeviceQueueFamilyProperties(PHYSICAL_DEVICE_STATE *pd_state,
                                                                    uint32_t count)
{
    pd_state->queue_family_known_count = std::max(pd_state->queue_family_known_count, count);
}

void ValidationStateTracker::PostCallRecordGetPhysicalDeviceQueueFamilyProperties(
    VkPhysicalDevice physicalDevice,
    ...
    VkQueueFamilyProperties *pQueueFamilyProperties)
{
    ...
    assert(pd_state);
    StateUpdateCommonGetPhysicalDeviceQueueFamilyProperties(pd_state.get(),
...
void ValidationStateTracker::PreCallRecordDestroySurfaceKHR(VkInstance instance,
...
{
    Add(std::make_shared<SURFACE_STATE>(*pSurface));
}

void ValidationStateTracker::PostCallRecordCreateDisplayPlaneSurfaceKHR(
    VkInstance instance,
...
#ifdef VK_USE_PLATFORM_ANDROID_KHR
void ValidationStateTracker::PostCallRecordCreateAndroidSurfaceKHR(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateImagePipeSurfaceFUCHSIA(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateIOSSurfaceMVK(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateMacOSSurfaceMVK(
    VkInstance instance,
...
#ifdef VK_USE_PLATFORM_METAL_EXT
void ValidationStateTracker::PostCallRecordCreateMetalSurfaceEXT(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateWaylandSurfaceKHR(
    VkInstance instance,
...
#ifdef VK_USE_PLATFORM_WIN32_KHR
void ValidationStateTracker::PostCallRecordCreateWin32SurfaceKHR(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateXcbSurfaceKHR(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateXlibSurfaceKHR(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordCreateHeadlessSurfaceEXT(
    VkInstance instance,
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfaceCapabilitiesKHR(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
    ...
    VkResult result)
{
    ...
    if (pSurfaceInfo->surface)
    {
        ...
        const VkSurfacePresentModeEXT *surface_present_mode =
            LvlFindInChain<VkSurfacePresentModeEXT>(pSurfaceInfo->pNext);
        ...
        else
        {
            ...
            if (compatible_modes && compatible_modes->pPresentModes)
            {
                surface_state->SetCompatibleModes(
                    ...
                    vvl::span<const VkPresentModeKHR>(compatible_modes->pPresentModes,
                                                      compatible_modes->presentModeCount));
            }
            if (present_scaling_caps)
            {
                surface_state->SetPresentModeCapabilities(
                    physicalDevice, surface_present_mode->presentMode,
             ...
             LvlFindInChain<VkSurfaceProtectedCapabilitiesKHR>(pSurfaceCapabilities->pNext))
    {
        auto pd_state = Get<PHYSICAL_DEVICE_STATE>(physicalDevice);
        assert(pd_state);
        pd_state->surfaceless_query_state.capabilities =
            safe_VkSurfaceCapabilities2KHR(pSurfaceCapabilities);
    }
}

void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfaceCapabilities2EXT(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfaceSupportKHR(
    VkPhysicalDevice physicalDevice,
    uint32_t queueFamilyIndex,
    VkSurfaceKHR surface,
    VkBool32 *pSupported,
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfacePresentModesKHR(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
    ...
    VkResult result)
{
    ...
    if (pPresentModes)
    {
        ...
        {
            auto surface_state = Get<SURFACE_STATE>(surface);
            surface_state->SetPresentModes(physicalDevice, vvl::span<const VkPresentModeKHR>(
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfaceFormatsKHR(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
    ...
    VkResult result)
{
    ...
    if (pSurfaceFormats)
    {
        ...
        for (uint32_t surface_format_index = 0; surface_format_index < *pSurfaceFormatCount;
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceSurfaceFormats2KHR(
    VkPhysicalDevice physicalDevice,
    const VkPhysicalDeviceSurfaceInfo2KHR *pSurfaceInfo,
    uint32_t *pSurfaceFormatCount,
...
void ValidationStateTracker::PreCallRecordCmdBeginDebugUtilsLabelEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdInsertDebugUtilsLabelEXT(
    VkCommandBuffer commandBuffer,
    const VkDebugUtilsLabelEXT *pLabelInfo)
{
...
void ValidationStateTracker::RecordEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCounters(
    VkPhysicalDevice physicalDevice,
    uint32_t queueFamilyIndex,
    ...
    VkPerformanceCounterKHR *pCounters)
{
    ...
    std::unique_ptr<QUEUE_FAMILY_PERF_COUNTERS> queue_family_counters(
        new QUEUE_FAMILY_PERF_COUNTERS());
    queue_family_counters->counters.resize(*pCounterCount);
    for (uint32_t i = 0; i < *pCounterCount; i++)
        queue_family_counters->counters[i] = pCounters[i];

    pd_state->perf_counters[queueFamilyIndex] = std::move(queue_family_counters);
}

void ValidationStateTracker::
    PostCallRecordEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCountersKHR(
        VkPhysicalDevice physicalDevice,
        uint32_t queueFamilyIndex,
        uint32_t *pCounterCount,
        VkPerformanceCounterKHR *pCounters,
        ...
        VkResult result)
{
    if ((VK_SUCCESS != result) && (VK_INCOMPLETE != result))
        return;
    RecordEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCounters(
        physicalDevice, queueFamilyIndex, pCounterCount, pCounters);
}

void ValidationStateTracker::PostCallRecordAcquireProfilingLockKHR(
    VkDevice device,
    const VkAcquireProfilingLockInfoKHR *pInfo,
    VkResult result)
{
    if (result == VK_SUCCESS)
        performance_lock_acquired = true;
}
...
void ValidationStateTracker::PreCallRecordDestroyDescriptorUpdateTemplate(
    VkDevice device,
...
void ValidationStateTracker::PreCallRecordDestroyDescriptorUpdateTemplateKHR(
    VkDevice device,
    VkDescriptorUpdateTemplate descriptorUpdateTemplate,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::RecordCreateDescriptorUpdateTemplateState(
...
void ValidationStateTracker::RecordUpdateDescriptorSetWithTemplateState(
    ...
    const void *pData)
{
    ...
    if (template_state)
    {
        ...
        if (template_state->create_info.templateType ==
...
void ValidationStateTracker::PreCallRecordUpdateDescriptorSetWithTemplate(
    VkDevice device,
    VkDescriptorSet descriptorSet,
    VkDescriptorUpdateTemplate descriptorUpdateTemplate,
    const void *pData)
...
void ValidationStateTracker::PreCallRecordUpdateDescriptorSetWithTemplateKHR(
    VkDevice device,
    VkDescriptorSet descriptorSet,
    VkDescriptorUpdateTemplate descriptorUpdateTemplate,
    const void *pData)
    ...
    const void *pData)
{
    ...
    cvdescriptorset::DecodedTemplateUpdate decoded_template(
        this, VK_NULL_HANDLE, template_state.get(), pData, dsl->GetDescriptorSetLayout());
...
void ValidationStateTracker::RecordGetPhysicalDeviceDisplayPlanePropertiesState(
    VkPhysicalDevice physicalDevice,
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceDisplayPlanePropertiesKHR(
    VkPhysicalDevice physicalDevice,
    uint32_t *pPropertyCount,
...
void ValidationStateTracker::PostCallRecordGetPhysicalDeviceDisplayPlaneProperties2KHR(
    VkPhysicalDevice physicalDevice,
    uint32_t *pPropertyCount,
...
void ValidationStateTracker::PostCallRecordCmdBeginQueryIndexedEXT(VkCommandBuffer commandBuffer,
                                                                   VkQueryPool queryPool,
                                                                   uint32_t query,
...
void ValidationStateTracker::PostCallRecordCmdEndQueryIndexedEXT(VkCommandBuffer commandBuffer,
                                                                 VkQueryPool queryPool,
                                                                 uint32_t query,
                                                                 uint32_t index)
{
...
void ValidationStateTracker::RecordCreateSamplerYcbcrConversionState(
    ...
    VkSamplerYcbcrConversion ycbcr_conversion)
{
    ...
    Add(std::make_shared<SAMPLER_YCBCR_CONVERSION_STATE>(ycbcr_conversion, create_info,
                                                         format_features));
}

void ValidationStateTracker::PostCallRecordCreateSamplerYcbcrConversion(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordCreateSamplerYcbcrConversionKHR(
    VkDevice device,
    const VkSamplerYcbcrConversionCreateInfo *pCreateInfo,
    const VkAllocationCallbacks *pAllocator,
    VkSamplerYcbcrConversion *pYcbcrConversion,
...
void ValidationStateTracker::PostCallRecordDestroySamplerYcbcrConversion(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordDestroySamplerYcbcrConversionKHR(
    VkDevice device,
    VkSamplerYcbcrConversion ycbcrConversion,
    const VkAllocationCallbacks *pAllocator)
{
...
void ValidationStateTracker::RecordResetQueryPool(VkDevice device,
                                                  VkQueryPool queryPool,
                                                  uint32_t firstQuery,
                                                  uint32_t queryCount)
{
    ...
    const uint32_t max_query_count =
        std::min(queryCount, query_pool_state->createInfo.queryCount - firstQuery);
    for (uint32_t i = 0; i < max_query_count; ++i)
    {
        ...
        {
            for (uint32_t pass_index = 0; pass_index < query_pool_state->n_performance_passes;
                 pass_index++)
            {
...
void ValidationStateTracker::PostCallRecordResetQueryPoolEXT(VkDevice device,
                                                             VkQueryPool queryPool,
                                                             uint32_t firstQuery,
                                                             uint32_t queryCount)
{
...
void ValidationStateTracker::PostCallRecordResetQueryPool(VkDevice device,
                                                          VkQueryPool queryPool,
                                                          uint32_t firstQuery,
                                                          uint32_t queryCount)
{
...
void ValidationStateTracker::PerformUpdateDescriptorSetsWithTemplateKHR(
    VkDescriptorSet descriptorSet,
    const UPDATE_TEMPLATE_STATE *template_state,
    const void *pData)
{
    // Translate the templated update into a normal update for validation...
    cvdescriptorset::DecodedTemplateUpdate decoded_update(this, descriptorSet, template_state,
                                                          pData);
    cvdescriptorset::PerformUpdateDescriptorSets(
        this, static_cast<uint32_t>(decoded_update.desc_writes.size()),
        decoded_update.desc_writes.data(), 0, NULL);
}

// Update the common AllocateDescriptorSetsData
void ValidationStateTracker::UpdateAllocateDescriptorSetsData(
    ...
    for (uint32_t i = 0; i < p_alloc_info->descriptorSetCount; i++)
    {
        ...
        if (layout)
        {
            ...
            for (uint32_t j = 0; j < layout->GetBindingCount(); ++j)
            {
                ...
                ds_data->required_descriptors_by_type[type_index] +=
                    binding_layout->descriptorCount;
            }
        }
        // Any unknown layouts will be flagged as errors during ValidateAllocateDescriptorSets()
        // call
    }
}

void ValidationStateTracker::PostCallRecordCmdDraw(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdDrawMultiEXT(VkCommandBuffer commandBuffer,
                                                           uint32_t drawCount,
...
void ValidationStateTracker::PostCallRecordCmdDrawIndexed(VkCommandBuffer commandBuffer,
                                                          uint32_t indexCount,
                                                          uint32_t instanceCount,
                                                          uint32_t firstIndex,
...
void ValidationStateTracker::PostCallRecordCmdDrawMultiIndexedEXT(
    VkCommandBuffer commandBuffer,
    uint32_t drawCount,
...
void ValidationStateTracker::PostCallRecordCmdDrawIndirect(VkCommandBuffer commandBuffer,
                                                           VkBuffer buffer,
                                                           VkDeviceSize offset,
                                                           uint32_t count,
                                                           uint32_t stride)
...
void ValidationStateTracker::PostCallRecordCmdDrawIndexedIndirect(VkCommandBuffer commandBuffer,
                                                                  VkBuffer buffer,
                                                                  VkDeviceSize offset,
                                                                  uint32_t count,
                                                                  uint32_t stride)
...
void ValidationStateTracker::PostCallRecordCmdDispatch(VkCommandBuffer commandBuffer,
                                                       uint32_t x,
                                                       uint32_t y,
                                                       uint32_t z)
{
...
void ValidationStateTracker::PostCallRecordCmdDispatchIndirect(VkCommandBuffer commandBuffer,
                                                               VkBuffer buffer,
                                                               VkDeviceSize offset)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCmdDispatchBaseKHR(VkCommandBuffer commandBuffer,
                                                              uint32_t,
                                                              uint32_t,
                                                              uint32_t,
                                                              uint32_t,
...
void ValidationStateTracker::PostCallRecordCmdDispatchBase(VkCommandBuffer commandBuffer,
                                                           uint32_t,
                                                           uint32_t,
                                                           uint32_t,
                                                           uint32_t,
...
void ValidationStateTracker::RecordCmdDrawIndirectCount(VkCommandBuffer commandBuffer,
                                                        VkBuffer buffer,
                                                        VkDeviceSize offset,
...
void ValidationStateTracker::PreCallRecordCmdDrawIndirectCountKHR(VkCommandBuffer commandBuffer,
                                                                  VkBuffer buffer,
                                                                  VkDeviceSize offset,
                                                                  VkBuffer countBuffer,
                                                                  VkDeviceSize countBufferOffset,
                                                                  ...
                                                                  uint32_t stride)
{
    RecordCmdDrawIndirectCount(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                               maxDrawCount, stride, CMD_DRAWINDIRECTCOUNTKHR);
}

void ValidationStateTracker::PreCallRecordCmdDrawIndirectCount(VkCommandBuffer commandBuffer,
                                                               VkBuffer buffer,
                                                               VkDeviceSize offset,
                                                               VkBuffer countBuffer,
                                                               VkDeviceSize countBufferOffset,
...
{
    RecordCmdDrawIndirectCount(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                               maxDrawCount, stride, CMD_DRAWINDIRECTCOUNT);
}

void ValidationStateTracker::RecordCmdDrawIndexedIndirectCount(VkCommandBuffer commandBuffer,
                                                               VkBuffer buffer,
                                                               VkDeviceSize offset,
                                                               VkBuffer countBuffer,
                                                               VkDeviceSize countBufferOffset,
...
void ValidationStateTracker::PreCallRecordCmdDrawIndexedIndirectCountKHR(
    VkCommandBuffer commandBuffer,
    VkBuffer buffer,
    VkDeviceSize offset,
    VkBuffer countBuffer,
    ...
    uint32_t stride)
{
    RecordCmdDrawIndexedIndirectCount(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                                      maxDrawCount, stride, CMD_DRAWINDEXEDINDIRECTCOUNTKHR);
}

void ValidationStateTracker::PreCallRecordCmdDrawIndexedIndirectCount(
    VkCommandBuffer commandBuffer,
    VkBuffer buffer,
    VkDeviceSize offset,
    VkBuffer countBuffer,
...
{
    RecordCmdDrawIndexedIndirectCount(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                                      maxDrawCount, stride, CMD_DRAWINDEXEDINDIRECTCOUNT);
}

void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksNV(VkCommandBuffer commandBuffer,
                                                             uint32_t taskCount,
                                                             uint32_t firstTask)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksIndirectNV(VkCommandBuffer commandBuffer,
                                                                     VkBuffer buffer,
                                                                     VkDeviceSize offset,
                                                                     uint32_t drawCount,
                                                                     uint32_t stride)
...
void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksIndirectCountNV(
    VkCommandBuffer commandBuffer,
    VkBuffer buffer,
    VkDeviceSize offset,
    VkBuffer countBuffer,
...
void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksIndirectEXT(VkCommandBuffer commandBuffer,
                                                                      VkBuffer buffer,
                                                                      VkDeviceSize offset,
                                                                      uint32_t drawCount,
                                                                      uint32_t stride)
...
void ValidationStateTracker::PreCallRecordCmdDrawMeshTasksIndirectCountEXT(
    VkCommandBuffer commandBuffer,
    VkBuffer buffer,
    VkDeviceSize offset,
    VkBuffer countBuffer,
...
void ValidationStateTracker::PostCallRecordCmdTraceRaysNV(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdTraceRaysKHR(
    VkCommandBuffer commandBuffer,
...
std::shared_ptr<SHADER_MODULE_STATE> ValidationStateTracker::CreateShaderModuleState(
    const VkShaderModuleCreateInfo &create_info,
    uint32_t unique_shader_id,
    VkShaderModule handle) const
{
    spv_target_env spirv_environment =
        PickSpirvEnv(api_version, IsExtEnabled(device_extensions.vk_khr_spirv_1_4));
    if ((create_info.pCode[0] != spv::MagicNumber))
    {
        return std::make_shared<SHADER_MODULE_STATE>();  // not valid SPIR-V
    }
    const auto module_state =
        std::make_shared<SHADER_MODULE_STATE>(create_info, handle, unique_shader_id);
    if (module_state->static_data_.has_group_decoration)
    {
        spvtools::Optimizer optimizer(spirv_environment);
        optimizer.RegisterPass(spvtools::CreateFlattenDecorationPass());
        std::vector<uint32_t> optimized_binary;
        // Run optimizer to flatten decorations only, set skip_validation so as to not re-run
        // validator
        auto result = optimizer.Run(module_state->words_.data(), module_state->words_.size(),
                                    &optimized_binary, spvtools::ValidatorOptions(), true);

        if (result)
        {
            // Easier to just re-create the ShaderModule as StaticData uses itself when building
            // itself up It is really rare this will get here as Group Decorations have been
            // deprecated and before this was added no one ever raised an issue for a bug that would
            // crash the layers that was around for many releases
...
void ValidationStateTracker::PostCallRecordCmdTraceRaysIndirect2KHR(
    VkCommandBuffer commandBuffer,
    VkDeviceAddress indirectDeviceAddress)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::PostCallRecordCreateShaderModule(
    VkDevice device,
    ...
    void *csm_state_data)
{
    ...
    create_shader_module_api_state *csm_state =
        reinterpret_cast<create_shader_module_api_state *>(csm_state_data);

    Add(CreateShaderModuleState(*pCreateInfo, csm_state->unique_shader_id, *pShaderModule));
}

void ValidationStateTracker::PostCallRecordGetSwapchainImagesKHR(VkDevice device,
                                                                 VkSwapchainKHR swapchain,
                                                                 ...
                                                                 VkResult result)
{
    ...
    if (*pSwapchainImageCount > swapchain_state->images.size())
        swapchain_state->images.resize(*pSwapchainImageCount);
    ...
    if (pSwapchainImages)
    {
        for (uint32_t i = 0; i < *pSwapchainImageCount; ++i)
        {
            ...
            auto format_features = GetImageFormatFeatures(
                physical_device, has_format_feature2,
                IsExtEnabled(device_extensions.vk_ext_image_drm_format_modifier), device,
                pSwapchainImages[i], swapchain_state->image_create_info.format,
                swapchain_state->image_create_info.tiling);

            auto image_state =
                CreateImageState(pSwapchainImages[i], swapchain_state->image_create_info.ptr(),
...
void ValidationStateTracker::PostCallRecordCopyAccelerationStructureKHR(
    VkDevice device,
    VkDeferredOperationKHR deferredOperation,
...
void ValidationStateTracker::PostCallRecordCmdCopyAccelerationStructureKHR(
    VkCommandBuffer commandBuffer,
    const VkCopyAccelerationStructureInfoKHR *pInfo)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::RecordCmdSetCullMode(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetCullModeEXT(VkCommandBuffer commandBuffer,
                                                             VkCullModeFlags cullMode)
{
    RecordCmdSetCullMode(commandBuffer, cullMode, CMD_SETCULLMODEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetCullMode(VkCommandBuffer commandBuffer,
                                                          VkCullModeFlags cullMode)
{
    RecordCmdSetCullMode(commandBuffer, cullMode, CMD_SETCULLMODE);
}

void ValidationStateTracker::RecordCmdSetFrontFace(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetFrontFaceEXT(VkCommandBuffer commandBuffer,
                                                              VkFrontFace frontFace)
{
    RecordCmdSetFrontFace(commandBuffer, frontFace, CMD_SETFRONTFACEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetFrontFace(VkCommandBuffer commandBuffer,
                                                           VkFrontFace frontFace)
{
    RecordCmdSetFrontFace(commandBuffer, frontFace, CMD_SETFRONTFACE);
}

void ValidationStateTracker::RecordCmdSetPrimitiveTopology(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetPrimitiveTopologyEXT(
    VkCommandBuffer commandBuffer,
    VkPrimitiveTopology primitiveTopology)
{
...
void ValidationStateTracker::PostCallRecordCmdSetPrimitiveTopology(
    VkCommandBuffer commandBuffer,
    VkPrimitiveTopology primitiveTopology)
{
...
void ValidationStateTracker::RecordCmdSetViewportWithCount(VkCommandBuffer commandBuffer,
                                                           uint32_t viewportCount,
                                                           const VkViewport *pViewports,
                                                           CMD_TYPE cmd_type)
{
    ...
    cb_state->dynamicViewports.resize(
        std::max(size_t(viewportCount), cb_state->dynamicViewports.size()));
    for (size_t i = 0; i < viewportCount; ++i)
    {
...
void ValidationStateTracker::PostCallRecordCmdSetViewportWithCountEXT(VkCommandBuffer commandBuffer,
                                                                      uint32_t viewportCount,
                                                                      const VkViewport *pViewports)
{
    RecordCmdSetViewportWithCount(commandBuffer, viewportCount, pViewports,
                                  CMD_SETVIEWPORTWITHCOUNTEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetViewportWithCount(VkCommandBuffer commandBuffer,
                                                                   uint32_t viewportCount,
                                                                   const VkViewport *pViewports)
{
    RecordCmdSetViewportWithCount(commandBuffer, viewportCount, pViewports,
...
void ValidationStateTracker::RecordCmdSetScissorWithCount(VkCommandBuffer commandBuffer,
                                                          uint32_t scissorCount,
                                                          const VkRect2D *pScissors,
                                                          CMD_TYPE cmd_type)
{
...
void ValidationStateTracker::PostCallRecordCmdSetScissorWithCountEXT(VkCommandBuffer commandBuffer,
                                                                     uint32_t scissorCount,
                                                                     const VkRect2D *pScissors)
{
    RecordCmdSetScissorWithCount(commandBuffer, scissorCount, pScissors,
                                 CMD_SETSCISSORWITHCOUNTEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetScissorWithCount(VkCommandBuffer commandBuffer,
                                                                  uint32_t scissorCount,
                                                                  const VkRect2D *pScissors)
{
...
void ValidationStateTracker::RecordCmdBindVertexBuffers2(VkCommandBuffer commandBuffer,
                                                         uint32_t firstBinding,
                                                         uint32_t bindingCount,
                                                         const VkBuffer *pBuffers,
                                                         const VkDeviceSize *pOffsets,
                                                         ...
                                                         CMD_TYPE cmd_type)
{
    ...
    for (uint32_t i = 0; i < bindingCount; ++i)
    {
        ...
        vertex_buffer_binding = BufferBinding(Get<BUFFER_STATE>(pBuffers[i]), binding_size,
                                              pOffsets[i], binding_stride);
...
void ValidationStateTracker::PostCallRecordCmdBindVertexBuffers2EXT(VkCommandBuffer commandBuffer,
                                                                    uint32_t firstBinding,
                                                                    uint32_t bindingCount,
                                                                    const VkBuffer *pBuffers,
                                                                    const VkDeviceSize *pOffsets,
                                                                    ...
                                                                    const VkDeviceSize *pStrides)
{
    RecordCmdBindVertexBuffers2(commandBuffer, firstBinding, bindingCount, pBuffers, pOffsets,
                                pSizes, pStrides, CMD_BINDVERTEXBUFFERS2EXT);
}

void ValidationStateTracker::PostCallRecordCmdBindVertexBuffers2(VkCommandBuffer commandBuffer,
                                                                 uint32_t firstBinding,
                                                                 uint32_t bindingCount,
                                                                 const VkBuffer *pBuffers,
                                                                 const VkDeviceSize *pOffsets,
...
{
    RecordCmdBindVertexBuffers2(commandBuffer, firstBinding, bindingCount, pBuffers, pOffsets,
                                pSizes, pStrides, CMD_BINDVERTEXBUFFERS2);
}

void ValidationStateTracker::RecordCmdSetDepthTestEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthTestEnableEXT(VkCommandBuffer commandBuffer,
                                                                    VkBool32 depthTestEnable)
{
    RecordCmdSetDepthTestEnable(commandBuffer, depthTestEnable, CMD_SETDEPTHTESTENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthTestEnable(VkCommandBuffer commandBuffer,
                                                                 VkBool32 depthTestEnable)
{
    RecordCmdSetDepthTestEnable(commandBuffer, depthTestEnable, CMD_SETDEPTHTESTENABLE);
}

void ValidationStateTracker::RecordCmdSetDepthWriteEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthWriteEnableEXT(VkCommandBuffer commandBuffer,
                                                                     VkBool32 depthWriteEnable)
{
    RecordCmdSetDepthWriteEnable(commandBuffer, depthWriteEnable, CMD_SETDEPTHWRITEENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthWriteEnable(VkCommandBuffer commandBuffer,
                                                                  VkBool32 depthWriteEnable)
{
    RecordCmdSetDepthWriteEnable(commandBuffer, depthWriteEnable, CMD_SETDEPTHWRITEENABLE);
}

void ValidationStateTracker::RecordCmdSetDepthCompareOp(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthCompareOpEXT(VkCommandBuffer commandBuffer,
                                                                   VkCompareOp depthCompareOp)
{
    RecordCmdSetDepthCompareOp(commandBuffer, depthCompareOp, CMD_SETDEPTHCOMPAREOPEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthCompareOp(VkCommandBuffer commandBuffer,
                                                                VkCompareOp depthCompareOp)
{
    RecordCmdSetDepthCompareOp(commandBuffer, depthCompareOp, CMD_SETDEPTHCOMPAREOP);
}

void ValidationStateTracker::RecordCmdSetDepthBoundsTestEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthBoundsTestEnableEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 depthBoundsTestEnable)
{
    RecordCmdSetDepthBoundsTestEnable(commandBuffer, depthBoundsTestEnable,
                                      CMD_SETDEPTHBOUNDSTESTENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthBoundsTestEnable(
    VkCommandBuffer commandBuffer,
    VkBool32 depthBoundsTestEnable)
{
    RecordCmdSetDepthBoundsTestEnable(commandBuffer, depthBoundsTestEnable,
                                      CMD_SETDEPTHBOUNDSTESTENABLE);
}

void ValidationStateTracker::RecordCmdSetStencilTestEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetStencilTestEnableEXT(VkCommandBuffer commandBuffer,
                                                                      VkBool32 stencilTestEnable)
{
    RecordCmdSetStencilTestEnable(commandBuffer, stencilTestEnable, CMD_SETSTENCILTESTENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetStencilTestEnable(VkCommandBuffer commandBuffer,
                                                                   VkBool32 stencilTestEnable)
{
    RecordCmdSetStencilTestEnable(commandBuffer, stencilTestEnable, CMD_SETSTENCILTESTENABLE);
}

void ValidationStateTracker::RecordCmdSetStencilOp(VkCommandBuffer commandBuffer,
                                                   VkStencilFaceFlags faceMask,
                                                   VkStencilOp failOp,
                                                   VkStencilOp passOp,
...
void ValidationStateTracker::PostCallRecordCmdSetStencilOpEXT(VkCommandBuffer commandBuffer,
                                                              VkStencilFaceFlags faceMask,
                                                              VkStencilOp failOp,
                                                              VkStencilOp passOp,
                                                              VkStencilOp depthFailOp,
                                                              VkCompareOp compareOp)
{
    RecordCmdSetStencilOp(commandBuffer, faceMask, failOp, passOp, depthFailOp, compareOp,
                          CMD_SETSTENCILOPEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetStencilOp(VkCommandBuffer commandBuffer,
                                                           VkStencilFaceFlags faceMask,
                                                           VkStencilOp failOp,
                                                           VkStencilOp passOp,
                                                           VkStencilOp depthFailOp,
...
void ValidationStateTracker::PostCallRecordCmdSetDiscardRectangleEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetSampleLocationsEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetCoarseSampleOrderNV(
    VkCommandBuffer commandBuffer,
...
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOARSESAMPLEORDERNV,
                             CB_DYNAMIC_VIEWPORT_COARSE_SAMPLE_ORDER_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetPatchControlPointsEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetLogicOpEXT(VkCommandBuffer commandBuffer,
                                                            VkLogicOp logicOp)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
...
void ValidationStateTracker::RecordCmdSetRasterizerDiscardEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetRasterizerDiscardEnableEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 rasterizerDiscardEnable)
{
    RecordCmdSetRasterizerDiscardEnable(commandBuffer, rasterizerDiscardEnable,
                                        CMD_SETRASTERIZERDISCARDENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetRasterizerDiscardEnable(
    VkCommandBuffer commandBuffer,
    VkBool32 rasterizerDiscardEnable)
{
    RecordCmdSetRasterizerDiscardEnable(commandBuffer, rasterizerDiscardEnable,
                                        CMD_SETRASTERIZERDISCARDENABLE);
}

void ValidationStateTracker::RecordCmdSetDepthBiasEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthBiasEnableEXT(VkCommandBuffer commandBuffer,
                                                                    VkBool32 depthBiasEnable)
{
    RecordCmdSetDepthBiasEnable(commandBuffer, depthBiasEnable, CMD_SETDEPTHBIASENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthBiasEnable(VkCommandBuffer commandBuffer,
                                                                 VkBool32 depthBiasEnable)
{
    RecordCmdSetDepthBiasEnable(commandBuffer, depthBiasEnable, CMD_SETDEPTHBIASENABLE);
}

void ValidationStateTracker::RecordCmdSetPrimitiveRestartEnable(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetPrimitiveRestartEnableEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 primitiveRestartEnable)
{
    RecordCmdSetPrimitiveRestartEnable(commandBuffer, primitiveRestartEnable,
                                       CMD_SETPRIMITIVERESTARTENABLEEXT);
}

void ValidationStateTracker::PostCallRecordCmdSetPrimitiveRestartEnable(
    VkCommandBuffer commandBuffer,
    VkBool32 primitiveRestartEnable)
{
    RecordCmdSetPrimitiveRestartEnable(commandBuffer, primitiveRestartEnable,
                                       CMD_SETPRIMITIVERESTARTENABLE);
}
    ...
    const VkVertexInputAttributeDescription2EXT *pVertexAttributeDescriptions)
{
    ...
    if (pipeline_state)
    {
        ...
        if (dynamic_state)
        {
            ...
            {
                if (dynamic_state->pDynamicStates[i] ==
...
void ValidationStateTracker::PostCallRecordCmdSetColorWriteEnableEXT(
    VkCommandBuffer commandBuffer,
    uint32_t attachmentCount,
    const VkBool32 *pColorWriteEnables)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordColorWriteEnableStateCmd(
        CMD_SETCOLORWRITEENABLEEXT, CB_DYNAMIC_COLOR_WRITE_ENABLE_EXT_SET, attachmentCount);
}

#ifdef VK_USE_PLATFORM_WIN32_KHR
void ValidationStateTracker::PostCallRecordAcquireFullScreenExclusiveModeEXT(
    VkDevice device,
    VkSwapchainKHR swapchain,
    VkResult result)
{
...
void ValidationStateTracker::PostCallRecordReleaseFullScreenExclusiveModeEXT(
    VkDevice device,
    VkSwapchainKHR swapchain,
    VkResult result)
{
...
void ValidationStateTracker::PostCallRecordCmdSetTessellationDomainOriginEXT(
    VkCommandBuffer commandBuffer,
    VkTessellationDomainOrigin domainOrigin)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETTESSELLATIONDOMAINORIGINEXT,
                             CB_DYNAMIC_TESSELLATION_DOMAIN_ORIGIN_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthClampEnableEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetPolygonModeEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetRasterizationSamplesEXT(
    VkCommandBuffer commandBuffer,
    VkSampleCountFlagBits rasterizationSamples)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETRASTERIZATIONSAMPLESEXT,
                             CB_DYNAMIC_RASTERIZATION_SAMPLES_EXT_SET);
    cb_state->dynamic_state_value.rasterization_samples = rasterizationSamples;
}

void ValidationStateTracker::PostCallRecordCmdSetSampleMaskEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetAlphaToCoverageEnableEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 alphaToCoverageEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETALPHATOCOVERAGEENABLEEXT,
                             CB_DYNAMIC_ALPHA_TO_COVERAGE_ENABLE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetAlphaToOneEnableEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetLogicOpEnableEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetColorBlendEnableEXT(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetColorBlendEquationEXT(
    VkCommandBuffer commandBuffer,
    uint32_t firstAttachment,
    uint32_t attachmentCount,
...
void ValidationStateTracker::PostCallRecordCmdSetColorWriteMaskEXT(
    VkCommandBuffer commandBuffer,
    uint32_t firstAttachment,
    uint32_t attachmentCount,
...
void ValidationStateTracker::PostCallRecordCmdSetRasterizationStreamEXT(
    VkCommandBuffer commandBuffer,
    uint32_t rasterizationStream)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETRASTERIZATIONSTREAMEXT,
                             CB_DYNAMIC_RASTERIZATION_STREAM_EXT_SET);
}
...
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCONSERVATIVERASTERIZATIONMODEEXT,
                             CB_DYNAMIC_CONSERVATIVE_RASTERIZATION_MODE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetExtraPrimitiveOverestimationSizeEXT(
    VkCommandBuffer commandBuffer,
    float extraPrimitiveOverestimationSize)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETEXTRAPRIMITIVEOVERESTIMATIONSIZEEXT,
                             CB_DYNAMIC_EXTRA_PRIMITIVE_OVERESTIMATION_SIZE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetDepthClipEnableEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetSampleLocationsEnableEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 sampleLocationsEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETSAMPLELOCATIONSENABLEEXT,
                             CB_DYNAMIC_SAMPLE_LOCATIONS_ENABLE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetColorBlendAdvancedEXT(
    VkCommandBuffer commandBuffer,
    uint32_t firstAttachment,
    uint32_t attachmentCount,
...
void ValidationStateTracker::PostCallRecordCmdSetProvokingVertexModeEXT(
    VkCommandBuffer commandBuffer,
    VkProvokingVertexModeEXT provokingVertexMode)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETPROVOKINGVERTEXMODEEXT,
                             CB_DYNAMIC_PROVOKING_VERTEX_MODE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetLineRasterizationModeEXT(
    VkCommandBuffer commandBuffer,
    VkLineRasterizationModeEXT lineRasterizationMode)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETLINERASTERIZATIONMODEEXT,
                             CB_DYNAMIC_LINE_RASTERIZATION_MODE_EXT_SET);
    cb_state->dynamic_state_value.line_rasterization_mode = lineRasterizationMode;
}

void ValidationStateTracker::PostCallRecordCmdSetLineStippleEnableEXT(VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdSetDepthClipNegativeOneToOneEXT(
    VkCommandBuffer commandBuffer,
    VkBool32 negativeOneToOne)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETDEPTHCLIPNEGATIVEONETOONEEXT,
                             CB_DYNAMIC_DEPTH_CLIP_NEGATIVE_ONE_TO_ONE_EXT_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetViewportWScalingEnableNV(
    VkCommandBuffer commandBuffer,
    VkBool32 viewportWScalingEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETVIEWPORTWSCALINGENABLENV,
                             CB_DYNAMIC_VIEWPORT_W_SCALING_ENABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetViewportSwizzleNV(
    VkCommandBuffer commandBuffer,
    uint32_t firstViewport,
    uint32_t viewportCount,
...
void ValidationStateTracker::PostCallRecordCmdSetCoverageToColorEnableNV(
    VkCommandBuffer commandBuffer,
    VkBool32 coverageToColorEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGETOCOLORENABLENV,
                             CB_DYNAMIC_COVERAGE_TO_COLOR_ENABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetCoverageToColorLocationNV(
    VkCommandBuffer commandBuffer,
    uint32_t coverageToColorLocation)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGETOCOLORLOCATIONNV,
                             CB_DYNAMIC_COVERAGE_TO_COLOR_LOCATION_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetCoverageModulationModeNV(
    VkCommandBuffer commandBuffer,
    VkCoverageModulationModeNV coverageModulationMode)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGEMODULATIONMODENV,
                             CB_DYNAMIC_COVERAGE_MODULATION_MODE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetCoverageModulationTableEnableNV(
    VkCommandBuffer commandBuffer,
    VkBool32 coverageModulationTableEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGEMODULATIONTABLEENABLENV,
                             CB_DYNAMIC_COVERAGE_MODULATION_TABLE_ENABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetCoverageModulationTableNV(
    VkCommandBuffer commandBuffer,
    uint32_t coverageModulationTableCount,
    const float *pCoverageModulationTable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGEMODULATIONTABLENV,
                             CB_DYNAMIC_COVERAGE_MODULATION_TABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetShadingRateImageEnableNV(
    VkCommandBuffer commandBuffer,
    VkBool32 shadingRateImageEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETSHADINGRATEIMAGEENABLENV,
                             CB_DYNAMIC_SHADING_RATE_IMAGE_ENABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetRepresentativeFragmentTestEnableNV(
    VkCommandBuffer commandBuffer,
    VkBool32 representativeFragmentTestEnable)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETREPRESENTATIVEFRAGMENTTESTENABLENV,
                             CB_DYNAMIC_REPRESENTATIVE_FRAGMENT_TEST_ENABLE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdSetCoverageReductionModeNV(
    VkCommandBuffer commandBuffer,
    VkCoverageReductionModeNV coverageReductionMode)
{
    auto cb_state = GetWrite<CMD_BUFFER_STATE>(commandBuffer);
    cb_state->RecordStateCmd(CMD_SETCOVERAGEREDUCTIONMODENV,
                             CB_DYNAMIC_COVERAGE_REDUCTION_MODE_NV_SET);
}

void ValidationStateTracker::PostCallRecordCmdControlVideoCodingKHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::PostCallRecordCmdDecodeVideoKHR(
    VkCommandBuffer commandBuffer,
...
void ValidationStateTracker::RecordGetBufferDeviceAddress(const VkBufferDeviceAddressInfo *pInfo,
    ...
    if (buffer_state && address != 0)
    {
        ...
        buffer_address_map_.split_and_merge_insert(
            {address_range, {buffer_state}}, [](auto &current_buffer_list, const auto &new_buffer) {
                assert(!current_buffer_list.empty());
                const auto buffer_found_it = std::find(current_buffer_list.begin(),
                                                       current_buffer_list.end(), new_buffer[0]);
                if (buffer_found_it == current_buffer_list.end())
                {
                    current_buffer_list.emplace_back(new_buffer[0]);
                }
...
void ValidationStateTracker::PostCallRecordGetShaderModuleIdentifierEXT(
    VkDevice,
...
void ValidationStateTracker::PostCallRecordGetShaderModuleCreateInfoIdentifierEXT(
    VkDevice,
    const VkShaderModuleCreateInfo *pCreateInfo,
    VkShaderModuleIdentifierEXT *pIdentifier)
{
    WriteLockGuard guard(shader_identifier_map_lock_);
    shader_identifier_map_.emplace(*pIdentifier,
                                   CreateShaderModuleState(*pCreateInfo, 0, VK_NULL_HANDLE));
}

void ValidationStateTracker::PreCallRecordCmdBindShadersEXT(VkCommandBuffer commandBuffer,
                                                            uint32_t stageCount,
...
void ValidationStateTracker::PostCallRecordGetBufferDeviceAddress(
    VkDevice device,
...
void ValidationStateTracker::PostCallRecordGetBufferDeviceAddressKHR(
    VkDevice device,
    const VkBufferDeviceAddressInfo *pInfo,
    VkDeviceAddress address)
{
...
void ValidationStateTracker::PostCallRecordGetBufferDeviceAddressEXT(
    VkDevice device,
    const VkBufferDeviceAddressInfo *pInfo,
    VkDeviceAddress address)
{
...
std::shared_ptr<SWAPCHAIN_NODE> ValidationStateTracker::CreateSwapchainState(
...
std::shared_ptr<CMD_BUFFER_STATE> ValidationStateTracker::CreateCmdBufferState(
    VkCommandBuffer cb,
...
{
    return std::make_shared<DEVICE_MEMORY_STATE>(mem, p_alloc_info, fake_address, memory_type,
                                                 memory_heap, std::move(dedicated_binding),
                                                 physical_device_count);
}
