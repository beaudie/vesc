...
def SetupBuild(build_dir, source_dir, options, pkg_config_paths=[]):
    ...
    extra_env = {
        ...
        'VPYTHON_DEFAULT_SPEC': os.path.join(ANGLE_DIR, '.vpython3'),
    }
...
def GenerateGni(args):
    ...
    libdrm_outputs_filter = lambda target: True
...
def LazySetup(args, build_dir):
    in_libdrm_stamp = os.path.join(SCRIPT_DIR, LIBDRM_STAMP)
    libdrm_dir = os.path.join(build_dir, 'libdrm')
    out_libdrm_stamp = os.path.join(libdrm_dir, LIBDRM_STAMP)
    if not args.wipe and not SameStamps(in_libdrm_stamp, out_libdrm_stamp):
        SetupBuild(libdrm_dir, LIBDRM_SOURCE_DIR, LIBDRM_OPTIONS)
        Compile(args, libdrm_dir)
        logging.info('Completed libdrm build.')
        shutil.copyfile(in_libdrm_stamp, out_libdrm_stamp)

    in_mesa_stamp = os.path.join(SCRIPT_DIR, MESA_STAMP)
    out_mesa_stamp = os.path.join(build_dir, 'mesa', MESA_STAMP)
    if not args.wipe and SameStamps(in_mesa_stamp, out_mesa_stamp):
        logging.info('Mesa setup up-to-date.')
        sys.exit(EXIT_SUCCESS)

    SetupBuild(
        os.path.join(build_dir, 'mesa'), MESA_SOURCE_DIR, MESA_OPTIONS,
        [os.path.join(libdrm_dir, 'meson-uninstalled')])
    shutil.copyfile(in_mesa_stamp, out_mesa_stamp)
    logging.info('Finished setup and updated %s.' % MESA_STAMP)
