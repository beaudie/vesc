...
class openCLCaseList
{
  public:
    ...
    void initialize(std::string expectationsPath, std::string testPath, angle::TestSuite *instance);
...
{
    std::string expectationsPath = "expectations.txt";
    std::string testPath         = "mustpass.txt";
    angle::TestSuite *instance   = angle::TestSuite::GetInstance();
    static openCLCaseList sCaseList;
    sCaseList.initialize(expectationsPath, testPath, instance);
    return sCaseList;
}

void openCLCaseList::initialize(std::string expectationsPath,
                                std::string testPath,
                                angle::TestSuite *instance)
{
    mInitialized = true;
    if (!instance->loadTestExpectationsFromFileWithConfig(angle::GPUTestConfig::kAPIVulkan,
                                                          expectationsPath))
    {
        exit(EXIT_FAILURE);
    }
    std::ifstream caseListStream(testPath);
...
void openCLTest::TestBody()
{
    ...
    for (int index = 0; index < *CtsSetup::getArgC(); index++)
    {
        ...
        {
            strcpy(command, "./");
            strcat(command, testSuite.c_str());
        }
        else
        {
            strcat(command, " ");
