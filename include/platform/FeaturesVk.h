...
struct FeaturesVk
{
    ...
    // On some android devices, the memory barrier between the compute shader that converts vertex attributes and
    // the vertex shader that reads from it is ineffective.  Only known workaround is to perform a flush after the
    // conversion.
