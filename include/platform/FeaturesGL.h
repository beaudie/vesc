...
struct FeaturesGL : FeatureSetBase
{
    ...
    // glClearColor does not always work on Intel 6xxx Mac drivers when the clear color is all zeros
    // or all ones.
    Feature clearToZeroOrOneBroken = {
        "clear_to_zero_or_one_broken", FeatureCategory::OpenGLWorkarounds,
        "Clears when the clear color is all zeros or all ones do not work.", &members,
