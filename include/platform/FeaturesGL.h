...
struct FeaturesGL : FeatureSetBase
{
    ...
    // The driver supports GL_FRAMEBUFFER_SRGB so make sure it's set before doing any draw calls
    // with sRGB framebuffers.
    Feature setFramebufferSRGB = {"set_framebuffer_srgb", FeatureCategory::OpenGLWorkarounds,
                                  "Set GL_FRAMEBUFFER_SRGB to ensure sRGB rendering is done.",
