...
def make_camel_case(display_name):
    return re.sub('_(.)', lambda m: m.group(1).upper(), display_name)
...
def main():
    ...
    for src_file, (category_prefix, header_file) in feature_files.items():
        ...
        for feature_json in features_json:
            display_name = feature_json['name']
            var_name = make_camel_case(display_name)
