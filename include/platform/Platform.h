...
// Wall clock time in seconds since the epoch.
// TODO(jmadill): investigate using an ANGLE internal time library
inline double ANGLE_currentTime()
{
    return 0.0;
}
...
// This function is expected to return at least millisecond-precision values. For this reason,
// it is recommended that the fixed point be no further in the past than the epoch.
inline double ANGLE_monotonicallyIncreasingTime()
{
    return 0.0;
}
...
// Log an error message within the platform implementation.
inline void ANGLE_logError(const char *errorMessage)
{
}

// Log a warning message within the platform implementation.
inline void ANGLE_logWarning(const char *warningMessage)
{
}

// Log an info message within the platform implementation.
inline void ANGLE_logInfo(const char *infoMessage)
{
}
...
// the unsigned char is non-zero, tracing is enabled. If tracing is enabled,
// addTraceEvent is expected to be called by the trace event macros.
inline const unsigned char *ANGLE_getTraceCategoryEnabledFlag(const char *categoryName)
{
    return nullptr;
}
...
//   - MANGLE_ID (0x4): specify this flag if the id parameter is the value
//     of a pointer.
inline angle::TraceEventHandle ANGLE_addTraceEvent(char phase,
                                                   const unsigned char *categoryEnabledFlag,
                                                   const char *name,
                                                   unsigned long long id,
                                                   double timestamp,
...
// Set the duration field of a COMPLETE trace event.
inline void ANGLE_updateTraceEventDuration(const unsigned char *categoryEnabledFlag,
                                           const char *name,
...
// would do.
inline void ANGLE_histogramCustomCounts(const char *name,
                                        int sample,
                                        int min,
                                        int max,
                                        int bucketCount)
...
// value.
inline void ANGLE_histogramEnumeration(const char *name, int sample, int boundaryValue)
{
}
// Unlike enumeration histograms, sparse histograms only allocate memory for non-empty buckets.
inline void ANGLE_histogramSparse(const char *name, int sample)
{
}
// Boolean histograms track two-state variables.
inline void ANGLE_histogramBoolean(const char *name, bool sample)
{
}

// Allows us to programatically override ANGLE's default workarounds for testing purposes.
inline void ANGLE_overrideWorkaroundsD3D(angle::WorkaroundsD3D *workaroundsD3D)
{
}

// Default methods are set below so an implementer doesn't need to override every method.

#define ANGLE_PLATFORM_METHOD(Name) decltype(&ANGLE_##Name) Name = ANGLE_##Name;

struct PlatformMethods
{
    ANGLE_PLATFORM_METHOD(currentTime);
    ANGLE_PLATFORM_METHOD(monotonicallyIncreasingTime);
    ANGLE_PLATFORM_METHOD(logError);
    ANGLE_PLATFORM_METHOD(logWarning);
    ANGLE_PLATFORM_METHOD(logInfo);
    ANGLE_PLATFORM_METHOD(getTraceCategoryEnabledFlag);
    ANGLE_PLATFORM_METHOD(addTraceEvent);
    ANGLE_PLATFORM_METHOD(updateTraceEventDuration);
    ANGLE_PLATFORM_METHOD(histogramCustomCounts);
    ANGLE_PLATFORM_METHOD(histogramEnumeration);
    ANGLE_PLATFORM_METHOD(histogramSparse);
    ANGLE_PLATFORM_METHOD(histogramBoolean);
    ANGLE_PLATFORM_METHOD(overrideWorkaroundsD3D);
};

#undef ANGLE_PLATFORM_METHOD
