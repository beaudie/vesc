...
struct ShPixelLocalStorageOptions
{
    ...
    // AMD when the render pass doesn't have attachments. To work around this we attach an unused
    // texture to the framebuffer during PLS-only render passes.
    bool renderPassNeedsAMDColorAttachmentWorkaround = false;
};
