...
typedef enum {
  ...
  SH_RESOURCE_STRING_LENGTH         = 0x600A
} ShShaderInfo;
...
//         It is assumed that outStr has enough memory to accormodate
//         the resources tring. The size of the buffer required to store the
//         resources string can be obtained by calling ShGetInfo with 
//         SH_RESOURCE_STRING_LENGTH
COMPILER_EXPORT void ShGetBuiltInResourcesString(const ShHandle handle, char *outStr);
