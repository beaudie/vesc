...
struct CompiledShaderState
{
    ...
    // Geometry Shader
    // Optional parameters are only initialized if the corresponding 'hasValid' is true.
    // @todo discuss this - we assume that PrimitiveMode is a uint8_t. Assert this somewhere?
    ...
    uint8_t geometryShaderMaxVertices;
