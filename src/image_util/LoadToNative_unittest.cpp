...
// Tests the ubyte RGB to RGBA loading function when there is only one pixel.
TEST(LoadToNative3To4, LoadUbyteRGBToRGBADataOnePixel)
{
    ImageLoadContext context;
    bool isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 1, 1, 1, 0, 0, 0);
    EXPECT_EQ(isSuccess, true);
}
...
// Tests the ubyte RGB to RGBA loading function when the input row alignment is 4.
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithAlignment4)
{
    ImageLoadContext context;
    bool isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 7, 7, 1, 0, 0, 4);
    EXPECT_EQ(isSuccess, true);
}
