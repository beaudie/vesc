...
size_t RoundUpWithAlignment(size_t size, size_t alignment)
{
    if (alignment == 0)
    {
        return size;
    ...
    // Make sure that alignment is a power of two before rounding up the size.
    ASSERT((alignment & (alignment - 1)) == 0);
    return ((size - 1) | (alignment - 1)) + 1;
}

void CheckSuccessForCase(bool success, size_t inputCase)
{
    if (!success)
    {
        ERR() << "Failure for the case: " << inputCase;
    }
                            ...
                            size_t inputRowAlignment)
{
    ...
    size_t inputRowPitch    = RoundUpWithAlignment(width * inputPixelBytes, inputRowAlignment);
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBADataAlignedWidth)
{
    ...
    for (auto &width : alignedTestWidths)
    {
        ...
        CheckSuccessForCase(isSuccess, width);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithAlignedWidthAndInputAddressOffset)
{
    ...
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 8, 8, 1, inputOffset, 0, 0);
        CheckSuccessForCase(isSuccess, inputOffset);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithUnalignedWidthAndInputAddressOffset)
{
    ...
    for (auto &inputOffset : inputOffsetList)
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 7, 7, 1, inputOffset, 0, 0);
        CheckSuccessForCase(isSuccess, inputOffset);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithAlignedWidthAndOutputAddressOffset)
{
    ...
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 8, 8, 1, 0, outputOffset, 0);
        CheckSuccessForCase(isSuccess, outputOffset);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithUnalignedWidthAndOutputAddressOffset)
{
    ...
    for (auto &outputOffset : outputOffsetList)
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 7, 7, 1, 0, outputOffset, 0);
        CheckSuccessForCase(isSuccess, outputOffset);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithAlignedWidthAndAlignment4)
{
    ...
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 4, 4, 1, 0, 0, alignment);
        CheckSuccessForCase(isSuccess, alignment);
    }
}
...
TEST(LoadToNative3To4, LoadUbyteRGBToRGBAWithUnalignedWidthAndAlignment4)
{
    ...
    for (auto &alignment : inputRowAlignmentList)
    {
        isSuccess = TestLoadUbyteRGBToRGBA<0xFF>(context, 5, 5, 1, 0, 0, alignment);
        CheckSuccessForCase(isSuccess, alignment);
    }
}
