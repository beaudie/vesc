...
class AstcDecompressorImpl : public AstcDecompressor
{
  public:
    AstcDecompressorImpl()
        : AstcDecompressor(),
          mContextCache(std::make_unique<AstcDecompressorContextCache>()),
          mSingleThreadPool(WorkerThreadPool::Create(1)),
          mMultiThreadPool(WorkerThreadPool::Create(kNumThreads))
    {}
    ...
    int32_t decompress(const uint32_t imgWidth,
    ...
    {
        std::array<std::shared_ptr<DecompressTask>, kNumThreads> tasks;
        std::array<std::shared_ptr<WaitableEvent>, kNumThreads> waitEvents;
        ...
        std::shared_ptr<WorkerThreadPool> threadPool =
            imgHeight <= 32 && imgWidth <= 32 ? mSingleThreadPool : mMultiThreadPool;

        for (uint32_t i = 0; i < kNumThreads; ++i)
        {
            tasks[i] = std::make_shared<DecompressTask>(context, i, input, inputLength, &image);
            waitEvents[i] = WorkerThreadPool::PostWorkerTask(threadPool, tasks[i]);
        }
