...
class WorkerThread {
   public:
    explicit WorkerThread(uint32_t index) : mIndex(index), mThread(&WorkerThread::main, this) {}
    ...
    std::future<astcenc_error> decompress(astcenc_context* context, const uint8_t* data,
                                          size_t dataLength, astcenc_image* image) {
        std::lock_guard lock(mWorkerMutex);
        mTask = std::packaged_task<astcenc_error()>{
            [context, data, dataLength, image, index = this->mIndex] {
                return astcenc_decompress_image(context, data, dataLength, image, &kSwizzle, index);
            }};
        mWorkerCondition.notify_one();
   ...
   private:
    ...
    const uint32_t mIndex;                          // Thread index (between 0 and kNumThreads - 1)
...
class AstcCpuDecompressorImpl : public AstcCpuDecompressor {
   public:
    ...
    int32_t decompress(const uint32_t imgWidth, const uint32_t imgHeight, const uint32_t blockWidth,
        ...
        std::vector<std::future<astcenc_error>> futures;
        futures.reserve(mWorkerThreads.size());
        for (auto& worker : mWorkerThreads) {
            futures.push_back(worker.decompress(context, astcData, astcDataLength, &image));
        }
   ...
   private:
    ...
    // Note: Using deque instead of vector here because WorkerThread is non-movable / non-copyable.
    std::deque<WorkerThread> mWorkerThreads;
};

}  // namespace

AstcCpuDecompressor& AstcCpuDecompressor::get() {
    static auto* instance = new AstcCpuDecompressorImpl();
    return *instance;
}
