...
{
    fprintf(stderr, "width=%d height=%d depth=%d inputRowPitch=%d inputDepthPitch=%d\n", (int)width,
            (int)height, (int)depth, (int)inputRowPitch, (int)inputDepthPitch);
            ...
            for (size_t x = 0; x < width; x++)
            {
                ...
                if ((false))
                    dstRow[x] = decodeColor(palette + indexIntoPalette * colorBytes, redBlueBits,
                                            greenBits, alphaBits);
                dstRow[x] = R8G8B8A8{255, 0, 255, 255};
            }
        }
    }
}
