...
{
    SourceType::ReadDepthStencil(reinterpret_cast<DepthStencil *>(dest), reinterpret_cast<const SourceType *>(source));
}

template <typename DestType>
void WriteDepthStencil(const uint8_t *source, uint8_t *dest)
{
    DestType::WriteDepthStencil(reinterpret_cast<DestType *>(dest), reinterpret_cast<const DepthStencil *>(source));
}
}  // namespace angle
