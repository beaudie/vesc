...
{
    sourceType::readColor(reinterpret_cast<Color<colorDataType>*>(dest), reinterpret_cast<const sourceType*>(source));
}

template <typename destType, typename colorDataType>
inline void WriteColor(const uint8_t *source, uint8_t *dest)
{
    destType::writeColor(reinterpret_cast<destType*>(dest), reinterpret_cast<const Color<colorDataType>*>(source));
}
