...
{
    dst->R = gl::linearToSRGB((gl::sRGBToLinear(src1->R) >> 1) + (gl::sRGBToLinear(src2->R) >> 1));
    dst->G = gl::linearToSRGB((gl::sRGBToLinear(src1->G) >> 1) + (gl::sRGBToLinear(src2->G) >> 1));
    dst->B = gl::linearToSRGB((gl::sRGBToLinear(src1->B) >> 1) + (gl::sRGBToLinear(src2->B) >> 1));
    dst->A = (src1->A >> 1) + (src2->A >> 1);
}
