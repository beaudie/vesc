    ...
    for (size_t z = 0; z < depth; z++)
    {
        for (size_t y = 0; y < height; y++)
        {
            const R8G8B8A8 *srcRow =
                priv::OffsetDataPointer<R8G8B8A8>(input, y, z, inputRowPitch, inputDepthPitch);
            uint8_t *dstRow =
                priv::OffsetDataPointer<uint8_t>(texels, y, z, outputRowPitch, outputDepthPitch);
            ...
            for (size_t x = 0; x < width; x++)
            {
                ...
                switch (indexBits)
                {
                    case 4:
                        ...
                        // bits.
                        if (x % 2 == 0)
                            dstRow[x] = static_cast<uint8_t>(paletteIndex) << 4;
                        else
