...
class TestBatch():
    ...
    def Run(self, test_exe_path, trace_folder_path, child_processes_manager):
        ...
        if returncode == -1:
            self.results.append(
                GroupedResult(GroupedResult.Skipped, "Capture run has crashed", output,
                              self.tests))
            return False
        elif returncode == -2:
            self.results.append(
    ...
    def BuildReplay(self, gn_path, autoninja_path, build_dir, trace_dir, replay_exec,
        ...
        if returncode != 0:
            self.results.append(
                GroupedResult(GroupedResult.Skipped, "Build replay has failed at gn generation",
                              output, tests))
            return False
        ...
        if returncode != 0:
            self.results.append(
                GroupedResult(GroupedResult.Skipped, "Build replay has failed at compilation",
                              output, tests))
            return False
        return True
