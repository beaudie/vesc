...
class TestBatchResult():
    ...
    def __init__(self, group_results):
        ...
        for group_result in group_results:
            if group_result.resultcode == GroupResult.Passed:
                for test in group_result.tests:
                    self.passes.append(test.full_test_name)
            elif group_result.resultcode == GroupResult.Failed:
                for test in group_result.tests:
                    self.fails.append(test.full_test_name)
            elif group_result.resultcode == GroupResult.Skipped:
                for test in group_result.tests:
                    self.skips.append(test.full_test_name)
            elif group_result.resultcode == GroupResult.Timeout:
                for test in group_result.tests:
                    self.timeouts.append(test.full_test_name)
        self.repr_str = ""
        self.GenerateRepresentationString(group_results)

    def __str__(self):
        return self.repr_str

    def GenerateRepresentationString(self, group_results):
        self.repr_str += "TestBatch\n"
        for group_result in group_results:
            self.repr_str += group_result.resultcode + ": " + group_result.message + "\n"
            for test in group_result.tests:
                self.repr_str += "\t" + test.full_test_name + "\n"
            self.repr_str += "\n"
            self.repr_str += group_result.output
        self.repr_str += "-" * 30 + "\n"
...
class TestBatch():
    ...
    def Run(self, test_exe_path, trace_folder_path, child_processes_manager):
        ...
        if returncode == -1:
            self.results.append(
                GroupResult(GroupResult.Failed, "Capture run has crashed", output, self.tests))
            return False
        elif returncode == -2:
            self.results.append(
                GroupResult(GroupResult.Timeout, "Capture run has timeouted", "", self.tests))
            return False
        return True
    ...
    def RemoveTestsThatDoNotProduceAppropriateTraceFiles(self, trace_folder_path):
        ...
        if len(skipped_tests) > 0:
            self.results.append(
                GroupResult(GroupResult.Skipped,
    ...
    def BuildReplay(self, gn_path, autoninja_path, build_dir, trace_dir, replay_exec,
        ...
        if not os.path.isfile(os.path.join(build_dir, "args.gn")):
            ...
            if returncode != 0:
                self.results.append(
                    GroupResult(GroupResult.Failed, "Build replay has failed at gn generation",
                                output, tests))
                return False
        ...
        if returncode != 0:
            self.results.append(
                GroupResult(GroupResult.Failed, "Build replay has failed at compilation", output,
                            tests))
            return False
        return True
    ...
    def RunReplay(self, replay_exe_path, child_processes_manager, tests):
        ...
        if returncode == -1:
            self.results.append(
                GroupResult(GroupResult.Failed, "Replay run has crashed", output, tests))
            return
        elif returncode == -2:
            self.results.append(
                GroupResult(GroupResult.Timeout, "Replay run has timeout", "", tests))
            return
        output_lines = output.splitlines()
        passes = []
        fails = []
        ...
        for output_line in output_lines:
            ...
            if len(words) == 3 and words[0] == RESULT_TAG:
                if int(words[2]) == 0:
                    passes.append(self.FindTestByName(words[1]))
                else:
                    fails.append(self.FindTestByName(words[1]))
                count += 1
        if len(passes) > 0:
            self.results.append(GroupResult(GroupResult.Passed, "", "", passes))
        if len(fails) > 0:
            self.results.append(GroupResult(GroupResult.Failed, "", "", fails))

    def FindTestByName(self, name):
        for test in self.tests:
            if test.GetLabel() == name:
                return test
        return None
