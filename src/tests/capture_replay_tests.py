...
class ChildProcessesManager():
    ...
    def RunSubprocess(self, command, env=None, pipe_stdout=True, timeout=None):
        sub_process = SubProcess(command, env, pipe_stdout)
        debug('Creating subprocess: %s with pid %d' % (' '.join(command), sub_process.Pid()))
        self.subprocesses[sub_process.Pid()] = sub_process
        try:
            returncode, output = self.subprocesses[sub_process.Pid()].Join(timeout)
            self.RemoveSubprocess(sub_process.Pid())
            if returncode != 0:
                return -1, output
