...
class TestBatch():
    ...
    def BuildReplay(self, gn_path, autoninja_path, build_dir, trace_dir, trace_folder_path,
        ...
        gn_command = CreateGNGenCommand(gn_path, capture_dir, gn_args)
        ...
        if returncode != 0:
            self.results.append(
                GroupedResult(GroupedResult.CompileFailed, "Build replay failed at gn generation",
                              output, tests))
            return False
        autoninja_command = CreateAutoninjaCommand(autoninja_path, build_dir, REPLAY_BINARY)
...
def RunTests(args, worker_id, job_queue, gn_path, autoninja_path, trace_dir, result_list,
    ...
    replay_exec_path = os.path.join(args.out_dir, 'Replay%d' % worker_id, REPLAY_BINARY)
