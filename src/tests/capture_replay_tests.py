    ...
    def ReadOneExpectation(self, line, platform):
        ...
        if platform in platforms:
            if result_stripped == 'DISABLED_FOR_CAPTURE':
                ...
                self.non_pass_re[test_name] = re.compile('^' + test_name.replace('*', '.*') + '$')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--ignore-exclude-from-run',
        action='store_true',
        help='Also run tests that are disabled in the expectations by EXCLUDE_FROM_RUN')
