...
def ProcessGetTestNamesAndParamsCommandOutput(output):
    ...
    test_expectations_filename = "test_expectations.txt"
    skipped_test_patterns = [
        line.rstrip()
        for line in open(os.path.join(REPLAY_SAMPLE_FOLDER, test_expectations_filename), "r")
    ]
...
def RunTests(worker_id, job_queue, gn_path, autoninja_path, capture_build_dir, replay_build_dir,
    ...
    while not job_queue.empty():
        try:
            ...
            message_queue.put(str(test_batch))
            if not success:
