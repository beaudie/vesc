...
def GetTestsListForFilter(args, test_path, filter):
    cmd = GetRunCommand(test_path) + ["--list-tests", "--gtest_filter=%s" % filter]
...
def GetRunCommand(args, command):
    if args.xvfb:
        cmd = ['vpython', 'testing/xvfb.py', command]
    else:
        cmd = [command]
...
class TestBatch():
    ...
    def RunWithCapture(self, args, child_processes_manager):
        ...
        cmd = GetRunCommand(test_exe_path)
    ...
    def RunReplay(self, args, replay_build_dir, replay_exe_path, child_processes_manager, tests):
        ...
        returncode, output = child_processes_manager.RunSubprocess(
            GetRunCommand(replay_exe_path), env, timeout=SUBPROCESS_TIMEOUT)
        if returncode == -1:
            cmd = replay_exe_path
            self.results.append(
