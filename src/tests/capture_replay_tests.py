        ...
        try:
            if not gn_completed_list[gn_completed_list_index]:
                RunGnGen(build_dir, [("use_goma", self.use_goma),
                                     ...
                                     ("angle_trace_directory", '\\"' + trace_dir + '\\"')])
...
def RunTest(test, capture_build_dir, replay_build_dir, test_exec, replay_exec, trace_dir,
    ...
    if run_output[0] != 0 or not CanRunReplay(trace_folder_path):
        result_queue.put ((test.full_test_name, "Skipped", "Skipped: " + test.full_test_name + ". Skipping replay since capture" + \
    ...
    build_output = test.BuildReplay(replay_build_dir, trace_dir, replay_exec, gn_completed_list,
                                    gn_completed_list_index)
    if build_output[0] != 0:
        result_queue.put((test.full_test_name, "Skipped", "Skipped: " + test.full_test_name + ". Skipping replay since failing to" + \
            " build replay"))
        return
...
def main(capture_build_dir, replay_build_dir, verbose, use_goma, gtest_filter, test_exec):
    ...
    while not all_tests.empty():
        ...
        proc = multiprocessing.Process(target=RunTest, args=(test, capture_build_dir, replay_build_dir + str(first_available_cpu), \
            test_exec, replay_exec, trace_folder + str(first_available_cpu), gn_completed_list, first_available_cpu, result_queue))
