...
class TestBatch():
    ...
    def __init__(self, use_goma, batch_count, delete_generated, goma_dir):
    ...
    def CreateTestsCompositeFiles(self, trace_folder_path, composite_file_id, tests):
        ...
        h_file.write("std::vector<uint8_t> " +
                     "GetSerializedContext1StateData(uint32_t test, uint32_t frameIndex);\n")
        h_file.write(
        ...
        cpp_file.write("std::vector<uint8_t> " +
                       "GetSerializedContext1StateData(uint32_t test, uint32_t frameIndex)\n")
        cpp_file.write("{\n")
        ...
        try:
            ...
            for i in range(len(test_results)):
                ...
                if returncode != 0 or not CanRunReplay(test_batch[i].GetLabel(),
                    ...
                    "Skipping replay since capture didn't produce appropriate files or has" + \
                     "crashed. STDOUT: " + output))
...
def main(capture_build_dir, replay_build_dir, use_goma, gtest_filter, test_exec, batch_count,
    ...
    try:
        ...
        for batch_index in range(test_batch_num):
            batch = TestBatch(use_goma, batch_count, delete_generated, goma_dir)
        ...
        # delete generated folders if --delete_generated flag is set to True
        if delete_generated:
...
if __name__ == "__main__":
    ...
    main(args.capture_build_dir, args.replay_build_dir, args.use_goma,
         ...
         distutils.util.strtobool(args.delete_generated), args.goma_dir)
