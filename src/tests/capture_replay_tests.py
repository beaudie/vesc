...
class TestExpectation():
    ...
    def ReadOneExpectation(self, line, is_debug):
        ...
        if self._CheckTagsWithConfig(tags, config_tags):
            ...
            if result_stripped == 'CRASHED' or result_stripped == 'COMPILE_FAILED':
...
def main(args):
    ...
    try:
        ...
        # Put the tests into batches and these into the job queue; jobs that are
        # expected to crash, timeout, or fail compilation will be run in batches
        # of sizeone, because a crash or failing to compile brings down the whole
        # batch, so that we would give false negatives if such a batch contains
        # jobs that would otherwise poss or fail differently.
