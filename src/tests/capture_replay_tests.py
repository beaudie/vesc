...
def main(capture_build_dir, replay_build_dir, use_goma, gtest_filter, test_exec):
    ...
    # objects created by manager can be shared by multiple processes. We use it to create collections
    # that are shared by multiple processes such as job queue or result list.
    ...
    # result list is created by manager and can be shared by multiple processes. Each subprocess populates
    # the result list with the results of its test runs. After all subprocesses finish, the main process
    # processes the results in the result list.
    # An item in the result list is a tuple with 3 values (testname, result, output). The "result" can take
    # 3 values "Passed", "Failed", "Skipped". The output is the stdout and the stderr of the test appended
    # together.
