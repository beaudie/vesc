...
class TestBatch():
    ...
    def Run(self, test_exe_path, trace_folder_path, child_processes_manager):
        ...
        if returncode == -1:
            self.results.append(
                GroupedResult(GroupedResult.Failed, "Capture run has crashed", output, self.tests))
            return False
        elif returncode == -2:
            self.results.append(
                GroupedResult(GroupedResult.Timeout, "Capture run has timeouted", "", self.tests))
            return False
        return True
    ...
    def RemoveTestsThatDoNotProduceAppropriateTraceFiles(self, trace_folder_path):
        ...
        if len(skipped_tests) > 0:
            self.results.append(
                GroupedResult(GroupedResult.Skipped,
    ...
    def BuildReplay(self, gn_path, autoninja_path, build_dir, trace_dir, replay_exec,
        ...
        if returncode != 0:
            self.results.append(
                GroupedResult(GroupedResult.Failed, "Build replay has failed at gn generation",
                              output, tests))
            return False
        ...
        if returncode != 0:
            self.results.append(
                GroupedResult(GroupedResult.Failed, "Build replay has failed at compilation",
                              output, tests))
            return False
        return True
    ...
    def RunReplay(self, replay_exe_path, child_processes_manager, tests):
        ...
        if returncode == -1:
            self.results.append(
                GroupedResult(GroupedResult.Failed, "Replay run has crashed", output, tests))
            return
        elif returncode == -2:
            self.results.append(
                GroupedResult(GroupedResult.Timeout, "Replay run has timeout", "", tests))
            return
        output_lines = output.splitlines()
        passes = []
        fails = []
