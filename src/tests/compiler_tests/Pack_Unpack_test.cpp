...
class PackUnpackTest : public testing::TestWithParam<ShShaderOutput>
{
  ...
  protected:
    ...
    {
        std::string infoLog;
        bool compilationSuccess = compileTestShader(GL_FRAGMENT_SHADER, SH_GLES3_SPEC, GetParam(),
...
TEST_P(PackUnpackTest, PackSnorm2x16Emulation)
{
    ...
    ASSERT_EQ(GetParam() < SH_GLSL_420_CORE_OUTPUT,
              foundInGLSLCode("uint webgl_packSnorm2x16_emu(vec2 v)"));
}
...
TEST_P(PackUnpackTest, UnpackSnorm2x16Emulation)
{
    ...
    compile(shaderString);
    ASSERT_EQ(GetParam() < SH_GLSL_420_CORE_OUTPUT,
              foundInGLSLCode("vec2 webgl_unpackSnorm2x16_emu(uint u)"));
}
...
TEST_P(PackUnpackTest, PackUnorm2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(GetParam() < SH_GLSL_410_CORE_OUTPUT,
              foundInGLSLCode("uint webgl_packUnorm2x16_emu(vec2 v)"));
}
...
TEST_P(PackUnpackTest, UnpackUnorm2x16Emulation)
{
    ...
    compile(shaderString);
    ASSERT_EQ(GetParam() < SH_GLSL_410_CORE_OUTPUT,
              foundInGLSLCode("vec2 webgl_unpackUnorm2x16_emu(uint u)"));
}
...
TEST_P(PackUnpackTest, PackHalf2x16Emulation)
{
    ...
    compile(shaderString);
    ASSERT_EQ(GetParam() < SH_GLSL_420_CORE_OUTPUT,
              foundInGLSLCode("uint webgl_packHalf2x16_emu(vec2 v)"));
}
...
TEST_P(PackUnpackTest, UnpackHalf2x16Emulation)
{
    ...
    compile(shaderString);
    ASSERT_EQ(GetParam() < SH_GLSL_420_CORE_OUTPUT,
              foundInGLSLCode("vec2 webgl_unpackHalf2x16_emu(uint u)"));
}

INSTANTIATE_TEST_CASE_P(,
                        PackUnpackTest,
                        testing::Values(SH_GLSL_330_CORE_OUTPUT,
