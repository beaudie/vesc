...
class PackUnpackTest : public testing::Test, public testing::WithParamInterface<ShShaderOutput>
{
  public:
    PackUnpackTest() {}
...
TEST_P(PackUnpackTest, PackSnorm2x16Emulation)
{
    ...
    ASSERT_EQ(foundInGLSLCode("uint webgl_packSnorm2x16_emu(vec2 v)"), GetParam() < SH_GLSL_420_CORE_OUTPUT);
}
...
TEST_P(PackUnpackTest, UnpackSnorm2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(foundInGLSLCode("vec2 webgl_unpackSnorm2x16_emu(uint u)"), GetParam() < SH_GLSL_420_CORE_OUTPUT);
}

//Check if PackUnorm2x16 Emulation for GLSL < 4.1 compile correctly.
TEST_P(PackUnpackTest, PackUnorm2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(foundInGLSLCode("uint webgl_packUnorm2x16_emu(vec2 v)"), GetParam() < SH_GLSL_410_CORE_OUTPUT);
}
...
TEST_P(PackUnpackTest, UnpackUnorm2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(foundInGLSLCode("vec2 webgl_unpackUnorm2x16_emu(uint u)"), GetParam() < SH_GLSL_410_CORE_OUTPUT);
}

//Check if PackHalf2x16 Emulation for GLSL < 4.2 compile correctly.
TEST_P(PackUnpackTest, PackHalf2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(foundInGLSLCode("uint webgl_packHalf2x16_emu(vec2 v)"), GetParam() < SH_GLSL_420_CORE_OUTPUT);
}

//Check if UnpackHalf2x16 Emulation for GLSL < 4.2 compile correctly.
TEST_P(PackUnpackTest, UnpackHalf2x16Emulation)
{
    const std::string &shaderString =
        ...
        "}\n";
    compile(shaderString);
    ASSERT_EQ(foundInGLSLCode("vec2 webgl_unpackHalf2x16_emu(uint u)"), GetParam() < SH_GLSL_420_CORE_OUTPUT);
}

INSTANTIATE_TEST_CASE_P(All, PackUnpackTest,
