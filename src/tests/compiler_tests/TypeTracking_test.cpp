...
TEST_F(TypeTrackingTest, BuiltInSignedBitfieldInsertResultTypeAndPrecision)
{
    const std::string &shaderString =
        "#version 310 es\n"
        "precision mediump float;\n"
        ...
        "   my_FragColor = vec4(i, 0.0, 0.0, 1.0); \n"
        "}\n";
    compile(shaderString);
    ASSERT_FALSE(foundErrorInIntermediateTree());
...
// Test that unsigned bitfieldInsert returns a precision consistent with its "base/insert"
// parameters.
TEST_F(TypeTrackingTest, BuiltInUnsignedBitfieldInsertResultTypeAndPrecision)
{
    const std::string &shaderString =
        "#version 310 es\n"
        "precision mediump float;\n"
        ...
        "   my_FragColor = vec4(u, 0.0, 0.0, 1.0); \n"
        "}\n";
    compile(shaderString);
    ASSERT_FALSE(foundErrorInIntermediateTree());
