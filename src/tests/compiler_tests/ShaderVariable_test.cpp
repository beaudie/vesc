...
TCompiler *GetCompilerFromHandleHelper(ShHandle handle)
{
    if (!handle)
        return nullptr;
...
TEST(ShaderVariableTest, IllegalInvariantVarying)
{
    ...
    TCompiler *tcompiler = GetCompilerFromHandleHelper(compiler);
...
TEST(ShaderVariableTest, InvariantLeakAcrossShaders)
{
    ...
    TCompiler *tcompiler = GetCompilerFromHandleHelper(compiler);
    EXPECT_TRUE(ShCompile(compiler, program1, 1, SH_VARIABLES));
    EXPECT_FALSE(tcompiler->getSymbolTable().isVaryingInvariant(std::string("v_varying")));
