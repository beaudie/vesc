  ...
  public:
    ...
    bool visitBinary(Visit visit, TIntermBinary *node)
    {
        ...
        switch (node->getOp())
        {
            case EOpAssign:
            {
                // Handles variable = initialValue. Structures are also handled by this case.
...
class InitOutputVariablesTest : public ShaderCompileTreeTest
{
  ...
  protected:
    ...
    void checkCorrectness(const ExpectedTypes &expectedTypes)
    {
