...
//   Tests correctness of the AST pass enabled through SH_INIT_OUTPUT_VARIABLES.
//
...
// of any of the output variables in the provided list.
  ...
  public:
    ...
    bool visitBinary(Visit visit, TIntermBinary *node)
    {
        ...
        switch (node->getOp())
        {
            case EOpAssign:
            {
                // Handles variable = initialValue. Structures also handled by this case.
            ...
            case EOpIndexDirect:
            {
                // Handles variable[index] = initialValue.
                const TIntermSymbol *leftSymbol       = node->getLeft()->getAsSymbolNode();
                const TIntermConstantUnion *subscript = node->getRight()->getAsConstantUnion();
                if (leftSymbol && subscript)
                {
                    canBeHandled                   = true;
                    std::string subscriptStdString = ToString(subscript->getUConst(0));
                    name =
                        leftSymbol->getSymbol() + "[" + TString(subscriptStdString.c_str()) + "]";
                    qualifier = leftSymbol->getType().getQualifier();
                }
            }
            break;
        ...
        if (canBeHandled)
        {
            // If the initializer is of the expected format, then check whether the name is
            // contained in the list.
            ExpectedTypes::iterator it =
                std::find(mExpectedTypes.begin(), mExpectedTypes.end(), name);

            // Check whether the qualifiers match.
...
TEST_F(InitOutputVariablesVertexShaderTest, OutputAllQualifiers)
{
    ...
    VerifyOutputVariableInitializers verifier(expected);
    mASTRoot->traverse(&verifier);
