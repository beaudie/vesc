...
TEST_F(InitOutputVariablesWebGL2FragmentShaderTest, FragData)
{
    ...
    TType fragDataType = TType(EbtFloat, EbpMedium, EvqFragData, 4);
    fragDataType.setArraySize(2);

    ExpectedLValues expectedLValues = CreateIndexedLValueNodeList("gl_FragData", fragDataType, 2);
    EXPECT_TRUE(verifier.isExpectedLValueFound(expectedLValues[0]));
    EXPECT_FALSE(verifier.isExpectedLValueFound(expectedLValues[1]));
    ReleaseExpectedLValuesMemory(expectedLValues);
}
