...
TEST_F(BufferVariablesTest, AssignToBufferVariableWithinBlockWithReadonly)
{
    const std::string &source =
        "#version 310 es\n"
...
// Test that can't read/write from/to a buffer variable with readonly and writeonly.
TEST_F(BufferVariablesTest, RWBufferVariableWithReadonlyWriteonly)
{
    const std::string &source =
        "#version 310 es\n"
        "layout(binding = 3) buffer buf {\n"
