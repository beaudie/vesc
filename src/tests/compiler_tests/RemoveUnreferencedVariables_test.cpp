...
// Test that a user-defined type that's only referenced in an unreferenced variable is removed in
// the presence of another user-defined type. This triggers reference counting of empty symbols.
TEST_F(RemoveUnreferencedVariablesTest,
       UserDefinedTypeReferencedInUnreferencedVariableAndAnotherType)
{
    const std::string &shaderString =
        R"(
...
// Test that a user-defined type that is referenced in another user-defined type is removed in case
// neither type is used in referenced variables.
