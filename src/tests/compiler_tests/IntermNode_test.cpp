...
class IntermNodeTest : public testing::Test
{
  ...
  protected:
    ...
    void checkTypeEqualWithQualifiers(const TType &copy, const TType &original)
    {
    ...
    void checkSymbolCopy(TIntermNode *aCopy, TIntermNode *aOriginal)
    {
        ...
        checkTypeEqualWithQualifiers(copy->getType(), original->getType());
...
TEST_F(IntermNodeTest, DeepCopyBinaryNode)
{
    ...
    checkSymbolCopy(copy->getLeft(), original->getLeft());
    checkSymbolCopy(copy->getRight(), original->getRight());
}
...
TEST_F(IntermNodeTest, DeepCopyUnaryNode)
{
    ...
    checkSymbolCopy(copy->getOperand(), original->getOperand());
}
...
TEST_F(IntermNodeTest, DeepCopyAggregateNode)
{
    ...
    for (auto *copyChild : *copy->getSequence())
    {
        ...
        checkSymbolCopy(copyChild, originalChild);
        ++i;
    }
}
...
TEST_F(IntermNodeTest, DeepCopySelectionNode)
{
    ...
    TIntermSelection *original = new TIntermSelection(createTestSymbol(TType(EbtBool, EbpUndefined)), createTestSymbol(), createTestSymbol());
    original->setLine(getTestSourceLoc());
    TIntermTyped *copyTyped = original->deepCopy();
    ...
    checkSymbolCopy(copy->getCondition(), original->getCondition());
    checkSymbolCopy(copy->getTrueBlock(), original->getTrueBlock());
    checkSymbolCopy(copy->getFalseBlock(), original->getFalseBlock());
}