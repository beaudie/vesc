...
const char ESSL300_LocationIndexFailureShader[] =
    R"(precision mediump float;
layout(index = 0) out mediump vec4 fragColor;
void main() {
    fragColor = vec4(1.0);
})";
...
R"(precision mediump float;
layout(index = 0, location = 0, index = 1) out mediump vec4 fragColor;
void main() {
    fragColor = vec4(1.0);
})";
...
INSTANTIATE_TEST_CASE_P(IncorrectESSL300Shaders,
                        ...
                        Combine(Values(SH_GLES3_1_SPEC),
                                ...
                                Values(ESSL300_LocationIndexFailureShader, ESSL300_DoubleIndexFailureShader)));
