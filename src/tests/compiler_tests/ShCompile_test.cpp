...
TEST_F(ShCompileTest, DecimalSepLocale)
{
    ...
    const std::string availableLocales[] = {"de_DE", "de-DE", "German_germany", "German_Germany"};
    ...
    for (const std::string &locale : availableLocales)
    {
        ...
        // On fail, the locale constructor throws an exception
        // We use setlocale() (through localeExists) to test whether a locale exists before calling
