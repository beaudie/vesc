...
{
  public:
    float parse(std::string str) { return sh::NumericLexFloat32OutOfRangeToInfinity(str); }
};
...
class FloatLexTest : public testing::Test
{
  ...
  protected:
    ...
    bool parsedMatches(std::string str, float expected) { return (mParser.parse(str) == expected); }

    bool isInfinity(std::string str)
    {
        float f = mParser.parse(str);
        return gl::isInf(f);
    }
...
{
    ASSERT_TRUE(parsedMatches("1.0", 1.0f));
}

TYPED_TEST(FloatLexTest, Ten)
{
    ASSERT_TRUE(parsedMatches("10.0", 10.0f));
}

TYPED_TEST(FloatLexTest, TenScientific)
{
    ASSERT_TRUE(parsedMatches("1.0e1", 10.0f));
}

TYPED_TEST(FloatLexTest, ScientificWithSmallMantissa)
{
    std::stringstream ss;
    ss << "0." << zeros(100) << "125e102";
    ASSERT_TRUE(parsedMatches(ss.str(), 12.5f));
}

TYPED_TEST(FloatLexTest, ScientificWithLargeMantissa)
{
    std::stringstream ss;
    ss << "9" << zeros(100) << ".0e-100";
    ASSERT_TRUE(parsedMatches(ss.str(), 9.0f));
}

TYPED_TEST(FloatLexTest, ScientificWithVerySmallMantissa)
{
    std::stringstream ss;
    ss << "0." << zeros(5000) << "125e5002";
    ASSERT_TRUE(parsedMatches(ss.str(), 12.5f));
}
