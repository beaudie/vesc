...
class ShaderImageFinder : public TIntermTraverser
{
  public:
    ShaderImageFinder(const TString &imageName, TBasicType basicType)
    ...
    {
        if (node->getBasicType() == mBasicType && node->getSymbol() == mImageName)
        {
            mNodeFound = node;
        }
    }
...
class ShaderLoadStoreFinder : public TIntermTraverser
{
  public:
    ShaderLoadStoreFinder(const TString &functionName)
...
class ShaderImageTest : public testing::Test
{
  ...
  protected:
    ...
    bool isSymbolInAST(const TString &stringToFind, TBasicType basicType)
    {
        ShaderImageFinder finder(stringToFind, basicType);
    ...
    bool isFunctionInAST(const TString &stringToFind)
    {
        ShaderLoadStoreFinder finder(stringToFind);
        mASTRoot->traverse(&finder);
        return finder.isFound();
    }

    const TIntermAggregate *findFunctionInAST(const TString &stringToFind)
    {
        ShaderLoadStoreFinder finder(stringToFind);
        mASTRoot->traverse(&finder);
        return finder.getNode();
...
TEST_F(ShaderImageTest, ImageDeclaration)
{
    ...
    const TIntermSymbol *myImageNode = findSymbolInAST("myImage", EbtImage2D);
    ...
    const TIntermSymbol *myImage2Node = findSymbolInAST("myImage2", EbtImage2D);
...
TEST_F(ShaderImageTest, ImageStore)
{
    ...
    const TIntermAggregate *imageStoreFunctionCall = findFunctionInAST("imageStore(im21;vi2;vf4;");
    ...
    // tests for imageLoad
    const TIntermAggregate *imageLoadFunctionCall = findFunctionInAST("imageLoad(im21;vi2;");
