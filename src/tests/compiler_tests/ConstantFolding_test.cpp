...
class ConstantFinder : public TIntermTraverser
{
  ...
  private:
    bool isEqual(const TConstantUnion &node, const T &value) const
    {
        if (mFaultTolerance && node.getType() == EbtFloat)
...
TEST_F(ConstantFoldingTest, Fold3x3MatrixInverse)
{
    ...
    ASSERT_TRUE(constantVectorNearFoundInAST(result, floatFaultTolarance));
}
