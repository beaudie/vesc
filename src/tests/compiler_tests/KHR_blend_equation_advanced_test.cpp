...
class KHRBlendEquationAdvancedTest : public sh::ShaderExtensionTest
{
  public:
    ...
    testing::AssertionResult TestShaderCompile(ShShaderOutput shaderOutputType, const char *pragma)
    {
        ...
        bool success =
            sh::Compile(mCompilerList[shaderOutputType], shaderStrings, 3,
                        SH_VARIABLES | SH_OBJECT_CODE | SH_GENERATE_SPIRV_THROUGH_GLSLANG);
        if (success)
        {
...
TEST_P(KHRBlendEquationAdvancedEnabledSeparatelyTest, DeclaredEquationSeparately)
{
    ...
    sh::AdvancedBlendEquation enabledBlendEquation = sh::GetBlendEquation(compilerHandle);
    EXPECT_TRUE(enabledBlendEquation.isEnabled(sh::TLayoutBlendEquation::Multiply));
    EXPECT_TRUE(enabledBlendEquation.isEnabled(sh::TLayoutBlendEquation::Screen));
}
