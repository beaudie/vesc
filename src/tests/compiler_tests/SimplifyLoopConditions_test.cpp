...
TEST_F(SimplifyLoopConditionsTest, ForWithContinueToWhile)
{
    ...
    ASSERT_TRUE(foundInCode("while"));
    ASSERT_TRUE(foundInCodeInOrder({"(_up++);", "(_up++);"}));
}

// Another case that could result in infinite loop.
TEST_F(SimplifyLoopConditionsTest, ForWithContinueToWhile2)
{
    const std::string shaderString =
        R"(precision mediump float;
        void main()
