...
// Test that using an array constructor in an operand of the sequence (comma) operator is accepted
// as a constant expression.
TEST_F(MalformedShaderTest, SequenceOperatorAppliedToArrayConstructorIsConst)
{
    const std::string &shaderString =
        "#version 300 es\n"
        "precision mediump float;\n"
        ...
        "void main()\n"
        "{\n"
        "    const float f = (1.0, float[2](0.0, 1.0)[0]);\n"
        "    my_FragColor = vec4(f);\n"
        "}\n";
    if (!compile(shaderString))
    {
