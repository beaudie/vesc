...
// Redeclare gl_LastFragData with noncoherent qualifier and use it in the for-loop
const char ESSL100_LastFragDataRedeclared2[] =
    R"(
    uniform highp vec4 u_color;
    uniform mediump int drawBufferCount;
    layout(noncoherent) highp vec4 gl_LastFragData[gl_MaxDrawBuffers];
    ...
    void main (void)
    {
        ...
        for (mediump int i = 0 ; i < drawBufferCount; i++)
        {
            gl_FragColor += gl_LastFragData[i];
        }
    })";
...
INSTANTIATE_TEST_SUITE_P(CorrectESSL100Shaders,
                         ...
                         Combine(Values(SH_GLES2_SPEC),
                                 ...
                                 Values(ESSL100_LastFragDataRedeclared1,
                                        ESSL100_LastFragDataRedeclared2)));
