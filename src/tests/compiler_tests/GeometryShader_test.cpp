...
TEST_F(GeometryShaderTest, MixedUseOESAndEXTExtension)
{
    ...
    for (const std::string &extensionBehavior1 : kExtensionBehaviors)
    {
        ...
        {
            std::ostringstream stream;
            stream << kVersion << kExtensionStrings[0] << " : " << extensionBehavior1 << "\n"
                   << kExtensionStrings[1] << " : " << extensionBehavior2 << "\n"
                   << kShaderBody;

            bool shouldCompile = extensionBehavior1 != "disable" || extensionBehavior2 != "disable";
