...
// Test collecting inputs declared before the input primitive declaration in a geometry shader.
TEST_F(CollectGeometryVariablesTest, CollectInputsDeclaredBeforeInputPrimitive)
{
    const std::string &shaderString =
        "#version 310 es\n"
        "#extension GL_OES_geometry_shader : require\n"
    ...
    constexpr int kLocations[] = {-1, 1};

    for (size_t i = 0; i < inputVaryings.size(); ++i)
    {
        const auto &varying = inputVaryings[i];
        ...
        EXPECT_GLENUM_EQ(GL_HIGH_FLOAT, varying.precision);
        EXPECT_GLENUM_EQ(GL_FLOAT_VEC4, varying.type);
        EXPECT_EQ(kLocations[i], varying.location);
        EXPECT_EQ(3u, varying.arraySize);
    }
}

// Test collecting inputs declared after the input primitive declaration in a geometry shader.
TEST_F(CollectGeometryVariablesTest, CollectInputsDeclaredAfterInputPrimitive)
{
    const std::string &shaderString =
        "#version 310 es\n"
        "#extension GL_OES_geometry_shader : require\n"
...
TEST_F(CollectGeometryVariablesTest, CollectInputArraySize)
{
    const std::array<std::string, 5> kInputPrimitives = {
        {"points", "lines", "lines_adjacency", "triangles", "triangles_adjacency"}};
...
TEST_F(CollectGeometryVariablesTest, CollectInputsWithInterpolationQualifiers)
{
    ...
    for (size_t i = 0; i < kInterpolationQualifiers.size(); ++i)
    {
        ...
        const auto &inputVaryings1 = mTranslator->getInputVaryings();
        ASSERT_EQ(1u, inputVaryings1.size());
        const Varying *varying1 = &inputVaryings1[0];
        EXPECT_EQ("texcoord", varying1->name);
        EXPECT_EQ(kInterpolationType[i], varying1->interpolation);

        std::ostringstream stream2;
        stream2 << kHeader << qualifier << " in vec4 texcoord[];\n" << kLayout << kFunctionBody;
        compile(stream2.str());

        const auto &inputVaryings2 = mTranslator->getInputVaryings();
        ASSERT_EQ(1u, inputVaryings2.size());
        const Varying *varying2 = &inputVaryings2[0];
        EXPECT_EQ("texcoord", varying2->name);
        EXPECT_EQ(kInterpolationType[i], varying2->interpolation);
    }
}
