...
TEST_F(CollectGeometryVariablesTest, CollectInputsDeclaredBeforeInputPrimitive)
{
    const std::string &shaderString =
        ...
        "#extension GL_OES_geometry_shader : require\n"
        "in vec4 texcoord1[];\n"
        "layout (location = 1) in vec4 texcoord2[1];\n"
        "layout (points) in;\n"
        "layout (points, max_vertices = 2) out;\n"
        "void main()\n"
        "{\n"
...
TEST_F(CollectGeometryVariablesTest, CollectInputArraySize)
{
    ...
    for (size_t i = 0; i < kInputPrimitives.size(); ++i)
    {
        ...
        const Varying *varying = &inputVaryings[0];
        ASSERT_EQ("texcoord", varying->name);
...
TEST_F(CollectGeometryVariablesTest, CollectInputsWithInterpolationQualifiers)
{
    ...
    for (int i = 0; i < kInterpolationQualifiers.size(); ++i)
    {
        ...
        ASSERT_EQ("texcoord", varying1->name);
        ...
        ASSERT_EQ("texcoord", varying2->name);
