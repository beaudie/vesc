...
def diff_files(path, expected_path):
    ...
    if fn.endswith('.angledata'):
        logging.error('Binary file is different: %s', fn)
        return True
    ...
    # Might be empty due to CRLF
    if not diff:
        return False

    logging.error('Found diff in %s:\n%s\n', fn, '\n'.join(diff))
    return True
    ...
    with temporary_dir() as temp_dir:
        ...
        if overwrite_expected:
            for f in expected_files:
                os.remove(os.path.join(expected_dir, expected_files))
...
def main():
    ...
    test_name = 'CapturedTest.MultiFrame/ES3_Vulkan_SwiftShader'
    had_error = False
    try:
