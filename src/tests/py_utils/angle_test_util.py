...
def RunTestSuite(test_suite,
    ...
    with contextlib.ExitStack() as stack:
        ...
        results_matches = [a for a in cmd_args if a.startswith('--isolated-script-test-output=')]
        if results_matches:
            results_path = results_matches[0].split('=')[1]
        else:
        ...
        with open(results_path) as f:
            data = f.read()
            json_results = json.loads(data) if data else None
