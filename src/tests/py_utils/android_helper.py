...
def _InitializeAndroid(apk_path):
    # Determine our user scenario first
    _GetUserInfo()
...
def _GetUserInfo():
    shell_id, current_user, su_path, data_permissions = _AdbShell(
        'id -u; am get-current-user; which su || echo noroot; stat --format %a /data').decode(
        ).strip().split('\n')
    ...
    if su_path == 'noroot':
        ...
        for _ in range(20):  # `adb root` restarts adbd which can take quite a few seconds
            ...
            if id_out == '0':
                ...
                _Global.has_adb_root = True
                break

        if _Global.has_adb_root == False:
            # Device has "su" but we couldn't get adb root. Something is wrong.
            raise Exception('Failed to get adb root')

    if current_user.isnumeric():
        _Global.current_user = current_user
        logging.debug('Current user: %s', _Global.current_user)
    else:
        raise Exception('Failed to determine current_user: %s', current_user)
