...
@contextlib.contextmanager
def _TempDeviceFileTmpfs():
    path = '/sdcard/test_tmpfs/temp_file-%s' % _RandomHex()
    try:
        yield path
    finally:
        _AdbShell('rm -f ' + path)
...
def RunTests(test_suite, args, stdoutfile=None, output_dir=None, log_output=True):
    ...
    _AdbShell(
        'mkdir -p /sdcard/test_tmpfs/ && mount -t tmpfs -o size=128m tmpfs /sdcard/test_tmpfs')

    result = 0
    output = b''
    try:
        with contextlib.ExitStack() as stack:
            device_test_output_path = stack.enter_context(_TempDeviceFileTmpfs())
