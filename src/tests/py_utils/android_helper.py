...
def _RunInstrumentationWithTimeout(flags, timeout):
    ...
    while True:
        ...
        if time.time() - start > 70:
            ...
            try:
                pid = _AdbShell('pidof com.android.angle.test:test_process').decode()
                ps_out = _AdbShell('ps -o ni -p %s' % pid).decode()
                niceness = int(ps_out.strip().split('\n')[-1].strip())
                if niceness != last_niceness:
                    logging.info('niceness: %d', niceness)
                    last_niceness = niceness
                if niceness >= 10:
                    logging.info('Changing niceness from %d', niceness)
                    renice_out = _AdbShell('renice -n -10 -p %s' % pid).decode()
                    logging.info('renice_out:\n%s', renice_out)
            except Exception as e:
                logging.warning('niceness failed: %s', e)
