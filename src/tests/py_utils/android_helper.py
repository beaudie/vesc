...
def _Run(cmd):
    if log:
        logging.info('adb_helper cmd: %s', cmd[1:])
    startupinfo = None
...
def PrepareRestrictedTrace(adb, trace):
    fn = 'src/tests/restricted_traces/' + trace + '/' + trace + '.angledata.gz'
    adb.Run(['push', '../../' + fn, '/sdcard/chromium_tests_root/' + fn])
...
def AngleSystemInfo(adb, args, output_dir):
