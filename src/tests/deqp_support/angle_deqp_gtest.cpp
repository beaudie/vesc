...
const std::vector<const char *> gTestConfigParameters[] = {
...
void dEQPTestSuite::SetUpTestSuite()
{
    ...
    // Add config name
    const std::vector<const char *> testConfig = GetTestSuiteConfigString(GetTestModuleIndex());

    // TODO parse the testConfig string
    for (const char *testConfigArgument : testConfig)
    {
        argv.push_back(testConfigArgument);
    }
...
{
    return gTestConfigParameters[testModuleIndex];
}
...
{
    std::cout << "dEQPTest::TestBody() is called caseInfo.testName: " << std::endl;
