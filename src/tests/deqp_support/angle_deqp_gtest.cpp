...
{
    std::string replaced = dEQPName;
    angle::ReplaceAllSubstrings(&replaced, "-", ".");
    return replaced;
}
...
std::array<char, 500> gFilterStringBuffer;
...
void HandleFilterArg(const char *caseString, int *argc, int argIndex, char **argv)
{
    std::string googleTestName = DrawElementsToGoogleTestName(caseString);
    gFilterStringBuffer.fill(0);
...
void RegisterGLCTSTests()
{
    ...
    for (size_t caseIndex = 0; caseIndex < caseList.numCases(); ++caseIndex)
    {
        ...
        std::string moduleName = DrawElementsToGoogleTestName(testCaseName.substr(0, pos));
