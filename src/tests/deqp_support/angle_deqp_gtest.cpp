...
const std::vector<const char *> gTestSuiteConfigParameters[] = {
    ...
    {"--deqp-gl-config-name=rgb565d0s0ms0"},     // gl46
};
...
// If gEGLConfigName is overwritten by --deqp-gl-config-name passed from command arguments,
// use gEGLConfigName instead
...
const std::vector<const char *> GetTestSuiteConfigString(size_t testModuleIndex)
{
...
void dEQPTest::SetUpTestSuite()
{
    ...
    std::string configArgFromCmdLineString =
    ...
    for (const char *configParam : testSuiteConfigParams)
    {
        ...
        // command line
        if (strncmp(configParam, gdEQPEGLConfigNameString, strlen(gdEQPEGLConfigNameString)) == 0)
        {
            if (std::strcmp(gEGLConfigNameFromCmdLine, "") != 0)
            {
                configParam = configArgFromCmdLineString.c_str();
            }
        }
