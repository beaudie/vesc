...
class dEQPTestSuiteData
{
  public:
    static const std::string GetTestCaseName(size_t testModuleIndex, size_t caseIndex);

    static const dEQPCaseList &GetTestList(size_t testModuleIndex);

    dEQPTestSuiteData();

  private:
    static void SetUpTestStats();
...
uint32_t dEQPTestSuiteData::sTestCount             = 0;
uint32_t dEQPTestSuiteData::sPassedTestCount       = 0;
uint32_t dEQPTestSuiteData::sFailedTestCount       = 0;
uint32_t dEQPTestSuiteData::sTestExceptionCount    = 0;
uint32_t dEQPTestSuiteData::sNotSupportedTestCount = 0;
uint32_t dEQPTestSuiteData::sSkippedTestCount      = 0;
std::vector<std::string> dEQPTestSuiteData::sUnexpectedFailed;
std::vector<std::string> dEQPTestSuiteData::sUnexpectedPasses;

dEQPTestSuiteData::dEQPTestSuiteData() {}

const std::string dEQPTestSuiteData::GetTestCaseName(size_t testModuleIndex, size_t caseIndex)
{
    const auto &caseInfo = GetTestList(testModuleIndex).getCaseInfo(caseIndex);
    return caseInfo.testName;
}

const dEQPCaseList &dEQPTestSuiteData::GetTestList(size_t testModuleIndex)
{
...
void dEQPTestSuiteData::SetUpTestStats()
{
    sPassedTestCount       = 0;
    sFailedTestCount       = 0;
...
void dEQPTestSuiteData::PrintTestStats()
{
...
void dEQPTestSuiteData::countTestResult(dEQPTestResult result)
{
    switch (result)
    {
...
{
    dEQPTestSuiteData::SetUpTestStats();
...
{
    dEQPTestSuiteData::PrintTestStats();
...
{
    if (dEQPTestSuiteData::sTestExceptionCount > 1)
    {
    ...
    const auto &caseInfo =
        dEQPTestSuiteData::GetTestList(mTestModuleIndex).getCaseInfo(mTestCaseIndex);
    ...
    dEQPTestSuiteData::sTestCount++;

    if (caseInfo.expectation == GPUTestExpectationsParser::kGpuTestSkip)
    {
        dEQPTestSuiteData::sSkippedTestCount++;
    ...
    dEQPTestSuiteData::countTestResult(result);
        ...
        caseInfo.expectation == GPUTestExpectationsParser::kGpuTestFlaky)
    {
        ...
        if (!testSucceeded)
        {
            dEQPTestSuiteData::sUnexpectedFailed.push_back(caseInfo.testName);
        }
    }
    else if (testSucceeded)
    {
        std::cout << "Test expected to fail but passed!" << std::endl;
        dEQPTestSuiteData::sUnexpectedPasses.push_back(caseInfo.testName);
    }
}
...
void RegisterGLCTSTests()
{
    ...
    const dEQPCaseList &caseList = dEQPTestSuiteData::GetTestList(testModuleIndex);
    ...
    for (size_t caseIndex = 0; caseIndex < caseList.numCases(); ++caseIndex)
    {
        ...
        const std::string testCaseName =
            dEQPTestSuiteData::GetTestCaseName(testModuleIndex, caseIndex);
