...
namespace tcu
{
class NativeWindowFactoryPosix : public NativeWindowFactory
{
  public:
    explicit NativeWindowFactoryPosix(EventState *eventState,
                                      uint32_t preRotation,
                                      const eglu::NativeWindow::Capability windowCapability);
    ~NativeWindowFactoryPosix() override = default;
};

class NativeWindowPosix : public NativeWindow
{
  public:
    NativeWindowPosix(ANGLENativeDisplay *NativeDisplay,
                      const eglu::WindowParams &params,
                      EventState *eventState,
                      uint32_t preRotation,
                      const eglu::NativeWindow::Capability windowCapability);
    ~NativeWindowPosix() override = default;
};
}  // namespace tcu
