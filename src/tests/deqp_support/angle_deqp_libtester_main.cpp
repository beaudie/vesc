...
bool FindDataDir(const char *rootDir, std::string *dataDirOut)
{
    ...
    for (const char *dataPath : kDataPaths)
    {
        ...
        dirStream << rootDir << "/" << dataPath << "/" << ANGLE_DEQP_DATA_DIR;
...
ANGLE_LIBTESTER_EXPORT int deqp_libtester_main(int argc, const char *argv[], const char *rootDir)
{
    if (!deqp_libtester_init_platform(argc, argv, nullptr, rootDir))
    {
