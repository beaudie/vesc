...
class GPUTestConfigTest : public ANGLETest
{
  protected:
                           ...
                           uint32_t preRotation)
    {
        ...
        {
            case 90:
                EXPECT_TRUE(config.getConditions()[GPUTestConfig::kConditionPreRotate90]);
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate180]);
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate270]);
                break;
            case 180:
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate90]);
                EXPECT_TRUE(config.getConditions()[GPUTestConfig::kConditionPreRotate180]);
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate270]);
                break;
            case 270:
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate90]);
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate180]);
                EXPECT_TRUE(config.getConditions()[GPUTestConfig::kConditionPreRotate270]);
                break;
            default:
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate90]);
                EXPECT_FALSE(config.getConditions()[GPUTestConfig::kConditionPreRotate180]);
...
TEST_P(GPUTestConfigTest, GPUTestConfigConditions_Vulkan_PreRotate90)
{
    GPUTestConfig config(GPUTestConfig::kAPIVulkan, 90);
    validateConfigAPI(config, GPUTestConfig::kAPIVulkan, 90);
}

TEST_P(GPUTestConfigTest, GPUTestConfigConditions_Vulkan_PreRotate180)
{
    GPUTestConfig config(GPUTestConfig::kAPIVulkan, 180);
    validateConfigAPI(config, GPUTestConfig::kAPIVulkan, 180);
}

TEST_P(GPUTestConfigTest, GPUTestConfigConditions_Vulkan_PreRotate270)
{
