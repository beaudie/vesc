...
int32_t GPUTestExpectationsParser::getTestExpectation(const std::string &testName)
{
    for (size_t i = 0; i < mEntries.size(); ++i)
    {
        if (NamesMatching(mEntries[i].testName, testName))
        {
            ...
            return mEntries[i].testExpectation;
        }
    }
    return kGpuTestPass;
}
...
const std::vector<std::string> GPUTestExpectationsParser::getUnusedExpectationsMessages() const
{
...
const std::vector<GPUTestExpectationsParser::GPUTestExpectationEntry>
