...
// Start XFB on one thread, read the result on another thread without flusing on the first thread.
TEST_P(EGLContextSharingTest, StartXFBAndReadOnDifferentThreadNoFlush)
{
    ...
    auto xfbStartThread = [&](EGLDisplay dpy, EGLSurface surface, EGLContext context) {
        ...
        threadSynchronization.nextStep(Step::Thread0XFBEnd);
        ASSERT_TRUE(threadSynchronization.waitForStep(Step::Thread1DidReadXFB));
    ...
    auto xfbReadThread = [&](EGLDisplay dpy, EGLSurface surface, EGLContext context) {
        ...
        // Wait for thread 0 to finish the XFB process.
        ASSERT_TRUE(threadSynchronization.waitForStep(Step::Thread0XFBEnd));
        ...
        threadSynchronization.nextStep(Step::Thread1DidReadXFB);
