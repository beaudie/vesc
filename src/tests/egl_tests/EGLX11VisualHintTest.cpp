...
TEST_P(EGLX11VisualHintTest, ValidVisualIDAndClear)
{
    ...
    ASSERT_NE(0, XGetWindowAttributes(xDisplay, xWindow, &windowAttributes));
    ...
    ASSERT_EQ(EGL_TRUE, eglInitialize(display, nullptr, nullptr));
    ...
    int nConfigs = 0;
    ASSERT_EQ(EGL_TRUE, eglGetConfigs(display, nullptr, 0, &nConfigs));
    ASSERT_EQ(1, nConfigs);

    int nReturnedConfigs = 0;
    EGLConfig config;
    ASSERT_EQ(EGL_TRUE, eglGetConfigs(display, &config, 1, &nReturnedConfigs));
    ASSERT_EQ(nConfigs, nReturnedConfigs);

    EGLint eglNativeId;
    ASSERT_EQ(EGL_TRUE, eglGetConfigAttrib(display, config, EGL_NATIVE_VISUAL_ID, &eglNativeId));
