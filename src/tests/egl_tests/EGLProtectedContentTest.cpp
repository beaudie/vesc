...
class EGLProtectedContentTest : public ANGLETest
{
  public:
    ...
    bool hasGLProtectedTexturesExtension() const
    {
        return IsGLExtensionEnabled("GL_EXT_protected_textures");
    }
    ...
    bool createTexture(EGLBoolean isProtected, GLuint *textureId)
    {
        ...
        if (isProtected)
        {
            ...
            std::cout << "glTexParam error: " << error << std::endl;
    ...
    void checkReadPixelsResult(GLColor color, bool isProtectedContext, bool isProtectedSurface)
    {
        // Results
...
void EGLProtectedContentTest::pbufferTest(bool isProtectedContext, bool isProtectedSurface)
{
    ...
    checkReadPixelsResult(GLColor::red, isProtectedContext, isProtectedSurface);
...
#if 0  // Following have undefined behavior - may crash with memory violation
...
void EGLProtectedContentTest::textureTest(bool isProtectedContext, bool isProtectedTexture)
{
    ...
    if (hasGLProtectedTexturesExtension())
    {
        GLuint texture = 0;
...
void EGLProtectedContentTest::textureFromImageTest(bool isProtectedContext, bool isProtectedTexture)
{
    ...
    if (IsGLExtensionEnabled(
            "GL_OES_EGL_image"))  // && IsGLExtensionEnabled("GL_EXT_protected_textures"))
    {
        GLuint srcTexture = 0;
        if (isProtectedTexture && !isProtectedContext)
        {
                                                                 ...
                                                                 bool isProtectedTexture)
{
    ...
    if (hasGLProtectedTexturesExtension())
    {
        ...
        else
        {
            ...
            glFinish();
            ASSERT_GL_NO_ERROR() << "glFinish failed";
            checkReadPixelsResult(GLColor::red, isProtectedContext, isProtectedTexture);
