  ...
  public:
                       ...
                       GLuint *textureId)
    {
        ...
        *textureId  = 0;
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kWidth, kHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                     nullptr);
        glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA, kWidth, kHeight);
        EXPECT_GL_NO_ERROR();
        result = (*textureId != 0);
        EXPECT_TRUE(result);
        return result;
                                  ...
                                  GLuint *textureId)
    {
        ...
        *textureId  = 0;
        ...
        result = eglBindTexImage(mDisplay, pBuffer, *textureId);
        return result;
    }
    ...
    bool fillTexture(GLuint textureId)
    {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
        EXPECT_GL_NO_ERROR();
        return true;
    }
...
{
    if (getPlatformRenderer() == EGL_PLATFORM_ANGLE_TYPE_VULKAN_ANGLE)
    {
        std::cout << "Operator should see BLACK on screen for Vulkan backend" << std::endl;
    }
...
{
    std::cout << "Operator should see BLACK on screen" << std::endl;
...
bool EGLProtectedContentTest::renderTexture(GLuint textureId)
{
...
void EGLProtectedContentTest::textureFromImageTest(bool isProtectedContext, bool isProtectedTexture)
{
    ...
    GLuint srcTexture = 0;
    EXPECT_TRUE(createTexture(isProtectedTexture, context, GL_TEXTURE1, &srcTexture));
    EXPECT_TRUE(fillTexture(srcTexture));

    EGLImage image               = EGL_NO_IMAGE;
    EGLClientBuffer clientBuffer = IntToPtr(srcTexture);
...
#    endif
#    if 1
void EGLProtectedContentTest::textureFromPbufferTest(bool isProtectedContext, bool isProtectedTexture)
{
    if (isProtectedContext || isProtectedTexture)
    {
        ANGLE_SKIP_TEST_IF(!IsEGLDisplayExtensionEnabled(mDisplay, "EGL_EXT_protected_content"));
    ...
    GLuint texture = 0;
    EXPECT_TRUE(createTextureFromPbuffer(isProtectedTexture, pBufferSurface, GL_TEXTURE0, &texture));
    EXPECT_TRUE(renderTexture(texture));
    ...
    eglReleaseTexImage(mDisplay, pBufferSurface, texture);
...
#    endif
#    if 1
void EGLProtectedContentTest::textureFromAndroidNativeBufferTest(bool isProtectedContext, bool isProtectedTexture)
{
    if (isProtectedContext || isProtectedTexture)
    {
        ANGLE_SKIP_TEST_IF(!IsEGLDisplayExtensionEnabled(mDisplay, "EGL_EXT_protected_content"));
    ...
    EXPECT_TRUE(createAndroidClientBuffer(isProtectedTexture, false, true,
                                          &clientBuffer));
