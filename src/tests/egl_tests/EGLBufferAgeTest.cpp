...
TEST_P(EGLBufferAgeTest, VerifyContents)
{
    ...
    for (int i = 0; i < loopCount; i++)
    {
        ...
        auto red   = static_cast<float>(kColorSet[i].R / 255.0);
        auto green = static_cast<float>(kColorSet[i].G / 255.0);
        auto blue  = static_cast<float>(kColorSet[i].B / 255.0);
        auto alpha = static_cast<float>(kColorSet[i].A / 255.0);
