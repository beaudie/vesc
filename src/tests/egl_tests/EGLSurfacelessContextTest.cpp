...
class EGLSurfacelessContextTest : public ANGLETest
{
  public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(eglGetConfigs(mDisplay, configs.data(), nConfigs, &nReturnedConfigs) ==
                    EGL_TRUE);
...
TEST_P(EGLSurfacelessContextTest, DefaultScissorAndViewport)
{
    ...
    EGLContext context = createContext();
    ASSERT_TRUE(eglMakeCurrent(mDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, context));
...
TEST_P(EGLSurfacelessContextTest, CheckFramebufferStatus)
{
    ...
    ASSERT_EQ(GL_FRAMEBUFFER_UNDEFINED_OES, glCheckFramebufferStatus(GL_FRAMEBUFFER));
...
TEST_P(EGLSurfacelessContextTest, Switcheroo)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        ASSERT_TRUE(eglMakeCurrent(mDisplay, pbuffer, pbuffer, context));
