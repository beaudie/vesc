...
class EGLLockSurface3Test : public ANGLETest
{
  public:
    ...
    void fillBitMapRGBA32(GLColor colorIn, EGLAttribKHR bitMapPtr, EGLint pitch)
    {
    ...
    bool checkBitMapRGBA32(GLColor colorIn, EGLAttribKHR bitMapPtr, EGLint pitch)
    {
        GLColor color = ChannelOrder(colorIn);
...
// Create PBufferSurface, glClear Green, Lock, check buffer, Write pixels, Unlock, Test pixels
TEST_P(EGLLockSurface3Test, PbufferSurfaceReadWriteTest)
{
    ...
    glFinish();
    ASSERT_GL_NO_ERROR() << "glClear/glFinish failed";
    ...
    EXPECT_TRUE(checkBitMapRGBA32(clearColor, bitMapPtr, bitMapPitch));
...
// Create WindowSurface, Clear Color to GREEN, Lock with PRESERVE_PIXELS, read/check pixels, Unlock.
TEST_P(EGLLockSurface3Test, WindowSurfaceReadTest)
{
    ...
    const GLColor clearColor = GLColor::green;
    glClearColor(clearColor.R, clearColor.G, clearColor.B, clearColor.A);
    // triggers fail    glClear(GL_COLOR_BUFFER_BIT);
    glFinish();
    ASSERT_GL_NO_ERROR() << "glClear/glFinish failed";
    EXPECT_TRUE(eglMakeCurrent(mDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, context));
    ...
    // Fails    EXPECT_TRUE(checkBitMapRGBA32(clearColor, bitMapPtr, bitMapPitch));
...
// make current, check pixels, then swapbuffers - visually check.
