...
#define QUERY_AND_LOG_CAPABILITY_HELPER(enumValue, enumString, stream, index)              \
    {                                                                                      \
        GLint result;                                                                      \
        (index < 0) ? glGetIntegerv(enumValue, &result)                                    \
...
#define QUERY_AND_LOG_CAPABILITY(enum, stream) \
    QUERY_AND_LOG_CAPABILITY_HELPER(enum, #enum, stream, -1)

#define QUERY_AND_LOG_CAPABILITY_INDEXED(enum, stream, index) \
    QUERY_AND_LOG_CAPABILITY_HELPER(enum, #enum "[" #index "]", stream, index)
...
static void LogGles2Capabilities(std::ostream &stream)
{
    ...
    QUERY_AND_LOG_CAPABILITY(GL_MAX_VIEWPORT_DIMS, stream);
...
static void LogGles31Capabilities(std::ostream &stream)
{
    ...
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_COUNT, stream, 0);
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_COUNT, stream, 1);
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_COUNT, stream, 2);
    QUERY_AND_LOG_CAPABILITY(GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS, stream);
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_SIZE, stream, 0);
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_SIZE, stream, 1);
    QUERY_AND_LOG_CAPABILITY_INDEXED(GL_MAX_COMPUTE_WORK_GROUP_SIZE, stream, 2);
...
TEST_P(EGLPrintEGLinfoTest, PrintGLESCapabilitiesInfo)
{
    std::cout << std::endl << "Capibility name, value" << std::endl << std::endl;
