...
TEST_P(EGLRobustnessTest, ContextResetOnFragmentShader)
{
    ...
    ANGLE_SKIP_TEST_IF(!IsEGLDisplayExtensionEnabled(mDisplay, "EGL_KHR_create_context"));
    ANGLE_SKIP_TEST_IF(
...
void main (void)
{
    ...
    // delibrately pass in -1 to u_index to test robustness extension protects write out of bound
    GLint index = -1;
    ...
    // When command buffers are submitted to GPU, if robustness is working properly, the fragment
    // shader will not suffer from write out-of-bounds issue, which resulted in context reset and
    // context loss.
    glFinish();
