...
// on Pixel 6
TEST_P(EGLRobustnessTest, ContextResetOnInvalidLocalShaderVariableAccess)
{
    ANGLE_SKIP_TEST_IF(!mInitialized);
    ...
    // Test that robust access works with EGL_LOSE_CONTEXT_ON_RESET_EXT as the
    // EGL_CONTEXT_OPENGL_RESET_NOTIFICATION_STRATEGY_EXT
    createRobustContext(true);
    invalidShaderLocalVariableAccess();

    // Test that robust access works with EGL_NO_RESET_NOTIFICATION_EXT as the
    // EGL_CONTEXT_OPENGL_RESET_NOTIFICATION_STRATEGY_EXT
