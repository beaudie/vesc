    ...
    for (EGLint renderer : renderers)
    {
        ...
        for (size_t configIndexA = 0; configIndexA < configs.size(); ++configIndexA)
        {
            ...
            for (size_t configIndexB = 0; configIndexB < configs.size(); ++configIndexB)
            {
                ...
                std::cerr << "ECCT WindowDifferentConfig configIndexA=" << configIndexA
                          << " configIndexB=" << configIndexB << " randomFloat=" << f
                          << " kSkipP=" << kSkipP << "\n";
                if (f < kSkipP)
                    continue;
        ...
        for (size_t configIndexA = 0; configIndexA < configs.size(); ++configIndexA)
        {
            ...
            for (size_t configIndexB = 0; configIndexB < configs.size(); ++configIndexB)
            {
                ...
                auto f = rng.randomFloat();
                std::cerr << "ECCT PbufferDifferentConfig configIndexA=" << configIndexA
                          << " configIndexB=" << configIndexB << " randomFloat=" << f
                          << " kSkipP=" << kSkipP << "\n";
                if (f < kSkipP)
                    continue;
