...
TEST_P(EGLQueryStringIndexedTest, LargeIndex)
{
...
TEST_P(EGLQueryStringIndexedTest, GetNames)
{
    EGLDisplay dpy              = getEGLWindow()->getDisplay();
    egl::Display *display       = static_cast<egl::Display *>(dpy);
    ...
    {
        // Test that the actual pointer is still pointing to the static data.
        EXPECT_EQ(features[i]->name, eglQueryStringiANGLE(dpy, EGL_WORKAROUND_NAME_ANGLE, i));
        printf("%p\n", features[i]->name);
        // Also test that the strings are equal
        // EXPECT_STREQ(features[i]->name, eglQueryStringiANGLE(dpy, EGL_WORKAROUND_NAME_ANGLE, i));

        // EXPECT_EQ(FeatureCategoryToString(features[i]->category), eglQueryStringiANGLE(dpy,
        // EGL_WORKAROUND_CATEGORY_ANGLE, i));
        // EXPECT_STREQ(FeatureCategoryToString(features[i]->category), eglQueryStringiANGLE(dpy,
        // EGL_WORKAROUND_CATEGORY_ANGLE, i));

        EXPECT_EQ(features[i]->description,
                  eglQueryStringiANGLE(dpy, EGL_WORKAROUND_DESCRIPTION_ANGLE, i));
        // EXPECT_STREQ(features[i]->description, eglQueryStringiANGLE(dpy,
        // EGL_WORKAROUND_DESCRIPTION_ANGLE, i));

        EXPECT_EQ(features[i]->bug, eglQueryStringiANGLE(dpy, EGL_WORKAROUND_BUG_ANGLE, i));
