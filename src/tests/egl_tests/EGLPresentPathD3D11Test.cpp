...
class EGLPresentPathD3D11 : public testing::TestWithParam<PlatformParameters>
{
  protected:
    ...
    void checkPixelsUsingD3D(bool usingPresentPathFast)
    {
        ...
        EXPECT_EQ(expectedTopLeftPixel.B, byteData[0]);
        EXPECT_EQ(expectedTopLeftPixel.G, byteData[1]);
        EXPECT_EQ(expectedTopLeftPixel.R, byteData[2]);
        EXPECT_EQ(expectedTopLeftPixel.A, byteData[3]);

        EXPECT_EQ(expectedTopRightPixel.B, byteData[(mWindowWidth - 1) * 4 + 0]);
        EXPECT_EQ(expectedTopRightPixel.G, byteData[(mWindowWidth - 1) * 4 + 1]);
        EXPECT_EQ(expectedTopRightPixel.R, byteData[(mWindowWidth - 1) * 4 + 2]);
        EXPECT_EQ(expectedTopRightPixel.A, byteData[(mWindowWidth - 1) * 4 + 3]);

        EXPECT_EQ(expectedBottomLeftPixel.B, byteData[(mWindowWidth - 1) * 4 * mWindowWidth + 0]);
        EXPECT_EQ(expectedBottomLeftPixel.G, byteData[(mWindowWidth - 1) * 4 * mWindowWidth + 1]);
        EXPECT_EQ(expectedBottomLeftPixel.R, byteData[(mWindowWidth - 1) * 4 * mWindowWidth + 2]);
        EXPECT_EQ(expectedBottomLeftPixel.A, byteData[(mWindowWidth - 1) * 4 * mWindowWidth + 3]);
