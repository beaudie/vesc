...
class EGLPresentPathD3D11 : public testing::TestWithParam<PlatformParameters>
{
  protected:
    ...
    void checkPixelsUsingD3D(bool usingPresentPathFast)
    {
        ...
        GLColor expectedTopLeftPixel     = GLColor(255, 0, 0, 255);    // Red
        GLColor expectedTopRightPixel    = GLColor(0, 255, 0, 255);    // Green
        GLColor expectedBottomLeftPixel  = GLColor(0, 0, 255, 255);    // Blue
        GLColor expectedBottomRightPixel = GLColor(255, 255, 0, 255);  // Red + Green
        ...
        // Note that the texture is in BGRA format
        EXPECT_EQ(expectedTopLeftPixel.B, byteData[0].R);
        EXPECT_EQ(expectedTopLeftPixel.G, byteData[0].G);
        EXPECT_EQ(expectedTopLeftPixel.R, byteData[0].B);
        EXPECT_EQ(expectedTopLeftPixel.A, byteData[0].A);

        EXPECT_EQ(expectedTopRightPixel.B, byteData[(mWindowWidth - 1)].R);
        EXPECT_EQ(expectedTopRightPixel.G, byteData[(mWindowWidth - 1)].G);
        EXPECT_EQ(expectedTopRightPixel.R, byteData[(mWindowWidth - 1)].B);
        EXPECT_EQ(expectedTopRightPixel.A, byteData[(mWindowWidth - 1)].A);

        EXPECT_EQ(expectedBottomLeftPixel.B, byteData[(mWindowWidth - 1) * mWindowWidth].R);
        EXPECT_EQ(expectedBottomLeftPixel.G, byteData[(mWindowWidth - 1) * mWindowWidth].G);
        EXPECT_EQ(expectedBottomLeftPixel.R, byteData[(mWindowWidth - 1) * mWindowWidth].B);
        EXPECT_EQ(expectedBottomLeftPixel.A, byteData[(mWindowWidth - 1) * mWindowWidth].A);

        EXPECT_EQ(expectedBottomRightPixel.B,
                  byteData[(mWindowWidth - 1) * mWindowWidth + (mWindowWidth - 1)].R);
        EXPECT_EQ(expectedBottomRightPixel.G,
                  byteData[(mWindowWidth - 1) * mWindowWidth + (mWindowWidth - 1)].G);
        EXPECT_EQ(expectedBottomRightPixel.R,
                  byteData[(mWindowWidth - 1) * mWindowWidth + (mWindowWidth - 1)].B);
        EXPECT_EQ(expectedBottomRightPixel.A,
                  byteData[(mWindowWidth - 1) * mWindowWidth + (mWindowWidth - 1)].A);
