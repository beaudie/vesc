...
class EGLDirectCompositionTest : public ANGLETest
{
  protected:
    ...
    void SetUp() override
    {
        if (!IsWindows10OrGreater() || !mRoHelper.WinRtAvailable())
        {
            return;
        }
        ...
        auto hr = mRoHelper.GetStringReference(RuntimeClass_Windows_UI_Composition_Compositor, &act, &header);
                       ...
                       EGLSurface &surface)
    {
        ...
        const EGLint surfaceAttributes[] = {EGL_WINDOWS_UI_COMPOSITION_ANGLE, EGL_TRUE, EGL_NONE};

        surface = eglCreateWindowSurface(mEglDisplay, mEglConfig,
                                         static_cast<EGLNativeWindowType>((void *)visual.Get()),
                                         surfaceAttributes);
...
{
    // Only attempt this test when not on Win7
    ANGLE_SKIP_TEST_IF(!mRoHelper.WinRtAvailable());
...
TEST_P(EGLDirectCompositionTest, RenderSolidColor)
{
    ...
    // ensure user/DWM have a chance to paint the window and kick it to the top of the desktop zorder before we attempt to sample
    angle::Sleep(200);
