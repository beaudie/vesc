...
class EGLImageDmaBufImportTest : public ANGLETest
{
  protected:
    ...
    EGLConfig mConfig         = EGL_NO_CONFIG;
};
                          ...
                          uint32_t version)
{
    ...
    {
        wl->wlompositor = wl_registry_bind(registry, id, &wl_compositor_interface, 1);
    }
    else if (strcmp(interface, "xdg_wm_base") == 0)
    {
        wl->xdgWmBase = wl_registry_bind(registry, id, &xdg_wm_base_interface, 1);
    ...
    else if (strcmp(interface, "zwp_linux_dmabuf_v1") == 0)
    {
        if (version < 3)
            return;
        wl->wpDmabuf = wl_registry_bind(registry, id, &zwp_linux_dmabuf_v1_interface, 3);
...
void EGLImageDmaBufImportTest::testSetUp()
{
    ...
    mWl.planeInfo.resize(planeCount);
    for (uint32_t plane = 0; plane < planeCount; ++plane)
    {
        ...
        int ret = drmPrimeHandleToFD(mWl.drmFd, handle.u32, 0, &mWl.planeInfo[plane].fd);
        ASSERT(ret >= 0 && mWl.planeInfo[plane].fd >= 0);

        mWl.planeInfo[plane].stride = gbm_bo_get_stride_for_plane(mWl.gbmBo, plane);
        mWl.planeInfo[plane].offset = gbm_bo_get_offset(mWl.gbmBo, plane);

        zwp_linux_buffer_params_v1_add(params, mWl.planeInfo[plane].fd, plane,
                                       mWl.planeInfo[plane].offset, mWl.planeInfo[plane].stride,
                                       mWl.bufferModifier >> 32, mWl.bufferModifier & 0xFFFFFFFF);
    }
