  ...
  protected:
    EGLDeviceCreationTest()
        : mD3D11Available(false),
          ...
          mCreateDeviceANGLE(nullptr),
          mReleaseDeviceANGLE(nullptr),
    ...
    void SetUp() override
    {
        ...
        if (strstr(extensionString, "EGL_ANGLE_device_creation"))
        {
            if (strstr(extensionString, "EGL_ANGLE_device_creation_d3d11"))
            {
                ...
                ASSERT_NE(nullptr, mCreateDeviceANGLE);
                ASSERT_NE(nullptr, mReleaseDeviceANGLE);
                ASSERT_NE(nullptr, mQueryDeviceAttribEXT);
            }
        }
    }
    ...
    PFNEGLCREATEDEVICEANGLEPROC mCreateDeviceANGLE;
    PFNEGLRELEASEDEVICEANGLEPROC mReleaseDeviceANGLE;
...
TEST_F(EGLDeviceCreationTest, BasicD3D11Device)
{
    ...
    EGLDeviceEXT eglDevice =
        mCreateDeviceANGLE(EGL_D3D11_DEVICE_ANGLE, reinterpret_cast<void *>(mDevice), nullptr);
    ASSERT_NE(EGL_NO_DEVICE_EXT, eglDevice);
    ASSERT_EGL_SUCCESS();

    EGLAttrib deviceAttrib;
    mQueryDeviceAttribEXT(eglDevice, EGL_D3D11_DEVICE_ANGLE, &deviceAttrib);
    ASSERT_EGL_SUCCESS();
...
TEST_F(EGLDeviceCreationTest, InvalidD3D11Device)
{
    ...
    EGLDeviceEXT eglDevice = mCreateDeviceANGLE(EGL_D3D11_DEVICE_ANGLE,
                                                reinterpret_cast<void *>(mDeviceContext), nullptr);
...
TEST_F(EGLDeviceCreationTest, AnyD3D9Device)
{
    ...
    EGLDeviceEXT eglDevice = mCreateDeviceANGLE(EGL_D3D9_DEVICE_ANGLE,
                                                reinterpret_cast<void *>(&fakeD3DDevice), nullptr);
    EXPECT_EQ(EGL_NO_DEVICE_EXT, eglDevice);
    EXPECT_EGL_ERROR(EGL_BAD_ATTRIBUTE);
}
