...
// Draw a predictable pattern (for testing pre-rotation) into an FBO, and the use glBlitFramebuffer
...
TEST_P(EGLPreRotationLargeSurfaceTest, OrientedWindowWithBlitFramebuffer)
{
    ...
    EXPECT_PIXEL_COLOR_EQ(mSize - 1, 0, GLColor::red);
    EXPECT_PIXEL_COLOR_EQ(mSize - 1, mSize - 1, GLColor::yellow);
    EXPECT_PIXEL_COLOR_EQ(127, 127, GLColor(127, 127, 0, 255));
    ASSERT_GL_NO_ERROR();
    ...
    EXPECT_PIXEL_COLOR_EQ(xOffset + 127, yOffset + 127, GLColor(127, 127, 0, 255));
    ASSERT_GL_NO_ERROR();
