...
TEST_P(EGLPreRotationLargeSurfaceTest, OrientedWindowWithBlitFramebuffer)
{
    ...
    constexpr GLColor kMidWayColor = GLColor(127, 127, 0, 255);
    constexpr GLuint kMidWayCoord  = 127;
    ...
    EXPECT_PIXEL_COLOR_EQ(mSize - 1, 0, GLColor::red);
    EXPECT_PIXEL_COLOR_EQ(mSize - 1, mSize - 1, GLColor::yellow);
    EXPECT_PIXEL_COLOR_EQ(kMidWayCoord, kMidWayCoord, kMidWayColor);
    ASSERT_GL_NO_ERROR();
    ...
    EXPECT_PIXEL_COLOR_EQ(xOffset + kMidWayCoord, yOffset + kMidWayCoord, kMidWayColor);
    ASSERT_GL_NO_ERROR();
