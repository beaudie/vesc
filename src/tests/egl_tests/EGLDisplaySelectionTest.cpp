...
class EGLDisplaySelectionTestVulkanDeviceUUID : public EGLDisplaySelectionTestNoFixture
{
  ...
  protected:
    void initializeDisplayWithDeviceUUID(EGLDisplay *display,
                                         ...
                                         bool expectValidDisplay)
    {
        ...
        displayAttributes.push_back(EGL_PLATFORM_ANGLE_TYPE_ANGLE);
        displayAttributes.push_back(platformType);
        displayAttributes.push_back(EGL_PLATFORM_ANGLE_VULKAN_DEVICE_UUID);
        displayAttributes.push_back(reinterpret_cast<EGLAttrib>(deviceId));
        displayAttributes.push_back(EGL_NONE);
...
// ensuring that it is correctly rejected by everything but the Vulkan backend,
// and that the intended device is used for the newly created context.
TEST_P(EGLDisplaySelectionTestVulkanDeviceUUID, ValidDeviceUUID)
{
    ...
    {
        uint8_t const *targetDeviceUUID = mSystemInfo.gpus[i].deviceUUID;
        ...
        initializeDisplayWithDeviceUUID(&display, targetDeviceUUID, isVulkan);
