...
TEST_F(IfTest, BitShiftLeftOperatorNegativeOffset)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_UNDEFINED_SHIFT,
        pp::SourceLocation(0, 1), "2 << -1"));
...
TEST_F(IfTest, BitShiftLeftOperatorOffset32)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_UNDEFINED_SHIFT,
        pp::SourceLocation(0, 1), "2 << 32"));

    pp::Token token;
    mPreprocessor.lex(&token);
}

// Undefined shift: left hand side of shift is negative.
