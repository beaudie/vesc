...
TEST_F(VersionTest, Valid)
{
    ...
    EXPECT_CALL(mDirectiveHandler,
                handleVersion(pp::SourceLocation(0, 1), 200, pp::ClientType::GLES));
    // No error or warning.
...
TEST_F(VersionTest, GLSpec)
{
    ...
    using testing::_;
    EXPECT_CALL(mDirectiveHandler,
                handleVersion(pp::SourceLocation(0, 1), 330, pp::ClientType::GL));
    // No error or warning.
    EXPECT_CALL(mDiagnostics, print(_, _, _)).Times(0);
...
TEST_F(VersionTest, AfterComments)
{
    ...
    // Directive successfully parsed.
    EXPECT_CALL(mDirectiveHandler,
                handleVersion(pp::SourceLocation(0, 3), 200, pp::ClientType::GLES));
    // No error or warning.
    EXPECT_CALL(mDiagnostics, print(_, _, _)).Times(0);
