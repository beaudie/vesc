...
ADB_PATH = None  # FIXME: global set via flag
...
def _adb_path():
    return ADB_PATH if ADB_PATH else adb_wrapper.AdbWrapper.GetAdbPath()
...
def run(cmd, log=False):
    logging.info('android_helper cmd %s', cmd)
    out = subprocess.check_output(cmd)
    if log:
        for ln in out.decode('utf-8').split('\n'):
            logging.info('android_helper out=%s', ln)
    return out
...
def get_adb_root():
    ...
    for _ in range(20):
        time.sleep(0.5)
        try:
            id_out = run([adb_path, 'shell', 'id']).decode('ascii')
            if 'uid=0(root)' in id_out:
                return
        except Exception:
            continue
    raise Exception("can't get adb root")
...
def install_apk(suite):
    ...
    logging.info('android_helper adb_path=%s apk_path=%s apk_size=%s' %
    ...
    #logging.info('android_helper permissions=%s', permissions)
...
def _read_device_file(adb_path, path):
    ...
    out = run([adb_path, 'exec-out', 'cat %s' % path])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adb-path', type=str)
