...
class InstancingTest : public ANGLETest
{
  protected:
    ...
    {
        const std::string fs = SHADER_SOURCE(precision mediump float;
...
{
    const std::string vs =
        SHADER_SOURCE(attribute vec3 a_instancePos; attribute vec3 a_position;
                      void main() { gl_Position = vec4(a_position.xyz + a_instancePos.xyz, 1.0); });
...
{
    const std::string vs =
        SHADER_SOURCE(attribute vec3 a_position; attribute vec3 a_instancePos;
                      void main() { gl_Position = vec4(a_position.xyz + a_instancePos.xyz, 1.0); });
...
{
    const std::string vs = SHADER_SOURCE(
        attribute vec3 a_position; attribute vec3 a_instancePos; uniform vec3 u_offset;
        void main() { gl_Position = vec4(a_position.xyz + a_instancePos.xyz + u_offset, 1.0); });
...
// Verify that VertexAttribDivisor is equivilant to the combination of several Vertex Attrib Binding
// APIs.
TEST_P(InstancingTestES31, EquivilantAttribDivisor)
{
    const std::string vs =
        "attribute vec3 a_instancePos;\n"
        "attribute vec3 a_position;\n"
    ...
    GLint positionLoc    = glGetAttribLocation(program, "a_position");
    GLint instancePosLoc = glGetAttribLocation(program, "a_instancePos");
    ASSERT_NE(mBinding, instancePosLoc);
    ...
    glBufferData(GL_ARRAY_BUFFER, mInstances.size() * kFloatStride, mInstances.data(),
                 GL_STATIC_DRAW);
    glBindVertexBuffer(mBinding, buffer2, 0, kFloatStride * 3);

    // The attribute indexed instancePosLoc is using the binding indexed mBinding.
    glVertexAttribFormat(instancePosLoc, 3, GL_FLOAT, GL_FALSE, 0);
    glVertexAttribBinding(instancePosLoc, mBinding);
    glEnableVertexAttribArray(instancePosLoc);

    // Enable instancing
    glVertexBindingDivisor(mBinding, 1);
    ...
    constexpr size_t kOffset3 = 12u;
    const size_t kDataSize    = mInstances.size() * kFloatStride;
    GLuint buffer3;
    glGenBuffers(1, &buffer3);
    glBindBuffer(GL_ARRAY_BUFFER, buffer3);
    glBufferData(GL_ARRAY_BUFFER, kDataSize + kOffset3, nullptr, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, kOffset3, kDataSize, mInstances.data());
    glBindVertexBuffer(instancePosLoc, buffer3, kOffset3, kFloatStride * 3);

    // Following two Vertex Attrib Binding APIs should be merged into VertexAttribDivisor in the
    // OpenGL back-end.
    glVertexBindingDivisor(instancePosLoc, 1);
    glVertexAttribBinding(instancePosLoc, instancePosLoc);
