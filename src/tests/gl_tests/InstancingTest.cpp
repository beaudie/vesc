...
class InstancingTest : public ANGLETest
{
  protected:
        ...
        bool buffer)               // true: use instance data in buffer, false: in client memory
    {
        ...
        ANGLE_SKIP_TEST_IF(IsD3D9() && IsAMD() && points && !indexed);
        ANGLE_SKIP_TEST_IF(IsD3D11_FL93() && indexed && points && attribZeroInstanced);
        ANGLE_SKIP_TEST_IF(IsD3D11_FL93() && indexed && points && buffer && !attribZeroInstanced);
        ANGLE_SKIP_TEST_IF(IsD3D11_FL93() && !indexed && !points && buffer && offset && attribZeroInstanced);
        ANGLE_SKIP_TEST_IF(IsD3D11_FL93() && numInstance == 1 && divisor == 5 && !attribZeroInstanced && !points && !indexed && !offset && buffer);
