...
namespace {
    enum Geometry { Quad, Point };
    enum Storage { Buffer, Memory };
    enum Draw { Indexed, NonIndexed };
}
...
class InstancingTest : public ANGLETest
{
  protected:
    ...
    void runTest(
        ...
        unsigned divisor,
        const int instanceAttrib,   // which attribute is instanced: 0 or 1
        Geometry geometry,
        Draw draw,
        Storage storage,
        unsigned offset)             // offset for DrawArrays
    {
        ...
        //XXX according to old tests this will fail - runTest(4, 1, 1, Quad, NonIndexed, Buffer, 4);
        //ANGLE_SKIP_TEST_IF(IsD3D11_FL93() && instanceAttrib == 1 && geometry == Quad && draw == NonIndexed && storage == Buffer && offset > 0);
        ...
        assert(instanceAttrib == 0 || instanceAttrib == 1);
...
// The following tests produce each value of 'lastDrawn' in runTest() from 1 to kMaxDrawn a few different ways.
TEST_DIVISOR(1, 1)
TEST_DIVISOR(1, 2)
TEST_DIVISOR(2, 1)
TEST_DIVISOR(3, 1)
...
{
    //XXX runTest(4, 1, 0, Quad, Indexed, Memory, 0);
}
...
{
    //XXX runTest(4, 1, 1, Quad, Indexed, Memory, 0);
}
...
{
    //XXX runTest(4, 1, 1, Quad, NonIndexed, Buffer, 4);
}
...
TEST_P(InstancingTestPoints, DrawArrays)
{
    ...
    //runTest(4, 1, 1, Point, NonIndexed, Buffer, 0);
}
...
TEST_P(InstancingTestPoints, DrawElements)
{
    ...
    //runTest(4, 1, 1, Point, Indexed, Buffer,0);
}
