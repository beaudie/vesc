...
struct MultiviewRenderTestParams final : public MultiviewImplementationParams
{
    MultiviewRenderTestParams(int samples,
        ...
        : MultiviewImplementationParams(implementationParams), mIsMultiview(true), mSamples(samples)
    {}
    bool mIsMultiview;
    int mSamples;
};
...
{
  protected:
    MultiviewFramebufferTestBase(const PlatformParameters &params, bool isMultiview, int samples)
    ...
    void updateFBOs(int viewWidth, int height, int numViews, int numLayers, int baseViewIndex)
    {
        ...
        CreateMultiviewBackingTextures(mIsMultiview, mSamples, viewWidth, height, numLayers,
        ...
        AttachMultiviewTextures(GL_DRAW_FRAMEBUFFER, mIsMultiview, viewWidth, numViews,
    ...
    void resolveMultisampledFBO()
    {
        ...
        if (mResolveFramebuffer.empty())
        {
            ...
            CreateMultiviewBackingTextures(mIsMultiview, 0, mViewWidth, mViewHeight, numLayers,
                            ...
                            public ::testing::TestWithParam<MultiviewRenderTestParams>
{
  protected:
    MultiviewRenderTest()
        : MultiviewFramebufferTestBase(GetParam(), GetParam().mIsMultiview, GetParam().mSamples)
    {}
...
{
  protected:
    MultiviewLayeredRenderTest() : MultiviewFramebufferTestBase(GetParam(), true, 0) {}
