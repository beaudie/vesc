...
class MultiviewRenderTestBase : public MultiviewDrawTest
{
  protected:
    ...
    void createFBO(int viewWidth, int height, int numViews)
    {
        ...
        mColorTexture    = GLTexture();
        mDepthTexture    = GLTexture();
        mDrawFramebuffer = GLFramebuffer();
...
TEST_P(MultiviewRenderTest, ProgramRelinkUpdatesAttribDivisor)
{
    ...
    auto generateVertexShaderSource = [&vsSource](int numViews) {
        vsSource =
            "#version 300 es\n"
            "#extension GL_OVR_multiview2 : require\n"
            "layout(num_views = " +
    ...
    generateVertexShaderSource(kNumViews);
    ANGLE_GL_PROGRAM(program, vsSource, fsSource);
    glUseProgram(program);
...

    {
        ...
        GLint xOffsetLoc = glGetAttribLocation(program, "vOffsetX");
        glVertexAttribPointer(xOffsetLoc, 1, GL_FLOAT, GL_FALSE, 0, 0);
...

    {
        ...
        generateVertexShaderSource(kNewNumViews);
