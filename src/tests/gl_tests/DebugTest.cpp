...
TEST_P(DebugTest, GetPointer)
{
    ...
    EXPECT_EQ(reinterpret_cast<void *>(&messages), userData);
}
...
TEST_P(DebugTest, ObjectLabels)
{
    ...
    glGetObjectLabelKHR(GL_RENDERBUFFER, renderbuffer, labelBuf.size(), &labelLengthBuf,
                        labelBuf.data());
...
TEST_P(DebugTest, ObjectPtrLabels)
{
    ...
    std::vector<char> labelBuf(label.length() + 1);
    GLsizei labelLengthBuf = 0;
    glGetObjectPtrLabelKHR(sync, labelBuf.size(), &labelLengthBuf, labelBuf.data());
