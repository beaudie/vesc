  ...
  protected:
    ...
    virtual void SetUp()
    {
        ...
        mProgram = CompileProgramWithTransformFeedback(
            vertexShaderSource, fragmentShaderSource, transformFeedbackVaryings, GL_SEPARATE_ATTRIBS);
        if (mProgram == 0)
        {
            FAIL() << "shader compilation failed.";
        }
...
TEST_P(ProgramBinaryTransformFeedbackTest, GetTransformFeedbackVarying)
{
    ...
    EXPECT_EQ(true, (linkStatus != 0));

    if (linkStatus != 0)
    {
        // If the link was successful, then query information about the transform feedback varying
        char varyingName[64];
        ...
        EXPECT_EQ(GL_FLOAT_VEC4, varyingType);
    }
