...
TEST_P(ProgramBinaryES3Test, UniformBlockBinding)
{
    ...
    std::vector<uint8_t> binary(programLength);
    glGetProgramBinaryOES(program.get(), programLength, &readLength, &binaryFormat, binary.data());
    ASSERT_GL_NO_ERROR();

    EXPECT_EQ(static_cast<GLsizei>(programLength), readLength);

    // Load a new program with the binary and draw.
    ANGLE_GL_BINARY_PROGRAM(binaryProgram, binary, binaryFormat);
...
ANGLE_INSTANTIATE_TEST(ProgramBinaryES3Test, ES3_D3D11());
