...
class BuiltinVariableVertexIdTest : public ANGLETest
{
  protected:
    ...
    void SetUp() override
    {
        ...
        ASSERT_NE(mPositionLocation, -1);
        mExpectedIdLocation = glGetAttribLocation(mProgram, "expectedID");
        ASSERT_NE(mExpectedIdLocation, -1);
    ...
    void runTest(GLuint drawMode, const std::vector<GLint> &indices, int count)
    {
        ...
        for (int y = 0; y < getWindowHeight(); y++)
        {
            for (int x = 0; x < getWindowWidth(); x++)
            {
                ...
                EXPECT_EQ(0, pixel[0]);
                EXPECT_TRUE(pixel[1] == 0 || pixel[1] == 255);
                EXPECT_EQ(0, pixel[2]);
    ...
    std::vector<GLint> makeConsecutiveIndices(int n) const
    {
