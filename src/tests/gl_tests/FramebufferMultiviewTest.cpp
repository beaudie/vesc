...
TEST_P(FramebufferMultiviewTest, InvalidMultiviewideBySideArguments)
{
    if (!requestMultiviewExtension())
    {
        return;
...
TEST_P(FramebufferMultiviewTest, SideBySideState)
{
    if (!requestMultiviewExtension())
    {
        return;
    ...
    GLint internalViewportOffsets[4] = {-1, -1, -1, -1};
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                          GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_VIEWPORT_OFFSETS_ANGLE,
