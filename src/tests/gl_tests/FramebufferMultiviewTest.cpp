...
TEST_P(FramebufferMultiviewTest, DefaultState)
{
    ...
    const GLint kViewportOffsets[2] = {-1, -1};
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                          GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_VIEWPORT_OFFSETS_ANGLE,
                                          &kViewportOffsets[0]);
    ASSERT_GL_NO_ERROR();
    EXPECT_EQ(0, kViewportOffsets[0]);
    EXPECT_EQ(0, kViewportOffsets[1]);
}
...
TEST_P(FramebufferMultiviewTest, SideBySideClear)
{
    ...
    glFramebufferTextureMultiviewSideBySideANGLE(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, tex, 0,
                                                 2, &kViewportOffsets[0]);
