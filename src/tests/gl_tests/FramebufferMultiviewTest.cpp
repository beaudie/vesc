...
TEST_P(FramebufferMultiviewTest, DefaultState)
{
    ...
    mTexture2D = CreateTexture2D(GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE);
    ASSERT_GL_NO_ERROR();
...
TEST_P(FramebufferMultiviewTest, IncompleteViewTargetsSideBySide)
{
    ...
    // Set the 0th attachment and keep it as it is till the end of the test. The 1st attachment will
    // me modified to change the framebuffer's status.
    glFramebufferTextureMultiviewSideBySideANGLE(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, mTexture2D,
                                                 0, 2, &viewportOffsets[0]);
    ASSERT_GL_NO_ERROR();
    ...
    // Color attachment 1.
    {
        ...
        glFramebufferTextureMultiviewSideBySideANGLE(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1,
                                                     mTexture2D, 0, 1, &viewportOffsets[0]);
        ASSERT_GL_NO_ERROR();
        EXPECT_EQ(GL_FRAMEBUFFER_INCOMPLETE_VIEW_TARGETS_ANGLE,
                  glCheckFramebufferStatus(GL_FRAMEBUFFER));

        // Test framebuffer completeness when the viewport offsets differ.
        glFramebufferTextureMultiviewSideBySideANGLE(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1,
                                                     mTexture2D, 0, 2, &otherViewportOffsets[0]);
        ASSERT_GL_NO_ERROR();
        EXPECT_EQ(GL_FRAMEBUFFER_INCOMPLETE_VIEW_TARGETS_ANGLE,
                  glCheckFramebufferStatus(GL_FRAMEBUFFER));

        // Test that framebuffer is incomplete when attachment layouts differ.
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, mTexture2D, 0);
        ASSERT_GL_NO_ERROR();
        EXPECT_EQ(GL_FRAMEBUFFER_INCOMPLETE_VIEW_TARGETS_ANGLE,
                  glCheckFramebufferStatus(GL_FRAMEBUFFER));
        ...
        // are the same.
        glFramebufferTextureMultiviewSideBySideANGLE(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1,
                                                     mTexture2D, 0, 2, &viewportOffsets[0]);
        ASSERT_GL_NO_ERROR();
