...
// test has been used, depth clear does not get optimized out. It also tests that if the depth
// buffer has not been used, the rendering is still correct.
TEST_P(DiscardFramebufferEXTTest, DepthClearOptimizationShouldKickin)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_discard_framebuffer"));
    // TODO: fix crash issue. http://anglebug.com/4141
    ANGLE_SKIP_TEST_IF(IsD3D11());
...
TEST_P(DiscardFramebufferEXTTest, DepthClearOptimizationShouldNotKickin)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_discard_framebuffer"));
