...
TEST_P(DepthStencilFormatsTest, DepthStencilReadback)
{
    ...
    {
        bool hasFloatDepth = (type.type == GL_FLOAT);
        ...
        if (type.type == GL_DEPTH_ATTACHMENT)
        {
            if (shouldHaveReadDepthSupport)
            {
                EXPECT_GL_NO_ERROR();
                if (hasFloatDepth)
                {
                    ...
                    ASSERT_TRUE((pixels[0] >= minValues[0]) && (pixels[0] <= maxValues[0]) &&
                                ...
                                (pixels[0 + destRes] <= maxValues[1]) &&
                                (pixels[1] >= minValues[2]) && (pixels[1] <= maxValues[2]) &&
                                (pixels[1 + destRes] >= minValues[3]) &&
