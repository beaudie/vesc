...
void DepthStencilFormatsTestBase::depthStencilReadbackCase(const ReadbackTestParam &type)
{
    ...
    // unorm is a helper for converting floating point values to UNORM.
    auto unorm = [](int bits, float x) {
        uint32_t maxUNorm = (1 << bits) - 1;
        return static_cast<uint32_t>(static_cast<float>(maxUNorm) * x);
    };
    ...
    if (hasReadDepthSupport())
    {
        ...
        else
        {
            ...
            ASSERT_NEAR(pixels[0], unorm(16, d00), kEpsilon);
            ASSERT_NEAR(pixels[0 + destRes], unorm(16, d01), kEpsilon);
            ASSERT_NEAR(pixels[1], unorm(16, d10), kEpsilon);
            ASSERT_NEAR(pixels[1 + destRes], unorm(16, d11), kEpsilon);
        }
    }
    else
    {
    ...
    if (hasStencil)
    {
        ...
        if (hasReadDepthStencilSupport())
        {
            ...
            ASSERT_NEAR(pixels[0].d24(), unorm(24, d00), kEpsilon);
            ASSERT_NEAR(pixels[0 + destRes].d24(), unorm(24, d01), kEpsilon);
            ASSERT_NEAR(pixels[1].d24(), unorm(24, d10), kEpsilon);
            ASSERT_NEAR(pixels[1 + destRes].d24(), unorm(24, d11), kEpsilon);
