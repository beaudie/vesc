...
void main()
{
    ...
    for (auto const &type : types)
    {
        ...
        EXPECT_EQ((GLuint)GL_NO_ERROR, glGetError());
        ...
        for (auto const &target : targets)
        {
            ...
            EXPECT_EQ((GLuint)GL_INVALID_OPERATION, glGetError());
        }
        ...
        for (auto const &filterMode : filterModes)
        {
            ...
            if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
            {
                ...
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                       colorTex, 0);
                EXPECT_EQ((GLuint)GL_FRAMEBUFFER_COMPLETE,
                          glCheckFramebufferStatus(GL_FRAMEBUFFER));
            }

            EXPECT_EQ((GLuint)GL_FRAMEBUFFER_COMPLETE, glCheckFramebufferStatus(GL_FRAMEBUFFER));
            ...
            if (type.attachment == GL_DEPTH_ATTACHMENT)
            {
                ...
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D,
                                       tex, 0);
                EXPECT_NE((GLuint)GL_NO_ERROR, glGetError());
                EXPECT_NE((GLuint)GL_FRAMEBUFFER_COMPLETE,
                          glCheckFramebufferStatus(GL_FRAMEBUFFER));
                glClear(GL_DEPTH_BUFFER_BIT);
                EXPECT_EQ((GLuint)GL_INVALID_FRAMEBUFFER_OPERATION, glGetError());
                glBindFramebuffer(GL_FRAMEBUFFER, 0);
                EXPECT_EQ((GLuint)GL_NO_ERROR, glGetError());
            }
        }
