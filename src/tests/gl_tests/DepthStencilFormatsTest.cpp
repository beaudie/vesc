...
{};
// LAO
TEST_P(DepthStencilFormatsTestES31, DepthTextureFragmentShaderReadThenComputeShaderRead)
{
...
void main(void) {
    ...
    // Clear depth texture to 0.5f
    GLFramebuffer framebuffer;
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    glFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthTexture,
                           0);
    ...
    EXPECT_NEAR(depthValue, actualDepthValue / (float)UINT_MAX, 0.0001f);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    // Sample the depth texture from fragment shader and verify
    glDepthMask(false);
    glDisable(GL_DEPTH_TEST);
    glDrawArrays(GL_TRIANGLES, 0, 6);
    EXPECT_PIXEL_COLOR_EQ(0, 0, GLColor((GLubyte)(depthValue * 255), 0u, 0u, 255u));

    // Sample the depth texture from fragment shader
    glDrawArrays(GL_TRIANGLES, 0, 6);
...
void main()
{
    ...
    EXPECT_NEAR(depthValue, ptr[0], 0.0001f);
    EXPECT_GL_NO_ERROR();
}
