...
class TextureMultisampleArrayWebGLTest : public TextureMultisampleTest
{
  protected:
    ...
    bool requestArrayExtension()
    {
        ...
        {
            std::cout << "Test skipped due to missing GL_ANGLE_texture_multisample_array."
                      << std::endl;
            return false;
        }
        return true;
...
TEST_P(TextureMultisampleArrayWebGLTest, TexStorage3DMultisample)
{
    ...
    if (getClientMajorVersion() >= 3 && getClientMinorVersion() >= 1)
    {
        ...
        glGetTexLevelParameteriv(GL_TEXTURE_2D_MULTISAMPLE_ARRAY_ANGLE, 0, GL_TEXTURE_WIDTH,
                                 &width);
        glGetTexLevelParameteriv(GL_TEXTURE_2D_MULTISAMPLE_ARRAY_ANGLE, 0, GL_TEXTURE_HEIGHT,
                                 &height);
        glGetTexLevelParameteriv(GL_TEXTURE_2D_MULTISAMPLE_ARRAY_ANGLE, 0, GL_TEXTURE_DEPTH,
                                 &depth);
