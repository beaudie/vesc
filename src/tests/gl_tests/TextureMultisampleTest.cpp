...
{
    if (!(IsD3D11() && IsWindows()))
    {
        return;
    }

    GLsizei max_samples = 0;
    glGetIntegerv(GL_MAX_SAMPLES, &max_samples);

    GLuint mTextures[max_samples];
    glGenTextures(max_samples, mTextures);
    ...
    {
        size_t indexKey = static_cast<size_t>(ceil(log2(i)));
        glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, mTextures[i - 1]);
        glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, i, GL_RGBA8, 1, 1, GL_FALSE);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE,
                               mTextures[i - 1], 0);
        ...
        for (int j = 0; j < i; j++)
        {
            glGetMultisamplefv(GL_SAMPLE_POSITION, j, samplePosition);
            EXPECT_EQ(samplePosition[0], kSamplePositions[indexKey][2 * j]);
            EXPECT_EQ(samplePosition[1], kSamplePositions[indexKey][2 * j + 1]);
        }
    }

    glDeleteTextures(max_samples, mTextures);
    ASSERT_GL_NO_ERROR();
}
