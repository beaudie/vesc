...
{
    os << "layout(binding=" << (decl.unit * 2) << ", " << decl.format
       << ") coherent volatile readonly highp uniform image2D " << decl.name << "_R;\n";
    os << "layout(binding=" << (decl.unit * 2 + 1) << ", " << decl.format
...
void PixelLocalStoragePrototype::endPixelLocalStorage()
{
    ...
    {
        glBindImageTexture(i * 2, 0, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8);
        glBindImageTexture(i * 2 + 1, 0, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8);
    }
...
TEST_P(PixelLocalStorageTest, RGBA)
{
    ...
    useProgram({}, R"(
    layout(binding=0, rgba8) coherent volatile writeonly highp uniform image2D framebuffer_R;
    void main() {
