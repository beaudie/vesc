...
void PixelLocalStoragePrototype::beginPixelLocalStorage(GLsizei n, const GLenum *loadOps)
{
    ...
    // Leave the textures attached during rendering as hints for OpenGL driver bugs. Detach detach
    // them otherwise because this generates Vulkan validation errors.
    if (!IsOpenGL())
    {
        for (int i = 0; i < n; ++i)
        {
