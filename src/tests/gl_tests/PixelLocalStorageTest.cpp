...
TEST_P(PixelLocalStorageValidationTest, EndAndBarrierANGLE)
{
    ...
    glBeginPixelLocalStorageANGLE(
        MAX_PIXEL_LOCAL_STORAGE_PLANES,
        std::vector<GLenum>(MAX_PIXEL_LOCAL_STORAGE_PLANES, GL_ZERO).data(), nullptr);
    EXPECT_GL_NO_ERROR();
...
#define EXPECT_BANNED_CAP(cap)                                                          \
    {                                                                                   \
        const char *msg =                                                               \
            #cap " cannot be enabled or disabled while pixel local storage is active."; \
        EXPECT_BANNED(glEnable(cap), msg);                                              \
        EXPECT_BANNED(glDisable(cap), msg);                                             \
        glIsEnabled(cap);                                                               \
        EXPECT_GL_NO_ERROR();                                                           \
    }
...
TEST_P(PixelLocalStorageValidationTest, BannedCommands)
{
    ...
    glBeginPixelLocalStorageANGLE(numActivePlanes,
                                  std::vector<GLenum>(numActivePlanes, GL_ZERO).data(), nullptr);
    ...
    EXPECT_BANNED_DEFAULT_MSG(glClientWaitSync(0, 0, 0));
    EXPECT_BANNED_DEFAULT_MSG(glTexSubImage2D(0, 0, 0, 0, 0, 0, 0, 0, 0));
    EXPECT_BANNED_DEFAULT_MSG(glCopyTexSubImage2D(0, 0, 0, 0, 0, 0, 0, 0));
