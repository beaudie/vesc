...
TEST_P(PixelLocalStorageValidationTest, BannedCommands)
{
    ...
    // BLEND is the only indexed capability in ES, but we can at least use SCISSOR_TEST to check the
    // debug output and verify that PLS validation is banning it, which ensures it is future proof
    // against hypothetical future extensions that add indexed capabilities.
    EXPECT_BANNED_CAP_INDEXED(GL_SCISSOR_TEST);
