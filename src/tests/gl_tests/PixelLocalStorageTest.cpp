            ...
            switch (planes[i].internalformat)
            {
                ...
                case GL_RGBA32F:
                    glClearBufferfv(GL_COLOR, MAX_FRAGMENT_OUTPUTS_WITH_LOCAL_STORAGE + i,
                                    loadOps[i] == GL_REPLACE ? planes[i].clearValuef : zerof);
                    break;
                case GL_RGBA8I:
                case GL_RGBA16I:
                    glClearBufferiv(GL_COLOR, MAX_FRAGMENT_OUTPUTS_WITH_LOCAL_STORAGE + i,
                                    loadOps[i] == GL_REPLACE ? planes[i].clearValuei : zeroi);
                    break;
                case GL_RGBA8UI:
                case GL_R32UI:
                case GL_RGBA16UI:
                case GL_RGBA32UI:
                    glClearBufferuiv(GL_COLOR, MAX_FRAGMENT_OUTPUTS_WITH_LOCAL_STORAGE + i,
                                     loadOps[i] == GL_REPLACE ? planes[i].clearValueui : zeroui);
                    break;
                default:
                    ASSERT(false);  // Internal error. Invalid internalformat.
            }
        }
