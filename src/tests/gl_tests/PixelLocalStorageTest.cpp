...
TEST_P(PixelLocalStorageTest, ProgramCache)
{
    ...
    for (int j = 0; j < 10; ++j)
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
        glBeginPixelLocalStorageANGLE(4,
                                      GLenumArray({GL_ZERO, GL_ZERO, GL_DISABLE_ANGLE, GL_ZERO}));
        mProgram.drawBoxes({{FULLSCREEN, {255, 254, 253, 252}, {0, 1, 2, 3}, {4, 5, 6, 7}}});
        glEndPixelLocalStorageANGLE(4, GLenumArray({GL_KEEP, GL_KEEP, GL_DONT_CARE, GL_KEEP}));
...
TEST_P(PixelLocalStorageValidationTest, EndAndBarrierANGLE)
{
    ...
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_ENUM);
    EXPECT_GL_SINGLE_ERROR_MSG("Invalid pixel local storage Store Operation: 0x1500.");
...
TEST_P(PixelLocalStorageValidationTest, GetFramebufferPixelLocalStorageParametersANGLE)
{
    ...
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_ENUM);
    EXPECT_GL_SINGLE_ERROR_MSG("Enum 0x96E5 is currently not supported.");
