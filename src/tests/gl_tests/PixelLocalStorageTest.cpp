...
TEST_P(PixelLocalStorageValidationTest, BeginPixelLocalStorageANGLE_context_state)
{
    ...
    // BLEND_SRC_ALPHA is a blend function requiring the secondary color input (SRC1_COLOR_EXT,
    // ONE_MINUS_SRC1_COLOR_EXT, SRC1_ALPHA_EXT or ONE_MINUS_SRC1_ALPHA_EXT).
    if (EnsureGLExtensionEnabled("GL_EXT_blend_func_extended"))
    {
                               ...
                               GL_ONE_MINUS_SRC1_ALPHA_EXT})
        {
            ...
            // secondary color input, as specified in EXT_blend_func_extended.
            if (IsGLExtensionEnabled("GL_OES_draw_buffers_indexed"))
            {
                ...
                if (maxDrawBuffersWithPLS > 0)
                {
                    ...
                    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
                    EXPECT_GL_SINGLE_ERROR_MSG(
                        "Blend function requiring the secondary color input are not supported when "
                        "pixel local storage is active.");

                    glBlendFuncSeparatei(maxDrawBuffersWithPLS - 1, GL_ONE, GL_ONE, GL_ONE,
                                         blendFunc);
                    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
                    EXPECT_GL_SINGLE_ERROR_MSG(
                        "Blend function requiring the secondary color input are not supported when "
    ...
    if (EnsureGLExtensionEnabled("GL_KHR_blend_equation_advanced"))
    {
             ...
             })
        {
            ...
            EXPECT_GL_SINGLE_ERROR(GL_INVALID_OPERATION);
            EXPECT_GL_SINGLE_ERROR_MSG(
                "Attempted to begin pixel local storage with an advanced blend equation "
                "enabled.");
            EXPECT_GL_INTEGER(GL_PIXEL_LOCAL_STORAGE_ACTIVE_PLANES_ANGLE, 0);
...
TEST_P(PixelLocalStorageValidationTest, BannedCommands)
{
    ...
    // <buffer> is GL_COLOR and and any of the following are true:
    //
    ...
    if (IsGLExtensionEnabled("GL_OES_draw_buffers_indexed"))
    {
        ...
        // BlendFunci*(), BlendFuncSeparatei*(), ColorMaski*() if and any of the following are true:
        //
...
TEST_P(PixelLocalStorageCompilerTest, BlendFuncExtended)
{
    ...
    EXPECT_TRUE(log.has(
        "ERROR: 0:4: 'layout' : illegal index qualifier when pixel local storage is declared"));
