...
#define EXPECT_FRAMEBUFFER_PIXEL_RECT(T, x1, y1, x2, y2, r, g, b, a)                               \
    do                                                                                             \
    {                                                                                              \
        int h = y2 - y1, w = x2 - x1;                                                              \
        ASSERT(w >= 0);                                                                            \
...
#define EXPECT_FRAMEBUFFER_PIXELS(T, r, g, b, a) \
    EXPECT_FRAMEBUFFER_PIXEL_RECT(T, 0, 0, W, H, r, g, b, a)
...
TEST_P(PixelLocalStorageTest, AllFormats)
{
...

    {
        ...
        EXPECT_FRAMEBUFFER_PIXELS(uint8_t, 255, 255, 255, 0);

        attachTextureToScratchFBO(tex2);
        EXPECT_FRAMEBUFFER_PIXELS(int32_t, -5, -100, -70, 22);

        attachTextureToScratchFBO(tex3);
        EXPECT_FRAMEBUFFER_PIXELS(uint32_t, 1, 50, 100, 255);
...

    {
        ...
        attachTextureToScratchFBO(tex1);
        EXPECT_FRAMEBUFFER_PIXELS(float, -111.5, 0, 0, 1);

        attachTextureToScratchFBO(tex2);
        EXPECT_FRAMEBUFFER_PIXELS(uint32_t, 8765, 0, 0, 1);
...

    {
        ...
        attachTextureToScratchFBO(tex1);
        EXPECT_FRAMEBUFFER_PIXELS(float, -100.5, 1024, -4096, 16384);

        attachTextureToScratchFBO(tex2);
        EXPECT_FRAMEBUFFER_PIXELS(int32_t, -500, -10000, -7000, 2200);

        attachTextureToScratchFBO(tex3);
        EXPECT_FRAMEBUFFER_PIXELS(uint32_t, 1, 500, 10000, 65535);
