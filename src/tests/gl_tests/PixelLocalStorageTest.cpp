...
// Check that enabling (or not) the GL_ANGLE_shader_pixel_local_storage #extension behaves as
// expected.
TEST_P(PixelLocalStorageTest, CompilerValidation_extension)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ...
    const char *kPLSEnabledTypesAsNames = R"(#version 310 es
    #extension all : warn
    void main()
    {
    ...
    const char *kPLSDisabledTypesAsNames = R"(#version 310 es
    ...
    const char *kPLSInVertexShader = R"(#version 310 es
...
TEST_P(PixelLocalStorageTest, CompilerValidation_declarations)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ...
    const char *kPLSTypesMustBeUniform = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : enable
    ...
    const char *kPLSMemoryQualifiers = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    layout(binding=2, rgba32ui) uniform highp writeonly upixelLocalANGLE pls3;
    // TODO(anglebug.com/7279): If supported, function args don't allow memory qualifiers.
    void main()
    {
    })";
    ...
    const char *kPLSNoPrecision = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : enable
    layout(binding=0, rgba8) uniform pixelLocalANGLE pls1;
    layout(binding=1, rgba8i) uniform ipixelLocalANGLE pls2;
    // anglebug.com/7279: PLS function args aren't supported, but would require precision if so.
    ...
    EXPECT_TRUE(log.has("ERROR: 0:6: 'upixelLocalANGLE' : No precision specified"));

    // PLS handles cannot cannot be aggregated in arrays.
    const char *kPLSArrays = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    const char *kPLSUseBeforeDeclaration = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void f()
    {
        pixelLocalStoreANGLE(pls, vec4(0));
...
TEST_P(PixelLocalStorageTest, CompilerValidation_layout_qualifiers)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ...
    const char *kPLSInvalidLayoutQualifiers = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    layout(foo) highp uniform pixelLocalANGLE pls23;
    highp uniform pixelLocalANGLE pls25;
    layout(binding=999999999, rgba8i) highp uniform ipixelLocalANGLE pls26;
    layout(binding=0, location=0, rgba8ui) highp uniform upixelLocalANGLE pls27;
    // TODO(anglebug.com/7279): If supported, function args require a format.
    void main()
    {
    })";
    EXPECT_FALSE(log.compileFragmentShader(kPLSInvalidLayoutQualifiers));
    EXPECT_TRUE(
    ...
    EXPECT_TRUE(log.has("ERROR: 0:26: 'foo' : invalid layout qualifier"));
    EXPECT_TRUE(log.has(
    ...
    EXPECT_TRUE(log.has(
        "ERROR: 0:27: 'layout qualifier' : pixel local storage requires a format specifier"));
    EXPECT_TRUE(
        log.has("ERROR: 0:27: 'layout qualifier' : pixel local storage requires a binding index"));
    EXPECT_TRUE(log.has(  // TODO(anglebug.com/7279): gl_MaxPixelLocalStoragePlanesANGLE.
        "ERROR: 0:28: 'binding' : pixel local storage binding greater than gl_MaxImageUnits"));
    EXPECT_TRUE(
        log.has("ERROR: 0:29: 'location' : location must only be specified for a single input or "
                "output variable"));

    // PLS handles cannot have duplicate binding indices.
    const char *kPLSDuplicateBindings = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
...
TEST_P(PixelLocalStorageTest, CompilerValidation_discard)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));

    ShaderInfoLog log;

    // Discard is not allowed when pixel local storage has been declared. The shader image polyfill
    // requires early_fragment_tests, which causes discard to interact differently with the depth
    // and stencil tests.
    //
    ...
    const char *kDiscardWithPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    const char *kDiscardNoPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void a()
    {
        discard;
    ...
    const char *kDiscardBeforePLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void a()
    {
        discard;
...
TEST_P(PixelLocalStorageTest, CompilerValidation_return)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ...
    const char *kReturnFromMainWithPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    layout(binding=0, rgba16f) highp uniform pixelLocalANGLE pls;
    void main()
    {
    ...
    const char *kReturnFromMainNoPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void main()
    {
        if (gl_FragDepth == 3.14)
    ...
    const char *kReturnFromSubroutinesWithPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
...
TEST_P(PixelLocalStorageTest, CompilerValidation_fragment_test_variables)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));

    ShaderInfoLog log;

    // gl_FragDepth is not assignable when pixel local storage has been declared. The shader image
    // polyfill requires early_fragment_tests, which causes gl_FragDepth to be ignored.
    //
    ...
    const char *kAssignFragDepthWithPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    const char *kAssignFragDepthNoPLS = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void set(out mediump float x, mediump float val)
    {
        x = val;
    ...
    const char *kReadFragDepth = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    if (IsGLExtensionEnabled("GL_OES_sample_variables"))
    {
        ...
        const char *kAssignSampleMaskWithPLS = R"(#version 310 es
        #extension GL_ANGLE_shader_pixel_local_storage : require
        ...
        const char *kAssignSampleMaskNoPLS = R"(#version 310 es
        #extension GL_ANGLE_shader_pixel_local_storage : require
        #extension GL_OES_sample_variables : require
        void set(out highp int x, highp int val)
        {
        ...
        const char *kReadSampleMask = R"(#version 310 es
        #extension GL_ANGLE_shader_pixel_local_storage : require
        #extension GL_OES_sample_variables : require
        layout(binding=0, rgba16i) highp uniform ipixelLocalANGLE pls;
...
TEST_P(PixelLocalStorageTest, CompilerValidation_function_arguments)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ...
    const char *kPLSHandleComplexExpression = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    ...
    // PLS handles are not supported as function arguments.
    //
    // TODO(anglebug.com/7484): Consider allowing this once it is properly supported for images.
    const char *kPLSFunctionArgs = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    layout(rgba8, binding=0) mediump uniform pixelLocalANGLE uniformPLS0;
    layout(rgba8, binding=1) mediump uniform pixelLocalANGLE uniformPLS1;
    layout(rgba8, binding=2) mediump uniform pixelLocalANGLE uniformPLS2;
    mediump vec4 load(mediump pixelLocalANGLE src);
    void store(mediump pixelLocalANGLE dst, mediump vec4 val)
    {
        pixelLocalStoreANGLE(dst, val);
    }
    void main()
    {
        mediump vec4 val;
        if (gl_FragCoord.y < 50.0)
            val = load(uniformPLS0);
        else
            val = load(uniformPLS1);
        if (gl_FragCoord.x < 50.0)
            store(uniformPLS1, val);
        else
            store(uniformPLS2, val);
    }
    mediump vec4 load(mediump pixelLocalANGLE src)
    {
        return pixelLocalLoadANGLE(src);
    })";
    EXPECT_FALSE(log.compileFragmentShader(kPLSFunctionArgs));
    EXPECT_TRUE(
        log.has("ERROR: 0:6: 'src' : function arguments cannot be pixel local storage handles"));
    EXPECT_TRUE(
        log.has("ERROR: 0:7: 'dst' : function arguments cannot be pixel local storage handles"));
    EXPECT_TRUE(
        log.has("ERROR: 0:23: 'src' : function arguments cannot be pixel local storage handles"));
...
ANGLE_INSTANTIATE_TEST_ES2_AND_ES3(PixelLocalStorageTestPreES31);
