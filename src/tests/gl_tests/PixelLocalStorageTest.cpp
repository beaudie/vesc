...
TEST_P(PixelLocalStorageTest, Coherency)
{
    ...
    useProgram(R"(
    layout(binding=0, rgba8ui) highp uniform upixelLocalANGLE framebuffer;
    ...
    void main()
    {
        pixelLocalStoreANGLE(framebuffer, uvec4(color));
    })");
    ...
    for (auto &box : boxes)
    {
        for (int i = 0; i < H * W * 11; ++i)
        {
            ...
            float x   = rand() % (W - w);
            float y   = rand() % (H - h);
            uint8_t r = boxID & 0xff;
            uint8_t g = (boxID >> 8) & 0xff;
            uint8_t b = (boxID >> 16) & 0xff;
            uint8_t a = (boxID >> 24) & 0xff;
            ++boxID;
            // Update expectations.
            ...
            if (rand() & 1)
                std::swap(y0, y1);
            box.push_back({{x0, y0, x1, y1}, {(float)r, (float)g, (float)b, (float)a}});
        }
    }
...
TEST_P(PixelLocalStorageCompilerTest, Declarations)
{
    ...
    // interface blocks.
    const char *kPLSInStruct = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
    struct Foo
    {
...
TEST_P(PixelLocalStorageCompilerTest, FunctionArguments)
{
    ...
    const char *kPLSFnArgWithLayoutQualifiers = R"(#version 310 es
    #extension GL_ANGLE_shader_pixel_local_storage : require
