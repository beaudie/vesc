  ...
  public:
    ...
    void testSetUp() override
    {
        if (IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"))
        {
            ...
            glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS_ANGLE,
                          &MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS);
            glGetIntegerv(GL_MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES_ANGLE,
                          &MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES);
        }
        ANGLETest::testSetUp();
    }
    ...
    GLint MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS    = 0;
    GLint MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES = 0;
...
TEST_P(PixelLocalStorageTest, R32)
{
    ...
    drawBoxes({{FULLSCREEN, {-1.5, 0, 0, 0}, {5, 0, 0, 0}},
...
// Check proper functioning of glFramebufferPixelLocalClearValue{f,i,ui}vANGLE.
TEST_P(PixelLocalStorageTest, ClearValue)
{
    ...
    PLSTestTexture texi(GL_RGBA8I);
    PLSTestTexture texui(GL_RGBA8UI);

    GLFramebuffer fbo;
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glFramebufferTexturePixelLocalStorageANGLE(0, texf, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(1, texi, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(2, texui, 0, 0);
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glFramebufferTexturePixelLocalStorageANGLE(1, texf, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(2, texi, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(0, texui, 0, 0);
    glBeginPixelLocalStorageANGLE(3, clearLoads);
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor(0, 0, 0, 0));
    ...
    glFramebufferPixelLocalClearValueivANGLE(2, MakeArray<int32_t>({10, -20, 30, -40}));
    glFramebufferPixelLocalClearValueuivANGLE(0, MakeArray<uint32_t>({50, 60, 70, 80}));
    glBeginPixelLocalStorageANGLE(3, clearLoads);
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor(0, 255, 0, 0));
    attachTextureToScratchFBO(texi);
    EXPECT_PIXEL_RECT32I_EQ(0, 0, W, H, GLColor32I(10, -20, 30, -40));
    attachTextureToScratchFBO(texui);
    EXPECT_PIXEL_RECT32UI_EQ(0, 0, W, H, GLColor32UI(50, 60, 70, 80));

    // Different clear value types are separate state values (final rotation).
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glFramebufferTexturePixelLocalStorageANGLE(2, texf, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(0, texi, 0, 0);
    glFramebufferTexturePixelLocalStorageANGLE(1, texui, 0, 0);
    glBeginPixelLocalStorageANGLE(3, clearLoads);
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor(0, 0, 0, 0));
    ...
    glFramebufferPixelLocalClearValueivANGLE(0, MakeArray<int32_t>({101, 102, -103, -104}));
    glFramebufferPixelLocalClearValueuivANGLE(1, MakeArray<uint32_t>({105, 106, 107, 108}));
    glBeginPixelLocalStorageANGLE(3, clearLoads);
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor(0, 0, 255, 0));
    attachTextureToScratchFBO(texi);
    EXPECT_PIXEL_RECT32I_EQ(0, 0, W, H, GLColor32I(101, 102, -103, -104));
    attachTextureToScratchFBO(texui);
    EXPECT_PIXEL_RECT32UI_EQ(0, 0, W, H, GLColor32UI(105, 106, 107, 108));
...
TEST_P(PixelLocalStorageTest, MaxCombinedDrawBuffersAndPLSPlanes)
{
    ...
    for (int numDrawBuffers : {0, 1, MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES - 1})
    {
        numDrawBuffers   = std::min(numDrawBuffers, MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS);
        int numPLSPlanes = MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES - numDrawBuffers;
...
class PixelLocalStorageValidationTest : public ANGLETest<>
{
  protected:
    void testSetUp() override
    {
        ...
        ASSERT(IsGLExtensionEnabled("GL_KHR_debug"));
        glDebugMessageControlKHR(GL_DEBUG_SOURCE_API, GL_DEBUG_TYPE_ERROR, GL_DEBUG_SEVERITY_HIGH,
                                 0, NULL, GL_TRUE);
...
TEST_P(PixelLocalStorageValidationTest, FramebufferTexturePixelLocalStorageANGLE)
{
    ...
    glTexStorage3D(GL_TEXTURE_3D, 3, GL_RGBA8I, 10, 10, 587);
    EXPECT_GL_NO_ERROR();
    glFramebufferTexturePixelLocalStorageANGLE(0, tex3D, 2, 587);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Layer is larger than texture depth.");
    glFramebufferTexturePixelLocalStorageANGLE(0, tex3D, 3, 586);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Level is larger than texture level count.");
    glFramebufferTexturePixelLocalStorageANGLE(0, tex3D, 2, 586);
    EXPECT_GL_NO_ERROR();
    EXPECT_GL_INTEGER_INDEXED(GL_PIXEL_LOCAL_FORMAT_ANGLE, 0, GL_RGBA8I);
    ...
    EXPECT_GL_INTEGER_INDEXED(GL_PIXEL_LOCAL_TEXTURE_LAYER_ANGLE, 0, 586);
    tex3D.reset();
    EXPECT_GL_INTEGER_INDEXED(GL_PIXEL_LOCAL_FORMAT_ANGLE, 0, GL_RGBA8I);
    EXPECT_GL_INTEGER_INDEXED(GL_PIXEL_LOCAL_TEXTURE_NAME_ANGLE, 0, 0);
    EXPECT_GL_INTEGER_INDEXED(GL_PIXEL_LOCAL_TEXTURE_LEVEL_ANGLE, 0, 0);
...
// Check that glFramebufferPixelLocalClearValue{f,i,ui}vANGLE validates as specified.
TEST_P(PixelLocalStorageValidationTest, PixelLocalClearValueANGLE)
{
    GLfloat clearf[4]{};
    GLint cleari[4]{};
    GLuint clearui[4]{};
    ...
    // INVALID_FRAMEBUFFER_OPERATION is generated if the default framebuffer object name 0 is bound
    // to DRAW_FRAMEBUFFER.
    glFramebufferPixelLocalClearValuefvANGLE(0, clearf);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION);
    EXPECT_GL_SINGLE_ERROR_MSG(
        "Default framebuffer object name 0 does not support pixel local storage.");

    glFramebufferPixelLocalClearValueivANGLE(0, cleari);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION);
    EXPECT_GL_SINGLE_ERROR_MSG(
        "Default framebuffer object name 0 does not support pixel local storage.");

    glFramebufferPixelLocalClearValueuivANGLE(0, clearui);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION);
    EXPECT_GL_SINGLE_ERROR_MSG(
        "Default framebuffer object name 0 does not support pixel local storage.");
    ...
    // INVALID_VALUE is generated if <plane> < 0 or <plane> >= MAX_PIXEL_LOCAL_STORAGE_PLANES_ANGLE.
    glFramebufferPixelLocalClearValuefvANGLE(-1, clearf);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane cannot be less than 0.");

    glFramebufferPixelLocalClearValueivANGLE(-2, cleari);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane cannot be less than 0.");

    glFramebufferPixelLocalClearValueuivANGLE(-3, clearui);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane cannot be less than 0.");

    glFramebufferPixelLocalClearValuefvANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES, clearf);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane must be less than GL_MAX_PIXEL_LOCAL_STORAGE_PLANES_ANGLE.");

    glFramebufferPixelLocalClearValueivANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES + 1, cleari);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane must be less than GL_MAX_PIXEL_LOCAL_STORAGE_PLANES_ANGLE.");

    glFramebufferPixelLocalClearValueuivANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES + 9999, clearui);
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    EXPECT_GL_SINGLE_ERROR_MSG("Plane must be less than GL_MAX_PIXEL_LOCAL_STORAGE_PLANES_ANGLE.");

    glFramebufferPixelLocalClearValuefvANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES - 1, clearf);
    EXPECT_GL_NO_ERROR();

    glFramebufferPixelLocalClearValueivANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES - 1, cleari);
    EXPECT_GL_NO_ERROR();

    glFramebufferPixelLocalClearValueuivANGLE(MAX_PIXEL_LOCAL_STORAGE_PLANES - 1, clearui);
    EXPECT_GL_NO_ERROR();

    ASSERT_GL_NO_ERROR();
}

// Check that the glBeginPixelLocalStorageANGLE command validates as specified.
TEST_P(PixelLocalStorageValidationTest, BeginPixelLocalStorageANGLE)
{
    ...
    glBeginPixelLocalStorageANGLE(0, GLenumArray({GL_DISABLE_ANGLE}));
    EXPECT_GL_SINGLE_ERROR(GL_INVALID_VALUE);
    ...
    // INVALID_OPERATION is generated if a draw buffer is not NONE at index greater than or equal
    // to:
    //
    ...
    // OR:
    //
    //   MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS_ANGLE
    //
    // (Whichever is lesser.)
    int firstReservedDrawBuffer =
        std::min(MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES - MAX_PIXEL_LOCAL_STORAGE_PLANES,
                 MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS);
    int MAX_DRAW_BUFFERS;
    glGetIntegerv(GL_MAX_DRAW_BUFFERS, &MAX_DRAW_BUFFERS);
    if (firstReservedDrawBuffer < MAX_DRAW_BUFFERS)
    {
        for (int reservedDrawBuffer : {firstReservedDrawBuffer, MAX_DRAW_BUFFERS - 1})
        {
            PLSTestTexture tmp(GL_RGBA8, 100, 100);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tmp, 0);
            std::vector<GLenum> drawBuffers(reservedDrawBuffer, GL_NONE);
            drawBuffers.push_back(GL_COLOR_ATTACHMENT0 + reservedDrawBuffer);
            glDrawBuffers(drawBuffers.size(), drawBuffers.data());
            glBeginPixelLocalStorageANGLE(
            ...
            EXPECT_GL_SINGLE_ERROR(GL_INVALID_OPERATION);
            EXPECT_GL_SINGLE_ERROR_MSG(
                "When beginning pixel local storage, glDrawBuffers must all be GL_NONE at indices "
                "greater than or equal to: min(GL_MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS_ANGLE, "
                "GL_MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES_ANGLE - <planes>).");
            ASSERT_GL_BOOLEAN(GL_PIXEL_LOCAL_STORAGE_ACTIVE_ANGLE, GL_FALSE);
        }
        glDrawBuffers(0, nullptr);
    }

    // INVALID_OPERATION is generated if the draw framebuffer has color attachments on or after:
    //
    //   COLOR_ATTACHMENT0 + MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES_ANGLE - <planes>
    //
    // OR:
    //
    ...
    if (firstReservedDrawBuffer < MAX_COLOR_ATTACHMENTS)
    {
        for (int reservedAttachment : {firstReservedDrawBuffer, MAX_COLOR_ATTACHMENTS - 1})
        {
            PLSTestTexture tmp(GL_RGBA8, 100, 100);
            ...
            EXPECT_GL_SINGLE_ERROR(GL_INVALID_OPERATION);
            EXPECT_GL_SINGLE_ERROR_MSG(
                "When beginning pixel local storage, the framebuffer cannot have color attachments "
                "on "
                "or after: GL_COLOR_ATTACHMENT0 + "
                "min(GL_MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PLS_ANGLE, "
                "GL_MAX_COMBINED_DRAW_BUFFERS_AND_PLS_PLANES_ANGLE - <planes>).");
            ASSERT_GL_BOOLEAN(GL_PIXEL_LOCAL_STORAGE_ACTIVE_ANGLE, GL_FALSE);
        }
    }
