...
vec4 barrierAfter(vec4 value)
{
    memoryBarrier();
    return value;
}
ivec4 barrierAfter(ivec4 value)
{
    memoryBarrier();
    return value;
}
uvec4 barrierAfter(uvec4 value)
{
    memoryBarrier();
    return value;
}
#define pixelLocalStoreImpl(NAME_W, VALUE) \
    { \
        imageStore(NAME_W, PIXEL_I_COORD, barrierAfter(VALUE)); \
        memoryBarrier(); \
    }
...
// Implement this as a macro so our __LINE__ points to the failing line(s) in the test(s).
#define ASSERT_FRAMEBUFFER_PIXEL_RECT(T, x1, y1, x2, y2, r, g, b, a)                               \
    do                                                                                             \
    {                                                                                              \
        int h = y2 - y1, w = x2 - x1;                                                              \
        ASSERT(w >= 0);                                                                            \
        ...
        glReadPixels(x1, y1, w, h, ReadPixelsArgs<T>::format, ReadPixelsArgs<T>::type,             \
                     pixels.data());                                                               \
        std::array<T, 4> expectedColor{r, g, b, a};                                                \
        for (int y = 0; y < h; ++y)                                                                \
        {                                                                                          \
            for (int x = 0; x < w; ++x)                                                            \
            {                                                                                      \
                ...
                {                                                                                  \
                    printf(__FILE__ "(%i): FAILED at pixel location [%i, %i]\n", __LINE__, x1 + x, \
                           y1 + y);                                                                \
                }                                                                                  \
...
#define ASSERT_FRAMEBUFFER_PIXELS(T, r, g, b, a) \
    ASSERT_FRAMEBUFFER_PIXEL_RECT(T, 0, 0, W, H, r, g, b, a)
...
TEST_P(PixelLocalStorageTest, AllFormats)
{
...

    {
        ...
        ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 255, 255, 255, 0);

        attachTextureToScratchFBO(tex2);
        ASSERT_FRAMEBUFFER_PIXELS(int32_t, -5, -100, -70, 22);

        attachTextureToScratchFBO(tex3);
        ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 1, 50, 100, 255);
...

    {
        ...
        attachTextureToScratchFBO(tex1);
        ASSERT_FRAMEBUFFER_PIXELS(float, -111.5, 0, 0, 1);

        attachTextureToScratchFBO(tex2);
        ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 8765, 0, 0, 1);
...

    {
        ...
        attachTextureToScratchFBO(tex1);
        ASSERT_FRAMEBUFFER_PIXELS(float, -100.5, 1024, -4096, 16384);

        attachTextureToScratchFBO(tex2);
        ASSERT_FRAMEBUFFER_PIXELS(int32_t, -500, -10000, -7000, 2200);

        attachTextureToScratchFBO(tex3);
        ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 1, 500, 10000, 65535);
...
TEST_P(PixelLocalStorageTest, ClearValue)
{
    ...
    ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 0, 0, 0, 0);
    attachTextureToScratchFBO(texi);
    ASSERT_FRAMEBUFFER_PIXELS(int32_t, 0, 0, 0, 0);
    attachTextureToScratchFBO(texui);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 0, 0, 0, 0);
    ...
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 255, 0, 0, 0);
    attachTextureToScratchFBO(texi);
    ASSERT_FRAMEBUFFER_PIXELS(int32_t, 1, 2, 3, 4);
    attachTextureToScratchFBO(texui);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 5, 6, 7, 8);
    ...
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 0, 255, 0, 0);
    attachTextureToScratchFBO(texi);
    ASSERT_FRAMEBUFFER_PIXELS(int32_t, 100, 200, 300, 400);
    attachTextureToScratchFBO(texui);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 500, 600, 700, 800);
    ...
    glEndPixelLocalStorageANGLE();
    attachTextureToScratchFBO(texf);
    ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 0, 0, 255, 0);
    attachTextureToScratchFBO(texi);
    ASSERT_FRAMEBUFFER_PIXELS(int32_t, 1000, 2000, 3000, 4000);
    attachTextureToScratchFBO(texui);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 5000, 6000, 7000, 8000);
...
TEST_P(PixelLocalStorageTest, LoadOps)
{
    ...
    for (int i = 0; i < MAX_LOCAL_STORAGE_PLANES; ++i)
    {
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        if (loadOps[i] == GL_REPLACE)
        {
            ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 0, 20, H, 0, 255, 0, 255);
        }
        else /*GL_KEEP*/
        {
            ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 0, 20, H, 255, 255, 0, 255);
        }
        // Check that the scissor didn't get perturbed by local storage.
        if (loadOps[i] == GL_REPLACE)
        {
            ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 20, 0, W, H, 0, 0, 0, 255);
        }
        else /*GL_KEEP*/
        {
            ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 20, 0, W, H, 255, 0, 0, 255);
        }
    }
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 0, 20, H, 255, 0, 0, 255);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 20, 0, W, H, 0, 0, 0, 0);
...
#define CHECK_TEX()                                                                   \
    ...
    {                                                                                 \
        ...
        ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 0, DRAW_W, DRAW_H, 0, 255, 0, 255); \
        ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, DRAW_W, 0, W, DRAW_H, 0, 0, 0, 255);   \
        ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, DRAW_H, W, H, 0, 0, 0, 255);        \
                                                                                      \
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);                                       \
        glUseProgram(mProgram);                                                       \
    } while (0)
    ...
    ASSERT_FRAMEBUFFER_PIXELS(uint8_t, 0, 0, 0, 255);
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
...
TEST_P(PixelLocalStorageTest, ForgetBarrier)
{
    ...
    attachTextureToScratchFBO(tex);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 211, 0, 0, 1);
...
TEST_P(PixelLocalStorageTest, CoherentStoreLoad)
{
    ...
    PLSTestTexture tex(GL_RGBA32UI);
    ...
    attachTextureToScratchFBO(tex);
    ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 233, 144, 89, 55);  // fib(13, 12, 11, 10)
...
TEST_P(PixelLocalStorageTest, MemorylessStorage)
{
    ...
    attachTextureToScratchFBO(tex);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 60, W, H, 0, 0, 0, 255);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 40, W, 60, 0, 0, 255, 255);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 20, W, 40, 0, 255, 255, 255);
    ASSERT_FRAMEBUFFER_PIXEL_RECT(uint8_t, 0, 0, W, 20, 255, 255, 255, 255);
...
TEST_P(PixelLocalStorageTest, MaxCapacity)
{
    ...
    for (int i = 0; i < MAX_LOCAL_STORAGE_PLANES; ++i)
    {
        ...
        ASSERT_FRAMEBUFFER_PIXELS(uint32_t, 255u - i, 254u - i, 253u - i, 252u - i);
    }
    for (int i = 0; i < MAX_FRAGMENT_OUTPUTS_WITH_LOCAL_STORAGE; ++i)
    {
...
TEST_P(PixelLocalStorageTest, LoadOnly)
{
    ...
    GLFramebuffer fbo;
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glFramebufferPixelLocalStorageANGLE(0, 0 /*Memoryless*/, 0, 0, W, H, GL_RGBA16F);
