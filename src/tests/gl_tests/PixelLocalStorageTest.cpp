...
TEST_P(PixelLocalStorageTest, ForgetBarrier)
{
    ...
    ExpectFramebufferPixels<uint32_t>({211, 0, 0, 1});
    ...
    for (int r = 0; r < NUM_PIXELS * 4; r += 4)
    {
        ...
        bool isAcceptableValue =
            pixels[r] == 211 ||  // A, then B  (  7 + (100 + 1 * 2) * 2 == 211)
            pixels[r] == 118 ||  // B, then A  (100 + (  7 + 1 * 2) * 2 == 118)
            pixels[r] == 102 ||  // A only     (100 +             1 * 2 == 102)
            pixels[r] == 9 ||    // B only     (  7 +             1 * 2 ==   9)
            pixels[r] == 1;      // It's also acceptable to draw nothing when a barrier is missed,
                                 // leaving behind the clear color (1).
        if (!isAcceptableValue)
        {
            printf("UNACCEPTABLE value at pixel location [%i, %i]\n", (r / 4) % W, (r / 4) / W);
            printf("              Got: %u\n", pixels[r]);
            printf("  Expected one of: { 211, 118, 102, 9, 1 }\n");
        }
