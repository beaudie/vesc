  ...
  private:
    class PLSPlane
    {
      public:
        ...
        void clearValuef(const float val[4]) { memcpy(mClearValuef, val, sizeof(mClearValuef)); }
        void clearValuei(const int32_t val[4]) { memcpy(mClearValuei, val, sizeof(mClearValuei)); }
        void clearValueui(const uint32_t val[4])
        {
...
{
    ASSERT(0 <= unit && unit < MAX_LOCAL_STORAGE_PLANES);  // GL_INVALID_VALUE!
    boundLocalStoragePlanes()[unit].clearValueui(value);
}
