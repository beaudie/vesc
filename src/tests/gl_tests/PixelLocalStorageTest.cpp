...
class PixelLocalStorageTest : public ANGLETest
{
  public:
    ...
    bool hasImageLoadANGLEBug7398() { return IsWindows() && IsIntel(); }
...
//   * return from main, after potential calls to pixelLocalLoad/Store, is disallowed by the spec
...
// Pixels inside {FRAG_REJECT_TEST_WIDTH, FRAG_REJECT_TEST_HEIGHT} should be drawn green. Fragments
// outside should have been rejected, leaving the pixels black.
#define EXPECT_FRAGMENT_REJECT_TEST_PIXELS()                                         \
    EXPECT_PIXEL_RECT_EQ(0, 0, FRAG_REJECT_TEST_WIDTH, FRAG_REJECT_TEST_HEIGHT,      \
                         GLColor(0, 255, 0, 255));                                   \
    EXPECT_PIXEL_RECT_EQ(FRAG_REJECT_TEST_WIDTH, 0, W - FRAG_REJECT_TEST_WIDTH,      \
                         FRAG_REJECT_TEST_HEIGHT, GLColor(0, 0, 0, 255));            \
    EXPECT_PIXEL_RECT_EQ(0, FRAG_REJECT_TEST_HEIGHT, W, H - FRAG_REJECT_TEST_HEIGHT, \
                         GLColor(0, 0, 0, 255))

// Check that discard prevents stores to pls.
// Discard is illegal after pixelLocalLoad/Store because that has different behavior with shader
// images vs framebuffer fetch.
TEST_P(PixelLocalStorageTest, FragmentReject_discard)
{
    ...
    EXPECT_FRAGMENT_REJECT_TEST_PIXELS();
    ASSERT_GL_NO_ERROR();
}

// Check that return from main prevents stores to PLS.
// Return is illegal after pixelLocalLoad/Store because that has different behavior with shader
// images vs framebuffer fetch. It is also forbidden by GL_ARB_fragment_shader_interlock.
