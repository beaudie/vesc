...
constexpr static int FIRST_LOCAL_STORAGE_IMAGE_UNIT          = 1;  // anglebug.com/7355
...
{
    os << "layout(binding=" << (FIRST_LOCAL_STORAGE_IMAGE_UNIT + decl.unit * 2) << ", "
       << decl.format << ") coherent volatile readonly highp uniform image2D " << decl.name
       << "_R;\n";
    os << "layout(binding=" << (FIRST_LOCAL_STORAGE_IMAGE_UNIT + decl.unit * 2 + 1) << ", "
       << decl.format << ") coherent volatile writeonly highp uniform image2D " << decl.name
       << "_W;\n";
    return os;
}
...
#define pixelLocalLoad(NAME_R) \
    (memoryBarrierImage(), imageLoad(NAME_R, ivec2(floor(gl_FragCoord.xy))))
...
void PixelLocalStoragePrototype::beginPixelLocalStorage(GLsizei n, const GLenum *loadOps)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // write??)
        glBindImageTexture(FIRST_LOCAL_STORAGE_IMAGE_UNIT + i * 2, tex, 0, GL_FALSE, 0,
                           GL_READ_WRITE, internalformat);
        glBindImageTexture(FIRST_LOCAL_STORAGE_IMAGE_UNIT + i * 2 + 1, tex, 0, GL_FALSE, 0,
                           GL_READ_WRITE, internalformat);
    }
    if (needsClear)
    {
...
void PixelLocalStoragePrototype::endPixelLocalStorage()
{
    ...
    {
        glBindImageTexture(FIRST_LOCAL_STORAGE_IMAGE_UNIT + i * 2, 0, 0, GL_FALSE, 0, GL_READ_WRITE,
                           GL_RGBA8);
        glBindImageTexture(FIRST_LOCAL_STORAGE_IMAGE_UNIT + i * 2 + 1, 0, 0, GL_FALSE, 0,
...
class PixelLocalStorageTest : public ANGLETest
{
  protected:
    ...
    bool supportsPixelLocalStorage()
    {
        ...
        if (isD3D11Renderer())
        {
            // Shader image support is not fully implemented on the D3D11 graphics pipeline:
            // https://bugs.chromium.org/p/angleproject/issues/detail?id=7355#c7
            return false;
        }
...
TEST_P(PixelLocalStorageTest, RGBA)
{
    ...
    // Clear via imageStore in order to force ANGLE to establish the right binding on unit
    // FIRST_LOCAL_STORAGE_IMAGE_UNIT.
    useProgram({}, R"(
    layout(binding=1, rgba8) coherent volatile writeonly highp uniform image2D framebuffer_R;
    void main() {
        pixelLocalStore(framebuffer_R, color);
    })");
    glUniform4f(mRectUniform, 0, 0, W, H);
    glUniform4f(mColorUniform, 0, 0, 0, 0);
    ...
    // glUniform4f(mColorUniform, 0, 0, 0, 1);
    // glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    // glPixelLocalStorageBarrierANGLE();
    ...
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor(255, 255, 255, 0 /*255*/));
