...
static bool CompileFragmentShader(const char *source, std::string *infoLog)
{
    ...
    if (compileResult == 0)
    {
        ...
        infoLog->resize(std::max(infoLogLength - 1, 0));
...
//
// TODO: we can relax the min supported version once the implementation details are inside ANGLE.
TEST_P(PixelLocalStorageTestPreES31, UnsupportedClientVersion)
{
    ...
    void main() {})";
    std::string infoLog;
    EXPECT_FALSE(CompileFragmentShader(kRequireUnsupportedPLS, &infoLog));
    EXPECT_TRUE(
        strstr(infoLog.c_str(),
               "ERROR: 0:2: 'GL_ANGLE_shader_pixel_local_storage' : extension is not supported"));
