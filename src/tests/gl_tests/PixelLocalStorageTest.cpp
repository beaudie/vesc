...
// Descibes a GLSL "pixelLocal" declaration.
struct pixelLocal
{
...
// around this limitation, we bind the texture to both a sampler2D and an image2D, load with
// texelFetch, and store with imageStore.
std::ostream &operator<<(std::ostream &os, const pixelLocal &decl)
{
    int maxTextureUnits;
    glGetIntegerv(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS, &maxTextureUnits);
    os << "layout(binding=" << (maxTextureUnits - decl.unit - 1) << ") highp uniform sampler2D "
       << decl.name << "_R;\n";
...
// client code is responsible to know the image is bound to both a sampler2D and an image2D, append
// "_R" for pixelLocalLoad, and append "_W" for pixelLocalStore.
constexpr static const char kLocalStorageGLSLDefines[] = R"(
#define PIXEL_I_COORD ivec2(floor(gl_FragCoord.xy))
#define pixelLocalLoad(NAME_R) texelFetch(NAME_R, PIXEL_I_COORD, 0)
...
void PixelLocalStoragePrototype::beginPixelLocalStorage(GLsizei n, const GLenum *loadOps)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // Bind all local storage textures to a sampler2D for reading, and an image2D for writing.
        glActiveTexture(GL_TEXTURE0 + maxTextureUnits - i - 1);
        glBindTexture(GL_TEXTURE_2D, tex);
        glBindImageTexture(i, tex, 0, GL_FALSE, 0, GL_WRITE_ONLY, internalformat);
    }
    if (needsClear)
    {
