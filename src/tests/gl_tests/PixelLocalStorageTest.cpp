...
#define PIXEL_LOCAL_DECL_IMPL(NAME_R, NAME_W, UNIT, FORMAT) \
...
void PixelLocalStoragePrototype::beginPixelLocalStorage(GLsizei n, const GLenum *loadOps)
{
    ...
    GLint size;
    glGetFramebufferParameteriv(GL_DRAW_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_WIDTH, &size);
    mFramebufferPreviousDefaultWidth = size;
    glGetFramebufferParameteriv(GL_DRAW_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_HEIGHT, &size);
    mFramebufferPreviousDefaultHeight = size;
...
class PixelLocalStorageTest : public ANGLETest
{
  protected:
    ...
    {
        // Rewrite: PIXEL_LOCAL_DECL(name, ...) -> PIXEL_LOCAL_DECL_IMPL(name_R, name_W, ...)
        //          pixelLocalLoad(name)        -> pixelLocalLoadImpl(name_R)
        //          pixelLocalStore(name, ...)  -> pixelLocalStoreImpl(name_W, ...)
        ...
        std::string fs(fsMain);
        fs = std::regex_replace(fs, kDeclPattern, "PIXEL_LOCAL_DECL_IMPL($1_R, $1_W");
        fs = std::regex_replace(fs, kLoadPattern, "pixelLocalLoadImpl($1_R");
        fs = std::regex_replace(fs, kStorePattern, "pixelLocalStoreImpl($1_W");
...
TEST_P(PixelLocalStorageTest, RGB)
{
    ...
    useProgram(R"(
    PIXEL_LOCAL_DECL(framebuffer, 0, rgba8);
    void main() {
