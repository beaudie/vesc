...
std::ostream &operator<<(std::ostream &os, const PixelLocalGLSLDecl &decl)
{
    os << "layout(binding=" << ((FIRST_LOCAL_STORAGE_IMAGE_UNIT + decl.unit) * 2) << ", "
       << decl.format << ") volatile readonly highp uniform image2D " << decl.name << "_R;\n";
    os << "layout(binding=" << ((FIRST_LOCAL_STORAGE_IMAGE_UNIT + decl.unit) * 2 + 1) << ", "
       << decl.format << ") volatile writeonly highp uniform image2D " << decl.name << "_W;\n";
    return os;
}
...
void PixelLocalStoragePrototype::endPixelLocalStorage()
{
    ...
    {
        glBindImageTexture((FIRST_LOCAL_STORAGE_IMAGE_UNIT + i) * 2, 0, 0, GL_FALSE, 0,
...
class PixelLocalStorageTest : public ANGLETest
{
  protected:
    ...
    void useProgram(std::initializer_list<PixelLocalGLSLDecl> pixelLocalDecls, const char fsMain[])
    {
...
TEST_P(PixelLocalStorageTest, RGBA)
{
    ...
    useProgram({PixelLocalGLSLDecl{0, "rgba8", "framebuffer"}}, R"(
    void main() {
