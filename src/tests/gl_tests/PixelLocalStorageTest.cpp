...
void PixelLocalStoragePrototype::beginPixelLocalStorage(GLsizei n, const GLenum *loadOps)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (IsD3D() || IsOpenGLES())
        {
            // GLES and D3D only allow r32f/r32i/r32ui images to be both read and written in a
            // single shader. When outputting shader code to these backends, we have to pack all PLS
            // formats into r32* images.
...
TEST_P(PixelLocalStorageTest, RGBA8)
{
    ...
    useProgram(R"(
        layout(binding=0, rgba8) mediump uniform pixelLocalANGLE plane1;
        layout(rgba8i, binding=1) mediump uniform ipixelLocalANGLE plane2;
        layout(binding=2, rgba8ui) mediump uniform upixelLocalANGLE plane3;
        void main()
        {
    ...
    {
        // anglebug.com/7524 -- rgba8i, rgba8ui images are not clamped on Swiftshader.
        drawBoxes(pls, {{FULLSCREEN, {2, -1, -2, -3}, {-128, 0, 0, 0}, {1, 0, 0, 0}},
                        {FULLSCREEN, {0, 1, 0, 100}, {0, -128, 0, 0}, {0, 50, 0, 0}},
                        {FULLSCREEN, {0, 0, 1, 0}, {0, 0, -70, 0}, {0, 0, 100, 0}},
                        {FULLSCREEN, {0, 0, 0, -1}, {128, 0, 0, 127}, {0, 0, 0, 255}}});
    }
...
TEST_P(PixelLocalStorageTest, FunctionArguments)
{
    ...
    useProgram(R"(
    layout(binding=0, rgba8) mediump uniform pixelLocalANGLE dst;
    layout(binding=1, rgba8) mediump uniform pixelLocalANGLE src1;
    void store2(mediump pixelLocalANGLE d);
    void store(highp pixelLocalANGLE d, mediump pixelLocalANGLE s)
    {
...
TEST_P(PixelLocalStorageCompilerTest, Declarations)
{
    ...
    #extension GL_ANGLE_shader_pixel_local_storage : require
    layout(binding=0, rgba8) uniform mediump volatile coherent restrict pixelLocalANGLE pls1;
    ...
    #extension GL_ANGLE_shader_pixel_local_storage : require
    layout(binding=0, rgba8) uniform mediump pixelLocalANGLE pls1[1];
    ...
    layout(binding=1, rgba8i) uniform mediump ipixelLocalANGLE pls2;)";
    ...
    struct Foo
    {
        mediump pixelLocalANGLE pls;
    };
    uniform Foo foo;
    uniform PLSBlock
    {
        mediump pixelLocalANGLE blockpls;
    };
    void main()
    {
...
TEST_P(PixelLocalStorageCompilerTest, Discard)
{
    ...
    #extension GL_ANGLE_shader_pixel_local_storage : require
    void f(mediump pixelLocalANGLE pls);  // Function arguments don't trigger PLS restrictions.
    void a()
    {
        discard;
    }
...
TEST_P(PixelLocalStorageCompilerTest, FragmentTestVariables)
{
    ...
    layout(binding=0, rgba8i) mediump uniform ipixelLocalANGLE pls;)";
...
TEST_P(PixelLocalStorageCompilerTest, FunctionArguments)
{
    ...
    void g(layout(rgba8) mediump pixelLocalANGLE pls);
    void main()
    {
    })";
