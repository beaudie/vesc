...
void PixelLocalStorageTest::doStateRestorationTest()
{
    ...
    if (hasDrawBuffersIndexedOES)
    {
        for (int i = 0; i < MAX_DRAW_BUFFERS; ++i)
        {
            ...
            glColorMaski(i, i % 3 == 0, i % 3 == 1, i % 3 == 2, i % 2 == 0);
        }
    }
    else
    {
...
// Check that PLS and KHR_blend_equation_advanced_test can be used together.
TEST_P(PixelLocalStorageTest, ParallelBlendEquationAdvanced)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ANGLE_shader_pixel_local_storage"));
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_KHR_blend_equation_advanced"));
    ANGLE_SKIP_TEST_IF(MAX_COLOR_ATTACHMENTS_WITH_ACTIVE_PIXEL_LOCAL_STORAGE == 0);

    useProgram("#extension GL_KHR_blend_equation_advanced : require", R"(
    layout (blend_support_screen) out;
    layout(binding=0, rgba8) mediump uniform pixelLocalANGLE pls;
    out highp vec4 bea;
    void main()
    {
        bea = pixelLocalLoadANGLE(pls);
        pixelLocalStoreANGLE(pls, vec4(0, 1, 1, 1));
    })");

    PLSTestTexture pls(GL_RGBA8), bea(GL_RGBA8);

    GLFramebuffer fbo;
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glFramebufferTexturePixelLocalStorageANGLE(0, pls, 0, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, bea, 0);
    ...
    EXPECT_PIXEL_RECT_EQ(0, 0, W, H, GLColor::white);
...
#define PLS_INSTANTIATE_RENDERING_TEST(TEST, API)                                                  \
    ANGLE_INSTANTIATE_TEST(                                                                        \
        ...
        PLATFORM(API, OPENGL).enable(Feature::EmulatePixelLocalStorage), /* OpenGL coherent. */    \
        ...
        PLATFORM(API, OPENGLES)                                                                    \
            .enable(Feature::EmulatePixelLocalStorage), /* OpenGL ES coherent. */                  \
        PLATFORM(                                                                                  \
            API,                                                                                   \
            OPENGLES) /* OpenGL ES noncoherent (EXT_shader_framebuffer_fetch_non_coherent). */     \
            .enable(Feature::EmulatePixelLocalStorage)                                             \
            .disable(Feature::SupportsNativeShaderFramebufferFetchEXT),                            \
        PLATFORM(API, OPENGLES) /* OpenGL ES noncoherent (shader images). */                       \
            .enable(Feature::EmulatePixelLocalStorage)                                             \
            .disable(Feature::SupportsNativeShaderFramebufferFetchEXT)                             \
            .disable(Feature::SupportsNativeShaderFramebufferFetchNonCoherentEXT),                 \
        PLATFORM(API, VULKAN)                                                                      \
            .enable(Feature::AsyncCommandQueue)                                                    \
            .enable(Feature::EmulatePixelLocalStorage), /* Vulkan coherent. */                     \
        PLATFORM(API, VULKAN)                           /* Vulkan noncoherent. */                  \
            ...
            .enable(Feature::EmulatePixelLocalStorage),                                            \
        PLATFORM(API, VULKAN) /* Vulkan coherent, GLSL instead of SPIR-V: The coherent versions of \
                                 the extension rely on ARB_fragment_shader_interlock and           \
                                 EXT_shader_framebuffer_fetch. Ensure they work in Vulkan GLSL. */ \
            .enable(Feature::EmulatePixelLocalStorage)                                             \
