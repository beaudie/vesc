...
TEST_P(BlitFramebufferANGLETest, BlitWithMissingAttachments)
{
    ...
    // unbind color attachment
    glFramebufferTexture2D(GL_READ_FRAMEBUFFER_ANGLE, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0);
    // generate INVALID_OPERATION if the read FBO has no color attachment
    glBlitFramebufferANGLE(0, 0, getWindowWidth(), getWindowHeight(), 0, 0, getWindowWidth(),
                           getWindowHeight(), GL_COLOR_BUFFER_BIT, GL_NEAREST);

    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // bind color attachment again
    glFramebufferTexture2D(GL_READ_FRAMEBUFFER_ANGLE, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           mColorOnlyColorBuffer, 0);
}
