...
TEST_P(VulkanImageTest, VulkanImage)
{
    ...
    constexpr GLuint kHeight = 32u;
    GLuint texture           = 0;
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);
    std::vector<uint32_t> pixels(kWidth * kHeight, 0xcfcfcfcf);
    ...
    std::vector<uint32_t> pixels_out(kWidth * kHeight);
    ...
    helper.readPixels(vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, info.format, {}, info.extent,
                      pixels_out.data(), pixels_out.size() * sizeof(uint32_t));
    EXPECT_EQ(pixels_out, std::vector<uint32_t>(kWidth * kHeight, 0xffffffff));
