...
TEST_P(VulkanImageTest, AllocateVMAImageAfterFreeing2DArrayGarbageWhenDeviceOOM)
{
    // Skipping when major version is below 3, since glFenceSync() and glWaitSync() are not
    // supported until 3.2. In addition, the shader used is also written for 3.0.
...
TEST_P(VulkanImageTest, AllocateVMAImageAfterFreeing2DGarbageWhenDeviceOOM)
{
    // Skipping when major version is below 3, since glFenceSync() and glWaitSync() are not
    // supported until 3.2.
    ANGLE_SKIP_TEST_IF(getEGLWindow()->getClientMajorVersion() < 3);
