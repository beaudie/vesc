...
// the device by freeing garbage memory.
TEST_P(VulkanImageTest, AllocateVMAImageAfterFreeingGarbageWhenDeviceOOM)
{
    ANGLE_SKIP_TEST_IF(!getEGLWindow()->isFeatureEnabled(Feature::UseVmaForImageSuballocation));
    ...
    // Use a 2D texture array to allocate some of the available device memory.
    ...
    std::vector<GLColor> textureColor(kTextureWidth * kTextureHeight, GLColor::blue);
