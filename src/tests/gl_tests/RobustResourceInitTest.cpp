...
TEST_P(RobustResourceInitTest, CopyTexSubImage3DTextureWronglyInitialized)
{
    ...
    constexpr GLint width     = 2;
    constexpr GLint height    = 2;
    constexpr GLint depth     = 2;
    constexpr size_t dataSize = width * height * 4;

    GLTexture texture2D;
    glBindTexture(GL_TEXTURE_2D, texture2D);
    constexpr std::array<uint8_t, dataSize> data = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
    ...
    glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, texture3D, 0, layer);
    std::array<uint8_t, dataSize> pixels = {0};
