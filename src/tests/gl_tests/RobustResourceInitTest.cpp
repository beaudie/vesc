...
TEST_P(RobustResourceInitTest, ExtensionInit)
{
    ...
    else
    {
        ...
        getEGLWindow()->setRobustResourceInit(false);
        ASSERT_TRUE(getEGLWindow()->initializeGL(GetOSWindow()));

        // Context extension string should not be exposed.
        EXPECT_FALSE(extensionEnabled("GL_ANGLE_robust_resource_initialization"));
