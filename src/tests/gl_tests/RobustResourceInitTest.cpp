...
// FIXME: jmadill
TEST_P(RobustResourceInitTestES3, DISABLED_CompressedTexSubImage2DInitialization)
{
    ANGLE_SKIP_TEST_IF(!hasGLExtension());
...
{
    ANGLE_SKIP_TEST_IF(!hasEGLExtension() || !hasRobustSurfaceInit());

    checkFramebufferNonZeroPixels(0, 0, 0, 0, GLColor::black);
}

// Tests that surfaces are initialized after swapping if they are not preseved
