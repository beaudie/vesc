...
void main() {
    ...
    for (int testcase = 0; testcase < 4; ++testcase)
    {
        if (testcase == 0)
        {
            mProgram =
                CompileProgram(essl3_shaders::vs::Simple(), kFragColorShader, [](GLuint program) {
                    // Specs aren't very clear on what kind of name should be used when binding
                    // location for array variables. We only allow names that do include the "[0]"
                    // suffix.
                    glBindFragDataLocationEXT(program, kUnusedLocation, "FragData[0]");
                    glBindFragDataLocationEXT(program, kFragData0Location, "FragData");
                    glBindFragDataLocationEXT(program, kFragData1Location, "FragData[1]");
                });
        }
        else if (testcase == 1)
        {
            mProgram =
                CompileProgram(essl3_shaders::vs::Simple(), kFragColorShader, [](GLuint program) {
                    ...
                    // location for array variables. We only allow names that do include the "[0]"
                    // suffix.
                    glBindFragDataLocationEXT(program, kUnusedLocation, "FragData");
                    glBindFragDataLocationEXT(program, kFragData0Location, "FragData[0]");
                    glBindFragDataLocationEXT(program, kFragData1Location, "FragData[1]");
                });
        }
        else if (testcase == 2)
        {
            mProgram =
                CompileProgram(essl3_shaders::vs::Simple(), kFragColorShader, [](GLuint program) {
                    ...
                    // location for array variables. We only allow names that do include the "[0]"
                    // suffix.
                    glBindFragDataLocationIndexedEXT(program, kUnusedLocation, 0, "FragData[0]");
                    glBindFragDataLocationIndexedEXT(program, kFragData0Location, 0, "FragData");
                    glBindFragDataLocationIndexedEXT(program, kFragData1Location, 0, "FragData[1]");
                });
        }
        else if (testcase == 3)
        {
            mProgram =
                CompileProgram(essl3_shaders::vs::Simple(), kFragColorShader, [](GLuint program) {
                    ...
                    // location for array variables. We only allow names that do include the "[0]"
                    // suffix.
                    glBindFragDataLocationIndexedEXT(program, kUnusedLocation, 0, "FragData");
                    glBindFragDataLocationIndexedEXT(program, kFragData0Location, 0, "FragData[0]");
                    glBindFragDataLocationIndexedEXT(program, kFragData1Location, 0, "FragData[1]");
                });
        }
        ...
        LinkProgram();
        GLint getFragDataLocation = glGetFragDataLocation(mProgram, "FragData");
        EXPECT_EQ(kFragData0Location, getFragDataLocation);
