...
TEST_P(VertexAttributeTestES31, MaxVertexAttribRelativeOffset)
{
    ...
    glGetIntegerv(GL_MAX_VERTEX_ATTRIB_STRIDE, &maxRelativeOffset);
    EXPECT_GL_NO_ERROR();
...
TEST_P(VertexAttributeTestES31, DrawArraysWithLargeStride)
{
    ...
    // To support all platforms, use 2048 (minimum MAX_VERTEX_ATTRIB_STRIDE in ES3.1 ) as stride if
    // maxStride is larger than this value.
    GLint largeStride = std::min(ES31_MAX_STRIDE, maxStride);
...
TEST_P(VertexAttributeTestES31, DrawArraysWithLargeOffset)
{
    ...
    // To support all platforms, use 2047 (minimum MAX_VERTEX_ATTRIB_RELATIVE_OFFSET in ES3.1) as
    // offset if maxOffset is larger than this value.
    GLint largeOffset = std::min(ES31_MAX_ATTRIB_RELATIVE_OFFSET, maxRelativeOffset);
