...
TEST_P(VertexAttributeTest, DrawArraysWithDisabledAttribute)
{
    ...
    std::array<GLfloat, mVertexCount> inputData;
    std::array<GLfloat, mVertexCount> expectedData;
    for (size_t count = 0; count < mVertexCount; ++count)
    {
        inputData[count]    = static_cast<GLfloat>(count);
        expectedData[count] = inputData[count];
    ...
    ANGLE_GL_PROGRAM(program2, testVertexShaderSource2, testFragmentShaderSource);
    GLuint mProgram2 = program2.get();
