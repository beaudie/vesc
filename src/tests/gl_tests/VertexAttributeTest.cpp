...
class VertexAttributeTest : public ANGLETest
{
  protected:
    ...
    void checkRGBPixels(bool includeAlpha)
    {
        ...
        for (GLint i = 0; i < 4; i++)
        {
            ...
            if (checkPixelEqual)
            {
                if ((test.type == GL_HALF_FLOAT || test.type == GL_HALF_FLOAT_OES) && IsVulkan() &&
                    typeSize == 3)
                {  // We need a special care for RGB16F format in case of Vulkan backend since
                   // we convert RGB16F to RGBA16F and new ahpha value would be 0 in this case.
