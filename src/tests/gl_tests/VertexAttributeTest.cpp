...
void main() {
    ...
    GLuint kColorTestData[kDataSize + 1];
    GLushort *offsetPtr = reinterpret_cast<GLushort *>(kColorTestData) + 1;
    for (size_t dataIndex = 0; dataIndex < kDataSize; dataIndex++)
    {
        GLuint *curPtr = reinterpret_cast<GLuint *>(offsetPtr) + dataIndex;
        *curPtr        = 1;
    }
