...
class VertexAttributeTestES31 : public VertexAttributeTestES3
{
  protected:
    ...
    void initOnlyUpdateBindingTest()
    {
        initTest();
        ...
        ASSERT_NE(kTestBinding, mTestAttrib);
        ASSERT_NE(kTestBinding, mExpectedAttrib);

        // Set mTestAttrib using the binding kTestBinding.
        glVertexAttribFormat(mTestAttrib, 1, GL_FLOAT, GL_FALSE, 0);
        glBindVertexBuffer(kTestBinding, testBuffer1, kTestFloatOffset1 * kFloatStride,
                           kFloatStride);
    ...
    // Default binding index for tests.
    static constexpr GLint kTestBinding = 10;
