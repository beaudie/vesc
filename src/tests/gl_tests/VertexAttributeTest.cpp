  ...
  protected:
    ...
    void setupTest(const TestData &test, GLint typeSize)
    {
        ...
        if (test.source == Source::BUFFER)
        {
            ...
            glBufferData(GL_ARRAY_BUFFER, dataSize + test.bufferOffset, nullptr, GL_STATIC_DRAW);
            glBufferSubData(GL_ARRAY_BUFFER, test.bufferOffset, dataSize, test.inputData);
