...
class VertexAttributeTestES31 : public VertexAttributeTestES3
{
  protected:
    ...
    bool IsOverflowWithFloat(GLint stride, GLint offset)
    {
        GLint effectiveStride = stride ? (stride / TypeStride(GL_FLOAT)) : 1;
        GLint totalData       = effectiveStride * TypeStride(GL_FLOAT) * mVertexCount + offset;
        return totalData < offset;
    }
...
// Verify using value of MAX_VERTEX_ATTRIB_STRIDE as stride doesn't mess up the draw as long as
// there is no math overflow.
TEST_P(VertexAttributeTestES31, DrawArraysWithLargeStride)
{
    ...
    GLint largeStride = IsOverflowWithFloat(maxStride, 0) ? ES31_MAX_STRIDE : maxStride;
    drawArraysWithStrideAndOffset(largeStride, 0);
}

// Verify using value of MAX_VERTEX_ATTRIB_RELATIVE_OFFSET as offset doesn't mess up the draw as
// long as there is no math overflow.
TEST_P(VertexAttributeTestES31, DrawArraysWithLargeOffset)
{
    ...
    GLint largeOffset = IsOverflowWithFloat(0, maxRelativeOffset) ? ES31_MAX_ATTRIB_RELATIVE_OFFSET
                                                                  : maxRelativeOffset;
