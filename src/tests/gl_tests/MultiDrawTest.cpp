...
constexpr uint32_t x_count                = 8;
constexpr uint32_t y_count                = 8;
...
using MultiDrawTestParams = std::tuple<angle::PlatformParameters, bool, bool>;
...
struct PrintToStringParamName
{
    ...
    {
        ::std::stringstream ss;
        ss << (std::get<2>(info.param) ? "Instanced_" : "")
           << (std::get<1>(info.param) ? "DrawID_" : "") << std::get<0>(info.param);
        return ss.str();
    }
};
...
class MultiDrawTest : public ANGLETestBase, public ::testing::TestWithParam<MultiDrawTestParams>
{
  protected:
    ...
    bool IsDrawIDTest() const { return std::get<1>(GetParam()); }

    bool IsInstancedTest() const { return std::get<2>(GetParam()); }
...
void main()
{
    ...
    void DoDrawArrays()
    {
        ...
        if (IsInstancedTest())
        {
            ...
            std::vector<GLsizei> instanceCounts(tri_count, 4);
            glMultiDrawArraysInstancedANGLE(GL_TRIANGLES, firsts.data(), counts.data(),
                                            instanceCounts.data(), tri_count);
        }
        else
        {
            glMultiDrawArraysANGLE(GL_TRIANGLES, firsts.data(), counts.data(), tri_count);
        }
    }
    ...
    void DoDrawElements()
    {
        ...
        if (IsInstancedTest())
        {
            ...
            glMultiDrawElementsInstancedANGLE(GL_TRIANGLES, counts.data(), GL_UNSIGNED_SHORT,
                                              offsets.data(), instanceCounts.data(), tri_count);
        }
        else
        {
    ...
    void CheckDrawResult()
    {
        for (uint32_t y = 0; y < y_count; ++y)
        {
            for (uint32_t x = 0; x < x_count; ++x)
            {
                ...
                uint32_t quadID               = IsDrawIDTest() ? y * x_count + x : 0;
                ...
                if (IsInstancedTest())
                {
                    EXPECT_PIXEL_RECT_EQ(center_x / 2 - pixelCheckSize[0] / 4,
                                         center_y / 2 - pixelCheckSize[1] / 4,
                                         pixelCheckSize[0] / 2, pixelCheckSize[1] / 2, expected);
                    EXPECT_PIXEL_RECT_EQ(center_x / 2 - pixelCheckSize[0] / 4 + width / 2,
                                         center_y / 2 - pixelCheckSize[1] / 4,
                                         pixelCheckSize[0] / 2, pixelCheckSize[1] / 2, expected);
                    EXPECT_PIXEL_RECT_EQ(center_x / 2 - pixelCheckSize[0] / 4,
                                         center_y / 2 - pixelCheckSize[1] / 4 + height / 2,
                                         pixelCheckSize[0] / 2, pixelCheckSize[1] / 2, expected);
                    EXPECT_PIXEL_RECT_EQ(center_x / 2 - pixelCheckSize[0] / 4 + width / 2,
                                         center_y / 2 - pixelCheckSize[1] / 4 + height / 2,
                                         pixelCheckSize[0] / 2, pixelCheckSize[1] / 2, expected);
                ...
                {
                    EXPECT_PIXEL_RECT_EQ(center_x - pixelCheckSize[0] / 2,
                                         center_y - pixelCheckSize[1] / 2, pixelCheckSize[0],
                                         pixelCheckSize[1], expected);
                }
            }
        }
    }
