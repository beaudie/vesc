...
TEST_P(ProgramInterfaceTestES31, GetResource)
{
    ...
    constexpr int kPropCount = ArraySize(props);
    GLint params[kPropCount];
    GLsizei length;

    glGetProgramResourceiv(program, GL_PROGRAM_INPUT, index, kPropCount, props, kPropCount, &length,
                           params);
    EXPECT_GL_NO_ERROR();
    ...
    glGetProgramResourceiv(program, GL_PROGRAM_OUTPUT, index, kPropCount, props, kPropCount - 1,
                           &length, params);
    EXPECT_GL_NO_ERROR();
...
TEST_P(ProgramInterfaceTestES31, GetUniformProperties)
{
    ...
    constexpr int kPropCount = ArraySize(props);
    GLint params[kPropCount];
    glGetProgramResourceiv(program, GL_UNIFORM, index, kPropCount, props, kPropCount, &length,
                           params);
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(kPropCount, length);
    EXPECT_EQ(GL_FLOAT_VEC4, params[0]);  // type
...
TEST_P(ProgramInterfaceTestES31, GetUniformBlockProperties)
{
    ...
    RNG rng;
    GLint intact = rng.randomInt();

    // Tests bufSize is respected even some prop returns more than one value.
    params[kPropCount] = intact;
    glGetProgramResourceiv(program, GL_UNIFORM_BLOCK, index, kPropCount, props, kPropCount, &length,
                           params);
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(kPropCount, length);
    ...
    EXPECT_EQ(intact, params[8]);

    glGetProgramResourceiv(program, GL_UNIFORM_BLOCK, index, kPropCount, props, kBufSize, &length,
                           params);
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(kPropCount + 1, length);
...
TEST_P(ProgramInterfaceTestES31, QueryAtomicCounteBuffer)
{
    ...
    GLint params[kPropCount];
    GLsizei length = 0;
    glGetProgramResourceiv(program, GL_ATOMIC_COUNTER_BUFFER, 0, kPropCount, props, kPropCount,
                           &length, params);
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(kPropCount, length);
    EXPECT_EQ(2, params[0]);  // buffer_binding
