...
TEST_P(ProgramInterfaceTestES31, GetBufferVariableProperties)
{
    const std::string &vertexShaderSource =
        ...
        "    float f2;\n"
        "};\n"
        "layout(binding = 3) buffer blockName3 {\n"
        "    int i3;\n"
        "};\n"
        "void main()\n"
        "{\n"
        "    uint test1 = instanceName1[0].u1[0];\n"
        "    float test2 = instanceName1[1].f1;\n"
        "    int test3 = i3;\n"
        "}\n";
    ...
    std::array<GLint, kPropCount> params;
    std::array<GLenum, kPropCount> props = {
        {GL_ARRAY_SIZE, GL_ARRAY_STRIDE, GL_BLOCK_INDEX, GL_IS_ROW_MAJOR, GL_MATRIX_STRIDE,
         GL_NAME_LENGTH, GL_OFFSET, GL_REFERENCED_BY_VERTEX_SHADER,
         GL_REFERENCED_BY_FRAGMENT_SHADER, GL_REFERENCED_BY_COMPUTE_SHADER, GL_TOP_LEVEL_ARRAY_SIZE,
         GL_TOP_LEVEL_ARRAY_STRIDE, GL_TYPE}};
    glGetProgramResourceiv(program, GL_BUFFER_VARIABLE, index, kPropCount, props.data(), kPropCount,
                           &length, params.data());
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(kPropCount, length);
    EXPECT_EQ(0, params[0]);   // array_size
    EXPECT_LE(0, params[1]);   // array_stride
    EXPECT_LE(0, params[2]);   // block_index
    EXPECT_EQ(0, params[3]);   // is_row_major
    EXPECT_LE(0, params[4]);   // matrix_stride
    ...
    EXPECT_EQ(GL_INT_VEC3, params[12]);  // type
}
...
TEST_P(ProgramInterfaceTestES31, GetShaderStorageBlockProperties)
{
    ...
    constexpr int kBufSize   = 50;
    std::array<GLint, kBufSize> params;
    std::array<GLenum, kPropCount> props = {
        {GL_ACTIVE_VARIABLES, GL_BUFFER_BINDING, GL_NUM_ACTIVE_VARIABLES, GL_BUFFER_DATA_SIZE,
         GL_NAME_LENGTH, GL_REFERENCED_BY_VERTEX_SHADER, GL_REFERENCED_BY_FRAGMENT_SHADER,
         GL_REFERENCED_BY_COMPUTE_SHADER}};
    glGetProgramResourceiv(program, GL_SHADER_STORAGE_BLOCK, index, kPropCount, props.data(),
                           kBufSize, &length, params.data());
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(13, length);
    ...
    EXPECT_EQ(0, params[10]);  // referenced_by_vertex_shader
