...
{
    EXPECT_TRUE(!extensionEnabled("GL_OES_geometry_shader") &&
                !extensionEnabled("GL_EXT_geometry_shader"));
    GLuint geometryShader = glCreateShader(GL_GEOMETRY_SHADER_OES);
    EXPECT_EQ(0u, geometryShader);
    EXPECT_GL_ERROR(GL_INVALID_ENUM);
}

// Verify that GL_OES_geometry_shader and GL_EXT_geometry_shader are always both enabled or
// disabled.
TEST_P(GeometryShaderTest, GeometryShaderExtensionString)
{
    EXPECT_TRUE(extensionEnabled("GL_OES_geometry_shader") ==
                extensionEnabled("GL_EXT_geometry_shader"));
}
