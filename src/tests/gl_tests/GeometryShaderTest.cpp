...
TEST_P(GeometryShaderTest, ReferencedByGeometryShaderWithoutExtensionEnabled)
{
    ...
    constexpr GLenum props[]    = {GL_REFERENCED_BY_GEOMETRY_SHADER_EXT};
    constexpr GLsizei propCount = static_cast<GLsizei>(ArraySize(props));
...
TEST_P(GeometryShaderTest, ReferencedByGeometryShader)
{
    ...
    constexpr GLenum props[]    = {GL_REFERENCED_BY_GEOMETRY_SHADER_EXT};
    constexpr GLsizei propCount = static_cast<GLsizei>(ArraySize(props));
    std::array<GLint, ArraySize(props)> params;
    GLsizei length;
    ...
    glGetProgramResourceiv(program, GL_PROGRAM_OUTPUT, index, propCount, props, propCount, &length,
                           params.data());
    EXPECT_GL_NO_ERROR();
    EXPECT_EQ(0, params[0]);
    ...
    if (maxGeometryShaderStorageBlocks > 0)
    {
        ...
        glGetProgramResourceiv(programWithSSBO, GL_SHADER_STORAGE_BLOCK, index, propCount, props,
                               propCount, &length, params.data());
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(1, params[0]);

        params.fill(0);
        index = glGetProgramResourceIndex(programWithSSBO, GL_BUFFER_VARIABLE, "ssbo.ssbo_value");
        glGetProgramResourceiv(programWithSSBO, GL_BUFFER_VARIABLE, index, propCount, props,
                               propCount, &length, params.data());
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(1, params[0]);
    }
    ...
    if (maxGeometryAtomicCounterBuffers > 0)
    {
        ...
        EXPECT_GL_NO_ERROR();
        glGetProgramResourceiv(programWithAtomicCounter, GL_ATOMIC_COUNTER_BUFFER, index, propCount,
                               props, propCount, &length, params.data());
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(1, params[0]);
    }
}
