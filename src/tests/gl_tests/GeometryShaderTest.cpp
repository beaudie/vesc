...
class GeometryShaderTest : public ANGLETest
{
  protected:
    ...
    const std::string &emptyVertexShader =
        R"(#version 310 es
        void main()
        {
    ...
    const std::string &emptyFragmentShader =
        R"(#version 310 es
...
TEST_P(GeometryShaderTest, LinkWithUncompiledGeoemtryShader)
{
    ...
    GLuint vertexShader   = CompileShader(GL_VERTEX_SHADER, emptyVertexShader);
    GLuint fragmentShader = CompileShader(GL_FRAGMENT_SHADER, emptyFragmentShader);
...
TEST_P(GeometryShaderTest, LinkValidationOnGeometryShader)
{
    ANGLE_SKIP_TEST_IF(!extensionEnabled("GL_EXT_geometry_shader"));
    ...
    // link failure.
    GLuint program = CompileProgram(emptyVertexShader, gsWithoutInvocations, emptyFragmentShader);
    ...
    program = CompileProgram(emptyVertexShader, gsWithoutInputPrimitive, emptyFragmentShader);
    EXPECT_EQ(0u, program);

    program = CompileProgram(emptyVertexShader, gsWithoutOutputPrimitive, emptyFragmentShader);
    EXPECT_EQ(0u, program);

    program = CompileProgram(emptyVertexShader, gsWithoutMaxVertices, emptyFragmentShader);
    EXPECT_EQ(0u, program);
