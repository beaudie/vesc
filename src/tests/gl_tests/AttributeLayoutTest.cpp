...
class AttributeLayoutTest : public ANGLETest
{
  protected:
    ...
    void Run(bool firstTriangleDrawn)
    {
        ...
        for (unsigned i = 0; i < mTestCases.size(); ++i)
        {
            ...
            if (firstTriangleDrawn)
            {
                EXPECT_PIXEL_EQ(w * 2, h * 3, 255, 255, 0, 0) << " test case " << i;
            }
            else
            {
                EXPECT_PIXEL_EQ(w * 2, h * 3, 51, 51, 51, 0) << " test case " << i;
            }
...
{
    void Draw(void) override { glDrawArrays(GL_TRIANGLES, 0, kNumVertices); }
};

class AttributeLayoutNonIndexedNonZeroStart : public AttributeLayoutTest
{
    void Draw(void) override { glDrawArrays(GL_TRIANGLES, 3, kNumVertices - 3); }
};
...
class AttributeLayoutMemoryIndexed : public AttributeLayoutTest
{
    void Draw(void) override
    {
        ...
        glDrawElements(GL_TRIANGLES, kNumVertices, GL_UNSIGNED_SHORT, mIndices);
    }
};
...
class AttributeLayoutBufferIndexed : public AttributeLayoutTest
{
    ...
    {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mIndexBuffer);
        glDrawElements(GL_TRIANGLES, kNumVertices, GL_UNSIGNED_SHORT, nullptr);
    }
};
...
TEST_P(AttributeLayoutNonIndexedNonZeroStart, Test)
{
    if (IsWindows() && IsATI() && IsOpenGL())
    {
        std::cout << "test skipped on Windows ATI OpenGL: non-zero vertex start" << std::endl;
        return;
    }
    Run(false);
}
...
ANGLE_INSTANTIATE_TEST(AttributeLayoutNonIndexedNonZeroStart, PARAMS);
