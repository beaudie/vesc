  ...
  protected:
    ...
    void Run(bool drawFirstTriangle)
    {
        ...
        for (unsigned i = 0; i < mTestCases.size(); ++i)
        {
            ...
            std::cout << "trying case " << i << std::endl;
...
void AttributeLayoutTest::GetTestCases(void)
{
    ...
    // 6. normalized byte
    if (IsVulkan() && IsWindows() && IsAMD())
    {
        // TODO(fjhenigman): Enable this case once vertex format conversion is implemented.  anglebug.com/2405
    ...
    // 7. normalized short
    if (IsAndroid() || (IsVulkan() && IsWindows() && IsAMD()))
    {
        // TODO(fjhenigman): Enable this case once vertex format conversion is implemented.  anglebug.com/2405
        mTestCases.push_back({});
    }
    ...
    if (IsVulkan())
    {
        std::cout << "cases skipped on Vulkan: fixed or non-normalized byte/short data, non-zero buffer offsets" << std::endl;
        return;
    }
