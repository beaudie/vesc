...
typedef void (*Convert)(const VertexData &src, char *dst, unsigned strideInBytes);
...
struct Format
{
    ...
    static constexpr size_t CTypeSize = sizeof(CType);
    static constexpr double kMaxInt   = (1ull << (8 * CTypeSize)) - 1;

    static_assert(!(kNormalized && kGLType == GL_FLOAT), "Normalized float does not make sense.");

    static CType convert(double value)
    {
        if (kGLType == GL_FIXED)
            return value * (1 << 16);
        if (kNormalized)
            return std::is_signed<CType>::value ? ((value * kMaxInt) - 1) / 2 : value * kMaxInt;
        return value;
    }

    static void convert(const VertexData &src, char *dst, unsigned strideInBytes)
    {
        for (unsigned i = 0; i < kNumVertices; ++i)
        {
            CType *cdst = reinterpret_cast<CType *>(dst + i * strideInBytes);
            for (int j = 0; j < src.getDimension(); ++j)
            {
                cdst[j] = convert(src.getValue(i, j));
            }
        }
    }
};
...
class Attrib
{
  public:
    ...
    {
        ASSERT(mStride >= Format::CTypeSize * data.getDimension());
    ...
    void openContainer(void) const { memset(mContainer->mMemory, 0xff, mContainer->kSize); }

    //TODO check for overflow
    void fillContainer(void) const { mConvert(mData, mContainer->mMemory + mOffset, mStride); }
