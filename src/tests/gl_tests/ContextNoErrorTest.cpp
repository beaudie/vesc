...
class ContextNoErrorInvalidTextureType : public ContextNoErrorTest
{};

// Test that we ignore an invalid texture type when EGL_KHR_create_context_no_error is enabled.
// Only the vulkan backend has the workaround needed for this usecase.
TEST_P(ContextNoErrorInvalidTextureType, Basic)
{
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(ContextNoErrorInvalidTextureType);
ANGLE_INSTANTIATE_TEST(ContextNoErrorInvalidTextureType,
                       WithIgnoreInvalidTextureType(ES2_VULKAN()),
                       WithIgnoreInvalidTextureType(ES3_VULKAN()));
