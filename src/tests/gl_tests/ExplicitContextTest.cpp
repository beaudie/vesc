...
namespace angle
{
    ...
    protected:
        ...
        {
            if (!extensionEnabled("EGL_ANGLE_explicit_context"))
            {
                std::cout << "Test skipped because EGL_ANGLE_explicit_context is not available."
                    << std::endl;
                return false;
            }
...
TEST_P(ExplicitContextTest, GetProcAddress)
{
    ...
    PFNGLCLEARCOLORCONTEXTANGLE clearColor = reinterpret_cast<PFNGLCLEARCOLORCONTEXTANGLE>(eglGetProcAddress("glClearColorContextANGLE"));
    PFNGLCLEARCONTEXTANGLE clear = reinterpret_cast<PFNGLCLEARCONTEXTANGLE>(eglGetProcAddress("glClearContextANGLE"));
