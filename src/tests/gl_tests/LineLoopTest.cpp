...
class LineLoopIndirectTest : public LineLoopTest
{
  protected:
    ...
    void preTestUBOAndInitUpdateBuffers(GLuint vertexArray,
                                        ...
                                        GLuint arrayUpdateTex,
                                        GLuint elementUpdateFbo,
                                        GLuint elementUpdateTex)
    {
        preTestUpdateBuffer(arrayUpdateFbo, arrayUpdateTex, vertBuffer, positionsSize);
    ...
    void initIndirectForLineLoopRectangle(GLuint indirectBuffer, GLuint firstIndex)
    {
    ...
    void setVertexAttribs(const void *positions, GLColor32F color)
    {
        glEnableVertexAttribArray(mPositionLocation);
...
TEST_P(LineLoopIndirectTest, UByteIndexIndirectBuffer)
{
    ...
    initUpdateBuffers(vertexArray, vertBuffer, buf, kLoopPositions, sizeof(kLoopPositions), indices,
                      sizeof(indices));

    GLBuffer indirectBuffer;
    initIndirectForLineLoopRectangle(indirectBuffer, firstIndex);

    glEnable(GL_BLEND);
    setVertexAttribs(nullptr, GLColor32F(0.0f, 0.0f, 1.0f, 1.0f));
    ...
    setVertexAttribs(kStripPositions, GLColor32F(0.0f, 1.0f, 0.0f, 1.0f));
...
TEST_P(LineLoopIndirectTest, UseAsUBOThenUpdateThenUByteIndexIndirectBuffer)
{
    ...
    preTestUBOAndInitUpdateBuffers(vertexArray, vertBuffer, buf, kLoopPositions,
...
// Test that two back-to-back indirect line-loop draws that share the same index buffer works.
...
TEST_P(LineLoopIndirectTest, IndirectAndElementDrawsShareIndexBuffer)
{
    ...
    // The line-loop and the non-line-loop calls should draw the same object.
    GLuint firstIndex       = 10;
    const GLubyte indices[] = {0, 6, 9, 8, 7, 6, 0, 0, 9, 8, 7, 6, 9, 8, 0};
    ...
    ASSERT_GL_NO_ERROR();
    glDrawElements(GL_LINE_STRIP, 5, GL_UNSIGNED_BYTE, reinterpret_cast<const void *>(8));
    ASSERT_GL_NO_ERROR();
