...
// Regression test for a user bug
TEST_P(LineLoopTest, Bug)
{
...
void main() {
    ...
    const int timeLoc        = glGetUniformLocation(program, "time");
    const int vertexCountLoc = glGetUniformLocation(program, "vertexCount");
    const int resolutionLoc  = glGetUniformLocation(program, "resolution");

    glUniform1f(timeLoc, 123.345);
    glUniform1f(vertexCountLoc, 2048);
    glUniform2f(resolutionLoc, getWindowWidth(), getWindowHeight());

    std::vector<float> vertexId(2048);
    for (size_t i = 0; i < vertexId.size(); ++i)
        vertexId[i] = (float)i;

    GLBuffer buffer;
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glBufferData(GL_ARRAY_BUFFER, 2048 * sizeof(float), vertexId.data(), GL_STATIC_DRAW);
