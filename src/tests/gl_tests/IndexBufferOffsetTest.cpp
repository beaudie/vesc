...
void main()
{
                 ...
                 bool useBuffersAsUboFirst)
    {
        ...
        {
            assert(updateType == UpdateType::FullUpdate);
...
// Test using an offset for an UInt32 index buffer after uploading data to a buffer that is in use,
// with small buffer updates
TEST_P(IndexBufferOffsetTestES3, UseAsUBOThenUpdateThenUInt32IndexSmallThenBigUpdates)
{
    ANGLE_SKIP_TEST_IF(getClientMajorVersion() < 3 &&
                       !IsGLExtensionEnabled("GL_OES_element_index_uint"));

    // http://anglebug.com/5957
    ANGLE_SKIP_TEST_IF(IsVulkan() && (IsPixel2() || IsPixel2XL() || IsPixel4() || IsPixel4XL()));
    ANGLE_SKIP_TEST_IF(/*IsOpenGLES() &&*/ /*(IsPixel4() || IsPixel4XL())*/ IsAdreno());

    GLuint indexData[] = {0, 1, 2, 1, 2, 3};
    runTest(GL_UNSIGNED_INT, 4, indexData, UpdateType::SmallThenBigUpdate, true);
}

TEST_P(IndexBufferOffsetTestES3, UseAsUBOThenUpdateThenUInt32IndexBigThenSmallUpdates)
{
    ANGLE_SKIP_TEST_IF(getClientMajorVersion() < 3 &&
                       !IsGLExtensionEnabled("GL_OES_element_index_uint"));
