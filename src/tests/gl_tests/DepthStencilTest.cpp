...
static uint32_t clearFirstByte(uint32_t pixel)
{
    union
    {
        uint32_t i;
    ...
    uint32_t_union.i        = pixel;
    uint32_t_union.bytes[0] = 0;
    return uint32_t_union.i;
}
...
void DepthStencilTestES3::compareDepth(uint32_t expected)
{
    ...
    // Only compare 24 bits, as the unused byte may differ between GL and Vulkan back ends.
    pixel    = clearFirstByte(pixel);
    expected = clearFirstByte(expected);
