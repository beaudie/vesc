...
class DepthStencilTest : public ANGLETest
{
  protected:
    ...
    // Override a feature to force emulation of stencil-only and depth-only formats with a packed depth/stencil format
...
void DepthStencilTest::ensureDepthUnaffected()
{
    ...
    // Since depth shouldn't exist, the drawQuad above should succeed in turning the whole image blue.
...
void DepthStencilTest::ensureStencilUnaffected()
{
    ...
    // Since stencil shouldn't exist, the drawQuad above should succeed in turning the whole image green.
...
void DepthStencilTest::prepareSingleEmulatedWithPacked()
{
    ...
    // Use masked color to clear two channels of the image to a second color, 0.25 depth and 0x59 stencil.
    ...
    // Use scissor to draw to the right half of the image with a fourth color, 0.6 depth and 0x84 stencil.
    glEnable(GL_SCISSOR_TEST);
