...
#include "gl_enum_to_string_formats_and_types.h"
...
TEST_P(FormatPrintTest, PrintAllFormatSupport)
{
    ...
    for (const auto &internalFormat : allSupportedFormats)
    {
        for (const auto &typeFormatPair : internalFormat.second)
        {
            ...
            std::string internalFormatString(
                ::gl::GLenumToStringFormatsAndTypes(internalFormat.first));
            std::string typeString(::gl::GLenumToStringFormatsAndTypes(typeFormatPair.first));

            // Omit any internal format or type that's either not in gl.xml or part of an
            // unsupported extension, like an ANGLE extension defined format
            if (internalFormatString.compare("EnumUnknown") == 0 ||
                typeString.compare("EnumUnknown") == 0)
            {
                continue;
            }
