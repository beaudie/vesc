...
class CopyTexture3DTest : public ANGLETest
{
  protected:
                  ...
                  const GLColor &expectedColor)
    {
        ...
        glCopyTexture3DANGLE(sourceTexture.get(), 0, testTarget, destTexture.get(), 0, destInternalFormat,
...
TEST_P(Texture3DCopy, CopySubTexture)
{
    ...
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAX_LEVEL, 0);
    EXPECT_GL_NO_ERROR();
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_3D, destTexture.get(), 0, 0, 0, 0, 0, 0, 0, 2, 2,
                            2, false, false, false);
    glBindTexture(GL_TEXTURE_3D, destTexture.get());
    EXPECT_GL_NO_ERROR();
    drawQuad(mProgram, "position", 0.5f);
...
TEST_P(Texture3DCopy, CopyFromMipmap)
{
    ...
    glCopyTexture3DANGLE(sourceTexture.get(), 1, GL_TEXTURE_3D, destTexture.get(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         false, false, false);
    glBindTexture(GL_TEXTURE_3D, destTexture.get());
    EXPECT_GL_NO_ERROR();
    drawQuad(mProgram, "position", 0.5f);
...
TEST_P(Texture3DCopy, OffsetSubCopy)
{
    ...
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_3D, destTexture.get(), 0, 0, 0, 0, 1, 1, 1, 2, 2,
                            2, false, false, false);
    glBindTexture(GL_TEXTURE_3D, destTexture.get());
    EXPECT_GL_NO_ERROR();
    ...
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_3D, destTexture.get(), 0, 1, 1, 1, 0, 0, 0, 1, 1,
                            1, false, false, false);
    EXPECT_GL_NO_ERROR();
    drawQuad(mProgram, "position", 1.0f);
    EXPECT_PIXEL_COLOR_EQ(0, 0, GLColor::green);
...
TEST_P(Texture3DCopy, FlipY)
{
    ...
    glCopyTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_3D, destTexture.get(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         true, false, false);
...
TEST_P(Texture2DArrayCopy, CopySubTexture)
{
    ...
    glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAX_LEVEL, 0);
    EXPECT_GL_NO_ERROR();
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_2D_ARRAY, destTexture.get(), 0, 0, 0, 0, 0, 0, 0,
...
TEST_P(Texture2DArrayCopy, OffsetSubCopy)
{
    ...
    EXPECT_GL_NO_ERROR();
    // Copy the 2x2x2 green cube into a new texture
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_2D_ARRAY, destTexture.get(), 0, 0, 0, 0, 1, 1, 1,
                            2, 2, 2, false, false, false);
    glBindTexture(GL_TEXTURE_2D_ARRAY, destTexture.get());
    EXPECT_GL_NO_ERROR();
    drawQuad(mProgram, "position", 1.0f);
    ...
    // Copy the 1x1x1 blue cube into the the 2x2x2 green cube at location (1, 1, 1)
    glCopySubTexture3DANGLE(sourceTexture.get(), 0, GL_TEXTURE_2D_ARRAY, destTexture.get(), 0, 1, 1, 1, 0, 0, 0,
