  ...
  protected:
                  ...
                  const GLColor &expectedColor)
    {
        ...
        if (renderType == GL_RGBA8)
        {
            ...
            if (tolerance != 1 && IsNVIDIA() && IsVulkan())
            {
...
{
    {
        ...
        // TODO(syoussefi): sRGB copy in Vulkan is forced to use the CPU path, which doesn't handle
        // 2DArray textures correctly yet.  http://anglebug.com/4744
        if (!(IsVulkan() && testTarget == GL_TEXTURE_2D_ARRAY))
        {
            ...
            testCopy(testTarget, kColorNoAlpha, GL_SRGB8_ALPHA8, GL_UNSIGNED_BYTE, false, false,
                     false, GLColor(244, 148, 78, 100));
            testCopy(testTarget, kColorPreAlpha, GL_SRGB8_ALPHA8, GL_UNSIGNED_BYTE, false, true,
                     false, GLColor(31, 19, 11, 100));
            testCopy(testTarget, kColorUnAlpha, GL_SRGB8_ALPHA8, GL_UNSIGNED_BYTE, false, false,
                     true, GLColor(52, 253, 53, 100));
        }
...
void CopyTexture3DTest::testFloatFormats(const GLenum testTarget)
{
    ...
    for (GLenum floatType : floatTypes)
    {
        ...
        // TODO: Vulkan's CPU-path implementation doesn't handle 3D/2DArray textures correctly yet.
