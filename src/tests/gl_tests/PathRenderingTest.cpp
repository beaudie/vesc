...
void dumpPixels(GLsizei w, GLsizei h)
{
    using u8 = std::uint8_t;
    struct RGBA {
        u8 r, g, b, a;
    };
    std::vector<RGBA> buff;
    buff.resize(w * h);
    glReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, &buff[0]);

    std::ofstream out("/tmp/path-rendering-test.ppm", std::ios::binary);
    out << "P6 " << w << " " << h << " 255\n";
    for (const auto& pixel : buff)
...
protected:
    ...
    {
        return extensionEnabled("GL_CHROMIUM_path_rendering");
    }
...
TEST_P(CHROMIUMPathRenderingTest, TestMatrix)
{
    ...
    static const GLenum kMatrixModes[] =
    {
        GL_PATH_MODELVIEW_CHROMIUM,
        GL_PATH_PROJECTION_CHROMIUM
    };

    static const GLenum kGetMatrixModes[] =
    {
...
TEST_P(CHROMIUMPathRenderingTest, TestPathParameter)
{
    ...
    static const GLenum kEndCaps[] =
    {
        GL_FLAT_CHROMIUM, GL_SQUARE_CHROMIUM, GL_ROUND_CHROMIUM
    };
    for (std::size_t i=0; i<3; ++i)
    {
        GLint x;
        glPathParameteriCHROMIUM(path, GL_PATH_END_CAPS_CHROMIUM,
            static_cast<GLenum>(kEndCaps[i]));
        EXPECT_GL_NO_ERROR();
        glGetPathParameterivCHROMIUM(path, GL_PATH_END_CAPS_CHROMIUM, &x);
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(static_cast<GLenum>(x), kEndCaps[i]);
        ...
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(static_cast<GLenum>(f), kEndCaps[(i + 1) % 3]);
    }

    static const GLenum kJoinStyles[] =
    {
    ...
    for (std::size_t i=0; i<3; ++i)
    {
        ...
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(static_cast<GLenum>(x), kJoinStyles[i]);
        ...
        EXPECT_GL_NO_ERROR();
        EXPECT_EQ(static_cast<GLenum>(f), kJoinStyles[(i + 1) % 3]);
    }
...
TEST_P(CHROMIUMPathRenderingTest, TestPathCommandsErrors)
{
    ...
    glPathCommandsCHROMIUM(path, 2, commands,
...
const GLfloat kProjectionMatrix[16] = {
    2.0f / kResolution,
    0.0f,
    0.0f,
    0.0f,
...
protected:
    ...
    void setupPathStateForTestPattern(GLuint path)
    {
        ...
        {
            50.0f, 50.0f, 75.0f, 75.0f,
            100.0f, 62.5f, 50.0f, 25.5f,
            0.0f, 62.5f, 50.0f, 50.0f,
            25.0f, 75.0f
        };
...
ANGLE_INSTANTIATE_TEST(CHROMIUMPathRenderingTest,     ES2_OPENGL(), ES2_OPENGLES(), ES3_OPENGL(), ES3_OPENGLES());
ANGLE_INSTANTIATE_TEST(CHROMIUMPathRenderingDrawTest, ES2_OPENGL(), ES2_OPENGLES(), ES3_OPENGL(), ES3_OPENGLES());
