...
class CHROMIUMPathRenderingWithTexturingTest : public ANGLETest
{
  protected:
    ...
    void drawTestPattern()
    {
        ...
        GLuint path = glGenPathsCHROMIUM(1);
        glPathCommandsCHROMIUM(path, ArraySize(kCommands), kCommands, ArraySize(kCoords), GL_FLOAT,
                               kCoords);
        ASSERT_GL_NO_ERROR();
...
TEST_P(CHROMIUMPathRenderingWithTexturingTest, TestProgramPathFragmentInputGenArgs)
{
    ...
    for (int variable = 0; variable < ArraySize(variables); ++variable)
    {
        for (GLint components = 0; components <= 4; ++components)
        {
            for (int genmode = 0; genmode < ArraySize(kGenModes); ++genmode)
            {
    ...
    glProgramPathFragmentInputGenCHROMIUM(mProgram, -1, kValidGenMode, kValidComponents, kCoefficients16);
    ASSERT_GL_NO_ERROR();
    ...
    // GL policy.
    glProgramPathFragmentInputGenCHROMIUM(mProgram, -1, kInvalidGenMode, kValidComponents, kCoefficients16);
    ASSERT_GL_ERROR(GL_INVALID_ENUM);

    glProgramPathFragmentInputGenCHROMIUM(mProgram, -1, kInvalidGenMode, kInvalidComponents, kCoefficients16);
    ASSERT_GL_ERROR(GL_INVALID_ENUM);

    glProgramPathFragmentInputGenCHROMIUM(mProgram, -1, kValidGenMode, kInvalidComponents, kCoefficients16);
    ASSERT_GL_ERROR(GL_INVALID_VALUE);
