...
class TransformFeedbackTestBase : public ANGLETest
{
  protected:
    ...
    static constexpr size_t mTransformFeedbackBufferSize = 1 << 24;
...
TEST_P(TransformFeedbackTest, BufferRebinding)
{
    ...
    constexpr float finalZ = 0.95f;

    RNG rng;

    constexpr size_t loopCount = 64;
...
TEST_P(TransformFeedbackTest, RecordAndDraw)
{
    ...
    constexpr GLfloat vertices[] = {-1.0f, 1.0f, 0.5f, -1.0f, -1.0f, 0.5f, 1.0f, -1.0f, 0.5f,
...
TEST_P(TransformFeedbackTest, BufferBinding)
{
    ...
    EXPECT_EQ(mTransformFeedbackBuffer, static_cast<GLuint>(currentBufferBinding));
    ...
    EXPECT_EQ(scratchBuffer, static_cast<GLuint>(currentBufferBinding));
...
{
    constexpr size_t drawSize = 1024;
    ...
    constexpr size_t transformFeedbackCount = 8;
...
TEST_P(TransformFeedbackTest, MultiContext)
{
    ...
    EGLint contextAttributes[] = {EGL_CONTEXT_MAJOR_VERSION_KHR, GetParam().majorVersion,
                                  EGL_CONTEXT_MINOR_VERSION_KHR, GetParam().minorVersion, EGL_NONE};
    ...
    constexpr size_t maxDrawSize = 1024;
    ...
    for (const auto &context : contexts)
    {
        ...
        for (const auto &primCount : context.primitiveCounts)
        {
            ...
            {
                EXPECT_EQ(prim + 1, bufferData[curBufferIndex]);
                curBufferIndex++;
            }
        }
        ...
        {
            EXPECT_EQ(0.0f, bufferData[curBufferIndex]);
            curBufferIndex++;
        }
