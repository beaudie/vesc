...
TEST_P(TransformFeedbackTestES31, TriangleStripAdjacency)
{
    ...
    GLBuffer xfbBuffer2;
    glBindBuffer(GL_UNIFORM_BUFFER, xfbBuffer2);
    glBufferData(GL_UNIFORM_BUFFER, mTransformFeedbackBufferSize, nullptr, GL_DYNAMIC_DRAW);
    glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 1, xfbBuffer2);
    ...
    mappedBuffer = glMapBufferRange(GL_UNIFORM_BUFFER, 0, sizeof(float) * 12, GL_MAP_READ_BIT);
    ASSERT_NE(nullptr, mappedBuffer);

    printf("Order of vertices:\n");
    mappedFloats = static_cast<float *>(mappedBuffer);
