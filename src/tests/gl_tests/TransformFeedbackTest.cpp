...
// Test that you can pause transform feedback before drawing.
...
// Test that the workaround "useProgramBeforeEndTransformFeedback" works correctly.
TEST_P(TransformFeedbackTest, UseProgramWorkaround)
{
    std::vector<std::string> tfVaryings;
    tfVaryings.push_back("gl_Position");
    compileDefaultProgram(tfVaryings, GL_INTERLEAVED_ATTRIBS);
...
// Test that the workaround "useProgramBeforeEndTransformFeedback" works correctly with context
// switching.
TEST_P(TransformFeedbackTest, UseProgramWorkaroundContextSwitch)
{
    std::vector<std::string> tfVaryings;
    tfVaryings.push_back("gl_Position");
    compileDefaultProgram(tfVaryings, GL_INTERLEAVED_ATTRIBS);

    // Context switching is required to trigger the bug that needs the workaround.
    EGLWindow *window          = getEGLWindow();
    EGLDisplay display         = window->getDisplay();
    EGLConfig config           = window->getConfig();
    EGLSurface surface         = window->getSurface();
