...
class TransformFeedbackTest : public TransformFeedbackTestBase
{
  protected:
    void compileDefaultProgram(const std::vector<std::string> &tfVaryings, GLenum bufferMode)
    {
        ...
        mProgram = CompileProgramWithTransformFeedback(shader_library::essl1::vs::simple(),
                                                       shader_library::essl1::fs::red(), tfVaryings,
                                                       bufferMode);
...
TEST_P(TransformFeedbackTest, BufferRebinding)
{
    ...
    for (size_t loopIdx = 0; loopIdx < loopCount; loopIdx++)
    {
        ...
        drawQuad(mProgram, shader_library::positionAttribName(), z);
...
TEST_P(TransformFeedbackTest, RecordAndDraw)
{
    ...
    GLint positionLocation = glGetAttribLocation(mProgram, shader_library::positionAttribName());
...
TEST_P(TransformFeedbackLifetimeTest, DeletedBuffer)
{
    ...
    glBeginTransformFeedback(GL_TRIANGLES);
    drawQuad(mProgram, shader_library::positionAttribName(), 0.5f, 1.0f, true);
    glEndTransformFeedback();
    ...
    glBindBuffer(GL_ARRAY_BUFFER, mTransformFeedbackBuffer);
    GLint loc = glGetAttribLocation(mProgram, shader_library::positionAttribName());
    ASSERT_NE(-1, loc);
...
TEST_P(TransformFeedbackTest, NonExistentTransformFeedbackVarying)
{
    ...
    mProgram = CompileProgramWithTransformFeedback(shader_library::essl3::vs::simple(),
                                                   shader_library::essl3::fs::red(), tfVaryings,
...
{
    ANGLE_GL_PROGRAM(program, shader_library::essl3::vs::simple(),
