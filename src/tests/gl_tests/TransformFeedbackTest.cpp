...
{
    // Unclear why the QC driver fails this test. No need to investigate at this point.
    ANGLE_SKIP_TEST_IF(IsAndroid() && IsAdreno() && IsOpenGL());
    ...
    // We might be hitting OOM, overflowing the buffer, or another error.
    err = glGetError();
    EXPECT_GLENUM_NE(err, GL_NO_ERROR);
}
