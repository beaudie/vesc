...
TEST_P(TransformFeedbackTest, Overrun)
{
    ...
    for (size_t floatIndex = 0; floatIndex < vertices.size(); ++floatIndex)
    {
        ASSERT_EQ(mapPtrFloat[floatIndex], vertices[floatIndex]) << "at float index " << floatIndex;
    }

    size_t numFloats = mTransformFeedbackBufferSize / sizeof(GLfloat);
    for (size_t floatIndex = vertices.size(); floatIndex < numFloats; ++floatIndex)
    {
...
{
    // Fails on Adreno GLES drivers.
    ANGLE_SKIP_TEST_IF(IsOpenGL() && IsAdreno() && IsAndroid());

    // Fails on Windows Intel GL drivers.
    ...
    for (size_t floatIndex = vertices.size(); floatIndex < vertices.size() * 2; ++floatIndex)
    {
        ASSERT_EQ(mapPtrFloat[floatIndex], vertices[floatIndex % vertices.size()])
            << "at float index " << floatIndex;
    }

    size_t numFloats = mTransformFeedbackBufferSize / sizeof(GLfloat);
    for (size_t floatIndex = vertices.size() * 2; floatIndex < numFloats; ++floatIndex)
    {
        ASSERT_EQ(mapPtrFloat[floatIndex], 0) << "at float index " << floatIndex;
    }
}
...
TEST_P(TransformFeedbackTest, OverrunWithMultiplePauseAndResume)
{
    ...
    // Should have 3 packed sets of floats.
    for (size_t floatIndex = 0; floatIndex < vertices.size() * 3; ++floatIndex)
    {
    ...
    size_t numFloats = mTransformFeedbackBufferSize / sizeof(GLfloat);
    for (size_t floatIndex = vertices.size() * 3; floatIndex < numFloats; ++floatIndex)
    {
        ASSERT_EQ(mapPtrFloat[floatIndex], 0) << "at float index " << floatIndex;
    }
}
