...
{
    // ANGLE's FF shader computes lighting incorrectly
    ANGLE_SKIP_TEST_IF(GetParam().isANGLE());
...
// Check fog
TEST_P(FixedFunctionTest, FoggyMesh)
{
    glEnable(GL_FOG);
    drawTestQuad();
...
// Check smooth lighting combined with fog
TEST_P(FixedFunctionTest, FoggySmoothLitMesh)
{
    // ANGLE's FF shader computes lighting incorrectly
    ANGLE_SKIP_TEST_IF(GetParam().isANGLE());
    ...
    glEnable(GL_FOG);
    drawTestQuad();
    EXPECT_PIXEL_COLOR_NEAR(16, 16, GLColor(180, 30, 81, 255), 1);
}

// Check flat lighting combined with fog
TEST_P(FixedFunctionTest, FoggyFlatLitMesh)
{
    // ANGLE's FF shader computes lighting incorrectly
    ANGLE_SKIP_TEST_IF(GetParam().isANGLE());
    ...
    glShadeModel(GL_FLAT);
    drawTestQuad();
    EXPECT_PIXEL_COLOR_NEAR(16, 16, GLColor(186, 30, 173, 255), 1);
}
