...
TEST_P(FogBlendTest, ShadowEffect)
{
    ...
    // Draws shadow. This is broken on angle
    glEnable(GL_FOG);
    ...
    EXPECT_PIXEL_COLOR_NEAR(w / 2, h / 2, GLColor(0, 255u, 0, 255u), 0);

    // The background should be red
    EXPECT_PIXEL_COLOR_NEAR(w / 8, h / 8, GLColor(255u, 0, 0, 255u), 0);
    EXPECT_PIXEL_COLOR_NEAR(w - w / 8, h / 8, GLColor(255u, 0, 0, 255u), 0);
    EXPECT_PIXEL_COLOR_NEAR(w / 8, h - h / 8, GLColor(255u, 0, 0, 255u), 0);
    EXPECT_PIXEL_COLOR_NEAR(w - w / 8, h - h / 8, GLColor(255u, 0, 0, 255u), 0);
