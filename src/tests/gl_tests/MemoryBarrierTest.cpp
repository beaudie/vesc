...
void ParseMemoryBarrierVariationsTestParams(
    const MemoryBarrierVariationsTestParams &params,
...
protected:
    ...
    void createStorageBuffer(WriteResource writeResource, GLuint buffer, GLuint textureBuffer, size_t size, const void *initialData);
    void createStorageImage(WriteResource writeResource, GLuint bufferStorage, GLuint texture, const std::array<float, 4> &initialData);
    void createProgram(ShaderWritePipeline writePipeline, WriteResource writeResource, GLProgram *programOut);
    ...
    template<typename T>
    void verifyFramebufferAndBufferContents(ShaderWritePipeline writePipeline, const std::array<T, 4> &expected);

    void verifyFramebufferAndImageContents(ShaderWritePipeline writePipeline, WriteResource writeResource, GLuint texture, const std::array<float, 4> &expected);
    ...
    void vertexAttribArrayBitBufferWriteThenVertexRead(ShaderWritePipeline writePipeline,
            WriteResource writeResource,
            NoopOp preBarrierOp, NoopOp postBarrierOp);
    void vertexAttribArrayBitVertexReadThenBufferWrite(ShaderWritePipeline writePipeline,
            WriteResource writeResource,
            NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit);
    ...
    void createTextureVerifyProgram(WriteResource writeResource, GLuint texture, GLProgram *programOut);
    ...
    void createImageVerifyProgram(WriteResource writeResource, GLuint texture, GLProgram *programOut);
...
{
    return writeResource != WriteResource::ImageBuffer || IsGLExtensionEnabled("GL_OES_texture_buffer");
}
...
void MemoryBarrierTestBase::createFramebuffer(GLuint color, GLuint fbo, GLColor initialColor)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTextureSize, kTextureSize, GL_RGBA, GL_UNSIGNED_BYTE, &initialColor);
    EXPECT_GL_NO_ERROR();
...
void MemoryBarrierTestBase::createStorageBuffer(WriteResource writeResource, GLuint buffer, GLuint textureBuffer,
...
void MemoryBarrierTestBase::createStorageImage(WriteResource writeResource, GLuint bufferStorage, GLuint texture, const std::array<float, 4> &initialData)
{
    ...
    if (writeResource == WriteResource::ImageBuffer)
    {
        ...
        glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(initialData), initialData.data(), GL_STATIC_DRAW);
        EXPECT_GL_NO_ERROR();
    ...
    else
    {
        ...
        glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, kTextureSize, kTextureSize);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTextureSize, kTextureSize, GL_RGBA, GL_UNSIGNED_BYTE, initialDataAsUnorm.data());
        glBindImageTexture(0, texture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8);
    }
}

void MemoryBarrierTestBase::createProgram(ShaderWritePipeline writePipeline, WriteResource writeResource,
...
void MemoryBarrierTestBase::verifyImageContents(GLuint texture, const std::array<float, 4> &expected)
{
    ...
    const GLColor kExpected(expected[0] * 255,
            expected[1] * 255,
            expected[2] * 255,
            expected[3] * 255);
...
template<typename T>
void MemoryBarrierTestBase::verifyFramebufferAndBufferContents(ShaderWritePipeline writePipeline, const std::array<T, 4> &expected)
{
    // Verify the result of the verify shader
    const GLColor kExpected = writePipeline == ShaderWritePipeline::Graphics ? GLColor::cyan : GLColor::green;
...
void MemoryBarrierTestBase::verifyFramebufferAndImageContents(ShaderWritePipeline writePipeline, WriteResource writeResource, GLuint texture, const std::array<float, 4> &expected)
{
    // Verify the result of the verify shader
    const GLColor kExpected = writePipeline == ShaderWritePipeline::Graphics ? GLColor::cyan : GLColor::green;
    EXPECT_PIXEL_COLOR_EQ(0, 0, kExpected);
...
void MemoryBarrierTestBase::vertexAttribArrayBitBufferWriteThenVertexRead(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
    ...
    createStorageBuffer(writeResource, vertexBuffer, vertexTextureBuffer, sizeof(kInitData), kInitData.data());
...
void MemoryBarrierTestBase::vertexAttribArrayBitVertexReadThenBufferWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
    GLFramebuffer fbo;
    GLProgram writeProgram;
...
void MemoryBarrierTestBase::elementArrayBitBufferWriteThenIndexRead(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
    ...
    constexpr std::array<float, 4> kInitData = {12.34, 5.6, 78.91, 123.456};
    createStorageBuffer(writeResource, indexBuffer, indexTextureBuffer, sizeof(kInitData), kInitData.data());
...
void MemoryBarrierTestBase::elementArrayBitIndexReadThenBufferWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
    GLFramebuffer fbo;
        ...
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    constexpr std::array<float, 4> kInitData = {12.34, 5.6, 78.91, 123.456};
    createStorageBuffer(writeResource, uniformBuffer, uniformTextureBuffer, sizeof(kInitData), kInitData.data());
        ...
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    constexpr std::array<float, 4> kInitData = {12.34, 5.6, 78.91, 123.456};
    createStorageBuffer(writeResource, indirectBuffer, indirectTextureBuffer, sizeof(kInitData), kInitData.data());
        ...
        NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    constexpr std::array<float, 4> kInitData = {1.5, 3.75, 5.0, 12.125};
    createStorageBuffer(writeResource, packBuffer, packTextureBuffer, sizeof(kInitData), kInitData.data());
        ...
        NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    constexpr std::array<float, 4> kInitData = {1.5, 3.75, 5.0, 12.125};
    createStorageBuffer(writeResource, unpackBuffer, unpackTextureBuffer, sizeof(kInitData), kInitData.data());

    const std::array<float, 4> kWriteData = {*reinterpret_cast<const float *>(&GLColor::green), 5.6, 78.91, 123.456};
    setUniformData(writeProgram, kWriteData);
        ...
        NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    ...
    GLBuffer unpackBuffer;
    GLTexture unpackTextureBuffer;
    const std::array<float, 4> kInitData = {
        *reinterpret_cast<const float *>(&GLColor::green), 3.75, 5.0, 12.125};
    createStorageBuffer(writeResource, unpackBuffer, unpackTextureBuffer, sizeof(kInitData), kInitData.data());
        ...
        NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    createStorageBuffer(WriteResource::Buffer, srcBuffer, srcTextureBuffer, sizeof(kSrcInitData), kSrcInitData.data());
    ...
    constexpr std::array<float, 4> kInitData = {12.34, 5.6, 78.91, 123.456};
    createStorageBuffer(writeResource, writeBuffer, writeTextureBuffer, sizeof(kInitData), kInitData.data());
...
void MemoryBarrierTestBase::transformFeedbackBitBufferWriteThenCapture(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
    ...
    constexpr std::array<float, 4> kInitData = {12.34, 5.6, 78.91, 123.456};
    createStorageBuffer(writeResource, xfbBuffer, xfbTextureBuffer, sizeof(kInitData) * 6, kInitData.data());
...
void MemoryBarrierTestBase::transformFeedbackBitCaptureThenBufferWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
    GLFramebuffer fbo;
        ...
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    constexpr std::array<uint32_t, 4> kInitData = {0x12345678u, 0x9ABCDEF0u, 0x13579BDFu, 0x2468ACE0u};
    createStorageBuffer(writeResource, atomicCounterBuffer, atomicCounterTextureBuffer, sizeof(kInitData), kInitData.data());
...
void MemoryBarrierTestBase::createSsboVerifyProgram(WriteResource writeResource, GLProgram *programOut)
{
    constexpr char kVS[] = R"(#version 310 es
void main()
{
...
void MemoryBarrierTestBase::shaderStorageBitBufferWriteThenBufferRead(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
...
void MemoryBarrierTestBase::shaderStorageBitBufferReadThenBufferWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
    GLFramebuffer fbo;
...
void MemoryBarrierTestBase::createTextureVerifyProgram(WriteResource writeResource, GLuint texture, GLProgram *programOut)
{
    constexpr char kVS[] = R"(#version 310 es
void main()
{
...
void main()
{
    ...
    programOut->makeRaster(kVS, writeResource == WriteResource::ImageBuffer ? kImageBufferFS : kImageFS);
    ASSERT_TRUE(programOut->valid());
    glUseProgram(*programOut);

    glBindTexture(writeResource == WriteResource::ImageBuffer ? GL_TEXTURE_BUFFER : GL_TEXTURE_2D, texture);
    EXPECT_GL_NO_ERROR();
}

void MemoryBarrierTestBase::textureFetchBitImageWriteThenSamplerRead(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
...
void MemoryBarrierTestBase::textureFetchBitSamplerReadThenImageWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
    GLFramebuffer fbo;
...
void MemoryBarrierTestBase::createImageVerifyProgram(WriteResource writeResource, GLuint texture, GLProgram *programOut)
{
    constexpr char kVS[] = R"(#version 310 es
void main()
{
...
void MemoryBarrierTestBase::shaderImageAccessBitImageWriteThenImageRead(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
...
void MemoryBarrierTestBase::shaderImageAccessBitImageReadThenImageWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource, NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    GLFramebuffer fbo;
        ...
        NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    ...
    const std::array<float, 4> kExpectedData = {0, 1.0, writePipeline == ShaderWritePipeline::Graphics ? 1.0 : 0, 1.0};
...
void MemoryBarrierTestBase::framebufferBitImageWriteThenReadPixels(ShaderWritePipeline writePipeline,
        WriteResource writeResource,
        NoopOp preBarrierOp, NoopOp postBarrierOp)
{
    GLTexture color;
    ...
    GLBuffer packBuffer;
    glBindBuffer(GL_PIXEL_PACK_BUFFER, packBuffer);
    constexpr std::array<uint32_t, 4> kPBOInitData = {0x12345678u, 0x9ABCDEF0u, 0x13579BDFu, 0x2468ACE0u};
...
void MemoryBarrierTestBase::framebufferBitReadPixelsThenImageWrite(ShaderWritePipeline writePipeline,
        WriteResource writeResource,
        NoopOp preBarrierOp, NoopOp postBarrierOp, GLbitfield barrierBit)
{
    GLTexture color;
...
TEST_P(MemoryBarrierBufferTest, VertexAtrribArrayBitWriteThenVertexRead)
{
    ...
    NoopOp preBarrierOp;
    NoopOp postBarrierOp;
    ParseMemoryBarrierVariationsTestParams(GetParam(), &writePipeline, &writeResource, &preBarrierOp, &postBarrierOp);

    ANGLE_SKIP_TEST_IF(!hasExtensions(writeResource));

    vertexAttribArrayBitBufferWriteThenVertexRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, VertexAtrribArrayBitVertexReadThenWrite)
{
    ...
    vertexAttribArrayBitVertexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_VERTEX_ATTRIB_ARRAY_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, ElementArrayBitWriteThenIndexRead)
{
    ...
    elementArrayBitBufferWriteThenIndexRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, ElementArrayBitIndexReadThenWrite)
{
    ...
    elementArrayBitIndexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_ELEMENT_ARRAY_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, UniformBitUBOReadThenWrite)
{
    ...
    uniformBitUBOReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_UNIFORM_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageTest, TextureFetchBitWriteThenSamplerRead)
{
    ...
    textureFetchBitImageWriteThenSamplerRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierImageTest, TextureFetchBitSamplerReadThenWrite)
{
    ...
    textureFetchBitSamplerReadThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_TEXTURE_FETCH_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageTest, ShaderImageAccessBitWriteThenImageRead)
{
    ...
    shaderImageAccessBitImageWriteThenImageRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierImageTest, ShaderImageAccessBitImageReadThenWrite)
{
    ...
    // TODO: bug tag.  Looks like the implementation is missing this line from the spec:
    //
    ...
    shaderImageAccessBitImageReadThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitVertexReadThenWrite)
{
    ...
    vertexAttribArrayBitVertexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitIndexReadThenWrite)
{
    ...
    elementArrayBitIndexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitUBOReadThenWrite)
{
    ...
    uniformBitUBOReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageTest, ShaderImageAccessBitSamplerReadThenWrite)
{
    ...
    textureFetchBitSamplerReadThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp,
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitIndirectReadThenWrite)
{
    ...
    commandBitIndirectReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitPackThenWrite)
{
    ...
    pixelBufferBitPackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitUnpackThenWrite)
{
    ...
    pixelBufferBitUnpackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, ShaderImageAccessBitCopyThenWrite)
{
    ...
    textureUpdateBitCopyThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitCopyThenWrite)
{
    ...
    bufferUpdateBitCopyThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, ShaderImageAccessBitDrawThenWrite)
{
    ...
    framebufferBitDrawThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, ShaderImageAccessBitReadPixelsThenWrite)
{
    ...
    framebufferBitReadPixelsThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, ShaderImageAccessBitFBOCopyThenWrite)
{
    ...
    framebufferBitCopyThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, ShaderImageAccessBitBlitThenWrite)
{
    ...
    framebufferBitBlitThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitCaptureThenWrite)
{
    ...
    transformFeedbackBitCaptureThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitAtomicThenWrite)
{
    ...
    atomicCounterBitAtomicThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageBufferOnlyTest, ShaderImageAccessBitBufferReadThenWrite)
{
    ...
    shaderStorageBitBufferReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, CommandBitWriteThenIndirectRead)
{
    ...
    commandBitBufferWriteThenIndirectRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, CommandBitIndirectReadThenWrite)
{
    ...
    commandBitIndirectReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_COMMAND_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, PixelBufferBitPackThenWrite)
{
    ...
    pixelBufferBitPackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_PIXEL_BUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, PixelBufferBitUnpackThenWrite)
{
    ...
    pixelBufferBitUnpackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_PIXEL_BUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, TextureUpdateBitCopyThenWrite)
{
    ...
    textureUpdateBitCopyThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_TEXTURE_UPDATE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, BufferUpdateBitCopyThenWrite)
{
    ...
    bufferUpdateBitCopyThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_BUFFER_UPDATE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, FramebufferBitWriteThenReadPixels)
{
    ...
    framebufferBitImageWriteThenReadPixels(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierImageOnlyTest, FramebufferBitDrawThenWrite)
{
    ...
    framebufferBitDrawThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_FRAMEBUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, FramebufferBitReadPixelsThenWrite)
{
    ...
    framebufferBitReadPixelsThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_FRAMEBUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, FramebufferBitCopyThenWrite)
{
    ...
    framebufferBitCopyThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_FRAMEBUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierImageOnlyTest, FramebufferBitBlitThenWrite)
{
    ...
    framebufferBitBlitThenImageWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_FRAMEBUFFER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, TransformFeedbackBitWriteThenCapture)
{
    ...
    transformFeedbackBitBufferWriteThenCapture(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, TransformFeedbackBitCaptureThenWrite)
{
    ...
    transformFeedbackBitCaptureThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_TRANSFORM_FEEDBACK_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, AtomicCounterBitWriteThenAtomic)
{
    ...
    atomicCounterBitBufferWriteThenAtomic(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, AtomicCounterBitAtomicThenWrite)
{
    ...
    atomicCounterBitAtomicThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_ATOMIC_COUNTER_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferTest, ShaderStorageBitWriteThenRead)
{
    ...
    shaderStorageBitBufferWriteThenBufferRead(writePipeline, writeResource, preBarrierOp, postBarrierOp);
}
...
TEST_P(MemoryBarrierBufferTest, ShaderStorageBitReadThenWrite)
{
    ...
    shaderStorageBitBufferReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitVertexReadThenWrite)
{
    ...
    vertexAttribArrayBitVertexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitIndexReadThenWrite)
{
    ...
    elementArrayBitIndexReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitUBOReadThenWrite)
{
    ...
    uniformBitUBOReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitIndirectReadThenWrite)
{
    ...
    commandBitIndirectReadThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitPackThenWrite)
{
    ...
    pixelBufferBitPackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitUnpackThenWrite)
{
    ...
    pixelBufferBitUnpackThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitCopyThenWrite)
{
    ...
    bufferUpdateBitCopyThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitCaptureThenWrite)
{
    ...
    transformFeedbackBitCaptureThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}
...
TEST_P(MemoryBarrierBufferOnlyTest, ShaderStorageBitAtomicThenWrite)
{
    ...
    atomicCounterBitAtomicThenBufferWrite(writePipeline, writeResource, preBarrierOp, postBarrierOp, GL_SHADER_STORAGE_BARRIER_BIT);
}

constexpr ShaderWritePipeline kWritePipelines[] = {
    ShaderWritePipeline::Graphics, ShaderWritePipeline::Compute,
};
constexpr WriteResource kBufferWriteResources[] = {
    WriteResource::Buffer, WriteResource::ImageBuffer,
};
constexpr WriteResource kImageWriteResources[] = {
    WriteResource::Image, WriteResource::ImageBuffer,
};
...
constexpr NoopOp kNoopOps[] = {
    NoopOp::None, NoopOp::Draw, NoopOp::Dispatch,
};
