...
struct SizedFloat {
    ...
    static uint32_t
    Assemble(const uint32_t sVal, const uint32_t eVal, const uint32_t mVal)
    {
    ...
    Encode(const float signedV)
    {
        ...
        if (v != v) { // NaN
            ...
            eVal = 0;
            mVal = 0;
        } else if (eVal >= eValMax) { // overfloat to Inf
            eVal = eValMax;
            mVal = 0;
        } else {
            float mFloat;
...
TEST_P(TextureUploadFormatTest, All)
{
    ...
    {
        const uint16_t src[] = {
            Float16::Encode(srcVals[0]),
            Float16::Encode(srcVals[1]),
            Float16::Encode(srcVals[2]),
            Float16::Encode(srcVals[3])
        };
        ZeroAndCopy(srcBuffer, src);
