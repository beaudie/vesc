...
static uint32_t
EncodeNormUint(const float val)
{
...
static uint32_t
EncodeFloat(const float signedV)
{
    ...
    if (v != v) { // NaN
        eVal = eMax;
        mVal = 1;
    } else if (eVal < 0) { // underflow to zero
        mVal = 0;
    } else if (eVal == 0) { // denormal
        mVal = v * powf(2, mBits + eBias - 1);
    } else if (eVal >= eMax) { // overfloat to Inf
        eVal = eMax;
        mVal = 0;
    } else { // standard range
        mVal = uint32_t(v * powf(2, eBias - eVal - mBits) - powf(2, -mBits));
    }

    const auto sVal = uint32_t(v < 0.0f);
    return (signBit ? (sVal << (eBits + mBits)) : 0) |
           (uint32_t(eVal) << mBits) |
           mVal;
}

static uint32_t
EncodeFloat16(const float v)
{
    return EncodeFloat<1,5,10>(v);
}
static uint32_t
EncodeUFloat11(const float v)
{
    return EncodeFloat<0,5,6>(v);
}
static uint32_t
EncodeUFloat10(const float v)
{
    return EncodeFloat<0,5,5>(v);
}

static uint32_t
EncodeRGB9_E5_Rev(const float signedR, const float signedG, const float signedB)
{
...
static void
ZeroAndCopy(DestT& dest, const SrcT (&src)[SrcN])
{
    dest.fill(0);
...
static std::string
EnumStr(const GLenum v)
{
#define _(X) case GL_##X: return #X;
    switch (v) {
...
TEST_P(TextureUploadFormatTest, All)
{
    ...
    const uint8_t refVals[4] = {
        EncodeNormUint<8>(srcVals[0]),
        EncodeNormUint<8>(srcVals[1]),
        EncodeNormUint<8>(srcVals[2]),
        EncodeNormUint<8>(srcVals[3])
    };

    // --

    const auto fnTestData = [&](const TexFormat& format, const void* const data,
                                const GLColor& err, const char* const info)
    {
        ASSERT_GL_NO_ERROR();
        glTexImage2D(GL_TEXTURE_2D, 0, format.internalFormat, 1, 1, 0,
                     format.unpackFormat, format.unpackType, data);
        ...
        if (!result) {
             result << " [" << EnumStr(format.internalFormat) << "/"
    ...
    {
        const uint8_t src[] = {
            EncodeNormUint<7>(srcVals[0]),
            EncodeNormUint<7>(srcVals[1]),
            EncodeNormUint<7>(srcVals[2]),
            EncodeNormUint<7>(srcVals[3])
        };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint16_t src[] = { (EncodeNormUint<4>(srcVals[0]) << 12) |
                                 (EncodeNormUint<4>(srcVals[1]) <<  8) |
                                 (EncodeNormUint<4>(srcVals[2]) <<  4) |
                                 (EncodeNormUint<4>(srcVals[3]) <<  0) };
        ZeroAndCopy(srcBuffer, src);
    ...
    // RGBA+UNSIGNED_SHORT_5_5_5_1
    {
        const uint16_t src[] = { (EncodeNormUint<5>(srcVals[0]) << 11) |
                                 (EncodeNormUint<5>(srcVals[1]) <<  6) |
                                 (EncodeNormUint<5>(srcVals[2]) <<  1) |
                                 (EncodeNormUint<1>(srcVals[3]) <<  0) };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint32_t src[] = { (EncodeNormUint<10>(srcVals[0]) <<  0) |
                                 (EncodeNormUint<10>(srcVals[1]) << 10) |
                                 (EncodeNormUint<10>(srcVals[2]) << 20) |
                                 (EncodeNormUint< 2>(srcVals[3]) << 30) };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint16_t src[] = {
            EncodeFloat16(srcVals[0]),
            EncodeFloat16(srcVals[1]),
            EncodeFloat16(srcVals[2]),
            EncodeFloat16(srcVals[3])
        };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint32_t src[] = { (srcIntVals[0] <<  0) |
                                 (srcIntVals[1] << 10) |
                                 (srcIntVals[2] << 20) |
                                 (srcIntVals[3] << 30) };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint32_t src[] = { (EncodeUFloat11(srcVals[0]) <<  0) |
                                 (EncodeUFloat11(srcVals[1]) << 11) |
                                 (EncodeUFloat10(srcVals[2]) << 22) };
        ZeroAndCopy(srcBuffer, src);
    ...
    {
        const uint32_t src[] = { EncodeRGB9_E5_Rev(srcVals[0],
                                                   srcVals[1],
                                                   srcVals[2]) };
        ZeroAndCopy(srcBuffer, src);
    ...
    // DEPTH_COMPONENT+FLOAT
    {
        ...
        fnTest({GL_DEPTH32F_STENCIL8, GL_DEPTH_STENCIL, GL_FLOAT_32_UNSIGNED_INT_24_8_REV}, {1,0,0,0});
    }
