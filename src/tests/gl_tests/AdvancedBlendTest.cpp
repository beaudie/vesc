...
// Test that when we use the feature emulateAdvancedBlendEquations,
// the blend is not applied if we call glDisable(GL_BLEND).
TEST_P(AdvancedBlendTest, advancedBlendEmulationNotAppliedWhenBlendIsDisabled)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_KHR_blend_equation_advanced") ||
                       !getEGLWindow()->isFeatureEnabled(Feature::EmulateAdvancedBlendEquations));
...
// Test that when we use the feature emulateAdvancedBlendEquations,
// the blend is not applied after we call glDisable(GL_BLEND),
// the blend is applied after we call glEnable(GL_BLEND).
TEST_P(AdvancedBlendTest, advancedBlendEmulationAppliedBasedOnGLBlendState)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_KHR_blend_equation_advanced") ||
                       !getEGLWindow()->isFeatureEnabled(Feature::EmulateAdvancedBlendEquations));
