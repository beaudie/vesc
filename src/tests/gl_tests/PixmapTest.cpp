  ...
  protected:
    ...
    void testSetUp() override
    {
        ...
        if (mSupportsPixmaps)
        {
            ...
            ASSERT_TRUE(
                mOSPixmap->initialize(osWindow->getNativeDisplay(), mPixmapSize, mPixmapSize, 32));
...
{
    // Test skipped because Pbuffers are not supported or Pixmap does not support binding to RGBA
    // textures.
    ANGLE_SKIP_TEST_IF(!mSupportsPixmaps || !mSupportsBindTexImage);
