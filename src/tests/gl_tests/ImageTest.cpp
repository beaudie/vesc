...
class ImageTest : public ANGLETest
{
  protected:
                                       ...
                                       EGLImageKHR *outSourceImage)
    {
        ...
        EGLint attribs[] = {
            EGL_GL_TEXTURE_ZOFFSET_KHR, imageLayer, EGL_NONE,
        };
        EGLImageKHR image =
...
TEST_P(ImageTest, ValidationImageBase)
{
    ...
    // generated.
    image =
        eglCreateImageKHR(reinterpret_cast<EGLDisplay>(0xBAADF00D), context, EGL_GL_TEXTURE_2D_KHR,
    ...
    image =
        eglCreateImageKHR(display, reinterpret_cast<EGLContext>(0xBAADF00D), EGL_GL_TEXTURE_2D_KHR,
                          reinterpret_cast<EGLClientBuffer>(texture2D), nullptr);
    EXPECT_EQ(image, EGL_NO_IMAGE_KHR);
    EXPECT_EGL_ERROR(EGL_BAD_CONTEXT);

    image = eglCreateImageKHR(display, EGL_NO_CONTEXT, EGL_GL_TEXTURE_2D_KHR,
                              reinterpret_cast<EGLClientBuffer>(texture2D), nullptr);
    EXPECT_EQ(image, EGL_NO_IMAGE_KHR);
    EXPECT_EGL_ERROR(EGL_BAD_CONTEXT);
    ...
    EGLint badAttributes[] = {
        0xDEADBEEF, 0, EGL_NONE,
    };

    image = eglCreateImageKHR(display, context, EGL_GL_TEXTURE_2D_KHR,
                              reinterpret_cast<EGLClientBuffer>(texture2D), badAttributes);
    EXPECT_EQ(image, EGL_NO_IMAGE_KHR);
...
TEST_P(ImageTest, ValidationGLImage)
{
    ...
    if (eglDisplayExtensionEnabled(window->getDisplay(), "EGL_KHR_gl_texture_2D_image"))
    {
        ...
        // EGL_GL_TEXTURE_3D_KHR and <buffer> is not the name of a
        ...
        // EGL_GL_TEXTURE_CUBE_MAP_*_KHR or
        // EGL_GL_TEXTURE_3D_KHR, <buffer> is the name of an incomplete GL texture object, and any
        // mipmap levels other than mipmap level 0
        ...
        // EGL_GL_TEXTURE_3D_KHR, <buffer> is not
        // the name of a complete GL texture object, and mipmap level 0 is not specified, the error
        // EGL_BAD_PARAMETER is generated.
        ...
        // EGL_GL_RENDERBUFFER_KHR or
        // EGL_GL_TEXTURE_3D_KHR and <buffer> refers to the default GL texture object(0) for the
        // corresponding GL target, the error
        // EGL_BAD_PARAMETER is generated.
        ...
        // If <target> is EGL_GL_TEXTURE_2D_KHR, EGL_GL_TEXTURE_CUBE_MAP_*_KHR, or
        // EGL_GL_TEXTURE_3D_KHR, and the value specified in <attr_list>
        // for EGL_GL_TEXTURE_LEVEL_KHR is not a valid mipmap level for the specified GL texture
        // object <buffer>, the error EGL_BAD_MATCH is
        // generated.
    ...
    if (eglDisplayExtensionEnabled(window->getDisplay(), "EGL_KHR_gl_texture_cubemap_image"))
    {
        ...
        // not the name of a
        // complete GL texture object, and one or more faces do not have mipmap level 0 specified,
        // the error EGL_BAD_PARAMETER is
        // generated.
        ...
        getClientVersion() >= 3)
    {
        ...
        // EGL_GL_TEXTURE_ZOFFSET_KHR exceeds the depth
        // of the specified mipmap level - of - detail in <buffer>, the error EGL_BAD_PARAMETER is
        // generated.
        GLuint texture3D;
...
TEST_P(ImageTest, MipLevels)
{
    ...
    for (size_t level = 0; level < mipLevels; level++)
    {
        // Create the Image
        EGLint attribs[] = {
            EGL_GL_TEXTURE_LEVEL_KHR, level, EGL_NONE,
        };
        EGLImageKHR image =
            eglCreateImageKHR(window->getDisplay(), window->getContext(), EGL_GL_TEXTURE_2D_KHR,
                              reinterpret_cast<EGLClientBuffer>(source), attribs);
