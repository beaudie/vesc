...
class ImageTest : public ANGLETest
{
  protected:
    ...
    bool hasAHardwareBufferSupport() const
    {
#if defined(ANGLE_AHARDWARE_BUFFER_SUPPORT)
        return true;
#else
        std::cout << "Android Hardware Buffer is not supported." << std::endl;
        return false;
#endif
    }
...
void ImageTest::SourceAHBTarget2D_helper(const EGLint *attribs)
{
    ...
    ANGLE_SKIP_TEST_IF(!hasOESExt() || !hasBaseExt() || !has2DTextureExt() ||
                       !hasAndroidImageNativeBufferExt() || !hasAHardwareBufferSupport());
...
void ImageTest::SourceAHBTarget2DArray_helper(const EGLint *attribs)
{
    ...
    ANGLE_SKIP_TEST_IF(!hasOESExt() || !hasBaseExt() || !has2DTextureExt() ||
                       !hasAndroidImageNativeBufferExt() || !hasEglImageArrayExt() ||
                       !hasAHardwareBufferSupport());
...
void ImageTest::SourceAHBTargetExternal_helper(const EGLint *attribs)
{
    EGLWindow *window = getEGLWindow();
    ANGLE_SKIP_TEST_IF(!hasOESExt() || !hasBaseExt() || !has2DTextureExt() ||
                       !hasAndroidImageNativeBufferExt() || !hasExternalExt() ||
                       !hasAHardwareBufferSupport());
...
void ImageTest::SourceAHBTargetExternalESSL3_helper(const EGLint *attribs)
{
    EGLWindow *window = getEGLWindow();
    ANGLE_SKIP_TEST_IF(!hasOESExt() || !hasBaseExt() || !has2DTextureExt() ||
                       !hasAndroidImageNativeBufferExt() || !hasExternalESSL3Ext() ||
                       !hasAHardwareBufferSupport());
