...
// Test with FRONT_CONVENTION if we have ARB_provoking_vertex.
TEST_P(ProvokingVertexTest, ARBProvokingVertex)
{
    ...
    const bool hasExt = strstr(exts, "GL_ARB_provoking_vertex") != nullptr;
    if (IsD3D11())
    {
        EXPECT_TRUE(hasExt);
    }
