...
TEST_P(VulkanFormatTablesTest, TestFormatSupport)
{
    ...
    for (GLenum internalFormat : allFormats)
    {
        ...
        vkGetPhysicalDeviceFormatProperties(renderer->getPhysicalDevice(), vkFormat.vkTextureFormat,
                                            &formatProperties);

        VkImageFormatProperties imageProperties;

        // isTexturable?
        bool isTexturable = vkGetPhysicalDeviceImageFormatProperties(
                                renderer->getPhysicalDevice(), vkFormat.vkTextureFormat,
                                VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL,
                                VK_IMAGE_USAGE_SAMPLED_BIT, 0, &imageProperties) == VK_SUCCESS;
