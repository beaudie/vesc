...
// Test uniform block bindings specified in layout shader text work properly.
TEST_P(UniformBufferTest31, UniformBufferBindings)
{
    ...
    ASSERT_NE(-1, uniformBufferIndex);
...
TEST_P(UniformBufferTest31, ConsecutiveBindingsForBlockArray)
{
    ...
    ANGLE_GL_PROGRAM(program, vertexShaderSource, fragmentShaderSource);
    GLint uniformBufferIndex = glGetUniformBlockIndex(program, "uni[0]");
    ASSERT_NE(-1, uniformBufferIndex);
    std::array<GLuint, 2> uniformBuffer;
    glGenBuffers(2, uniformBuffer.data());
    ASSERT_GL_NO_ERROR();
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, uniformBuffer[0]);
    glBufferData(GL_UNIFORM_BUFFER, vec4Size, v.data(), GL_STATIC_DRAW);
    EXPECT_GL_NO_ERROR();
    glBindBufferBase(GL_UNIFORM_BUFFER, 2, uniformBuffer[0]);

    uniformBufferIndex = glGetUniformBlockIndex(program, "uni[1]");
    ASSERT_NE(-1, uniformBufferIndex);
    ASSERT_GL_NO_ERROR();
    glBindBuffer(GL_UNIFORM_BUFFER, uniformBuffer[1]);
    glBufferData(GL_UNIFORM_BUFFER, vec4Size, v.data(), GL_STATIC_DRAW);
    EXPECT_GL_NO_ERROR();
    glBindBufferBase(GL_UNIFORM_BUFFER, 3, uniformBuffer[1]);

    drawQuad(program, "position", 0.5f);
    EXPECT_GL_NO_ERROR();
    EXPECT_PIXEL_EQ(px, py, 20, 40, 60, 80);

    glDeleteBuffers(2, uniformBuffer.data());
}
