...
// Test two uniform blocks share the same uniform buffer, one of them contains a large struct array
// member.
TEST_P(UniformBufferTest, UniformBlocksSharedSameUniformBuffer)
{
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, mUniformBuffer);
    glBufferData(GL_UNIFORM_BUFFER, std::max(alignment, 16) + blockSize, nullptr, GL_STATIC_DRAW);

    const GLuint kVectorPerMat   = 4;
    const GLuint kFloatPerVector = 4;
    GLuint kVectorCount          = arraySize * kVectorPerMat;
    GLuint kFloatCount           = kVectorCount * kFloatPerVector + std::max(alignment / 4, 4);
    std::vector<GLfloat> floatData(kFloatCount, 0.0f);
    const GLuint kPositionCount                    = 12;
    unsigned int positionToTest[kPositionCount][2] = {{0, 0},  {75, 0},  {98, 13},  {31, 31},
                                                      {0, 32}, {65, 33}, {23, 54},  {63, 63},
    ...
    floatData[2] = 0.5f;
    floatData[3] = 0.5f;
    for (GLuint i = std::max(alignment / 16, 1); i < kFloatCount / 4; i++)
    {
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 1, mUniformBuffer, std::max(alignment, 16),
