...
// correctly. This is meant to test that HLSL structure padding works together with uniform blocks.
TEST_P(UniformBufferTest, UniformBlockInstanceWithNestedStructsContainingVec3s)
{
    const std::string &fragmentShader =
        R"(#version 300 es
        ...
        layout(std140) uniform matBuffer { S s; } buffer;
    ...
    ANGLE_GL_PROGRAM(program, mVertexShaderSource, fragmentShader);
    GLint uniformBufferIndex = glGetUniformBlockIndex(program, "matBuffer");

    glBindBuffer(GL_UNIFORM_BUFFER, mUniformBuffer);
    const GLsizei kVectorsPerStruct  = 2;
    const GLsizei kElementsPerVector = 4;
    const GLsizei kBytesPerElement   = 4;
    const GLsizei kDataSize          = kVectorsPerStruct * kElementsPerVector * kBytesPerElement;
    std::vector<GLubyte> v(kDataSize, 0);
