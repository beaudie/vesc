...
TEST_P(UniformBufferTest, UniformBlockWithOneLargeStructArray)
{
    ...
    glBufferData(GL_UNIFORM_BUFFER, blockSize, floatData.data(), GL_STATIC_DRAW);
    drawQuad(program.get(), essl3_shaders::PositionAttrib(), 0.5f);
...
TEST_P(UniformBufferTest, UniformBlockWithOneLargeMixStructArray)
{
    constexpr char kFS[] = R"(#version 300 es
        ...
        layout(std140) uniform buffer { S s[128]; };
        void main()
        {
            uvec2 coord = uvec2(floor(gl_FragCoord.xy));
            uint index = coord.x +  coord.y * uint(128);
            uint index_x = index / uint(128);
            uint index_y = (index % uint(128)) / uint(32);
