...
TEST_P(UniformBufferTest, SizeOver65535)
{
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, uboBuf, offs0 * 4, 0x10);
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, 0);
    ASSERT_GL_NO_ERROR();  // No errors from draw

    // Draw upper triangle - should be green
    GLBuffer uboBuf2;
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, uboBuf2, offs1 * 4, 0x10);
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, reinterpret_cast<void *>(6));
    ASSERT_GL_NO_ERROR();  // No errors from draw
