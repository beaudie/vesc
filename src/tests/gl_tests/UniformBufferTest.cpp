...
TEST_P(UniformBufferTest, UniformBlockWithOneStructLargeArrayMember)
{
    ...
    glGetInteger64v(GL_MAX_UNIFORM_BLOCK_SIZE, &maxUniformBlockSize);
    EXPECT_GL_NO_ERROR();
    if (maxUniformBlockSize < blockSize)
    {
    ...
    GLint updateSize = kFloatCount * sizeof(GLfloat);
    EXPECT_EQ(blockSize, bufferSize);
    EXPECT_EQ(blockSize, updateSize);
...
TEST_P(UniformBufferTest, UniformBlockWithOneMixStructLargeArrayMember)
{
    ...
    GLint64 maxUniformBlockSize;
    glGetInteger64v(GL_MAX_UNIFORM_BLOCK_SIZE, &maxUniformBlockSize);
    EXPECT_GE(maxUniformBlockSize, blockSize);
...
TEST_P(UniformBufferTest, UniformBlocksInSameProgramShareUniformBuffer)
{
    ...
    GLint64 maxUniformBlockSize;
    glGetInteger64v(GL_MAX_UNIFORM_BLOCK_SIZE, &maxUniformBlockSize);
    EXPECT_GE(maxUniformBlockSize, blockSize1 + blockSize2);
    ...
    glGetBufferParameteriv(GL_UNIFORM_BUFFER, GL_BUFFER_SIZE, &bufferSize);
    GLint updateSize = kFloatCount * sizeof(GLfloat);
    EXPECT_EQ(blockSize2 + blockSize1, bufferSize);
    EXPECT_EQ(blockSize2 + blockSize1, updateSize);
...
TEST_P(UniformBufferTest, UniformBlocksInDiffProgramShareUniformBuffer)
{
    ...
    glGetBufferParameteriv(GL_UNIFORM_BUFFER, GL_BUFFER_SIZE, &bufferSize);
    GLint updateSize = kFloatCount * sizeof(GLfloat);
    EXPECT_EQ(std::max(blockSize1, blockSize2), bufferSize);
    EXPECT_EQ(std::max(blockSize1, blockSize2), updateSize);
