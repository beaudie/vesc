...
class UniformBufferTest : public ANGLETest
{
  protected:
    ...
    void SetUp() override
    {
        ...
        mProgram = CompileProgram(shader_library::essl3::vs::simple(), mFragmentShaderSource);
...
{
    GLuint vertexShader = CompileShader(GL_VERTEX_SHADER, shader_library::essl3::vs::simple());
...
TEST_P(UniformBufferTest, SimpleBindingChange)
{
    ...
    ANGLE_GL_PROGRAM(program, shader_library::essl3::vs::simple(), kFragmentShader);

    glBindAttribLocation(program, 0, shader_library::positionAttribName());
