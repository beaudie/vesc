...
layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
void main() {
    instance.f = uintBitsToFloat(0x0da5cc2fu);
})";
    ...
    uint32_t result = static_cast<const uint32_t *>(bufferData)[0];

    // Compare against the uint32_t representation of the float (which was passed
    // to uintBitsToFloat)
    EXPECT_EQ(result, 0x0da5cc2fu);
