...
// Test a shader storage block where a marix field member is set as row-major.
TEST_P(ShaderStorageBufferTest31, ShaderStorageBlockWithRowMajorQualifierOnMatrix)
{
...
// Test a shader storage block where a struct field member is set as row-major.
TEST_P(ShaderStorageBufferTest31, ShaderStorageBlockWithRowMajorQualifierOnStruct)
{
    // TODO(jiajia.qin@intel.com): Figure out why it fails on Intel Linux platform.
    // http://anglebug.com/1951
    ANGLE_SKIP_TEST_IF(IsIntel() && IsLinux());
