...
TEST_P(ShaderStorageBufferTest31, ShaderStorageBufferReadWrite)
{
    ...
    int arrayStride = 0;
    unsigned int index =
        glGetProgramResourceIndex(program.get(), GL_BUFFER_VARIABLE, "blockName.data");
    glGetProgramResourceiv(program.get(), GL_BUFFER_VARIABLE, index, 1, props, 1, 0, &arrayStride);
    constexpr unsigned int arraySize = 2;
    // Create shader storage buffer
    GLBuffer shaderStorageBuffer;
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, shaderStorageBuffer);
    glBufferData(GL_SHADER_STORAGE_BUFFER, arraySize * arrayStride, nullptr, GL_STATIC_DRAW);
    ...
    constexpr unsigned int expectedData[2] = {3u, 4u};
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, shaderStorageBuffer);
    void *ptr =
        glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, arraySize * arrayStride, GL_MAP_READ_BIT);
    for (unsigned int idx = 0; idx < arraySize; idx++)
    {
        EXPECT_EQ(expectedData[idx],
...
TEST_P(ShaderStorageBufferTest31, AtomicMemoryFunctions)
{
    ...
    // Read back shader storage buffer
    constexpr unsigned int expectedData[2] = {5u, 7u};
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, shaderStorageBuffer);
    void *ptr =
        glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, arraySize * arrayStride, GL_MAP_READ_BIT);
    for (unsigned int idx = 0; idx < arraySize; idx++)
