...
// Test readPixels returns the correctly converted colors based on the framebuffer srgb state
TEST_P(SRGBFramebufferTest, ReadPixels)
{
    // http://anglebug.com/5075
    ANGLE_SKIP_TEST_IF(IsOpenGL());

    // Support for this is incomplete
    // http://anglebug.com/5075
    ...
    // Since FRAMEBUFFER_SRGB_EXT is disabled, we should not convert the colors back to linear
    glDisable(GL_FRAMEBUFFER_SRGB_EXT);
    EXPECT_PIXEL_COLOR_NEAR(0, 0, srgbColor, 1.0);
}

// Test that we behave correctly when we disable sRGB on a framebuffer that is already using a
// linear colorspace
