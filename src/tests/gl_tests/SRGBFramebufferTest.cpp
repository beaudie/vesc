...
{
    GLenum expectedError = extensionEnabled("GL_EXT_sRGB_write_control") ? GL_NO_ERROR : GL_INVALID_ENUM;
...
{
    if (!extensionEnabled("GL_EXT_sRGB_write_control") || (!extensionEnabled("GL_EXT_sRGB") && getClientMajorVersion() < 3))
    {
        std::cout << "Test skipped because GL_EXT_sRGB_write_control and GL_EXT_sRGB are not available."
            << std::endl;
        return;
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB_ALPHA_EXT, 1, 1, 0, GL_SRGB_ALPHA_EXT, GL_UNSIGNED_BYTE, nullptr);
