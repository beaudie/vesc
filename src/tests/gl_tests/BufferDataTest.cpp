...
void main()
{
    ...
    // With GL_MAP_UNSYNCHRONIZED_BIT, we expect the buffer to not have the updated data
    // from the glCopyBufferSubData call. It must match it's initial value, dstData.
    // What we intend to do is perform an unsynchronized read access but the spec disallows that.
    // Workaround is to request an unsynchronized write instead (possible undefined behavior)
    ...
    // While describing the GL_MAP_UNSYNCHRONIZED_BIT bit, the spec states -
    ...
    // Our pending operation was the glCopyBufferSubData call. Force a flush/finish here
    // and call glCopyBufferSubData again before expecting updated data
    ...
    // Now we don't use the GL_MAP_UNSYNCHRONIZED_BIT, we expect the data from pending operations
    // to be flushed and the buffer to have updated data to match srcData
