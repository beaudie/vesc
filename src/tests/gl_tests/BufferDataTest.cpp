...
class BufferDataDrawingTest : public ANGLETest
{
  protected:
    ...
    void testTearDown() override
    {
        glDeleteFramebuffers(kNumFramebuffers, mFrameBuffers);
        glDeleteTextures(kNumFramebuffers, mFrameBuffers);
        glDeleteProgram(mProgram);
    }
...
void main()
{
    ...
    bool setupTest()
    {
        glGenFramebuffers(kNumFramebuffers, mFrameBuffers);
        glGenTextures(kNumFramebuffers, mFrameTextures);
    ...
    {
        assert(index < kNumFramebuffers);

        glBindFramebuffer(GL_FRAMEBUFFER, mFrameBuffers[index]);
    ...
    void verifySameRenderResult()
    {
        ...
        for (uint32_t bufferIndex = 0; bufferIndex < kNumFramebuffers; bufferIndex++)
        {
    ...
    static const GLsizei kNumFramebuffers = 2;
    static const GLsizei kRenderWidth = 8, kRenderHeight = 8;
    GLint mAttribLocation;
    GLuint mFrameTextures[kNumFramebuffers];
    GLuint mFrameBuffers[kNumFramebuffers];
    GLuint mProgram;
};

// Test to ensure buffer data format is correctly converted when VBO update is the only state change
...
ANGLE_INSTANTIATE_TEST(IndexedBufferCopyTest,
                       ...
                       ES3_OPENGLES(),
                       ES3_VULKAN());
ANGLE_INSTANTIATE_TEST(BufferDataDrawingTest,
                       ES2_VULKAN(),
                       ES3_VULKAN(),
                       ES3_D3D11(),
                       ES3_OPENGLES(),
