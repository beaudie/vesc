...
TEST_P(BufferDataTestES3, CopyBufferSubDataSelfDependencyUBO)
{
    constexpr char kFS[] = R"(#version 300 es
precision mediump float;
out vec4 color;
...
void main()
{
    ...
    constexpr uint32_t kUBOSize    = 128 * 4 * sizeof(float);
    constexpr uint32_t kDataOffset = 12 * 4 * sizeof(float);

    // Init data is 4 times the size of UBO as the buffer is created larger than the UBO througout
    // the test.
    const std::vector<float> kInitData(kUBOSize, 123.45);

    // Set up a throw-away buffer just to make buffer suballocations not to use offset 0.
    GLBuffer throwaway;
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, kDataOffset, 4 * sizeof(float), kColorData.data());
    ...
    constexpr uint32_t kCopySrcOffset = 4 * 4 * sizeof(float);
    constexpr uint32_t kCopyDstOffset = 68 * 4 * sizeof(float);
