...
class ETCToBCTextureTest : public ANGLETest<>
{
  protected:
    ...
    static constexpr int abs_error = 6;
    ...
    // 0xff000000 0xff000000 0xffa5a6a5 0xffa5a6a5
    static constexpr uint32_t etcAllZero[2]        = {0x0, 0x0};
    static constexpr uint32_t etcRGBData[2]        = {0x14050505, 0x00ffff33};
    static constexpr uint32_t expectedRGBColor[16] = {
    ...
    static constexpr uint32_t etcRGBAData[4] = {0xd556975c, 0x088ff048, 0x9e6c6c6c, 0x3f11f1ff};
    static constexpr uint32_t expectedRGBAColor[16] = {
    ...
    static constexpr uint32_t eacR11Signed[2]            = {0xb068efff, 0x00b989e7};
    static constexpr uint32_t expectedR11SignedColor[16] = {
...
TEST_P(ETCToBCTextureTest, ETC2Rgb8UnormToBC1_2D)
{
    ...
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTexSize, kTexSize, GL_COMPRESSED_RGB8_ETC2,
                              sizeof(etcRGBData), etcRGBData);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    draw2DTexturedQuad(0.5f, 1.0f, false);
    EXPECT_PIXEL_COLOR_NEAR(0, 0, GLColor(expectedRGBColor[0]), abs_error);
    EXPECT_PIXEL_COLOR_NEAR(1, 1, GLColor(expectedRGBColor[5]), abs_error);
    EXPECT_PIXEL_COLOR_NEAR(2, 2, GLColor(expectedRGBColor[10]), abs_error);
    EXPECT_PIXEL_COLOR_NEAR(3, 3, GLColor(expectedRGBColor[15]), abs_error);
}
...
TEST_P(ETCToBCTextureTest, ETC2Rgb8UnormToBC1_Cube)
{
    ...
    glCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, 0, 0, kTexSize, kTexSize,
                              GL_COMPRESSED_RGB8_ETC2, sizeof(etcAllZero), etcAllZero);
    ...
    glCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, 0, 0, kTexSize, kTexSize,
                              GL_COMPRESSED_RGB8_ETC2, sizeof(etcRGBData), etcRGBData);
    ...
    EXPECT_PIXEL_COLOR_NEAR(0, 0, GLColor(expectedRGBColor[10]), abs_error);
}
...
TEST_P(ETCToBCTextureTest, ETC2Rgb8UnormToBC1_2DArray)
{
    ...
    uint32_t data[2 * kArraySize]   = {
          etcAllZero[0], etcAllZero[1],  // array 0
          etcRGBData[0], etcRGBData[1],  // array 1
          etcRGBData[0], etcRGBData[1],  // array 2
          etcAllZero[0], etcAllZero[1],  // array 3
          etcAllZero[0], etcAllZero[1],  // array 4
          etcAllZero[0], etcAllZero[1],  // array 5
    };
...
TEST_P(ETCToBCTextureTest, ETC2Rgba8UnormToBC3)
{
    ...
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTexSize, kTexSize,
                              GL_COMPRESSED_RGBA8_ETC2_EAC, sizeof(etcRGBAData), etcRGBAData);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    draw2DTexturedQuad(0.5f, 1.0f, false);
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            EXPECT_PIXEL_COLOR_NEAR(j, i, GLColor(expectedRGBAColor[i * 4 + j]), abs_error);
        }
    }
}
...
TEST_P(ETCToBCTextureTest, ETC2R11SignedToBC4)
{
    ...
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTexSize, kTexSize,
                              GL_COMPRESSED_SIGNED_R11_EAC, sizeof(eacR11Signed), eacR11Signed);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    constexpr char kVS[] =
    ...
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            EXPECT_PIXEL_COLOR_NEAR(j, i, GLColor(expectedR11SignedColor[i * 4 + j]), abs_error);
        }
    }
}
...
TEST_P(ETCToBCTextureTest, ETC2RG11ToBC5)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            uint32_t color = (expectedRGBAColor[i * 4 + j] & 0xff000000) >> 24;
            color |= color << 8;
            color |= 0xff000000;
            EXPECT_PIXEL_COLOR_NEAR(j, i, GLColor(color), abs_error);
        }
    }
}
