...
class ClientSideDataTest : public ANGLETest
{
  protected:
    ...
    void checkPixels() const
    {
        ...
        {
            EXPECT_PIXEL_EQ(location.first, location.second, mTestColor[0], mTestColor[1], mTestColor[2], mTestColor[3]);
        }
    }
...
TEST_P(ClientSideDataTest, NonIndexedClientSideVertexWithStartVertex)
{
    ...
    triangleCopy.insert(triangleCopy.begin(), 30.0f);
    triangleCopy.insert(triangleCopy.begin(), 20.0f);
...
TEST_P(ClientSideDataTest, NonIndexedClientSideVertexAndBufferVertex)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, mTestTriangle.size() * sizeof(GLfloat), &mTestTriangle[0], GL_STATIC_DRAW);
...
TEST_P(ClientSideDataTest, NonIndexedClientSideVertexAndBufferVertexWithOffset)
{
    ...
    glGenBuffers(1, &buffer);
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glBufferData(GL_ARRAY_BUFFER, triangleCopy.size() * sizeof(GLfloat), &triangleCopy[0], GL_STATIC_DRAW);
    glVertexAttribPointer(mPositionAttrib, 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<const GLvoid*>(sizeof(GLfloat)));
    glEnableVertexAttribArray(mPositionAttrib);
...
// Test a DrawArrays call with client side and buffered vertex data with a buffer offset and non-zero first vertex
...
// Test a DrawElements call with buffered and client side vertex data and buffered, offset index data
TEST_P(ClientSideDataTest, ByteIndexedClientSideVertexAndBufferVertexAndBufferedOffsetIndices)
{
    ...
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_BYTE, reinterpret_cast<const GLvoid*>(sizeof(GLubyte) * 4));
...
TEST_P(ClientSideDataTest, ShortIndexedClientSideVertexAndBufferVertexAndBufferedOffsetIndices)
{
    ...
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, reinterpret_cast<const GLvoid*>(sizeof(GLushort) * 4));
...
TEST_P(ClientSideDataTest, UintIndexedClientSideVertexAndBufferVertexAndBufferedOffsetIndices)
{
    ...
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, reinterpret_cast<const GLvoid*>(sizeof(GLuint) * 4));
...
ANGLE_INSTANTIATE_TEST(ClientSideDataTest, ES2_D3D9(), ES2_D3D11(), ES2_D3D11_FL9_3(), ES2_OPENGL(), ES3_OPENGL());
