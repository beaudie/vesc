...
TEST_P(VulkanPerformanceCounterTest, ReadOnlyDepthFeedbackLoopWithClearAndScissoredDraw)
{
...
void main()
{
    ...
    // We expect depth clear will be done in the renderpass loadOp. When defer clear is broken, we
    ...
    EXPECT_EQ(expectedRenderPassDepthClearCount, counters.depthClears);
    EXPECT_EQ(expectedRenderPassDepthLoadCount, counters.depthLoads);
}
