...
TEST_P(VulkanPerformanceCounterTest, IndependentBufferCopiesShareSingleBarrier)
{
    ...
    rx::ContextVk *contextVk            = hackANGLE();
    uint32_t expectedCommandBufferCount = contextVk->getSecondaryCommandBufferCounter();
    ...
    uint32_t actualCommandBufferCount = contextVk->getSecondaryCommandBufferCounter();
    EXPECT_EQ(expectedCommandBufferCount, actualCommandBufferCount);
}
