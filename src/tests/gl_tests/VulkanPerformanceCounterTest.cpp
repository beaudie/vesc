...
TEST_P(VulkanPerformanceCounterTest, InvalidateDrawDisable)
{
    ...
    // Start and end another render pass, to check that the load ops are as expected
    // Stencil never had valid contents (clearStencil=false, glInvalidateFramebuffer()), so it's
    // never stored or loaded.
    setAndIncrementLoadCountersForInvalidateTest(counters, 1, 0, &expected);
    drawQuad(program, essl1_shaders::PositionAttrib(), 0.5f);
    ASSERT_GL_NO_ERROR();
    EXPECT_PIXEL_COLOR_EQ(0, 0, GLColor::red);
...
TEST_P(VulkanPerformanceCounterTest, DrawThenInceptionScissorClears)
{
    ...
    EXPECT_EQ(hackANGLE().colorClears, 1u);
    // The rest of the scissored clears should use vkCmdClearAttachments().
    EXPECT_EQ(hackANGLE().vkCmdClearAttachments, numScissoredClears - 1);
