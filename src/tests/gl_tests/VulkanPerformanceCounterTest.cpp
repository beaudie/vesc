...
TEST_P(VulkanPerformanceCounterTest, ReadOnlyDepthBufferLayout)
{
    ...
    uint32_t expectedReadOnlyDepthStencilCount = counters.readOnlyDepthStencils;
    GLTexture depthTexture;
    glBindTexture(GL_TEXTURE_2D, depthTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, kSize, kSize, 0, GL_DEPTH_COMPONENT,
                 GL_UNSIGNED_INT, nullptr);
    ...
    glFinish();
    uint32_t actualReadOnlyDepthStencilCount = counters.readOnlyDepthStencils;
    ...
    expectedReadOnlyDepthStencilCount = counters.readOnlyDepthStencils + 1;
    GLTexture colorTexture;
    glBindTexture(GL_TEXTURE_2D, colorTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kSize, kSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    GLFramebuffer depthAndColorFBO;
    ...
    actualReadOnlyDepthStencilCount = counters.readOnlyDepthStencils;
    EXPECT_EQ(expectedReadOnlyDepthStencilCount, actualReadOnlyDepthStencilCount);
}
