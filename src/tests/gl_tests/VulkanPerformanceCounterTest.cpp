...
TEST_P(VulkanPerformanceCounterTest, SubmittingOutsideCommandBufferDoesNotBreakRenderPass)
{
    ...
    uint32_t expectedSubmitFrameCount = counters.submittedFrames + kNumSubmits;
    ...
    auto maxLoadCount = static_cast<size_t>(
        (rx::vk::ImageHelper::kMaxCopySize / (texDim * texDim * pixelSizeRGBA)) * kNumSubmits + 1);
    ...
    EXPECT_EQ(counters.submittedFrames, expectedSubmitFrameCount);
}
