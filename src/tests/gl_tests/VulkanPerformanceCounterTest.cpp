...
TEST_P(VulkanPerformanceCounterTest, InvalidateAndClear)
{
    ...
    // Do in renderpass clear. This should result in StoreOp and mark content as
    // defined.
    glClearDepthf(1.0f);
    ...
    // Bind FBO again and try to use the depth buffer without clear. This should result in loadOp
    // and StoreOP on depth
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);
    glDisable(GL_STENCIL_TEST);
