...
TEST_P(OcclusionQueriesTest, IsOccluded)
{
    ...
    drawQuad(mProgram, shader_library::positionAttribName(), 0.3f);
    glUseProgram(0);
    ...
    drawQuad(mProgram, shader_library::positionAttribName(),
...
TEST_P(OcclusionQueriesTest, IsNotOccluded)
{
    ...
    glBeginQueryEXT(GL_ANY_SAMPLES_PASSED_EXT, query);
    drawQuad(mProgram, shader_library::positionAttribName(),
             0.8f);  // this quad should not be occluded
    glEndQueryEXT(GL_ANY_SAMPLES_PASSED_EXT);
...
TEST_P(OcclusionQueriesTest, MultiContext)
{
    ...
    for (size_t pass = 0; pass < passCount; pass++)
    {
        for (const auto &context : contexts)
        {
            ...
            drawQuad(context.program, shader_library::positionAttribName(), depth);
