...
TEST_P(OcclusionQueriesNoSurfaceTestES3, SwitchingContextsWithQuery)
{
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        for (ContextInfo *pinfo : infos)
        {
            ...
            // create context with no draw and read surface
            ASSERT_NE(info.context, EGL_NO_CONTEXT);
            eglMakeCurrent(display, nullptr, nullptr, info.context);
        ...
        eglMakeCurrent(display, nullptr, nullptr, info1.context);

        glBeginQuery(GL_ANY_SAMPLES_PASSED_CONSERVATIVE, info1.query);
        glFlush();

        eglMakeCurrent(display, nullptr, nullptr, info2.context);
    ...
    {
        eglMakeCurrent(display, nullptr, nullptr, pinfo->context);
        EXPECT_GL_NO_ERROR();
        delete pinfo;
        eglDestroyContext(display, pinfo->context);
    }
}
