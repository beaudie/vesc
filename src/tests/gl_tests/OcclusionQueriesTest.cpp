...
TEST_P(OcclusionQueriesTest, MultiContext)
{
    ...
    {
        std::cout << "Test skipped because the D3D cannot support simultaneous queries on multiple "
                     "contexts yet."
                  << std::endl;
        return;
    }
    ...
    for (const auto &context : contexts)
    {
        ...
        {
            angle::Sleep(0);
            glGetQueryObjectuivEXT(context.query, GL_QUERY_RESULT_AVAILABLE_EXT, &ready);
        }
