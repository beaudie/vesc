...
TEST_P(ClipCullDistanceTest, SizedArrayLength)
{
    ...
    auto vs = [=, &vertexSource](std::string name, bool declare, int size) {
        ...
        vertexSource << "out float v_length;\n";
        vertexSource << "void main()\n";
        vertexSource << "{\n";
        vertexSource << "    gl_Position = a_position;\n";
        vertexSource << "    v_length = float(" << name << ".length()) / 16.0;\n";
        if (!declare)
        {
            vertexSource << "    " << name << "[" << (size - 1) << "] = 1.0;\n";
        }
...
void main()
{
    ...
    auto checkLength = [=, &vertexSource](std::string name, bool declare, int size) {
        ...
        EXPECT_PIXEL_RECT_EQ(0, 0, getWindowWidth(), getWindowHeight(),
                             GLColor(size * 16, 0, 0, 255));
    };
