...
void main()
{
    ...
    // kInputValues11); glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0, 1, 1, GL_RED_INTEGER,
    // GL_UNSIGNED_INT, kInputValues12);
    EXPECT_GL_NO_ERROR();

    glBindTexture(GL_TEXTURE_2D, texture[1]);
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA32UI, 1, 1);
    // glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1, 1, GL_RGBA_INTEGER, GL_UNSIGNED_INT,
    // kInputValues);
    EXPECT_GL_NO_ERROR();
