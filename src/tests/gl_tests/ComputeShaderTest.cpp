...
{
    // TODO(xinghua.cao@intel.com): There may be a bug on a version of Nvidia driver.
    // Texture2D.GetDimensions returned the dimensions of the resource's level zero
    // even if SRV had specified that MostDetailedMip was not zero.
    ANGLE_SKIP_TEST_IF(IsNVIDIA() && IsD3D11());

    // TODO(xinghua.cao@intel.com): There may be a bug on a version of Intel driver.
    // Crash happened when calling glDispatchCompute, in condition that glTexStorage2D
    // specified multi-levles.
