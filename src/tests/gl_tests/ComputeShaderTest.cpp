...
// Verify using atomic functions in an initializers and using unsigned int can work correctly.
TEST_P(ComputeShaderTest, AtomicFunctionsInitializerWithUnsigned)
{
    ...
    const std::array<GLuint, 9> inputData      = {{1, 2, 4, 8, 16, 32, 64, 128, 1}};
    const std::array<GLuint, 9> expectedValues = {{0, 1, 1, 4, 0, 16, 48, 64, 128}};
    runSharedMemoryTest<GLuint, 9, 1>(kCSShader, GL_R32UI, GL_UNSIGNED_INT, inputData,
                                      expectedValues);
}

// Verify using atomic functions returns and unsigned int
TEST_P(ComputeShaderTest, AtomicFunctionsReturnWithUnsigned)
{
    ...
    const std::array<GLuint, 9> inputData      = {{1, 2, 4, 8, 16, 32, 64, 128, 1}};
    const std::array<GLuint, 9> expectedValues = {{1, 2, 2, 5, 1, 17, 49, 65, 129}};
    runSharedMemoryTest<GLuint, 9, 1>(kCSShader, GL_R32UI, GL_UNSIGNED_INT, inputData,
                                      expectedValues);
}

// Verify using atomic function returns and multiple variables.
TEST_P(ComputeShaderTest, AtomicFunctionsReturnWithMultipleTypes)
{
    ...
    const std::array<GLuint, 4> inputData      = {{1, 2, 3, 0}};
    const std::array<GLuint, 4> expectedValues = {{1, 4, 6, 6}};
    runSharedMemoryTest<GLuint, 4, 1>(kCSShader, GL_R32UI, GL_UNSIGNED_INT, inputData,
                                      expectedValues);
}
