...
class ComputeShaderTest : public ANGLETest
{
  protected:
    ...
    template <class T, GLint kWidth, GLint kHeight, GLenum internalFormat, GLenum format>
...
TEST_P(ComputeShaderTest, NonArraySharedVariable)
{
    ...
    runSharedMemoryTest<GLuint, 2, 2, GL_R32UI, GL_UNSIGNED_INT>(kCSShader, inputData,
                                                                 expectedValues);
}
...
TEST_P(ComputeShaderTest, AtomicFunctionsNoReturnValue)
{
    const char kCSShader[] =
        ...
        const highp uint kSumIndex = 0u;
        const highp uint kMinIndex = 1u;
        const highp uint kMaxIndex = 2u;
        const highp uint kOrIndex = 3u;
        const highp uint kAndIndex = 4u;
        const highp uint kXorIndex = 5u;
        ...
        void main()
        {
            ...
            {
                results[gl_LocalInvocationID.x] = 0xFFFFFFFFu;
            }
            else
            {
    ...
    runSharedMemoryTest<GLuint, 6, 1, GL_R32UI, GL_UNSIGNED_INT>(kCSShader, inputData,
                                                                 expectedValues);
}
...
TEST_P(ComputeShaderTest, AtomicFunctionsInNonInitializerSingleAssignment)
{
    const char kCSShader[] =
        ...
        shared highp int result;
        shared highp int inputData[8];
        ...
        void main()
        {
            ...
            if (gl_LocalInvocationID.x == 0u)
            {
                ...
                oldValue = atomicAdd(result, inputData[0]);
                imageStore(dstImage, ivec2(0, 0), ivec4(oldValue));
            
                oldValue = atomicMin(result, inputData[1]);
                imageStore(dstImage, ivec2(1, 0), ivec4(oldValue));

                oldValue = atomicMax(result, inputData[2]);
                imageStore(dstImage, ivec2(2, 0), ivec4(oldValue));

                oldValue = atomicAnd(result, inputData[3]);
                imageStore(dstImage, ivec2(3, 0), ivec4(oldValue));

                oldValue = atomicOr(result, inputData[4]);
                imageStore(dstImage, ivec2(4, 0), ivec4(oldValue));

                oldValue = atomicXor(result, inputData[5]);
                imageStore(dstImage, ivec2(5, 0), ivec4(oldValue));

                oldValue = atomicExchange(result, inputData[6]);
                imageStore(dstImage, ivec2(6, 0), ivec4(oldValue));

                oldValue = atomicCompSwap(result, 64, inputData[7]);
                imageStore(dstImage, ivec2(7, 0), ivec4(oldValue));

                imageStore(dstImage, ivec2(8, 0), ivec4(result)); 
            }
        })";

    const std::array<GLint, 9> inputData      = {{1, 2, 4, 8, 16, 32, 64, 128, 0}};
    const std::array<GLint, 9> expectedValues = {{0, 1, 1, 4, 0, 16, 48, 64, 128}};
    runSharedMemoryTest<GLint, 9, 1, GL_R32I, GL_INT>(kCSShader, inputData, expectedValues);
}
