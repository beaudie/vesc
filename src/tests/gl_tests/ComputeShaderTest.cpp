...
TEST_P(ComputeShaderTest, LinkComputeProgramWithUniforms)
{
    const std::string csSource =
        ...
        {
            int q = myUniformInt;
            vec4 sample = texture(myUniformSampler, vec2(0.0));
            imageStore(imageOut, ivec2(0), ivec4(sample) * q);
        })";
...
TEST_P(ComputeShaderTest, AccessPartSpecialVariables)
{
    const std::string csSource =
        ...
        void main()
        {
            ...
            imageStore(imageOut, ivec2(0), uvec4(temp1 + temp2, temp3));
        })";
...
TEST_P(ComputeShaderTest, ImageLoad)
{
    const std::string csSource =
        ...
        void main()
        {
            ...
            imageStore(imageOut, ivec2(0), sizeCube.x + size2D.x + size2DArray.x);
        })";
...
TEST_P(ComputeShaderTest, ImageSize)
{
    const std::string csSource =
        ...
        layout(r32i) uniform highp writeonly uimage2D imageOut;
        void main()
        {
