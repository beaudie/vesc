...
TEST_P(ComputeShaderTest, LinkComputeProgramWithUniforms)
{
    const std::string csSource =
        ...
        {
            int q = myUniformInt;
            vec4 v = texture(myUniformSampler, vec2(0.0));
...
TEST_P(ComputeShaderTest, ImageLoad)
{
    const std::string csSource =
        ...
        void main()
        {
            ...
            imageStore(imageOut, ivec2(0), ivec4(result2d) + resultCube + ivec4(result3d));
        })";
...
TEST_P(ComputeShaderTest, ImageSize)
{
    const std::string csSource =
        ...
        void main()
        {
            ...
            imageStore(imageOut, ivec2(0), ivec4(sizeCube, size2D.x, size2DArray.x));
        })";
