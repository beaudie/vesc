...
// Test that render pipeline writes to a |texture1| and compute pipeline reads from |texture1|, does
// some changes and saves the result to |texture2|.
TEST_P(ComputeShaderTest, DrawTexture1DispatchTexture2)
{
    // See http://anglebug.com/3658
    ANGLE_SKIP_TEST_IF(IsD3D11());
    const char kCSSource[] = R"(#version 310 es
layout(local_size_x=1, local_size_y=1, local_size_z=1) in;
