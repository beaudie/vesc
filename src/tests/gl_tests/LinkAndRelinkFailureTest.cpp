...
TEST_P(LinkAndRelinkFailureTest, RenderingProgramLinkRelinkFailWithProgramInstalledTest)
{
    ...
    // Link failure, and a valid program has been installed in the pipeline.
    ...
    // To use the unsuccessfully relinked program, report error.
    glUseProgram(program);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);

    // To use the unsuccessfully relinked program, that program should not
    // replace the program binary resided in pipeline. It will not make the
    // installed program invalid, like what UseProgram(0) can do.
    // So, to start rendering should success.
    glDrawArrays(GL_POINTS, 0, 1);
