...
TEST_P(UniformTestES31, StructLocationLayoutQualifier)
{
    ...
    ASSERT_EQ(1u, mProgram);

    EXPECT_EQ(12, glGetUniformLocation(mProgram, "uS.f"));
    EXPECT_EQ(13, glGetUniformLocation(mProgram, "uS.f2"));
}

// Set uniform location by a layout qualifier in the fragment shader. The same uniform exists in the
// vertex shader, but doesn't have a location specified there.
TEST_P(UniformTestES31, UniformLocationInFragmentShader)
{
    ...
    EXPECT_EQ(12, glGetUniformLocation(mProgram, "tex2D"));
}
...
ANGLE_INSTANTIATE_TEST(UniformTestES31, ES31_OPENGL(), ES31_OPENGLES());
