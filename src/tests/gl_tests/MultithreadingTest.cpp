...
TEST_P(MultithreadingTestES3, RenderThenSampleDifferentContextPriority)
{
    ...
    std::thread thread0 = std::thread([&]() {
        ...
        for (size_t i = 1; i <= kIterationCount; ++i)
        {
            // Simulate heavy work...
            ...
            ASSERT_TRUE(threadSynchronization.waitForStep(
                i == kIterationCount ? Step::Finish : Step::Thread1Draw));
        }
    ...
    std::thread thread1 = std::thread([&]() {
        ...
        for (size_t i = 1; i <= kIterationCount; ++i)
        {
            ...
            threadSynchronization.nextStep(i == kIterationCount ? Step::Finish : Step::Thread1Draw);
        }
...
TEST_P(MultithreadingTestES3, ContextPriorityMixing)
{
    ...
    std::thread thread0 = std::thread([&]() {
        ...
        for (size_t i = 1; i <= kIterationCount; ++i)
        {
            ASSERT_TRUE(threadSynchronization.waitForStep(Step::Thread1DrawColor));
            ...
            threadSynchronization.nextStep(Step::Thread0Iterate);
    ...
    std::thread thread1 = std::thread([&]() {
        ...
        for (size_t i = 1; i <= kIterationCount; ++i)
        {
            ...
            // This should force "flushToPrimary()" on glEndQuery()
            GLuint query;
            ...
            threadSynchronization.nextStep(Step::Thread1DrawColor);
            ASSERT_TRUE(threadSynchronization.waitForStep(Step::Thread0Iterate));
