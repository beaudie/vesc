        ...
        for (size_t threadIdx = 0; threadIdx < threadCount; threadIdx++)
        {
            threads[threadIdx] = std::thread([&, threadIdx]() {
...

                {
                    ...
                    // Give the first thread the window surface to work on, otherwise create a
                    // pbuffer
                    if (threadIdx == 0)
                    {
...
TEST_P(MultithreadingTest, MultiContextDraw)
{
    ...
    auto testBody = [this](EGLSurface surface, size_t thread) {
        EGLWindow *window = getEGLWindow();
        EGLDisplay dpy    = window->getDisplay();

        constexpr size_t kIterationsPerThread = 10000;
        constexpr size_t kDrawsPerIteration   = 1000;
        ...
        for (size_t iteration = 0; iteration < kIterationsPerThread; iteration++)
        {
            ...
            std::cout << kDrawsPerIteration << " draws complete.\n";
            EXPECT_PIXEL_COLOR_EQ(0, 0, color);
