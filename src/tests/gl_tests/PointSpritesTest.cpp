...
TEST_P(PointSpritesTest, PointSizeAboveMaxIsClamped)
{
    ...
    if (IsOSX() && IsAMD() && IsOpenGL())
    {
        // Failed on AMD OSX trybot - no pixels from the point were detected in the framebuffer.
        // http://anglebug.com/2113
        std::cout << "Test skipped on AMD OpenGL on Mac." << std::endl;
        return;
    }
