...
class CopyTexImageTest : public ANGLETest
{
  protected:
    ...
    GLuint createTextureFromFBO(GLuint fbo, GLenum format) const
    {
        GLuint tex;
        glGenTextures(1, &tex);
        glBindTexture(GL_TEXTURE_2D, tex);
    ...
    void verifyResults(GLuint texture, GLubyte data[4]) const
    {
...
TEST_P(CopyTexImageTest, RGBAToL)
{
    ...
    GLuint tex = createTextureFromFBO(fbo, GL_LUMINANCE);

    GLubyte expected[] = {
        64, 64, 64, 255,
    };
    verifyResults(tex, expected);
}
...
TEST_P(CopyTexImageTest, RGBAToLA)
{
    ...
    GLuint fbo = createFramebuffer(GL_RGBA, GL_UNSIGNED_BYTE, color);
    GLuint tex = createTextureFromFBO(fbo, GL_LUMINANCE_ALPHA);
...
TEST_P(CopyTexImageTest, RGBAToA)
{
    ...
    GLuint fbo = createFramebuffer(GL_RGBA, GL_UNSIGNED_BYTE, color);
    GLuint tex = createTextureFromFBO(fbo, GL_ALPHA);
