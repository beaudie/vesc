...
{
    // Seems to fail on AMD D3D11 Windows.
    ANGLE_SKIP_TEST_IF(IsAMD() && IsD3D11() & IsWindows());
    ...
    glTexImage3D(GL_TEXTURE_3D, 0 /* level */, GL_RGBA8, kTexSize, kTexSize, kLayers,
                 0 /* border */, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    glTexSubImage3D(GL_TEXTURE_3D, 0 /* level */, 0 /* xoffset */, 0 /* yoffset */, 1 /* zoffset */,
                    kTexSize, kTexSize, 1 /* depth */, GL_RGBA, GL_UNSIGNED_BYTE, red.data());
    glFramebufferTextureLayer(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, src_object_id,
                              0 /* level */, 1 /* layer */);
    ASSERT_GL_NO_ERROR();

    GLTexture dst_object_id;
    glBindTexture(GL_TEXTURE_2D, dst_object_id);
    glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 0 /* x */, 0 /* y */, kTexSize, kTexSize,
                     0 /* border */);
