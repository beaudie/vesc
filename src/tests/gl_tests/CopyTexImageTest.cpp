...
class CopyTexImageTest : public ANGLETest
{
  protected:
    ...
    void runCopyTexImageTestCheckered(GLenum format,
                                      ...
                                      const GLubyte expected_data0[4],
                                      const GLubyte expected_data1[4],
                                      const GLubyte expected_data2[4],
                                      const GLubyte expected_data3[4],
                                      GLint flip_y)
    {
        GLTexture tex;
        glBindTexture(GL_TEXTURE_2D, tex);
        ...
        for (size_t i = 0; i < kFboCount; ++i)
        {
            ...
            glFramebufferParameteriMESA(GL_FRAMEBUFFER, GL_FRAMEBUFFER_FLIP_Y_MESA, flip_y);
            glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, x[i], y[i], width[i], height[i], 0);
            ASSERT_GL_NO_ERROR();

            verifyCheckeredResults(tex, expected_data0, expected_data1, expected_data2,
                                   expected_data3, kFboSizes[i], kFboSizes[i]);
        }
    }
...
TEST_P(CopyTexImageTest, CopyTexImageMesaYFlip)
{
    ...
    std::array<uint32_t, 3> copy_origin{0};
    std::array<uint32_t, 3> copy_size;
    ;
    for (size_t i = 0; i < kFboCount; i++)
        copy_size[i] = kFboSizes[i];

    initializeResources(GL_RGBA, GL_UNSIGNED_BYTE);
    runCopyTexImageTestCheckered(GL_RGBA, copy_origin.data(), copy_origin.data(), copy_size.data(),
                                 copy_size.data(), GLColor::green.data(), GLColor::red.data(),
                                 GLColor::yellow.data(), GLColor::blue.data(), 1 /* flip y */);
}
...
TEST_P(CopyTexImageTest, CopyTexImageMesaYFlipPartial)
{
    ...
    std::array<uint32_t, kFboCount> copy_x;
    std::array<uint32_t, kFboCount> copy_y{0};
    std::array<uint32_t, kFboCount> copy_width;
    std::array<uint32_t, kFboCount> copy_height;

    for (size_t i = 0; i < kFboCount; i++)
    {
        copy_x[i]      = kFboSizes[i] / 2;
        copy_height[i] = kFboSizes[i];
    }
    copy_width = copy_x;

    initializeResources(GL_RGBA, GL_UNSIGNED_BYTE);
    runCopyTexImageTestCheckered(GL_RGBA, copy_x.data(), copy_y.data(), copy_width.data(),
                                 copy_height.data(), GLColor::yellow.data(), GLColor::blue.data(),
                                 GLColor::yellow.data(), GLColor::blue.data(), 1 /* flip y */);
}
