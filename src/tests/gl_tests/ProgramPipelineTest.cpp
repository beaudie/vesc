...
// Test glUniform in the vertex shader
// This uses (essentially) the same shaders as SimpleOperationTest::DrawQuadWithVertexUniform()
TEST_P(ProgramPipelineTest31, VertexStageUniformTest)
{
    ANGLE_SKIP_TEST_IF(!IsVulkan());
...
void main()
{
    my_FragColor = vec4(vcolor, 0.0, 0.0, 1.0);
})";
    ...
    GLint location = glGetUniformLocation(vertProg, "color");
    glActiveShaderProgram(pipeline, vertProg);
    glUniform1f(location, 1.0f);

    ProgramPipelineTest::drawQuad("position", 0.5f, 1.0f);
    ASSERT_GL_NO_ERROR();
    EXPECT_PIXEL_COLOR_EQ(0, 0, GLColor::red);
}

// Test glUniform in the fragment shader
TEST_P(ProgramPipelineTest31, FragmentStageUniformTest)
{
    ...
    const GLchar *fragString = R"(#version 310 es
precision highp float;
uniform float fragColorIn;
out vec4 my_FragColor;
void main()
{
    my_FragColor = vec4(fragColorIn, 0.0, 0.0, 1.0);
})";
    ...
    // Set the output color to red
    GLint location = glGetUniformLocation(fragProg, "fragColorIn");
