...
// Test a PPO scenario from a game that was crashing,
// calling BindBufferRange between two draws with multiple ubos
...
void main()
{
    ...
    glBufferData(GL_UNIFORM_BUFFER, 512, 0, GL_DYNAMIC_DRAW);
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo1, 0, 64);
    glBindBufferRange(GL_UNIFORM_BUFFER, 1, ubo1, 64, 128);
    glBindBufferRange(GL_UNIFORM_BUFFER, 2, ubo1, 128, 196);
    glBindBufferRange(GL_UNIFORM_BUFFER, 3, ubo1, 192, 256);
    EXPECT_GL_NO_ERROR();
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo1, 256, 320);
    EXPECT_GL_NO_ERROR();
