...
void PrecisionIntTest(GLProgram &program,
                      int bits,
                      ...
                      EvalFunc evaluationFunction)
{
    ...
    for (int testIteration = 0; testIteration < 1; testIteration++)
    {
        ...
        //        INFO() << "Yuxin Debug in0 is: " << in0 << " in1 is: " << in1;
        int refMasked =
            static_cast<int>(evaluationFunction(in0, in1) & static_cast<uint64_t>(mask));
        //      int refOut = deSignExtendTo32(refMasked, bits);
        //        INFO() << "Yuxin Debug refOut is: " << refOut << " refMasked is: " << refMasked;
        ...
        for (int y = 0; y < 32; y++)
        {
            for (int x = 0; x < 32; x++)
            {
                ...
                //                if(x == 0 && y == 0)
                //                {
                //                    INFO() << "Yuxin Debug cmpOut is: " << cmpOut << " cmpMasked
                //                    is: " << cmpMasked;
                //                }
...
void PrecisionIntMediumpAddInVertexShader()
{
    // Setup Shader Programs
    GLProgram intMediumpAddInVertexShader;
    intMediumpAddInVertexShader.makeRaster(kIntMediumpAddVertexVS, kIntMediumpAddVertexFS);
    glUseProgram(intMediumpAddInVertexShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpAddInVertexShader, 16, int0Range, int1Range, &add);
}

// dEQP-GLES3.functional.shaders.precision.int.medium_add_fragment
void PrecisionIntMediumpAddInFragmentShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpAddInFragmentShader;
    intMediumpAddInFragmentShader.makeRaster(kIntMediumpAddFragmentVS, kIntMediumpAddFragmentFS);
    glUseProgram(intMediumpAddInFragmentShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpAddInFragmentShader, 16, int0Range, int1Range, &add);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_sub_vertex
void PrecisionIntMediumpSubInVertexShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpSubInVertexShader;
    intMediumpSubInVertexShader.makeRaster(kIntMediumpSubVertexVS, kIntMediumpSubVertexFS);
    glUseProgram(intMediumpSubInVertexShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpSubInVertexShader, 16, int0Range, int1Range, &sub);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_sub_fragment
void PrecisionIntMediumpSubInFragmentShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpSubInFragmentShader;
    intMediumpSubInFragmentShader.makeRaster(kIntMediumpSubFragmentVS, kIntMediumpSubFragmentFS);
    glUseProgram(intMediumpSubInFragmentShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpSubInFragmentShader, 16, int0Range, int1Range, &sub);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_mul_vertex
void PrecisionIntMediumpMulInVertexShader()
{
    // Setup shaderPrograms
    GLProgram intMediumpMulInVertexShader;
    intMediumpMulInVertexShader.makeRaster(kIntMediumpMulVertexVS, kIntMediumpMulVertexFS);
    glUseProgram(intMediumpMulInVertexShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpMulInVertexShader, 16, int0Range, int1Range, &mul);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_mul_fragment
void PrecisionIntMediumpMulInFragmentShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpMulInFragmentShader;
    intMediumpMulInFragmentShader.makeRaster(kIntMediumpMulFragmentVS, kIntMediumpMulFragmentFS);
    glUseProgram(intMediumpMulInFragmentShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(-(1 << 15), (1 << 15) - 1);
    PrecisionIntTest(intMediumpMulInFragmentShader, 16, int0Range, int1Range, &mul);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_div_vertex
void PrecisionIntMediumpDivInVertexShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpDivInVertexShader;
    intMediumpDivInVertexShader.makeRaster(kIntMediumpDivVertexVS, kIntMediumpDivVertexFS);
    glUseProgram(intMediumpDivInVertexShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(1, 1000);
    PrecisionIntTest(intMediumpDivInVertexShader, 16, int0Range, int1Range, &div);
}

// dEQP-GLES3.functional.shaders.precision.int.mediump_div_fragment
void PrecisionIntMediumpDivInFragmentShader()
{
    // Setup ShaderPrograms
    GLProgram intMediumpDivInFragmentShader;
    intMediumpDivInFragmentShader.makeRaster(kIntMediumpDivFragmentVS, kIntMediumpDivFragmentFS);
    glUseProgram(intMediumpDivInFragmentShader);
    angle::Vector2U int0Range(-(1 << 15), (1 << 15) - 1);
    angle::Vector2U int1Range(1, 1000);
    PrecisionIntTest(intMediumpDivInFragmentShader, 16, int0Range, int1Range, (&div));
}

// dEQP-GLES3.functional.shaders.precision.int.lowp_add_vertex
void PrecisionIntLowpAddIntVertexShader()
{
    // Setup ShaderPrograms
    GLProgram intLowpAddInVertexShader;
    intLowpAddInVertexShader.makeRaster(kIntLowpAddVertexVS, kIntLowpAddVertexFS);
    glUseProgram(intLowpAddInVertexShader);
    angle::Vector2U int0Range(-(1 << 7), (1 << 7) - 1);
    angle::Vector2U int1Range(-(1 << 7), (1 << 7) - 1);
    PrecisionIntTest(intLowpAddInVertexShader, 8, int0Range, int1Range, (&add));
}
...
TEST_P(ShaderPrecisionTest, MediumpToLowp)
{
    ...
    // iterations
    PrecisionIntMediumpAddInVertexShader();
    PrecisionIntMediumpAddInFragmentShader();
    PrecisionIntMediumpSubInVertexShader();
    PrecisionIntMediumpSubInFragmentShader();
    PrecisionIntMediumpMulInVertexShader();
    PrecisionIntMediumpMulInFragmentShader();
    PrecisionIntMediumpDivInVertexShader();
    PrecisionIntMediumpDivInFragmentShader();
    PrecisionIntLowpAddIntVertexShader();
}
