...
struct OpaqueFdTraits
{
    ...
    static void glImportSemaphore(GLuint semaphore, Handle handle)
    {
    ...
    static void glImportMemory(GLuint memoryObject, GLuint64 size, Handle handle)
    {
...
void RunShouldImportMemoryTest(bool isSwiftshader)
{
...

    {
        ...
        glMemoryObjectParameterivEXT(memoryObject, GL_DEDICATED_MEMORY_OBJECT_EXT,
                                     &dedicatedMemory);
        Traits::glImportMemory(memoryObject, deviceMemorySize, memoryHandle);
...
TEST_P(VulkanExternalImageTest, ShouldImportMemoryOpaqueFd)
{
    ...
    RunShouldImportMemoryTest<OpaqueFdTraits>(isSwiftshader());
}
...
TEST_P(VulkanExternalImageTest, ShouldImportMemoryZirconVmo)
{
    ...
    RunShouldImportMemoryTest<FuchsiaTraits>(isSwiftshader());
}

template <typename Traits>
void RunShouldImportSemaphoreTest(bool isSwiftshader)
{
...

    {
        ...
        Traits::glImportSemaphore(glSemaphore, semaphoreHandle);
    }
...
TEST_P(VulkanExternalImageTest, ShouldImportSemaphoreOpaqueFd)
{
    ...
    RunShouldImportSemaphoreTest<OpaqueFdTraits>(isSwiftshader());
}
...
TEST_P(VulkanExternalImageTest, ShouldImportSemaphoreZirconEvent)
{
    ...
    RunShouldImportSemaphoreTest<FuchsiaTraits>(isSwiftshader());
}

template <typename Traits>
void RunShouldClearTest(bool isSwiftshader)
{
    ASSERT(EnsureGLExtensionEnabled(Traits::memoryObjectExtension()));
...
TEST_P(VulkanExternalImageTest, ShouldClearOpaqueFdRGBA8)
{
    ...
    RunShouldClearTest<OpaqueFdTraits>(isSwiftshader());
}
...
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_ANGLE_memory_object_fuchsia"));
    RunShouldClearTest<FuchsiaTraits>(isSwiftshader());
}

template <typename Traits>
void RunTextureFormatCompatChromiumTest(bool isSwiftshader)
{
    ASSERT(EnsureGLExtensionEnabled(Traits::memoryObjectExtension()));
...
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_EXT_memory_object_fd"));
    RunTextureFormatCompatChromiumTest<OpaqueFdTraits>(isSwiftshader());
}
...
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_ANGLE_memory_object_fuchsia"));
    RunTextureFormatCompatChromiumTest<FuchsiaTraits>(isSwiftshader());
}

template <typename Traits>
void RunShouldClearWithSemaphoresTest(bool isSwiftshader)
{
    ASSERT(EnsureGLExtensionEnabled(Traits::memoryObjectExtension()));
    ASSERT(EnsureGLExtensionEnabled(Traits::semaphoreExtension()));
...

    {
        ...
        Traits::glImportSemaphore(glAcquireSemaphore, acquireSemaphoreHandle);
        ...
        Traits::glImportSemaphore(glReleaseSemaphore, releaseSemaphoreHandle);
...
TEST_P(VulkanExternalImageTest, ShouldClearOpaqueFdWithSemaphores)
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_EXT_memory_object_fd"));
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_EXT_semaphore_fd"));
    RunShouldClearWithSemaphoresTest<OpaqueFdTraits>(isSwiftshader());
}
...
TEST_P(VulkanExternalImageTest, ShouldClearZirconVmoWithSemaphores)
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_ANGLE_memory_object_fuchsia"));
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_ANGLE_semaphore_fuchsia"));
    RunShouldClearWithSemaphoresTest<FuchsiaTraits>(isSwiftshader());
}
