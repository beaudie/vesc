...
class DifferentStencilMasksWithoutStencilBufferTest : public DifferentStencilMasksTest
{
  protected:
    DifferentStencilMasksWithoutStencilBufferTest()
    {
        ...
        mUnsupported = (IsOSX() && (IsAMD() || IsIntel())) ||
                        (IsLinux() && (IsAMD() || IsIntel()));

        setConfigDepthBits(mUnsupported ? 24 : 0);
        setConfigStencilBits(mUnsupported ? 8 : 0);
    }
...
{
    ANGLE_SKIP_TEST_IF(mUnsupported);
