...
TEST_P(AtomicCounterBufferTest31, TessellationControlShaderMaxAtomicCounterTests)
{
    ...
    const uint tessellationControlPointsCount = 3;
    ...
    // gl_InvocationID = 1: increment all elements in atomic counters acs[].
    // gl_InvocationID = 2: increment elements in atomic counters acs[] whose index is multiples
    // of 2. We have tessellationControlPointsCount=3 vertices output from the Tessellation Control
    ...
    GLuint atomicBufferData[maxTessellationControlAtomicCounters];
    ...
    // Bind n_loop_iterations_uniform uniform
    GLint n_loop_iterations_uniform_location = -1;
    n_loop_iterations_uniform_location       = glGetUniformLocation(program, "n_loop_iterations");
    EXPECT_NE(-1, n_loop_iterations_uniform_location);
    glUniform1i(n_loop_iterations_uniform_location, maxTessellationControlAtomicCounters);
...
TEST_P(AtomicCounterBufferTest31, TessellationEvaluationShaderMaxAtomicCounterTests)
{
    ...
    GLuint atomicBufferID;
    glGenBuffers(1, &atomicBufferID);
    GLuint atomicBufferData[maxTessellationEvaluationAtomicCounters];
    ...
    n_loop_iterations_uniform_location       = glGetUniformLocation(program, "n_loop_iterations");
    EXPECT_NE(-1, n_loop_iterations_uniform_location);
    glUniform1i(n_loop_iterations_uniform_location, maxTessellationEvaluationAtomicCounters);
    ...
    for (GLint n_ac = 1; n_ac <= maxTessellationEvaluationAtomicCounters; ++n_ac)
    {
        EXPECT_EQ(atomicBufferResult[n_ac - 1], expected_value);
    }
