...
// DrawBaseVertexVariantsTest: Tests of GL_ANGLE_base_vertex_base_instance
...
using DrawBaseVertexVariantsTestParams = std::tuple<angle::PlatformParameters>;
// using DrawBaseVertexVariantsTestParams =
//     angle::PlatformParameters;

struct PrintToStringParamName
{
    std::string operator()(
        const ::testing::TestParamInfo<DrawBaseVertexVariantsTestParams> &info) const
    {
        ::std::stringstream ss;
...
class DrawBaseVertexVariantsTest : public ANGLETestBase,
                                   // public ::testing::TestWithParam<angle::PlatformParameters>
                                   public ::testing::TestWithParam<DrawBaseVertexVariantsTestParams>
{
  protected:
    // DrawBaseVertexVariantsTest() : ANGLETestBase(GetParam())
    DrawBaseVertexVariantsTest() : ANGLETestBase(std::get<0>(GetParam()))
    {
        ...
        mColorPalette = {{0x7f, 0x7f, 0x7f, 0xff}, {0xff, 0x00, 0x00, 0xff},
        ...
        for (uint32_t y = 0; y < kCountY; ++y)
        {
            for (uint32_t x = 0; x < kCountX; ++x)
            {
                ...
                for (int i = 0; i < 4; ++i)
                {
                    mVertexColors.insert(mVertexColors.end(), colorPicked.begin(),
                                         colorPicked.end());
                }
            }
        }
...
void main()
{
    ...
    {
        program.makeRaster(vertexShaderSource().c_str(), fragmentShaderSource().c_str());
        EXPECT_GL_NO_ERROR();
    ...
    void doDrawElementsInstancedBaseVertexBaseInstance()
    {
        ...
        {
            for (uint32_t i = 0; i < kCountX; i++)
            {
                glDrawElementsInstancedBaseVertexBaseInstanceANGLE(
                    GL_TRIANGLES, countPerDraw, GL_UNSIGNED_SHORT,
                    // reinterpret_cast<GLvoid *>(static_cast<uintptr_t>(0)), 1, (c) * 4, 0);
                    reinterpret_cast<GLvoid *>(static_cast<uintptr_t>(0)), 1, (i + v * kCountX) * 4,
                    0);
        ...
        // glDrawElementsInstancedBaseVertexBaseInstanceANGLE(
        //             GL_TRIANGLES, 9 * 6, GL_UNSIGNED_SHORT,
        //             reinterpret_cast<GLvoid *>(static_cast<uintptr_t>(0)), 1, 0, 0);
    ...
    std::vector<std::array<GLubyte, 4>> mColorPalette;
...
TEST_P(DrawBaseVertexVariantsTest, CanCompile)
{
