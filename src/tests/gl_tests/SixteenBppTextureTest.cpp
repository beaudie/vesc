...
class SixteenBppTextureTest : public ANGLETest
{
  protected:
    ...
    void simpleValidate565(bool useTexSubImage2D)
    {
        GLushort pixels[4] =
        {
            0xF800, // Red
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 2, 2, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, useTexSubImage2D ? nullptr : pixels);
...
TEST_P(SixteenBppTextureTest, RGB565ValidationSubImage2D)
{
    simpleValidate565(true);
}

// Simple validation test for GL_RGB565 textures.
// Samples from the texture, renders to it, generates mipmaps etc.
TEST_P(SixteenBppTextureTest, RGB565ValidationNonSubImage2D)
{
    simpleValidate565(false);
}
