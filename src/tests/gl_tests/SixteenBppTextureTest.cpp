...
TEST_P(SixteenBppTextureTestES3, RGB565FramebufferReadback)
{
    ...
    Vector3 rawColor(0.5f, 0.7f, 1.0f);
    GLColorRGB expectedColor(rawColor);
    ...
    GLTexture tex;
    glBindTexture(GL_TEXTURE_2D, tex.get());
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB565, 2, 2, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex.get(), 0);

    glClearColor(rawColor.x, rawColor.y, rawColor.z, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);
    ...
    ASSERT_GLENUM_EQ(GL_RGB, colorReadFormat);

    if (colorReadType == GL_UNSIGNED_BYTE)
    {
        GLColorRGB actualColor = GLColorRGB::black;
        glReadPixels(0, 0, 1, 1, colorReadFormat, colorReadType, &actualColor);
        EXPECT_COLOR_RGB_NEAR(expectedColor, actualColor, 20);
    }
    else
    {
        ASSERT_TRUE(colorReadType == GL_UNSIGNED_SHORT_5_6_5);

        uint16_t rgb = 0;
        glReadPixels(0, 0, 1, 1, colorReadFormat, colorReadType, &rgb);

        GLubyte r5 = static_cast<GLubyte>((rgb & 0xF800) >> 11);
        GLubyte g6 = static_cast<GLubyte>((rgb & 0x07E0) >> 5);
        GLubyte b5 = static_cast<GLubyte>(rgb & 0x001F);

        GLColorRGB actualColor(r5 << 3, g6 << 2, b5 << 3);
        EXPECT_COLOR_RGB_NEAR(expectedColor, actualColor, 20);
    }
