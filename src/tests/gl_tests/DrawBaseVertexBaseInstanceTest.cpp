...
void main()
{
    ...
    void SetupBuffers()
    {
        ...
        std::array<GLfloat, kCountX> instances;
        ...
        for (uint32_t y = 0; y < kCountY; ++y)
        {
            for (uint32_t x = 0; x < kCountX; ++x)
            {
                ...
                EXPECT_PIXEL_NEAR(
                    ...
                    255.0 * (1.0 - (float)x / (float)kCountX),
                    255.0 * (1.0 - (float)y / (float)kCountY),
                    hasBaseVertex ? 255.0 * (1.0 - (float)y / (float)kCountY) : 255, 255, 3);
            }
        }
    }
...
// Check that *Draw*Instanced[BaseVertex]BaseInstance without instancing support or results in
// GL_INVALID_OPERATION
TEST_P(DrawBaseVertexBaseInstanceTest, InvalidOperation)
{
    ANGLE_SKIP_TEST_IF(IsGLExtensionEnabled("GL_ANGLE_instanced_arrays"));
    requestDrawBaseVertexBaseInstanceExtension();
    SetupBuffers();
    SetupProgram(true);

    const uint32_t drawCount    = 1;
    const uint32_t countPerDraw = 6;
    ...
    const std::vector<GLvoid *> indices(drawCount, 0);
    std::vector<GLint> baseVertices(drawCount);
    std::vector<GLuint> baseInstances(drawCount, 0);

    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glBindBuffer(GL_ARRAY_BUFFER, mNonIndexedVertexBuffer);
    glEnableVertexAttribArray(mPositionLoc);
    glVertexAttribPointer(mPositionLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glDrawArraysInstancedBaseInstanceANGLE(GL_TRIANGLES, 0, 3, 1, 0);
    ...
    glVertexAttribPointer(mPositionLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glDrawElementsInstancedBaseVertexBaseInstanceANGLE(
        GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, reinterpret_cast<GLvoid *>(static_cast<uintptr_t>(0)),
        1, 0, 0);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    glMultiDrawElementsInstancedBaseVertexBaseInstanceANGLE(
        GL_TRIANGLES, GL_UNSIGNED_SHORT, drawCount, counts.data(), instanceCounts.data(),
