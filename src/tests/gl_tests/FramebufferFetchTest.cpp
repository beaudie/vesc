...
TEST_P(FramebufferFetchES31, SingleSampledMultiSampledMixedTest)
{
    ...
    glClearColor(0.0f, 1.0f, 0.0f, 1.0f);
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, multiSampledFramebuffer);
    glClear(GL_COLOR_BUFFER_BIT);
    ...
    // Bind first single sampled framebuffer, draw once with green, fetching red from the
    // framebuffer
    glUseProgram(fetchProgram);
    glUniform4fv(colorLocation, 1, GLColor::green.toNormalizedVector().data());
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, singleSampledFramebuffer1);
    render(positionLocation, false);
    glBindFramebuffer(GL_READ_FRAMEBUFFER, singleSampledFramebuffer1);
    EXPECT_PIXEL_COLOR_EQ(kViewportWidth / 2, kViewportHeight / 2, GLColor::yellow);
    ASSERT_GL_NO_ERROR();

    // Bind the multi sampled framebuffer, draw once with blue, fetching green from the framebuffer
    glUseProgram(fetchProgram);
    glUniform4fv(colorLocation, 1, GLColor::blue.toNormalizedVector().data());
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, multiSampledFramebuffer);
    render(positionLocation, false);
    ...
    glUseProgram(fetchProgram);
    glUniform4fv(colorLocation, 1, GLColor::red.toNormalizedVector().data());
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, singleSampledFramebuffer2);
    render(positionLocation, false);
    glBindFramebuffer(GL_READ_FRAMEBUFFER, singleSampledFramebuffer2);
    EXPECT_PIXEL_COLOR_EQ(kViewportWidth / 2, kViewportHeight / 2, GLColor::magenta);
    ASSERT_GL_NO_ERROR();
}
