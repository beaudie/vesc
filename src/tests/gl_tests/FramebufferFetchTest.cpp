...
// ARM version of a 3.1 GLSL fragment shader that writes to 4 attachments
static constexpr char k310ARM4AttachmentFS[] = R"(#version 310 es
#extension GL_ARM_shader_framebuffer_fetch : require
...
void main (void)
{
    o_color0 = u_color + gl_LastFragColorARM;
    o_color1 = u_color + gl_LastFragColorARM;
    o_color2 = u_color + gl_LastFragColorARM;
    o_color3 = u_color + gl_LastFragColorARM;
})";

// ARM version of a 3.1 GLSL fragment shader that writes to 4 attachments with the order of
// non-fetch program and fetch program with different attachments (version 1)
static constexpr char k310ARMDifferent4AttachmentFS1[] = R"(#version 310 es
#extension GL_ARM_shader_framebuffer_fetch : require
layout(location = 0) out highp vec4 o_color0;
layout(location = 1) out highp vec4 o_color1;
layout(location = 2) out highp vec4 o_color2;
...
// ARM version of a 3.1 GLSL fragment shader that writes to 4 attachments via an inout
// array
static constexpr char k310ARM4AttachmentArrayFS[] = R"(#version 310 es
#extension GL_ARM_shader_framebuffer_fetch : require
out highp vec4 o_color[4];
uniform highp vec4 u_color;

void main (void)
{
    o_color[0] = u_color + gl_LastFragColorARM;
    o_color[1] = u_color + gl_LastFragColorARM;
    o_color[2] = u_color + gl_LastFragColorARM;
    o_color[3] = u_color + gl_LastFragColorARM;
})";

// ARM version of a 3.1 GLSL fragment shader that writes to 4 attachments with the order
// of non-fetch program and fetch program with different attachments (version 2)
static constexpr char k310ARMDifferent4AttachmentFS2[] = R"(#version 310 es
#extension GL_ARM_shader_framebuffer_fetch : require
layout(location = 0) out highp vec4 o_color0;
layout(location = 1) out highp vec4 o_color1;
layout(location = 2) out highp vec4 o_color2;
        ...
        if (mARMExtension)
        {
            switch (whichFragmentShader)
            {
                ...
                case GLSL310_4ATTACHMENT:
                    return k310ARM4AttachmentFS;
                case GLSL310_4ATTACHMENT_ARRAY:
                    return k310ARM4AttachmentArrayFS;
                case GLSL310_4ATTACHMENT_DIFFERENT1:
                    return k310ARMDifferent4AttachmentFS1;
                case GLSL310_4ATTACHMENT_DIFFERENT2:
                    return k310ARMDifferent4AttachmentFS2;
                default:
                    UNREACHABLE();
                    return nullptr;
            }
...
// Testing ARM extension with multiple render target
TEST_P(FramebufferFetchES31, MultipleRenderTarget_ARM)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    setWhichExtension(ARM);
...
// Testing ARM extension with multiple render target using inout array
TEST_P(FramebufferFetchES31, MultipleRenderTargetWithInoutArray_ARM)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    setWhichExtension(ARM);
...
// Testing ARM extension with the order of non-fetch program and fetch program with
// different attachments
TEST_P(FramebufferFetchES31, DrawNonFetchDrawFetchWithDifferentAttachments_ARM)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    setWhichExtension(ARM);
...
// Testing ARM extension with the order of non-fetch program and fetch with different
// programs
TEST_P(FramebufferFetchES31, DrawNonFetchDrawFetchWithDifferentPrograms_ARM)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    setWhichExtension(ARM);
...
// Testing ARM extension with the order of draw fetch, blit and draw fetch
TEST_P(FramebufferFetchES31, DrawFetchBlitDrawFetch_ARM)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    setWhichExtension(ARM);
