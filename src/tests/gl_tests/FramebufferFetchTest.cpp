...
// Coherent version of a 1.0 GLSL fragment shader that writes to 4 attachments with variable indices
static constexpr char k100Coherent4AttachmentVariableFS[] = R"(#version 100
#extension GL_EXT_shader_framebuffer_fetch : require
#extension GL_EXT_draw_buffers : require
uniform highp vec4 u_color;
...
{
    for (int i = 0; i < 4; ++i)
        gl_FragData[i] = gl_LastFragData[i] + u_color;
})";
...
vec4 getLastFragData(int i)
{
    return gl_LastFragData[i];
}

void addLastFragData(inout vec4 outVar, int i)
{
    outVar += getLastFragData(i);
}
...
void main (void)
{
    ...
    {
        int index = i * 2 + 1;
        gl_FragData[index] = addColor(gl_LastFragData[index], u_color);
    }
    ...
    {
        int index = i * 2 + 1;
        gl_FragData[index] -= getLastFragData(index);
        // Test passing to inout variables.
        addLastFragData(gl_FragData[index], index);
    }
})";
...
// Non-coherent version of a 1.0 GLSL fragment shader that writes to 4 attachments with variable
// indices
static constexpr char k100NonCoherent4AttachmentVariableFS[] = R"(#version 100
#extension GL_EXT_shader_framebuffer_fetch_non_coherent : require
#extension GL_EXT_draw_buffers : require
layout(noncoherent) mediump vec4 gl_LastFragData[gl_MaxDrawBuffers];
uniform highp vec4 u_color;
...
static constexpr char k100Both4AttachmentVariableFS[] = R"(#version 100
#extension GL_EXT_shader_framebuffer_fetch : require
#extension GL_ARM_shader_framebuffer_fetch : require
#extension GL_EXT_draw_buffers : require
uniform highp vec4 u_color;
...
void main (void)
{
    ...
    for (i = 3; i > 0; --i)
        gl_FragData[i] = gl_LastFragData[i] + u_color;
    // i is now 0
    gl_FragData[i] = (gl_LastFragData[i] + gl_LastFragColorARM) / 2. + u_color;
})";
...
void main (void)
{
    ...
    {
        int index = i * 2;
        gl_FragData[index] = addColor(
            i == 0 ? (gl_LastFragData[index] + gl_LastFragColorARM) / 2.
                   : gl_LastFragData[index],
            u_color);
    }
    ...
    {
        int index = i * 2 + 1;
        gl_FragData[index] -= i == 0 ? gl_LastFragColorARM : getLastFragData(index);
        // Test passing to inout variables.
        addLastFragData(gl_FragData[index], index);
    }
})";
    ...
    const char *getFragmentShader(WhichFragmentShader whichFragmentShader)
    {
        if (mBothExtensions)
        {
            switch (whichFragmentShader)
            {
                ...
                case GLSL100_4ATTACHMENT_VARIABLE:
                    return k100Both4AttachmentVariableFS;
        ...
        else if (mCoherentExtension)
        {
            switch (whichFragmentShader)
            {
                ...
                case GLSL100_4ATTACHMENT_VARIABLE:
                    return k100Coherent4AttachmentVariableFS;
        ...
        else
        {
            switch (whichFragmentShader)
            {
                ...
                case GLSL100_4ATTACHMENT_VARIABLE:
                    return k100NonCoherent4AttachmentVariableFS;
                case GLSL310_4ATTACHMENT:
...
// Testing coherent extension with multiple render target, using gl_FragData with variable indices
TEST_P(FramebufferFetchES31, MultipleRenderTarget_Coherent_FragData_Variable)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_shader_framebuffer_fetch"));
    ...
    GLProgram program;
    program.makeRaster(k100VS, getFragmentShader(GLSL100_4ATTACHMENT_VARIABLE));
    glUseProgram(program);
    ASSERT_GL_NO_ERROR();

    MultipleRenderTargetTest(program, GLSL100_4ATTACHMENT_VARIABLE);
}
...
// Testing non-coherent extension with multiple render target, using gl_FragData with variable
// indices
TEST_P(FramebufferFetchES31, MultipleRenderTarget_NonCoherent_FragData_Variable)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_shader_framebuffer_fetch_non_coherent"));
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_draw_buffers"));
    setWhichExtension(NON_COHERENT);
...
// Testing using both extentions simultaneously with multiple render target, using gl_FragData with
// variable indices
TEST_P(FramebufferFetchES31, MultipleRenderTarget_Both_FragData_Variable)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_ARM_shader_framebuffer_fetch"));
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_shader_framebuffer_fetch"));
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_draw_buffers"));
