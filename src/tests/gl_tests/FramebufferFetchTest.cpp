...
void main (void)
{
    ...
    GLProgram programFetch;
    programFetch.makeRaster(kVS, kFetchFS);
    GLint positionLocation2 = glGetAttribLocation(programFetch, "a_position");
    GLint colorLocation2    = glGetUniformLocation(programFetch, "u_color");
    ASSERT_GL_NO_ERROR();

    // Create a framebuffer and render directly to it, no fetching
    ...
    // Draw to it with green, without using fetch, overwrite what's there
    glUseProgram(program);
    glUniform4fv(colorLocation2, 1, GLColor::green.toNormalizedVector().data());
    render(positionLocation2, false);
    EXPECT_PIXEL_COLOR_EQ(kViewportWidth / 2, kViewportHeight / 2, GLColor::green);
    ASSERT_GL_NO_ERROR();

    // Create a framebuffer that is not default and initialize it with blue
    ...
    // Now use the same program and render to the other framebuffer that was not used with fetch
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer1);
    render(positionLocation, false);
    EXPECT_PIXEL_COLOR_EQ(kViewportWidth / 2, kViewportHeight / 2, GLColor::yellow);
    ASSERT_GL_NO_ERROR();
