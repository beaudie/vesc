...
// initial draw will need to unresolve color.  Redefining the color attachment forces framebuffer to
// recreate when the clear is called.  The second copy resolves the clear and the final draw
// unresolves again.  In the Vulkan backend, mismatches in unresolve state between framebuffer and
// render pass will result in an ASSERT and a validation error (if ASSERT is removed).
TEST_P(MultisampledRenderToTextureES3Test, DrawCopyRedefineClearCopyThenDraw)
{
    ...
    // Add another mip to the texture, forcing its image to be recreated.
    glBindTexture(GL_TEXTURE_2D, colorMS);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, kSize, kSize, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
    ASSERT_GL_NO_ERROR();
    EXPECT_GL_FRAMEBUFFER_COMPLETE(GL_FRAMEBUFFER);
