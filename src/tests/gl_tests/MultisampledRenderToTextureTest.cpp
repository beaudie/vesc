...
class MultisampledRenderToTextureTest : public ANGLETest
{
  protected:
    ...
    void testTearDown() override
    {
        ...
        glDeleteProgram(mCopyTextureProgram);
    }

    void setupCopyTexProgram()
    {
        mCopyTextureProgram = CompileProgram(kVS, kFS);
        if (mCopyTextureProgram == 0)
        {
    ...
    GLuint mFBO                       = 0;
    GLuint mPBO                       = 0;
    GLuint mCopyTextureProgram        = 0;
    GLint mCopyTextureUniformLocation = 0;
};
...
TEST_P(MultisampledRenderToTextureTest, MultisampleReadPixelsTest)
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_EXT_multisampled_render_to_texture"));
...
TEST_P(MultisampledRenderToTextureTest, MultisampleCopyTexSubImageTest)
{
    ...
    GLuint copyFBO[2] = {};
    glGenFramebuffers(2, copyFBO);

    setupCopyTexProgram();

    // Create texture in copyFBO[0] with color (.25, 1, .75, .5)
    glBindTexture(GL_TEXTURE_2D, mTextures[0]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size, size, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, copyFBO[0]);
    glFramebufferTexture2DMultisampleEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                         mTextures[0], 0, 4);
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, copyFBO[1]);
    glFramebufferTexture2DMultisampleEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                         mTextures[1], 0, 4);
    ...
    // copyFBO[0] -> copyToTex
    // copyToTex should hold what was originally in copyFBO[0] : (.25, 1, .75, .5)
    glBindFramebuffer(GL_FRAMEBUFFER, copyFBO[0]);
    glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, size, size, 0);
    ASSERT_GL_NO_ERROR();
    ...
    glDeleteFramebuffers(2, copyFBO);
}

TEST_P(MultisampledRenderToTextureTest, MultisampleBlitFramebufferTest)
{
    ANGLE_SKIP_TEST_IF(!EnsureGLExtensionEnabled("GL_EXT_multisampled_render_to_texture"));
...
void main()
{
    ...
    GLuint m2DProgram = CompileProgram(kVS, kFS);
