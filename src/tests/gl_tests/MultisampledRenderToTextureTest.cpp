  ...
  protected:
    ...
    void testSetUp() override
    {
        if (getClientMajorVersion() >= 3 && getClientMinorVersion() >= 1)
        {
            ...
            ASSERT_GE(mMaxIntegerSamples, 1);
        }
    }
...
void MultisampledRenderToTextureES31Test::drawCopyThenBlendAllAttachmentsMixed(bool useRenderbuffer)
{
    ...
    const GLint kSampleCounts[kImplMaxDrawBuffers] = {
        mTestSampleCount,   mMaxIntegerSamples, mMaxIntegerSamples, mTestSampleCount,
        mMaxIntegerSamples, mMaxIntegerSamples, mMaxIntegerSamples, mTestSampleCount,
    };
        ...
        fsStr[index] << R"(void main()
{
    ...
    for (GLint drawBuffer = 0; drawBuffer < maxDrawBuffers; ++drawBuffer)
    {
        createAndAttachColorAttachment(useRenderbuffer, kSize, GL_COLOR_ATTACHMENT0 + drawBuffer,
                                       &kGLType[drawBuffer], kSampleCounts[drawBuffer],
                                       &textureMS[drawBuffer], &renderbufferMS[drawBuffer]);
    }
    EXPECT_GL_FRAMEBUFFER_COMPLETE(GL_FRAMEBUFFER);
