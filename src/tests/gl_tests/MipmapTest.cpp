...
TEST_P(MipmapTest, ExtSRGBInvalidOPForES2)
{
    ANGLE_SKIP_TEST_IF(getClientMajorVersion() < 3 && !extensionEnabled("GL_EXT_sRGB"));

    glBindTexture(GL_TEXTURE_2D, mTexture2D);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB_ALPHA_EXT, 2, 2, 0, GL_SRGB_ALPHA_EXT, +GL_UNSIGNED_BYTE,
                 nullptr);
    EXPECT_GL_NO_ERROR();
