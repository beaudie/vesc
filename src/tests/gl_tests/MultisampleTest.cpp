...
TEST_P(MultisampleTest, AlphaToSampleCoverage)
{
    constexpr char kFS[] =
        ...
        "}\n";
    GLuint transParentRedProgram = CompileProgram(essl1_shaders::vs::Simple(), kFS);
    glUseProgram(transParentRedProgram);
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);
    glClearDepthf(1.0f);
    GLfloat *clearColor = GLColor::green.toNormalizedVector().data();
    glClearColor(clearColor[0], clearColor[1], clearColor[2], clearColor[3]);  // clear to green
    ...
    GLuint blueProgram = CompileProgram(essl1_shaders::vs::Simple(), essl1_shaders::fs::Blue());
