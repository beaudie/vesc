...
// from 4815 to 4877.
// When rendering with Stencil buffer enabled and depth buffer disabled,
// and large viewport will lead to memory leak and driver crash. And the pixel
...
TEST_P(RenderStencilBufferTest, DrawWithLargeViewport)
{
    ...
    while (i < 10)
    {
        ...
        GLColor actualColor;
        glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &actualColor);

        EXPECT_EQ(255, actualColor.R);
        EXPECT_EQ(0, actualColor.G);
        EXPECT_EQ(0, actualColor.B);
        EXPECT_EQ(255, actualColor.A);
        angle::Sleep(500);
        i++;
        EXPECT_GL_NO_ERROR();
    }
