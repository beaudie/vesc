...
TEST_P(FenceSyncTest, Errors)
{
    ...
    EXPECT_EQ(static_cast<GLenum>(GL_WAIT_FAILED), glClientWaitSync(sync, GL_SYNC_FLUSH_COMMANDS_BIT | 0x2, 0));
    EXPECT_GL_ERROR(GL_INVALID_VALUE);

    // glClientWaitSync generates GL_INVALID_VALUE and returns GL_WAIT_FAILED if the sync object is not valid
    EXPECT_EQ(static_cast<GLenum>(GL_WAIT_FAILED), glClientWaitSync(reinterpret_cast<GLsync>(30), GL_SYNC_FLUSH_COMMANDS_BIT, 0));
    EXPECT_GL_ERROR(GL_INVALID_VALUE);
