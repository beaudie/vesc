...
{
    // This test only applies to ES2 and a non-D3D9 renderer.
    if (getClientMajorVersion() != 2 || getPlatformRenderer() == EGL_PLATFORM_ANGLE_TYPE_D3D9_ANGLE)
    {
        return;
    }
    ...
    // Test for error when we bind with a non-zero attachment point.
