...
TEST_P(WebGLCompatibilityTest, HalfFloatBlend)
{
    ...
    // D3D9 supports float rendering, but not float blending.
    // Float32 blending is gated behind EXT_float_blend, but float16 is technically required to
    // blend if it's renderable.
    // However, float16 rendering is too useful to disable on d3d9 for now.
