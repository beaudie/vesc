...
void main()
{
    ...
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following passes because attribute 1 only accesses index 4 regardless of count
    glDrawArrays(GL_POINTS, 4, 2);
    EXPECT_GL_NO_ERROR();
    glDrawArrays(GL_POINTS, 4, 3);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following fails because attribute 1 cannot access index 5
    glDrawArrays(GL_POINTS, 5, 1);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    ...
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following passes because attribute 1 and 3 only access index 4 regardless of count
    ...
    glDrawArrays(GL_POINTS, 5, 1);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following fails because attributes 1 and 3 cannot access index 6
    ...
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following passes because attribute 1 accesses vertices [2, 5), and attribute 3 accesses
    // vertices [2, 6)
    glDrawArraysInstanced(GL_POINTS, 2, 4, 11);
    EXPECT_GL_NO_ERROR();
    glDrawArraysInstanced(GL_POINTS, 2, 4, 12);
    EXPECT_GL_NO_ERROR();
    // The following fails because attribute 3 accesses vertices [2, 7).  Attribute 1 still accesses
    // within limits of [2, 5)
    ...
    EXPECT_GL_NO_ERROR();
    // The following passes because all attributes only access index 4
    ...
    //                     \        \-- overflow
    //                      \---------- overflow
    glDrawArraysInstanced(GL_POINTS, 3, 359, 10);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    // The following passes because attributes accessed are:
    // [4, 5), [4, 6), [4, 6), [4, 9)
    glDrawArraysInstanced(GL_POINTS, 4, 359, 5);
    EXPECT_GL_NO_ERROR();
    // The following fails because attributes accessed are:
    // [4, 6), [4, 6), [4, 6), [4, 10)
    //     \-- overflow
    glDrawArraysInstanced(GL_POINTS, 4, 359, 6);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
}

// Test a case where individually, each instanced attribute would allow for instances than the
// aggregate would have allowed.  Regression test for a bug where the instance count was checked
// against an aggregated limit.
TEST_P(WebGL2CompatibilityTest, InstancedAttributesIndividualLimits)
{
    constexpr char kVS[] = R"(#version 300 es
in vec4 attr1;
in vec2 attr2;
...
void main()
{
    ...
    // Set up the attributes such that:
    //
    // - Buffer 1 has 120 bytes with a stride of 12.  It can fit 10 vertices.
    // - Buffer 2 has 24 bytes with a stride of 2.  It can fit 12 vertices.
    constexpr GLsizei kBufferSizes[2] = {
        120,
        24,
    };
    constexpr GLsizei kBufferOffsets[2] = {
        556,
        1000,
    };
    constexpr GLint kAttrComponents[2] = {
        4,
        2,
    };
    constexpr GLenum kAttrTypes[2] = {
        GL_UNSIGNED_SHORT,
        GL_UNSIGNED_BYTE,
    };
    constexpr GLsizei kAttrStrides[4] = {
    ...
    // Set the divisors to 5 and 4 for the two attributes respectively.
    //
    // - Buffer 1 can fit 10 vertices.  With a divisor of 5, it can render 50 instances.
    // - Buffer 2 can fit 12 vertices.  With a divisor of 4, it can render 48 instances.
    glVertexAttribDivisor(attrLocations[0], 5);
    glVertexAttribDivisor(attrLocations[1], 4);

    glDrawArraysInstanced(GL_POINTS, 0, 123, 48);
    EXPECT_GL_NO_ERROR();
    glDrawArraysInstanced(GL_POINTS, 0, 123, 49);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);

    // Draw with offset 3.  In this case, buffer 1 supports (10-3)*5 = 35 instances and buffer 2
    // supports (12-3)*4 = 36 instances.  In combination, only 35 instances are allowed.
    glDrawArraysInstanced(GL_POINTS, 3, 123, 35);
    EXPECT_GL_NO_ERROR();
    glDrawArraysInstanced(GL_POINTS, 3, 123, 36);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
}
