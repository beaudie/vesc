...
TEST_P(WebGLCompatibilityTest, InvalidTextureFormat)
{
    ...
    {
        // Verify a format from enableable extension (GL_EXT_texture_storage) is allowed when
        // requested.
        glRequestExtensionANGLE("GL_EXT_texture_storage");
        EXPECT_TRUE(extensionEnabled("GL_EXT_texture_storage"));

        glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F_EXT, 1, 1, 0, GL_RG32F_EXT, GL_UNSIGNED_BYTE,
                     nullptr);
        ASSERT_GL_NO_ERROR();
    }
    ...
    {
        // Verify a format from enableable extension (GL_EXT_texture_sRGB) is allowed when
        // requested.
        glRequestExtensionANGLE("GL_EXT_texture_sRGB");
        EXPECT_TRUE(extensionEnabled("GL_EXT_texture_sRGB"));

        glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB_EXT, 1, 1, 0, GL_SRGB_EXT, GL_UNSIGNED_BYTE,
                     nullptr);
        ASSERT_GL_NO_ERROR();
    }
}
