...
TEST_P(WebGLCompatibilityTest, FramebufferAttachmentSizeMissmatch)
{
    ...
    glBindTexture(GL_TEXTURE_2D, textures[0].get());
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textures[0].get(),
                           0);
    ...
    ASSERT_GL_NO_ERROR();
    ASSERT_EQ(GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS, glCheckFramebufferStatus(GL_FRAMEBUFFER));
    ...
    if (extensionRequestable("GL_EXT_draw_buffers"))
    {
        ...
        glBindTexture(GL_TEXTURE_2D, textures[1].get());
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
