...
class WebGLCompatibilityTest : public ANGLETest
{
  protected:
                                ...
                                const float floatData[4])
    {
        ...
        ANGLE_GL_PROGRAM(renderingProgram, shader_library::essl1::vs::simple(),
                         shader_library::essl1::fs::uniformColor());
        glUseProgram(renderingProgram.get());

        glUniform4fv(glGetUniformLocation(renderingProgram.get(), "color"), 1, floatData);

        drawQuad(renderingProgram.get(), shader_library::positionAttribName(), 0.5f, 1.0f, true);
