...
TEST_P(TimerQueriesTest, TimeElapsedMulticontextTest)
{
    ...
    // Test skipped because the Vulkan backends cannot support simultaneous queries on multiple
    // contexts yet.
    ANGLE_SKIP_TEST_IF(GetParam() == ES2_VULKAN());
...
ANGLE_INSTANTIATE_TEST(TimerQueriesTestES3, ES3_D3D11(), ES3_OPENGL(), ES2_VULKAN());
