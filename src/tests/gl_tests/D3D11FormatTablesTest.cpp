...
TEST_P(D3D11FormatTablesTest, TestFormatSupport)
{
    ...
    for (GLenum internalFormat : allFormats)
    {
        ...
        UINT texSupport;
        ASSERT_TRUE(SUCCEEDED(device->CheckFormatSupport(formatInfo.texFormat, &texSupport)));
        bool textureable = (texSupport & texSupportMask) == texSupportMask;
        EXPECT_EQ(textureable, textureInfo.texturable);

        // Bits for filtering
        UINT filterSupport;
        ASSERT_TRUE(SUCCEEDED(device->CheckFormatSupport(formatInfo.srvFormat, &filterSupport)));
        bool filterable = (filterSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE) != 0;
        ...
        {
            UINT depthSupport;
            ASSERT_TRUE(SUCCEEDED(device->CheckFormatSupport(formatInfo.dsvFormat, &depthSupport)));
            renderable = (depthSupport & D3D11_FORMAT_SUPPORT_DEPTH_STENCIL) != 0;
        }
        else
        {
            UINT rtSupport;
            ASSERT_TRUE(SUCCEEDED(device->CheckFormatSupport(formatInfo.rtvFormat, &rtSupport)));
