...
class WebGLReadOutsideFramebufferTest : public ANGLETest
{
  ...
  protected:
    ...
    // Tag the framebuffer pixels differently than the initial read buffer pixels, so we know for sure which pixels are changed by reading.
    ...
    void SetUp() override
    {
        ...
        // TODO(fjhenigman): Factor out this shader and others like it in other tests, into ANGLETest.
    ...
    void Main(void (WebGLReadOutsideFramebufferTest::*testFunc)(int x, int y, PixelRect *dest), bool zeroOutside)
    {
        ...
        for (int i = -1; i < 4; ++i)
        {
            for (int j = -1; j < 4; ++j)
            {
                int x = i * fbWidth / 2 - readWidth / 2;
                int y = j * fbHeight / 2 - readHeight / 2;
...
// TODO(fjhenigman): Uncomment each test as part of a CL that lets the test pass.
