...
class PixelRect
{
  public:
    ...
    PixelRect(int width, int height, GLuint startColor)
        : mWidth(width), mHeight(height), mData(width * height)
    {
        for (GLsizei i = 0; i < mWidth * mHeight; ++i)
        {
            mData[i] = angle::GLColor(startColor++);
        }
    }

    void toTexture() const
    {
    ...
    void read(const PixelRect &other, int x, int y, bool zeroOutside)
    {
    ...
    void expectEQ(const PixelRect &expected) const
    {
...
class WebGLReadOutsideFramebufferTest : public ANGLETest
{
  protected:
    ...
    WebGLReadOutsideFramebufferTest() : mFBData(fbWidth, fbHeight, fbStartColor)
    {
    ...
    void SetUp() override
    {
        ...
        GLTexture fbTexture;
        glBindTexture(GL_TEXTURE_2D, fbTexture.get());
        mFBData.toTexture();
...
TEST_P(WebGLReadOutsideFramebufferTest, ReadPixels)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            // fill read buffers with unique pixels
            PixelRect actual(readWidth, readHeight, readStartColor);
            PixelRect expected(readWidth, readHeight, readStartColor);

            // real and simulated framebuffer read
            actual.read(x, y);
            expected.read(mFBData, x, y, false);
...
TEST_P(WebGLReadOutsideFramebufferTest, CopyTexSubImage2D)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            // fill texture with unique pixels
            GLTexture destTexture;
            glBindTexture(GL_TEXTURE_2D, destTexture.get());
            PixelRect destData(readWidth, readHeight, readStartColor);
            destData.toTexture();
            ASSERT_GL_NO_ERROR();

            // real framebuffer -> texture -> 'actual'
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
            glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, x, y, readWidth, readHeight);
            PixelRect actual(readWidth, readHeight);
            readTexture(readWidth, readHeight, actual);
            ASSERT_GL_NO_ERROR();

            // simulated framebuffer read into 'expected'
            PixelRect expected(readWidth, readHeight, readStartColor);
            expected.read(mFBData, x, y, false);
...
TEST_P(WebGLReadOutsideFramebufferTest, CopyTexImage2D)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            // simulated framebuffer read into 'expected'
            PixelRect expected(readWidth, readHeight, readStartColor);
            expected.read(mFBData, x, y, false);  // XXX true, but false lets it pass
