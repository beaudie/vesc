...
class PixelRect
{
  public:
    ...
    void setPixel(int x, int y, int z, int w)
    {
    ...
    void Test(void (WebGLReadOutsideFramebufferTest::*readFunc)(int x, int y, PixelRect *dest), int test)
    {
        ...
        for (int i = -1; i < 4; ++i)
        {
            for (int j = -1; j < 4; ++j)
            {
                ...
                {
                    mFBData.setPixel(a, b, 200 + (i + 1)*10 + (j + 1), test*10 + mPlatform);
                }
                GLTexture fbTexture;
                mFBData.toTexture2D(fbTexture.get());
                ...
                for (int b = 0; b < readHeight; ++b)
                {
                    ...
                    actual.setPixel(a, b, 100 + (i + 1)*10 + (j + 1), w*100 + test*10 + mPlatform);
                }
...
{
    Test(&WebGLReadOutsideFramebufferTest::UseReadPixels, 1);
}
...
{
    Test(&WebGLReadOutsideFramebufferTest::UseCopyTexImage2D, 3);  // XXX should be true but then the test fails - false lets it pass
}
