...
TEST_P(WebGLReadOutsideFramebufferTest, ReadPixels)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            actual.fill(readStartColor);
            PixelRect expected(readWidth, readHeight);
            expected.fill(readStartColor);

            // real and simulated framebuffer read
            actual.readFB(x, y);
            expected.readOther(mFBData, x, y, false);
...
TEST_P(WebGLReadOutsideFramebufferTest, CopyTexSubImage2D)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            PixelRect destData(readWidth, readHeight);
            destData.fill(readStartColor);
            GLTexture destTexture;
            destData.toTexture2D(destTexture.get());
            ASSERT_GL_NO_ERROR();
...
TEST_P(WebGLReadOutsideFramebufferTest, CopyTexImage2D)
{
    ...
    for (int i = -1; i < 4; ++i)
    {
        for (int j = -1; j < 4; ++j)
        {
            ...
            PixelRect expected(readWidth, readHeight);
            expected.fill(readStartColor);
            expected.readOther(mFBData, x, y, true);  //XXX should be true, but then test fails - false lets it pass
