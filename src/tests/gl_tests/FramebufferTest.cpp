...
TEST_P(FramebufferTest_ES3, CompleteWithNonZeroBaseLevel)
{
    ...
    if ((IsWindows() && IsOpenGL()) || (IsNVIDIA() && IsLinux()))
    {
        std::cout << "Test skipped on Windows OpenGL and on NVIDIA Linux." << std::endl;
        return;
    }
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mTexture, level);
