...
// The application creates a rendering feedback loop with sampler2DShadow.
TEST_P(FramebufferTest_ES3, ShadowSamplerFeedbackLoop)
{
    // When this empty draw is not done, the "Render pass closed due to framebuffer change" command
    // buffer label is not given, but the Validation warning persists.
    {
    ...
    // When the sampler type of this draw call is replaced with sampler2D, the issue does not occur.
    {
        constexpr char kFS[] = R"(#version 300 es
...
void main()
{
    ...
    // When the sampler type of this draw call is replaced with sampler2D, the issue does not occur.
    // When the texture is accessed in the fragment shader instead of the vertex shader, like above,
    // the issue does not
    {
