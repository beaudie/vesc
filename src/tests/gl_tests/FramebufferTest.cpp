...
{
    // This fails on Windows OpenGL, NVIDIA Linux, AMD Linux, and Mac OSX.
    if ((IsWindows() && IsOpenGL()) || (IsNVIDIA() || IsAMD() && IsLinux()) || IsOSX())
    {
        std::cout
            << "Test skipped due to driver bug on Windows OpenGL, NVIDIA/AMD Linux, and Mac OSX."
            << std::endl;
        return;
    }
