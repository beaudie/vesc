...
TEST_P(FramebufferTest_ES31, RenderingLimitToDefaultFBOSizeWithNoAttachments)
{
    ...
    // anglebug.com/7666 Occlusion query reports fragments outside the render area are still
    // rendered
    ANGLE_SKIP_TEST_IF(IsPixel4XL() || IsPixel4() || (IsWindows() && (IsIntel() || IsAMD())));
...
{
    // anglebug:7666 Occlusion query reports fragments outside the render area are still rendered
    ANGLE_SKIP_TEST_IF(IsPixel4() || IsPixel4XL() || (IsWindows() && (IsIntel() || IsAMD())));
    constexpr char kVS1[] = R"(#version 310 es
in layout(location = 0) highp vec2 a_position;
void main()
{
