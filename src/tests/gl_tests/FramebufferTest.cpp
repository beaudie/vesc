...
// D3D11 is unable to mix 3D color attachments with non 3D depth attachments. Test that a
// framebuffer completeness error is generated.
TEST_P(FramebufferTest_ES3, Mixed3DColorAnd2DDepthAttachments)
{
    ANGLE_GL_PROGRAM(program, essl1_shaders::vs::Simple(), essl1_shaders::fs::UniformColor());
    glUseProgram(program);
    GLint colorLocation = glGetUniformLocation(program, angle::essl1_shaders::ColorUniform());
