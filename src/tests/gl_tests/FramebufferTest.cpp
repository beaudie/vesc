...
// Tests that we can support a color textures also attached to the color attachment but
...
// (https://issuetracker.google.com/175584609) is doing exact this.
TEST_P(FramebufferTest_ES3, SampleFromAttachedTextureWithDifferentLOD)
{
    // TODO: anglebug.com/5760.
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0, kLevel2Size, kLevel2Size, GL_RGBA, GL_UNSIGNED_BYTE,
                    gData.data());
    ...
    // Render to FBO with color texture level 0 and use max_level to limit the sampling from only
    // level 0.
