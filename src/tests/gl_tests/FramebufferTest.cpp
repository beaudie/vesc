...
TEST_P(FramebufferTest_ES31, MultisampleResolveIntoMipMapWithBlit)
{
    ...
    constexpr int kMsaaSize    = 64;
    constexpr int kResolveSize = 128;
    glViewport(0, 0, kMsaaSize, kMsaaSize);
    ...
    GLTexture texture;
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, texture.get());
    glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA8, kMsaaSize, kMsaaSize, false);
    ASSERT_GL_NO_ERROR();
    ...
    GLTexture resolveTexture;
    glBindTexture(GL_TEXTURE_2D, resolveTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kResolveSize, kResolveSize, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, nullptr);
    ...
    glBindFramebuffer(GL_READ_FRAMEBUFFER, msaaFBO);
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, resolveFBO);
    glBlitFramebuffer(0, 0, kMsaaSize, kMsaaSize, 0, 0, kMsaaSize, kMsaaSize, GL_COLOR_BUFFER_BIT,
                      GL_NEAREST);
    ASSERT_GL_NO_ERROR();

    glBindFramebuffer(GL_READ_FRAMEBUFFER, resolveFBO);
    EXPECT_PIXEL_NEAR(0, 0, 0, 0, 0, 255, 1.0);                              // Black
    EXPECT_PIXEL_NEAR(kMsaaSize - 1, 1, 251, 0, 0, 255, 1.0);                // Red
    EXPECT_PIXEL_NEAR(0, kMsaaSize - 1, 0, 251, 0, 255, 1.0);                // Green
    EXPECT_PIXEL_NEAR(kMsaaSize - 1, kMsaaSize - 1, 251, 251, 0, 255, 1.0);  // Yellow
}
