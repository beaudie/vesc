...
// This extends the test SampleFromAttachedTextureWithDifferentLOD. The RenderPassUsage bits are
// cleared in the endRenderPass. This is to ensure that even though texture binding did not change,
// the ImageHelper::mRenderPassUseFlags will still set properly.
...
// Test render to a depth texture level that is excluded from [base_level, max_level]. This specific
// test renders to an immutable texture at the level that is less than GL_TEXTURE_BASE_LEVEL.
TEST_P(FramebufferTest_ES3, RenderSampleImmutableDepthTextureWithExcludedLevel)
{
    // ToDo: https://issuetracker.google.com/181800403
