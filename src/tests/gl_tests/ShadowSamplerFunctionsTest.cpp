...
TEST_P(ShadowSamplerFunctionTextureCubeArrayTest, Test)
{
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP_ARRAY_OES, tex);
    glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY_OES, 4, GL_DEPTH_COMPONENT32F, 8, 8, 12);
    for (size_t k = 0; k < 6 * 2; ++k)
    {
        glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY_OES, 0, 0, 0, k, 8, 8, 1, GL_DEPTH_COMPONENT,
                        GL_FLOAT, k > 5 ? level0.data() : unused.data());
        glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY_OES, 1, 0, 0, k, 4, 4, 1, GL_DEPTH_COMPONENT,
                        GL_FLOAT, k > 5 ? level1.data() : unused.data());
        glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY_OES, 2, 0, 0, k, 2, 2, 1, GL_DEPTH_COMPONENT,
                        GL_FLOAT, k > 5 ? level2.data() : unused.data());
        glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY_OES, 3, 0, 0, k, 1, 1, 1, GL_DEPTH_COMPONENT,
                        GL_FLOAT, k > 5 ? level3.data() : unused.data());
    }

    glTexParameteri(GL_TEXTURE_CUBE_MAP_ARRAY_OES, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_CUBE_MAP_ARRAY_OES, GL_TEXTURE_MIN_FILTER,
                    mipmapped ? GL_NEAREST_MIPMAP_NEAREST : GL_NEAREST);
    ASSERT_GL_NO_ERROR();
    ...
    // First sample the texture directly for easier debugging
    glTexParameteri(GL_TEXTURE_CUBE_MAP_ARRAY_OES, GL_TEXTURE_COMPARE_MODE, GL_NONE);
    ...
    // Try shadow samplers
    glTexParameteri(GL_TEXTURE_CUBE_MAP_ARRAY_OES, GL_TEXTURE_COMPARE_MODE,
    ...
    for (const float refValue : kRefValues)
    {
        ...
        for (const GLenum compareFunc : kCompareFuncs)
        {
            glTexParameteri(GL_TEXTURE_CUBE_MAP_ARRAY_OES, GL_TEXTURE_COMPARE_FUNC, compareFunc);
            ASSERT_GL_NO_ERROR();
