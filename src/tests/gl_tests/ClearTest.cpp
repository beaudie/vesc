...
{
    const int kPBufferSurfaceWidth  = 16;
    const int kPBufferSurfaceHeight = 16;
    const int kScissorWidth         = kPBufferSurfaceWidth - 2;
    const int kScissorHeight        = kPBufferSurfaceHeight - 2;
    EGLint surfaceAttribs[] = {EGL_HEIGHT, kPBufferSurfaceHeight, EGL_WIDTH, kPBufferSurfaceWidth,
                               EGL_NONE};
         ...
         ++iter)
    {
        ...
        // make the Pbuffer surface and the context current
        eglMakeCurrent(mDisplay, mPbufferSurface, mPbufferSurface, mContext);
        ASSERT_EGL_SUCCESS();

        // apply a clear with scissor
        glEnable(GL_SCISSOR_TEST);
        ...
        // verify that the middle of the scissored area has the clear color
        EXPECT_PIXEL_NEAR(kScissorWidth / 2, kScissorHeight / 2, 128, 64, 192, 255, 1.0);
    }
}
