...
// Covers a bug in the Vulkan back-end where inline clear does not mask properly
TEST_P(ClearTest, ClearPlusMaskDrawAndClear)
{
    // Initialize a program with a uniform.
    ANGLE_GL_PROGRAM(program, essl1_shaders::vs::Simple(), essl1_shaders::fs::UniformColor());
    glUseProgram(program);
    ...
    ASSERT_GL_NO_ERROR();
    // EXPECT_PIXEL_32F_EQ(0, 0, 1.0f, 1.0f, 1.0f, 1.0f);
