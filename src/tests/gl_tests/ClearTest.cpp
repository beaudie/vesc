...
TEST_P(ClearTextureEXTTest, Luma)
{
    ...
    EXPECT_PIXEL_RECT_EQ(
        0, 0, 16, 16, GLColor(luminanceClearValue, luminanceClearValue, luminanceClearValue, 255));
...
// Test clearing depth 32f textures with GL_EXT_clear_texture
...
// TODO: separateDepthStencilLayouts is needed for some platforms? (VVL)
// Test clearing stencil textures with GL_EXT_clear_texture
TEST_P(ClearTextureEXTTest, StencilTexture)
{
    // TODO: Fails on some devices. Even if glTexSubImage2D is used instead of glClearTexImage.
    ANGLE_SKIP_TEST_IF(getClientMajorVersion() < 3);
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_clear_texture"));
