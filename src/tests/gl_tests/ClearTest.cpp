                                                      ...
                                                      bool clearStencil)
{
    ...
    {
        bool depthMask = mask && false;
        // We use a small shader to verify depth.
        ANGLE_GL_PROGRAM(depthTestProgram, essl1_shaders::vs::Passthrough(),
                         essl1_shaders::fs::Blue());
        glEnable(GL_DEPTH_TEST);
        glDepthFunc(depthMask ? GL_GREATER : GL_EQUAL);
        ...
        drawQuad(depthTestProgram, essl1_shaders::PositionAttrib(), depthMask ? 1.0f : 0.0f);
        glDisable(GL_DEPTH_TEST);
        ASSERT_GL_NO_ERROR();
        ...
        expectedCornerColorRGB =
            mHasDepth && scissor && !depthMask ? expectedCornerColorRGB : GLColor::blue;
