...
TEST_P(ClearTestES3, RepeatedClear)
{
    ...
    auto Vec4ToColor = [](const Vector4 &vec)
    {
        RGBA8 color;
    ...
    //{
        for (int cellY = 0; cellY < numRowsCols; cellY++)
        {
            for (int cellX = 0; cellX < numRowsCols; cellX++)
            {
                ...
                // Method 2: Use vertex data to specify quad corners
                ...
                //auto l = [](int v, int s)
                //{
                //    return static_cast<GLfloat>(v) / static_cast<GLfloat>(s) * 2.0f - 1.0f;
                //};

                //GLfloat minXf = l(minX, getWindowWidth());
                //GLfloat maxXf = l(maxX, getWindowWidth());
                //GLfloat minYf = l(minY, getWindowHeight());
                //GLfloat maxYf = l(maxY, getWindowHeight());

                //const GLfloat quadDepth = 0.5f;

                //const GLfloat vertices[] =
                //{
                //    minXf, maxYf, quadDepth,
                //    minXf, minYf, quadDepth,
                //    maxXf, minYf, quadDepth,

                //    minXf, maxYf, quadDepth,
                //    maxXf, minYf, quadDepth,
                //    maxXf, maxYf, quadDepth,
                //};
    ...
    std::vector<RGBA8> pixelData(fboSize * fboSize * numRowsCols * numRowsCols);
    glReadPixels(0, 0, fboSize, fboSize, GL_RGBA, GL_UNSIGNED_BYTE, pixelData.data());
    ...
    for (int cellY = 0; cellY < numRowsCols; cellY++)
    {
        for (int cellX = 0; cellX < numRowsCols; cellX++)
        {
            int seed            = cellX + cellY * numRowsCols;
            const Vector4 color = RandomVec4(seed, fmtValueMin, fmtValueMax);
            RGBA8 pixelColor    = Vec4ToColor(color);

            int testN = cellX * cellSize + cellY * cellSize * numRowsCols * cellSize +
                        cellSize * numRowsCols + 1;
            EXPECT_NEAR(pixelColor.R, pixelData[testN].R, 1);
            EXPECT_NEAR(pixelColor.G, pixelData[testN].G, 1);
            EXPECT_NEAR(pixelColor.B, pixelData[testN].B, 1);
            EXPECT_NEAR(pixelColor.A, pixelData[testN].A, 1);
        }
    }
