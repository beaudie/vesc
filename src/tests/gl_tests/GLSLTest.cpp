...
TEST_P(GLSLTest_ES31, MixedRowAndColumnMajorMatrices)
{
    ...
    // read column-major despite explicit row-major qualifier.  http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsLinux() && IsIntel() && IsOpenGL());

    // Fails on windows AMD on GL: http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsWindows() && IsOpenGL() && IsAMD());

    // Fails to compile the shader on Android.  http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsAndroid() && IsOpenGL());

    // Fails on assertion in translation to D3D.  http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsD3D11());

    // Fails on SSBO validation on Android/Vulkan.  http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsAndroid() && IsVulkan());
...
TEST_P(GLSLTest_ES3, MixedRowAndColumnMajorMatrices_ArrayBufferDeclaration)
{
    ...
    // http://anglebug.com/TODO (should be same as uber test)
    ANGLE_SKIP_TEST_IF(IsLinux() && IsIntel() && IsOpenGL());

    // Fails on Mac on Intel and AMD: http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsOSX() && IsOpenGL() && (IsIntel() || IsAMD()));

    // Fails on windows AMD on GL: http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsWindows() && IsOpenGL() && IsAMD());

    // Fails on D3D due to mistranslation: http://anglebug.com/TODO (same as uber test)
    ANGLE_SKIP_TEST_IF(IsD3D11());
...
TEST_P(GLSLTest_ES3, MixedRowAndColumnMajorMatrices_ReadSideEffect)
{
    ...
    // Fails on Mac on Intel and AMD: http://anglebug.com/TODO (same as ArrayBufferDeclaration
    // probably)
    ANGLE_SKIP_TEST_IF(IsOSX() && IsOpenGL() && (IsIntel() || IsAMD()));
...
TEST_P(GLSLTest_ES3, MixedRowAndColumnMajorMatrices_ReadSideEffectOrder)
{
    ...
    // http://anglebug.com/TODO (same reason as uber test)
    ANGLE_SKIP_TEST_IF(IsLinux() && IsIntel() && IsOpenGL());
...
TEST_P(GLSLTest_ES3, MixedRowAndColumnMajorMatrices_ReadSideEffectShortCircuit)
{
    ...
    // Fails on Android: http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsAndroid() && IsOpenGLES());
    ...
    // Fails on Mac on Nvidia: http://anglebug.com/TODO
