...
// According to GLSL ES 3.00.6 section 4.3.10 "The term static use means that after preprocessing the shader includes at least one statement that accesses the input or output, even if that statement is never actually executed."
TEST_P(GLSLTest_ES3, StaticUseOfFragmentInputInUnreferencedVariableInitializer)
{
    ...
    // Should fail to link according to table in GLSL ES 3.00.6 section 4.3.10: static use of input in fragment shader and no reference to the corresponding output in vertex shader.
    EXPECT_EQ(0u, program);
}
