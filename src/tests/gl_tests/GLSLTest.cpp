...
TEST_P(GLSLTest, ScopedStructsOrderBug)
{
    ...
    // TODO(geofflang): Find out why this doesn't compile on Apple OpenGL drivers (BUG 1292)
...
#elif defined(_WIN32)
    // TODO(geofflang): Find out why this doesn't compile on Windows AMD OpenGL drivers (BUG 1291)
...
{
    // TODO(geofflang): Figure out why this fails on OpenGL AMD (BUG 1291)
    if (isAMD() && getPlatformRenderer() == EGL_PLATFORM_ANGLE_TYPE_OPENGL_ANGLE)
    {
        std::cout << "Test disabled on OpenGL." << std::endl;
        return;
