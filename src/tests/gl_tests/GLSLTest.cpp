...
TEST_P(GLSLTest_ES31, AtomicCounterArrayLength)
{
    ...
    //
    // The nvidia driver is optimizing the suscription out, and calling the atomic counter inactive.
...
layout(binding = 0) uniform atomic_uint ac1[3][4];
...
void main() {
    ...
    uint value = 0u;
    if (ac1[(value = testSideEffectValue)].length() != 4)
    {
        passed = false;
    }
    ...
    constexpr unsigned int kBufferData[3 * 4] = {};
