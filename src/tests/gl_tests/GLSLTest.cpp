    ...
    for (int i = 5; i >= 0; --i)
    {
        ANGLE_GL_PROGRAM(program,
                         ...
                         (std::stringstream() <<
                          ...
                          precision highp float;
                          layout(binding=)" << i
                                              << R"(, rgba8) readonly highp uniform image2D img;
                          out vec4 fragColor;
                          void main() {
        ...
        // Try to force an update of the image 0 binding.
        glBindImageTexture(i, 0, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32UI);
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

        glBindImageTexture(i, tex, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA8);
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
