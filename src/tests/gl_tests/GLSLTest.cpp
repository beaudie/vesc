...
TEST_P(GLSLTest_ES31, ArraysOfArraysImage)
{
    ...
    constexpr char kComputeShader[] = R"(#version 310 es
        layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
        layout(binding = 0, r32ui) uniform highp readonly uimage2D image[2][2];
        ...
        void main(void)
        {
            ...
            outbuf.image_value += imageLoad(image[0][0], ivec2(0, 0)).x;
            outbuf.image_value += imageLoad(image[0][1], ivec2(0, 0)).x;
            outbuf.image_value += imageLoad(image[1][0], ivec2(0, 0)).x;
            outbuf.image_value += imageLoad(image[1][1], ivec2(0, 0)).x;
        })";
    ...
    GLTexture images[2][2];
    for (int i = 0; i < 2; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            glBindTexture(GL_TEXTURE_2D, images[i][j]);
            glTexStorage2D(GL_TEXTURE_2D, 1, GL_R32UI, 1, 1);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1, 1, GL_RED_INTEGER, GL_UNSIGNED_INT,
                            &imageData);
            glBindImageTexture(i * 2 + j, images[i][j], 0, GL_FALSE, 0, GL_READ_ONLY, GL_R32UI);
            EXPECT_GL_NO_ERROR();
        }
    }
    ...
    EXPECT_EQ(outputInitData[0], imageData * 4);
    glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
}
