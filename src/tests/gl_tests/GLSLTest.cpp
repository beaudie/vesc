...
{
    // TODO(anglebug.com/5491): see base case.
    ANGLE_SKIP_TEST_IF(IsIOS() && IsOpenGLES());
    constexpr char kVS[] = R"(#version 300 es
in vec4 inputAttribute;
...
{
    // TODO(anglebug.com/8590): This does not work yet, to keep the initial
    // implementation reviewble.
    ANGLE_SKIP_TEST_IF(true);
...
{
    // TODO(anglebug.com/8590): This does not work yet, to keep the initial
    // implementation reviewble. Note: the below skip will likely be left unfixed
    // in 8590.
    ANGLE_SKIP_TEST_IF(true);

    // TODO(anglebug.com/8613): Does not work.
    ANGLE_SKIP_TEST_IF(IsOpenGLES() || IsOpenGLES());
