...
TEST_P(GLSLTest_ES31, LoadAfterClear)
{
    ...
    ANGLE_GL_PROGRAM(clearProgram,
                     ...
                     R"(#version 310 es
                     precision highp float;
                     layout(binding=1, rgba8) volatile writeonly highp uniform image2D img;
                     void main() {
                         ivec2 coord = ivec2(floor(gl_FragCoord.xy));
                         imageStore(img, coord, vec4(0, 1, 0, 1));
                     })");
    ...
    ANGLE_GL_PROGRAM(program,
                     ...
                     R"(#version 310 es
                     precision highp float;
                     layout(binding=0, rgba8) volatile readonly highp uniform image2D img;
                     out vec4 fragColor;
                     void main() {
                         ivec2 coord = ivec2(floor(gl_FragCoord.xy));
