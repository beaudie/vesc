...
TEST_P(GLSLTest_ES31, MixedRowAndColumnMajorMatrices)
{
    ...
    // read column-major despite explicit row-major qualifier.
    // http://anglebug.com/TODO
    ANGLE_SKIP_TEST_IF(IsLinux() && IsIntel() && IsOpenGL());

    // Fails to compile the shader on Android.
    // http://anglebug.com/TODO
    ...
    // No support in Vulkan yet.  http://anglebug.com/3443
...
void main()
{
    ...
    COPY(ssbo140cOut.m.m2c3r[1], ssbo430cIn.m.m2c3r[1], 2, 3);
    ...
    auto roundUpPow2 = [](uint32_t dim, uint32_t align) {
    ...
    auto initData = [&](float data[], bool isStd430, bool isTransposed) {
        ...
        {
            uint32_t cols   = kMatrixDims[m].first;
            uint32_t rows   = kMatrixDims[m].second;
            ...
            offset += roundUpPow2(arraySize * stride, 4);
        }
        return offset;
    };
    ...
    const uint32_t sizeStd140ColMajor = initData(dataStd140ColMajor, false, false);
    const uint32_t sizeStd140RowMajor = initData(dataStd140RowMajor, false, true);
    const uint32_t sizeStd430ColMajor = initData(dataStd430ColMajor, true, false);
    const uint32_t sizeStd430RowMajor = initData(dataStd430RowMajor, true, true);
