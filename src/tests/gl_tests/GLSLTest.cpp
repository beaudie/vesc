...
{
  protected:
    void TestRGBTextureBufferEmulationInt(GLuint format);
};
...
void GLSLTest_ES31::TestRGBTextureBufferEmulationInt(GLuint format)
{
...
// Tests RGB32 texture buffer emulation (GL_RGB32UI)
TEST_P(GLSLTest_ES31, RGBTextureBufferEmulationUint)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_texture_buffer"));
    // On D3D11 pixels remain transparent black.
    ANGLE_SKIP_TEST_IF(IsD3D11());

    TestRGBTextureBufferEmulationInt(GL_RGB32UI);
}

// Tests RGB32 texture buffer emulation (GL_RGB32I)
TEST_P(GLSLTest_ES31, RGBTextureBufferEmulationSint)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_texture_buffer"));
    // On D3D11 pixels remain transparent black.
    ANGLE_SKIP_TEST_IF(IsD3D11());

    TestRGBTextureBufferEmulationInt(GL_RGB32I);
}

// Tests RGB32 texture buffer emulation (GL_RGB32F)
TEST_P(GLSLTest_ES31, RGBTextureBufferEmulationFloat)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_texture_buffer"));
    // On D3D11 pixels remain transparent black.
    ANGLE_SKIP_TEST_IF(IsD3D11());
...
// Tests RGB32 texture buffer emulation with a SSBO write
TEST_P(GLSLTest_ES31, RGBTextureBufferEmulationSSBOWrite)
{
...
// Tests RGB32 texture buffer emulation with a SSBO write (without barrier)
TEST_P(GLSLTest_ES31, RGBTextureBufferEmulationSSBOWriteWithoutBarrier)
{
    GLProgram programSSBO;
    SetupSSBOProgram(programSSBO);
    ...
    glTexBufferEXT(GL_TEXTURE_BUFFER, GL_RGB32UI, buffer);
    drawQuad(programBufferDraw.get(), "inputAttribute", 0.5f);
    // No memory barrier so the results might be undefined, just check for errors.
    ASSERT_GL_NO_ERROR();
}
