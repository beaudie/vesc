...
// Validate behaviour of using defined in a macro with undef.
...
// Tests edge case behaviour with defined in a macro.
TEST_P(GLSLTest, DefinedInMacroEdgeCase)
{
    constexpr char kVS[] = R"(precision mediump float;
attribute highp vec4 position;
varying vec2 out0;
...
// Test using defined in a macro parameter name.
TEST_P(GLSLTest, DefinedAsParameterName)
{
    constexpr char kVS[] = R"(precision mediump float;
attribute highp vec4 position;
varying vec2 out0;
