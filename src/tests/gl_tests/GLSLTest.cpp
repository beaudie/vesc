...
TEST_P(GLSLTest_ES31, ImageLoadAfterClear)
{
    ...
    ANGLE_GL_PROGRAM(clearProgram,
                     ...
                     R"(#version 310 es
                     precision highp float;
                     layout(binding=0, rgba8) writeonly volatile coherent highp uniform image2D img;
                     void main() {
                         ivec2 imgcoord = ivec2(floor(gl_FragCoord.xy));
                         imageStore(img, imgcoord, vec4(0, 1, 0, 1));
                     })");
    ASSERT_TRUE(clearProgram.valid());
    ...
    ANGLE_GL_PROGRAM(program,
                     ...
                     R"(#version 310 es
                     precision highp float;
                     layout(binding=0, rgba8) readonly volatile coherent highp uniform image2D img;
                     out vec4 fragColor;
                     void main() {
                         ivec2 imgcoord = ivec2(floor(gl_FragCoord.xy));
    ...
    // Draw the texture plus red into the main framebuffer via imageLoad, and make sure the
    // texture was still green. (green + red == yellow.)
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    EXPECT_GL_FRAMEBUFFER_COMPLETE(GL_FRAMEBUFFER);
    glBindImageTexture(0, tex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8);

    // Issue a no-op shader that stores to unit 0 in order to force our binding to take effect.
    glUseProgram(clearProgram.get());
