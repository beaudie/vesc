...
{
    // Results of -float on NVIDIA Windows and NVIDIA Linux are wrong. But It cannot be worked
    // around by replacing -float with "0.0 - float" or "-1.0 * float". (http://crbug.com/672380)
    if ((IsWindows() || IsLinux()) && IsNVIDIA())
    {
        std::cout << "Test disabled on this OpenGL configuration." << std::endl;
        return;
    }
