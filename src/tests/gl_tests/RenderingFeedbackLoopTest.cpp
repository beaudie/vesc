...
void main()
{
    ...
    void setupTextures()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, internalColorFormat, getWindowWidth(), getWindowHeight(),
        ...
        if (getClientMajorVersion() >= 3 || IsGLExtensionEnabled("GL_OES_depth_texture"))
        {
            glGenTextures(1, &mUsefulDepthTexture);
            glBindTexture(GL_TEXTURE_2D, mUsefulDepthTexture);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            glTexImage2D(GL_TEXTURE_2D, 0, internalDepthFormat, getWindowWidth(), getWindowHeight(),
    ...
    void testTearDown() override
    {
        ...
        glDeleteTextures(1, &mUsefulDepthTexture);
...
TEST_P(RenderingFeedbackLoopTest, SamplerReferencedRealLoop)
{
    ...
    ANGLE_SKIP_TEST_IF(
        IsD3D11());  // The d3d11 icd produces a debug output message when it detects and breaks a
                     // rendering feedback loop. The d3d11 backend treats this as an error.
...
TEST_P(RenderingFeedbackLoopTest, SamplerReferencedWritesDisabledReadAfterWrite)
{
    ANGLE_SKIP_TEST_IF(getClientMajorVersion() < 3 &&
                       !IsGLExtensionEnabled("GL_OES_depth_texture"));
...
ANGLE_INSTANTIATE_TEST_ES2_AND_ES3(RenderingFeedbackLoopTest);
