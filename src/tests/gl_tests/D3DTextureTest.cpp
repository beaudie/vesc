...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void RunYUVWritePixelTest(DXGI_FORMAT format)
    {
        ...
        std::cout << "Saif --- test - internal format = " << internalFormat << "\n";
        ...
        std::cout << "Saif --- test - internal format = " << internalFormat << "\n";
        // make this uint16_t for p010
        std::vector<uint8_t> uvData((bufferSize * bufferSize) / 2);
        for (UINT i = 0; i < (bufferSize * bufferSize) / 2; i++)
        {
            if (i % 2 == 0)
            {
                uvData.push_back(kUFillValue);
            }
            else
            {
                uvData.push_back(kVFillValue);
            }
        }
        // pass gl usigned short for p010
        glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0, bufferSize / 2,
                        bufferSize / 2, internalFormat, GL_UNSIGNED_BYTE, uvData.data());
        ASSERT_GL_NO_ERROR();
