...
TEST_P(D3DTextureTest, DepthStencil)
{
    ...
    EGLSurface pbuffer = createPBuffer(bufferSize, bufferSize, EGL_NO_TEXTURE, EGL_NO_TEXTURE, 1, 0);
    ASSERT_EGL_SUCCESS();
    ASSERT_NE(pbuffer, EGL_NO_SURFACE);
...
// Test creating a pbuffer from a multisampled d3d surface and binding a texture.
TEST_P(D3DTextureTestMS, BindTexImage)
{
    ...
    // Apply the window surface
    eglMakeCurrent(display, window->getSurface(), window->getSurface(),
                   window->getContext());
