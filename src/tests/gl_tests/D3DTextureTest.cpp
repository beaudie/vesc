...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void RunYUVWritePixelTest(DXGI_FORMAT format)
    {
        ...
        std::vector<unsigned> yData(bufferSize * bufferSize, kYFillValue);
        ...
        std::vector<unsigned> uvData(bufferSize * bufferSize);
        for (UINT i = 0; i < bufferSize * bufferSize; i++)
        {
            if (i % 2 == 0)
            {
            ...
            {
                uvData.push_back(kYFillValue);
            }
        }
        // pass gl usigned short for p010
        glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0, bufferSize,
                        bufferSize, internalFormat, GL_UNSIGNED_BYTE, uvData.data());
        ASSERT_GL_NO_ERROR();
