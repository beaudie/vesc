...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void RunYUVWritePixelTest(DXGI_FORMAT format)
    {
        ...
        uint8_t *yPlane  = reinterpret_cast<uint8_t *>(mapped.pData);
        uint8_t *uvPlane = yPlane + bufferSize * mapped.RowPitch;
        int pixelSize    = isNV12 ? 1 : 2;
        if (isNV12)
        {
            // Compare first y pixel with full write values.
            EXPECT_EQ(yPlane[0], kYFillValueFull);
            // Compare last y pixel with overwritten subregion values.
            // Index = pitch * y + x * numPlanes * pixelSize
            EXPECT_EQ(yPlane[mapped.RowPitch * (bufferSize - 1) + (bufferSize - 1) * 1 * pixelSize],
                      kYFillValueOffset);
            // Compare first uv pixel with full write values.
            EXPECT_EQ(uvPlane[0], kUFillValueFull);
            EXPECT_EQ(uvPlane[1], kVFillValueFull);
            // Compare last uv pixel with overwritten subregion values.
            // Index = pitch * y + x * numPlanes * pixelSize
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize],
                      kUFillValueOffset);
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize + 1],
                      kVFillValueOffset);
        }
        else
        {
            // Compare first y pixel with full write values.
            EXPECT_EQ(yPlane[0], kYFillValueFull & 0xff);
            EXPECT_EQ(yPlane[1], (kYFillValueFull >> 8) & 0xff);
            // Compare last y pixel with overwritten subregion values.
            // Index = pitch * y + x * numPlanes * pixelSize
            EXPECT_EQ(yPlane[mapped.RowPitch * (bufferSize - 1) + (bufferSize - 1) * 1 * pixelSize],
                      kYFillValueOffset & 0xff);
            EXPECT_EQ(
                yPlane[mapped.RowPitch * (bufferSize - 1) + (bufferSize - 1) * 1 * pixelSize + 1],
                (kYFillValueOffset >> 8) & 0xff);
            // Compare first uv pixel with full write values.
            EXPECT_EQ(uvPlane[0], kUFillValueFull & 0xff);
            EXPECT_EQ(uvPlane[1], (kUFillValueFull >> 8) & 0xff);
            EXPECT_EQ(uvPlane[2], kVFillValueFull & 0xff);
            EXPECT_EQ(uvPlane[3], (kVFillValueFull >> 8) & 0xff);
            // Compare last uv pixel with overwritten subregion values.
            // Index = pitch * y + x * numPlanes * pixelSize
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize],
                      kUFillValueOffset & 0xff);
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize + 1],
                      (kUFillValueOffset >> 8) & 0xff);
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize + 2],
                      kVFillValueOffset & 0xff);
            EXPECT_EQ(uvPlane[mapped.RowPitch * (bufferSize / 2 - 1) +
                              (bufferSize / 2 - 1) * 2 * pixelSize + 3],
                      (kVFillValueOffset >> 8) & 0xff);
        }
