...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void testSetUp() override
    {
        ...
        mTextureExternalOESProgram = CompileProgram(kVS, kTextureExternalOESFS);
        ASSERT_NE(0u, mTextureExternalOESProgram) << "shader compilation failed.";

        mTextureExternalOESUniformLocation =
            glGetUniformLocation(mTextureExternalOESProgram, "tex");
        ASSERT_NE(-1, mTextureExternalOESUniformLocation);
    }

    void testTearDown() override
    {
        glDeleteProgram(mTextureExternalOESProgram);
        D3DTextureTest::testTearDown();
    }
    ...
    void RunYUVTest(DXGI_FORMAT format)
    {
        ...
        EXPECT_TRUE(mD3D11Device != nullptr);
        ID3D11Texture2D *d3d11_texture = nullptr;
        CD3D11_TEXTURE2D_DESC desc(DXGI_FORMAT_NV12, bufferSize, bufferSize, 1, 1,
                                   D3D11_BIND_SHADER_RESOURCE);
        ...
        glUseProgram(mTextureExternalOESProgram);
        glUniform1i(mTextureExternalOESUniformLocation, 0);
        ASSERT_GL_NO_ERROR();

        glViewport(0, 0, bufferSize, bufferSize);
        drawQuad(mTextureExternalOESProgram, "position", 1.0f);
        ASSERT_GL_NO_ERROR();
    ...
    GLuint mTextureExternalOESProgram;
    GLint mTextureExternalOESUniformLocation;
};
