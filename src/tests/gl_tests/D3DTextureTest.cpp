...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void RunYUVSamplerTest(DXGI_FORMAT format)
    {
        ...
        GLenum internal_format = format == DXGI_FORMAT_NV12 ? GL_RED_EXT : GL_R16_EXT;
        CreateAndBindImageToTexture(display, d3d11Texture.Get(), 0, internal_format,
        ...
        internal_format = format == DXGI_FORMAT_NV12 ? GL_RG_EXT : GL_RG16_EXT;
        CreateAndBindImageToTexture(display, d3d11Texture.Get(), 1, internal_format,
    ...
    void RunYUVRenderTest(DXGI_FORMAT format)
    {
        ...
        GLenum internal_format = format == DXGI_FORMAT_NV12 ? GL_RED_EXT : GL_R16_EXT;
        CreateAndBindImageToTexture(display, d3d11Texture.Get(), 0, internal_format, GL_TEXTURE_2D,
                                    &yImage, &yTexture);
        ...
        internal_format = format == DXGI_FORMAT_NV12 ? GL_RG_EXT : GL_RG16_EXT;
        CreateAndBindImageToTexture(display, d3d11Texture.Get(), 1, internal_format, GL_TEXTURE_2D,
                                    &uvImage, &uvTexture);
    ...
    void RunYUVReadPixelTest(DXGI_FORMAT format)
    {
        ...
        GLubyte y_pixels[4] = {};
        glReadPixels(0, bufferSize / 2, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, y_pixels);
        if (isNV12)
        {
            EXPECT_EQ(y_pixels[0], kYFillValue);
        }
        else
        {
            EXPECT_EQ(y_pixels[0], kYFillValue & 0xff);
            EXPECT_EQ(y_pixels[1], (kYFillValue >> 8) & 0xff);
        }
        ...
        GLubyte uv_pixels[4] = {};
        glReadPixels(0, bufferSize / 4, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, uv_pixels);
        if (isNV12)
        {
            EXPECT_EQ(uv_pixels[0], kUFillValue);
            EXPECT_EQ(uv_pixels[1], kVFillValue);
        }
        else
        {
            EXPECT_EQ(uv_pixels[0], kUFillValue & 0xff);
            EXPECT_EQ(uv_pixels[1], (kUFillValue >> 8) & 0xff);
            EXPECT_EQ(uv_pixels[2], kVFillValue & 0xff);
            EXPECT_EQ(uv_pixels[3], (kVFillValue >> 8) & 0xff);
        }
