...
class D3DTextureYUVTest : public D3DTextureTest
{
  protected:
    ...
    void RunYUVWritePixelTest(DXGI_FORMAT format)
    {
        ...
        // Write the Y plane data.
        if (isNV12)
        {
            std::vector<uint8_t> yData(bufferSize * bufferSize, kYFillValue);
            glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0, bufferSize,
                            bufferSize, GL_RED_EXT, GL_UNSIGNED_BYTE, yData.data());
        }
        else
        {
            std::vector<uint16_t> yData(bufferSize * bufferSize, kYFillValue);
            glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0, bufferSize,
                            bufferSize, GL_RED_EXT, GL_UNSIGNED_SHORT, yData.data());
        }
        ASSERT_GL_NO_ERROR();
        ...
        if (isNV12)
        {
            ...
            GLubyte yPixels[4 * 4 * 4] = {};
            glReadPixels(0, 0, 4, 4, GL_RGBA, GL_UNSIGNED_BYTE, yPixels);
            EXPECT_EQ(yPixels[0], kYFillValue);
            EXPECT_EQ(yPixels[4], kYFillValue);
            EXPECT_EQ(yPixels[16], kYFillValue);
        }
        else
        {
            // yPixels of size (4*4) region (*4) bytes per RGBA pixel
            GLushort yPixels[4 * 4 * 4] = {};
            glReadPixels(0, 0, 4, 4, GL_RGBA, GL_UNSIGNED_SHORT, yPixels);
            EXPECT_EQ(yPixels[0], kYFillValue);
            EXPECT_EQ(yPixels[4], kYFillValue);
            EXPECT_EQ(yPixels[16], kYFillValue);
        }
        ...
        if (isNV12)
        {
            // yPixels of size (2*2) region (*4) bytes per RGBA pixel
            GLubyte yPixels[2 * 2 * 4] = {};
            glReadPixels(bufferSize / 2, bufferSize / 2, 2, 2, GL_RGBA, GL_UNSIGNED_BYTE, yPixels);
            EXPECT_EQ(yPixels[0], kYFillValue);
            EXPECT_EQ(yPixels[12], kYFillValue);
        }
        else
        {
            // yPixels of size (2*2) region (*4) bytes per RGBA pixel
            GLushort yPixels[2 * 2 * 4] = {};
            glReadPixels(bufferSize / 2, bufferSize / 2, 2, 2, GL_RGBA, GL_UNSIGNED_SHORT, yPixels);
            EXPECT_EQ(yPixels[0], kYFillValue);
            EXPECT_EQ(yPixels[12], kYFillValue);
        }
        ...
        // Write the UV plane data.
        if (isNV12)
        {
            std::vector<uint8_t> uvData((bufferSize * bufferSize) / 2);
            for (UINT i = 0; i < (bufferSize * bufferSize) / 2; i++)
            {
                uvData[i] = i % 2 == 0 ? kUFillValue : kVFillValue;
            }
            // pass gl usigned short for p010
            glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0,
                            bufferSize / 2, bufferSize / 2, GL_RG_EXT, GL_UNSIGNED_BYTE,
                            uvData.data());
        }
        else
        {
            std::vector<uint16_t> uvData((bufferSize * bufferSize) / 2);
            for (UINT i = 0; i < (bufferSize * bufferSize) / 2; i++)
            {
                uvData[i] = i % 2 == 0 ? kUFillValue : kVFillValue;
            }
            // pass gl usigned short for p010
            glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0,
                            bufferSize / 2, bufferSize / 2, GL_RG_EXT, GL_UNSIGNED_SHORT,
                            uvData.data());
        }
        ASSERT_GL_NO_ERROR();
        ...
        if (isNV12)
        {
            ...
            GLubyte uvPixels[4 * 4 * 4] = {};
            glReadPixels(0, 0, 4, 4, GL_RGBA, GL_UNSIGNED_BYTE, uvPixels);
            EXPECT_EQ(uvPixels[0], kUFillValue);
            EXPECT_EQ(uvPixels[1], kVFillValue);
            EXPECT_EQ(uvPixels[4], kUFillValue);
            EXPECT_EQ(uvPixels[5], kVFillValue);
            EXPECT_EQ(uvPixels[16], kUFillValue);
            EXPECT_EQ(uvPixels[17], kVFillValue);
        }
        else
        {
            // uvPixels of size (4*4) region (*4) bytes per RGBA pixel
            GLushort uvPixels[4 * 4 * 4] = {};
            glReadPixels(0, 0, 4, 4, GL_RGBA, GL_UNSIGNED_SHORT, uvPixels);
            EXPECT_EQ(uvPixels[0], kUFillValue);
            EXPECT_EQ(uvPixels[1], kVFillValue);
            EXPECT_EQ(uvPixels[4], kUFillValue);
            EXPECT_EQ(uvPixels[5], kVFillValue);
        ...
        if (isNV12)
        {
            // uvPixels of size (2*2) region (*4) bytes per RGBA pixel
            GLubyte uvPixels[2 * 2 * 4] = {};
            glReadPixels(bufferSize / 4, bufferSize / 4, 2, 2, GL_RGBA, GL_UNSIGNED_BYTE, uvPixels);
            EXPECT_EQ(uvPixels[0], kUFillValue);
            EXPECT_EQ(uvPixels[1], kVFillValue);
            EXPECT_EQ(uvPixels[12], kUFillValue);
            EXPECT_EQ(uvPixels[13], kVFillValue);
        }
        else
        {
            // uvPixels of size (2*2) region (*4) bytes per RGBA pixel
            GLushort uvPixels[2 * 2 * 4] = {};
            glReadPixels(bufferSize / 4, bufferSize / 4, 2, 2, GL_RGBA, GL_UNSIGNED_SHORT,
                         uvPixels);
            EXPECT_EQ(uvPixels[0], kUFillValue);
            EXPECT_EQ(uvPixels[1], kVFillValue);
            EXPECT_EQ(uvPixels[12], kUFillValue);
...
{
    RunYUVWritePixelTest(DXGI_FORMAT_NV12);
}
...
{
    RunYUVWritePixelTest(DXGI_FORMAT_P010);
}
...
{
    RunYUVWritePixelTest(DXGI_FORMAT_P016);
}
