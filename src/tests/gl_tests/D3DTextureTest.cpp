...
TEST_P(D3DTextureTest, TestD3D11TypelessTexture)
{
    ...
    ANGLE_SKIP_TEST_IF(
        !valid() ||
        !eglDisplayExtensionEnabled(display, "EGL_ANGLE_d3d_typeless_texture_client_buffer") ||
        !mD3D11Device || !srgbSupported);
...
// Test that trying to create a pbuffer from a typeless texture fails if the extension is not
// supported.
