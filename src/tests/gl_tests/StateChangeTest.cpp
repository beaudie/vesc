...
TEST_P(SimpleStateChangeTestES3, MultipleSamplersWithSingleTextureObject)
{
    ...
    // have FS that samples from two samplers
...
void main()
{
    gl_FragColor = texture2D(samp1, v_texCoord) + texture2D(samp2, v_texCoord);
})";

    // Draw. This first step is a sanitycheck and not really necessary for the test
