...
TEST_P(SimpleStateChangeTest, ChangeTextureFilterModeBetweenTwoDraws)
{
    ...
    // The second half (right) should be a gradient so we shouldn't find plain black/white in the middle.
    EXPECT_NE(angle::ReadColor((getWindowWidth() / 2) + 5, 0), GLColor::black);
    EXPECT_NE(angle::ReadColor((getWindowWidth() / 2) + 6, 0), GLColor::black);
    EXPECT_NE(angle::ReadColor(getWindowWidth() - 5, 0), GLColor::white);
    EXPECT_NE(angle::ReadColor(getWindowWidth() - 6, 0), GLColor::white);
}
