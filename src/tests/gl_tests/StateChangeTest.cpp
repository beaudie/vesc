...
class SimpleStateChangeTest : public ANGLETest
{
  protected:
    ...
    void drawWithCulling(const GLenum face, const bool doClear);
    void drawToFboWithCulling(const GLenum face, const bool doClear);
};
...
void SimpleStateChangeTest::drawWithCulling(const GLenum face, const bool doClear)
{
    if (doClear)
    {
    ...
    if (face == GL_FRONT)
    {
        ...
        {
            // In this case, we expect the quad rendered nothing since the front faces
            // should have been culled. However, since we also didn't clear it to a
            // known value first, we are just checking the quad does *not* have exactly
            // green in it. It's possible the memory for the texture buffer does have
            // that value left over in it, but hopefully that's unlikely enough to not
            // hit (m)any flakes. Unfortunately, we can't clear first since we need to
            // test that path through the state machine.
            EXPECT_NE(GLColor::green, angle::ReadColor(0, 0));
        }
    }
    else
    {
...
void SimpleStateChangeTest::drawToFboWithCulling(const GLenum face, const bool doClear)
{
    // Render to an FBO
    GLFramebuffer fbo;
    GLTexture texture;
    ...
    drawWithCulling(face, doClear);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

// Validates if culling rasterization states work with FBOs. Simply draws a quad with
...
{
    drawToFboWithCulling(GL_FRONT, false);
}
...
{
    drawToFboWithCulling(GL_BACK, false);
}

// Validates if culling rasterization states work with FBOs. Clears the texture and then draws
...
{
    drawToFboWithCulling(GL_FRONT, true);
}
...
{
    drawToFboWithCulling(GL_BACK, true);
}

// Validates if culling rasterization states work. Simply draws a quad with
// cull face enabled with cullface front and make sure the face have not been rendered.
TEST_P(SimpleStateChangeTest, CullFaceFrontEnabledCCWState)
{
    drawWithCulling(GL_FRONT, false);
}

// Validates if culling rasterization states work. Simply draws a quad with
// cull face enabled with cullface back and make sure the face has been rendered.
TEST_P(SimpleStateChangeTest, CullFaceBackEnabledCCWState)
{
    drawWithCulling(GL_BACK, false);
}

// Validates if culling rasterization states work. Clears the texture and then draws
// a quad with cull face enabled with cullface front and make sure the face have not
// been rendered.
TEST_P(SimpleStateChangeTest, ClearCullFaceFrontEnabledCCWState)
{
    drawWithCulling(GL_FRONT, true);
}

// Validates if culling rasterization states work. Clears the texture and then draws
// a quad with cull face enabled with cullface back and make sure the face has
// been rendered.
TEST_P(SimpleStateChangeTest, ClearCullBackEnabledCCWState)
{
    drawWithCulling(GL_BACK, true);
}
}  // anonymous namespace
