...
TEST_P(SimpleStateChangeTest, DrawElementsUBYTEThenDrawElementsUSHORT)
{
    ANGLE_GL_PROGRAM(program, essl1_shaders::vs::Simple(), essl1_shaders::fs::Blue());
    ...
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_BYTE, (void *)(3 * sizeof(GLubyte)));

    // Bind the 16bit element buffer.
    auto indices16 = std::vector<GLushort>{0, 1, 2, 1, 2, 3};
