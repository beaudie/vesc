...
class LineLoopStateChangeTest : public StateChangeTest
{
  protected:
    ...
    void validateSquareAndHourglass()
    {
        ...
        // OpenGL on AMD does not compute the rasterization location exactly the same way.
        int rasterizationCorrection = IsOpenGL() && IsAMD() ? 0 : -1;

        // Bottom left
        EXPECT_PIXEL_COLOR_EQ(quarterWidth + rasterizationCorrection, quarterHeight,
                              GLColor::green);

        // Top left
        EXPECT_PIXEL_COLOR_EQ(quarterWidth + rasterizationCorrection,
                              (quarterHeight * 3) + rasterizationCorrection, GLColor::green);

        // Top right
        EXPECT_PIXEL_COLOR_EQ((quarterWidth * 3) + rasterizationCorrection, (quarterHeight * 3) - 1,
                              GLColor::green);

        // bottom right
        EXPECT_PIXEL_COLOR_EQ((quarterWidth * 3) + rasterizationCorrection, quarterHeight,
                              GLColor::green);
        ...
        EXPECT_PIXEL_COLOR_EQ((quarterWidth * 2) + rasterizationCorrection,
                              quarterHeight + rasterizationCorrection, GLColor::green);
    }
...
TEST_P(LineLoopStateChangeTest, DrawElementsThenDrawArrays)
{
    ...
    auto pixelPoints = std::vector<Vector2>{{8, 8}, {8, 24}, {24, 8}, {24, 24}};
