...
// VulkanRasterizeDiscardTest.cpp: Validate the optimization for using a dummy image when
...
class VulkanRasterizeDiscardTest : public ANGLETest
{
  protected:
...
// not.
TEST_P(VulkanRasterizeDiscardTest, SimpleTest)
{
    // Make mNeedToAcquireNextSwapchainImage "true"
    swapBuffers();
    ASSERT_EQ(true, hackWindowSurface()->getDeferredStatusForSwapchainImage());
...
TEST_P(VulkanRasterizeDiscardTest, UsingFBOBetweenDraw)
{
...
// dummy framebuffer)
TEST_P(VulkanRasterizeDiscardTest, DisablingRasterizerDiscardAfterBindingFBO)
{
    initializeRenderToRenderbuffer();
...
// Check that
TEST_P(VulkanRasterizeDiscardTest, SwapBuffersWithNoDrawCall)
{
    glEnable(GL_RASTERIZER_DISCARD);
    EXPECT_GL_NO_ERROR();
...
ANGLE_INSTANTIATE_TEST(VulkanRasterizeDiscardTest, ES3_VULKAN());
