...
// Test that generating mipmaps after incompatibly chnage base level.
...
TEST_P(Texture2DBaseMaxTestES3, GenerateMipmapAfterRebaseImmutable)
{
    ...
    // http://anglebug.com/4711
    ANGLE_SKIP_TEST_IF(IsOpenGL() && IsNVIDIA() && IsWindows());
...
void Texture2DBaseMaxTestES3::testGenerateMipmapAfterRebase(bool immutable)
{
    ...
    // with kNewMipColor and leave levle 0 and 3 unchanged.
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 1);
