...
#define EXPECT_PIXEL16_EQ(x, y, format, r, g, b, a) \
    EXPECT_EQ((std::vector<GLushort>{r, g, b, a}), readPixel16(x, y, format))
...
class Texture2DNorm16TestES3 : public Texture2DTestES3
{
  protected:
    ...
    static std::vector<GLushort> readPixel16(GLint x, GLint y, GLenum format)
    {
        std::vector<GLushort> pixel;
        pixel.resize(4, 0);
        glReadPixels(x, y, 1, 1, format, GL_UNSIGNED_SHORT, pixel.data());
        EXPECT_GL_NO_ERROR();
        return pixel;
    }
    ...
    void testNorm16Texture(GLint internalformat, GLenum format, GLenum type)
    {
        ...
        switch (format)
        {
          case GL_RED:    EXPECT_PIXEL16_EQ(0, 0, GL_RGBA, rvalue, 0,      0,      0xFFFF); break;
          case GL_RG:     EXPECT_PIXEL16_EQ(0, 0, GL_RGBA, rvalue, rvalue, 0,      0xFFFF); break;
          case GL_RGB:    EXPECT_PIXEL16_EQ(0, 0, GL_RGBA, rvalue, rvalue, rvalue, 0xFFFF); break;
          case GL_RGBA:   EXPECT_PIXEL16_EQ(0, 0, GL_RGBA, rvalue, rvalue, rvalue, rvalue); break;
        }
    ...
    void testNorm16Render(GLint internalformat, GLenum format, GLenum type)
    {
        ...
        switch (format)
        {
          case GL_RED:    EXPECT_PIXEL16_EQ(0, 0, format, pixelValue, 0,          0,          0); break;
          case GL_RG:     EXPECT_PIXEL16_EQ(0, 0, format, pixelValue, pixelValue, 0,          0); break;
          case GL_RGBA:   EXPECT_PIXEL16_EQ(0, 0, format, pixelValue, pixelValue, pixelValue, pixelValue); break;
        }
        ...
        switch (format)
        {
          case GL_RED:    EXPECT_PIXEL16_EQ(0, 0, format, 0xFFFF, 0,      0,      0); break;
          case GL_RG:     EXPECT_PIXEL16_EQ(0, 0, format, 0xFFFF, 0xFFFF, 0,      0); break;
          case GL_RGBA:   EXPECT_PIXEL16_EQ(0, 0, format, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF); break;
        }
...
ANGLE_INSTANTIATE_TEST(Texture2DNorm16TestES3, ES3_D3D11());
