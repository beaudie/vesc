...
// Create an integer format texture but specify a FLOAT sampler. This is
// valid OpenGL but causes a Vulkan validation error.
TEST_P(Texture2DTestES3, TexImageFormatMismatch)
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0);
    glTexImage2D(GL_TEXTURE_2D, 0, 33332, 8, 8, 0, GL_RED_INTEGER, GL_UNSIGNED_SHORT,
                 &genericBufferMemory);
    ...
    glSamplerParameteri(sampler, GL_TEXTURE_MIN_FILTER, 9728);
    glSamplerParameteri(sampler, GL_TEXTURE_MAG_FILTER, 9728);
