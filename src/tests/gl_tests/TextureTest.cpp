...
// Test that calling glBufferData on a buffer that is used as texture buffer.
TEST_P(TextureBufferTestES31, TextureBufferThenBufferData)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_OES_texture_buffer"));
    // angleproject:7283
    ANGLE_SKIP_TEST_IF(IsVulkan());
    // Somehow ARM native driver claims support GL_OES_texture_buffer but can't compile the shader
    // wiuth GL_OES_texture_buffer string in it.
    ANGLE_SKIP_TEST_IF(IsOpenGLES() && IsARM());
