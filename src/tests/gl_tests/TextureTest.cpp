...
// Test that when a mutable texture becomes immutable, there is no more attempt to flush it using
// the previous texture pointer in the Vulkan backend.
TEST_P(Texture2DTestES3, MutablePrevTextureBecomesImmutable)
{
    GLTexture texture1;
    glBindTexture(GL_TEXTURE_2D, texture1);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE,
