...
{
  public:
    // Test sampling stencil from a depth/stencil texture.
    void TestSampleStencilFromDepthStencil(GLenum format)
    {
    ...
    void main()
    {
        ...
        EXPECT_PIXEL_RECT_EQ(0, 0, kSize, kSize, GLColor(42, 0, 0, 1));
    }
...
{
    TestSampleStencilFromDepthStencil(GL_DEPTH24_STENCIL8);
}
...
{
    TestSampleStencilFromDepthStencil(GL_DEPTH32F_STENCIL8);
}

// Test that a depth & stencil texture bound as both sampler2D and usampler2D works.
TEST_P(Texture2DTestES31, TexSampleStencilFromDepthStencil)
{
    constexpr GLsizei kSize = 4;
