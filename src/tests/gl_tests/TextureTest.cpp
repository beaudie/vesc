...
// The test is added to cover anglebug.com/2153. Cubemap completeness checks used to start always at
// level 0 instead of the base level resulting in an incomplete texture if the faces at level 0 are
// not created.
// The test creates a cubemap texture, specifying the images only for mip level 1 filled with white
// color, updates the base level to be 1 and renders a quad. The program samples the cubemap using a
