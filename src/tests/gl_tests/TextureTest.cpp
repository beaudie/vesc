...
// Test render to image level 1, then full write to image level 0.
TEST_P(Texture2DTestES3, RenderToTextureThenFullUpdate)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 16, 16, GL_RGB, GL_UNSIGNED_BYTE, texDataBlue.data());
    drawQuad(program, essl1_shaders::PositionAttrib(), 0.0f);
    EXPECT_GL_NO_ERROR();
...
// Test copy to image level 1, then full write to image level 0.
TEST_P(Texture2DTestES3, CopyImageSubDataThenFullUpdate)
{
    ANGLE_SKIP_TEST_IF(!IsGLExtensionEnabled("GL_EXT_copy_image"));
