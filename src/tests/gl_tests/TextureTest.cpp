...
{
    // Init unpack buffer.
    std::vector<GLColor> pixels(getWindowWidth() * getWindowHeight() / 2, GLColor::red);

    GLBuffer unpackBuffer;
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, unpackBuffer.get());
    GLsizei size = getWindowWidth() * getWindowHeight() * sizeof(GLColor) / 2;
    glBufferData(GL_PIXEL_UNPACK_BUFFER, size, pixels.data(), GL_STATIC_DRAW);
