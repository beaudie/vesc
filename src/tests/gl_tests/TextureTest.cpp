...
// This test sets up a cube map with three distincly colored MIP levels.
// The size of the texture and the geometry is chosen such that level 1 should be selected at the
// edges of the screen. This test verifies that the color from level 1 is output in each corner.
TEST_P(TextureCubeIntegerEdgeTestES3, IntegerCubeTextureEdgeMidpoint)
{
    glActiveTexture(GL_TEXTURE0);
    ...
    GLColor color[3] = {GLColor::red, GLColor::green, GLColor::blue};
...
// This test verifies that the correct LOD is selected around the center 4x4 block of pixels.
TEST_P(TextureCubeIntegerEdgeTestES3, IntegerCubeTextureInfiniteLOD)
{
    glActiveTexture(GL_TEXTURE0);
    ...
    ASSERT_EQ(height, 1 << 10);
    for (GLint level = 0; level <= 10; level++)
    {
        for (GLenum faceIndex = 0; faceIndex < 6; faceIndex++)
        {
            ...
            GLColor color =
                level == 8 ? GLColor::blue : level == 10 ? GLColor::green : GLColor::red;
            std::vector<GLColor> pixels(levelWidth * levelHeight, color);
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIndex, level, GL_RGBA8UI, levelWidth,
                         levelHeight, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, pixels.data());
            EXPECT_GL_NO_ERROR();
        }
    ...
    EXPECT_GL_NO_ERROR();
    EXPECT_PIXEL_COLOR_EQ(width / 2 - 1, height / 2 - 1, GLColor::green);
    EXPECT_PIXEL_COLOR_EQ(width / 2, height / 2 - 1, GLColor::green);
    EXPECT_PIXEL_COLOR_EQ(width / 2 - 1, height / 2, GLColor::green);
    EXPECT_PIXEL_COLOR_EQ(width / 2, height / 2, GLColor::green);
}
