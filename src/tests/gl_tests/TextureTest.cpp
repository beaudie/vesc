...
// Tests invalid dimension input for TexImage3D with cube map arrays.
TEST_P(TextureCubeTestES32, ValidateCubeMapArrayTexImageInvalidDims)
{
    GLTexture cubeMapArrayTexture;
    glBindTexture(GL_TEXTURE_CUBE_MAP_ARRAY, cubeMapArrayTexture);
...
// Tests invalid dimension input for TexStorage3D with cube map arrays.
TEST_P(TextureCubeTestES32, ValidateCubeMapArrayTexStorageInvalidDims)
{
    GLTexture cubeMapArrayTexture;
    glBindTexture(GL_TEXTURE_CUBE_MAP_ARRAY, cubeMapArrayTexture);
    ...
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE, &maxTextureSize);
    EXPECT_GT(maxTextureSize, 0);

    glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 1, GL_RGBA8, maxTextureSize + 1, maxTextureSize + 1,
                   6);
    EXPECT_GL_ERROR(GL_INVALID_VALUE);

    // Level count must not exceed log2 of max texture size.
    GLint maxLevelCount = 1 + static_cast<GLint>(std::log2(maxTextureSize));
    glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY, maxLevelCount + 1, GL_RGBA8, maxTextureSize,
                   maxTextureSize, 6);
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
}
