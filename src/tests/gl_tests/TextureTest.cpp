...
class Texture2DTestES3 : public Texture2DTest
{
  protected:
    ...
    void testCopyImage(const APIExtensionVersion usedExtension)
    {
        ...
        {
            glCopyImageSubDataOES(srcTexture, GL_TEXTURE_2D, 0, 2, 2, 0, destTexture, GL_TEXTURE_2D,
                                  0, 2, 2, 0, 2, 2, 1);
        }
        else
        {
            glCopyImageSubDataEXT(srcTexture, GL_TEXTURE_2D, 0, 2, 2, 0, destTexture, GL_TEXTURE_2D,
                                  0, 2, 2, 0, 2, 2, 1);
        }
        glBindTexture(GL_TEXTURE_2D, destTexture);
        EXPECT_GL_NO_ERROR();
    ...
    void testCopyImageDepthStencil(const APIExtensionVersion usedExtension)
    {
        ...
        if (usedExtension == APIExtensionVersion::OES)
        {
            glCopyImageSubDataOES(src, GL_TEXTURE_2D, 0, 0, 0, 0, dst, GL_TEXTURE_2D, 0, 0, 0, 0,
                                  kSize, kSize, 1);
        }
        else
        {
            glCopyImageSubDataEXT(src, GL_TEXTURE_2D, 0, 0, 0, 0, dst, GL_TEXTURE_2D, 0, 0, 0, 0,
                                  kSize, kSize, 1);
        }
        ASSERT_GL_NO_ERROR();
