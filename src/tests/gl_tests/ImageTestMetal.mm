...
TEST_P(ImageTestMetal, SharedEventSyncWhenThereIsTextureUploadBetweenDraws)
{
    ...
    {
        // Create MTLTexture
        constexpr int kSharedTextureSize = 128;
        ScopedMetalTextureRef textureMtl =
            createMtlTexture2D(kSharedTextureSize, kSharedTextureSize, MTLPixelFormatR32Float);
        ...
        // Copy from texture1 to dstBuffer
        id<MTLBlitCommandEncoder> blitEncoder = [commandBuffer blitCommandEncoder];
...
void main()
{
    ...
    outColor = vec4(value);
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, kNumValues, 1, 0, GL_RED, GL_FLOAT, nullptr);
        glFlush();
        EXPECT_GL_ERROR(GL_NO_ERROR);
        ...
        glBindTexture(GL_TEXTURE_2D, texture2);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kNumValues, 1, GL_RED, GL_FLOAT, values.data());
        ...
        // Read dstBuffer
        const float kExpectedSum = kNumValues * (kNumValues - 1) * 0.5f;
        float *mappedFloats      = static_cast<float *>(dstBuffer.get().contents);
        EXPECT_FLOAT_EQ(mappedFloats[0], kExpectedSum);
