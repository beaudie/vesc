...
TEST_P(BufferSubDataTest, WTF)
{
    ...
    for (unsigned i = 0; i < 2; ++i)
    {
        ...
        EXPECT_PIXEL_EQ(w * 2, h * 3, 255, 255, 0, 0) << " LOOP " << i;
        EXPECT_PIXEL_EQ(w * 3, h * 2, 0, 255, 0, 0) << " LOOP " << i;
        EXPECT_PIXEL_EQ(w * 2, h * 1, 0, 255, 255, 0) << " LOOP " << i;
        EXPECT_PIXEL_EQ(w * 1, h * 2, 255, 0, 255, 0) << " LOOP " << i;
    }
}

ANGLE_INSTANTIATE_TEST(BufferSubDataTest,
                       ES2_VULKAN(),
                       ES2_OPENGL(),
                       ES2_D3D9(),
                       ES2_D3D11(),
