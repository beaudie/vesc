...
TEST_P(GetImageTestES1, PalettedTexImage)
{
    ...
    std::vector<Vector2U> testSampleLocations{
        Vector2U(11, 7),  Vector2U(13, 14), Vector2U(7, 14), Vector2U(11, 6),  Vector2U(7, 13),
        Vector2U(11, 15), Vector2U(16, 15), Vector2U(8, 15), Vector2U(14, 11), Vector2U(0, 11),
    };
    ...
    for (PalettedFormat format : kPalettedFormats)
    {
        ...
        // Take a bunch of samples at random locations

        glDrawArrays(GL_TRIANGLE_STRIP, 0, vertices.size());
        EXPECT_GL_NO_ERROR();

        std::vector<GLColor> samples(testSampleLocations.size());
        for (size_t i = 0; i < testSampleLocations.size(); i++)
            glReadPixels(testSampleLocations[i].x(), testSampleLocations[i].y(), 1, 1, GL_RGBA,
        ...
        // Take the samples again

        std::vector<GLColor> samples2(testSampleLocations.size());
        for (size_t i = 0; i < testSampleLocations.size(); i++)
            glReadPixels(testSampleLocations[i].x(), testSampleLocations[i].y(), 1, 1, GL_RGBA,
                         GL_UNSIGNED_BYTE, &samples2[i]);

        EXPECT_EQ(samples, samples2);
    }
}
