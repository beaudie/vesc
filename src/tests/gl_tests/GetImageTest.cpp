...
TEST_P(GetImageTest, NegativeAPI)
{
    ...
    // Tests GetCompressed on a uncompressed texture.
...
TEST_P(GetImageTest, CompressedTexImageAll)
{
    ...
    auto fun = [](const CompressionExtension &ext, const CompressedFormat &format) {
        for (GLsizei multiplier : {1, 2})
        {
    ...
    TestAllCompressedFormats(fun);
}

// Test a resolution that is not a multiple of the block size with an ASTC 8x6 format.
...
{
    auto fun = [target, numLayers](const CompressionExtension &ext,
