...
class CopyTextureTest : public ANGLETest
{
  protected:
    ...
    void calculateCopyTextureResults(GLenum sourceFormat, GLenum destFormat, bool premultiplyAlpha, bool unmultiplyAlpha, uint8_t *sourceColor, GLColor *destColor)
    {
        GLColor color;
        ...
        if (premultiplyAlpha != unmultiplyAlpha)
        {
            ...
            {
                fprintf(stderr, "unmultiply %u, %u, %u, %u -> ",
    ...
    uint8_t *getSourceColors(GLenum sourceFormat, size_t *colorCount, uint8_t *componentCount)
    {
        // Note: in all the following values, alpha is larger than RGB so unmultiply alpha doesn't overflow
        static uint8_t rgbaColors[7 * 4] = {255u, 127u, 63u, 255u,
            31u, 127u, 63u, 127u,
            31u, 63u, 127u, 255u,
            15u, 127u, 31u, 127u,
            127u, 255u, 63u, 0u,
        ...
        static uint8_t rgbColors[7 * 3] = {255u, 127u, 63u,
            31u, 127u, 63u,
            31u, 63u, 127u,
            15u, 127u, 31u,
            127u, 255u, 63u,
        ...
        static uint8_t lumColors[7 * 1] = {255u,
            163u,
            78u,
            114u,
            51u,
        ...
        static uint8_t lumaColors[7 * 2] = {255u, 255u,
            67u, 163u,
            78u, 231u,
            8u, 114u,
            51u, 199u,
        ...
        static uint8_t alphaColors[7 * 1] = {255u,
            67u,
            231u,
            8u,
            199u,
    ...
    void initializeSourceTexture(GLenum sourceFormat, uint8_t *srcColors, uint8_t componentCount)
    {
        // The texture is initialized as 2x2.  If the componentCount is 1 or 3, then the input data will have a row pitch of
        // 2 or 6, which needs to be padded to 4 or 8 respectively.
        uint8_t srcColorsPadded[4 * 4];
        size_t srcRowPitch = 2 * componentCount + (componentCount == 1 || componentCount == 3 ? 2 : 0);
        size_t inputRowPitch = 2 * componentCount;
        for (size_t row = 0; row < 2; ++row)
        {
            memcpy(&srcColorsPadded[row * srcRowPitch], &srcColors[row * inputRowPitch], inputRowPitch);
            memset(&srcColorsPadded[row * srcRowPitch + inputRowPitch], 0, srcRowPitch - inputRowPitch);
        }

        glBindTexture(GL_TEXTURE_2D, mTextures[0]);
        glTexImage2D(GL_TEXTURE_2D, 0, sourceFormat, 2, 2, 0, sourceFormat, GL_UNSIGNED_BYTE, srcColorsPadded);
    }

    void testCopyTexture(GLenum sourceFormat, GLenum destFormat, bool flipY, bool premultiplyAlpha, bool unmultiplyAlpha)
    {
        if (!checkExtensions())
        {
            return;
        ...
        uint8_t *srcColors = getSourceColors(sourceFormat, &colorCount, &componentCount);

        std::vector<GLColor> destColors(colorCount);
        for (size_t i = 0; i < colorCount; ++i)
        {
            calculateCopyTextureResults(sourceFormat, destFormat, premultiplyAlpha, unmultiplyAlpha, &srcColors[i * componentCount],
                    &destColors[i]);
        }
    ...
    void testCopySubTexture(GLenum sourceFormat, GLenum destFormat, bool flipY, bool premultiplyAlpha, bool unmultiplyAlpha)
    {
        if (!checkExtensions())
        {
            return;
        ...
        for (size_t i = 0; i < colorCount - 3; ++i)
        {
            ...
            glBindTexture(GL_TEXTURE_2D, mTextures[1]);
            glTexImage2D(GL_TEXTURE_2D, 0, destFormat, 2, 2, 0, destFormat, GL_UNSIGNED_BYTE, nullptr);

            glCopySubTextureCHROMIUM(mTextures[0], 0, GL_TEXTURE_2D, mTextures[1], 0, 0, 0, 0, 0, 2, 2,
                    flipY, premultiplyAlpha, unmultiplyAlpha);

            EXPECT_GL_NO_ERROR();

            if (sourceFormat != GL_LUMINANCE && sourceFormat != GL_LUMINANCE_ALPHA && sourceFormat != GL_ALPHA)
            {
                // Check that FB is complete.
                EXPECT_GLENUM_EQ(GL_FRAMEBUFFER_COMPLETE, glCheckFramebufferStatus(GL_FRAMEBUFFER));
            }
...
#if 0
TEST_P(CopyTextureTest, EveryCombination)
{
    if (!checkExtensions())
    {
        return;
    ...
    for (GLint sourceFormat : sourceFormats)
    {
        for (GLint destFormat : destFormats)
        {
            for (bool flipY : boolValues)
            {
                for (bool premultiplyAlpha : boolValues)
                {
                    for (bool unmultiplyAlpha : boolValues)
                    {
                        fprintf(stderr, "copy(%x, %x, %d, %d, %d)\n", sourceFormat, destFormat, flipY, premultiplyAlpha, unmultiplyAlpha);
                        testCopyTexture(sourceFormat, destFormat, flipY, premultiplyAlpha, unmultiplyAlpha);
                    }
                }
            }
        }
        ...
        for (GLint destFormat : destFormats)
        {
            for (bool flipY : boolValues)
            {
                for (bool premultiplyAlpha : boolValues)
                {
                    for (bool unmultiplyAlpha : boolValues)
                    {
                        fprintf(stderr, "subCopy(%x, %x, %d, %d, %d)\n", sourceFormat, destFormat, flipY, premultiplyAlpha, unmultiplyAlpha);
                        testCopySubTexture(sourceFormat, destFormat, flipY, premultiplyAlpha, unmultiplyAlpha);
                    }
                }
            }
        }
...
TEST_P(CopyTextureTest, CubeMapTarget)
{
    ...
    GLColor pixels[7] = {
        GLColor(10u, 13u, 16u, 19u),
        GLColor(20u, 23u, 26u, 29u),
        GLColor(30u, 33u, 36u, 39u),
        GLColor(40u, 43u, 46u, 49u),
        GLColor(50u, 53u, 56u, 59u),
        GLColor(60u, 63u, 66u, 69u),
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        {
            glBindTexture(GL_TEXTURE_2D, textures[0]);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &pixels[face - GL_TEXTURE_CUBE_MAP_POSITIVE_X + i]);

            glCopySubTextureCHROMIUM(textures[0], 0, face, textures[1], 0, 0, 0, 0, 0, 1, 1, false, false, false);
        }
...
TEST_P(CopyTextureTest, CubeMapTargetBGRA)
{
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        {
            glBindTexture(GL_TEXTURE_2D, textures[0]);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_BGRA_EXT, 1, 1, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, &pixels[face - GL_TEXTURE_CUBE_MAP_POSITIVE_X + i]);
