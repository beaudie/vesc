...
class CaptureReplayTest
{
  public:
    ...
    int run()
    {
        ...
        for (uint32_t frame = kReplayFrameStart; frame <= kReplayFrameEnd; frame++)
        {
            ...
            if (frame == kReplayFrameEnd)
            {
                isBuffersSwapped = kIsBuffersSwappedInLastFrame;
            }
            // If swapBuffers is not called, then default framebuffer should not be serialized
            if (angle::SerializeContext(&bos, context, isBuffersSwapped) != angle::Result::Continue)
            {
                return -1;
            }
  ...
  private:
    ...
    {
        std::vector<uint8_t> capture = GetSerializedContextStateData(frame);
        std::vector<uint8_t> replay  = replaySerializedContextData.getData();
