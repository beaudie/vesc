...
def main():
    ...
    for (test, sample_index, sample_count) in tests:
        ...
        for sample in range(sample_count):
            test_and_sample = '%s_%d' % (test, sample)
        ...
        test_name = test if (sample_index == 0 and sample_count > 1) else ('%s_sample_%d' %
                                                                           (test, sample_index))
        ...
        if not wall_times:
        ...
        elif len(wall_times) == (sample_count * args.trials_per_sample):
            if len(wall_times) > 7:
            ...
            test_results[test_name] = {'expected': PASS, 'actual': PASS}
            ...
            test_results[test_name] = {'expected': PASS, 'actual': FAIL, 'is_unexpected': True}
            results['num_failures_by_type'][FAIL] += 1
