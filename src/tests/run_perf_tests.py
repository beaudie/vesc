...
def _run_and_get_output(args, cmd, env):
    lines = []
    modified_cmd = cmd
    for verbose_cnt in range(args.verbose):
        modified_cmd += ['-v']
    with common.temporary_file() as tempfile_path:
        if args.xvfb:
            ret = xvfb.run_executable(modified_cmd, env, stdoutfile=tempfile_path)
        else:
            ret = test_env.run_command_with_output(modified_cmd, env=env, stdoutfile=tempfile_path)
        if ret:
...
def _init_output_path(out_arg, test_suite, file_name):
    isolated_out_dir = os.path.dirname(out_arg)
    if not os.path.isdir(isolated_out_dir):
        return out_arg
    benchmark_path = os.path.join(isolated_out_dir, test_suite)
    os.makedirs(benchmark_path, exist_ok=True)
    return os.path.join(benchmark_path, file_name)
...
def main():
    ...
    for test in tests:
        ...
        for sample in range(args.samples_per_test):
            ...
            with common.temporary_file() as histogram_file_path:
                cmd_run += ['--histogram-json-file=%s' % histogram_file_path]
    ...
    # Uses special output file naming to match the merge script.
    if args.isolated_script_test_output:
        output_path = _init_output_path(args.isolated_script_test_output, args.test_suite,
                                        'test_results.json')
        with open(output_path, 'w') as out_file:
            out_file.write(json.dumps(results, indent=2))

    if args.isolated_script_test_perf_output:
        output_path = _init_output_path(args.isolated_script_test_perf_output, args.test_suite,
                                        'perf_results.json')
        with open(output_path, 'w') as out_file:
