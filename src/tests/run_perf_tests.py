...
def main():
    ...
    if android_helper.IsAndroid():
        ...
        exit_code, output = angle_test_util.RunTestSuite(
            args.test_suite, cmd_args, env, runner_args=[], use_xvfb=args.xvfb)
    ...
    for test_index in range(num_tests):
        ...
        if args.steps_per_trial:
            ...
            exit_code, calibrate_output = angle_test_util.RunTestSuite(
                args.test_suite, calibrate_args, env, runner_args=runner_args, use_xvfb=args.xvfb)
            if exit_code != EXIT_SUCCESS:
        ...
        for sample in range(args.samples_per_test):
            ...
            with common.temporary_file() as histogram_file_path:
                ...
                exit_code, output = angle_test_util.RunTestSuite(
                    args.test_suite, run_args, env, runner_args=runner_args, use_xvfb=args.xvfb)
                if exit_code != EXIT_SUCCESS:
