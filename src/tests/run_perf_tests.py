...
def _run_tests(test_suite, tests, args, extra_flags, env):
    ...
    for test_index in range(len(tests)):
        ...
        if angle_test_util.IsAndroid():
            ...
            if trace and trace not in prepared_traces:
                android_helper.PrepareRestrictedTraces(test_suite, [trace])
...
def main():
    ...
    suite_group.add_argument(
        ...
        action='store_true')
    parser.add_argument(
        '--android-external-binaries',
        help='Use in combination with --trace-tests on Android.',
        action='store_true')
    ...
    if args.trace_tests:
        if args.android_external_binaries:
    ...
    try:
        with _maybe_lock_gpu_clocks():
            results, histograms, metrics = _run_tests(args.test_suite, tests, args, extra_flags,
                                                      env)
