...
class TestSuite::TestEventListener : public testing::EmptyTestEventListener
{
  public:
    ...
    void OnTestProgramEnd(const testing::UnitTest &testProgramInfo) override
    {
        ...
        js::Document::AllocatorType &allocator = mJson.GetAllocator();
        ...
        mJson.AddMember("interrupted", false, allocator);
        mJson.AddMember("path_delimiter", ".", allocator);
        mJson.AddMember("version", 3, allocator);
        mJson.AddMember("seconds_since_epoch", ltime, allocator);
        mJson.AddMember("num_failures_by_type", numFailuresByType, allocator);
        ...
        mJson.AddMember("tests", tests, allocator);

        char buf[500];
        sprintf(buf, "%s%c%s", mOutputDirectory, kPathSeparator, "results.json");
        FILE *fp = fopen(buf, "w");

        char writeBuffer[65535];
        js::FileWriteStream os(fp, writeBuffer, sizeof(writeBuffer));
    ...
    : mResultsDirectory(nullptr), mShardCount(-1), mShardIndex(-1)
{
    ...
    {
        mTestEventListener.reset(new TestEventListener(mResultsDirectory, mTestSuiteName.c_str()));
        testing::TestEventListeners &listeners = testing::UnitTest::GetInstance()->listeners();
        listeners.Append(mTestEventListener.get());
    }
