      ...
      mTestTimeout(60)
{
    ...
    if (!mFilterFile.empty())
    {
        ...
        mFilterString.reserve(fileSize);
        if (!ReadEntireFileToString(mFilterFile.c_str(), &mFilterString[0], fileSize))
        {
...
bool TestSuite::parseSingleArg(const char *argument)
{
    return (ParseIntArg("--shard-count=", argument, &mShardCount) ||
            ...
            ParseIntArg("--test-timeout=", argument, &mTestTimeout) ||
...
bool TestSuite::launchChildTestProcess(const std::vector<TestIdentifier> &testsInBatch)
{
    ...
    processInfo.filterFileName.reserve(kMaxPath);
    if (!CreateTemporaryFile(&processInfo.filterFileName[0], kMaxPath))
    {
    ...
    processInfo.resultsFileName.reserve(kMaxPath);
    if (!CreateTemporaryFile(&processInfo.resultsFileName[0], kMaxPath))
    {
        std::cerr << "Error creating temporary file for test list.\n";
        return false;
    }
...
bool TestSuite::finishProcess(ProcessInfo *processInfo)
{
    ...
    {
        std::ifstream ifs(processInfo->resultsFileName);
        if (!ifs.is_open())
        {
...
int TestSuite::run()
{
    ...
    while (!mTestQueue.empty() || !mCurrentProcesses.empty())
    {
        ...
        for (auto processIter = mCurrentProcesses.begin(); processIter != mCurrentProcesses.end();)
        {
            ...
            else if (processInfo.process->getElapsedTimeSeconds() > mTestTimeout)
            {
