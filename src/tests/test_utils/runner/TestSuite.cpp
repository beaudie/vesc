...
bool TestSuite::launchChildTestProcess(const std::vector<TestIdentifier> &testsInBatch)
{
    ...
    if (!CreateTemporaryFile(&processInfo.filterFileName))
    {
    ...
    if (!CreateTemporaryFile(&processInfo.resultsFileName))
    {
        std::cerr << "Error creating temporary file for test list.\n";
        return false;
    }
    ...
    processInfo.process.reset(RunProcessAsync(args, true, true));
...
bool TestSuite::finishProcess(ProcessInfo *processInfo)
{
    ...
    processInfo->process->getStdout(&batchStdout);
