...
void ParseArgsFromString(const std::string &input, std::vector<std::string> *args)
{
    std::vector<std::string> split;
    angle::SplitStringAlongWhitespaceSkipQuotes(input, '"', &split);
    for (size_t i = 0; i < split.size(); ++i)
    {
        angle::RemoveChars(&split[i], '"');
    }
                                                           ...
                                                           jstring jstdoutFilePath)
{
    ...
    else
    {
        ...
        {
            ParseArgsFromString(commandLineString, &args);
        }
    }
    ParseArgsFromString(commandLineFlags, &args);
