...
def GetScriptsDir():
    angle_testing = os.path.join(GetANGLESrcDir(), 'testing', 'scripts')
    if os.path.isdir(angle_testing):
        return os.path.join(GetANGLESrcDir(), 'testing', 'scripts')
    chrome_testing = os.path.join(GetANGLESrcDir(), '..', '..', 'testing', 'scripts')
    if os.path.isdir(chrome_testing):
        return chrome_testing
    raise "Cannot find testing/scripts directory!"
...
def main():
    ...
    try:
        ...
        if args.isolated_script_test_output:
            extra_flags.append(
                '--results-directory=%s' % os.path.dirname(args.isolated_script_test_output))
