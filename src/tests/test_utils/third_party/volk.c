...
{
    //printf("In vkGIPAS, calling GIPA for %s\n", name);
...
VkResult volkInitialize(void)
{
...
#else
        printf("Opening libvulkan.so\n");
	void* module = dlopen("libvulkan.so", RTLD_NOW | RTLD_LOCAL);
	if (!module) {
            printf("Opening libvulkan.so.1\n");
	...
	if (!module) {
        ...
        printf("Finding vkGetInstanceProcAddr\n");
        fflush(stdout);
	vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)dlsym(module, "vkGetInstanceProcAddr");
#endif

        printf("Calling volkGenLoadLoader in rendererVK\n");
        fflush(stdout);
	volkGenLoadLoader(NULL, vkGetInstanceProcAddrStub);
        printf("Called volkGenLoadLoader\n");
        fflush(stdout);
        printf("vkCreateInstance fptr:%p, address:%p\n", vkCreateInstance, &vkCreateInstance);
        printf("vkEnumerateInstanceLayerProperties fptr:%p, address:%p\n", vkEnumerateInstanceLayerProperties, &vkEnumerateInstanceLayerProperties);
...
static void volkGenLoadLoader(void* context, PFN_vkVoidFunction (*load)(void*, const char*))
{
    printf("In volkGenLoadLoader\n");
	/* VOLK_GENERATE_LOAD_LOADER */
#if defined(VK_VERSION_1_0)
    printf("Leaving volkGenLoadLoader, VK_VER_1_0 defined\n");
	vkCreateInstance = (PFN_vkCreateInstance)load(context, "vkCreateInstance");
        printf("vkCreateInstance fptr:%p\n", vkCreateInstance);
	vkEnumerateInstanceExtensionProperties = (PFN_vkEnumerateInstanceExtensionProperties)load(context, "vkEnumerateInstanceExtensionProperties");
        printf("vkEnumerateInstanceExtensionProperties fptr:%p\n", vkEnumerateInstanceExtensionProperties);
	vkEnumerateInstanceLayerProperties = (PFN_vkEnumerateInstanceLayerProperties)load(context, "vkEnumerateInstanceLayerProperties");
        printf("vkEnumerateInstanceLayerProperties fptr:%p\n", vkEnumerateInstanceLayerProperties);
#endif /* defined(VK_VERSION_1_0) */
#if defined(VK_VERSION_1_1)
        printf("Leaving volkGenLoadLoader, VK_VER_1_1 defined\n");
	vkEnumerateInstanceVersion = (PFN_vkEnumerateInstanceVersion)load(context, "vkEnumerateInstanceVersion");
#endif /* defined(VK_VERSION_1_1) */
	/* VOLK_GENERATE_LOAD_LOADER */
            printf("Leaving volkGenLoadLoader\n");
}
