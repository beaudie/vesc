...
constexpr uint32_t kResetLimit = 50;
}  // anonymous namespace
      ...
      mCurrentPlatform(nullptr)
{
    ...
    if (iter != gPlatforms.end())
    {
        ...
        if (mCurrentPlatform->resetCounter++ < kResetLimit)
        {
            return;
        }

        mCurrentPlatform->resetCounter = 0;

        if (mCurrentPlatform->eglWindow)
        {
            mCurrentPlatform->eglWindow->destroyGL();
            EGLWindow::Delete(&mCurrentPlatform->eglWindow);
        }

        if (mCurrentPlatform->wglWindow)
        {
#if defined(ANGLE_USE_UTIL_LOADER) && defined(ANGLE_PLATFORM_WINDOWS)
            mCurrentPlatform->wglWindow->destroyGL();
            WGLWindow::Delete(&mCurrentPlatform->wglWindow);
#endif  // defined(ANGLE_USE_UTIL_LOADER) && defined(ANGLE_PLATFORM_WINDOWS)
        }

        if (mCurrentPlatform->osWindow)
        {
            mCurrentPlatform->osWindow->destroy();
            OSWindow::Delete(&mCurrentPlatform->osWindow);
        }
    }
    else
    {
    ...
    switch (params.driver)
    {
        ...
        {
#if defined(ANGLE_USE_UTIL_LOADER) && defined(ANGLE_PLATFORM_WINDOWS)
            mCurrentPlatform->wglWindow = WGLWindow::New(params.majorVersion, params.minorVersion);
#else
            std::cerr << "Unsupported driver." << std::endl;
#endif  // defined(ANGLE_USE_UTIL_LOADER) && defined(ANGLE_PLATFORM_WINDOWS)
            break;
...
void ANGLETestBase::ANGLETestSetUp()
{
    ...
    if (mCurrentPlatform->wglWindow)
    {
        ...
        if (!mCurrentPlatform->wglWindow->initializeGL(mCurrentPlatform->osWindow,
                                                       ANGLETestEnvironment::GetWGLLibrary(),
    ...
    else
    {
        ...
        // Only enable display reuse on Intel for now. There seem to be quite a few potential driver
        // bugs that we need to track down before we can enable it everywhere.
        if (!systemInfo->hasIntelGPU())
        {
...
{
    return angle::GetTestSystemInfo()->hasNVIDIAGPU();
}
...
{
    return mCurrentPlatform->wglWindow
               ? reinterpret_cast<GLWindowBase *>(mCurrentPlatform->wglWindow)
               : mCurrentPlatform->eglWindow;
}
