...
Library *ANGLETestEnvironment::GetEGLLibrary()
{
    ...
    if (!gEGLLibrary)
    {
        gEGLLibrary.reset(OpenSharedLibrary(ANGLE_EGL_LIBRARY_NAME, SearchType::APPLICATION_DIR));
    }
#endif  // defined(ANGLE_USE_UTIL_LOADER)
...
Library *ANGLETestEnvironment::GetWGLLibrary()
{
    ...
    if (!gWGLLibrary)
    {
        gWGLLibrary.reset(OpenSharedLibrary("opengl32", SearchType::SYSTEM_DIR));
    }
