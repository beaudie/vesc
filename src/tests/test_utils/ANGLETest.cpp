      ...
      mDeferContextInit(false)
{
    switch (params.driver)
    {
        case angle::GLESDriverType::AngleEGL:
        {
            ...
            for (angle::GPUDeviceInfo gpu : ANGLETestEnvironment::GetSystemInfo()->gpus)
            {
                if (angle::IsNvidia(gpu.vendorId))
                {
                    // Check all GPUs, since detecting currently used GPU is not simple
                    hasNvidia = true;
                    break;
                }
            }
            ...
            bool needsWindowSwap =
                hasNvidia && mLastRendererType.valid() &&
