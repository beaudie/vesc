      ...
      mCurrentPlatform(nullptr)
{
    ...
    if (alwaysForceNewDisplay() && mOSWindow)
    {
        mCurrentPlatform->osWindow = mOSWindow;
    }
    ...
    if (!mCurrentPlatform->osWindow)
    {
        ...
        mOSWindow = mCurrentPlatform->osWindow;
    }
...
bool ANGLETestBase::alwaysForceNewDisplay() const
{
    // Attempt to work around an Intel issue where creating many displays causes crashes.
    angle::SystemInfo *systemInfo = angle::GetTestSystemInfo();
    return (!systemInfo || !angle::IsWindows() || systemInfo->hasAMDGPU());
}
...
void ANGLETestBase::ANGLETestTearDown()
{
    ...
    if (mCurrentPlatform->resetCounter++ >= kResetLimit || mForceNewDisplay ||
        alwaysForceNewDisplay())
    {
