...
bool IsConfigSupported(const SystemInfo &systemInfo, const PlatformParameters &param)
{
    ...
    // Only Vulkan ES 2.0 is currently supported.
    if (param.eglParameters.majorVersion != 2 &&
        param.getRenderer() == EGL_PLATFORM_ANGLE_TYPE_VULKAN_ANGLE)
    {
        return false;
    }

    // ES 3.2 is not yet supported.
    if (param.majorVersion == 3 && param.minorVersion >= 2)
    {
        return false;
    }
            ...
            case GLESDriverType::AngleEGL:
                switch (param.getRenderer())
                {
                    ...
                    case EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE:
                        // ES 3.1+ is not supported properly.
    ...
    else if (IsAndroid())
    {
        ...
        // Some devices don't support backing 3.2 contexts.
...
bool IsPlatformAvailable(const PlatformParameters &param)
{
    ...
    else
    {
        ...
        if (systemInfo)
        {
            bool result                   = IsConfigSupported(*systemInfo, param);
