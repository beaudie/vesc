...
{
    EXPECT_TRUE(mOutputCode.find(output) != mOutputCode.end());
    return mOutputCode.find(output)->second.find(stringToFind) != std::string::npos;
}
                                      ...
                                      const int expectedOccurrences) const
{
    ...
    EXPECT_TRUE(code != mOutputCode.end());
    size_t currentPos  = 0;
