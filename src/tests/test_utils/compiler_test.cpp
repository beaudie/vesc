...
class FunctionCallFinder : public TIntermTraverser
{
  public:
    FunctionCallFinder(const TString &functionMangledName)
        : TIntermTraverser(true, false, false), mFunctionMangledName(functionMangledName), mNodeFound(nullptr)
    {
    }

    bool visitAggregate(Visit visit, TIntermAggregate *node) override
    {
        if (node->isFunctionCall() && TFunction::GetMangledNameFromCall(node->getFunctionSymbolInfo()->getName(), *node->getSequence()) == mFunctionMangledName)
        {
            mNodeFound = node;
            return false;
        }
