...
class ShaderFunctionFinder : public TIntermTraverser
{
  public:
    ShaderFunctionFinder(const TString &functionName)
    ...
    {
        if (node->getFunctionSymbolInfo()->getName() == mFunctionName)
        {
            mNodeFound = node;
            return false;
        }
...
{
    ShaderFunctionFinder finder(functionName);
    root->traverse(&finder);
    return finder.getNode();
}
