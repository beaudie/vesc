                                  ...
                                  public ::testing::WithParamInterface<DrawElementsPerfParams>
{
  ...
  private:
    ...
    int mCount               = 3 * mNumTris;
    GLenum mType             = GetParam().type;
    bool mIndexBufferChanged = GetParam().indexBufferChanged;
...
void DrawElementsPerfBenchmark::initializeBenchmark()
{
    ...
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(mType) * mIndexData.size(),
                    mIndexData.data());
...
void DrawElementsPerfBenchmark::drawBenchmark()
{
    ...
    for (unsigned int it = 0; it < params.iterations; it++)
    {
        ...
        glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(mCount), mType, 0);
    }
...
ANGLE_INSTANTIATE_TEST(DrawElementsPerfBenchmark,
                       DrawElementsPerfD3D9Params(false, false),
                       DrawElementsPerfD3D9Params(true, false),
                       DrawElementsPerfWithChangedIndexBufferD3D11Params(false, false),
                       DrawElementsPerfWithChangedIndexBufferD3D11Params(true, false),
                       DrawElementsPerfWithChangedIndexBufferD3D11Params(true, true),
                       DrawElementsPerfD3D11Params(false, false),
                       DrawElementsPerfD3D11Params(true, false),
                       DrawElementsPerfD3D11Params(true, true),
                       DrawElementsPerfOpenGLParams(false, false),
                       DrawElementsPerfOpenGLParams(true, false),
                       DrawElementsPerfOpenGLParams(true, true),
                       DrawElementsPerfValidationOnly());
