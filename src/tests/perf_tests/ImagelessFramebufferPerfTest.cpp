...
constexpr std::size_t kTextureCount       = 50;
...
std::string ImagelessFramebufferAttachmentParams::story() const
{
    ...
    strstr << (isImagelessFramebufferEnabled ? "_imageless_framebuffer_enabled"
                                             : "imageless_framebuffer_disabled");
...
void ImagelessFramebufferAttachmentBenchmark::initializeBenchmark()
{
    if (IsPixel4())
    {
        skipTest("Imageless framebuffers are not supported on Pixel 4.");
    }
...
ImagelessFramebufferAttachmentParams ImagelessVulkanEnabledParams()
{
    ...
    params.eglParameters = egl_platform::VULKAN().enable(Feature::SupportsImagelessFramebuffer);
...
{
    ImagelessFramebufferAttachmentParams params;
    params.eglParameters = egl_platform::VULKAN().disable(Feature::SupportsImagelessFramebuffer);
