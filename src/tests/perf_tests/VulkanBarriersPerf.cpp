...
{
    VulkanBarriersPerfParams(bool largeTransfers, bool longFS)
    {
...
constexpr char kLongFS[] = R"(precision mediump float;
varying vec2 v_texCoord;
uniform sampler2D s_texture;
void main()
{
...
void VulkanBarriersPerfBenchmark::createResources()
{
    ...
    mProgram.makeRaster(kVS, params.doLongFragmentShaders ? kLongFS : kShortFS);
