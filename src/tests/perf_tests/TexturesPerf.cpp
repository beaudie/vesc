...
std::string TexturesParams::suffix() const
{
    ...
    strstr << "_" << textureRebindFrequency << "_rebind_freq";
    strstr << "_" << textureStateUpdateFrequency << "_state_freq";
...
void TexturesBenchmark::initTextures()
{
    ...
    for (size_t texIndex = 0; texIndex < params.numTextures; texIndex++)
    {
        ...
        for (size_t mip = 0; mip < params.textureMipCount; mip++)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, static_cast<GLint>(mip), GL_RGBA, levelSize, levelSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData.data());
        }
...
void TexturesBenchmark::drawBenchmark()
{
    ...
    for (size_t it = 0; it < params.iterations; ++it)
    {
        ...
        if (it % params.textureStateUpdateFrequency == 0)
        {
            ...
            size_t updateTexture = stateUpdateCount % params.numTextures;
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minFilters[stateUpdateCount % ArraySize(minFilters)]);

            const GLenum magFilters[] = {
                GL_NEAREST,
                GL_LINEAR,
            };
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magFilters[stateUpdateCount % ArraySize(magFilters)]);
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrapParameters[stateUpdateCount % ArraySize(wrapParameters)]);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapParameters[stateUpdateCount % ArraySize(wrapParameters)]);
        }
