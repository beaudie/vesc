...
struct UniformsParams final : public RenderTestParams
{
    UniformsParams()
    {
        ...
        numVertexUniforms   = 16;
        numFragmentUniforms = 16;
    }
...
std::string UniformsParams::suffix() const
{
    ...
    strstr << "_" << numVertexUniforms << "vertex_uniforms";
    strstr << "_" << numFragmentUniforms << "fragment_uniforms";
...
static std::string GetUniformLocationName(size_t idx, bool vertexShader)
{
    std::stringstream strstr;
...
void UniformsBenchmark::drawBenchmark()
{
    ...
    {
        for (size_t i = 0; i < mUniformLocations.size(); i++)
        {
            float value = static_cast<float>(i);
            glUniform4f(mUniformLocations[i], value, value, value, value);
        }
