...
std::string UniformsParams::suffix() const
{
    ...
    else if (dataType == DataType::MAT3)
    {
        strstr << "_" << (numVertexUniforms + numFragmentUniforms) << "_mat3";
    }
    else
    {
        strstr << "_" << (numVertexUniforms + numFragmentUniforms) << "_mat4";
    }
...
void UniformsBenchmark::initializeBenchmark()
{
    ...
    bool isMatrix = params.dataType != DataType::VEC4;
    if (isMatrix)
    {
...
void UniformsBenchmark::initShaders()
{
    ...
    const std::string kUniformVarPosHolder = "%s";
    ...
    switch (params.dataType)
    {
        case DataType::VEC4:
            typeString               = "vec4";
            uniformOperationTemplate = kUniformVarPosHolder;
            break;
        case DataType::MAT3:
            typeString               = "mat3";
            uniformOperationTemplate = "mat4(" + kUniformVarPosHolder + ") * vec3(1.0, 1.0, 1.0)";
            break;
        case DataType::MAT4:
            typeString               = "mat4";
            uniformOperationTemplate = kUniformVarPosHolder + "* vec4(1.0, 1.0, 1.0, 1.0)";
            break;
        default:
            UNREACHABLE();
    }
    ...
    for (size_t i = 0; i < params.numVertexUniforms; i++)
    {
        ...
        std::size_t pos              = uniformOperation.find(kUniformVarPosHolder);
        ASSERT(pos != std::string::npos);
        uniformOperation.replace(pos, pos + kUniformVarPosHolder.size(),
...
ANGLE_INSTANTIATE_TEST(
    ...
    MatrixUniforms(D3D11(), DataMode::UPDATE, DataType::MAT4, MatrixLayout::NO_TRANSPOSE),
    MatrixUniforms(OPENGL_OR_GLES(), DataMode::UPDATE, DataType::MAT3, MatrixLayout::NO_TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::UPDATE, DataType::MAT4, MatrixLayout::NO_TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::UPDATE, DataType::MAT4, MatrixLayout::TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::UPDATE, DataType::MAT3, MatrixLayout::NO_TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::UPDATE, DataType::MAT3, MatrixLayout::TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::REPEAT, DataType::MAT4, MatrixLayout::NO_TRANSPOSE),
    MatrixUniforms(VULKAN_NULL(), DataMode::REPEAT, DataType::MAT4, MatrixLayout::TRANSPOSE),
