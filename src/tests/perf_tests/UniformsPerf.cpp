...
void UniformsBenchmark::initShaders()
{
    ...
    switch (params.dataType)
    {
        case DataType::VEC4:
            typeString               = "vec4";
            uniformOperationTemplate = "%s";
            break;
        case DataType::MAT3:
            typeString               = "mat3";
            uniformOperationTemplate = "mat4(%s) * vec3(1.0, 1.0, 1.0)";
            break;
        case DataType::MAT4:
            typeString               = "mat4";
            uniformOperationTemplate = "%s * vec4(1.0, 1.0, 1.0, 1.0)";
            break;
        default:
            UNREACHABLE();
    }
    ...
    for (size_t i = 0; i < params.numVertexUniforms; i++)
    {
        std::string uniformOperation;
        vstrstr << uniformOperationTemplate;
        vstrstr << ";\n";
    }
    vstrstr << "}";
