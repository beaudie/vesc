...
void ANGLERenderTest::computeGPUTime()
{
    if (mTestParams.trackGpuTime && mIsTimestampQueryAvailable)
    {
        for (const TimestampSample &sample : mTimestampQueries)
        {
            ...
            if (gVerboseLogging)
            {
                printf("Timestamp sample: begin: %ld end: %ld delta %ld\n", beginGLTimeNs,
                       endGLTimeNs, endGLTimeNs - beginGLTimeNs);
            }
            glDeleteQueries(1, &sample.beginQuery);
            glDeleteQueries(1, &sample.endQuery);
