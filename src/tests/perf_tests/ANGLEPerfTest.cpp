...
int ANGLEPerfTest::estimateStepsToRun() const
{
    double trialTime   = mTrialTimer.getElapsedWallClockTime();
...
void ANGLEPerfTest::calibrateStepsToRun()
{
    ...
    if (isRenderTest())
    {
        ...
        runTrial(gTrialTimeSeconds, std::numeric_limits<int>::max(),
        ...
        {
            int stepsToRun = estimateStepsToRun();
            runTrial(gTrialTimeSeconds, stepsToRun, RunTrialPolicy::RunContinuously);
    ...
    {
        while (calibrationTimer.getElapsedWallClockTime() + gTrialTimeSeconds <
    ...
    mStepsToRun = estimateStepsToRun();
