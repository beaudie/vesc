...
void ANGLEPerfTest::runTrial(double maxRunTime, int maxStepsToRun, RunTrialPolicy runPolicy)
{
    ...
    int lastTrialNumStepsPerformed = mTrialNumStepsPerformed;
    ...
    while (mRunning)
    {
        ...
        {
            if (mTrialNumStepsPerformed > lastTrialNumStepsPerformed)
            {
                double wallTime    = mTrialTimer.getElapsedWallClockTime();
                int frameTimeAvgUs = int(1e6 * (wallTime - lastWallTime) /
                                         (mTrialNumStepsPerformed - lastTrialNumStepsPerformed));
                ATraceCounter("TraceLoopFrameTimeAvgUs", frameTimeAvgUs);
                lastWallTime = wallTime;
            }
            lastTrialNumStepsPerformed = mTrialNumStepsPerformed;
        }
