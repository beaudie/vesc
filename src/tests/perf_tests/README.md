...
Each test runs N trials and prints metrics for each trial. Trials are limited by time (default), step/frame limits can also be set. Note that at the beginning performance might be affected by hitting new code paths, cold caches etc (see warmup args below).
...
* `--warmup-steps x`: Maximum number of steps for the warmup loops. Defaults to 0 (no warmup). Set to -1 to use a trace's frame count.
* `--fixed-test-time-with-warmup x`: Start with a warmup (`--warmup-steps -1`), then run the tests until this much time has elapsed.
