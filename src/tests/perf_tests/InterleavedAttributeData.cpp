...
void InterleavedAttributeDataBenchmark::initializeBenchmark()
{
    ...
    const std::string vs = SHADER_SOURCE(attribute vec4 aPosition; attribute vec4 aColor;
    ...
    const std::string fs = SHADER_SOURCE(precision mediump float; varying vec4 vColor; void main()
                                         {
                                             gl_FragColor = vColor;
                                         });
    ...
    for (size_t i = 0; i < ARRAYSIZE(mPositionColorBuffer); i++)
    {
        ...
        for (unsigned int j = 0; j < params.numSprites; j++)
        {
            ...
            *reinterpret_cast<float *>(
                ...
                (2 * static_cast<float>(rand() % getWindow()->getWidth()) /
                     getWindow()->getWidth() -
                 1.0f);  // X
            *reinterpret_cast<float *>(
                &(positionColorData[j * mBytesPerSprite + 1 * sizeof(float) + 0])) =
                (2 * static_cast<float>(rand() % getWindow()->getHeight()) /
                     getWindow()->getHeight() -
                 1.0f);  // Y                                               // Z

            // Add color data for the pointsprite
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 0] =
                (static_cast<GLubyte>(rand() % 255));  // R
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 1] =
                (static_cast<GLubyte>(rand() % 255));  // G
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 2] =
                (static_cast<GLubyte>(rand() % 255));  // B
        }
