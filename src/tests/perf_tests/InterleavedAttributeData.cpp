...
inline std::ostream &operator<<(std::ostream &os, const InterleavedAttributeDataParams &params)
{
...
void InterleavedAttributeDataBenchmark::initializeBenchmark()
{
    ...
    for (size_t i = 0; i < ArraySize(mPositionColorBuffer); i++)
    {
        ...
        {
            float pointSpriteX =
                (static_cast<float>(rand() % getWindow()->getWidth()) / getWindow()->getWidth()) * 2.0f - 1.0f;
            float pointSpriteY =
                (static_cast<float>(rand() % getWindow()->getHeight()) / getWindow()->getHeight()) * 2.0f - 1.0f;
