...
void InterleavedAttributeDataBenchmark::initializeBenchmark()
{
    ...
    for (size_t i = 0; i < ARRAYSIZE(mPositionColorBuffer); i++)
    {
        ...
        byte *positionColorData = new byte[mBytesPerSprite * params.numSprites];
        for (unsigned int j = 0; j < params.numSprites; j++)
        {
            // Add position data for the pointsprite
            *reinterpret_cast<float *>(&(positionColorData[j * mBytesPerSprite + 0 * sizeof(float) + 0])) = (2 * static_cast<float>(rand() % getWindow()->getWidth()) / getWindow()->getWidth() - 1.0f);  // X
            *reinterpret_cast<float *>(&(positionColorData[j * mBytesPerSprite + 1 * sizeof(float) + 0])) = (2 * static_cast<float>(rand() % getWindow()->getHeight()) / getWindow()->getHeight() - 1.0f); // Y                                               // Z

            // Add color data for the pointsprite
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 0] = (static_cast<GLubyte>(rand() % 255)); // R
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 1] = (static_cast<GLubyte>(rand() % 255)); // G
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 2] = (static_cast<GLubyte>(rand() % 255)); // B
        }
