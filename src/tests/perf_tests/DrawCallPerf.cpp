...
void main()
{
    vec4 sum = vec4(0);
        ...
        for (GLint varyingIndex = 0; varyingIndex < maxVaryings; ++varyingIndex)
        {
            if (varyingIndex < activeVaryingCount)
            {
                vs << "v" << varyingIndex << " = texture2D(sampler" << (varyingIndex % 2)
                   << ", vec2(mod(nonConstZero + " << varyingIndex << ".0, 15.0) / " << maxVaryings
                   << ".0, mod(nonConstZero + " << varyingIndex << ".0, 17.0) / " << maxVaryings
                   << ".0));\n";
                fs << "sum += v" << varyingIndex << ";\n";
            }
        }
