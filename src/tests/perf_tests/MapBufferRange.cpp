...
void MapBufferRangeBenchmark::initializeBenchmark()
{
    ...
    ASSERT_LT(params.updateSize + params.updateOffset, params.bufferSize);
    ...
    mTriSize = static_cast<GLsizei>(GetVertexData(params.vertexType, params.vertexComponentCount,
                                                  params.vertexNormalized, &mVertexData));

    mNumTris = static_cast<int>(params.updateSize / mTriSize);
...
void MapBufferRangeBenchmark::drawBenchmark()
{
    ...
    for (unsigned int it = 0; it < params.iterationsPerStep; it++)
    {
        ...
        glDrawArrays(GL_TRIANGLES, params.updateOffset / mTriSize, 3 * mNumTris);
    }
