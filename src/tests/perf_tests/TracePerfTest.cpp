...
class TracePerfTest : public ANGLERenderTest, public ::testing::WithParamInterface<TracePerfParams>
{
  ...
  private:
    ...
    std::array<GLuint, mMaxOffscreenBufferCount> mOffscreenDepthStencils = {0, 0};
    int mWindowWidth                                                     = 0;
    int mWindowHeight                                                    = 0;
...
void TracePerfTest::initializeBenchmark()
{
    ...
    if (params.surfaceType == SurfaceType::Offscreen)
    {
        ...
        glGenFramebuffers(mMaxOffscreenBufferCount, mOffscreenTextures.data());
        glGenRenderbuffers(mMaxOffscreenBufferCount, mOffscreenDepthStencils.data());
        for (int i = 0; i < mMaxOffscreenBufferCount; i++)
        {
            ...
            glBindRenderbuffer(GL_RENDERBUFFER, mOffscreenDepthStencils[i]);
            glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, mWindowWidth,
                                  mWindowHeight);

            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                   mOffscreenTextures[i], 0);
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                      mOffscreenDepthStencils[i]);
...
void TracePerfTest::destroyBenchmark()
{
    ...
    glDeleteRenderbuffers(mMaxOffscreenBufferCount, mOffscreenDepthStencils.data());
    mOffscreenDepthStencils.fill(0);
...
void TracePerfTest::drawBenchmark()
{
    ...
    if (params.surfaceType == SurfaceType::Offscreen)
    {
        // We ping pong between each offscreen buffer
