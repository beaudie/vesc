...
void TracePerfTest::initializeBenchmark()
{
    ...
    if (params.surfaceType == SurfaceType::Offscreen)
    {
        if (!IsAndroid())
        {
            mWindowWidth  = mWindowWidth / 4;
            mWindowHeight = mWindowHeight / 4;
            getWindow()->resize(mWindowWidth, mWindowHeight);
        }
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, mTestParams.windowWidth, mTestParams.windowHeight,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

        glGenRenderbuffers(1, &mOffscreenDepthStencil);
        glBindRenderbuffer(GL_RENDERBUFFER, mOffscreenDepthStencil);
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, mTestParams.windowWidth,
...
void TracePerfTest::drawBenchmark()
{
    ...
    const uint32_t kOffscreenOffsetX =
        static_cast<uint32_t>(static_cast<double>(mWindowWidth) / 3.0f);
    const uint32_t kOffscreenOffsetY =
        static_cast<uint32_t>(static_cast<double>(mWindowHeight) / 3.0f);
    ...
    for (uint32_t frame = mStartFrame; frame <= mEndFrame; ++frame)
    {
        ...
        else
        {
            ...
            glBlitFramebuffer(0, 0, mTestParams.windowWidth, mTestParams.windowHeight, windowX,
                              windowY, windowX + kOffscreenFrameWidth,
                              windowY + kOffscreenFrameHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
