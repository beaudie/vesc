    ...
    : ANGLERenderTest("TracePerf", params, "ms"), mParams(params), mStartFrame(0), mEndFrame(0)
{
    ...
    {
        const TraceInfo &traceInfo = GetTraceInfo(mParams.testID);
        mStepsToRun                = (traceInfo.endFrame - traceInfo.startFrame) * 2;
    }
}
...
void TracePerfTest::initializeBenchmark()
{
    ...
    if (gRetraceMode)
    {
        SetEnvironmentVar("ANGLE_CAPTURE_TRIGGER", "0");
        getGLWindow()->swap();
    }
}
...
{
    if (!gTraceTestValidation ||
        mTotalNumStepsPerformed <= static_cast<int>(mEndFrame - mStartFrame))
    {
        return;
    }

#if defined(ANGLE_HAS_RAPIDJSON)
    printf("Checking serialized state.\n");
    ...
    {
        printf("Saving actual replay serializaion to %s\n", aFilePath);
    ...
    {
        printf("Saving expected capture serializaion to %s\n", bFilePath);
