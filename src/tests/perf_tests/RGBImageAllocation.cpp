...
std::ostream &operator<<(std::ostream &os, const RGBImageAllocationParams &params)
{
    return os << params.backendAndStory().substr(1)
              << (params.usesSameDataPerUpdate ? "_sameDataPerUpdate" : "_changeDataPerUpdate");
}
...
RGBImageAllocationParams VulkanParamsChangeDataPerUpdate()
{
    RGBImageAllocationParams params;
    params.eglParameters         = egl_platform::VULKAN();
    params.majorVersion          = 3;
    params.minorVersion          = 0;
    params.usesSameDataPerUpdate = false;
    return params;
}

RGBImageAllocationParams VulkanParamsSameDataPerUpdate()
{
    RGBImageAllocationParams params;
    params.eglParameters         = egl_platform::VULKAN();
    params.majorVersion          = 3;
    params.minorVersion          = 0;
    params.usesSameDataPerUpdate = true;
    return params;
}
...
ANGLE_INSTANTIATE_TEST(RGBImageAllocationBenchmark,
                       VulkanParamsChangeDataPerUpdate(),
                       VulkanParamsSameDataPerUpdate());
