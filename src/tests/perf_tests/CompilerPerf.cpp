...
class CompilerPerfTest : public ANGLEPerfTest,
                         public ::testing::WithParamInterface<angle::CompilerParameters>
{
  public:
    CompilerPerfTest();
...
CompilerPerfTest::CompilerPerfTest() : ANGLEPerfTest("CompilerPerf", GetParam().str())
{
}
...
TEST_P(CompilerPerfTest, SimpleESSL100)
{
    setTestShader(kSimpleESSL100FragSource);
    run();
}

TEST_P(CompilerPerfTest, SimpleESSL300)
{
    setTestShader(kSimpleESSL300FragSource);
    run();
}

TEST_P(CompilerPerfTest, RealWorldESSL100)
{
    setTestShader(kRealWorldESSL100FragSource);
    run();
}

ANGLE_INSTANTIATE_TEST(CompilerPerfTest,
                       angle::CompilerParameters(SH_HLSL_4_1_OUTPUT),
                       angle::CompilerParameters(SH_GLSL_450_CORE_OUTPUT),
                       angle::CompilerParameters(SH_ESSL_OUTPUT));
