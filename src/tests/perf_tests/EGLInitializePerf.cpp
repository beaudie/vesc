                              ...
                              public WithParamInterface<angle::PlatformParameters>
{
  ...
  private:
    std::vector<EGLint> mDisplayAttributes;
    OSWindow *mOSWindow;
    EGLDisplay mDisplay;
};
      ...
      mDisplay(EGL_NO_DISPLAY)
{
    ...
    mDisplayAttributes.push_back(EGL_PLATFORM_ANGLE_TYPE_ANGLE);
    mDisplayAttributes.push_back(platform.renderer);
    mDisplayAttributes.push_back(EGL_PLATFORM_ANGLE_MAX_VERSION_MAJOR_ANGLE);
    mDisplayAttributes.push_back(platform.majorVersion);
    mDisplayAttributes.push_back(EGL_PLATFORM_ANGLE_MAX_VERSION_MINOR_ANGLE);
    mDisplayAttributes.push_back(platform.minorVersion);

    if (platform.renderer == EGL_PLATFORM_ANGLE_TYPE_D3D9_ANGLE ||
        platform.renderer == EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE)
    {
        mDisplayAttributes.push_back(EGL_PLATFORM_ANGLE_DEVICE_TYPE_ANGLE);
        mDisplayAttributes.push_back(platform.deviceType);
    }
    mDisplayAttributes.push_back(EGL_NONE);
    ...
    mDisplay = eglGetPlatformDisplayEXT(EGL_PLATFORM_ANGLE_ANGLE, mOSWindow->getNativeDisplay(), &mDisplayAttributes[0]);
}
