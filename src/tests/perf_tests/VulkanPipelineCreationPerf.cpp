...
#include "src/libANGLE/renderer/vulkan/RendererVk.h"
...
{
  public:
    ContextVkEmulator(RendererVk *renderer) : vk::Context(renderer) {}
    ~ContextVkEmulator() {}
      ...
      public ::testing::WithParamInterface<VulkanPipelineCreationTestParams>
{
  ...
  private:
    ...
    std::unique_ptr<ContextVkEmulator> mContext;
    ...
    vk::GraphicsPipelineDesc mPipelineDesc;
};

VulkanPipelineCreationPerf::VulkanPipelineCreationPerf()
    : ANGLEPerfTest("VulkanPipelineCreationPerf", "", GetParam().suffix, GetParam().repeation),
      mBlobCache(256)
{
    mInfo        = {};
    mSampleTitle = "Pipeline Creation Perf";
}
...
VulkanPipelineCreationPerf::~VulkanPipelineCreationPerf()
{
    ...
    mPipelineLayout.destroy(mContext->getDevice());
    mRenderPass.destroy(mContext->getDevice());
    mPipelineCache.destroy(mContext->getDevice());
}
...
void VulkanPipelineCreationPerf::initShaderModules()
{
    ...
    angle::Result res =
        GlslangWrapper::GetShaderCode(mContext.get(), cap, false, glslShaders, &spvShaders);
    ...
    mFragmentShader.init(mContext->getDevice(), fragModuleCreateInfo);
}
...
void VulkanPipelineCreationPerf::SetUp()
{
    ...
    mRenderer = std::unique_ptr<RendererVk>(new RendererVk());
    mRenderer->setDevice(mInfo.device);
    mContext = std::unique_ptr<ContextVkEmulator>(new ContextVkEmulator(mRenderer.get()));
    ...
    initialDesc.initDefaults();
    angle::Result res =
        mPipelineCache.getPipeline(mContext.get(), mCache, mRenderPass, mPipelineLayout,
                                   gl::AttributesMask(), gl::ComponentTypeMask(), &mVertexShader,
                                   &mFragmentShader, initialDesc, &descOut, &mGraphicsPipeline);
    EXPECT_EQ(res, angle::Result::Continue);

    std::vector<int32_t> pipelineTransitions;
    ...
    {
        pipelineTransitions = kFixedTransitions;
    }

    // for (const TransitionOp op : pipelineTransitions)
    // {
...
void VulkanPipelineCreationPerf::step()
{
    ...
    mPipelineDesc.setRenderPassDesc(mRenderPassDesc);
    const vk::GraphicsPipelineDesc *out;
    angle::Result res =
        mPipelineCache.getPipeline(mContext.get(), mCache, mRenderPass, mPipelineLayout,
                                   gl::AttributesMask(), gl::ComponentTypeMask(), &mVertexShader,
                                   &mFragmentShader, mPipelineDesc, &out, &mGraphicsPipeline);
    ASSERT(res == angle::Result::Continue);
}

}  // anonymous namespace

TEST_P(VulkanPipelineCreationPerf, Run)
{
    run();
}
...
{
    VulkanPipelineCreationTestParams params = VulkanPipelineCreationTestParams();

    params.suffix    = "default";
    params.repeation = 1;

    params.numOfTransition  = 0;
    params.randomTransition = false;
    params.creationOnly     = true;
