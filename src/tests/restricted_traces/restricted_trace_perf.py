...
def cleanup():
    run_command('adb {} shell rm -f /sdcard/Download/out.txt /sdcard/Download/gpumem.txt'.format(
        SELECTED_DEVICE))
...
def run_trace(trace, renderer, args):
    ...
    run_command('adb {} push gpumem.sh /data/local/tmp'.format(SELECTED_DEVICE))
    memory_command = 'adb {} shell sh /data/local/tmp/gpumem.sh 0.25'.format(SELECTED_DEVICE)
    memory_process = run_async_command(memory_command)

    adb_command = 'adb {} shell am instrument -w '.format(SELECTED_DEVICE)
...
def get_test_time(renderer, time):
    ...
    result = run_command(
        'adb {} shell cat /sdcard/Download/out.txt | grep -v Error | grep -v Frame'.format(
            SELECTED_DEVICE))
...
def get_gpu_memory(trace_duration):
    # Pull the results from the device and parse
    result = run_command(
        'adb {} shell cat /sdcard/Download/gpumem.txt | awk "NF"'.format(SELECTED_DEVICE))
...
def get_gpu_time():
    # Pull the results from the device and parse
    result = run_command('adb {} shell cat /sdcard/Download/out.txt'.format(SELECTED_DEVICE))
    gpu_time = ''
...
class GPUPowerStats():
    ...
    def get_power_data(self):
        gpu_power_command = 'adb {} shell "'.format(SELECTED_DEVICE)
        ...
        cpu_power_command = 'adb {} shell "'.format(SELECTED_DEVICE)
...
def main():
    ...
    if num_connected_devices == 0:
        print('Device Error: No devices detected. Please connect your device and try again.')
        return 1
    elif num_connected_devices > 1 and args.device == '':
        print(
            'Device Error: More than one device detected. Please specify your target device using the --device option and try again.'
        )
        return 2
    elif num_connected_devices == 1 and args.device not in result_dev_out:
        print('Device Error: Invalid device serial.')
        return 3
    else:
        print('Selected device serial: {}'.format(args.device))
    ...
    run_command('adb {} root'.format(SELECTED_DEVICE))
    ...
    android_result = run_command(
        'adb {} shell getprop ro.build.fingerprint'.format(SELECTED_DEVICE))
