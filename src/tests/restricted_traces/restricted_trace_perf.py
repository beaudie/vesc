...
def collect_power(done_event, test_fixedtime, results):
    ...
    while True:
        ...
        if time.time() - start_time > timeout:
            logging.warning('Test not seen in logcat after %s seconds', timeout)
            p.kill()
            break
        if done_event.is_set():
            logging.warning('Test finished without being seen in logcat')
            p.kill()
            break
    ...
    # Grab starting power once we've seen test in logcat
    ...
    results.update({
        'cpu': int(ending_power.cpu_delta(starting_power) / dt),
        'gpu': int(ending_power.gpu_delta(starting_power) / dt),
    })
...
def run_traces(args):
    ...
    if args.walltimeonly:
        ...
        output_writer.writerow([
            'trace', 'wall_time(ms)', 'gpu_time(ms)', 'cpu_time(ms)', 'gpu_power(uW)',
            'cpu_power(uW)', 'gpu_mem_sustained', 'gpu_mem_peak', 'proc_mem_median',
            'proc_mem_peak'
        ])
    else:
    ...
    for renderer in renderers:
        ...
        for i in range(int(args.loop_count)):
            ...
            for trace in fnmatch.filter(traces, args.filter):
                ...
                gpu_powers[test][renderer].append(safe_cast_int(gpu_power))

                if len(cpu_powers[test]) == 0:
                    cpu_powers[test] = defaultdict(list)
                cpu_powers[test][renderer].append(safe_cast_int(cpu_power))
                ...
                if args.walltimeonly:
                    ...
                    print('%-*s %-*s %-*s %-*s %-*i %-*i %-*i %-*i %-*i %-*i' %
                          (column_width['trace'], trace_name, column_width['wall_time'], wall_time,
                           column_width['gpu_time'], gpu_time, column_width['cpu_time'], cpu_time,
                           column_width['gpu_power'], gpu_power, column_width['cpu_power'],
                           cpu_power, column_width['gpu_mem_sustained'], gpu_mem_sustained,
