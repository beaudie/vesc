...
def main():
    ...
    if num_connected_devices > 1 and args.device == '' and android_serial_env is None:
        print('Device Error: More than one device detected. Please specify a target device'
              ' through either the --device option or $ANDROID_SERIAL, and try again.')
        print_device_list()
        return 2

    if args.device != '' and args.device not in result_dev_out:
        print('Device Error: Device with serial {} not detected.'
              ' Please update the --device input and try again.'.format(args.device))
        print_device_list()
        return 3

    if args.device == '' and android_serial_env is not None and android_serial_env not in result_dev_out:
        print('Device Error: Device with serial {} not detected.'
              ' Please update $ANDROID_SERIAL and try again.'.format(android_serial_env))
        print_device_list()
        return 4
    ...
    elif android_serial_env is not None:
        print('Device with serial {} selected.'.format(android_serial_env))
        SELECTED_DEVICE = '-s {}'.format(android_serial_env)
