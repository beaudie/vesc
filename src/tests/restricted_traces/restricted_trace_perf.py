...
def select_device(device_arg):
    ...
    if device_serial is not None and device_serial not in result_dev_out:
        print('Device Error: Device with serial {} not detected.'.format(device_serial))
        if device_arg != '':
            print('Please update the --device input and try again.')
        else:
            print('Please update $ANDROID_SERIAL and try again.')
        print_device_list()
        exit()

    # Select device
    if device_serial is not None:
        print('Device with serial {} selected.'.format(device_serial))
