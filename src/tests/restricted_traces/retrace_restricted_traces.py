...
# We preserver select metadata in the trace header that can't be re-captured properly.
...
def replace_metadata(trace, trace_path, metadata):
    header_file = context_header(trace, trace_path)
    lines = []
    replaced = False
    with open(header_file, 'rt') as f:
...
def main():
    ...
    for trace in fnmatch.filter(traces, args.filter):
        ...
        run_args = [
            binary, trace_filter, '--no-warmup', '--trials', '1', '--start-trace-after-setup',
            '--steps',
            str(num_frames)
        ]
        ...
        replace_metadata(trace, trace_path, metadata)
