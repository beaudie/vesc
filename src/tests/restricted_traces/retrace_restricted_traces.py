...
def main():
    ...
    # Load trace names
    with open(os.path.join(script_dir, 'restricted_traces.json')) as f:
        traces = json.loads(f.read())

    traces = traces['traces']

    binary = os.path.join(args.gn_path, 'angle_perftests')
    if os.name == 'nt':
        binary += '.exe'
    ...
    for trace in traces:
        ...
        env['ANGLE_CAPTURE_FRAME_END'] = str(num_frames + num_frames)
