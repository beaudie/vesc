...
def upgrade_single_trace(args, trace, trace_path):
...
def upgrade_traces(args, traces):
    ...
    for trace in fnmatch.filter(traces, args.traces):
        ...
        if args.no_overwrite and path_contains_header(trace_path):
            logging.info('Skipping "%s" because the out folder already exists' % trace)
            continue
        if not upgrade_single_trace(args, trace, trace_path):
            failures += [trace]
    ...
    for trace in fnmatch.filter(traces, args.traces):
        ...
        with tempfile.TemporaryDirectory() as backup_path:
            ...
            with tempfile.TemporaryDirectory() as trace_path:
                logging.debug('Using temporary path %s.' % trace_path)
                if upgrade_single_trace(args, trace, trace_path):
                    restore_single_trace(trace, trace_path)
