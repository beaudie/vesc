...
def upgrade_single_trace(args, trace, out_path, no_overwrite, c_sources):
...
def upgrade_traces(args, traces):
    ...
    for trace in fnmatch.filter(traces, args.traces):
        if not upgrade_single_trace(args, trace, args.out_path, args.no_overwrite, args.c_sources):
            failures += [trace]
    ...
    for trace in fnmatch.filter(traces, args.traces):
        ...
        with tempfile.TemporaryDirectory() as backup_path:
            ...
            with tempfile.TemporaryDirectory() as out_path:
                ...
                if upgrade_single_trace(args, trace, out_path, False, True):
