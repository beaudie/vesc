...
case. This will be the name of the trace files, including the trace directory.
```
...
```
jq ".traces = (.traces + [\"$LABEL 1\"] | unique)" restricted_traces.json \ | sponge restricted_traces.json
```
...
be done by Googlers with write access to the trace CIPD prefix.
