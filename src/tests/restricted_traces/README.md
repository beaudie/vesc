...
produce the failure. Once you can reproduce the issue you can use a debugger
or other standard debugging processes to find the root cause and a fix.
...
If the mismatch is with a Buffer or Texture object contentx, you can manually
...
data differences might be causing the issue. Note: in the future, we might
make this option exposed via an envioronment variable, or even allow
serialization of entire data blocks in text-encoded form that could be
decoded to separate files.
...
implementations. The differences can also simple errors that don't show up as
validation mismatches because we have incomplete state serialization for some
GLES features. It can help to check the state serialization logic and add
the missing serialization as necessary.
