...
gclient runhooks
```
This should result in a number of directories created in `src/tests/restricted_traces` that contain the trace files listed in [restricted_traces.json](restricted_traces.json):
```
...
To build for Android, follow the steps in [DevSetupAndroid.md](../../../doc/DevSetupAndroid.md) (Recommend using the Performance arguments)
...
```
autoninja -C out/Performance angle_perftests
```
Run them like so:
```
out/Performance/angle_perftests --gtest_filter=TracePerfTest*
```

# Adding new Android traces
Generally we want to use a Debug setup for recording new traces. That allows us to see asserts and errors if the tracer needs to be improved.
Add the following GN arg to your Debug setup:
```
angle_with_capture_by_default = true
```

After [building](../../../doc/DevSetupAndroid.md#Build) and [installing](../../../doc/DevSetupAndroid.md#Install) the APK with the above arg, we're ready to start capturing.

## Determine the target app
We first need to identify which application we want to trace.  That can generally be done by looking at web based Play Store entry for your app, for instance Angry Birds 2 is listed here: https://play.google.com/store/apps/details?id=com.rovio.baba

If there is no Play Store entry for your app, you can guess at the package name, then check your device to see if it is installed. Keep trying combinations until you find it:
```
...
From here, you can start the application. You should see logcat entries like the folloiwng, indicating that we've succesfully turned on capturing:
```
...
```
Allow the app to run until the “*angledata.gz” file is non-zero:
```
...
```
pushd $CHROMIUM_SRC/third_party/angle/src/tests/restricted_traces
mkdir -p $LABEL
...
```
After uploading, the sha1 file you touched above will now be populated. Add that to your CL submit it with your changes.
