...
def upload_test_result_to_skia_gold(args, gold_session_manager, gold_session, gold_properties,
                                    screenshot_dir, image_name, artifacts, artifact_types):
    ...
    artifacts[artifact_name] = [png_file_name]
    artifact_types[artifact_name] = 'image/png'
    ...
    if status == status_codes.AUTH_FAILURE:
        ...
        if not triage_link:
            ...
            artifact_types['triage_link_for_entire_cl'] = 'text/plain'
        else:
            artifacts['gold_triage_link'] = [triage_link]
            artifact_types['gold_triage_link'] = 'text/plain'
        return False
        ...
        for test in tests['traces']:
            with common.temporary_file() as tempfile_path:
                ...
                if len(artifacts) > 0:
                    ...
                    results['artifact_types'].update(artifact_types)
