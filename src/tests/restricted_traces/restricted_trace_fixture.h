...
//
// restricted_trace_fixture.h:
//   Common code for the ANGLE restricted trace replays.
//

#ifndef ANGLE_RESTRICTED_TRACES_FIXTURE_H_
#define ANGLE_RESTRICTED_TRACES_FIXTURE_H_
...
ANGLE_REPLAY_EXPORT const uint8_t *GetSerializedContextState(uint32_t frameIndex);
}  // extern "C"

using SetBinaryDataDecompressCallbackFunc = decltype(&SetBinaryDataDecompressCallback);
using SetBinaryDataDirFunc                = decltype(&SetBinaryDataDir);
using SetupReplayFunc                     = decltype(&SetupReplay);
using ReplayFrameFunc                     = decltype(&ReplayFrame);
using ResetReplayFunc                     = decltype(&ResetReplay);
using FinishReplayFunc                    = decltype(&FinishReplay);
using GetSerializedContextStateFunc       = decltype(&GetSerializedContextState);
...
#endif  // ANGLE_RESTRICTED_TRACES_FIXTURE_H_
