...
# TODO(jmadill): Remove this function once all traces updated. FIXME bug link
def get_start_frame(trace):
    try:
        return int(trace.split('_')[-1])
    except:
        if trace == 'cod_mobile':
            return 1843
        elif trace == 'mobile_legends':
            return 5292
        else:
            raise Exception('Unknown trace: ' + trace)
...
def get_include_name(trace):
    return trace + '_include_autogen.h'


def gen_include_files(traces, args):

    for trace in traces:
        d = copy.deepcopy(args)
        d['lib_name'] = 'angel_restricted_trace_%s' % trace
        d['namespace'] = trace

        context = int(get_context(trace))
        start_frame = get_start_frame(trace)
        funcs = [
            'void SetupContext%dReplay();' % context,
            'void ReplayContext%dFrame(uint32_t frameIndex);' % context,
            'void ResetContext%dReplay();' % context,
            'void SetBinaryDataDecompressCallback(uint8_t *(*)(const std::vector<uint8_t> &));',
            'void SetBinaryDataDir(char const *dataDir);',
        ]
        for frame in range(start_frame, start_frame + 10):
            funcs += ['void ReplayContext%dFrame%d();' % (context, frame)]

        funcs = ['ANGLE_RESTRICTED_TRACE_EXPORT %s\n' % func for func in funcs]
        d['funcs'] = ''.join(funcs)
        txt = INCLUDE_FILE_TEMPLATE.format(**d)

        with open(get_include_name(trace), 'w') as f:
            f.write(txt)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        outputs = [gni_file, header_file, source_file, '.gitignore'
                  ] + [get_include_name(trace) for trace in traces]
    ...
    gen_include_files(traces, format_args)
