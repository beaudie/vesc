...
CIPD_PREFIX = 'experimental/google.com/jmadill/angle/traces'
LOG_LEVEL = 'info'
...
def main(args):
    ...
    for trace in traces['traces']:
        ...
        if cipd('describe', trace_name, '-version', 'version:%d' % trace_version) == 0:
            logging.info('%s already present', trace)
            continue

        logging.info('%s missing. calling create.')
        trace_folder = os.path.join(SCRIPT_DIR, trace)
        if cipd('create', '-name', trace_name, '-in', trace_folder, '-tag',
                'version:%d' % trace_version, '-log-level', args.log.lower()) != 0:
            logging.error('%s create failed')
            return 1
