...
def upload_trace(args, trace, trace_version):
    ...
    cipd_args.extend(['-in', trace_folder])
    cipd_args.extend(['-tag', 'version:%s' % cipd_trace_version])
    cipd_args.extend(['-log-level', args.log.lower()])
    cipd_args.extend(['-install-mode', 'copy'])
