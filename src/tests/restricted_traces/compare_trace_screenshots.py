...
* `versus_native`
  ...
  This mode expects to be run in a directory full of two sets of screenshots
    ...
    pushd /tmp/screenshots
    python3 compare_trace_screenshots.py versus_native --trace-list-path ~/angle/src/tests/restricted_traces/

* `versus_upgrade`

  This mode expects to be pointed to two directories of identical images
...
def versus_native(args):
    ...
    png_files = os.listdir(working_dir)
    ...
    def get_trace_key_frame(restricted_traces_path, trace):
        with open(os.path.join(restricted_traces_path, trace, trace + ".json")) as f:
            single_trace_data = json.loads(f.read())
    ...
    for trace in sorted(traces):
        ...
        for fuzz in {0, 1, 2, 5, 10, 20}:
            ...
            diff = subprocess.run(
                command, shell=True,
                capture_output=True)  #stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            ...
            #print(" for " + trace + " " + str(fuzz) + "%")

        print(trace, vulkan_file, native_file, results[0].decode('UTF-8'),
              results[1].decode('UTF-8'), results[2].decode('UTF-8'), results[3].decode('UTF-8'),
              results[4].decode('UTF-8'), results[5].decode('UTF-8'))
