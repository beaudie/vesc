...
BOOL GL_APIENTRY wglSwapBuffers(HDC hdc)
{
    ...
    ANGLE_EGL_TRY_RETURN(thread, display->getSurface()->swap(thread->getContext()),
                         "wglSwapBuffers", display->getSurface(), FALSE);
    return TRUE;
}
...
HGLRC GL_APIENTRY wglCreateContext(HDC hDc)
{
    ...
    return dynamic_cast<HGLRC>(context);
}
...
BOOL GL_APIENTRY wglMakeCurrent(HDC hDc, HGLRC newContext)
{
    ...
    ANGLE_EGL_TRY_RETURN(
        thread, display->makeCurrent(thread, display->getSurface(), display->getSurface(), context),
