...
// BlockLayoutEncoder implementation.
BlockLayoutEncoderMTL::BlockLayoutEncoderMTL() : mCurrentOffset(0) {}
...
void BlockLayoutEncoderMTL::getBlockLayoutInfo(GLenum type,
                                            ...
                                            int *matrixStrideOut)
{
    size_t componentBytes = mtl::GetMetalSizeForGLType(gl::VariableComponentType(type);
    ...
    if (gl::IsMatrixType(type))
    {
        ...
        matrixStride  = mtl::GetMetalSizeForGLType(type);

        if (!arraySizes.empty())
        {
            arrayStride = mtl::GetMetalSizeForGLType(type);
        }
    }
    else if (!arraySizes.empty())
    {
        baseAlignment = static_cast<int>(mtl::GetMetalAlignmentForGLType(type, false));
        arrayStride   = static_cast<int>(mtl::GetMetalSizeForGLType(type, false));
    }
    else
    {
        baseAlignment              =  mtl::GetMetalAlignmentForGLType(type);
                                               ...
                                               bool isRowMajorMatrix)
{
    ...
    const BlockMemberInfo memberInfo(static_cast<int>(mCurrentOffset),
                                     static_cast<int>(arrayStride),
                                     static_cast<int>(matrixStride),
                                     isRowMajorMatrix);
...
size_t BlockLayoutEncoderMTL::getShaderVariableSize(const ShaderVariable &structVar, bool isRowMajor)
{
    size_t currentOffset = mCurrentOffset;
    mCurrentOffset       = 0;
    BlockEncoderVisitor visitor("", "", this);
...
void BlockLayoutEncoderMTL::align(size_t baseAlignment)
{
    angle::base::CheckedNumeric<size_t> checkedOffset(mCurrentOffset);
    checkedOffset += baseAlignment;
    checkedOffset -= 1;
