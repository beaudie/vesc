    ...
    for (unsigned int arrayElement = 0u; arrayElement < count; ++arrayElement)
    {
        ...
        else
        {
            if (gl::IsSamplerType(variable.type) || gl::IsImageType(variable.type) ||
                gl::IsSubpassInputType(variable.inputAttachmentIndex))
            {
                visitor->visitSamplerOrImageOrSubpassInput(elementVar);
            }
            else
            {
...
void VariableNameVisitor::visitSamplerOrImageOrSubpassInput(const sh::ShaderVariable &variable)
{
    ...
    visitNamedSamplerOrImageOrSubpassInput(variable, name, mappedName, mArraySizeStack);
}
                            ...
                            ShaderVariableVisitor *visitor)
{
    ...
    else if (variable.isArrayOfArrays())
    {
        TraverseArrayOfArraysVariable(variable, 0u, isRowMajor, visitor);
             ...
             gl::IsSubpassInputType(variable.inputAttachmentIndex))
    {
        visitor->visitSamplerOrImageOrSubpassInput(variable);
    }
    else
    {
