...
template <int size>
float MatrixDeterminant(const float *m);

template<>
float MatrixDeterminant<2>(const float *m)
{
    return m[0] * m[3] - m[1] * m[2];
}

template<>
float MatrixDeterminant<3>(const float *m)
{
    return m[0] * m[4] * m[8] +
           m[1] * m[5] * m[6] +
           m[2] * m[3] * m[7] -
...
template <>
float MatrixDeterminant<4>(const float *m)
{
    ...
    return m[0] * MatrixDeterminant<3>(minorMatrices[0]) -
           m[1] * MatrixDeterminant<3>(minorMatrices[1]) +
           m[2] * MatrixDeterminant<3>(minorMatrices[2]) -
           m[3] * MatrixDeterminant<3>(minorMatrices[3]);
}
        ...
        switch (op)
        {
          ...
          case EOpDeterminant:
            if (getType().getBasicType() == EbtFloat)
            {
                ...
                {
                  case 2:
                    det = MatrixDeterminant<2>(matrix);
                    break;
                  case 3:
                    det = MatrixDeterminant<3>(matrix);
                    break;
                  case 4:
                    det = MatrixDeterminant<4>(matrix);
                    break;
                  default:
                    UNREACHABLE();
                    break;
                }
                resultArray->setFConst(det);
