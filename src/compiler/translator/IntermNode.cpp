...
const TConstantUnion *TIntermAggregate::getConstantValue() const
{
    ...
    {
        TIntermTyped *argumenTyped                  = argument->getAsTyped();
        size_t argumentSize                         = argumenTyped->getType().getObjectSize();
        const TConstantUnion *argumentConstantValue = argumenTyped->getConstantValue();
...
TIntermTyped *TIntermBinary::fold(TDiagnostics *diagnostics)
{
    ...
    switch (mOp)
    {
        ...
        case EOpIndexDirectStruct:
        {
            ...
            if (!getType().canReplaceWithConstantUnion() && !mLeft->getAsConstantUnion())
            {
                return this;
            }
...
const TConstantUnion *TIntermBinary::getConstantValue() const
{
    ...
    size_t index = static_cast<size_t>(mRight->getConstantValue()->getIConst());
