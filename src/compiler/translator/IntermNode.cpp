    ...
    else
    {
        ...
        for (size_t i = 0; i < objectSize; i++)
        {
            switch(op)
            {
              ...
              case EOpSin:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &sin, infoSink))
                    return nullptr;
                break;

              case EOpCos:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &cos, infoSink))
                    return nullptr;
                break;

              case EOpTan:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &tan, infoSink))
                    return nullptr;
                break;

              case EOpAsin:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &asin, infoSink))
                    return nullptr;
                break;

              case EOpAcos:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &acos, infoSink))
                    return nullptr;
                break;

              case EOpAtan:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &atan, infoSink))
                    return nullptr;
                break;

              case EOpSinh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &sinh, infoSink))
                    return nullptr;
                break;

              case EOpCosh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &cosh, infoSink))
                    return nullptr;
                break;

              case EOpTanh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &tanh, infoSink))
                    return nullptr;
                break;

              case EOpAsinh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &asinh, infoSink))
                    return nullptr;
                break;

              case EOpAcosh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &acosh, infoSink))
                    return nullptr;
                break;

              case EOpAtanh:
                if (!foldFloatTypeUnary(tempConstArray[i], unionArray[i], &atanh, infoSink))
                    return nullptr;
                break;
