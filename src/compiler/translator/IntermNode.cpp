...
TOperator TIntermBinary::GetMulAssignOpBasedOnOperands(const TType &left, const TType &right)
{
    if (!left.isMatrix() && right.isMatrix())
    {
        // Left should be vector, but this may not be validated yet.
    ...
    else if (!left.isMatrix() && !right.isMatrix())
    {
        ...
        {
            // left should be vector and right should be scalar, but this may not be validated yet.
...
bool TIntermBinary::promote()
{
    ...
    {
        case EOpMul:
            ASSERT(mOp == GetMulOpBasedOnOperands(mLeft->getType(), mRight->getType()));
            break;
        case EOpMatrixTimesScalar:
            ASSERT(mOp == GetMulOpBasedOnOperands(mLeft->getType(), mRight->getType()));
            if (mRight->isMatrix())
