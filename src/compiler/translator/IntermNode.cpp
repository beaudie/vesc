...
const TConstantUnion *TIntermConstantUnion::foldSubscript(int index)
{
    if (isArray())
    {
...
TIntermTyped *TIntermBinary::fold(TDiagnostics *diagnostics)
{
    ...
    switch (mOp)
    {
        case EOpIndexDirect:
        {
            ...
            const TConstantUnion *constArray = leftConstant->foldSubscript(index);
        ...
        case EOpVectorSwizzle:
        {
            ...
            for (size_t i = 0; i < numFields; i++)
            {
                ...
                constArray[i]   = *leftConstant->foldSubscript(fieldOffset);
            }
            return CreateFoldedNode(constArray, this, mType.getQualifier());
        }
        default:
...
TIntermTyped *TIntermUnary::fold(TDiagnostics *diagnostics)
{
    ...
    switch (mOp)
    {
      ...
      case EOpPackHalf2x16:
      case EOpUnpackHalf2x16:
          constArray = operandConstant->foldUnaryWithDifferentReturnType(mOp);
          break;
      default:
          constArray = operandConstant->foldUnaryWithSameReturnType(mOp, diagnostics);
          break;
    }
...
// Returns the constant value to keep using or nullptr.
//
TConstantUnion *TIntermConstantUnion::foldUnaryWithDifferentReturnType(TOperator op)
{
...
TConstantUnion *TIntermConstantUnion::foldUnaryWithSameReturnType(TOperator op,
                                                                  TDiagnostics *diagnostics)
{
