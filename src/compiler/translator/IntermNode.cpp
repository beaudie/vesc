...
const float kDegreesToRadiansMultiplier = static_cast<float>(kPi / 180.0);
const float kRadiansToDegreesMultiplier = static_cast<float>(180.0 / kPi);
              ...
              case EOpRoundEven:
                if (getType().getBasicType() == EbtFloat)
                {
                    ...
                    if (abs(fractPart) == 0.5)
                        result = static_cast<float>(2.0 * round(x / 2.0));
                    else
                        result = round(x);
