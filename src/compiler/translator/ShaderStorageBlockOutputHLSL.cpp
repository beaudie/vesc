...
bool IsInArrayOfArray(TIntermTyped *node)
{
                                                                       ...
                                                                       TIntermBinary *node)
{
    ...
    if (visit == InVisit)
    {
        ...
        // For array of arrays, we calculate the offset using below formula:
        // arrayStride * (
        // a[0] * a[1] * a[2] *... * a[n-2] * curIndex + a[1] * a[2] *... * a[n-2] * curIndex +
        // a[2] *... * a[n-2] * curIndex + ... + a[n-2] * curIndex)
        // Note: a is *type.getArraySizes(), curIndex is the current dimension index.
        // For example,
        // layout(std140, binding = 0) buffer block
        // {
        //    S s[3][2];
        // }
        // s[1][0] will be translated as  48 * (2 * 1 + 0). Assume that the structure stride is 48.
        if (IsInArrayOfArray(node->getLeft()))
        {
            if (type.isArrayOfArrays())
            {
                const TVector<unsigned int> &arraySizes = *type.getArraySizes();
                // Don't need to concern the trail comma which will be used to multiply the index.
    ...
    else if (visit == PostVisit)
    {
        ...
        // true.
        if (IsInArrayOfArray(node->getLeft()) && !node->getType().isArray())
        {
            out << ")";
        }
