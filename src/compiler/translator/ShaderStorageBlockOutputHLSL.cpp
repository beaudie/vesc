...
void ShaderStorageBlockOutputHLSL::traverseSSBOAccess(TIntermTyped *node,
                                                      ...
                                                      bool rowMajor)
{
    // If the current node's type is vector which comes from a matrix, we should calculate the
    // matrix stride and row major and pass those values to registerShaderStorageBlockFunction.
    // Note that we haven't get correct BlockMemberInfo from mBlockMemberInfoMap in current
    // point. But we must use those information to generate the function name and the function body.
    // So here we have to calculate them again.
