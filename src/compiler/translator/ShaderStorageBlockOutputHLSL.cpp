                                                  ...
                                                  bool *isRowMajor) const
{
    ...
    if (node->getType().isVector())
    {
        ...
        if (binaryNode)
        {
            return getMatrixStride(binaryNode->getLeft(), storage, rowMajor, isRowMajor);
        }
        else
        {
            TIntermSwizzle *swizzleNode = node->getAsSwizzleNode();
            if (swizzleNode)
            {
                return getMatrixStride(swizzleNode->getOperand(), storage, rowMajor, isRowMajor);
            }
        }
    }
    return 0;
...
void ShaderStorageBlockOutputHLSL::traverseSSBOAccess(TIntermTyped *node, SSBOMethod method)
{
    ...
    int matrixStride = getMatrixStride(node, storage, rowMajor, &isRowMajor);

    const TString &functionName = mSSBOFunctionHLSL->registerShaderStorageBlockFunction(
        node->getType(), method, storage, isRowMajor, matrixStride, unsizedArrayStride,
