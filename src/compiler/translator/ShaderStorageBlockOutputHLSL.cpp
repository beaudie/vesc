...
void ShaderStorageBlockOutputHLSL::outputStoreFunctionCallPrefix(TIntermBinary *node)
{
    mBufferVariableInAssignLeft = true;
    traverseAssignmentInternal(node->getLeft(), SSBOMethod::STORE);
}

void ShaderStorageBlockOutputHLSL::outputLoadFunctionCall(TIntermBinary *node)
{
    mBufferVariableInAssignLeft = false;
    traverseAssignmentInternal(node->getRight(), SSBOMethod::LOAD);
}

void ShaderStorageBlockOutputHLSL::traverseAssignmentInternal(TIntermTyped *node, SSBOMethod method)
{
