                                          ...
                                          BlockMemberInfoMap *blockInfoOut)
{
    for (const TField *field : fields)
    {
        ...
        if (fieldType.getStruct())
        {
            ...
            {
                unsigned int numElements = variable->getArraySizeProduct();
                for (unsigned int elementIndex = 0; elementIndex < numElements; ++elementIndex)
                {
                    ...
                    GetShaderStorageBlockFieldMemberInfo(structure->fields(), fieldToVarMap,
                                                         encoder, storage, isRowMajorLayout, false,
                                                         blockInfoOut);

                    BlockMemberInfo memberInfo = encoder->exitAggregateType(isRowMajorLayout);

                    if (blockInfoOut->count(field) == 0)
                    {
            ...
            {
                encoder->enterAggregateType(variable->fields.data(), variable->fields.size());
                GetShaderStorageBlockFieldMemberInfo(structure->fields(), fieldToVarMap, encoder,
                                                     storage, isRowMajorLayout, false,
                                                     blockInfoOut);
        ...
        else if (fieldType.isArrayOfArrays())
        {
            ...
            encoder->enterAggregateType(variable, 1);
            for (size_t arraySizeIndex = 0; arraySizeIndex < arraySizes.size() - 1;
                 ++arraySizeIndex)
            {
                const unsigned int arraySize = arraySizes[arraySizeIndex];
            ...
            encoder->exitAggregateType(isRowMajorLayout);
        }
        else
        {
...
void MapFieldToVariable(const TField *field,
