                        ...
                        bool rowMajorAlreadyAssigned)
{
    ...
    if (swizzleNode)
    {
        GetBlockLayoutInfo(swizzleNode->getOperand(), storage, rowMajor, rowMajorAlreadyAssigned);
    }
        ...
        switch (binaryNode->getOp())
        {
            case EOpIndexDirectInterfaceBlock:
            {
                ...
                *rowMajor         = type.getLayoutQualifier().matrixPacking == EmpRowMajor;
                return GetBlockLayoutInfo(binaryNode->getLeft(), storage, rowMajor, true);
            }
            case EOpIndexIndirect:
            case EOpIndexDirect:
            case EOpIndexDirectStruct:
                return GetBlockLayoutInfo(binaryNode->getLeft(), storage, rowMajor,
    ...
    *storage = interfaceBlock->blockStorage();
    // If the block doesn't have an instance name, rowMajorAlreadyAssigned will be false. In this
    // situation, we still need to set rowMajor's value.
...
void ShaderStorageBlockOutputHLSL::traverseSSBOAccess(TIntermTyped *node, SSBOMethod method)
{
    ...
    TLayoutBlockStorage storage;
    bool rowMajor;
    GetBlockLayoutInfo(node, &storage, &rowMajor, false);
