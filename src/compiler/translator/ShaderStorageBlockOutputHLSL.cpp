                                        ...
                                        const std::string &prefix)
{
    ...
    SetShaderStorageBlockFieldMemberInfo(interfaceBlock->fields(), prefix, encoder, false);
}
    ...
    else if (qualifier == EvqBuffer)
    {
        ...
        if (mReferencedShaderStorageBlocks.count(interfaceBlock->uniqueId().get()) == 0)
        {
            ...
            if (variableType.isInterfaceBlock())
            {
                ...
                SetShaderStorageBlockMembersOffset(interfaceBlock, interfaceBlock->name().data());
            }
            else
            {
