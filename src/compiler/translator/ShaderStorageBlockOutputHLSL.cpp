                                                  ...
                                                  bool rowMajor) const
{
    ...
    if (node->getType().isVector())
    {
        ...
        if (binaryNode)
        {
            return getMatrixStride(binaryNode->getLeft(), storage, rowMajor);
        }
        else
        {
            TIntermSwizzle *swizzleNode = node->getAsSwizzleNode();
...
void ShaderStorageBlockOutputHLSL::traverseSSBOAccess(TIntermTyped *node, SSBOMethod method)
{
    ...
    int matrixStride = getMatrixStride(node, storage, rowMajor);

    const TString &functionName = mSSBOFunctionHLSL->registerShaderStorageBlockFunction(
        node->getType(), method, storage, rowMajor, matrixStride, unsizedArrayStride,
        node->getAsSwizzleNode());
    TInfoSinkBase &out = mOutputHLSL->getInfoSink();
    out << functionName;
    out << "(";
    BlockMemberInfo blockMemberInfo;
    TIntermNode *loc = createByteAddressExpression(out, node, &blockMemberInfo);
    out << ", ";
...
const TField *ShaderStorageBlockOutputHLSL::findBaseField(TIntermTyped *node) const
{
    if (TIntermSwizzle *swizzleNode = node->getAsSwizzleNode())
    {
        return findBaseField(swizzleNode);
    }
    else if (TIntermBinary *binaryNode = node->getAsBinaryNode())
    {
        switch (binaryNode->getOp())
        {
            ...
            case EOpIndexIndirect:
            {
                return findBaseField(binaryNode->getLeft());
                break;
            }
            case EOpIndexDirectStruct:
            {
                ...
                const TIntermConstantUnion *index = binaryNode->getRight()->getAsConstantUnion();
                return structure->fields()[index->getIConst(0)];
                break;
            }
            case EOpIndexDirectInterfaceBlock:
            {
                const TInterfaceBlock *interfaceBlock =
                    binaryNode->getLeft()->getType().getInterfaceBlock();
                const TIntermConstantUnion *index = binaryNode->getRight()->getAsConstantUnion();
                return interfaceBlock->fields()[index->getIConst(0)];
                break;
            }
            default:
                return nullptr;
    ...
    else if (TIntermSymbol *symbolNode = node->getAsSymbolNode())
    {
        const TVariable &variable = symbolNode->variable();
        ...
        {
            const TInterfaceBlock *interfaceBlock = type.getInterfaceBlock();
            return GetFieldMemberInShaderStorageBlock(interfaceBlock, variable.name());
        }
    }
    return nullptr;
}

TIntermTyped *ShaderStorageBlockOutputHLSL::createByteAddressExpression(
    TInfoSinkBase &out,
    TIntermTyped *node,
    BlockMemberInfo *blockMemberInfo)
{
    if (TIntermSwizzle *swizzleNode = node->getAsSwizzleNode())
    {
        return createByteAddressExpression(out, swizzleNode->getOperand(), blockMemberInfo);
    }
    else if (TIntermBinary *binaryNode = node->getAsBinaryNode())
    {
        switch (binaryNode->getOp())
        {
            case EOpIndexDirect:
            {
                ...
                {
                    return convertEOpIndexDirectOrIndirectOutput(out, binaryNode, blockMemberInfo);
                }
                break;
            }
            case EOpIndexIndirect:
            ...
            case EOpIndexDirectStruct:
            {
                ...
                ASSERT(binaryNode->getLeft()->getBasicType() != EbtInterfaceBlock);
                TIntermTyped *left =
                    createByteAddressExpression(out, binaryNode->getLeft(), blockMemberInfo);
                const TStructure *structure       = binaryNode->getLeft()->getType().getStruct();
                const TIntermConstantUnion *index = binaryNode->getRight()->getAsConstantUnion();
    ...
    BlockMemberInfo *blockMemberInfo)
{
    ...
    TIntermTyped *left  = createByteAddressExpression(out, node->getLeft(), blockMemberInfo);
