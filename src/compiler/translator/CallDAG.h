...
class CallDAG
{
  public:
    ...
    // Returns false if it was not able to create the DAG, signals initialization
    // errors in info, it present.
    InitResult init(TIntermNode *root, TInfoSinkBase *info);

    // Returns -1 if the function wasn't found
    int getIndex(const TIntermAggregate *function) const;
  ...
  private:
    ...
    std::map<int, int> mUniqueIdToIndex;
