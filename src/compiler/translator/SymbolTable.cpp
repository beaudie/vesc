...
{
    return insertVariable(currentLevel(), name, type, SymbolType::USER_DEFINED);
}
...
TVariable *TSymbolTable::insertVariable(ESymbolLevel level, const char *name, const TType &type)
{
    ...
    return insertVariable(level, NewPoolTString(name), type, SymbolType::BUILT_IN);
}
...
TVariable *TSymbolTable::insertVariableExt(ESymbolLevel level,
                                           ...
                                           const TType &type)
{
    TVariable *var = new TVariable(this, NewPoolTString(name), type, SymbolType::BUILT_IN, ext);
    if (insert(level, var))
    {
        if (var->getType().getBasicType() == EbtStruct)
        {
                                 ...
                                 const TType *ptype5)
{
    ...
    {
        TFunction *function =
            new TFunction(this, NewPoolTString(name), rvalue, SymbolType::BUILT_IN, op, ext);
...
{
    insertUnmangledBuiltInName(name, level);
    insert(level, new TFunction(this, NewPoolTString(name), rvalue, SymbolType::BUILT_IN, op));
}
...
{
    insertUnmangledBuiltInName(name, level);
    insert(level, new TFunction(this, NewPoolTString(name), rvalue, SymbolType::BUILT_IN, op, ext));
}
