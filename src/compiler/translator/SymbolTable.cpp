...
class TSymbolTable::TSymbolTableLevelBase
{
  public:
    TSymbolTableLevelBase() = default;
    virtual ~TSymbolTableLevelBase() {}
...
class TSymbolTable::TSymbolTableLevel : public TSymbolTable::TSymbolTableLevelBase
{
  public:
...
class TSymbolTable::TSymbolTableBuiltInLevel : public TSymbolTable::TSymbolTableLevelBase
{
  public:
...
{
    const TSymbol *symbol = nullptr;

    int userDefinedLevel = currentLevel() - LAST_BUILTIN_LEVEL - 1;
    while (userDefinedLevel >= 0)
    {
        symbol = table[userDefinedLevel]->find(name);
        if (symbol)
        {
            return symbol;
        }
...
TSymbolTable::TSymbolTableLevelBase *TSymbolTable::getCurrentLevel() const
{
    ESymbolLevel level = currentLevel();
    if (level <= LAST_BUILTIN_LEVEL)
    {
        return mBuiltInTable[level];
    }
    return table[level - LAST_BUILTIN_LEVEL - 1];
}
...
void TSymbolTable::addInvariantVarying(const std::string &originalName)
{
    ...
    getCurrentLevel()->addInvariantVarying(originalName);
}

bool TSymbolTable::isVaryingInvariant(const std::string &originalName) const
{
    ASSERT(atGlobalLevel());
    return getCurrentLevel()->isVaryingInvariant(originalName);
}

void TSymbolTable::setGlobalInvariant(bool invariant)
{
    ASSERT(atGlobalLevel());
    getCurrentLevel()->setGlobalInvariant(invariant);
}
