    ...
    std::string debugString)
{
    for (const ShaderVariable &shaderVar : shaderVars)
    {
        ...
        if (shaderVar.isBuiltIn())
        {
            ...
            ImmutableStringBuilder globalStructVar(builtinReplacement.length() + 3 +
                                                   globalType.length() + 1);
            globalStructVar << builtinReplacement;
            globalStructVar << " : ";
            globalStructVar << globalType;
            globalStructVar << ",";
                    ...
                    std::get_if<BuiltinAnnotation>(&wgslName.wgslPipelineAnnotation))
            {
                ...
                ImmutableStringBuilder annotatedStructVar(
                    strlen(builtinAnnotationStart) + builtinAnnotation->wgslBuiltinName.length() +
                    strlen(builtinAnnotationEnd) + builtinReplacement.length() + 3 +
                    wgslName.wgslBuiltinType.length() + 1);
                annotatedStructVar << builtinAnnotationStart << builtinAnnotation->wgslBuiltinName
                                   << builtinAnnotationEnd;
                annotatedStructVar << builtinReplacement;
                annotatedStructVar << " : ";
                annotatedStructVar << wgslName.wgslBuiltinType;
                annotatedStructVar << ",";
                         ...
                         std::get_if<LocationAnnotation>(&wgslName.wgslPipelineAnnotation))
            {
                ...
                ImmutableStringBuilder annotatedStructVar(strlen(locationAnnotationStr) +
                                                          builtinReplacement.length() + 3 +
                                                          wgslName.wgslBuiltinType.length() + 1);
                annotatedStructVar << locationAnnotationStr;
                annotatedStructVar << builtinReplacement;
                annotatedStructVar << " : ";
                annotatedStructVar << wgslName.wgslBuiltinType;
                annotatedStructVar << ",";
            ...
            const size_t conversionFuncSize =
                wgslName.conversionFunc.empty()
                    ? 0
                    : wgslName.conversionFunc.length() + 2;  // space for parantheses.
            ImmutableStringBuilder conversion(toStruct.length() + 1 + builtinReplacement.length() +
                                              3 + conversionFuncSize + fromStruct.length() + 1 +
                                              builtinReplacement.length() + 1);
            conversion << toStruct;
            conversion << ".";
            conversion << builtinReplacement;
            conversion << " = ";
            if (!wgslName.conversionFunc.empty())
            {
                conversion << wgslName.conversionFunc << "(";
            }
            conversion << fromStruct;
            conversion << ".";
            conversion << builtinReplacement;
            if (!wgslName.conversionFunc.empty())
            {
        ...
        else
        {
            ...
            ImmutableStringBuilder globalStructVar(userVarName.length() + 3 + type.length() + 1);
            globalStructVar << userVarName;
            globalStructVar << " : ";
            globalStructVar << type.c_str();
            globalStructVar << ",";
            ImmutableString globalStructVarStr = globalStructVar;
            ioblock->angleGlobalMembers.push_back(globalStructVarStr);

            // E.g. `@location(@@@@@@) _uuserVar : i32,`.
            const char *locationAnnotationStr = "@location(@@@@@@) ";
            ImmutableStringBuilder annotatedStructVar(strlen(locationAnnotationStr) +
                                                      globalStructVarStr.length() + 1);
            annotatedStructVar << locationAnnotationStr;
            annotatedStructVar << globalStructVarStr;
            ioblock->angleAnnotatedMembers.push_back(annotatedStructVar);

            // E.g. `ANGLE_input_global._uuserVar = ANGLE_input_annotated._uuserVar;`
            ImmutableStringBuilder conversion(toStruct.length() + 1 + userVarName.length() + 3 +
                                              fromStruct.length() + 1 + userVarName.length() + 1);
            conversion << toStruct;
            conversion << ".";
            conversion << userVarName;
            conversion << " = ";
            conversion << fromStruct;
            conversion << ".";
            conversion << userVarName;
