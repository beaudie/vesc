...
// found in the LICENSE file.
//
// ImmutableString_ESSL_autogen.cpp: Wrapper for static or pool allocated char arrays
// that are guaranteed to be valid and unchanged for the duration of the compilation.
...
bool checkShaderType(GLenum expected, GLenum actual)
{{
    if(expected == NOT_GL_COMPUTE_SHADER)
        return actual != GL_COMPUTE_SHADER;

    // shaderType == 0: supported in all shaders
...
{{
    ...
    {{
        ...
        if(checkESSLVersion(entry.esslVersion, version))
        {{
            return entry.symbol ? entry.symbol : this->*(entry.var);
        }}
    }}
    if(entry.esslExtension && mResources.*(entry.esslExtension) &&
...
{{
    ...
    if(entry.name == name && checkESSLVersion(entry.esslVersion, version))
    {{
    ...
    def add_obj(self,
        ...
        if essl_extension != 'UNDEFINED':
            if (var_obj and 'essl_ext_var' in self.objs[name_hash] and
                ...
                # Adding a previously added variable with suffix (variable is part of two extensions)
                if essl_extension != 'UNDEFINED':
    ...
    def get_array(self):
        ...
        for hash_val in range(0, self.num_names):
            if hash_val in self.objs:
                ...
                shader_type = obj_data['shader_type'] if 'shader_type' in obj_data and obj_data[
                    'shader_type'] != 'NONE' else '0'
...
class UnmangledGroupedList:
    ...
    def get_array(self, unmangled_builtin_declarations):
        ...
        for hash_val in range(0, self.num_names):
            ...
            shader_type = obj_data['shader_type'] if obj_data['shader_type'] != 'NONE' else '0'
            data = []
            data.append('ImmutableString("{name}")'.format(name=obj_data['name']))
...
class TType:
    ...
    def parse_type(self, glsl_header_type):
        ...
        # Need to parse, right now treating it as a mat4x2
...
def main():
    ...
    test_filename = '../../tests/compiler_tests/ImmutableString_test_ESSL_autogen.cpp'
