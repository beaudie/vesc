...
constexpr bool operator==(ParamType paramType, const TType *ttype)
{
    return GetBasicType(paramType) == ttype->getBasicType() &&
           ttype->getNominalSize() == GetPrimarySize(paramType) &&
           ttype->getSecondarySize() == GetSecondarySize(paramType);
}
...
class FunctionId final
{
  ...
  private:
    friend constexpr bool operator==(const MiniFunctionId &miniId, const FunctionId &functionId);
    TOperator mOp;
...
constexpr bool operator==(const MiniFunctionId &miniId, const FunctionId &functionId)
{
...
using QueryFunc = const char *(const FunctionId &);

class StaticBuiltInFunctionMap final
{
  public:
    StaticBuiltInFunctionMap() : mQueryFunc(nullptr) {}

    StaticBuiltInFunctionMap(QueryFunc *queryFunc) : mQueryFunc(queryFunc) {}
    StaticBuiltInFunctionMap(const StaticBuiltInFunctionMap &other) = default;
    StaticBuiltInFunctionMap &operator=(const StaticBuiltInFunctionMap &other) = default;

    // Returns nullptr on failure.
    const char *find(const FunctionId &functionID) const { return mQueryFunc(functionID); }

  private:
    QueryFunc *mQueryFunc;
};
...
class BuiltInFunctionEmulator
{
  public:
    ...
    void addFunctionMap(StaticBuiltInFunctionMap &&map)
    {
        mStaticFunctionMaps.push_back(std::move(map));
    }
  ...
  private:
    ...
    std::vector<StaticBuiltInFunctionMap> mStaticFunctionMaps;
};
