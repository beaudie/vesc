...
{
    if (!t.mArraySizes.empty())
    {
        TVector<unsigned int> arraySizesCopy(t.mArraySizes.begin(), t.mArraySizes.end());
        mArraySizes = arraySizesCopy;
    }
}
...
TType &TType::operator=(const TType &t)
{
    ...
    primarySize        = t.primarySize;
    secondarySize      = t.secondarySize;
    mArraySizes        = TSpan<unsigned int>();
    mInterfaceBlock    = t.mInterfaceBlock;
    mStructure         = t.mStructure;
    mIsStructSpecifier = t.mIsStructSpecifier;
    mMangledName       = t.mMangledName;
...
void TType::sizeUnsizedArrays(const TSpan<unsigned int> &newArraySizes)
{
...
void TType::makeIdenticalArrays(const TType &t)
{
    const TSpan<unsigned int> &tArraySizes = t.getArraySizes();
    TVector<unsigned int> newSizes(tArraySizes.begin(), tArraySizes.end());

    makeArraysInternal(newSizes);
}

void TType::makeArraysInternal(const TSpan<unsigned int> &sizes)
{
    mArraySizes = sizes;
    invalidateMangledName();
}
