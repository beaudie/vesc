    ...
    if (isStructureContainingSamplers())
    {
        if (isArray())
        {
            for (unsigned int arrayIndex = 0u; arrayIndex < getArraySize(); ++arrayIndex)
            {
                ...
                elementType.createSamplerSymbols(elementName.str(), elementApiName.str(), outputSymbols, outputSymbolsToAPINames);
            }
        }
        else
        {
            structure->createSamplerSymbols(namePrefix, apiNamePrefix,
                                      ...
                                      TMap<TIntermSymbol *, TString> *outputSymbolsToAPINames) const
{
    ...
    for (auto &field : *mFields)
    {
        ...
        if (IsSampler(fieldType->getBasicType()) || fieldType->isStructureContainingSamplers())
        {
            ...
            fieldType->createSamplerSymbols(fieldName, fieldApiName, outputSymbols, outputSymbolsToAPINames);
        }
    }
}
