...
void TType::makeArray(unsigned int s)
{
    ...
    makeArraysInternal(*mArraySizesStorage);
}
...
{
    const TSpan<const unsigned int> &tArraySizes = t.getArraySizes();

    if (mArraySizesStorage == nullptr)
    {
        mArraySizesStorage = new TVector<unsigned int>();
    }
    mArraySizesStorage->assign(tArraySizes.begin(), tArraySizes.end());
    makeArraysInternal(*mArraySizesStorage);
}

void TType::makeArraysInternal(const TSpan<const unsigned int> &sizes)
{
    mArraySizes = sizes;
    invalidateMangledName();
}
...
void TType::toArrayBaseType()
{
    ...
    makeArraysInternal(TSpan<const unsigned int>());
}
