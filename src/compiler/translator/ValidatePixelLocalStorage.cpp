  ...
  public:
    ...
    bool visitDeclaration(Visit visit, TIntermDeclaration *decl) override
    {
        ...
        if (declType.isArray())
        {
            ...
            error(declVariable->getLine(),
                  "pixel local storage handles cannot be aggregated in arrays", "array");
        }
        else if (binding < 0)
        {
            error(declVariable->getLine(), "pixel local storage requires a binding index",
                  "layout qualifier");
        }
        // TODO(anglebug.com/7279): else if (binding >= GL_MAX_LOCAL_STORAGE_PLANES_ANGLE) ...
        else if (mPLSBindings.find(binding) != mPLSBindings.end())
        {
        ...
        // Validate the format layout qualifier.
        switch (auto format = declType.getLayoutQualifier().imageInternalFormat)
        {
            case EiifRGBA32F:
            case EiifRGBA16F:
            case EiifR32F:
            case EiifRGBA8:
                if (declType.getBasicType() != EbtPixelLocalANGLE)
                {
                    error(declVariable->getLine(),
                          "pixel local storage format requires pixelLocalANGLE",
                          getImageInternalFormatString(format));
                }
                break;
            case EiifRGBA16I:
            case EiifRGBA8I:
                if (declType.getBasicType() != EbtIPixelLocalANGLE)
                {
                    error(declVariable->getLine(),
            ...
            case EiifRGBA8UI:
            case EiifR32UI:
                if (declType.getBasicType() != EbtUPixelLocalANGLE)
                {
                    error(declVariable->getLine(),
            ...
            case EiifRGBA32I:
            default:
                error(declVariable->getLine(), "illegal pixel local storage format",
                      getImageInternalFormatString(format));
                break;
            case EiifUnspecified:
                error(declVariable->getLine(), "pixel local storage requires a format specifier",
                      "layout qualifier");
                break;
        }
