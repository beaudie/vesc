...
void RegenerateStructNames::visitSymbol(TIntermSymbol *symbol)
{
    ...
    if (mScopeDepth == 1)
    {
        ...
        // which will be different for the same struc in vertex/fragment
        // shaders.
        ...
        // no conflits among unmapped struct names from global scope and
        // mapped struc names from other scopes.
    ...
    TString tmp = kPrefix + TString(Str(uniqueId).c_str());
    tmp += "_" + userType->name();
...
bool RegenerateStructNames::visitAggregate(Visit, TIntermAggregate *aggregate)
{
    ...
    switch (aggregate->getOp())
    {
      case EOpSequence:
        ++mScopeDepth;
        for (TIntermSequence::const_iterator iter = aggregate->getSequence()->begin();
             iter != aggregate->getSequence()->end(); ++iter)
        {
