...
{
    insertStatementsInParentBlock(insertions, TIntermSequence());
}

void TIntermTraverser::insertStatementsInParentBlock(const TIntermSequence &insertionsBefore, const TIntermSequence &insertionsAfter)
{
    ASSERT(!mParentBlockStack.empty());
    NodeInsertMultipleEntry insert(mParentBlockStack.back().node, mParentBlockStack.back().pos, insertionsBefore, insertionsAfter);
    ...
    if (visit)
    {
        ...
        else
        {
            ...
            for (TIntermSequence::iterator sit = mSequence.begin(); sit != mSequence.end(); sit++)
            {
                ...
                // TODO: Clean this up by introducing a more generic mechanism for accessing built-in function signatures.
                if (mOp == EOpModf && sit != mSequence.begin())
