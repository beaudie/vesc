    ...
    : BuiltInFunctionEmulator()
{
    ...
    AddEmulatedFunction(EOpUnpackSnorm2x16, uint1,
        "float webgl_fromSnorm(in int x) {\n"
        "    return clamp(float(x) / 32767.0, -1.0, 1.0);\n"
        "}\n"
        "\n"
        "float2 webgl_unpackSnorm2x16_emu(in uint2 u) {\n"
        "    int y = (asint(u) >> 16);\n"
        "    int x = int(u & 0x7fffu) - int(u & 0x8000u);\n"
    ...
    AddEmulatedFunction(EOpUnpackUnorm2x16, uint1,
        ...
        "}\n"
        "\n"
        "float2 webgl_unpackUnorm2x16_emu(in uint2 u) {\n"
        "    uint y = (u >> 16);\n"
        "    uint x = u & 0xffffu;\n"
