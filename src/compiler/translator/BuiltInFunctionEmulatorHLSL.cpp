...
void InitBuiltInFunctionEmulatorForHLSL(BuiltInFunctionEmulator *emu)
{
    ...
    // TODO: This isn't returning expected results in dEQP for some reason. Might be due to handling
    // of negative x or something similar.
    emu->addEmulatedFunction(EOpFrexp, float1, int1,
                             "float webgl_frexp_emu(float x, out int exp)\n"
                             "{\n"
                             "    float fexp;\n"
                             "    float mantissa = frexp(x, fexp);\n"
                             "    exp = int(fexp);\n"
                             "    return mantissa;\n"
                             "}\n"
                             "\n");
