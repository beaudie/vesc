    ...
    : BuiltInFunctionEmulator()
{
    ...
    // transpose(c * r) where c is a column vector and r is a row vector.
    // This can be simplified by using the following formula:
    //   transpose(c * r) = transpose(r) * transpose(c)
    // since to get the transpose of r, we simply can build a column matrix
    // out of the original vector instead of a row matrix.
