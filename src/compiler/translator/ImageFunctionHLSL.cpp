                       ...
                       TString *imageReference)
{
    ...
    {
        HLSLSRVTextureGroup srvTextureGroup = SRVTextureGroup(imageFunction.image, imageFunction.imageInternalFormat);
	...
	{
	    HLSLUAVTextureGroup uavTextureGroup = UAVTextureGroup(imageFunction.image, imageFunction.imageInternalFormat);
...
void OutputImageLoadFunctionBody(TInfoSinkBase &out,
                                 ...
                                 const TString &imageReference)
{
    if (IsImage3D(imageFunction.image) || IsImage2DArray(imageFunction.image) || IsImageCube(imageFunction.image))
    {
        // "depth" stores either the number of layers in an array texture or 3D depth
        out << "    uint width; uint height; uint depth;\n"
            << "    " << imageReference << ".GetDimensions(width, height, depth);\n"
...
TString ImageFunctionHLSL::useImageFunction(const TString &name, const TBasicType &type, TLayoutImageInternalFormat imageInternalFormat, bool readOnly)
{
