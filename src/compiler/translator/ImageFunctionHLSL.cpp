                                     ...
                                     const ImageFunctionHLSL::ImageFunction &imageFunction)
{
    ...
    {
        switch (imageFunction.image.getBasicType())
        {
            case EbtImage2D:
            case EbtIImage2D:
            case EbtUImage2D:
...
{
    TString suffix = ResourceViewGroupSuffix(ResourceViewGroup(imageFunction.image));
    if (ResourceViewGroup(imageFunction.image) == HLSL_SRV_TEXTURE_2D_FLOAT4 ||
        ResourceViewGroup(imageFunction.image) == HLSL_UAV_TEXTURE_2D_FLOAT4)
    {
        *imageReference = TString("images") + suffix + "[imageIndex]";
    }
    else
    {
        out << "    const uint resourceViewIndex = imageIndex - imageIndexOffset" << suffix
            << ";\n";
        *imageReference = TString("images") + suffix + "[resourceViewIndex]";
    }
}
...
{
    if (IsImage3D(imageFunction.image.getBasicType()) ||
        IsImage2DArray(imageFunction.image.getBasicType()))
    {
        // "depth" stores either the number of layers in an array texture or 3D depth
        out << "    uint width; uint height; uint depth;\n"
            << "    " << imageReference << ".GetDimensions(width, height, depth);\n";
    }
    else if (IsImage2D(imageFunction.image.getBasicType()) ||
             IsImageCube(imageFunction.image.getBasicType()))
    {
                                  ...
                                  const TString &imageReference)
{
    if (IsImage3D(imageFunction.image.getBasicType()) ||
        IsImage2DArray(imageFunction.image.getBasicType()) ||
        IsImageCube(imageFunction.image.getBasicType()) ||
        IsImage2D(imageFunction.image.getBasicType()))
    {
...
TString ImageFunctionHLSL::ImageFunction::name() const
{
    ...
    name += ResourceViewGroupSuffix(ResourceViewGroup(image));
...
{
    return std::tie(image, imageInternalFormat, method) <
           std::tie(rhs.image, rhs.imageInternalFormat, rhs.method);
}
                                            ...
                                            sh::GLenum shaderType)
{
    ...
    imageFunction.imageInternalFormat = type.getLayoutQualifier().imageInternalFormat;
