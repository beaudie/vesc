...
sh::InterfaceBlockField *CollectVariables::findInterfaceBlockField(const TString &fieldName,
                                                                   const TInterfaceBlock &interfaceBlock)
{
    if (interfaceBlock.hasInstanceName())
    {
        sh::InterfaceBlock *namedBlock = FindVariable(interfaceBlock.instanceName(), mInterfaceBlocks);
        return FindVariable(fieldName, &namedBlock->fields);
    }

    for (size_t blockIndex = 0; blockIndex < mInterfaceBlocks->size(); blockIndex++)
    {
        sh::InterfaceBlock *interfaceBlock = &(*mInterfaceBlocks)[blockIndex];
        sh::InterfaceBlockField *field = FindVariable(fieldName, &interfaceBlock->fields);
...
void CollectVariables::visitSymbol(TIntermSymbol *symbol)
{
    ...
    {
        sh::InterfaceBlock *interfaceBlock = FindVariable(symbolName, mInterfaceBlocks);
        if (interfaceBlock)
        {
          ...
          case EvqUniform:
            {
                ...
                if (interfaceBlock)
                {
                    var = findInterfaceBlockField(symbolName, *interfaceBlock);
                }
                else
                {
