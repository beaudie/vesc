...
OutputVariable CollectVariables::recordOutputVariable(const TIntermSymbol *variable) const
{
    ...
    outputVariable.location = variable->getType().getLayoutQualifier().location;
...
bool CollectVariables::visitDeclaration(Visit, TIntermDeclaration *node)
{
    ...
    bool qualifierIsRecorded = qualifier == EvqAttribute || qualifier == EvqVertexIn ||
                               qualifier == EvqFragmentOut || qualifier == EvqUniform ||
                               IsVarying(qualifier);

    bool isRecorded = typedNode.getBasicType() == EbtInterfaceBlock || qualifierIsRecorded;
    ...
    if (isRecorded)
    {
        for (TIntermNode *variableNode : sequence)
        {
            ...
            // uniforms, varyings, outputs and interface blocks cannot be initialized in a shader,
            // we
