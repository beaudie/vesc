...
{
  public:
    Separator(TCompiler &compiler, bool separateAnonymousInterfaceStruct)
        : TIntermRebuild(compiler, true, true),
          mSeparateAnonymousInterfaceStruct(separateAnonymousInterfaceStruct)
    {}
  ...
  private:
    ...
    {
        // If we have replacements in the loop initializer, we must create a block for the loop and
        // do { init; loop }.
        mLoopBlocks.push_back(nullptr);
        return {node, VisitBits::Both};
    }

    PostResult visitLoopPost(TIntermLoop &node) override
    {
        TIntermBlock *declarations = mLoopBlocks.back();
        mLoopBlocks.pop_back();
        if (declarations == nullptr)
        {
            return node;
        }
        declarations->appendStatement(&node);
        return *declarations;
    }
        ...
        }
        {
            ...
            if (structure->symbolType() == SymbolType::Empty)
            {
                if (!mSeparateAnonymousInterfaceStruct)
                {
                    ...
                    // are fully separated, but GLSL output vertex and fragment shaders needs types
                    // that match so we cannot rewrite an anonymous struct to a stuct with a name.
                    if (IsShaderIn(declaratorType.getQualifier()) ||
    ...
    PostResult visitDeclarationPost(TIntermDeclaration &node) override
    {
        ...
        if (getParentNode()->getAsLoopNode() == nullptr)
        {
            return PostResult::Multi(std::move(replacements));
        }
        // Loop bodies are always blocks. If current parent is a loop, it must be the initializer.
        // Move struct specifiers and multiple declarations out of loop initializer.
        TIntermBlock *loopBlock = new TIntermBlock();
        for (TIntermNode *replacement : replacements)
        {
            loopBlock->appendStatement(replacement);
        }
        mLoopBlocks.back() = loopBlock;
        return nullptr;
    }
    ...
    std::vector<TIntermBlock *> mLoopBlocks;
...
bool SeparateDeclarations(TCompiler &compiler,
                          TIntermBlock &root,
                          bool separateAnonymousInterfaceStruct)
{
    Separator separator(compiler, separateAnonymousInterfaceStruct);
