...
const TVariable *GetBaseOpaqueUniform(TIntermTyped *node)
{
                                      ...
                                      VariableReplacementMap *argumentMapOut)
{
    ...
    for (size_t paramIndex = 0; paramIndex < original->getParamCount(); ++paramIndex)
    {
        ...
        else
        {
            ...
            // index (which is necessarily a symbol).  Replace the symbol in the argument with said
            // variable.  This is later used to replace the parameter in the function body.
...
{
  public:
    explicit MonomorphizeTraverser(TCompiler *compiler, TSymbolTable *symbolTable)
    ...
    bool visitFunctionDefinition(Visit visit, TIntermFunctionDefinition *node) override
    {
        ...
        mFunctionMap[function] = FunctionData{false, node, {}};
        return true;
    }
    ...
    const FunctionMap &getFunctionMap() { return mFunctionMap; }
  ...
  private:
                                                   ...
                                                   bool *isOriginalUsedOut)
    {
        ...
        for (size_t argIndex = 0; argIndex < callArguments.size(); ++argIndex)
        {
            ...
            if (!IsOpaqueType(funcArgument->getType().getBasicType()))
            {
                continue;
            }
            ...
            const TVariable *opaqueUniform = GetBaseOpaqueUniform(callArgument);
            if (opaqueUniform == nullptr)
            {
                continue;
            }
            ...
            //
            const TType &type = opaqueUniform->getType();
            const bool isArrayOfArrayOfSamplerOrImage =
                (type.isSampler() || type.isImage()) && type.isArrayOfArrays();
            const bool isParameterArrayOfSamplerOrImage = funcArgument->getType().isArray();
            const bool isAtomicCounter                  = type.isAtomicCounter();
            if (!((isArrayOfArrayOfSamplerOrImage && isParameterArrayOfSamplerOrImage) ||
                  isAtomicCounter))
            {
                continue;
            }
                                                  ...
                                                  TSymbolTable *symbolTable)
{
    while (true)
    {
        MonomorphizeTraverser monomorphizer(compiler, symbolTable);
        ...
        UpdateFunctionsDefinitionsTraverser functionUpdater(symbolTable,
                                                            monomorphizer.getFunctionMap());
