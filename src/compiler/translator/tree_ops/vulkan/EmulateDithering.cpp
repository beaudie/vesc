                              ...
                              uint32_t location)
{
    ...
    const TType *vec3Type      = StaticType::GetBasic<EbtFloat, EbpMedium, 3>();
    TIntermSymbol *ditherValue = new TIntermSymbol(CreateTempVariable(symbolTable, vec3Type));
    TIntermDeclaration *ditherValueDecl =
        CreateTempInitDeclarationNode(&ditherValue->variable(), CreateZeroNode(*vec3Type));
    ...
    //     ditherValue = vec3(ditherParam * 2)
    {
        ...
        TIntermAggregate *value =
            TIntermAggregate::CreateConstructor(*vec3Type, &ditherValueElements);
    ...
    TIntermTyped *fragmentOutputRGB = new TIntermSwizzle(fragmentOutput, {0, 1, 2});
    ditherBlock->appendStatement(new TIntermBinary(EOpAddAssign, fragmentOutputRGB, ditherValue));
}
