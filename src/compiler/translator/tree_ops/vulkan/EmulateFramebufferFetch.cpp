...
bool InputAttachmentUsageTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    if (!constIndex)
    {
        // If the shader code uses gl_LastFragData with a non-const index, assume all input
        // attachment indices may be used.
        mIndexUsage = InputAttachmentIndexUsage(angle::BitMask<uint32_t>(mMaxDrawBuffers));
        std::fill(mAttachmentTypes->begin(), mAttachmentTypes->end(), &type);
        return true;
    }
    else
    {
