...
bool OutputVariableTraverser::visitAggregate(Visit visit, TIntermAggregate *node)
{
    ...
    if (!subpassLoadFunc || subpassLoadFunc->symbolType() == SymbolType::BuiltIn ||
        subpassLoadFunc->getFunctionMangledName().contains("subpassLoad") == false)
    {
        return true;
    }
...
TIntermAggregate *CallSubpassLoadFunc(TSymbolTable *symbolTable,
                                      TIntermSymbol *inputAttachmentSymbol)
{
    TFunction *subpassLoadFunc =
        new TFunction(symbolTable, ImmutableString("subpassLoad"), SymbolType::AngleInternal,
                      new TType(EbtFloat, EbpUndefined, EvqGlobal, 4, 1), false);
    subpassLoadFunc->addParameter(&inputAttachmentSymbol->variable());

    TIntermSequence *argsForSubpassLoad = new TIntermSequence;
    argsForSubpassLoad->push_back(inputAttachmentSymbol->deepCopy());

    return TIntermAggregate::CreateFunctionCall(*subpassLoadFunc, argsForSubpassLoad);
}
                                      ...
                                      const int inputAttachmentIndex)
{
    ...
    TType *srcColorType           = new TType(outputType.getBasicType(), outputType.getPrecision(),
                                    EvqTemporary, outputType.getNominalSize());
...
TIntermTyped *DivideFloatNode(TIntermTyped *dividend, TIntermTyped *divisor)
{
    ...
    return new TIntermTernary(cond, CreateFloatNode(1.0f), divideExpr->deepCopy());
}
                                     ...
                                     const ImmutableString &premulName)
{
    ...
    TIntermBinary *isBlendAlphaZero =
        new TIntermBinary(EOpEqual, origVarA->deepCopy(), CreateFloatNode(0.0));
...
void BlendMainFunctionBuilder::createFunction()
{
    ...
    TType *funcSrcType = new TType(TBasicType::EbtFloat, EbpMedium, EvqIn, 4);
    TVariable *firstVar =
...
void BlendMainFunctionBuilder::calcAlphaCoeff()
{
    ...
    TIntermBinary *oneMinusDstAlpha =
        new TIntermBinary(EOpSub, CreateFloatNode(1.0), dstAlpha->deepCopy());
    ...
    TIntermBinary *oneMinusSrcAlpha =
        new TIntermBinary(EOpSub, CreateFloatNode(1.0), srcAlpha->deepCopy());
...
{
    TType *paramType = new TType(TBasicType::EbtFloat, EbpMedium, EvqTemporary, vectorSize);
                              ...
                              const size_t vectorSize)
{
    ...
    {
        constructorArgs.push_back(CreateFloatNode(0.0));
    }
    constructorArgs.push_back(CreateFloatNode(1.0));
                                         ...
                                         const AdvancedBlendEquation &blendEquation)
{
    ...
    {
        const int inputAttachmentIndex = iter->first;
