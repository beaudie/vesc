                                         ...
                                         const ImageMap &imageMap)
{
    ...
    bool castReturnValueToFloat         = false;

    if (functionName == "imageStore" || functionName == "imageAtomicExchange")
    {
        ...
        castReturnValueToFloat = functionName == "imageAtomicExchange";
    }
    else if (functionName == "imageLoad")
    {
        castReturnValueToFloat = true;
    }
    else
    {
    ...
    // uintBitsToFloat.  The alpha channel should always read 1.0 regardless.
    if (castReturnValueToFloat)
    {
        // imageLoad().rgb
        replacementCall = new TIntermSwizzle(replacementCall, {0, 1, 2});

        // uintBitsToFloat(imageLoad().rgb)
...
class RewriteR32fImagesTraverser : public TIntermTraverser
{
  public:
    ...
    {
        // Connot encounter the image symbol directly.  It can only be used with built-in functions,
