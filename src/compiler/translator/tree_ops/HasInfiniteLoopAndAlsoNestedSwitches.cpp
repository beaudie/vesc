...
//
// HasInfiniteLoopAndAlsoNestedSwitches.cpp: Infinite loops in the following form are detected:
//
// 1. while (true) { ... }
//
...
// the basic case.
//
// At the same time, there is a check for nested switch statements.
...
class CheckTraverser : public TIntermTraverser
{
  public:
    CheckTraverser(TSymbolTable *symbolTable, const VariableSet &constVariables)
        : TIntermTraverser(true, false, false, symbolTable),
          mConstVariables(constVariables),
          mHasInfiniteLoops(false),
          mIsInSwitch(false),
          mHasNestedSwitch(false)
    {}

    bool hasInfiniteLoops() const { return mHasInfiniteLoops; }
    bool hasNestedSwitchStatements() const { return mHasNestedSwitch; }
  ...
  private:
    ...
    bool mHasInfiniteLoops;

    bool mIsInSwitch;
    bool mHasNestedSwitch;
};

bool CheckTraverser::visitLoop(Visit visit, TIntermLoop *loop)
{
    onScopeBegin();
...
bool CheckTraverser::visitSwitch(Visit visit, TIntermSwitch *node)
{
    ...
    // Technically this is not accurate, as multiple nested switch statements aren't detected (a
    // counter would be needed for that), but that's irrelevant.  Detecting the existance of a
    // single nested switch is sufficient for this traverser.
    mIsInSwitch = false;
...
bool CheckTraverser::visitBranch(Visit visit, TIntermBranch *node)
{
    if (!mLoopStats.empty())
    {
...
bool HasInfiniteLoopAndAlsoNestedSwitches(TCompiler *compiler,
                                          TIntermBlock *root,
                                          TSymbolTable *symbolTable)
{
    ...
    CheckTraverser traverser(symbolTable, constVarTransverser.getConstVariables());
    root->traverse(&traverser);
    return traverser.hasInfiniteLoops() && traverser.hasNestedSwitchStatements();
}
