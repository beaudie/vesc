...
class RewriteCubeMapSamplersAs2DArrayTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    void declareCoordTranslationFunction()
    {
        ...
        TransformDerivativeXMajor(calculateXUcVc, mSymbolTable, x, y, z, dPdxX, dPdxY, dPdxZ, dUdx,
                                  dVdx);
        TransformDerivativeXMajor(calculateXUcVc, mSymbolTable, x, y, z, dPdyX, dPdyY, dPdyZ, dUdy,
                                  dVdy);
        ...
        TransformDerivativeYMajor(calculateYUcVc, mSymbolTable, x, y, z, dPdxX, dPdxY, dPdxZ, dUdx,
                                  dVdx);
        TransformDerivativeYMajor(calculateYUcVc, mSymbolTable, x, y, z, dPdyX, dPdyY, dPdyZ, dUdy,
                                  dVdy);
        ...
        TransformDerivativeZMajor(calculateZUcVc, mSymbolTable, x, y, z, dPdxX, dPdxY, dPdxZ, dUdx,
                                  dVdx);
        TransformDerivativeZMajor(calculateZUcVc, mSymbolTable, x, y, z, dPdyX, dPdyY, dPdyZ, dUdy,
                                  dVdy);
    ...
    void convertBuiltinFunction(TIntermAggregate *node)
    {
        ...
        else if (isTranslatedGrad && mIsFragmentShader)
        {
            dPdx = CreateBuiltInFunctionCallNode("dFdx", new TIntermSequence({(*arguments)[1]}),
                                                 *mSymbolTable, 100);
            dPdy = CreateBuiltInFunctionCallNode("dFdy", new TIntermSequence({(*arguments)[1]}),
                                                 *mSymbolTable, 100);
            if (arguments->size() == 3)
            {
                hasBias = true;
        ...
        if (hasBias)
        {
            ...
            dPdx = new TIntermBinary(EOpVectorTimesScalar, biasFac, dPdx);
            dPdy = new TIntermBinary(EOpVectorTimesScalar, biasFac, dPdy);
        }
