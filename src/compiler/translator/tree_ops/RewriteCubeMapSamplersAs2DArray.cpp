...
{
    // (du v - dv u) / v^2
    return new TIntermBinary(EOpMul,
                             new TIntermBinary(EOpSub, new TIntermBinary(EOpMul, du, v),
                                               new TIntermBinary(EOpMul, dv, u)),
                             new TIntermBinary(EOpMul, vRecip, vRecip));
}
                               ...
                               TIntermTyped *dv)
{
    ...
    TIntermSymbol *xRecip     = new TIntermSymbol(CreateTempVariable(symbolTable, floatType));
    TIntermTyped *xRecipValue = new TIntermBinary(EOpDiv, CreateFloatNode(1.0f), x);
    block->appendStatement(CreateTempInitDeclarationNode(&xRecip->variable(), xRecipValue));
    ...
    block->appendStatement(new TIntermBinary(EOpAssign, du, duValue));
    block->appendStatement(new TIntermBinary(EOpAssign, dv, dvValue));
}
...
void TransformImplicitDerivativeXMajor(TIntermBlock *block,
                                       ...
                                       TIntermTyped *dv)
{
    block->appendStatement(new TIntermBinary(EOpAssign, du->deepCopy(), IndexDirect(dOuter, 2)));
    block->appendStatement(new TIntermBinary(EOpAssign, dv->deepCopy(), IndexDirect(dOuter, 1)));
}
                               ...
                               TIntermTyped *dv)
{
    ...
    // and the negations.
    const TType *floatType    = StaticType::GetBasic<EbtFloat>();
    TIntermSymbol *yRecip     = new TIntermSymbol(CreateTempVariable(symbolTable, floatType));
    TIntermTyped *yRecipValue = new TIntermBinary(EOpDiv, CreateFloatNode(1.0f), y);
    block->appendStatement(CreateTempInitDeclarationNode(&yRecip->variable(), yRecipValue));
...
void TransformImplicitDerivativeYMajor(TIntermBlock *block,
                                       ...
                                       TIntermTyped *dv)
{
    block->appendStatement(new TIntermBinary(EOpAssign, du->deepCopy(), IndexDirect(dOuter, 0)));
    block->appendStatement(new TIntermBinary(EOpAssign, dv->deepCopy(), IndexDirect(dOuter, 2)));
}
                               ...
                               TIntermTyped *dv)
{
    ...
    // and the negations.
    const TType *floatType    = StaticType::GetBasic<EbtFloat>();
    TIntermSymbol *zRecip     = new TIntermSymbol(CreateTempVariable(symbolTable, floatType));
    TIntermTyped *zRecipValue = new TIntermBinary(EOpDiv, CreateFloatNode(1.0f), z);
    block->appendStatement(CreateTempInitDeclarationNode(&zRecip->variable(), zRecipValue));
...
class RewriteCubeMapSamplersAs2DArrayTraverser : public TIntermTraverser
{
  ...
  private:
    void declareSampler2DArray(const TVariable *samplerCubeVar, TIntermDeclaration *node)
    {
        ...
        if (mCubeXYZToArrayUVLImplicit == nullptr)
        {
                                         ...
                                         TIntermFunctionDefinition **declOut)
    {
        ...
        TIntermTyped *pRecip               = new TIntermBinary(EOpDiv, CreateFloatNode(0.5), p);
        TIntermDeclaration *recipOuterDecl = CreateTempInitDeclarationNode(
            &recipOuter->variable(),
            CreateBuiltInFunctionCallNode("outerProduct", new TIntermSequence({p, pRecip}),
                                          *mSymbolTable, 300));
        body->appendStatement(recipOuterDecl);

        TIntermSymbol *dPDXdx = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));
        TIntermSymbol *dPDYdx = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));
        TIntermSymbol *dPDZdx = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));
        TIntermSymbol *dPDXdy = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));
        TIntermSymbol *dPDYdy = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));
        TIntermSymbol *dPDZdy = new TIntermSymbol(CreateTempVariable(mSymbolTable, vec3Type));

        TFunction *dFdx = new TFunction(mSymbolTable, ImmutableString("dFdx"), SymbolType::BuiltIn,
                                        vec3Type, false);
        TFunction *dFdy = new TFunction(mSymbolTable, ImmutableString("dFdy"), SymbolType::BuiltIn,
                                        vec3Type, false);
        dFdx->addParameter(CreateTempVariable(mSymbolTable, vec3Type));
        dFdy->addParameter(CreateTempVariable(mSymbolTable, vec3Type));

        TIntermDeclaration *dPDXdxDecl = CreateTempInitDeclarationNode(
            &dPDXdx->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdx, new TIntermSequence({IndexDirect(recipOuter, 0)})));
        TIntermDeclaration *dPDYdxDecl = CreateTempInitDeclarationNode(
            &dPDYdx->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdx, new TIntermSequence({IndexDirect(recipOuter, 1)})));
        TIntermDeclaration *dPDZdxDecl = CreateTempInitDeclarationNode(
            &dPDZdx->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdx, new TIntermSequence({IndexDirect(recipOuter, 2)})));
        TIntermDeclaration *dPDXdyDecl = CreateTempInitDeclarationNode(
            &dPDXdy->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdy, new TIntermSequence({IndexDirect(recipOuter, 0)})));
        TIntermDeclaration *dPDYdyDecl = CreateTempInitDeclarationNode(
            &dPDYdy->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdy, new TIntermSequence({IndexDirect(recipOuter, 1)})));
        TIntermDeclaration *dPDZdyDecl = CreateTempInitDeclarationNode(
            &dPDZdy->variable(), TIntermAggregate::CreateRawFunctionCall(
                                     *dFdy, new TIntermSequence({IndexDirect(recipOuter, 2)})));
    ...
    void convertBuiltinFunction(TIntermAggregate *node)
    {
        ...
        if (hasBias)
        {
            ...
            coordTransform->push_back(CreateTempDeclarationNode(&biasFac->variable()));
            coordTransform->push_back(new TIntermBinary(EOpAssign, biasFac, exp2Call));
            dUVdxArg = new TIntermBinary(EOpVectorTimesScalar, biasFac, dUVdx);
            dUVdyArg = new TIntermBinary(EOpVectorTimesScalar, biasFac, dUVdy);
        }
    ...
    // A specializer version of the same function which uses implicit derivatives.
