...
class RewriteCubeMapSamplersAs2DArrayTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    void declareCoordTranslationFunction()
    {
        ...
        TransformXMajor(calculateXUcVc, x, dPdxY, dPdxZ, dUdx, dVdx);
        TransformXMajor(calculateXUcVc, x, dPdyY, dPdyZ, dUdy, dVdy);
        ...
        TransformYMajor(calculateYUcVc, dPdxX, y, dPdxZ, dUdx, dVdx);
        TransformYMajor(calculateYUcVc, dPdyX, y, dPdyZ, dUdy, dVdy);
        ...
        TransformZMajor(calculateZUcVc, dPdxX, dPdxY, z, dUdx, dVdx);
        TransformZMajor(calculateZUcVc, dPdyX, dPdyY, z, dUdy, dVdy);
        ...
        TIntermTyped *dUdxNormalized = new TIntermBinary(EOpMul, dUdx, ma->deepCopy());
        TIntermTyped *dVdxNormalized = new TIntermBinary(EOpMul, dVdx, ma->deepCopy());
        TIntermTyped *dUdyNormalized = new TIntermBinary(EOpMul, dUdy, ma->deepCopy());
        TIntermTyped *dVdyNormalized = new TIntermBinary(EOpMul, dVdy, ma->deepCopy());
    ...
    void convertBuiltinFunction(TIntermAggregate *node)
    {
        ...
        {
            TIntermTyped *P = (*arguments)[1]->getAsTyped();
            TIntermTyped *absX =
                new TIntermUnary(EOpAbs, new TIntermSwizzle(P->deepCopy(), {0}), nullptr);
            TIntermTyped *absY =
                new TIntermUnary(EOpAbs, new TIntermSwizzle(P->deepCopy(), {1}), nullptr);
            TIntermTyped *absZ =
                new TIntermUnary(EOpAbs, new TIntermSwizzle(P->deepCopy(), {2}), nullptr);
            TIntermTyped *ma = CreateBuiltInFunctionCallNode(
                "max",
                new TIntermSequence(
                    {absX, CreateBuiltInFunctionCallNode("max", new TIntermSequence({absY, absZ}),
                                                         *mSymbolTable, 100)}),
                *mSymbolTable, 100);
            TIntermTyped *Pnorm = new TIntermBinary(EOpDiv, P->deepCopy(), ma);
            dPdx = CreateBuiltInFunctionCallNode("dFdx", new TIntermSequence({Pnorm}),
                                                 *mSymbolTable, 100);
            dPdy = CreateBuiltInFunctionCallNode("dFdy", new TIntermSequence({Pnorm}),
                                                 *mSymbolTable, 100);
            dPdx = new TIntermBinary(EOpVectorTimesScalar, dPdx, ma->deepCopy());
            dPdy = new TIntermBinary(EOpVectorTimesScalar, dPdy, ma->deepCopy());
            if (arguments->size() == 3)
            {
                hasBias = true;
            }
