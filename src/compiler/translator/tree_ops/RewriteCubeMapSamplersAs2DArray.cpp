...
class RewriteCubeMapSamplersAs2DArrayTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    TIntermFunctionDefinition *declareCoordTranslationFunction()
    {
        ...
        TransformXMajor(calculateXUcVc, p, uc, vc);
        TransformXMajor(calculateXUcVc, dPdx, dUdx, dVdx);
        TransformXMajor(calculateXUcVc, dPdy, dUdy, dVdy);

        TIntermBlock *calculateYUcVc = new TIntermBlock;
        TransformYMajor(calculateYUcVc, p, uc, vc);
        TransformYMajor(calculateYUcVc, dPdx, dUdx, dVdx);
        TransformYMajor(calculateYUcVc, dPdy, dUdy, dVdy);

        TIntermBlock *calculateZUcVc = new TIntermBlock;
        TransformZMajor(calculateZUcVc, p, uc, vc);
        TransformZMajor(calculateZUcVc, dPdx, dUdx, dVdx);
        TransformZMajor(calculateZUcVc, dPdy, dUdy, dVdy);
    ...
    void convertBuiltinFunction(TIntermAggregate *node)
    {
        ...
        const TType *floatType = StaticType::GetBasic<EbtFloat>();
        TFunction *quadSwapHorizontalFloat =
            new TFunction(mSymbolTable, ImmutableString("subgroupQuadSwapHorizontal"),
                          SymbolType::AngleInternal, floatType, true);
        quadSwapHorizontalFloat->addParameter(CreateTempVariable(mSymbolTable, floatType));
        ...
        TIntermTyped *substitute = TIntermAggregate::CreateConstructor(
            *StaticType::GetBasic<EbtFloat, 4>(),
            new TIntermSequence(
                {u, v,
                 TIntermAggregate::CreateRawFunctionCall(*quadSwapHorizontalFloat,
                                                         new TIntermSequence({u->deepCopy()})),
                 TIntermAggregate::CreateRawFunctionCall(*quadSwapHorizontalFloat,
                                                         new TIntermSequence({v->deepCopy()}))}));
        queueReplacement(substitute, OriginalNode::IS_DROPPED);
#else
        TIntermSwizzle *u = new TIntermSwizzle(uvl->deepCopy(), {0});
        TIntermSwizzle *v = new TIntermSwizzle(uvl->deepCopy(), {1});
