                                ...
                                TIntermTyped *out)
{
    ...
    TIntermTyped *neighborValue =
        TIntermAggregate::CreateRawFunctionCall(*quadSwap, new TIntermSequence({in}));

    // Assume the non-helper neighbor has value L.  We want to make sure the helper doesn't have a
    // value larger than this limit.  If it does, we mirror it around it.
    //
    ...
    //     |in| >= |L|:    out = 2L - in
    //
    // TODO: Looks like this cannot be done per channel.  Could do the quadSwap for the whole vec3,
    // look at the major axis and compare it with my major axis.  If different, then mirror the
    // whole vec3 against the neighbor.  That should keep the length of the derivative vector
    // constant.

    TIntermSymbol *absIn = new TIntermSymbol(CreateTempVariable(symbolTable, floatType));
    body->appendStatement(CreateTempInitDeclarationNode(
        &absIn->variable(), new TIntermUnary(EOpAbs, in->deepCopy(), nullptr)));

    TIntermSymbol *L = new TIntermSymbol(CreateTempVariable(symbolTable, floatType));
    body->appendStatement(CreateTempInitDeclarationNode(&L->variable(), neighborValue));

    // 2*L - in
    TIntermTyped *LTimes2  = new TIntermBinary(EOpMul, L, CreateFloatNode(2.0f));
    TIntermTyped *mirrored = new TIntermBinary(EOpSub, LTimes2, in->deepCopy());

    // |in| <= |L|
    TIntermTyped *cond = new TIntermBinary(EOpLessThanEqual, absIn,
                                           new TIntermUnary(EOpAbs, L->deepCopy(), nullptr));
                                             ...
                                             TIntermSymbol *p)
{
    ...
    body->appendStatement(CreateTempInitDeclarationNode(&outP->variable(), p));
    ...
    TIntermSwizzle *outX = new TIntermSwizzle(outP->deepCopy(), {0});
    ...
    TIntermBlock *horizontalNonHelperBody = new TIntermBlock;
    TIntermBlock *verticalNonHelperBody   = new TIntermBlock;
    TIntermBlock *diagonalNonHelperBody   = new TIntermBlock;

    TIntermTyped *horizontalIsHelper = TIntermAggregate::CreateRawFunctionCall(
        *quadSwapHorizontalBool, new TIntermSequence({gl_HelperInvocation->deepCopy()}));
    TIntermTyped *verticalIsHelper = TIntermAggregate::CreateRawFunctionCall(
        *quadSwapHorizontalBool, new TIntermSequence({gl_HelperInvocation->deepCopy()}));
    ...
    MirrorOutOfRangeCoordinate(symbolTable, horizontalNonHelperBody, quadSwapHorizontalFloat, x,
                               outX);
    MirrorOutOfRangeCoordinate(symbolTable, horizontalNonHelperBody, quadSwapHorizontalFloat, y,
                               outY);
    MirrorOutOfRangeCoordinate(symbolTable, horizontalNonHelperBody, quadSwapHorizontalFloat, z,
                               outZ);

    MirrorOutOfRangeCoordinate(symbolTable, verticalNonHelperBody, quadSwapVerticalFloat,
                               x->deepCopy(), outX->deepCopy());
    MirrorOutOfRangeCoordinate(symbolTable, verticalNonHelperBody, quadSwapVerticalFloat,
                               y->deepCopy(), outY->deepCopy());
    MirrorOutOfRangeCoordinate(symbolTable, verticalNonHelperBody, quadSwapVerticalFloat,
                               z->deepCopy(), outZ->deepCopy());

    MirrorOutOfRangeCoordinate(symbolTable, diagonalNonHelperBody, quadSwapDiagonalFloat,
                               x->deepCopy(), outX->deepCopy());
    MirrorOutOfRangeCoordinate(symbolTable, diagonalNonHelperBody, quadSwapDiagonalFloat,
                               y->deepCopy(), outY->deepCopy());
    MirrorOutOfRangeCoordinate(symbolTable, diagonalNonHelperBody, quadSwapDiagonalFloat,
    ...
    TIntermIfElse *ifVerticalElseDiagnoal =
        new TIntermIfElse(verticalIsNonHelper, verticalNonHelperBody, diagonalNonHelperBody);
    TIntermBlock *ifVerticalElseDiagnoalBlock = new TIntermBlock;
    ifVerticalElseDiagnoalBlock->appendStatement(ifVerticalElseDiagnoal);
    TIntermIfElse *ifHorizontalElseVerticalElseDiagnoal = new TIntermIfElse(
        horizontalIsNonHelper, horizontalNonHelperBody, ifVerticalElseDiagnoalBlock);
    body->appendStatement(ifHorizontalElseVerticalElseDiagnoal);

    TIntermBlock *mainBlock = new TIntermBlock;
    mainBlock->appendStatement(new TIntermBinary(EOpAssign, outP->deepCopy(), p->deepCopy()));

    TIntermIfElse *ifHelper = new TIntermIfElse(
        new TIntermUnary(EOpLogicalNot, gl_HelperInvocation, nullptr), mainBlock, nullptr);
...
#if 0
        TIntermBranch *ret = new TIntermBranch(
            EOpReturn, TIntermAggregate::CreateConstructor(
                           *vec3Type, new TIntermSequence({xyz})));
        body->appendStatement(ret);
    ...
    void convertBuiltinFunction(TIntermAggregate *node)
    {
        ...
        TIntermTyped *substitute = TIntermAggregate::CreateConstructor(
            *StaticType::GetBasic<EbtFloat, 4>(),
            new TIntermSequence({uv, l /*lDiv6*/, CreateFloatNode(1.0f)}));
