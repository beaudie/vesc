...
class Rescoper : public TIntermTraverser
{
  public:
    ...
    bool rescope(TIntermBlock &root)
    {
        ...
        for (auto &pair : mGlobalVarsNeedRescope)
        {
            if (pair.second.functions.size() == 1)
            {
                ...
                TIntermSequence *funcOriginal   = func->getBody()->getSequence();
                TIntermSequence funcReplacement;
                funcReplacement.push_back(pair.second.declaration);
                for (TIntermNode *node : *funcOriginal)
                {
                    funcReplacement.push_back(node);
                }
                *funcOriginal = std::move(funcReplacement);
