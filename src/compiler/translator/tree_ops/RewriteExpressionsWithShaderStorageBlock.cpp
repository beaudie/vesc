...
bool IsReadonlyBinaryOperator(TOperator op)
{
    switch (op)
    {
        case EOpAdd:
                                                                    ...
                                                                    TIntermBinary *node)
{
    ...
    //      var temp2 = ssbo2;
    //      temp0 + temp1 + temp2;
    else if (IsReadonlyBinaryOperator(node->getOp()) && (leftSSBO || rightSSBO))
    {
        mFoundSSBO              = true;
            ...
            if (node->getFunction() != nullptr)
            {
                ...
                if (qual == EvqInOut || qual == EvqOut)
                {
                    TIntermBinary *readBackToSSBO =
                        new TIntermBinary(EOpAssign, ssboArgument, argumentCopy);
