...
class RewriteRowMajorMatricesTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    void convertInterfaceBlock(TIntermDeclaration *node)
    {
        ...
        {
            // TODO: multideclarations are separated, so this should only be one!
            ...
            if (!valueExpression)
            {
                ...
                if (assignment)
                {
                    ...
                    RewriteRowMajorMatricesTraverser rhsTraverser(
                        mSymbolTable, mInterfaceBlockMapIn, mInterfaceBlockFieldConvertedIn,
                        mStructMapOut, mCopyFunctionDefinitionsOut, assignment);
                    getRootNode()->traverse(&rhsTraverser);
                    rhsTraverser.updateTree();
                    ...
                    TIntermTyped *assignmentResultExpression =
                        transformReadExpression(baseExpression->deepCopy(), primaryIndex,
                                                &secondaryIndices, structure, &prependStatements);
