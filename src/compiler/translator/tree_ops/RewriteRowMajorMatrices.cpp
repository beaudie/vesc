...
bool IsFieldRowMajor(const TField *field, bool isBlockRowMajor)
{
...
TType *TransposeMatrix(const TType *type)
{
...
class RewriteRowMajorMatricesTraverser : public TIntermTraverser
{
  public:
    ...
    TIntermSequence *getStructCopyFunctions() { return &mCopyFunctionDefinitions; }
  ...
  private:
    ...
    void transformExpression(TIntermSymbol *symbol)
    {
        ...
        // effects.  TODO: add a test for this.
        //
        // ---
        //
        ...
        //
        // TODO: add a test for this to demonstrate the bug and tag it with an issue for tracking
        // purposes. I think the proper solution would be for insertStatementsInParentBlock to
        // undertand if short-circuiting and cache the if expressions before the prepend statements.
        //
        TIntermTyped *baseExpression = new TIntermSymbol(mInterfaceBlockMap[&symbol->variable()]);
        bool isStruct                = false;
        bool isMatrix                = false;
        ...
        while (IsIndexNode(getAncestorNode(accessorIndex), previousAncestor))
        {
            ...
            bool isArrayIndex = opIsIndex && previousAncestor->getType().isArray();

            // If it's a direct index in a matrix, it's the primary index.
            bool isMatrixPrimarySubscript = opIsIndex && !isArrayIndex && ancestorIsMatrix;
            ...
            else
            {
                ...
                baseExpression =
                    ReplicateIndexNode(mSymbolTable, ancestor, baseExpression, &prependStatements);
        ...
        if (isWrite)
        {
            ...
            if (!valueExpression)
            {
                ...
                if (assignment)
                {
                    ...
                    queueReplacementWithParent(accessorParent, accessor, valueExpression,
                                               OriginalNode::IS_DROPPED);
                }
                else
                {
                                          ...
                                          TIntermSequence *prependStatements)
    {
        if (structure)
        {
            ...
            const TFunction *copyToOriginal = mStructMap[structure].copyToOriginal;
            return TIntermAggregate::CreateFunctionCall(*copyToOriginal,
        ...
        if (primaryIndex == nullptr)
        {
            ...
            return CreateBuiltInFunctionCallNode("transpose", new TIntermSequence({baseExpression}),
                                                 *mSymbolTable, 300);
        }
                                  ...
                                  TIntermSequence *writeStatements)
    {
        if (structure)
        {
            ...
            // expression.
            const TFunction *copyFromOriginal = mStructMap[structure].copyFromOriginal;
            TIntermTyped *functionCall        = TIntermAggregate::CreateFunctionCall(
                *copyFromOriginal, new TIntermSequence({valueExpression}));
            writeStatements->push_back(new TIntermBinary(EOpAssign, baseExpression, functionCall));
        ...
        if (primaryIndex == nullptr)
        {
            ...
            TIntermTyped *valueTransposed = CreateBuiltInFunctionCallNode(
                "transpose", new TIntermSequence({valueExpression}), *mSymbolTable, 300);
    ...
    {
        ASSERT(fromFieldType->isStructSpecifier());
        ASSERT(toFieldType->isStructSpecifier());
        ...
        // should expect to find toField or fromField in mStructMap, if true or false
        // respectively.
        ...
        if (isCopyToOriginal)
        {
            ...
            auto iter = mStructMap.find(toFieldStruct);
            if (iter != mStructMap.end())
            {
        ...
        else
        {
            ...
            auto iter = mStructMap.find(fromFieldStruct);
            if (iter != mStructMap.end())
            {
                                 ...
                                 bool isCopyToOriginal)
    {
        ...
        for (size_t fieldIndex = 0; fieldIndex < fromFields.size(); ++fieldIndex)
        {
            ...
            TIntermTyped *fromField =
                new TIntermBinary(EOpIndexDirectStruct, fromSymbol, fieldIndexNode);
            TIntermTyped *toField =
                new TIntermBinary(EOpIndexDirectStruct, toSymbol, fieldIndexNode->deepCopy());

            const TType *fromFieldType = fromFields[fieldIndex]->type();
            const TType *toFieldType   = toFields[fieldIndex]->type();

            TIntermTyped *rhs = fromField;

            if (fromFieldType->isStructSpecifier())
            {
                const TFunction *fieldCopyFunction =
                    getCopyStructFieldFunction(fromFieldType, toFieldType, isCopyToOriginal);

                if (fieldCopyFunction)
                {
                    rhs = TIntermAggregate::CreateFunctionCall(*fieldCopyFunction,
                                                               new TIntermSequence({fromField}));
                }
            }
            else if (fromFieldType->isMatrix())
            {
                rhs = CreateBuiltInFunctionCallNode("transpose", new TIntermSequence({fromField}),
                                                    *mSymbolTable, 300);
            }

            body->appendStatement(new TIntermBinary(EOpAssign, toField, rhs));
        }
        ...
        mCopyFunctionDefinitions.push_back(functionDef);
    ...
    TIntermSequence mCopyFunctionDefinitions;
};
