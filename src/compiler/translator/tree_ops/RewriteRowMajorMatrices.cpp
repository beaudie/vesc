...
class TransformArrayHelper
{
  public:
    ...
    TIntermTyped *getNextElement(TIntermTyped *valueExpression, TIntermTyped **valueElementOut)
    {
        ...
        // If the first index overflows, element enumeration is done.
        if (mArrayIndices[0] >= (*arraySizes)[0])
        {
            return nullptr;
        }
    ...
    TIntermTyped *constructReadTransformExpression()
    {
        ...
        TType *transformedType = new TType(firstElement->getType());
        transformedType->makeArrays(*arraySizes);

        return TIntermAggregate::CreateConstructor(*transformedType,
  ...
  private:
    TIntermTyped *getCurrentElement(TIntermTyped *expression)
    {
        ...
        for (unsigned int index : mArrayIndices)
        {
    ...
    void incrementIndices(const TVector<unsigned int> *arraySizes)
    {
        ...
        // [0, arraySizes[i]).  This function increments this number.  digit 0 is the most
        // significant digit.
        for (size_t i = arraySizes->size(); i > 0; --i)
        {
            size_t digitIndex = i - 1;
...
class RewriteRowMajorMatricesTraverser : public TIntermTraverser
{
  ...
  private:
    void convertInterfaceBlock(TIntermDeclaration *node)
    {
        ...
        for (const TField *field : block->fields())
        {
            ...
            if (IsFieldRowMajor(field, isBlockRowMajor))
            {
                ...
                mInterfaceBlockFieldConverted[field] = true;
            }
            else
            {
        ...
        while (IsIndexNode(getAncestorNode(accessorIndex), previousAncestor))
        {
            ...
            else
            {
                ...
                requiresTransformation    = requiresTransformation ||
                                         IsConvertedField(ancestor, mInterfaceBlockFieldConverted);
        ...
        if (isWrite)
        {
            ...
            if (!valueExpression)
            {
                ...
                if (assignment)
                {
                    ...
                    // Replace the whole assignment expression with the temp value.  If the result
                    ...
                    //
                    // Additionally, a subexpression of exp itself could require a transformation.
                    // This complicates things as there would be two replacements:
                    //
                    // - Replace M=exp with temp, where parent is ==
                    ...
                    //
                    // TODO: make an inner pass that replaces exp, commits the tree then use that as
                    // rhs, instead of assignment->getRight().
                    //
                                  ...
                                  TIntermSequence *writeStatements)
    {
        ...
        if (structure)
        {
            ...
            if (baseExpressionType.isArray())
            {
                ...
                while ((element = transformHelper.getNextElement(valueElement, &valueElement)) !=
                       nullptr)
                {
    ...
    std::unordered_map<const TField *, bool> mInterfaceBlockFieldConverted;
