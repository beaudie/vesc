...
class ReplaceSubpassInputUtils
{
  ...
  protected:
    ...
    TIntermNode *assignSubpassLoad(TIntermTyped *resultVar, TIntermTyped *inputAttachmentSymbol);
                                                         ...
                                                         TIntermTyped *inputAttachmentSymbol)
{
    ...
    return new TIntermBinary(EOpAssign, resultVar, subpassLoadFuncCall);
}
    ...
    const unsigned int inputAttachmentIndex)
{
    ...
    TIntermSymbol *loadInputAttachmentDataSymbol =
        new TIntermSymbol(mDataLoadVarList[inputAttachmentIndex]);
    ...
    if (arraySize > 0)
    {
        ...
        for (uint32_t index = 0; index < arraySize; index++)
        {
            ...
            blockNode->appendStatement(assignSubpassLoad(
                left, new TIntermSymbol(mInputAttachmentVarList[attachmentIndex])));
        }
...
bool ReplaceGlLastFragDataUtils::loadInputAttachmentData()
{
    ...
    for (auto declaredVar : mDeclaredVarVec)
    {
        ...
        if (mConstIndices.test(inputAttachmentIndex))
        {
            loadInputAttachmentBlock->getAsBlock()->appendStatement(
                loadInputAttachmentDataImpl(kArraySizeZero, inputAttachmentIndex));
        }
    }
...
bool ReplaceInOutUtils::loadInputAttachmentData()
{
    ...
    for (auto declaredVar : mDeclaredVarVec)
    {
        ...
        loadInputAttachmentBlock->appendStatement(
            loadInputAttachmentDataImpl(arraySize, inputAttachmentIndex));
    }
