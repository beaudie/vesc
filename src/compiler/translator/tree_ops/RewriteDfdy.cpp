...
TIntermTyped *Traverser::getMultiplierX(const TIntermUnary *node)
{
    ...
    if (node->getOp() == EOpDFdx)
    {
        sequences = new TIntermSequence({CreateFloatNode(1.0f), CreateFloatNode(0.0f),
                                         CreateFloatNode(-1.0f), CreateFloatNode(0.0f),
                                         CreateFloatNode(1.0f), CreateFloatNode(1.0f),
                                         CreateFloatNode(1.0f), CreateFloatNode(1.0f)});
    }
    else
    {
        ASSERT(node->getOp() == EOpDFdy);
        sequences = new TIntermSequence({CreateFloatNode(0.0f), CreateFloatNode(1.0f),
                                         CreateFloatNode(0.0f), CreateFloatNode(-1.0f),
                                         CreateFloatNode(0.0f), CreateFloatNode(1.0f),
                                         CreateFloatNode(1.0f), CreateFloatNode(1.0f)});
    }
