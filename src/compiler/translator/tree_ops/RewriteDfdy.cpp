...
bool Traverser::visitUnary(Visit visit, TIntermUnary *node)
{
    ...
    // used for fragment shader rotation.  The top half (a vec2 that is either (1,0) or (0,1)) is
    // used for rewriting dFdx() and the bottom half (either (0,1) or (1,0)) is used for rewriting
    // dFdy().  Otherwise, the formula for the rewrite is the same:
    //
    //   correctedDfdx(operand) becomes the following:
    //
    //     vec2 tmp = vec2(dFdx(operand), dFdy(operand)) * mFragRotation[topHalf] * mFlipXY
    //     result = tmp.x + tmp.y
    //
    //   correctedDfdy(operand) becomes the following:
    //
    //     vec2 tmp = vec2(dFdx(operand), dFdy(operand)) * mFragRotation[bottomHalf] * mFlipXY
    //     result = tmp.x + tmp.y
    //
    // Because of how mFragRotation is initialized, either tmp.x or tmp.y will be zero.  That means
    // that the above code results in the following for 0 and 180 degrees:
    //
    ...
    // Get a vec2 with the correct half of mFragRotation
    // Note: I don't know how to properly get a vec2 from a mat2; so I'll brute-force it: multiply
    // mFragRotation by a vec2 that's (1,0) for dFdx or (0,1) for dFdy
    TIntermConstantUnion *zero = CreateFloatNode(1.0f);
    TIntermConstantUnion *one = CreateFloatNode(1.0f);
    TIntermBinary *copyFlipXY = new TIntermBinary(EOpVectorTimesScalar, mFlipXY, one);
    TVector<int> swizzleOffsetX = {0};
    TVector<int> swizzleOffsetY = {1};
    TIntermSwizzle *flipX = new TIntermSwizzle(copyFlipXY, swizzleOffsetX);
    TIntermSwizzle *flipY = new TIntermSwizzle(copyFlipXY, swizzleOffsetY);
    TIntermBinary *tmp = nullptr;
    if (node->getOp() == EOpDFdx)
    {
        tmp = new TIntermBinary(EOpMul, flipY, zero);
    }
    else
    {
        tmp = new TIntermBinary(EOpMul, flipX, zero);
    }
    TIntermBinary *rotatedFlipXY =
        new TIntermBinary(EOpMatrixTimesVector, mFragRotation->deepCopy(), copyFlipXY->deepCopy());
