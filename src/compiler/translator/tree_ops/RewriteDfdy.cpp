...
bool Traverser::visitUnary(Visit visit, TIntermUnary *node)
{
    ...
    // If this is a vec3/vec4, we need to correct Z also
    if (node->getType().getObjectSize() >= 3)
    {
        // Create a swizzle to "dfdyResult.z"
        TVector<int> swizzleZ;
        swizzleZ.push_back(2);
        TIntermSwizzle *dfdyResultSwizzleZ = new TIntermSwizzle(dfdyResult, swizzleZ);

        // Correct dfdyResult's X value:
        // (s413.z *= ANGLEUniforms.viewportYScale);
        TIntermBinary *correctedZ =
            new TIntermBinary(EOpMulAssign, dfdyResultSwizzleZ, mViewportYScale);
        replacementSequence->push_back(correctedZ);
    }
