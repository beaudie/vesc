...
bool Traverser::visitAggregate(Visit visit, TIntermAggregate *node)
{
    ...
    TIntermTyped *xMultiplier =
        new TIntermBinary(EOpMul, swapXMultiplier, new TIntermSwizzle(flipXY, {0}));
    TIntermTyped *yMultiplier =
        new TIntermBinary(EOpMul, swapYMultiplier, new TIntermSwizzle(flipXY->deepCopy(), {1}));

    TIntermTyped *rotatedFlippedDfdx = new TIntermBinary(EOpMul, dFdx, xMultiplier);
    TIntermTyped *rotatedFlippedDfdy = new TIntermBinary(EOpMul, dFdy, yMultiplier);
