...
class RewritePLSToFramebufferFetchTraverser : public RewritePLSTraverser
{
  public:
    ...
    void visitPLSDeclaration(TIntermSymbol *plsSymbol) override
    {
        ...
        PLSAttachment attachment(mCompiler, mSymbolTable, plsSymbol->variable());
                         ...
                         size_t plsBeginPosition) override
    {
        ...
        // become undefined if not explicitly written to, but either way, when this compiles to
        // subpass loads in Vulkan, we definitely get undefined behavior if PLS variables are not
        // written to.
        //
        // To make sure every PLS variable gets assigned, we read them all before PLS operations,
        // then write them all back out after all PLS operations are complete.
    ...
    void injectFinalizeCode(TCompiler *,
                            ...
                            size_t plsEndPosition) override
    {
        std::vector<TIntermNode *> plsWriteouts;
        plsWriteouts.reserve(mPLSAttachments.bindingOrderedMap().size());
        for (const auto &entry : mPLSAttachments.bindingOrderedMap())
        {
            const PLSAttachment &attachment = entry.second;
            plsWriteouts.push_back(new TIntermBinary(EOpAssign, attachment.swizzleFragmentVar(),
                                                     new TIntermSymbol(attachment.accessVar)));
        }
        mainBody->getSequence()->insert(mainBody->getSequence()->begin() + plsEndPosition,
                                        plsWriteouts.begin(), plsWriteouts.end());
    }

  private:
    struct PLSAttachment
    {
        PLSAttachment(const TCompiler *compiler, TSymbolTable *symbolTable, const TVariable &plsVar)
        {
            ...
            // compile error from "inout uint". The spec also seems silent on how many components an
            // output must have, so we always use 4.
            TType *fragmentVarType = new TType(accessVarType->getBasicType(), 4);
            fragmentVarType->setPrecision(plsType.getPrecision());
            fragmentVarType->setQualifier(TQualifier::EvqFragmentInOut);
        ...
        {
            TIntermTyped *value = new TIntermSymbol(accessVar);
            if (accessVar->getType().getNominalSize() == 1)
            {
                switch (accessVar->getType().getBasicType())
                {
                    case EbtFloat:
                        value = TIntermAggregate::CreateConstructor(  // "vec4(r, 0, 0, 1)"
                            TType(EbtFloat, 4),
                            {value, CreateFloatNode(0, EbpHigh), CreateFloatNode(0, EbpHigh),
                             CreateFloatNode(1, EbpHigh)});
                        break;
                    case EbtUInt:
                        value = TIntermAggregate::CreateConstructor(  // "uvec4(r, 0, 0, 1)"
                            TType(EbtUInt, 4),
                            {value, CreateUIntNode(0), CreateUIntNode(0), CreateUIntNode(1)});
                        break;
                    default:
                        UNREACHABLE();
                        break;
        ...
        // Swizzles our 4-component "inout" fragment variable to the size of the pixel local storage
        // internalformat.
        TIntermTyped *swizzleFragmentVar() const
        {
            TIntermTyped *swizzle = new TIntermSymbol(fragmentVar);
            switch (accessVar->getType().getNominalSize())
            {
                case 1:
                    swizzle = CreateSwizzle(swizzle, 0);  // "fragmentVar.r"
                    break;
            }
            return swizzle;
        }
        ...
        {
            TIntermTyped *value = new TIntermSymbol(var);
            if (accessVar->getType().getNominalSize() == 1)
            {
                value = CreateSwizzle(value, 0);
            }
            return value;
        }
                              ...
                              int shaderVersion)
{
    ...
    // Inject the cose that needs to run before and after all PLS operations.
