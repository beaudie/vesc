...
class RewritePLSToFramebufferFetchTraverser : public RewritePLSTraverser
{
  public:
                         ...
                         size_t plsBeginPosition) override
    {
        ...
        // sample) position.
        //
        // It's not clear from EXT_shader_framebuffer_fetch whether inout fragment variables are
        // undefined if not explicitly written to, so read them all before PLS operation, then write
        // them all back out after PLS operation.
        ...
        for (const auto &entry : mPLSAttachments.bindingOrderedMap())
        {
            ...
            plsPreloads.push_back(
                CreateTempAssignmentNode(attachment.accessVar, attachment.fragmentSwizzle()));
        }
                            ...
                            size_t plsEndPosition) override
    {
        ...
        {
            const PLSAttachment &attachment = entry.second;
            plsWriteouts.push_back(new TIntermBinary(EOpAssign, attachment.fragmentSwizzle(),
  ...
  private:
    struct PLSAttachment
    {
        ...
        TIntermTyped *expandAccessVar() const
        {
            ...
            if (accessVar->getType().getNominalSize() == 1)
            {
                switch (accessVar->getType().getBasicType())
                {
                    case EbtFloat:
                        // vec4(r, 0, 0, 1)
                        value = TIntermAggregate::CreateConstructor(
                            TType(EbtFloat, 4),
        ...
        TIntermTyped *fragmentSwizzle() const
        {
