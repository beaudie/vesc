...
class RewriteToImagesTraverser : public TIntermTraverser
{
  public:
    ...
    bool visitDeclaration(Visit, TIntermDeclaration *decl) override
    {
        ...
        // writeonly. To work around this, we create two aliases of the same image: one readonly and
        // one writeonly.
        //
        // TODO(anglebug.com/7279): Maybe we could manually pack 4-byte formats into GL_R23UI
        ...
        if (mCompiler->getOutputType() == ShShaderOutput::SH_ESSL_OUTPUT &&
            ...
            format != TLayoutImageInternalFormat::EiifR32UI)  // NOTE: r32i isn't supported by PLS.
        {
    ...
    bool visitAggregate(Visit, TIntermAggregate *aggregate) override
    {
        ...
        if (aggregate->getOp() == EOpPixelLocalStoreANGLE)
        {
            ...
            insertStatementsInParentBlock(
                {valueDecl, mNodeBuilder->createBuiltInFunctionCall("memoryBarrier")},  // Before.
                {mNodeBuilder->createBuiltInFunctionCall("memoryBarrier")});            // After.
                                      ...
                                      int shaderVersion)
{
    ...
    {
        mainBody->insertStatement(0, nodeBuilder.createBuiltInFunctionCall("memoryBarrier"));
        if (TIntermNode *begin = nodeBuilder.createBuiltInInterlockBeginCall())
        {
            mainBody->insertStatement(0, begin);
        }
        mainBody->appendStatement(nodeBuilder.createBuiltInFunctionCall("memoryBarrier"));
        if (TIntermNode *end = nodeBuilder.createBuiltInInterlockEndCall())
        {
            mainBody->appendStatement(end);
        }
    }
