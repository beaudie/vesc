  ...
  public:
    ...
    bool visitAggregate(Visit, TIntermAggregate *aggregate) override
    {
        ...
        if (aggregate->getOp() == EOpPixelLocalStoreANGLE)
        {
            ...
            valueDecl->traverse(this);  // Rewrite a potential pixelLocalLoadANGLE in the valueDecl.
                                      ...
                                      int shaderVersion)
{
    ...
    if (traverser.globalPixelCoord())
    {
        ...
        ASSERT(mainBody);
        TIntermTyped *expr;  // "ivec2(floor(gl_FragCoord.xy))"
        expr =
        ...
        mainBody->getSequence()->insert(
            ...
            CreateTempAssignmentNode(traverser.globalPixelCoord(), expr));
    }
    return true;
}
