                                      ...
                                      int shaderVersion)
{
    ...
    if (compileOptions & SH_FRAGMENT_SYNCHRONIZATION_TYPE_MASK)
    {
        ...
        {
            // It seems like these memory barriers surrounding the interlockBeginCall ought to be
            // implicit, but the linux bot fails the Coherency test without them.
            mainBody->getSequence()->insert(
                mainBody->getSequence()->begin(),
                {nodeBuilder.createBuiltInFunctionCall("memoryBarrierImage"), interlockBeginCall,
                 nodeBuilder.createBuiltInFunctionCall("memoryBarrierImage")});
        }
