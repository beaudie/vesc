...
class RewritePLSTraverser : public TIntermTraverser
{
  ...
  protected:
    ...
    // has different behavior on the various APIs.
    //
            ...
            switch (type.getBasicType())
            {
                case EbtFloat:
                    expr = TIntermAggregate::CreateConstructor(  // "vec4(r, 0, 0, 1)"
                        TType(EbtFloat, 4),
                        {expr, CreateFloatNode(0, EbpHigh), CreateFloatNode(0, EbpHigh),
...
class RewriteANGLEToEXTTraverser : public RewritePLSTraverser
{
  public:
    ...
    {
        // Create EXT fields for ANGLE uniforms and delete them.
        ...
        for (size_t i = 0; i < rootSequence.size(); ++i)
        {
            ...
            // Create a native pixel local storage field that describes this declaration.
            TIntermSymbol *symbolANGLE = declVariable->getAsSymbolNode();
            ASSERT(symbolANGLE);
            const TVariable &angleVar = symbolANGLE->variable();
            const TType &typeANGLE    = angleVar.getType();
            TType *typeEXT;
            ...
            mFieldsEXT.insertNew(
                symbolANGLE,
                FieldEXT(typeEXT, angleVar.name(), symbolANGLE->getLine(), angleVar.symbolType()));
            rootSequence.erase(rootSequence.begin() + i);
            --i;
        }

        // Insert the global __pixel_localEXT interface block and determine each field's index.
    ...
    void visitPLSStore(TIntermSymbol *symbolANGLE, TVariable *value) override
    {
        ...
        TIntermTyped *fieldAccess =
            new TIntermBinary(EOpIndexDirectInterfaceBlock, new TIntermSymbol(mBlockEXT), index);
        clampPLSVarIfNeeded(value, symbolANGLE->getType().getLayoutQualifier().imageInternalFormat);
        queueReplacement(
                              ...
                              int shaderVersion)
{
    ...
    switch (compileOptions.pls.type)
    {
        ...
        case ShPixelLocalStorageType::PixelLocalStorageEXT:
        {
            ...
            // shader and establishes their indices.
