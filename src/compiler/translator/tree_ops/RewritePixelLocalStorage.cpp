...
// Base class for rewriting high level PLS operations to GLSL operations specified by
  ...
  private:
    struct PLSAttachment
    {
        PLSAttachment(const TCompiler *compiler, TSymbolTable *symbolTable, const TVariable &plsVar)
        {
            ...
            // output must have, so we always use 4 on GLES. Vulkan and Metal can definitely handle
            // outputs of any size.
            int numOutputComponents = compiler->getOutputType() == ShShaderOutput::SH_ESSL_OUTPUT
                                          ? 4
                                          : accessVarType->getNominalSize();
            TType *fragmentVarType  = new TType(accessVarType->getBasicType(), numOutputComponents);
        ...
        TIntermTyped *swizzle(TVariable *var) const
        {
            ...
            if (var->getType().getNominalSize() != accessVar->getType().getNominalSize())
            {
                ...
                {
                    case 1:
                        swizzled = CreateSwizzle(swizzled, 0);  // "swizzled.r"
                        break;
                }
            }
            return swizzled;
