...
class RewriteToImagesTraverser : public TIntermTraverser
{
  public:
    ...
    bool visitAggregate(Visit, TIntermAggregate *aggregate) override
    {
        ...
        if (aggregate->getOp() == EOpPixelLocalStoreANGLE)
        {
            ...
            if (image2D->getType().getLayoutQualifier().imageInternalFormat == EiifR32UI &&
                plsSymbol->getType().getLayoutQualifier().imageInternalFormat == EiifRGBA8)
            {
                valueType->setPrecision(EbpHigh);
            }
            ...
            TIntermTyped *imageData = new TIntermSymbol(valueVar);
            imageData               = packPLSDataIfNecessary(imageData, plsSymbol, image2D);
            queueReplacement(mNodeBuilder->createBuiltInFunctionCall(
                                 "imageStore", {new TIntermSymbol(image2D),
                                                new TIntermSymbol(mGlobalPixelCoord), imageData}),
                             OriginalNode::IS_DROPPED);
  ...
  private:
    ...
    TIntermTyped *packPLSDataIfNecessary(TIntermTyped *plsData,
                                         TIntermSymbol *plsSymbol,
                                         TVariable *image2D)
    {
        TIntermTyped *packedData = plsData;
        TLayoutImageInternalFormat plsFormat =
            plsSymbol->getType().getLayoutQualifier().imageInternalFormat;
        TLayoutImageInternalFormat imageFormat =
            image2D->getType().getLayoutQualifier().imageInternalFormat;
        ...
        switch (plsFormat)
        {
            case EiifRGBA8:
            {
                ...
                insertStatementInParentBlock(
                    CreateTempInitDeclarationNode(workaroundHighpVar, packedData));
                // packUnorm4x8(workaroundHighpVar)
                packedData = mNodeBuilder->createBuiltInFunctionCall(
                    "packUnorm4x8", {new TIntermSymbol(workaroundHighpVar)});
                break;
            }
            case EiifRGBA8I:
            case EiifRGBA8UI:
            {
                ...
                auto maskAndShift = [&](int componentIdx) {
                    TIntermTyped *expr = CreateSwizzle(plsData, componentIdx);
                    if (componentIdx < 3)
                    {
                ...
                packedData = maskAndShift(0);
                packedData = new TIntermBinary(EOpBitwiseOr, packedData, maskAndShift(1));
                packedData = new TIntermBinary(EOpBitwiseOr, packedData, maskAndShift(2));
                packedData = new TIntermBinary(EOpBitwiseOr, packedData, maskAndShift(3));
                break;
            }
            default:
                UNREACHABLE();
        ...
        packedData = NodeBuilder::CreateConstructor(imageStoreType, {packedData});
        return packedData;
    }
