...
class RewriteToImagesTraverser : public TIntermTraverser
{
  public:
    ...
    bool visitAggregate(Visit, TIntermAggregate *aggregate) override
    {
        ...
        if (aggregate->getOp() == EOpPixelLocalStoreANGLE)
        {
            ...
            TIntermTyped *imageData;
            imageData = new TIntermSymbol(valueVar);
  ...
  private:
    ...
    // must not already exist in the map for this symbol.
    ...
    // Looks up the image2D backing storage for the given plsSymbol's binding. An entry must already
                                             ...
                                             TVariable *image2D)
    {
        ...
        if (plsFormat == image2D->getType().getLayoutQualifier().imageInternalFormat)
        {
    ...
    TIntermTyped *packPLSDataIfNecessary(TIntermTyped *pls,
                                         TIntermSymbol *plsSymbol,
                                         TVariable *image2D)
    {
        TLayoutImageInternalFormat plsFormat =
        ...
        if (plsFormat == image2D->getType().getLayoutQualifier().imageInternalFormat)
        {
            return pls;  // This PLS storage isn't packed.
        }
        TIntermTyped *data;
        switch (plsFormat)
        {
            case EiifRGBA8:
                // uvec4(packUnorm4x8(pls))
                data = mNodeBuilder->createBuiltInFunctionCall("packUnorm4x8", {pls});
                data = NodeBuilder::CreateConstructor(TType(EbtUInt, 4), {data});
                break;
            case EiifRGBA8I:
            case EiifRGBA8UI:
            {
                ...
                //
                //     gvec4((r & 0xff) | ((g & 0xff) << 8) | ((b & 0xff) << 16) | (a << 24))
                //
                TIntermTyped *mask =
                    plsFormat == EiifRGBA8I ? CreateIntNode(0xff) : CreateUIntNode(0xff);
                data = new TIntermBinary(EOpBitwiseAnd, CreateSwizzle(pls, 0), mask);
                data = new TIntermBinary(
                    EOpBitwiseOr, data,
                    new TIntermBinary(EOpBitShiftLeft,
                                      new TIntermBinary(EOpBitwiseAnd, CreateSwizzle(pls, 1), mask),
                                      CreateUIntNode(8)));
                data = new TIntermBinary(
                    EOpBitwiseOr, data,
                    new TIntermBinary(EOpBitShiftLeft,
                                      new TIntermBinary(EOpBitwiseAnd, CreateSwizzle(pls, 2), mask),
                                      CreateUIntNode(16)));
                data = new TIntermBinary(
                    EOpBitwiseOr, data,
                    new TIntermBinary(EOpBitShiftLeft, CreateSwizzle(pls, 3), CreateUIntNode(24)));
                data = NodeBuilder::CreateConstructor(TType(pls->getBasicType(), 4), {data});
                break;
            }
            default:
                UNREACHABLE();
