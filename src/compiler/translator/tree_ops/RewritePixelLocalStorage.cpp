...
class RewriteToImagesTraverser : public TIntermTraverser
{
  ...
  private:
                                         ...
                                         TVariable *image2D)
    {
        ...
        if (plsFormat == imageFormat)
        {
            return new TIntermSymbol(plsVar);  // This PLS storage isn't packed.
        }
        TIntermTyped *packedData;
        switch (plsFormat)
        {
            case EiifRGBA8:
            {
                ...
                insertStatementInParentBlock(
                    CreateTempInitDeclarationNode(workaroundHighpVar, new TIntermSymbol(plsVar)));
            ...
            case EiifRGBA8UI:
            {
                ...
                auto maskAndShift = [&](int componentIdx) {
                    TIntermTyped *expr = CreateSwizzle(new TIntermSymbol(plsVar), componentIdx);
                    if (componentIdx < 3)
                    {
