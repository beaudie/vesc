...
class FindConstantVariablesTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    bool visitLoop(Visit visit, TIntermLoop *loop) override
    {
        ...
        if (condition->getAsSymbolNode() == nullptr || loop->getType() == ELoopDoWhile)
        {
...
bool PruneInfiniteLoopsTraverser::visitLoop(Visit visit, TIntermLoop *loop)
{
    ...
    // constant variable.
    TIntermConstantUnion *constCondition = loop->getCondition()->getAsConstantUnion();
    TIntermSymbol *condition             = loop->getCondition()->getAsSymbolNode();

    const bool isConditionConstant =
        (condition != nullptr &&
         mConstVariables.find(&condition->variable()) != mConstVariables.end()) ||
        constCondition != nullptr;
