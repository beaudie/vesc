...
class MonomorphizeTraverser final : public TIntermTraverser
{
  public:
    explicit MonomorphizeTraverser(TCompiler *compiler,
                                   ...
                                   uint32_t unsupportedFunctionArgumentFlags,
                                   FunctionMap *functionMap)
        : TIntermTraverser(true, false, false, symbolTable),
          mCompiler(compiler),
          mCompileOptions(compileOptions),
          mUnsupportedFunctionArgumentFlags(unsupportedFunctionArgumentFlags),
  ...
  private:
    bool isUnsupportedArgument(TIntermTyped *callArgument, const TVariable *funcArgument) const
    {
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaStructContainingSamplers)
        {
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaArrayOfArrayOfSamplerOrImage)
        {
            // Monomorphize if:
            //
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaAtomicCounter)
        {
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaSamplerCubeEmulation)
        {
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaImage)
        {
            if (type.isImage())
            {
                return true;
        ...
        if (mUnsupportedFunctionArgumentFlags & EufaPixelLocalStorage)
        {
    ...
    uint32_t mUnsupportedFunctionArgumentFlags;
                                          ...
                                          uint32_t unsupportedFunctionArgumentFlags)
{
    ...
    while (true)
    {
        ...
        MonomorphizeTraverser monomorphizer(compiler, symbolTable, compileOptions,
                                            unsupportedFunctionArgumentFlags, &functionMap);
