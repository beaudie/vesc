...
class MonomorphizeTraverser final : public TIntermTraverser
{
  public:
    explicit MonomorphizeTraverser(TCompiler *compiler,
                                   ...
                                   UnsupportedFunctionArguments unsupportedFunctionArguments,
                                   FunctionMap *functionMap)
        : TIntermTraverser(true, false, false, symbolTable),
          mCompiler(compiler),
          mCompileOptions(compileOptions),
          mUnsupportedFunctionArguments(unsupportedFunctionArguments),
  ...
  private:
    bool isUnsupportedArgument(TIntermTyped *callArgument, const TVariable *funcArgument) const
    {
        ...
        int unsupportedArgs = (int)mUnsupportedFunctionArguments;
        if ((unsupportedArgs & (int)UnsupportedFunctionArguments::StructContainingSamplers))
        {
        ...
        if (unsupportedArgs & (int)UnsupportedFunctionArguments::ArrayOfArrayOfSamplerOrImage)
        {
            // Monomorphize if:
            //
        ...
        if (unsupportedArgs & (int)UnsupportedFunctionArguments::AtomicCounter)
        {
        ...
        if (unsupportedArgs & (int)UnsupportedFunctionArguments::SamplerCubeEmulation)
        {
        ...
        if (unsupportedArgs & (int)UnsupportedFunctionArguments::R32fImage)
        {
            if (type.isImage() && type.getLayoutQualifier().imageInternalFormat == EiifR32F)
            {
                return true;
            }
        }

        if (unsupportedArgs & (int)UnsupportedFunctionArguments::PixelLocalStorage)
        {
    ...
    UnsupportedFunctionArguments mUnsupportedFunctionArguments;
                                          ...
                                          UnsupportedFunctionArguments unsupportedFunctionArguments)
{
    ...
    while (true)
    {
        ...
        MonomorphizeTraverser monomorphizer(compiler, symbolTable, compileOptions,
                                            unsupportedFunctionArguments, &functionMap);
