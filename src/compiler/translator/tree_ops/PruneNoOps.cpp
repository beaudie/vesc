...
namespace
{
uint32_t GetSwitchConsantAsUInt(const TConstantUnion *value)
{
...
bool IsNoOpSwitch(TIntermSwitch *node)
{
    ...
    const uint32_t exprValue = GetSwitchConsantAsUInt(expr->getConstantValue());
    ...
    for (TIntermNode *statement : statements)
    {
        ...
        const uint32_t caseValue = GetSwitchConsantAsUInt(condition->getConstantValue());
