...
class RewriteAtomicCountersTraverser : public TIntermTraverser
{
  ...
  private:
        ...
        uint32_t *subscriptCountOut)
    {
        ...
        if (!arrayExpression)
        {
            // If the parent is not an array subscription operation, we have reached the end of the
            // subscription chain.  Note the depth that's traversed so the corresponding node can be
    ...
    {
        // Assume a general case of array declaration (GLSL only allows a maximum of 2 dimensions,
        // but it's easier to reason with N dimensions:
        //
