...
TType *GetStructSamplerParameterType(TSymbolTable *symbolTable, const TVariable &param)
{
    ...
    {
        structType->makeIdenticalArrays(param.getType());
    }
  ...
  private:
        ...
        for (const TField *field : structure->fields())
        {
            ...
            if (!fieldType.isSampler() && !isRemovedStructType(fieldType))
            {
                ...
                if (fieldType.isStructureContainingSamplers())
                {
                    ...
                    {
                        newType->makeIdenticalArrays(fieldType);
                    }
                }
                else
                {
                                ...
                                TIntermSequence *newSequence)
    {
        if (containingType.isArray())
        {
            ...
            const TSpan<unsigned int> &arraySizes = containingType.getArraySizes();
    ...
    class StructSamplerFunctionVisitor : angle::NonCopyable
    {
      ...
      private:
                                     ...
                                     const TField *field)
        {
            ...
            if (baseType.isArray())
            {
                const TSpan<unsigned int> &arraySizes = baseType.getArraySizes();
