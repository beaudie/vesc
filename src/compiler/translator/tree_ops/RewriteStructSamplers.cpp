                                                     ...
                                                     TIntermTyped *offset)
{
    ...
    while (currentNode->getAsBinaryNode())
    {
        ...
        switch (asBinary->getOp())
        {
            ...
            case EOpIndexIndirect:
            {
                TIntermBinary *multiply = new TIntermBinary(
                    EOpMul, CreateIndexNode(static_cast<int>(*it++)), asBinary->getRight());
...
TFunction *GenerateFunctionFromArguments(
    ...
    const FunctionInstantiations *generatedInstantiations)
{
    ...
    if (!newFunction)
    {
        ...
        newFunction =
            new TFunction(symbolTable, stringBuilder, function->symbolType(),
    ...
    class CreateStructSamplerFunctionVisitor final : public StructSamplerFunctionVisitor
    {
      public:
                                       ...
                                       size_t paramIndex) override
        {
            ...
            WARN() << "created sampler parameter with name " << name;
  ...
  public:
    ...
    bool visitFunctionDefinition(Visit visit, TIntermFunctionDefinition *node) override
    {
        ...
        for (TIntermNode *replacementNode : mReplacementPrototypes)
        {
            ...
            collectNewInstantiations.updateTree();
...
int RewriteStructSamplers(TIntermBlock *root, TSymbolTable *symbolTable)
{
    ...
    // Generate instantations caused by other instantiations.
    while (monomorphizeFunctions.hasPending())
    {
        monomorphizeFunctions.switchToPending();
        WARN() << "making another pass for pending instantiations!";
        root->traverse(&monomorphizeFunctions);
        monomorphizeFunctions.updateTree();
    }
