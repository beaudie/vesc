...
void CollectVariablesTraverser::visitSymbol(TIntermSymbol *symbol)
{
    ...
    else
    {
        switch (qualifier)
        {
            ...
            break;
            case EvqFragCoord:
                recordBuiltInVaryingUsed(symbol->variable(), &mFragCoordAdded,
                                         mInputVaryings);
                return;
            case EvqFrontFacing:
                recordBuiltInVaryingUsed(symbol->variable(), &mFrontFacingAdded,
                                         mInputVaryings);
                return;
            case EvqPointCoord:
                recordBuiltInVaryingUsed(symbol->variable(), &mPointCoordAdded,
                                         mInputVaryings);
                return;
            case EvqInstanceID:
                ...
                // InstanceID. Note that gl_InstanceID is not added to the symbol table for ESSL1 shaders.
            ...
            case EvqVertexID:
                ...
                return;
            case EvqPosition:
                recordBuiltInVaryingUsed(symbol->variable(), &mPositionAdded,
                                         mOutputVaryings);
                return;
            case EvqPointSize:
                recordBuiltInVaryingUsed(symbol->variable(), &mPointSizeAdded,
                                         mOutputVaryings);
                return;
            case EvqLastFragData:
                recordBuiltInVaryingUsed(symbol->variable(), &mLastFragDataAdded,
                                         mInputVaryings);
                return;
            case EvqFragColor:
            ...
            case EvqFragDepthEXT:
                recordBuiltInFragmentOutputUsed(symbol->variable(),
                                                &mFragDepthEXTAdded);
                return;
            case EvqFragDepth:
            ...
            case EvqInvocationID:
                recordBuiltInVaryingUsed(symbol->variable(), &mInvocationIDAdded,
                                         mInputVaryings);
                break;
            case EvqPrimitiveIDIn:
                recordBuiltInVaryingUsed(symbol->variable(), &mPrimitiveIDInAdded,
                                         mInputVaryings);
                break;
            case EvqPrimitiveID:
            ...
            case EvqLayer:
                if (mShaderType == GL_GEOMETRY_SHADER_EXT)
                {
                    recordBuiltInVaryingUsed(symbol->variable(), &mLayerAdded,
                                             mOutputVaryings);
                }
                      ...
                      const TExtensionBehavior &extensionBehavior)
{
    CollectVariablesTraverser collect(attributes, outputVariables, uniforms, inputVaryings,
                                      ...
                                      hashFunction, symbolTable, shaderType,
                                      extensionBehavior);
