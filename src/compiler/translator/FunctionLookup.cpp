                                                                ...
                                                                int currentIndex)
{
    ...
    std::vector<std::vector<int>> combinations =
        GetImplicitConversionCombinations(list, currentIndex - 1);
                                             ...
                                             const TIntermSequence &arguments)
{
    ...
    std::vector<std::vector<int>> combinations =
        GetImplicitConversionCombinations(indexes, indexes.size() - 1);
    for (std::vector<int> &combination : combinations)
    {
        // combination: ordered list of indexes for arguments that should be converted to int
        ImmutableStringBuilder newName(sizeof(functionName) + arguments.size() * 2);
