                                             ...
                                             bool usedStructuredbuffer)
{
    ...
    {
        // If uniform block's layout is std140, translating it to StructuredBuffer,
        // should pack StructuredBuffer's element in the end, in order to fit API
        // buffer.
    ...
    const ReferencedInterfaceBlocks &referencedInterfaceBlocks)
{
    ...
    for (const auto &blockReference : referencedInterfaceBlocks)
    {
        ...
        // buffer. https://bugs.chromium.org/p/angleproject/issues/detail?id=3682
        // TODO(xinghua.cao@intel.com): support "instanceVariable != nullptr".
        if (instanceVariable == nullptr &&
...
bool ResourcesHLSL::shouldTranslateUniformBlockToStructuredBuffer(
    const TInterfaceBlock &interfaceBlock)
{
    ASSERT(interfaceBlock.fields().size() == 1u);
    const TType &fieldType = *interfaceBlock.fields()[0]->type();

    // TODO(xinghua.cao@intel.com): support member is matrix.
    return mSRVRegister < kMaxInputResourceSlotCount && fieldType.getStruct() != nullptr &&
