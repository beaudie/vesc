...
class TParseContext : angle::NonCopyable
{
  ...
  private:
    ...
    enum class PLSIllegalOperations
    {
        ...
        // main(), we disallow return from main().
        ReturnFromMain,
    ...
    // Potential errors that get generated immediately if the parser encounters a pixel local
    // storage uniform.
    std::vector<std::tuple<const TSourceLoc, PLSIllegalOperations>> mPLSPotentialErrors;

    // Are we currently parsing the function main()?
    bool mParsingMain = false;
