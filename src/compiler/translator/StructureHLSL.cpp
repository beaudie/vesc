                                                                      ...
                                                                      const TString &name)
{
    ...
    DefinedStructs::iterator addedStruct =
        mDefinedStructs.insert(std::make_pair(name, StructProperties())).first;
    // Add element index
...
TString StructureHLSL::addStructConstructor(const TStructure &structure)
{
    ...
    if (definedStruct->second.hasConstructor)
    {
        return constructorFunctionName;  // Already added
    }
    definedStruct->second.hasConstructor = true;

    TString constructor = name + " " + constructorFunctionName + "(";
    ...
    for (const TField *field : fields)
    {
        ...
        {
            addStructConstructor(*fieldType->getStruct());
        }
    }
