...
void InsertBuiltInFunctions(sh::GLenum type, ShShaderSpec spec, const ShBuiltInResources &resources, TSymbolTable &symbolTable)
{
    ...
    if (spec != SH_CSS_SHADERS_SPEC)
    {
        ...
        {
            symbolTable.insertConstInt(ESSL1_BUILTINS, "GL_EXT_blend_func_extended", "gl_MaxDualSourceDrawBuffersEXT",
    ...
    switch (type)
    {
      case GL_FRAGMENT_SHADER:
        ...
        if (spec != SH_CSS_SHADERS_SPEC)
        {
            ...
            if (resources.EXT_blend_func_extended)
            {
                symbolTable.insert(ESSL1_BUILTINS, "GL_EXT_blend_func_extended",
                    new TVariable(NewPoolTString("gl_SecondaryFragColorEXT"),
                    TType(EbtFloat, EbpMedium, EvqSecondaryFragColor, 4)));
                TType secondaryFragData(EbtFloat, EbpMedium, EvqSecondaryFragData, 4, 1, true);
