...
TIntermNode *ElseBlockRewriter::rewriteSelection(TIntermSelection *selection)
{
    ...
    TIntermSymbol *temporarySymbolA = MakeNewTemporary(temporaryName, EbtBool);
    TIntermSymbol *temporarySymbolB = MakeNewTemporary(temporaryName, EbtBool);
    TIntermSymbol *temporarySymbolC = MakeNewTemporary(temporaryName, EbtBool);
    TIntermBinary *initializer = MakeNewBinary(EOpInitialize, temporarySymbolA,
                                               typedCondition, resultType);
    TIntermUnary *invertedSymbol = MakeNewUnary(EOpLogicalNot, temporarySymbolB);

    TIntermAggregate *declaration = new TIntermAggregate(EOpDeclaration);
    declaration->getSequence().push_back(initializer);

    TIntermAggregate *block = new TIntermAggregate(EOpSequence);
    block->getSequence().push_back(declaration);
    block->getSequence().push_back(new TIntermSelection(temporarySymbolC, selection->getTrueBlock(), NULL));
    block->getSequence().push_back(new TIntermSelection(invertedSymbol, selection->getFalseBlock(), NULL));
