...
{
  public:
    TIntermTernary(TIntermTyped *cond,
                   TIntermTyped *trueExpression,
                   TIntermTyped *falseExpression,
                   const TType &type)
        : TIntermTyped(type),
          mCondition(cond),
          mTrueBlock(trueExpression),
          mFalseBlock(falseExpression)
    {
    }
    ...
    TIntermTyped *getTrueExpression() const { return mTrueBlock; }
    TIntermTyped *getFalseExpression() const { return mFalseBlock; }
    TIntermTernary *getAsTernaryNode() override { return this; }

    TIntermTyped *deepCopy() const override { return new TIntermTernary(*this); }

    // Conservatively assume ternary operators have side-effects
    bool hasSideEffects() const override { return true; }

  protected:
    TIntermTyped *mCondition;
    TIntermTyped *mTrueBlock;
    TIntermTyped *mFalseBlock;
