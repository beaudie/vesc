...
class UnfoldShortCircuitTraverser : public TIntermTraverser
{
  ...
  protected:
    ...
    // Parent of the loop node while a loop condition is being traversed.
...
bool UnfoldShortCircuitTraverser::copyLoopConditionOrExpression()
{
    if (mInLoopCondition)
    {
        TIntermTyped *condition = mParentLoop->getCondition();
        mReplacements.push_back(NodeUpdateEntry(mParentLoop, condition,
        ...
        {
            // The loop initializer expression and one copy of the loop condition are executed before the loop.
            // They need to be in a new scope.
            ...
            loopScope->getSequence()->push_back(createTempInitDeclaration(condition));
    ...
    if (mInLoopExpression)
    {
        ...
        mReplacements.push_back(
            NodeUpdateEntry(mParentLoop, movedExpression, nullptr, false));
        TIntermAggregate *body = mParentLoop->getBody();
        TIntermSequence empty;
        TIntermSequence insertions;
