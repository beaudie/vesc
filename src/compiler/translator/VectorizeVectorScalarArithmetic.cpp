...
class VectorizeVectorScalarArithmeticTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    bool mReplaced;
    std::set<const TIntermBlock *> mInsertedInBlocks;
};
    ...
    switch (argBinary->getOp())
    {
        ...
        case EOpDivAssign:
        {
            ...
            if (!argBinary->getLeft()->hasSideEffects())
            {
                ...
                if (mInsertedInBlocks.find(parentBlock) != mInsertedInBlocks.end())
                {
                    replaceAssignInsideConstructor(node, argBinary);
                    mInsertedInBlocks.insert(parentBlock);
                    mReplaced = true;
