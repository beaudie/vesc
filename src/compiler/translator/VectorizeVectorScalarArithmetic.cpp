                ...
                switch (argBinary->getOp())
                {
                    ...
                    case EOpDivAssign:
                    {
                        ...
                        TIntermDeclaration *tempDeclaration =
                            createTempInitDeclaration(argument->deepCopy(), EvqTemporary);
                        insertStatementInParentBlock(tempDeclaration);

                        TIntermSequence argumentCopies;
                        const TType &argType =
                            tempDeclaration->getSequence()->back()->getAsTyped()->getType();
                        for (int i = 0; i < node->getNominalSize(); ++i)
                        {
                            argumentCopies.push_back(createTempSymbol(argType));
                        }
                        mMultiReplacements.push_back(
                            NodeReplaceWithMultipleEntry(node, argument, argumentCopies));
                        mReplaced = true;
                        return false;
                    }
                    default:
