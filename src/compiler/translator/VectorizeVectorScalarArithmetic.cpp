                ...
                switch (argBinary->getOp())
                {
                    ...
                    case EOpDivAssign:
                    {
                        ...
                        if (!left->hasSideEffects())
                        {
                            ...
                            nextTemporaryId();
                            // gvec s0 = gvec(a);
                            TIntermSequence assignmentTargetConstructorArguments;
                            assignmentTargetConstructorArguments.push_back(left->deepCopy());
                            TIntermAggregate *assignmentTargetConstructor =
                                TIntermAggregate::CreateConstructor(
                                    vecType, &assignmentTargetConstructorArguments);
                            TIntermDeclaration *assignmentTargetDeclaration =
                                createTempInitDeclaration(assignmentTargetConstructor);
                            // s0.x
                            TVector<int> swizzleXOffset;
                            swizzleXOffset.push_back(0);
                            TIntermSwizzle *assignmentTargetX =
                            ...
                            TIntermBinary *replacementAssignBackToTarget =
                                new TIntermBinary(EOpAssign, left->deepCopy(), assignmentTargetX);
                            // s0 *= b, a = s0.x
                            ...
                            insertStatementInParentBlock(assignmentTargetDeclaration);
