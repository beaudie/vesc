...
constexpr uint32_t kMaxVertexAttribs = MAX_VERTEX_ATTRIBS;
...
class GenMetalTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    size_t mMainUniformBufferIndex = kDefaultUniformsBindingIndex;
};
                                     ...
                                     const TType *argType1 = nullptr)
{
    switch (op)
    {
        ...
        case TOperator::EOpKill:
            ASSERT(false);  // TODO(jonahr): Needs better comment
            return "kill";
...
void GenMetalTraverser::emitFunctionParameter(const TFunction &func, const TVariable &param)
{
    ...
    if (isMain)
    {
        ...
        if (structure)
        {
            ...
            {
                mOut << " [[buffer(" << kDriverUniformsBindingIndex << ")]]";
            }
            else if (mPipelineStructs.uniformBuffers.matches(*structure))
            {
                mOut << " [[buffer(" << kUBOArgumentBufferBindingIndex << ")]]";
