...
// traverse the callees of that function and that the loop containing gradients have
// been computed; computes the discontinuous loops and the if statements that contain
// in their call graph a loop containing a gradient.
class PullComputeDiscontinuousAndGradientLoops : public TIntermTraverser
{
  public:
    ...
    // Called when a loop containing a gradient or a call to a function with a
    // loop containing a gradient in its call graph is found.
    void onGradientLoop()
    {
...
MetadataList CreateASTMetadataHLSL(TIntermNode *root, const CallDAG &callDag)
{
    ...
    // the ifs and functions using a loop containing a gradient.
    for (size_t i = 0; i < callDag.size(); i++)
    {
