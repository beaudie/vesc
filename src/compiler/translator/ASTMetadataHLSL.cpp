...
typedef std::vector<ASTMetadataHLSL> Metadatas;
...
{
  public:
    PullGradient(Metadatas *analyses, size_t index, const CallDAG &dag)
        : TIntermTraverser(true, false, true),
          mMetadatas(analyses),
          mMetadata(&(*analyses)[index]),
          mIndex(index),
          mDag(dag)
    {
        ...
        if (visit == PreVisit)
        {
            if (node->getOp() == EOpFunctionCall)
            {
                if (node->isUserDefined())
                {
                    ...
                    if ((*mMetadatas)[calleeIndex].mUsesGradient) {
                        onGradient();
                    }
                }
                else
...
{
  public:
    PullComputeDiscontinuousLoops(Metadatas *analyses, size_t index, const CallDAG &dag)
        : TIntermTraverser(true, false, true),
          mMetadatas(analyses),
          mMetadata(&(*analyses)[index]),
          mIndex(index),
    ...
    bool visitAggregate(Visit visit, TIntermAggregate *node) override
    {
        if (visit == PreVisit && node->getOp() == EOpFunctionCall)
        {
            if (node->isUserDefined())
            {
                ...
                if ((*mMetadatas)[calleeIndex].mHasDiscontinuousLoopInCallGraph) {
                    onDiscontinuousLoop();
                }
            }
        }
...
{
  public:
    PushDiscontinuousLoops(Metadatas *analyses, size_t index, const CallDAG &dag)
    ...
    bool visitAggregate(Visit visit, TIntermAggregate *node) override
    {
        switch (node->getOp())
        {
          case EOpFunctionCall:
            if (visit == PreVisit && node->isUserDefined())
            {
                size_t calleeIndex = mDag.findIndex(node);
                ASSERT(calleeIndex != CallDAG::InvalidIndex && calleeIndex < mIndex);

                (*mMetadatas)[calleeIndex].mCalledInDiscontinuousLoop = true;
            }
            break;
          default:
            break;
...
{
    std::vector<ASTMetadataHLSL> analyses(callDag.size());
    ...
    {
        analysis.mNeedsLod0 = analysis.mCalledInDiscontinuousLoop && analysis.mUsesGradient;
    }
