    ...
    : BuiltInFunctionEmulator()
{
    ...
    if (shaderType == GL_FRAGMENT_SHADER)
    {
        ...
        AddEmulatedFunction(EOpCos, float2, "webgl_emu_precision vec2 webgl_cos_emu(webgl_emu_precision float a) { return cos(a); }");
        AddEmulatedFunction(EOpCos, float3, "webgl_emu_precision vec3 webgl_cos_emu(webgl_emu_precision float a) { return cos(a); }");
        AddEmulatedFunction(EOpCos, float4, "webgl_emu_precision vec4 webgl_cos_emu(webgl_emu_precision float a) { return cos(a); }");
    }
