...
void InitBuiltInAtanFunctionEmulatorForGLSLWorkarounds(BuiltInFunctionEmulator *emu)
{
    ...
    emu->addEmulatedFunctionWithDependency(
        BuiltInFunctionEmulator::IS_DEPENDENCY, EOpAtan, float1, float1,
        "float webgl_atan_emu(float y, float x)\n"
        "{\n"
        "    if (x > 0) return atan(y / x);\n"
        "    else if (x < 0 && y >= 0) return atan(y / x) + 3.14159265;\n"
        "    else if (x < 0 && y < 0) return atan(y / x) - 3.14159265;\n"
        "    else if (x == 0 && y > 0) return 1.57079632;\n"
        "    else if (x == 0 && y < 0) return -1.57079632;\n"
        "    else return 0.0;\n"
        "}\n");
    for (int dim = 2; dim <= 4; ++dim)
    {
        const TType *floatVec = TCache::getType(EbtFloat, dim);
        std::stringstream ss;
        ss << "vec" << dim << " webgl_atan_emu(vec" << dim << " y, vec" << dim << " x)\n"
                                                                                  "{\n"
                                                                                  "    return vec"
           << dim << "(";
        for (int i = 0; i < dim; ++i)
        ...
        ss << ");\n"
              "}\n";
        emu->addEmulatedFunctionWithDependency(BuiltInFunctionEmulator::IS_DEPENDENT, EOpAtan,
                                               floatVec, floatVec, ss.str().c_str());
    }
}
