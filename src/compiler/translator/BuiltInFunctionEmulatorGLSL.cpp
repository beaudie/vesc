...
void InitBuiltInFunctionEmulatorForGLSL4_1(BuiltInFunctionEmulator *emu, sh::GLenum shaderType){
    ...
    emu->addEmulatedFunction(EOpUnpackSnorm2x16, uint1,
        ...
        "}\n");
//
//Functions uint webgl_f32tof16(float val) and float webgl_f16tof32(uint val) are based on the OpenGL redbook Appendix Session "Floating-Point Formats Used in OpenGL". 
//
    emu->addEmulatedFunction(EOpPackHalf2x16, float2,
        ...
        "    if(exponent == 128) { f16 = sign | F16_MAX_EXPONENT; if (mantissa != 0) f16 |= (mantissa & F16_MANTISSA_BITS); }\n"
        "        else if (exponent > 15) { f16 = sign | F16_MAX_EXPONENT; }\n"
        "        else if (exponent > -15) { exponent += F16_EXPONENT_BIAS; mantissa >>= F16_MANTISSA_SHIFT; f16 = sign | exponent << F16_EXPONENT_SHIFT | mantissa; }\n"
        "        else { f16 = sign; }\n"
        "    return f16;\n"
        "}\n"
    ...
    emu->addEmulatedFunction(EOpUnpackHalf2x16, uint1,
        ...
        "        else if (exponent == 31) { uint f32u = F32_INFINITY | mantissa; f32 = intBitsToFloat(int(f32u)); }\n"
        "        else{\n"
