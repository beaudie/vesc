...
// A map from index AST nodes to the access chain they define (to avoid having to reprocess the
// nodes).
// using IndexNodeToAccessChainMap = angle::HashMap<TIntermTyped *, ObjectAndAccessChain>;
// A map from variables to the access chains in which they are used and that which are |precise|.
// using VariableToPreciseAccessChainsMap = angle::HashMap<const TVariable *, TVector<AccessChain>>;
...
struct ASTInfo
{
    ...
    // IndexNodeToAccessChainMap indexNodeAccessChainMap;
    // Information pertaining to |precise| expressions:
    // VariableToPreciseAccessChainsMap preciseAccessChainMap;
...
int GetObjectPreciseSubChainLength(const ObjectAndAccessChain &object)
{
    ...
    const TFieldListCollection *structure = type.getStruct();
    if (structure == nullptr)
    {
        structure = type.getInterfaceBlock();
    }
    ...
    for (size_t length = 0; length < accessChain.size(); ++length)
    {
        ...
        const TField *field = structure->fields()[accessChain[length]];
        if (field->type()->isPrecise())
        {
            return static_cast<int>(length + 1);
        }

        structure = field->type()->getStruct();
    }
...
void AddPreciseObject(ASTInfo *info, const ObjectAndAccessChain &object)
{
    ...
    // info->preciseAccessChainMap[object.variable].push_back(object.accessChain);
...
void AddPreciseSubObjects(ASTInfo *info, const ObjectAndAccessChain &object)
{
    const TFieldListCollection *structure = object.variable->getType().getStruct();
    if (structure == nullptr)
    {
        structure = object.variable->getType().getInterfaceBlock();
    }
    const TVector<size_t> &accessChain = object.accessChain.getChain();

    for (size_t length = 0; length < accessChain.size(); ++length)
    {
        structure = structure->fields()[accessChain[length]]->type()->getStruct();
    }
    ...
    for (size_t fieldIndex = 0; fieldIndex < structure->fields().size(); ++fieldIndex)
    {
        ...
        if (structure->fields()[fieldIndex]->type()->isPrecise())
        {
...
// //- For each index AST node, the access chain it defines.
...
class PropagatePreciseTraverser : public TIntermTraverser
{
  public:
    ...
    bool visitAggregate(Visit visit, TIntermAggregate *node) override
    {
        ...
        // chain.  Otherwise there should cannot be any access chain for constructors.
