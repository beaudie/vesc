...
//   * If anyFunctionParamsReplaced(), create a new function with the replaced parameters.
...
//     replacement (getGlobalVariableReplacement() or getFunctionParamReplacement()).
...
//   * If the function is to be replaced, create a new function call with the replaced function
//     (getFunctionReplacement()) and replaced arguments (getFunctionCallArgReplacement()).
//
// Used in:
//
...
class RetypeOpaqueVariablesHelper
{
  public:
    ...
    TVariable *getVariableReplacement(const TVariable *oldVar) const
    {
        ...
        {
            return mReplacedGlobalVariables[oldVar];
        }
        else
        {
            ...
            return mReplacedFunctionParams[oldVar];
        }
    }

    // Function handling:
    void replaceFunction(const TFunction *oldFunc, TFunction *newFunc)
    {
        mReplacedFunctions[oldFunc] = newFunc;
    }
    bool isFunctionReplaced(const TFunction *func) const
    {
        return mReplacedFunctions.count(func) != 0;
    }
    TFunction *getFunctionReplacement(const TFunction *oldFunc) const
    {
        return mReplacedFunctions[oldFunc];
    }
    ...
    bool anyFunctionParamsReplaced() const { return !mReplacedFunctionParams.empty(); }
    TVariable *getFunctionParamReplacement(const TVariable *oldParam) const
    {
        ASSERT(mReplacedFunctionParams.count(oldParam) != 0);
        return mReplacedFunctionParams[oldParam];
    }

    // Function call arguments handling:
    void preVisitAggregate() { mReplacedFunctionCallArgs.clear(); }
    void replaceFunctionCallArg(const TIntermNode *oldArg, TIntermNode *newArg)
    {
        mReplacedFunctionCallArgs[oldArg] = newArg;
    }
    TIntermNode *getFunctionCallArgReplacement(const TIntermNode *oldArg) const
    {
        ASSERT(mReplacedFunctionCallArgs.count(oldArg) != 0);
        return mReplacedFunctionCallArgs[oldArg];
    }
  ...
  private:
    ...
    // call.
    std::unordered_map<const TIntermNode *, TIntermTyped *> mReplacedFunctionCallArgs;
};
