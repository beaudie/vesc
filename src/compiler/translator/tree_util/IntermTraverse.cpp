...
const TAction *TIntermPreprocessorDirective::visit(Visit visit, TIntermTraverser2 *it)
{
    return it->visitPreprocessorDirective(visit, this);
}
...
TIntermTraverser2::TIntermTraverser2(bool preVisit,
    ...
    bool postVisit,
    TSymbolTable *symbolTable)
    : mPreVisit(preVisit), mInVisit(inVisit), mPostVisit(postVisit), mSymbolTable(symbolTable), mDefaultLeafNodeAction(Continue::FromNextSibling)
{
}
...
void TIntermTraverser2::traverse(TIntermNode *node)
{
    ...
    {
        if (mTraversalStack.back().childIndex >= std::max(static_cast<unsigned int>(currentNode->getChildCount()), 1u))
        {
            if (mPostVisit)
            {
