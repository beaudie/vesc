...
TFieldList *DriverUniformExtended::createUniformFields(TSymbolTable *symbolTable)
{
    ...
    constexpr size_t kNumGraphicsDriverUniformsExt = 6;
    constexpr std::array<const char *, kNumGraphicsDriverUniformsExt>
        kGraphicsDriverUniformNamesExt = {{kHalfRenderArea, kFlipXY, kNegFlipXY, kFragRotation,
                                           kPreRotation,
                                           kEmulatedInstanceId}};  //, kCoverageMask}};

    const std::array<TType *, kNumGraphicsDriverUniformsExt> kDriverUniformTypesExt = {{
        new TType(EbtFloat, 2), new TType(EbtFloat, 2), new TType(EbtFloat, 2),
        // NOTE: There's a vec2 gap here that can be used in the future
        new TType(EbtFloat, 2, 2), new TType(EbtFloat, 2, 2), new TType(EbtUInt),
        // new TType(EbtUInt),
    }};
