  ...
  public:
    ...
    void fillDataStructures(std::vector<Record> &records, std::map<int, int> &idToIndex)
    {
        ...
        for (auto &it : mFunctions)
        {
            ...
            idToIndex[data.node->getUniqueId()] = data.index;
        }
    }
...
int CallDAG::getIndex(const TIntermAggregate *function) const
{
    TOperator op = function->getOp();
    ASSERT(op == EOpPrototype || op == EOpFunction || op == EOpFunctionCall);

    auto it = mUniqueIdToIndex.find(function->getUniqueId());

    if (it == mUniqueIdToIndex.end())
    {
        return -1;
    }
    else
...
{
    return mRecords[getIndex(function)];
}
...
CallDAG::InitResult CallDAG::init(TIntermNode *root, TInfoSinkBase *info)
{
    ...
    creator.fillDataStructures(mRecords, mUniqueIdToIndex);
    return INITDAG_SUCCESS;
}