...
bool SimplifyLoopConditionsTraverser::visitAggregate(Visit visit, TIntermAggregate *node)
{
    ...
    // We only need to traverse nodes that may contain statements outside loop conditions.
    if (!mInsideLoopConditionOrExpression)
...
void SimplifyLoopConditionsTraverser::traverseLoop(TIntermLoop *node)
{
    ...
    // Note: No need to traverse the init node.
    ...
    if (node->getCondition())
    {
        ...
        if (mFoundLoopToChange)
        {
            ...
            if (loopType == ELoopWhile)
            {
                ...
                // Can't use NodeUpdateEntry to replace old body, since it may have been nullptr.
                // It's safe to replace the body in place here.
                node->setBody(newBody);

                NodeUpdateEntry replaceCondition(node, node->getCondition(),
                                                 createTempSymbol(node->getCondition()->getType()),
                                                 false);
                mReplacements.push_back(replaceCondition);
            }
            ...
            else if (loopType == ELoopFor)
            {
                ...
                NodeUpdateEntry replaceLoop(getAncestorNode(1), node, loopScope, false);
                mReplacements.push_back(replaceLoop);
            }
        }
    }
