...
void SimplifyLoopConditionsTraverser::traverseLoop(TIntermLoop *node)
{
    ...
    if (mFoundLoopToChange)
    {
        ...
        else if (loopType == ELoopFor)
        {
            ...
            TIntermSequence *loopSequence = loopScope->getSequence();

            // Insert "init;"
            if (node->getInit())
            {
                loopSequence->push_back(node->getInit());
            }
            ...
            loopSequence->push_back(createTempInitDeclaration(conditionInitializer));
