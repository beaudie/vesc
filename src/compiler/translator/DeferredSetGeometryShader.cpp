...
class DeferredGSTraverser : public TIntermTraverser
{
  public:
    ...
    {
        mStatusOfVisitDeclaration = true;
    }

    bool IsVisitDeclarationNormal() { return mStatusOfVisitDeclaration; }

  private:
    bool visitDeclaration(Visit visit, TIntermDeclaration *node) override;
    bool mStatusOfVisitDeclaration;
};
...
bool DeferredGSTraverser::visitDeclaration(Visit visit, TIntermDeclaration *node)
{
    ...
    if (symbol->getQualifier() == EvqGeometryIn && symbol->isArray())
    {
        ...
        if (!mSymbolTable || !mSymbolTable->getGlInVariableWithArraySize())
        {
            mStatusOfVisitDeclaration = false;
            return false;
        }
        else
        {
            TLayoutPrimitiveType primitiveType = mSymbolTable->getGlInVariableWithArraySize()
                                                     ->getType()
                                                     .getLayoutQualifier()
                                                     .primitiveType;
            if (primitiveType == TLayoutPrimitiveType::EptUndefined)
            {
                mStatusOfVisitDeclaration = false;
                return false;
            }
                                                         ...
                                                         TSymbolTable *symbolTable)
{
    ...
    if (!traverser.IsVisitDeclarationNormal())
    {
        return false;
    }
