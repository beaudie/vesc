    ...
    TIntermFunctionDefinition *node)
{
    ...
    if (visit == PostVisit)
    {
        ASSERT(!node->getFunctionPrototype()->isArray());
        // This isn't conditional on node->isArray() since the type has already been changed when visiting the prototype node that stores the function header of the function definition.
