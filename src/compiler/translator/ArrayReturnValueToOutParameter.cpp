...
TIntermSymbol *GetReturnValueOutSymbol(const TType &type)
{
    TType outType(type);
    outType.setQualifier(EvqOut);
    return GetReturnValueSymbol(outType);
}
    ...
    if (visit == PreVisit)
    {
        if (node->isArray())
        {
            if (node->getOp() == EOpFunction)
            {
                ...
                replacementParams->getSequence()->push_back(GetReturnValueOutSymbol(node->getType()));
            ...
            else if (node->getOp() == EOpPrototype)
            {
                ...
                replacement->getSequence()->push_back(GetReturnValueOutSymbol(node->getType()));
...
bool ArrayReturnValueToOutParameterTraverser::visitBranch(Visit visit, TIntermBranch *node)
{
    if (mInFunctionWithArrayReturnValue && node->getFlowOp() == EOpReturn)
    {
        ...
        replacementAssignment->setLeft(GetReturnValueSymbol(expression->getType()));
