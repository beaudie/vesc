                                        ...
                                        TIntermTyped *returnValueTarget)
{
    ...
    for (auto &param : *originalArguments)
    {
        replacementArguments->push_back(param);
    }
    replacementArguments->push_back(returnValueTarget);
    TIntermAggregate *replacementCall = new TIntermAggregate(TType(EbtVoid), EOpCallFunctionInAST, replacementArguments);
