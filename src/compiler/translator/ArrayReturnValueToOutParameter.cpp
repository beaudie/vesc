...
TIntermSymbol *GetReturnValueSymbol(const TType &type)
{
    TType outType(type);
    outType.setQualifier(EvqOut);
    return new TIntermSymbol(0, "angle_return", outType);
}

};

ArrayReturnValueToOutParameter::ArrayReturnValueToOutParameter()
    : TIntermTraverser(true, false, true),
      mInFunctionWithArrayReturnValue(false)
{
}

bool ArrayReturnValueToOutParameter::visitAggregate(Visit visit, TIntermAggregate *node)
{
    if (visit == PreVisit)
    {
        if (node->getOp() == EOpFunction && node->isArray())
        {
            ...
            replacementParams->getSequence()->push_back(GetReturnValueSymbol(node->getType()));
        ...
        else if (node->getOp() == EOpPrototype && node->isArray())
        {
            ...
            replacement->getSequence()->push_back(GetReturnValueSymbol(node->getType()));
...
bool ArrayReturnValueToOutParameter::visitBranch(Visit visit, TIntermBranch *node)
{
    if (mInFunctionWithArrayReturnValue && node->getFlowOp() == EOpReturn)
    {
        ...
        replacementAssignment->setLeft(new TIntermSymbol(0, "angle_return", expression->getType()));
