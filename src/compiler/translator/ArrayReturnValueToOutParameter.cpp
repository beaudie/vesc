...
TIntermAggregate *CreateReplacementCall(TIntermAggregate *originalCall, TIntermTyped *returnValueTarget)
{
    ...
    *replacementCall->getFunctionInfo() = *originalCall->getFunctionInfo();
...
bool ArrayReturnValueToOutParameterTraverser::visitFunctionDefinition(Visit visit, TIntermFunctionDefinition *node)
{
    if (node->isArray())
    {
        if (visit == PreVisit)
    ...
    if (visit == PreVisit)
    {
        if (node->isArray())
        {
            if (node->getOp() == EOpPrototype)
            {
                ...
                *replacement->getFunctionInfo() = *node->getFunctionInfo();
