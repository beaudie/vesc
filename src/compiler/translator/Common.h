...
class TSpan
{
  public:
    ...
    // so the memory will live on even if the TVector is destroyed.  Note that the input TVectors
    // are not const.
    TSpan(TVector<T> &vec) : mData(vec.data()), mSize(vec.size()) {}
    TSpan &operator=(TVector<T> &vec)
    {
        mData = vec.data();
        mSize = vec.size();
        return *this;
    ...
    bool operator==(const TSpan &that) const
    {
        if (mSize != that.mSize)
        {
            return false;
    ...
    bool operator!=(const TSpan &that) const { return !(*this == that); }
