...
class ShaderStorageBlockOutputHLSL : public TIntermTraverser
{
  public:
    ...
    // function call.
    void outputStoreFunctionCallPrefix(TIntermBinary *node);
    // This writes the funciton call to load a SSBO value to the output stream.
    void outputLoadFunctionCall(TIntermBinary *node);
  ...
  private:
    void traverseAssignmentInternal(TIntermTyped *node, SSBOMethod method);
