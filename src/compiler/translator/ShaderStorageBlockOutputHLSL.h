...
class ShaderStorageBlockOutputHLSL
{
  public:
    ...
    TIntermTyped *createByteAddressExpression(TInfoSinkBase &out,
                                              TIntermTyped *node,
                                              BlockMemberInfo *blockMemberInfo);

  private:
    void traverseSSBOAccess(TIntermTyped *node, SSBOMethod method);
    int getMatrixStride(TIntermTyped *node, TLayoutBlockStorage storage, bool rowMajor) const;
    TIntermTyped *convertEOpIndexDirectOrIndirectOutput(TInfoSinkBase &out,
                                                        TIntermBinary *node,
                                                        BlockMemberInfo *blockMemberInfo);
    // Common part in dot operations.
    const TField *findBaseField(TIntermTyped *node) const;
