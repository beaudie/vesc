...
void TOutputGLSL::visitSymbol(TIntermSymbol *node)
{
	...
	else if (symbol == "gl_FragColor" && isCoreGLSLOutput(getShaderOutput()))
    {
        out << "webgl_FragColor";
    }
	else if (symbol == "gl_FragData" && isCoreGLSLOutput(getShaderOutput()))
    {
        out << "webgl_FragData";
    }
    else
...
TString TOutputGLSL::translateTextureFunction(TString &name)
{
	...
	const char **mapping = (isCoreGLSLOutput(getShaderOutput())) ?
