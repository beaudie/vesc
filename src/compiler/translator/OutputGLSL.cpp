...
void TOutputGLSL::visitSymbol(TIntermSymbol *node)
{
    ...
    else if (symbol == "gl_FragColor" && IsGLSLOutput(getShaderOutput()))
    {
        out << "webgl_FragColor";
    }
    else if (symbol == "gl_FragData" && IsGLSLOutput(getShaderOutput()))
    {
        out << "webgl_FragData";
    }
    else
...
TString TOutputGLSL::translateTextureFunction(TString &name)
{
    ...
    const char **mapping = (IsGLSLOutput(getShaderOutput())) ?
