...
postfix_expression
    ...
    | postfix_expression INC_OP {
        if (context->lValueErrorCheck(@2, "++", $1))
            context->recover();
        $$ = context->addUnaryMath(EOpPostIncrement, "++", $1, @2);
    }
    | postfix_expression DEC_OP {
        if (context->lValueErrorCheck(@2, "--", $1))
            context->recover();
        $$ = context->addUnaryMath(EOpPostDecrement, "--", $1, @2);
    }
    ;
...
unary_expression
    ...
    | INC_OP unary_expression {
        if (context->lValueErrorCheck(@1, "++", $2))
            context->recover();
        $$ = context->addUnaryMath(EOpPreIncrement, "++", $2, @1);
    }
    | DEC_OP unary_expression {
        if (context->lValueErrorCheck(@1, "--", $2))
            context->recover();
        $$ = context->addUnaryMath(EOpPreDecrement, "--", $2, @1);
    }
    | unary_operator unary_expression {
        if ($1.op != EOpNull) {
            const char* opStr = "";
            switch($1.op) {
              case EOpNegative:   opStr = "-"; break;
              case EOpPositive:   opStr = "+"; break;
              case EOpLogicalNot: opStr = "!"; break;
              case EOpBitwiseNot: opStr = "~"; break;
              default: break;
            }
            $$ = context->addUnaryMath($1.op, opStr, $2, @1);
        } else
            $$ = $2;
    }
    ;
