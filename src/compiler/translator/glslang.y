...
%token <lex> LAYOUT
%token <lex> YUVCSCSTANDARDEXT ITU601 ITU601FULLRANGE ITU709
...
primary_expression
    ...
    | ITU601 {
        if (!context->supportsExtension("GL_EXT_YUV_target")) {
           context->error(@1, "unsupported value", "itu_601");
        }
        TConstantUnion *unionArray = new TConstantUnion[1];
        unionArray->setYuvCscStandardEXTConst(EycsItu601);
        $$ = context->intermediate.addConstantUnion(unionArray, TType(EbtYuvCscStandardEXT, EbpUndefined, EvqConst), @1);
    }
    | ITU601FULLRANGE {
        if (!context->supportsExtension("GL_EXT_YUV_target")) {
           context->error(@1, "unsupported value", "itu_601_full_range");
        }
        TConstantUnion *unionArray = new TConstantUnion[1];
        unionArray->setYuvCscStandardEXTConst(EycsItu601FullRange);
        $$ = context->intermediate.addConstantUnion(unionArray, TType(EbtYuvCscStandardEXT, EbpUndefined, EvqConst), @1);
    }
    | ITU709 {
        if (!context->supportsExtension("GL_EXT_YUV_target")) {
           context->error(@1, "unsupported value", "itu_709");
        }
        TConstantUnion *unionArray = new TConstantUnion[1];
        unionArray->setYuvCscStandardEXTConst(EycsItu709);
        $$ = context->intermediate.addConstantUnion(unionArray, TType(EbtYuvCscStandardEXT, EbpUndefined, EvqConst), @1);
    }
