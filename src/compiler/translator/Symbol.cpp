...
ImmutableString TSymbol::name() const
{
    ...
    return ImmutableString(symbolNameOut.str());
}
...
{
    return symbolType() == SymbolType::UserDefined && strcmp(name(), "main") == 0;
}

bool TFunction::isImageFunction() const
{
    return symbolType() == SymbolType::BuiltIn &&
           (strcmp(name(), "imageSize") == 0 || strcmp(name(), "imageLoad") == 0 ||
            strcmp(name(), "imageStore") == 0);
}
