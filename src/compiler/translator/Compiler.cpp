                                    ...
                                    ShCompileOptions compileOptions)
{
    ...
    {
        auto plsUsage =
            sh::ValidatePixelLocalStorage(root, mShaderType, &mSymbolTable, &mDiagnostics);
        if (plsUsage == PixelLocalStorageUsage::Invalid)
        {
            return false;  // The shader has PLS errors.
        }
        if (plsUsage == PixelLocalStorageUsage::Valid &&
