                                         ...
                                         const ShCompileOptions compileOptions)
{
    ...
    if (success)
    {
        ...
        if (success && shaderType == GL_GEOMETRY_SHADER_OES)
        {
            ...
            // compile error.
            // [Section 11.1gs.1, Geometry Shader Input Primitives] A program will fail to link if
            // the input primitive type is not specified by the geometry shader object attached to
            // the program.
            // We only do this when a valid geometry shader input primitive declaration is present.
