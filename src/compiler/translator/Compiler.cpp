    ...
    size_t numStrings, int compileOptions)
{
    ...
    if (success)
    {
        ...
        if (success)
            success = initDag(root);

        if (success && (compileOptions & SH_LIMIT_CALL_STACK_DEPTH))
            success = checkCallDepth();

        // Checks which functions are used and also returns true if "main" exists
        if (success)
        {
            functionAnalyses.clear();
            functionAnalyses.resize(mDag.size());
...
bool TCompiler::initDag(TIntermNode *root)
{
    mDag.clear();
    if (mDag.init(root, &infoSink) != CallDAG::INITDAG_SUCCESS)
    {
        infoSink.info.prefix(EPrefixError);
...
{
    std::vector<int> depths(mDag.size());

    for (int i = 0; i < mDag.size(); i++)
    {
        int depth = 0;
        auto &record = mDag.getRecord(i);
        ...
        if (depth >= maxCallStackDepth)
        {
            ...
            {
                infoSink.info << " -> " << mDag.getRecord(currentFunction).name;

                int nextFunction = -1;
                for (auto& calleeIndex : mDag.getRecord(currentFunction).callees)
                {
...
{
    int mainIndex = mDag.mangledNameToIndex("main(");
    if (mainIndex == -1)
    {
        infoSink.info.prefix(EPrefixError);
...
{
    if (functionAnalyses[index].used)
    {
        return;
    }
    ...
    for (int calleeIndex : mDag.getRecord(index).callees)
    {
