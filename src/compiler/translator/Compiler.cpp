    ...
    size_t numStrings, int compileOptions)
{
    ...
    if (success)
    {
        ...
        if (success)
        {
            ...
            functionAnalyses.resize(mCallDag.size());
...
{
    mCallDag.clear();
    if (mCallDag.init(root, &infoSink) != CallDAG::INITDAG_SUCCESS)
    {
        infoSink.info.prefix(EPrefixError);
...
bool TCompiler::checkCallDepth()
{
    ...
    for (int i = 0; i < mCallDag.size(); i++)
    {
        int depth = 0;
        auto &record = mCallDag.getRecord(i);
        ...
        if (depth >= maxCallStackDepth)
        {
            ...
            {
                infoSink.info << " -> " << mCallDag.getRecord(currentFunction).name;

                int nextFunction = -1;
                for (auto& calleeIndex : mCallDag.getRecord(currentFunction).callees)
                {
...
{
    int mainIndex = mCallDag.mangledNameToIndex("main(");
    if (mainIndex == -1)
    {
        infoSink.info.prefix(EPrefixError);
...
void TCompiler::internalTagUsedFunction(int index)
{
    ...
    for (int calleeIndex : mCallDag.getRecord(index).callees)
    {
