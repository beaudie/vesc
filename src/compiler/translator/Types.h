...
class TType
{
  ...
  private:
    ...
    constexpr void makeArraysInternal(const TSpan<const unsigned int> &sizes)
    {
        mArraySizes = sizes;
    ...
    // modifications to array sizes happen on the storage (and if dimensions change, also
    // mArraySizes) and all reads are from mArraySizes.
