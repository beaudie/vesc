    ...
    bool getDimensionsIgnoresBaseLevel)
{
    ...
    if (IsSamplerCube(textureFunction.sampler))
    {
            ...
            textureFunction.method == TextureFunctionHLSL::TextureFunction::GRAD)
        {
            ...
            {
                // clang-format off
                // We would like to calculate tha maximum of how many texels we move in the major face's texture as we
                // move across the screen in any direction.
                // Namely, we want the length of the directional derivative of the function p (defined below),
                // maximized over screen space directions. (For short: we want the norm of Dp.)
                // For simplicity, assume that z-axis is the major axis. By symmetry, we can assume that the positive
                // z direction is major. (The calculated value will be the same even if this is false.)
                // Let r denote the function from screen position to cube texture coordinates.
                // Then p can be written as p = s . P . r, where P(r) = (r.x, r.y)/r.z is the projection onto the major
                // cube face, and s = diag(width, height)/2.
                // (s linearly maps from the cube face into texture space, so that p(r) is in units of texels.)
                // The derivative is
                // Dp(r) = s |1 0 -r.x/r.z|
                ...
                // Since ddx(r) and ddy(r) are unknown, the best we can do is to sample L in some directions and take
                // the maximum across the samples.
                //
                // Some implementations use max(L(n1), L(n2)) where n1 = vec2(1,0) and n2 = vec2(0,1).
                //
                // Some implementations use max(L(n1), L(n2), L(n3), L(n4)),
                // where n3 = (n1 + n2) / |n1 + n2| = (n1 + n2)/sqrt(2)
                //       n4 = (n1 - n2) / |n1 - n2| = (n1 - n2)/sqrt(2).
                // In other words, two samples along the diagonal screen space directions have been added, giving a
                // strictly better estimate of the true maximum.
                //
                // It turns out we can get twice the sample count very cheaply.
                // We can use the linearity of Dp(r) to get these extra samples of L cheaply in terms of the already
                // taken samples, L(n1) and L(n2):
                // Denoting
                // dpx = Dp(r)n1, dpy = Dp(r)n2, dpxx = dot(dpx, dpx), dpyy = dot(dpy, dpy), dpxy = dot(dpx, dpy),
                // we obtain
                ...
                // So the extra cost is: one dot, one abs, one add, one multiply-add and one max. (All scalar.)
                //
                // In section 3.8.10.1, the OpenGL ES 3 specification defines the "scale factor" (rho).
                // In our terminology, this definition works out to taking sqrt(max(L(n1), L(n2))).
                // Some implementations will use this estimate, here we use the strictly better
                // sqrt(max(L(n1), L(n2), L(n3), L(n4))), since it's not much more expensive to calculate.
                // clang-format on
