                                     ...
                                     const TString &samplerReference)
{
    ...
    else if (textureFunction.method == TextureFunctionHLSL::TextureFunction::GRAD)
    {
        ...
        {
            out << ".SampleCmpLevelZero(" << samplerReference << ", ";
        }
        else
        {
            out << ".SampleGrad(" << samplerReference << ", ";
        }
    }
...
const TextureFunctionHLSL::TextureFunction &TextureFunctionHLSL::useTextureFunction(
    const TString &name,
    ...
    sh::GLenum shaderType)
{
    ...
    {
        unsigned int mandatoryArgumentCount =
            2;  // All functions have sampler and coordinate arguments
    ...
    return *(mUsesTexture.insert(textureFunction).first);
}
...
void TextureFunctionHLSL::textureFunctionHeader(TInfoSinkBase &out, const ShShaderOutput outputType)
{
    for (const TextureFunction &textureFunction : mUsesTexture)
    {
        ...
        {
            int hlslCoords = GetHLSLCoordCount(textureFunction, outputType);
            ...
            {
                OutputHLSL4SampleFunctionPrefix(out, textureFunction, textureReference,
                                                samplerReference);
            }
            else
                UNREACHABLE();
            ...
            out << ");\n";  // Close the sample function call
        }
