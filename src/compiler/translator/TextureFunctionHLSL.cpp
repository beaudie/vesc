    ...
    if (IsSamplerCube(textureFunction.sampler))
    {
            ...
            textureFunction.method == TextureFunctionHLSL::TextureFunction::GRAD)
        {
            if (textureFunction.method == TextureFunctionHLSL::TextureFunction::IMPLICIT)
            {
                ...
                // Note that if the major axis changes sign when differentiating, the correct LOD is
                // infinity.
                out << "    float2 dims = float2(width, height)/2.0;\n"
                       "    float2 dx = dims * (xMajor ? ddx(r.zy/r.x) : yMajor ? ddx(r.xz/r.y) : "
                       "ddx(r.xy/r.z));\n"
                       "    float2 dy = dims * (xMajor ? ddy(r.zy/r.x) : yMajor ? ddy(r.xz/r.y) : "
                       "ddy(t.xy/r.z));\n"
                       "    float3 ddxts = ddx(sign(t))\n;"
                       "    float3 ddyts = ddy(sign(t))\n;"
                       "    bool infiniteDerivative = any(xMajor ? float2(ddxts.x, ddyts.x) : "
                       "yMajor ? float2(ddxts.y, ddyts.y) : float2(ddxts.z, ddyts.z));\n"
                       "    float lod = infiniteDerivative ? 1.#INF : 0.5f * log2(max(dot(dx, dx), "
                       "dot(dy, dy)));\n";
            }
