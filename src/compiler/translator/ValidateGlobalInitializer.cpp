...
{
    // Constant unions that are not constant expressions may result from folding a ternary expression.
    switch (node->getType().getQualifier())
    {
        case EvqConst:
            break;
...
ValidateGlobalInitializerTraverser::ValidateGlobalInitializerTraverser(int shaderVersion)
    : TIntermTraverser(true, false, false), mShaderVersion(shaderVersion), mIsValid(true), mIssueWarning(false)
{
}
