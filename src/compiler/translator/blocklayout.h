...
{
  public:
    VariableNameVisitor(const std::string &instanceName);
    ~VariableNameVisitor();
  ...
  protected:
    ...
    std::string collapseMappedNameStack(const std::string &top) const;
...
{
  public:
    BlockEncoderVisitor(const std::string &instanceName, BlockLayoutEncoder *encoder);
    ~BlockEncoderVisitor();
