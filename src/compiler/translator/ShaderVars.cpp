...
bool Varying::isSameVaryingAtLinkTime(const Varying &other, int shaderVersion) const
{
    return (ShaderVariable::isSameVariableAtLinkTime(other, false) &&
            GetNonAuxiliaryInterpolationType(interpolation) ==
                GetNonAuxiliaryInterpolationType(other.interpolation) &&
...
{
    if (name != other.name || mappedName != other.mappedName ||
        instanceName != other.instanceName || arraySize != other.arraySize ||
