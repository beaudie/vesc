...
CompiledShaderState::CompiledShaderState()
    : shaderVersion(100), numViews(-1), geometryShaderInvocations(1)
{
    localSize.fill(-1);
}
...
void CompiledShaderState::serialize(gl::BinaryOutputStream &stream, GLenum shaderType) const
{
    ...
    stream.writeInt(specConstUsageBits);
    ...
    switch (shaderType)
    {
        ...
        case GL_FRAGMENT_SHADER:
        {
            ...
            stream.writeInt(advancedBlendEquationBits);
            break;
        }
        case GL_GEOMETRY_SHADER:
        {
            ...
            stream.writeBool(hasValidGeometryShaderInputPrimitiveType);
            if (hasValidGeometryShaderInputPrimitiveType)
            {
                unsigned char value = (unsigned char)geometryShaderInputPrimitiveType;
                stream.writeBytes(&value, 1);
            }
            stream.writeBool(hasValidGeometryShaderOutputPrimitiveType);
            if (hasValidGeometryShaderOutputPrimitiveType)
            {
                unsigned char value = (unsigned char)geometryShaderOutputPrimitiveType;
                stream.writeBytes(&value, 1);
            }
            stream.writeBool(hasValidGeometryShaderMaxVertices);
            if (hasValidGeometryShaderMaxVertices)
            {
                unsigned char value = (unsigned char)geometryShaderMaxVertices;
                stream.writeBytes(&value, 1);
            }
...
void CompiledShaderState::deserialize(gl::BinaryInputStream &stream, GLenum shaderType)
{
    ...
    specConstUsageBits = stream.readInt<uint32_t>();
    ...
    switch (shaderType)
    {
        ...
        case GL_GEOMETRY_SHADER:
        {
            ...
            stream.readBool(&hasValidGeometryShaderInputPrimitiveType);
            if (hasValidGeometryShaderInputPrimitiveType)
            {
                unsigned char value;
                stream.readBytes(&value, 1);
                geometryShaderInputPrimitiveType = static_cast<uint8_t>(value);
            }

            stream.readBool(&hasValidGeometryShaderOutputPrimitiveType);
            if (hasValidGeometryShaderOutputPrimitiveType)
            {
                unsigned char value;
                stream.readBytes(&value, 1);
                geometryShaderOutputPrimitiveType = static_cast<uint8_t>(value);
            }

            stream.readBool(&hasValidGeometryShaderMaxVertices);
            if (hasValidGeometryShaderMaxVertices)
            {
                unsigned char value;
                stream.readBytes(&value, 1);
                geometryShaderMaxVertices = static_cast<uint8_t>(value);
            }
