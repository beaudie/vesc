...
{
    // It's not an error to specify a binding on some but not all declarations for the same name
    // according to GLSL ES Spec 3.10.4, section 4.4.5. But GLSL ES Spec 3.10.4, section 9.2.1.
    // requires the qualifiers must match exactly. It looks like a spec bug to be resolved. For now
    // we favor the section 4.4.5 enforcing a consistent match merely.
