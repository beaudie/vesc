...
class UseUniformBlockMembers : public TIntermTraverser
{
  ...
  private:
    ...
    TType GenerateInterfaceBlockType(const InterfaceBlock &block);
                                                   ...
                                                   TIntermSequence *sequence)
{
    ...
    if (var.isArray())
    {
        ...
        for (unsigned int i = 0; i < var.arraySize; ++i)
        {
            ...
            TIntermAggregate *tempDeclaration = createTempInitDeclaration(element);
    ...
    else
    {
        ...
        TIntermAggregate *tempDeclaration = createTempInitDeclaration(symbol);
        sequence->insert(sequence->begin(), tempDeclaration);
        nextTemporaryIndex();
    }
}

TType UseUniformBlockMembers::GenerateInterfaceBlockType(const InterfaceBlock &block)
{
    TType type;
...
void UseUniformBlockMembers::insertUseCode(TIntermSequence *sequence)
{
    for (const auto &block : mBlocks)
    {
        ...
        {
            TType type                 = GenerateInterfaceBlockType(block);
