                                                   ...
                                                   TIntermSequence *sequence)
{
    ...
    if (var.isArray())
    {
        ...
        // |type| is not the array element type,
        TType type = GetShaderVariableBasicType(var);
    ...
    {
        TVariable *structInfo = reinterpret_cast<TVariable *>(mSymbolTable.findAtGlobalLevel(name));
        ASSERT(structInfo);
        TIntermSymbol *structSymbol = new TIntermSymbol(0, name, structInfo->getType());
    ...
    {
        TIntermSymbol *symbol = new TIntermSymbol(0, name, GetShaderVariableBasicType(var));
...
void UseUniformBlockMembers::insertUseCode(TIntermSequence *sequence)
{
    for (const auto &block : mBlocks)
    {
        ...
        else if (block.arraySize > 0)
        {
            ...
            TVariable *ubInfo = reinterpret_cast<TVariable *>(mSymbolTable.findAtGlobalLevel(name));
            ASSERT(ubInfo);
