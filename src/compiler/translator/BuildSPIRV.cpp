...
bool operator==(const SpirvType &a, const SpirvType &b)
{
    ...
    return a.type == b.type && a.primarySize == b.primarySize &&
           ...
           a.typeDiff.blockStorage == b.typeDiff.blockStorage &&
           a.typeDiff.isRowMajorQualifiedArray == b.typeDiff.isRowMajorQualifiedArray;
}
...
bool IsNonSquareRowMajorArrayInBlock(const TType &type,
                                     const SpirvTypeDifferentiation &parentTypeDiff)
{
    return parentTypeDiff.blockStorage != EbsUnspecified && type.isArray() && type.isMatrix() &&
           type.getCols() != type.getRows() &&
           IsBlockFieldRowMajorQualified(type, parentTypeDiff.isRowMajorQualifiedBlock);
}
...
void SpirvTypeDifferentiation::onArrayElementSelection(bool isElementTypeBlock,
                                                       bool isElementTypeArray)
{
...
SpirvTypeData SPIRVBuilder::declareType(const SpirvType &type, const TSymbol *block)
{
    ...
    if (!type.arraySizes.empty())
    {
        ...
        subType.typeDiff.onArrayElementSelection(subType.block != nullptr,
...
// These functions are nearly identical to getTypeData(), except for row-major matrices.  For the
// purposes of base alignment and size calculations, they swap the primary and secondary sizes such
...
//
// Since block storage doesn't propagate to the base matrix type (as it doesn't affect the type),
// its size, alignment and stride cannot be calculated just be looking at the type itself.  For the
// purposes of size, alignment and stride calculations, GLSL treats matrices the same as arrays, so
// this function produces the vector type corresponding to a single column of the matrix and returns
// the number of columns.  The caller applies the std140 or std430 rules and applies the column
// count as if it was another array dimension.
const SpirvTypeData &SPIRVBuilder::getArrayElementTypeDataForAlignmentAndSize(
    SpirvType elementSpirvType,
    const SpirvTypeDifferentiation &parentTypeDiff,
    uint32_t *matrixColumnsCountOut)
{
    ASSERT(elementSpirvType.arraySizes.empty());

    // If the element is row-major, swap the rows and columns for the purposes of base alignment
    // calculation.
    if (parentTypeDiff.isRowMajorQualifiedArray)
    {
        std::swap(elementSpirvType.primarySize, elementSpirvType.secondarySize);
    }

    *matrixColumnsCountOut = 1;
    if (elementSpirvType.secondarySize > 1)
    {
        *matrixColumnsCountOut         = elementSpirvType.primarySize;
        elementSpirvType.primarySize   = elementSpirvType.secondarySize;
        elementSpirvType.secondarySize = 1;
    }

    return getSpirvTypeData(elementSpirvType, nullptr);
}
...
{
    SpirvTypeDifferentiation fieldTypeDiff = parentTypeDiff;
    fieldTypeDiff.onBlockFieldSelection(type);

    // Get the array element type if arrayed, and return the array size for size calculations.
    TType elementType(type);
    elementType.toArrayBaseType();
    fieldTypeDiff.onArrayElementSelection(type.getStruct() != nullptr, false);

    SpirvType fieldSpirvType = getSpirvType(elementType, fieldTypeDiff);

    // If the field is row-major, swap the rows and columns for the purposes of base alignment
    // calculation.  If the type is already marked with |isRowMajorQualifiedArray|, there's no need
    // for this as the alignment and size is already calculated for the swapped dimensions.
    const bool isRowMajor =
        IsBlockFieldRowMajorQualified(type, parentTypeDiff.isRowMajorQualifiedBlock);
    if (isRowMajor && !fieldTypeDiff.isRowMajorQualifiedArray)
    {
        std::swap(fieldSpirvType.primarySize, fieldSpirvType.secondarySize);
    }

    if (totalColumnsCountOut)
    {
        *totalColumnsCountOut = GetTotalArrayElements(type.getArraySizes());
        if (fieldSpirvType.secondarySize > 1)
        {
            *totalColumnsCountOut *= fieldSpirvType.primarySize;
        }
    }

    if (fieldSpirvType.secondarySize > 1)
    {
        fieldSpirvType.primarySize   = fieldSpirvType.secondarySize;
        fieldSpirvType.secondarySize = 1;
    }
                                                     ...
                                                     uint32_t *sizeInStorageBlockOut)
{
    ...
    if (!type.arraySizes.empty())
    {
        ...
        baseType.arraySizes = {};
        baseType.typeDiff.onArrayElementSelection(baseType.block != nullptr, false);

        uint32_t matrixColumnsCount       = 0;
        const SpirvTypeData &baseTypeData = getArrayElementTypeDataForAlignmentAndSize(
            baseType, type.typeDiff, &matrixColumnsCount);
        ...
        // aligned to baseAlignment) multiplied by the number of elements.  Matrix column count is
        // treated as another array dimension for all intents and purposes.
        *sizeInStorageBlockOut =
            baseSizeInStorageBlock * GetTotalArrayElements(type.arraySizes) * matrixColumnsCount;
    ...
    if (type.block != nullptr)
    {
        ...
        for (const TField *field : type.block->fields())
        {
            const SpirvTypeData &fieldTypeData =
                getFieldTypeDataForAlignmentAndSize(*field->type(), type.typeDiff, nullptr);
    ...
    {
        // Matrices in blocks are handled specially where the containing array or block takes the
        // alignment and size from the column type directly.  As a result, this piece of code is
        // reached only for matrices outside interface blocks, which don't require any alignment or
        // size calculation.
        *sizeInStorageBlockOut = 0;
        return 0;
    }
                                                              ...
                                                              uint32_t blockBaseAlignment)
{
    ...
    for (const TField *field : type.block->fields())
    {
        ...
        // > base alignment.
        uint32_t totalColumnsCount = 0;
        const SpirvTypeData &fieldTypeData =
            getFieldTypeDataForAlignmentAndSize(fieldType, type.typeDiff, &totalColumnsCount);
        uint32_t fieldAlignment          = fieldTypeData.baseAlignment;
        uint32_t fieldSizeInStorageBlock = fieldTypeData.sizeInStorageBlock;
        ...
        // looking up the base alignment and size.  Additionally, for matrices |fieldTypeData|
        // returns the size and alignment for a single column, which needs to be padded if std140
        // before the final size and alignment can be calculated.

        // For std140 only:
        // > Rule 4. ... and rounded up to the base alignment of a vec4.
        if ((fieldType.isArray() || fieldType.isMatrix() || fieldType.getStruct() != nullptr) &&
            type.typeDiff.blockStorage != EbsStd430)
        {
            fieldAlignment          = std::max(fieldAlignment, 16u);
            fieldSizeInStorageBlock = std::max(fieldSizeInStorageBlock, 16u);
        }

        nextOffset = rx::roundUp(nextOffset, fieldAlignment);
        ...
        nextOffset = nextOffset + fieldSizeInStorageBlock * totalColumnsCount;
        nextOffset = rx::roundUp(nextOffset, fieldAlignment);

        // For matrix types, also output matrix stride while the alignment is calculated.
        if (fieldType.isMatrix())
        {
            // The matrix stride is simply the alignment of the vector constituting a column or row.
            const uint32_t matrixStride = fieldAlignment;
