...
ShaderVariable SpirvTypeToShaderVariable(const SpirvType &type)
{
    const bool isRowMajor =
        type.typeDiff.isRowMajorQualifiedBlock || type.typeDiff.isRowMajorQualifiedArray;
    const GLenum glType =
        type.block != nullptr
            ? EbtStruct
                ...
                spirv::Blob *decorationsBlob)
{
    ...
    for (const ShaderVariable &fieldVar : var.fields)
    {
        ...
        if (fieldVar.isStruct())
        {
            ...
            fieldInfo = encoder->encodeStruct(structSize, fieldVar.arraySizes);
...
SpirvTypeData SPIRVBuilder::declareType(const SpirvType &type, const TSymbol *block)
{
    ...
    if (type.typeDiff.blockStorage != EbsUnspecified)
    {
        ...
        const bool isStd140         = type.typeDiff.blockStorage != EbsStd430;
