...
class RemoveUnreferencedVariablesTraverser : public TIntermTraverser
{
  public:
    ...
    // Traverse loop and block nodes in reverse order. This way reference counts can be decremented
    // when removing initializers, and variables that become unused when initializers are removed
    // can be removed on the same traversal. Note that as a consequence this traverser does not
    // track parent block positions!
