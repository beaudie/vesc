...
void SetInternalFunctionName(TFunctionInfo *functionInfo, const char *name)
{
...
TIntermAggregate *CreateFunctionPrototypeNode(const char *name, const int functionId)
{
    ...
    SetInternalFunctionName(functionNode->getFunctionInfo(), name);
    TType returnType(EbtVoid);
    functionNode->setType(returnType);
    functionNode->getFunctionInfo()->setId(functionId);
    return functionNode;
}
                                               ...
                                               const int functionId)
{
    ...
    TIntermFunctionDefinition *functionNode = new TIntermFunctionDefinition(returnType, paramsNode, functionBody);
...
void DeferGlobalInitializersTraverser::insertInitFunction(TIntermBlock *root)
{
    ...
    for (TIntermNode *node : *root->getSequence())
    {
        ...
        if (nodeFunction != nullptr && nodeFunction->getFunctionInfo()->isMain())
        {
