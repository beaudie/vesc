...
// have been
// folded may have had precision qualifiers, which should affect the precision of the consuming
// operation.
// If the folded constant union nodes are written to output as such they won't have any precision
// qualifiers,
...
// and hoists
// the constants outside the containing expression as precision qualified named variables in case
// that is
...
bool RecordConstantPrecisionTraverser::operandAffectsParentOperationPrecision(TIntermTyped *operand)
{
    ...
    if (parentAsAggregate != nullptr)
    {
        ...
        // other children
        // has a precision that's at least as high as the precision of the constant, the precision
        // of the constant
        // has no effect.
