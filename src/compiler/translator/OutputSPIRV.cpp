                                                             ...
                                                             spirv::IdRef resultTypeId)
{
    ...
    bool makeLodExplicit = mCompiler->getShaderType() != GL_FRAGMENT_SHADER && lodIndex == 0;
    ...
    if (lodIndex > 0 || makeLodExplicit)
    {
        ...
        imageOperandsList.push_back(lodIndex > 0 ? parameters[lodIndex]
    ...
    {
        const bool isExplicitLod = lodIndex != 0 || makeLodExplicit;
        if (isDref)
        {
            if (isProj)
            {
