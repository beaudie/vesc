                                                      ...
                                                      spirv::IdRef resultTypeId)
{
    ...
    //
    // ONote that in GLSL, in parameters are considered "copied" to the function.  In SPIR-V, every
    ...
    // of the function; this has observable side effects if the same variable is passed to more than
    // one out parameter of a function.  We could potentially optimize passing an unindexed lvalue
    // to an out parameter iff it's not aliased in another parameter.
    //
    const size_t parameterCount = node->getChildCount();
    spirv::IdRefList parameters;
