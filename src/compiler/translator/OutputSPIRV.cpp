...
class OutputSPIRVTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    void accessChainReset(NodeData *data) const;
    ...
    spirv::IdRef createConstant(const TType &type, const TConstantUnion *constUnion);
                                                  ...
                                                  uint8_t componentCount) const
{
    ...
    // Remember the swizzle as multi-component swizzles require special handling.
                                                           ...
                                                           spirv::IdRef typeId)
{
    ...
    if (!accessChain.swizzles.empty())
    {
        ...
        SpirvType type;
        type.type               = EbtUInt;
        spirv::IdRef uintTypeId = mBuilder.getSpirvTypeData(type, "").id;

        type.primarySize        = static_cast<uint8_t>(swizzleIds.size());
        spirv::IdRef uvecTypeId = mBuilder.getSpirvTypeData(type, "").id;

        spirv::IdRef swizzlesId = mBuilder.getNewId();
        ...
        spirv::IdRef newIndex = mBuilder.getNewId();
                                                  ...
                                                  const TConstantUnion *constUnion)
{
    ...
    if (type.getBasicType() == EbtStruct)
    {
        ...
        for (const TField *field : type.getStruct()->fields())
        {
            ...
            componentIds.push_back(createConstant(*fieldType, constUnion));
    ...
    else
    {
        ...
        for (size_t component = 0; component < size; ++component, ++constUnion)
        {
            ...
            {
                case EbtFloat:
                    componentId = mBuilder.getFloatConstant(constUnion->getFConst());
                    break;
                case EbtInt:
                    componentId = mBuilder.getIntConstant(constUnion->getIConst());
                    break;
                case EbtUInt:
                    componentId = mBuilder.getUintConstant(constUnion->getUConst());
                    break;
                case EbtBool:
                    componentId = mBuilder.getBoolConstant(constUnion->getBConst());
                    break;
                default:
                    UNREACHABLE();
            }
...
void OutputSPIRVTraverser::visitConstantUnion(TIntermConstantUnion *node)
{
    ...
    const spirv::IdRef constId = createConstant(node->getType(), node->getConstantValue());
...
bool OutputSPIRVTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    switch (node->getOp())
    {
        ...
        case EOpIndexIndirect:
            accessChainPush(
                &left, rightValue,
                mBuilder.getTypeData(node->getType(), left.accessChain.baseBlockStorage).id);
            break;
        case EOpAssign:
