...
struct AccessChain
{
    ...
    // SPIR-V type differentiations due to the base type.  Used to correctly select the SPIR-V type
    // id when visiting EOpIndex* binary nodes (i.e. reading from or writing to an access chain).
    // This always corresponds to the differentiations specific to the end result of the access
    // chain, not the base or any intermediary types.  For example, a struct nested in a
    // column-major interface block, with a parent block qualified as row-major would specify
    // row-major here.
...
class OutputSPIRVTraverser : public TIntermTraverser
{
  ...
  private:
    ...
    // Called before pushing indices to access chain to adjust |typeDiff| (which is then used to
    ...
    spirv::IdRef cast(spirv::IdRef value,
                      const TType &valueType,
                      const SpirvTypeDifferentiation &valueTypeDiff,
                      const SpirvTypeDifferentiation &expectedTypeDiff);
                                              ...
                                              const SpirvTypeDifferentiation &typeDiff) const
{
    *data = {};
    ...
    data->accessChain.typeDiff         = typeDiff;
}
...
void OutputSPIRVTraverser::accessChainOnPush(NodeData *data, const TType &parentType, size_t index)
{
    ...
    // Adjust typeDiff based on the type (which implies what the index does; select an array
    // element, a block field etc).  Index is only meaningful for selecting block fields.
    if (parentType.isArray())
    {
        accessChain.typeDiff.onArrayElementSelection(
    ...
    else if (parentType.isInterfaceBlock() || parentType.getStruct() != nullptr)
    {
        ...
        accessChain.typeDiff.onBlockFieldSelection(fieldType);
    }
    else if (parentType.isMatrix())
    {
        accessChain.typeDiff.onMatrixColumnSelection();
    }
    else
    {
        ASSERT(parentType.isVector());
        accessChain.typeDiff.onVectorComponentSelection();
    }
}
...
spirv::IdRef OutputSPIRVTraverser::accessChainLoad(NodeData *data, const TType &valueType)
{
    ...
    const spirv::IdRef castResult = cast(loadResult, valueType, accessChain.typeDiff, {});
                                            ...
                                            const TType &valueType)
{
    ...
    const spirv::IdRef castValue = cast(value, valueType, {}, accessChain.typeDiff);
...
spirv::IdRef OutputSPIRVTraverser::cast(spirv::IdRef value,
                                        ...
                                        const SpirvTypeDifferentiation &expectedTypeDiff)
{
    // If there's no difference in type differentiations, there's nothing to cast.
    if (valueTypeDiff.blockStorage == expectedTypeDiff.blockStorage &&
        valueTypeDiff.isInvariantBlock == expectedTypeDiff.isInvariantBlock &&
        valueTypeDiff.isRowMajorQualifiedBlock == expectedTypeDiff.isRowMajorQualifiedBlock &&
        valueTypeDiff.isRowMajorQualifiedArray == expectedTypeDiff.isRowMajorQualifiedArray)
    {
        // TODO: isRowMajorQualifiedArray was missing, make sure to add a test
        return value;
    }

    // At this point, a value is loaded with the |valueType| GLSL type which is of a SPIR-V type
    // differentiated by |valueTypeDiff|.  However, it's being assigned (for example through
    // operator=, used in a constructor or passed as a function argument) where the same GLSL type
    // is expected but with differnet SPIR-V type differenations (|expectedTypeDiff|).  SPIR-V 1.4
    // has OpCopyLogical that does exactly that, but we generate SPIR-V 1.0 at the moment.
    //
    // The following code recursively copies the array elements or struct fields until constructs
    ...
    {
        // Find the SPIR-V type differentiation for the element type.
        SpirvTypeDifferentiation valueElementTypeDiff    = valueTypeDiff;
        SpirvTypeDifferentiation expectedElementTypeDiff = expectedTypeDiff;

        const bool isElementBlock = valueType.getStruct() != nullptr;
        const bool isElementArray = valueType.isArrayOfArrays();

        valueElementTypeDiff.onArrayElementSelection(isElementBlock, isElementArray);
        expectedElementTypeDiff.onArrayElementSelection(isElementBlock, isElementArray);
        ...
        const spirv::IdRef elementTypeId =
            mBuilder.getTypeData(elementType, valueElementTypeDiff).id;
             ...
             ++elementIndex)
        {
            ...
            constituents.push_back(
                cast(elementId, elementType, valueElementTypeDiff, expectedElementTypeDiff));
        }
    }
    else if (valueType.getStruct() != nullptr)
    {
        ...
        for (const TField *field : valueType.getStruct()->fields())
        {
            ...
            // Find the SPIR-V type differentiation for the field type.
            SpirvTypeDifferentiation valueFieldTypeDiff    = valueTypeDiff;
            SpirvTypeDifferentiation expectedFieldTypeDiff = expectedTypeDiff;

            valueFieldTypeDiff.onBlockFieldSelection(fieldType);
            expectedFieldTypeDiff.onBlockFieldSelection(fieldType);

            // Get the field type id.
            const spirv::IdRef fieldTypeId = mBuilder.getTypeData(fieldType, valueFieldTypeDiff).id;
            ...
            constituents.push_back(
                cast(fieldId, fieldType, valueFieldTypeDiff, expectedFieldTypeDiff));
        }
    }
    else
    {
    ...
    const spirv::IdRef expectedTypeId = mBuilder.getTypeData(valueType, expectedTypeDiff).id;
...
void OutputSPIRVTraverser::visitSymbol(TIntermSymbol *node)
{
    ...
    SpirvTypeDifferentiation typeDiff;
    typeDiff.inferDefaults(type, mCompiler);

    const spirv::IdRef typeId = mBuilder.getTypeData(type, typeDiff).id;
    ...
    nodeDataInitLValue(&mNodeData.back(), symbolId, typeId, storageClass, typeDiff);
...
bool OutputSPIRVTraverser::visitSwizzle(Visit visit, TIntermSwizzle *node)
{
    ...
    const spirv::IdRef typeId =
        mBuilder.getTypeData(node->getType(), mNodeData.back().accessChain.typeDiff).id;
...
bool OutputSPIRVTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    SpirvTypeDifferentiation resultTypeDiff;
    if (node->getOp() == EOpIndexIndirect || node->getOp() == EOpAssign)
    {
        ...
        resultTypeDiff = mNodeData[mNodeData.size() - 2].accessChain.typeDiff;
    }
    const spirv::IdRef resultTypeId = mBuilder.getTypeData(node->getType(), resultTypeDiff).id;
