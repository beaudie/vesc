...
unsigned int GetLocationCount(const TIntermSymbol *varying, bool ignoreVaryingArraySize)
{
    ...
    {
        return varyingType.getNominalSize() *
               static_cast<unsigned int>(varyingType.getArraySizeProduct());
    }
    else
    {
        return static_cast<unsigned int>(varyingType.getArraySizeProduct());
    }
}
