                                                 ...
                                                 const unsigned int maxCombinedClipAndCullDistances)
{
    ...
    unsigned int combinedClipAndCullDistances = enabledClipDistances + enabledCullDistances;

    if (enabledClipDistances > maxClipDistances)
    {
        std::stringstream strstr = sh::InitializeStream<std::stringstream>();
        strstr << "The size of '" << mClipDistance->getName()
               << "' is greater than gl_MaxClipDistances (" << enabledClipDistances << " > "
               << maxClipDistances << ")";
        error(*mClipDistance, strstr.str().c_str(), diagnostics);
    }
    else if (enabledCullDistances > maxCullDistances)
    {
        std::stringstream strstr = sh::InitializeStream<std::stringstream>();
        strstr << "The size of '" << mCullDistance->getName()
               << "' is greater than gl_MaxCullDistances (" << enabledCullDistances << " > "
               << maxCullDistances << ")";
        error(*mCullDistance, strstr.str().c_str(), diagnostics);
    }
    else if (combinedClipAndCullDistances > maxCombinedClipAndCullDistances)
    {
        const TIntermSymbol *greaterSymbol =
            (enabledClipDistances >= enabledCullDistances ? mClipDistance : mCullDistance);

        std::stringstream strstr = sh::InitializeStream<std::stringstream>();
        strstr << "The sum of both 'gl_ClipDistance' and 'gl_CullDistance' size is greater than "
                              ...
                              const unsigned int maxCombinedClipAndCullDistances)
{
    ...
    varyingValidator.validate(diagnostics, maxClipDistances, maxCullDistances,
