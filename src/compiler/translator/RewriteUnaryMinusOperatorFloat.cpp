...
bool Traverser::visitUnary(Visit visit, TIntermUnary *node)
{
    ...
    zero->setFConst(0.0);
    TIntermConstantUnion *zeroNode = new TIntermConstantUnion(zero, fValue->getType());
    zeroNode->getTypePointer()->setQualifier(EvqConst);
    zeroNode->setLine(fValue->getLine());

    // 0.0 - float
    TIntermBinary *sub = new TIntermBinary(EOpSub, zeroNode, fValue);
