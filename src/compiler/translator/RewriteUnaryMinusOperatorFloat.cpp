...
bool Traverser::visitUnary(Visit visit, TIntermUnary *node)
{
    ...
    // Constant -1.0
    TConstantUnion *negativeOne = new TConstantUnion();
    negativeOne->setFConst(-1.0);
    TIntermConstantUnion *negativeOneNode =
        new TIntermConstantUnion(negativeOne, fValue->getType());
    negativeOneNode->setLine(fValue->getLine());

    // -1.0 * float
    TIntermBinary *mul = new TIntermBinary(EOpMul, negativeOneNode, fValue);
    mul->setLine(fValue->getLine());
