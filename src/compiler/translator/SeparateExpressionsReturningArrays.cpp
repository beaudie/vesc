...
bool SeparateExpressionsTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    if (visit != PreVisit || !node->getType().isArray() || parentNodeIsBlock())
        return true;
    ...
    switch (node->getOp())
    {
      case EOpAssign:
        mFoundArrayExpression = true;
        {
            ...
            // TODO: In some cases it would be more optimal to not add the temporary node, but just use the original
            // target of the assignment. Care must be taken so that this doesn't happen when the same array symbol is
            // a target of assignment more than once in one expression.
...
{
    if (visit == PreVisit && mFoundArrayExpression)
        return false; // No need to traverse further

    if (visit == PreVisit && getParentNode() != nullptr)
    {
