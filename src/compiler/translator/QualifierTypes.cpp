...
void SortSequence(std::vector<const TQualifierWrapperBase *> &qualifiers)
{
    ...
    bool areQualifierChecksRelaxed) const
{
    ...
    std::vector<const TQualifierWrapperBase *> sortedQualifierSequence;
    const std::vector<const TQualifierWrapperBase *> *qualifierSequencePtr = nullptr;
    // If the qualifier checks are relaxed, then it is easier to sort the qualifiers so
    // that the order imposed by the GLSL ES 3.00 spec is kept. Then we can use the same piece of
    // code to do the combination.
    if (areQualifierChecksRelaxed)
    {
        // Copy the qualifier sequence so that we can sort them.
        sortedQualifierSequence = mQualifiers;
        SortSequence(sortedQualifierSequence);
        qualifierSequencePtr = &sortedQualifierSequence;
    }
    else
    {
        qualifierSequencePtr = &mQualifiers;
    }

    for (size_t i = 1; i < qualifierSequencePtr->size(); ++i)
    {
        const TQualifierWrapperBase *qualifier = (*qualifierSequencePtr)[i];
