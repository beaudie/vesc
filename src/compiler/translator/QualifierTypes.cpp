    ...
    for (size_t i = 1; i < qualifiers.size(); ++i)
    {
        switch (qualifiers[i]->getType())
        {
            ...
            case QtPrecision:
                ...
                break;
            case QtPrecise:
                // This keyword is available in ES3.1 (with extension) or with ES3.2, but the
                // function should early-out in such a case as the spec doesn't require a particular
                // order to the qualifiers.
                UNREACHABLE();
            default:
                UNREACHABLE();
        }
