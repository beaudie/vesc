    ...
    const TString &identifier)
{
    ...
    if (publicType.getBasicType() == EbtAtomicCounter)
    {
        ...
        {
            parseGlobalAtomicCounterBindingDefaultOffset(publicType, identifierOrTypeLocation);
        }
    }
...
void TParseContext::parseGlobalAtomicCounterBindingDefaultOffset(const TPublicType &publicType,
                                                                 const TSourceLoc &location)
{
    const TLayoutQualifier &layoutQualifier = publicType.layoutQualifier;
