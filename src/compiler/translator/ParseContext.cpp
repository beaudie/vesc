    ...
    const TSourceLoc &arrayIndexLine)
{
    ...
    {
        checkMemoryQualifierIsNotSpecified(blockMemoryQualifier, typeQualifier.line);
    }
    ...
    for (size_t memberIndex = 0; memberIndex < fieldList->size(); ++memberIndex)
    {
        ...
        if (typeQualifier.qualifier == EvqBuffer)
        {
            ...
            if (blockMemoryQualifier.readonly && !fieldMemoryQualifier.readonly)
            {
                fieldMemoryQualifier.readonly = true;
            }
            if (blockMemoryQualifier.writeonly && !fieldMemoryQualifier.writeonly)
            {
                fieldMemoryQualifier.writeonly = true;
            }
            if (blockMemoryQualifier.coherent && !fieldMemoryQualifier.coherent)
            {
                fieldMemoryQualifier.coherent = true;
            }
            if (blockMemoryQualifier.restrictQualifier && !fieldMemoryQualifier.restrictQualifier)
            {
                fieldMemoryQualifier.restrictQualifier = true;
            }
            if (blockMemoryQualifier.volatileQualifier && !fieldMemoryQualifier.volatileQualifier)
            {
                fieldMemoryQualifier.volatileQualifier = true;
            }
