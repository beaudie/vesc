                                                     ...
                                                     const TSourceLoc &loc)
{
    if (thisNode != nullptr)
    {
        return addMethod(fnCall, argumentsList, thisNode, loc);
    }

    TOperator op = fnCall->getBuiltInOp();
    if (op != EOpNull)
    {
        return addConstructor(argumentsList, op, fnCall->getReturnType(), loc);
    }
    else
    {
        return addNonConstructorFunctionCall(fnCall, argumentsList, loc);
    }
}
                                                           ...
                                                           const TSourceLoc &loc)
{
    ...
    {
        symbol =
            symbolTable.find(TFunction::GetMangledNameFromCall(fnCall->getName(), *argumentsList),
        ...
        else
        {
            ...
            if (builtIn && op != EOpNull)
            {
                ...
                if (fnCandidate->getParamCount() == 1)
                {
                    ...
                    TIntermNode *unaryParamNode = argumentsList->front();
                ...
                {
                    TIntermAggregate *callNode = new TIntermAggregate(fnCandidate->getReturnType(), op, argumentsList);
            ...
            else
            {
                ...
                if (builtIn)
                {
                    callNode = new TIntermAggregate(fnCandidate->getReturnType(), EOpCallBuiltInFunction, argumentsList);
                ...
                {
                    callNode = new TIntermAggregate(fnCandidate->getReturnType(), EOpCallFunctionInAST, argumentsList);
                    callNode->getFunctionSymbolInfo()->setFromFunction(*fnCandidate);
