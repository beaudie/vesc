                                                ...
                                                bool outOfRangeIndexIsError)
{
    ...
    TType newType              = node->getType();
    newType.setPrimarySize(static_cast<unsigned char>(fields.num));

    for (int i = 0; i < fields.num; i++)
    {
        int offset = fields.offsets[i];
        if (offset >= newType.getNominalSize())
        {
            std::stringstream extraInfoStream;
            extraInfoStream << "vector field selection out of range '" << offset << "'";
            std::string extraInfo = extraInfoStream.str();
            outOfRangeError(outOfRangeIndexIsError, line, "", "[", extraInfo.c_str());
            fields.offsets[i] = newType.getNominalSize() - 1;
        }

        constArray[i] = unionArray[fields.offsets[i]];
    }
    return intermediate.addConstantUnion(constArray, newType, line);
}
