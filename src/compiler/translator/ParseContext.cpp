...
void TParseContext::setArraySizeToGeometryShaderInputs()
{
    ...
    // TODO(jiawei.shao@intel.com): set array size to all user-defined input varyings.
...
bool TParseContext::parseGeometryShaderInputLayoutQualifier(const TTypeQualifier &typeQualifier)
{
    ...
    if (layoutQualifier.primitiveType != EptUndefined)
    {
        ...
        if (mGeometryShaderInputPrimitiveType == EptUndefined)
        {
            ...
            setArraySizeToGeometryShaderInputs();
        }
    ...
    const TSourceLoc &arrayIndexLine)
{
    ...
    TInterfaceBlock *interfaceBlock = new TInterfaceBlock(
        &blockName, fieldList, instanceName, arraySize > 0, arraySize, blockLayoutQualifier);
    ...
    if (indexExpression->getQualifier() != EvqConst || indexConstantUnion == nullptr)
    {
        if (baseExpression->isInterfaceBlock())
        {
            ...
            switch (baseExpression->getQualifier())
            {
                ...
                case EvqBuffer:
                    error(location,
                          "array indexes for uniform and buffer interface blocks arrays must be "
