...
bool IsAtomicBuiltin(const char *name)
{
    for (size_t i = 0; i < kAtomicBuiltin.size(); ++i)
    {
        if (strcmp(name, kAtomicBuiltin[i]) == 0)
        {
            return true;
        }
    }
...
bool TParseContext::checkCanBeLValue(const TSourceLoc &line, const char *op, TIntermTyped *node)
{
    ...
    if (symNode)
    {
        ...
        const char *symbol = symNode->getName();
        std::stringstream reasonStream;
...
void TParseContext::checkTextureGather(TIntermAggregate *functionCall)
{
    ...
    const char *name           = functionCall->getFunction()->name();
    bool isTextureGather       = strcmp(name, "textureGather") == 0;
    bool isTextureGatherOffset = strcmp(name, "textureGatherOffset") == 0;
...
void TParseContext::checkTextureOffsetConst(TIntermAggregate *functionCall)
{
    ...
    if (strcmp(name, "texelFetchOffset") == 0 || strcmp(name, "textureLodOffset") == 0 ||
        strcmp(name, "textureProjLodOffset") == 0 || strcmp(name, "textureGradOffset") == 0 ||
        strcmp(name, "textureProjGradOffset") == 0)
    {
        offset = arguments->back();
    }
    else if (strcmp(name, "textureOffset") == 0 || strcmp(name, "textureProjOffset") == 0)
    {
        // A bias parameter might follow the offset parameter.
        ASSERT(arguments->size() >= 3);
        offset = (*arguments)[2];
    }
    else if (strcmp(name, "textureGatherOffset") == 0)
    {
...
{
    ASSERT(functionCall->getOp() == EOpCallBuiltInFunction);
    const char *functionName = functionCall->getFunction()->name();
