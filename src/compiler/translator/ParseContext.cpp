...
struct TParseContext::BindingOffset
{
    BindingOffset() : defaultOffset(0){};
                                                            ...
                                                            const TSourceLoc &loc)
{
    ...
    {
        publicType.layoutQualifier.offset = bindingOffset.defaultOffset;
    }
...
void TParseContext::checkIsNotGlobalAtomicCounter(const TSourceLoc &location, const TPublicType &type)
{
    if (!symbolTable.atGlobalLevel() && type.getBasicType() == EbtAtomicCounter)
    {
        error(location, "atomic_uint can be only used in global scope", "type");
    }
}
                                                         ...
                                                         const TSourceLoc &location)
{
    ...
    {
        mAtomicCounterBindingOffsets[layoutQualifier.binding].defaultOffset =
            layoutQualifier.offset;
    }
}
