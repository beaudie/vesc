...
static const ImmutableString kTexelFetchOffsetName("texelFetchOffsetName");
static const ImmutableString kTextureLodOffsetName("textureLodOffset");
static const ImmutableString kTextureProjLodOffsetName("textureProjLodOffset");
static const ImmutableString kTextureGradOffsetName("textureGradOffset");
static const ImmutableString kTextureProjGradOffsetName("textureProjGradOffset");
static const ImmutableString kTextureOffsetName("textureOffset");
static const ImmutableString kTextureProjOffsetName("textureProjOffset");
static const ImmutableString kTextureGatherName("textureGather");
static const ImmutableString kTextureGatherOffsetName("textureGatherOffset");

const std::array<ImmutableString, 8> kAtomicBuiltin = {
...
TFunction *TParseContext::parseFunctionDeclarator(const TSourceLoc &location, TFunction *function)
{
    ...
    if (getShaderVersion() >= 300 &&
        symbolTable.hasUnmangledBuiltInForShaderVersion(function->name(), getShaderVersion()))
    {
...
TFunction *TParseContext::addNonConstructorFunc(const ImmutableString &name, const TSourceLoc &loc)
{
...
TFunction *TParseContext::addConstructorFunc(const TPublicType &publicType)
{
    ...
    return new TFunction(&symbolTable, ImmutableString(""), type, SymbolType::NotResolved, true,
                         EOpConstruct);
}
...
void TParseContext::checkImageMemoryAccessForBuiltinFunctions(TIntermAggregate *functionCall)
{
    ...
    if ((name.length() == 9u || name.length() == 10u) && name.beginsWith("image"))
    {
        TIntermSequence *arguments = functionCall->getSequence();
...
TIntermTyped *TParseContext::addMethod(const ImmutableString &name,
                                       TIntermSequence *arguments,
                                       TIntermNode *thisNode,
                                       const TSourceLoc &loc)
{
...
TIntermTyped *TParseContext::addNonConstructorFunctionCall(const ImmutableString &name,
                                                           TIntermSequence *arguments,
                                                           const TSourceLoc &loc)
{
    ...
    {
        error(loc, "function name expected", name);
    }
    else
    {
        ...
        if (symbol == nullptr)
        {
            error(loc, "no matching overloaded function found", name);
        }
        else
        {
