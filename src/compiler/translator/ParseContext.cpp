...
bool TParseContext::arrayTypeErrorCheck(const TSourceLoc &line, const TPublicType &type)
{
    ...
    {
        error(line, "cannot declare arrays of structs of this qualifier", TType(type).getCompleteString().c_str());
        return true;
    }
...
void TParseContext::es3InputOutputTypeCheck(const TQualifier qualifier, const TPublicType &type,
                                            const TSourceLoc &qualifierLocation)
{
    ...
    // Vertex shader outputs / fragment shader inputs have a different, slightly more lenient set of restrictions.
