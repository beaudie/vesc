...
bool TParseContext::checkIsNotReserved(const TSourceLoc &line, const ImmutableString &identifier)
{
    ...
    {
        // Using double underscores is allowed, but may result in unintended behaviors, so a warning
        // is issued.
        // OpenGL ES Shader Language 3.2 specification:
        // > 3.7. Keywords
        // > ...
        // > In addition, all identifiers containing two consecutive underscores (__) are reserved
        // > for use by underlying software layers. Defining such a name in a shader does not itself
        // > result in an error, but may result in unintended behaviors that stem from having
