...
bool TParseContext::checkIsValidQualifierForArray(const TSourceLoc &line, const TPublicType &elementQualifier)
{
    if ((elementQualifier.qualifier == EvqAttribute) || (elementQualifier.qualifier == EvqVertexIn) ||
...
bool TParseContext::checkIsValidTypeForArray(const TSourceLoc &line, const TPublicType &elementType)
{
    ...
    {
        error(line, "cannot declare arrays of arrays", TType(elementType).getCompleteString().c_str());
        return false;
    }
    // In ESSL1.00 shaders, structs cannot be varying (section 4.3.5). This is checked elsewhere.
    // In ESSL3.00 shaders, struct inputs/outputs are allowed but not arrays of structs (section
    // 4.3.4).
    if (mShaderVersion >= 300 && elementType.type == EbtStruct && sh::IsVarying(elementType.qualifier))
    {
...
bool TParseContext::checkIsValidTypeAndQualifierForArray(const TSourceLoc &indexLocation, const TPublicType &elementType)
{
