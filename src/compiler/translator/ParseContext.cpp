...
TPublicType TParseContext::addStructure(const TSourceLoc& structLine, const TSourceLoc& nameLine, const TString *structName, TFieldList* fieldList)
{
    ...
    // Check for global struct. Because structs introduce their own local scope, we will be
    // one level depper than global in this method, if the struct is at the global scope.
    if (symbolTable.currentLevel()-1 == GLOBAL_LEVEL)
    {
