                                        ...
                                        const TSourceLoc &loc)
{
    ...
    if (!isBitShift && left->getBasicType() != right->getBasicType())
    {
        if (IsDesktopGLSpec(mShaderSpec))
        {
            ...
            // If LHS can be implicitly converted to LHS type, this is valid for some cases
            if (right->getBasicType() == EbtFloat &&
                (left->getBasicType() == EbtInt || left->getBasicType() == EbtUInt))
            {
                convertLeft = true;
            }
            // If neither side can be implicitly converted, this is invalid
            else if (left->getBasicType() != EbtFloat ||
    ...
    switch (op)
    {
        ...
        case EOpBitwiseOrAssign:
            ...
            if (IsAssignment(op) && convertLeft)
            {
                error(loc, "type mismatch", GetOperatorString(op));
                return false;
            }
