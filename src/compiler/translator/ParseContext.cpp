...
void TParseContext::assignArraySizeForGeometryShaderInputs()
{
    ...
    GLuint inputArraySize = GetGeometryShaderInputArraySize(mGeometryShaderInputPrimitiveType);

    // TODO(jiawei.shao@intel.com): set array size for user-defined geometry shader input varyings.
    glIn->getType().setArraySize(inputArraySize);
    glIn->getType().getInterfaceBlock()->setArraySize(inputArraySize);
}
