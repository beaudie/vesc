                                                               ...
                                                               TIntermTyped *indexExpression)
{
    ...
    if (indexExpression == nullptr)
    {
        ...
        {
            error(indexLocation, "Invalid implicitly sized array declaration", identifier.c_str());
        }
...
bool TParseContext::parseGeometryShaderInputLayouts(const TTypeQualifier &typeQualifier)
{
    ...
    if (layoutQualifier.primitiveType != EgsUndefined)
    {
                 ...
                 GetGeometryShaderInputArraySize(layoutQualifier.primitiveType))
        {
            error(
                typeQualifier.line,
                "it is not allowed to set geometry shader input layout primitive type for different"
                                                ...
                                                TIntermTyped *indexExpression)
{
    ...
    if (baseExpression->getQualifier() == EvqGeometryIn && baseExpression->isUnsizedArray())
    {
        error(location, "Missing array size or declared before input primitive declaration", "[");
    }
