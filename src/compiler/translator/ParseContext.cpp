                                    ...
                                    TVariable **variable)
{
    ...
    (*variable) = new TVariable(&symbolTable, &identifier, type);
    ...
    bool insertParametersToSymbolTable)
{
    ...
    for (size_t i = 0; i < function.getParamCount(); i++)
    {
        ...
        if (param.name != nullptr)
        {
            ...
            {
                TVariable *variable = new TVariable(&symbolTable, param.name, *param.type);
                if (symbolTable.declareVariable(variable))
                {
                    symbol = new TIntermSymbol(variable);
                }
    ...
    const TSourceLoc &arrayIndexLine)
{
    ...
    if (!instanceName)
    {
        instanceName = new TString("");  // TODO: Reuse the same empty TString pointer.
    }
    TVariable *instanceVariable = new TVariable(&symbolTable, instanceName, interfaceBlockType);
    ...
    if (instanceName->empty())
    {
        ...
        for (size_t memberIndex = 0; memberIndex < fieldList->size(); ++memberIndex)
        {
            ...
            TVariable *fieldVariable = new TVariable(&symbolTable, &field->name(), *fieldType);
