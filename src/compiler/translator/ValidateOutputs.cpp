...
int ValidateOutputs::validateAndCountErrors(TInfoSinkBase& sink) const
{
    ...
    for (OutputVector::const_iterator iter = mOutputs.begin();
         iter != mOutputs.end();
         ++iter) {
        TIntermSymbol* symbol = *iter;
        ...
        else
        {
            if (elementCount > 0)
            {
                error(errorCount, sink, *symbol, elementCount > 1
    ...
    {
        for (OutputVector::const_iterator iter = mUnspecifiedLocationOutputs.begin();
             iter != mUnspecifiedLocationOutputs.end();
             ++iter)
        {
            error(errorCount, sink, **iter, "must explicitly specify all locations when using multiple fragment outputs");
        }
    }
    return errorCount;
}
