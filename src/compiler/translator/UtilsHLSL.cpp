...
HLSLRVTextureGroup ResourceViewGroup(const TType type)
{
    ...
    TLayoutImageInternalFormat imageInternalFormat = type.getLayoutQualifier().imageInternalFormat;
    if (type.getMemoryQualifier().writeonly)
    {
    ...
    return HLSL_RV_TEXTURE_UNKNOWN;
}
...
TString ResourceViewGroupSuffix(const HLSLRVTextureGroup type)
{
    ...
    return "<unknown resource view type>";
}

bool IsHLSLUAVTexture(HLSLRVTextureGroup RVTextureGroup)
{
    return RVTextureGroup >= HLSL_UAV_TEXTURE_BEIGN && RVTextureGroup <= HLSL_UAV_TEXTURE_END;
}

TString ResourceViewString(const HLSLRVTextureGroup RVTextureGroup)
{
    switch (RVTextureGroup)
    {
        case HLSL_SRV_TEXTURE_2D_FLOAT4:
