...
pub fn calculate_function_decl_order(
    ...
    // Call DFS from main(), calculating the topoligical sort and whether there are any cycles
    // in one go.
            ...
            VisitOrder::Pre => {
                match *state {
                    ...
                    VisitState::NotVisited => {
                        // Schedule this not to be visited again after the calless.
...
fn new_block_to_duplicate(
    ...
    new_block.variables = block_variables
        ...
        .map(|&variable_id| {
            ...
            // Note that variables inside blocks should always be Temporary, not AngleInternal or
    ...
    instructions.iter().for_each(|instruction| {
        ...
        match result {
            Some(original_id) => {
                ...
                block.add_typed_instruction(new_id.id);
            }
            None => {
