...
OutputHLSL::~OutputHLSL()
{
    ...
    for (StructEqualityFunction *eqFunction : mStructEqualityFunctions) {
        delete eqFunction;
    }
    for (ArrayEqualityFunction *eqFunction : mArrayEqualityFunctions) {
        delete eqFunction;
    }
}
...
TString OutputHLSL::addStructEqualityFunction(const TStructure &structure)
{
    ...
    for (size_t i = 0; i < fields.size(); i++)
    {
        ...
        fnOut << "(";
        outputEqual(fnOut, PreVisit, nonArrayType, EOpEqual);
        fnOut << fieldNameA;
        outputEqual(fnOut, InVisit, nonArrayType, EOpEqual);
        fnOut << fieldNameB;
        outputEqual(fnOut, PostVisit, nonArrayType, EOpEqual);
        fnOut << ")";
    }
...
TString OutputHLSL::addArrayEqualityFunction(const TType& type)
{
    ...
    outputEqual(fnOut, PreVisit, nonArrayType, EOpNotEqual);
    fnOut << "a[i]";
    outputEqual(fnOut, InVisit, nonArrayType, EOpNotEqual);
    fnOut << "b[i]";
    outputEqual(fnOut, PostVisit, nonArrayType, EOpNotEqual);
