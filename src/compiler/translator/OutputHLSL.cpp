...
const char *GetHLSLAtomicFunctionStringAndLeftBracket(TOperator op)
{
    switch (op)
    {
        case EOpAtomicAdd:
...
bool OutputHLSL::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    switch (node->getOp())
    {
        ...
        case EOpAssign:
            ...
            else if (node->isAtomicFunctionDirectAssign())
            {
                TIntermAggregate *atomicFunctionNode = node->getRight()->getAsAggregate();
                TOperator atomicFunctionOp           = atomicFunctionNode->getOp();
                out << GetHLSLAtomicFunctionStringAndLeftBracket(atomicFunctionOp);
                TIntermSequence *parameterSeq = atomicFunctionNode->getSequence();
                ASSERT(parameterSeq->size() >= 2u);
                for (auto &parameter : *parameterSeq)
                {
...
bool OutputHLSL::visitAggregate(Visit visit, TIntermAggregate *node)
{
    ...
    switch (node->getOp())
    {
        ...
        case EOpAtomicOr:
        case EOpAtomicXor:
            outputTriplet(out, visit, GetHLSLAtomicFunctionStringAndLeftBracket(node->getOp()), ",",
                          ")");
            break;
