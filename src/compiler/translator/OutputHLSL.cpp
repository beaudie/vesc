...
void OutputHLSL::output(TIntermNode *treeRoot, TInfoSinkBase &objSink)
{
    ...
    mASTMetadatas = CreateASTMetadataHLSL(treeRoot, mCallDag);
    ...
    switch (node->getOp())
    {
      ...
      case EOpPrototype:
        if (visit == PreVisit)
        {
            ...
            bool needsLod0 = mASTMetadatas[index].mNeedsLod0;
      ...
      case EOpFunction:
        {
            ...
            mCurrentFunctionMetadata = &mASTMetadatas[index];
      ...
      case EOpFunctionCall:
        {
            ...
            if (node->isUserDefined())
            {
                size_t index = mCallDag.findIndex(node);
                ASSERT(index != CallDAG::InvalidIndex);
                lod0 &= mASTMetadatas[index].mNeedsLod0;
