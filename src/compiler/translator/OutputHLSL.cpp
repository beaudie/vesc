      ...
      case EOpFunctionCall:
        {
            ...
            for (TIntermSequence::iterator arg = arguments->begin(); arg != arguments->end(); arg++)
            {
                ...
                if (mOutputType == SH_HLSL_4_0_FL9_3_OUTPUT &&
                    IsSampler(typedArg->getBasicType()))
                {
                    out << "texture_";
                    (*arg)->traverse(this);
                    out << ", sampler_";
                ...
                if (typedArg->getType().isStructureContainingSamplers())
                {
                    ...
                    // TODO: Determine struct name to use here in case arg is an indexing expression.
                    TString structName;
                    if (typedArg->getAsSymbolNode())
                    {
                        structName += typedArg->getAsSymbolNode()->getSymbol();
                    }
                    argType.createSamplerSymbols(Decorate(structName), argType.isArray() ? argType.getArraySize() : 0, &samplerSymbols);
...
TString OutputHLSL::argumentString(const TIntermSymbol *symbol)
{
    ...
    TString argString = QualifierString(qualifier) + " " + TypeString(type) + " " + nameStr + ArrayString(type);
    ...
    if (type.isStructureContainingSamplers())
    {
        ...
        for (auto &sampler : samplerSymbols)
        {
            ...
            else if (mOutputType == SH_HLSL_4_0_FL9_3_OUTPUT)
            {
                ...
                argString += ", " + QualifierString(qualifier) + " " + TextureString(samplerType.getBasicType()) + " texture_" + sampler->getSymbol() + ArrayString(type) + ", " + QualifierString(qualifier) + " " + SamplerString(samplerType.getBasicType()) + " sampler_" + sampler->getSymbol() + ArrayString(type);
            }
            else
            {
                const TType &samplerType = sampler->getType();
                argString += ", " + QualifierString(qualifier) + " " + TypeString(samplerType) + " " + sampler->getSymbol() + ArrayString(type);
            }
        }
    }
