...
bool OutputHLSL::visitFunctionDefinition(Visit visit, TIntermFunctionDefinition *node)
{
    ...
    {
        const TConstParameter &param = func->getParam(i);
        ensureStructDefined(*param.type);
...
void OutputHLSL::writeParameter(const TConstParameter &param, TInfoSinkBase &out)
{
    const TType &type    = *param.type;
    TQualifier qualifier = type.getQualifier();

    TString nameStr = Decorate(param.name);
