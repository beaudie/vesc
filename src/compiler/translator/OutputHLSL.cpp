        ...
        if (IsDeclarationWrittenOut(node))
        {
                    ...
                    SymbolType::Empty)  // Variable declaration
            {
                ...
                if (symbol)
                {
                    ...
                    // Avoid initializing shared variables when not requested to, because it is very
                    // slow for D3D11 drivers to compile a compute shader if we add code to
                    // initialize a groupshared array variable with a large array size.
                    if (declarator->getQualifier() != EvqShared ||
