                        ...
                        const BuiltInFunctionEmulator *builtInFunctionEmulator) const
{
    ...
    writeShaderStorageBlocksHeader(out);
...
bool OutputHLSL::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    switch (node->getOp())
    {
        ...
        case EOpAssign:
            ...
            {
                outputSSBOStoreFunctionCallPrefix(node->getLeft());
                out << ", ";
                if (IsInShaderStorageBlock(node->getRight()))
                {
                    outputSSBOLoadFunctionCall(node->getRight());
                }
                else
                {
...
bool OutputHLSL::visitUnary(Visit visit, TIntermUnary *node)
{
    ...
    switch (node->getOp())
    {
        ...
        case EOpArrayLength:
        {
            ...
            outputSSBOLengthFunctionCall(operand);
            return false;
        }
        default:
            UNREACHABLE();
    ...
    switch (node->getOp())
    {
        ...
        case EOpAtomicCompSwap:
        {
            ...
            if (IsInShaderStorageBlock(memNode))
            {
                ...
                outputSSBOAtomicMemoryFunctionCallPrefix(memNode, node->getOp());
                ...
                for (size_t i = 1; i < node->getChildCount(); i++)
                {
                    ...
                    {
                        outputSSBOLoadFunctionCall(argument);
                    }
                    else
                    {
                        argument->traverse(this);
...
void OutputHLSL::outputSSBOStoreFunctionCallPrefix(TIntermTyped *node)
{
    traverseSSBOAccess(node, SSBOMethod::STORE);
}

void OutputHLSL::outputSSBOLoadFunctionCall(TIntermTyped *node)
{
    traverseSSBOAccess(node, SSBOMethod::LOAD);
    getInfoSink() << ")";
}

void OutputHLSL::outputSSBOLengthFunctionCall(TIntermTyped *node)
{
    traverseSSBOAccess(node, SSBOMethod::LENGTH);
    getInfoSink() << ")";
}

void OutputHLSL::outputSSBOAtomicMemoryFunctionCallPrefix(TIntermTyped *node, TOperator op)
{
    switch (op)
    {
        case EOpAtomicAdd:
...
int OutputHLSL::getMatrixStride(TIntermTyped *node,
                                TLayoutBlockStorage storage,
                                bool rowMajor,
...
void OutputHLSL::collectShaderStorageBlocks(TIntermTyped *node)
{
    TIntermSwizzle *swizzleNode = node->getAsSwizzleNode();
    if (swizzleNode)
    {
...
void OutputHLSL::traverseSSBOAccess(TIntermTyped *node, SSBOMethod method)
{
    ...
    TIntermNode *loc = traverseSSBONode(out, node, &blockMemberInfo);
    out << ", ";
    loc->traverse(this);
}

void OutputHLSL::writeShaderStorageBlocksHeader(TInfoSinkBase &out) const
{
...
TIntermTyped *OutputHLSL::traverseSSBONode(TInfoSinkBase &out,
                                           TIntermTyped *node,
                                           BlockMemberInfo *blockMemberInfo)
{
    ...
    {
        return traverseSSBONode(out, swizzleNode->getOperand(), blockMemberInfo);
    }
    else if (TIntermBinary *binaryNode = node->getAsBinaryNode())
    {
        switch (binaryNode->getOp())
        {
            ...
            case EOpIndexDirectStruct:
            {
                ...
                ASSERT(binaryNode->getLeft()->getBasicType() != EbtInterfaceBlock);
                TIntermTyped *left = traverseSSBONode(out, binaryNode->getLeft(), blockMemberInfo);
            ...
            case EOpIndexDirectInterfaceBlock:
            {
                ...
                traverseSSBONode(out, binaryNode->getLeft(), blockMemberInfo);
                const TInterfaceBlock *interfaceBlock =
                    binaryNode->getLeft()->getType().getInterfaceBlock();
                const TIntermConstantUnion *index = binaryNode->getRight()->getAsConstantUnion();
                const TField *field               = interfaceBlock->fields()[index->getIConst(0)];
...
TIntermTyped *OutputHLSL::writeEOpIndexDirectOrIndirectOutput(TInfoSinkBase &out,
                                                              TIntermBinary *node,
                                                              BlockMemberInfo *blockMemberInfo)
{
    ASSERT(IsInShaderStorageBlock(node->getLeft()));
    TIntermTyped *left  = traverseSSBONode(out, node->getLeft(), blockMemberInfo);
...
TIntermTyped *OutputHLSL::createFieldOffset(const TField *field, BlockMemberInfo *blockMemberInfo)
{
    auto fieldInfoIter = mBlockMemberInfoMap.find(field);
    ASSERT(fieldInfoIter != mBlockMemberInfoMap.end());
