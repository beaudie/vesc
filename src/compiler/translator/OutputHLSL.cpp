...
bool OutputHLSL::ancestorReturnsSamplerInStruct(Visit visit)
{
    ...
    const unsigned int initialN = visit == InVisit ? 2 : 1;
      ...
      case EOpIndexDirectStruct:
        {
            ...
            if (visit == InVisit)
            {
                if (indexingReturnsSampler)
                {
                    // All parts of an expression that access a sampler need to use _ as separator
                    // to access the sampler variable that has been moved out of the struct.
                    out << "_" + field->name();
                }
                else
                {
...
TString OutputHLSL::argumentString(const TIntermSymbol *symbol)
{
    ...
    if (type.isStructureContainingSamplers())
    {
        ASSERT(qualifier != EvqOut && qualifier != EvqInOut);
        TVector<TIntermSymbol *> samplerSymbols;
        type.createSamplerSymbols("angle" + nameStr, 0, &samplerSymbols);
        for (auto &sampler : samplerSymbols)
        {
            if (mOutputType == SH_HLSL_4_1_OUTPUT)
            {
