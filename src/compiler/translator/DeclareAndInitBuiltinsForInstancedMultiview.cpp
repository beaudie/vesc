...
// Replaces every occurrence of a symbol with the name specified in symbolName with the new symbol
// pointed to by newSymbolNode.
...
void DeclareGlobalVariables(TIntermBlock *root,
                            TIntermTyped *viewIDSymbol,
                            TIntermTyped *instanceIDSymbol,
                            unsigned numberOfViews,
                            GLenum shaderType)
{
    ...
    viewIDOVRDeclaration->appendDeclarator(viewIDSymbol->deepCopy());
    ...
    if (shaderType == GL_VERTEX_SHADER)
    {
        ...
        instanceIDDeclaration->appendDeclarator(instanceIDSymbol->deepCopy());
                                                 ...
                                                 GLenum shaderType)
{
    ...
    DeclareGlobalVariables(root, viewIDSymbol, instanceIDSymbol, numberOfViews, shaderType);
    if (shaderType == GL_VERTEX_SHADER)
    {
