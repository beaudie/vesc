...
TIntermBinary *CreateIndexDirectBaseSymbolNode(const TType &indexedType, const TType &fieldType, const int index)
{
    ...
    if (node->getOp() == EOpIndexIndirect)
    {
        ...
        else if (!node->getLeft()->isArray() && node->getLeft()->getBasicType() != EbtStruct)
        {
            ...
            if (write)
            {
                ...
                TIntermAggregate *initIndex =
                    createTempInitDeclaration(indexInitializer);
            ...
            else
            {
                ...
                TIntermAggregate *indexingCall =
                    CreateIndexFunctionCall(node, node->getLeft(), EnsureSignedInt(node->getRight()));
