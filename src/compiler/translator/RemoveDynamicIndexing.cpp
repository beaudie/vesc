...
TIntermSymbol *CreateValueSymbol(const TType &type, TSymbolTable *symbolTable)
{
    ...
    TVariable *valueVariable =
        new TVariable(symbolTable, valueString, valueType, SymbolType::ANGLE_INTERNAL);
                                          ...
                                          TFunction *indexingFunction)
{
    ...
    TIntermAggregate *indexingCall = TIntermAggregate::CreateFunctionCall(*indexingFunction, arguments);
...
bool RemoveDynamicIndexingTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
    ...
    if (node->getOp() == EOpIndexIndirect)
    {
        ...
        else if (IntermNodePatternMatcher::IsDynamicIndexingOfVectorOrMatrix(node))
        {
            ...
            {
                indexingFunction                = new TFunction(mSymbolTable, indexingFunctionName,
                                                 GetFieldType(type), SymbolType::ANGLE_INTERNAL);
            ...
            if (write)
            {
                ...
                if (mWrittenVecAndMatrixTypes.find(type) == mWrittenVecAndMatrixTypes.end())
                {
                    ...
                    indexedWriteFunction = new TFunction(
                        mSymbolTable, functionName, new TType(EbtVoid), SymbolType::ANGLE_INTERNAL);
