        ...
        else if (!node->getLeft()->isArray() && node->getLeft()->getBasicType() != EbtStruct)
        {
            ...
            if (write)
            {
                ...
                if (node->getLeft()->hasSideEffects())
                {
                    ...
                    // The only case where an l-value can have side effects is that when it is
                ...
                TIntermAggregate *indexingCall = CreateIndexFunctionCall(
                    node, node->getLeft(), createTempSymbol(node->getRight()->getType()));
            ...
            else
            {
                ...
                TIntermAggregate *indexingCall =
                    CreateIndexFunctionCall(node, node->getLeft(), node->getRight());
