...
RemoveDynamicIndexingTraverser::RemoveDynamicIndexingTraverser()
    : TIntermTraverser(true, false, false), mUsedInsertion(false)
{
}
...
bool RemoveDynamicIndexingTraverser::visitBinary(Visit visit, TIntermBinary *node)
{
        ...
        node->getLeft()->getBasicType() != EbtStruct)
    {
        ...
        if (write)
        {
            ...
            // TODO(oetuaho@nvidia.com): This doesn't work if v has some side effects. For example,
            // it can be V[j++] where V is an array of vectors.
            // TODO(oetuaho@nvidia.com): This is not optimal if the expression using the value only
            // writes it and doesn't need the previous value.
