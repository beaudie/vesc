    ...
    size_t operator()(const sh::SpirvType &type) const
    {
        ...
        if (type.block != nullptr)
        {
            return result ^ angle::ComputeGenericHash(&type.block, sizeof(type.block)) ^
                   ...
                   (static_cast<size_t>(type.typeDiff.isRowMajorQualifiedArray) << 2) ^
                   (type.typeDiff.blockStorage << 3);
        }
...
class SPIRVBuilder : angle::NonCopyable
{
  ...
  private:
    ...
    const SpirvTypeData &getArrayElementTypeDataForAlignmentAndSize(
        SpirvType elementSpirvType,
        const SpirvTypeDifferentiation &parentTypeDiff,
        uint32_t *matrixColumnsCountOut);
    const SpirvTypeData &getFieldTypeDataForAlignmentAndSize(
        const TType &type,
        const SpirvTypeDifferentiation &parentTypeDiff,
        uint32_t *totalColumnsCountOut);
