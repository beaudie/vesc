                                              ...
                                              unsigned int arrayIndex)
{
    ...
    {
        hlsl += "RWStructuredBuffer<" + InterfaceBlockStructName(interfaceBlock) + "> " +
                Decorate(interfaceBlock.name()) + ": register(u" + str(registerIndex) + ");\n";
    }
    return hlsl;
}
                                                 ...
                                                 TLayoutBlockStorage blockStorage)
{
    ...
    for (unsigned int typeIndex = 0; typeIndex < interfaceBlock.fields().size(); typeIndex++)
    {
        ...
        {
            // TODO(jiajia.qin@intel.com): implement the unsized array buffer. Currently, we just
            // set a temporary value 128 to pass the unsized array tests.
            fieldType.sizeOutermostUnsizedArray(128);
            UNIMPLEMENTED();
        }
