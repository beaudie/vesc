...
void UniformHLSL::outputHLSL11SamplerUniformGroup(TInfoSinkBase &out,
                                                  ...
                                                  unsigned int *groupTextureRegisterIndex)
{
    ...
    {
        out << "uniform SamplerComparisonState samplers" << suffix << "[" << groupRegisterCount
            << "]"
            << " : register(s" << (*groupTextureRegisterIndex) << ");\n";
    }
    else
    {
        out << "uniform SamplerState samplers" << suffix << "[" << groupRegisterCount << "]"
            << " : register(s" << (*groupTextureRegisterIndex) << ");\n";
    }
                                 ...
                                 const ReferencedSymbols &referencedUniforms)
{
    ...
    if (outputType == SH_HLSL11_OUTPUT)
    {
        ...
        auto groupTexture2DIter = groupedSamplerUniforms.find(HLSL_TEXTURE_2D);
        if (groupTexture2DIter != groupedSamplerUniforms.end())
        {
            outputHLSL11SamplerUniformGroup(out, HLSL_TEXTURE_2D, groupTexture2DIter->second,
                                            &groupTextureRegisterIndex);
        }
        for (const auto &group : groupedSamplerUniforms)
        {
            if (group.first != HLSL_TEXTURE_2D)
            {
                outputHLSL11SamplerUniformGroup(out, group.first, group.second,
                                                &groupTextureRegisterIndex);
            }
        }
    }
...
TString UniformHLSL::samplerMetadataConstants()
{
