                                               ...
                                               unsigned int *groupTextureRegisterIndex)
{
    ...
    TString suffix = ResourceViewGroupSuffix(RVTextureGroup);
    if (RVTextureGroup != HLSL_SRV_TEXTURE_2D_FLOAT4 &&
        RVTextureGroup != HLSL_UAV_TEXTURE_2D_FLOAT4)
    {
        out << "static const uint imageIndexOffset" << suffix << " = "
            << (*groupTextureRegisterIndex) << ";\n";
    }
    if (IsHLSLUAVTexture(RVTextureGroup))
    {
        out << "uniform " << ResourceViewString(RVTextureGroup) << " images" << suffix << "["
            << groupRegisterCount << "]"
                                 ...
                                 const ReferencedSymbols &referencedUniforms)
{
    ...
    TVector<TVector<const TIntermSymbol *>> groupedImageUniforms(HLSL_RV_TEXTURE_MAX + 1);
    ...
    for (auto &uniformIt : referencedUniforms)
    {
        ...
        {
            HLSLRVTextureGroup group = ResourceViewGroup(type);
    ...
    if (outputType == SH_HLSL_4_1_OUTPUT)
    {
        ...
        unsigned int groupSRVRegisterIndex = groupTextureRegisterIndex;
        for (int groupId = HLSL_SRV_TEXTURE_BEIGN; groupId < HLSL_SRV_TEXTURE_END; ++groupId)
        {
            outputHLSLImagesUniformGroup(out, HLSLRVTextureGroup(groupId),
                                         groupedImageUniforms[groupId], &groupSRVRegisterIndex);
        }

        unsigned int groupUAVRegisterIndex = 0;
        for (int groupId = HLSL_UAV_TEXTURE_BEIGN; groupId < HLSL_UAV_TEXTURE_END; ++groupId)
        {
            outputHLSLImagesUniformGroup(out, HLSLRVTextureGroup(groupId),
                                         groupedImageUniforms[groupId], &groupUAVRegisterIndex);
        }
    }
}
