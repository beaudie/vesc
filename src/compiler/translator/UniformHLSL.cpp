...
struct GetUniformCallback
{
  public:
    GetUniformCallback(unsigned int registerIndex, ShShaderOutput outputType)
...
{
    GetUniformCallback callback(registerIndex, mOutputType);
...
TString UniformHLSL::interfaceBlocksHeader(const ReferencedSymbols &referencedInterfaceBlocks)
{
         ...
         interfaceBlockIt != referencedInterfaceBlocks.end(); interfaceBlockIt++)
    {
        ...
        for (unsigned int typeIndex = 0; typeIndex < fieldList.size(); typeIndex++)
        {
            ...
            MatrixPackingCallback callback(field.type()->getLayoutQualifier().matrixPacking == EmpRowMajor);
