                                 ...
                                 const ReferencedSymbols &referencedUniforms)
{
    ...
    for (auto &uniformIt : referencedUniforms)
    {
        ...
        if (outputType == SH_HLSL_4_OUTPUT && IsSampler(type.getBasicType()))
        {
        ...
        else if (outputType == SH_HLSL_4_FL9_3_OUTPUT && IsSampler(type.getBasicType()))
        {
    ...
    if (outputType == SH_HLSL_4_OUTPUT)
    {
        ...
        {
            outputHLSLSamplerUniformGroup(out, HLSL_TEXTURE_2D, groupTexture2DIter->second,
                                          &groupTextureRegisterIndex);
        }
        for (const auto &group : groupedSamplerUniforms)
        {
            ...
            {
                outputHLSLSamplerUniformGroup(out, group.first, group.second,
                                              &groupTextureRegisterIndex);
            }
        }
    }
