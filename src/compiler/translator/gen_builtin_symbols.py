            ...
            for condition, objs in self.objs[level].iteritems():
                if len(objs) > 0:
                    ...
                    for name_hash, obj in sorted(switch.iteritems()):
                        code.append('case 0x' + ('%x' % name_hash) + 'u:\n{')
                        code += obj
                        code.append('break;\n}')
                    code.append('}')
...
def process_single_variable_group(condition, group_name, group):
    ...
    for variable_name, props in group['variables'].iteritems():
        ...
        if 'fields' in props:
            ...
            template_define_variable = '    {class} *{name_with_suffix} = new {class}(BuiltInId::{name_with_suffix}, BuiltInName::{name}, TExtension::{extension}, {fields});'
            init_member_variables.append(template_define_variable.format(**template_args))
            if 'private' in props and props['private']:
                is_member = False
            else:
                template_assign_variable = '    mVar_{name_with_suffix} = {name_with_suffix};'
                init_member_variables.append(template_assign_variable.format(**template_args))
            ...
            template_define_variable = """    {initDynamicType}
    {type_name}->realize();
    mVar_{name_with_suffix} = new TVariable(BuiltInId::{name_with_suffix}, BuiltInName::{name}, SymbolType::BuiltIn, TExtension::{extension}, {type});"""
            ...
            template_define_variable = """    mVar_{name_with_suffix} = new TVariable(BuiltInId::{name_with_suffix}, BuiltInName::{name}, SymbolType::BuiltIn, TExtension::{extension}, {type});
    {{
