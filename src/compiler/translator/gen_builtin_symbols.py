...
def allow_in_chromium(props, name):
    ...
    if essl_level == 'ESSL3_1_BUILTINS' or essl_level == 'ESSL3_2_BUILTINS':
        # Exceptions:
        #
        # - imageLoad(gimage2D), imageStore(gimage2D), memoryBarrierImage(),
        #   unpackUnorm4x8(), and packUnorm4x8() are used by ANGLE_shader_pixel_local_storage
        # - unpackSnorm4x8() is used by DriverUniform
        if name in ['imageLoad', 'imageStore']:
            return props['parameters'][0].data['basic'] == 'Image2D'
        if name in ['memoryBarrierImage', 'unpackSnorm4x8', 'unpackUnorm4x8', 'packUnorm4x8']:
            return True
        return False
