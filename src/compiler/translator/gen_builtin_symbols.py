...
{{
    ...
    if((entry.symbol && name.beginsWith(entry.symbol->getName())) || 
       (entry.var && name.beginsWith((this->*(entry.var))->getName())))
    {{
    ...
    {{
        if(entry.esslExtSymbol && name.beginsWith(entry.esslExtSymbol->getName()))
        {{
    ...
    {{
        if(entry.glslExtSymbol && name.beginsWith(entry.glslExtSymbol->getName()))
        {{
...
"""
            ...
            if essl_level != 'GLSL_BUILTINS':
                obj = '&BuiltInVariable::var_{name_with_suffix}'.format(**template_args)
