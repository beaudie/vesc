    ...
    for line in lines:
        ...
        if line.startswith('GROUP BEGIN '):
            ...
            if current_group['name'] != group_end_name:
                raise Exception('Unexpected function group name"' + group_end_name + '" was expecting "' + current_group['name'] + '"')
            group_stack.pop()
...
def process_single_function_group(condition, group_name, group):
    ...
    for function_props in group['functions']:
        ...
        template_args = {
            'name': function_name,
            'level': level,
            'suffix': get_suffix(function_props),
            'extension': extension,
...
{{
    return &UnmangledBuiltIns::{extension};
        ...
        for function_props in function_variants:
            ...
            template_args['unique_name'] = get_unique_identifier_name(function_name, parameters)
            ...
            template_function_declaration = 'constexpr const TFunction kFunction_{unique_name}(BuiltInId::{unique_name}, BuiltInName::{name}{suffix}, TExtension::{extension}, BuiltInParameters::{parameters_var_name}, {param_count}, {return_type}, BuiltInName::{unique_name}, EOp{op}, {known_to_not_have_side_effects});'
...
def process_variable_group(group_name, group):
    ...
    if 'variables' in group:
        for variable_name, props in group['variables'].iteritems():
            ...
            type_string = TType(props['type']).get_statictype_string()
            ...
            variable_declarations.append(template_variable_declaration.format(name = variable_name, suffix = name_suffix, extension = extension, type = type_string))
