...
constexpr SymbolRule kRules[] = {{ {mangled_rules} }};
...
template_rule = """{{ {spec}, {version}, {shaders}, {extension}, {symbol_or_var} }}"""
...
class GroupedList:
    ...
    def update_arrays(self):
        ...
        def add_rule(rules, spec, level, shaders, extension, symbol):
            var = ("&TableBase::m%s" % symbol) if symbol.startswith("gl") else None
            ...
            rules.append({
                ...
                "symbol_or_var": symbol if var is None else var
            })
        ...
        for hash_val in range(0, self.num_names):
            if hash_val in self.objs:
                ...
                self.offsets.append("%d, // %s" % (self.rule_offset, name))
                self.rules.append("// For %s\n %s" % (name, self.format_rules(rules)))
