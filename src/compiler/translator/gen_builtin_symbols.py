...
{{
    ...
    if (entry.esslExtSymbol && entry.esslExtSymbol->getName() == name && !IsDesktopGLSpec(mShaderSpec) &&
        checkESSLVersion(entry.esslExtVersion, version))
    {{
        if(mResources.*(entry.esslExtension))
        {{
            if (entry.isVarPointer)
                return this->*(entry.var);
    ...
    if (entry.glslExtSymbol && entry.glslExtSymbol->getName() == name && IsDesktopGLSpec(mShaderSpec) &&
        checkGLSLVersion(entry.glslExtVersion, version))
    {{
        if(mResources.*(entry.glslExtension))
        {{
            if (entry.isVarPointer)
                return this->*(entry.var);
