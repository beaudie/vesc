...
{{
    ...
    if (entry.isVarPointer && name.beginsWith((this->*(entry.var))->getName()))
    {{
        if(!IsDesktopGLSpec(mShaderSpec))
        {{
            if(checkESSLVersion(entry.esslVersion, version))
                return this->*(entry.var);
            if(entry.esslExtension != nullptr && checkESSLVersion(entry.esslExtVersion, version) && mResources.*(entry.esslExtension))
                return this->*(entry.var);
        }}
        if(IsDesktopGLSpec(mShaderSpec))
        {{
            if(checkGLSLVersion(entry.glslVersion, version))
                return this->*(entry.var);
            if(entry.glslExtension != nullptr && checkGLSLVersion(entry.glslExtVersion, version) && mResources.*(entry.glslExtension))
                return this->*(entry.var);
        }}
    }}
    return nullptr;
    ...
    def get_array(self):
        ...
        for hash_val in range(0, self.num_names):
            if hash_val in self.objs:
                ...
                data = []
                data.append(obj_data['obj']) if 'obj' in obj_data and not var_obj else data.append(
                    'nullptr')
                ...
                if 'essl_extension' in obj_data:
                    data.append(
                        obj_data['essl_ext_obj']) if not var_obj else data.append('nullptr')
                ...
                if 'glsl_extension' in obj_data:
                    data.append(
                        obj_data['glsl_ext_obj']) if not var_obj else data.append('nullptr')
                ...
                if var_obj:
                    data.append('&TSymbolTableBase::{}'.format(obj_data['var']))

                code.append('SymbolEntry(' + ', '.join(data) + ')')
            else:
                code.append(
                    'SymbolEntry(nullptr, -1, -1, 0, nullptr, -1, nullptr, nullptr, -1, nullptr)')
        return ', '.join(code)
