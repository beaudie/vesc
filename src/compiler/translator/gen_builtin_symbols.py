...
{{
    ...
    {{
        ...
        if(expected != 100 && actual >= expected)
        {{
            return true;
        }}
    }}
...
{{
    if(expected != -1)
    {{
        if(expected == 110 && actual == 110)
        {{
            return true;
        }}
        if(expected != 110 && actual >= expected)
        {{
            return true;
        }}
    }}
...
{{
    if(entry.shaderType != 0 && entry.shaderType != mShaderType)
    {{
        return nullptr;
    }}
    if(entry.symbol && name.beginsWith(entry.symbol->getName()))
    {{
    ...
    if (entry.esslExtSymbol && name.beginsWith(entry.esslExtSymbol->getName()) && !IsDesktopGLSpec(mShaderSpec) &&
        checkESSLVersion(entry.esslExtVersion, version) && mResources.*(entry.esslExtension))
    {{
        return entry.esslExtSymbol;
    }}
    if (entry.glslExtSymbol && name.beginsWith(entry.glslExtSymbol->getName()) && IsDesktopGLSpec(mShaderSpec) &&
        checkGLSLVersion(entry.glslExtVersion, version) && mResources.*(entry.glslExtension))
    {{
        return entry.glslExtSymbol;
    }}
    if (entry.var && name.beginsWith((this->*(entry.var))->getName()))
    {{
        if(!IsDesktopGLSpec(mShaderSpec) && checkESSLVersion(entry.esslVersion, version))
        {{
            return this->*(entry.var);
        }}
        if(IsDesktopGLSpec(mShaderSpec) && checkGLSLVersion(entry.glslVersion, version))
        {{
            return this->*(entry.var);
    ...
    if (entry.esslExtVar && name.beginsWith((this->*(entry.esslExtVar))->getName()) &&
        !IsDesktopGLSpec(mShaderSpec) && checkESSLVersion(entry.esslExtVersion, version))
    {{
        return this->*(entry.esslExtVar);
    }}
    if (entry.glslExtVar && name.beginsWith((this->*(entry.glslExtVar))->getName()) &&
        IsDesktopGLSpec(mShaderSpec) && checkGLSLVersion(entry.glslExtVersion, version))
    {{
        return this->*(entry.glslExtVar);
    }}
    return nullptr;
}}
...
{{
    ...
    if(nameHash >= {max_mangled_name_hash})
        return nullptr;
...
{{
    ...
    if(nameHash >= {max_unmangled_name_hash})
        return nullptr;
    ...
    def get_array(self):
        ...
        for hash_val in range(0, self.num_names):
            if hash_val in self.objs:
                ...
                data = []
                data.append(obj_data['obj']) if 'obj' in obj_data else data.append('nullptr')
                ...
                if 'essl_extension' in obj_data:
                    data.append(obj_data['essl_ext_obj']
                               ) if 'essl_ext_obj' in obj_data else data.append('nullptr')
                    data.append(get_essl_shader_version_for_level(obj_data['essl_ext_level']))
                    data.append('&ShBuiltInResources::{}'.format(obj_data['essl_extension']))
                else:
                    data.append('nullptr')
                    data.append('-1')
                    data.append('nullptr')

                if 'glsl_extension' in obj_data:
                    data.append(obj_data['glsl_ext_obj']
                               ) if 'glsl_ext_obj' in obj_data else data.append('nullptr')
                    data.append(get_essl_shader_version_for_level(obj_data['glsl_ext_level']))
                    data.append('&ShBuiltInResources::{}'.format(obj_data['glsl_extension']))
                else:
                    data.append('nullptr')
                    data.append('-1')
                    data.append('nullptr')

                data.append('&TSymbolTableBase::{}'.format(
                    obj_data['var'])) if 'var' in obj_data else data.append('nullptr')
                data.append('&TSymbolTableBase::{}'.format(obj_data['essl_ext_var'])
                           ) if 'essl_ext_var' in obj_data else data.append('nullptr')
                data.append('&TSymbolTableBase::{}'.format(obj_data['glsl_ext_var'])
                           ) if 'glsl_ext_var' in obj_data else data.append('nullptr')

                code.append('SymbolEntry(' + ', '.join(data) + ')')
            else:
                code.append(
                    'SymbolEntry(nullptr, -1, -1, 0, nullptr, -1, nullptr, nullptr, -1, nullptr, nullptr, nullptr, nullptr)'
                )
        return ', '.join(code)
...
class UnmangledGroupedList:
    ...
    def add_obj(self, essl_level, glsl_level, shader_type, name, obj, extension,
                unmangled_builtin_declaration, unmangled_script_generated_hash_tests):
        if essl_level and essl_level not in essl_levels:
            raise Exception('Unexpected essl level: ' + str(essl_level))
        if glsl_level and glsl_level not in glsl_levels:
            raise Exception('Unexpected glsl level: ' + str(glsl_level))
        ...
        self.objs[name_hash]['shader_type'] = shader_type
        self.objs[name_hash]['obj'] = obj
        self.objs[name_hash]['extension'] = extension
        self.objs[name_hash]['declaration'] = unmangled_builtin_declaration
    ...
    def get_array(self, unmangled_builtin_declarations):
        ...
        for hash_val in range(0, self.num_names):
            ...
            unmangled_builtin_declarations.add(obj_data['declaration'])
        return ', '.join(code)
...
def process_single_function_group(
    ...
    for function_props in group['functions']:
        ...
        obj = '&UnmangledBuiltIns::{extension}'.format(**template_args)
        ...
        if unmangled_builtin_no_shader_type != None and unmangled_builtin_no_shader_type[
            ...
            pass
                ...
                essl_level, glsl_level, shader_type, function_name)) or extension == 'UNDEFINED':
            ...
            unmangled_builtin_declaration = 'constexpr const UnmangledBuiltIn {extension}(TExtension::{extension});'.format(
                **template_args)
            unmangled_function_if_statements.add_obj(
                essl_level, glsl_level, shader_type, function_name, obj, extension,
                unmangled_builtin_declaration, unmangled_script_generated_hash_tests)
        ...
        for function_props in function_variants:
            ...
            obj = '&BuiltInFunction::kFunction_{unique_name}'.format(**template_args)
...
def process_single_variable_group(
        shader_type, group_name, group, builtin_id_declarations, builtin_id_definitions,
        name_declarations, init_member_variables, get_variable_declarations,
        get_builtin_if_statements, declare_member_variables, variable_declarations,
        get_variable_definitions, variable_name_count, script_generated_hash_tests):
    global id_counter
    ...
    mVar_{name_with_suffix} = new TVariable(BuiltInId::{name_with_suffix}, BuiltInName::{name}, SymbolType::BuiltIn, TExtension::{extension}, {type});"""
            ...
            template_init_variable = """    mVar_{name_with_suffix} = new TVariable(BuiltInId::{name_with_suffix}, BuiltInName::{name}, SymbolType::BuiltIn, TExtension::{essl_extension}, {type});
    {{
...
def process_variable_group(
    ...
    if 'subgroups' in group:
        for subgroup_name, subgroup in group['subgroups'].iteritems():
            process_variable_group(shader_type, subgroup_name, subgroup, builtin_id_declarations,
                                   builtin_id_definitions, name_declarations,
                                   init_member_variables, get_variable_declarations,
                                   get_builtin_if_statements, declare_member_variables,
                                   variable_declarations, get_variable_definitions,
                                   variable_name_count, script_generated_hash_tests)
...
def main():
    ...
    for group_name, group in parsed_variables.iteritems():
        process_variable_group(
            'NONE', group_name, group, builtin_id_declarations, builtin_id_definitions,
            name_declarations, init_member_variables, get_variable_declarations,
            get_builtin_if_statements, declare_member_variables, variable_declarations,
            get_variable_definitions, variable_name_count, script_generated_hash_tests)
    ...
    output_strings = {
        ...
        'max_unmangled_name_hash':
            num_unmangled_names,
        'max_mangled_name_hash':
            num_mangled_names,
        'unmangled_builtin_declarations':
