...
def get_parsed_functions():
    ...
    group_stack = []
    group_stack.append(parsed_functions)
    default_metadata = {}
    ...
    for line in lines:
        ...
        if line.startswith('GROUP'):
            if line.startswith('GROUP END '):
                ...
                if 'subgroups' in super_group:
                    super_group['subgroups'][current_group['name']] = current_group
                else:
                    super_group[current_group['name']] = current_group
                current_group = super_group
            ...
            if line == 'DEFAULT CLEAR':
                default_metadata = {}
            else:
                line_rest = line[8:].strip()
            ...
            current_group['functions'].append(function_props)
...
output_strings['variable_data_source_name'] = variables_json_filename
output_strings['variable_declarations'] = '\n'.join(sorted(variable_declarations))
output_strings['get_variable_declarations'] = '\n'.join(sorted(get_variable_declarations))
output_strings['get_variable_definitions'] = '\n'.join(sorted(get_variable_definitions))
output_strings['insert_variables'] = '\n'.join(insert_variables)
