...
{{
    return &UnmangledBuiltIns::{extension};
        ...
        for function_props in function_variants:
            ...
            template_builtin_id_declaration = 'constexpr const TSymbolUniqueId {unique_name} = TSymbolUniqueId({id});'
...
def process_single_variable_group(group_name, group):
    ...
    for variable_name, props in group['variables'].iteritems():
        ...
        if 'initDynamicType' in props:
            ...
            raise Exception('Cannot determine type for variable: ' + str(template_args['name']))
        template_args['extension'] = get_extension(props)

        template_builtin_id_declaration = 'constexpr const TSymbolUniqueId {name_with_suffix} = TSymbolUniqueId({id});'
        builtin_id_declarations.append(template_builtin_id_declaration.format(**template_args))
