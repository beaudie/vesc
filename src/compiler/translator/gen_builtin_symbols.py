...
# Inserting builtin TVariables to the symbol table.
insert_variables = []
...
def process_group(group_name, group):
    ...
    if 'variables' in group:
        for variable_name, props in group['variables'].iteritems():
            ...
            template_variable_declaration = 'constexpr const TVariable kVar_{name}{suffix}(BuiltInId::{name}{suffix}, BuiltInName::{name}, SymbolType::BuiltIn, TExtension::{extension}, StaticType::Get<{type}>());'
            ...
            get_variable_declarations.append(template_get_variable_declaration.format(id = variable_id, name = variable_name, suffix = name_suffix));
