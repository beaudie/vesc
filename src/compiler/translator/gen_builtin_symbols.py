...
def process_function_group(group_name, group):
    ...
    if 'functions' in group:
        for function_name, props in group['functions'].iteritems():
            ...
            template_name_declaration = 'constexpr const ImmutableString {name}("{name}");'
            name_declaration = template_name_declaration.format(name = function_name)
            if not name_declaration in name_declarations:
                name_declarations.add(name_declaration)

                template_insert_unmangled = '    insertUnmangledBuiltInName(BuiltInName::{name}.data(), {level});'
                insert_unmangled_functions.append(template_insert_unmangled.format(name = function_name, level = props['level']))
