...
bool ValidateSwitch::visitCase(Visit, TIntermCase *node)
{
    ...
    {
        mContext->error(node->getLine(), "case statement nested inside control flow", nodeStr);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                ++mCasesUnsigned[uConst];
                mContext->error(condition->getLine(), "duplicate case label", nodeStr);
                mDuplicateCases = true;
            }
            else
            {
                mCasesUnsigned[uConst] = 1;
            }

        }
    }
    // No need to traverse the condition of the case statement
    return false;
}
...
bool ValidateSwitch::validate(const TSourceLoc &line)
{
