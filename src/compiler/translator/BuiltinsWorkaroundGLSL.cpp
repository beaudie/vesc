...
void TBuiltinsWorkaroundGLSL::visitSymbol(TIntermSymbol *node)
{
    if (node->variable().symbolType() == SymbolType::BuiltIn)
    {
        ...
        {
            if ((mCompileOptions & SH_ADD_BASE_VERTEX_TO_VERTEX_ID) && isBaseVertexDeclared &&
                node->getName() == kGlVertexIDString)
            {
                TVariable *baseVertex =
                    new TVariable(mSymbolTable, ImmutableString("angle_BaseVertex"),
                                  StaticType::Get<EbtInt, EbpHigh, EvqUniform, 1, 1>(),
                                  SymbolType::AngleInternal);
                TIntermSymbol *baseVertexRef = new TIntermSymbol(baseVertex);
...
bool TBuiltinsWorkaroundGLSL::visitDeclaration(Visit, TIntermDeclaration *node)
{
    ...
    for (TIntermNode *variableNode : sequence)
    {
        ...
        if (variable && variable->variable().symbolType() == SymbolType::AngleInternal)
        {
            ...
            else if (variable->getName() == "angle_BaseVertex")
            {
                isBaseVertexDeclared = true;
            }
        }
    }
    return true;
