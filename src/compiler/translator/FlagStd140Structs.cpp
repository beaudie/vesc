...
void FlagStd140Structs::mapBlockStructMembers(TIntermSymbol *blockDeclarator,
                                              TInterfaceBlock *block)
{
    for (auto *field : block->fields())
    {
        if (field->type()->getBasicType() == EbtStruct)
        {
            ...
            mappedStruct.staticallyUsed  = false;
            mMappedStructs.push_back(mappedStruct);
        }
    }
}

bool FlagStd140Structs::visitDeclaration(Visit visit, TIntermDeclaration *node)
{
...
std::vector<MappedStruct> FlagStd140ValueStructs(TIntermNode *node)
{
