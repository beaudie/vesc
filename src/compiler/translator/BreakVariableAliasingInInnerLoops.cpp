...
// BreakVariableAliasingInInnerLoops.cpp: Work around for a HLSL compiler frontend optimization
...
class AliasingBreaker : public TIntermTraverser
{
  ...
  protected:
    bool visitBinary(Visit visit, TIntermBinary *binary)
    {
        ...
        TIntermBinary *bPlusZero = new TIntermBinary(EOpAdd);
        bPlusZero->setLeft(B);
        bPlusZero->setRight(TIntermConstantUnion::CreateZero(type));
