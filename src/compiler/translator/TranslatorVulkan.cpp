...
void TranslatorVulkan::translate(TIntermBlock *root, ShCompileOptions compileOptions)
{
    ...
    {
        sink << "\nlayout(set = 0, binding = 0) uniform defaultUniforms\n{\n";
    ...
    TOutputVulkanGLSL outputGLSL(
        getInfoSink().obj, getArrayIndexClampingStrategy(), getHashFunction(), getNameMap(),
        &getSymbolTable(), getShaderType(), getShaderVersion(), getOutputType(), compileOptions);
