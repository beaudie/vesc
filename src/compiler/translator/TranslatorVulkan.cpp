...
// |uniform atomic_uint| declarations with a global declaration that holds the binding/offset, as
// well as substituting |atomicVar[n]| with |offset + n|.
class ReplaceAtomicCountersTraverser : public TIntermTraverser
{
  public:
    ReplaceAtomicCountersTraverser(TSymbolTable *symbolTable, const TVariable *atomicCounters)
    ...
    void visitFunctionPrototype(TIntermFunctionPrototype *node) override
    {
        ...
        for (size_t paramIndex = 0; paramIndex < function->getParamCount(); ++paramIndex)
        {
            ...
            if (IsAtomicCounter(param->getType().getBasicType()))
            {
                                 ...
                                 PerformanceDiagnostics * /*perfDiagnostics*/)
{
    ...
    if (atomicCounterCount > 0)
    {
        ...
        ReplaceAtomicCountersTraverser replaceAtomicCounters(&getSymbolTable(), atomicCounters);
