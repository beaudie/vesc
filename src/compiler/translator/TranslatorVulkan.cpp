...
// These are only used when createRotationFlipSpecConst returns null.
                                                   ...
                                                   const std::vector<TField *> &additionalFields)
{
    ...
    if (rotationSpecConst)
    {
        ...
        const std::array<TType *, kNumGraphicsDriverUniforms> kDriverUniformTypes = {
            {new TType(EbtFloat, 4), new TType(EbtFloat, 2),
             new TType(
                 EbtUInt),  // uint clipDistancesEnabled;  // 32 bits for 32 clip distances max
    ...
    else
    {
        ...
        constexpr std::array<const char *, kNumGraphicsDriverUniforms> kGraphicsDriverUniformNames =
            {{kViewport, kHalfRenderArea, kFlipXY, kNegFlipXY, kClipDistancesEnabled,
              kXfbActiveUnpaused, kXfbVerticesPerDraw, kXfbBufferOffsets, kAcbBufferOffsets,
              kDepthRange}};
                                     ...
                                     TOutputVulkanGLSL *outputGLSL)
{
    ...
    else
    {
        ...
        driverUniforms = AddGraphicsDriverUniformsToShader(
            root, &getSymbolTable(), surfaceRotationSpecConst, additionalFields);
    }
...
ANGLE_NO_DISCARD TIntermSymbol *TranslatorVulkan::createRotationFlipSpecConst(TInfoSinkBase &sink)
{
    sink << "layout(constant_id="
