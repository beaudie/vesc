                             ...
                             const TVariable *driverUniforms)
{
    ...
    TIntermConstantUnion *scaleFactor = CreateFloatNode(64.0f);
    TIntermBinary *scaledViewport =
        new TIntermBinary(EOpVectorTimesScalar, viewportRef, scaleFactor);
    ...
    TIntermBinary *scaledPosition =
        new TIntermBinary(EOpMul, anglePositionRef->deepCopy(), scaleRef);
    // floor(ANGLEPosition * scale)
    TIntermUnary *flooredScaledPosition = new TIntermUnary(EOpFloor, scaledPosition, nullptr);
    // floor(ANGLEPosition * scale) / scale;
    TIntermBinary *newPosition =
        new TIntermBinary(EOpDiv, flooredScaledPosition, scaleRef->deepCopy());
    TIntermBinary *newAssignment =
        new TIntermBinary(EOpAssign, anglePositionRef->deepCopy(), newPosition);
