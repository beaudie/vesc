...
constexpr ImmutableString kLineRasterEmulationSpecConstVarName =
    ImmutableString("ANGLEEmulateLine");
                                     ...
                                     TOutputVulkanGLSL *outputGLSL)
{
    ...
    if (getShaderType() != GL_COMPUTE_SHADER)
    {
        ...
        // Add specialization constant declarations.  Glslang wrapper itself selectively removes the
        // specialization constant declarations and dependent code paths from the generated SPIR-V,
        // so the specialization constants are always using their default values when creating the
        // pipeline.  Therefore, the constant_id assigned here is arbitrary.  Furthermore, the
        // default value of the specialization constants is to **enable** the specialization.
        // TODO: actually, selecting based on constant id is simpler, so switch to that.  Let's see
        // what the SPIR-V looks like to begin with.
        sink << "layout(constant_id=0) const bool " << kLineRasterEmulationSpecConstVarName
             << " = true;\n\n";
    }
