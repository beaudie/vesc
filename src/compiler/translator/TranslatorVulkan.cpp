...
constexpr const char *kInvViewportYScale    = "invViewportYScale";

constexpr size_t kNumDriverUniforms                                        = 6;
constexpr std::array<const char *, kNumDriverUniforms> kDriverUniformNames = {
    {"viewport", kHalfRenderAreaHeight, kViewportYScale, kInvViewportYScale, "padding",
     "depthRange"}};
                                          ...
                                          bool usesFragCoord)
{
    ...
    TIntermConstantUnion *epsilon = CreateBasicConstant(0.00001f);
    ...
    if (getShaderType() == GL_FRAGMENT_SHADER)
    {
        ...
        if (usesPointCoord)
        {
            TIntermBinary *viewportYScale =
                CreateDriverUniformRef(driverUniforms, kInvViewportYScale);
