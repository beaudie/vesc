...
bool IsLooseUniform(const TType &type)
{
    return type.getQualifier() == EvqUniform && !type.isInterfaceBlock() &&
           !IsOpaqueType(type.getBasicType());
}

using VariableMap = angle::HashMap<const TVariable *, TVariable *>;
class ReplaceDefaultUniformsTraverser : public TIntermTraverser
{
  public:
    ReplaceDefaultUniformsTraverser(const VariableMap &variableMap)
    ...
    void visitSymbol(TIntermSymbol *symbol) override
    {
        ...
        if (!IsLooseUniform(type) || variable.name().beginsWith("gl_"))
        {
            return;
        }
        ...
        queueReplacement(new TIntermSymbol(mVariableMap.at(&variable)), OriginalNode::IS_DROPPED);
    }

  private:
    const VariableMap &mVariableMap;
};
...
{
    // First, collect all loose uniforms and declare a uniform block.
    ...
    for (size_t fieldIndex = 0; fieldIndex < uniformVars.size(); ++fieldIndex)
    {
        ...
        variableMap[variable] = replacementVariable;
    }
