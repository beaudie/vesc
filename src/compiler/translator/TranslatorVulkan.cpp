...
ANGLE_NO_DISCARD bool AppendPreRotation(TCompiler *compiler,
                                        ...
                                        const DriverUniform *driverUniforms)
{
    TIntermTyped *preRotationRef = specConsts->getPreRotationMatrix();
    if (!preRotationRef)
    {
                                              ...
                                              const DriverUniform *driverUniforms)
{
    ...
    TIntermIfElse *ifEmulation =
        new TIntermIfElse(specConsts->getLineRasterEmulation(), emulationBlock, nullptr);
...
ANGLE_NO_DISCARD bool InsertFragCoordCorrection(TCompiler *compiler,
                                                ...
                                                const DriverUniform *driverUniforms)
{
    TIntermTyped *flipXY = specConsts->getFlipXY();
    if (!flipXY)
    {
        flipXY = driverUniforms->getFlipXYRef();
    }

    TIntermBinary *pivot = specConsts->getHalfRenderArea();
    if (!pivot)
    {
    ...
    {
        fragRotation = specConsts->getFragRotationMatrix();
        if (!fragRotation)
        {
                                              ...
                                              bool usesFragCoord)
{
    ...
    if (!usesFragCoord)
    {
        if (!InsertFragCoordCorrection(compiler, compileOptions, root, emulationSequence,
                                       symbolTable, specConsts, driverUniforms))
        {
            return false;
        }
    }
                                     ...
                                     TOutputVulkanGLSL *outputGLSL)
{
    ...
    if (getShaderType() == GL_FRAGMENT_SHADER)
    {
        ...
        if (compileOptions & SH_ADD_BRESENHAM_LINE_RASTER_EMULATION)
        {
            if (!AddBresenhamEmulationFS(this, compileOptions, sink, root, &getSymbolTable(),
                                         specConsts, driverUniforms, usesFragCoord))
            {
                return false;
            }
        }
        ...
        if (usesPointCoord)
        {
            TIntermTyped *flipNegXY = specConsts->getNegFlipXY();
            if (!flipNegXY)
            {
        ...
        if (usesFragCoord)
        {
            if (!InsertFragCoordCorrection(this, compileOptions, root, GetMainSequence(root),
                                           &getSymbolTable(), specConsts, driverUniforms))
            {
                return false;
            }
        }
    ...
    else if (getShaderType() == GL_VERTEX_SHADER)
    {
        if (compileOptions & SH_ADD_BRESENHAM_LINE_RASTER_EMULATION)
        {
            if (!AddBresenhamEmulationVS(this, root, &getSymbolTable(), specConsts, driverUniforms))
            {
                return false;
            }
        }
        ...
        if ((compileOptions & SH_ADD_PRE_ROTATION) != 0 &&
            !AppendPreRotation(this, root, &getSymbolTable(), specConsts, driverUniforms))
        {
            return false;
        }
    }
    ...
    specConsts->outputLayoutString(sink);

    // Gather specialization constant usage bits so that we can feedback to context.
    mSpecConstUsageBits = specConsts->getSpecConstUsageBits();
                                 ...
                                 PerformanceDiagnostics *perfDiagnostics)
{
    ...
    SpecConst specConsts(&getSymbolTable(), compileOptions);
    DriverUniform driverUniforms;
    if (!translateImpl(root, compileOptions, perfDiagnostics, &specConsts, &driverUniforms,
                       &outputGLSL))
    {
        return false;
    }
