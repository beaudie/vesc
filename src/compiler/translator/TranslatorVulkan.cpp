...
constexpr const char kFlipY[]                = "flipY";
...
constexpr size_t kNumGraphicsDriverUniforms                                                = 14;
constexpr std::array<const char *, kNumGraphicsDriverUniforms> kGraphicsDriverUniformNames = {
    {kViewport, kHalfRenderArea, kFlipXY, kNegFlipXY, kFlipY, kNegFlipY, kClipDistancesEnabled,
...
const TVariable *AddGraphicsDriverUniformsToShader(TIntermBlock *root, TSymbolTable *symbolTable)
{
    ...
    const std::array<TType *, kNumGraphicsDriverUniforms> kDriverUniformTypes = {{
        ...
        new TType(EbtUInt),
        new TType(EbtUInt),
        // NOTE: There's a float gap here that can be used in the future
        new TType(EbtInt, 4),
        new TType(EbtUInt, 4),
                                     ...
                                     TOutputVulkanGLSL *outputGLSL)
{
    ...
    if (getShaderType() == GL_FRAGMENT_SHADER)
    {
        ...
        {
            TIntermBinary *flipY = CreateDriverUniformRef(driverUniforms, kFlipY);
