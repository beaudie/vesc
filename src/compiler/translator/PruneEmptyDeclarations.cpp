    ...
    if (node->getOp() == EOpDeclaration)
    {
        ...
        if (sequence->size() >= 1)
        {
            ...
            if (sym != nullptr && sym->getSymbol() == "" && !sym->isInterfaceBlock())
            {
                ...
                else if (sym->getType().getQualifier() != EvqGlobal && sym->getType().getQualifier() != EvqTemporary)
                {
                    // We've hit an empty struct declaration with a qualifier, for example like this:
                    // const struct a { int i; };
                    // NVIDIA GL drivers version 367.27 don't accept this kind of declarations, so
