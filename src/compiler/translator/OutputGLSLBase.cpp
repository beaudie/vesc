...
void TOutputGLSLBase::declareInterfaceBlockLayout(const TInterfaceBlock *interfaceBlock)
{
    ...
    // If all matrix members are row-major, declare row-major layout per block. This attempts to
    // work around bug on AMD GL drivers - they don't seem to take matrix layout of individual
    // member fields into account. Otherwise it should not be needed as we always declare matrix
    // layout for individual members.
