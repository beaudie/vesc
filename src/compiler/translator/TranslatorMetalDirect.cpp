...
bool TranslatorMetalDirect::translateImpl(TIntermBlock &root, ShCompileOptions compileOptions)
{
    ...
    if (getShaderType() == GL_FRAGMENT_SHADER)
    {
        ...
        {
            // FIXME(dino): WebKit version is different here.
            SpecConst specConst(&symbolTable, compileOptions);
            DriverUniform driverUniforms;
            if (getShaderType() == GL_COMPUTE_SHADER)
            {
                driverUniforms.addComputeDriverUniformsToShader(&root, &symbolTable);
            }
            else
            {
                driverUniforms.addGraphicsDriverUniformsToShader(&root, &symbolTable);
            }

            if (!RewriteDfdy(this, compileOptions, &root, symbolTable, getShaderVersion(), &specConst, &driverUniforms))
            {
                return false;
            }
        }
