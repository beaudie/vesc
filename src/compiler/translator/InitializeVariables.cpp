...
void AddArrayZeroInitSequence(const TIntermTyped *initializedNode,
                              TIntermSequence *initSequenceOut, TSymbolTable *symbolTable);
                               ...
                               TIntermSequence *initSequenceOut, TSymbolTable *symbolTable)
{
    ...
    for (int i = 0; i < static_cast<int>(structType->fields().size()); ++i)
    {
        ...
        if (element->isArray())
        {
            AddArrayZeroInitSequence(element, initSequenceOut, symbolTable);
        }
        else if (element->getType().isStructureContainingArrays())
        {
            AddStructZeroInitSequence(element, initSequenceOut, symbolTable);
        }
        else
        {
...
void AddArrayZeroInitSequence(const TIntermTyped *initializedNode, TIntermSequence *initSequenceOut, TSymbolTable *symbolTable)
{
    ...
    TIntermDeclaration *indexInit = CreateTempInitDeclarationNode(indexSymbol, CreateZeroNode(TType(EbtInt)), EvqTemporary);
    TIntermConstantUnion *arraySizeNode = CreateIndexNode(initializedNode->getOutermostArraySize());
    TIntermBinary *indexSmallerThanSize = new TIntermBinary(EOpLessThan, indexSymbolNode->deepCopy(), arraySizeNode);
    ...
    TIntermBinary *element =
        new TIntermBinary(EOpIndexDirect, initializedNode->deepCopy(), indexSymbolNode->deepCopy());
    if (element->isArray())
    {
        AddArrayZeroInitSequence(element, forLoopBodySeq, symbolTable);
    }
    else if (element->getType().isStructureContainingArrays())
    {
        AddStructZeroInitSequence(element, forLoopBodySeq, symbolTable);
    }
    else
    {
        forLoopBodySeq->push_back(CreateZeroInitAssignment(element));
    }

    TIntermLoop *forLoop = new TIntermLoop(ELoopFor, indexInit, indexSmallerThanSize, indexIncrement, forLoopBody);
                    ...
                    const TExtensionBehavior &extensionBehavior)
{
    for (const auto &var : variables)
    {
        ...
        TIntermSequence *initCode = CreateInitCode(initializedSymbol, symbolTable);
...
{
  public:
    InitializeLocalsTraverser(int shaderVersion, TSymbolTable *symbolTable)
        ...
        for (TIntermNode *declarator : *node->getSequence())
        {
            if (!mInGlobalScope && !declarator->getAsBinaryNode())
            {
                ...
                if (arrayConstructorUnavailable || IsNamelessStruct(symbol))
                {
                    ...
                    insertStatementsInParentBlock(TIntermSequence(), *CreateInitCode(symbol, mSymbolTable));
                }
                else
                {
                    TIntermBinary *init =
...
TIntermSequence *CreateInitCode(const TIntermTyped *initializedSymbol, TSymbolTable *symbolTable)
{
    ...
    {
        AddArrayZeroInitSequence(initializedSymbol, initCode, symbolTable);
    }
    else if (initializedSymbol->getType().isStructureContainingArrays() ||
             IsNamelessStruct(initializedSymbol))
    {
        AddStructZeroInitSequence(initializedSymbol, initCode, symbolTable);
    }
    else
    {
...
{
    InitializeLocalsTraverser traverser(shaderVersion, symbolTable);
