    ...
    for (size_t ii = 0; ii < mVariables.size(); ++ii)
    {
        ...
        if (var.isStruct())
        {
            ...
            for (int fieldIndex = 0; fieldIndex < static_cast<int>(var.fields.size()); ++fieldIndex)
            {
                ...
                TIntermTyped *indexNode          = TIntermTyped::CreateIndexNode(fieldIndex);
                TIntermBinary *indexDirectStruct = new TIntermBinary(EOpIndexDirectStruct, symbol, indexNode);


                const sh::ShaderVariable &field = var.fields[fieldIndex];
                TType fieldType                 = sh::ConvertShaderVariableTypeToTType(field.type);
                TIntermTyped *zero              = TIntermTyped::CreateZero(fieldType);
                TIntermBinary *assign           = new TIntermBinary(EOpAssign, indexDirectStruct, zero);
