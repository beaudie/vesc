...
{
    TIntermTyped *zero = CreateZero(initializedNode->getType());
                    ...
                    const TExtensionBehavior &extensionBehavior)
{
    for (const auto &var : variables)
    {
        ...
        if (var.isBuiltIn())
        {
            initializedSymbol = ReferToBuiltInSymbol(name, symbolTable, shaderVersion);
        ...
        {
            initializedSymbol = ReferToGlobalSymbol(name, symbolTable);
        }
    ...
    bool visitDeclaration(Visit visit, TIntermDeclaration *node) override
    {
        for (TIntermNode *declarator : *node->getSequence())
        {
            if (!mInGlobalScope && !declarator->getAsBinaryNode())
            {
                ...
                {
                    TIntermBinary *init =
                        new TIntermBinary(EOpInitialize, symbol, CreateZero(symbol->getType()));
