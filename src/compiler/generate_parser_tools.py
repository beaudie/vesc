...
def run_flex(basename):
    ...
    output_source = basename + '_lex.cpp'

    flex_args = [flex, '--noline', '--nounistd', '--outfile=' + output_source, input_file]
    process = subprocess.Popen(flex_args, cwd=sys.path[0])
    process.communicate()
...
def run_bison(basename, generate_header):
    ...
    output_header = basename + '_tab.h'
    output_source = basename + '_tab.cpp'
    ...
    bison_env['BISON_PKGDATADIR'] = get_tool_path_platform('', 'common')
...
def get_output_files(basename, generate_header):
    optional_header = [basename + '_tab.h'] if generate_header else []
    files = [basename + '_lex.cpp', basename + '_tab.cpp'] + optional_header
    return [os.path.join(sys.path[0], f) for f in files]
...
def generate_parser(basename, generate_header):
    ...
    if not run_flex(basename):
        return 1
    if not run_bison(basename, generate_header):
        return 2
