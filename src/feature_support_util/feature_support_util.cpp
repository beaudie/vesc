...
{
  public:
    ListOf(const std::string listType) : mListType(listType), mWildcard(true) {}
    ~ListOf() { mList.clear(); }
    ...
    T &front() const { return (mList.front()); }
    void logListOf(const std::string prefix, const std::string name)
    {
        if (mWildcard)
        {
...
class RuleList
{
  public:
    ...
    bool getAppChoice(const Scenario &toCheck)
    {
        ...
        for (std::vector<Rule>::iterator it = mRuleList.begin(); it != mRuleList.end(); ++it)
        {
                                             ...
                                             int *rulesVersion)
{
    ...
    if (!rulesLength)
    {
        ALOGW("Asked to read an empty rules file");
        return false;
    }
                                               ...
                                               const char *appName)
{
    ...
    // TODO: Query whether there's really one and only one GPU:
