...
{
  public:
    Rule(const std::string description, bool answer)
    ...
    void logRule() const
    {
        VERBOSE("  Rule: \"%s\" %s ANGLE", mDescription.c_str(), mAnswer ? "enables" : "disables");
  ...
  public:
    ...
    static RuleList *ReadRulesFromJsonString(const std::string jsonFileContents)
    {
        ...
        for (unsigned int ruleIndex = 0; ruleIndex < jRules.size(); ruleIndex++)
        {
            ...
            bool ruleAnswer             = jRule[kJsonUseANGLE].asBool();
            Rule *newRule = new Rule(ruleDescription, ruleAnswer);
