...
{
  public:
    Application(const std::string name, const Version &version, const std::string intent)
        : mName(name), mVersion(version), mIntent(intent), mWildcard(false)
    {
    }
    Application(const std::string name, const std::string intent)
    ...
    Application(const std::string name) : mName(name), mVersion(), mIntent(), mWildcard(false) {}
    ...
    void logItem() const
    {
        ...
        else if (!mVersion.mWildcard)
        {
            ...
            {
                VERBOSE("      Application \"%s\" (version: %s; intent: \"%s\")", mName.mPart.c_str(),
        ...
        {
            VERBOSE("      Application \"%s\" (intent: \"%s\")", mName.mPart.c_str(),
                  mIntent.mPart.c_str());
        }
        else
        {
...
class GPU
{
  public:
    ...
    GPU(uint32_t deviceId, const Version &version)
    ...
    static GPU *CreateGpuFromJson(const Json::Value &jObject)
    {
        ...
        {
            WARN("Asked to parse a GPU, but no GPU found");
        }
...
{
  public:
    Rule(const std::string description, bool appChoice, bool answer)
    ...
    bool getAppChoice() const { return mAppChoice; }
    bool getAnswer() const { return mAnswer; }
    void logRule() const
    {
        VERBOSE("  Rule: \"%s\" %s ANGLE, and %s the app a choice if matched", mDescription.c_str(),
...
class RuleList
{
  public:
    ...
    bool getAppChoice(const Scenario &toCheck)
    {
        ...
        for (const Rule &rule : mRuleList)
        {
            ...
            {
                VERBOSE("  -> Rule matches.  Setting the app choice to %s",
                      rule.getAppChoice() ? "true" : "false");
                appChoice = rule.getAppChoice();
            }
            else
            {
    ...
    bool getAnswer(const Scenario &toCheck)
    {
        ...
        for (const Rule &rule : mRuleList)
        {
            ...
            if (rule.match(toCheck))
            {
                VERBOSE("  -> Rule matches.  Setting the answer to %s",
                      rule.getAnswer() ? "true" : "false");
...
// This function is part of the version-2 API:
ANGLE_EXPORT bool ANGLEAndroidParseRulesFile(const char *rulesString,
                                             long rulesLength,
                                             ...
                                             int *rulesVersion)
{
    ...
    if (!rulesLength)
    {
        WARN("Asked to read an empty rules file");
        return false;
    }
...
{
    angle::SystemInfo *systemInfo = reinterpret_cast<angle::SystemInfo *>(systemInfoHandle);
    if (systemInfo)
    {
...
// This function is part of the version-2 API:
ANGLE_EXPORT bool ANGLEEvalRulesForApplication(const RulesHandle rulesHandle,
                                               int rulesVersion,
                                               const SystemInfoHandle systemInfoHandle,
                                               const char *appName)
{
    RuleList *rules               = reinterpret_cast<RuleList *>(rulesHandle);
    angle::SystemInfo *systemInfo = reinterpret_cast<angle::SystemInfo *>(systemInfoHandle);
    ...
    bool rtn = rules->getAnswer(scenario);
    VERBOSE("Application \"%s\" should %s ANGLE", appName, rtn ? "use" : "NOT use");
