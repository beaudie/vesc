...
class Version
{
  public:
    ...
    static Version FromJson(const Json::Value &jObject)
    {
        ...
        VERBOSE("Created Version::FromJson: (%d) %d (%d) %d (%d) %d (%d) %d\n",
                version.mMajor.mWildcard, version.mMajor.mPart, version.mMinor.mWildcard,
                version.mMinor.mPart, version.mSubminor.mWildcard, version.mSubminor.mPart,
                version.mPatch.mWildcard, version.mPatch.mPart);
        return version;
    }
    ...
    static RuleList *ReadRulesFromJsonString(const std::string jsonFileContents)
    {
        ...
        for (const auto &jRule : jTopLevelObject[kJsonRules])
        {
            ...
            for (const auto &jApp : jRule[kJsonApplications])
            {
                ...
                {
                    VERBOSE("Failed to parse application.\n");
                }
            }
