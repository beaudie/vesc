...
// This function is part of both the version-1 and version-2 APIs:
...
// This function is part of the version-1 API:
...
{
    // TODO: WHO/HOW WILL THIS BE FREED?
//    struct SystemInfo *systemInfo =
//            reinterpret_cast<struct SystemInfo *>(malloc(sizeof(struct SystemInfo)));
//            new struct SystemInfo;
...
ANGLE_EXPORT bool ANGLEUseForAndroidApplication(void* rules_handle,
                                                ...
                                                const char *appName)
{
    RuleList *rules = reinterpret_cast<RuleList *>(rules_handle);
    angle::SystemInfo *systemInfo = reinterpret_cast<angle::SystemInfo *>(system_info_handle);
    Scenario scenario(appName, "FIXME-PLUMB deviceMfr", systemInfo->machineModelName.c_str());
    scenario.logScenario();
