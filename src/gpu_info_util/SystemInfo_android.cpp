...
bool GetAndroidSystemProperty(const std::string &propertyName, std::string *value)
{
    ...
    *value = FormatString("%s", propertyBuf.data());
    return true;
}
...
bool GetSystemInfo(SystemInfo *info)
{
    ...
    isFullyPopulated &= GetAndroidSystemProperty("ro.product.model", &info->machineModelName);
    isFullyPopulated &=
        GetAndroidSystemProperty("ro.build.version.release", &info->machineModelVersion);
