...
#    endif
        ...
        while ((entry = IOIteratorNext(entryIterator)) != IO_OBJECT_NULL)
        {
            ...
            uint32_t classCode;
            GPUDeviceInfo info;
            // The matching registry entry points to the system device ID used for
            ...
            {
                // If the matching entry is a IOGraphicsAccelerator2, get the parent entry that will
                // be the IOPCIDevice which holds vendor-id and device-id
