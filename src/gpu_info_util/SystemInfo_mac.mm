...
int64_t GetGpuIDFromDisplayID(PlatformDisplayID displayID)
{
    ...
    error = CGLDescribeRenderer(rendererInfo, 0, static_cast<CGLRendererProperty>(140), &gpuIDLow);
    ...
    error = CGLDescribeRenderer(rendererInfo, 0, static_cast<CGLRendererProperty>(141), &gpuIDHigh);
    if (error != kCGLNoError)
    {
        CGLDestroyRendererInfo(rendererInfo);
        return 0;
    ...
    CGLDestroyRendererInfo(rendererInfo);
    return static_cast<int64_t>(gpuIDHigh) << 32 | gpuIDLow;
}
