...
int64_t GetGpuIDFromDisplayID(PlatformDisplayID displayID)
{
    ...
    // kCGLRPRegistryIDLow is 140
    error = CGLDescribeRenderer(rendererInfo, 0, (CGLRendererProperty)140, &gpuIDLow);
    ...
    // kCGLRPRegistryIDHigh is 141
    error = CGLDescribeRenderer(rendererInfo, 0, (CGLRendererProperty)141, &gpuIDHigh);
    if (error != kCGLNoError)
    {
        CGLDestroyRendererInfo(rendererInfo);
        return 0;
    ...
    CGLDestroyRendererInfo(rendererInfo);
    return (int64_t)gpuIDHigh << 32 | gpuIDLow;
}
...
void SetActiveGPUIndex(SystemInfo *info)
{
    ...
    int64_t gpuID = GetGpuIDFromDisplayID(kCGDirectMainDisplay);

    if (gpuID == 0)
        return;

    CFMutableDictionaryRef matchDictionary = IORegistryEntryIDMatching(gpuID);
    io_service_t gpu_entry = IOServiceGetMatchingService(kIOMasterPortDefault, matchDictionary);

    if (!(GetEntryProperty(gpu_entry, CFSTR("vendor-id"), &activeVendor) &&
          GetEntryProperty(gpu_entry, CFSTR("device-id"), &activeDevice)))
    {
        return;
    }
...
bool GetSystemInfo(SystemInfo *info)
{
    ...
    // Call the generic FindActiveGpu function to initialize info fields
