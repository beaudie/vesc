...
bool Display::initialize()
{
    ...
    EGLint maxTextureSize = mRenderer->getCaps().getMax2DTextureSize();
...
EGLSurface Display::createOffscreenSurface(EGLConfig config, HANDLE shareHandle, const EGLint *attribList)
{
    ...
    if (textureFormat != EGL_NO_TEXTURE && !mRenderer->getCaps().getNPOTTextureSupport() && (!gl::isPow2(width) || !gl::isPow2(height)))
    {
        return error(EGL_BAD_MATCH, EGL_NO_SURFACE);
    }
