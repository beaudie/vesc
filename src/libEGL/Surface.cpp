...
Surface::Surface(Display *display, const Config *config, EGLNativeWindowType window, EGLint fixedSize, EGLint width, EGLint height, EGLint postSubBufferSupported) 
    : mDisplay(display), mConfig(config), mHandler(window), mPostSubBufferSupported(postSubBufferSupported)
{
...
Surface::Surface(Display *display, const Config *config, HANDLE shareHandle, EGLint width, EGLint height, EGLenum textureFormat, EGLenum textureType)
    : mDisplay(display), mHandler(NULL), mConfig(config), mShareHandle(shareHandle), mWidth(width), mHeight(height), mPostSubBufferSupported(EGL_FALSE)
{
    mRenderer = mDisplay->getRenderer();
    mSwapChain = NULL;
...
bool Surface::resetSwapChain()
{
    ...
    {
        RECT windowRect;
        if (!mHandler.getClientRect(&windowRect))
        {
            ASSERT(false);
    ...
    mSwapChain = mRenderer->createSwapChain(mHandler, mShareHandle,
...
{
    return mHandler.getNativeWindow();
}
...
{
    HWND window = mHandler.getNativeWindow();
    if (!window)
    {
        return;
    }
...
bool Surface::checkForOutOfDateSwapChain()
{
    ...
    if (!mFixedSize && !mHandler.isIconic())
    {
        // The window is automatically resized to 150x22 when it's minimized, but the swapchain shouldn't be resized
        // because that's not a useful size to render to.
        if (!mHandler.getClientRect(&client))
        {
            ASSERT(false);
            return false;
        }
