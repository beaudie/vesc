...
ANGLE_EXPORT void STCompileAndResolve(
    ...
    const std::vector<sh::ShaderVariable>* uniforms = sh::GetUniforms(handle);
    res->uniformsCount = uniforms->size();
    res->pUniforms = createShaderVariableArray(*uniforms);

    const std::vector<sh::InterfaceBlock>* uniformBlocks = sh::GetUniformBlocks(handle);
    res->uniformBlocksCount = uniformBlocks->size();
    res->pUniformBlocks = createInterfaceBlockArray(*uniformBlocks);

    const std::vector<sh::InterfaceBlock>* shaderStorageBlocks = sh::GetShaderStorageBlocks(handle);
    res->shaderStorageBlocksCount = shaderStorageBlocks->size();
    res->pShaderStorageBlocks = createInterfaceBlockArray(*shaderStorageBlocks);
