...
TYPED_TEST(BitSetTest, BitwiseOperators)
{
    ...
    mBits |= kSelfValue;
    TestFixture::BitSet other(kOtherValue);
    TestFixture::BitSet anded(kSelfMaskedValue & kOtherMaskedValue);
    TestFixture::BitSet ored(kSelfMaskedValue | kOtherMaskedValue);
    TestFixture::BitSet xored(kSelfMaskedValue ^ kOtherMaskedValue);
    ...
    mBits ^= other;
    mBits ^= anded;
    EXPECT_EQ(mBits, TestFixture::BitSet(kSelfValue));

    EXPECT_EQ(mBits << kShift, TestFixture::BitSet(kSelfShiftedLeft));
    EXPECT_EQ(mBits >> kShift, TestFixture::BitSet(kSelfShiftedRight));

    mBits <<= kShift;
    EXPECT_EQ(mBits, TestFixture::BitSet(kSelfShiftedLeft));
    EXPECT_EQ(mBits.bits() & ~kMask, 0u);

    mBits = TestFixture::BitSet(kSelfValue);
    mBits >>= kShift;
    EXPECT_EQ(mBits, TestFixture::BitSet(kSelfShiftedRight));
    EXPECT_EQ(mBits.bits() & ~kMask, 0u);
...
TYPED_TEST(BitSetIteratorTest, NonLValueBitset)
{
    ...
    TestFixture::BitSet maskedBits = (mStateBits & otherBits);
