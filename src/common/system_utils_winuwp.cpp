    ...
    UwpLibrary(const char *libraryName, SearchType searchType)
    {
        switch (searchType)
        {
            case SearchType::ApplicationDir:
                mModule = LoadPackagedLibrary(libraryName, 0);
                break;
...
Library *OpenSharedLibrary(const char *libraryName, SearchType searchType)
{
    ...
    {
        return new UwpLibrary(wideBuffer.c_str(), searchType);
    }
    else
    {
        fprintf(stderr, "Error loading shared lbirary: 0x%x", ret);
        return nullptr;
    }
}
...
{
    return new UwpLibrary(libraryName, SearchType::SystemDir);
}
}  // namespace angle
