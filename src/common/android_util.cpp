...
void UnwindedBacktraceInfo::populateBacktraceInfo(void **stackAddressBuffer, size_t count)
{
    ASSERT(mStackAddresses.size() == 0 && mStackSymbols.size() == 0);
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            mStackSymbols.push_back(dlInfo.dli_sname);
        }
        else
        {
            mStackSymbols.push_back("Unknown symbol");
        }
    }
...
void UnwindedBacktraceInfo::printBacktrace()
{
    ...
    {
        WARN() << mStackAddresses[i] << " -> " << mStackSymbols[i];
    }
    WARN() << "Backtrace end";
}
...
// Used to obtain the stack addresses and symbols from the device.
UnwindedBacktraceInfo getBacktraceInfo()
{
    ...
    backtraceInfo.populateBacktraceInfo(stackAddrBuffer, count);
