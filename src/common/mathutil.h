...
{
    static const long double destLo =
        static_cast<long double>(std::numeric_limits<DestT>::lowest());
    static const long double destHi = static_cast<long double>(std::numeric_limits<DestT>::max());
    static const long double srcLo  = static_cast<long double>(std::numeric_limits<SrcT>::lowest());
    static const long double srcHi  = static_cast<long double>(std::numeric_limits<SrcT>::max());

    if (destHi < srcHi)
    {
        static DestT destMax    = std::numeric_limits<DestT>::max();
        const SrcT destMaxInSrc = static_cast<SrcT>(destMax);
        if (value > destMaxInSrc)
        {
            value = destMaxInSrc;
        }
    }
    ...
    if (destLo > srcLo)
    {
        static DestT destLow    = std::numeric_limits<DestT>::lowest();
        const SrcT destLowInSrc = static_cast<SrcT>(destLow);
        if (value < destLowInSrc)
        {
            value = destLowInSrc;
        }
    }
