...
TEST(MatrixUtilsTest, 2x2MatrixInverseTest)
{
    ...
    std::vector<float> identityInput(identityElements, identityElements + numElements);
    // A * inverse(A) = I, where I is identity matrix.
    Matrix<float> result = inputMatrix * inputMatrix.inverse();
    EXPECT_EQ(identityInput, result.elements());
}
...
TEST(MatrixUtilsTest, 3x3MatrixInverseTest)
{
    ...
    for (size_t i = 0; i < numElements; i++)
        EXPECT_NEAR(resultElements[i], identityInput[i], floatFaultTolarance);
}
