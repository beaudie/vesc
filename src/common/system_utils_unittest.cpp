...
TEST(SystemUtils, RunApp)
{
    ...
    EXPECT_NE(NormalizeNewLines(stdoutOutput).find(kRunAppTestStdout), std::string::npos);
    EXPECT_EQ(NormalizeNewLines(stderrOutput), kRunAppTestStderr);
    ...
    EXPECT_TRUE(ranApp);
    EXPECT_EQ(NormalizeNewLines(stderrOutput), kRunAppTestEnvVarValue);
    EXPECT_EQ(exitCode, EXIT_SUCCESS);
}
