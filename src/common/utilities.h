...
// A texture may be forced to skip decoding to an sRGB colorspace even if its format
// is in sRGB colorspace.
//
// Default - decode data according to the image's format's colorspace
// Skip - no decoding is performed on the data during sampling
...
//
// Default - decode data according to the image's format's colorspace
// SRGB - data will be decoded to linear colorspace even if the texture format is in linear
// colorspace
...
// A render target may be forced to not decode data to linear colorspace even if its format
// is in sRGB colorspace.
//
...
struct ColorspaceState
{
  public:
    ...
    // States that affect sampling operations
    bool hasStaticTexelFetchAccess;
    gl::SrgbDecode srgbDecode;
    gl::SrgbOverride srgbOverride;

    // States that affect rendering operations
    gl::SrgbWriteControlMode srgbWriteControl;

    // States that affect both sampling and rendering operations
