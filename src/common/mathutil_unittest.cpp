...
{
    const float input[2][8] =
    {
        0.0f, 1.0f, -1.0f, -1.0f, 0.875f, 0.00392f, -0.000675f, -0.6937f,
        0.0f, 1.0f, 1.0f, -1.0f, 0.75f, -0.99215f, 0.004954f, -0.02146f
    };
    ...
    {
        unpackSNorm2x16(packSNorm2x16(input[0][i], input[1][i]), &outputVal1, &outputVal2);
        EXPECT_NEAR(input[0][i], outputVal1, floatFaultTolerance);
        EXPECT_NEAR(input[1][i], outputVal2, floatFaultTolerance);
    }
}
...
TEST(MathUtilTest, packAndUnpackUnorm2x16Test)
{
    ...
    for (size_t i = 0; i < 8; i++)
    {
        unpackUNorm2x16(packUNorm2x16(input[0][i], input[1][i]), &outputVal1, &outputVal2);
        float expected = input[0][i] < 0.0f ? 0.0f : input[0][i];
        EXPECT_NEAR(expected, outputVal1, floatFaultTolerance);
        expected = input[1][i] < 0.0f ? 0.0f : input[1][i];
...
TEST(MathUtilTest, packAndUnpackHalf2x16Test)
{
    ...
    for (size_t i = 0; i < 8; i++)
    {
        unpackHalf2x16(packHalf2x16(input[0][i], input[1][i]), &outputVal1, &outputVal2);
        EXPECT_NEAR(input[0][i], outputVal1, floatFaultTolerance);
        EXPECT_NEAR(input[1][i], outputVal2, floatFaultTolerance);
    }
}
