...
{
    static constexpr std::bitset<N> wordMask(std::numeric_limits<unsigned long>::max());
...
template <size_t N>
using BitSetUL = BitSetT<N, unsigned long>;

template <size_t N>
using BitSetULL = BitSetT<N, unsigned long long>;
...
#if defined(ANGLE_X64_CPU)
template <size_t N>
struct GetBitSet<N, typename std::enable_if<N <= 64>::type>
{
    using Type = BitSet64<N>;
};
#else
template <size_t N>
struct GetBitSet<N, typename std::enable_if<N <= 32>::type>
{
...
template <size_t N>
struct GetBitSet<N, typename std::enable_if<N <= sizeof(unsigned long) * 8>::type>
{
    using Type = BitSetUL<N>;
};
#endif
#endif
