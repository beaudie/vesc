...
using BaseBitSetDataType              = uint64_t;
...
using BaseBitSetDataType              = uint32_t;
...
class BitSetArray final
{
  private:
    ...
    constexpr static std::size_t kLastElementCount = (N & kOneLessThanBaseBitSetSize);
    constexpr static std::size_t kLastElementMask =
        (static_cast<priv::BaseBitSetDataType>(1) << kLastElementCount) - 1;
  ...
  public:
    ...
    {
        // Fixme: When serializing BitSetArray, it no longer fits in a ulong
...
{
    static_assert(N > priv::kMinBitSetSize, "BitSetArray type can't support requested size.");
    reset();
}
...
{
    for (std::size_t index = kStartIndex; index < kArraySize - 1; index++)
    {
    ...
    return (kLastElementMask) ? (mBaseBitSetArray[kArraySize - 1].count() == kLastElementCount)
                              : mBaseBitSetArray[kArraySize - 1].all();
}
