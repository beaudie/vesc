  ...
  public:
    ...
    class Iterator final
    {
      ...
      private:
        ...
        ANGLE_INLINE void updateIterator()
        {
            std::size_t currentPos = *mCurrentIterator;
            mCurrentIterator       = mParentCopy.mBaseBitSetArray[mIndex].begin();
            while (*mCurrentIterator != currentPos)
            {
                ++mCurrentIterator;
            }
        }
      ...
      mCurrentIterator(mParent.mBaseBitSetArray[0].begin())
{
    ...
    {
        mCurrentIterator = mCurrentParent->mBaseBitSetArray[mParent.kArraySize - 1].end();
    }
}
