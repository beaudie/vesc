...
using EnableIfBitsFits = typename std::enable_if<N <= sizeof(T) * 8>::type;
...
#if defined(ANGLE_X64_CPU)
template <size_t N>
struct GetBitSet<N, EnableIfBitsFits<N, uint64_t>>
{
    using Type = BitSet64<N>;
};
#else
template <size_t N>
struct GetBitSet<N, EnableIfBitsFits<N, uint32_t>>
{
...
template <size_t N>
struct GetBitSet<N, EnableIfBitsFits<N, unsigned long>>
{
