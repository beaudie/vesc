...
class BitSetT final
{
  ...
  private:
    constexpr static BitsT BitsMask = Mask(N).bits();
...
template <size_t N, typename BitsT, typename ParamT>
constexpr BitSetT<N, BitsT, ParamT>::BitSetT(BitsT value) : mBits(value & BitsMask)
{}
...
{
    return BitSetT<N, BitsT, ParamT>(~mBits & BitsMask);
}
