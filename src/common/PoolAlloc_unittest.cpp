...
TEST(PoolAllocator, Interface)
{
    ...
    // Move construct pool allocator and allocate from it
    PoolAllocator poolAllocator2(std::move(poolAllocator));
    void *allocation2 = poolAllocator2.allocate(numBytes);
    EXPECT_NE(nullptr, allocation2);
    EXPECT_EQ(0u, (reinterpret_cast<std::uintptr_t>(allocation2) % alignment));

    // Move assign pool allocator and allocate from it
    PoolAllocator poolAllocator3 = std::move(poolAllocator);
    void *allocation3            = poolAllocator3.allocate(numBytes);
    EXPECT_NE(nullptr, allocation3);
    EXPECT_EQ(0u, (reinterpret_cast<std::uintptr_t>(allocation3) % alignment));
}
