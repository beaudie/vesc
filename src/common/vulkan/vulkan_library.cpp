...
namespace vk
{
std::unique_ptr<angle::Library> OpenLibVulkan()
{
    ...
    constexpr angle::SearchType kSearchTypes[] = {
        angle::SearchType::ModuleDir,
        angle::SearchType::SystemDir,
        angle::SearchType::Default,
    };
    ...
    for (angle::SearchType searchType : kSearchTypes)
    {
        ...
        {
            std::unique_ptr<angle::Library> library(
                angle::OpenSharedLibraryWithExtension(libraryName, searchType));
            if (library)
            {
                return library;
            }
