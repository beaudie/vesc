...
ScopedVkLoaderEnvironment::ScopedVkLoaderEnvironment(bool enableVulkanLayers, vk::ICD icd)
    : mEnableVulkanLayers(enableVulkanLayers),
      mICD(icd),
      mChangedCWD(false),
...
#    if !defined(ANGLE_PLATFORM_MACOS)
    if (mEnableVulkanLayers || icd != vk::ICD::Default)
    {
        const auto &cwd = angle::GetCWD();
        if (!cwd.valid())
        {
            ERR() << "Error getting CWD for Vulkan layers init.";
            mEnableVulkanLayers = false;
            mICD                = vk::ICD::Default;
        }
        else
        {
