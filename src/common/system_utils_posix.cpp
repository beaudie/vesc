...
std::string GetModulePath(void *moduleOrSymbol)
{
    ...
    // Chrome changes process title on Linux that causes dladdr returns wrong module file name for executable
    // binary, so return GetExecutablePath() if dli_fname doesn't exist.
    // https://source.chromium.org/chromium/chromium/src/+/main:content/common/set_process_title.cc;l=46?q=SetProcessTitle&ss=chromium
    struct stat buf;
