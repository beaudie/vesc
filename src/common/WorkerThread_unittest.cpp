...
void RunDependencyChainTest(const std::vector<std::shared_ptr<WorkerThreadPool>> &pools)
{
    ...
    constexpr size_t kMaxTaskCount   = 1000;
    ...
    struct TestContext
    {
        ...
        TestContext(const std::vector<std::shared_ptr<WorkerThreadPool>> &pools) : pools(pools) {}
                               ...
                               WaitableEvent *dependencyEvent = nullptr)
        {
            ...
            bool isLastTask  = false;
            {
                ...
                ++taskCounter;
                isLastTask    = (taskCounter == kMaxTaskCountLocal);
    ...
    ASSERT_EQ(context.events.size(), kMaxTaskCount);
    ASSERT_TRUE(context.lastTaskEvent);
...
TEST(WorkerPoolTest, DependencyHint)
{
    ...
    ASSERT_TRUE(context.lastDependentTaskEvent);
