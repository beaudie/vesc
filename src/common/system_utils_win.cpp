            ...
            int *exitCodeOut)
{
    ...
    {
        fprintf(stderr, "failed at %s:%d (waitResult: 0x%llx, last error: 0x%llx)\n", __FILE__,
                __LINE__, (unsigned long long)waitResult, (unsigned long long)GetLastError());
    }

    CloseHandle(processInfo.hProcess);
    CloseHandle(processInfo.hThread);

    fprintf(stderr, "RunApp result: %d\n", success);
    return success;
}
