...
1. If newSize <= oldSize, reduce the size of the allocation (and move the post-guard ahead, if
guarding)
2. If the allocation being resized is at the end of the pool, just increase its size (and push the
post-guard back, if guarding)
3. Otherwise make a new allocation and copy the old contents over.

... except it looks like the size of the allocation is not stored in the allocation blocks if not
adding guards.
 */
