...
PoolAllocator::PoolAllocator(int growthIncrement, int allocationAlignment)
    : alignment(allocationAlignment),
      ...
      mLocked(false)
{
    ERR() << "PoolAllocator(" << this << ") regular constructor";
    //
 ...
 */
{
    ERR() << "PoolAllocator move constructor";
    *this = std::move(rhs);
}

PoolAllocator &PoolAllocator::operator=(PoolAllocator &&rhs)
{
    ERR() << "PoolAllocator(" << this << ")  move assignment";
    if (this != &rhs)
    {
        alignment     = std::exchange(rhs.alignment, 0);
        alignmentMask = std::exchange(rhs.alignmentMask, 0);
...
{
    ERR() << "PoolAllocator(" << this << ")  destructor";
#if !defined(ANGLE_DISABLE_POOL_ALLOC)
    while (inUseList)
    {
...
#ifdef GUARD_BLOCKS
const size_t Allocation::guardBlockSize = 16;
#else
const size_t Allocation::guardBlockSize = 0;
#endif
...
{
    ERR() << "PoolAllocator(" << this << ")  allocate";
    ASSERT(!mLocked);
    ...
    //
    ERR() << "PoolAllocator allocate3, allocationSize:" << allocationSize
          << ", pageSize:" << pageSize << ", currentPageOffset:" << currentPageOffset;
    if (allocationSize <= pageSize - currentPageOffset)
    {
        ...
        ERR() << "PoolAllocator allocate4";
        return initializeAllocation(inUseList, memory, numBytes);
    }

    ERR() << "PoolAllocator allocate5";
    if (allocationSize > pageSize - headerSkip)
    {
        ...
        // Detect integer overflow.
        ERR() << "PoolAllocator allocate6";
        if (numBytesToAlloc < allocationSize)
            return 0;

        ERR() << "PoolAllocator allocate7";
        tHeader *memory = reinterpret_cast<tHeader *>(::new char[numBytesToAlloc]);
        if (memory == 0)
            return 0;

        ERR() << "PoolAllocator allocate8 w/ pageSize: " << pageSize;
