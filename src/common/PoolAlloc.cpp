...
class Allocation
{
  ...
  private:
    ...
    size_t mSize;            // size of the user data area; TODO remove if !GUARD
...
{
  public:
    AllocationHeader(AllocationHeader *nextPage, size_t pageCount)
        : nextPage(nextPage),
          pageCount(pageCount)
#    if defined(ANGLE_POOL_ALLOC_GUARD_BLOCKS)
          ,
...
void PoolAllocator::initialize(int pageSize, int alignment)
{
...
#if !defined(ANGLE_DISABLE_POOL_ALLOC)
    mPageSize   = pageSize;
    mHeaderSkip = sizeof(AllocationHeader);
    ...
    if (mAlignment != 1)
    {
        ...
        // TODO: this doesn't do anything, remove
...
PoolAllocator::~PoolAllocator()
{
    ...
    while (mInUseList)
    {
        AllocationHeader *next = mInUseList->nextPage;
        mInUseList->~AllocationHeader();
    ...
    while (mFreeList)
    {
        AllocationHeader *next = mFreeList->nextPage;
...
void PoolAllocator::pop()
{
...
#if !defined(ANGLE_DISABLE_POOL_ALLOC)
    AllocationHeader *page = mStack.back().page;
    ...
    while (mInUseList != page)
    {
        ...
        AllocationHeader *nextInUse = mInUseList->nextPage;
...
void *PoolAllocator::allocate(size_t numBytes)
{
    ...
    if (allocationSize > mPageSize - mHeaderSkip)
    {
        ...
        allocationSize = Allocation::AllocationSize(reinterpret_cast<uint8_t *>(mHeaderSkip),
                                                    numBytes, mAlignment, &preAllocationPadding);

        size_t numBytesToAlloc = allocationSize + mHeaderSkip + mAlignment;

        // Integer overflow is unexpected.
        ASSERT(numBytesToAlloc >= allocationSize);

        AllocationHeader *memory =
            reinterpret_cast<AllocationHeader *>(::new char[numBytesToAlloc]);
        if (memory == nullptr)
        {
            return nullptr;
        }
        ...
        new (memory) AllocationHeader(mInUseList, (numBytesToAlloc + mPageSize - 1) / mPageSize);
        mInUseList = memory;
        ...
        currentPagePtr = reinterpret_cast<uint8_t *>(memory) + mHeaderSkip;
        Allocation::AllocationSize(currentPagePtr, numBytes, mAlignment, &preAllocationPadding);
...
uint8_t *PoolAllocator::allocateNewPage(size_t numBytes)
{
    ...
    {
        memory = reinterpret_cast<AllocationHeader *>(::new char[mPageSize]);
        if (memory == nullptr)
        {
            return nullptr;
        }
    ...
    // Use placement-new to initialize header
    new (memory) AllocationHeader(mInUseList, 1);
    mInUseList = memory;

    // Leave room for the page header.
    mCurrentPageOffset      = mHeaderSkip;
    uint8_t *currentPagePtr = reinterpret_cast<uint8_t *>(mInUseList) + mCurrentPageOffset;
    ...
    return reinterpret_cast<uint8_t *>(mInUseList) + mHeaderSkip + preAllocationPadding;
}
