...
RingBufferCheckPoint RingBufferAllocator::getReleaseCheckPoint() const
{
    ASSERT(valid());
    RingBufferCheckPoint result;
    result.mBuffferId  = mBuffer.getId();
    result.mReleasePtr = mDataEnd;
    return result;
}

void RingBufferAllocator::release(const RingBufferCheckPoint &checkPoint)
{
#if ANGLE_RING_BUFFER_ALLOCATOR_DEBUG
    INFO() << "RingBufferAllocator[" << debugName << "] mOldBuffers.size(): " << mOldBuffers.size()
    ...
    if (mOldBuffers.size() > 0)
    {
        ...
        int removeCount = 0;
        for (uint32_t i = 0;
             (i < mOldBuffers.size()) && (mOldBuffers[i].getId() < checkPoint.mBuffferId); ++i)
        {
            ++removeCount;
        }
    ...
    if (checkPoint.mBuffferId == mBuffer.getId())
    {
...
uint8_t *RingBufferAllocator::allocateInNewFragment(uint32_t size)
{
    ...
    INFO() << "RingBufferAllocator[" << debugName << "] mOldBuffers.size(): " << mOldBuffers.size()
           << "; mCurrentCapacity: " << mCurrentCapacity << "; NumAllocated: " << numAllocated
...
SharedRingBufferCheckPoint *SharedRingBufferAllocator::acquireSharedCP()
{
    if (!mSharedCP)
    {
        mSharedCP = new SharedRingBufferCheckPoint();
    }
#if defined(ANGLE_ENABLE_ASSERTS)
...
void SharedRingBufferCheckPoint::releaseAndUpdate(RingBufferCheckPoint *newValue)
{
...
RingBufferCheckPoint SharedRingBufferCheckPoint::pop()
{
    std::lock_guard<std::mutex> lock(mMutex);
    RingBufferCheckPoint value = mValue;
    mValue.reset();
    return value;
}
