...
constexpr BlendEquationType FromGLenum<BlendEquationType>(GLenum from)
{
    ...
    if (from >= GL_MULTIPLY_KHR && from <= GL_EXCLUSION_KHR)
    {
        const GLenum scaled =
            (from - GL_MULTIPLY_KHR + static_cast<uint32_t>(BlendEquationType::Multiply));
        return (scaled == static_cast<GLenum>(BlendEquationType::UnusedTwo) ||
                scaled == static_cast<GLenum>(BlendEquationType::UnusedThree))
                   ? BlendEquationType::InvalidEnum
                   : static_cast<BlendEquationType>(scaled);
    }
    if (from >= GL_HSL_HUE_KHR && from <= GL_HSL_LUMINOSITY_KHR)
    {
...
constexpr GLenum ToGLenum(BlendEquationType from)
{
    ...
    if (from >= BlendEquationType::Multiply && from <= BlendEquationType::Exclusion)
        return static_cast<GLenum>(from) - 6 + GL_MULTIPLY_KHR;
    return static_cast<GLenum>(from) - 19 + GL_HSL_HUE_KHR;
}
