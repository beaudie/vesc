...
constexpr DrawElementsType FromGLenum<DrawElementsType>(GLenum from)
{
    ...
    // This code sequence generates a ROR instruction on x86/arm. We want to check if the
    // lowest bit of scaled is set and if (scaled >> 1) is greater than a non-pot value.
    // If we rotate the lowest bit to the hightest bit both conditions can be checked
    // with a single test.
