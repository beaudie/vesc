...
inline size_t ComputeGenericHash(const void *key, size_t keySize, size_t seed)
{
    // We can't support "odd" alignments.  ComputeGenericHash requires aligned types
    ASSERT(keySize % 4 == 0);
#if defined(ANGLE_IS_64_BIT_CPU)
    return XXH64(key, keySize, seed);
#else
    return XXH32(key, keySize, seed);
...
inline size_t ComputeGenericHash(const void *key, size_t keySize)
{
    ...
    return ComputeGenericHash(key, keySize, kSeed);
}
