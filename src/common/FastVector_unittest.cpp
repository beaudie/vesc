...
// Large key values functionality for FastAndSparseIntegerMap
TEST(FastIntegerMap, LargeKeysSparse)
{
    using KeyValuePair             = std::pair<int, std::string>;
    std::set<KeyValuePair> entries = {KeyValuePair(17, "testing"), KeyValuePair(63555, "fast"),
                                      KeyValuePair(9787, "integer"), KeyValuePair(25625, "map")};

    FastAndSparseIntegerMap<std::string> testMap;
    EXPECT_TRUE(testMap.empty());
