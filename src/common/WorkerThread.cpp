...
class WorkerThreadLoopPolicy
{
  public:
    ...
    // false and |result.second| may be empty or |lock| argument (locked or unlocked).  Allowed to
    // wait on the conditional variable using the passed lock.
    ...
    while (true)
    {
        std::shared_ptr<AsyncTask> task;
        {
            const auto [hasNextTaskToRun, lock] =
                policy->ensureHasNextTaskToRunLocked(std::unique_lock<std::mutex>(mMutex));
            if (!hasNextTaskToRun)
            {
                return;
            }
