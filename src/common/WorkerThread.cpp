...
class WorkerThreadLoopPolicy
{
  public:
    ...
    // the conditional variable using the passed lock.  When returned false, the lock may (or may
    // not) be unlocked after the call.
...
class MultiThreadedWorkerPool : public AsyncTaskWorkerPool
{
  ...
  protected:
    ...
    // Creates new or wakes existing thread.  The lock may (or may not) be unlocked after the call.
