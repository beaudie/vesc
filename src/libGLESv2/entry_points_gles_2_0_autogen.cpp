                                      ...
                                      GLint level)
{
    ...
    {
        TextureTarget targetPacked = FromGLenum<TextureTarget>(target);
        context->gatherParams<EntryPoint::FramebufferTexture2D>(targetPacked, attachment, textarget,
                                                                texture, level);

        if (context->skipValidation() ||
            ValidateFramebufferTexture2D(context, targetPacked, attachment, textarget, texture,
                                         level))
        {
            context->framebufferTexture2D(targetPacked, attachment, textarget, texture, level);
        }
    }
}
