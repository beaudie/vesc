...
Error Program::link(const Caps &caps)
{
    ...
    rx::LinkResult result = mProgram->link(mInfoLog, mFragmentShader, mVertexShader, mTransformFeedbackVaryings, mTransformFeedbackBufferMode,
    ...
    if (!mProgram->linkUniforms(mInfoLog, *mVertexShader, *mFragmentShader, caps))
    {
        return Error(GL_NO_ERROR);
    }

    if (!linkUniformBlocks(mInfoLog, *mVertexShader, *mFragmentShader, caps))
    {
        return Error(GL_NO_ERROR);
    }

    if (!gatherTransformFeedbackLinkedVaryings(mInfoLog, linkedVaryings, mTransformFeedbackVaryings,
                                               mTransformFeedbackBufferMode, &mProgram->getTransformFeedbackLinkedVaryings(), caps))
    {
        return Error(GL_NO_ERROR);
    }
