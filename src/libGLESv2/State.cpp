...
{
    ASSERT((size_t)index < mUniformBuffers.size());
...
{
    ASSERT((size_t)index < mTransformFeedbackBuffers.size());
...
{
    ASSERT((size_t)index < mVertexAttribCurrentValues.size());
...
{
    ASSERT((size_t)attribNum < mVertexAttribCurrentValues.size());
...
bool State::getIndexedIntegerv(GLenum target, GLuint index, GLint *data)
{
    ...
    {
      case GL_TRANSFORM_FEEDBACK_BUFFER_BINDING:
          if ((size_t)index < mTransformFeedbackBuffers.size())
        {
            *data = mTransformFeedbackBuffers[index].id();
        }
        break;
      case GL_UNIFORM_BUFFER_BINDING:
        if ((size_t)index < mUniformBuffers.size())
        {
