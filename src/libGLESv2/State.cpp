...
State::State()
{
    ...
    mBoundReadFramebuffer = NULL;
    mBoundDrawFramebuffer = NULL;
}
...
void State::setCap(GLenum cap, bool enabled)
{
    switch (cap)
    {
...
ClearParameters State::getClearParameters(GLbitfield mask) const
{
    ...
    {
        if (mDepthStencil.depthMask && framebufferObject->getDepthbufferType() != GL_NONE)
        {
...
GLuint State::getReadFramebufferHandle() const
{
    return mReadFramebufferId;
}

GLuint State::getDrawFramebufferHandle() const
{
    return mDrawFramebufferId;
}

GLuint State::getRenderbufferHandle() const
{
...
GLuint State::getVertexArrayHandle() const
{
    return mVertexArrayId;
}

void State::setVertexArrayBinding(GLuint handle, VertexArray *vertexArray)
{
    mVertexArrayId = handle;
    mBoundVertexArray = vertexArray;
}

GLuint State::getSamplerHandle(GLuint textureUnit) const
{
...
GLuint State::getArrayBufferHandle() const
{
...
void State::setDrawFramebufferBinding(GLuint handle, Framebuffer *framebuffer)
{
    mDrawFramebufferId = handle;
    mBoundDrawFramebuffer = framebuffer;
}

void State::setReadFramebufferBinding(GLuint handle, Framebuffer *framebuffer)
{
    mReadFramebufferId = handle;
    mBoundReadFramebuffer = framebuffer;
}
...
GLuint State::getUniformBufferHandle(GLuint index) const
{
...
Buffer *State::getUniformBuffer(GLuint index) const
{
    ASSERT(index < IMPLEMENTATION_MAX_COMBINED_SHADER_UNIFORM_BUFFERS);
...
GLuint State::getTransformFeedbackBufferHandle(GLuint index) const
{
...
Buffer *State::getTransformFeedbackBuffer(GLuint index) const
{
    ASSERT(index < IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_BUFFERS);
...
GLuint State::getTransformFeedbackBufferOffset(GLuint index) const
{
    ASSERT(index < IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_BUFFERS);
...
VertexArray *State::getCurrentVertexArray() const
{
    ASSERT(mBoundVertexArray != NULL);
...
GLuint State::getTargetFramebufferHandle(GLenum target) const
{
...
void State::getIntegerv(GLenum pname, GLint *params)
{
    if (pname >= GL_DRAW_BUFFER0_EXT && pname <= GL_DRAW_BUFFER15_EXT)
    {
        ...
        ASSERT(colorAttachment < mContext->getMaximumRenderTargets());
        Framebuffer *framebuffer = mBoundDrawFramebuffer;
    ...
    case GL_DRAW_FRAMEBUFFER_BINDING_ANGLE:           *params = mDrawFramebufferId;                             break;
    case GL_READ_FRAMEBUFFER_BINDING_ANGLE:           *params = mReadFramebufferId;                             break;
    case GL_RENDERBUFFER_BINDING:                     *params = mRenderbuffer.id();                             break;
    case GL_VERTEX_ARRAY_BINDING:                     *params = mVertexArrayId;                                 break;
    ...
    case GL_SAMPLES:
    {
        gl::Framebuffer *framebuffer = mBoundDrawFramebuffer;
...
void State::detachTexture(GLuint texture)
{
    ...
    {
        mBoundReadFramebuffer->detachTexture(texture);
    }

    if (mBoundDrawFramebuffer)
    {
        mBoundDrawFramebuffer->detachTexture(texture);
    }
}

bool State::removeBufferBinding(GLuint buffer)
{
...
{
    if (mReadFramebufferId == framebuffer)
    {
        mReadFramebufferId = 0;
        mBoundReadFramebuffer = NULL;
        return true;
    }
...
{
    if (mDrawFramebufferId == framebuffer)
    {
        mDrawFramebufferId = 0;
        mBoundDrawFramebuffer = NULL;
        return true;
    }
...
bool State::removeRenderbufferBinding(GLuint renderbuffer)
{
...
bool State::removeVertexArrayBinding(GLuint vertexArray)
{
    ...
    if (mVertexArrayId == vertexArray)
    {
        mVertexArrayId = 0;
        mBoundVertexArray = NULL;
        return true;
    }
