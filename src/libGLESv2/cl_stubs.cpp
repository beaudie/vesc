...
{
    if (IsError(Platform::GetPlatformIDs(num_entries, platforms, num_platforms)))
    {
...
{
    if (IsError(Platform::CastOrDefault(platform)->getInfo(param_name, param_value_size,
...
cl_int GetDeviceIDs(cl_platform_id platform,
                    ...
                    cl_uint *num_devices)
{
    if (IsError(Platform::CastOrDefault(platform)->getDeviceIDs(device_type, num_entries, devices,
                                                                num_devices)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetDeviceInfo(cl_device_id device,
                     ...
                     size_t *param_value_size_ret)
{
    if (IsError(device->cast<Device>().getInfo(param_name, param_value_size, param_value,
...
{
    if (IsError(in_device->cast<Device>().createSubDevices(properties, num_devices, out_devices,
                                                           num_devices_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetContextInfo(cl_context context,
                      ...
                      size_t *param_value_size_ret)
{
    if (IsError(context->cast<Context>().getInfo(param_name, param_value_size, param_value,
                                                 param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetCommandQueueInfo(cl_command_queue command_queue,
                           ...
                           size_t *param_value_size_ret)
{
    if (IsError(command_queue->cast<CommandQueue>().getInfo(param_name, param_value_size,
                                                            param_value, param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError(context->cast<Context>().getSupportedImageFormats(
...
cl_int GetMemObjectInfo(cl_mem memobj,
                        ...
                        size_t *param_value_size_ret)
{
    if (IsError(memobj->cast<Memory>().getInfo(param_name, param_value_size, param_value,
                                               param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetImageInfo(cl_mem image,
                    ...
                    size_t *param_value_size_ret)
{
    if (IsError(image->cast<Image>().getInfo(param_name, param_value_size, param_value,
                                             param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int SetMemObjectDestructorCallback(cl_mem memobj,
                                      ...
                                      void *user_data)
{
    if (IsError(memobj->cast<Memory>().setDestructorCallback(pfn_notify, user_data)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int GetSamplerInfo(cl_sampler sampler,
                      ...
                      size_t *param_value_size_ret)
{
    if (IsError(sampler->cast<Sampler>().getInfo(param_name, param_value_size, param_value,
                                                 param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int BuildProgram(cl_program program,
                    ...
                    void *user_data)
{
    if (IsError(program->cast<Program>().build(num_devices, device_list, options, pfn_notify,
                                               user_data)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int CompileProgram(cl_program program,
                      ...
                      void *user_data)
{
    if (IsError(program->cast<Program>().compile(num_devices, device_list, options,
...
{
    if (IsError(platform->cast<Platform>().unloadCompiler()))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int GetProgramInfo(cl_program program,
                      ...
                      size_t *param_value_size_ret)
{
    if (IsError(program->cast<Program>().getInfo(param_name, param_value_size, param_value,
                                                 param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetProgramBuildInfo(cl_program program,
                           ...
                           size_t *param_value_size_ret)
{
    if (IsError(program->cast<Program>().getBuildInfo(device, param_name, param_value_size,
                                                      param_value, param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError(program->cast<Program>().createKernels(num_kernels, kernels, num_kernels_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
{
    if (IsError(kernel->cast<Kernel>().setArg(arg_index, arg_size, arg_value)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int GetKernelInfo(cl_kernel kernel,
                     ...
                     size_t *param_value_size_ret)
{
    if (IsError(kernel->cast<Kernel>().getInfo(param_name, param_value_size, param_value,
                                               param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetKernelArgInfo(cl_kernel kernel,
                        ...
                        size_t *param_value_size_ret)
{
    if (IsError(kernel->cast<Kernel>().getArgInfo(arg_index, param_name, param_value_size,
                                                  param_value, param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int GetKernelWorkGroupInfo(cl_kernel kernel,
                              ...
                              size_t *param_value_size_ret)
{
    if (IsError(kernel->cast<Kernel>().getWorkGroupInfo(device, param_name, param_value_size,
                                                        param_value, param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError((*event_list)->cast<Event>().getContext().waitForEvents(num_events, event_list)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int GetEventInfo(cl_event event,
                    ...
                    size_t *param_value_size_ret)
{
    if (IsError(event->cast<Event>().getInfo(param_name, param_value_size, param_value,
                                             param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError(event->cast<Event>().setUserEventStatus(execution_status)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int GetEventProfilingInfo(cl_event event,
                             ...
                             size_t *param_value_size_ret)
{
    if (IsError(event->cast<Event>().getProfilingInfo(param_name, param_value_size, param_value,
                                                      param_value_size_ret)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError(command_queue->cast<CommandQueue>().flush()))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
{
    if (IsError(command_queue->cast<CommandQueue>().finish()))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int EnqueueReadBuffer(cl_command_queue command_queue,
                         ...
                         cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueReadBuffer(
            buffer, blocking_read, offset, size, ptr, num_events_in_wait_list, event_wait_list,
            event)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int EnqueueReadBufferRect(cl_command_queue command_queue,
                             ...
                             cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueReadBufferRect(
...
cl_int EnqueueWriteBuffer(cl_command_queue command_queue,
                          ...
                          cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueWriteBuffer(
...
cl_int EnqueueWriteBufferRect(cl_command_queue command_queue,
                              ...
                              cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueWriteBufferRect(
...
cl_int EnqueueFillBuffer(cl_command_queue command_queue,
                         ...
                         cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueFillBuffer(
...
cl_int EnqueueCopyBuffer(cl_command_queue command_queue,
                         ...
                         cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueCopyBuffer(
...
cl_int EnqueueCopyBufferRect(cl_command_queue command_queue,
                             ...
                             cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueCopyBufferRect(
...
cl_int EnqueueReadImage(cl_command_queue command_queue,
                        ...
                        cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueReadImage(
            image, blocking_read, origin, region, row_pitch, slice_pitch, ptr,
...
cl_int EnqueueWriteImage(cl_command_queue command_queue,
                         ...
                         cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueWriteImage(
...
cl_int EnqueueFillImage(cl_command_queue command_queue,
                        ...
                        cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueFillImage(
...
cl_int EnqueueCopyImage(cl_command_queue command_queue,
                        ...
                        cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueCopyImage(
...
cl_int EnqueueCopyImageToBuffer(cl_command_queue command_queue,
                                ...
                                cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueCopyImageToBuffer(
...
cl_int EnqueueCopyBufferToImage(cl_command_queue command_queue,
                                ...
                                cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueCopyBufferToImage(
...
cl_int EnqueueUnmapMemObject(cl_command_queue command_queue,
                             ...
                             cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueUnmapMemObject(
...
cl_int EnqueueMigrateMemObjects(cl_command_queue command_queue,
                                ...
                                cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueMigrateMemObjects(
...
cl_int EnqueueNDRangeKernel(cl_command_queue command_queue,
                            ...
                            cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueNDRangeKernel(
...
cl_int EnqueueNativeKernel(cl_command_queue command_queue,
                           ...
                           cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueNativeKernel(
...
cl_int EnqueueMarkerWithWaitList(cl_command_queue command_queue,
                                 ...
                                 cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueMarkerWithWaitList(
            num_events_in_wait_list, event_wait_list, event)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
cl_int EnqueueBarrierWithWaitList(cl_command_queue command_queue,
                                  ...
                                  cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueBarrierWithWaitList(
            num_events_in_wait_list, event_wait_list, event)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
...
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueMarker(event)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueWaitForEvents(num_events, event_list)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueBarrier()))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int UnloadCompiler()
{
    ...
    if (platform != nullptr && IsError(platform->unloadCompiler()))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
    return cl::gClErrorTls;
...
cl_int EnqueueTask(cl_command_queue command_queue,
                   ...
                   cl_event *event)
{
    if (IsError(command_queue->cast<CommandQueue>().enqueueTask(kernel, num_events_in_wait_list,
                                                                event_wait_list, event)))
    {
        ERR() << "failed with error code: " << cl::gClErrorTls;
    }
