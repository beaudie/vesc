...
bool FindProc(const ProcEntry a, const ProcEntry &b)
{
...
__eglMustCastToProperFunctionPointerType EGLAPIENTRY GetProcAddress(const char *procname)
{
    ...
    ProcEntry *entry = std::lower_bound(&g_procTable[0], &g_procTable[g_numProcs],
                                        std::make_pair(procname, nullptr), FindProc);

    thread->setError(NoError());

    if (entry == &g_procTable[g_numProcs])
    {
        return nullptr;
    }
