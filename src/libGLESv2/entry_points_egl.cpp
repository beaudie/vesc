...
using DataEntry = std::pair<const uint8_t *, EGLsizeiANDROID>;
...
class AngleBlob
{
  public:
    ...
    std::map<DataBlob, DataEntry> cache;
};
...
void setBlob(const void *key, EGLsizeiANDROID keySize, const void *value, EGLsizeiANDROID valueSize)
{
    ...
    GetAngleBlob()->cache[keyBlob] = DataEntry(reinterpret_cast<const uint8_t *>(value), valueSize);
}
...
{
    DataBlob keyBlob(reinterpret_cast<const uint8_t *>(key), keySize);
    std::map<DataBlob, DataEntry>::iterator it = GetAngleBlob()->cache.find(keyBlob);
    if (it != GetAngleBlob()->cache.end())
    {
        if (it->second.second <= valueSize)
        {
            memcpy(value, it->second.first, it->second.second);
        }
