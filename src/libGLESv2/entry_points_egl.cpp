...
EGLBoolean EGLAPIENTRY MakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    if (error.isError())
    {
        thread->setError(error, GetDebug(), "eglMakeCurrent", GetContextIfValid(display, context));
        return EGL_FALSE;
    }
    ...
    {
        thread->setError(makeCurrentError, GetDebug(), "eglMakeCurrent",
                         GetContextIfValid(display, context));
        return EGL_FALSE;
    }
    ...
    if (previousContext != nullptr && context != previousContext)
    {
        ...
        if (err.isError())
        {
            thread->setError(err, GetDebug(), "eglMakeCurrent",
                             GetContextIfValid(display, context));
            return EGL_FALSE;
        }
    }
