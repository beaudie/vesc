...
EGLBoolean EGLAPIENTRY WaitGL(void)
{
    ...
    // eglWaitGL like calling eglWaitClient with the OpenGL ES API bound. Since we only implement OpenGL ES we can do the call directly.
...
EGLBoolean EGLAPIENTRY WaitNative(EGLint engine)
{
    ...
    {
        SetGlobalError(Error(EGL_BAD_PARAMETER, "the 'engine' parameter has an unrecognized value"));
    }

    error = display->waitNative();
    if (error.isError())
    {
        SetGlobalError(error);
        return EGL_FALSE;
