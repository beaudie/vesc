...
{
    EVENT("(EGLDisplay dpy = 0x%p, EGLint *major = 0x%p, EGLint *minor = 0x%p)", dpy,
          major, minor);
    Thread *thread = GetCurrentThread();
...
{
    EVENT("(EGLDisplay dpy = 0x%p, EGLSurface surface = 0x%p, EGLint buffer = %d)", dpy,
          surface, buffer);
    Thread *thread = GetCurrentThread();
...
{
    EVENT("(EGLDisplay dpy = 0x%p, EGLenum type = 0x%X, const EGLint* attrib_list = 0x%p)",
...
{
    EVENT("(EGLDisplay dpy = 0x%p, EGLSync sync = 0x%p, EGLint flags = 0x%X)", dpy, sync,
          flags);
    Thread *thread = GetCurrentThread();
