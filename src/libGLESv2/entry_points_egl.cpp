...
EGLBoolean EGLAPIENTRY MakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    {
        Error makeUnCurrentError =
            display->makeUnCurrent(previousDraw, previousRead, previousContext);
        if (makeUnCurrentError.isError())
        {
            thread->setError(makeUnCurrentError);
            return EGL_FALSE;
        }
