...
EGLBoolean EGLAPIENTRY MakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    if (readSurface)
    {
        Error readCompatError = ValidateConfigAreCompatible(readSurface->getConfig(), context->getConfig(), readSurface->getType());
    ...
    if (draw != read)
    {
        ...
        if (drawSurface)
        {
            Error drawCompatError = ValidateConfigAreCompatible(drawSurface->getConfig(), context->getConfig(), drawSurface->getType());
