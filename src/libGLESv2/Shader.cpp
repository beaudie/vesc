...
const std::vector<gl::Uniform> &Shader::getUniforms() const
{
    return mActiveUniforms;
}

const std::vector<gl::InterfaceBlock> &Shader::getInterfaceBlocks() const
{
    return mActiveInterfaceBlocks;
}

std::vector<gl::Varying> &Shader::getVaryings()
{
    return mVaryings;
}
...
void Shader::parseVaryings(void *compiler)
{
    if (!mHlsl.empty())
    {
        std::vector<gl::Varying> *activeVaryings;
...
void Shader::compileToHLSL(void *compiler)
{
    ...
    else if (result)
    {
        ...
        mActiveUniforms = *(std::vector<gl::Uniform>*)activeUniforms;

        void *activeInterfaceBlocks;
        ShGetInfoPointer(compiler, SH_ACTIVE_INTERFACE_BLOCKS_ARRAY, &activeInterfaceBlocks);
        mActiveInterfaceBlocks = *(std::vector<gl::InterfaceBlock>*)activeInterfaceBlocks;
    }
    else
    {
        size_t infoLogLen = 0;
...
bool Shader::compareVarying(const gl::ShaderVariable &x, const gl::ShaderVariable &y)
{
    if (x.type == y.type)
    {
...
int VertexShader::getSemanticIndex(const std::string &attributeName)
{
    if (!attributeName.empty())
    {
        ...
        {
            const gl::ShaderVariable &attribute = mActiveAttributes[attributeIndex];
...
void VertexShader::parseAttributes()
{
    ...
    if (!hlsl.empty())
    {
        ...
        mActiveAttributes = *(std::vector<gl::Attribute>*)activeAttributes;
    }
}
...
void FragmentShader::compile()
{
    ...
    if (!hlsl.empty())
    {
        ...
        mActiveOutputVariables = *(std::vector<gl::Attribute>*)activeOutputVariables;
    }
}
...
const std::vector<gl::Attribute> &FragmentShader::getOutputVariables() const
{
