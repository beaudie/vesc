...
bool Context::applyTransformFeedbackBuffers()
{
    ...
    {
        std::vector<Buffer*> transformFeedbackBuffers;
        std::vector<GLintptr> transformFeedbackOffsets;
        for (size_t i = 0; i < mCaps.maxTransformFeedbackSeparateAttributes; i++)
        {
            transformFeedbackBuffers.push_back(mState.getIndexedTransformFeedbackBuffer(i));
            transformFeedbackOffsets.push_back(mState.getIndexedTransformFeedbackBufferOffset(i));
        }
