...
void Context::bindElementArrayBuffer(unsigned int buffer)
{
    ...
    mState.getCurrentVertexArray()->setElementArrayBuffer(getBuffer(buffer));
}
...
void Context::bindReadFramebuffer(GLuint framebuffer)
{
    ...
    mState.setReadFramebufferBinding(framebuffer, getFramebuffer(framebuffer));
}
...
void Context::bindDrawFramebuffer(GLuint framebuffer)
{
    ...
    mState.setDrawFramebufferBinding(framebuffer, getFramebuffer(framebuffer));
}
...
void Context::bindVertexArray(GLuint vertexArray)
{
    ...
    mState.setVertexArrayBinding(vertexArray, getVertexArray(vertexArray));
}
...
void Context::setFramebufferZero(Framebuffer *buffer)
{
    ...
    if (mState.getDrawFramebufferHandle() == 0)
    {
        mState.setDrawFramebufferBinding(0, buffer);
    }

    if (mState.getReadFramebufferHandle() == 0)
    {
        mState.setReadFramebufferBinding(0, buffer);
    }
}
...
{
    return mState.getTargetBuffer(GL_ARRAY_BUFFER);
}

Buffer *Context::getElementArrayBuffer() const
{
    return mState.getTargetBuffer(GL_ELEMENT_ARRAY_BUFFER);
}
...
{
    return mState.getTargetBuffer(GL_UNIFORM_BUFFER);
}

Buffer *Context::getGenericTransformFeedbackBuffer()
{
    return mState.getTargetBuffer(GL_TRANSFORM_FEEDBACK_BUFFER);
}

Buffer *Context::getCopyReadBuffer()
{
    return mState.getTargetBuffer(GL_COPY_READ_BUFFER);
}

Buffer *Context::getCopyWriteBuffer()
{
    return mState.getTargetBuffer(GL_COPY_WRITE_BUFFER);
}

Buffer *Context::getPixelPackBuffer()
{
    return mState.getTargetBuffer(GL_PIXEL_PACK_BUFFER);
}

Buffer *Context::getPixelUnpackBuffer()
{
    return mState.getTargetBuffer(GL_PIXEL_UNPACK_BUFFER);
}
...
{
    const VertexAttribute *vertexAttributes = mState.getCurrentVertexArray()->getVertexAttributes();
...
bool Context::applyUniformBuffers()
{
    ...
    for (unsigned int uniformBlockIndex = 0; uniformBlockIndex < programBinary->getActiveUniformBlockCount(); uniformBlockIndex++)
    {
        ...
        if (mState.getUniformBufferHandle(blockBinding) == 0)
        {
            // undefined behaviour
            return false;
        }
        ...
        {
            Buffer *uniformBuffer = mState.getUniformBuffer(blockBinding);
...
bool Context::applyTransformFeedbackBuffers()
{
    ...
    if (curTransformFeedback && curTransformFeedback->isStarted() && !curTransformFeedback->isPaused())
    {
        ...
        {
            transformFeedbackBuffers[i] = mState.getTransformFeedbackBuffer(i);
            transformFeedbackOffsets[i] = mState.getTransformFeedbackBufferOffset(i);
        }
...
void Context::markTransformFeedbackUsage()
{
    for (size_t i = 0; i < IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_BUFFERS; i++)
    {
        Buffer *buffer = mState.getTransformFeedbackBuffer(i);
        if (buffer)
        {
...
{
    if (mState.getCurrentProgramId() == 0)
    {
    ...
    GLenum err = mRenderer->applyVertexBuffer(programBinary, mState.getCurrentVertexArray()->getVertexAttributes(), mState.getVertexAttribCurrentValues(), first, count, instances);
    if (err != GL_NO_ERROR)
    {
        return gl::error(err);
    }
...
void Context::drawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices, GLsizei instances)
{
    ...
    VertexArray *vao = mState.getCurrentVertexArray();
...
void Context::detachBuffer(GLuint buffer)
{
    ...
    mState.removeBufferBinding(buffer);
...
void Context::detachRenderbuffer(GLuint renderbuffer)
{
    ...
    if (mState.removeRenderbufferBinding(renderbuffer))
    {
        bindRenderbuffer(0);
    }
...
void Context::detachSampler(GLuint sampler)
{
    ...
    // SKW TODO: May need to be managed, not simple. If not, elaborate on the comment.
...
{
    mState.getCurrentVertexArray()->setVertexAttribDivisor(index, divisor);
}
