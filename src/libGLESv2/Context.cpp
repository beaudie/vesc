...
void Context::makeCurrent(egl::Surface *surface)
{
    if (!mHasBeenCurrent)
    {
        ...
        mNumCompressedTextureFormats = 0;
        if (getCaps().getDXT1TextureSupport())
        {
            mNumCompressedTextureFormats += 2;
        }
        if (getCaps().getDXT3TextureSupport())
        {
            mNumCompressedTextureFormats += 1;
        }
        if (getCaps().getDXT5TextureSupport())
        {
            mNumCompressedTextureFormats += 1;
        }
...
{
    const TextureCaps &formatCaps = getCaps().getTextureFormatCaps(internalformat);
    ...
    else if (formatCaps.getDepthRenderingSupport() && formatCaps.getStencilRenderingSupport())
    {
        renderbuffer = new gl::DepthStencilbuffer(mRenderer, width, height, samples);
    }
    else if (formatCaps.getDepthRenderingSupport())
    {
        renderbuffer = new gl::Depthbuffer(mRenderer, width, height, samples);
    }
    else if (formatCaps.getStencilRenderingSupport())
    {
...
void Context::getFloatv(GLenum pname, GLfloat *params)
{
    ...
    switch (pname)
    {
      ...
      case GL_ALIASED_LINE_WIDTH_RANGE:
        params[0] = getCaps().getMinAliasedLineWidth();
        params[1] = getCaps().getMaxAliasedLineWidth();
        break;
      case GL_ALIASED_POINT_SIZE_RANGE:
        params[0] = getCaps().getMinAliasedPointSize();
        params[1] = getCaps().getMaxAliasedPointSize();
        break;
      case GL_DEPTH_RANGE:
      ...
      case GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT:
        ASSERT(getCaps().getAnisotropicFilteringSupport());
        *params = getCaps().getMaxTextureAnisotropy();
        break;
      default:
        UNREACHABLE();
        break;
...
void Context::getIntegerv(GLenum pname, GLint *params)
{
    ...
    if (pname >= GL_DRAW_BUFFER0_EXT && pname <= GL_DRAW_BUFFER15_EXT)
    {
        ...
        ASSERT(colorAttachment < caps.getMaxDrawBuffers());
    ...
    switch (pname)
    {
      ...
      case GL_MAX_RENDERBUFFER_SIZE:                    *params = caps.getMaxRenderbufferSize();                        break;
      ...
      case GL_MAX_TEXTURE_SIZE:                         *params = caps.getMax2DTextureSize();                           break;
      case GL_MAX_CUBE_MAP_TEXTURE_SIZE:                *params = caps.getMaxCubeMapTextureSize();                      break;
      case GL_MAX_3D_TEXTURE_SIZE:                      *params = caps.getMax3DTextureSize();                           break;
      case GL_MAX_ARRAY_TEXTURE_LAYERS:                 *params = caps.getMaxArrayTextureLayers();                      break;
      ...
      case GL_MAX_VIEWPORT_DIMS:
        {
            params[0] = getCaps().getMaxViewportWidth();
            params[1] = getCaps().getMaxViewportHeight();
        }
        break;
      ...
      case GL_NUM_EXTENSIONS:
        *params = static_cast<GLint>(getCaps().getExtensionStringCount());
        break;
      default:
        UNREACHABLE();
        break;
...
void Context::getInteger64v(GLenum pname, GLint64 *params)
{
    switch (pname)
    {
      case GL_MAX_ELEMENT_INDEX:
        *params = getCaps().getMaxElementIndex();
        break;
...
bool Context::getQueryParameterInfo(GLenum pname, GLenum *type, unsigned int *numParams)
{
    ...
    switch (pname)
    {
      ...
      case GL_PIXEL_UNPACK_BUFFER_BINDING:
        {
            if (getCaps().getPixelBufferObjectSupport())
            {
                *type = GL_INT;
                *numParams = 1;
            }
      ...
      case GL_BLEND_COLOR:
        ...
        return true;
      case GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT:
        if (!getCaps().getMaxTextureAnisotropy())
        {
            return false;
        }
        *type = GL_FLOAT;
