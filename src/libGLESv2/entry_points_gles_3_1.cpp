...
void GL_APIENTRY DrawArraysIndirect(GLenum mode, const void *indirect)
{
    ...
    if (context)
    {
        if (!ValidateDrawArraysIndirect(context, mode, indirect))
        {
            return;
        }

        Error error = context->drawArraysIndirect(mode, indirect);
        if (error.isError())
        {
...
void GL_APIENTRY DrawElementsIndirect(GLenum mode, GLenum type, const void *indirect)
{
    ...
    if (context)
    {
        if (!ValidateDrawElementsIndirect(context, mode, type, indirect))
        {
            return;
        }

        Error error = context->drawElementsIndirect(mode, type, indirect);
        if (error.isError())
        {
            context->handleError(error);
            return;
