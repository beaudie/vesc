...
static bool packVarying(gl::Varying *varying, const int maxVaryingVectors, const gl::ShaderVariable *packing[][4])
{
    GLenum transposedType = TransposeMatrixType(varying->type);

    // matrices within varying structs are not transposed
    int registers = (varying->isStruct() ? gl::HLSLVariableRegisterCount(*varying) : gl::VariableRowCount(transposedType)) * varying->elementCount();
...
int DynamicHLSL::packVaryings(InfoLog &infoLog, const gl::ShaderVariable *packing[][4], FragmentShader *fragmentShader,
                              VertexShader *vertexShader, const std::vector<std::string>& transformFeedbackVaryings)
{
    ...
    {
        gl::Varying *varying = &fragmentShader->mVaryings[varyingIndex];
    ...
    for (unsigned int feedbackVaryingIndex = 0; feedbackVaryingIndex < transformFeedbackVaryings.size(); feedbackVaryingIndex++)
    {
        ...
        if (packedVaryings.find(transformFeedbackVarying) == packedVaryings.end())
        {
            ...
            {
                gl::Varying *varying = &vertexShader->mVaryings[varyingIndex];
                                             ...
                                             std::vector<LinkedVarying> *linkedVaryings) const
{
    ...
    {
        gl::Varying *varying = &shader->mVaryings[varyingIndex];
        if (varying->registerAssigned())
        {
            ...
            for (unsigned int elementIndex = 0; elementIndex < varying->elementCount(); elementIndex++)
            {
                for (int row = 0; row < variableRows; row++)
                {
                    ...
                    {
                      case gl::INTERPOLATION_SMOOTH:   varyingHLSL += "    ";                 break;
                      case gl::INTERPOLATION_FLAT:     varyingHLSL += "    nointerpolation "; break;
                      case gl::INTERPOLATION_CENTROID: varyingHLSL += "    centroid ";        break;
                      default:  UNREACHABLE();
                    }
...
std::string DynamicHLSL::generateInputLayoutHLSL(const VertexFormat inputLayout[], const gl::Attribute shaderAttributes[]) const
{
    ...
    for (unsigned int attributeIndex = 0; attributeIndex < MAX_VERTEX_ATTRIBS; attributeIndex++)
    {
        ...
        const gl::Attribute &shaderAttribute = shaderAttributes[attributeIndex];
...
bool DynamicHLSL::generateShaderLinkHLSL(InfoLog &infoLog, int registers, const gl::ShaderVariable *packing[][4],
                                         ...
                                         std::map<int, VariableLocation> *programOutputVars) const
{
    ...
    {
        gl::Varying *varying = &vertexShader->mVaryings[vertVaryingIndex];
        if (varying->registerAssigned())
        {
            for (unsigned int elementIndex = 0; elementIndex < varying->elementCount(); elementIndex++)
            {
    ...
    else
    {
        ...
        const std::vector<gl::Attribute> &shaderOutputVars = fragmentShader->getOutputVariables();
        for (auto locationIt = programOutputVars->begin(); locationIt != programOutputVars->end(); locationIt++)
        {
            const VariableLocation &outputLocation = locationIt->second;
            const gl::ShaderVariable &outputVariable = shaderOutputVars[outputLocation.index];
...
void DynamicHLSL::defineOutputVariables(FragmentShader *fragmentShader, std::map<int, VariableLocation> *programOutputVars) const
{
    ...
    {
        const gl::Attribute &outputVariable = shaderOutputVars[outputVariableIndex];
...
std::string DynamicHLSL::generateGeometryShaderHLSL(int registers, const gl::ShaderVariable *packing[][4], FragmentShader *fragmentShader, VertexShader *vertexShader) const
{
...
std::string DynamicHLSL::generatePointSpriteHLSL(int registers, const gl::ShaderVariable *packing[][4], FragmentShader *fragmentShader, VertexShader *vertexShader) const
{
    ASSERT(registers >= 0);
...
std::string DynamicHLSL::generateAttributeConversionHLSL(const VertexFormat &vertexFormat, const gl::ShaderVariable &shaderAttrib) const
{
