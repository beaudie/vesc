...
std::string DynamicHLSL::VERTEX_ATTRIBUTE_STUB_STRING = "<<VERTEX ATTRIBUTES>>";
...
std::string DynamicHLSL::generateInputLayoutHLSL(const VertexFormat inputLayout[], const sh::Attribute shaderAttributes[]) const
{
    ...
    for (unsigned int attributeIndex = 0; attributeIndex < MAX_VERTEX_ATTRIBS; attributeIndex++)
    {
        ...
        if (!shaderAttribute.name.empty())
        {
            ...
            // TODO: handle improper user specified conversions (pure integer->float, float->pure integer)
