                                   ...
                                   GLint border, GLenum format, GLenum type, const GLvoid *pixels)
{
    ...
    switch (target)
    {
      case GL_TEXTURE_2D:
        {
            if (static_cast<GLuint>(width) > (caps.getMax2DTextureSize() >> level) ||
                static_cast<GLuint>(height) > (caps.getMax2DTextureSize() >> level))
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
      ...
      case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z:
        {
            ...
            if (static_cast<GLuint>(width) > (caps.getMaxCubeMapTextureSize() >> level) ||
                static_cast<GLuint>(height) > (caps.getMaxCubeMapTextureSize() >> level))
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
    ...
    if (isCompressed)
    {
        ...
        switch (actualInternalFormat)
        {
          ...
          case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
            if (!context->getCaps().getDXT1TextureSupport())
            {
                return gl::error(GL_INVALID_ENUM, false);
            }
            break;
          case GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE:
            if (!context->getCaps().getDXT3TextureSupport())
            {
                return gl::error(GL_INVALID_ENUM, false);
            }
            break;
          case GL_COMPRESSED_RGBA_S3TC_DXT5_ANGLE:
              if (!context->getCaps().getDXT5TextureSupport())
            {
                return gl::error(GL_INVALID_ENUM, false);
            }
            break;
    ...
    else
    {
        ...
        switch (format)
        {
          ...
          case GL_RED:
          case GL_RG:
              if (!context->getCaps().getRGTextureSupport())
              {
                  return gl::error(GL_INVALID_ENUM, false);
              }
              switch (type)
        ...
        switch (format)
        {
          ...
          case GL_DEPTH_COMPONENT:
          case GL_DEPTH_STENCIL_OES:
            if (!context->getCaps().getDepthTextureSupport())
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
        ...
        if (type == GL_FLOAT)
        {
            if (!context->getCaps().getFloatTextureSupport())
            {
                return gl::error(GL_INVALID_ENUM, false);
            }
        }
        ...
        {
            if (!context->getCaps().getHalfFloatTextureSupport())
            {
                return gl::error(GL_INVALID_ENUM, false);
            }
        }
                                     ...
                                     GLsizei width, GLsizei height)
{
    ...
    {
      case GL_TEXTURE_2D:
        if (static_cast<GLuint>(width) > caps.getMax2DTextureSize() ||
            static_cast<GLuint>(height) > caps.getMax2DTextureSize())
        {
            return gl::error(GL_INVALID_VALUE, false);
        }
        break;
      case GL_TEXTURE_CUBE_MAP:
        if (static_cast<GLuint>(width) > caps.getMaxCubeMapTextureSize() ||
            static_cast<GLuint>(height) > caps.getMaxCubeMapTextureSize())
        {
            return gl::error(GL_INVALID_VALUE, false);
        }
        break;
    ...
    if (levels != 1 && !context->getCaps().getNPOTTextureSupport())
    {
                                             ...
                                             GLenum textarget, GLuint texture, GLint level)
{
    ...
    if (attachment >= GL_COLOR_ATTACHMENT0 && attachment <= GL_COLOR_ATTACHMENT15)
    {
        ...
        if (colorAttachment >= context->getCaps().getMaxColorAttachments())
        {
            return gl::error(GL_INVALID_VALUE, false);
        }
    }
