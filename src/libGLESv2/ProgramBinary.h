...
class ProgramBinary : public RefCountObject
{
  ...
  private:
    ...
    {
    public:
        PixelExecutable(const std::vector<GLenum> outputTypes, rx::ShaderExecutable *shaderExecutable);
        ~PixelExecutable();

        bool matchesOutput(const std::vector<GLenum> &outputs) const;

        const std::vector<GLenum> &outputs() const { return mOutputs; }
        rx::ShaderExecutable *shaderExecutable() const { return mShaderExecutable; }
