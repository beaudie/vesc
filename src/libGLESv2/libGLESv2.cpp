...
void GL_APIENTRY glClear(GLbitfield mask)
{
    ...
    if (context)
    {
        ...
        GLenum completeness = framebufferObject->completeness(context->getClientVersion(),
                                                              context->getTextureCaps(),
                                                              context->getExtensions());

        if (!framebufferObject || completeness != GL_FRAMEBUFFER_COMPLETE)
        {
...
void GL_APIENTRY glInvalidateFramebuffer(GLenum target, GLsizei numAttachments, const GLenum* attachments)
{
    ...
    if (context)
    {
        ...
        gl::Framebuffer *framebuffer = context->getState().getTargetFramebuffer(target);
        GLenum completeness = framebuffer->completeness(context->getClientVersion(),
                                                        context->getTextureCaps(),
                                                        context->getExtensions());
