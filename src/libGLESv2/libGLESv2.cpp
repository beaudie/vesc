...
void __stdcall glGenerateMipmap(GLenum target)
{
    ...
    try
    {
        ...
        if (context)
        {
            ...
            // unsized formats or are color renderable and filterable.  Since we do not track if
            ...
            if (gl::IsDepthRenderingSupported(internalFormat, context) ||
                ...
                (!gl::IsColorRenderingSupported(internalFormat, context) && !isLUMA))
            {
                return gl::error(GL_INVALID_OPERATION);
            }
