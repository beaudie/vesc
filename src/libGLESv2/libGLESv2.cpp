...
void __stdcall glDrawBuffersEXT(GLsizei n, const GLenum *bufs)
{
    ...
    try
    {
        ...
        if (context)
        {
            ...
            for (size_t colorAttachment = 0; colorAttachment < static_cast<size_t>(n); colorAttachment++)
            {
                framebuffer->setDrawBufferState(colorAttachment, bufs[colorAttachment]);
            }

            for (size_t colorAttachment = n; colorAttachment < context->getCaps().maxDrawBuffers; colorAttachment++)
            {
