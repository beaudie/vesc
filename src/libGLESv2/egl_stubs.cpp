...
{
  public:
    ScopedSyncCurrentContextFromThread(Thread *thread) : mThread(thread) { ASSERT(mThread); }
    ~ScopedSyncCurrentContextFromThread() { SetContextCurrent(mThread, mThread->getContext()); }

  private:
    Thread *const mThread;
};
...
EGLBoolean DestroyContext(Thread *thread, Display *display, gl::Context *context)
{
    ...
    ScopedSyncCurrentContextFromThread scoped_sync_current(thread);
