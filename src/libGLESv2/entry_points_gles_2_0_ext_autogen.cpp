...
void GL_APIENTRY TextureStorage1DEXT(GLuint texture,
                                     GLenum target,
                                     GLsizei levels,
                                     GLenum internalformat,
                                     GLsizei width)
{
    EVENT(
        "(GLuint texture = %u, GLenum target = 0x%X, GLsizei levels = %d, GLenum internalformat = "
        "0x%X, GLsizei width = %d)",
        texture, target, levels, internalformat, width);

    Context *context = GetValidGlobalContext();
    if (context)
    {
        context->gatherParams<EntryPoint::TextureStorage1DEXT>(texture, target, levels,
                                                               internalformat, width);

        if (context->skipValidation() ||
            ValidateTextureStorage1DEXT(context, texture, target, levels, internalformat, width))
        {
            context->textureStorage1D(texture, target, levels, internalformat, width);
        }
    }
}

void GL_APIENTRY TextureStorage2DEXT(GLuint texture,
                                     GLenum target,
                                     GLsizei levels,
                                     GLenum internalformat,
                                     GLsizei width,
                                     GLsizei height)
{
    EVENT(
        "(GLuint texture = %u, GLenum target = 0x%X, GLsizei levels = %d, GLenum internalformat = "
        "0x%X, GLsizei width = %d, GLsizei height = %d)",
        texture, target, levels, internalformat, width, height);

    Context *context = GetValidGlobalContext();
    if (context)
    {
        context->gatherParams<EntryPoint::TextureStorage2DEXT>(texture, target, levels,
                                                               internalformat, width, height);

        if (context->skipValidation() ||
            ValidateTextureStorage2DEXT(context, texture, target, levels, internalformat, width,
                                        height))
        {
            context->textureStorage2D(texture, target, levels, internalformat, width, height);
        }
    }
}

void GL_APIENTRY TextureStorage3DEXT(GLuint texture,
                                     GLenum target,
                                     GLsizei levels,
                                     GLenum internalformat,
                                     GLsizei width,
                                     ...
                                     GLsizei depth)
{
    EVENT(
        "(GLuint texture = %u, GLenum target = 0x%X, GLsizei levels = %d, GLenum internalformat = "
        "0x%X, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d)",
        texture, target, levels, internalformat, width, height, depth);

    Context *context = GetValidGlobalContext();
    if (context)
    {
        context->gatherParams<EntryPoint::TextureStorage3DEXT>(
            texture, target, levels, internalformat, width, height, depth);

        if (context->skipValidation() ||
            ValidateTextureStorage3DEXT(context, texture, target, levels, internalformat, width,
                                        height, depth))
        {
            context->textureStorage3D(texture, target, levels, internalformat, width, height,
                                      depth);
        }
    }
}
...
{
    EVENT("(GLDEBUGPROC callback = 0x%0.8p, const void *userParam = 0x%0.8p)", callback, userParam);

    Context *context = GetValidGlobalContext();
    if (context)
    {
        context->gatherParams<EntryPoint::DebugMessageCallback>(callback, userParam);
                                     ...
                                     GLboolean enabled)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::DebugMessageControl>(source, type, severity, count, ids,
                                                               enabled);
                                    ...
                                    const GLchar *buf)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::DebugMessageInsert>(source, type, id, severity, length,
                                                              buf);
                                      ...
                                      GLchar *messageLog)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::GetDebugMessageLog>(count, bufSize, sources, types, ids,
...
GetObjectLabel(GLenum identifier, GLuint name, GLsizei bufSize, GLsizei *length, GLchar *label)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::GetObjectLabel>(identifier, name, bufSize, length, label);
...
void GL_APIENTRY GetObjectPtrLabel(const void *ptr, GLsizei bufSize, GLsizei *length, GLchar *label)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::GetObjectPtrLabel>(ptr, bufSize, length, label);
...
void GL_APIENTRY ObjectLabel(GLenum identifier, GLuint name, GLsizei length, const GLchar *label)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::ObjectLabel>(identifier, name, length, label);
...
void GL_APIENTRY ObjectPtrLabel(const void *ptr, GLsizei length, const GLchar *label)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::ObjectPtrLabel>(ptr, length, label);
...
void GL_APIENTRY PopDebugGroup()
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::PopDebugGroup>();
...
void GL_APIENTRY PushDebugGroup(GLenum source, GLuint id, GLsizei length, const GLchar *message)
{
    ...
    if (context)
    {
        context->gatherParams<EntryPoint::PushDebugGroup>(source, id, length, message);
