...
ANGLE_REQUIRE_CONSTANT_INIT std::atomic<angle::GlobalMutex *> g_SharedContextMutex(nullptr);
static_assert(std::is_trivially_destructible<decltype(g_SharedContextMutex)>::value,
              "global mutex is not trivially destructible");
...
void AllocateSharedContextMutex()
{
    if (g_SharedContextMutex == nullptr)
    {
        std::unique_ptr<angle::GlobalMutex> newMutex(new angle::GlobalMutex());
        angle::GlobalMutex *expected = nullptr;
        if (g_SharedContextMutex.compare_exchange_strong(expected, newMutex.get()))
        {
            newMutex.release();
        }
    }
...
{
    AllocateSharedContextMutex();
    return *g_SharedContextMutex;
}
...
void DeallocateSharedContextMutex()
{
    angle::GlobalMutex *mutex = g_SharedContextMutex.exchange(nullptr);
    {
        // Wait for the mutex to become released by other threads before deleting.
        std::lock_guard<angle::GlobalMutex> lock(*mutex);
    }
