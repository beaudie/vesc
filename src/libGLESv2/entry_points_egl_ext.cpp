...
EGLDisplay EGLAPIENTRY GetPlatformDisplayEXT(EGLenum platform, void *native_display, const EGLint *attrib_list)
{
    ...
    if (platform == EGL_PLATFORM_ANGLE_ANGLE)
    {
        ...
        if (attrib_list)
        {
            for (const EGLint *curAttrib = attrib_list; curAttrib[0] != EGL_NONE; curAttrib += 2)
            {
                switch (curAttrib[0])
                {
                    ...
                    case EGL_EXPERIMENTAL_DIRECT_RENDERING_ANGLE:
                        switch (curAttrib[1])
                        {
                            case EGL_TRUE:
                            case EGL_FALSE:
                            ...
                            default:
                                SetGlobalError(Error(
                                    EGL_BAD_ATTRIBUTE,
                                    "Invalid value for EGL_EXPERIMENTAL_DIRECT_RENDERING_ANGLE"));
                                return EGL_NO_DISPLAY;
                        }
        ...
        if (directRenderingEnabled && platformType != EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE)
        {
            SetGlobalError(
                Error(EGL_BAD_ATTRIBUTE,
                      "EGL_EXPERIMENTAL_DIRECT_RENDERING_ANGLE requires a device type of "
                      "EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE."));
            return EGL_NO_DISPLAY;
        }
