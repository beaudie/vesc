...
EGLBoolean EGLAPIENTRY QueryDeviceAttribEXT(EGLDeviceEXT device, EGLint attribute, EGLAttrib *value)
{
    ...
    if (!display->getExtensions().displayDevice)
    {
        SetGlobalError(Error(EGL_SUCCESS));
        return EGL_FALSE;
    }
    ...
    switch (attribute)
    {
      case EGL_D3D11_DEVICE_ANGLE:
        if (!display->getExtensions().deviceD3D || display->getRendererClass() != rx::RENDERER_D3D11)
        {
            SetGlobalError(Error(EGL_BAD_ATTRIBUTE));
            return EGL_FALSE;
        }
      ...
      case EGL_D3D9_DEVICE_ANGLE:
        if (!display->getExtensions().deviceD3D || display->getRendererClass() != rx::RENDERER_D3D9)
        {
            SetGlobalError(Error(EGL_BAD_ATTRIBUTE));
            return EGL_FALSE;
        }
