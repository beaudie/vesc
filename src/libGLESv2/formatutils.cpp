...
static InternalFormatInfoMap BuildInternalFormatInfoMap()
{
    ...
    // - Multisampled buffer ar disallowed for non-normalized integer component types and we want to support it for STENCIL_INDEX8
    // - All other stencil formats (all depth-stencil) are either float or normalized
    // - It is consistent with the observed behavior of the NVIDIA driver
    // - It doesn't affect the application apart from the error condition of RenderbufferStorageMultismple which is way we changed it in the first place
    //                               | Internal format  |                  |D |S |X | Format          | Type            | Component type        | Supported                 | Renderable                                                                         | Filterable                                  |
