...
{
    return memcmp(&a, &b, sizeof(TypeInfo)) < 0;
}
...
InternalFormatInfo DefaultFormat()
{
    InternalFormatInfo formatInfo;

    formatInfo.redBits = 0;
    formatInfo.greenBits = 0;
    formatInfo.blueBits = 0;

    formatInfo.luminanceBits = 0;

    formatInfo.alphaBits = 0;
    formatInfo.sharedBits = 0;

    formatInfo.depthBits = 0;
    formatInfo.stencilBits = 0;

    formatInfo.pixelBytes = 0;

    formatInfo.componentCount = 0;

    formatInfo.compressedBlockWidth = 0;
    formatInfo.compressedBlockHeight = 0;

    formatInfo.format = GL_NONE;
    formatInfo.type = GL_NONE;

    formatInfo.componentType = GL_NONE;
    formatInfo.colorEncoding = GL_NONE;

    formatInfo.isCompressed = false;

    formatInfo.textureSupport = NeverSupported;
    formatInfo.renderSupport = NeverSupported;
    formatInfo.filterSupport = NeverSupported;

    return formatInfo;
}

static InternalFormatInfo UnsizedFormat(GLenum format, InternalFormatInfo::SupportCheckFunction textureSupport,
                                        InternalFormatInfo::SupportCheckFunction renderSupport,
                                        InternalFormatInfo::SupportCheckFunction filterSupport)
{
    InternalFormatInfo formatInfo = DefaultFormat();
...
static InternalFormatInfo RGBAFormat(GLuint red, GLuint green, GLuint blue, GLuint alpha, GLuint shared,
                                     ...
                                     InternalFormatInfo::SupportCheckFunction textureSupport,
                                     InternalFormatInfo::SupportCheckFunction renderSupport,
                                     InternalFormatInfo::SupportCheckFunction filterSupport)
{
    InternalFormatInfo formatInfo = DefaultFormat();
...
static InternalFormatInfo DepthStencilFormat(GLuint depthBits, GLuint stencilBits, GLuint unusedBits, GLenum format,
                                             GLenum type, GLenum componentType, InternalFormatInfo::SupportCheckFunction textureSupport,
                                             InternalFormatInfo::SupportCheckFunction renderSupport,
                                             InternalFormatInfo::SupportCheckFunction filterSupport)
{
    InternalFormatInfo formatInfo = DefaultFormat();
...
const FormatTypeInfo &GetFormatTypeInfo(GLenum format, GLenum type)
{
    ...
    {
        static const FormatTypeInfo defaultInfo = { GL_NONE, NULL };
        return defaultInfo;
    }
}
...
const TypeInfo &GetTypeInfo(GLenum type)
{
    ...
    {
        static const TypeInfo defaultInfo = { 0, false };
        return defaultInfo;
    }
}
...
const InternalFormatInfo &GetInternalFormatInfo(GLenum internalFormat)
{
    ...
    {
        static const InternalFormatInfo defaultInternalFormat = DefaultFormat();
        return defaultInternalFormat;
    }
}

GLuint InternalFormatInfo::computeRowPitch(GLenum type, GLsizei width, GLint alignment) const
{
    ASSERT(alignment > 0 && isPow2(alignment));
    return rx::roundUp(computeBlockSize(type, width, 1), static_cast<GLuint>(alignment));
}

GLuint InternalFormatInfo::computeDepthPitch(GLenum type, GLsizei width, GLsizei height, GLint alignment) const
{
    return computeRowPitch(type, width, alignment) * height;
}

GLuint InternalFormatInfo::computeBlockSize(GLenum type, GLsizei width, GLsizei height) const
{
    if (isCompressed)
    {
