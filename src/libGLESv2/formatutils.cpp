                                       ...
                                       GLuint clientVersion, GLenum *outEffectiveFormat)
{
    ...
    for (EffectiveInternalFormatList::const_iterator itr = list->begin(); itr != list->end(); itr++)
    {
        if ((itr->mDestFormat == targetFormat) &&
            (itr->mMinRedBits   <= srcFormat.mRedBits   && itr->mMaxRedBits   >= srcFormat.mRedBits)   &&
            (itr->mMinGreenBits <= srcFormat.mGreenBits && itr->mMaxGreenBits >= srcFormat.mGreenBits) &&
            (itr->mMinBlueBits  <= srcFormat.mBlueBits  && itr->mMaxBlueBits  >= srcFormat.mBlueBits)  &&
            (itr->mMinAlphaBits <= srcFormat.mAlphaBits && itr->mMaxAlphaBits >= srcFormat.mAlphaBits))
        {
            *outEffectiveFormat = itr->mEffectiveFormat;
            return true;
        }
    }
