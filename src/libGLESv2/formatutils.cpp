...
static EffectiveInternalFormatList BuildUnsizedEffectiveInternalFormatList()
{
    ...
    list.push_back(EffectiveInternalFormatInfo(GL_ALPHA8_EXT,              GL_ALPHA,           0, MAXUINT, 0, MAXUINT, 0, MAXUINT, 1, 8      ));
    list.push_back(EffectiveInternalFormatInfo(GL_LUMINANCE8_EXT,          GL_LUMINANCE,       1, 8,       0, MAXUINT, 0, MAXUINT, 0, MAXUINT));
    list.push_back(EffectiveInternalFormatInfo(GL_LUMINANCE8_ALPHA8_EXT,   GL_LUMINANCE_ALPHA, 1, 8,       0, MAXUINT, 0, MAXUINT, 1, 8      ));
    list.push_back(EffectiveInternalFormatInfo(GL_RGB565,                  GL_RGB,             1, 5,       1, 6,       1, 5,       0, MAXUINT));
    list.push_back(EffectiveInternalFormatInfo(GL_RGB8,                    GL_RGB,             6, 8,       7, 8,       6, 8,       0, MAXUINT));
                                       ...
                                       GLuint clientVersion, GLenum *outEffectiveFormat)
{
    ...
    {
        if (((*list)[curFormat].mDestFormat == targetFormat) &&
            ((*list)[curFormat].mMinRedBits   <= srcFormat.mRedBits   && (*list)[curFormat].mMaxRedBits   >= srcFormat.mRedBits)   &&
            ((*list)[curFormat].mMinGreenBits <= srcFormat.mGreenBits && (*list)[curFormat].mMaxGreenBits >= srcFormat.mGreenBits) &&
            ((*list)[curFormat].mMinBlueBits  <= srcFormat.mBlueBits  && (*list)[curFormat].mMaxBlueBits  >= srcFormat.mBlueBits)  &&
            ((*list)[curFormat].mMinAlphaBits <= srcFormat.mAlphaBits && (*list)[curFormat].mMaxAlphaBits >= srcFormat.mAlphaBits))
        {
            *outEffectiveFormat = (*list)[curFormat].mEffectiveFormat;
            return true;
        }
    }
        ...
        GetInternalFormatInfo(frameBufferInternalFormat, clientVersion, &framebufferInternalFormatInfo))
    {
        ...
        else if (clientVersion == 3)
        {
            ...
            if (conversionSet.find(conversion) != conversionSet.end())
            {
                ...
                //      with the values of the source buffer's [channel sizes]. Table 3.17 is used 
