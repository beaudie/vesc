...
// If "context" is not current in the "thread", then this function will ensure that
// SharedContextMutex is activate (to avoid locking SingleContextMutex from multiple threads).
ANGLE_INLINE egl::ContextMutex *TryGetContextMutexForThread(Context *context,
                                                            const egl::Thread *thread)
{
    if (context != nullptr)
    {
...
ANGLE_INLINE egl::ContextMutex *TryEnsureActiveAndGetSharedContextMutex(Context *context)
{
    if (context != nullptr)
    {
        context->ensureSharedMutexActive();
