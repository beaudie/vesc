...
FramebufferAttachment *Framebuffer::getAttachment(GLenum type, GLuint handle, GLint level, GLint layer) const
{
    if (handle == 0)
    {
        return NULL;
...
void Framebuffer::setColorbuffer(unsigned int colorAttachment, GLenum type, GLuint colorbuffer, GLint level, GLint layer)
{
    ...
    SafeDelete(mColorbuffers[colorAttachment]);
    mColorbuffers[colorAttachment] = getAttachment(type, colorbuffer, level, layer);
}

void Framebuffer::setDepthbuffer(GLenum type, GLuint depthbuffer, GLint level, GLint layer)
{
    SafeDelete(mDepthbuffer);
    mDepthbuffer = getAttachment(type, depthbuffer, level, layer);
}

void Framebuffer::setStencilbuffer(GLenum type, GLuint stencilbuffer, GLint level, GLint layer)
{
    SafeDelete(mStencilbuffer);
    mStencilbuffer = getAttachment(type, stencilbuffer, level, layer);
}

void Framebuffer::setDepthStencilBuffer(GLenum type, GLuint depthStencilBuffer, GLint level, GLint layer)
{
    FramebufferAttachment *attachment = getAttachment(type, depthStencilBuffer, level, layer);
...
{
    return (saneDepthStencil() ? mDepthbuffer : NULL);
}
...
{
    return (saneDepthStencil() ? mDepthbuffer->type() : GL_NONE);
}
...
{
    return (saneDepthStencil() ? mDepthbuffer->id() : 0);
}
...
{
    return (saneDepthStencil() ? mDepthbuffer->mipLevel() : 0);
}
...
{
    return (saneDepthStencil() ? mDepthbuffer->layer() : 0);
}
...
GLenum Framebuffer::completeness() const
{
        ...
        if (colorbuffer)
        {
            ...
            if (!missingAttachment)
            {
                ...
                for (unsigned int previousColorAttachment = 0; previousColorAttachment < colorAttachment; previousColorAttachment++)
                {
                    ...
                    if (currentAttachment.id() == getColorbufferHandle(previousColorAttachment) &&
                        currentAttachment.type() == getColorbufferType(previousColorAttachment))
                    {
                        return GL_FRAMEBUFFER_UNSUPPORTED;
                    }
                }
    ...
    if (mDepthbuffer && mStencilbuffer && !saneDepthStencil())
    {
        return GL_FRAMEBUFFER_UNSUPPORTED;
    }
...
bool Framebuffer::saneDepthStencil() const
{
