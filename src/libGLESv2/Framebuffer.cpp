...
GLenum Framebuffer::completeness(GLint clientVersion, const gl::TextureCapsMap &textureCaps,
                                 const gl::Extensions &extensions) const
{
    int width = 0;
    int height = 0;
    ...
    for (unsigned int colorAttachment = 0; colorAttachment < IMPLEMENTATION_MAX_DRAW_BUFFERS; colorAttachment++)
    {
        ...
        if (colorbuffer)
        {
            ...
            const TextureCaps &formatCaps = textureCaps.get(internalformat);
...
int Framebuffer::getSamples(GLint clientVersion, const gl::TextureCapsMap &textureCaps,
                            const gl::Extensions &extensions) const
{
    if (completeness(clientVersion, textureCaps, extensions) == GL_FRAMEBUFFER_COMPLETE)
    {
...
GLenum DefaultFramebuffer::completeness(GLint /*clientVersion*/, const gl::TextureCapsMap &/*textureCaps*/,
                                        const gl::Extensions &/*extensions*/) const
{
