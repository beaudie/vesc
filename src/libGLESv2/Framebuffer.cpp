...
GLenum Framebuffer::completeness() const
{
    ...
    for (unsigned int colorAttachment = 0; colorAttachment < IMPLEMENTATION_MAX_DRAW_BUFFERS; colorAttachment++)
    {
        ...
        if (colorbuffer)
        {
            ...
            {
                if (!formatCaps.rendering || formatInfo.depthBits > 0 || formatInfo.stencilBits > 0)
                {
                    return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
                }
            }
    ...
    if (mDepthbuffer)
    {
        ...
        {
            if (!formatCaps.rendering || formatInfo.depthBits == 0)
            {
                return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
            }
        }
    ...
    if (mStencilbuffer)
    {
        ...
        {
            if (!formatCaps.rendering || formatInfo.stencilBits == 0)
            {
                return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
            }
        }
