...
template <typename T>
inline T *OffsetDataPointer(byte *data, size_t y, size_t z, size_t rowPitch, size_t depthPitch)
{
    return reinterpret_cast<T*>(data + (y * rowPitch) + (z * depthPitch));
}

template <typename T>
inline const T *OffsetDataPointer(const byte *data, size_t y, size_t z, size_t rowPitch, size_t depthPitch)
{
                                   ...
                                   byte *output, size_t outputRowPitch, size_t outputDepthPitch)
{
    ...
    for (size_t z = 0; z < depth; ++z)
    {
        ...
        {
            const byte *source = OffsetDataPointer<byte>(input, y, z, inputRowPitch, inputDepthPitch);
            byte *dest = OffsetDataPointer<byte>(output, y, z, outputRowPitch, outputDepthPitch);
