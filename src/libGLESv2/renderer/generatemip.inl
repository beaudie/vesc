...
template <typename T>
static inline T *GetPixel(byte *data, size_t x, size_t y, size_t z, size_t rowPitch, size_t depthPitch)
{
    return reinterpret_cast<T*>(data + (x * sizeof(T)) + (y * rowPitch) + (z * depthPitch));
}

template <typename T>
static inline const T *GetPixel(const byte *data, size_t x, size_t y, size_t z, size_t rowPitch, size_t depthPitch)
{
