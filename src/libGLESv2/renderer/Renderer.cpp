...
rx::Renderer *glCreateRenderer(egl::Display *display, EGLNativeDisplayType nativeDisplay, EGLint requestedDisplayType,
                               bool anglePlatformAngle)
{
    std::vector<rx::CreateRendererFunction> rendererCreationFunctions;

#   if defined(ANGLE_ENABLE_D3D11)
        if (anglePlatformAngle)
        {
            if (requestedDisplayType == EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE ||
...
#   if defined(ANGLE_ENABLE_D3D9)
        if (anglePlatformAngle)
        {
            if (requestedDisplayType == EGL_PLATFORM_ANGLE_TYPE_D3D9_ANGLE)
            {
...
#   endif
    ...
    if (anglePlatformAngle && requestedDisplayType == EGL_PLATFORM_ANGLE_TYPE_DEFAULT_ANGLE)
    {
