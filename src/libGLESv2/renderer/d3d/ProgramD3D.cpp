...
ProgramD3D::PixelExecutable::PixelExecutable(const std::vector<GLenum> &outputSignature, rx::ShaderExecutable *shaderExecutable)
...
sh::Attribute *ProgramD3D::getShaderAttributes() { 
...
ShaderExecutable *ProgramD3D::getPixelExecutableForOutputLayout(const std::vector<GLenum> &outputSignature)
{
    ...
    ShaderExecutable *pixelExecutable = mRenderer->compileToExecutable(tempInfoLog, finalPixelHLSL.c_str(), rx::SHADER_PIXEL,
                                                                        mTransformFeedbackLinkedVaryings,
                                                                        (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS),
                                                                        mPixelWorkarounds);
...
rx::ShaderExecutable *ProgramD3D::getVertexExecutableForInputLayout(const gl::VertexFormat inputLayout[gl::MAX_VERTEX_ATTRIBS])
{
    GLenum signature[gl::MAX_VERTEX_ATTRIBS];
    getInputLayoutSignature(inputLayout, signature);
    ...
    gl::InfoLog tempInfoLog;
    ShaderExecutable *vertexExecutable = mRenderer->compileToExecutable(tempInfoLog, finalVertexHLSL.c_str(),
                                                                        rx::SHADER_VERTEX,
                                                                        mTransformFeedbackLinkedVaryings,
                                                                        (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS),
                                                                        mVertexWorkarounds);
                                           ...
                                           int registers)
{
    ...
    if (usesGeometryShader())
    {
        ...
        mGeometryExecutable = mRenderer->compileToExecutable(infoLog, geometryHLSL.c_str(),
