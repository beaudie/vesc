...
gl::LinkedUniform *ProgramD3D::getUniformByLocation(GLint location) const
{
    ASSERT(location >= 0 && static_cast<size_t>(location) < mUniformIndex.size());
    return mUniforms[mUniformIndex[location].index];
}

GLuint ProgramD3D::getUniformBlockIndex(std::string name) const
{
...
bool ProgramD3D::load(gl::InfoLog &infoLog, gl::BinaryInputStream *stream)
{
    ...
    initializeUniformStorage(mUniforms);
...
gl::LinkedUniform *ProgramD3D::getUniformByName(const std::string &name) const
{
    for (size_t uniformIndex = 0; uniformIndex < mUniforms.size(); uniformIndex++)
    {
...
bool ProgramD3D::linkUniformBlocks(gl::InfoLog &infoLog, const gl::Shader &vertexShader, const gl::Shader &fragmentShader,
                                   const gl::Caps &caps)
{
...
bool ProgramD3D::areMatchingInterfaceBlocks(gl::InfoLog &infoLog, const sh::InterfaceBlock &vertexInterfaceBlock,
