...
void ShaderD3D::compileToHLSL(void *compiler, const std::string &source)
{
    ...
    else if (result)
    {
        ...
        std::vector<char> outputHLSL = std::vector<char>(objCodeLen);
        ShGetObjectCode(compiler, &outputHLSL[0]);
    ...
    else
    {
        ...
        std::vector<char> infoLog = std::vector<char>(infoLogLen);
        ShGetInfoLog(compiler, &infoLog[0]);
        mInfoLog = &infoLog[0];
