                                   ...
                                   gl::Framebuffer *source)
{
    ...
    else
    {
        ...
        if (width != 0 && height != 0 && isValidLevel(level))
        {
            ...
            error = mRenderer->copyImage2D(source, sourceRect, format, 0, 0, mTexStorage, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_2D::setCompleteTexStorage(TextureStorage *newCompleteTexStorage)
{
    ...
    if (mTexStorage && mTexStorage->isManaged())
    {
        ...
        {
            gl::Error error = mImageArray[level]->setManagedSurface2D(mTexStorage, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_2D::ensureRenderTarget()
{
    ...
    if (getBaseLevelWidth() > 0 && getBaseLevelHeight() > 0)
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            ...
            error = mRenderer->copyToRenderTarget2D(newRenderTargetStorage, mTexStorage);
            if (error.isError())
            {
...
gl::Error TextureD3D_Cube::setCompleteTexStorage(TextureStorage *newCompleteTexStorage)
{
    ...
    if (mTexStorage && mTexStorage->isManaged())
    {
        for (int faceIndex = 0; faceIndex < 6; faceIndex++)
        {
            for (int level = 0; level < mTexStorage->getLevelCount(); level++)
            {
                gl::Error error = mImageArray[faceIndex][level]->setManagedSurfaceCube(mTexStorage, faceIndex, level);
                if (error.isError())
                {
                    return error;
                }
...
gl::Error TextureD3D_Cube::ensureRenderTarget()
{
    ...
    if (getBaseLevelWidth() > 0)
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            ...
            error = mRenderer->copyToRenderTargetCube(newRenderTargetStorage, mTexStorage);
            if (error.isError())
            {
                SafeDelete(newRenderTargetStorage);
                return error;
...
gl::Error TextureD3D_3D::ensureRenderTarget()
{
    ...
    if (getBaseLevelWidth() > 0 && getBaseLevelHeight() > 0 && getBaseLevelDepth() > 0)
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            ...
            error = mRenderer->copyToRenderTarget3D(newRenderTargetStorage, mTexStorage);
            if (error.isError())
            {
                SafeDelete(newRenderTargetStorage);
                return error;
...
gl::Error TextureD3D_2DArray::ensureRenderTarget()
{
    ...
    if (getBaseLevelWidth() > 0 && getBaseLevelHeight() > 0 && getLayers(0) > 0)
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            ...
            error = mRenderer->copyToRenderTarget2DArray(newRenderTargetStorage, mTexStorage);
            if (error.isError())
            {
                SafeDelete(newRenderTargetStorage);
                return error;
