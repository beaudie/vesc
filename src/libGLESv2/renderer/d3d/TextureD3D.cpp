...
const uint8_t *GetUnpackPointer(const gl::PixelUnpackState &unpack, const void *pixels)
{
    if (unpack.pixelBuffer.id() != 0)
    {
        ...
        return static_cast<const uint8_t *>(bufferData) + offset;
    }
    else
    {
...
gl::Error TextureD3D::setImage(const gl::PixelUnpackState &unpack, GLenum type, const void *pixels, const gl::ImageIndex &index)
{
    ...
    const uint8_t *pixelData = GetUnpackPointer(unpack, pixels);
