...
void *Buffer11::getData()
{
    ...
    if (stagingBuffer->getDataRevision() > mResolvedDataRevision)
    {
        ...
        ID3D11DeviceContext *context = mRenderer11->getDeviceContext();
...
ID3D11ShaderResourceView *Buffer11::getSRV(DXGI_FORMAT srvFormat)
{
    ...
    ID3D11Device *device = mRenderer11->getDevice();
...
Buffer11::BufferStorage11 *Buffer11::getBufferStorage(BufferUsage usage)
{
    ...
    if (!directBuffer)
    {
        ...
        {
            directBuffer = new PackStorage11(mRenderer11);
        }
        else
        {
            // buffer is not allocated, create it
            directBuffer = new NativeBuffer11(mRenderer11, usage);
        }
