...
void Renderer11::applyTransformFeedbackBuffers(const gl::State& state)
{
    ...
    for (size_t i = 0; i < numXFBBindings; i++)
    {
        ...
        if (curXFBBuffer)
        {
            ...
            mCurrentD3DBuffers[i] = buffer;
            mCurrentD3DOffsets[i] = (mAppliedTFBuffers[i] != buffer) ? static_cast<UINT>(curXFBOffset) : -1;
        }
        else
        {
            mCurrentD3DBuffers[i] = NULL;
            mCurrentD3DOffsets[i] = 0;
        }

        if (mCurrentD3DBuffers[i] != mAppliedTFBuffers[i] || curXFBOffset != mAppliedTFOffsets[i])
        {
            requiresUpdate = true;
        }
    }
    ...
    if (requiresUpdate)
    {
        mDeviceContext->SOSetTargets(numXFBBindings, mCurrentD3DBuffers, mCurrentD3DOffsets);

        for (size_t i = 0; i < numXFBBindings; i++)
        {
            // TODO: mAppliedTFBuffers and friends should also be kept in a vector.
            mAppliedTFBuffers[i] = mCurrentD3DBuffers[i];
            mAppliedTFOffsets[i] = state.getIndexedTransformFeedbackBufferOffset(i);
        }
    }
}
