...
gl::Error Renderer11::copyToRenderTarget(TextureStorageInterface2D *dest, TextureStorageInterface2D *source)
{
    if (!source || !dest)
    {
...
gl::Error Renderer11::copyToRenderTarget(TextureStorageInterfaceCube *dest, TextureStorageInterfaceCube *source)
{
    if (!source || !dest)
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Internal texture storage was unexpectedly NULL.");
...
gl::Error Renderer11::copyToRenderTarget(TextureStorageInterface3D *dest, TextureStorageInterface3D *source)
{
    if (!source || !dest)
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Internal texture storage was unexpectedly NULL.");
...
gl::Error Renderer11::copyToRenderTarget(TextureStorageInterface2DArray *dest, TextureStorageInterface2DArray *source)
{
    if (!source || !dest)
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Internal texture storage was unexpectedly NULL.");
...
gl::Error Renderer11::copyImage(gl::Framebuffer *framebuffer, const gl::Rectangle &sourceRect, GLenum destFormat,
...
gl::Error Renderer11::getRenderTargetResource(gl::FramebufferAttachment *colorbuffer, unsigned int *subresourceIndex, ID3D11Texture2D **resource)
{
    ASSERT(colorbuffer);
    ASSERT(resource);
    ...
    *subresourceIndex = renderTarget->getSubresourceIndex();
    ...
    ASSERT(colorBufferRTV);

    ID3D11Resource *textureResource = NULL;
    colorBufferRTV->GetResource(&textureResource);
    if (!textureResource)
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Failed to extract the resource from a color buffer render target view.");
    }

    HRESULT result = textureResource->QueryInterface(__uuidof(ID3D11Texture2D), reinterpret_cast<void**>(resource));
    SafeRelease(textureResource);
    if (FAILED(result))
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Failed to extract the ID3D11Texture2D from the render target resource, result: 0x%X.", result);
    }
    ASSERT(*resource);
