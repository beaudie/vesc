...
gl::Error Renderer11::createRenderTarget(int width, int height, GLenum format, GLsizei samples, RenderTarget **outRT)
{
    ...
    if (width > 0 && height > 0)
    {
        ...
        if (bindDSV)
        {
            ...
            *outRT = new ResourceRenderTarget11(dsv, texture, srv, format, width, height, 1, supportedSamples);
        ...
        else if (bindRTV)
        {
            ...
            *outRT = new ResourceRenderTarget11(rtv, texture, srv, format, width, height, 1, supportedSamples);
    ...
    {
        *outRT = new ResourceRenderTarget11(reinterpret_cast<ID3D11RenderTargetView*>(NULL), NULL, NULL, format, width, height, 1, supportedSamples);
    }
...
DefaultAttachmentImpl *Renderer11::createDefaultAttachment(GLenum type, egl::Surface *surface)
{
    ...
    {
      case GL_BACK:
        return new DefaultAttachmentD3D(new SwapChainRenderTarget11(swapChain, false));

      case GL_DEPTH:
        if (gl::GetInternalFormatInfo(swapChain->GetDepthBufferInternalFormat()).depthBits > 0)
        {
            return new DefaultAttachmentD3D(new SwapChainRenderTarget11(swapChain, true));
        }
        else
        {
            return NULL;
