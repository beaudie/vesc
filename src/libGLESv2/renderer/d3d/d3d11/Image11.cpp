...
gl::Error Image11::copyToStorageImpl(TextureStorage11 *storage11, int level, int layerTarget, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height)
{
    ...
    error = storage11->updateSubresourceLevel(stagingTexture, stagingSubresourceIndex, level, layerTarget,
...
gl::Error Image11::recoverFromAssociatedStorage()
{
    if (mRecoverFromStorage)
    {
        ...
        if (textureStorageCorrect)
        {
            ...
            error = mAssociatedStorage->copySubresourceLevel(mStagingTexture, mStagingSubresource, mAssociatedStorageLevel, mAssociatedStorageLayerTarget, 0, 0, 0, mWidth, mHeight, mDepth);
            if (error.isError())
            {
                return error;
            }
