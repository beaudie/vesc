                   ...
                   GLenum componentType, MipGenerationFunction mipFunc, ColorReadFunction readFunc)
{
    ...
    ASSERT(dxgiToESIter != dxgiToESMap.end());
    info.internalFormat = dxgiToESIter->second;
...
const DXGIFormatInfo &GetDXGIFormatInfo(DXGI_FORMAT format)
{
    ...
    {
        static DXGIFormatInfo defaultInfo = { 0 };
        return defaultInfo;
    }
}
...
typedef std::multimap<GLenum, TypeLoadFunctionPair> D3D11LoadFunctionMap;
...
{
    map->insert(std::make_pair(internalFormat, TypeLoadFunctionPair(type, loadFunc)));
}
                                         ...
                                         DXGI_FORMAT srvFormat, DXGI_FORMAT rtvFormat, DXGI_FORMAT dsvFormat)
{
    ...
    bool fourComp = (formatInfo.componentCount == 4);
    info.swizzleTexFormat = (texFormat == DXGI_FORMAT_UNKNOWN && fourComp) ? swizzleInfo.mTexFormat : texFormat;
    info.swizzleSRVFormat = (srvFormat == DXGI_FORMAT_UNKNOWN && fourComp) ? swizzleInfo.mSRVFormat : srvFormat;
    info.swizzleRTVFormat = (rtvFormat == DXGI_FORMAT_UNKNOWN && fourComp) ? swizzleInfo.mRTVFormat : rtvFormat;
    ...
    std::pair<D3D11LoadFunctionMap::const_iterator, D3D11LoadFunctionMap::const_iterator> loadFunctionIter = loadFunctions.equal_range(internalFormat);
    for (D3D11LoadFunctionMap::const_iterator i = loadFunctionIter.first; i != loadFunctionIter.second; i++)
    {
        info.loadFunctions.insert(std::make_pair(i->second.first, i->second.second));
    }
...
const D3D11FormatInfo &GetD3D11FormatInfo(GLenum internalFormat)
{
    ...
    {
        static const D3D11FormatInfo defaultInfo = { DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN };
        return defaultInfo;
    }
}
...
const D3D11VertexFormatInfo &GetD3D11VertexInfo(const gl::VertexFormat &vertexFormat)
{
    ...
    {
        static const D3D11VertexFormatInfo defaultInfo = { VERTEX_CONVERT_NONE, DXGI_FORMAT_UNKNOWN, NULL };
        return defaultInfo;
    }
}
