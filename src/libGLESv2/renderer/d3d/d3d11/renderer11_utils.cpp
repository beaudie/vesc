...
static gl::TextureCaps GenerateTextureFormatCaps(GLenum internalFormat, ID3D11Device *device)
{
    ...
    const gl_d3d11::D3D11FormatInfo &formatInfo = gl_d3d11::GetD3D11FormatInfo(internalFormat);
    ...
    if (SUCCEEDED(device->CheckFormatSupport(formatInfo.texFormat, &formatSupport)))
    {
        ...
        {
            textureCaps.texture = ((formatSupport & D3D11_FORMAT_SUPPORT_TEXTURE2D) != 0);
        }
        else
        {
            textureCaps.texture = ((formatSupport & D3D11_FORMAT_SUPPORT_TEXTURE2D) != 0) &&
    ...
    textureCaps.filtering = SUCCEEDED(device->CheckFormatSupport(formatInfo.srvFormat, &formatSupport)) &&
                            ((formatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE)) != 0;
    textureCaps.rendering = (SUCCEEDED(device->CheckFormatSupport(formatInfo.rtvFormat, &formatSupport)) &&
...
static size_t GetMaximumBufferSize(D3D_FEATURE_LEVEL featureLevel)
{
...
void GenerateCaps(ID3D11Device *device, gl::Caps *caps, gl::TextureCapsMap *textureCapsMap, gl::Extensions *extensions)
{
    ...
    caps->maxUniformBlockSize = GetMaximumBufferSize(featureLevel);
...
{
    const DXGIFormatInfo &dxgiFormatInfo = d3d11::GetDXGIFormatInfo(format);
...
{
    const gl_d3d11::D3D11FormatInfo &d3dFormatInfo = gl_d3d11::GetD3D11FormatInfo(internalFormat);
    ASSERT(d3dFormatInfo.dataInitializerFunction != NULL);

    const d3d11::DXGIFormatInfo &dxgiFormatInfo = d3d11::GetDXGIFormatInfo(d3dFormatInfo.texFormat);
