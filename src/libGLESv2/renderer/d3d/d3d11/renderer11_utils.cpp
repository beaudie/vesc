...
static gl::TextureCaps GenerateTextureFormatCaps(GLenum internalFormat, ID3D11Device *device)
{
    ...
    textureCaps.filtering = SUCCEEDED(device->CheckFormatSupport(srvFormat, &formatSupport)) &&
                            ((formatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE)) != 0;
    textureCaps.rendering = (SUCCEEDED(device->CheckFormatSupport(rtvFormat, &formatSupport)) &&
