...
UINT TextureStorage11::getSubresourceIndex(const gl::ImageIndex &index) const
{
    ...
    return D3D11CalcSubresource(mipSlice, arraySlice, mMipLevels);
}
      ...
      mSwizzleTexture(NULL)
{
    ...
    mMipLevels = ((levels > 0) ? (mTopLevel + levels) : (gl::log2(std::max(width, height)) + 1));
    mTextureWidth = width;
...
gl::Error TextureStorage11_2D::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == NULL && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        if (d3d11::isDeviceLostError(result))
        {
            ...
            gl::Error(GL_OUT_OF_MEMORY, "Failed to create 2D texture storage, result: 0x%X.", result);
        }
...
gl::Error TextureStorage11_2D::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!mRenderTarget[level])
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[level] = new ResourceRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1);
        ...
        else if (mDepthStencilFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[level] = new ResourceRenderTarget11(dsv, texture, srv, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1);
    ...
    : TextureStorage11(renderer, GetTextureBindFlags(internalformat, renderTarget))
{
    ...
    mMipLevels = ((levels > 0) ? (mTopLevel + levels) : (gl::log2(size) + 1));
    mTextureWidth = size;
    mTextureHeight = size;
    mTextureDepth = 1;
...
gl::Error TextureStorage11_Cube::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == NULL && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        {
            mRenderer->notifyDeviceLost();
            gl::Error(GL_OUT_OF_MEMORY, "Failed to create cube texture storage, result: 0x%X.", result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
...
gl::Error TextureStorage11_Cube::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!mRenderTarget[faceIndex][level])
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[faceIndex][level] = new ResourceRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1);
        ...
        else if (mDepthStencilFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[faceIndex][level] = new ResourceRenderTarget11(dsv, texture, srv, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1);
    ...
    : TextureStorage11(renderer, GetTextureBindFlags(internalformat, renderTarget))
{
    ...
    mMipLevels = ((levels > 0) ? (mTopLevel + levels) : (gl::log2(std::max(std::max(width, height), depth)) + 1));
    mTextureWidth = width;
    mTextureHeight = height;
    mTextureDepth = depth;
...
gl::Error TextureStorage11_3D::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == NULL && mTextureWidth > 0 && mTextureHeight > 0 && mTextureDepth > 0)
    {
        ...
        {
            mRenderer->notifyDeviceLost();
            gl::Error(GL_OUT_OF_MEMORY, "Failed to create 3D texture storage, result: 0x%X.", result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
...
gl::Error TextureStorage11_3D::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!index.hasLayer())
    {
        if (!mLevelRenderTargets[mipLevel])
        {
            ...
            mLevelRenderTargets[mipLevel] = new ResourceRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), getLevelDepth(mipLevel));
    ...
    else
    {
        ...
        if (mLevelLayerRenderTargets.find(key) == mLevelLayerRenderTargets.end())
        {
            ...
            mLevelLayerRenderTargets[key] = new ResourceRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1);
...
gl::Error TextureStorage11_2DArray::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == NULL && mTextureWidth > 0 && mTextureHeight > 0 && mTextureDepth > 0)
    {
        ...
        {
            mRenderer->notifyDeviceLost();
            gl::Error(GL_OUT_OF_MEMORY, "Failed to create 2D array texture storage, result: 0x%X.", result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
...
gl::Error TextureStorage11_2DArray::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (mRenderTargets.find(key) == mRenderTargets.end())
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTargets[key] = new ResourceRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1);
