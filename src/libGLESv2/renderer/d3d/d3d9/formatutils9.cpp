...
{
    DWORD capsFlag;
    D3D9VertexFormat preferredConversion;
    D3D9VertexFormat fallbackConversion;
};
...
D3D9VertexFormat CreateVertexFormatInfo(bool identity, size_t elementSize, VertexCopyFunction copyFunc, D3DDECLTYPE nativeFormat)
{
    D3D9VertexFormat formatInfo;
...
const D3D9VertexFormat &GetD3D9VertexFormatInfo(DWORD supportedDeclTypes, const gl::VertexFormat &vertexFormat)
{
    ...
    static D3D9VertexFormat formatConverters[NUM_GL_VERTEX_ATTRIB_TYPES][2][4];
    if (!initialized)
    {
...
typedef std::pair<GLenum, D3D9Format> D3D9FormatPair;
typedef std::map<GLenum, D3D9Format> D3D9FormatMap;
...
const D3D9Format &GetD3D9FormatInfo(GLenum internalFormat)
{
    ...
    {
        static const D3D9Format defaultInfo;
        return defaultInfo;
    }
}
