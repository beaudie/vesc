                                                 ...
                                                 UINT adapter, D3DFORMAT adapterFormat)
{
    ...
    const gl_d3d9::D3D9FormatInfo &d3dFormatInfo = gl_d3d9::GetD3D9FormatInfo(internalFormat);
    const gl::InternalFormatInfo &formatInfo = gl::GetInternalFormatInfo(internalFormat);
    if (formatInfo.depthBits > 0 || formatInfo.stencilBits > 0)
    {
        textureCaps.texture = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, 0, D3DRTYPE_TEXTURE, d3dFormatInfo.renderFormat));
        textureCaps.filtering = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, D3DUSAGE_QUERY_FILTER, D3DRTYPE_TEXTURE, d3dFormatInfo.renderFormat));
        textureCaps.rendering = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE, d3dFormatInfo.renderFormat)) ||
    ...
    {
        textureCaps.texture = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, 0, D3DRTYPE_TEXTURE, d3dFormatInfo.texFormat)) &&
                              SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, 0, D3DRTYPE_CUBETEXTURE, d3dFormatInfo.texFormat));
        textureCaps.filtering = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, D3DUSAGE_QUERY_FILTER, D3DRTYPE_TEXTURE, d3dFormatInfo.texFormat));
        textureCaps.rendering = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE, d3dFormatInfo.texFormat)) ||
                  ...
                  gl::TextureCapsMap *textureCapsMap, gl::Extensions *extensions)
{
    ...
    caps->maxElementsIndices = deviceCaps.MaxPrimitiveCount;
    caps->maxElementsVertices = deviceCaps.MaxPrimitiveCount;
...
{
    const D3DFormatInfo &d3dFormatInfo = d3d9::GetD3DFormatInfo(format);
