...
void TextureStorage9_2D::generateMipmaps()
{
    ...
    for (int level = 1; level < getLevelCount(); level++)
    {
        ...
        gl::Error error = getSurfaceLevel(level - 1, false, &upper);
        if (error.isError())
        {
            return;
        }
        ...
        error = getSurfaceLevel(level, false, &lower);
        if (error.isError())
        {
            SafeRelease(upper);
            return;
...
void TextureStorage9_Cube::generateMipmaps()
{
    ...
    for (int faceIndex = 0; faceIndex < 6; faceIndex++)
    {
        ...
        {
            IDirect3DSurface9 *upper = NULL;
            gl::Error error = getCubeMapSurface(GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIndex, level - 1, false, &upper);
            if (error.isError())
            {
                return;
            }
            ...
            IDirect3DSurface9 *lower = NULL;
            error = getCubeMapSurface(GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIndex, level, true, &upper);
            if (error.isError())
            {
                SafeRelease(upper);
                return;
