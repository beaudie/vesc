...
SwapChain9::SwapChain9(Renderer9 *renderer, rx::SurfaceHandler handler, HANDLE shareHandle,
...
EGLint SwapChain9::reset(int backbufferWidth, int backbufferHeight, EGLint swapInterval)
{
    ...
    if (!mHandler.getNativeWindow() && mRenderer->getShareHandleSupport())
    {
    ...
    EGLNativeWindowType window = mHandler.getNativeWindow();
    if (window)
    {
