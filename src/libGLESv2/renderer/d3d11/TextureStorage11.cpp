...
ID3D11ShaderResourceView *TextureStorage11_2D::getSRV(const gl::SamplerState &samplerState)
{
    ...
    if (!(*resultSRV))
    {
        ...
        createSRV(0, mipLevels, format, texture, resultSRV);
    }
...
ID3D11ShaderResourceView *TextureStorage11_2D::getSRVLevel(int mipLevel)
{
    if (mipLevel >= 0 && mipLevel < getLevelCount())
    {
        ...
        {
            createSRV(mipLevel, 1, mShaderResourceFormat, mTexture, &mLevelSRVs[mipLevel]);
        }
...
void TextureStorage11_2D::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format, ID3D11Resource *texture, ID3D11ShaderResourceView **SRV)
{
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
    ...
    if (result == E_OUTOFMEMORY)
    {
        gl::error(GL_OUT_OF_MEMORY, static_cast<ID3D11ShaderResourceView*>(NULL));
    }
    ASSERT(SUCCEEDED(result));
}
...
ID3D11ShaderResourceView *TextureStorage11_Cube::getSRV(const gl::SamplerState &samplerState)
{
    ...
    if (!(*resultSRV))
    {
        ...
        DXGI_FORMAT format = (swizzleRequired ? mSwizzleShaderResourceFormat : mShaderResourceFormat);
        ID3D11Texture2D *texture = swizzleRequired ? getSwizzleTexture() : mTexture;
        createSRV(unnormalizedInteger, 0, mipLevels, format, texture, resultSRV);
    }
...
ID3D11ShaderResourceView *TextureStorage11_Cube::getSRVLevel(int mipLevel)
{
    if (mipLevel >= 0 && mipLevel < getLevelCount())
    {
        if (!mLevelSRVs[mipLevel])
        {
            createSRV(true, mipLevel, 1, mShaderResourceFormat, mTexture, &mLevelSRVs[mipLevel]);
        }
...
void TextureStorage11_Cube::createSRV(bool array, int baseLevel, int mipLevels, DXGI_FORMAT format, ID3D11Resource *texture, ID3D11ShaderResourceView **SRV)
{
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
    srvDesc.Format = format;
...
void TextureStorage11_3D::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format, ID3D11Resource *texture, ID3D11ShaderResourceView **SRV)
{
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
    srvDesc.Format = format;
...
void TextureStorage11_2DArray::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format, ID3D11Resource *texture, ID3D11ShaderResourceView **SRV)
{
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
    srvDesc.Format = format;
    srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
