...
ID3D11BlendState *RenderStateCache::getBlendState(const gl::BlendState &blendState, const gl::Framebuffer *framebuffer)
{
    if (!mDevice)
    {
    ...
    GLuint clientVersion = framebuffer->getRenderer()->getCurrentClientVersion();
    bool mrt = false;
    ...
    for (unsigned int i = 0; i < D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT; i++)
    {
        ...
        if (renderBuffer)
        {
            ...
            key.rtChannels[i][0] = gl::GetRedBits(internalFormat, clientVersion) > 0;
            key.rtChannels[i][1] = gl::GetGreenBits(internalFormat, clientVersion) > 0;
            key.rtChannels[i][2] = gl::GetBlueBits(internalFormat, clientVersion) > 0;
            key.rtChannels[i][3] = gl::GetAlphaBits(internalFormat, clientVersion) > 0;
        }
        else
        {
