                             ...
                             bool ignoreViewport)
{
    ...
    const std::pair<int, int> viewportRange = d3d11::GetViewportRange(mFeatureLevel);

    // Clamp width and height first to the gl maximum, then clamp further if we extend past the D3D maximum bounds
    D3D11_VIEWPORT dxViewport;
    dxViewport.TopLeftX = gl::clamp(actualViewport.x, viewportRange.first, viewportRange.second);
    dxViewport.TopLeftY = gl::clamp(actualViewport.y, viewportRange.first, viewportRange.second);
