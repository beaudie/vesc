                                 ...
                                 GLint packAlignment, void *pixels)
{
    ...
    int negativeX = -std::min(area.x, 0);
    int negativeY = -std::min(area.y, 0);

    gl::Rectangle safeArea;
    safeArea.x = std::min(area.x + negativeX, static_cast<int>(textureDesc.Width));
    safeArea.y = std::min(area.y + negativeY, static_cast<int>(textureDesc.Height));
    safeArea.width  = std::min(area.width  - negativeX, static_cast<int>(textureDesc.Width)  - safeArea.x);
    safeArea.height = std::min(area.height - negativeY, static_cast<int>(textureDesc.Height) - safeArea.y);

    ASSERT(safeArea.x >= 0 && safeArea.y >= 0);
    ASSERT(safeArea.width <= static_cast<int>(textureDesc.Width) && safeArea.height <= static_cast<int>(textureDesc.Height));
