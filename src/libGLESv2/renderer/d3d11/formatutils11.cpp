...
void addDXGIFormat(DXGIFormatInfoMap *map, DXGI_FORMAT dxgiFormat, GLuint pixelBits, GLuint blockWidth, GLuint blockHeight,
                   MipGenerationFunction mipFunc, ColorReadFunction readFunc)
{
    map->insert(DXGIFormatInfoPair(dxgiFormat, DXGIFormatInfo(pixelBits, blockWidth, blockHeight, mipFunc, readFunc)));
}
...
static DXGIFormatInfoMap BuildDXGIFormatInfoMap()
{
    ...
    addDXGIFormat(&map, DXGI_FORMAT_R16_TYPELESS,             16,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R16_UNORM,                16,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_D16_UNORM,                16,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R24G8_TYPELESS,           32,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R24_UNORM_X8_TYPELESS,    32,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_D24_UNORM_S8_UINT,        32,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R32G8X24_TYPELESS,        64,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS, 64,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_D32_FLOAT_S8X24_UINT,     64,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_R32_TYPELESS,             32,  1, 1, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_D32_FLOAT,                32,  1, 1, NULL,                       NULL);

    addDXGIFormat(&map, DXGI_FORMAT_BC1_UNORM,                64,  4, 4, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_BC2_UNORM,                128, 4, 4, NULL,                       NULL);
    addDXGIFormat(&map, DXGI_FORMAT_BC3_UNORM,                128, 4, 4, NULL,                       NULL);

    return map;
}

typedef std::map<DXGI_FORMAT, GLenum> DXGIToGLFormatMap;

static DXGIToGLFormatMap BuildCommonDXGIToESFormatMap()
{
    DXGIToGLFormatMap map;

    map[DXGI_FORMAT_UNKNOWN]                    = GL_NONE;
    map[DXGI_FORMAT_A8_UNORM]                   = GL_ALPHA8_EXT;
    map[DXGI_FORMAT_R8_UNORM]                   = GL_R8;
    map[DXGI_FORMAT_R8G8_UNORM]                 = GL_RG8;
    map[DXGI_FORMAT_R8G8B8A8_UNORM]             = GL_RGBA8;
    map[DXGI_FORMAT_R8G8B8A8_UNORM_SRGB]        = GL_SRGB8_ALPHA8;
    map[DXGI_FORMAT_B8G8R8A8_UNORM]             = GL_BGRA8_EXT;

    map[DXGI_FORMAT_R8_SNORM]                   = GL_R8_SNORM;
    map[DXGI_FORMAT_R8G8_SNORM]                 = GL_RG8_SNORM;
    map[DXGI_FORMAT_R8G8B8A8_SNORM]             = GL_RGBA8_SNORM;

    map[DXGI_FORMAT_R8_UINT]                    = GL_R8UI;
    map[DXGI_FORMAT_R16_UINT]                   = GL_R16UI;
    map[DXGI_FORMAT_R32_UINT]                   = GL_R32UI;
    map[DXGI_FORMAT_R8G8_UINT]                  = GL_RG8UI;
    map[DXGI_FORMAT_R16G16_UINT]                = GL_RG16UI;
    map[DXGI_FORMAT_R32G32_UINT]                = GL_RG32UI;
    map[DXGI_FORMAT_R32G32B32_UINT]             = GL_RGB32UI;
    map[DXGI_FORMAT_R8G8B8A8_UINT]              = GL_RGBA8UI;
    map[DXGI_FORMAT_R16G16B16A16_UINT]          = GL_RGBA16UI;
    map[DXGI_FORMAT_R32G32B32A32_UINT]          = GL_RGBA32UI;

    map[DXGI_FORMAT_R8_SINT]                    = GL_R8I;
    map[DXGI_FORMAT_R16_SINT]                   = GL_R16I;
    map[DXGI_FORMAT_R32_SINT]                   = GL_R32I;
    map[DXGI_FORMAT_R8G8_SINT]                  = GL_RG8I;
    map[DXGI_FORMAT_R16G16_SINT]                = GL_RG16I;
    map[DXGI_FORMAT_R32G32_SINT]                = GL_RG32I;
    map[DXGI_FORMAT_R32G32B32_SINT]             = GL_RGB32I;
    map[DXGI_FORMAT_R8G8B8A8_SINT]              = GL_RGBA8I;
    map[DXGI_FORMAT_R16G16B16A16_SINT]          = GL_RGBA16I;
    map[DXGI_FORMAT_R32G32B32A32_SINT]          = GL_RGBA32I;

    map[DXGI_FORMAT_R10G10B10A2_UNORM]          = GL_RGB10_A2;
    map[DXGI_FORMAT_R10G10B10A2_UINT]           = GL_RGB10_A2UI;

    map[DXGI_FORMAT_R16_FLOAT]                  = GL_R16F;
    map[DXGI_FORMAT_R16G16_FLOAT]               = GL_RG16F;
    map[DXGI_FORMAT_R16G16B16A16_FLOAT]         = GL_RGBA16F;

    map[DXGI_FORMAT_R32_FLOAT]                  = GL_R32F;
    map[DXGI_FORMAT_R32G32_FLOAT]               = GL_RG32F;
    map[DXGI_FORMAT_R32G32B32_FLOAT]            = GL_RGB32F;
    map[DXGI_FORMAT_R32G32B32A32_FLOAT]         = GL_RGBA32F;

    map[DXGI_FORMAT_R9G9B9E5_SHAREDEXP]         = GL_RGB9_E5;
    map[DXGI_FORMAT_R11G11B10_FLOAT]            = GL_R11F_G11F_B10F;

    map[DXGI_FORMAT_R16_TYPELESS]               = GL_DEPTH_COMPONENT16;
    map[DXGI_FORMAT_R16_UNORM]                  = GL_DEPTH_COMPONENT16;
    map[DXGI_FORMAT_D16_UNORM]                  = GL_DEPTH_COMPONENT16;
    map[DXGI_FORMAT_R24G8_TYPELESS]             = GL_DEPTH24_STENCIL8_OES;
    map[DXGI_FORMAT_R24_UNORM_X8_TYPELESS]      = GL_DEPTH24_STENCIL8_OES;
    map[DXGI_FORMAT_D24_UNORM_S8_UINT]          = GL_DEPTH24_STENCIL8_OES;
    map[DXGI_FORMAT_R32G8X24_TYPELESS]          = GL_DEPTH32F_STENCIL8;
    map[DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS]   = GL_DEPTH32F_STENCIL8;
    map[DXGI_FORMAT_D32_FLOAT_S8X24_UINT]       = GL_DEPTH32F_STENCIL8;

    map[DXGI_FORMAT_BC1_UNORM]                  = GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
    map[DXGI_FORMAT_BC2_UNORM]                  = GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE;
    map[DXGI_FORMAT_BC3_UNORM]                  = GL_COMPRESSED_RGBA_S3TC_DXT5_ANGLE;

    return map;
}

static DXGIToGLFormatMap BuildDXGIToES2FormatMap()
{
    DXGIToGLFormatMap map = BuildCommonDXGIToESFormatMap();

    map[DXGI_FORMAT_R32_TYPELESS] = GL_DEPTH_COMPONENT32_OES;
    map[DXGI_FORMAT_D32_FLOAT]    = GL_DEPTH_COMPONENT32_OES;

    return map;
}

static DXGIToGLFormatMap BuildDXGIToES3FormatMap()
{
    DXGIToGLFormatMap map = BuildCommonDXGIToESFormatMap();

    map[DXGI_FORMAT_R32_TYPELESS] = GL_DEPTH_COMPONENT32F;
    map[DXGI_FORMAT_D32_FLOAT]    = GL_DEPTH_COMPONENT32F;
...
GLenum GetInternalFormat(DXGI_FORMAT format, GLuint clientVersion)
{
    if (clientVersion == 2)
    {
        static DXGIToGLFormatMap es2FormatMap = BuildDXGIToES2FormatMap();
    ...
    {
        static DXGIToGLFormatMap es3FormatMap = BuildDXGIToES3FormatMap();
