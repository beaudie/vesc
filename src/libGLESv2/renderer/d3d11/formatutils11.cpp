...
static const SwizzleFormatInfo GetSwizzleFormatInfo(GLint internalFormat, GLuint clientVersion)
{
    ...
    {
        maxBits = std::max((gl::GetPixelBytes(internalFormat, clientVersion) * 8) /
                           (gl::GetCompressedBlockWidth(internalFormat, clientVersion) * gl::GetCompressedBlockHeight(internalFormat, clientVersion)),
                           maxBits);
    }
    else
    {
