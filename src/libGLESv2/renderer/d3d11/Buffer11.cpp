...
// Each instance of Buffer11::TypedBuffer11 is specialized for a class of D3D binding points
// - vertex/transform feedback buffers
// - index buffers
// - pixel unpack buffers
// - uniform buffers
class Buffer11::TypedBuffer11
{
  public:
    virtual ~TypedBuffer11() {}
    ...
    virtual bool copyFromStorage(TypedBuffer11 *source, size_t sourceOffset,
  ...
  protected:
    TypedBuffer11(Renderer11 *renderer, BufferUsage usage);
...
// type of storage.
class Buffer11::NativeBuffer11 : public Buffer11::TypedBuffer11
{
  public:
...
class Buffer11::PackStorage11 : public Buffer11::TypedBuffer11
{
  public:
...
Buffer11::Buffer11(rx::Renderer11 *renderer)
    : BufferD3D(),
      mRenderer(renderer),
      mSize(0),
      mMappedStorage(NULL),
...
void Buffer11::copySubData(BufferImpl* source, GLintptr sourceOffset, GLintptr destOffset, GLsizeiptr size)
{
    ...
    if (sourceBuffer)
    {
        TypedBuffer11 *dest = getLatestTypedBuffer();
        if (!dest)
        {
            dest = getStagingBuffer();
        }

        TypedBuffer11 *source = sourceBuffer->getLatestTypedBuffer();
        if (source && dest)
        {
...
GLvoid *Buffer11::map(size_t offset, size_t length, GLbitfield access)
{
    ...
    TypedBuffer11 *latestStorage = getLatestTypedBuffer();
    if (latestStorage &&
...
{
    TypedBuffer11 *transformFeedbackStorage = getTypedBuffer(BUFFER_USAGE_VERTEX_OR_TRANSFORM_FEEDBACK);
...
ID3D11Buffer *Buffer11::getBuffer(BufferUsage usage)
{
    ...
    TypedBuffer11 *typedBuffer = getTypedBuffer(usage);
...
{
    TypedBuffer11 *storage = getTypedBuffer(BUFFER_USAGE_PIXEL_UNPACK);
...
Buffer11::TypedBuffer11 *Buffer11::getTypedBuffer(BufferUsage usage)
{
    TypedBuffer11 *directBuffer = NULL;
    ...
    TypedBuffer11 *latestBuffer = getLatestTypedBuffer();
...
Buffer11::TypedBuffer11 *Buffer11::getLatestTypedBuffer() const
{
    // Even though we iterate over all the direct buffers, it is expected that only
    // 1 or 2 will be present.
    TypedBuffer11 *latestStorage = NULL;
    DataRevision latestRevision = 0;
    for (auto it = mTypedBuffers.begin(); it != mTypedBuffers.end(); it++)
    {
        TypedBuffer11 *storage = it->second;
...
{
    TypedBuffer11 *stagingStorage = getTypedBuffer(BUFFER_USAGE_STAGING);
...
{
    TypedBuffer11 *packStorage = getTypedBuffer(BUFFER_USAGE_PIXEL_PACK);
...
Buffer11::TypedBuffer11::TypedBuffer11(Renderer11 *renderer, BufferUsage usage)
    : mRenderer(renderer),
      mUsage(usage),
      mRevision(0),
      mBufferSize(0)
...
bool Buffer11::NativeBuffer11::copyFromStorage(TypedBuffer11 *source, size_t sourceOffset,
                                                      size_t size, size_t destOffset)
{
    ID3D11DeviceContext *context = mRenderer->getDeviceContext();
    ...
    if (source->getUsage() == BUFFER_USAGE_PIXEL_PACK)
    {
        ...
        unsigned char *sourcePointer = static_cast<unsigned char *>(source->map(sourceOffset, size, GL_MAP_READ_BIT));
...
Buffer11::PackStorage11::PackStorage11(Renderer11 *renderer)
    : TypedBuffer11(renderer, BUFFER_USAGE_PIXEL_PACK),
...
bool Buffer11::PackStorage11::copyFromStorage(TypedBuffer11 *source, size_t sourceOffset,
                                                     size_t size, size_t destOffset)
{
    UNIMPLEMENTED();
    return false;
...
void *Buffer11::PackStorage11::map(size_t offset, size_t length, GLbitfield access)
{
    ...
    return static_cast<GLubyte*>(&mMemoryBuffer[0]) + offset;
}
