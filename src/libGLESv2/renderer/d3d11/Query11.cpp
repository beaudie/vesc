...
static bool checkOcculsionQuery(ID3D11DeviceContext *context, ID3D11Query *query, UINT64 *numPixels)
{
    ...
    if (mQuery != NULL && mStatus != GL_TRUE)
    {
        ...
        switch (getType())
        {
          ...
          case GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT:
            {
                UINT64 numPixels = 0;
                queryFinished = checkOcculsionQuery(context, mQuery, &numPixels);
                if (queryFinished)
                {
