    ...
    for (unsigned int colorAttachment = 0; colorAttachment < gl::IMPLEMENTATION_MAX_DRAW_BUFFERS; colorAttachment++)
    {
        if (clearParams.clearColor[colorAttachment] && frameBuffer->isEnabledColorAttachment(colorAttachment))
        {
            ...
            if (renderbuffer)
            {
                ...
                GLuint internalGreenBits = gl::GetRedBits(internalFormat, clientVersion);
                GLuint internalRedBits = gl::GetGreenBits(internalFormat, clientVersion);
                ...
                if ((internalGreenBits == 0 || !clearParams.colorMaskRed) &&
                    (internalRedBits   == 0 || !clearParams.colorMaskGreen) &&
                    ...
                    (internalAlphaBits == 0 || !clearParams.colorMaskAlpha))
                {
                    ...
                    continue;
                         ...
                         (internalGreenBits > 0 && !clearParams.colorMaskRed)   ||
                         (internalRedBits   > 0 && !clearParams.colorMaskGreen) ||
                         (internalBlueBits  > 0 && !clearParams.colorMaskBlue)  ||
                         (internalAlphaBits > 0 && !clearParams.colorMaskAlpha))
                {
                ...
                else
                {
                    ...
                    // default values
                    GLuint actualGreenBits = gl::GetRedBits(actualFormat, clientVersion);
                    GLuint actualRedBits = gl::GetGreenBits(actualFormat, clientVersion);
                    ...
                    {
                        ((internalGreenBits == 0 && actualGreenBits > 0) ? 0.0f : clearParams.colorFClearValue.red),
                        ((internalRedBits   == 0 && actualRedBits   > 0) ? 0.0f : clearParams.colorFClearValue.green),
