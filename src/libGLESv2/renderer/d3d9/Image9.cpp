...
void Image9::createSurface()
{
    ...
    if (mWidth != 0 && mHeight != 0)
    {
        ...
        if (gl_d3d9::RequiresTextureDataInitialization(mInternalFormat))
        {
            ...
            initializeFunc(mWidth, mHeight, 1, reinterpret_cast<byte*>(lockedRect.pBits), lockedRect.Pitch, 0);
                      ...
                      GLint unpackAlignment, GLenum type, const void *input)
{
    ...
    loadFunction(width, height, depth,
                 reinterpret_cast<const byte*>(input), inputRowPitch, 0,
                 reinterpret_cast<byte*>(locked.pBits), locked.Pitch, 0);
