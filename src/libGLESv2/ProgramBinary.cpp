...
static inline void SetIfDirty(T *dest, const T& source, bool *dirtyFlag)
{
    ...
    *dirtyFlag = (memcmp(dest, &source, sizeof(T)) != 0);
    *dest = source;
}
...
bool ProgramBinary::setUniformMatrixfv(GLint location, GLsizei count, GLboolean transpose, const GLfloat *value, GLenum targetUniformType) 
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            targetUniform->dirty = targetUniform->dirty | transposeMatrix<GLfloat>(target, value, 4, rows, rows, cols);
        }
        else
        {
            targetUniform->dirty = targetUniform->dirty | expandMatrix<GLfloat>(target, value, 4, rows, cols, rows);
        }
