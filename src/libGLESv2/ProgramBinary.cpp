...
rx::ShaderExecutable *ProgramBinary::getVertexExecutableForInputLayout(const VertexFormat inputLayout[gl::MAX_VERTEX_ATTRIBS])
{
    ...
    InfoLog dummyInfoLog;
    rx::ShaderExecutable *vertexExecutable = mRenderer->compileToExecutable(dummyInfoLog, mVertexHLSL.c_str(), rx::SHADER_VERTEX, mVertexWorkarounds);

    if (!vertexExecutable)
    {
        char *buffer = new char[dummyInfoLog.getLength()+3];
        GLsizei length;
        dummyInfoLog.getLog(dummyInfoLog.getLength(), &length, buffer);
...
bool ProgramBinary::load(InfoLog &infoLog, const void *binary, GLsizei length)
{
    ...
    for (unsigned int vertexShaderIndex = 0; vertexShaderIndex < vertexShaderCount; vertexShaderIndex++)
    {
        ...
        stream.read(reinterpret_cast<unsigned char *>(&vertexInputs[0]), gl::MAX_VERTEX_ATTRIBS * sizeof(VertexFormat));
...
bool ProgramBinary::save(void* binary, GLsizei bufSize, GLsizei *length)
{
    ...
    for (size_t vertexExecutableIndex = 0; vertexExecutableIndex < mVertexExecutables.size(); vertexExecutableIndex++)
    {
        ...
        stream.write(reinterpret_cast<const unsigned char *>(vertexExecutable->inputs()), gl::MAX_VERTEX_ATTRIBS * sizeof(VertexFormat));
