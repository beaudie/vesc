...
#define DELEGATE_SET_UNIFORM_TO_IMPL(name, type)                         \
void ProgramBinary::name(GLint location, GLsizei count, const type *v) { \
    mProgram->name(location, count, v);                                  \
}

#define DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(name)                                                \
void ProgramBinary::name(GLint location, GLsizei count, GLboolean transpose, const GLfloat *v) { \
    mProgram->name(location, count, transpose, v);                                               \
}

#define DELEGATE_GET_UNIFORM_TO_IMPL(name, type)    \
void ProgramBinary::name(GLint location, type *v) { \
    mProgram->name(location, v);                    \
}
...
GLint ProgramBinary::getUniformLocation(std::string name)
{
    ...
    unsigned int numUniforms = mProgram->getUniformIndices().size();
    for (unsigned int location = 0; location < numUniforms; location++)
    {
        if (mProgram->getUniformIndices()[location].name == name)
        {
            const int index = mProgram->getUniformIndices()[location].index;
            const bool isArray = mProgram->getUniforms()[index]->isArray();

            if ((isArray && mProgram->getUniformIndices()[location].element == subscript) ||
...
GLuint ProgramBinary::getUniformIndex(std::string name)
{
    ...
    {
        if (mProgram->getUniforms()[index]->name == name)
        {
            if (mProgram->getUniforms()[index]->isArray() || subscript == GL_INVALID_INDEX)
            {
                return index;
            }
        }
...
GLuint ProgramBinary::getUniformBlockIndex(std::string name)
{
    ...
    {
        const UniformBlock &uniformBlock = *mProgram->getUniformBlocks()[blockIndex];
...
{
    ASSERT(blockIndex < mProgram->getUniformBlocks().size());
    return mProgram->getUniformBlocks()[blockIndex];
}
...
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform1fv, GLfloat);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform2fv, GLfloat);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform3fv, GLfloat);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform4fv, GLfloat);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform1iv, GLint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform2iv, GLint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform3iv, GLint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform4iv, GLint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform1uiv, GLuint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform2uiv, GLuint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform3uiv, GLuint);
DELEGATE_SET_UNIFORM_TO_IMPL(setUniform4uiv, GLuint);

DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix2fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix3fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix4fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix2x3fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix2x4fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix3x2fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix3x4fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix4x2fv);
DELEGATE_SET_UNIFORM_MATRIX_TO_IMPL(setUniformMatrix4x3fv);

DELEGATE_GET_UNIFORM_TO_IMPL(getUniformfv, GLfloat);
DELEGATE_GET_UNIFORM_TO_IMPL(getUniformiv, GLint);
DELEGATE_GET_UNIFORM_TO_IMPL(getUniformuiv, GLuint);
...
Error ProgramBinary::applyUniforms()
{
    ...
    Error error = mProgram->applyUniforms(mProgram->getUniforms());
    if (error.isError())
    {
        return error;
    }

    for (size_t uniformIndex = 0; uniformIndex < mProgram->getUniforms().size(); uniformIndex++)
    {
        mProgram->getUniforms()[uniformIndex]->dirty = false;
    }
                         ...
                         const std::vector<std::string>& transformFeedbackVaryings, GLenum transformFeedbackBufferMode, const Caps &caps)
{
    ...
    if (!mProgram->linkUniformBlocks(infoLog, *vertexShader, *fragmentShader, caps))
    {
        success = false;
    }
...
LinkedUniform *ProgramBinary::getUniformByName(const std::string &name) const
{
    for (size_t uniformIndex = 0; uniformIndex < mProgram->getUniforms().size(); uniformIndex++)
    {
        if (mProgram->getUniforms()[uniformIndex]->name == name)
        {
            return mProgram->getUniforms()[uniformIndex];
        }
    }
