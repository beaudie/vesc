...
ProgramBinary::ProgramBinary(rx::ProgramBinaryImpl *impl)
    : RefCountObject(0),
      mProgramBinary(impl),
...
rx::ShaderExecutable *ProgramBinary::getPixelExecutableForOutputLayout(const std::vector<GLenum> &outputSignature)
{
    ...
    std::string finalPixelHLSL = mProgramBinary->getDynamicHLSL()->generatePixelShaderForOutputSignature(
            mPixelHLSL, mPixelShaderKey, mUsesFragDepth, outputSignature);

    // Generate new pixel executable
    InfoLog tempInfoLog;
    rx::ShaderExecutable *pixelExecutable = mProgramBinary->getRenderer()->compileToExecutable(
            tempInfoLog, finalPixelHLSL.c_str(), rx::SHADER_PIXEL, mTransformFeedbackLinkedVaryings,
            (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS), mPixelWorkarounds);
...
rx::ShaderExecutable *ProgramBinary::getVertexExecutableForInputLayout(const VertexFormat inputLayout[MAX_VERTEX_ATTRIBS])
{
    ...
    mProgramBinary->getDynamicHLSL()->getInputLayoutSignature(inputLayout, signature);
    ...
    std::string finalVertexHLSL = mProgramBinary->getDynamicHLSL()->generateVertexShaderForInputLayout(mVertexHLSL, inputLayout, mShaderAttributes);

    // Generate new vertex executable
    InfoLog tempInfoLog;
    rx::ShaderExecutable *vertexExecutable = mProgramBinary->getRenderer()->compileToExecutable(
            tempInfoLog, finalVertexHLSL.c_str(), rx::SHADER_VERTEX, mTransformFeedbackLinkedVaryings,
            (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS), mVertexWorkarounds);
...
{
    return mUsesPointSize && mProgramBinary->getRenderer()->getMajorShaderModel() >= 4;
}
...
void ProgramBinary::applyUniforms()
{
    ...
    mProgramBinary->getRenderer()->applyUniforms(*this);
...
bool ProgramBinary::applyUniformBuffers(const std::vector<gl::Buffer*> boundBuffers, const Caps &caps)
{
    ...
    const unsigned int reservedBuffersInVS = mProgramBinary->getRenderer()->getReservedVertexUniformBuffers();
    const unsigned int reservedBuffersInFS = mProgramBinary->getRenderer()->getReservedFragmentUniformBuffers();
    ...
    return mProgramBinary->getRenderer()->setUniformBuffers(vertexUniformBuffers, fragmentUniformBuffers);
}
...
bool ProgramBinary::load(InfoLog &infoLog, GLenum binaryFormat, const void *binary, GLsizei length)
{
    ...
    for (unsigned int vertexShaderIndex = 0; vertexShaderIndex < vertexShaderCount; vertexShaderIndex++)
    {
        ...
        rx::ShaderExecutable *shaderExecutable = mProgramBinary->getRenderer()->loadExecutable(reinterpret_cast<const DWORD*>(vertexShaderFunction),
    ...
    for (size_t pixelShaderIndex = 0; pixelShaderIndex < pixelShaderCount; pixelShaderIndex++)
    {
        ...
        rx::ShaderExecutable *shaderExecutable = mProgramBinary->getRenderer()->loadExecutable(
                pixelShaderFunction, pixelShaderSize, rx::SHADER_PIXEL, mTransformFeedbackLinkedVaryings,
                (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS));
    ...
    if (geometryShaderSize > 0)
    {
        ...
        mGeometryExecutable = mProgramBinary->getRenderer()->loadExecutable(
                reinterpret_cast<const DWORD*>(geometryShaderFunction),  geometryShaderSize, rx::SHADER_GEOMETRY,
    ...
    GUID identifier = mProgramBinary->getRenderer()->getAdapterIdentifier();
    ...
    mProgramBinary->initializeUniformStorage(mUniforms);
                         ...
                         const std::vector<std::string>& transformFeedbackVaryings, GLenum transformFeedbackBufferMode, const Caps &caps)
{
    ...
    int registers = mProgramBinary->getDynamicHLSL()->packVaryings(infoLog, packing, fragmentShaderD3D, vertexShaderD3D, transformFeedbackVaryings);
    ...
    if (!mProgramBinary->getDynamicHLSL()->generateShaderLinkHLSL(
            infoLog, registers, packing, mPixelHLSL, mVertexHLSL, fragmentShaderD3D, vertexShaderD3D,
            transformFeedbackVaryings, &linkedVaryings, &mOutputVariables, &mPixelShaderKey, &mUsesFragDepth))
    {
        return false;
    }
    ...
    if (success)
    {
        ...
        {
            std::string geometryHLSL = mProgramBinary->getDynamicHLSL()->generateGeometryShaderHLSL(registers, fragmentShaderD3D, vertexShaderD3D);
            mGeometryExecutable = mProgramBinary->getRenderer()->compileToExecutable(
                    infoLog, geometryHLSL.c_str(), rx::SHADER_GEOMETRY, mTransformFeedbackLinkedVaryings,
                    (mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS), rx::ANGLE_D3D_WORKAROUND_NONE);
        }
...
bool ProgramBinary::indexSamplerUniform(const LinkedUniform &uniform, InfoLog &infoLog, const Caps &caps)
{
    ...
    if (uniform.vsRegisterIndex != GL_INVALID_INDEX)
    {
        ...
        unsigned int maxVertexVectors = mProgramBinary->getRenderer()->getReservedVertexUniformVectors() + caps.maxVertexUniformVectors;
    ...
    if (uniform.psRegisterIndex != GL_INVALID_INDEX)
    {
        ...
        unsigned int maxFragmentVectors = mProgramBinary->getRenderer()->getReservedFragmentUniformVectors() + caps.maxFragmentUniformVectors;
...
bool ProgramBinary::assignUniformBlockRegister(InfoLog &infoLog, UniformBlock *uniformBlock, GLenum shader, unsigned int registerIndex, const Caps &caps)
{
    if (shader == GL_VERTEX_SHADER)
    {
        ...
        if (registerIndex - mProgramBinary->getRenderer()->getReservedVertexUniformBuffers() >= caps.maxVertexUniformBlocks)
        {
    ...
    else if (shader == GL_FRAGMENT_SHADER)
    {
        ...
        if (registerIndex - mProgramBinary->getRenderer()->getReservedFragmentUniformBuffers() >= caps.maxFragmentUniformBlocks)
        {
