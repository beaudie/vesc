...
bool Texture2D::isSamplerComplete(const SamplerState &samplerState, const Context &context) const
{
    ...
    if (!context.getTextureCaps().get(getInternalFormat(0)).filterable)
    {
    ...
    bool npotSupport = context.getExtensions().textureNPOT;
    ...
    if (formatInfo.depthBits > 0 && context.getClientVersion() > 2)
    {
...
bool TextureCubeMap::isSamplerComplete(const SamplerState &samplerState, const Context &context) const
{
    int size = getBaseLevelWidth();

    bool mipmapping = IsMipmapFiltered(samplerState);

    if (!context.getTextureCaps().get(getInternalFormat(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0)).filterable)
    {
        if (samplerState.magFilter != GL_NEAREST ||
            (samplerState.minFilter != GL_NEAREST && samplerState.minFilter != GL_NEAREST_MIPMAP_NEAREST))
        {
    ...
    if (!gl::isPow2(size) && !context.getExtensions().textureNPOT)
    {
...
bool Texture3D::isSamplerComplete(const SamplerState &samplerState, const Context &context) const
{
    GLsizei width = getBaseLevelWidth();
    GLsizei height = getBaseLevelHeight();
...
bool Texture2DArray::isSamplerComplete(const SamplerState &samplerState, const Context &context) const
{
    GLsizei width = getBaseLevelWidth();
    GLsizei height = getBaseLevelHeight();
    ...
    if (!context.getTextureCaps().get(getBaseLevelInternalFormat()).filterable)
    {
        if (samplerState.magFilter != GL_NEAREST ||
            (samplerState.minFilter != GL_NEAREST && samplerState.minFilter != GL_NEAREST_MIPMAP_NEAREST))
        {
