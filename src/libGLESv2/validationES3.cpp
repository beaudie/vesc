                                   ...
                                   GLint border, GLenum format, GLenum type, const GLvoid *pixels)
{
    ...
    switch (target)
    {
      ...
      case GL_TEXTURE_3D:
        {
            if (static_cast<GLuint>(width) > (caps.getMax3DTextureSize() >> level) ||
                static_cast<GLuint>(height) > (caps.getMax3DTextureSize() >> level) ||
                static_cast<GLuint>(depth) > (caps.getMax3DTextureSize() >> level))
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
        ...
        case GL_TEXTURE_2D_ARRAY:
          {
              if (static_cast<GLuint>(width) > (caps.getMax2DTextureSize() >> level) ||
                  static_cast<GLuint>(height) > (caps.getMax2DTextureSize() >> level) ||
                  static_cast<GLuint>(depth) > (caps.getMaxArrayTextureLayers() >> level))
              {
                  return gl::error(GL_INVALID_VALUE, false);
              }
    ...
    {
        if (!gl::IsValidInternalFormat(actualInternalFormat, context->getCaps(), context->getClientVersion()) ||
            !gl::IsValidFormat(format, context->getCaps(), context->getClientVersion()) ||
            !gl::IsValidType(type, context->getCaps(), context->getClientVersion()))
        {
            return gl::error(GL_INVALID_ENUM, false);
        }

        if (!gl::IsValidFormatCombination(actualInternalFormat, format, type, context->getCaps(), context->getClientVersion()))
        {
            return gl::error(GL_INVALID_OPERATION, false);
        }
    ...
    if (pixelUnpackBuffer != NULL)
    {
        ...
        size_t pixelBytes = static_cast<size_t>(gl::GetPixelBytes(actualInternalFormat));
                                     ...
                                     GLsizei width, GLsizei height, GLsizei depth)
{
      ...
      case GL_TEXTURE_3D:
        {
            ...
            if (static_cast<GLuint>(width) > caps.getMax3DTextureSize() ||
                static_cast<GLuint>(height) > caps.getMax3DTextureSize() ||
                static_cast<GLuint>(depth) > caps.getMax3DTextureSize())
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
        }
      ...
      case GL_TEXTURE_2D_ARRAY:
        {
            ...
            if (static_cast<GLuint>(width) > caps.getMax2DTextureSize() ||
                static_cast<GLuint>(height) > caps.getMax2DTextureSize() ||
                static_cast<GLuint>(depth) > caps.getMaxArrayTextureLayers())
            {
                return gl::error(GL_INVALID_VALUE, false);
            }
        }
    ...
    if (!gl::IsValidInternalFormat(internalformat, context->getCaps(), context->getClientVersion()))
    {
        return gl::error(GL_INVALID_ENUM, false);
    }
                                             ...
                                             bool layerCall)
{
    ...
    if (attachment >= GL_COLOR_ATTACHMENT0 && attachment <= GL_COLOR_ATTACHMENT15)
    {
        ...
        if (colorAttachment >= caps.getMaxColorAttachments())
        {
            return gl::error(GL_INVALID_VALUE, false);
        }
    }
    ...
    if (texture != 0)
    {
        ...
        if (!layerCall)
        {
            switch (textarget)
            {
              case GL_TEXTURE_2D:
                {
                    if (level > gl::log2(caps.getMax2DTextureSize()))
                    {
                        return gl::error(GL_INVALID_VALUE, false);
                    }
              ...
              case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z:
                {
                    if (level > gl::log2(caps.getMaxCubeMapTextureSize()))
                    {
                        return gl::error(GL_INVALID_VALUE, false);
                    }
        ...
        else
        {
            switch (tex->getTarget())
            {
              case GL_TEXTURE_2D_ARRAY:
                {
                    ...
                    if (static_cast<GLuint>(layer) >= caps.getMaxArrayTextureLayers())
                    {
                        return gl::error(GL_INVALID_VALUE, false);
                    }
              ...
              case GL_TEXTURE_3D:
                {
                    if (level > gl::log2(caps.getMax3DTextureSize()))
                    {
                        return gl::error(GL_INVALID_VALUE, false);
                    }

                    if (static_cast<GLuint>(layer) >= caps.getMax3DTextureSize())
                    {
                        return gl::error(GL_INVALID_VALUE, false);
                    }
                                             ...
                                             const GLenum* attachments)
{
    ...
    for (int i = 0; i < numAttachments; ++i)
    {
        if (attachments[i] >= GL_COLOR_ATTACHMENT0 && attachments[i] <= GL_COLOR_ATTACHMENT15)
        {
            ...
            if (attachments[i] >= GL_COLOR_ATTACHMENT0 + context->getCaps().getMaxColorAttachments())
            {
                return gl::error(GL_INVALID_OPERATION, false);
            }
        }
