...
static bool ValidateTexImageFormatCombination(gl::Context *context, GLenum internalFormat, GLenum format, GLenum type)
{
    ...
    for (ES3FormatCombinationSet::const_iterator i = es3FormatSet.begin(); i != es3FormatSet.end(); i++)
    {
        ...
        {
            const gl::InternalFormatInfo &info = gl::GetInternalFormatInfo(i->internalFormat);
                                   ...
                                   GLint border, GLenum format, GLenum type, const GLvoid *pixels)
{
    ...
    const gl::InternalFormatInfo &actualFormatInfo = gl::GetInternalFormatInfo(actualInternalFormat);
    if (isCompressed)
    {
        ...
        if (!actualFormatInfo.isCompressed)
        {
            return gl::error(GL_INVALID_ENUM, false);
        }
    ...
    if (pixelUnpackBuffer != NULL)
    {
        ...
        GLenum sizedFormat = gl::GetInternalFormatInfo(actualInternalFormat).pixelBytes > 0 ? actualInternalFormat
                                                                                            : gl::GetFormatTypeInfo(actualInternalFormat, type).internalFormat;
...
{
    const InternalFormatInfo &textureInternalFormatInfo = GetInternalFormatInfo(textureInternalFormat);
    const InternalFormatInfo &framebufferInternalFormatInfo = GetInternalFormatInfo(frameBufferInternalFormat);
    ...
    if (conversionSet.find(CopyConversion(textureInternalFormatInfo.format, framebufferInternalFormatInfo.format)) != conversionSet.end())
    {
        ...
        if (readBufferHandle != 0)
        {
            ...
            else
            {
                ...
                const FormatTypeInfo &typeInfo = GetFormatTypeInfo(framebufferInternalFormatInfo.format, framebufferInternalFormatInfo.type);
        ...
        else
        {
            ...
            else if (framebufferInternalFormatInfo.colorEncoding == GL_SRGB)
            {
                ...
                if ((GetInternalFormatInfo(textureInternalFormat).pixelBytes > 0) &&
                                       ...
                                       GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
    ...
    {
        if (!IsValidES3CopyTexImageCombination(internalformat, colorbufferInternalFormat,
                                               context->getReadFramebufferHandle()))
        {
            return gl::error(GL_INVALID_OPERATION, false);
        }
