                                       ...
                                       GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
    ...
    if (isSubImage)
    {
        if (!gl::IsValidCopyTexImageCombination(textureInternalFormat, colorbufferInternalFormat,
                                                context->getState().getReadFramebufferHandle(),
                                             ...
                                             const GLenum* attachments)
{
    ...
    switch (target)
    {
      ...
      case GL_FRAMEBUFFER:
        defaultFramebuffer = context->getState().getDrawFramebufferHandle() == 0;
        break;
      case GL_READ_FRAMEBUFFER:
        defaultFramebuffer = context->getState().getReadFramebufferHandle() == 0;
        break;
      default:
        return gl::error(GL_INVALID_ENUM, false);
    }
