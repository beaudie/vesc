...
ANGLE_EXPORT void GL_APIENTRY MatrixLoadIdentityCHROMIUM(GLenum matrixMode)
{
    ...
    MatrixLoadfCHROMIUM(matrixMode, I);
}
...
ANGLE_EXPORT void GL_APIENTRY PathCommandsCHROMIUM(GLuint path,
                                                   ...
                                                   const void* coords)
{
    EVENT("(GLuint path = %u, GLsizei numCommands = %d, commands = %p, "
    ...
    if (context)
    {
        if (!context->skipValidation())
        {
            if (!ValidatePathCommands(context, path,
...
{
    EVENT("(GLuint path = %u, GLenum pname = %u, GLfloat value = %f)",
        path, pname, value);

    Context* context = GetValidGlobalContext();
    if (context)
    {
        GLfloat actual = value;
        if (!context->skipValidation() && !ValidateSetPathParameter(context, path, pname, value, &actual))
        {
            return;
        }
        context->setPathParameter(path, pname, value);
    }
}
...
ANGLE_EXPORT void GL_APIENTRY GetPathParameterfCHROMIUM(GLuint path, GLenum pname, GLfloat* value)
{
    ...
    if (context)
    {
        ...
        context->getPathParameter(path, pname, value);
    }
}
...
{
    EVENT("(GLenum func = %u, GLint ref = %d, GLuint mask = %u)",
        func, ref, mask);
...
{
    EVENT("(GLuint path = %u, GLenum fillMode = %u, GLuint mask = %u)",
        path, fillMode, mask);
...
{
    EVENT("(GLuint path = %u, GLint ference = %d, GLuint mask = %u)",
        path, reference, mask);

    Context* context = GetValidGlobalContext();
    if (context)
    {
        if (!context->skipValidation() && !ValidateStencilStrokePath(context, path, reference, mask))
        {
            return;
        }
...
{
    EVENT("(GLuint path = %u, GLenum fillMode = %u, GLuint mask = %u, GLenum coverMode = %u)",
    ...
    if (context)
    {
        if (!context->skipValidation())
        {
            if (!ValidateStencilFillPath(context, path, fillMode, mask))
                return;
            if (!ValidateCoverPath(context, path, coverMode))
                return;
        }
...
{
    EVENT("(GLuint path = %u, GLint reference = %d, GLuint mask = %u, GLenum coverMode = %u)",
    ...
    if (context)
    {
        if (!context->skipValidation())
        {
            if (!ValidateStencilStrokePath(context, path, reference, mask))
                return;
            if (!ValidateCoverPath(context, path, coverMode))
                return;
        }
