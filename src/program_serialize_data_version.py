...
def GenerateHashOfAffectedFiles(input_files, angle_root):
    script_path = os.path.dirname(__file__)
    script_parent_path = os.path.join(script_path, os.pardir)

    hash_md5 = hashlib.md5()
    for file in input_files:
        assert file.startswith(angle_root)
        file = file.replace(angle_root, '')
...
angle_root_dir = sys.argv[4]
print("$angle_root " + angle_root_dir)
angle_shader_program_version_hash_result = GenerateHashOfAffectedFiles(input_files, angle_root_dir)
