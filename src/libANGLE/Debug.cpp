...
bool Debug::isMessageEnabled(GLenum source, GLenum type, GLuint id, GLenum severity) const
{
    ...
    for (auto groupIter = rbegin(mGroups); groupIter != rend(mGroups); groupIter++)
    {
        const auto &controls = groupIter->controls;
        for (auto controlIter = rbegin(controls); controlIter != rend(controls); controlIter++)
        {
