...
class PathAllocatorTest : public testing::Test
{
  protected:

  gl::PathAllocator* getAllocator()
  {
     return &mAllocator;
  }

  private:
    gl::PathAllocator mAllocator;
};

// Checks basic functionality: allocate, release, isUsed.
TEST_F(PathAllocatorTest, TestBasic)
{
...
TEST_F(PathAllocatorTest, TestAdvanced)
{
    auto *allocator = getAllocator();
...
TEST_F(PathAllocatorTest, MarkAsUsed)
{
    auto * allocator = getAllocator();
...
TEST_F(PathAllocatorTest, AllocateAtOrAbove)
{
...
TEST_F(PathAllocatorTest, AllocateIdAtOrAboveWrapsAround)
{
...
TEST_F(PathAllocatorTest, RedundantFreeIsIgnored)
{
    auto* allocator = getAllocator();
    GLuint id1 = allocator->allocate();
    allocator->release(0);
    ...
    GLuint id3 = allocator->allocate();
    EXPECT_NE(id2, id3);
    EXPECT_NE(allocator->kInvalidPath, id2);
    EXPECT_NE(allocator->kInvalidPath, id3);
}

TEST_F(PathAllocatorTest, allocateRange)
{
...
TEST_F(PathAllocatorTest, AllocateRangeEndNoEffect)
{
    const GLuint kMaxPossibleOffset = std::numeric_limits<GLuint>::max();
...
TEST_F(PathAllocatorTest, AllocateFullRange)
{
...
TEST_F(PathAllocatorTest, AllocateRangeNoWrapInRange)
{
    const uint32_t kMaxPossibleRange = std::numeric_limits<uint32_t>::max();
...
TEST_F(PathAllocatorTest, AllocateMax)
{
    const uint32_t kMaxPossibleRange = std::numeric_limits<uint32_t>::max();
...
TEST_F(PathAllocatorTest, ZeroIdCases)
{
    auto* allocator = getAllocator();
