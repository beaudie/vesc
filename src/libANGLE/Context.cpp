...
egl::Error Context::onDestroy(const egl::Display *display)
{
    ...
    ASSERT(isDestroyed() == true && mShared.getRefCount() == 0);
    ...
    mOverlay.destroy(asSharedContext());
...
void SharedContext::bindProgramPipeline(ProgramPipeline *pipeline)
{
...
void Context::bindProgramPipeline(ProgramPipelineID pipelineHandle)
{
    ...
    mShared.bindProgramPipeline(pipeline);
}
...
void SharedContext::bindBufferRange(BufferBinding target,
                                    GLuint index,
                                    Buffer *buffer,
                                    GLintptr offset,
                                    GLsizeiptr size)
                              ...
                              GLsizeiptr size)
{
    ...
    ANGLE_CONTEXT_TRY(mState.setIndexedBufferBinding(this, target, index, buffer, offset, size));
    mShared.bindBufferRange(target, index, object, offset, size);
}
