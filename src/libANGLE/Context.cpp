...
void Context::gles1_draw(bool indexed, GLenum mode, GLint first, GLsizei count, GLenum indexType, const GLvoid* indices) {
    ...
    if (logicOpEnabled) {
        ...
        fprintf(stderr, "%s: logic op enabled. 0x%x 0x%x\n", __func__, drawBuf, readBuf);
        // if (drawBuf == readBuf) {
        //     fprintf(stderr, "%s: ready for logic ops, read == draw\n", __func__);
        // } else {
        //     fprintf(stderr, "%s: changing read buffer to 0x%x\n", __func__, drawBuf);
        //     readBuffer(drawBuf);
        //     restoreReadBuffer = true;
        // }
    }
