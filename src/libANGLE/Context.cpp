...
{
    return mState.mSyncManager->getSync(syncPacked.value);
}
...
{
    return getSync({static_cast<uint32_t>(reinterpret_cast<uintptr_t>(ptr))});
}
...
GLsync Context::fenceSync(GLenum condition, GLbitfield flags)
{
    ...
    return reinterpret_cast<GLsync>(reinterpret_cast<void *>(syncHandle.value));
}
