...
#    define LOG_LIMITED_CAP(cap, limit)                                                     \
...
#define LIMIT_CAP(cap, limit)            \
    do                                   \
    {                                    \
        if ((cap) > (limit))             \
        {                                \
            LOG_LIMITED_CAP(cap, limit); \
            (cap) = (limit);             \
        }                                \
    } while (0)

    // Apply/Verify implementation limits
    LIMIT_CAP(mState.mCaps.maxVertexAttributes, MAX_VERTEX_ATTRIBS);
    ...
    {
        LIMIT_CAP(mState.mCaps.maxVertexAttribBindings, MAX_VERTEX_ATTRIB_BINDINGS);
    }

    LIMIT_CAP(mState.mCaps.max2DTextureSize, IMPLEMENTATION_MAX_2D_TEXTURE_SIZE);
    LIMIT_CAP(mState.mCaps.maxCubeMapTextureSize, IMPLEMENTATION_MAX_CUBE_MAP_TEXTURE_SIZE);
    LIMIT_CAP(mState.mCaps.max3DTextureSize, IMPLEMENTATION_MAX_3D_TEXTURE_SIZE);
    LIMIT_CAP(mState.mCaps.maxArrayTextureLayers, IMPLEMENTATION_MAX_2D_ARRAY_TEXTURE_LAYERS);
    LIMIT_CAP(mState.mCaps.maxRectangleTextureSize, IMPLEMENTATION_MAX_2D_TEXTURE_SIZE);

    LIMIT_CAP(mState.mCaps.maxShaderUniformBlocks[ShaderType::Vertex],
              IMPLEMENTATION_MAX_VERTEX_SHADER_UNIFORM_BUFFERS);
    LIMIT_CAP(mState.mCaps.maxShaderUniformBlocks[ShaderType::Geometry],
              IMPLEMENTATION_MAX_GEOMETRY_SHADER_UNIFORM_BUFFERS);
    LIMIT_CAP(mState.mCaps.maxShaderUniformBlocks[ShaderType::Fragment],
              IMPLEMENTATION_MAX_FRAGMENT_SHADER_UNIFORM_BUFFERS);
    LIMIT_CAP(mState.mCaps.maxShaderUniformBlocks[ShaderType::Compute],
              IMPLEMENTATION_MAX_COMPUTE_SHADER_UNIFORM_BUFFERS);
    LIMIT_CAP(mState.mCaps.maxCombinedUniformBlocks,
              IMPLEMENTATION_MAX_COMBINED_SHADER_UNIFORM_BUFFERS);
    LIMIT_CAP(mState.mCaps.maxUniformBufferBindings, IMPLEMENTATION_MAX_UNIFORM_BUFFER_BINDINGS);

    LIMIT_CAP(mState.mCaps.maxVertexOutputComponents, IMPLEMENTATION_MAX_VARYING_VECTORS * 4);
    LIMIT_CAP(mState.mCaps.maxFragmentInputComponents, IMPLEMENTATION_MAX_VARYING_VECTORS * 4);

    LIMIT_CAP(mState.mCaps.maxTransformFeedbackInterleavedComponents,
              IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS);
    LIMIT_CAP(mState.mCaps.maxTransformFeedbackSeparateAttributes,
              IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS);
    LIMIT_CAP(mState.mCaps.maxTransformFeedbackSeparateComponents,
    ...
    LIMIT_CAP(mState.mCaps.maxCombinedTextureImageUnits, IMPLEMENTATION_MAX_ACTIVE_TEXTURES);
    for (ShaderType shaderType : AllShaderTypes())
    {
        LIMIT_CAP(mState.mCaps.maxShaderTextureImageUnits[shaderType],
                  IMPLEMENTATION_MAX_SHADER_TEXTURES);
    }

    LIMIT_CAP(mState.mCaps.maxImageUnits, IMPLEMENTATION_MAX_IMAGE_UNITS);
    LIMIT_CAP(mState.mCaps.maxCombinedImageUniforms, IMPLEMENTATION_MAX_IMAGE_UNITS);
    for (ShaderType shaderType : AllShaderTypes())
    {
        LIMIT_CAP(mState.mCaps.maxShaderImageUniforms[shaderType], IMPLEMENTATION_MAX_IMAGE_UNITS);
    }

    for (ShaderType shaderType : AllShaderTypes())
    {
        LIMIT_CAP(mState.mCaps.maxShaderAtomicCounterBuffers[shaderType],
                  IMPLEMENTATION_MAX_ATOMIC_COUNTER_BUFFERS);
    }
    LIMIT_CAP(mState.mCaps.maxCombinedAtomicCounterBuffers,
              IMPLEMENTATION_MAX_ATOMIC_COUNTER_BUFFERS);

    for (ShaderType shaderType : AllShaderTypes())
    {
        LIMIT_CAP(mState.mCaps.maxShaderStorageBlocks[shaderType],
                  IMPLEMENTATION_MAX_SHADER_STORAGE_BUFFER_BINDINGS);
    }
    LIMIT_CAP(mState.mCaps.maxShaderStorageBufferBindings,
              IMPLEMENTATION_MAX_SHADER_STORAGE_BUFFER_BINDINGS);
    LIMIT_CAP(mState.mCaps.maxCombinedShaderStorageBlocks,
              IMPLEMENTATION_MAX_SHADER_STORAGE_BUFFER_BINDINGS);

    LIMIT_CAP(mState.mCaps.maxSampleMaskWords, MAX_SAMPLE_MASK_WORDS);
