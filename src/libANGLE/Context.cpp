                           ...
                           unsigned int line)
{
    ...
    mContext->getGLState().getDebug().insertMessage(GL_DEBUG_SOURCE_API, GL_DEBUG_TYPE_ERROR, 0,
                                                    GL_DEBUG_SEVERITY_HIGH, errorStream.str());
}

void ErrorSet::validationError(GLenum errorCode, const char *message)
{
    ASSERT(errorCode != GL_NO_ERROR && errorCode != GL_OUT_OF_MEMORY);
    mErrors.insert(errorCode);
