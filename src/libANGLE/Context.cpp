...
void Context::updateActiveAttribsMask()
{
    if (!isGLES1() && !mGLState.getProgram())
    {
    ...
    AttributesMask activeAttribs = isGLES1()
                                       ? mGLState.gles1().getVertexArraysAttributeMask()
                                   ...
                                   angle::SubjectMessage message)
{
    ...
    switch (index)
    {
        case kVertexArraySubjectIndex:
            mGLState.setObjectDirty(State::DIRTY_OBJECT_VERTEX_ARRAY);
            break;

        case kReadFramebufferSubjectIndex:
            mGLState.setObjectDirty(State::DIRTY_OBJECT_READ_FRAMEBUFFER);
            break;

        case kDrawFramebufferSubjectIndex:
            mGLState.setObjectDirty(State::DIRTY_OBJECT_DRAW_FRAMEBUFFER);
            break;
