...
void Context::gles1_draw(bool indexed, GLenum mode, GLint first, GLsizei count, GLenum indexType, const GLvoid* indices) {
    ...
    if (!mGLState.isClientStateEnabled(GL_TEXTURE_COORD_ARRAY)) {
        bindBufferPrivate(BufferBinding::Array, drawState.texcoordVbo);
        bufferData(BufferBinding::Array, 4 * sizeof(GLfloat), mGLState.getMultiTexCoord(), BufferUsage::StreamDraw);
        vertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, 0, 0);
