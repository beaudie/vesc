...
{
    return mState.mProgramPipelineManager->genProgramPipeline();
}
                               ...
                               ShaderProgramID program)
{
    ...
    {
        programPipeline =
            mState.mProgramPipelineManager->createProgramPipeline(mImplementation.get(), pipeline);
    }
...
GLboolean Context::isProgramPipeline(ProgramPipelineID pipeline) const
{
    ...
    if (isProgramPipelineGenerated({pipeline}))
    {
        return GL_TRUE;
    }
...
angle::Result Context::onProgramLink(Program *programObject)
{
    ...
    if (foundPipeline)
    {
        if (programObject->getExecutable().isCompute())
        {
            mComputeDirtyObjects.set(State::DIRTY_OBJECT_PROGRAM_PIPELINE);
        }
        else
