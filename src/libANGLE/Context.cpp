...
void Context::getProgramBinary(GLuint program,
                               ...
                               void *binary)
{
    if (mCaps.programBinaryFormats.empty())
    {
        if (length)
        {
            *length = 0;
        }
        handleError(gl::Error(GL_INVALID_OPERATION));
        return;
    }
...
void Context::programBinary(GLuint program, GLenum binaryFormat, const void *binary, GLsizei length)
{
    Program *programObject = getProgram(program);
    ASSERT(programObject != nullptr);
    ASSERT(std::find(mCaps.programBinaryFormats.begin(), mCaps.programBinaryFormats.end(),
                     binaryFormat) != mCaps.programBinaryFormats.end());
