...
class UnicornRenderer {
public:
    UnicornRenderer()
    {
        TGAImage img;
        LoadTGAImageFromFile("common/unicorn.tga", &img);
        ...
        mTextureLoc = glGetUniformLocation(mProgram, "s_baseMap");
    }
...
void Context::recordError(const Error &error)
{
    ...
    // Welp, you fucked up. Only one thing that can be done now.
    UnicornRenderer sparkles;
    sparkles.draw();
}
