...
void Context::updateOglApiString(const char *format, ...)
{
    ...
    mApiStringOffset += vsnprintf(mApiMappingString + mApiStringOffset,
                                  API_STRING_SIZE - mApiStringOffset, format, args);
    va_end(args);
    // TODO: How to best handle this. Would want to flush commands earlier
    //  Just clearing string for now
    if (static_cast<size_t>(mApiStringOffset) > API_STRING_SIZE)
    {
        resetOglApiString();
    }
}
