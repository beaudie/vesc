...
void Context::initGles1()
{
    ...
    { // regular draw state
...

        {
            ...
            drawState.enableClipPlanesLoc =
                getUniformLocation(drawProg, "enable_clip_planes");
            drawState.clipPlaneEnablesLoc =
                getUniformLocation(drawProg, "clip_plane_enables");
            drawState.clipPlanesLoc =
                getUniformLocation(drawProg, "clip_planes");

            useProgram(mGlesEmu.draw.program);

            uniform1i(drawState.textureSampler0Loc, 0);
            uniform1i(drawState.textureSampler1Loc, 1);
            uniform1i(drawState.textureSampler2Loc, 2);
            uniform1i(drawState.textureSampler3Loc, 3);
            uniform1i(drawState.textureCubeSampler0Loc, 4);
            uniform1i(drawState.textureCubeSampler1Loc, 5);
            uniform1i(drawState.textureCubeSampler2Loc, 6);
            uniform1i(drawState.textureCubeSampler2Loc, 7);
        }
    }
...
void Context::gles1_draw(bool indexed, GLenum mode, GLint first, GLsizei count, GLenum indexType, const GLvoid* indices) {
    ...
    if (!mGLState.isClientStateEnabled(GL_NORMAL_ARRAY)) {
        auto normal = mGLState.getNormal();
        vertexAttrib3f(1, normal[0], normal[1], normal[2]);
    }

    if (!mGLState.isClientStateEnabled(GL_COLOR_ARRAY)) {
        auto color = mGLState.getColor();
        vertexAttrib4f(2, color[0], color[1], color[2], color[3]);
    }

    if (!mGLState.isClientStateEnabled(GL_POINT_SIZE_ARRAY_OES)) {
        GLfloat pointSize = mGLState.getPointSize();
        vertexAttrib1f(3, pointSize);
    }

    for (int i = 0; i < 4; i++) {
        if (!mGLState.isTexCoordArrayEnabled(i)) {
            auto texcoord = mGLState.getMultiTexCoord(i);
            vertexAttrib4f(4 + i, texcoord[0], texcoord[1], texcoord[2], texcoord[3]);
        }
    }
...
//      the currently bound texture image of the appropriate dimensionality.
        ...
        for (int i = 0; i < 6; i++) {
            clipPlaneEnables[i] = mGLState.getEnableFeature(GL_CLIP_PLANE0 + i);
            enableClipPlanes = enableClipPlanes || clipPlaneEnables[i];
            mGLState.getClipPlanef(GL_CLIP_PLANE0 + i, (GLfloat*)(clipPlanes.data() + 4 * i));
        }

        uniform1i(drawState.enableClipPlanesLoc, enableClipPlanes);
        uniform1iv(drawState.clipPlaneEnablesLoc, 6, clipPlaneEnables.data());
        uniform4fv(drawState.clipPlanesLoc, 6, clipPlanes.data());
    }
