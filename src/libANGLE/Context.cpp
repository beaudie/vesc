...
void Context::initialize()
{
    ...
    (void)mImplementation->initialize();
}
...
{
    (void)(mState.mPaths->createPaths(this, range, createdOut));
}
                           ...
                           const void *coords)
{
    ...
    (void)(pathObject->setCommands(numCommands, commands, numCoords, coordType, coords));
}
...
void Context::endQuery(QueryType target)
{
    ...
    ANGLE_CONTEXT_TRY(queryObject->end(this));
...
void Context::updateCaps()
{
    ...
    mDrawDirtyObjects.set(State::DIRTY_OBJECT_INIT_DRAW_ATTACHMENTS, robustInit);
    mDrawDirtyObjects.set(State::DIRTY_OBJECT_INIT_TEXTURES, robustInit);
    mDrawDirtyObjects.set(State::DIRTY_OBJECT_INIT_TEXTURE_IMAGES, robustInit);
    mBlitDirtyObjects.set(State::DIRTY_OBJECT_INIT_DRAW_ATTACHMENTS, robustInit);
    mComputeDirtyObjects.set(State::DIRTY_OBJECT_INIT_TEXTURES, robustInit);
    mComputeDirtyObjects.set(State::DIRTY_OBJECT_INIT_TEXTURE_IMAGES, robustInit);
...
ANGLE_INLINE angle::Result Context::prepareForDraw(PrimitiveMode mode)
{
    ...
    ASSERT(!isRobustResourceInitEnabled() || !mGLState.getDrawFramebuffer()->hasResourceThatNeedsInit());
...
{
    mGLState.onActiveTextureStateChange(this, textureUnitIndex);
    mStateCache.onActiveTextureChange(this);
}
                           ...
                           unsigned int line)
{
    ...
    mContext->getGLState().getDebug().insertMessage(GL_DEBUG_SOURCE_API, GL_DEBUG_TYPE_ERROR,
                                                    errorCode, GL_DEBUG_SEVERITY_HIGH,
                                                    errorStream.str());
}
