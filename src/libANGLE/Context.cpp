...
#define ANGLE_LIMIT_CAP(cap, limit)            \
        ...
        (!captureLimitsEnabledFromEnv.empty() && captureLimitsEnabledFromEnv != "0"))
    {
        const char *limiting_reason =
            getFrameCapture()->enabled() ? "FrameCapture enabled" : "FrameCapture limits forced";

        INFO() << "Limiting binary format support count to zero while " << limiting_reason
               << std::endl;
        ...
        INFO() << "Limiting image unit count to " << maxImageUnits << " while " << limiting_reason
               << std::endl;
        ...
        INFO() << "Setting uniform buffer offset alignment to " << uniformBufferOffsetAlignment
               << " while " << limiting_reason << std::endl;
        ...
        INFO() << "Limiting draw buffer count to " << maxDrawBuffers << " while " << limiting_reason
               << std::endl;
