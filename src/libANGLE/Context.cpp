      ...
      mZeroFilledBuffer(1000u)
{
    ...
    if (getClientVersion() >= Version(3, 1))
    {
        ...
        {
            bindBufferRange(BufferTarget::AtomicCounter, 0, i, 0, 0);
        }

        for (unsigned int i = 0; i < mCaps.maxShaderStorageBufferBindings; i++)
        {
            bindBufferRange(BufferTarget::ShaderStorage, i, 0, 0, 0);
        }
    }
    ...
    {
        bindBufferRange(BufferTarget::Uniform, i, 0, 0, -1);
    }
...
void Context::getBufferParameteriv(BufferTarget target, GLenum pname, GLint *params)
{
...
void Context::getBufferPointerv(BufferTarget target, GLenum pname, void **params)
{
    Buffer *buffer = mGLState.getTargetBuffer(target);
    ASSERT(buffer);
...
void *Context::mapBuffer(BufferTarget target, GLenum access)
{
    Buffer *buffer = mGLState.getTargetBuffer(target);
    ASSERT(buffer);
...
GLboolean Context::unmapBuffer(BufferTarget target)
{
    Buffer *buffer = mGLState.getTargetBuffer(target);
    ASSERT(buffer);
...
void *Context::mapBufferRange(BufferTarget target,
                              GLintptr offset,
                              GLsizeiptr length,
                              GLbitfield access)
{
...
void Context::flushMappedBufferRange(BufferTarget /*target*/,
                                     GLintptr /*offset*/,
                                     GLsizeiptr /*length*/)
{
...
void Context::vertexAttribPointer(GLuint index,
                                  ...
                                  const void *ptr)
{
    mGLState.setVertexAttribPointer(this, index, mGLState.getTargetBuffer(BufferTarget::Array),
...
void Context::bufferData(BufferTarget target, GLsizeiptr size, const void *data, BufferUsage usage)
{
    Buffer *buffer = mGLState.getTargetBuffer(target);
    ASSERT(buffer);
...
void Context::bufferSubData(BufferTarget target, GLintptr offset, GLsizeiptr size, const void *data)
{
    if (data == nullptr)
    {
        return;
...
void Context::copyBufferSubData(BufferTarget readTarget,
...
void Context::bindBuffer(BufferTarget target, GLuint buffer)
{
...
void Context::bindBufferBase(BufferTarget target, GLuint index, GLuint buffer)
{
    bindBufferRange(target, index, buffer, 0, 0);
}

void Context::bindBufferRange(BufferTarget target,
                              GLuint index,
                              GLuint buffer,
                              GLintptr offset,
                              GLsizeiptr size)
...
void Context::getBufferParameteri64v(BufferTarget target, GLenum pname, GLint64 *params)
{
    Buffer *buffer = mGLState.getTargetBuffer(target);
