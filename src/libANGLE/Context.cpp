                                     ...
                                     GLint stencil)
{
    ...
    for (size_t i = 0u; i < viewportOffsets.size(); ++i)
    {
        ...
        switch (clearCommandType)
        {
            ...
            {
                const GLfloat *valuesAsFloat = reinterpret_cast<const GLfloat *>(values);
                handleError(mGLState.getDrawFramebuffer()->clearBufferfv(this, buffer, drawbuffer,
                                                                         valuesAsFloat));
            }
            break;
            case ClearCommandType::ClearBufferuiv:
            {
                const GLuint *valuesAsUint = reinterpret_cast<const GLuint *>(values);
                handleError(mGLState.getDrawFramebuffer()->clearBufferuiv(this, buffer, drawbuffer,
                                                                          valuesAsUint));
            }
            break;
            case ClearCommandType::ClearBufferiv:
            {
                const GLint *valuesAsInt = reinterpret_cast<const GLint *>(values);
                handleError(mGLState.getDrawFramebuffer()->clearBufferiv(this, buffer, drawbuffer,
                                                                         valuesAsInt));
            }
            break;
            case ClearCommandType::ClearBufferfi:
                handleError(mGLState.getDrawFramebuffer()->clearBufferfi(this, buffer, drawbuffer,
                                                                         depth, stencil));
                break;
            default:
                UNREACHABLE();
