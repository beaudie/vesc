    ...
    if (mIsGles1) {
        ...
        switch (pname) {
            ...
            case GL_TEXTURE_COORD_ARRAY_BUFFER_BINDING:
                getVertexAttribiv((GLuint)vertexArrayIndex(pname),
...
void Context::getPointerv(GLenum pname, void **params)
{
    if (mIsGles1)
    {
        ...
        case GL_POINT_SIZE_ARRAY_POINTER_OES:
            getVertexAttribPointerv((GLuint)vertexArrayIndex(pname),
...
void Context::initGles1()
{
    ...
    { // regular draw state
        ...
        getProgramiv(mGlesEmu.draw.program, GL_LINK_STATUS, &stat);
        if (!stat) {
            fprintf(stderr, "%s: fragment shader link failed.\n", __func__);
            getProgramInfoLog(mGlesEmu.draw.program, 4095, nullptr, &buf[0]);
            fprintf(stderr, "%s: info log: %s\n", __func__, buf.c_str());
        }

        {
            ...
            drawState.textureSampler0Loc =
                getUniformLocation(drawProg, "tex_sampler0");
            drawState.textureCubeSampler0Loc =
                getUniformLocation(drawProg, "tex_cube_sampler0");
            drawState.textureSampler1Loc =
                getUniformLocation(drawProg, "tex_sampler1");
            drawState.textureCubeSampler1Loc =
                getUniformLocation(drawProg, "tex_cube_sampler1");
            drawState.textureSampler2Loc =
                getUniformLocation(drawProg, "tex_sampler2");
            drawState.textureCubeSampler2Loc =
                getUniformLocation(drawProg, "tex_cube_sampler2");
            drawState.textureSampler3Loc =
                getUniformLocation(drawProg, "tex_sampler3");
            drawState.textureCubeSampler3Loc =
                getUniformLocation(drawProg, "tex_cube_sampler3");
            ...
            drawState.enableAlphaTestLoc =
                getUniformLocation(drawProg, "enable_alpha_test");
            drawState.enableLogicOpLoc =
                getUniformLocation(drawProg, "enable_logic_op");
            drawState.logicOpLoc =
                getUniformLocation(drawProg, "logic_op");

            drawState.alphaFuncLoc =
                getUniformLocation(drawProg, "alpha_func");
            drawState.alphaTestRefLoc =
                getUniformLocation(drawProg, "alpha_test_ref");
            ...
            drawState.combineRgbLoc =
                getUniformLocation(drawProg, "combine_rgb");
            drawState.combineAlphaLoc =
                getUniformLocation(drawProg, "combine_alpha");
            drawState.src0rgbLoc =
                getUniformLocation(drawProg, "src0_rgb");
            drawState.src0alphaLoc =
                getUniformLocation(drawProg, "src0_alpha");
            drawState.src1rgbLoc =
                getUniformLocation(drawProg, "src1_rgb");
            drawState.src1alphaLoc =
                getUniformLocation(drawProg, "src1_alpha");
            drawState.src2rgbLoc =
                getUniformLocation(drawProg, "src2_rgb");
            drawState.src2alphaLoc =
                getUniformLocation(drawProg, "src2_alpha");
            drawState.op0rgbLoc =
                getUniformLocation(drawProg, "op0_rgb");
            drawState.op0alphaLoc =
                getUniformLocation(drawProg, "op0_alpha");
            drawState.op1rgbLoc =
                getUniformLocation(drawProg, "op1_rgb");
            drawState.op1alphaLoc =
                getUniformLocation(drawProg, "op1_alpha");
            drawState.op2rgbLoc =
                getUniformLocation(drawProg, "op2_rgb");
            drawState.op2alphaLoc =
                getUniformLocation(drawProg, "op2_alpha");
            drawState.textureEnvColorLoc =
                getUniformLocation(drawProg, "texture_env_color");
            drawState.rgbScaleLoc =
                getUniformLocation(drawProg, "texture_env_rgb_scale");
            drawState.alphaScaleLoc =
                getUniformLocation(drawProg, "texture_env_alpha_scale");
            drawState.pointSpriteCoordReplaceLoc =
                getUniformLocation(drawProg, "texture_env_point_sprite_coord_replace");
            ...
            drawState.pointRasterizationLoc =
                getUniformLocation(drawProg, "point_rasterization");
            drawState.pointSpriteEnabledLoc =
                getUniformLocation(drawProg, "point_sprite_enabled");
            drawState.pointSmoothEnabledLoc =
                getUniformLocation(drawProg, "point_smooth_enabled");
            drawState.pointSizeMinLoc =
                getUniformLocation(drawProg, "point_size_min");
            drawState.pointSizeMaxLoc =
                getUniformLocation(drawProg, "point_size_max");
            drawState.pointDistanceAttenuationLoc =
                getUniformLocation(drawProg, "point_distance_attenuation");
            ...
            genBuffers(4, drawState.texcoordVbo);
            genBuffers(1, &drawState.ibo);

            for (int i = 0; i < 16; i++) {
                drawState.savedAttribs[i].needsSaving = false;
            }
        }
    }
    ...
    std::vector<GLuint> currPrivateBufs = {
            ...
            mGlesEmu.draw.texcoordVbo[0],
            mGlesEmu.draw.texcoordVbo[1],
            mGlesEmu.draw.texcoordVbo[2],
            mGlesEmu.draw.texcoordVbo[3],
            mGlesEmu.draw.ibo };
...
GLboolean Context::isBuffer(GLuint buffer)
{
    ...
    if (mIsGles1) {
        ...
        if (buffer == mGlesEmu.drawTex.vbo ||
            ...
            buffer == mGlesEmu.draw.texcoordVbo[0] ||
            buffer == mGlesEmu.draw.texcoordVbo[1] ||
            buffer == mGlesEmu.draw.texcoordVbo[2] ||
            buffer == mGlesEmu.draw.texcoordVbo[3] ||
...
void Context::enableClientState(GLenum clientState)
{
    switch (clientState) {
        ...
        case GL_TEXTURE_COORD_ARRAY:
            enableVertexAttribArray(4 + mGLState.getActiveSampler());
            break;
        default:
            break;
    }
...
void Context::vertexPointer(GLint size, GLenum type, GLsizei stride, const void* ptr) {
    ...
    auto& saved = mGlesEmu.draw.savedAttribs[0];
    saved.needsSaving = true;
    saved.size = size;
    saved.type = type;
    saved.stride = stride;
    saved.ptr = ptr;
...
void Context::normalPointer(GLenum type, GLsizei stride, const void* ptr) {
    ...
    auto& saved = mGlesEmu.draw.savedAttribs[1];
    saved.needsSaving = true;
    saved.size = 3;
    saved.type = type;
    saved.stride = stride;
...
void Context::colorPointer(GLint size, GLenum type, GLsizei stride, const void* ptr) {
    ...
    auto& saved = mGlesEmu.draw.savedAttribs[2];
    saved.needsSaving = true;
    saved.size = size;
    saved.type = type;
    saved.stride = stride;
...
void Context::pointSizePointer(GLenum type, GLsizei stride, const void* ptr) {
    ...
    auto& saved = mGlesEmu.draw.savedAttribs[3];
    saved.needsSaving = true;
    saved.size = 1;
    saved.type = type;
    saved.stride = stride;
...
void Context::texCoordPointer(GLint size, GLenum type, GLsizei stride, const void* ptr) {
    vertexAttribPointer(4 + mGLState.getActiveSampler(), size, type, GL_FALSE, stride, ptr);
    auto& saved = mGlesEmu.draw.savedAttribs[4 + mGLState.getActiveSampler()];
    saved.needsSaving = true;
    saved.size = size;
    saved.type = type;
    saved.stride = stride;
...
GLint Context::vertexArrayIndex(GLenum type) const {
    switch (type) {
    case GL_VERTEX_ARRAY:
    case GL_VERTEX_ARRAY_BUFFER_BINDING:
    case GL_VERTEX_ARRAY_STRIDE:
...
void Context::gles1_draw(bool indexed, GLenum mode, GLint first, GLsizei count, GLenum indexType, const GLvoid* indices) {
    ...
    bool logicOpEnabled = mGLState.getEnableFeature(GL_COLOR_LOGIC_OP);

    GLint drawBuf;
    GLint readBuf;

    if (logicOpEnabled) {
        getIntegerv(GL_DRAW_BUFFER0_EXT, &drawBuf);
        getIntegerv(GL_READ_BUFFER, &readBuf);
        fprintf(stderr, "%s: logic op enabled (warning: NYI). 0x%x 0x%x\n", __func__, drawBuf, readBuf);
    }
    ...
    getIntegerv(GL_ARRAY_BUFFER_BINDING, &prevArrayBufferBinding);

    if (!mGLState.isClientStateEnabled(GL_NORMAL_ARRAY)) {
        bindBufferPrivate(BufferBinding::Array, drawState.normalVbo);
        bufferData(BufferBinding::Array, 3 * sizeof(GLfloat), mGLState.getNormal(), BufferUsage::StreamDraw);
    ...
    if (!mGLState.isClientStateEnabled(GL_COLOR_ARRAY)) {
        bindBufferPrivate(BufferBinding::Array, drawState.colorVbo);
        bufferData(BufferBinding::Array, 4 * sizeof(GLfloat), mGLState.getColor(), BufferUsage::StreamDraw);
        vertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 0, 0);
        vertexAttribDivisor(2, 1);
        enableVertexAttribArray(2);
    }

    if (!mGLState.isClientStateEnabled(GL_POINT_SIZE_ARRAY_OES)) {
        bindBufferPrivate(BufferBinding::Array, drawState.pointsizeVbo);
        GLfloat pointSize = mGLState.getPointSize();
        bufferData(BufferBinding::Array, sizeof(GLfloat), &pointSize, BufferUsage::StreamDraw);
        vertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, 0, 0);
        vertexAttribDivisor(3, 1);
        enableVertexAttribArray(3);
    }

    for (int i = 0; i < 4; i++) {
        if (!mGLState.isTexCoordArrayEnabled(i)) {
            bindBufferPrivate(BufferBinding::Array, drawState.texcoordVbo[i]);
            bufferData(BufferBinding::Array, 4 * sizeof(GLfloat), mGLState.getMultiTexCoord(i), BufferUsage::StreamDraw);
            vertexAttribPointer(4 + i, 4, GL_FLOAT, GL_FALSE, 0, 0);
            vertexAttribDivisor(4 + i, 1);
            enableVertexAttribArray(4 + i);
        }
    }

    bindBuffer(BufferBinding::Array, (GLuint)prevArrayBufferBinding);

    {
        auto proj = mGLState.projMatrix();
        auto modelview = mGLState.modelviewMatrix();
        auto modelviewInvTr = modelview.transpose().inverse();

        uniformMatrix4fv(drawState.projMatrixLoc, 1, GL_FALSE, proj.data());
        uniformMatrix4fv(drawState.modelviewMatrixLoc, 1, GL_FALSE, modelview.data());
        uniformMatrix4fv(drawState.modelviewInvTrLoc, 1, GL_FALSE, modelviewInvTr.data());

        for (int i = 0; i < 4; i++) {
            auto tex = mGLState.textureMatrix(i);
            memcpy(drawState.texEnvBuffer.textureMatrices + i * 16,
                   tex.data(),
                   16 * sizeof(float));
        }
        uniformMatrix4fv(drawState.textureMatrixLoc, 4, GL_FALSE, drawState.texEnvBuffer.textureMatrices);
    }
...
//      the currently bound texture image of the appropriate dimensionality.
            ...
            texCubeenables[i] = mGLState.isTextureTargetEnabled(GL_TEXTURE0 + i, GL_TEXTURE_CUBE_MAP);
            tex2Denables[i] = !texCubeenables[i] && (mGLState.isTextureTargetEnabled(GL_TEXTURE0 + i, GL_TEXTURE_2D));

            Texture* curr2DTexture = mGLState.getSamplerTexture(i, GL_TEXTURE_2D);
            if (curr2DTexture) {
                tex2DFormats[i] =
                    gl::GetUnsizedFormat(curr2DTexture->getFormat(GL_TEXTURE_2D, 0).info->internalFormat);
            }
            // TODO: Cube map formats
        }

        uniform1iv(drawState.enableTexture2DLoc, 4, tex2Denables.data());
        uniform1iv(drawState.enableTextureCubeMapLoc, 4, texCubeenables.data());
        uniform1iv(drawState.textureFormatLoc, 4, tex2DFormats.data());

        // Fixed sampler locations
        uniform1i(drawState.textureSampler0Loc, (tex2Denables[0] || !texCubeenables[0]) ? 0 : 4);
        uniform1i(drawState.textureSampler1Loc, (tex2Denables[1] || !texCubeenables[1]) ? 1 : 5);
        uniform1i(drawState.textureSampler2Loc, (tex2Denables[2] || !texCubeenables[2]) ? 2 : 6);
        uniform1i(drawState.textureSampler3Loc, (tex2Denables[3] || !texCubeenables[3]) ? 3 : 7);
        uniform1i(drawState.textureCubeSampler0Loc, texCubeenables[0] ? 0 : 4);
        uniform1i(drawState.textureCubeSampler1Loc, texCubeenables[1] ? 1 : 5);
        uniform1i(drawState.textureCubeSampler2Loc, texCubeenables[2] ? 2 : 6);
        //uniform1i(drawState.textureCubeSampler3Loc, texCubeenables[3] ? 3 : 7);

        for (int i = 0; i < 4; i++) {
            const auto& texEnv = mGLState.getTexEnv(i);
        ...
        uniform1iv(drawState.textureEnvModeLoc, 4, drawState.texEnvBuffer.modes);
        uniform1iv(drawState.combineRgbLoc, 4, drawState.texEnvBuffer.combineRgbs);
        uniform1iv(drawState.combineAlphaLoc, 4, drawState.texEnvBuffer.combineAlphas);

        uniform1iv(drawState.src0rgbLoc, 4, drawState.texEnvBuffer.src0rgbs);
        uniform1iv(drawState.src0alphaLoc, 4, drawState.texEnvBuffer.src0alphas);
        uniform1iv(drawState.src1rgbLoc, 4, drawState.texEnvBuffer.src1rgbs);
        uniform1iv(drawState.src1alphaLoc, 4, drawState.texEnvBuffer.src1alphas);
        uniform1iv(drawState.src2rgbLoc, 4, drawState.texEnvBuffer.src2rgbs);
        uniform1iv(drawState.src2alphaLoc, 4, drawState.texEnvBuffer.src2alphas);

        uniform1iv(drawState.op0rgbLoc, 4, drawState.texEnvBuffer.op0rgbs);
        uniform1iv(drawState.op0alphaLoc, 4, drawState.texEnvBuffer.op0alphas);
        uniform1iv(drawState.op1rgbLoc, 4, drawState.texEnvBuffer.op1rgbs);
        uniform1iv(drawState.op1alphaLoc, 4, drawState.texEnvBuffer.op1alphas);
        uniform1iv(drawState.op2rgbLoc, 4, drawState.texEnvBuffer.op2rgbs);
        uniform1iv(drawState.op2alphaLoc, 4, drawState.texEnvBuffer.op2alphas);

        uniform4fv(drawState.textureEnvColorLoc, 4, drawState.texEnvBuffer.envColors);
        uniform1fv(drawState.rgbScaleLoc, 4, drawState.texEnvBuffer.rgbScales);
        uniform1fv(drawState.alphaScaleLoc, 4, drawState.texEnvBuffer.alphaScales);
        uniform1iv(drawState.pointSpriteCoordReplaceLoc, 4, drawState.texEnvBuffer.pointSpriteCoordReplaces);
        ...
        uniform1i(drawState.enableAlphaTestLoc, mGLState.getEnableFeature(GL_ALPHA_TEST));
        uniform1i(drawState.enableLogicOpLoc, mGLState.getEnableFeature(GL_COLOR_LOGIC_OP));

        uniform1i(drawState.alphaFuncLoc, mGLState.getAlphaFunc());
        uniform1f(drawState.alphaTestRefLoc, mGLState.getAlphaTestRef());

        uniform1i(drawState.logicOpLoc, mGLState.getLogicOp());
        ...
        // Point rasterization
        uniform1i(drawState.pointRasterizationLoc, (GLint)(mode == GL_POINTS));
        uniform1i(drawState.pointSpriteEnabledLoc, mGLState.getEnableFeature(GL_POINT_SPRITE_OES));
        // uniform1i(drawState.pointSmoothEnabledLoc, mGLState.getEnableFeature(GL_POINT_SMOOTH));
        GLfloat pointSizeLimits[2];
        mGLState.getPointSizeLimits(pointSizeLimits, pointSizeLimits + 1);
        uniform1f(drawState.pointSizeMinLoc, pointSizeLimits[0]);
        uniform1f(drawState.pointSizeMaxLoc, pointSizeLimits[1]);
        GLfloat pointDistanceAttenuation[4];
        mGLState.getPointDistanceAttenuation(pointDistanceAttenuation);
        uniform4fv(drawState.pointDistanceAttenuationLoc, 1, pointDistanceAttenuation);
    }
    ...
    vertexAttribDivisor(3, 0);
    vertexAttribDivisor(4, 0);
    vertexAttribDivisor(5, 0);
    vertexAttribDivisor(6, 0);
    vertexAttribDivisor(7, 0);

    if (drawState.savedAttribs[1].needsSaving)
    {
        const auto& a = drawState.savedAttribs[1];
        normalPointer(a.type, a.stride, a.ptr);
    }

    if (drawState.savedAttribs[2].needsSaving)
    {
        const auto& a = drawState.savedAttribs[2];
        colorPointer(a.size, a.type, a.stride, a.ptr);
    }

    if (drawState.savedAttribs[3].needsSaving)
    {
        const auto& a = drawState.savedAttribs[3];
        pointSizePointer(a.type, a.stride, a.ptr);
    }

    for (int i = 0; i < 4; i++) {
        if (drawState.savedAttribs[4 + i].needsSaving)
        {
            const auto& a = drawState.savedAttribs[4 + i];
            vertexAttribPointer(4 + i, a.size, a.type, GL_FALSE, a.stride, a.ptr);
        }
    }
