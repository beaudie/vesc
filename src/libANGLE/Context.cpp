    ...
    for (size_t i = 0u; i < viewportOffsets.size(); ++i)
    {
        ...
        switch (clearCommandType)
        {
            ...
            case ClearCommandType::ClearBufferfv:
            {
                ...
                handleError(
                    drawFramebuffer->clearBufferfv(this, buffer, drawbuffer, valuesAsFloat));
            }
            break;
            case ClearCommandType::ClearBufferuiv:
            {
                ...
                handleError(
                    drawFramebuffer->clearBufferuiv(this, buffer, drawbuffer, valuesAsUint));
            }
            break;
            case ClearCommandType::ClearBufferiv:
            {
                const GLint *valuesAsInt = reinterpret_cast<const GLint *>(values);
                handleError(drawFramebuffer->clearBufferiv(this, buffer, drawbuffer, valuesAsInt));
            }
            break;
