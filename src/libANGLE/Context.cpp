...
void Context::initExtensionStrings()
{
    auto mergeExtensionStrings = [](const std::vector<const char*>& strings)
    {
        ...
        std::copy(strings.begin(), strings.end(), std::ostream_iterator<const char *>(combinedStringStream, " "));
    ...
    {
        if (extensionInfo.second.Requestable && !(mExtensions.*(extensionInfo.second.ExtensionsMember)))
        {
