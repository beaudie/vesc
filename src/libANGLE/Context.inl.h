...
ANGLE_INLINE bool Context::noopDraw(PrimitiveMode mode, GLsizei count)
{
    ...
    // We have to check if GLES1 here, since there isn't a good place to update
    // the State beforehand since there are no programs.
    if (!mGLES1Renderer && !state.canDraw())
    {
        return true;
    }
