...
bool State::getEnableFeature(GLenum feature) const
{
    switch (feature)
    {
      ...
      case GL_TEXTURE_2D:
      case GL_TEXTURE_CUBE_MAP:
          return isTextureTargetEnabled(mActiveSampler + GL_TEXTURE0, feature);
...
void State::getIntegerv(const Context *context, GLenum pname, GLint *params)
{
    ...
    switch (pname)
    {
      ...
      case GL_CLIENT_ACTIVE_TEXTURE:
          params[0] = GL_TEXTURE0 + mActiveSampler;
          break;
      case GL_MATRIX_MODE:
          params[0] = mCurrMatrixMode;
          break;
      case GL_MODELVIEW_STACK_DEPTH:
          params[0] = mModelviewMatrices.size();
          break;
      case GL_PROJECTION_STACK_DEPTH:
          params[0] = mProjMatrices.size();
          break;
      case GL_TEXTURE_STACK_DEPTH:
          params[0] = mTextureMatrices[mActiveSampler].size();
          break;
      case GL_FOG_COLOR:
...
void State::getTexEnviv(GLenum target, GLenum pname, GLint* params)
{
    ...
    if (target == GL_TEXTURE_ENV) {
        ...
        case GL_TEXTURE_ENV_COLOR:
            params[0] = env.envColor[0] * 255;
            params[1] = env.envColor[1] * 255;
            params[2] = env.envColor[2] * 255;
            params[3] = env.envColor[3] * 255;
            break;
        case GL_RGB_SCALE:
...
void State::enableClientState(GLenum clientState)
{
    if (clientState == GL_TEXTURE_COORD_ARRAY) {
        clientState = GL_TEXTURE_COORD_ARRAY + mActiveSampler;
    }
