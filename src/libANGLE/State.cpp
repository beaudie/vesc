...
angle::Result State::setProgram(const Context *context, Program *newProgram)
{
    ...
    if (mProgram != newProgram)
    {
        ...
        {
            newProgram->addRef();
            ANGLE_TRY(onProgramExecutableChange(context));
        }
...
angle::Result State::onProgramExecutableChange(const Context *context)
{
    // OpenGL Spec:
    ...
    // The bound Program alwasys overrides the ProgramPipeline, so install the executable regardless
