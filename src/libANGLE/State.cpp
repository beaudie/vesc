...
void State::signal(const Context *context, angle::ChannelID channelID, angle::MessageID message)
{
    // Conservatively assume all textures are dirty.
    // TODO(jmadill): More fine-grained update.
    mDirtyObjects.set(DIRTY_OBJECT_PROGRAM_TEXTURES);

    if (!mCompleteTextureCache[channelID] ||
        mCompleteTextureCache[channelID]->initState() == InitState::MayNeedInit)
    {
        mCachedTexturesInitState = InitState::MayNeedInit;
    }
}
