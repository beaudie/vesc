...
void State::getIntegeri_v(const Context *context, GLenum target, GLuint index, GLint *data) const
{
    switch (target)
    {
        ...
        case GL_PIXEL_LOCAL_TEXTURE_LAYER_ANGLE:
        {
            ...
            PixelLocalStorage *pls = mDrawFramebuffer->peekPixelLocalStorage();
            if (!pls || pls->getPlane(index).isDeinitialized())
            {
            ...
            {
                *data = pls->getPlane(index).getIntegeri(context, target, index);
            }
            break;
        }
        default:
