...
void State::getFloatv(GLenum pname, GLfloat *params)
{
    ...
    switch (pname)
    {
        ...
        case GL_CURRENT_COLOR:
            memcpy(params, mCurrentColor.data(), gl::kRGBA32FBytes);
            break;
        case GL_CURRENT_TEXTURE_COORDS:
            params[0] = mCurrentTextureCoords[mActiveSampler].s;
            params[1] = mCurrentTextureCoords[mActiveSampler].t;
            params[2] = mCurrentTextureCoords[mActiveSampler].r;
            params[3] = mCurrentTextureCoords[mActiveSampler].q;
            break;
        case GL_CURRENT_NORMAL:
            params[0] = mCurrentNormal.x;
            params[1] = mCurrentNormal.y;
            params[2] = mCurrentNormal.z;
            break;
        ...
        case GL_FOG_COLOR:
            memcpy(params, mFog.color.data(), gl::kRGBA32FBytes);
            break;
        case GL_FOG_DENSITY:
            *params = mFog.density;
            break;
        ...
        case GL_LIGHT_MODEL_AMBIENT:
            memcpy(params, &mLightModel.color, sizeof(mLightModel.color));
            break;
        case GL_POINT_SIZE:
            params[0] = mPointSize;
            break;
...
void State::getIntegerv(const Context *context, GLenum pname, GLint *params)
{
    ...
    switch (pname)
    {
        ...
        case GL_MAX_MODELVIEW_STACK_DEPTH:
            *params = mMaxModelviewMatrixStackDepth;
            break;
        case GL_MAX_PROJECTION_STACK_DEPTH:
            *params = mMaxProjectionMatrixStackDepth;
            break;
        case GL_MAX_TEXTURE_STACK_DEPTH:
            *params = mMaxTextureMatrixStackDepth;
            break;
        ...
        case GL_MAX_CLIP_PLANES:
            *params = mMaxClipPlanes;
            break;
        case GL_CURRENT_COLOR:
            params[0] = (int)mCurrentColor.red;
            params[1] = (int)mCurrentColor.green;
            params[2] = (int)mCurrentColor.blue;
            params[3] = (int)mCurrentColor.alpha;
            break;
        ...
        case GL_TEXTURE_STACK_DEPTH:
            ...
            break;
        case GL_FOG_COLOR:
            params[0] = (int)mFog.color.red;
            params[1] = (int)mFog.color.green;
            params[2] = (int)mFog.color.blue;
            params[3] = (int)mFog.color.alpha;
            break;
        case GL_FOG_DENSITY:
        ...
        case GL_LIGHT_MODEL_TWO_SIDE:
            *params = mLightModel.twoSided;
            break;
        case GL_LIGHT_MODEL_AMBIENT:
            params[0] = (int)mLightModel.color.red;
            params[1] = (int)mLightModel.color.green;
            params[2] = (int)mLightModel.color.blue;
            params[3] = (int)mLightModel.color.alpha;
            break;
        case GL_POINT_SIZE:
...
void State::texEnvfv(GLenum target, GLenum pname, const GLfloat *params)
{
    ...
    if (target == GL_TEXTURE_ENV)
    {
        ...
        switch (pname)
        {
            ...
            case GL_TEXTURE_ENV_COLOR:
                memcpy(&env.envColor, params, gl::kRGBA32FBytes);
                break;
            case GL_RGB_SCALE:
                env.rgbScale = param;
                break;
...
void State::texEnviv(GLenum target, GLenum pname, const GLint *params)
{
    ...
    if (target == GL_TEXTURE_ENV)
    {
        ...
        switch (pname)
        {
            ...
            case GL_TEXTURE_ENV_COLOR:
                env.envColor.red   = params[0] / 255.0f;
                env.envColor.green = params[1] / 255.0f;
                env.envColor.blue  = params[2] / 255.0f;
                env.envColor.alpha = params[3] / 255.0f;
                break;
...
void State::getTexEnvfv(GLenum target, GLenum pname, GLfloat *params)
{
    ...
    if (target == GL_TEXTURE_ENV)
    {
        ...
        switch (pname)
        {
            ...
            case GL_TEXTURE_ENV_COLOR:
                memcpy(params, &env.envColor, gl::kRGBA32FBytes);
                break;
            case GL_RGB_SCALE:
...
void State::getTexEnviv(GLenum target, GLenum pname, GLint *params)
{
    ...
    if (target == GL_TEXTURE_ENV)
    {
        ...
        switch (pname)
        {
            ...
            case GL_TEXTURE_ENV_COLOR:
                params[0] = (GLint)(env.envColor.red * 255.0f);
                params[1] = (GLint)(env.envColor.green * 255.0f);
                params[2] = (GLint)(env.envColor.blue * 255.0f);
                params[3] = (GLint)(env.envColor.alpha * 255.0f);
                break;
            case GL_RGB_SCALE:
...
const TextureEnvironmentParameters &State::getTextureEnvironment(unsigned int activeSampler) const
{
    return mTexUnitEnvs[activeSampler];
}
...
void State::materialfv(GLenum face, GLenum pname, const GLfloat *params)
{
    ...
    {
        case GL_AMBIENT:
            memcpy(mMaterial.ambient.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_DIFFUSE:
            memcpy(mMaterial.diffuse.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_AMBIENT_AND_DIFFUSE:
            ...
            break;
        case GL_SPECULAR:
            memcpy(mMaterial.specular.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_EMISSION:
            memcpy(mMaterial.emissive.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_SHININESS:
...
void State::getMaterialfv(GLenum face, GLenum pname, GLfloat *params)
{
    switch (pname)
    {
        case GL_AMBIENT:
            ...
            {
                memcpy(params, mMaterial.ambient.data(), gl::kRGBA32FBytes);
            }
            break;
        case GL_DIFFUSE:
            if (mColorMaterialEnabled)
            ...
            {
                memcpy(params, mMaterial.diffuse.data(), gl::kRGBA32FBytes);
            }
            break;
        case GL_SPECULAR:
            memcpy(params, mMaterial.specular.data(), gl::kRGBA32FBytes);
            break;
        case GL_EMISSION:
            memcpy(params, mMaterial.emissive.data(), gl::kRGBA32FBytes);
            break;
        case GL_SHININESS:
...
void State::lightModelfv(GLenum pname, const GLfloat *params)
{
    ...
    {
        case GL_LIGHT_MODEL_AMBIENT:
            memcpy(&mLightModel.color, params, gl::kRGBA32FBytes);
            break;
        case GL_LIGHT_MODEL_TWO_SIDE:
...
void State::lightfv(GLenum light, GLenum pname, const GLfloat *params)
{
    ...
    {
        case GL_AMBIENT:
            memcpy(mLights[lightIndex].ambient.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_DIFFUSE:
            memcpy(mLights[lightIndex].diffuse.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_SPECULAR:
            memcpy(mLights[lightIndex].specular.ptr(), params, gl::kRGBA32FBytes);
            break;
        case GL_POSITION:
...
void State::getLightfv(GLenum light, GLenum pname, GLfloat *params)
{
    ...
    {
        case GL_AMBIENT:
            memcpy(params, mLights[lightIndex].ambient.data(), gl::kRGBA32FBytes);
            break;
        case GL_DIFFUSE:
            memcpy(params, mLights[lightIndex].diffuse.data(), gl::kRGBA32FBytes);
            break;
        case GL_SPECULAR:
            memcpy(params, mLights[lightIndex].specular.data(), gl::kRGBA32FBytes);
            break;
        case GL_POSITION:
...
{
    mCurrentTextureCoords[target - GL_TEXTURE0] = {s, t, r, q};
}

void State::normal3f(GLfloat nx, GLfloat ny, GLfloat nz)
{
    mCurrentNormal = {nx, ny, nz};
}
...
void State::fogfv(GLenum pname, const GLfloat *params)
{
    switch (pname)
    {
        ...
        case GL_FOG_END:
            ...
            break;
        case GL_FOG_COLOR:
            memcpy(mFog.color.ptr(), params, gl::kRGBA32FBytes);
            break;
        default:
            return;
    }
...
bool State::isTexCoordArrayEnabled(int unit) const
{
    return mEnabledClientStates.find(GL_TEXTURE_COORD_ARRAY + unit) != mEnabledClientStates.end();
}

const ColorF State::getCurrentColor() const
{
    return mCurrentColor;
}

const NormalF State::getCurrentNormal() const
{
    return mCurrentNormal;
}

const TextureCoordF State::getCurrentTextureCoordinate() const
{
    return mCurrentTextureCoords[mActiveSampler];
}

const TextureCoordF State::getTextureCoordinate(int unit) const
{
...
{
    mCurrentColor = {red, green, blue, alpha};
}
...
GLenum State::getAlphaFunc() const
{
    return mAlphaFunc;
}

GLfloat State::getAlphaTestRef() const
{
...
GLfloat State::getPointSize() const
{
    return mPointSize;
}

void State::getPointSizeLimits(GLfloat *min, GLfloat *max) const
{
    *min = mPointSizeMin;
    *max = mPointSizeMax;
}

void State::getPointDistanceAttenuation(GLfloat *att) const
{
    att[0] = mPointDistanceAttenuation[0];
    att[1] = mPointDistanceAttenuation[1];
    att[2] = mPointDistanceAttenuation[2];
    att[3] = 0.0f;
}
...
GLenum State::getLogicOp() const
{
    return mLogicOp;
}

angle::Mat4 State::projMatrix()
{
    return mProjMatrices.back();
}

angle::Mat4 State::modelviewMatrix()
{
    return mModelviewMatrices.back();
}

angle::Mat4 State::textureMatrix()
{
    return mTextureMatrices[mActiveSampler].back();
}

angle::Mat4 State::textureMatrix(int i)
{
    return mTextureMatrices[i].back();
}

const MaterialParameters &State::getMaterialParameters() const
{
    return mMaterial;
}

const LightModelParameters &State::getLightModelParameters() const
{
    return mLightModel;
}

const LightParameters &State::getLightParameters(unsigned int i) const
{
    return mLights[i];
}

const FogParameters &State::getFogParameters() const
{
    return mFog;
}
