...
void ShareGroupAccessibleState::reset(const SharedContext *context)
{
...
void State::reset(const Context *context)
{
    ...
    mPrivateState.reset();
    mShareGroupAccessibleState.reset(context->asSharedContext());
...
{
    const ActiveTextureMask &dirtyActiveTextures =
        mShareGroupAccessibleState.getDirtyActiveTextures();
    if (dirtyActiveTextures.none())
    {
        return angle::Result::Continue;
    }

    for (size_t textureUnit : dirtyActiveTextures)
    {
        if (mExecutable)
        {
            TextureType type       = mExecutable->getActiveSamplerTypes()[textureUnit];
    ...
    mShareGroupAccessibleState.clearDirtyActiveTextures();
    return angle::Result::Continue;
}
