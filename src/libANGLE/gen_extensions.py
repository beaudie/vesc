...
_EXT_MEMBER_TEMPLATE = """\
    // {api_prefix}_{vendor}_{name_with_underscores}
...
_EXT_STRING_TEMPLATE = """\
        map["{api_prefix}_{vendor}_{name_with_underscores}"] = {mode}Extension(&Extensions::{name_camel_case}{vendor});"""
...
def main():
    ...
    data_source_name = '../../scripts/registry_xml.py'
    ...
    with open(gles_h_output_name, 'w') as f:
        gles_h_formatted = _GLES_EXTENSIONS_TEMPLATE.format(**format_args)
        f.write(gles_h_formatted)
        f.close()

    with open(gles_cpp_output_name, 'w') as f:
        gles_cpp_formatted = _GLES_EXT_STRINGS_TEMPLATE.format(**format_args)
        f.write(gles_cpp_formatted)
        f.close()
