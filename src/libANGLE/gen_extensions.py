...
_MD_GLES_EXT_LINK_TEMPLATE = """[`{full_name}`](https://khronos.org/registry/OpenGL/extensions/{vendor}/{vendor}_{name_with_underscores}.txt)"""
_MD_ANGLE_EXT_LINK_TEMPLATE = """[`{full_name}`](https://chromium.googlesource.com/angle/angle/+/refs/heads/main/extensions/{vendor}_{name_with_underscores}.txt)"""
...
def get_ext_support(ext_name, gpu_data):
    ...
    def s(ext, support):
        return '&#10004;' if ext in support['Extensions'] else '&#10060;'

    return ' | '.join([s(ext_name, support) for support in gpu_data])


def get_md_table_header(title):
    configs = ' | '.join(_MD_GPU_CONFIGS)
    dashes = ' | '.join([(':%s:' % ('-' * (len(config) - 2))) for config in _MD_GPU_CONFIGS])
    return _MD_TABLE_HEADER_TEMPLATE.format(title=title, configs=configs, dashes=dashes)
...
def main():
    ...
    gpu_json_data = []
    for (gpu_config, ext_json) in zip(_MD_GPU_CONFIGS, ext_jsons):
        with open(ext_json) as f:
            config_support = json.loads(f.read())
            config_support['config'] = gpu_config
            gpu_json_data.append(config_support)

    gles_md_exts = format_md_exts(gles_ext_infos, gpu_json_data, _MD_GLES_EXT_LINK_TEMPLATE)
    angle_md_exts = format_md_exts(angle_ext_infos, gpu_json_data, _MD_ANGLE_EXT_LINK_TEMPLATE)
    gles1_md_exts = format_md_exts(gles1_ext_infos, gpu_json_data, _MD_GLES_EXT_LINK_TEMPLATE)
    md_gpu_info = [format_md_gpu_info(gpu_data) for gpu_data in gpu_json_data]
    ...
    format_args = {
        ...
        'gles_md_table_header': get_md_table_header('GLES 2.0+'),
        'gles_md_exts': gles_md_exts,
        'angle_md_table_header': get_md_table_header('ANGLE'),
        'angle_md_exts': angle_md_exts,
        'gles1_md_table_header': get_md_table_header('GLES 1.x'),
