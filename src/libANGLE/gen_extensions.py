 ...
 * `GL_RENDERER` is `{nvidia_p400_windows_renderer}`
 * `GL_VENDOR` is `{nvidia_p400_windows_vendor}`
 * `GL_VERSION` is `{nvidia_p400_windows_version}`
 * Data updated {nvidia_p44_windows_date}
"""
...
def get_ext_support(ext_name, platform_info):
    return '&#2714;' if ext_name in platform_info['Extensions'] else '&#274C;'
...
def main():
    ...
    nv_ext_name = '../../scripts/extension_data/nvidia_p400_windows10_vulkan.json'
    if len(sys.argv) > 1:
    ...
    with open(nv_ext_name) as f:
        nv_support = json.loads(f.read())

    sorted_names = sorted([ext_info['full_name'] for ext_info in ext_infos])
    md_exts = '\n'.join([
        '| `%s` | %s |' % (ext_name, get_ext_support(ext_name, nv_support))
    ...
    format_args = {
        ...
        'md_exts': md_exts,
        'nvidia_p400_windows_renderer': nv_support['Renderer'],
        'nvidia_p400_windows_version': nv_support['Version'],
        'nvidia_p400_windows_vendor': nv_support['Vendor'],
        'nvidia_p44_windows_date': nv_support['DateRecorded'],
    }
