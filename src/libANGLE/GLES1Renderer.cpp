...
void GLES1Renderer::prepareDraw(GLenum mode)
{
...

    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            memcpy(drawState.texEnvBuffer.textureMatrices + i * 16, textureMatrix.data(), 16 * sizeof(float));
        }
...

    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            texCubeenables[i] = gles1State.isTextureTargetEnabled(GL_TEXTURE0 + i, TextureType::CubeMap);
            tex2Denables[i] =
                !texCubeenables[i] && (gles1State.isTextureTargetEnabled(GL_TEXTURE0 + i, TextureType::_2D));
        ...
        for (int i = 0; i < 4; i++)
        {
            const auto &texEnv = gles1State.mTextureEnvironments[i];
        ...
        gl->uniform1i(drawState.shadeModelFlatLoc, gles1State.mShadeModel == ShadingModel::Flat);
        ...
        gl->uniform1i(drawState.alphaFuncLoc, ToGLenum(gles1State.mAlphaTestFunc));
        gl->uniform1f(drawState.alphaTestRefLoc, gles1State.mAlphaTestRef);

        gl->uniform1i(drawState.logicOpLoc, ToGLenum(gles1State.mLogicOp));
        ...
        const auto &fog = gles1State.mFog;
        ...
        gl->uniform1f(drawState.pointSizeMinLoc, gles1State.mPointParameters.pointSizeMin);
        gl->uniform1f(drawState.pointSizeMaxLoc, gles1State.mPointParameters.pointSizeMax);

        gl->uniform4fv(drawState.pointDistanceAttenuationLoc, 1,
                       gles1State.mPointParameters.pointDistanceAttenuation.data());
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            auto clipPlane = gles1State.mClipPlanes[i];
