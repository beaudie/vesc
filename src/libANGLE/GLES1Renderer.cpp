...
Error GLES1Renderer::prepareForDraw()
{
    ...
    if (!gles1State.isClientStateEnabled(ClientVertexArrayType::Color))
    {
        ...
        gl->vertexAttrib4f(programState.colorAttribIndex, color.red, color.green, color.blue, color.alpha);
    }
    ...
    for (int i = 0; i < kTexUnitCount; i++)
    {
        if (!gles1State.mTexCoordArrayEnabled[i])
        {
            ...
            gl->vertexAttrib4f(programState.texcoordAttribIndexBase + i, texcoord.s, texcoord.t, texcoord.r,
                               texcoord.q);
        }
    }
...
GLuint GLES1Renderer::compileShader(ShaderType shaderType, const char *src)
{
...

    {
        ...
        if (!status)
        {
            ...
            ERR() << "GLES1Renderer shader compile failed. Source: " << src << " Info log: " << infoLog.data();
            return 0;
        }
        ASSERT(status);
    }
...
bool GLES1Renderer::initializeRendererProgram()
{
    ...
    programState.pointsizeAttribIndex    = mContext->vertexArrayIndex(ClientVertexArrayType::PointSize);
    programState.texcoordAttribIndexBase = mContext->vertexArrayIndex(ClientVertexArrayType::TextureCoord);
    ...
    programState.enableTextureCubeMapLoc = gl->getUniformLocation(program, "enable_texture_cube_map");
