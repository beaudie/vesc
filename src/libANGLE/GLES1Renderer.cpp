    ...
    // Texturing
    {
        ...
        for (int i = 0; i < kTexUnitCount; i++)
        {
            ...
            if (curr2DTexture)
            {
                ...
                cropRectBuffer[i][0]          = cropRect.x;
                cropRectBuffer[i][1]          = cropRect.y;

                cropRectBuffer[i][2] = 0.0f;
                cropRectBuffer[i][3] = 0.0f;
                ...
                if (textureWidth > 0.0f && textureHeight > 0.0f)
                {
                    ...
                    fprintf(stderr,
                            "%s: crop rect: unnormalized: %d %d %d %d texwh %f %f normzlied: %f %f "
                            "%f %f\n",
                            __func__, cropRect.x, cropRect.y, cropRect.width, cropRect.height,
                            textureWidth, textureHeight, cropRectBuffer[i][0], cropRectBuffer[i][1],
                            cropRectBuffer[i][2], cropRectBuffer[i][3]);
                }
            }
        }
                                ...
                                float height)
{
    ...
    float midpointX = 0.0f / viewport.width;
    float midpointY = 0.0f / viewport.height;

    float xNdc =
        midpointX + 2.0f * (float)(x - viewport.x - viewport.width / 2) / (float)viewport.width;
    float yNdc =
        midpointY + 2.0f * (float)(y - viewport.y - viewport.height / 2) / (float)viewport.height;
    float wNdc = 2.0f * (float)width / (float)viewport.width;
    float hNdc = 2.0f * (float)height / (float)viewport.height;

    float zClamped;
    zClamped   = z >= 1.0f ? 1.0f : z;
    zClamped   = zClamped <= 0.0f ? 0.0f : zClamped;
    float zNdc = zClamped * 2.0f - 1.0f;
    ...
    fprintf(
        stderr,
        "%s: vp %d %d %d %d drawTextureCoords %f %f %f dims %f %f normalized: %f %f %f, %f %f\n",
        __func__, viewport.x, viewport.y, viewport.width, viewport.height, x, y, z, width, height,
        mDrawTextureCoords[0], mDrawTextureCoords[1], mDrawTextureCoords[2], mDrawTextureDims[0],
        mDrawTextureDims[1]);
