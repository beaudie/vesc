...
class DelegateWorkerTask
{
  public:
    ...
    static void RunTask(void *user_data)
    {
        DelegateWorkerTask *worker_task = static_cast<DelegateWorkerTask *>(user_data);
        (*worker_task->mTask)();
        worker_task->mWaitable->markAsReady();
...
std::shared_ptr<WaitableEvent> DelegateWorkerPool::postWorkerTask(std::shared_ptr<Closure> task)
{
    ...
    DelegateWorkerTask *worker_task = new DelegateWorkerTask(task, waitable);
    auto *platform                  = ANGLEPlatformCurrent();
    platform->postWorkerTask(platform, DelegateWorkerTask::RunTask, worker_task);
