...
#if (ANGLE_DELEGATE_WORKERS == ANGLE_ENABLED)
    // Use the delegate workers only if an implementation is provided.
    const auto *platform               = ANGLEPlatformCurrent();
    const bool postTaskDelegateHasImpl = platform->postWorkerTask != DefaultPostWorkerTask;
    if (postTaskDelegateHasImpl && multithreaded)
    {
