...
#define ANGLE_VALIDATE_CHECK(expression)      \
    do                                        \
    {                                         \
                             ...
                             const void *user_data)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateContextProperties(properties, platform));
...
{
    ANGLE_VALIDATE_CHECK(ValidateCommandQueueAndEventWaitList(
        command_queue, false, num_events_in_wait_list, event_wait_list));
    ANGLE_VALIDATE_CHECK(
        ValidateEnqueueBuffer(command_queue->cast<CommandQueue>(), buffer, true, false));
                                  ...
                                  const cl_event *event)
{
    ANGLE_VALIDATE_CHECK(ValidateCommandQueueAndEventWaitList(
        command_queue, false, num_events_in_wait_list, event_wait_list));
    ANGLE_VALIDATE_CHECK(
        ValidateEnqueueBuffer(command_queue->cast<CommandQueue>(), buffer, false, true));
                                 ...
                                 const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueBuffer(queue, src_buffer, false, false));
    const Buffer &src = src_buffer->cast<Buffer>();

    ANGLE_VALIDATE_CHECK(ValidateEnqueueBuffer(queue, dst_buffer, false, false));
...
{
    ANGLE_VALIDATE_CHECK(ValidateCommandQueueAndEventWaitList(
        command_queue, true, num_events_in_wait_list, event_wait_list));
    const CommandQueue &queue = command_queue->cast<CommandQueue>();

    ANGLE_VALIDATE_CHECK(ValidateEnqueueImage(queue, image, true, false));
    const Image &img = image->cast<Image>();

    ANGLE_VALIDATE_CHECK(ValidateImageForDevice(img, queue.getDevice(), origin, region));
    ANGLE_VALIDATE_CHECK(ValidateHostRegionForImage(img, region, row_pitch, slice_pitch, ptr));
                                 ...
                                 const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueImage(queue, image, false, true));
    const Image &img = image->cast<Image>();
                                ...
                                const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueImage(queue, src_image, false, false));
    const Image &src = src_image->cast<Image>();

    ANGLE_VALIDATE_CHECK(ValidateEnqueueImage(queue, dst_image, false, false));
    ...
    ANGLE_VALIDATE_CHECK(ValidateImageForDevice(src, queue.getDevice(), src_origin, region));
    ANGLE_VALIDATE_CHECK(ValidateImageForDevice(dst, queue.getDevice(), dst_origin, region));
                                     ...
                                     const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueBuffer(queue, buffer, true, false));
    ANGLE_VALIDATE_CHECK(ValidateBufferRect(buffer->cast<Buffer>(), buffer_origin, region,
                                            buffer_row_pitch, buffer_slice_pitch));
    ANGLE_VALIDATE_CHECK(
        ValidateHostRect(host_origin, region, host_row_pitch, host_slice_pitch, ptr));
                                      ...
                                      const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueBuffer(queue, buffer, false, true));
    ANGLE_VALIDATE_CHECK(ValidateBufferRect(buffer->cast<Buffer>(), buffer_origin, region,
                                            buffer_row_pitch, buffer_slice_pitch));
    ANGLE_VALIDATE_CHECK(
        ValidateHostRect(host_origin, region, host_row_pitch, host_slice_pitch, ptr));
                                     ...
                                     const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(
        ValidateBufferRect(src, src_origin, region, src_row_pitch, src_slice_pitch));
    ANGLE_VALIDATE_CHECK(
        ValidateBufferRect(dst, dst_origin, region, dst_row_pitch, dst_slice_pitch));
                                 ...
                                 const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueBuffer(queue, buffer, false, false));
                                ...
                                const cl_event *event)
{
    ...
    ANGLE_VALIDATE_CHECK(ValidateEnqueueImage(queue, image, false, false));
    const Image &img = image->cast<Image>();
...
cl_int ValidateCreateBufferWithProperties(cl_context context,
                                          ...
                                          const void *host_ptr)
{
    ANGLE_VALIDATE_CHECK(ValidateCreateBuffer(context, flags, size, host_ptr));
...
cl_int ValidateCreateImageWithProperties(cl_context context,
                                         ...
                                         const void *host_ptr)
{
    ANGLE_VALIDATE_CHECK(ValidateCreateImage(context, flags, image_format, image_desc, host_ptr));
