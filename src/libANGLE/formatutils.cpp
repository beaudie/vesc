...
static InternalFormatInfoMap BuildInternalFormatInfoMap()
{
    ...
    //                 |Internal format |sized | R | G | B | A |S | Format         | Type                           | Comp    | SRGB | Texture supported           | Renderable                            | Filterable                                               | ES3 required renderbuffer
...
static FormatSet BuildES3RequiredRenderbufferMultisampleFormatSet()
{
    FormatSet result;
    ...
    for (const auto &internalFormat : GetInternalFormatMap())
    {
        for (const auto &type : internalFormat.second)
        {
            // GLES 3.0.4 section 4.4: "Implementations must support creation of renderbuffers in
            // these required formats with up to the value of MAX_SAMPLES multisamples, with the
            // exception of signed and unsigned integer formats."
            if (type.second.es3RequiredRenderbufferFormat && type.second.componentType != GL_INT &&
                type.second.componentType != GL_UNSIGNED_INT)
            {
                result.insert(internalFormat.first);
            }
        }
...
{
    static FormatSet formatSet = BuildES3RequiredRenderbufferMultisampleFormatSet();
    return formatSet;
}
