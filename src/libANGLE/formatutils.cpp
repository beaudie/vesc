...
template <size_t extensionOffset1>
static bool RequireExt(const Version &, const Extensions &extensions)
{
    ExtensionRef ref1(extensionOffset1);
    return ref1.get(extensions);
}

// Check for a minimum client version or a single extension
template <GLuint minCoreGLMajorVersion, GLuint minCoreGLMinorVersion, size_t extensionOffset1>
...
template <GLuint minCoreGLMajorVersion,
          ...
          size_t extensionOffset1,
          size_t extensionOffset2>
static bool RequireESOrExtAndExt(const Version &clientVersion, const Extensions &extensions)
{
    ExtensionRef ref1(extensionOffset1);
    ExtensionRef ref2(extensionOffset2);
    return clientVersion >= Version(minCoreGLMajorVersion, minCoreGLMinorVersion) ||
           (ref1.get(extensions) && ref2.get(extensions));
}
...
template <size_t extensionOffset1, size_t extensionOffset2>
static bool RequireExtAndExt(const Version &, const Extensions &extensions)
{
    ExtensionRef ref1(extensionOffset1);
    ExtensionRef ref2(extensionOffset2);
    return ref1.get(extensions) && ref2.get(extensions);
}
...
static InternalFormatInfoMap BuildInternalFormatInfoMap()
{
    ...
    AddRGBAFormat(&map, GL_R8,                true,  8,  0,  0,  0, 0, GL_RED,          GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, false, SizedRGSupport,                                    AlwaysSupported, SizedRGSupport,                                    RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureRG)>  );
    ...
    AddRGBAFormat(&map, GL_RG8,               true,  8,  8,  0,  0, 0, GL_RG,           GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, false, SizedRGSupport,                                    AlwaysSupported, SizedRGSupport,                                    RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureRG)>  );
    ...
    AddRGBAFormat(&map, GL_RGB8,              true,  8,  8,  8,  0, 0, GL_RGB,          GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, false, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(rgb8rgba8)>  );
    ...
    AddRGBAFormat(&map, GL_RGB565,            true,  5,  6,  5,  0, 0, GL_RGB,          GL_UNSIGNED_SHORT_5_6_5,           GL_UNSIGNED_NORMALIZED, false, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, RequireES<2, 0>                               );
    AddRGBAFormat(&map, GL_RGBA4,             true,  4,  4,  4,  4, 0, GL_RGBA,         GL_UNSIGNED_SHORT_4_4_4_4,         GL_UNSIGNED_NORMALIZED, false, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, RequireES<2, 0>                               );
    AddRGBAFormat(&map, GL_RGB5_A1,           true,  5,  5,  5,  1, 0, GL_RGBA,         GL_UNSIGNED_SHORT_5_5_5_1,         GL_UNSIGNED_NORMALIZED, false, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, RequireES<2, 0>                               );
    AddRGBAFormat(&map, GL_RGBA8,             true,  8,  8,  8,  8, 0, GL_RGBA,         GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, false, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(textureStorage)>, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(rgb8rgba8)>  );
    ...
    AddRGBAFormat(&map, GL_SRGB8_ALPHA8,      true,  8,  8,  8,  8, 0, GL_RGBA,         GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, true,  RequireES<3, 0>,                                   AlwaysSupported, RequireES<3, 0>,                                   RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(sRGB)>       );
    AddRGBAFormat(&map, GL_R11F_G11F_B10F,    true, 11, 11, 10,  0, 0, GL_RGB,          GL_UNSIGNED_INT_10F_11F_11F_REV,   GL_FLOAT,               false, RequireES<3, 0>,                                   AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>,         RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>     );
    ...
    AddRGBAFormat(&map, GL_BGRA8_EXT,         true,  8,  8,  8,  8, 0, GL_BGRA_EXT,     GL_UNSIGNED_BYTE,                  GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>);
    AddRGBAFormat(&map, GL_BGRA4_ANGLEX,      true,  4,  4,  4,  4, 0, GL_BGRA_EXT,     GL_UNSIGNED_SHORT_4_4_4_4_REV_EXT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>);
    AddRGBAFormat(&map, GL_BGR5_A1_ANGLEX,    true,  5,  5,  5,  1, 0, GL_BGRA_EXT,     GL_UNSIGNED_SHORT_1_5_5_5_REV_EXT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>,    RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>);
    ...
    AddRGBAFormat(&map, GL_R32F,          true, 32,  0,  0,  0, 0, GL_RED,  GL_FLOAT,          GL_FLOAT,        false, SizedFloatRGSupport,        RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>, RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>,    RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>);
    AddRGBAFormat(&map, GL_RG32F,         true, 32, 32,  0,  0, 0, GL_RG,   GL_FLOAT,          GL_FLOAT,        false, SizedFloatRGSupport,        RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>, RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>,    RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>);
    AddRGBAFormat(&map, GL_RGB32F,        true, 32, 32, 32,  0, 0, GL_RGB,  GL_FLOAT,          GL_FLOAT,        false, SizedFloatRGBSupport,       RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>, RequireExt<EXTENSIONREF_OFFSET(colorBufferFloatRGB)>, NeverSupported                           );
    AddRGBAFormat(&map, GL_RGBA32F,       true, 32, 32, 32, 32, 0, GL_RGBA, GL_FLOAT,          GL_FLOAT,        false, SizedFloatRGBASupport,      RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>, SizedFloatRGBATextureAttachmentSupport,       RequireExt<EXTENSIONREF_OFFSET(colorBufferFloat)>);

    // Depth stencil formats
    //                         | Internal format         |sized| D |S | X | Format            | Type                             | Component type        | Texture supported                                                | Filterable                                      | Texture attachment                                                                   | Renderbuffer                                                                         |
    AddDepthStencilFormat(&map, GL_DEPTH_COMPONENT16,     true, 16, 0,  0, GL_DEPTH_COMPONENT, GL_UNSIGNED_SHORT,                 GL_UNSIGNED_NORMALIZED, RequireES<1, 0>,                                                   RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures)>, RequireES<1, 0>,                                                                        RequireES<1, 0>                                                                       );
    AddDepthStencilFormat(&map, GL_DEPTH_COMPONENT24,     true, 24, 0,  0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT,                   GL_UNSIGNED_NORMALIZED, RequireES<3, 0>,                                                   RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures)>, RequireES<3, 0>,                                                                        RequireES<3, 0>                                                                       );
    AddDepthStencilFormat(&map, GL_DEPTH_COMPONENT32F,    true, 32, 0,  0, GL_DEPTH_COMPONENT, GL_FLOAT,                          GL_FLOAT,               RequireES<3, 0>,                                                   RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures)>, RequireES<3, 0>,                                                                        RequireES<3, 0>                                                                       );
    AddDepthStencilFormat(&map, GL_DEPTH_COMPONENT32_OES, true, 32, 0,  0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT,                   GL_UNSIGNED_NORMALIZED, RequireExtOrExt<EXTENSIONREF_OFFSET(depthTextures), EXTENSIONREF_OFFSET(depth32)>, AlwaysSupported,                                  RequireExtOrExt<EXTENSIONREF_OFFSET(depthTextures), EXTENSIONREF_OFFSET(depth32)>,                      RequireExtOrExt<EXTENSIONREF_OFFSET(depthTextures), EXTENSIONREF_OFFSET(depth32)>                     );
    AddDepthStencilFormat(&map, GL_DEPTH24_STENCIL8,      true, 24, 8,  0, GL_DEPTH_STENCIL,   GL_UNSIGNED_INT_24_8,              GL_UNSIGNED_NORMALIZED, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures)>,                  AlwaysSupported,                                  RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures), EXTENSIONREF_OFFSET(packedDepthStencil)>, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(depthTextures), EXTENSIONREF_OFFSET(packedDepthStencil)>);
    ...
    AddLUMAFormat(&map, GL_ALPHA8_EXT,             true,  0,  8, GL_ALPHA,           GL_UNSIGNED_BYTE,  GL_UNSIGNED_NORMALIZED, RequireExt<EXTENSIONREF_OFFSET(textureStorage)>,                                      AlwaysSupported,                                 NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE8_EXT,         true,  8,  0, GL_LUMINANCE,       GL_UNSIGNED_BYTE,  GL_UNSIGNED_NORMALIZED, RequireExt<EXTENSIONREF_OFFSET(textureStorage)>,                                      AlwaysSupported,                                 NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE8_ALPHA8_EXT,  true,  8,  8, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,  GL_UNSIGNED_NORMALIZED, RequireExt<EXTENSIONREF_OFFSET(textureStorage)>,                                      AlwaysSupported,                                 NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_ALPHA16F_EXT,           true,  0, 16, GL_ALPHA,           GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE16F_EXT,       true, 16,  0, GL_LUMINANCE,       GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE_ALPHA16F_EXT, true, 16, 16, GL_LUMINANCE_ALPHA, GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_ALPHA32F_EXT,           true,  0, 32, GL_ALPHA,           GL_FLOAT,          GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE32F_EXT,       true, 32,  0, GL_LUMINANCE,       GL_FLOAT,          GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE_ALPHA32F_EXT, true, 32, 32, GL_LUMINANCE_ALPHA, GL_FLOAT,          GL_FLOAT,               RequireExtAndExt<EXTENSIONREF_OFFSET(textureStorage), EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);

    // Compressed formats, From ES 3.0.1 spec, table 3.16
    //                       | Internal format                             |W |H | BS |CC| Format | Type            | SRGB | Texture supported                                                                                                      | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_COMPRESSED_R11_EAC,                        4, 4,  64, 1, GL_RED,  GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedEACR11UnsignedTexture)>,              AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SIGNED_R11_EAC,                 4, 4,  64, 1, GL_RED,  GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedEACR11SignedTexture)>,                AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RG11_EAC,                       4, 4, 128, 2, GL_RG,   GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedEACRG11UnsignedTexture)>,             AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SIGNED_RG11_EAC,                4, 4, 128, 2, GL_RG,   GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedEACRG11SignedTexture)>,               AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB8_ETC2,                      4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2RGB8Texture)>,                    AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ETC2,                     4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, true,  RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2sRGB8Texture)>,                   AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2,  4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2PunchthroughARGB8Texture)>,       AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2, 4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, true,  RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2PunchthroughAsRGB8AlphaTexture)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA8_ETC2_EAC,                 4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2RGBA8Texture)>,                   AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ETC2_EAC,          4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireESOrExtOrExt<3, 0, EXTENSIONREF_OFFSET(compressedTextureETC), EXTENSIONREF_OFFSET(compressedETC2sRGB8Alpha8Texture)>,             AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_EXT_texture_compression_dxt1
    //                       | Internal format                   |W |H | BS |CC| Format | Type            | SRGB | Texture supported                                 | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_COMPRESSED_RGB_S3TC_DXT1_EXT,    4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionDXT1)>,    AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_S3TC_DXT1_EXT,   4, 4,  64, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionDXT1)>,    AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_ANGLE_texture_compression_dxt3
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE, 4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionDXT3)>,    AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_ANGLE_texture_compression_dxt5
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_S3TC_DXT5_ANGLE, 4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionDXT5)>,    AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_OES_compressed_ETC1_RGB8_texture
    AddCompressedFormat(&map, GL_ETC1_RGB8_OES,                   4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(compressedETC1RGB8Texture)>, AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_EXT_texture_compression_s3tc_srgb
    //                       | Internal format                       |W |H | BS |CC| Format | Type            | SRGB | Texture supported                                 | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB_S3TC_DXT1_EXT,       4, 4,  64, 3, GL_RGB,  GL_UNSIGNED_BYTE, true, RequireExt<EXTENSIONREF_OFFSET(textureCompressionS3TCsRGB)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT, 4, 4,  64, 4, GL_RGBA, GL_UNSIGNED_BYTE, true, RequireExt<EXTENSIONREF_OFFSET(textureCompressionS3TCsRGB)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT, 4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true, RequireExt<EXTENSIONREF_OFFSET(textureCompressionS3TCsRGB)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT, 4, 4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true, RequireExt<EXTENSIONREF_OFFSET(textureCompressionS3TCsRGB)>, AlwaysSupported, NeverSupported,      NeverSupported);

    // From KHR_texture_compression_astc_hdr
    //                       | Internal format                          | W | H | BS |CC| Format | Type            | SRGB | Texture supported                                                                              | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_4x4_KHR,            4,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_5x4_KHR,            5,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_5x5_KHR,            5,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_6x5_KHR,            6,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_6x6_KHR,            6,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_8x5_KHR,            8,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_8x6_KHR,            8,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_8x8_KHR,            8,  8, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_10x5_KHR,          10,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_10x6_KHR,          10,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_10x8_KHR,          10,  8, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_10x10_KHR,         10, 10, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_12x10_KHR,         12, 10, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_ASTC_12x12_KHR,         12, 12, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);

    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR,    4,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4_KHR,    5,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x5_KHR,    5,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x5_KHR,    6,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x6_KHR,    6,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x5_KHR,    8,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x6_KHR,    8,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x8_KHR,    8,  8, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x5_KHR,  10,  5, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x6_KHR,  10,  6, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x8_KHR,  10,  8, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x10_KHR, 10, 10, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x10_KHR, 12, 10, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x12_KHR, 12, 12, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExtOrExt<EXTENSIONREF_OFFSET(textureCompressionASTCHDR), EXTENSIONREF_OFFSET(textureCompressionASTCLDR)>, AlwaysSupported, NeverSupported,      NeverSupported);

    // From EXT_texture_compression_bptc
    //                       | Internal format                         | W | H | BS |CC| Format | Type            | SRGB | Texture supported                              | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_COMPRESSED_RGBA_BPTC_UNORM_EXT,         4,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionBPTC)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_EXT,   4,  4, 128, 4, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExt<EXTENSIONREF_OFFSET(textureCompressionBPTC)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_EXT,   4,  4, 128, 4, GL_RGB,  GL_FLOAT,         false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionBPTC)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_EXT, 4,  4, 128, 4, GL_RGB,  GL_FLOAT,         false, RequireExt<EXTENSIONREF_OFFSET(textureCompressionBPTC)>, AlwaysSupported, NeverSupported,      NeverSupported);
    ...
    //                       | Internal format                                                |W |H |BS |CC| Format | Type            | SRGB | Texture supported                      | Filterable     | Texture attachment | Renderbuffer |
    AddCompressedFormat(&map, GL_ETC1_RGB8_LOSSY_DECODE_ANGLE,                                 4, 4, 64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(lossyETCDecode)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB8_LOSSY_DECODE_ETC2_ANGLE,                      4, 4, 64, 3, GL_RGB,  GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(lossyETCDecode)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_LOSSY_DECODE_ETC2_ANGLE,                     4, 4, 64, 3, GL_RGB,  GL_UNSIGNED_BYTE, true,  RequireExt<EXTENSIONREF_OFFSET(lossyETCDecode)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_LOSSY_DECODE_ETC2_ANGLE,  4, 4, 64, 3, GL_RGBA, GL_UNSIGNED_BYTE, false, RequireExt<EXTENSIONREF_OFFSET(lossyETCDecode)>, AlwaysSupported, NeverSupported,      NeverSupported);
    AddCompressedFormat(&map, GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_LOSSY_DECODE_ETC2_ANGLE, 4, 4, 64, 3, GL_RGBA, GL_UNSIGNED_BYTE, true,  RequireExt<EXTENSIONREF_OFFSET(lossyETCDecode)>, AlwaysSupported, NeverSupported,      NeverSupported);

    // From GL_EXT_texture_norm16
    //                 | Internal format    |sized| R | G | B | A |S | Format | Type             | Component type        | SRGB | Texture supported                     | Filterable     | Texture attachment                    | Renderbuffer                         |
    AddRGBAFormat(&map, GL_R16_EXT,          true, 16,  0,  0,  0, 0, GL_RED,  GL_UNSIGNED_SHORT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>);
    AddRGBAFormat(&map, GL_R16_SNORM_EXT,    true, 16,  0,  0,  0, 0, GL_RED,  GL_SHORT,          GL_SIGNED_NORMALIZED,   false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, NeverSupported,                         NeverSupported                        );
    AddRGBAFormat(&map, GL_RG16_EXT,         true, 16, 16,  0,  0, 0, GL_RG,   GL_UNSIGNED_SHORT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>);
    AddRGBAFormat(&map, GL_RG16_SNORM_EXT,   true, 16, 16,  0,  0, 0, GL_RG,   GL_SHORT,          GL_SIGNED_NORMALIZED,   false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, NeverSupported,                         NeverSupported                        );
    AddRGBAFormat(&map, GL_RGB16_EXT,        true, 16, 16, 16,  0, 0, GL_RGB,  GL_UNSIGNED_SHORT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, NeverSupported,                         NeverSupported                        );
    AddRGBAFormat(&map, GL_RGB16_SNORM_EXT,  true, 16, 16, 16,  0, 0, GL_RGB,  GL_SHORT,          GL_SIGNED_NORMALIZED,   false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, NeverSupported,                         NeverSupported                        );
    AddRGBAFormat(&map, GL_RGBA16_EXT,       true, 16, 16, 16, 16, 0, GL_RGBA, GL_UNSIGNED_SHORT, GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>);
    AddRGBAFormat(&map, GL_RGBA16_SNORM_EXT, true, 16, 16, 16, 16, 0, GL_RGBA, GL_SHORT,          GL_SIGNED_NORMALIZED,   false, RequireExt<EXTENSIONREF_OFFSET(textureNorm16)>, AlwaysSupported, NeverSupported,                         NeverSupported                        );

    // Unsized formats
    //                 | Internal format  |sized | R | G | B | A |S | Format           | Type                          | Component type        | SRGB | Texture supported                             | Filterable     | Texture attachment                            | Renderbuffer |
    AddRGBAFormat(&map, GL_RED,            false,  8,  0,  0,  0, 0, GL_RED,            GL_UNSIGNED_BYTE,               GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureRG)>,             AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureRG)>,             NeverSupported);
    ...
    AddRGBAFormat(&map, GL_RG,             false,  8,  8,  0,  0, 0, GL_RG,             GL_UNSIGNED_BYTE,               GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureRG)>,             AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureRG)>,             NeverSupported);
    ...
    AddRGBAFormat(&map, GL_SRGB,           false,  8,  8,  8,  0, 0, GL_SRGB,           GL_UNSIGNED_BYTE,               GL_UNSIGNED_NORMALIZED, true,  RequireExt<EXTENSIONREF_OFFSET(sRGB)>,                  AlwaysSupported, NeverSupported,                                 NeverSupported);
    AddRGBAFormat(&map, GL_SRGB_ALPHA_EXT, false,  8,  8,  8,  8, 0, GL_SRGB_ALPHA_EXT, GL_UNSIGNED_BYTE,               GL_UNSIGNED_NORMALIZED, true,  RequireExt<EXTENSIONREF_OFFSET(sRGB)>,                  AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(sRGB)>,                  NeverSupported);
    AddRGBAFormat(&map, GL_BGRA_EXT,       false,  8,  8,  8,  8, 0, GL_BGRA_EXT,       GL_UNSIGNED_BYTE,               GL_UNSIGNED_NORMALIZED, false, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>, AlwaysSupported, RequireExt<EXTENSIONREF_OFFSET(textureFormatBGRA8888)>, NeverSupported);
    ...
    AddRGBAFormat(&map, GL_RED,          false, 16,  0,  0,  0, 0, GL_RED,  GL_HALF_FLOAT_OES,               GL_FLOAT, false, RequireExtAndExt<EXTENSIONREF_OFFSET(textureHalfFloat), EXTENSIONREF_OFFSET(textureRG)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,                                                                     NeverSupported);
    AddRGBAFormat(&map, GL_RG,           false, 16, 16,  0,  0, 0, GL_RG,   GL_HALF_FLOAT_OES,               GL_FLOAT, false, RequireExtAndExt<EXTENSIONREF_OFFSET(textureHalfFloat), EXTENSIONREF_OFFSET(textureRG)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,                                                                     NeverSupported);
    AddRGBAFormat(&map, GL_RGB,          false, 16, 16, 16,  0, 0, GL_RGB,  GL_HALF_FLOAT_OES,               GL_FLOAT, false, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloat)>,                               RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,                                                                     NeverSupported);
    AddRGBAFormat(&map, GL_RGBA,         false, 16, 16, 16, 16, 0, GL_RGBA, GL_HALF_FLOAT_OES,               GL_FLOAT, false, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloat)>,                               RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, RequireExtAndExt<EXTENSIONREF_OFFSET(textureHalfFloat), EXTENSIONREF_OFFSET(colorBufferHalfFloat)>, NeverSupported);
    AddRGBAFormat(&map, GL_RED,          false, 32,  0,  0,  0, 0, GL_RED,  GL_FLOAT,                        GL_FLOAT, false, RequireExtAndExt<EXTENSIONREF_OFFSET(textureFloat), EXTENSIONREF_OFFSET(textureRG)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,                                                                     NeverSupported);
    AddRGBAFormat(&map, GL_RG,           false, 32, 32,  0,  0, 0, GL_RG,   GL_FLOAT,                        GL_FLOAT, false, RequireExtAndExt<EXTENSIONREF_OFFSET(textureFloat), EXTENSIONREF_OFFSET(textureRG)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,                                                                     NeverSupported);
    AddRGBAFormat(&map, GL_RGB,          false, 32, 32, 32,  0, 0, GL_RGB,  GL_FLOAT,                        GL_FLOAT, false, RequireExt<EXTENSIONREF_OFFSET(textureFloat)>,                                   RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,                                                                     NeverSupported);
    ...
    AddRGBAFormat(&map, GL_RGBA,         false, 32, 32, 32, 32, 0, GL_RGBA, GL_FLOAT,                        GL_FLOAT, false, RequireExt<EXTENSIONREF_OFFSET(textureFloat)>,                                   RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,                                                                     NeverSupported);
    ...
    AddLUMAFormat(&map, GL_ALPHA,           false,  0, 16, GL_ALPHA,           GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE,       false, 16,  0, GL_LUMINANCE,       GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE_ALPHA, false, 16, 16, GL_LUMINANCE_ALPHA, GL_HALF_FLOAT_OES, GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureHalfFloat)>, RequireExt<EXTENSIONREF_OFFSET(textureHalfFloatLinear)>, NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_ALPHA,           false,  0, 32, GL_ALPHA,           GL_FLOAT,          GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE,       false, 32,  0, GL_LUMINANCE,       GL_FLOAT,          GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);
    AddLUMAFormat(&map, GL_LUMINANCE_ALPHA, false, 32, 32, GL_LUMINANCE_ALPHA, GL_FLOAT,          GL_FLOAT,               RequireExt<EXTENSIONREF_OFFSET(textureFloat)>,     RequireExt<EXTENSIONREF_OFFSET(textureFloatLinear)>,     NeverSupported,      NeverSupported);
    ...
    AddDepthStencilFormat(&map, GL_DEPTH_STENCIL,   false, 24, 8,  0, GL_DEPTH_STENCIL,   GL_UNSIGNED_INT_24_8,              GL_UNSIGNED_NORMALIZED, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(packedDepthStencil)>, AlwaysSupported, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(packedDepthStencil)>, RequireESOrExt<3, 0, EXTENSIONREF_OFFSET(packedDepthStencil)>);
