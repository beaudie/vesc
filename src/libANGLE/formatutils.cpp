...
static bool SizedHalfFloatOESRenderbufferSupport(const Version &clientVersion,
                                                 const Extensions &extensions)
{
    return extensions.textureHalfFloat && extensions.colorBufferHalfFloat;
}
...
{
    return SizedHalfFloatOESSupport(clientVersion, extensions) || clientVersion >= Version(3, 0);
}
...
static bool SizedHalfFloatRGBARenderbufferSupport(const Version &clientVersion,
                                                  const Extensions &extensions)
{
    return SizedHalfFloatOESRenderbufferSupport(clientVersion, extensions) ||
           extensions.colorBufferFloat;
}
...
static bool SizedHalfFloatOESRGRenderbufferSupport(const Version &clientVersion,
                                                   const Extensions &extensions)
{
    return extensions.textureRG && extensions.textureHalfFloat && extensions.colorBufferHalfFloat;
}
...
{
    return SizedHalfFloatOESRGSupport(clientVersion, extensions) || clientVersion >= Version(3, 0);
}
...
static bool SizedHalfFloatRGRenderbufferSupport(const Version &clientVersion,
                                                const Extensions &extensions)
{
    return SizedHalfFloatOESRGRenderbufferSupport(clientVersion, extensions) ||
           extensions.colorBufferFloat;
}
...
// 2. "AlwaysSupported" in "Filterable" actually means - on the condition of "Texture supported".
//    "Texture attachment" and "Renderbuffer" are explicit, however.
// 3. Sized half float types are a combination of GL_HALF_FLOAT and GL_HALF_FLOAT_OES support,
//    due to a limitation that only one type for sized formats is allowed.
// TODO(ynovikov): verify support fields of BGRA, depth, stencil and compressed formats
static InternalFormatInfoMap BuildInternalFormatInfoMap()
{
    ...
    //                 | Internal format |sized| R | G | B | A |S | Format | Type             | Component type | SRGB | Texture supported         | Filterable                                               | Texture attachment                          | Renderbuffer                            |
    AddRGBAFormat(&map, GL_R16F,          true, 16,  0,  0,  0, 0, GL_RED,  GL_HALF_FLOAT,     GL_FLOAT,        false, SizedHalfFloatRGSupport,    RequireESOrExt<3, 0, &Extensions::textureHalfFloatLinear>, SizedHalfFloatRGTextureAttachmentSupport,     SizedHalfFloatRGRenderbufferSupport      );
    AddRGBAFormat(&map, GL_RG16F,         true, 16, 16,  0,  0, 0, GL_RG,   GL_HALF_FLOAT,     GL_FLOAT,        false, SizedHalfFloatRGSupport,    RequireESOrExt<3, 0, &Extensions::textureHalfFloatLinear>, SizedHalfFloatRGTextureAttachmentSupport,     SizedHalfFloatRGRenderbufferSupport      );
    AddRGBAFormat(&map, GL_RGB16F,        true, 16, 16, 16,  0, 0, GL_RGB,  GL_HALF_FLOAT,     GL_FLOAT,        false, SizedHalfFloatSupport,      RequireESOrExt<3, 0, &Extensions::textureHalfFloatLinear>, SizedHalfFloatOESTextureAttachmentSupport,    SizedHalfFloatOESRenderbufferSupport     );
    AddRGBAFormat(&map, GL_RGBA16F,       true, 16, 16, 16, 16, 0, GL_RGBA, GL_HALF_FLOAT,     GL_FLOAT,        false, SizedHalfFloatSupport,      RequireESOrExt<3, 0, &Extensions::textureHalfFloatLinear>, SizedHalfFloatRGBATextureAttachmentSupport,   SizedHalfFloatRGBARenderbufferSupport    );
    // Only one type is allowed for sized formats, thus GL_HALF_FLOAT combines both types
    //AddRGBAFormat(&map, GL_R16F,        true, 16,  0,  0,  0, 0, GL_RED,  GL_HALF_FLOAT_OES, GL_FLOAT,        false, SizedHalfFloatOESRGSupport, RequireExt<&Extensions::textureHalfFloatLinear>,           SizedHalfFloatOESRGTextureAttachmentSupport,  SizedHalfFloatOESRGRenderbufferSupport   );
    //AddRGBAFormat(&map, GL_RG16F,       true, 16, 16,  0,  0, 0, GL_RG,   GL_HALF_FLOAT_OES, GL_FLOAT,        false, SizedHalfFloatOESRGSupport, RequireExt<&Extensions::textureHalfFloatLinear>,           SizedHalfFloatOESRGTextureAttachmentSupport,  SizedHalfFloatOESRGRenderbufferSupport   );
    //AddRGBAFormat(&map, GL_RGB16F,      true, 16, 16, 16,  0, 0, GL_RGB,  GL_HALF_FLOAT_OES, GL_FLOAT,        false, SizedHalfFloatOESSupport,   RequireExt<&Extensions::textureHalfFloatLinear>,           SizedHalfFloatOESTextureAttachmentSupport,    SizedHalfFloatOESRenderbufferSupport     );
    //AddRGBAFormat(&map, GL_RGBA16F,     true, 16, 16, 16, 16, 0, GL_RGBA, GL_HALF_FLOAT_OES, GL_FLOAT,        false, SizedHalfFloatOESSupport,   RequireExt<&Extensions::textureHalfFloatLinear>,           SizedHalfFloatOESTextureAttachmentSupport,    SizedHalfFloatOESRenderbufferSupport     );
