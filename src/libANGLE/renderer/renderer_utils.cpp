...
{
  public:
    InterfaceBlockInfo(bool allowHLSLEncoder) : mAllowHLSLEncoder(allowHLSLEncoder) {}
                                                 ...
                                                 const gl::ProgramLinkedResources &resources) const
{
    ...
    InterfaceBlockInfo uniformBlockInfo(mAllowHLSLEncoder);
    ...
    InterfaceBlockInfo shaderStorageBlockInfo(mAllowHLSLEncoder);
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        gl::Shader *shader = programState.getAttachedShader(shaderType);
        if (shader)
