...
{
    if (fastCopyFunctions.has(format->id))
    {
        return fastCopyFunctions.get(format->id);
    }
    else
    {
        return nullptr;
                ...
                uint8_t *destWithoutOffset)
{
    ...
    ColorCopyFunction fastCopyFunc =
        GetFastCopyFunction(sourceFormat.fastCopyFunctions, params.destFormat);
    ...
    for (int y = 0; y < params.area.height; ++y)
    {
        for (int x = 0; x < params.area.width; ++x)
        {
            uint8_t *dest      = destWithOffset + y * params.outputPitch + x * params.destFormat->pixelBytes;
            const uint8_t *src = source + y * inputPitch + x * sourceFormat.pixelBytes;
