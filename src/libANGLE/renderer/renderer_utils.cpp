...
// In the case of gl::SamplerFormat::Shadow, clearColor value is set to {0, 0, 0, 0}. However, the
// necessary format and size for this case is GLushort[1]. Even though the gap between the real and
// necessary format exists there is no problem. Because GLubyte[4] can be interpreted as
// GLushort[2]. So, the first item, the zero value, will used as data of the texture.
    ...
    gl::Texture **textureOut)
{
    *textureOut = mIncompleteTextures[format][type].get();
    ...
    {
        // To avoid the undefined result, the shadow sampler with a depth texture should be set the
