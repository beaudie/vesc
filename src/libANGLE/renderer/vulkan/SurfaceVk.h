...
struct UnlockedTryAcquireResult
{
...
class WindowSurfaceVk : public SurfaceVk
{
  ...
  private:
    ...
    bool needsAcquireNextSwapchainImage() const;
    ...
    std::atomic<bool> mNeedToAcquireNextSwapchainImage;

    // Data used to call vkAcquireNextImageKHR without necessarily holding the share group lock.
    // The result of this operation can be found in mUnlockedTryAcquireResult, which is processed
    // once the share group lock is taken in the future.
    //
    // |mUnlockedTryAcquireData::mutex| is necessary to hold when making the vkAcquireNextImageKHR
    ...
    // called during a GL call, immediately afterwards), the contents of |mUnlockedTryAcquireResult|
    // can be accessed without |mUnlockedTryAcquireData::mutex| because the share group lock is
    // already taken, and no thread can be attempting an unlocked vkAcquireNextImageKHR.
    impl::UnlockedTryAcquireData mUnlockedTryAcquireData;
    impl::UnlockedTryAcquireResult mUnlockedTryAcquireResult;
