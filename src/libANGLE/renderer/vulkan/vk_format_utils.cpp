                        ...
                        SupportTest hasSupport)
{
    ...
    // We couldn't find a valid fallback, try ignoring the skip otherwise return -1
    return (skip > 0 && hasSupport(renderer, info[0].format)) ? 0 : -1;
}
...
void Format::initImageFallback(Renderer *renderer, const ImageFormatInitInfo *info, int numInfo)
{
    ...
    if (i == -1)
    {
        // If no fallback formats are supported, nothing left to do
        return;
    }
