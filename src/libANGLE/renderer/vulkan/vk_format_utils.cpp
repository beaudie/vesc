        ...
        default:
            if (angleFormat.hasDepthOrStencilBits())
            {
                ...
                swizzleStateOut->swizzleGreen =
                    angleFormat.depthBits > 8 ? swizzleState.swizzleGreen : GL_ZERO;
                swizzleStateOut->swizzleBlue =
                    angleFormat.depthBits > 16 ? swizzleState.swizzleBlue : GL_ZERO;
                swizzleStateOut->swizzleAlpha =
                    angleFormat.stencilBits > 0 ? swizzleState.swizzleAlpha : GL_ZERO;
            }
            else
            {
