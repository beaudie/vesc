    ...
    switch (format.internalFormat)
    {
        ...
        default:
            ...
            else
            {
                ...
                bool hasRed   = angleFormat.redBits > 0;
                bool hasGreen = angleFormat.greenBits > 0;
                bool hasBlue  = angleFormat.blueBits > 0;
                bool hasAlpha = angleFormat.alphaBits > 0;
                if (angleFormat.isBlock)
                {
                    // Color bits are all zero for blocked formats.
                    hasRed = hasGreen = hasBlue = hasAlpha = true;
                }
                internalSwizzle.swizzleRed   = hasRed ? GL_RED : GL_ZERO;
                internalSwizzle.swizzleGreen = hasGreen ? GL_GREEN : GL_ZERO;
                internalSwizzle.swizzleBlue  = hasBlue ? GL_BLUE : GL_ZERO;
                internalSwizzle.swizzleAlpha = hasAlpha ? GL_ALPHA : GL_ONE;
            }
            break;
    }
