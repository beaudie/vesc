...
ANGLE_INLINE void CommandBuffer::setEvent(const vk::Event &event, VkPipelineStageFlags stageMask)
{
    ASSERT(valid() && event.valid());
    vkCmdSetEvent(mHandle, event.getHandle(), stageMask);
}

ANGLE_INLINE void CommandBuffer::resetEvent(const vk::Event &event, VkPipelineStageFlags stageMask)
{
    ASSERT(valid() && event.valid());
