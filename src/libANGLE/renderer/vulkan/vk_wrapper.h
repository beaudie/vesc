...
class Semaphore final : public WrappedObject<Semaphore, VkSemaphore>
{
  public:
    ...
    VkResult init(VkDevice device, const VkAllocationCallbacks *callbacks);
...
ANGLE_INLINE VkResult Semaphore::init(VkDevice device, const VkAllocationCallbacks *callbacks)
{
    ASSERT(!valid());
...
ANGLE_INLINE void SamplerYcbcrConversion::destroy(VkDevice device)
{
    if (valid())
    {
        ...
        MemoryAllocationCallback memTest;
        VkAllocationCallbacks memACB = (VkAllocationCallbacks)memTest;
        vkDestroySamplerYcbcrConversionKHR(device, mHandle, &memACB);
        mHandle = VK_NULL_HANDLE;
    }
}
...
ANGLE_INLINE void Fence::destroy(VkDevice device)
{
    if (valid())
    {
        ...
        MemoryAllocationCallback memTest;
        VkAllocationCallbacks memACB = (VkAllocationCallbacks)memTest;
        vkDestroyFence(device, mHandle, &memACB);
        mHandle = VK_NULL_HANDLE;
    }
}

ANGLE_INLINE VkResult Fence::init(VkDevice device,
                                  const VkFenceCreateInfo &createInfo,
                                  const VkAllocationCallbacks *callbacks)
{
    ASSERT(!valid());
    WARN() << "Fence::init";
    return vkCreateFence(device, &createInfo, callbacks, &mHandle);
}
