...
const vk::CommandBufferAndState &CommandBufferNode::getOutsideRenderPassCommands() const
{
    return mOutsideRenderPassCommands;
}

const vk::CommandBufferAndState &CommandBufferNode::getInsideRenderPassCommands() const
{
    return mInsideRenderPassCommands;
}

vk::Error CommandBufferNode::startRecording(VkDevice device,
                                            const vk::CommandPool &commandPool,
                                            CommandBufferAndState **commandsOut)
{
    if (!mOutsideRenderPassCommands.valid())
    {
        ANGLE_TRY(mOutsideRenderPassCommands.ensureStarted(device, commandPool,
...
vk::Error CommandBufferNode::startRenderPassRecording(VkDevice device,
                                                      const vk::CommandPool &commandPool,
                                                      CommandBufferAndState **commandsOut)
{
    if (!mInsideRenderPassCommands.valid())
    {
        ANGLE_TRY(mInsideRenderPassCommands.ensureStarted(device, commandPool,
                                                          VK_COMMAND_BUFFER_LEVEL_SECONDARY));
    }
...
void CommandBufferNode::storeRenderPassFramebuffer(const vk::Framebuffer &framebuffer,
...
const vk::Framebuffer &CommandBufferNode::getRenderPassFramebuffer() const
{
