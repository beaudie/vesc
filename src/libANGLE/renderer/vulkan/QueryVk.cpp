...
angle::Result QueryVk::onRenderPassStart(ContextVk *contextVk)
{
    ...
    bool isXfbActive = contextVk->getState().getCurrentTransformFeedback()->isActive();
    bool isXfbPaused = contextVk->getState().getCurrentTransformFeedback()->isPaused();
    if (mType == gl::QueryType::TransformFeedbackPrimitivesWritten && (!isXfbActive || isXfbPaused))
    {
        // XFB is either not active or it's paused, in which case no primitives will be generated.
        // Additionally, ARM will not complete any PrimitivesGenerated queries while XFB is inactive
        // or paused.
