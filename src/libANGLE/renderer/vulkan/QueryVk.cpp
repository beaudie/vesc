...
angle::Result QueryVk::getResult(const gl::Context *context, bool wait)
{
    ...
    if (wait)
    {
        ...
        uint64_t v;
        for (auto q = mStashedQueryHelpers.begin(); q != mStashedQueryHelpers.end();)
        {
            ANGLE_TRY(q->getUint64Result(contextVk, &v));
            mCachedResult += v;
            q = mStashedQueryHelpers.erase(q);
        }
    }
    else
    {
        ...
        uint64_t v;
        for (auto q = mStashedQueryHelpers.rbegin(); q != mStashedQueryHelpers.rend(); q++)
        {
            available = false;
            // we need to check results availability here as well in case renderpasses execute out
            // of order
            ANGLE_TRY(q->getUint64ResultNonBlocking(contextVk, &v, &available));
            if (!available)
            {
                return angle::Result::Continue;
            }
