...
void QueryVk::releaseQueries(ContextVk *contextVk)
{
    ...
    {
        mQueryHelper.resetAndRelease([contextVk, queryPool](vk::QueryHelper &&queryHelper) {
            queryPool->freeQuery(contextVk, &queryHelper);
        });
    }
...
void QueryVk::releaseStashedQueries(ContextVk *contextVk)
{
    ...
    for (vk::Shared<vk::QueryHelper> &query : mStashedQueryHelpers)
    {
        ...
        query.resetAndRelease([contextVk, queryPool](vk::QueryHelper &&queryHelper) {
            queryPool->freeQuery(contextVk, &queryHelper);
        });
    }
...
void QueryVk::stashQueryHelper()
{
    ...
    mQueryHelper.resetAndRelease([](vk::QueryHelper &&) { UNREACHABLE(); });
}
