...
angle::Result QueryVk::getResult(const gl::Context *context)
{
    ...
    ANGLE_TRY(mQueryHelper.getQueryPool()->getResults(contextVk, mQueryHelper.getQuery(), 1,
                                                      sizeof mCachedResult, &mCachedResult,
...
gl::Error QueryVk::isResultAvailable(const gl::Context *context, bool *available)
{
    ...
    ANGLE_TRY_RESULT(mQueryHelper.getQueryPool()->getResults(
                         contextVk, mQueryHelper.getQuery(), 1, sizeof mCachedResult,
                         &mCachedResult, sizeof mCachedResult, VK_QUERY_RESULT_64_BIT),
                     result);

    mCachedResultValid = result == angle::Result::Continue();
