...
{
    mQueryHelper = new vk::RefCountedQueryHelper;
    mQueryHelper->addRef();
...
{
    mQueryHelper = shareQuery->mQueryHelper;
    ASSERT(mQueryHelper);
...
void QueryVk::releaseQueries(ContextVk *contextVk)
{
    ...
    if (mQueryHelper)
    {
        mQueryHelper->releaseRef();
        if (!mQueryHelper->isReferenced())
        {
            queryPool->freeQuery(contextVk, &mQueryHelper->get());
            SafeDelete(mQueryHelper);
        }
    }
...
void QueryVk::releaseStashedQueries(ContextVk *contextVk)
{
    ...
    for (vk::RefCountedQueryHelper *query : mStashedQueryHelpers)
    {
        query->releaseRef();
        if (!query->isReferenced())
        {
            queryPool->freeQuery(contextVk, &query->get());
            delete query;
        }
    }
...
{
    ASSERT(mQueryHelper != nullptr);
    mStashedQueryHelpers.push_back(mQueryHelper);
    mQueryHelper = nullptr;
}
...
angle::Result QueryVk::end(const gl::Context *context)
{
    ...
    switch (mType)
    {
        ...
        {
            QueryVk *shareQuery = GetShareQuery(contextVk, mType);
            ASSERT(shareQuery == nullptr || mQueryHelper == shareQuery->mQueryHelper);
            ...
            if (shareQuery && shareQuery->mQueryHelper != nullptr)
            {
                // This serves the following scenario (TF = TransformFeedbackPrimitivesWritten, PG =
                // PrimitivesGenerated):
                //
...
bool QueryVk::isUsedInRecordedCommands() const
{
    ...
    for (const vk::RefCountedQueryHelper *query : mStashedQueryHelpers)
    {
...
angle::Result QueryVk::getResult(const gl::Context *context, bool wait)
{
    ...
    ASSERT(!IsRenderPassQuery(contextVk, mType) || mQueryHelper->get().hasCommands());
    ...
    switch (mType)
    {
        ...
        case gl::QueryType::TransformFeedbackPrimitivesWritten:
            ...
            {
                fprintf(stderr, "%lu vs %lu\n", result.getResult(), result.getSecondaryResult());
            }
            else
            {
                fprintf(stderr, "getResult returned empty!\n");
            }
            break;
        case gl::QueryType::PrimitivesGenerated:
            mCachedResult = result.getSecondaryResult();
            fprintf(stderr, "%lu vs %lu\n", result.getResult(), result.getSecondaryResult());
            break;
        default:
            UNREACHABLE();
