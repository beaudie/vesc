...
bool QueryVk::isUsedInRecordedCommands(vk::Context *context) const
{
    ...
    if (mQueryHelper.get().usedInRecordedCommand(context))
    {
        return true;
    }
    ...
    {
        if (query.get().usedInRecordedCommand(context))
        {
            return true;
        }
    }
...
bool QueryVk::isCurrentlyInUse(RendererVk *renderer) const
{
    ...
    if (mQueryHelper.get().isCurrentlyInUse(renderer->getLastCompletedQueueSerials()))
    {
        return true;
    }
    ...
    for (const vk::Shared<vk::QueryHelper> &query : mStashedQueryHelpers)
    {
        if (query.get().isCurrentlyInUse(renderer->getLastCompletedQueueSerials()))
        {
            return true;
        }
    }
...
{
    if (mQueryHelper.get().usedInRunningCommands(contextVk->getLastCompletedQueueSerials()))
    {
    ...
    for (vk::Shared<vk::QueryHelper> &query : mStashedQueryHelpers)
    {
        if (query.get().usedInRunningCommands(contextVk->getLastCompletedQueueSerials()))
        {
...
angle::Result QueryVk::getResult(const gl::Context *context, bool wait)
{
    ...
    if (isUsedInRecordedCommands(contextVk))
    {
        ...
        ASSERT(!mQueryHelperTimeElapsedBegin.usedInRecordedCommand(contextVk));
        ASSERT(!mQueryHelper.get().usedInRecordedCommand(contextVk));
    }
