                                ...
                                const gl::TextureBarrierVector &textureBarriers)
{
    ...
    if (!bufferBarriers.empty())
    {
        ...
        for (gl::Buffer *buffer : bufferBarriers)
        {
            ...
            // If there were GL commands using this buffer prior to this call, flush them.
            if (bufferHelper.isCurrentlyInGraph())
            {
            ...
            // Ownership queue transfer.
            bufferHelper.changeQueue(contextVk->getRenderer()->getQueueFamilyIndex(), queueChange);
        }
    }
    ...
    if (!textureBarriers.empty())
    {
        ...
        for (const gl::TextureAndLayout &textureAndLayout : textureBarriers)
        {
            ...
            // If there were GL commands using this image prior to this call, flush them.
            if (image.isCurrentlyInGraph())
            {
                ANGLE_TRY(contextVk->flushImpl(nullptr));
            }
            ...
            // Ownership queue transfer.
            image.changeLayoutAndQueue(image.getAspectFlags(), layout,
                                  ...
                                  const gl::TextureBarrierVector &textureBarriers)
{
    ...
    if (!textureBarriers.empty())
    {
        ...
        for (const gl::TextureAndLayout &textureAndLayout : textureBarriers)
        {
            ...
            // Ownership queue transfer and layout transition.
