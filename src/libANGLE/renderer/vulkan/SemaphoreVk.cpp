                                ...
                                const gl::TextureBarrierVector &textureBarriers)
{
    ...
    if (!bufferBarriers.empty())
    {
        ...
        for (gl::Buffer *buffer : bufferBarriers)
        {
            ...
            access.onBufferSemaphoreAcquire(this, &bufferHelper);
    ...
    if (!textureBarriers.empty())
    {
        ...
        for (const gl::TextureAndLayout &textureBarrier : textureBarriers)
        {
            ...
            vk::CommandBufferAccess access;
            vk::OutsideRenderPassCommandBuffer *commandBuffer;
            access.onImageSemaphoreAcquire(this, &image);
            ANGLE_TRY(contextVk->getOutsideRenderPassCommandBuffer(access, &commandBuffer));
                                  ...
                                  const gl::TextureBarrierVector &textureBarriers)
{
    ...
    if (!bufferBarriers.empty())
    {
        ...
        for (gl::Buffer *buffer : bufferBarriers)
        {
            ...
            vk::CommandBufferAccess access;
            vk::OutsideRenderPassCommandBuffer *commandBuffer;
            access.onBufferSemaphoreRelease(this, &bufferHelper);
            ANGLE_TRY(contextVk->getOutsideRenderPassCommandBuffer(access, &commandBuffer));
    ...
    if (!textureBarriers.empty())
    {
        ...
        for (const gl::TextureAndLayout &textureBarrier : textureBarriers)
        {
            ...
            vk::CommandBufferAccess access;
            vk::OutsideRenderPassCommandBuffer *commandBuffer;
            access.onImageSemaphoreRelease(this, &image);
            ANGLE_TRY(contextVk->getOutsideRenderPassCommandBuffer(access, &commandBuffer));
