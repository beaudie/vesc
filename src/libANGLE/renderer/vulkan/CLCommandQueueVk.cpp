                                                      ...
                                                      ImageBufferCopyDirection direction)
{
    ...
    imageVk.setExtent(region, &copyRegion.imageExtent);
    imageVk.setOffset(origin, &copyRegion.imageOffset);
    imageVk.setSubresourceLayers(origin, imageVk.getDesc().type, &copyRegion.imageSubresource);
                                                 ...
                                                 CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    if (blocking)
    {
        ...
        {
            ANGLE_TRY(imageVk.copyStagingToFromWithPitch(ptr, region, rowPitch, slicePitch, true));
        }
    }
    else
    {
        ...
        if (rowPitch != 0 || slicePitch != 0)
        {
            // TODO: Look into better enqueue handling of this op if non-blocking
            ANGLE_TRY(finish());
            ANGLE_TRY(imageVk.copyStagingToFromWithPitch(ptr, region, rowPitch, slicePitch, true));
        }
    }
                                                 ...
                                                 CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    srcImageVk->setExtent(region, &copyRegion.extent);
    srcImageVk->setOffset(srcOrigin, &copyRegion.srcOffset);
    dstImageVk->setOffset(dstOrigin, &copyRegion.dstOffset);
    srcImageVk->setSubresourceLayers(srcOrigin, dstImageVk->getDesc().type,
                                     &copyRegion.srcSubresource);
    dstImageVk->setSubresourceLayers(dstOrigin, srcImageVk->getDesc().type,
