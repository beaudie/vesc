                                                   ...
                                                   CLEventImpl::CreateFunc *eventCreateFunc)
{
    if (!waitEvents.empty())
    {
        ANGLE_TRY(mContext->waitForEvents(waitEvents));
    }
                                                     ...
                                                     CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    cl::NDRange ndrange(workDim, globalWorkOffset, globalWorkSize, localWorkSize);
                                            ...
                                            CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    return enqueueNDRangeKernel(kernel, 1, NULL, globalWorkSize, localWorkSize, waitEvents,
                                eventCreateFunc);
}
...
{
    if (!mHostBufferUpdateList.empty())
    {
        for (const auto &bufUpdate : mHostBufferUpdateList)
        {
            ANGLE_TRY(bufUpdate.vkBuf->copyStagingTo(bufUpdate.hostPtr, bufUpdate.offset,
                                                     bufUpdate.size));
...
angle::Result CLCommandQueueVk::processKernelResources(CLKernelVk &kernelVk, cl::NDRange &ndrange)
{
    ...
    for (const auto &arg : kernelVk.getArgs())
    {
        switch (arg.type)
        {
            ...
            {
                PushConstant pc;
                pc.offset = arg.pushConstOffset;
                pc.size   = arg.pushConstantSize;
                pc.data.resize(arg.handleSize);
                std::memcpy(pc.data.data(), arg.handle, arg.handleSize);
                pcs.push_back(pc);
                break;
            }
    ...
    // Bind resources to command buffer
    ...
    // Push argument push-constant(s) to command buffer
    for (const auto &pc : pcs)
    {
        mComputePassCommands->getCommandBuffer().pushConstants(kernelVk.getPipelineLayout().get(),
                                                               VK_SHADER_STAGE_COMPUTE_BIT,
                                                               pc.offset, pc.size, pc.data.data());
    }
...
angle::Result CLCommandQueueVk::onResourceAccess(const vk::CommandBufferAccess &access)
{
    ...
    for (const vk::CommandBufferBufferAccess &bufferAccess : access.getWriteBuffers())
    {
        ...
        {
            // currently all are host visible visible nothing to do
        }
    }

    // TODO (g.tammana) handle images angleproject:8512
