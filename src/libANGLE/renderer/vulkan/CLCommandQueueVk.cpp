                                                 ...
                                                 CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    {
        ANGLE_TRY(
            copyImageToFromBuffer(imageVk, imageVk.getStagingBuffer(), origin, region, 0, true));
        ANGLE_TRY(finish());
    ...
    else
    {
        ...
        ANGLE_TRY(
            copyImageToFromBuffer(imageVk, transferBufferVk.getBuffer(), origin, region, 0, true));
    }
                                                  ...
                                                  CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    ANGLE_TRY(copyImageToFromBuffer(imageVk, imageVk.getStagingBuffer(), origin, region, 0, false));
                                                ...
                                                void *&mapPtr)
{
    ...
    ANGLE_TRY(copyImageToFromBuffer(imageVk, imageVk.getStagingBuffer(), origin, region, 0, true));

    if (blocking || mComputePassCommands->usesBufferForWrite(imageVk.getStagingBuffer()))
    {
        ANGLE_TRY(finish());
    }
    mapPtr = static_cast<void *>(imageVk.getStagingBuffer().getMappedMemory());
    if (image.getFlags().isSet(CL_MEM_USE_HOST_PTR))
    {
