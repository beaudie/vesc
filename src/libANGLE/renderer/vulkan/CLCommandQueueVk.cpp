                                                  ...
                                                  CLEventImpl::CreateFunc *eventCreateFunc)
{
    ...
    else
    {
        ...
        if (!mComputePassCommands->getCommandBuffer().empty() && bufferVk.isWritable())
        {
            ...
            mComputePassCommands->executeBarriers2(mContext, 0, 1, &memoryBarrier2, 0, nullptr, 0,
                                                   nullptr);
        }
...
angle::Result CLCommandQueueVk::enqueueMarker(CLEventImpl::CreateFunc &eventCreateFunc)
{
    ...
    mComputePassCommands->executeBarriers2(mContext, 0, 1, &memoryBarrier2, 0, nullptr, 0, nullptr);
    ...
    if (!waitEvents.empty())
    {
        ...
        for (const cl::EventPtr &event : waitEvents)
        {
            ...
            else if (event->getCommandQueue() == &mCommandQueue && !insertedBarrier)
            {
                ...
                mComputePassCommands->executeBarriers2(mContext, 0, 1, &memoryBarrier2, 0, nullptr,
                                                       0, nullptr);
            }
