...
std::string ProcessBuildOptions(std::string options, CLProgramVk::BuildType buildType)
{
    std::string processedOptions;
    std::vector<std::string> optionTokens;
    angle::SplitStringAlongWhitespace(options, &optionTokens);

    // Need to remove/replace options that are not 1-1 mapped to clspv
    for (const std::string &optionToken : optionTokens)
    {
        if (optionToken.find("-create-library") != std::string::npos &&
            buildType == CLProgramVk::BuildType::LINK)
        {
                                ...
                                const DeviceProgramDatas &inputProgramDatas)
{
    ...
    const bool createLibrary = options.find(" -create-library ") != std::string::npos;
    std::string processedOptions =
        ProcessBuildOptions(std::string(options + " " + internalOptions), buildType);
        ...
        if (buildType != BuildType::BINARY)
        {
            ...
            switch (buildType)
            {
                ...
                case BuildType::LINK:
                {
                    ...
                    std::memcpy(deviceProgramData.IR.data(), clspvCtx.mOutputBin,
                                clspvCtx.mOutputBinSize);
                    deviceProgramData.binaryType = createLibrary
