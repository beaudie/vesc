                                   ...
                                   size_t *valueSizeRet) const
{
    ...
    switch (name)
    {
        case cl::ProgramInfo::NumKernels:
            for (const DeviceProgramsEntry &dev : mAssociatedDevicePrograms)
            {
...
{
    for (const DeviceProgramsEntry &deviceProgramEntry : mAssociatedDevicePrograms)
    {
        if (deviceProgramEntry.second.containsKernel(kernelName))
        {
            return &deviceProgramEntry.second;
        }
    }
                                ...
                                const DeviceProgramDatas &inputProgramDatas)
{
    ...
    for (const cl::RefPointer<cl::Device> &device : devices)
    {
        ...
        if (deviceProgramData.binaryType == CL_PROGRAM_BINARY_TYPE_EXECUTABLE)
        {
            ...
            for (const PushConstantsEntry &pc : deviceProgramData.reflectionData.pushConstants)
            {
