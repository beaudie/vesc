...
class ProgramExecutableVk : public ProgramExecutableImpl
{
  public:
    ...
    bool hasPostLinkTasks() const { return !mPostLinkTasks.empty(); }
    void schedulePostLinkTasks(const std::shared_ptr<angle::WorkerThreadPool> &workerThreadPool);
  ...
  private:
    ...
    std::vector<std::shared_ptr<rx::WarmUpTaskCommon>> mPostLinkTasks;
