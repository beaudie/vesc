...
class TextureVk : public TextureImpl, public angle::ObserverInterface
{
  ...
  private:
    ...
    // the array is a bitmask because GL allows redefining each cube map face separately.  For other
    // texture types, only bit 0 is meaningful as all array levels are redefined simultaneously.
    //
    ...
    //
    // Note: this array is indexed by gl::LevelIndex, not vk::LevelIndex
    gl::TexLevelArray<gl::CubeFaceMask> mRedefinedLevels;
