...
class TextureVk : public TextureImpl, public angle::ObserverInterface
{
  public:
    ...
    vk::ImageHelper *releaseOwnershipOfImage(const gl::Context *context);
  ...
  private:
    ...
    void releaseAndResetImage(ContextVk *contextVk);
    angle::Result ensureImageHelperSet(ContextVk *contextVk, const vk::Format &format);
    ...
    const vk::ImageViewHelper &imageViews() const;
    ...
    template <typename T>
    using PerSampleCountArray = std::array<T, 5>;

    // - index 0: the texture's image, if |mOwnsImage|, i.e. image is owned by the texture.
    // - index N: intermediate multisampled image used for multisampled rendering to texture with
    //   1 << N samples
    PerSampleCountArray<vk::ImageHelper> mMultisampledImages;
    ...
    PerSampleCountArray<vk::ImageViewHelper> mMultisampledImageViews;
    ...
    PerSampleCountArray<std::vector<RenderTargetVector>> mRenderTargets;

    // |mImage| is either:
    //
    // - nullptr: indicating that the image and its staging buffer are uninitialized
    // - |mMultisampledImages[0]|: indicating that |TextureVk| owns the image (set if |mOwnsImage|)
    // - A weak pointer shared with another class
