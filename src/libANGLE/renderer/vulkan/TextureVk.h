...
class TextureVk : public TextureImpl
{
  public:
    ...
    // Initializes a single subresource (level and layer) of the image.  If there are staged
    // changes, the subresource is cleared first (if necessary) and the changes uploaded.  Otherwise
    // the image is not cleared, allowing it to be cleared through render pass loadOps.
  ...
  private:
    ...
    angle::Result ensureImageInitializedImpl(ContextVk *contextVk,
                                             ...
                                             uint32_t levelCount,
                                             const vk::Format &format);
    void initializeContentsImpl(const gl::ImageIndex &imageIndex);
