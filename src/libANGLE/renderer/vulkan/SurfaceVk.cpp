...
void SwapchainCleanupData::destroy(RendererVk *renderer,
                                   vk::Recycler<vk::Fence> *fenceRecycler,
                                   vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    VkDevice device = renderer->getDevice();

    // Wait and recycle fences before destroying the Swapchain.
    // There is a bug on some Intel drivers, that causes crash if wait/check Fence status after
    // Swapchain destruction. Resetting Fence after Swapchain destruction works as expected.
    for (vk::Fence &fence : fences)
    {
        //   Technically, wait is not required when Present Fence is inferred from Acquire, because
        // successfully Acquired Image is always presented, and the corresponding Acquire Fence
        // is submitted. We wait for submission to finish before Swapchain destruction, therefore
        // Acquire Semaphore must be signalled, as well as the corresponding Acquire Fence.
        // However, Intel driver on Windows, reports Acquire Fence as not signalled, despite
        // that submission Fence that was waiting for the Acquire Semaphore is signalled.
        //   When using |VkSwapchainPresentFenceInfoEXT| extension, it is suggested to wait on
...
void ImagePresentOperation::destroy(RendererVk *renderer,
                                    vk::Recycler<vk::Fence> *fenceRecycler,
                                    vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    VkDevice device = renderer->getDevice();
    ...
    {
        oldSwapchain.destroy(renderer, fenceRecycler, semaphoreRecycler);
    }
    oldSwapchains.clear();
}
...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    {
        oldSwapchain.destroy(renderer, &mPresentFenceRecycler, &mPresentSemaphoreRecycler);
    }
...
angle::Result WindowSurfaceVk::recreateSwapchain(ContextVk *contextVk, const gl::Extents &extents)
{
    ...
    if (mOldSwapchains.size() > kMaxOldSwapchains)
    {
        ...
        for (SwapchainCleanupData &oldSwapchain : mOldSwapchains)
        {
            oldSwapchain.destroy(contextVk->getRenderer(), &mPresentFenceRecycler,
