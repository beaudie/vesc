                           ...
                           vk::Semaphore *semaphoreOut)
{
    if (semaphoreRecycler->empty())
    {
        ...
        ANGLE_VK_TRY(context, semaphoreOut->init(context->getDevice(), &callback));
    }
    else
    {
...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    for (impl::ImagePresentOperation &presentOperation : mPresentHistory)
    {
        ...
        VkAllocationCallbacks callback =
            vk::MemoryAllocationCallback::BuildCallbackSwapchain(callbacks);
    ...
    for (SwapchainCleanupData &oldSwapchain : mOldSwapchains)
    {
        ...
        oldSwapchain.destroy(device, &mPresentFenceRecycler, &mPresentSemaphoreRecycler, &callback);
    }
...
angle::Result WindowSurfaceVk::initializeImpl(DisplayVk *displayVk)
{
    ...
    for (vk::Semaphore &semaphore : mAcquireOperation.unlockedTryAcquireData.acquireImageSemaphores)
    {
        ANGLE_VK_TRY(displayVk, semaphore.init(displayVk->getDevice(), &callback));
    }
    ...
    if (mOldSwapchains.size() > kMaxOldSwapchains)
    {
        ...
        for (SwapchainCleanupData &oldSwapchain : mOldSwapchains)
        {
            ...
            oldSwapchain.destroy(contextVk->getDevice(), &mPresentFenceRecycler,
                                 &mPresentSemaphoreRecycler, &callback);
        }
        mOldSwapchains.clear();
    }
                                               ...
                                               VkSwapchainKHR lastSwapchain)
{
    ...
    ANGLE_VK_TRY(context, vkCreateSwapchainKHR(device, &swapchainInfo, &callback, &newSwapChain));
    ...
    if (isMultiSampled())
    {
        ...
        if (swapchainResolveMode == SwapchainResolveMode::Enabled)
        {
            ...
            ANGLE_VK_TRY(contextVk, swapchainImage.framebufferResolveMS.init(
                                        contextVk->getDevice(), framebufferInfo, &callback));
        }
        else
        {
            ...
            VkAllocationCallbacks callback =
                vk::MemoryAllocationCallback::BuildCallbackFramebuffer(callbacks);
            ANGLE_VK_TRY(contextVk,
                         mFramebufferMS.init(contextVk->getDevice(), framebufferInfo, &callback));
        }
    }
    else
    {
