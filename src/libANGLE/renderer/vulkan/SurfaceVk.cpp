                                       ...
                                       bool &swapchainOutOfDate)
{
    ...
    {
        ANGLE_TRY(generateWaitAndSignalSemaphores(displayVk, &waitSemaphore, &signalSemaphore));
    }
    ...
    presentInfo.pWaitSemaphores    = image.semaphores.back().ptr();
    ...
    swap.semaphores = std::move(image.semaphores);
...
angle::Result WindowSurfaceVk::nextSwapchainImage(DisplayVk *displayVk)
{
    ...
    ASSERT(image.semaphores.empty());
    image.semaphores.push_back(std::move(aquireImageSemaphore));
...
angle::Result WindowSurfaceVk::generateWaitAndSignalSemaphores(
    vk::Context *context,
    const vk::Semaphore **outWaitSemaphore,
    const vk::Semaphore **outSignalSempahore)
{
    SwapchainImage &swapImage = mSwapchainImages[mCurrentSwapchainImageIndex];
    ASSERT(!swapImage.semaphores.empty());

    vk::Semaphore nextSemaphore;
    ANGLE_VK_TRY(context, nextSemaphore.init(context->getDevice()));
    swapImage.semaphores.push_back(std::move(nextSemaphore));

    *outWaitSemaphore   = &swapImage.semaphores[swapImage.semaphores.size() - 2];
    *outSignalSempahore = &swapImage.semaphores[swapImage.semaphores.size() - 1];
