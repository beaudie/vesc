...
VkResult WindowSurfaceVk::acquireNextSwapchainImage(vk::Context *context)
{
    ...
    if (mSwapchainPresentMode == VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR)
    {
        ...
        {
            image.image.oneOffTransitionLayoutBarrier(context, vk::ImageLayout::SharedPresent,
                                                      &primaryCommandBuffer);
            (void)primaryCommandBuffer.end();
            Serial serial;
            (void)rendererVk->queueSubmitOneOff(context, std::move(primaryCommandBuffer), false,
