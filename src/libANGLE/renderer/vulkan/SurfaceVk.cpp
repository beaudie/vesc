...
angle::Result WindowSurfaceVk::handleOutOfDataSwapchain(ContextVk *contextVk,
                                                        uint32_t swapHistoryIndex)
{
...
angle::Result WindowSurfaceVk::swapImpl(const gl::Context *context, EGLint *rects, EGLint n_rects)
{
    ...
    {
        ANGLE_TRY(handleOutOfDataSwapchain(contextVk, currentSwapHistoryIndex));
    }

    {
        ...
        // before continuing.
        if (ANGLE_UNLIKELY(swapchainOutOfDate || (result == VK_ERROR_OUT_OF_DATE_KHR) ||
