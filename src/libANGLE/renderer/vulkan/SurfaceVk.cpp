...
egl::Error WindowSurfaceVk::initialize(const egl::Display *display)
{
    DisplayVk *displayVk = vk::GetImpl(display);
    bool success         = false;
    angle::Result result = initializeImpl(displayVk, &success);
    if (result == angle::Result::Continue && !success)
    {
...
angle::Result WindowSurfaceVk::initializeImpl(DisplayVk *displayVk, bool *successOut)
{
    RendererVk *renderer = displayVk->getRenderer();
