                                       ...
                                       EGLint interval)
{
    ...
    fprintf(stderr, "%s:%d (%d, %d)\n", __FILE__, __LINE__, mailboxAvailable, immediateAvailable);
                                                 ...
                                                 uint32_t swapHistoryIndex)
{
    ...
    fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);
    VkResult xx = vkCreateSwapchainKHR(device, &swapchainInfo, nullptr, &mSwapchain);
    fprintf(stderr, "%s:%d (vkCreateSwapchainKHR(minImageCount: %u, present mode: %u)->%d)\n",
            __FILE__, __LINE__, mMinImageCount, mDesiredSwapchainPresentMode, xx);
    ANGLE_VK_TRY(displayVk, xx);
                                                          ...
                                                          bool presentOutOfDate)
{
    ...
    if (currentExtents.width != getWidth() || currentExtents.height != getHeight())
    {
        fprintf(stderr,
                "%s:%d (present out of date: %d, swapintervalchanged: %d, current: %ux%u, "
                "swapchain: %ux%u)\n",
                __FILE__, __LINE__, presentOutOfDate, swapIntervalChanged, currentExtents.width,
                currentExtents.height, getWidth(), getHeight());
    ...
    if (presentOutOfDate || swapIntervalChanged || currentExtents.width != getWidth() ||
...
angle::Result WindowSurfaceVk::resizeSwapHistory(DisplayVk *displayVk, size_t imageCount)
{
    ...
    for (size_t i = 0; i < toCopy; ++i)
    {
        ...
        fprintf(stderr, "%s:%d (%zu->%zu)\n", __FILE__, __LINE__, historyIndex,
                resizedHistoryIndex);
...
void WindowSurfaceVk::setSwapInterval(EGLint interval)
{
    ...
    interval = std::max(interval, minSwapInterval);
    interval = std::min(interval, maxSwapInterval);
    ...
    fprintf(stderr, "%s:%d (%u)\n", __FILE__, __LINE__, mMinImageCount);
