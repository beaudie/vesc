                                       ...
                                       bool *presentOutOfDate)
{
    ...
    const vk::QueueSerial &swapSerial = mSwapHistory.front();
    mSwapHistory.next();

    {
        ANGLE_TRACE_EVENT0("gpu.angle", "WindowSurfaceVk::present: Throttle CPU");
        vk::QueueSerials serialsToWaitOn;
        serialsToWaitOn[swapSerial.index] = swapSerial.serial;
        ANGLE_TRY(renderer->finishToSerials(contextVk, serialsToWaitOn));
    }
        ...
        (image.image.getCurrentImageLayout() != vk::ImageLayout::SharedPresent))
    {
                                               ...
                                               &primaryCommandBuffer) == angle::Result::Continue)
        {
            ...
            if (rendererVk->queueSubmitOneOff(
                    ...
                    vk::SubmitPolicy::EnsureSubmitted) != angle::Result::Continue)
            {
                mDesiredSwapchainPresentMode = vk::PresentMode::FifoKHR;
                return VK_ERROR_OUT_OF_DATE_KHR;
            }
