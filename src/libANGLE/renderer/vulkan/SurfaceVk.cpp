...
egl::Error WindowSurfaceVk::makeCurrent(const gl::Context *context)
{
    ...
    if (!mColorRenderTarget.isImageValid())
    {
        result = nextSwapchainImage(vk::GetImpl(context), vk::GetImpl(context),
...
angle::Result WindowSurfaceVk::swapImpl(const gl::Context *context, EGLint *rects, EGLint n_rects)
{
    ...
    // TIMTIM -- Is this still necessary if we recreateSwapchain() in nextSwapchainImage()?
...

    {
        ...
        ANGLE_TRY(nextSwapchainImage(contextVk, vk::GetImpl(context), currentSwapHistoryIndex));
    }
                                                  ...
                                                  uint32_t swapHistoryIndex)
{
    ...
    auto result = vkAcquireNextImageKHR(device, mSwapchain, UINT64_MAX,
    ...
    {
        gl::Extents currentExtents;
        ANGLE_TRY(getCurrentWindowSize(context, &currentExtents));
        ANGLE_TRY(recreateSwapchain(contextVk, currentExtents, swapHistoryIndex));
