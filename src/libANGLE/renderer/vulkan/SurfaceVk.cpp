                              ...
                              vk::ImageHelper *imageHelper)
{
    ...
    VkImageCreateFlags imageCreateFlags =
        isProtectedMemory ? VK_IMAGE_CREATE_PROTECTED_BIT : vk::kVkImageCreateFlagsNone;
                                                              ...
                                                              bool isProtectedMemory)
{
    ANGLE_TRY(InitImageHelper(displayVk, width, height, vkFormat, samples,
                              isRobustResourceInitEnabled, isProtectedMemory, &image));
    ...
    ANGLE_TRY(
        image.initMemory(displayVk, isProtectedMemory, renderer->getMemoryProperties(), flags));
...
angle::Result OffscreenSurfaceVk::initializeImpl(DisplayVk *displayVk)
{
    ...
    if (config->renderTargetFormat != GL_NONE)
    {
        ANGLE_TRY(mColorAttachment.initialize(displayVk, mWidth, mHeight,
                                              ...
                                              samples, robustInit, mState.isProtectedMemory));
    ...
    if (config->depthStencilFormat != GL_NONE)
    {
        ANGLE_TRY(mDepthStencilAttachment.initialize(
            ...
            robustInit, mState.isProtectedMemory));
        mDepthStencilRenderTarget.init(&mDepthStencilAttachment.image,
                                       &mDepthStencilAttachment.imageViews, nullptr, nullptr,
                                       gl::LevelIndex(0), 0, 1, RenderTargetTransience::Default);
    }
                                               ...
                                               VkSwapchainKHR lastSwapchain)
{
    ...
    swapchainInfo.flags   = mState.isProtectedMemory ? VK_SWAPCHAIN_CREATE_PROTECTED_BIT_KHR : 0;
    ...
    if (samples > 1)
    {
        ...
        ANGLE_TRY(mColorImageMS.initMemory(context, mState.isProtectedMemory,
    ...
    if (mState.config->depthStencilFormat != GL_NONE)
    {
        ...
        ANGLE_TRY(mDepthStencilImage.initMemory(context, mState.isProtectedMemory,
                                                renderer->getMemoryProperties(),
                                                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    *swapSerial = renderer->getLastSubmittedQueueSerial();
    ...
    VkResult result = renderer->queuePresent(contextVk, contextVk->isProtectedMemory(),
                                             contextVk->getPriority(), presentInfo);
