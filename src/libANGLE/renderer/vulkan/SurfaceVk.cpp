...
VkResult WindowSurfaceVk::acquireNextSwapchainImage(vk::Context *context)
{
        ...
        (image.image.getCurrentImageLayout() != vk::ImageLayout::SharedPresent))
    {
        ...
        auto commandContent = vk::ConvertProtectedBitToCommandContent(mState.hasProtectedContent());
        if (rendererVk->getCommandBufferOneOff(context, commandContent, &primaryCommandBuffer) ==
