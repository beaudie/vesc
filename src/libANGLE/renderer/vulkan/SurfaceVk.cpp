...
void ImagePresentOperation::waitFences(VkDevice device, uint64_t timeout) const
{
    if (fence.valid())
    {
        VkResult status = fence.getStatus(device);
    ...
    {
        oldSwapchain.waitFences(device, timeout);
    }
}
...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    for (impl::ImagePresentOperation &presentOperation : mPresentHistory)
    {
        //   Technically, wait is not required when Present Fence is inferred from Acquire,
        // because successfully Acquired Image is always presented, and the corresponding Acquire
        // Fence is submitted. We wait for submission to finish before Swapchain destruction,
        // therefore Acquire Semaphore must be signalled, as well as the corresponding Acquire
        // Fence. However, Intel driver on Windows, reports Acquire Fence as not signalled, despite
