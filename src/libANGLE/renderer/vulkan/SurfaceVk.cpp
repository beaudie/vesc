...
void WindowSurfaceVk::destroySwapChainImages(DisplayVk *displayVk)
{
    ...
    for (SwapchainImage &swapchainImage : mSwapchainImages)
    {
        // We don't own the swapchain image handles, so we just remove our reference to it.
        swapchainImage.image.get().resetImageWeakReference();
        swapchainImage.image.reset(device);
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    if (mColorImageMS.valid())
    {
        ...
        ANGLE_TRY(image.image.get().recordCommands(contextVk, &resolveCommands));
        mColorImageMS.resolve(&image.image.get(), resolveRegion, resolveCommands);
    }

    ANGLE_TRY(updateAndDrawOverlay(contextVk, &image));

    vk::CommandBuffer *transitionCommands = nullptr;
    ANGLE_TRY(image.image.get().recordCommands(contextVk, &transitionCommands));
                                                    ...
                                                    SwapchainImage *image) const
{
    ...
    // Draw overlay
    ANGLE_TRY(overlayVk->onPresent(contextVk, &image->image.get(), &image->imageView));
