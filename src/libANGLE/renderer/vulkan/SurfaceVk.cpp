                                       ...
                                       bool &swapchainOutOfDate)
{
    ...
    if (renderer->getFeatures().supportsIncrementalPresent && (n_rects > 0))
    {
        ...
        {
            VkRectLayerKHR &rect = vk_rects[i];

            // Make sure the damage rects are within swapchain bounds.
            rect.offset.x      = gl::clamp(*egl_rects++, 0, width);
            rect.offset.y      = gl::clamp(*egl_rects++, 0, height);
            rect.extent.width  = gl::clamp(*egl_rects++, 0, width - rect.offset.x);
            rect.extent.height = gl::clamp(*egl_rects++, 0, height - rect.offset.y);
            rect.layer         = 0;
        }
