                                       ...
                                       bool &swapchainOutOfDate)
{
    ...
    presentInfo.pWaitSemaphores    = mSemaphores.back().ptr();
    ...
    swap.semaphores = std::move(mSemaphores);
...
angle::Result WindowSurfaceVk::nextSwapchainImage(DisplayVk *displayVk)
{
    ...
    mSemaphores.push_back(std::move(aquireImageSemaphore));
    ...
    const vk::Semaphore **outSignalSempahore)
{
    ...
    mSemaphores.push_back(std::move(nextSemaphore));

    *outWaitSemaphore   = &mSemaphores[mSemaphores.size() - 2];
    *outSignalSempahore = &mSemaphores[mSemaphores.size() - 1];
