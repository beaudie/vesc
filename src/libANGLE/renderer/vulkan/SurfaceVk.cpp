...
bool IsAllFencesSignaled(VkDevice device, const std::vector<vk::Fence> &fences)
{
...
VkResult SwapchainCleanupData::getFencesStatus(VkDevice device) const
{
    ...
    ASSERT(result != VK_SUCCESS || IsAllFencesSignaled(device, fences));
    return result;
}
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    {
        // Associate swapSerial of this present with the previous preset of the same imageIndex.
        // Completion of swapSerial implies that current ANI semaphore was waited, presentation
        // engine handed imageIndex back to the application since previous present, previous
        // present finished waiting on the present semaphore.
...
angle::Result WindowSurfaceVk::cleanUpOldSwapchains(vk::Context *context)
{
    ...
    // Old Swapchain clean up uses the |mPresentFenceRecycler|, ensure no vkAcquireNextImageKHR
    // cannot be pending (which also accesses |mPresentFenceRecycler| and may be run without holding
    // the front-end lock.
    ASSERT(!needsAcquireImageOrProcessResult());
