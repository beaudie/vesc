...
void WindowSurfaceVk::SwapHistory::destroy(VkDevice device)
{
    ...
    acquireImageSemaphore.destroy(device);
    presentImageSemaphore.destroy(device);
}

VkResult WindowSurfaceVk::SwapHistory::waitFence(vk::Context *context)
{
    if (sharedFence.isReferenced())
    {
        return sharedFence.get().wait(context->getDevice(), std::numeric_limits<uint64_t>::max());
    }
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    ASSERT(swap.presentImageSemaphore.valid());
...
VkResult WindowSurfaceVk::nextSwapchainImage(vk::Context *context)
{
    ...
    SwapHistory &swap = mSwapHistory[mCurrentSwapHistoryIndex];
    {
        ANGLE_TRACE_EVENT0("gpu.angle", "WindowSurfaceVk: Throttle CPU");
        VkResult result = swap.waitFence(context);
        if (ANGLE_UNLIKELY(result != VK_SUCCESS))
        {
            return result;
        }
        swap.destroy(context->getDevice());
    }

    VkResult result = swap.acquireImageSemaphore.init(device);
    if (ANGLE_UNLIKELY(result != VK_SUCCESS))
    {
        return result;
    }
    ...
    result = vkAcquireNextImageKHR(device, mSwapchain, UINT64_MAX,
                                   swap.acquireImageSemaphore.getHandle(), VK_NULL_HANDLE,
...
{
    SwapHistory &swap = mSwapHistory[mCurrentSwapHistoryIndex];
    return std::move(swap.acquireImageSemaphore);
}
