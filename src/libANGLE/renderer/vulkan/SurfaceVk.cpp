...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    for (impl::ImagePresentOperation &presentOperation : mPresentHistory)
    {
        if (presentOperation.fence.valid())
        {
            ...
            fprintf(stderr, "INAZ: %p - destroy() fence(%p).getStatus: %d (before finish...);\n",
                                                          ...
                                                          bool presentOutOfDate)
{
    ...
    if (!contextVk->getRenderer()->getFeatures().perFrameWindowSizeQuery.enabled &&
        !presentOutOfDate)
    {
        fprintf(stderr, "INAZ: %p - checkForOutOfDateSwapchain() no changes 1...\n", this);
        return angle::Result::Continue;
    }
    ...
    if (!presentOutOfDate)
    {
        fprintf(stderr, "INAZ: %p - checkForOutOfDateSwapchain() no changes 2...\n", this);
        return angle::Result::Continue;
    }
