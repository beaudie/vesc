...
void WindowSurfaceVk::releaseSwapchainImages(ContextVk *contextVk)
{
    ...
    // Release swapchain imageviews with the mUse from current swap chain image
    // We use the imageviews of all swapchain images in getLevelLayerDrawImageView(),
    // but we might only use the image that pointed by mCurrentSwapchainImageIndex,
    // so we take the mUse of current swap chain image to retrieve the most accurate
    // ResourceUse.serial for all imageviews.
    SwapchainImage &currentSwapchainImage = mSwapchainImages[mCurrentSwapchainImageIndex];
    for (SwapchainImage &swapchainImage : mSwapchainImages)
    {
        currentSwapchainImage.image.collectViewGarbage(renderer, &swapchainImage.imageViews);
    }
    for (SwapchainImage &swapchainImage : mSwapchainImages)
    {
