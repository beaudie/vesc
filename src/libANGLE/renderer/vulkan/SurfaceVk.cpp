...
angle::Result WindowSurfaceVk::initializeImpl(DisplayVk *displayVk)
{
    ...
    VkResult vkResult =
        acquireNextSwapchainImage(displayVk, displayVk->getMutex(), displayVk->getSurfaceMutex());
                                                          ...
                                                          bool presentOutOfDate)
{
...

    {
        ...
        VkResult result =
            acquireNextSwapchainImage(contextVk, context->getMutex(), context->getSurfaceMutex());
...
VkResult WindowSurfaceVk::acquireNextSwapchainImage(vk::Context *context,
                                                    ...
                                                    angle::GlobalMutex &unlockSurfaceMutex)
{
    ...
    VkResult result = VK_SUCCESS;
    {
        auto scopedSurfaceUnlock = ScopedUnlockGlobalLockForAcquireNextImage(
            unlockSurfaceMutex, context->getRenderer()->getFeatures());
                                        ...
                                        EGLint *bufferPitchOut)
{
    ...
    {
        DisplayVk *displayVk = vk::GetImpl(display);
        if (acquireNextSwapchainImage(displayVk, displayVk->getMutex(),
                                      displayVk->getSurfaceMutex()) != VK_SUCCESS)
        {
            return egl::EglBadAccess();
        }
    }
