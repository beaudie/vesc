...
{
    // Image must be acquired here in any case, because it is required for the |getBufferAge|.
    if (!mAcquireOperation.needToAcquireNextSwapchainImage)
    {
        return egl::NoError();
    }

    // If shared present mode is not current, swapchain will be recreated in the
    // |doDeferredAcquireNextImage| call below, and mode will become shared.  Otherwise, mode will
    // remain shared.  In any case, present mode will be shared if it is desired.
    if (isSharedPresentModeDesired())
    {
        // Shared present mode requires special handling, because |swapImpl| may be skipped, which
        // will also skip |postProcessUnlockedTryAcquire|.  Additionally, this mode requires use of
        // |skipAcquireNextSwapchainImageForSharedPresentMode| method.
        // Blow call is not going to block.
        return angle::ToEGL(doDeferredAcquireNextImage(context, false), EGL_BAD_SURFACE);
    }
    // Similarly, if shared present mode is not desired, mode will change to non shared or remain
    // non shared in the |prepareForAcquireNextSwapchainImage| call below.

    vk::Renderer *renderer = vk::GetImpl(context)->getRenderer();

    const angle::Result result = prepareForAcquireNextSwapchainImage(context, false);
