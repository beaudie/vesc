...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    {
        for (vk::Fence &fence : oldSwapchain.fences)
        {
            //   Technically, wait is not required when Present Fence is inferred from Acquire,
            // because successfully Acquired Image is always presented, and the corresponding
            // Acquire Fence is submitted. We wait for submission to finish before Swapchain
            // destruction, therefore Acquire Semaphore must be signalled, as well as the
            // corresponding Acquire Fence. However, Intel driver on Windows, reports Acquire Fence
            // as not signalled, despite that submission Fence that was waiting for the Acquire
            // Semaphore is signalled.
            //   When using |VkSwapchainPresentFenceInfoEXT| extension, it is suggested to wait on
            // fences before destroying Present Semaphores or the Swapchain itself. When extension
            // is not supported, we can only wait until submission is finished and that Swapchain
