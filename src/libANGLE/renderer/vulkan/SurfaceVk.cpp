...
void SwapchainCleanupData::destroy(VkDevice device,
                                   ...
                                   vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    printf("INAZ: SwapchainCleanupData::destroy() BEGIN\n");
    ...
    for (vk::Fence &fence : fences)
    {
        ...
        printf("INAZ: SwapchainCleanupData::destroy() getStatus: %d;\n", (int)status);
    ...
    printf("INAZ: SwapchainCleanupData::destroy() END\n");
}
...
void ImagePresentOperation::destroy(VkDevice device,
                                    ...
                                    vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    printf("INAZ: ImagePresentOperation::destroy() fence.valid(): %d; oldSwapchains.size(): %d\n",
...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    printf("INAZ: %p - destroy() mPresentHistory.size(): %d;\n", this, (int)mPresentHistory.size());
    ...
    printf("INAZ: %p - destroy() mOldSwapchains.size(): %d;\n", this, (int)mOldSwapchains.size());
...
angle::Result WindowSurfaceVk::recreateSwapchain(ContextVk *contextVk, const gl::Extents &extents)
{
    ...
    printf("INAZ: %p - recreateSwapchain() mPresentHistory.size(): %d;\n", this,
    ...
    printf("INAZ: %p - recreateSwapchain() mOldSwapchains.size(): %d (begin);\n", this,
                                                          ...
                                                          bool presentOutOfDate)
{
    ...
    if (!contextVk->getRenderer()->getFeatures().perFrameWindowSizeQuery.enabled &&
        !presentOutOfDate)
    {
        printf("INAZ: %p - checkForOutOfDateSwapchain() no changes 1...\n", this);
        return angle::Result::Continue;
    }
        ...
        !presentOutOfDate)
    {
        ...
        printf("INAZ: %p - checkForOutOfDateSwapchain() size change...\n", this);
    }

    // If anything has changed, recreate the swapchain.
    if (!presentOutOfDate)
    {
        printf("INAZ: %p - checkForOutOfDateSwapchain() no changes 2...\n", this);
        return angle::Result::Continue;
    }
    ...
    printf("INAZ: %p - checkForOutOfDateSwapchain() -> recreateSwapchain()...\n", this);
    ...
    if (mNeedToAcquireNextSwapchainImage)
    {
        ...
        egl::Display::GetCurrentThreadUnlockedTailCall()->add([context, this]() {
            ...
            if (mNeedToAcquireNextSwapchainImage)
            {
                ...
                printf("INAZ: %p - prepareSwap() -> doDeferredAcquireNextImage()\n", this);
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    printf("INAZ: %p - present() lastPresentResult: %d;\n", this,
    ...
    printf("INAZ: %p - present() mOldSwapchains.size(): %d;\n", this, (int)mOldSwapchains.size());
    ...
    printf("INAZ: %p - present() mPresentHistory.size(): %d (after emplace_back);\n", this,
           (int)mPresentHistory.size());

    // Clean up whatever present is already finished.
    ANGLE_TRY(cleanUpPresentHistory(contextVk));
    printf("INAZ: %p - present() mPresentHistory.size(): %d (after cleanUpPresentHistory);\n", this,
           (int)mPresentHistory.size());
                                        ...
                                        const void *pNextChain)
{
    ...
    {
        printf("INAZ: %p - swapImpl() presentOutOfDate: %d; -> doDeferredAcquireNextImage()\n",
                                                          ...
                                                          bool presentOutOfDate)
{
    ...
    printf("INAZ: %p - doDeferredAcquireNextImage() -> checkForOutOfDateSwapchain()\n", this);
...
VkResult WindowSurfaceVk::acquireNextSwapchainImage(vk::Context *context)
{
    ...
    printf("INAZ: %p - acquireNextSwapchainImage() result: %d; mCurrentSwapchainImageIndex: %u\n",
    ...
    if (presentFenceInferredFromAcquire)
    {
        AssociateFenceWithPresentHistory(mCurrentSwapchainImageIndex, std::move(presentFence),
                                         &mPresentHistory);
        printf("INAZ: %p - acquireNextSwapchainImage() mPresentHistory.size(): %d;\n", this,
               (int)mPresentHistory.size());
    }
