...
{
    RendererVk *renderer            = displayVk->getRenderer();
    mPerFrameWindowSizeQueryEnabled = renderer->getFeatures().perFrameWindowSizeQuery.enabled;
                                                          ...
                                                          bool presentOutOfDate)
{
    ...
    bool gotNewSurfaceCaps   = false;

    if (mPerFrameWindowSizeQueryEnabled && !swapIntervalChanged && !presentOutOfDate)
    {
            ...
            mSurfaceCaps.currentExtent.height != swapchainHeight)
        {
            ...
            // Let the code below know that it doesn't need to update mSurfaceCaps.
            gotNewSurfaceCaps = true;
        }
    }
    ...
    if (swapIntervalChanged || presentOutOfDate)
    {
        ...
        // TODO(ianelliott): Someday deal with stereoscopic 3D?

        ANGLE_TRY(recreateSwapchain(contextVk, newSwapchainExtents));
    }
