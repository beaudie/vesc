                           ...
                           vk::Semaphore *semaphoreOut)
{
    if (semaphoreRecycler->empty())
    {
        ...
        INFO() << "INAZ: SurfaceVk.cpp NewSemaphore() device: " << context->getDevice()
               << "; handle: " << semaphoreOut->getHandle()
               << "; ContextVk: " << (ContextVk *)context << "; LINE: " << __LINE__;
    }
    else
    {
...
void SwapchainCleanupData::destroy(VkDevice device, vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    ...
    {
        INFO() << "INAZ: SwapchainCleanupData::destroy() device: " << device
               << "; handle: " << semaphore.getHandle() << "; - semaphoreRecycler->recycle()"
               << "; LINE: " << __LINE__;
                                    ...
                                    vk::Recycler<vk::Semaphore> *semaphoreRecycler)
{
    ...
    if (semaphore.valid())
    {
        INFO() << "INAZ: ImagePresentOperation::destroy() device: " << device
               << "; handle: " << semaphore.getHandle() << "; - semaphoreRecycler->recycle()"
               << "; LINE: " << __LINE__;
        semaphoreRecycler->recycle(std::move(semaphore));
    }
...
void WindowSurfaceVk::destroy(const egl::Display *display)
{
    ...
    {
        INFO() << "INAZ: WindowSurfaceVk::destroy() device: " << device
               << "; handle: " << semaphore.getHandle() << "; - semaphore.destroy(device)"
               << "; LINE: " << __LINE__;
    ...
    INFO() << "INAZ: WindowSurfaceVk::destroy() device: " << device
           << "; - mPresentSemaphoreRecycler.destroy()"
           << "; LINE: " << __LINE__;
...
angle::Result WindowSurfaceVk::initializeImpl(DisplayVk *displayVk)
{
    ...
    for (vk::Semaphore &semaphore : mAcquireImageSemaphores)
    {
        ...
        INFO() << "INAZ: WindowSurfaceVk::initializeImpl() device: " << renderer->getDevice()
               << "; handle: " << semaphore.getHandle() << "; LINE: " << __LINE__;
    }
                                                ...
                                                const vk::Semaphore &presentSemaphore)
{
    ...
    INFO() << "INAZ: WindowSurfaceVk::prePresentSubmit() device: " << renderer->getDevice()
                                       ...
                                       bool *presentOutOfDate)
{
    ...
    INFO() << "INAZ: WindowSurfaceVk::present() device: " << renderer->getDevice()
           << "; handle: " << presentSemaphore.getHandle() << "; ContextVk: " << contextVk
           << " - NewSemaphore()"
           << "; LINE: " << __LINE__;
...
void WindowSurfaceVk::flushAcquireImageSemaphore(const gl::Context *context)
{
    ...
    ContextVk *contextVk = vk::GetImpl(context);
    INFO() << "INAZ: WindowSurfaceVk::flushAcquireImageSemaphore() device: "
           << contextVk->getDevice() << "; handle: " << mAcquireImageSemaphore->getHandle()
...
VkResult WindowSurfaceVk::acquireNextSwapchainImage(vk::Context *context)
{
    ...
    INFO() << "INAZ: WindowSurfaceVk::acquireNextSwapchainImage() device: " << context->getDevice()
           << "; handle: " << acquireImageSemaphore->getHandle() << "; LINE: " << __LINE__;
