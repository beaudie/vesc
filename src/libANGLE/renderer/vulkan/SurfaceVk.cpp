...
void OffscreenSurfaceVk::AttachmentImage::destroy(const egl::Display *display)
{
    ...
    // destruction.  We could track the last submit fence for the last context that used this
    // surface to garbage collect the surfaces.
...
angle::Result WindowSurfaceVk::SwapHistory::waitFence(ContextVk *contextVk)
{
    if (fence)
    {
        ANGLE_VK_TRY(contextVk,
                     fence->wait(contextVk->getDevice(), std::numeric_limits<uint64_t>::max()));
    }
    return angle::Result::Continue;
}
