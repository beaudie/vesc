...
class CommandGraphResource : angle::NonCopyable
{
  public:
    ...
    // Begins a command buffer on the current graph node for in-RenderPass rendering.  Additionally,
    // returns the outside-RenderPass command buffer that's run prior to that RenderPass.  It is
    // guaranteed that the two command buffers correspond to the same graph node, so dependencies
    // can be set prior to recording to the outside-RenderPass command buffer and are maintained for
    // the sake of the in-RenderPass command buffer.
    //
  ...
  private:
    ...
    void onAccessedResourceDelete(CommandGraphResource *deleted);
    ...
    // A list of resources {R} where addDependency(R) has been called since the last acquisition of
    // a command buffer (recordCommands, beginRenderPass and appendToStartedRenderPass).
    std::vector<CommandGraphResource *> mAccessedResources;
    CommandGraphResource *mAccessedBy;
