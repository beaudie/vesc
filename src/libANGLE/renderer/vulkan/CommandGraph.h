...
//#define USE_CUSTOM_CMD_BUFFERS 1
...
class CommandBufferOwner
{
  ...
  protected:
#if USE_CUSTOM_CMD_BUFFERS
    vk::SecondaryCommandBuffer *mCommandBuffer = nullptr;
#else
...
class CommandGraphNode final : angle::NonCopyable
{
  public:
...
#if USE_CUSTOM_CMD_BUFFERS
    SecondaryCommandBuffer *getOutsideRenderPassCommands();
    SecondaryCommandBuffer *getInsideRenderPassCommands();
#else
    ...
    angle::Result beginInsideRenderPassRecording(Context *context,
#if USE_CUSTOM_CMD_BUFFERS
                                                 SecondaryCommandBuffer **commandsOut);
#else
                                                 CommandBuffer **commandsOut);
...
#if USE_CUSTOM_CMD_BUFFERS
    SecondaryCommandBuffer mOutsideRenderPassCommands;
    SecondaryCommandBuffer mInsideRenderPassCommands;
#else
...
class CommandGraphResource : angle::NonCopyable
{
  public:
    ...
    angle::Result recordCommands(Context *context,
#if USE_CUSTOM_CMD_BUFFERS
