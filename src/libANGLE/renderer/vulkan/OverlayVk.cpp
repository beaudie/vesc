...
angle::Result OverlayVk::createFont(ContextVk *contextVk)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, fontDataBuffer.get().init(contextVk, bufferCreateInfo,
                                             VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &isOutOfMemory));
    ...
    ANGLE_VK_TRY_ALLOC(contextVk,
                       mFontImage.initMemory(contextVk, false, renderer->getMemoryProperties(),
                                             VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                             vk::MemoryAllocationType::FontImage, &isOutOfMemory));
                                   ...
                                   bool is90DegreeRotation)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, textDataBuffer.get().init(contextVk, textBufferCreateInfo,
                                             VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &isOutOfMemory));
    ANGLE_VK_TRY_ALLOC(
        contextVk, graphDataBuffer.get().init(contextVk, graphBufferCreateInfo,
                                              VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &isOutOfMemory));
