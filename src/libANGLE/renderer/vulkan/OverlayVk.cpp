...
angle::Result OverlayVk::createFont(ContextVk *contextVk)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       fontDataBuffer.get().init(contextVk, bufferCreateInfo,
                                   ...
                                   bool is90DegreeRotation)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       textDataBuffer.get().init(contextVk, textBufferCreateInfo,
                                                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &result));
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       graphDataBuffer.get().init(contextVk, graphBufferCreateInfo,
                                                  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &result));
