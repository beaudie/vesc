                                           ...
                                           gl::InfoLog &infoLog)
{
    ...
    GlslangWrapperVk::GetShaderSource(contextVk->useOldRewriteStructSamplers(),
                                      contextVk->getRenderer()->getFeatures(), mState, resources,
                                      &mShaderSources);
...
angle::Result ProgramVk::linkImpl(const gl::Context *glContext, gl::InfoLog &infoLog)
{
    ...
    {
        vk::GetImpl(transformFeedback)
            ->updateDescriptorSetLayout(contextVk, mState, &uniformsAndXfbSetDesc);
    }
