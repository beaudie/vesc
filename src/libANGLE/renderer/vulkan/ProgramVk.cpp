                                        ...
                                        vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size();)
    {
        ...
        descOut->update(globalBindingOffset + bindingIndex, descType, arraySize, activeStages);
...
angle::Result ProgramVk::linkImpl(const gl::Context *glContext, gl::InfoLog &infoLog)
{
    ...
    mShaderStageCount = static_cast<uint32_t>(mShaderStages.count());
    ...
    std::array<VkDescriptorPoolSize, 2> uniformAndXfbSetSize = {
        {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, mShaderStageCount},
...
{
    uint32_t uniformBlockBindingCount = 0;
    for (const gl::InterfaceBlock &block : mState.getUniformBlocks())
    {
        if (!block.isArray || block.arrayElement == 0)
        {
            ++uniformBlockBindingCount;
        }
    }

    mStorageBlockBindingsOffset = uniformBlockBindingCount;
}
                                           ...
                                           VkDescriptorType descriptorType)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
    {
        ...
        writeInfo.dstBinding       = globalBindingOffset + currentBinding;
