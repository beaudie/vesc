...
void ProgramVk::updateTransformFeedbackDescriptorSetImpl(ContextVk *contextVk)
{
    ...
    if (!(hasTransformFeedbackOutput() && glState.isTransformFeedbackActive()))
    {
        ...
        // set. If xfb is active it is guaranteed that valid buffers are bound to xfb objects.
        // We update descriptor sets only when there are valid buffer bindings for xfb.
        return;
    }
