...
vk::Error ProgramVk::initDescriptorSets(ContextVk *contextVk)
{
    ...
    DescriptorPoolManager &descriptorPoolManager = contextVk->getDescriptorPoolManager();
    ...
    ANGLE_TRY(descriptorPoolManager.allocateDescriptorSets(descriptorSetLayouts[0].ptr(), descriptorSetCount, &mDescriptorSets[0]));
    return vk::NoError();
}
