                                        ...
                                        vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size();)
    {
        ...
        for (const gl::ShaderType shaderType : shaderStages)
        {
            ...
            currentBindings[shaderType] += arraySize;
        }
...
angle::Result ProgramVk::linkImpl(const gl::Context *glContext, gl::InfoLog &infoLog)
{
    ...
    mShaderStages     = mState.getLinkedShaderStages();
    mShaderStageCount = mShaderStages.count();
                                           ...
                                           bool isStorageBuffer)
{
    ...
    gl::ShaderMap<uint32_t> currentBindings   = {};
    gl::ShaderMap<uint32_t> currentArraySizes = {};
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
    {
        ...
        for (const gl::ShaderType shaderType : mShaderStages)
        {
            ...
            {
                currentBindings[shaderType] += currentArraySizes[shaderType];
                currentArraySizes[shaderType] = 0;
            }
                                                     ...
                                                     vk::FramebufferHelper *framebuffer)
{
         ...
         ++textureIndex)
    {
        ...
        for (const gl::ShaderType shaderType : mShaderStages)
        {
            ...
            currentBindings[shaderType] += samplerBinding.boundTextureUnits.size();
        }
    }
