...
void ProgramVk::getUniformImpl(GLint location, T *v, GLenum entryPointType) const
{
    ...
    size_t uniformIndex = linkedUniform.getUniformIndex();
    ASSERT(uniformIndex != gl::ShaderType::SHADER_TYPE_MAX);

    const DefaultUniformBlock &uniformBlock = mDefaultUniformBlocks[uniformIndex];
    const sh::BlockMemberInfo &layoutInfo   = uniformBlock.uniformLayout[location];
