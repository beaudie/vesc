...
angle::Result ProgramVk::initDefaultUniformBlocks(const gl::Context *glContext)
{
    ...
    for (gl::ShaderType shaderType : vk::AllShaderTypes())
    {
                                              ...
                                              vk::CommandBuffer *commandBuffer)
{
    ...
    {
        // Note: mUniformBlocksOffsets has entries for every gl::ShaderType, but we currently only
        // support Vertex and Fragment shaders.  As a result, we only look at the first two offsets.
        // TODO(syoussefi): revisit when the rest of shaders are supported http://anglebug.com/2959
        constexpr uint32_t kUniformBlockOffsetCount = 2;

        commandBuffer->bindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayout.get(),
                                          low, mUsedDescriptorSetRange.length(),
                                          &mDescriptorSets[low], kUniformBlockOffsetCount,
