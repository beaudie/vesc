...
void ProgramVk::initializeSourceOptions(const gl::Context *glContext)
{
    ...
    mGlslangSourceOptions.emulateBresenhamLines    = features.basicGLLineRasterization.enabled;
    ...
    {
        mGlslangSourceOptions.shaderResourceDescriptorSetIndexShaderMap[shaderType] = -1;
        mGlslangSourceOptions.interfaceBlockBindingsShaderMap[shaderType].clear();
    }
}
...
void ProgramVk::clearVariableInfoMap()
{
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
...
angle::Result ProgramVk::getShaderSource(ContextVk *contextVk,
                                         const gl::ProgramLinkedResources &resources)
{
    ...
    GlslangWrapperVk::GetShaderSource(mState, resources, mGlslangSourceOptions, &shaderSources,
                                      &mVariableInfoMap);

    // Compile the shaders.
    angle::Result status = mShaderInfo.initShaders(contextVk, shaderSources, mVariableInfoMap);
                                           ...
                                           gl::InfoLog &infoLog)
{
    ...
    angle::Result status = getShaderSource(contextVk, resources);
        ...
        if (location.used() && !location.ignored)
        {
            ...
            if (uniform.isInDefaultBlock() && !uniform.isSampler() && !uniform.isImage())
            {
                ...
                for (const gl::ShaderType shaderType :
                     mState.getExecutable().getLinkedShaderStages())
                {
...
void ProgramVk::setUniform1iv(GLint location, GLsizei count, const GLint *v)
{
    ...
    {
        // We could potentially cache some indexing here. For now this is a no-op since the
        // mapping is handled entirely in ContextVk.
        return;
    }
