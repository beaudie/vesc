                                           ...
                                           VkDescriptorType descriptorType)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
    {
        ...
        if (isStorageBuffer)
        {
            bufferHelper.onWrite(contextVk, agent, VK_ACCESS_SHADER_READ_BIT,
                                 VK_ACCESS_SHADER_WRITE_BIT);
        }
        else
        {
            bufferHelper.onRead(agent, VK_ACCESS_UNIFORM_READ_BIT);
        }
    ...
    vk::CommandGraphResource *agent)
{
    ...
    updateBuffersDescriptorSet(contextVk, agent, mState.getUniformBlocks(),
                               VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    updateBuffersDescriptorSet(contextVk, agent, mState.getShaderStorageBlocks(),
