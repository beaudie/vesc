...
angle::Result ProgramVk::loadSpirvBlob(ContextVk *contextVk, gl::BinaryInputStream *stream)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        // Read the SPIR-V
        spirvBlob->resize(stream->readInt<size_t>());
        stream->readRaw(spirvBlob->data(), spirvBlob->size());
    }
...
void ProgramVk::saveSpirvBlob(gl::BinaryOutputStream *stream)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        // Write the SPIR-V
        stream->writeInt(spirvBlob.size());
        stream->writeRaw(spirvBlob.data(), spirvBlob.size());
    }
}
