...
void ProgramVk::getUniformImpl(GLint location, T *v, GLenum entryPointType) const
{
    ...
    {
        const uint8_t *ptrToElement = uniformBlock.uniformData.data() +
                                      (locationInfo.arrayIndex * linkedUniform.getElementSize());
        GetMatrixUniform(gl::VariableColumnCount(linkedUniform.type),
                         gl::VariableRowCount(linkedUniform.type), v,
