                                                           ...
                                                           vk::FramebufferHelper *framebuffer)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < uniformBlocks.size();)
    {
        ...
        {
            const gl::InterfaceBlock &uniformBlock = uniformBlocks[bufferIndex + arrayElement];
            ...
            size = std::min(size ? size : (bufferHelper.getSize() - offset), blockSize);

            bufferInfo[arrayElement].buffer = bufferHelper.getBuffer().getHandle();
            bufferInfo[arrayElement].offset = offset;
            bufferInfo[arrayElement].range  = size;
