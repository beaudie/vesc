                               ...
                               angle::MemoryBuffer *uniformData)
{
    ...
    size_t elementSize = sizeof(T) * componentCount;
...
vk::Error ProgramVk::updateUniforms(ContextVk *contextVk)
{
    ...
    // FIXME: handle having only of of vertex/fragment uniforms.
    ...
    if (!mVertexUniformData.empty())
    {
        auto &info = bufferInfo[bufferCount++];

        info.buffer = mDefaultVertexUniformsBuffer.getHandle();
        info.offset = 0;
    ...
    if (!mFragmentUniformData.empty())
    {
        ...
        info.buffer = mDefaultFragmentUniformsBuffer.getHandle();
        info.offset = 0;
        info.range  = VK_WHOLE_SIZE;
    }
    ...
    writeInfo.descriptorCount  = bufferCount;
    writeInfo.descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    writeInfo.pImageInfo       = nullptr;
    writeInfo.pBufferInfo      = bufferInfo.data();
