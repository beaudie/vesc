...
angle::Result ProgramVk::updateUniforms(ContextVk *contextVk)
{
    ...
    do
    {
        ...
        if (!retry)
        {
            if (!mDefaultUniformStorage.tryAllocateFromCurrentBuffer(requiredSpace, &bufferData,
                                                                     &bufferOffset))
            {
                for (const gl::ShaderType shaderType : glExecutable.getLinkedShaderStages())
                {
    ...
    if (anyNewBufferAllocated)
    {
        ...
        else
        {
            ...
            ANGLE_TRY(mExecutable.allocDefaultUniformDescriptorSet(
                contextVk, defaultUniformBuffer->getUniqueObjectID(), &newDescriptorSetAllocated));
