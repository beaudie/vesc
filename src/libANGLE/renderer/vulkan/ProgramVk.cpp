...
angle::Result ProgramVk::linkImpl(const gl::Context *glContext, gl::InfoLog &infoLog)
{
    ...
    if (mState.getExecutable().hasLinkedShaderStage(gl::ShaderType::Vertex) && transformFeedback &&
    ...
    std::array<VkDescriptorPoolSize, 2> uniformAndXfbSetSize = {
        {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC,
          static_cast<uint32_t>(mState.getExecutable().getLinkedShaderStageCount())},
    ...
    mDynamicBufferOffsets.resize(mState.getExecutable().getLinkedShaderStageCount());
...
angle::Result ProgramVk::updateUniforms(ContextVk *contextVk)
{
    ...
    for (gl::ShaderType shaderType : mState.getExecutable().getLinkedShaderStages())
    {
        DefaultUniformBlock &uniformBlock = mDefaultUniformBlocks[shaderType];
