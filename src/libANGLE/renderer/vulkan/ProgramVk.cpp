                                  ...
                                  gl::InfoLog &infoLog)
{
    ...
    std::array<VkDescriptorPoolSize, 2> uniformAndXfbSetSize = {
        {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, GetUniformBufferDescriptorCount()},
         {VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, TransformFeedbackVk::GetMaxSeparateAttributes()}}};
...
angle::Result ProgramVk::updateUniforms(ContextVk *contextVk, vk::FramebufferHelper *framebuffer)
{
    ...
    if (anyNewBufferAllocated)
    {
        ...
        ANGLE_TRY(updateTransformFeedbackDescriptorSetImpl(contextVk, framebuffer));
    }
...
angle::Result ProgramVk::updateTransformFeedbackDescriptorSetImpl(
    ContextVk *contextVk,
    vk::FramebufferHelper *framebuffer)
{
    const gl::State &glState = contextVk->getState();
    ...
    TransformFeedbackVk *transformFeedbackVk = vk::GetImpl(glState.getCurrentTransformFeedback());
    return transformFeedbackVk->updateDescriptorSet(
