...
{
    ProgramLinkedResourcesLinker linker(nullptr);
                                                           ...
                                                           vk::FramebufferHelper *framebuffer)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < uniformBlocks.size(); ++bufferIndex)
    {
        ...
        bufferVk->onRead(framebuffer, VK_ACCESS_UNIFORM_READ_BIT);
                                              ...
                                              vk::CommandBuffer *commandBuffer)
{
    ...
    for (size_t descriptorSetIndex = 0; descriptorSetIndex < descriptorSetRange;
         ++descriptorSetIndex)
    {
        VkDescriptorSet set = mDescriptorSets[descriptorSetIndex];
        if (set == VK_NULL_HANDLE)
        {
            ...
            set = mEmptyDescriptorSets[descriptorSetIndex];
        }
