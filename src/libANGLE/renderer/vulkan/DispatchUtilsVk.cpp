                                                   ...
                                                   const ConvertVertexParameters &params)
{
    ...
    shaderParams.srcOffset   = params.srcOffset / shaderParams.Bs;
    shaderParams.destOffset  = params.destOffset;
    ...
    *reinterpret_cast<uint8_t *>(&endiannessTest) = 1;
    ...
    fprintf(stderr,
            ...
            shaderParams.Ss, shaderParams.Nd, shaderParams.Bd, shaderParams.Sd, shaderParams.Ed);
    ...
    angle::FormatID srcFormat =
        srcIsInt || srcIsSnorm || srcIsFixed
            ? shaderParams.Bs == 1
                  ? angle::FormatID::R8_SINT
                  : shaderParams.Bs == 2 ? angle::FormatID::R16_SINT : angle::FormatID::R32_SINT
            : shaderParams.Bs == 1
                  ? angle::FormatID::R8_UINT
                  : shaderParams.Bs == 2 ? angle::FormatID::R16_UINT : angle::FormatID::R32_UINT;
    ANGLE_TRY(src->initBufferView(context, renderer->getFormat(srcFormat)));
    ...
    writeInfo[0].sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeInfo[0].dstSet          = descriptorSet;
    writeInfo[0].dstBinding      = kConvertVertexDestinationBinding;
    writeInfo[0].descriptorCount = 1;
    writeInfo[0].descriptorType  = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    writeInfo[0].pBufferInfo     = buffers;

    writeInfo[1].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeInfo[1].dstSet           = descriptorSet;
    writeInfo[1].dstBinding       = kConvertVertexSourceBinding;
    writeInfo[1].descriptorCount  = 1;
    writeInfo[1].descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
    writeInfo[1].pTexelBufferView = src->getBufferView().ptr();
