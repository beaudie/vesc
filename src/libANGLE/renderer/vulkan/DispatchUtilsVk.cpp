...
void DispatchUtilsVk::destroy(vk::Context *context)
{
...
angle::Result DispatchUtilsVk::initialize(vk::Context *context)
{
    ...
    descriptorSetDesc[kBufferClear].update(kClearOutputBinding, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1);

    descriptorSetDesc[kBufferCopy].update(kCopyDestinationBinding, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1);
    descriptorSetDesc[kBufferCopy].update(kCopySourceBinding, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1);

    descriptorSetDesc[kTextureClear].update(kClearOutputBinding, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1);

    descriptorSetDesc[kTextureCopy].update(kCopyDestinationBinding, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1);
    descriptorSetDesc[kTextureCopy].update(kCopySourceBinding, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1);

    for (uint32_t i = 0; i < kFunctionCount; ++i)
    {
        ANGLE_TRY(renderer->getDescriptorSetLayout(context, descriptorSetDesc[i], &mDescriptorSetLayouts[i]));
    }
    ...
    for (uint32_t i = 0; i < kFunctionCount; ++i)
    {
        ...
        pipelineLayoutDesc.updatePushConstantRange(gl::ShaderType::Compute, 0, 32);

        ANGLE_TRY(renderer->getPipelineLayout(context, pipelineLayoutDesc, &mDescriptorSetLayouts[i], 1, &mPipelineLayouts[i]));
    }
                                            ...
                                            vk::CommandBuffer *commandBuffer)
{
    ...
    uint32_t pipelineLayoutIndex =
        isClear ? isBuffer ? kBufferClear : kTextureClear
                : isBuffer ? kBufferCopy : kTextureCopy;
    const vk::BindingPointer<vk::PipelineLayout> &pipelineLayout = mPipelineLayouts[pipelineLayoutIndex];
    ...
    commandBuffer->bindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout.get(), 0, 1, &descriptorSet, 0, nullptr);
    commandBuffer->pushConstants(pipelineLayout.get(), VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(params), &params);
...
angle::Result DispatchUtilsVk::clearBuffer(vk::Context *context, const BufferVk &dest,
                                           const ClearParameters &params,
                                           vk::CommandBuffer *commandBuffer)
{
    /* TEST CODE */
    uint32_t function = DispatchUtils_comp::kIsBuffer | DispatchUtils_comp::kIsClear/* | DispatchUtils_comp::kIsInt*/;
    Params cparams = {
        {0, 0, 0},
        {100, 0, 0},
        {0, 0, 0},
    ...
    ANGLE_TRY(mDescriptorPools[kBufferClear].allocateSets(context, mDescriptorSetLayouts[kBufferClear].get().ptr(),
    ...
    ANGLE_TRY(vk::AllocateBufferMemory(context, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
    ...
    float *memory;
    ANGLE_VK_TRY(context, bufferMemory.map(context->getDevice(), 0, createInfo.size, 0, (uint8_t **)&memory));

    fprintf(stderr, "Read back:\n");
    for (size_t i = 0; i < bufferSize; ++i)
    {
        fprintf(stderr, "%f ", memory[i]);
        if (i % 16 == 15)
            fprintf(stderr, "\n");
    }
    fprintf(stderr, "\n");
...
angle::Result DispatchUtilsVk::copyBuffer(vk::Context *context, const BufferVk &dest,
                                          const BufferVk &src,
...
angle::Result DispatchUtilsVk::clearTexture(vk::Context *context, const TextureVk &dest,
                                            const ClearParameters &params,
                                            vk::CommandBuffer *commandBuffer)
{
    UNIMPLEMENTED();
...
angle::Result DispatchUtilsVk::copyTexture(vk::Context *context, const TextureVk &dest,
                                           const TextureVk &src,
                                           const CopyParameters &params,
                                           vk::CommandBuffer *commandBuffer)
{
