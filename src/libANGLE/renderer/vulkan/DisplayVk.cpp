...
ShareGroupVk::ShareGroupVk()
{
    ...
    mBufferBlockOrphanAllowed = false;
}
...
void ShareGroupVk::addContext(ContextVk *contextVk)
{
    ...
    {
        mBufferBlockOrphanAllowed = true;
    }
}
...
void ShareGroupVk::onDestroy(const egl::Display *display)
{
    ...
    for (std::unique_ptr<vk::BufferPool> &pool : mDefaultBufferPools)
    {
        if (pool)
        {
            pool->destroy(renderer, mBufferBlockOrphanAllowed);
        }
    }

    if (mSmallBufferPool)
    {
        mSmallBufferPool->destroy(renderer, mBufferBlockOrphanAllowed);
    }
