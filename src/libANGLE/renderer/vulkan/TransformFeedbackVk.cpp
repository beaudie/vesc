    ...
    const gl::OffsetBindingPointer<gl::Buffer> &binding)
{
    ...
    {
        mBufferHelpers[index] = nullptr;
        mBufferHandles[index] = VK_NULL_HANDLE;
    }
    else
    {
        mBufferHelpers[index] = &vk::GetImpl(binding.get())->getBuffer();
    ...
    if (contextVk->getFeatures().supportsTransformFeedbackExtension.enabled)
    {
        if (mCounterBufferHandles[index] == VK_NULL_HANDLE)
        {
            ...
            vk::BufferHelper &bufferHelper = mCounterBufferHelpers[index];
            ANGLE_TRY(
                bufferHelper.init(contextVk, createInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));

            mCounterBufferHandles[index] = bufferHelper.getBuffer().getHandle();
        }
    }
    else
    {
        ...
        mAlignedBufferOffsets[index] = (mBufferOffsets[index] / offsetAlignment) * offsetAlignment;
    }
