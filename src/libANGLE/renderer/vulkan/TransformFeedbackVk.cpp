...
{
    size_t xfbBufferCount = mState.getIndexedBuffers().size();
                                              ...
                                              vk::FramebufferHelper *framebuffer) const
{
    ...
    size_t xfbBufferCount = xfbBuffers.size();
    if (bufferMode == GL_INTERLEAVED_ATTRIBS)
    {
        // If interleaved, there's only one output buffer.
        xfbBufferCount = 1;
...
void TransformFeedbackVk::updateBufferOffsets(GLenum bufferMode,
                                              uint32_t *offsetsOut,
