...
class RefCountedEventGarbageRecycler;
...
class Context : angle::NonCopyable
{
  public:
    ...
    RefCountedEventGarbageRecycler *getRefCountedEventGarbageRecycler()
    {
        return mShareGroupRefCountedEventGarbageRecycler;
    }

  protected:
    Renderer *const mRenderer;
    // Stash the ShareGroupVk's RefCountedEventRecycler here ImageHelper to conveniently access
    RefCountedEventGarbageRecycler *mShareGroupRefCountedEventGarbageRecycler;
...
{
  public:
    Recycler(const char *name) : mMaxSize(0), mName(name) {}
    ~Recycler() { WARN() << " Name:" << mName << " mMaxSize:" << mMaxSize; }
