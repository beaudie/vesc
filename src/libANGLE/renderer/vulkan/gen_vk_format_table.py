...
{space}{{
{space}    internalFormat = {internal_format};
{space}    VkFormatProperties formatProperties;
{space}    GetFormatProperties(physicalDevice, {vk_texture_format}, &formatProperties);
{space}    if (formatProperties.optimalTilingFeatures == 0)
{space}    {{
...
def gen_format_case(angle, internal_format, vk_json_data):
    ...
    if ((angle not in vk_map) and (angle not in vk_overrides) and (angle not in vk_fallbacks)) or angle == 'NONE':
...
vk_cases = [gen_format_case(angle, gl, vk_json_data) for angle, gl in sorted(angle_to_gl.iteritems())]
