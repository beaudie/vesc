...
texture_fallback_template = "initTextureFallback(physicalDevice, {{{texture_list}}});"
...
buffer_struct_template="{{{buffer}, {vk_buffer_format}, {vertex_load_function}, {vertex_load_converts}}}"
buffer_fallback_template = "initBufferFallback(physicalDevice, {{{buffer_list}}});"
...
def gen_format_case(angle, internal_format, vk_json_data):
    ...
    args = dict(format_id=angle, internal_format=internal_format, texture_template="", buffer_template="")
    ...
    def get_formats(format, type):
        ...
        if format not in vk_map:
            return []
        return [format] + vk_fallbacks.get(format, {}).get(type, [])

    def texture_args(format):
        return dict(
            texture="angle::FormatID::" + format,
            vk_texture_format=vk_map[format],
            texture_initializer=angle_format.get_internal_format_initializer(internal_format, format)
        )
    ...
    if len(textures) == 1:
        ...
        args.update(
            ...
            texture_list=", ".join(texture_struct_template.format(**texture_args(i)) for i in textures)
        )
