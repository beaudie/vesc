...
class CLProgramVk : public CLProgramImpl
{
  public:
    ...
    static constexpr uint32_t LatestSupportedBinaryVersion = 2;
    struct ProgramBinaryOutputHeader
    {
        uint32_t headerVersion{LatestSupportedBinaryVersion};
    ...
    struct DeviceProgramData
    {
        ...
        const VkPushConstantRange *getGlobalOffsetRange() const
        {
            const VkPushConstantRange *pushConstantRangePtr = nullptr;
            if (reflectionData.pushConstants.contains(
                    NonSemanticClspvReflectionPushConstantGlobalOffset))
            {
                pushConstantRangePtr = &reflectionData.pushConstants.at(
                    NonSemanticClspvReflectionPushConstantGlobalOffset);
            }
            return pushConstantRangePtr;
        }

        const VkPushConstantRange *getGlobalSizeRange() const
        {
            const VkPushConstantRange *pushConstantRangePtr = nullptr;
            if (reflectionData.pushConstants.contains(
