...
class CLProgramVk : public CLProgramImpl
{
  public:
    ...
    using PushConstantsEntry = std::pair<const unsigned int, VkPushConstantRange>;
    ...
    struct DeviceProgramData
    {
        ...
        CLKernelArguments getKernelArguments(const std::string &kernelName) const
        {
            ...
            if (containsKernel(kernelName))
            {
                ...
                {
                    CLKernelArgument kargCopy;
                    std::memcpy(&kargCopy, &karg, sizeof(CLKernelArgument));
    ...
    using DeviceProgramsEntry = std::pair<const _cl_device_id *const, DeviceProgramData>;
