...
class CLProgramVk : public CLProgramImpl
{
  public:
    ...
    struct DeviceProgramData
    {
        ...
        const KernelArgsMap &getKernelArgsMap() const { return reflectionData.kernelArgsMap; }
        ...
        KernelArguments getKernelArguments(const std::string &kernelName) const
        {
            KernelArguments kargsCopy;
            if (containsKernel(kernelName))
            {
                const KernelArguments &kargs = getKernelArgsMap().at(kernelName);
                for (const auto &karg : kargs)
                {
                    KernelArgument kargCopy;
                    std::memcpy(&kargCopy, &karg, sizeof(KernelArgument));
                    kargsCopy.push_back(std::move(kargCopy));
                }
            }
            return kargsCopy;
        }

        std::array<uint32_t, 3> getCompiledWGS(const std::string &kernelName) const
        {
            std::array<uint32_t, 3> compiledWGS{0, 0, 0};
    ...
    std::mutex &getProgramMutex() { return mProgramMutex; }
