                            ...
                            std::vector<uint32_t> *shaderCodeOut)
{
    ...
    int zResult = zlib_internal::GzipUncompressHelper(
        reinterpret_cast<uint8_t *>(shaderCodeOut->data()), &uncompressedSize,
                        ...
                        RefCounted<ShaderAndSerial> **shaderOut)
{
    ...
    *shaderOut                          = &shader;
    ...
    ANGLE_TRY(
        GetShaderCode(context, compressedShaderBlobs, shadersCount, shaderFlags, &shaderCode));
...
angle::Result ShaderLibrary::getBlitResolve_frag_code(Context *context,
                                                      uint32_t shaderFlags,
                                                      std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kBlitResolve_frag_shaders, ArraySize(kBlitResolve_frag_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getBlitResolveStencilNoExport_comp_code(
    Context *context,
    uint32_t shaderFlags,
    std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kBlitResolveStencilNoExport_comp_shaders,
                         ArraySize(kBlitResolveStencilNoExport_comp_shaders), shaderFlags,
                         shaderCodeOut);
}

angle::Result ShaderLibrary::getConvertIndex_comp_code(Context *context,
                                                       uint32_t shaderFlags,
                                                       std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kConvertIndex_comp_shaders, ArraySize(kConvertIndex_comp_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getConvertIndexIndirectLineLoop_comp_code(
    Context *context,
    uint32_t shaderFlags,
    std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kConvertIndexIndirectLineLoop_comp_shaders,
                         ArraySize(kConvertIndexIndirectLineLoop_comp_shaders), shaderFlags,
                         shaderCodeOut);
}

angle::Result ShaderLibrary::getConvertIndirectLineLoop_comp_code(
    Context *context,
    uint32_t shaderFlags,
    std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kConvertIndirectLineLoop_comp_shaders,
                         ArraySize(kConvertIndirectLineLoop_comp_shaders), shaderFlags,
                         shaderCodeOut);
}

angle::Result ShaderLibrary::getConvertVertex_comp_code(Context *context,
                                                        uint32_t shaderFlags,
                                                        std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kConvertVertex_comp_shaders,
                         ArraySize(kConvertVertex_comp_shaders), shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getFullScreenQuad_vert_code(Context *context,
                                                         uint32_t shaderFlags,
                                                         std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kFullScreenQuad_vert_shaders,
                         ArraySize(kFullScreenQuad_vert_shaders), shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getGenerateMipmap_comp_code(Context *context,
                                                         uint32_t shaderFlags,
                                                         std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kGenerateMipmap_comp_shaders,
                         ArraySize(kGenerateMipmap_comp_shaders), shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getImageClear_frag_code(Context *context,
                                                     uint32_t shaderFlags,
                                                     std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kImageClear_frag_shaders, ArraySize(kImageClear_frag_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getImageCopy_frag_code(Context *context,
                                                    uint32_t shaderFlags,
                                                    std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kImageCopy_frag_shaders, ArraySize(kImageCopy_frag_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getOverlayCull_comp_code(Context *context,
                                                      uint32_t shaderFlags,
                                                      std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kOverlayCull_comp_shaders, ArraySize(kOverlayCull_comp_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getOverlayDraw_comp_code(Context *context,
                                                      uint32_t shaderFlags,
                                                      std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kOverlayDraw_comp_shaders, ArraySize(kOverlayDraw_comp_shaders),
                         shaderFlags, shaderCodeOut);
}

angle::Result ShaderLibrary::getUnresolve_frag_code(Context *context,
                                                    uint32_t shaderFlags,
                                                    std::vector<uint32_t> *shaderCodeOut)
{
    return GetShaderCode(context, kUnresolve_frag_shaders, ArraySize(kUnresolve_frag_shaders),
                         shaderFlags, shaderCodeOut);
}
