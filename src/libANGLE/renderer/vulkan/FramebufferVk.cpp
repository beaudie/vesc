                                                ...
                                                gl::AttachmentsMask *unresolveAttachmentMask) const
{
    ...
    if (depthStencilRenderTarget)
    {
            ...
            depthStencilRenderTarget->isImageTransient())
        {
            ...
            {
                depthLoadOp = vk::RenderPassLoadOp::DontCare;
                unresolveAttachmentMask->set(gl::Framebuffer::DIRTY_BIT_DEPTH_ATTACHMENT);
            }
            ...
            if (unresolveStencil)
            {
                ...
                unresolveAttachmentMask->set(gl::Framebuffer::DIRTY_BIT_STENCIL_ATTACHMENT);
            }
        }
    ...
    bool *renderPassDescChangedOut)
{
    ...
    // DrawBufferMask constructor will automatically mask out excessive bits.
    const gl::DrawBufferMask unresolveColorMask(unresolveAttachmentMask.bits());
    bool unresolveDepth   = unresolveAttachmentMask[gl::Framebuffer::DIRTY_BIT_DEPTH_ATTACHMENT];
    bool unresolveStencil = unresolveAttachmentMask[gl::Framebuffer::DIRTY_BIT_STENCIL_ATTACHMENT];
    const bool unresolveChanged =
        mRenderPassDesc.getColorUnresolveAttachmentMask() != unresolveColorMask ||
        mRenderPassDesc.hasDepthUnresolveAttachment() != unresolveDepth ||
        mRenderPassDesc.hasStencilUnresolveAttachment() != unresolveStencil;
    if (unresolveChanged)
    {
