...
angle::Result FramebufferVk::getFramebuffer(ContextVk *contextVk, vk::Framebuffer **framebufferOut)
{
    ...
    for (size_t colorIndexGL : mState.getEnabledDrawBuffers())
    {
        ...
        // ASSERT(mCurrentFramebufferDesc.get(static_cast<uint32_t>(colorIndexGL)) ==
        //       colorRenderTarget->getAssignSerial(contextVk));
    ...
    if (depthStencilRenderTarget)
    {
        ...
        // ASSERT(mCurrentFramebufferDesc.get(vk::kFramebufferDescDepthStencilIndex) ==
    ...
    *framebufferOut = &mFramebuffer->getFramebuffer();
    mFramebufferCache.emplace(mCurrentFramebufferDesc, mFramebuffer);
    return angle::Result::Continue;
}
