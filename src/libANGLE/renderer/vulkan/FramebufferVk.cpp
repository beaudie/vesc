    ...
    if (!isGainZero)
    {
        ...
        for (uint32_t y = 0; y < fragmentShadingRateHeight; y++)
        {
            for (uint32_t x = 0; x < fragmentShadingRateWidth; x++)
            {
                ...
                float px =
                    (static_cast<float>(x) * fragmentShadingRateBlockWidth / attachmentWidth -
                     0.5f) *
                    2.0f;
                float py =
                    (static_cast<float>(y) * fragmentShadingRateBlockHeight / attachmentHeight -
                     0.5f) *
                    2.0f;
    ...
    const gl::Extents &attachmentExtent)
{
    ...
    const uint32_t fragmentShadingRateWidth =
        UnsignedCeilDivide(attachmentWidth, fragmentShadingRateBlockWidth);
    const uint32_t fragmentShadingRateHeight =
        UnsignedCeilDivide(attachmentHeight, fragmentShadingRateBlockHeight);
