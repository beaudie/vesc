                                           ...
                                           const gl::Rectangle &area)
{
    ...
    {
        // Because the invalidated area is within the render pass's render area, it is fine for
...
angle::Result FramebufferVk::updateDefinedContents(ContextVk *contextVk)
{
    vk::Framebuffer *currentFramebuffer = nullptr;
    ANGLE_TRY(getFramebuffer(contextVk, &currentFramebuffer));
    ...
    if (contextVk->hasStartedRenderPassWithFramebuffer(currentFramebuffer))
    {
        // At the end of a render pass, update the mDefinedContents of the render targets, based on
        // whether they were invalidated and potentially used afterwards
        RenderTargetVk *depthStencilRenderTarget = mRenderTargetCache.getDepthStencil(true);
        if (depthStencilRenderTarget && !depthStencilRenderTarget->hasDefinedContent() &&
            (contextVk->getStartedRenderPassCommands().isDepthAttachmentValid() ||
             contextVk->getStartedRenderPassCommands().isStencilAttachmentValid()))
        {
