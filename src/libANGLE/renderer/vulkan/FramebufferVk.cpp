...
angle::Result FramebufferVk::initColorFromResolveAttachment(ContextVk *contextVk,
                                                            uint32_t colorIndexVK,
                                                            RenderTargetVk *colorRenderTarget)
{
    // Because each attachment could have a different format, we can't unresolve multiple of them
    // in one draw call without creating a heap of UtilsVk shaders.  So, each render target is
    ...
    ANGLE_TRY(colorRenderTarget->getResolveImageView(contextVk, &srcView));

    UtilsVk::UnresolveParameters params;
    params.attachmentIndex = colorIndexVK;

    return contextVk->getUtils().unresolve(contextVk, this, src, srcView, params);
}
                                                ...
                                                vk::CommandBuffer **commandBufferOut)
{
    ...
    for (size_t colorIndexGL : mState.getEnabledDrawBuffers())
    {
        ...
        {
            if (renderPassAttachmentOps[colorAttachmentCount].loadOp == VK_ATTACHMENT_LOAD_OP_LOAD)
            {
                renderPassAttachmentOps[colorAttachmentCount].loadOp =
                    VK_ATTACHMENT_LOAD_OP_DONT_CARE;
    ...
    //
    // Note that render pass are compatible only if the differences are in loadOp/storeOp values, or
    // the existence of resolve attachments in single subpass render passes.  The modification here
    // can add/remove a subpass, or modify its input attachments.
    gl::DrawBufferMask unresolveMask = mRenderPassDesc.getColorUnresolveAttachmentMask();
    if (previousUnresolveMask != unresolveMask)
    {
        // TODO: is this possibly too late to update the render pass desc?
    ...
    {
        // Go over multisampled-render-to-texture attachments that had loadOp=LOAD.  These
        // attachments are initialized from the corresponding singlesampled data in an initial
        // subpass, which is done here.
        //
        // Note that the attachment indices of this subpass are not the same.  See
        // InitializeUnresolveSubpass for details.
        uint32_t colorIndexVK = 0;
        for (size_t colorIndexGL : unresolveMask)
        {
            RenderTargetVk *colorRenderTarget = colorRenderTargets[colorIndexGL];
            ANGLE_TRY(initColorFromResolveAttachment(contextVk, colorIndexVK, colorRenderTarget));
            ++colorIndexVK;
        }

        contextVk->nextSubpass();
    }
