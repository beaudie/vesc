                                           ...
                                           const gl::Rectangle &area)
{
    ...
    // If there are deferred clears, stage them.  syncState may have accumulated deferred clears,
                                  ...
                                  GLenum filter)
{
    ...
    // If any clears were picked up when syncing the read framebuffer (as the blit source), stage
    ...
    {
        srcFramebufferVk->stageDeferredClearsForReadFramebuffer(contextVk);
    }
                                            ...
                                            const gl::Rectangle &invalidateArea)
{
    ...
    // If there are still deferred clears, stage them.  See relevant comment in invalidateSub.
    stageDeferredClears(contextVk);
...
void FramebufferVk::stageDeferredClears(ContextVk *contextVk)
{
    // Called when staging clears of the draw framebuffer.  In that case, there can't be any
    ...
    // be deferred while this assumption doesn't hold (and stageDeferredClearsForReadFramebuffer
    // should be used instead).
    ASSERT(!contextVk->hasActiveRenderPass() || !mDeferredClears.any());
    stageDeferredClearsImpl(contextVk);
}

void FramebufferVk::stageDeferredClearsForReadFramebuffer(ContextVk *contextVk)
{
    stageDeferredClearsImpl(contextVk);
}

void FramebufferVk::stageDeferredClearsImpl(ContextVk *contextVk)
{
