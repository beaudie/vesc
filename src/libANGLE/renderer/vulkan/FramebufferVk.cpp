                                       ...
                                       const VkClearDepthStencilValue &clearDepthStencilValue)
{
    ...
    if (glState.isScissorTestEnabled() && (scissor.width == 0 || scissor.height == 0))
    {
    ...
    bool clearColor = clearColorBuffers.count() > 0;
    ...
    bool isScissorTestEffectivelyEnabled = glState.isScissorTestEnabled() && scissor != renderArea;
...
angle::Result FramebufferVk::clearWithRenderPassOp(
    ...
    const VkClearDepthStencilValue &clearDepthStencilValue)
{
    // TODO: Make sure to add an ES3 test where glClear is called with some color (for two
    // attachments), then glClearBuffer is called (for one attachment) with another color and make
    // sure the clear is done to the correct colors.

    // TODO: Make sure to add an ES3 test where each attachment is individually clearBuffered.  Make
    // sure then that they are all done within the same render pass.
    ...
    if (!mFramebuffer.renderPassStartedButEmpty())
    {
                                                ...
                                                vk::CommandBuffer **commandBufferOut)
{
    ...
    // Clear values are only used when loadOp=Clear is set in clearWithRenderPassOp.  Here, the
    // clear value is set to an arbitrary unlikely value to stand out better in case of a bug.
    VkClearValue unusedClearValue = {{{0.076f, 0.813f, 0.341f, 0.692f}}};
    ...
    for (size_t colorIndex : mState.getEnabledDrawBuffers())
    {
        ...
        renderPassAttachmentOps.initWithLoadStore(attachmentClearValues.size(),
                                                  ...
                                                  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
        attachmentClearValues.emplace_back(unusedClearValue);
    }
