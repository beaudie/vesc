...
angle::Result FramebufferVk::getAttachmentsAndImagesFromRenderTargets(
    ...
    vk::FramebufferAttachmentsVector<RenderTargetVk *> *renderTargetsOut)
{
    // Color attachments.
    const auto &colorRenderTargets = mRenderTargetCache.getColors();
    for (size_t colorIndexGL : mState.getColorAttachmentsMask())
    {
        ...
        renderTargetsOut->push_back(colorRenderTarget);
    }
    ...
    if (depthStencilRenderTarget)
    {
        ...
        attachments->push_back(imageView->getHandle());
        renderTargetsOut->push_back(depthStencilRenderTarget);
    }
    ...
    if (resolveImageViewIn)
    {
        ...
        renderTargetsOut->push_back(resolveRenderTargetIn);
    }
    else
    {
                                            ...
                                            const SwapchainResolveMode swapchainResolveMode)
{
    ...
    if (mCurrentFramebuffer.valid())
    {
        ...
        if (useImagelessFramebuffer)
        {
            ...
            std::vector<VkImageView> attachments;
            vk::FramebufferAttachmentsVector<RenderTargetVk *> fbRenderTargets;
    ...
    ANGLE_TRY(getAttachmentsAndImagesFromRenderTargets(
        contextVk, resolveImageViewIn, resolveRenderTargetIn, &attachments, &fbRenderTargets));
    ...
    else
    {
        ...
        for (auto const &renderTarget : fbRenderTargets)
        {
            vk::ImageHelper *rtImage = &renderTarget->getImageForRenderPass();
            uint32_t rtLevel         = renderTarget->getLevelIndex().get();
            uint32_t rtLayerCount    = renderTarget->getLayerCount();
            ...
            uint32_t baseLevel = static_cast<uint32_t>(rtImage->getFirstAllocatedLevel().get());
            uint32_t level     = (rtLevel > baseLevel) ? (rtLevel - baseLevel) : 0;
            uint32_t fbAttachmentWidth   = rtImage->getExtents().width >> level;
            uint32_t fbAttachmentHeight  = rtImage->getExtents().height >> level;
            ...
            fbAttachmentImageInfo.layerCount =
                (mCurrentFramebufferDesc.isMultiview())
                    ...
                    : rtLayerCount;
            fbAttachmentImageInfo.flags = rtImage->getCreateFlags();
            fbAttachmentImageInfo.usage = rtImage->getUsage();
            fbAttachmentImageInfo.viewFormatCount =
                static_cast<uint32_t>(rtImage->getViewFormats().max_size());
            fbAttachmentImageInfo.pViewFormats = rtImage->getViewFormats().data();
