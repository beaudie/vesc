                                            ...
                                            void *pixels)
{
    ...
    VkExtent3D srcExtent = {area.width, area.height, 1};
    ...
    vk::ImageHelper resolvedImage;
    if (isMultisampled)
    {
        ANGLE_TRY(resolvedImage.init2DStaging(
        ...
        resolvedImage.changeLayout(copyAspectFlags, vk::ImageLayout::TransferDst, commandBuffer);
        ...
        commandBuffer->resolveImage(srcImage->getImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                    resolvedImage.getImage(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                    1, &resolveRegion);

        resolvedImage.changeLayout(copyAspectFlags, vk::ImageLayout::TransferSrc, commandBuffer);

        // Make the resolved image the target of buffer copy.
        srcImage  = &resolvedImage;
        level     = 0;
        layer     = 0;
        srcOffset = {0, 0, 0};
    }
    ...
    {
        // The resolved image can be freed as soon as reading is done.
        resolvedImage.updateQueueSerial(renderer->getCurrentQueueSerial());
        resolvedImage.releaseImage(renderer);
    }
