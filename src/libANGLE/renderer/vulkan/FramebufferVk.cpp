...
void FramebufferVk::destroy(const gl::Context *context)
{
    ...
    inValidateCurrentFramebuffer(contextVk, mCurrentFramebufferDesc.attachmentCount() == 0);
}
...
void FramebufferVk::inValidateCurrentFramebuffer(ContextVk *contextVk, bool garbageCollect)
{
                                       ...
                                       gl::Command command)
{
    ...
    bool shouldInvalidateFramebuffer      = false;
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::Framebuffer::DIRTY_BIT_DEFAULT_FIXED_SAMPLE_LOCATIONS:
                ...
                shouldInvalidateFramebuffer = true;
                break;
    ...
    bool switchBetweenAttachmentAndNoAttachment = (priorFramebufferDesc.attachmentCount() == 0 &&
                                                   mCurrentFramebufferDesc.attachmentCount() > 0) ||
                                                  (priorFramebufferDesc.attachmentCount() > 0 &&
                                                   mCurrentFramebufferDesc.attachmentCount() == 0);
    if (switchBetweenAttachmentAndNoAttachment)
    {
        shouldInvalidateFramebuffer = true;
    }
    if (shouldInvalidateFramebuffer)
    {
        inValidateCurrentFramebuffer(contextVk, priorFramebufferDesc.attachmentCount() == 0);
    }
                                            ...
                                            const SwapchainResolveMode swapchainResolveMode)
{
    ...
    // Only check the cache if the Framebuffer has attachments
