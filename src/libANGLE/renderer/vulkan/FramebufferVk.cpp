    ...
    if (!isGainZero)
    {
        ...
        for (uint32_t y = 0; y < fragmentShadingRateHeight; y++)
        {
            for (uint32_t x = 0; x < fragmentShadingRateWidth; x++)
            {
                ...
                float px =
                    (((float)x * fragmentShadingRateBlockWidth) / attachmentWidth - 0.5f) * 2.0f;
                float py =
                    (((float)y * fragmentShadingRateBlockHeight) / attachmentHeight - 0.5f) * 2.0f;
    ...
    const gl::Extents &attachmentExtent)
{
    ...
    const uint32_t attachmentWidth                = attachmentExtent.width;
    const uint32_t attachmentHeight               = attachmentExtent.height;
    const uint32_t fragmentShadingRateWidth =
        static_cast<uint32_t>(std::ceil(attachmentWidth / fragmentShadingRateBlockWidth));
    const uint32_t fragmentShadingRateHeight =
        static_cast<uint32_t>(std::ceil(attachmentHeight / fragmentShadingRateBlockHeight));
    ...
    return generateFragmentShadingRateWithCPU(
        ...
        fragmentShadingRateBlockWidth, fragmentShadingRateBlockHeight, attachmentWidth,
        attachmentHeight, activeFocalPoints);
}
                                                  ...
                                                  const gl::Extents &attachmentExtents)
{
    ...
    {
        ANGLE_TRY(
            updateFragmentShadingRateAttachment(contextVk, newFoveationState, attachmentExtents));
        ASSERT(mFragmentShadingRateImage.valid());
                                       ...
                                       gl::Command command)
{
    ...
    gl::FoveationState newFoveationState;
    gl::Extents attachmentSize;
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::Framebuffer::DIRTY_BIT_FOVEATION:
                ...
                attachmentSize    = mState.getExtents();
                break;
            default:
            {
                ...
                if (attachment->hasFoveatedRendering())
                {
    ...
    if (mFoveationState != newFoveationState)
    {
        ANGLE_TRY(updateFoveationState(contextVk, newFoveationState, attachmentSize));
    }
