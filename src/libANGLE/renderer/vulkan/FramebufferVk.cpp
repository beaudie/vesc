    ...
    vk::FramebufferAttachmentsVector<RenderTargetInfo> *renderTargetsInfoOut)
{
    ...
    for (size_t colorIndexGL : mState.getColorAttachmentsMask())
    {
        ...
        renderTargetsInfoOut->emplace_back(RenderTargetInfo(colorRenderTarget, false));
    }
    ...
    if (depthStencilRenderTarget)
    {
        ...
        attachments->push_back(imageView->getHandle());
        renderTargetsInfoOut->emplace_back(RenderTargetInfo(depthStencilRenderTarget, false));
    }
    ...
    if (resolveImageViewIn)
    {
        ...
        renderTargetsInfoOut->emplace_back(RenderTargetInfo(resolveRenderTargetIn, true));
    }
    else
    {
        ...
        for (size_t colorIndexGL : mState.getColorAttachmentsMask())
        {
            ...
            if (colorRenderTarget->hasResolveAttachment())
            {
                ...
                renderTargetsInfoOut->emplace_back(RenderTargetInfo(colorRenderTarget, true));
            }
        }
    }
    ...
    if (depthStencilRenderTarget && depthStencilRenderTarget->hasResolveAttachment())
    {
        ...
        attachments->push_back(imageView->getHandle());
        renderTargetsInfoOut->emplace_back(RenderTargetInfo(depthStencilRenderTarget, true));
    }
                                            ...
                                            const SwapchainResolveMode swapchainResolveMode)
{
    ...
    else
    {
        ...
        {
            vk::ImageHelper *rtImage = (info.useResolveImage)
                                           ? &info.renderTarget->getResolveImageForRenderPass()
                                           : &info.renderTarget->getImageForRenderPass();
            uint32_t rtLevel         = info.renderTarget->getLevelIndex().get();
            uint32_t rtLayerCount    = info.renderTarget->getLayerCount();
