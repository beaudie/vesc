    ...
    for (size_t dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::Framebuffer::DIRTY_BIT_DRAW_BUFFERS:
                if ((mState.getEnabledDrawBuffers() & mCurrentEnabledDrawBuffers) ==
                    mState.getEnabledDrawBuffers())
                {
                    // If new draw buffers are the subset of the current draw buffers, we use color
                    // mask for the disabled draw buffers. By doing this, we avoid breaking the
                    // renderpass and helps performance on tiled renderer.
                    contextVk->updateColorMask(context->getState().getBlendState());
                }
                else
                {
