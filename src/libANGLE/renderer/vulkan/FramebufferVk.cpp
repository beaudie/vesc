                                  ...
                                  GLenum filter)
{
    ...
    if (blitColorBuffer)
    {
        ...
        // TODO: Just checking for SrgbWriteControlMode is too broad a condition. We end up
        // reinterpreting the colorspace iff format is sRGB and SrgbWriteControlMode is
        // SrgbWriteControlMode::Linear. But this forces the resolve to be performed without using a
        // draw which seems to have a bug related to image layout transition -
        // VUID-VkDescriptorImageInfo-imageLayout-00344 that is triggered by the skqp test -
        // UnitTest_DMSAA_dst_read. Enable the more specific check once the bug is fixed.
