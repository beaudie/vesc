                                    ...
                                    void *pixels)
{
    ...
    return readPixelsImpl(context, clippedArea, params,
                                                 ...
                                                 vk::RecordingMode *modeOut)
{
    ...
    {
        appendRenderPass(commandBufferOut);
                                        ...
                                        void *pixels)
{
    ...
    RenderTargetVk *renderTarget = getColorReadRenderTarget();
    const vk::Format &readFormat = renderTarget->getImageFormat();

    vk::ImageHelper stagingImage;
    ANGLE_TRY(stagingImage.init2DStaging(device, renderer->getMemoryProperties(), readFormat,
                                         gl::Extents(area.width, area.height, 1),
    ...
    const angle::Format &angleFormat = readFormat.textureFormat();
