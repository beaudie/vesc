                                                ...
                                                gl::AttachmentsMask *unresolveAttachmentMask) const
{
    ...
    for (size_t colorIndexGL : mState.getColorAttachmentsMask())
    {
        ...
        {
            renderPassAttachmentOps->setOps(colorIndexVk, vk::RenderPassLoadOp::Load, storeOp);
            packedClearValues->store(colorIndexVk, VK_IMAGE_ASPECT_COLOR_BIT,
    ...
    bool *renderPassDescChangedOut)
{
    ...
    // Optimize loadOp based on contentDefined bits
    vk::AttachmentOpsArray optimizedRenderPassAttachmentOps = renderPassAttachmentOps;
    {
        const auto &colorRenderTargets = mRenderTargetCache.getColors();
        vk::PackedAttachmentIndex colorIndexVk(0);
        for (size_t colorIndexGL : mState.getColorAttachmentsMask())
        {
            ...
            {
                vk::RenderPassLoadOp loadOp =
                    static_cast<vk::RenderPassLoadOp>(renderPassAttachmentOps[colorIndexVk].loadOp);
                if (loadOp == vk::RenderPassLoadOp::Load)
                {
                    SetBitField(optimizedRenderPassAttachmentOps[colorIndexVk].loadOp,
        ...
        if (depthStencilRenderTarget)
        {
            ...
            {
                vk::RenderPassLoadOp loadOp = static_cast<vk::RenderPassLoadOp>(
                    renderPassAttachmentOps[mDepthStencilAttachmentIndex].loadOp);
                if (loadOp == vk::RenderPassLoadOp::Load)
                {
                    SetBitField(
                        optimizedRenderPassAttachmentOps[mDepthStencilAttachmentIndex].loadOp,
                        vk::RenderPassLoadOp::DontCare);
                }
            }
            ...
            {
                vk::RenderPassLoadOp loadOp = static_cast<vk::RenderPassLoadOp>(
                    renderPassAttachmentOps[mDepthStencilAttachmentIndex].stencilLoadOp);
                if (loadOp == vk::RenderPassLoadOp::Load)
                {
                    SetBitField(optimizedRenderPassAttachmentOps[mDepthStencilAttachmentIndex]
                                    .stencilLoadOp,
                                vk::RenderPassLoadOp::DontCare);
                }
            }
    ...
    ANGLE_TRY(contextVk->beginNewRenderPass(
        framebuffer, renderArea, mRenderPassDesc, optimizedRenderPassAttachmentOps,
        mColorAttachmentCount, mDepthStencilAttachmentIndex, packedClearValues, commandBufferOut));
