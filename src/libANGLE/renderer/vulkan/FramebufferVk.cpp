...
gl::Error FramebufferVk::clear(ContextImpl *context, GLbitfield mask)
{
    ...
    for (const auto &colorAttachment : mState.getColorAttachments())
    {
        if (colorAttachment.isAttached())
        {
            ...
            commandBuffer->clearSingleColorImage(*renderTarget->getImage(), clearColorValue);
        }
    }
                                    ...
                                    GLvoid *pixels) const
{
    ...
    vk::Image *readImage = renderTarget->getImage();
...
gl::Error FramebufferVk::updateRenderPassAndFramebuffer(VkDevice device)
{
    if (!mDirty)
    {
        return vk::NoError();
    ...
    for (size_t attachmentIndex = 0; attachmentIndex < colorAttachments.size(); ++attachmentIndex)
    {
        ...
        if (colorAttachment.isAttached())
        {
            ...
            colorDesc.format  = renderTarget->getFormat().native;
            ...
            attachments.push_back(renderTarget->getImageView()->getHandle());
    ...
    if (useDepth)
    {
        ...
        depthStencilDesc.format         = renderTarget->getFormat().native;
...
{
    ANGLE_TRY(updateRenderPassAndFramebuffer(device));
    ASSERT(!mDirty && mRenderPass.getHandle() != VK_NULL_HANDLE);
    return &mRenderPass;
}
...
{
    ANGLE_TRY(updateRenderPassAndFramebuffer(device));
    ASSERT(!mDirty && mFramebuffer.getHandle() != VK_NULL_HANDLE);
    return &mFramebuffer;
}
