                                       ...
                                       const VkClearDepthStencilValue &clearDepthStencilValue)
{
    ...
    if (mDeferredClears.any())
    {
        ...
        else
        {
            ...
            {
                // clearWithLoad will operate on deferred clears.  Either these will affect the
...
angle::Result FramebufferVk::clearWithLoadOp(ContextVk *contextVk)
{
    ...
    // If the render pass has started, modify it's ops directly.  Note that the render pass should
