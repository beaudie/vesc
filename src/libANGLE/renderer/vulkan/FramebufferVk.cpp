                                  ...
                                  GLenum filter)
{
    ...
    if (blitColorBuffer)
    {
                 ...
                 (rotation == SurfaceRotation::Identity))
        {
            ...
            // the draw to still be open (and containing commands), so it can be updated to use the
            // resolve attachment to draw into. If there's no render pass with commands, then the
            // multisample draw has already occurred so the subpass resolve attachment no longer has
            // an opportunity to be written to.
                                       ...
                                       gl::Command command)
{
    ...
    const gl::Framebuffer *srcFramebuffer = glState.getReadFramebuffer();
    const bool isResolve                  = srcFramebuffer->getSamples(context) > 1;
