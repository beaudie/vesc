        ...
        switch (dirtyBit)
        {
            ...
            default:
            {
                ...
                if (renderTarget)
                {
                    ...
                    updateActiveColorMasks(colorIndex, sourceFormat.redBits > 0,
                                           sourceFormat.greenBits > 0, sourceFormat.blueBits > 0,
                                           sourceFormat.alphaBits > 0);

                    const angle::Format &destFormat = renderTarget->getImageFormat().angleFormat();

                    mEmulatedAlphaAttachmentMask.set(
                        colorIndex, destFormat.alphaBits == 0 && sourceFormat.alphaBits > 0);
                }
                else
                {
