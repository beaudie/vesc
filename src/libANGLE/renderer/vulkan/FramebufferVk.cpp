...
gl::Error FramebufferVk::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    ANGLE_TRY(startRendering(context, &commandBuffer));
...
gl::Error FramebufferVk::getRenderPassDesc(const gl::Context *context,
                                           const vk::RenderPassDesc **descOut)
{
    if (mRenderPassDesc.valid())
    {
        *descOut = &mRenderPassDesc.value();
        return gl::NoError();
    }
                                                                   ...
                                                                   RendererVk *rendererVk)
{
    ...
    ANGLE_TRY(getRenderPassDesc(context, &desc));

    vk::RenderPass *renderPass = nullptr;
    ANGLE_TRY(rendererVk->getRenderPass(*desc, &renderPass));
                                        ...
                                        vk::CommandBufferAndState **commandBufferOut)
{
    ...
    {
        return getCurrentInUseCommands()->startRenderPassRecording(
            renderer->getDevice(), renderer->getCommandPool(), commandBufferOut);
    }

    vk::CommandBufferNode *node = sireCommandNode(context, contextVk->getRenderer());
    ...
    for (size_t attachmentIndex = 0; attachmentIndex < colorAttachments.size(); ++attachmentIndex)
    {
        ...
        if (colorAttachment.isAttached())
        {
            ...
            node->appendColorRenderTarget(context, renderTarget);
        }
    }
    ...
    if (depthStencilAttachment && depthStencilAttachment->isAttached())
    {
        ...
        // TODO(jmadill): May need layout transition.
        node->appendDepthStencilRenderTarget(context, renderTarget);
    }
