...
void FramebufferVk::destroy(const gl::Context *context)
{
    ...
    invalidateCurrentFramebuffer(contextVk);
}
...
void FramebufferVk::invalidateCurrentFramebuffer(ContextVk *contextVk)
{
                                                     ...
                                                     const UtilsVk::BlitResolveParameters &params)
{
    ...
    srcFramebufferVk->invalidateCurrentFramebuffer(contextVk);
                                            ...
                                            const SwapchainResolveMode swapchainResolveMode)
{
    ...
    else
    {
        ...
        for (auto const &info : attachmentEmbeddedInfo)
        {
            ...
            // Some traces (e.g., aztec_ruins) fail without attachmentsSize
            //            fbAttachmentImageInfo.width  =
            //            static_cast<uint32_t>(attachmentsSize.width); fbAttachmentImageInfo.height
            //            = static_cast<uint32_t>(attachmentsSize.height);

            // BlitFramebufferTest.ResolveIntoBiggerFramebuffer/* fails without image extent
            fbAttachmentImageInfo.width  = static_cast<uint32_t>(info.image->getExtents().width);
            fbAttachmentImageInfo.height = static_cast<uint32_t>(info.image->getExtents().height);
