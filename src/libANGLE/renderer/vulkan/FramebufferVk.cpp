    ...
    const VkClearDepthStencilValue &clearDepthStencilValue)
{
    ...
    if (clearDepth)
    {
        ...
        // Because clear is not affected by depth test enable, we must explicitly wark it as a depth
        // write occured here.
    ...
    if (clearStencil)
    {
        dsAspectFlags |= VK_IMAGE_ASPECT_STENCIL_BIT;
        dsClearValue.depthStencil = clearDepthStencilValue;
        // Because clear is not affected by stencil test enable, we must explicitly mark it as a
        // stencil write occured here.
    ...
    if (dsAspectFlags != 0)
    {
        ...
        // Because we may have depth stencil changed access mode, update read only depth mode again.
                                                               ...
                                                               vk::CommandBufferHelper *renderPass)
{
    ...
    bool readOnlyDepth =
        mDepthReadOnlyFeedbackLoopMode || !renderPass->hasDepthStencilWriteOrClear();
    ...
    renderPass->updateRenderPassWithDepthMode(*currentFramebuffer, mRenderPassDesc, readOnlyDepth);
