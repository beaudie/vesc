                                       ...
                                       const gl::Framebuffer::DirtyBits &dirtyBits)
{
    ...
    bool needToRecreateFramebuffer           = false;
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::Framebuffer::DIRTY_BIT_COLOR_ATTACHMENT_ORPHANED:
                ...
                needToRecreateFramebuffer = true;
                break;
            default:
            {
    ...
    if (!needToRecreateFramebuffer && mCurrentFramebufferDesc == priorFramebufferDesc)
    {
        return angle::Result::Continue;
    }
    ...
    // Remove current VkFramebuffer object and create a new one
    if (needToRecreateFramebuffer)
    {
        auto iter = mFramebufferCache.find(mCurrentFramebufferDesc);
        if (iter != mFramebufferCache.end())
        {
            iter->second.release(contextVk);
            mFramebufferCache.erase(mCurrentFramebufferDesc);
        }
    }
