...
bool IsAnyAttachment3DAndNotAllLayers(const RenderTargetCache<RenderTargetVk> &renderTargetCache,
                                       ...
                                       const VkClearDepthStencilValue &clearDepthStencilValue)
{
    ...
    if (mDeferredClears.any())
    {
        ...
        else
        {
            ...
            // depth than the framebuffer layers, clears cannot be deferred.  This is because clear
            // may later need to be flushed with vkCmdClearColorImage, which cannot partially clear
            // the 3D texture.  In that case, the clears are flushed immediately too.
            bool anyAttachmentIs3DWithoutAllLayers = IsAnyAttachment3DAndNotAllLayers(
                mRenderTargetCache, mState.getColorAttachmentsMask(),
                mCurrentFramebufferDesc.getLayerCount());

            if (clearAnyWithDraw || anyAttachmentIs3DWithoutAllLayers)
            {
                ANGLE_TRY(flushDeferredClears(contextVk));
            }
            else
                                            ...
                                            const vk::ImageView *resolveImageViewIn)
{
    ...
    framebufferInfo.layers          = mCurrentFramebufferDesc.getLayerCount();
