...
angle::Result FramebufferVk::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    if (((clearColor && !hasEffectiveColorMask) || (clearDepth && clearStencil)) &&
                                                ...
                                                vk::CommandBuffer **commandBufferOut)
{
    ...
    for (size_t colorIndex : mState.getEnabledDrawBuffers())
    {
        RenderTargetVk *colorRenderTarget = colorRenderTargets[colorIndex];
        ASSERT(colorRenderTarget);
        ASSERT(colorRenderTarget->getImage().getCurrentLayout() ==
    ...
    if (depthStencilRenderTarget)
    {
        ...
        ASSERT(depthStencilRenderTarget->getImage().getCurrentLayout() ==
