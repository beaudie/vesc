...
bool BlitCompatibleSrcAndDstColorChannels(RenderTargetVk *srcRenderTarget,
...
bool BlitCompatibleSrcAndDstDepthStencilChannels(RenderTargetVk *srcRenderTarget,
                                                 RenderTargetVk *dstRenderTarget)
{
    const angle::Format &srcFormat = srcRenderTarget->getImageFormat().angleFormat();
    const angle::Format &dstFormat = dstRenderTarget->getImageFormat().angleFormat();
                                  ...
                                  GLenum filter)
{
    ...
    else
    {
        ...
        int x0Shift = std::round((clippedSourceArea.x - absSourceArea.x) / stretch[0]);
        int y0Shift = std::round((clippedSourceArea.y - absSourceArea.y) / stretch[1]);
        int x1Shift = std::round((absSourceArea.x1() - clippedSourceArea.x1()) / stretch[0]);
        int y1Shift = std::round((absSourceArea.y1() - clippedSourceArea.y1()) / stretch[1]);
        ...
        srcClippedDestArea.x = destArea.x + x0Shift;
        srcClippedDestArea.y = destArea.y + y0Shift;
        int x1               = destArea.x1() - x1Shift;
        int y1               = destArea.y1() - y1Shift;
    ...
    // Again, transfer the destination flip to source, so dest is unflipped.
    ...
    if (blitColorBuffer)
    {
        ...
        bool canBlitWithCommand =
            !isResolve && noClip && HasSrcBlitFeature(renderer, readRenderTarget);
        ...
        for (size_t colorIndexGL : mState.getEnabledDrawBuffers())
        {
            ...
            areChannelsBlitCompatible =
                areChannelsBlitCompatible &&
                BlitCompatibleSrcAndDstColorChannels(readRenderTarget, drawRenderTarget);
        }
    ...
    if (blitDepthBuffer || blitStencilBuffer)
    {
        ...
        bool areChannelsBlitCompatible =
            BlitCompatibleSrcAndDstDepthStencilChannels(readRenderTarget, drawRenderTarget);
                                                     ...
                                                     vk::ImageHelper *srcImage)
{
    ...
    {
        RenderTargetVk *drawRenderTarget = mRenderTargetCache.getColors()[colorIndexGL];
        drawRenderTarget->getImageForWrite(&mFramebuffer)
