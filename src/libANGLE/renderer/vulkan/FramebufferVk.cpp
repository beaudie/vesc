...
VkColorComponentFlags GetColorComponentFlagsFromFormat(const angle::Format &format)
{
    return (format.redBits > 0 ? VK_COLOR_COMPONENT_R_BIT : 0) |
           (format.greenBits > 0 ? VK_COLOR_COMPONENT_G_BIT : 0) |
           (format.blueBits > 0 ? VK_COLOR_COMPONENT_B_BIT : 0) |
           (format.alphaBits > 0 ? VK_COLOR_COMPONENT_A_BIT : 0);
}
