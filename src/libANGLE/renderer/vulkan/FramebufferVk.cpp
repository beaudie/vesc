...
FramebufferVk::FramebufferVk(const gl::FramebufferState &state)
    : FramebufferImpl(state),
      ...
      mBlitReadBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, kMinReadPixelsBufferSize)
{
}
...
void FramebufferVk::destroy(const gl::Context *context)
{
    ...
    mBlitReadBuffer.destroy(contextVk->getDevice());
}
    ...
    if (blitDepthBuffer || blitStencilBuffer)
    {
        ...
        else
        {
            ...
            if (flipSource || flipDest)
            {
                ...
                if (!mBlitReadBuffer.valid())
                {
                    mBlitReadBuffer.init(1, renderer);
                    ASSERT(mBlitReadBuffer.valid());
                }
                ...
                ANGLE_TRY(mBlitReadBuffer.allocate(renderer, sizeToRequest, &copyPtr, &handleOut,
                                                   &offsetOut, nullptr));
                ANGLE_TRY(readPixelsImpl(context, sourceArea, packPixelsParams, copyFlags,
                                         readRenderTarget, copyPtr));
                ANGLE_TRY(mBlitReadBuffer.flush(contextVk->getDevice()));
                ...
                mBlitReadBuffer.releaseRetainedBuffers(renderer);
            }
            else
            {
