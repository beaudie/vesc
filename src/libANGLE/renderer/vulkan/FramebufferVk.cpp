                                       ...
                                       const VkClearDepthStencilValue &clearDepthStencilValue)
{
    ...
    if (isMidRenderPassClear)
    {
        ...
        ASSERT(contextVk->hasStartedRenderPassWithSerial(mRenderPassSerial));
                                  ...
                                  GLenum filter)
{
    ...
    if (blitColorBuffer)
    {
                 ...
                 !reinterpretsColorspace)
        {
            ...
            {
                contextVk->restoreFinishedRenderPass(srcFramebufferVk->getRenderPassSerial());
            }
            ...
            bool canResolveWithSubpass =
                isCurrentFramebufferValid && mState.getEnabledDrawBuffers().count() == 1 &&
                mCurrentFramebufferDesc.getLayerCount() == 1 &&
                contextVk->hasStartedRenderPassWithSerial(srcFramebufferVk->getRenderPassSerial());
                                            ...
                                            const gl::Rectangle &invalidateArea)
{
    ...
    if (contextVk->hasStartedRenderPassWithSerial(mRenderPassSerial))
    {
                                                ...
                                                bool *renderPassDescChangedOut)
{
    ...
    ANGLE_TRY(contextVk->beginNewRenderPass(
        *framebuffer, renderArea, mRenderPassDesc, renderPassAttachmentOps, colorIndexVk,
        depthStencilAttachmentIndex, packedClearValues, commandBufferOut, &mRenderPassSerial));
