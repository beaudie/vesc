...
gl::Error FramebufferVk::getDepthStencilRenderTarget(const gl::Context *context,
                                                     RenderTargetVk *outRenderTarget) const
{
    const gl::FramebufferAttachment *drawBuffer = mState.getDepthOrStencilAttachment();
    ASSERT(drawBuffer);

    ANGLE_TRY(drawBuffer->getRenderTarget(context, &outRenderTarget));
    return gl::NoError();
}
                              ...
                              GLenum filter)
{
    ...
    if (blitColorBuffer)
    {
        RenderTargetVk *readRenderTarget =
            vk::GetImpl(sourceFramebuffer)->getColorReadRenderTarget();
    ...
    if (blitDepthBuffer || blitStencilBuffer)
    {
        ...
        ANGLE_TRY(getDepthStencilRenderTarget(context, drawRenderTarget));
