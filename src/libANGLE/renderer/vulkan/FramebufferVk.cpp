...
angle::Result FramebufferVk::resumeRenderPass(ContextVk *contextVk)
{
    ...
    for (size_t colorIndexGL : mState.getEnabledDrawBuffers())
    {
        ...
        {
            // TODO(ianelliott): more directly set mContentDefined = true
            colorRenderTarget->onColorDraw(contextVk);
            /* loadOp != DONT_CARE && storeOp == DONT_CARE */
            contextVk->getStartedRenderPassCommands().restoreRenderPassColorAttachment(
                attachmentIndexVk);
        }
        ++attachmentIndexVk;
    }
    ...
    if (depthStencilRenderTarget)
    {
        if (!depthStencilRenderTarget->hasDefinedContent())
        {
            ...
            depthStencilRenderTarget->onDepthStencilDraw(contextVk);
            /* loadOp != DONT_CARE && storeOp == DONT_CARE */
            contextVk->getStartedRenderPassCommands().restoreRenderPassDepthStencilAttachments(
                attachmentIndexVk);
        }
    }
