...
DynamicDescriptorPool::DynamicDescriptorPool()
    : mUniformBufferDescriptorsPerSet(0), mCombinedImageSamplerDescriptorsPerSet(0)
{
}
...
vk::Error DynamicDescriptorPool::allocateNewPool(const VkDevice &device)
{
    ...
    poolSizes[UniformBufferPool].descriptorCount =
        mUniformBufferDescriptorsPerSet * kMaxSets / kPoolSizeCount;
    poolSizes[TexturePool].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[TexturePool].descriptorCount =
        mCombinedImageSamplerDescriptorsPerSet * kMaxSets / kPoolSizeCount;
