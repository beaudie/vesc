...
// Synch the sampler state from the FE to this object
void SamplerVk::syncState(const gl::Context *context)
{
    ContextVk *contextVk = vk::GetImpl(context);
    ...
    if (mSampler.valid())
    {
        // TODO: When we have dirty bits, return existing valid sampler if nothing dirty
        contextVk->releaseObject(contextVk->getCurrentQueueSerial(), &mSampler);
    }
