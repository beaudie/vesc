...
1. **ANGLE Internal Translation**: The [shader translator][translator] converts application shaders
into Vulkan-compatible GLSL. Vulkan-compatible GLSL matches the
...
OpenGL line segment rasterization. For more info see [TranslatorVulkan.cpp][TranslatorVulkan.cpp].

2. **glslang To SPIR-V**: We then use Khronos' [glslang][glslang] to convert the Vulkan-compatible
GLSL into SPIR-V. The SPIR-V is then compiled into `VkShaderModules`. For details please see
[GlslangWrapper.cpp][GlslangWrapper.cpp].

The `VkShaderModules` are then used by `VkPipelines`. Note that we currently don't use
...
For non-multisampled rendering Vulkan uses an algorithm based on quad coverage. A small shape is
...
OpenGL's algorithm is based on [Bresenham's line algorithm][Bresenham]. Bresenham's algoirithm
selects pixels on the line between the two segment points. Bresenham's algorithm does not support
multisampling. When compared with Vulkan's line segment rasterization we can also notice visually
that it always selects a subset of the line segment pixels rasterized in Vulkan. See Figure 1:
