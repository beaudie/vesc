...
egl::Error IOSurfaceSurfaceVkMac::bindTexImage(const gl::Context *context,
                                               ...
                                               EGLint buffer)
{
    return angle::ToEGL(bindTexImage(context, texture, buffer), EGL_BAD_SURFACE);
}
                                                      ...
                                                      EGLint buffer)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, mColorAttachment.image.stageSubresourceUpdate(
                       contextVk, gl::ImageIndex::Make2D(0),
                       gl::Extents(static_cast<int>(width), pixelUnpack.imageHeight, 1),
                       gl::Offset(), internalFormatInfo, pixelUnpack,
                       kIOSurfaceFormats[mFormatIndex].type, reinterpret_cast<uint8_t *>(source),
                       format, vk::ImageAccess::Renderable, &isOutOfMemory));
...
angle::Result IOSurfaceSurfaceVkMac::releaseTexImageImpl(const gl::Context *context, EGLint buffer)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, mColorAttachment.image.readPixels(
                       contextVk, bounds, params, VK_IMAGE_ASPECT_COLOR_BIT, gl::LevelIndex(0), 0,
                       IOSurfaceGetBaseAddressOfPlane(mIOSurface, mPlane), &isOutOfMemory));
