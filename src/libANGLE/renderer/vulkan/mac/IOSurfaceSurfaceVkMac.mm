                                               ...
                                               EGLint buffer)
{
    ...
    if (!mBuffer.getBuffer().valid())
    {
        ...
        createInfo.flags              = 0;
        createInfo.size        = IOSurfaceGetBytesPerRowOfPlane(mIOSurface, mPlane) * mHeight;
    ...
    copyRegion.bufferImageHeight = IOSurfaceGetElementWidthOfPlane(mIOSurface, mPlane) & 0xFFFFFFFF;
    ...
    commandBuffer.copyBufferToImage(mBuffer.getBuffer().getHandle(),
                                    mColorAttachment.image.getImage(),
    ...
    result = renderer->queueSubmitOneOff(
        ...
        egl::ContextPriority::Medium, nullptr, vk::SubmitPolicy::AllowDeferred, &serial);
...
egl::Error IOSurfaceSurfaceVkMac::releaseTexImage(const gl::Context *context, EGLint buffer)
{
    ...
    commandBuffer.copyImageToBuffer(mColorAttachment.image.getImage(),
