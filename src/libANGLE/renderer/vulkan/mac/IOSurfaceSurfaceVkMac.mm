...
constexpr std::array<IOSurfaceFormatInfo, 9> kIOSurfaceFormats = {{
    ...
    {GL_RGB,      GL_UNSIGNED_BYTE,                4, GL_RGBX8_ANGLEX },
...
angle::Result IOSurfaceSurfaceVkMac::initializeImpl(DisplayVk *displayVk)
{
    ...
    GLenum nativeSizedInternalFormat = kIOSurfaceFormats[mFormatIndex].nativeSizedInternalFormat;
    const vk::Format &format         = renderer->getFormat(nativeSizedInternalFormat);
                                               ...
                                               EGLint buffer)
{
    ...
    // fprintf(stderr, "bindTexImage, %d, %d, %d, %d, %d, %d\n",
    // (int)IOSurfaceGetBytesPerRowOfPlane(mIOSurface, mPlane),
    // (int)IOSurfaceGetBytesPerElementOfPlane(mIOSurface, mPlane), (int)rowLengthInPixels,
    ...
    const gl::InternalFormat &internalFormatInfo =
        gl::GetSizedInternalFormatInfo(nativeSizedInternalFormat);
    const vk::Format &format = renderer->getFormat(nativeSizedInternalFormat);
...
egl::Error IOSurfaceSurfaceVkMac::releaseTexImage(const gl::Context *context, EGLint buffer)
{
    ...
    const angle::Format &destFormat =
        angle::Format::Get(angle::Format::InternalFormatToID(nativeSizedInternalFormat));
    ...
    /*RendererVk *renderer = displayVk->getRenderer();
    const vk::Format &vkFormat         = renderer->getFormat(nativeSizedInternalFormat);
    const angle::Format &storageFormat = vkFormat.getActualImageFormat(vk::ImageAccess::Renderable);

    fprintf(stderr, "releaseTexImage, %d, %d, %d, %d, %d\n", (int)destFormat.id, (int)mFormatIndex,
    (int)outputRowPitchInBytes, (int)storageFormat.pixelBytes, (int)bounds.width);*/
