                                     ...
                                     vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t textureIndex = 0; textureIndex < samplerBindings.size(); ++textureIndex)
    {
        ...
        const std::string samplerName =
            programVk->getGlslangSourceOptions().useOldRewriteStructSamplers
...
angle::Result ProgramPipelineVk::link(const gl::Context *glContext)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (glProgram && (glPrograms.find(glProgram) == glPrograms.end()))
        {
            ...
            // Re-assign locations.
            AssignLocations(programVk->getGlslangSourceOptions(), glProgram->getState(),
                            glProgram->getResources(), &variableInfoMap);
