...
angle::Result ProgramPipelineVk::createPipelineLayout(const gl::Context *glContext)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            for (const gl::ShaderType linkedShaderType : programState.getLinkedShaderStages())
            {
                uniformsAndXfbSetDesc.update(uniformBindingIndex++,
                                             VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1,
                                             gl_vk::kShaderStageMap[linkedShaderType]);
            }
                ...
                !programState.getLinkedTransformFeedbackVaryings().empty())
            {
                ...
                transformFeedbackVk->updateDescriptorSetLayout(contextVk, programState,
            ...
            AddInterfaceBlockDescriptorSetDesc(
                programState.getUniformBlocks(), getUniformBlockBindingsOffset(),
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, &resourcesSetDesc);
            AddInterfaceBlockDescriptorSetDesc(
                programState.getShaderStorageBlocks(), getStorageBlockBindingsOffset(),
            ...
            ANGLE_TRY(renderer->getDescriptorSetLayout(
                ...
                &mDescriptorSetLayouts[kShaderResourceDescriptorSetIndex]));
            ...
            std::array<VkDescriptorPoolSize, 2> uniformAndXfbSetSize = {
                {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC,
                  static_cast<uint32_t>(programState.getLinkedShaderStageCount())},
            ...
            {
                // For this workaround, we have to create an empty descriptor set for each
                // descriptor set index, so make sure their pools are initialized.
            ...
            {
                // Note that we always use an array of IMPLEMENTATION_MAX_ATOMIC_COUNTER_BUFFERS
                // storage buffers for emulating atomic counters, so if there are any atomic counter
                // buffers, we need to allocate IMPLEMENTATION_MAX_ATOMIC_COUNTER_BUFFERS
                // descriptors.
                const uint32_t atomicCounterStorageBufferCount =
                    atomicCounterBufferCount > 0 ? gl::IMPLEMENTATION_MAX_ATOMIC_COUNTER_BUFFERS
                                                 : 0;
                const uint32_t storageBufferDescCount =
                    storageBlockCount + atomicCounterStorageBufferCount;
                resourceSetSize.emplace_back(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                             storageBufferDescCount);
            }
            if (imageCount > 0)
            {
            ...
            if (resourceSetSize.size() > 0)
            {
                ANGLE_TRY(mDynamicDescriptorPools[kShaderResourceDescriptorSetIndex].init(
                    contextVk, resourceSetSize.data(),
                    static_cast<uint32_t>(resourceSetSize.size())));
            }
            if (textureCount > 0)
            {
            ...
            mDynamicBufferOffsets.resize(programState.getLinkedShaderStageCount());

            // Initialize an "empty" buffer for use with default uniform blocks where there are no
            // uniforms, or atomic counter buffer array indices that are unused.
    ...
    // TODO(timvp): Issue 3570: Support separable programs
    // We should have found the monolithic program, so it's a problem if we get here.
    ASSERT(false);
...
bool ProgramPipelineVk::relinkShaders(const gl::Context *glContext)
{
    ContextVk *contextVk                  = vk::GetImpl(glContext);
    const gl::State &glState              = glContext->getState();
    ...
    vk::PipelineHelper **pipelineOut)
{
    ...
    ShaderInfo &shaderInfo = enableLineRasterEmulation ? mLineRasterShaderInfo : mDefaultShaderInfo;
    vk::ShaderProgramHelper *shaderProgram = &shaderInfo.getShaderProgram();
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        {
            ProgramVk *programVk = getShaderProgram(glState, shaderType);
            ANGLE_TRY(programVk->initGraphicsShaders(contextVk, mode, shaderInfo,
                                                     enableLineRasterEmulation, shaderProgram));
            glPrograms.emplace(glProgram);
        }
    }
...
{
    uint32_t shaderStageCount = (getLinkedShaderStageCount(contextVk->getState()));
...
// TODO(timvp): Issue 3570: Need to handle separable programs.
// Assume program pipelines only contain monolithic programs.
angle::Result ProgramPipelineVk::updateTexturesDescriptorSet(ContextVk *contextVk)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            // TODO(timvp): Issue 3570: Need to handle separable programs.
            // Assume program pipelines only contain monolithic programs, so only a single iteration
            // is required.
            break;
        }
    }
                                                                ...
                                                                vk::CommandGraphResource *recorder)
{
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            const gl::State &glState = contextVk->getState();
            const std::vector<gl::AtomicCounterBuffer> &atomicCounterBuffers =
                program->getState().getAtomicCounterBuffers();
            ...
            {
                const gl::AtomicCounterBuffer &atomicCounterBuffer =
                ...
                WriteBufferDescriptorSetBinding(
                    bufferBinding, 0, descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                    bindingStart, binding, requiredOffsetAlignment, &bufferInfo, &writeInfo);
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            const std::vector<gl::ImageBinding> &imageBindings =
                program->getState().getImageBindings();
            ...
            for (uint32_t imageIndex = 0; imageIndex < imageBindings.size(); ++imageIndex)
            {
                     ...
                     ++arrayElement)
                {
                    ...
                    ANGLE_TRY(
                        textureVk->getStorageImageView(contextVk, (binding.layered == GL_TRUE),
    ...
    vk::CommandGraphResource *recorder)
{
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            updateBuffersDescriptorSet(contextVk, recorder, program->getState().getUniformBlocks(),
                                       VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            updateBuffersDescriptorSet(contextVk, recorder,
                                       program->getState().getShaderStorageBlocks(),
                                       VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
    ...
    vk::FramebufferHelper *framebuffer)
{
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        {
            const gl::State &glState = contextVk->getState();
            ASSERT(hasTransformFeedbackOutput(contextVk->getState()));

            TransformFeedbackVk *transformFeedbackVk =
                vk::GetImpl(glState.getCurrentTransformFeedback());
            transformFeedbackVk->addFramebufferDependency(contextVk, program->getState(),
                                                          framebuffer);
...
void ProgramPipelineVk::updateTransformFeedbackDescriptorSetImpl(ContextVk *contextVk)
{
    ...
    if (!hasTransformFeedbackOutput(contextVk->getState()))
    {
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            if (!glState.isTransformFeedbackActive())
            {
                ...
                if (transformFeedback)
                {
                    ...
                    transformFeedbackVk->initDescriptorSet(
                        contextVk, program->getState().getTransformFeedbackBufferCount(),
                        &mEmptyBuffer, mDescriptorSets[kUniformsAndXfbDescriptorSetIndex]);
                }
                return;
            }
            ...
            transformFeedbackVk->updateDescriptorSet(
                contextVk, program->getState(), mDescriptorSets[kUniformsAndXfbDescriptorSetIndex]);
