    ...
    vk::PipelineHelper **pipelineOut)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            pipelinePipelineLayoutDesc.push_back(program->getPipelineLayoutDesc());
            pipelineDescriptorSetLayout.push_back(
                program->getShaderStageDescriptorSetLayouts(shaderType));
        }
    }

    ANGLE_TRY(renderer->getPipelinePipelineLayout(contextVk, pipelinePipelineLayoutDesc,
                                                  pipelineDescriptorSetLayout, &mPipelineLayout));
                                                      ...
                                                      vk::CommandBuffer *commandBuffer)
{
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ANGLE_TRY(program->updateDescriptorSets(contextVk, commandBuffer, getPipelineLayout()));
        }
    }
