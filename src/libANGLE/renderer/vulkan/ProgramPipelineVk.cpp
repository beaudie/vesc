...
ProgramPipelineVk::ProgramPipelineVk(const gl::ProgramPipelineState &state)
    : ProgramPipelineImpl(state),
      ...
      mRelinkRequired(false),
      mPipelineCreationRequired(true)
{}
...
{
    if (!mPipelineCreationRequired)
    {
        return angle::Result::Continue;
    }
    mPipelineCreationRequired = false;
...
bool ProgramPipelineVk::relinkShaders(const gl::Context *glContext)
{
    ...
    const gl::State &glState              = glContext->getState();
    const gl::ProgramPipeline *glPipeline = glState.getProgramPipeline();
    GlslangSourceOptions options          = createSourceOptions(glContext);
...
#if 0  // TIMTIM
    reset(contextVk);
    ANGLE_TRY(createPipelineLayout(glContext));
#endif
