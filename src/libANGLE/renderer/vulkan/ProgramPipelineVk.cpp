...
void ProgramPipelineVk::initializeSourceOptions(const gl::Context *glContext)
{
...
uint32_t AddInterfaceBlockDescriptorSetDesc(
    const std::vector<gl::InterfaceBlock> &blocks,
    uint32_t bindingStart,
    const gl::ShaderType shaderType,
    VkDescriptorType descType,
    std::unordered_map<std::string, int> *interfaceBlockBindingsMapOut,
    vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size();)
    {
        ...
        if (activeStages)
        {
            ...
            (*interfaceBlockBindingsMapOut)[block.name] = bindingStart + bindingIndex;
        }
...
uint32_t AddTextureDescriptorSetDesc(const gl::ProgramState &programState,
                                     uint32_t bindingStart,
                                     bool useOldRewriteStructSamplers,
                                     VkShaderStageFlags activeStages,
                                     std::unordered_map<std::string, int> *textureBindingsMapOut,
                                     vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t textureIndex = 0; textureIndex < samplerBindings.size(); ++textureIndex)
    {
        ...
        // Record the descriptor set binding value for this interface block used by this shader
        (*textureBindingsMapOut)[GlslangGetMappedSamplerName(samplerUniform.name)] = bindingStart;
...
angle::Result ProgramPipelineVk::createPipelineLayout(const gl::Context *glContext)
{
    ...
    initializeSourceOptions(glContext);
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            mGlslangSourceOptions.uniformsAndXfbDescriptorSetIndexShaderMap[shaderType] =
                bindingIndex;
            bindingIndex++;
        }
    }
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            bindingIndex = AddInterfaceBlockDescriptorSetDesc(
                ...
                &mGlslangSourceOptions.interfaceBlockBindingsShaderMap[shaderType],
                &resourcesSetDesc);
        }
    }
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            mGlslangSourceOptions.shaderResourceDescriptorSetIndexShaderMap[shaderType] =
                bindingIndex;
        }
    }
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            bindingIndex = AddTextureDescriptorSetDesc(
                program->getState(), bindingIndex, contextVk->useOldRewriteStructSamplers(),
                gl_vk::kShaderStageMap[shaderType],
                &mGlslangSourceOptions.textureBindingsShaderMap[shaderType], &texturesSetDesc);
        }
    }
...
bool ProgramPipelineVk::relinkShaders(const gl::Context *glContext)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (glProgram && (glPrograms.find(glProgram) == glPrograms.end()))
        {
            ...
            programVk->setGlslangSourceOptions(mGlslangSourceOptions);
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
                 ...
                 textureIndex < program->getState().getSamplerBindings().size(); ++textureIndex)
            {
                ...
                // TIMTIM - Really broken for separable programs
                ...
                for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
                {
                    ...
                    mGlslangSourceOptions.textureDescriptorSetIndexShaderMap[shaderType] =
                        bindingIndex;
                }
            }
        }
