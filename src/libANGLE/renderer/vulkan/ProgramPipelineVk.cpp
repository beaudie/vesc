...
angle::Result ProgramPipelineVk::createPipelineLayout(const gl::Context *glContext)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            ...
            mGlslangSourceOptions.uniformsAndXfbDescriptorSetIndexMap[shaderType] =
                uniformBindingIndex;
            uniformBindingIndex++;
        }
    }
    ...
    uint32_t bindingIndex = getUniformBlockBindingsOffset();
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        const gl::Program *program = glPipeline->getShaderProgram(shaderType);
        if (program)
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program)
        {
            bindingIndex = AddTextureDescriptorSetDesc(program->getState(), bindingIndex,
