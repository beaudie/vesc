...
constexpr ShaderCode kShaderCodeArray[] = {{
{internal_shader_array_entries}
}};
}}  // anonymous namespace

const ShaderCode &GetInternalShaderCode(InternalShaderID shaderID)
{{
    ASSERT(static_cast<size_t>(shaderID) < static_cast<size_t>(InternalShaderID::EnumCount));
    return kShaderCodeArray[static_cast<size_t>(shaderID)];
}}
}}  // namespace priv
}}  // namespace vk
}}  // namespace rx
...
namespace priv
{{
// This is a pair of the shader SPIR-V binary blob and the size.
using ShaderCode = std::pair<const uint32_t *, size_t>;
const ShaderCode &GetInternalShaderCode(InternalShaderID shaderID);
}}  // priv
}}  // namespace vk
}}  // namespace rx
...
template_internal_shader_entry = """
"""
...
# Generates the code for a shader code array entry.
def gen_shader_code_entry(shader):
...
with open(out_file_cpp, 'w') as outfile:
    ...
    array_entries = ",\n".join([gen_shader_code_entry(shader) for shader in output_shaders])
