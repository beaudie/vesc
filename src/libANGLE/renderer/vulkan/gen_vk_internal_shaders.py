...
def compile_variation(shader_file, shader_basename, flags, enums,
    ...
    if do_compile:
        print get_output_path(output_name) + ': ' + shader_basename + variation_string
...
def get_variation_definition(shader_and_variation):
    ...
    if len(flags) > 0:
        ...
        definition += ''.join(['%s = 0x%08X,\n' % (flags[f], 1 << f) for f in range(len(flags))])
        definition += 'flagsMask = 0x%08X,\n' % ((1 << flags_bits) - 1)
        definition += '};\n'
...
def get_shader_table_h(shader_and_variation):
    ...
    table = 'ShaderAndSerial %s[' % table_name

    namespace_name = get_namespace_name(shader_file)

    first_or = True
    if len(flags) > 0:
        table += '%s::flagsMask' % namespace_name
        first_or = False
    ...
    for e in range(len(enums)):
        ...
        if not first_or:
            table += ' | '
        table += '%s::%sMask' % (namespace_name, enum_name)
        first_or = False
...
def get_shader_table_cpp(shader_and_variation):
    ...
    for variation in range(1 << current_bit_start):
...
def get_get_function_h(shader_and_variation):
    ...
    return 'angle::Result %s(Context *context, uint32_t shaderFlags, const ShaderAndSerial **shaderOut);' % function_name
...
def get_get_function_cpp(shader_and_variation):
    ...
    definition = 'angle::Result %s(Context *context, uint32_t shaderFlags, const ShaderAndSerial **shaderOut)\n{\n' % function_name
    definition += 'ASSERT(shaderFlags < ArraySize(%s))\n' % constant_table_name

    definition += ''.join(['ASSERT((shaderFlags & %sMask) >= %s::%s && (shaderFlags & %sMask) <= %s::%s)\n' %
        (enums[e][0], namespace_name, enums[e][1][0], enums[e][0], namespace_name, enums[e][1][-1])
        for e in range(len(enums))])

    definition += 'ShaderAndSerial &shader = %s[shaderFlags];\n' % member_table_name
    ...
    definition += 'return InitShaderAndSerial(context, &shader, shaderCode.code, shaderCode.codeSize);\n'
    definition += '}\n'
...
def get_destroy_call(shader_and_variation):
    ...
    destroy = 'for (ShaderAndSerial &shader : %s)\n' % table_name
