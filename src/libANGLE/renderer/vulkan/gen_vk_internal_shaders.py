...
def get_get_function_cpp(shader_and_variation):
    ...
    definition = 'angle::Result ShaderLibrary::%s(Context *context, uint32_t shaderFlags, const ShaderAndSerial **shaderOut)\n{\n' % function_name
    definition += 'ASSERT(shaderFlags < ArraySize(%s));\n' % constant_table_name

    definition += ''.join(['ASSERT((shaderFlags & %s::%sMask) >= %s::%s && (shaderFlags & %s::%sMask) <= %s::%s);\n' %
