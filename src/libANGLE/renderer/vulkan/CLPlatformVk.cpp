...
void CLPlatformVk::Initialize(CreateFuncs &createFuncs)
{
    createFuncs.emplace_back([](const cl::Platform &platform) {
        CLPlatformVk *platformVk = new CLPlatformVk(platform);
        if (IsError(platformVk->initBackendRenderer()))
        {
            delete platformVk;
            platformVk = nullptr;
        }
        return Ptr(platformVk);
    });
}
...
#        else
    handleError(VK_ERROR_INCOMPATIBLE_DRIVER);
