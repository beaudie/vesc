             ...
             currentCommand->id != CommandID::Invalid; currentCommand = NextCommand(currentCommand))
        {
            switch (currentCommand->id)
            {
                ...
                case CommandID::PipelineBarrier2:
                {
                    ...
                    const VkMemoryBarrier2 *memoryBarriers =
                        GetFirstArrayParameter<VkMemoryBarrier2>(params);
                    const VkImageMemoryBarrier2 *imageMemoryBarriers =
                        GetNextArrayParameter<VkImageMemoryBarrier2>(memoryBarriers,
                                                                     params->memoryBarrierCount);
                    VkDependencyInfo pDependencyInfo         = {};
                    pDependencyInfo.sType                    = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
                    pDependencyInfo.dependencyFlags          = params->dependencyFlags;
                    pDependencyInfo.memoryBarrierCount       = params->memoryBarrierCount;
                    pDependencyInfo.pMemoryBarriers          = memoryBarriers;
                    pDependencyInfo.bufferMemoryBarrierCount = 0;
                    pDependencyInfo.pBufferMemoryBarriers    = nullptr;
                    pDependencyInfo.imageMemoryBarrierCount  = params->imageMemoryBarrierCount;
                    pDependencyInfo.pImageMemoryBarriers     = imageMemoryBarriers;
                    vkCmdPipelineBarrier2(cmdBuffer, &pDependencyInfo);
                    break;
                }
                case CommandID::PushConstants:
                ...
                case CommandID::ResetEvent2:
                {
                    const ResetEvent2Params *params =
                        getParamPtr<ResetEvent2Params>(currentCommand);
                    vkCmdResetEvent2(cmdBuffer, params->event, params->stageMask);
                    break;
                }
                case CommandID::ResetQueryPool:
                {
                ...
                case CommandID::SetEvent2:
                {
                    const SetEvent2Params *params = getParamPtr<SetEvent2Params>(currentCommand);
                    vkCmdSetEvent2(cmdBuffer, params->event, params->dependencyInfo);
                    break;
                }
                case CommandID::SetFragmentShadingRate:
                {
                ...
                case CommandID::WaitEvents2:
                {
                    const WaitEvents2Params *params =
                        getParamPtr<WaitEvents2Params>(currentCommand);
                    const VkEvent *events = GetFirstArrayParameter<VkEvent>(params);
                    const VkDependencyInfo *dependencyInfos =
                        GetNextArrayParameter<VkDependencyInfo>(events, params->eventCount);
                    vkCmdWaitEvents2(cmdBuffer, params->eventCount, events, dependencyInfos);
                    break;
                }
                case CommandID::WriteTimestamp:
                {
