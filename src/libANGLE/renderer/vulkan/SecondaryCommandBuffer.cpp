...
{
    VkPipeline pipelineHandle = pipeline.getHandle();
    if (mLastBoundPipeline != pipelineHandle)
    {
        BindPipelineParams *paramStruct =
            initCommand<BindPipelineParams>(CommandID::BindPipeline, 0);
        paramStruct->pipelineBindPoint = pipelineBindPoint;
        paramStruct->pipeline          = pipelineHandle;
        mLastBoundPipeline             = pipelineHandle;
    }
}
