                                     ...
                                     void **mapPtr)
{
    ...
    mMappedRange = gl::RangeULL(static_cast<size_t>(offset), static_cast<size_t>(offset + length));
                                    ...
                                    BufferUpdateType updateType)
{
    ...
    // Update conversions
    dataUpdated(gl::RangeULL(updateOffset, updateOffset + updateSize));
                                                            ...
                                                            bool hostVisible)
{
    for (VertexConversionBuffer &buffer : mVertexConversionBuffers)
    {
        if (offsetMustMatch)
        {
            if (buffer.match(formatID, stride, offset, hostVisible, false))
            {
                ASSERT(buffer.valid());
                return &buffer;
            }
        ...
        {
            if (buffer.compatible(formatID, stride, offset, hostVisible))
            {
                ASSERT(buffer.valid());
...
void BufferVk::dataUpdated(const gl::RangeULL &range)
{
    for (VertexConversionBuffer &buffer : mVertexConversionBuffers)
    {
