...
void BufferVk::release(ContextVk *contextVk)
{
    ...
    {
        ASSERT(!mBuffer->isExternalBuffer());
                                         ...
                                         size_t offset)
{
    ...
    if (updateRegionBeforeSubData || updateRegionAfterSubData)
    {
        ...
        // mBuffer was not allocated from the pool, We set mBuffer to null to prevent
        // acquireBufferHelper from releasing it. The actual release call is deferred after we issue
        // copyFromBuffer.
        mBuffer               = nullptr;
