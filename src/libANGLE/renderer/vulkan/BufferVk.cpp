                                    ...
                                    GLsizeiptr size)
{
    ...
    if (mShadowBuffer.valid())
    {
        ...
        uint8_t *srcPtr;
        ANGLE_TRY(sourceBuffer->getBuffer().map(contextVk, &srcPtr, sourceOffset));
                                     ...
                                     void **mapPtr)
{
    if (!mShadowBuffer.valid())
    {
        ...
        ANGLE_TRY(mBuffer.map(contextVk, reinterpret_cast<uint8_t **>(mapPtr),
                                      ...
                                      gl::IndexRange *outRange)
{
    ...
    if (!mShadowBuffer.valid())
    {
        ...
        ANGLE_TRY(mBuffer.map(contextVk, &mapPointer, offset));
    }
    else
    {
