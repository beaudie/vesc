    ...
    if (!mShadowBuffer.valid())
    {
            ...
            !mBuffer->isExternalBuffer() && mBuffer->isHostVisible())
        {
            ...
            else if (!mBuffer->isCurrentlyInUseForWrite(contextVk->getLastCompletedQueueSerial()))
            {
                ...
                ANGLE_TRY(mBuffer->mapWithOffset(contextVk, &newBufferMapPtr,
                                                 static_cast<size_t>(mBufferOffset)));

                ASSERT(previousBuffer->isCoherent());
                ASSERT(mBuffer->isCoherent());
                memcpy(newBufferMapPtr, previousBufferMapPtr,
                       static_cast<size_t>(mState.getSize()));

                previousBuffer->unmap(contextVk->getRenderer());
                // Return the already mapped pointer with the offset adjustment so we only need to
                // unmapImpl() once, which will flush the full buffer.
