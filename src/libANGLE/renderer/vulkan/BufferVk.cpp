...
ANGLE_INLINE VkMemoryPropertyFlags GetStorageMemoryType(GLbitfield storageFlags)
{
    ...
    {
        memoryType |= (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT);
    }

    if (storageFlags & GL_MAP_COHERENT_BIT_EXT)
    {
        memoryType |= VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    }

    if (!(storageFlags & GL_MAP_PERSISTENT_BIT_EXT) && !(storageFlags & GL_DYNAMIC_STORAGE_BIT_EXT))
    {
...
angle::Result BufferVk::setStorageData(const gl::Context *context,
                                       gl::BufferBinding target,
                                       const void *data,
                                       size_t size,
                                       GLbitfield flags)
{
    VkMemoryPropertyFlags memoryPropertyFlags = GetStorageMemoryType(flags);
    bool persistentMapRequired                = (flags & GL_MAP_PERSISTENT_BIT_EXT);
    return setDataWithMemoryType(context, target, static_cast<const uint8_t *>(data), size,
                                 memoryPropertyFlags, persistentMapRequired);
}
