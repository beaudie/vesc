    ...
    if (!mShadowBuffer.valid())
    {
        ...
        if (isCurrentlyInUse(contextVk) && !mBuffer->isExternalBuffer() && mBuffer->isHostVisible())
        {
            ...
            else if (smallMapRange && rangeInvalidate)
            {
                ...
                mMapInvalidateRangeBuffer.map(0, reinterpret_cast<uint8_t **>(mapPtr));
                mMapInvalidateRangeMappedPtr = reinterpret_cast<uint8_t *>(*mapPtr);
                return angle::Result::Continue;
            }
...
angle::Result BufferVk::unmapImpl(ContextVk *contextVk)
{
    ...
    if (mMapInvalidateRangeMappedPtr != nullptr)
    {
        ...
        ANGLE_TRY(stagedUpdate(contextVk, mMapInvalidateRangeMappedPtr,
