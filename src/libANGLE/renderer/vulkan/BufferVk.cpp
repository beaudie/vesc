                                           ...
                                           uint8_t **mapPtr)
{
    ...
    if (mStagingBuffer.valid())
    {
        if (size <= mStagingBuffer.getSize() &&
            ...
            !mStagingBuffer.isCurrentlyInUse(contextVk->getLastCompletedQueueSerials()))
        {
                                     ...
                                     void **mapPtr)
{
    ...
    if ((access & GL_MAP_WRITE_BIT) == 0)
    {
        ...
        if (mBuffer.isCurrentlyInUseForWrite(contextVk->getLastCompletedQueueSerials()))
        {
            // If there are pending commands for the resource, flush them.
            if (mBuffer.usedInRecordedCommand(contextVk))
            {
                ANGLE_TRY(
                                         ...
                                         BufferUpdateType updateType)
{
    ...
    if (updateRegionBeforeSubData || updateRegionAfterSubData)
    {
        ...
        if (src.isHostVisible() &&
            !src.isCurrentlyInUseForWrite(contextVk->getLastCompletedQueueSerials()) &&
...
{
    return mBuffer.isCurrentlyInUse(contextVk->getLastCompletedQueueSerials());
}
