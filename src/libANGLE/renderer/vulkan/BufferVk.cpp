                                         ...
                                         BufferUpdateType updateType)
{
    ...
    if (updateRegionBeforeSubData || updateRegionAfterSubData)
    {
        ...
        mBuffer               = nullptr;
        srcBufferNeedsRelease = !mBufferPool && !src->isExternalBuffer();
                                            ...
                                            BufferUpdateType updateType)
{
    ...
    if (mUseDynamicBufferPool && !mBufferPool)
    {
        // We are switching from VMA sub-allocator to mBufferPool, so we need to release it. Once we
        // switch to mBufferPool we stay with mBufferPool.
        if (mBuffer)
        {
            mBuffer->release(renderer);
            mBuffer = nullptr;
        ...
        mBufferPool = std::make_unique<vk::DynamicBuffer>();
