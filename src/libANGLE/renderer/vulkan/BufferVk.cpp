...
bool ShouldAvoidRenderPassBreakOnUpdate(ContextVk *contextVk, const vk::BufferHelper &buffer)
{
    ...
    if (!renderer->getFeatures().preferCPUForBufferSubData.enabled)
    {
        return false;
    }
                                    ...
                                    BufferUpdateType updateType)
{
    ...
    if (isCurrentlyInUse(contextVk))
    {
        ...
        if (!isExternalBuffer() && updateType != BufferUpdateType::StorageRedefined &&
            (!mHasValidData || ShouldAvoidRenderPassBreakOnUpdate(contextVk, mBuffer) ||
