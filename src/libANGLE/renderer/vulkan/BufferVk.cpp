...
void BufferVk::release(ContextVk *contextVk)
{
    ...
    ASSERT(contextVk->checkAndFlushExcessivePendingGarbage() == angle::Result::Continue);
                                              ...
                                              VkMemoryPropertyFlags memoryPropertyFlags)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, mBuffer.initExternal(contextVk, memoryPropertyFlags, createInfo, clientBuffer));
    return angle::Result::Continue;
}
                                           ...
                                           uint8_t **mapPtr)
{
    ...
    if (mStagingBuffer.valid())
    {
        ...
        RendererVk *renderer = contextVk->getRenderer();
        mStagingBuffer.release(renderer);
        ANGLE_TRY(contextVk->checkAndFlushExcessivePendingGarbage());
    }

    ANGLE_VK_TRY_ALLOC(contextVk, mStagingBuffer.allocateForCopyBuffer(
                                      contextVk, static_cast<size_t>(size), coherency));
    *mapPtr                = mStagingBuffer.getMappedMemory();
    mIsStagingBufferMapped = true;
                                            ...
                                            BufferUsageType usageType)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, mBuffer.initSuballocation(contextVk, mMemoryTypeIndex, size,
                                                            alignment, usageType));
