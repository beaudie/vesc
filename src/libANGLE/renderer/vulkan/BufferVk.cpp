...
namespace
{
// Vertex attribute buffers are used as storage buffers for conversion in compute, where access to the buffer is made
// in 4-byte chunks.  Assume the size of the buffer is 4k+n where n is in [0, 3).  On some hardware, reading 4 bytes from
// address 4k returns 0, making it impossible to read the last n bytes.  By rounding up the buffer sizes to a multiple of 4,
