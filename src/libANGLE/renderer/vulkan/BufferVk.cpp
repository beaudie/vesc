                                    ...
                                    GLsizeiptr size)
{
    ...
    if (mShadowBuffer.valid())
    {
        ANGLE_TRY(sourceBuffer.waitForIdle(
            contextVk,
            "GPU bubble due to copy from buffer in use by the GPU to a pixel unpack buffer"));
    ...
    if (!mShadowBuffer.valid())
    {
        ...
        if ((access & GL_MAP_UNSYNCHRONIZED_BIT) == 0)
        {
            ANGLE_TRY(mBuffer->waitForIdle(contextVk,
                                           "GPU bubble due to mapping buffer in use by the GPU"));
        }
                                      ...
                                      gl::IndexRange *outRange)
{
    ...
    if (!mShadowBuffer.valid())
    {
        DEBUG_PERF_WARNING(contextVk->getState().getDebug(), GL_DEBUG_SEVERITY_HIGH,
                           "GPU bubble due to index range validation");
