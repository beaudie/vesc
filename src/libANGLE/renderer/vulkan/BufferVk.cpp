                                           ...
                                           uint8_t **mapPtr)
{
    ...
    ANGLE_TRY(mStagingBuffer.initStagingBuffer(contextVk, static_cast<size_t>(size), coherent));
                                           ...
                                           VkDeviceSize size)
{
    ...
    // We keep mStagingBuffer allocated but mark staging buffer is the one been mapped to user. Next
    // time when map get called we may have a chance to reuse the existing storage if size is big
    // enough.
    mIsStagingBufferMapped = false;
...
angle::Result BufferVk::handleDeviceLocalBufferMap(ContextVk *contextVk,
                                                   ...
                                                   uint8_t **mapPtr)
{
    ANGLE_TRY(allocStagingBuffer(contextVk, true, size, mapPtr));
                                     ...
                                     void **mapPtr)
{
    ...
    {
        ANGLE_TRY(allocStagingBuffer(contextVk, false, static_cast<size_t>(length), mapPtrBytes));
        return angle::Result::Continue;
    }
                                     ...
                                     size_t offset)
{
    ...
    uint8_t *mapPointer = nullptr;
    ANGLE_TRY(allocStagingBuffer(contextVk, false, size, &mapPointer));
    memcpy(mapPointer, data, size);
