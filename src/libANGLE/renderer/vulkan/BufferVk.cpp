                            ...
                            GLenum usage)
{
    ...
    // In experiementation, the requirements size is equal to the required size. This might
    // not always be true for all platforms, so validate this with an ASSERT and error check.
    ASSERT(memoryRequirements.size == size);
    ANGLE_VK_CHECK(memoryRequirements.size == size, VK_ERROR_INITIALIZATION_FAILED);
