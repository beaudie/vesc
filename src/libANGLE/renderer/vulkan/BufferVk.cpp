                                ...
                                gl::BufferUsage usage)
{
    ...
    if (size > static_cast<size_t>(mState.getSize()))
    {
        ...
        {
            const VkDeviceSize maxVertexAttribStride = std::min(
                static_cast<uint32_t>(gl::MAX_VERTEX_ATTRIB_STRIDE),
                rendererVk->getPhysicalDeviceProperties().limits.maxVertexInputBindingStride);
            createInfo.size = roundUp(createInfo.size, maxVertexAttribStride);
        }
