...
void BufferVk::release(ContextVk *contextVk)
{
    ...
    // external buffer dosen't own mBufferPool.
    if (mBuffer != nullptr && mBuffer->isExternalBuffer() == true)
    {
                                              ...
                                              GLbitfield flags)
{
    ...
    switch (usage)
    {
        case gl::BufferUsage::InvalidEnum:
        {
            ...
            const bool externalBuffer = clientBuffer != nullptr;
            memoryPropertyFlags       = GetStorageMemoryType(flags, externalBuffer);
    ...
    {
        return setExternalBufferData(context, target, clientBuffer, data, size,
                                     memoryPropertyFlags);
    }
...
angle::Result BufferVk::setDataImpl(ContextVk *contextVk,
                                    ...
                                    size_t offset)
{
    // This method should not be called in an ExternalBuffer
    ASSERT(mBuffer->isExternalBuffer() == false);
