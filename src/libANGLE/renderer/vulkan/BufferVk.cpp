                                ...
                                gl::BufferUsage usage)
{
    ...
    if (size > 0 && size != static_cast<size_t>(mState.getSize()))
    {
        ...
        // mBuffer will be allocated through a DynamicBuffer without suballocation
        constexpr bool kDisableDynamicBufferSubAllocation = true;

        mBufferPool.initWithFlags(contextVk->getRenderer(), usageFlags, 1, size,
                                  memoryPropertyFlags, kDisableDynamicBufferSubAllocation);
