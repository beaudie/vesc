...
bool ShouldAllocateNewMemoryForUpdate(ContextVk *contextVk, size_t subDataSize, size_t bufferSize)
{
    ...
    {
        // If buffer is small enough, the cost of barrier associated with GPU copy likely exceeds
        // overhead with CPU copy. Duplicating the buffer allows the CPU to write to the buffer
        // immediately, thus avoiding the barrier.
