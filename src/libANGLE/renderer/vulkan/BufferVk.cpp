...
bool ShouldAllocateNewMemoryForUpdate(ContextVk *contextVk, size_t subDataSize, size_t bufferSize)
{
    ...
    // If the buffer is already in use by the GPU, it is possible to use the CPU instead, but since
    // it would need to create a duplicate of the buffer, a large enough buffer copy could result in
