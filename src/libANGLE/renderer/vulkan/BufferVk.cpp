    ...
    if (!mShadowBuffer.valid())
    {
            ...
            !mBuffer->isExternalBuffer() && mBuffer->isHostVisible())
        {
            ...
            else if (!mBuffer->isCurrentlyInUseForWrite(contextVk->getLastCompletedQueueSerial()))
            {
                ...
                // Before returning the new buffer, map the previous buffer and copy it's entire
                ...
                // If the caller-specified offset is 0, then we can just use the already mapped
                // value for the new buffer that was used for the memcpy() and return immediately.
                if (offset == 0)
                {
                    return angle::Result::Continue;
                }

                // Unmap the new buffer so it can be re-mapped with the caller-specified offset.
