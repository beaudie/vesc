...
class DispatchUtilsVk : angle::NonCopyable
{
  public:
    ...
    angle::Result clearBuffer(vk::Context *context, const BufferVk &dest,
                              const ClearParameters &params,
                              vk::CommandBuffer *commandBuffer);
    angle::Result copyBuffer(vk::Context *context, const BufferVk &dest,
                             const BufferVk &src,
                             const CopyParameters &params,
                             vk::CommandBuffer *commandBuffer);
    angle::Result clearTexture(vk::Context *context, const TextureVk &dest,
                               const ClearParameters &params,
                               vk::CommandBuffer *commandBuffer);
    angle::Result copyTexture(vk::Context *context, const TextureVk &dest,
                              const TextureVk &src,
                              const CopyParameters &params,
                              vk::CommandBuffer *commandBuffer);
  ...
  private:
    ...
    static_assert(sizeof(Params) == 17 * sizeof(uint32_t), "Unexpected padding");

    // Common function that creates the pipeline for the specified function, binds it and prepares the dispatch call.
    // The possible values of `function` comes from vk::InternalShader::DispatchUtils_comp defined in
