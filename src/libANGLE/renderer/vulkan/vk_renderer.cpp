                                     ...
                                     const size_t maxTotalSize)
{
    ...
    const bool cacheEvictsOldItems = globalOps->isBlobCacheEvictsOldItemsFirst();

    // If blob cache evicts only necessary old items first, then freshly stored chunks must remain
    // in the cache in case of eviction (assuming cache is not full).
    if (cacheEvictsOldItems && globalOps->isBlobCacheEvictsOnlyNecessaryItems())
    {
    ...
    } while (!cacheEvictsOldItems && numStoredChunks > 0);
}
                            ...
                            angle::NativeWindowSystem nativeWindowSystem)
{
    ...
    ANGLE_FEATURE_CONDITION(&mFeatures, hasBlobCacheWithZeroSizedValuesSupport, false);

    // Disable by default, because currently it is uncommon that blob cache evicts old items first.
    ANGLE_FEATURE_CONDITION(&mFeatures, hasBlobCacheThatEvictsOldItemsFirst, false);

    // Disable by default, because currently it is uncommon that blob cache evicts ony necessary
    // items.
    ANGLE_FEATURE_CONDITION(&mFeatures, hasBlobCacheThatEvictsOnlyNecessaryItems, false);
