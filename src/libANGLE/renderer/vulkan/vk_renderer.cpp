...
angle::Result Renderer::initializeImageMemoryAllocator(vk::Context *context)
{
    ...
    constexpr VkDeviceSize expectedTestImageSize =
        testImageExtents.width * testImageExtents.height * testImageExtents.depth * kRGBA8PixelSize;
    ...
    // The VMA image block size is fine-tuned based on the memory requirement of the test image.
    ...
    // Initialize the VMA image allocator based on the new preferred image block size.
    ANGLE_VK_TRY(context,
...
{
    // Output the VMA stats strings for buffer and image allocations.
    // These JSON strings can be passed to VmaDumpVis.py to generate a visualization of the
    // allocations the VMA has performed.
    char *bufferStatsString;
    char *imageStatsString;
    mBufferAllocator.buildStatsString(&bufferStatsString, true);
    mImageAllocator.buildStatsString(&imageStatsString, true);

    INFO() << std::endl
           << "VMA Buffer Allocation Stats:" << std::endl
           << bufferStatsString << std::endl
           << "VMA Image Allocation Stats:" << std::endl
           << imageStatsString << std::endl;

    mBufferAllocator.freeStatsString(bufferStatsString);
    mImageAllocator.freeStatsString(imageStatsString);
}
