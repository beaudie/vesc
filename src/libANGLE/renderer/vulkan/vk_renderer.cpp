                                     ...
                                     const size_t maxTotalSize)
{
    ...
    const uint32_t slotIndex = renderer->getNextPipelineCacheBlobCacheSlotIndex(&previousSlotIndex);
                                ...
                                angle::MemoryBuffer *scratchBuffer)
{
    ...
    keyData.setSize(renderer->getFeatures().hasBlobCacheWithZeroSizedValuesSupport.enabled ? 0 : 1);
                                              ...
                                              bool *success)
{
    ...
    *success = false;
    ...
    const uint32_t firstSlotIndex = renderer->getNextPipelineCacheBlobCacheSlotIndex(nullptr);
    uint32_t slotIndex            = firstSlotIndex;
                                   ...
                                   const angle::FeatureOverrides &featureOverrides)
{
    ...
    ANGLE_TRY(setupDevice(context, clientAPI, featureOverrides, wsiLayer, useVulkanSwapchain,
                          nativeWindowSystem));
                                               ...
                                               angle::NativeWindowSystem nativeWindowSystem)
{
    ...
    initFeatures(deviceExtensionNames, clientAPI, featureOverrides, useVulkanSwapchain,
                 nativeWindowSystem);
                                    ...
                                    angle::NativeWindowSystem nativeWindowSystem)
{
    ...
    ANGLE_TRY(enableDeviceExtensions(context, clientAPI, featureOverrides, useVulkanSwapchain,
                                     nativeWindowSystem));
                            ...
                            angle::NativeWindowSystem nativeWindowSystem)
{
    ...
    // OpenCL uses |SizedMRUCache| and does not require dual slots.
    ANGLE_FEATURE_CONDITION(&mFeatures, useDualPipelineBlobCacheSlots,
                            clientAPI != ClientAPI::OpenCL && isSamsung);

    // Disable by default, because currently it is uncommon that blob cache supports storing
    // zero sized blobs (or erasing blobs).
    // OpenCL uses |SizedMRUCache| and supports zero sized values.
    ANGLE_FEATURE_CONDITION(&mFeatures, hasBlobCacheWithZeroSizedValuesSupport,
                            clientAPI == ClientAPI::OpenCL);
...
uint32_t Renderer::getNextPipelineCacheBlobCacheSlotIndex(uint32_t *previousSlotIndexOut)
{
