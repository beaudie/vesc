...
uintptr_t MakeQueryResourceId(const QueryPool *queryPool, uint32_t queryIndex)
{
    // The default size of query pools is 64, hence the shift by 6.  Used only for debugging the
    // graph.  If there are more than 64 queries and query pool addresses only differ in the top 6
    // bits, the only issue would be queries sharing the same name in the generated graph.
    return reinterpret_cast<uintptr_t>(queryPool) << 6 | queryIndex;
}
...
void CommandGraph::beginQuery(const QueryPool *queryPool, uint32_t queryIndex)
{
    CommandGraphNode *newNode =
        allocateBarrierNode(CommandGraphNodeFunction::BeginQuery, CommandGraphResourceType::Query,
                            MakeQueryResourceId(queryPool, queryIndex));
...
{
    CommandGraphNode *newNode =
        allocateBarrierNode(CommandGraphNodeFunction::EndQuery, CommandGraphResourceType::Query,
                            MakeQueryResourceId(queryPool, queryIndex));
    newNode->setQueryPool(queryPool, queryIndex);
}
...
void CommandGraph::writeTimestamp(const QueryPool *queryPool, uint32_t queryIndex)
{
    CommandGraphNode *newNode = allocateBarrierNode(CommandGraphNodeFunction::WriteTimestamp,
                                                    CommandGraphResourceType::Query,
                                                    MakeQueryResourceId(queryPool, queryIndex));
    newNode->setQueryPool(queryPool, queryIndex);
}
