...
class CompressTask : public angle::Closure
{
  public:
    ...
    {
        angle::Result mResult = CompressAndStorePipelineCacheVk(mPhysicalDeviceProperties,
                                                                mDisplayVk, mContextVk, mCacheData);

        if (mResult != angle::Result::Continue)
        {
            WARN() << "Failed to compress pipeline cache data.";
        }
    }
