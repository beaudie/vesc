...
{
    *anyBatchCleaned = false;
    if (!mCommandQueue.isInFlightCommandsEmpty())
    {
        ASSERT(mCommandQueue.finishOneCommandBatchAndCleanup(context, getMaxFenceWaitTimeNs()) ==
               angle::Result::Continue);
        INFO() << "Retrying allocation after attempting to free the garbage.";
        *anyBatchCleaned = true;
    }
}
