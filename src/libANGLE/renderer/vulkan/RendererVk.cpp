...
{
    bool canSupportFragmentShadingRate = false;

    // Device needs to support VK_KHR_fragment_shading_rate and specifically
    // pipeline fragment shading rate.
    if (ExtensionFound(VK_KHR_FRAGMENT_SHADING_RATE_EXTENSION_NAME, deviceExtensionNames) &&
        mFragmentShadingRateFeatures.pipelineFragmentShadingRate == VK_TRUE)
    {
        // Init required functions
#if !defined(ANGLE_SHARED_LIBVULKAN)
        InitFragmentShadingRateKHRFunctions(mPhysicalDevice);
#endif  // !defined(ANGLE_SHARED_LIBVULKAN)
        ...
        mSupportedFragmentShadingRates.fill(false);
        for (const VkPhysicalDeviceFragmentShadingRateKHR &shadingRate : shadingRates)
        {
            ...
            mSupportedFragmentShadingRates[GetShadingRateFromVkExtent(shadingRate.fragmentSize)] =
                true;
        }

        // To implement GL_QCOM_shading_rate extension the ICD needs to at least support the
        // following shading rates -
        //     {1, 1}
        //     {1, 2}
        //     {2, 1}
        //     {2, 2}
        if (mSupportedFragmentShadingRates[gl::ShadingRate::_1x1] &&
            mSupportedFragmentShadingRates[gl::ShadingRate::_1x2] &&
            mSupportedFragmentShadingRates[gl::ShadingRate::_2x1] &&
            mSupportedFragmentShadingRates[gl::ShadingRate::_2x2])
        {
            canSupportFragmentShadingRate = true;
        }
    }

    return canSupportFragmentShadingRate;
}
