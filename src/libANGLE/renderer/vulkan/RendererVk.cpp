...
class CacheDataHeader
{
  public:
    ...
    {
        *chunkIndexOut        = mChunkIndex;
        *numChunksOut         = mNumChunks;
        *compressedDataCRCOut = mCompressedDataCRC;
        *cacheDataSizeOut     = mCacheDataSize;
    }

    static constexpr size_t size() { return kBlobHeaderSize; }
  ...
  private:
    ...
    static constexpr size_t kBlobHeaderSize = 10 * sizeof(uint8_t);
                                     ...
                                     const size_t maxTotalSize)
{
    ...
    const size_t numChunks = UnsignedCeilDivide(static_cast<unsigned int>(compressedData.size()),
                                                kMaxBlobCacheSize - CacheDataHeader::size());
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        if (!keyData.resize(CacheDataHeader::size() + chunkSize))
        {
            ANGLE_PERF_WARNING(contextVk->getDebug(), GL_DEBUG_SEVERITY_LOW,
        ...
        memcpy(keyData.data(), &headerData, CacheDataHeader::size());
        memcpy(keyData.data() + CacheDataHeader::size(), compressedData.data() + compressedOffset,
               chunkSize);
                                              ...
                                              bool *success)
{
    ...
    *success = false;
    ...
    if (!displayVk->getBlobCache()->get(displayVk->getScratchBuffer(), chunkCacheHash, &keyData,
                                        &keySize) ||
        keyData.size() < CacheDataHeader::size())
    {
    ...
    CacheDataHeader headerData = {};
    memcpy(&headerData, keyData.data(), CacheDataHeader::size());
    ...
    size_t chunkSize      = keySize - CacheDataHeader::size();
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        chunkSize = keySize - CacheDataHeader::size();
        ...
        memcpy(compressedData.data() + compressedSize, keyData.data() + CacheDataHeader::size(),
               chunkSize);
