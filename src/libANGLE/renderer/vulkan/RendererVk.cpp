...
void RendererVk::queryDeviceExtensionFeatures(const ExtensionNameList &deviceExtensionNames)
{
    ...
    {
        vk::AppendToPNextChain(&deviceFeatures, &mLineRasterizationFeatures);
    }

    // Query provoking vertex features
    if (ExtensionFound(VK_EXT_PROVOKING_VERTEX_EXTENSION_NAME, deviceExtensionNames))
    {
        vk::AppendToPNextChain(&deviceFeatures, &mProvokingVertexFeatures);
    }
    ...
    {
        vk::AppendToPNextChain(&deviceFeatures, &mVertexAttributeDivisorFeatures);
        vk::AppendToPNextChain(&deviceProperties, &mVertexAttributeDivisorProperties);
    }

    // Query transform feedback features
    if (ExtensionFound(VK_EXT_TRANSFORM_FEEDBACK_EXTENSION_NAME, deviceExtensionNames))
    {
        vk::AppendToPNextChain(&deviceFeatures, &mTransformFeedbackFeatures);
    }
...
angle::Result RendererVk::initializeDevice(DisplayVk *displayVk, uint32_t queueFamilyIndex)
{
    ...
    if (mVertexAttributeDivisorFeatures.vertexAttributeInstanceRateDivisor)
    {
        ...
        vk::AppendToPNextChain(&createInfo, &mVertexAttributeDivisorFeatures);
    ...
    if (getFeatures().supportsTransformFeedbackExtension.enabled)
    {
        ...
        vk::AppendToPNextChain(&createInfo, &mTransformFeedbackFeatures);
    }
...
void RendererVk::initFeatures(const ExtensionNameList &deviceExtensionNames)
{
    ...
    ANGLE_FEATURE_CONDITION((&mFeatures), supportsTransformFeedbackExtension,
                            mTransformFeedbackFeatures.transformFeedback);
