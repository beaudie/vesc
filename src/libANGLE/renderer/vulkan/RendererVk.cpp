...
constexpr uint32_t kCompressionBit = 31;
...
void PackHeaderDataForPipelineCache(uint32_t cacheDataSize,
                                    ...
                                    uint64_t *dataOut)
{
    uint64_t concatenatedData = (compressed ? 0x1u : 0x0u) << kCompressionBit | cacheDataSize;
                                      ...
                                      bool *compressed)
{
    ...
    *compressedDataCRCOut = data & 0xFFFF;
    data >>= 16;
    *cacheDataSizeOut = static_cast<uint32_t>(data & ((0x1u << kCompressionBit) - 1));
    data >>= kCompressionBit;
                                     ...
                                     const size_t maxTotalSize)
{
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        ASSERT(numChunks <= UINT8_MAX && chunkIndex <= UINT8_MAX &&
               cacheData.size() < (0x1u << kCompressionBit));
