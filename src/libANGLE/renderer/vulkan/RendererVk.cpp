...
constexpr bool kTrackMemoryAllocation = true;
...
static_assert(kTrackMemoryAllocation,
              "kTrackMemoryAllocation must be enabled to use kDebugMemoryAllocationLogs.");
#else
    ...
    bool canLoadDebugUtils)
{
    ...
    // core in Vulkan 1.1
    ANGLE_FEATURE_CONDITION(&mFeatures, supportsExternalFenceCapabilities,
                            IsVulkan11(mInstanceVersion) ||
                                ExtensionFound(VK_KHR_EXTERNAL_FENCE_CAPABILITIES_EXTENSION_NAME,
                                               instanceExtensionNames));
                                     ...
                                     const char *wsiLayer)
{
    ...
    uint32_t apiVersion = mInstanceVersion = VK_API_VERSION_1_0;
    if (enumerateInstanceVersion)
    {
        ...
        if (IsVulkan11(mInstanceVersion))
        {
            ...
            apiVersion = kPreferredVulkanAPIVersion;
    ...
    mApplicationInfo.apiVersion         = apiVersion;
    ...
    VkPhysicalDeviceProperties2 *deviceProperties)
{
    ...
    if (IsVulkan12(mPhysicalDeviceProperties.apiVersion))
    {
    ...
    if (IsVulkan11(mPhysicalDeviceProperties.apiVersion))
    {
    ...
    VkPhysicalDeviceProperties2 *deviceProperties)
{
    ...
    if (IsVulkan13(mPhysicalDeviceProperties.apiVersion))
    {
    ...
    const vk::ExtensionNameList &deviceExtensionNames)
{
    if (mFeatures.supportsExtendedDynamicState2.enabled &&
        ExtensionFound(VK_EXT_EXTENDED_DYNAMIC_STATE_2_EXTENSION_NAME, deviceExtensionNames))
    {
    ...
    // If VK_EXT_extended_dynamic_state2 is supported, always enable it directly due to the
    // extendedDynamicState2LogicOp feature that is not promoted.
    //
    if (IsVulkan13(mPhysicalDeviceProperties.apiVersion))
    {
...
{
    if (mPhysicalDeviceProperties.apiVersion == 0)
    {
        return mInstanceVersion;
    }
    return mPhysicalDeviceProperties.apiVersion;
}
                              ...
                              const vk::ExtensionNameList &deviceExtensionNames)
{
    ...
    bool supportsNegativeViewport =
        IsVulkan11(mPhysicalDeviceProperties.apiVersion) ||
    ...
    ANGLE_FEATURE_CONDITION(
        ...
        ExtensionFound(VK_EXT_PIPELINE_CREATION_FEEDBACK_EXTENSION_NAME, deviceExtensionNames) ||
            IsVulkan13(mPhysicalDeviceProperties.apiVersion));
    ...
    ANGLE_FEATURE_CONDITION(
        ...
        IsVulkan12(mPhysicalDeviceProperties.apiVersion) ||
