...
MemoryReportCallback(const VkDeviceMemoryReportCallbackDataEXT *callbackData, void *userData)
{
    ...
    {
        VERBOSE("\t Avoided double-counting this object with non-unique id= %" PRIu64,
                              ...
                              const vk::ExtensionNameList &deviceExtensionNames)
{
    ...
    // FIXME/TODO(ianelliott): Put this in the correct spot:
    ANGLE_FEATURE_CONDITION(&mFeatures, logMemoryReportCallbacks, true);
    ANGLE_FEATURE_CONDITION(&mFeatures, logMemoryReportStats, true);
...
void RendererVk::logMemoryReportStats()
{
    ...
    {
        VERBOSE("- type= %-15s:  imported=%9" PRIu64 " (max=%9" PRIu64 "); ",
                GetVkObjectTypeName(it.first), it.second, mImportedMemoryMax[it.first]);
    }
}
}  // namespace rx
