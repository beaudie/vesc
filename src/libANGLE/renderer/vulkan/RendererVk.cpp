...
void RendererVk::getSubmitWaitSemaphores(vk::Context *context,
...
angle::Result RendererVk::waitBatchCompletion(vk::Context *context, Serial serial)
{
    ...
    auto found = std::lower_bound(mInFlightCommands.begin(), mInFlightCommands.end(), serial,
            [](const CommandBatch &batch, Serial serial){
                return batch.serial < serial;
            });
    size_t batchIndex = found == mInFlightCommands.end() ? mInFlightCommands.size() - 1 :
