...
{
    {
        ...
        ANGLE_VK_TRY(context, vkQueueSubmit(getQueue(priority), 1, &submitInfo, fence.getHandle()));
    }
...
angle::Result RendererVk::queueWaitIdle(vk::Context *context, egl::ContextPriority priority)
{
    {
        std::lock_guard<decltype(mQueueMutex)> lock(mQueueMutex);
        ANGLE_VK_TRY(context, vkQueueWaitIdle(getQueue(priority)));
    }
                                  ...
                                  const VkPresentInfoKHR &presentInfo)
{
...

    {
        ...
        return vkQueuePresentKHR(getQueue(priority), &presentInfo);
    }
}
