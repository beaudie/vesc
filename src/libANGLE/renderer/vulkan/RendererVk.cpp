...
vk::Error RendererVk::initialize(const egl::AttributeMap &attribs, const char *wsiName)
{
    ...
    if (mEnableValidationLayers)
    {
        ...
        mEnableValidationLayers = GetValidationLayers(instanceLayerProps, layersRequested,
...
vk::Error RendererVk::initializeDevice(uint32_t queueFamilyIndex)
{
    ...
    if (mEnableValidationLayers)
    {
        mEnableValidationLayers =
            GetValidationLayers(deviceLayerProps, false, &ppEnabledLayerNames, &enabledLayerCount);
    }
