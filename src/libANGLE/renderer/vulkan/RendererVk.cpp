...
void RendererVk::onDestroy(vk::Context *context)
{
    ...
    {
        ASSERT(mInstance && mExtensionFunctions.destroyDebugUtilsMessengerEXT);
        mExtensionFunctions.destroyDebugUtilsMessengerEXT(mInstance, mDebugUtilsMessenger, nullptr);

        ASSERT(mDebugReportCallback == VK_NULL_HANDLE);
    }
    else if (mDebugReportCallback)
    {
        ASSERT(mInstance && mExtensionFunctions.destroyDebugReportCallbackEXT);
        mExtensionFunctions.destroyDebugReportCallbackEXT(mInstance, mDebugReportCallback, nullptr);
    }
                                     ...
                                     const char *wsiName)
{
    ...
    if (enableDebugUtils)
    {
        ...
        mExtensionFunctions.initDebugUtils(mInstance);
        ...
        ANGLE_VK_TRY(displayVk, mExtensionFunctions.createDebugUtilsMessengerEXT(
                                    mInstance, &messengerInfo, nullptr, &mDebugUtilsMessenger));
    }
    else if (enableDebugReport)
    {
        // Fallback to EXT_debug_report.
        mExtensionFunctions.initDebugReport(mInstance);
        ...
        ANGLE_VK_TRY(displayVk, mExtensionFunctions.createDebugReportCallbackEXT(
                                    mInstance, &debugReportInfo, nullptr, &mDebugReportCallback));
    }
