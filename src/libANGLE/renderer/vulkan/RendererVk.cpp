...
// General CRC8 is used for header before the cache key data.
uint8_t ComputeCRC8(const uint8_t *data, const size_t size)
{
    constexpr uint8_t kPolynomialCRC8 = 0xAB;
    uint32_t rem                      = 0;
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            rem = (rem & 1) ? kPolynomialCRC8 ^ (rem >> 1) : rem >> 1;
        }
    }
    return rem;
}
                                     ...
                                     const size_t maxTotalSize)
{
    ...
    // Store {originalCacheSize, numChunks, chunkIndex, compressedDataCRC; chunkCompressedData} in
    ...
    // divided into two chunks: {70000,2,0,crc0;34421 bytes} and {70000,2,1,crc1;34420 bytes}.
    constexpr size_t kBlobHeaderSize = 3 * sizeof(uint8_t) + sizeof(size_t);
    ...
    uint8_t compressedDataCRC = ComputeCRC8(compressedData.data(), chunkSize);
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        *(keyData.data() + sizeof(size_t))                       = static_cast<uint8_t>(numChunks);
        *(keyData.data() + sizeof(size_t) + sizeof(uint8_t))     = static_cast<uint8_t>(chunkIndex);
        *(keyData.data() + sizeof(size_t) + 2 * sizeof(uint8_t)) = compressedDataCRC;
