...
angle::Result WaitFences(vk::Context *context,
                         ...
                         bool wait)
{
    uint64_t timeout = wait ? kMaxFenceWaitTimeNs : 0;
...
angle::Result RendererVk::cleanupFencedGarbage(vk::Context *context, bool wait)
{
    auto garbageIter = mFencedGarbage.begin();
    while (garbageIter != mFencedGarbage.end())
    {
        ANGLE_TRY(WaitFences(context, &garbageIter->first, wait));
