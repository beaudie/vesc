...
StagingImage::StagingImage()
    : mDevice(VK_NULL_HANDLE), mHandle(VK_NULL_HANDLE), mMemory(VK_NULL_HANDLE), mSize(0)
{
}

StagingImage::StagingImage(StagingImage &&other)
    : mDevice(other.mDevice), mHandle(other.mHandle), mMemory(other.mMemory), mSize(other.mSize)
{
                             ...
                             const gl::Extents &extent)
{
    ...
    ANGLE_VK_TRY(vkAllocateMemory(device, &allocateInfo, nullptr, &mMemory));
    ANGLE_VK_TRY(vkBindImageMemory(device, mHandle, mMemory, 0));
...
VkResult VerifyExtensionNameList(const std::vector<VkExtensionProperties> &extensionProps,
...
vk::Error RendererVk::initialize(const egl::AttributeMap &attribs)
{
    ...
    if (mEnableValidationLayers)
    {
        ...
        {
            ANGLEPlatformCurrent()->logWarning("Vulkan standard validation layers are missing.");
            mEnableValidationLayers = false;
        }
    }
...
#error Unsupported Vulkan platform.
    ...
    ANGLE_VK_TRY(VerifyExtensionNameList(instanceExtensionProps, enabledInstanceExtensions));
...
vk::Error RendererVk::initializeDevice(uint32_t queueFamilyIndex)
{
    ...
    ANGLE_VK_TRY(VerifyExtensionNameList(deviceExtensionProps, enabledDeviceExtensions));
...
vk::ErrorOrResult<StagingImage> RendererVk::createStagingImage(TextureDimension dimension,
