...
bool TestExtensionPresent(const std::vector<VkExtensionProperties> &extensionProps,
...
bool IsIgnoredDebugMessage(const char *expectedPrefix,
                           const char *message,
                           const char *expectedSuffix)
{
    ...
    {
        if (strncmp(expectedPrefix, message, expectedPrefixLen) != 0)
        {
            continue;
        }
        ...
        if (strncmp(msg, message + expectedPrefixLen, msgLen) != 0)
        {
            continue;
        }

        if (strncmp(expectedSuffix, message + expectedPrefixLen + msgLen, expectedSuffixLen) == 0)
        {
            return true;
        }
    }
                    ...
                    void *userData)
{
    ...
    // Switch to this if you want Vulkan spam;
    /*constexpr VkDebugUtilsMessageSeverityFlagBitsEXT kSeveritiesToLog =
        VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT |
        VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT |
...
#if !defined(NDEBUG)
        // Abort the call in Debug builds if error.
        return isError;
#endif
    }
...
void RendererVk::onDestroy(vk::Context *context)
{
    ...
    if (mDebugUtilsMessenger)
    {
        ...
        destroyDebugUtilsMessenger(mInstance, mDebugUtilsMessenger, nullptr);
                                     ...
                                     const char *wsiName)
{
    ...
    bool enableDebugUtils =
        ...
        TestExtensionPresent(instanceExtensionProps, VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
    bool enableDebugReport =
        mEnableValidationLayers && !enableDebugUtils &&
        TestExtensionPresent(instanceExtensionProps, VK_EXT_DEBUG_REPORT_EXTENSION_NAME);
...
void RendererVk::insertDebugMarker(GLenum source, GLuint id, const char *marker)
{
    mCommandGraph.insertDebugMarker(source, marker);
}

void RendererVk::pushDebugMarker(GLenum source, GLuint id, const char *marker)
{
    mCommandGraph.pushDebugMarker(source, marker);
}
