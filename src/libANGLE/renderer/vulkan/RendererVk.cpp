...
void RendererVk::initPipelineCacheVk(DisplayVk *display)
{
    ...
    display->getBlobCache()->get(&scratchBuffer, mPipelineCacheVkBlobKey, &initialData);
...
void RendererVk::syncPipelineCacheVk(DisplayVk *displayVk)
{
    if (!mPipelineCacheVk.valid())
    {
        return;
    }
    ...
    ANGLE_TRY_VOID_RETURN(mPipelineCacheVk.getCacheData(displayVk, &pipelineCacheSize, nullptr));

    angle::MemoryBuffer pipelineCacheData(pipelineCacheSize);
    ANGLE_TRY_VOID_RETURN(
        mPipelineCacheVk.getCacheData(displayVk, &pipelineCacheSize, pipelineCacheData.data()));

    displayVk->getBlobCache()->put(mPipelineCacheVkBlobKey, std::move(pipelineCacheData), false);
}
