...
angle::Result RendererVk::initializeDevice(DisplayVk *displayVk, uint32_t queueFamilyIndex)
{
    ...
    initPipelineCache(displayVk, &mPipelineCache);
...
void RendererVk::initPipelineCache(DisplayVk *display, vk::PipelineCache *pipelineCache)
{
    ...
    pipelineCache->init(mDevice, pipelineCacheCreateInfo);
}

void RendererVk::onGetPipelineCache(DisplayVk *display)
{
    // We should now recreate the pipeline cache with the blob cache pipeline data.
    vk::PipelineCache pipelineCache;
    initPipelineCache(display, &pipelineCache);
...
const vk::PipelineCache &RendererVk::getPipelineCache()
{
    if (!mPipelineCacheInitialized)
    {
        onGetPipelineCache(vk::GetImpl(mDisplay));
