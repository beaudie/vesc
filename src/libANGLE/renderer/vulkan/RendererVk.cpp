...
void RendererVk::onDestroy(vk::Context *context)
{
    ...
    vk::QueueSerials infinitQueueSerials;
    updateQueueSerialWithInfinite(infinitQueueSerials);
    cleanupGarbage(infinitQueueSerials);
    ASSERT(!hasSharedGarbage());
...
angle::Result RendererVk::queueSubmitOneOff(vk::Context *context,
                                            ...
                                            const vk::SharedFence *sharedFence,
                                            vk::SubmitPolicy submitPolicy)
{
    ...
    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.queueSubmitOneOff(
            ...
            waitSemaphoreStageMasks, &sharedFence->get(), submitPolicy));
    }
    else
    {
    ...
    if (primary.valid())
    {
        mPendingOneOffCommands.push_back({sharedFence, std::move(primary)});
    }
...
void RendererVk::cleanupGarbage(const vk::QueueSerials &lastCompletedQueueSerials)
{
    std::unique_lock<std::mutex> lock(mGarbageMutex);
    ...
    while (!mSharedGarbage.empty())
    {
        ...
        if (!garbage.destroyIfComplete(this, lastCompletedQueueSerials))
        {
            break;
        }
        mSharedGarbage.pop();
...
{
    cleanupGarbage(getLastCompletedQueueSerials());
}
                                                 ...
                                                 vk::PrimaryCommandBuffer *commandBufferOut)
{
    ...
    if (!mPendingOneOffCommands.empty() &&
        mPendingOneOffCommands.front().serial < getLastCompletedQueueSerials())
    {
...
angle::Result RendererVk::finishToSerials(vk::Context *context,
                                          const vk::QueueSerials &queueSerials)
{
    ...
    {
        ANGLE_TRY(
            mCommandProcessor.finishToSerials(context, queueSerials, getMaxFenceWaitTimeNs()));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.finishToSerials(context, queueSerials, getMaxFenceWaitTimeNs()));
    }

    return angle::Result::Continue;
}

angle::Result RendererVk::waitForQueueSerialsWithUserTimeout(vk::Context *context,
                                                             const vk::QueueSerials &serials,
                                                             uint64_t timeout,
                                                             VkResult *result)
{
    ANGLE_TRACE_EVENT0("gpu.angle", "RendererVk::waitForQueueSerialsWithUserTimeout");

    std::unique_lock<std::mutex> lock(mCommandQueueMutex);

    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.waitForQueueSerialsWithUserTimeout(context, serials, timeout,
                                                                       result));
    }
    else
    {
        ANGLE_TRY(
            mCommandQueue.waitForQueueSerialsWithUserTimeout(context, serials, timeout, result));
    }
...
void RendererVk::updateQueueSerialWithInfinite(vk::QueueSerials &queueSerials)
{
    for (vk::QueueSerialIndex queueSerialIndex = 0;
         queueSerialIndex < getLastSubmittedQueueSerials().size(); queueSerialIndex++)
    {
        queueSerials[queueSerialIndex] = Serial::Infinite();
    }
}

angle::Result RendererVk::newSharedFence(vk::Context *context, vk::SharedFence *sharedFenceOut)
{
    return mCommandQueue.newSharedFence(context, sharedFenceOut);
}
