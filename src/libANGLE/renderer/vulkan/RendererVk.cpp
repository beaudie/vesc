...
angle::Result RendererVk::initializeDevice(DisplayVk *displayVk, uint32_t graphicsQueueFamilyIndex)
{
...
#    endif  // !defined(ANGLE_SHARED_LIBVULKAN)
    ...
    queueCreateInfo[0].flags =
        graphicsQueueFamily.supportsProtected() ? VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT : 0;
    ...
    {
        mProtectedMemoryFeatures.pNext = nullptr;
        createInfo.pNext               = &mProtectedMemoryFeatures;
    }
    ...
    vk::DeviceQueueMap graphicsQueueMap = graphicsQueueFamily.initializeQueueMap(
        mDevice, graphicsQueueFamily.supportsProtected(), 0, queueCount);
...
angle::Result RendererVk::finishToSerial(vk::Context *context,
                                         bool hasProtectedContent,
                                         Serial serial)
{
    std::lock_guard<std::mutex> lock(mCommandQueueMutex);
    ...
    if (mFeatures.asyncCommandQueue.enabled)
    {
        ANGLE_TRY(mCommandProcessor.finishToSerial(context, hasProtectedContent, serial,
                                                   getMaxFenceWaitTimeNs()));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.finishToSerial(context, hasProtectedContent, serial,
                                               getMaxFenceWaitTimeNs()));
    }
                                                       ...
                                                       VkResult *result)
{
    ...
    if (mFeatures.asyncCommandQueue.enabled)
    {
        ANGLE_TRY(mCommandProcessor.waitForSerialWithUserTimeout(context, hasProtectedContent,
                                                                 serial, timeout, result));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.waitForSerialWithUserTimeout(context, hasProtectedContent, serial,
                                                             timeout, result));
    }
...
{
    return finishToSerial(context, hasProtectedContent, getLastSubmittedQueueSerial());
}

angle::Result RendererVk::checkCompletedCommands(vk::Context *context, bool hasProtectedContent)
{
    std::lock_guard<std::mutex> lock(mCommandQueueMutex);
    ...
    if (mFeatures.asyncCommandQueue.enabled)
    {
        ANGLE_TRY(mCommandProcessor.checkCompletedCommands(context, hasProtectedContent));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.checkCompletedCommands(context, hasProtectedContent));
    }
                                  ...
                                  const VkPresentInfoKHR &presentInfo)
{
    ...
    if (mFeatures.asyncCommandQueue.enabled)
    {
        result = mCommandProcessor.queuePresent(hasProtectedContent, priority, presentInfo);
    }
    else
    {
        result = mCommandQueue.queuePresent(hasProtectedContent, priority, presentInfo);
    }
