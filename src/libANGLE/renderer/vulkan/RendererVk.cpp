                                            ...
                                            QueueSerial *queueSerialOut)
{
    ...
    // most usage cases.
    if (mCurrentOneOffSubmitQueueSerial.index == kInvalidQueueSerialIndex)
    {
        ANGLE_TRY(allocateQueueSerialIndex(&mCurrentOneOffSubmitQueueSerial.index));
    }
    mCurrentOneOffSubmitQueueSerial.serial =
        generateQueueSerial(mCurrentOneOffSubmitQueueSerial.index, 1);
    ...
    *queueSerialOut = mCurrentOneOffSubmitQueueSerial;
    if (primary.valid())
    {
        mPendingOneOffCommands.push_back(
            {vk::ResourceUse(mCurrentOneOffSubmitQueueSerial), std::move(primary)});
    }
...
angle::Result RendererVk::finishQueueSerial(vk::Context *context, const QueueSerial &queueSerial)
{
    ...
    return finishResourceUse(context, use);
}

angle::Result RendererVk::waitForResourceUseToFinishWithTimeout(vk::Context *context,
                                                                ...
                                                                VkResult *result)
{
    ...
    {
        ANGLE_TRY(
            mCommandProcessor.waitForResourceUseToFinishWithTimeout(context, use, timeout, result));
    }
    else
    {
        ANGLE_TRY(
            mCommandQueue.waitForResourceUseToFinishWithTimeout(context, use, timeout, result));
    }
