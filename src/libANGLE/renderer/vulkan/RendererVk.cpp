...
angle::Result RendererVk::checkCompletedCommands(vk::Context *context)
{
    ...
    for (CommandBatch &batch : mInFlightCommands)
    {
        // If the last submission has not gotten through yet, don't try to query its fence.
        if (hasPendingWork(batch.serial))
        {
            break;
        }
                                                  ...
                                                  bool *outTimedOut)
{
    *outTimedOut = false;
    ...
    if (hasPendingWork(batch.serial))
    {
        mFlushThread.waitForPreviousOperation();
    }
...
bool RendererVk::hasPendingWork(Serial serial)
{
    return serial == mCurrentQueueSerial ||
           (serial == mLastSubmittedQueueSerial && mFlushThread.hasPendingWork());
}
