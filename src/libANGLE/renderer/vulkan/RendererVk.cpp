                                              ...
                                              bool *success)
{
    ...
    *success = false;
    ...
    angle::MemoryBuffer *tmpData =
        displayVk->getFeatures().disablePipelinecachedataCompression.enabled ? uncompressedData
                                                                             : &compressedData;
    ANGLE_VK_CHECK(displayVk, tmpData->resize(chunkSize * numChunks),
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        bool isHeaderDataCorrupted =
            ...
            (tmpData->size() < compressedSize + chunkSize);
        if (isHeaderDataCorrupted)
        {
            WARN() << "Pipeline cache chunk header corrupted: "
                   ...
                   << ", compressedData.size() = " << tmpData->size()
                   << ", (compressedSize + chunkSize) = " << (compressedSize + chunkSize);
            return angle::Result::Continue;
        }

        memcpy(tmpData->data() + compressedSize, keyData.data() + kBlobHeaderSize, chunkSize);
    ...
    if (kEnableCRCForPipelineCache)
    {
        uint16_t computedCompressedDataCRC = ComputeCRC16(tmpData->data(), compressedSize);
