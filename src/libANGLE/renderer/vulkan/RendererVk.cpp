...
angle::Result RendererVk::initPipelineCacheVk(DisplayVk *display)
{
    ...
    ANGLE_VK_TRY_RETURN(display, mPipelineCacheVk.init(mDevice, pipelineCacheCreateInfo));
}
                                      ...
                                      vk::CommandBuffer &&commandBuffer)
{
    ...
    ANGLE_VK_TRY_RETURN(context, mCommandPool.init(mDevice, poolInfo));
}
...
angle::Result RendererVk::syncPipelineCacheVk(DisplayVk *displayVk)
{
    ...
    ANGLE_VK_TRY_ALLOW_INCOMPLETE(
        displayVk, mPipelineCacheVk.getCacheData(mDevice, &pipelineCacheSize, nullptr));
    ...
    VkResult result =
        mPipelineCacheVk.getCacheData(mDevice, &pipelineCacheSize, pipelineCacheData->data());
...
angle::Result RendererVk::getTimestamp(vk::Context *context, uint64_t *timestampOut)
{
    ...
    ANGLE_VK_TRY_ALLOW_NOT_READY(
        context, timestampQuery.getQueryPool()->getResults(mDevice, timestampQuery.getQuery(), 1,
...
angle::Result RendererVk::synchronizeCpuGpuTime(vk::Context *context)
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        do
        {
            ...
            ANGLE_VK_TRY_EVENT_STATUS(context, result);
        ...
        ANGLE_VK_TRY_ALLOW_NOT_READY(
            context, timestampQuery.getQueryPool()->getResults(
