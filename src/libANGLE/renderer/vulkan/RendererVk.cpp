...
vk::Error RendererVk::initialize(const egl::AttributeMap &attribs, const char *wsiName)
{
    ...
    if (enableNullDriver)
    {
        ...
        const auto cwd     = angle::GetCWD().value();
        const auto icdPath = cwd + '/' + std::string(ANGLE_VK_ICD_JSON);
        ANGLE_VK_CHECK(angle::SetEnvironmentVar(g_VkICDPathEnv, icdPath.c_str()),
