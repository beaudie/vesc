...
{
    printf("In DisplayVk::terminate(), calling cleanupGarbage()\n");
    (void)cleanupGarbage(context, true);
    printf("In DisplayVk::terminate(), calling ASSERT(mSharedGarbage.empty())\n");
    ASSERT(mSharedGarbage.empty());
    // printf("In DisplayVk::terminate(), calling mFenceRecycler.destroy(), mFenceRecycler:0x%X\n",
    // mFenceRecycler);
    printf("In DisplayVk::terminate(), calling mFenceRecycler.destroy()\n");
    mFenceRecycler.destroy(mDevice);
    printf("In DisplayVk::terminate(), calling mPipelineLayoutCache.destroy()\n");
    mPipelineLayoutCache.destroy(mDevice);
    printf("In DisplayVk::terminate(), calling mDescriptorSetLayoutCache.destroy()\n");
    mDescriptorSetLayoutCache.destroy(mDevice);
    printf("In DisplayVk::terminate(), calling mPipelineCache.destroy()\n");
    mPipelineCache.destroy(mDevice);
    printf("In DisplayVk::terminate(), calling GlslangRelease()\n");
    GlslangRelease();
...
{
    printf("In RendererVk::newSharedFence()\n");
    vk::Fence fence;
    if (mFenceRecycler.empty())
    {
        printf("In RendererVk::newSharedFence(), mFenceRecycler.empty()\n");
    ...
    {
        printf("In RendererVk::newSharedFence(), NOT empty\n");
        mFenceRecycler.fetch(&fence);
        ANGLE_VK_TRY(context, fence.reset(mDevice));
    }
    printf("Assiging fence 0x%p\n", fence.getHandle());
