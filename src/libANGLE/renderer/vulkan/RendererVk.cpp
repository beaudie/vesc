...
{
    // If the vkCmd*DebugUtilsLabelEXT functions exist, and if the environment variables is set,
    // initialize DebugAnnotatorVk to log the OpenGL ES commands that are used, for debuggers (e.g.
    // AGI).  Otherwise, uninitialize the global  DebugAnnotator pointer so that applications run
    // full speed.
    bool enableDebugAnnotatorVk = false;
    if (vkCmdBeginDebugUtilsLabelEXT)
    {
        std::string enabled = angle::GetEnvironmentVarFromProperty(kEnableDebugMarkersVarName,
                                                                   kEnableDebugMarkersPropertyName);
        if (enabled.size() > 0)
        {
