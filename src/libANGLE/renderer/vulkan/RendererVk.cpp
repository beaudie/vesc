                                            ...
                                            QueueSerial *queueSerialOut)
{
    ...
    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.queueSubmitOneOff(
            context, commandContent, priority, primary.getHandle(), waitSemaphore,
                                         ...
                                         const QueueSerial &submitQueueSerial)
{
    ...
    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.submitCommands(
            context, commandContent, contextPriority, signalVkSemaphore,
            std::move(commandBuffersToReset), commandPools, submitQueueSerial));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.submitCommands(context, commandContent, contextPriority,
    ...
    std::vector<VkPipelineStageFlags> &&waitSemaphoreStageMasks)
{
    ...
    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.flushWaitSemaphores(commandContent, waitSemaphores,
                                                        waitSemaphoreStageMasks));
    }
    else
    {
        mCommandQueue.flushWaitSemaphores(commandContent, waitSemaphores, waitSemaphoreStageMasks);
    }
    ...
    vk::RenderPassCommandBufferHelper **renderPassCommands)
{
    ...
    if (isAsyncCommandQueueEnabled())
    {
        ANGLE_TRY(mCommandProcessor.flushRenderPassCommands(context, commandContent, renderPass,
                                                            renderPassCommands));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.flushRenderPassCommands(context, commandContent, renderPass,
                                                        renderPassCommands));
    }
    ...
    vk::OutsideRenderPassCommandBufferHelper **outsideRPCommands)
{
    ...
    {
        ANGLE_TRY(
            mCommandProcessor.flushOutsideRPCommands(context, commandContent, outsideRPCommands));
    }
    else
    {
        ANGLE_TRY(mCommandQueue.flushOutsideRPCommands(context, commandContent, outsideRPCommands));
    }
