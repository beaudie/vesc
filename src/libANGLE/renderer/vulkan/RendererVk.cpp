...
void RendererVk::initFeatures(DisplayVk *displayVk, const ExtensionNameList &deviceExtensionNames)
{
    ...
    // The compute shader used to generate mipmaps uses a 256-wide workgroup.  Disable this path on
    // devices that don't meet this minimum requirements.
    //
    // On the following platforms, VK_IMAGE_USAGE_STORAGE_BIT is detrimental to performance:
    //
    // - ARM
    //
    // Additionally, this path is disabled on buggy drivers:
    //
    // - AMD: Unfortunately the trybots use ancient AMD cards and drivers.
    const uint32_t maxComputeWorkGroupInvocations =
        mPhysicalDeviceProperties.limits.maxComputeWorkGroupInvocations;
    ANGLE_FEATURE_CONDITION(&mFeatures, disallowGenerateMipmapWithCompute,
                            maxComputeWorkGroupInvocations < 256 || isARM || isAMD);
