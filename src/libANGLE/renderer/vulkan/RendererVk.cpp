...
angle::Result RendererVk::getFormatDescriptorCount(ContextVk *contextVk,
                                                   uint64_t format,
                                                   bool isExternalFormat,
                                                   uint32_t *descriptorCountOut)
{
    if (isExternalFormat && getFeatures().useMultipleDescriptorsForExternalFormats.enabled)
    {
    ...
    if (mFormatDescriptorCountMap.count(format) == 0)
    {
        ...
        *descriptorCountOut = mFormatDescriptorCountMap[format];
    }
