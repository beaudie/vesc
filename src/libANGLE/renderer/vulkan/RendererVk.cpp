                              ...
                              const vk::ExtensionNameList &deviceExtensionNames)
{
    ...
    const bool isIMR = isNvidia || isAMD || isIntel || isSamsung || isSwiftShader;
    const bool isTBR = isARM || isPowerVR || isQualcomm || isBroadcom;

    // Make sure all known architectures are accounted for.
    ASSERT(isIMR || isTBR || isMockICDEnabled());
    ...
    // specified.
    ANGLE_FEATURE_CONDITION(&mFeatures, preferAggregateBarrierCalls, isIMR);
    ...
    ANGLE_FEATURE_CONDITION(&mFeatures, preferSkippingInvalidateForEmulatedFormats, isIMR);
