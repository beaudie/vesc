                                                      ...
                                                      const VkFormatFeatureFlags featureBits) const
{
    ...
    if (deviceProperties.bufferFeatures == kInvalidFormatFeatureFlags)
    {
        ...
        if (vk::IsYUVExternalFormat(formatID))
        {
            ...
            WARN() << "formatID:" << ToUnderlying(formatID) << " formatFeatures: 0x" << std::hex
                   << externalFormatInfo.formatFeatures
                   << " deviceProperties.linearTilingFeatures: 0x"
                   << deviceProperties.linearTilingFeatures
                   << " deviceProperties.optimalTilingFeatures: 0x"
                   << deviceProperties.optimalTilingFeatures
                   << " deviceProperties.bufferFeatures: 0x" << deviceProperties.bufferFeatures;
        }
        else
        {
