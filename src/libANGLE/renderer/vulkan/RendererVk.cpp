...
vk::Error RendererVk::initialize(const egl::AttributeMap &attribs, const char *wsiName)
{
    ...
    if (enableNullDriver)
    {
        ...
        ANGLE_VK_CHECK(angle::SetEnvironmentVar(g_VkICDPathEnv, ANGLE_VK_MOCK_ICD_DIR),
    ...
    if (enableNullDriver)
    {
        ...
        for (const auto &physicalDevice : physicalDevices)
        {
            ...
            if ((kMockVendorID == physicalDeviceProperties.vendorID) &&
                (kMockDeviceID == physicalDeviceProperties.deviceID) &&
                (strcmp(kMockDeviceName, physicalDeviceProperties.deviceName) == 0))
            {
