...
#    endif  // !defined(ANGLE_SHARED_LIBVULKAN)
    ...
    {
        VkQueue queue;
        vkGetDeviceQueue(mDevice, mProtectedQueueFamilyIndex, kQueueIndexLow, &queue);
        protectedQueueMap[egl::ContextPriority::Low]       = queue;
        protectedQueueMap[egl::ContextPriority::Medium]    = queue;
        protectedQueueMap[egl::ContextPriority::High]      = queue;
    ...
    {
        ANGLE_TRY(
            mCommandQueue.init(displayVk, queueMap, protectedQueueMap, supportsProtectedQueues));
    }
                ...
                kGraphicsAndComputeAndProtected)
            {
                if (queueInfo.queueCount > 0)
                {
                    ...
                    ANGLE_FEATURE_CONDITION(&mFeatures, supportsProtectedMemory,
                                            mProtectedMemoryFeatures.protectedMemory);
                    break;
                }
            }
        }
