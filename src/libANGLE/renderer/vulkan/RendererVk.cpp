...
// CommandBatchInfo implementation.
RendererVk::CommandBatchInfo::CommandBatchInfo()
{
}
...
RendererVk::CommandBatchInfo::CommandBatchInfo(CommandBatchInfo &&other)
    : commandPool(std::move(other.commandPool)), fence(std::move(other.fence)), serial(other.serial)
{
}

RendererVk::CommandBatchInfo &RendererVk::CommandBatchInfo::operator=(CommandBatchInfo &&other)
{
...
{
    for (CommandBatchInfo &inFlightCommand : mInFlightCommands)
    {
        inFlightCommand.fence.destroy(mDevice);
        inFlightCommand.commandPool.destroy(mDevice);
    }
...
vk::Error RendererVk::checkInFlightCommands()
{
    ...
    for (CommandBatchInfo &info : mInFlightCommands)
    {
        VkResult result = info.fence.getStatus(mDevice);
        if (result == VK_NOT_READY)
            break;

        ANGLE_VK_TRY(result);
        ASSERT(info.serial > mLastCompletedQueueSerial);
        mLastCompletedQueueSerial = info.serial;

        info.fence.destroy(mDevice);
        info.commandPool.destroy(mDevice);
        ++finishedCount;
    }
...
vk::Error RendererVk::submitFrame(const VkSubmitInfo &submitInfo, vk::CommandBuffer &&commandBuffer)
{
    ...
    CommandBatchInfo batchInfo;
    ANGLE_TRY(batchInfo.fence.init(mDevice, createInfo));

    ANGLE_VK_TRY(vkQueueSubmit(mQueue, 1, &submitInfo, batchInfo.fence.getHandle()));

    // Store this command buffer in the in-flight list.
    batchInfo.commandPool = std::move(mCommandPool);
    batchInfo.serial      = mCurrentQueueSerial;

    mInFlightCommands.emplace_back(std::move(batchInfo));
