...
{
    // Only the cache data less than 2M can be stored in android, and
    // zlib_internal::GzipCompressHelper() can compress 10M pipeline cache data into about 2M
    // compressed data, so there is no use to compress big pipeline cache which is larger than 10M.
    ...
    if (!egl::CompressBlobCacheData(pipelineCacheData, &compressedData))
    {
        WARN() << "Fail to compress blob cache data\n";
        return;
    }
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        {
            WARN() << "Fail to allocate memory for keyData\n";
            return;
        }
...
void RendererVk::onDestroy(vk::Context *context)
{
    ...
    // Wait compress thread to complete.
...
angle::Result RendererVk::syncPipelineCacheVk(DisplayVk *displayVk)
{
    ...
    compressThread = std::thread(CompressAndStorePipelineCacheVk, mPhysicalDeviceProperties,
                                 displayVk, pipelineCacheData);
