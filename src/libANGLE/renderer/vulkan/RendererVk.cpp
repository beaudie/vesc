                                              ...
                                              bool *success)
{
    ...
    *success = false;
    ...
    angle::MemoryBuffer *cacheData =
        displayVk->getFeatures().enablePipelineCacheDataCompression.enabled ? &compressedData
                                                                            : uncompressedData;
    ...
    for (size_t chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
    {
        ...
        bool isHeaderDataCorrupted =
            ...
            (cacheData->size() < compressedSize + chunkSize);
    ...
    if (kEnableCRCForPipelineCache)
    {
        uint16_t computedCompressedDataCRC = ComputeCRC16(cacheData->data(), compressedSize);
    ...
    if (displayVk->getFeatures().enablePipelineCacheDataCompression.enabled)
    {
        ANGLE_VK_CHECK(
            displayVk,
