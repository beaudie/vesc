...
void RendererVk::initFeatures(DisplayVk *displayVk, const ExtensionNameList &deviceExtensionNames)
{
    ...
    // Only enable when the validation layers are on. Keep maximum performance in official builds.
    ANGLE_FEATURE_CONDITION(&mFeatures, supportDepthStencilRenderingFeedbackLoops,
                            mEnableValidationLayers);
