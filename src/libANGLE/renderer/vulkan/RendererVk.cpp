...
MemoryReportCallback(const VkDeviceMemoryReportCallbackDataEXT *callbackData, void *userData)
{
    ...
    rendererVk->processCallback(callbackData, logCallback);
}
...
void vk::MemoryReport::logMemoryReportStats()
{
    std::lock_guard<std::mutex> lock(mMemoryReportMutex);

    INFO() << std::right << "GPU Memory: allocated=" << std::setw(10)
    ...
    for (const auto &it : mAllocatedMemory)
    {
    ...
    for (const auto &it : mImportedMemory)
    {
        VkObjectType objectType  = it.first;
        VkDeviceSize currentSize = it.second;
