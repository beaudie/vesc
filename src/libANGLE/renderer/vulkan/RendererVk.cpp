...
void RendererVk::getSubmitWaitSemaphores(vk::Context *context, VkSemaphore *waitSemaphores, uint32_t *outWaitSemaphoreCount)
{
    uint32_t waitSemaphoreCount = 0;

    if (mSubmitLastSignaledSemaphore.getSemaphore())
    {
        ASSERT(waitSemaphoreCount < *outWaitSemaphoreCount);
        waitSemaphores[waitSemaphoreCount++] = mSubmitLastSignaledSemaphore.getSemaphore()->getHandle();
    ...
    if (mSubmitWaitSemaphore)
    {
        ASSERT(waitSemaphoreCount < *outWaitSemaphoreCount);
        waitSemaphores[waitSemaphoreCount++] = mSubmitWaitSemaphore->getHandle();
    }

    *outWaitSemaphoreCount = waitSemaphoreCount;
}
...
angle::Result RendererVk::finish(vk::Context *context)
{
    if (!mCommandGraph.empty())
    {
        ...
        getSubmitWaitSemaphores(context, waitSemaphores, &waitSemaphoreCount);
