                                 ...
                                 const vk::Semaphore *signalSemaphore)
{
    if (!mCommandGraph.empty())
    {
        ...
        InitializeSubmitInfo(&submitInfo, commandBatch.get(), waitSemaphore,
                             VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, signalSemaphore);
...
angle::Result RendererVk::getTimestamp(vk::Context *context, uint64_t *timestampOut)
{
    ...
    VkSubmitInfo submitInfo         = {};
    InitializeSubmitInfo(&submitInfo, commandBatch.get(), nullptr, 0, nullptr);
