...
void MemoryAllocationTracker::initMemoryTrackers()
{
    ...
    // allocations.
    for (size_t type = 0; type < mActiveMemoryAllocationsSize.size(); type++)
    {
        mActiveMemoryAllocationsSize[type]  = 0;
        mActiveMemoryAllocationsCount[type] = 0;

        // Per-heap allocation counters are initialized here.
        for (size_t heap = 0; heap < mRenderer->getMemoryProperties().getMemoryHeapCount(); heap++)
        {
            mActivePerHeapMemoryAllocationsSize[heap][type]  = 0;
            mActivePerHeapMemoryAllocationsCount[heap][type] = 0;
        }
    }
