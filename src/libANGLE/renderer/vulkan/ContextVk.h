...
class ContextVk : public ContextImpl, public vk::Context, public MultisampleTextureInitializer
{
  ...
  private:
    ...
    // Whether framebuffer fetch has been used.  If any program uses framebuffer fetch, rendering
    // switches to assuming framebuffer fetch could happen in any render pass.  This incurs a
    // potential cost due to usage of the GENERAL layout instead of COLOR_ATTACHMENT_OPTIMAL, but
    // has definite benefits of avoiding render pass breaks when a framebuffer fetch program is used
    // mid render pass.
