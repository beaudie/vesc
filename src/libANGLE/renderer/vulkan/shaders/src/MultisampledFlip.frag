...
//
// MultisampledFlip.frag: Helper shader for multisampled color resolve.  If one of the framebuffers
...
#if SrcIsArray
    Type srcValue = texelFetch(src, ivec3(srcImageCoords, params.srcLayer), gl_SampleID);
#else
    Type srcValue = texelFetch(src, srcImageCoords, gl_SampleID);
#endif
