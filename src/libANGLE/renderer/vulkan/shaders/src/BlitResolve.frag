...
#if IsResolve || IsBlitDepth || IsBlitStencil
...
#define TEXTURE(src, coord, sample) texture(src, vec3(coord * params.invSrcExtent, params.srcLayer))
#define TEXEL_FETCH(src, coord, sample) texelFetch(src, ivec3(coord + 0.5, params.srcLayer), 0)
#else
#define SRC_RESOURCE_NAME texture2D
#define SRC_SAMPLER_NAME sampler2D
#define TEXTURE(src, coord, sample) texture(src, coord * params.invSrcExtent)
#define TEXEL_FETCH(src, coord, sample) texelFetch(src, ivec2(coord + 0.5), 0)
#endif

#define COLOR_TEXEL_FETCH(src, coord, sample) TEXTURE(COLOR_SRC_RESOURCE(SRC_SAMPLER_NAME)(src, blitSampler), coord, sample)
#define DEPTH_TEXEL_FETCH(src, coord, sample) TEXEL_FETCH(src, coord, sample)
#define STENCIL_TEXEL_FETCH(src, coord, sample) TEXEL_FETCH(src, coord, sample)
