...
angle::Result CommandProcessor::init(Context *context,
                                     ...
                                     bool supportsProtectedQueues)
{
    ANGLE_TRY(mCommandQueue.init(context, queueMap, protectedQueueMap, supportsProtectedQueues));
...
CommandQueue::CommandQueue()
    : mCurrentQueueSerial(mQueueSerialFactory.generate()), mSupportsProtectedQueues(false)
{}
...
void CommandQueue::destroy(Context *context)
{
    ...
    if (mSupportsProtectedQueues)
    {
        for (VkQueue queue : mProtectedQueues)
        {
            if (queue != VK_NULL_HANDLE)
            {
                vkQueueWaitIdle(queue);
                                 ...
                                 bool supportsProtectedQueues)
{
    ...
    uint32_t queueFamilyIndex = renderer->getQueueFamilyIndex(false);
    ANGLE_TRY(mPrimaryCommandPool.init(context, false, queueFamilyIndex));

    if (supportsProtectedQueues)
    {
        uint32_t protectedQueueFamilyIndex = renderer->getQueueFamilyIndex(true);
        if (protectedQueueFamilyIndex != std::numeric_limits<uint32_t>::max())
        {
            mSupportsProtectedQueues = supportsProtectedQueues;
            ANGLE_TRY(mProtectedCommandPool.init(context, true, protectedQueueFamilyIndex));
        }
    }

    mQueues          = queueMap;
    mProtectedQueues = protectedQueueMap;
                                                  ...
                                                  CommandBatch *batch)
{
    ...
    if (commandPool->valid())
    {
        ...
        poolInfo.queueFamilyIndex        = renderer->getQueueFamilyIndex(isProtectedMemory);
    ...
    Serial submitQueueSerial)
{
    ...
    /*  TODO: (jvigil) this crashes Pixel3 sometimes (unMakeCurrent), no crash without, mandatory by
       spec. if (isProtectedMemory)
        {
...
VkQueue CommandQueue::getQueue(bool isProtectedMemory, egl::ContextPriority priority)
{
    if (isProtectedMemory)
    {
        ASSERT(mSupportsProtectedQueues);
        return mProtectedQueues[priority];
    }
    else
    {
        return mQueues[priority];
    }
}
