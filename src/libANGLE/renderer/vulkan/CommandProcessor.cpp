                                            ...
                                            vk::SharedResourceUsePool *resourcePool)
{
    ...
    ANGLE_TRY(renderer->commandProcessorThreadQueueSubmit(
        this, priority, submitInfo, resourceList, resourcePool, &batch.fence.get(), &batch.serial));
        ...
        switch (task.mWorkerCommand)
        {
            ...
            case vk::CustomTask::Flush:
            {
                ...
                ANGLE_TRY(mCommandWorkQueue.submitFrame(
                    ...
                    &mCommandPool, std::move(mPrimaryCommandBuffer), &task.mResourceUseList,
                    task.mResourcePool));
