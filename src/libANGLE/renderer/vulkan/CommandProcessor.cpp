...
void CommandProcessor::processTasks()
{
    ...
    while (true)
    {
        ...
        if (result != angle::Result::Continue)
        {
            ...
            // ContextcommandProcessorSyncErrorsAndQueueCommand and WindowSurfacedestroy
...
angle::Result CommandProcessor::processTask(CommandProcessorTask *task)
{
    switch (task->getTaskCommand())
    {
        case CustomTask::Exit:
        {
            ...
            mCommandQueue.destroy(mRenderer->getDevice());
        ...
        case CustomTask::ProcessCommands:
        {
            ...
            ASSERT(task->getCommandBuffer()->empty());
            mRenderer->recycleCommandBuffer(task->getCommandBuffer());
            break;
        }
...
// TODO: https://issuetracker.google.com/170311829 - Add Context so that queueCommand has
