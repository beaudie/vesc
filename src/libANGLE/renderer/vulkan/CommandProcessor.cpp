...
void CommandQueue::handleDeviceLost(RendererVk *renderer)
{
    ...
    for (CommandBatch &batch : mInFlightCommands)
    {
        ...
        if (batch.fence)
        {
            ...
            // If the wait time7s out, it is probably not possible to recover from lost device
...
angle::Result CommandQueue::submitCommands(Context *context,
                                           ...
                                           const QueueSerial &submitQueueSerial)
{
    ANGLE_TRY(submitCommandsImpl(context, hasProtectedContent, priority, signalSemaphore,
                                               ...
                                               const QueueSerial &submitQueueSerial)
{
    ...
    if (needsQueueSubmit)
    {
        ...
        InitializeSubmitInfo(&submitInfo, batch.primaryCommands, state.waitSemaphores,
    ...
    ANGLE_TRY(
        queueSubmit(context, lock, priority, submitInfo, fence, scopedBatch, submitQueueSerial));
                                              ...
                                              const QueueSerial &submitQueueSerial)
{
    ...
    // Note queueSubmit will release the lock.
    return queueSubmit(context, lock, contextPriority, submitInfo, fence, scopedBatch,
                       submitQueueSerial);
}
                                        ...
                                        const QueueSerial &submitQueueSerial)
{
    ...
    // thread1/ctx1 issue present. We have to ensure the ordering of submission strictly
