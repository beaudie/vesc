                                 ...
                                 uint32_t queueCount)
{
    ...
    mQueueMap.initialize(device, queueFamily, enableProtectedContent, 0, queueCount);
...
/*DeviceQueueMap &DeviceQueueMap::operator=(const DeviceQueueMap &other)
{
    ...
    if ((this != &other) && other.valid())
    {
        ...
        mQueueAndIndices = other.mQueueAndIndices;
    }
    return *this;
}*/
                                ...
                                uint32_t queueCount)
{
    ...
    mFamilyIndex = queueFamily.getQueueFamilyIndex();
    mIsProtected = makeProtected;

    VkQueue queue = VK_NULL_HANDLE;
    GetDeviceQueue(device, makeProtected, mFamilyIndex, queueIndex + kQueueIndexMedium, &queue);
    ...
    if (queueCount > 1)
    {
        GetDeviceQueue(device, makeProtected, mFamilyIndex, queueIndex + kQueueIndexHigh, &queue);
    ...
    if (queueCount > 2)
    {
        GetDeviceQueue(device, makeProtected, mFamilyIndex, queueIndex + kQueueIndexLow, &queue);
                                               ...
                                               uint32_t queueCount)
{
    ...
    queueMap.mQueueAndIndices[egl::ContextPriority::Medium] = {egl::ContextPriority::Medium, queue,
                                                               queueIndex + kQueueIndexMedium};
    ...
    if (queueCount > 1)
    {
        ...
        queueMap.mQueueAndIndices[egl::ContextPriority::High] = {egl::ContextPriority::High, queue,
                                                                 queueIndex + kQueueIndexHigh};
    }
    else
    {
        queueMap.mQueueAndIndices[egl::ContextPriority::High] =
            queueMap.mQueueAndIndices[egl::ContextPriority::Medium];
    }
    // If at least 3 queues, Low has its own queue. Adjust Low priority.
    if (queueCount > 2)
    {
        getDeviceQueue(device, makeProtected, queueIndex + kQueueIndexLow, &queue);
        queueMap.mQueueAndIndices[egl::ContextPriority::Low] = {egl::ContextPriority::Low, queue,
                                                                queueIndex + kQueueIndexLow};
    }
    else
    {
        queueMap.mQueueAndIndices[egl::ContextPriority::Low] =
            queueMap.mQueueAndIndices[egl::ContextPriority::Medium];
    }
    return queueMap;
}
