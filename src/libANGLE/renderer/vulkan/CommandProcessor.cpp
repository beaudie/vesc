                                            ...
                                            vk::PrimaryCommandBuffer &&commandBuffer)
{
    ...
    ANGLE_TRY(renderer->commandProcessorThreadQueueSubmit(priority, submitInfo, &batch.fence.get(),
                                                          &batch.serial));
...
CommandProcessor::CommandProcessor(RendererVk *renderer)
    : mWorkerThreadIdle(true), mRenderer(renderer)
{}

void CommandProcessor::queueCommands(vk::CommandProcessorTask commands)
{
    ANGLE_TRACE_EVENT0("gpu.angle", "RendererVk::queueCommands");
    std::lock_guard<std::mutex> queueLock(mWorkerMutex);
    ASSERT(commands.workerCommand == vk::CustomTask::Flush || commands.commandBuffer == nullptr ||
...
angle::Result CommandProcessor::processCommandProcessorTasks()
{
    ...
    ANGLE_TRY(mCommandWorkQueue.init(mRenderer));
    // Allocate and begin primary command buffer
    ANGLE_TRY(mCommandWorkQueue.allocatePrimaryCommandBuffer(mRenderer->getDevice(),
                ...
                switch (task.workerCommand)
                {
                    case vk::CustomTask::Flush:
                    {
                        ...
                        ANGLE_TRY(mCommandWorkQueue.allocatePrimaryCommandBuffer(
                            mRenderer->getDevice(), &mPrimaryCommandBuffer));
                        // TODO: Need to catch error here.
                        mPrimaryCommandBuffer.begin(beginInfo);
