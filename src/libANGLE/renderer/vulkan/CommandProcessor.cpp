...
angle::Result SharedFenceManager::newSharedFence(vk::Context *context,
...
void SharedFenceManager::destroy(vk::Context *context)
{
    std::lock_guard<std::mutex> lock(mFenceRecyclerMutex);
    mFenceRecycler.destroy(context->getDevice());
}
...
angle::Result CommandQueue::retireFinishedCommands(Context *context, size_t finishedCount)
{
    ...
    for (size_t commandIndex = 0; commandIndex < finishedCount; ++commandIndex)
    {
        ...
        sharedFences.resetSharedFence(&batch.fence);
    ...
    Serial submitQueueSerial)
{
    ...
    ANGLE_TRY(sharedFences.newSharedFence(context, &batch.fence));
