...
angle::Result CommandProcessor::queueCommand(CommandProcessorTask &&task)
{
    ...
    if (mTaskQueue.full())
    {
        std::lock_guard<angle::Mutex> dequeueLock(mTaskDequeueMutex);
...
{
    std::lock_guard<angle::Mutex> lock(mMutex);
    std::lock_guard<angle::Mutex> enqueuelock(mQueueSubmitMutex);
...
void CommandQueue::handleDeviceLost(vk::Renderer *renderer)
{
    ...
    std::lock_guard<angle::Mutex> dequeuelock(mMutex);
    std::lock_guard<angle::Mutex> enqueuelock(mQueueSubmitMutex);
                                        ...
                                        const QueueSerial &submitQueueSerial)
{
    ...
    // submission order.
    std::lock_guard<angle::Mutex> queueSubmitLock(mQueueSubmitMutex);
