...
angle::Result CommandProcessor::ensureNoPendingSubmission(Context *context)
{
    // Technically this function should wait until the last _submission_ has been submitted, but
    // waiting for all tasks to complete works just as well.  This function is currently only called
    // after a submission, so that's the last task anyway.
    return waitForWorkComplete(context);
}
