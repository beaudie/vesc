...
void ResetSecondaryCommandBuffers(VulkanSecondaryCommandBufferList *commandBuffers)
{
...
void CommandProcessorTask::initFlushAndQueueSubmit(const VkSemaphore semaphore,
                                                   ProtectionType protectionType,
                                                   egl::ContextPriority priority,
...
void CommandBatch::destroy(VkDevice device)
{
    ...
    ResetSecondaryCommandBuffers(&secondaryCommands);
...
void CommandQueue::destroy(Context *context)
{
    ...
    for (auto &protectionMap : mCommandsStateMap)
    {
        for (CommandsState &state : protectionMap)
        {
            ...
            state.secondaryCommandsCollector.releaseCommandBuffers();
        }
    }
    ...
    OutsideRenderPassCommandBufferHelper **outsideRPCommands)
{
    ...
    return (*outsideRPCommands)
        ->flushToPrimary(context, &state.primaryCommands, &state.secondaryCommandsCollector);
}
    ...
    RenderPassCommandBufferHelper **renderPassCommands)
{
    ...
    return (*renderPassCommands)
        ->flushToPrimary(context, &state.primaryCommands, &renderPass,
                         &state.secondaryCommandsCollector);
}
                                           ...
                                           const QueueSerial &submitQueueSerial)
{
    ...
    state.secondaryCommandsCollector.releaseCommandBuffersToEmptyList(&batch.secondaryCommands);
