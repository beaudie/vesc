...
{
    ASSERT(!externalFence.isReferenced() || !fence);
    return fence || externalFence.isReferenced();
}

void CommandBatch::releaseFence(VkDevice device)
{
    fence.release();
    externalFence.reset(device);
}
...
{
    ASSERT(hasFence());
    return fence ? fence.get().getHandle() : externalFence.get().getHandle();
}

VkResult CommandBatch::getFenceStatus(VkDevice device) const
{
    ASSERT(hasFence());
    return fence ? fence.getStatus(device) : externalFence.get().getStatus(device);
}

VkResult CommandBatch::waitFence(VkDevice device, uint64_t timeout) const
{
    ASSERT(hasFence());
    return fence ? fence.wait(device, timeout) : externalFence.get().wait(device, timeout);
}
                                         ...
                                         std::unique_lock<std::mutex> *lock) const
{
    ...
    {
        SharedExternalFence localFenceToWaitOn;
        localFenceToWaitOn.copyUnreferenced(externalFence);
        lock->unlock();
                                           ...
                                           const QueueSerial &submitQueueSerial)
{
    ...
    const bool needsQueueSubmit = batch.primaryCommands.valid() ||
                                  ...
                                  externalFence.isReferenced() || !waitSemaphores.empty();
                                        ...
                                        const QueueSerial &submitQueueSerial)
{
    ...
    if (submitInfo.sType == VK_STRUCTURE_TYPE_SUBMIT_INFO)
    {
        ...
        if (batch.externalFence.isReferenced())
        {
            ExternalFence &externalFence       = batch.externalFence.get();
...
angle::Result CommandQueue::retireFinishedCommandsLocked(Context *context)
{
    ...
    while (!mFinishedCommandBatches.empty())
    {
        ...
        batch.releaseFence(context->getDevice());
