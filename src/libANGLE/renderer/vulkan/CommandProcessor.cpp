...
angle::Result CommandProcessor::processTask(CommandProcessorTask *task)
{
    switch (task->getTaskCommand())
    {
        ...
        case CustomTask::Present:
        {
            ...
            if (task->getSwapchainStatus()->lastPresentResult != VK_ERROR_OUT_OF_DATE_KHR &&
                task->getSwapchainStatus()->lastPresentResult != VK_SUBOPTIMAL_KHR &&
                task->getSwapchainStatus()->lastPresentResult != VK_SUCCESS)
            {
                ...
                handleError(task->getSwapchainStatus()->lastPresentResult, __FILE__, __FUNCTION__,
                            __LINE__);
            }
            break;
        }
...
void CommandProcessor::queuePresent(egl::ContextPriority contextPriority,
                                    const VkPresentInfoKHR &presentInfo,
                                    SwapchainStatus *swapchainStatus)
{
    {
        ...
        // Always return with VK_SUCCESS. When we call acquireNextImage we'll check the return code
        // again. This allows the app to continue working until we really need to know the return
        // code from present.
