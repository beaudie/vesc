                                           ...
                                           const QueueSerial &submitQueueSerial)
{
    ...
    ANGLE_TRY(queueSubmit(context, std::move(lock), priority, submitInfo, fence, externalFence,
                                              ...
                                              const QueueSerial &submitQueueSerial)
{
    ...
    ANGLE_VK_TRY(context, batch.fence.init(context->getDevice(), &mFenceRecycler));
    VkFence fence = batch.fence.get().getHandle();
    ...
    // Note queueSubmit will release the lock.
    return queueSubmit(context, std::move(lock), contextPriority, submitInfo, fence, VK_NULL_HANDLE,
                       scopedBatch, submitQueueSerial);
}
