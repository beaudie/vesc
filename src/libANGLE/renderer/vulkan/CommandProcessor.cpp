...
bool CommandsHaveValidOrdering(const std::vector<vk::CommandBatch> &commands)
{
    for (size_t i = 0; i < commands.size(); i++)
    {
        ...
        for (size_t j = i + 1; j < commands.size(); j++)
        {
            ...
            if (batch1.queueSerial.getIndex() == batch2.queueSerial.getIndex() &&
                batch1.queueSerial.getSerial() >= batch2.queueSerial.getSerial())
            {
                return false;
            }
        }
...
size_t GetBatchCountUpToSerial(std::vector<CommandBatch> &inFlightCommands,
                               const QueueSerialFixedArray &lastSubmittedSerials,
                               const QueueSerialFixedArray &mLastCompletedSerials,
...
angle::Result CommandQueue::retireFinishedCommands(Context *context, size_t finishedCount)
{
    ...
    while (!mGarbageQueue.empty())
    {
        ...
        if (mLastCompletedSerials >= garbageList.getQueueSerial())
        {
