...
size_t GetBatchCountUpToSerials(std::vector<CommandBatch> &inFlightCommands,
                                ...
                                const AtomicQueueSerialFixedArray &mLastCompletedSerials,
...
angle::Result CommandQueue::retireFinishedCommands(Context *context, size_t finishedCount)
{
    ...
    std::array<Serial, kMaxQueueSerialIndexCount> lastCompletedQueueSerials;
    BitSetArrayT finishedSerialBitMask;
    for (size_t commandIndex = 0; commandIndex < finishedCount; ++commandIndex)
    {
        ...
        finishedSerialBitMask.set(batch.queueSerial.getIndex());
    ...
    for (auto iter = finishedSerialBitMask.begin(), endIter = finishedSerialBitMask.end();
         iter != endIter; ++iter)
    {
        mLastCompletedSerials.setQueueSerial(static_cast<SerialIndex>(*iter),
                                             lastCompletedQueueSerials[*iter]);
    }
                                                                    ...
                                                                    size_t finishedCount)
{
    ...
    if (renderer->getLastAllocatedQueueSerialIndex() < 64)
    {
        ANGLE_TRY(retireFinishedCommands<angle::BitSet64<64>>(context, finishedCount));
    }
    else
    {
        ANGLE_TRY(retireFinishedCommands<angle::BitSetArray<kMaxQueueSerialIndexCount>>(
                                              ...
                                              uint64_t timeout)
{
    ...
    // Clean up finished batches.getBatchCountUpToSerials
    ANGLE_TRY(retireFinishedCommandsAndCleanupGarbage(context, finishCount));
