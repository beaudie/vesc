...
// same index. This should only be called in assert build.
    ...
    const QueueSerial &submitQueueSerial)
{
    ...
    ASSERT(submitQueueSerial.serial > mLastSubmittedSerials[submitQueueSerial.index]);
                                        ...
                                        const QueueSerial &submitQueueSerial)
{
    ...
    ASSERT(mLastSubmittedSerials[submitQueueSerial.index] < submitQueueSerial.serial);
    mLastSubmittedSerials[submitQueueSerial.index] = submitQueueSerial.serial;
...
bool CommandQueue::hasUnfinishedUse(const vk::ResourceUse &use) const
{
    ...
    for (SerialIndex i = 0; i < serials.size(); ++i)
    {
        if (serials[i].valid() && serials[i] > mLastCompletedSerials[i])
        {
            return true;
        }
    }
