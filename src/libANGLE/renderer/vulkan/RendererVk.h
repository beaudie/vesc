...
class BufferMemoryAllocator : angle::NonCopyable
{
  public:
    ...
    angle::Result initialize(RendererVk *renderer);
  ...
  private:
    ...
    VmaPool createVMAPool(const Allocator &allocator,
                          uint32_t memoryTypeIndex,
                          VMAPoolType poolType);
    VmaPool getVMAPool(uint32_t memoryTypeIndex, VMAPoolType poolType);
    ...
    using VMAPoolArray =
        angle::PackedEnumMap<VMAPoolType, std::array<VmaPool, VK_MAX_MEMORY_TYPES>>;
    VMAPoolArray mVMAPools;
