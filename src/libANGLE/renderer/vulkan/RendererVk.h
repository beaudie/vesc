...
class RendererVk : angle::NonCopyable
{
  ...
  private:
    ...
    // command buffer submission, for example for semaphores that surface's acquire image signals.
    ...
    // surface image to have to wait on it. If CommandGraphs are separated by context or share group
    // for example, this could be moved to the one that actually uses the image.
