...
class BufferMemoryAllocator : angle::NonCopyable
{
  ...
  private:
    VkResult createVMAPool(const Allocator &allocator,
                           uint32_t memoryTypeIndex,
                           int poolType,
                           VkDeviceSize preferredLargeHeapBlockSize)
    {
        ...
        bool useBuddyAlgorithm = poolType == kSmallPool;
        // Create a custom pool.
        return vma::CreatePool(allocator.getHandle(), memoryTypeIndex, useBuddyAlgorithm, blockSize,
                               &pool);
    }
    ...
    VmaPool getVMAPool(uint32_t memoryTypeIndex,
                       ...
                       bool *dedicatedMemory)
    {
        ...
        int poolType = (!robustResourceInitEnabled && requestedSize <= kMaxSizeToUseSmallPool)
                           ? kSmallPool
                           : kLargePool;
