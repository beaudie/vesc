...
class RendererVk : angle::NonCopyable
{
  public:
    ...
    angle::Result ensureNoPendingSubmission(vk::Context *context)
    {
        if (mFeatures.asyncCommandQueue.enabled)
        {
            return mCommandProcessor.ensureNoPendingSubmission(context);
        }
        else
        {
            return mCommandQueue.ensureNoPendingSubmission(context);
        }
    }
