...
class RendererVk : angle::NonCopyable
{
  public:
    ...
    VkDeviceSize getActiveHeapMemoryAllocationsSize(uint32_t allocTypeIndex, uint32_t heapIndex);
    uint64_t getActiveMemoryAllocationsCount(uint32_t allocTypeIndex);
    uint64_t getActiveHeapMemoryAllocationsCount(uint32_t allocTypeIndex, uint32_t heapIndex);
