...
class RendererVk : angle::NonCopyable
{
  public:
    ...
    void setSubmitWaitSemaphore(const vk::Semaphore *semaphore) { mSubmitWaitSemaphore = semaphore; }
    // Get the last signaled semaphore to wait on externally.  The semaphore will not be waited on by
    // next submission.
  ...
  private:
    ...
    void getSubmitWaitSemaphores(vk::Context *context, VkSemaphore *waitSemaphores, uint32_t *outWaitSemaphoreCount);
