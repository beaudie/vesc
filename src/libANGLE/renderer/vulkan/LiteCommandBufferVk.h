...
class LiteCommandBuffer final : angle::NonCopyable
{
  ...
  private:
    ...
    // Ptr to write next section of cmd data into
    uint8_t *mCurrentCmdData;
    ...
    // Copy sizeInBytes data from paramData to mCurrentCmdData and increment
    //  mCurrentCmdData by sizeInBytes
    void updateCurrentCmdData(const uint8_t *paramData, size_t sizeInBytes);
};
