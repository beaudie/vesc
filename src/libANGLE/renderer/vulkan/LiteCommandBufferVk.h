...
class LiteCommandBuffer final : angle::NonCopyable
{
  ...
  private:
    ...
    // Copy sizeInBytes data from paramData to mCurrentCmdData and assign *writePtr
    //  to mCurrentCmdData. Then increment mCurrentCmdData by sizeInBytes.
    // Precondition: initCommand() must have already been called on the given cmd
    template <class ptrType>
    void updatePtrCmdData(const ptrType *paramData, const ptrType **writePtr, size_t sizeInBytes);
};
