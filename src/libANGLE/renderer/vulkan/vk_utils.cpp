...
{
    ASSERT(!valid());
    ANGLE_VK_WRAPPED_EXEC(
        context, vkCreateCommandPool(context->getDevice(), &createInfo, nullptr, &mHandle));
}
...
{
    ASSERT(valid());
    ANGLE_VK_WRAPPED_EXEC(context, vkBeginCommandBuffer(mHandle, &info));
}

angle::Result CommandBuffer::end(Context *context)
{
    ASSERT(valid());
    ANGLE_VK_WRAPPED_EXEC(context, vkEndCommandBuffer(mHandle));
}

angle::Result CommandBuffer::reset(Context *context)
{
    ASSERT(valid());
    ANGLE_VK_WRAPPED_EXEC(context, vkResetCommandBuffer(mHandle, 0));
}
...
void CommandBuffer::resetQueryPool(const QueryPool &queryPool,
                                   uint32_t firstQuery,
                                   uint32_t queryCount)
{
...
void CommandBuffer::beginQuery(const QueryPool &queryPool,
                               uint32_t query,
...
void CommandBuffer::endQuery(const QueryPool &queryPool, uint32_t query)
{
    ASSERT(valid() && queryPool.valid());
...
angle::Result Semaphore::init(Context *context)
{
    ...
    ANGLE_VK_WRAPPED_EXEC(
        context, vkCreateSemaphore(context->getDevice(), &semaphoreInfo, nullptr, &mHandle));
}
...
angle::Result Framebuffer::init(Context *context, const VkFramebufferCreateInfo &createInfo)
{
    ASSERT(!valid());
    ANGLE_VK_WRAPPED_EXEC(
        context, vkCreateFramebuffer(context->getDevice(), &createInfo, nullptr, &mHandle));
}
...
{
    ASSERT(valid());
    ANGLE_VK_WRAPPED_EXEC(context, vkMapMemory(context->getDevice(), mHandle, offset, size, flags,
...
angle::Result ShaderModule::init(Context *context, const VkShaderModuleCreateInfo &createInfo)
{
    ASSERT(!valid());
    ANGLE_VK_WRAPPED_EXEC(
        context, vkCreateShaderModule(context->getDevice(), &createInfo, nullptr, &mHandle));
}
...
angle::Result PipelineCache::getCacheData(Context *context, size_t *cacheSize, void *cacheData)
{
    ...
    ANGLE_VK_WRAPPED_EXEC_ALLOW_INCOMPLETE(
                                                 ...
                                                 const VkDescriptorSet *descriptorSets)
{
    ...
    ANGLE_VK_WRAPPED_EXEC(context, vkFreeDescriptorSets(context->getDevice(), mHandle,
...
{
    ANGLE_VK_WRAPPED_EXEC_ALLOW_NOT_READY(
