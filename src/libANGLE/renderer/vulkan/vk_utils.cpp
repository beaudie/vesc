                                              ...
                                              VkResult *resultOut)
{
    ...
    do
    {
        *resultOut = deviceMemoryOut->allocate(device, allocInfo);
    ...
    } while (*resultOut != VK_SUCCESS && anyBatchCleaned);
    ...
    if (batchesWaitedAndCleaned > 0)
    {
        INFO() << "Initial allocation failed. Waited for " << batchesWaitedAndCleaned
               ...
               << ((*resultOut == VK_SUCCESS) ? "SUCCESS" : "FAIL");
    }
                                   ...
                                   VkResult *resultOut)
{
    return AllocateBufferOrImageMemory(context, memoryAllocationType, requestedMemoryPropertyFlags,
                                       ...
                                       memoryTypeIndexOut, deviceMemoryOut, sizeOut, resultOut);
}
