...
angle::Result InitializeBufferMemory(vk::Context *context, vk::Buffer *buffer, VkDeviceSize size)
{
    ...
    ANGLE_VK_TRY(context, commandBuffer.init(context->getDevice(), allocInfo));
    ...
    commandBuffer.copyBuffer(stagingBuffer.getBuffer(), *buffer, 1, &copyRegion);
                                              ...
                                              vk::DeviceMemory *deviceMemoryOut)
{
    ...
    // Wipe memory to an invalid value when debug layers are enabled. The invalid values ensures our
    // testing doesn't assume zero-initialized memory.
    RendererVk *renderer = context->getRenderer();
...
void StagingBuffer::collectGarbage(RendererVk *renderer,
                                   Serial serial,
                                   vk::GarbageObject &&extraGarbage)
{
    ...
    garbageList.emplace_back(std::move(extraGarbage));

    vk::SharedResourceUse sharedUse;
    sharedUse.init();
    sharedUse.releaseAndUpdateSerial(serial);
...
{
    VkDeviceSize size;
    ANGLE_TRY(AllocateBufferOrImageMemory(context, requestedMemoryPropertyFlags,
                                          memoryPropertyFlagsOut, extraAllocationInfo, buffer,
                                          deviceMemoryOut, &size));
    ...
    if (renderer->enableValidationLayers())
    {
        if ((*memoryPropertyFlagsOut & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) == 0)
        {
            ...
            ANGLE_TRY(InitializeBufferMemory(context, buffer, size));
        }
    }
