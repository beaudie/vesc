...
GarbageBindingPointer::GarbageBindingPointer(GarbageBindingPointer &&other)
    : mSerial(std::move(other.mSerial)), mGarbage(std::move(other.mGarbage))
{
}
...
bool GarbageBindingPointer::isSerialInUse(Serial lastCompletedSerial) const
{
    return lastCompletedSerial >= mSerial;
}
...
void GarbageQueue::PerContextGarbage::addGarbage(Serial serial, GarbageObject *garbage)
{
    if (serial > mLastCompletedSerial)
    {
        ...
        mGarbage.push_back(GarbageBindingPointer(serial, garbage));
    }
}
                                                              ...
                                                              Serial lastCompletedSerial)
{
    ...
    {
        if (mGarbage[freeIndex].isSerialInUse(mLastCompletedSerial))
        {
            break;
        }
