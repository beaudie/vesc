...
ScopedContextUnlock::ScopedContextUnlock(Context *context, ContextVk *contextVk)
    : mContextUnlock(contextVk != nullptr && !contextVk->getState().isDummyContextMutexLocked()
                                      ...
                                      : egl::GlobalMutexUnlockType::Always)
{
    ...
    ASSERT(contextVk == nullptr || contextVk == context);
    ASSERT(contextVk != nullptr || context->getType() == ContextType::kDisplayVk);
}
