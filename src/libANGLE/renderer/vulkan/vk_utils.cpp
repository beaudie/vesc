...
LineLoopHandler::LineLoopHandler()
    : mObserverBinding(this, 0u),
      ...
      mIndexBufferHandle(VK_NULL_HANDLE),
      mIndexBufferOffset(VK_NULL_HANDLE)
{
                                             ...
                                             VkDeviceSize *offsetOut)
{
    ...
    if (mIndexBufferHandle != VK_NULL_HANDLE && !mLineLoopBufferFirstIndex.valid() &&
        mLineLoopBufferLastIndex.valid() &&
    ...
    ANGLE_TRY(mStreamingLineLoopIndicesData->allocate(
        renderer, allocateBytes, reinterpret_cast<uint8_t **>(&indices), &mIndexBufferHandle,
        &mIndexBufferOffset, nullptr));

    auto unsignedFirstVertex = static_cast<uint32_t>(drawCallParams.firstVertex());
    for (auto vertexIndex = unsignedFirstVertex;
         vertexIndex < (drawCallParams.vertexCount() + unsignedFirstVertex); vertexIndex++)
    {
        *indices++ = vertexIndex;
    }
    *indices = unsignedFirstVertex;
    ...
    // writing.
    ANGLE_TRY(mStreamingLineLoopIndicesData->flush(renderer->getDevice()));
                                                                   ...
                                                                   VkDeviceSize *bufferOffsetOut)
{
    ...
    ANGLE_TRY(mStreamingLineLoopIndicesData->allocate(
        renderer, allocateBytes, reinterpret_cast<uint8_t **>(&indices), bufferHandleOut,
        &mIndexBufferOffset, nullptr));

    VkBufferCopy copy1 = {0, mIndexBufferOffset, static_cast<VkDeviceSize>(indexCount) * unitSize};
    VkBufferCopy copy2 = {0, mIndexBufferOffset + static_cast<VkDeviceSize>(indexCount) * unitSize,
                          unitSize};
    std::array<VkBufferCopy, 2> copies = {{copy1, copy2}};

    vk::CommandBuffer *commandBuffer;
    mStreamingLineLoopIndicesData->beginWriteResource(renderer, &commandBuffer);

    Serial currentSerial = renderer->getCurrentQueueSerial();
    elementArrayBufferVk->onReadResource(
        mStreamingLineLoopIndicesData->getCurrentWritingNode(currentSerial), currentSerial);
    commandBuffer->copyBuffer(elementArrayBufferVk->getVkBuffer().getHandle(), mIndexBufferHandle,
                              2, copies.data());
...
{
    // Our first index is always 0 because that's how we set it up in the bindLineLoopIndexBuffer.
