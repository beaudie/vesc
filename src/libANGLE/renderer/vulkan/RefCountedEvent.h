...
class RefCountedEvent final : public WrappedObject<RefCountedEvent, RefCountedEventAndLayoutHandle>
{
  public:
    ...
    {
        ASSERT(valid());
        // Since the underline object is refcounted, we always have to
    ...
    bool valid() const
    {
        ASSERT(!mHandle || mHandle->get().event.valid());
...
{
  public:
    // Move image's RefcountedEvent to the garbage collector
    void collect(ImageHelper *image);
    // Move event to the garbage collector
    void collect(RefCountedEvent *event);
    // Move the vector of events to the garbage collector
    void collect(std::vector<RefCountedEvent> *events);
    // Release all collected RefcountedEvents to the renderer's garbage list and track completion
    // with queueSerial
    void release(Renderer *renderer, const QueueSerial &queueSerial);
