...
class EventBarrier : angle::NonCopyable
{
  public:
    ...
    void mergeMemoryEvent(VkPipelineStageFlags srcStageMask,
...
class EventBarrierArray final
{
  public:
    ...
    void mergeMemoryEvent(PipelineStage stageIndex,
    ...
    {
        ASSERT(event.valid());
        mBarriers[stageIndex].mergeMemoryEvent(srcStageMask, dstStageMask, srcAccess, dstAccess,
                                               event.getEvent().getHandle());
        mBarrierMask.set(stageIndex);
    }

    void mergeImageEvent(PipelineStage stageIndex,
                         VkPipelineStageFlags srcStageMask,
                         VkPipelineStageFlags dstStageMask,
                         const RefCountedEvent &event,
                         const VkImageMemoryBarrier &imageMemoryBarrier)
    {
        ASSERT(event.valid());
        mBarriers[stageIndex].mergeImageEvent(srcStageMask, dstStageMask,
                                              event.getEvent().getHandle(), imageMemoryBarrier);
        mBarrierMask.set(stageIndex);
    }
  ...
  private:
    angle::PackedEnumMap<PipelineStage, EventBarrier> mBarriers;
