...
angle::Result DuplicateZirconVmo(zx_handle_t handle, zx_handle_t *duplicate)
{
#if defined(ANGLE_PLATFORM_FUCHSIA)
    zx_status_t status = zx_handle_duplicate(handle, ZX_RIGHT_SAME_RIGHTS, duplicate);
    if (status != ZX_OK)
    {
        ERR() << "failed to duplicate vmo: zx_handle_duplicate: " << zx_status_get_string(status);
...
VkExternalMemoryHandleTypeFlagBits ToVulkanHandleType(gl::HandleType handleType)
{
    switch (handleType)
    {
        ...
        case gl::HandleType::InvalidEnum:
                                                 ...
                                                 GLuint handle)
{
    ...
    {
        case gl::HandleType::ZirconVmo:
            return importZirconVmo(context, size, handle);
...
angle::Result MemoryObjectVk::importZirconVmo(gl::Context *context, GLuint64 size, GLuint handle)
{
    ASSERT(mHandleType == gl::HandleType::InvalidEnum);
                                          ...
                                          vk::ImageHelper *image)
{
    ...
    switch (mHandleType)
    {
        ...
        case gl::HandleType::ZirconVmo:
            ...
            ANGLE_TRY(DuplicateZirconVmo(mZirconHandle, &importMemoryZirconHandleInfo.handle));
