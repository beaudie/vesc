...
std::array<uint32_t, 3> CLDeviceVk::selectWorkGroupSize(
    ...
    const std::array<uint32_t, 3> &globalSize) const
{
    ...
    auto props = mRenderer->getPhysicalDeviceProperties();
    // Cap the total work-group size to the Vulkan device's limit.
    // TODO: select total local size limit
    ...
    do
    {
        ...
        // Alternate between increasing the X/Y/Z.
        for (cl_uint i = 0; i < workDim; i++)
        {
            // Double the dimension if we can.
            std::array<uint32_t, 3> newLocalSize = localSize;
            newLocalSize[i] *= 2;

            // TODO: Consider non-uniform sizes if supported
            if (globalSize[i] % newLocalSize[i] == 0 &&
