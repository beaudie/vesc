...
{
    ShaderAndSerial &shader = mShaders[static_cast<size_t>(shaderID)];
    *shaderOut              = &shader;
    ...
    createInfo.flags    = 0;
    createInfo.codeSize = GetInternalShaderCodeSize(shaderID);
    createInfo.pCode    = GetInternalShaderCode(shaderID);
