                                    ...
                                    ClearTextureParameters &params)
{
    ...
    uint32_t bufferSize = roundUpPow2<uint32_t>(bufferTexels * texelSize, 4);
    ...
    shaderParams.maxThreadId        = (params.dataSize < 4) ? bufferSize / 4 : bufferTexels;
    ...
    switch (params.dataSize)
    {
        case 1:
        {
            ...
            uint32_t data        = shaderParams.data[0] & 0xFF;
            shaderParams.data[0] = data | (data << 8) | (data << 16) | (data << 24);
            break;
        }
        case 2:
        {
            // One uint32 is used as input, which can hold two 2-byte values.
            uint32_t data        = shaderParams.data[0] & 0xFFFF;
            shaderParams.data[0] = data | (data << 16);
            break;
        }
        case 3:
        {
            ...
            // its ID.
            uint32_t data        = shaderParams.data[0] & 0xFFFFFF;
            shaderParams.data[0] = data | (data << 24);
            shaderParams.data[1] = (data >> 8) | (data << 16);
            shaderParams.data[2] = (data >> 16) | (data << 8);
            break;
        }
        case 4:
        {
            // One uint32 is used as input, which can hold one 4-byte value (unless DS).
            if (shouldClearDS)
            {
                ...
                uint32_t depthData   = shaderParams.data[0] & 0xFFFFFF;
                uint32_t stencilData = shaderParams.data[0] >> 24;
                shaderParams.data[0] = depthData;
                shaderParams.data[1] =
                    stencilData | (stencilData << 8) | (stencilData << 16) | (stencilData << 24);
            }
            break;
        }
        case 6:
        {
            ...
            // RRGG, BBRR, GGBB. These values can be loaded into 3 of the uint32 inputs and copied.
            UNIMPLEMENTED();
            break;
        }
        case 8:
        {
            if (shouldClearDS)
            {
                ...
                uint32_t depthData   = shaderParams.data[0];
                uint32_t stencilData = shaderParams.data[1] & 0xFF;
                shaderParams.data[0] = depthData;
                shaderParams.data[1] =
                    stencilData | (stencilData << 8) | (stencilData << 16) | (stencilData << 24);
            }
    ...
    if (shouldClearDS)
    {
        ...
        VkBufferImageCopy regionDepth           = region;
        ...
        ASSERT(format.depthBits % 8 == 0);
        VkBufferImageCopy regionStencil = region;
