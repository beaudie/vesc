                                              ...
                                              const bool isMipmapFiltered)
{
    ...
    barrier.oldLayout                       = image->getCurrentLayout(renderer);
    ...
    srcBarrier.newLayout                   = VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL_KHR;
    srcBarrier.dstAccessMask               = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT;
    ...
    {
        // Transition srcLevel of all layers to color attachment read layout
        outsideCommandBuffer = nullptr;
        ANGLE_TRY(contextVk->getOutsideRenderPassCommandBuffer({}, &outsideCommandBuffer));

        srcBarrier.subresourceRange.baseMipLevel = srcLevelVk.get();
        outsideCommandBuffer->imageBarrier(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
    ...
    // Close the render pass before layout transition.
    ANGLE_TRY(contextVk->flushCommandsAndEndRenderPass(RenderPassClosureReason::PrepareForBlit));

    // Transition the last mipLevel to color attachment read layout
    outsideCommandBuffer = nullptr;
    ANGLE_TRY(contextVk->getOutsideRenderPassCommandBuffer({}, &outsideCommandBuffer));

    srcBarrier.subresourceRange.baseMipLevel = (baseLevelVK + (levelCount - 1)).get();
    outsideCommandBuffer->imageBarrier(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                       VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, srcBarrier);
    ANGLE_TRY(contextVk->flushAndSubmitOutsideRenderPassCommands());
