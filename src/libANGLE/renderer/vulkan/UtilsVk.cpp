                                       ...
                                       const BlitResolveParameters &params)
{
    ...
    //
    // TODO(ianelliott): Address pre-rotation for resolve cases.  In particular, the
    // shaderParams.rotateXY value seems to be ignored in the custom shaders.
    ...
    switch (params.rotation)
    {
        ...
        case SurfaceRotation::Rotated180Degrees:
            if (isResolve)
            {
                shaderParams.offset.resolve[0] += params.blitArea.width;
                shaderParams.offset.resolve[1] += params.blitArea.height;
            }
            else
            {
