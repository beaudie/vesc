                                    ...
                                    vk::CommandBuffer *commandBuffer)
{
    ...
    fprintf(stderr, "Calling into %s\n", isCompute ? "COMPUTE" : "GRAPHICS");
...
angle::Result UtilsVk::clearImage(ContextVk *context,
                                  FramebufferVk *framebuffer,
                                  const ClearImageParameters &params)
{
    RendererVk *renderer = context->getRenderer();

    ANGLE_TRY(ensureImageClearResourcesInitialized(context));
    ...
    ANGLE_TRY(shaderLibrary.getImageClear_frag(context, 0, &fragmentShader));

    ANGLE_TRY(setupProgram(context, Function::ImageClear, fragmentShader, vertexShader,
