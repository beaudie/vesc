...
uint32_t GetResolveColorFlags(const vk::Format &format)
{
    ...
    flags |= angleFormat.isInt()
                 ? ResolveColor_frag::kIsInt
                 : angleFormat.isUint() ? ResolveColor_frag::kIsUint : ResolveColor_frag::kIsFloat;
...
angle::Result UtilsVk::ensureResolveColorResourcesInitialized(vk::Context *context)
{
                                           ...
                                           const DepthStencilResolveParameters &params)
{
    ...
    uint32_t writeInfoOffset = params.resolveDepth ? 0 : 1;
    uint32_t writeInfoCount  = params.resolveDepth + params.resolveStencil;

    fprintf(stderr, "Resolving depth (%p)? %d, stencil (%p)? %d\n", srcDepthView->getHandle(),
            params.resolveDepth, srcStencilView->getHandle(), params.resolveStencil);
