...
constexpr uint32_t kOverlayCullCulledItemsBinding            = 0;
constexpr uint32_t kOverlayCullItemCoordsBinding             = 1;
constexpr uint32_t kOverlayDrawOutputBinding                 = 0;
constexpr uint32_t kOverlayDrawTextItemsBinding              = 1;
constexpr uint32_t kOverlayDrawGraphItemsBinding             = 2;
constexpr uint32_t kOverlayDrawCulledItemsBinding            = 3;
...
angle::Result UtilsVk::cullOverlayItems(ContextVk *contextVk,
                                        vk::BufferHelper *enabledItemsBuffer,
                                        vk::ImageHelper *dest,
                                        const vk::ImageView *destView,
                                        const OverlayCullParameters &params)
{
    ...
    enabledItemsBuffer->onRead(dest, VK_ACCESS_SHADER_READ_BIT);
    ...
    VkDescriptorBufferInfo bufferInfo = {};
    bufferInfo.buffer                 = enabledItemsBuffer->getBuffer().getHandle();
    bufferInfo.offset                 = 0;
    bufferInfo.range                  = VK_WHOLE_SIZE;
    ...
    writeInfos[0].sType                = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeInfos[0].dstSet               = descriptorSet;
    writeInfos[0].dstBinding           = kOverlayCullCulledItemsBinding;
    writeInfos[0].descriptorCount      = 1;
    ...
    writeInfos[1].sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeInfos[1].dstSet          = descriptorSet;
    writeInfos[1].dstBinding      = kOverlayCullItemCoordsBinding;
    writeInfos[1].descriptorCount = 1;
...
angle::Result UtilsVk::drawOverlay(ContextVk *contextVk,
                                   vk::BufferHelper *textItemsBuffer,
                                   vk::BufferHelper *graphItemsBuffer,
                                   vk::ImageHelper *font,
                                   const vk::ImageView *fontView,
                                   vk::ImageHelper *culledItems,
                                   const vk::ImageView *culledItemsView,
                                   vk::ImageHelper *dest,
                                   const vk::ImageView *destView,
                                   const OverlayDrawParameters &params)
{
    ...
    culledItems->addReadDependency(dest);
    font->addReadDependency(dest);
    textItemsBuffer->onRead(dest, VK_ACCESS_SHADER_READ_BIT);
    graphItemsBuffer->onRead(dest, VK_ACCESS_SHADER_READ_BIT);

    VkDescriptorImageInfo imageInfos[3] = {};
    imageInfos[0].imageView             = destView->getHandle();
    imageInfos[0].imageLayout           = dest->getCurrentLayout();

    imageInfos[1].imageView   = culledItemsView->getHandle();
    imageInfos[1].imageLayout = culledItems->getCurrentLayout();

    imageInfos[2].imageView   = fontView->getHandle();
    imageInfos[2].imageLayout = font->getCurrentLayout();

    VkDescriptorBufferInfo bufferInfos[2] = {};
    bufferInfos[0].buffer                 = textItemsBuffer->getBuffer().getHandle();
    bufferInfos[0].offset                 = 0;
    bufferInfos[0].range                  = VK_WHOLE_SIZE;

    bufferInfos[1].buffer = graphItemsBuffer->getBuffer().getHandle();
    ...
    writeInfos[1].sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeInfos[1].dstSet          = descriptorSet;
    writeInfos[1].dstBinding      = kOverlayDrawCulledItemsBinding;
    writeInfos[1].descriptorCount = 1;
    writeInfos[1].descriptorType  = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    writeInfos[1].pImageInfo      = &imageInfos[1];
    ...
    writeInfos[3].dstBinding      = kOverlayDrawTextItemsBinding;
    ...
    writeInfos[4].dstBinding  = kOverlayDrawGraphItemsBinding;
    ...
    // Every pixel of culledItems corresponds to one workgroup, so we can use that as dispatch size.
    commandBuffer->dispatch(culledItems->getExtents().width, culledItems->getExtents().height, 1);
    descriptorPoolBinding.reset();
