                                 ...
                                 const CopyImageParameters &params)
{
    ...
    constexpr uint32_t kInvalidFlag = static_cast<uint32_t>(-1);
    uint32_t flags                  = kInvalidFlag;
    if (isYUV)
    {
        ASSERT(src->getType() == VK_IMAGE_TYPE_2D);
        flags = ImageCopyFloat_frag::kSrcIsYUV;
    }
    else if (isSrcMultisampled)
    {
        flags = ImageCopyFloat_frag::kSrcIs2DMS;
    }
    else
    {
        flags = GetImageCopyFlags(srcIntendedFormat, dstIntendedFormat);
    ...
    if (isYUV)
    {
        ANGLE_TRY(shaderLibrary.getImageCopyFloat_frag(contextVk, flags, &fragmentShader));
