...
// avoid needing a separate descriptor set.
//
//     #version 450 core
    ...
    bool unresolveStencil)
{
    ...
    InsertDepthStencilDecorations(unresolveDepth, unresolveStencil, &colorInputIndexStart,
                                  &colorBindingIndexStart, &code);
                                 ...
                                 const UnresolveParameters &params)
{
    ...
    if (params.unresolveStencil)
    {
        ...
        inputImageInfo[inputBindingIndex].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        ++inputBindingIndex;
    }
    if (params.unresolveDepth)
    {
    ...
    for (uint32_t attachmentIndex = 0; attachmentIndex < colorAttachmentCount; ++attachmentIndex)
    {
        inputImageInfo[inputBindingIndex].imageView   = colorSrcView[attachmentIndex]->getHandle();
        inputImageInfo[inputBindingIndex].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        ++inputBindingIndex;
    }
    ...
    ANGLE_TRY(GetUnresolveFrag(contextVk, colorAttachmentCount, colorAttachmentTypes,
                               params.unresolveDepth, unresolveStencilWithShaderExport,
                               fragmentShader));
    ...
    if (colorAttachmentCount > 0 || params.unresolveDepth || unresolveStencilWithShaderExport)
    {
        commandBuffer->draw(3, 0);
    }
    ...
    if (params.unresolveStencil && !unresolveStencilWithShaderExport)
    {
        ...
        // Note that the descriptor set that was set up for unresolve has conveniently put the
        // stencil image at the right binding (binding 0) so the same descriptor set can be used for
        // the stencil export calls.
        ANGLE_TRY(setupGraphicsProgram(contextVk, Function::ExportStencil, vertexShader,
                                       ...
                                       descriptorSet, nullptr, 0, commandBuffer));
