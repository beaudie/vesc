...
void CalculateBlitOffset(const UtilsVk::BlitResolveParameters &params, float offset[2])
{
    ...
    INFO(
        "%s(): (dstOffset[0] = %3d * stretch[0] = %5f) - "
        "(srcOffset[0] = %4d) * (srcOffsetFactorX = %2d)",
        __FUNCTION__, params.dstOffset[0], params.stretch[0], params.srcOffset[0],
        srcOffsetFactorX);
    INFO(
        "%s(): (dstOffset[1] = %3d * stretch[1] = %5f) - "
        "(srcOffset[1] = %4d) * (srcOffsetFactorY = %2d)",
        __FUNCTION__, params.dstOffset[1], params.stretch[1], params.srcOffset[1],
        srcOffsetFactorY);
...
void CalculateResolveOffset(const UtilsVk::BlitResolveParameters &params, int32_t offset[2])
{
    ...
    INFO("%s(): dstOffset[0] = %3d - (srcOffset[0] = %4d) * (srcOffsetFactorX = %2d)", __FUNCTION__,
         params.dstOffset[0], params.srcOffset[0], srcOffsetFactorX);
    INFO("%s(): dstOffset[1] = %3d - (srcOffset[1] = %4d) * (srcOffsetFactorY = %2d)", __FUNCTION__,
         params.dstOffset[1], params.srcOffset[1], srcOffsetFactorY);
                                 ...
                                 const CopyImageParameters &params)
{
    ...
    shaderParams.rotateXY     = 0;
    INFO(
        "%s(): Incoming params:\t srcOffset(%d, %d), dstOffset(%d, %d), Extents(%d, %d), "
        "srcFlipY=%5s, destFlipY=%5s",
        __FUNCTION__, params.srcOffset[0], params.srcOffset[1], params.dstOffset[0],
        params.dstOffset[1], params.srcExtents[0], params.srcExtents[1],
        params.srcFlipY ? "true" : "false", params.dstFlipY ? "true" : "false");
    ...
    INFO(
        "%s(): Post-rotation:\t\t srcOffset(%d, %d), dstOffset(%d, %d), Extents(%d, %d), "
        "flipX=%5s, flipY=%5s",
        __FUNCTION__, shaderParams.srcOffset[0], shaderParams.srcOffset[1],
        shaderParams.dstOffset[0], shaderParams.dstOffset[1], params.srcExtents[0],
