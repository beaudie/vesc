                           ...
                           SpirvBlob *blobOut)
{
    ...
    spirv::WriteTypeImage(blobOut, imageTypeId, baseId, spv::DimSubpassData,
                          ...
                          spirv::LiteralInteger(0), spirv::LiteralInteger(2),
    ...
    bool unresolveStencil)
{
    ...
    //  - Magic number
    //  - Version (1.0)
    //  - ANGLE's Generator number (24 for tool id (higher 16 bits), 0 for tool version (lower 16
    //  bits))
    //  - Bound (kIdCount)
    //  - 0 (reserved)
...
{
    gl::DrawBuffersArray<UnresolveColorAttachmentType> attachmentTypes;

    bool unresolveDepthValues[]   = {false, true};
    bool unresolveStencilValues[] = {false, true};

    for (bool unresolveDepth : unresolveDepthValues)
    {
        for (bool unresolveStencil : unresolveStencilValues)
        {
            attachmentTypes[0] = kUnresolveTypeFloat;
            std::vector<uint32_t> shaderCode =
                unresolve::MakeFragShader(1, attachmentTypes, unresolveDepth, unresolveStencil);
            ...
            shaderCode =
                unresolve::MakeFragShader(3, attachmentTypes, unresolveDepth, unresolveStencil);
            ASSERT(spirv::Validate(shaderCode));

            attachmentTypes[0] = kUnresolveTypeSint;
            attachmentTypes[1] = kUnresolveTypeFloat;
            attachmentTypes[2] = kUnresolveTypeUint;
            attachmentTypes[3] = kUnresolveTypeFloat;
            attachmentTypes[4] = kUnresolveTypeFloat;
            attachmentTypes[5] = kUnresolveTypeSint;
            attachmentTypes[6] = kUnresolveTypeUint;
            attachmentTypes[7] = kUnresolveTypeUint;
            shaderCode =
                unresolve::MakeFragShader(8, attachmentTypes, unresolveDepth, unresolveStencil);
            ASSERT(spirv::Validate(shaderCode));
        }
    }
}
