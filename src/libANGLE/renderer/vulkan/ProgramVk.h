...
class ProgramVk : public ProgramImpl
{
  ...
  private:
    ...
    ANGLE_INLINE angle::Result initProgram(ContextVk *contextVk,
                                           ProgramInfo::TransformOptionBits option_bits,
    ...
    {
        ProgramInfo::TransformOptionBits option_bits;
        option_bits[ProgramInfo::TransformOption::EnableLineRasterEmulation] =
            UseLineRaster(contextVk, mode);
        option_bits[ProgramInfo::TransformOption::RemoveEarlyFragmentTestsOptimization] =
            mState.hasEarlyFragmentTestsOptimization() &&
            !contextVk->getState().isEarlyFragmentTestsOptimizationAllowed();

        ProgramInfo *programInfo = &mExecutable.mProgramInfos[option_bits.to_ulong()];

        return initProgram(contextVk, option_bits, programInfo, shaderProgramOut);
    }
    ...
    {
        ProgramInfo::TransformOptionBits option_bits;
        return initProgram(contextVk, option_bits, &mExecutable.mProgramInfos[0], shaderProgramOut);
    }
