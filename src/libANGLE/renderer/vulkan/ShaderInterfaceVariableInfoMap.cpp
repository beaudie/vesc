...
void ShaderInterfaceVariableInfoMap::clear()
{
    mData.clear();
    mXFBData.clear();
    mPodStruct.xfbInfoCount = 0;
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        mIdToIndexMap[shaderType].clear();
    }
    std::fill(mPodStruct.inputPerVertexActiveMembers.begin(),
              mPodStruct.inputPerVertexActiveMembers.end(), gl::PerVertexMemberBitSet{});
    std::fill(mPodStruct.outputPerVertexActiveMembers.begin(),
              mPodStruct.outputPerVertexActiveMembers.end(), gl::PerVertexMemberBitSet{});
}
...
void ShaderInterfaceVariableInfoMap::save(gl::BinaryOutputStream *stream)
{
    ...
    stream->writeInt(mPodStruct.xfbInfoCount);
...
void ShaderInterfaceVariableInfoMap::load(gl::BinaryInputStream *stream)
{
    ...
    mPodStruct.xfbInfoCount = stream->readInt<size_t>();
    ASSERT(mPodStruct.xfbInfoCount <= mData.size());
    if (mPodStruct.xfbInfoCount > 0)
    {
        mXFBData.resize(mData.size());
        for (size_t i = 0; i < mPodStruct.xfbInfoCount; ++i)
        {
