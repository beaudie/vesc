                                           ...
                                           void *pMemory)
{
    ...
    WARN() << "onAlloc(): " << size << " | " << alignment << " -> " << allocationScope;
}
...
{
    //    std::unique_lock<std::mutex> lock(mMutex);
    // TODO: This should also be tracked like the other two. And then the mutex should be enabled.
    WARN() << "onRealloc(): " << size << " | " << alignment << " -> " << allocationScope;
}
...
void MemoryAllocationCallback::recordFree(void *pMemory)
{
        ...
        mMemoryPropertyMap.find(pMemory) != mMemoryPropertyMap.end())
    {
        ...
        mSystemAllocationSize[allocationScope] -= size;
        WARN() << "onFree(): " << size << " -> " << allocationScope;
                                               ...
                                               size_t alignment)
{
    ...
    return std::realloc(pOriginal, alignedSize);
}
