...
void CBHRingAllocatorHelper::resetAllocator()
{
...
void CBHRingAllocatorHelper::attachAllocator(CommandsRingAllocator *allocator)
{
    ASSERT(allocator);
    ASSERT(!mAllocator);
...
CommandsRingAllocator *CBHRingAllocatorHelper::detachAllocator(bool isCommandBufferEmpty)
{
...
void CommandRingAllocatorHelper::attachAllocator(CommandsRingAllocator *source)
{
    ASSERT(source);
...
void CommandRingAllocatorHelper::detachAllocator(CommandsRingAllocator *destination)
{
    ASSERT(destination);
...
uint32_t CommandRingAllocatorHelper::getCommandSize() const
{
...
void CommandRingAllocatorHelper::pushNewCommandBlock(uint8_t *block)
{
    mLastCommandBlock = block;
    mCommandBuffer->pushToCommands(block);
}

void CommandRingAllocatorHelper::getMemoryUsageStats(size_t *usedMemoryOut,
