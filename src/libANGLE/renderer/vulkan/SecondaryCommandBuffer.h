...
#if ANGLE_ENABLE_VULKAN_SHARED_RING_BUFFER_CMD_ALLOC
using CommandBufferHelperAllocator = SharedRingAllocatorHelper;
using CommandBufferAllocator       = RingAllocatorHelper;
using CommandsAllocator            = CommandsRingAllocator;
#else
using CommandBufferHelperAllocator = SecondaryPoolAllocatorHelper;
using CommandBufferAllocator       = CommandPoolAllocatorHelper;
using CommandsAllocator            = CommandsPoolAllocator;
#endif
...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  public:
    ...
    angle::Result initialize(CommandsAllocator *allocator)
    {
        return mAllocatorManager.initialize(allocator);
    }

    // Initialize the SecondaryCommandBuffer by setting the allocator it will use
    void attachAllocator(vk::CommandsAllocator *source)
    {
        mAllocatorManager.attachAllocator(source);
    }

    void detachAllocator(vk::CommandsAllocator *destination)
    {
  ...
  private:
                                         ...
                                         uint8_t **variableDataPtr)
    {
        ...
        bool usesFixedAllocationSize = mAllocatorManager.onNewVariableSizedCommand(
            requiredSize, allocationSize, &commandPtr, &header);
        StructType *const result = commonInit<StructType>(cmdID, allocationSize, header);
        *variableDataPtr         = (usesFixedAllocationSize)
    ...
    ANGLE_INLINE StructType *initCommand(CommandID cmdID)
    {
        ...
        uint8_t *commandPtr       = nullptr;
        uint8_t *header           = nullptr;
        mAllocatorManager.onNewCommand(requiredSize, allocationSize, &commandPtr, &header);
    ...
    CommandBufferAllocator mAllocatorManager;
...
{
    mAllocatorManager.setCommandBuffer(this);
}

ANGLE_INLINE SecondaryCommandBuffer::~SecondaryCommandBuffer()
{
    mAllocatorManager.resetCommandBuffer();
}
