...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  ...
  private:
    ...
    // Copy sizeInBytes data from paramData to writePointer. This is useful for the last block of
    // ptr data copied for this cmd as it skips the size increment of non-final version above.
    template <class PtrType>
    ANGLE_INLINE void storeFinalPointerParameter(uint8_t *writePointer,
                                                 const PtrType *paramData,
                                                 size_t sizeInBytes)
    {
        memcpy(writePointer, paramData, sizeInBytes);
    ...
    const uint32_t *dynamicOffsets)
{
    ...
    storeFinalPointerParameter(writePtr, dynamicOffsets, offsetSize);
}
    ...
    const VkDescriptorSet *descriptorSets)
{
    ...
    paramStruct->layout = layout.getHandle();
    // Copy variable sized data
    storeFinalPointerParameter(writePtr, descriptorSets, sizeof(VkDescriptorSet));
}
                                                            ...
                                                            const VkDeviceSize *offsets)
{
    ...
    storeFinalPointerParameter(writePtr, offsets, offsetsSize);
}
                                                           ...
                                                           const VkClearRect *rects)
{
    ...
    // Copy variable sized data
    storeFinalPointerParameter(writePtr, attachments, attachSize);
}
                                                     ...
                                                     const VkBufferCopy *regions)
{
    ...
    // Copy variable sized data
    storeFinalPointerParameter(writePtr, regions, regionSize);
}
    ...
    const VkImageMemoryBarrier *imageMemoryBarriers)
{
    ...
    storeFinalPointerParameter(writePtr, imageMemoryBarriers, imgBarrierSize);
}
                                                        ...
                                                        const void *data)
{
    ...
    // Copy variable sized data
    storeFinalPointerParameter(writePtr, data, static_cast<size_t>(size));
}
