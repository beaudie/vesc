...
VERIFY_8_BYTE_ALIGNMENT(ResetEvent2Params)
...
struct SetEvent2Params
{
    CommandHeader header;
    uint32_t memoryBarrier2Count;
    uint32_t bufferMemoryBarrier2Count;
    uint32_t imageMemoryBarrier2Count;
    VkEvent event;
    VkDependencyFlags dependencyFlags;
    uint32_t padding;
};
VERIFY_8_BYTE_ALIGNMENT(SetEvent2Params)
...
struct WaitEvents2Params
{
    CommandHeader header;

    uint32_t eventCount;
};
VERIFY_8_BYTE_ALIGNMENT(WaitEvents2Params)
...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  public:
    ...
    void resetEvent2(VkEvent event, VkPipelineStageFlags2 stageMask2);
...
ANGLE_INLINE void SecondaryCommandBuffer::resetEvent2(VkEvent event,
                                                      VkPipelineStageFlags2 stageMask2)
{
    ResetEvent2Params *paramStruct = initCommand<ResetEvent2Params>(CommandID::ResetEvent2);
    paramStruct->event             = event;
    paramStruct->stageMask         = stageMask2;
}
...
ANGLE_INLINE void SecondaryCommandBuffer::setEvent2(
    VkEvent event,
    VkDependencyFlags dependencyFlags,
    uint32_t memoryBarrier2Count,
    const VkMemoryBarrier2 *memoryBarriers2,
    ...
    const VkImageMemoryBarrier2 *imageMemoryBarriers2)
{
    ASSERT(bufferMemoryBarrier2Count == 0);
    ASSERT(bufferMemoryBarriers2 == nullptr);

    uint8_t *writePtr;
    const ArrayParamSize memBarrier2Size =
        calculateArrayParameterSize<VkMemoryBarrier2>(memoryBarrier2Count);
    const ArrayParamSize imgBarrier2Size =
        calculateArrayParameterSize<VkImageMemoryBarrier2>(imageMemoryBarrier2Count);

    SetEvent2Params *paramStruct = initCommand<SetEvent2Params>(
        CommandID::SetEvent2, memBarrier2Size.allocateBytes + imgBarrier2Size.allocateBytes,
        &writePtr);

    paramStruct->event                    = event;
    paramStruct->dependencyFlags          = dependencyFlags;
    paramStruct->memoryBarrier2Count      = memoryBarrier2Count;
    paramStruct->imageMemoryBarrier2Count = imageMemoryBarrier2Count;
    if (memoryBarrier2Count > 0)
    {
        writePtr = storeArrayParameter(writePtr, memoryBarriers2, memBarrier2Size);
    }
    if (imageMemoryBarrier2Count > 0)
    {
        storeArrayParameter(writePtr, imageMemoryBarriers2, imgBarrier2Size);
    }
}
...
ANGLE_INLINE void SecondaryCommandBuffer::waitEvents2(uint32_t eventCount,
                                                      const VkEvent *events,
                                                      const VkDependencyInfo *dependencyInfos)
{
    uint8_t *writePtr;
    const ArrayParamSize eventSize = calculateArrayParameterSize<VkEvent>(eventCount);
    const ArrayParamSize dependencyInfoSize =
        calculateArrayParameterSize<VkDependencyInfo>(eventCount);
    WaitEvents2Params *paramStruct = initCommand<WaitEvents2Params>(
        CommandID::WaitEvents2, eventSize.allocateBytes + dependencyInfoSize.allocateBytes,
        &writePtr);
    paramStruct->eventCount = eventCount;
    writePtr                = storeArrayParameter(writePtr, events, eventSize);
    storeArrayParameter(writePtr, dependencyInfos, dependencyInfoSize);
}
