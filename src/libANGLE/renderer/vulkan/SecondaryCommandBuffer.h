...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  ...
  private:
                                         ...
                                         uint8_t **variableDataPtr)
    {
        ...
        // Also make sure that the required size is 4-byte aligned
        ...
        if (mCurrentBytesRemaining < requiredSize)
        {
            ...
            {
                // Allocate a multiple of block size large enough to fit requiredSize
                uint32_t numBlocks = 2;
                while (requiredSize > (kBlockSize * numBlocks))
                    numBlocks++;
                allocateNewBlock(kBlockSize * numBlocks);
            }
        }
