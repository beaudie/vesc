...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  public:
    ...
    void pipelineBarrier2(const VkDependencyInfo dependencyInfo);
    ...
    void setEvent2(VkEvent event, const VkDependencyInfo *dependencyInfo);
...
ANGLE_INLINE void SecondaryCommandBuffer::pipelineBarrier2(const VkDependencyInfo dependencyInfo)
{
    // Other than for QFOT (where bufferBarrier() is used), ANGLE doesn't use buffer barriers.
    // Memory barriers are used instead.
    ASSERT(dependencyInfo.bufferMemoryBarrierCount == 0);
    ASSERT(dependencyInfo.pBufferMemoryBarriers == nullptr);

    PipelineBarrierParams2 *paramStruct =
        initCommand<PipelineBarrierParams2>(CommandID::PipelineBarrier2);
    paramStruct->dependencyInfo = dependencyInfo;
}
...
ANGLE_INLINE void SecondaryCommandBuffer::setEvent2(VkEvent event,
                                                    const VkDependencyInfo *dependencyInfo)
{
    SetEvent2Params *paramStruct = initCommand<SetEvent2Params>(CommandID::SetEvent2);
    paramStruct->event           = event;
    paramStruct->dependencyInfo  = dependencyInfo;
}
