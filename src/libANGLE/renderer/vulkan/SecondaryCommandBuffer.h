...
using RenderPassCommandsAllocatorType = RenderPassCommandsRingAllocatorType;
#else
using SharedAllocatorManager          = SharedPoolAllocatorManager;
using CommandBufferAllocatorManager   = PoolAllocatorManager;
using RenderPassCommandsAllocatorType = RenderPassCommandsPoolAllocatorType;
#endif
...
class SecondaryCommandBuffer final : angle::NonCopyable
{
  public:
    ...
    bool isCommandEmpty() { return mCommands.empty(); }
    void clearCommand() { mCommands.clear(); }
    void appendToCommand(uint8_t *command)
    {
  ...
  private:
    ...
    ANGLE_INLINE StructType *commonInit(CommandID cmdID, size_t allocationSize)
    {
        ...
        auto *header = reinterpret_cast<CommandHeader *>(
            mAllocatorManager.updateHeaderAndAllocatorParams(allocationSize));
                                         ...
                                         uint8_t **variableDataPtr)
    {
        ...
        if (mAllocatorManager.checkSizeToAllocateNewBlock(requiredSize, &commandPtr))
        {
            *variableDataPtr = Offset<uint8_t>(commandPtr, fixedAllocationSize);
    ...
    ANGLE_INLINE StructType *initCommand(CommandID cmdID)
    {
        ...
        const size_t requiredSize = allocationSize + sizeof(CommandHeader);
        uint8_t *commandPtr       = nullptr;
        mAllocatorManager.checkSizeToAllocateNewBlockWithoutVariable(requiredSize, &commandPtr);
...
{
    mAllocatorManager.setCommands(this);
}
