...
{
    mMipLodBias = 0.0f;
    for (size_t i = 0; i < featuresVk.forceTextureLODOffset.size(); i++)
    {
        if (featuresVk.forceTextureLODOffset[i].enabled)
        {
            mMipLodBias = static_cast<float>(i + 1);
        }
    }
    ...
    if (featuresVk.forcePointFiltering.enabled)
    {
        minFilter = gl::ConvertToPointFilterMode(minFilter);
        magFilter = gl::ConvertToPointFilterMode(magFilter);
    }

    SetBitField(mMagFilter, gl_vk::GetFilter(minFilter));
    SetBitField(mMinFilter, gl_vk::GetFilter(magFilter));
