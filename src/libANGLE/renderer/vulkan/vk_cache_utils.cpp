                         ...
                         DynamicRenderingInfo *infoOut)
{
    ...
    if (desc.viewCount() > 0)
    {
        infoOut->renderingInfo.viewMask = angle::BitMask<uint32_t>(desc.viewCount());
    }
    ...
    for (uint32_t colorIndexGL = 0; colorIndexGL < desc.colorAttachmentRange(); ++colorIndexGL)
    {
        ...
        {
            // TODO: Dynamic rendering doesn't do automatic transition, so when starting the render
            // pass the images should be put in the right layout beforehand.
