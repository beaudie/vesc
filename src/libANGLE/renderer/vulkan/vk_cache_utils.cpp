                                             ...
                                             vk::RenderPass **renderPassOut)
{
    ...
    if (desc.hasDepthStencilAttachment())
    {
        // Vulkan spec says image layout are not counted toward render pass compatibility:
        // "Two render passes are compatible if their corresponding color, input, resolve, and
        // depth/stencil attachment references are compatible and if they are otherwise identical
        // except for: Initial and final image layout in attachment descriptions; Image layout in
        // attachment references". We pick the most used layout here since it doesn't matter.
