...
constexpr bool kDumpPipelineCacheGraphFull = false;
...
void UnpackPipelineState(const vk::GraphicsPipelineDesc &state, UnpackedPipelineState *valuesOut)
{
    ...
    uint32_t *vaStrides    = &(*valuesOut)[PipelineState::VertexAttribFormat];
    ...
    if (inputAndRaster.misc.topology == VK_PRIMITIVE_TOPOLOGY_PATCH_LIST ||
        kDumpPipelineCacheGraphFull)
    {
    ...
    if (inputAndRaster.bits.sampleShadingEnable || kDumpPipelineCacheGraphFull)
    {
        (*valuesOut)[PipelineState::MinSampleShading] = inputAndRaster.misc.minSampleShading;
    }
    if (inputAndRaster.bits.rasterizationSamples > 1 || kDumpPipelineCacheGraphFull)
    {
        (*valuesOut)[PipelineState::RasterizationSamples] =
            inputAndRaster.bits.rasterizationSamples;
    ...
    if (dynamicState.ds1And2.depthTest || kDumpPipelineCacheGraphFull)
    {
    ...
    if (dynamicState.ds1And2.stencilTest || kDumpPipelineCacheGraphFull)
    {
    ...
    for (uint32_t colorIndex = 0; colorIndex < gl::IMPLEMENTATION_MAX_DRAW_BUFFERS; ++colorIndex)
    {
        ...
        if (blendEnableMask[colorIndex] || kDumpPipelineCacheGraphFull)
        {
            srcColorBlendFactors[colorIndex] =
                colorBlend.attachments[colorIndex].srcColorBlendFactor;
            dstColorBlendFactors[colorIndex] =
                colorBlend.attachments[colorIndex].dstColorBlendFactor;
            colorBlendOps[colorIndex] = colorBlend.attachments[colorIndex].colorBlendOp;
            srcAlphaBlendFactors[colorIndex] =
                colorBlend.attachments[colorIndex].srcAlphaBlendFactor;
            dstAlphaBlendFactors[colorIndex] =
                colorBlend.attachments[colorIndex].dstAlphaBlendFactor;
...
void OutputPipelineState(std::ostream &out, size_t stateIndex, uint32_t state)
{
    ...
    PipelineState pipelineState = PipelineState::InvalidEnum;
    bool isRanged               = false;
    size_t subIndex             = 0;
    ...
    for (PipelineState ps : kRangedStates)
    {
        ...
        if (IsPipelineState(stateIndex, ps, range))
        {
            pipelineState = ps;
            isRanged      = true;
            subIndex      = GetPipelineStateSubIndex(stateIndex, ps);
            break;
        }
    }
    ...
    if (!isRanged)
    {
        pipelineState = static_cast<PipelineState>(stateIndex);
    }
    ...
    constexpr angle::PackedEnumMap<PipelineState, const char *> kStateNames = {{
        ...
        {PipelineState::BlendEnableMask, "blend"},
...
void OutputAllPipelineState(std::ostream &out,
    ...
    const std::unordered_map<vk::GraphicsPipelineDesc, vk::PipelineHelper> &cache)
{
    ...
    out << "  label=\"";
    OutputAllPipelineState(out, pipelines[0], commonState);
    out << "\";\n";
    ...
    for (const auto &descAndPipeline : cache)
    {
        ...
        OutputAllPipelineState(out, pipelines[descId], nodeState);
        out << "\"];\n";
