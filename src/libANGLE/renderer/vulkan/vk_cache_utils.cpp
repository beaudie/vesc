    ...
    const WriteDescriptorDescs &writeDescriptorDescs)
{
    ...
    if (isUniformBuffer)
    {
        ...
        {
            static_assert(std::is_same<CommandBufferT, CommandBufferAccess>::value);
    ...
    const WriteDescriptorDescs &writeDescriptorDescs)
{
    ...
    for (uint32_t blockIndex = 0; blockIndex < blocks.size(); ++blockIndex)
    {
        ...
        else
        {
            ...
            updateOneShaderBuffer(contextVk, access, variableInfoMap, buffers, blocks, blockIndex,
    ...
    const WriteDescriptorDescs &writeDescriptorDescs)
{
    ...
    for (const gl::AtomicCounterBuffer &atomicCounterBuffer : atomicCounterBuffers)
    {
        ...
        {
            static_assert(
                std::is_same<CommandBufferT, OutsideRenderPassCommandBufferHelper>::value);
            access->onBufferComputeShaderWrite(&bufferHelper);
        }
