...
{
    return angle::ComputeGenericHash(mDescriptorSetLayoutBindings) ^
           angle::ComputeGenericHash(mImmutableSamplers);
}
...
{
    ASSERT(bindingIndex < std::numeric_limits<uint8_t>::max());
...
void DescriptorSetLayoutDesc::unpackBindings(DescriptorSetLayoutBindingVector *bindings) const
{
    ...
    {
        uint8_t bindingIndex                            = item.first;
