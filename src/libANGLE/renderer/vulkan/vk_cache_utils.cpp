...
// and the offset of 'Field' within 'Member'. All of the offset math should be done
// at compile time.
#define ANGLE_GET_TRANSITION_BIT(Member, Field, BitName) \
    constexpr size_t BitName =                           \
        (offsetof(GraphicsPipelineDesc, Member) + offsetof(decltype(Member), Field)) >> 2;

#define ANGLE_SET_TRANSITION_BIT(Transition, Member, Field) \
    do                                                      \
    {                                                       \
        ANGLE_GET_TRANSITION_BIT(Member, Field, kBit);      \
        Transition->set(kBit);                              \
    } while (0)

#define ANGLE_SET_INDEXED_TRANSITION_BIT(Transition, Member, Field, Index, BitWidth) \
    do                                                                               \
    {                                                                                \
        const size_t kOffset = (BitWidth * Index) >> 5;                              \
        ANGLE_GET_TRANSITION_BIT(Member, Field, kBit);                               \
        Transition->set(kBit + kOffset);                                             \
    } while (0)
                                             ...
                                             GLuint relativeOffset)
{
    ...
    packedAttrib.stride    = static_cast<uint16_t>(stride);
    packedAttrib.inputRate = static_cast<uint8_t>(divisor > 0 ? VK_VERTEX_INPUT_RATE_INSTANCE
    ...
    packedAttrib.format = static_cast<uint8_t>(format);
    packedAttrib.offset = static_cast<uint16_t>(relativeOffset);

    ANGLE_GET_TRANSITION_BIT(mVertexInputAttribs, attribs, kBit);
    const size_t kOffset = (48 * attribIndex) >> 5;

    // Cover the next dirty bit convervatively. Because each attribute is 6 bytes.
    transition->set(kBit + kOffset);
    transition->set(kBit + kOffset + 1);
}

void GraphicsPipelineDesc::updateTopology(GraphicsPipelineTransitionBits *transition,
                                          gl::PrimitiveMode drawMode)
{
    mInputAssemblyAndColorBlendStateInfo.primitive.topology =
        static_cast<uint32_t>(gl_vk::GetPrimitiveTopology(drawMode));
    ANGLE_SET_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo, primitive);
}
                                          ...
                                          const gl::RasterizerState &rasterState)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mRasterizationAndMultisampleStateInfo, bits);
}
                                           ...
                                           float lineWidth)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mRasterizationAndMultisampleStateInfo, lineWidth);
}
                                            ...
                                            const gl::ColorF &color)
{
    ...
    ANGLE_SET_INDEXED_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo,
                                     blendConstants, 0, kSize);
    ANGLE_SET_INDEXED_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo,
                                     blendConstants, 1, kSize);
    ANGLE_SET_INDEXED_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo,
                                     blendConstants, 2, kSize);
    ANGLE_SET_INDEXED_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo,
                                     blendConstants, 3, kSize);
}
                                              ...
                                              bool isBlendEnabled)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mInputAssemblyAndColorBlendStateInfo, blendEnableMask);
}
                                                  ...
                                                  const gl::Framebuffer *drawFramebuffer)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, enable);
}
                                           ...
                                           const gl::DepthStencilState &depthStencilState)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, depthCompareOp);
}
                                                   ...
                                                   const gl::DepthStencilState &depthStencilState)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, front);
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, frontStencilReference);
}
                                                  ...
                                                  const gl::DepthStencilState &depthStencilState)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, back);
    ANGLE_SET_TRANSITION_BIT(transition, mDepthStencilStateInfo, backStencilReference);
}
                                               ...
                                               const gl::RasterizerState &rasterState)
{
    ...
    ANGLE_SET_TRANSITION_BIT(transition, mRasterizationAndMultisampleStateInfo,
...
{
    mViewport = viewport;
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, x);
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, y);
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, width);
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, height);
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, minDepth);
    ANGLE_SET_TRANSITION_BIT(transition, mViewport, maxDepth);
}
...
{
    mScissor = scissor;
    ANGLE_SET_TRANSITION_BIT(transition, mScissor, offset.x);
    ANGLE_SET_TRANSITION_BIT(transition, mScissor, offset.y);
    ANGLE_SET_TRANSITION_BIT(transition, mScissor, extent.width);
    ANGLE_SET_TRANSITION_BIT(transition, mScissor, extent.height);
}
                                                ...
                                                const RenderPassDesc &renderPassDesc)
{
    ...
    constexpr size_t kFirstBit = offsetof(GraphicsPipelineDesc, mRenderPassDesc) >> 2;
    constexpr size_t kBitCount = kRenderPassDescSize >> 2;
