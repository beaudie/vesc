...
constexpr int32_t kDynamicScissorSentinel = std::numeric_limits<uint16_t>::max();
...
void GraphicsPipelineDesc::setScissor(const VkRect2D &scissor)
{
    ...
    mScissor.x      = static_cast<uint16_t>(scissor.offset.x);
    mScissor.y      = static_cast<uint16_t>(scissor.offset.y);
    mScissor.width  = static_cast<uint16_t>(scissor.extent.width);
    mScissor.height = static_cast<uint16_t>(scissor.extent.height);
}
