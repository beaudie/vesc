    ...
    const GLbitfield memoryBarrierBits)
{
    ...
    if (isUniformBuffer)
    {
        commandBufferHelper->bufferRead(VK_ACCESS_2_UNIFORM_READ_BIT, block.activeShaders(),
                                        &bufferHelper);
    }
    else
    {
        ...
        if (block.pod.isReadOnly)
        {
            ...
            // being written to.
            commandBufferHelper->bufferRead(VK_ACCESS_2_SHADER_READ_BIT, block.activeShaders(),
                                            &bufferHelper);
        }
        ...
        else
        {
            ...
            VkAccessFlags2 accessFlags = VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_SHADER_WRITE_BIT;
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < atomicCounterBuffers.size(); ++bufferIndex)
    {
        ...
        for (const gl::ShaderType shaderType : atomicCounterBuffer.activeShaders())
        {
            ...
            commandBufferHelper->bufferWrite(
                VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_SHADER_WRITE_BIT, pipelineStage,
                &bufferHelper);
        }
