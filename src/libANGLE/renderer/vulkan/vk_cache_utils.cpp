...
uint8_t ConvertGlBlendOpToVkBlendOp(GLenum blendOp)
{
    switch (blendOp)
    {
        case GL_FUNC_ADD:
...
void PipelineDesc::updateBlendEnabled(bool isBlendEnabled)
{
    ...
    fillAllColorAttachments(blendAttachmentState);
}

void PipelineDesc::fillAllColorAttachments(
    PackedColorBlendAttachmentState packedColorBlendAttachmentState)
{
    std::fill(&mColorBlendStateInfo.attachments[0],
              &mColorBlendStateInfo.attachments[gl::IMPLEMENTATION_MAX_DRAW_BUFFERS],
              packedColorBlendAttachmentState);
}

void PipelineDesc::updateBlendEquations(const gl::BlendState &blendState)
{
    PackedColorBlendAttachmentState blendAttachmentState = mColorBlendStateInfo.attachments[0];
    blendAttachmentState.colorBlendOp = ConvertGlBlendOpToVkBlendOp(blendState.blendEquationRGB);
    blendAttachmentState.alphaBlendOp = ConvertGlBlendOpToVkBlendOp(blendState.blendEquationAlpha);
    fillAllColorAttachments(blendAttachmentState);
}
