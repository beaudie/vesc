...
{
    // Framebuffer cache are implemented in a way that each cache tracks GPU progress and we always
    // guarantee caches are released before calling destroy.
}
...
template <class SharedCacheKeyT>
void SharedCacheKeyManager<SharedCacheKeyT>::assertAllCachesDestroyed()
{
    // Caller must have already freed all caches
    for (SharedCacheKeyT &sharedCacheKey : mSharedCacheKeys)
    {
