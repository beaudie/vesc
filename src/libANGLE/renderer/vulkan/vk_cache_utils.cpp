...
void SamplerDesc::reset()
{
    ...
    mCompareEnabled = 0;
    mCompareOp      = 0;
    std::fill(mReserved, mReserved + 3, 0);
}
    ...
    vk::BindingPointer<vk::SamplerYcbcrConversion> *yuvConversionOut)
{
    ...
    wrappedYuvConversion.init(context->getDevice(), externalFormat, yuvConversionCreateInfo);
...
VkSamplerYcbcrConversion SamplerYcbcrConversionCache::getYuvConversion(uint64_t externalFormat)
{
    auto iter = mPayload.find(externalFormat);
    if (iter != mPayload.end())
    {
