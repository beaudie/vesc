                                       ...
                                       Pipeline *pipelineOut) const
{
    ...
    return initializePipelineExplicit(
        renderer->getDevice(), *compatibleRenderPass, renderer->getGraphicsPipelineLayout(),
        programVk->getLinkedVertexModule(), programVk->getLinkedFragmentModule(), pipelineOut);
}

Error PipelineDesc::initializePipelineExplicit(VkDevice device,
...
vk::Error PipelineCache::getPipeline(RendererVk *renderer,
                                     ProgramVk *programVk,
                                     Serial serial,
                                     ...
                                     vk::Pipeline **pipelineOut)
{
    ...
    {
        item->second.updateSerial(serial);
        *pipelineOut = &item->second.get();
        return vk::NoError();
    }

    vk::Pipeline newPipeline;
    ANGLE_TRY(desc.initializePipeline(renderer, programVk, &newPipeline));

    auto insertedItem =
        mPayload.emplace(desc, vk::PipelineAndSerial(std::move(newPipeline), serial));
    *pipelineOut = &insertedItem.first->second.get();
