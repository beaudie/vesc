...
std::unique_lock<angle::Mutex> PipelineCacheAccess::getLock()
{
    if (mMutex == nullptr)
    {
        return std::unique_lock<angle::Mutex>();
    }

    return std::unique_lock<angle::Mutex>(*mMutex);
}
...
{
    std::unique_lock<angle::Mutex> lock = getLock();
