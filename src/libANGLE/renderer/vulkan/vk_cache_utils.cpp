    ...
    for (const gl::ShaderType shaderType : linkedStages)
    {
            ...
            context->getRenderer()->getFeatures().emulateTransformFeedback.enabled)
        {
            transformFeedbackVk->updateTransformFeedbackDescriptorDesc(
                context, executable, variableInfoMap, emptyBuffer, this);
        }
    }
}
    ...
    PipelineType pipelineType)
{
    ...
    for (uint32_t textureIndex = 0; textureIndex < samplerBindings.size(); ++textureIndex)
    {
        ...
        uint32_t parentOffset = samplerUniform.parentArrayIndex();
        ...
        for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
        {
            ...
            uint32_t infoIndex = mDesc.getInfoDescIndex(info.binding) + arrayElement + parentOffset;
