...
{
    // Exclude vertex strides from the hash. It's conveniently placed last in vertex input state, so
    // it would be easy to exclude it from hash.
    static_assert(sizeof(PackedInputAssemblyState) +
                      offsetof(PackedVertexInputAttributes, strides) +
                      sizeof(PackedVertexInputAttributes::strides) ==
                  sizeof(PipelineVertexInputState));
    size_t vertexInputSize = sizeof(mVertexInput);
    if (mVertexInput.inputAssembly.bits.supportsDynamicState1 &&
        !mVertexInput.inputAssembly.bits.forceStaticVertexStrideState)
    {
        vertexInputSize -= sizeof(PackedVertexInputAttributes::strides);
    }

    size_t result = angle::ComputeGenericHash(&mVertexInput, vertexInputSize);
    result        = angle::ComputeGenericHash(&mShaders, sizeof(mShaders), result);
    result =
        angle::ComputeGenericHash(&mSharedNonVertexInput, sizeof(mSharedNonVertexInput), result);
    result = angle::ComputeGenericHash(&mFragmentOutput, sizeof(mFragmentOutput), result);
    return result;
}
