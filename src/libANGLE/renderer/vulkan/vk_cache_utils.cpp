                                           ...
                                           RenderPassHelper *renderPassHelper)
{
    ...
    if (contextVk->getFeatures().supportsLegacyDithering.enabled && desc.isDitherEnabled())
    {
        for (VkSubpassDescription2 &subpass : subpassDesc)
        {
...
void RenderPassDesc::setDither(bool enabled)
{
    SetBitField(mDitherDisabled, enabled ? 0 : 1);
}
