...
void PipelineHelper::removeTransition(PipelineHelper **pipeline)
{
    auto transitionsIter = mTransitions.begin();
    while(transitionsIter != mTransitions.end())
    {
        if (transitionsIter->target == *pipeline)
        {
            transitionsIter = mTransitions.erase(transitionsIter);
        }
        else
        {
            transitionsIter++;
                                                        ...
                                                        vk::RenderPass **renderPassOut)
{
    ...
    else
    {
        if (mPayload.size() + 1 > mCacheEvictionSize + mCacheEvictionTolerance)
        {
            ...
            // Clear mCacheEvictionTolerance entries
            size_t evicted                              = 0;
            for (auto &outerIter : mPayload)
            {
                for (auto &innerIter : outerIter.second)
                {
                    innerIter.second.release(contextVk);
                }
                evicted++;
            ...
            // Then erase those mCacheEvictionTolerance entries
    ...
    vk::PipelineHelper **pipelineOut)
{
    ...
    if (mPayload.size() + 1 > mCacheEvictionSize + mCacheEvictionTolerance)
    {
        ...
        // Walk the unordered_map and evict mCacheEvictionTolerence entries
        size_t evicted = 0;
        auto evictIter = mPayload.begin();
        while (evicted < mCacheEvictionTolerance && evictIter != mPayload.end())
        {
            vk::PipelineHelper *pipelineHelper = &evictIter->second;

            // don't remove this entry if current in context
            if (contextVk->checkCurrentGraphicsPipeline(&pipelineHelper))
            {
                ANGLE_LOG(WARN) << "Cache Eviction: GraphicsPipelineCache context is current";
                evictIter++;
                continue;
            }

            // remove all pointers to this pipelineHelper from transitions
            // This is an N^2 operations which is sitting inside an M operation
            //   Where N is the number of entries in the cache
            //   and M is the number of entries to evict
            // So this is a MN^2 operation. This could be optimized
            auto pipelineHelperIter = mPayload.begin();
            while (pipelineHelperIter != mPayload.end())
            {
                pipelineHelperIter->second.removeTransition(&pipelineHelper);
                pipelineHelperIter++;
            }

            // finally we can remove it from the cache
            pipelineHelper->release(contextVk);
