...
// When converting a byte number to a transition bit index we divide by 4.
constexpr size_t kTransitionByteShift = 2;

// When converting a number of bits offset to a transition bit index we divide by 8 * 4 = 32.
constexpr size_t kTransitionBitShift = 5;
...
#define ANGLE_SET_INDEXED_TRANSITION_BIT(Transition, Member, Field, Index, BitWidth) \
    do                                                                               \
    {                                                                                \
        const size_t kOffset  = (BitWidth * Index) >> kTransitionBitShift;           \
        constexpr size_t kBit = ANGLE_GET_TRANSITION_BIT(Member, Field);             \
                                             ...
                                             GLuint relativeOffset)
{
    ...
    constexpr size_t kBit = ANGLE_GET_TRANSITION_BIT(mVertexInputAttribs, attribs);
