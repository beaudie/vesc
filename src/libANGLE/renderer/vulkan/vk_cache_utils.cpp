...
void PipelineDesc::updateScissor(const gl::Rectangle &rect)
{
    ...
    gl::Rectangle clippedRect =
        gl::Rectangle(static_cast<GLuint>(mViewport.x), static_cast<GLuint>(mViewport.y),
                      static_cast<GLuint>(mViewport.width), static_cast<GLuint>(mViewport.height));
    // Coordinates outside surface aren't valid in Vulkan but not error is returned, the scissor is
    // just ignored.
    if (ClipRectangle(rect, clippedRect, &intersection))
    {
        mScissor = {{intersection.x, intersection.y},
                    {static_cast<uint32_t>(intersection.width),
                     static_cast<uint32_t>(intersection.height)}};
    }
    else
    {
