...
{
    auto reduce = [implicitsBegin, implicitsEnd](uint32_t count, const gl::OutputVariable &var) {
                                    ...
                                    ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    for (const gl::VariableLocation &outputLocation : secondaryOutputLocations)
    {
        ...
        {
            const gl::OutputVariable &outputVar = outputVariables[outputLocation.index];
        ...
        programExecutable.getLinkedShaderVersion(gl::ShaderType::Fragment) == 100)
    {
        ...
        for (const gl::OutputVariable &outputVar : outputVariables)
        {
