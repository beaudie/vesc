                                                           ...
                                                           spirv::Blob *blobOut)
{
    ...
    if (mIntNIds.size() <= value)
    {
        ...
        mIntNIds.resize_uninitialized(value + 1);
    }
    ...
    spirv::Blob *blobOut)
{
    ...
    // All new elements initialized later after the resize.
    mIntNIds.resize_uninitialized(4);
...
void SpirvTransformer::transformInstruction()
{
    ...
    if (opCode == spv::OpFunction)
    {
        ...
        if (mOptions.useSpirvVaryingPrecisionFixer)
        {
            mInsertFunctionVariables = mCurrentFunctionId == mEntryPointId &&
