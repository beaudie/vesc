...
class SpirvIDDiscoverer final : angle::NonCopyable
{
  public:
    ...
    void visitMemberName(spirv::IdRef id,
...
void SpirvIDDiscoverer::visitMemberName(spirv::IdRef id,
                                        spirv::LiteralInteger member,
                                        const spirv::LiteralString &name)
{
    // This function is only called on active variables, so it can be used to determine which fields
    // of gl_PerVertex are active.

    if (id == ID::OutputPerVertexBlock && member > mOutputPerVertexMaxActiveMember)
    {
        mOutputPerVertexMaxActiveMember = member;
    }
    else if (id == ID::InputPerVertexBlock && member > mInputPerVertexMaxActiveMember)
    {
                                                        ...
                                                        spv::StorageClass storageClass)
{
    if (mIsEmulated)
    {
        ...
        if (typeId >= sh::vk::spirv::kIdXfbEmulationBufferBlockZero &&
            typeId <= sh::vk::spirv::kIdXfbEmulationBufferBlockThree)
        {
            const uint32_t bufferIndex = typeId - sh::vk::spirv::kIdXfbEmulationBufferBlockZero;
            mXfbBuffers[bufferIndex]   = id;
        }
    ...
    spirv::Blob *blobOut)
{
    ...
    for (const std::vector<XfbVarying> &varyings : mXfbVaryings)
    {
        ...
        {
            writeIntConstant(ids, varying.fieldIndex, ID::Int, blobOut);
            ...
            {
                writeIntConstant(ids, arrayIndex, ID::Int, blobOut);
            }
        }
    }
...
void SpirvTransformer::visitMemberName(const uint32_t *instruction)
{
    ...
    mIds.visitMemberName(id, member, name);
}
    ...
    if (mCurrentFunctionId != ID::EntryPoint)
    {
        if (mOptions.isTransformFeedbackStage)
        {
            ...
            mXfbCodeGenerator.writeTransformFeedbackEmulationOutput(
                mIds, mInactiveVaryingRemover, mVaryingPrecisionFixer,
