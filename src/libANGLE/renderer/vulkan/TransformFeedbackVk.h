...
class TransformFeedbackVk : public TransformFeedbackImpl
{
  public:
    ...
    void updateDescriptorSetLayout(GLenum bufferMode,
    ...
    void updateBufferOffsets(GLenum bufferMode, uint32_t *offsetsOut, size_t offsetsSize) const;
  ...
  private:
    ...
    // We are required to support at least 4 separate buffers.  8 is arbitrarily chosen.
    static constexpr uint32_t kMaxSeparateAttributes = 8;
