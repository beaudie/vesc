                                   ...
                                   gl::Command source)
{
    ...
    if (isGenerateMipmap && mImage && mImage->valid() && mState.getEffectiveBaseLevel() == 0)
    {
        const gl::ImageIndex index     = gl::ImageIndex::MakeFromType(mState.getType(), 0);
        const gl::ImageDesc &levelDesc = mState.getImageDesc(index);
        if (mImage->isOverWriteToSpecificLevel(gl::LevelIndex(0), levelDesc.size) &&
