                                     ...
                                     const gl::Texture *source)
{
    ...
    bool imageValid = mImage.valid();
    ANGLE_TRY(redefineImage(context, index, destFormatInfo, sourceImageDesc.size));
    fprintf(stderr, "copyTexture: %s\n",
            imageValid && mImage.valid() ? "image was and remains valid" :
            imageValid ? "image redefined" :
            "new image");
                                            ...
                                            TextureVk *source)
{
...

    {
        ...
        fprintf(stderr, "copySubTextureImpl:\n"
                        "  src offset: %dx%d, dest offset: %dx%d\n"
                        "  extents: %dx%d, source level: %zu\n"
                        "  dest level: %u, dest layers: %d+%d\n"
                        "  flipY? %d, premul Alpha? %d, unmulAlpha? %d\n"
                        "Image valid? %d, canDraw? %d, src format: %d, dest format: %d\n",
               sourceArea.x, sourceArea.y, destOffset.x, destOffset.y, sourceArea.width, sourceArea.height,
               sourceLevel, level, baseLayer, layerCount, unpackFlipY, unpackPremultiplyAlpha, unpackUnmultiplyAlpha,
               mImage.valid(), canDraw, sourceVkFormat.vkTextureFormat, destVkFormat.vkTextureFormat);
    }
