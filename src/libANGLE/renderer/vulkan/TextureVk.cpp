...
void TextureVk::releaseImage(ContextVk *contextVk)
{
         ...
         i < static_cast<int>(gl::RenderToTextureImageIndex::EnumCount); i++)
    {
        ...
        std::vector<vk::GarbageObject> imageViewGarbage;
        imageViews.garbageCollectOnly(&imageViewGarbage);
        if (mImage == nullptr)
        {
            ASSERT(imageViewGarbage.empty());
        }
        else if (!imageViewGarbage.empty())
        {
            vk::SharedResourceUse imagemUse;
            imagemUse.transfer(mImage->getSharedResourceUse());
            imageViews.sendGarbageWithmUse(std::move(imagemUse), renderer, &imageViewGarbage);
...
void TextureVk::releaseImageViews(ContextVk *contextVk)
{
         ...
         i < static_cast<int>(gl::RenderToTextureImageIndex::EnumCount); i++)
    {
        ...
        else if (!imageViewGarbageObjects.empty())
        {
            vk::SharedResourceUse imagemUse;
            imagemUse.transfer(mImage->getSharedResourceUse());
            imageViews.sendGarbageWithmUse(std::move(imagemUse), renderer,
                                           &imageViewGarbageObjects);
            imageViews.updateImageViewSerial(renderer);
        }
    }
}
...
angle::Result TextureVk::refreshImageViews(ContextVk *contextVk)
{
    ...
    else if (!defaultImageViewGarbage.empty())
    {
        vk::SharedResourceUse imagemUse;
        imagemUse.transfer(mImage->getSharedResourceUse());
        defaultImageView.sendGarbageWithmUse(std::move(imagemUse), renderer,
                                             &defaultImageViewGarbage);
        defaultImageView.updateImageViewSerial(renderer);
    }
