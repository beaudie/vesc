                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    // Sync the texture's image.  See comment on this function in the header.  The
    // DIRTY_BIT_MIN_FILTER is the only dirty bit used by this function and is always specified to
    // ensure all possible image-respecifying conditions are checked.
    const gl::Texture::DirtyBits syncDirtyBits{gl::Texture::DIRTY_BIT_MIN_FILTER};
    ANGLE_TRY(
        syncStateRespecifyImageStorageIfNecessary(contextVk, syncDirtyBits, gl::Command::Draw));
...
angle::Result TextureVk::syncStateRespecifyImageStorageIfNecessary(
    ContextVk *contextVk,
                                   ...
                                   gl::Command source)
{
    ...
    ANGLE_TRY(syncStateRespecifyImageStorageIfNecessary(contextVk, dirtyBits, source));
