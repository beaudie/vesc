                                        ...
                                        const uint8_t *pixels)
{
    ...
    GLuint pixelBytes = formatInfo.computePixelBytes(type);

    uint8_t *stagingPointer = nullptr;
    bool newBufferAllocated = false;
    uint32_t stagingOffset  = 0;
    size_t allocationSize   = inputDepthPitch * mExtents.depth * pixelBytes;
    ...
    loadFunction.loadFunction(mExtents.width, mExtents.height, mExtents.depth, source,
                              inputRowPitch, inputDepthPitch, stagingPointer, inputRowPitch,
                              inputDepthPitch);

    mCurrentCopyRegion.bufferOffset                    = static_cast<VkDeviceSize>(stagingOffset);
    mCurrentCopyRegion.bufferRowLength                 = inputRowPitch / pixelBytes;
    mCurrentCopyRegion.bufferImageHeight               = inputDepthPitch / inputRowPitch;
