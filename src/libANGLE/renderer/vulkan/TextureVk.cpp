...
bool TextureVk::isFastUnpackPossible(const vk::Format &vkFormat, size_t offset) const
{
    ...
    return mImage->valid() && !isCombinedDepthStencil &&
           (vkFormat.getIntendedFormatID() == vkFormat.getActualImageFormatID(isRenderable()) ||
                                   ...
                                   const uint32_t levelCount)
{
    ...
    ANGLE_TRY(mImage->initExternal(contextVk, mState.getType(), vkExtent,
                                   format.getIntendedFormatID(), actualImageFormatID, samples,
