        ...
        else
        {
            ...
            ANGLE_VK_TRY_ALLOC(contextVk, result,
                               mImage->stageSubresourceUpdateImpl(
                                   ...
                                   source, vkFormat, getRequiredImageAccess(), inputRowPitch,
                                   inputDepthPitch, inputSkipBytes, &result));
    ...
    else if (pixels)
    {
        ANGLE_VK_TRY_ALLOC(contextVk, result,
                           mImage->stageSubresourceUpdate(
                               ...
                               vkFormat, getRequiredImageAccess(), &result));
    }
                                          ...
                                          gl::Framebuffer *source)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       mImage->stageSubresourceUpdateFromFramebuffer(
                           ...
                           internalFormat, getRequiredImageAccess(), framebufferVk, &result));
                                            ...
                                            TextureVk *source)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       mImage->stageSubresourceUpdateAndGetData(
                           ...
                           stagingOffset, &destData, dstFormatID, &result));
                                                         ...
                                                         uint8_t **outDataPtr)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, result,
        mImage->copyImageDataToBuffer(contextVk, sourceLevelGL, layerCount, 0, modifiedSourceArea,
                                      copyBuffer, outDataPtr, &result));
                                               ...
                                               vk::ImageHelper *dstImage)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, result,
        stagingImage->get().initStaging(contextVk, mState.hasProtectedContent(),
                                        renderer->getMemoryProperties(), srcImage->getType(),
                                        srcImage->getExtents(), srcImage->getIntendedFormatID(),
                                        srcImage->getActualFormatID(), srcImage->getSamples(),
                                        kTransferImageFlags, levelCount, layerCount, &result));
                                                 ...
                                                 gl::TexLevelMask skipLevelsMask)
{
    ...
    for (vk::LevelIndex levelVk(0); levelVk < vk::LevelIndex(levelCount); ++levelVk)
    {
        ...
        ANGLE_VK_TRY_ALLOC(contextVk, result,
                           mImage->copyImageDataToBuffer(contextVk, levelGL, layerCount, 0,
                                                         sourceBox, srcBuffer, &srcData, &result));
        ...
        ANGLE_VK_TRY_ALLOC(contextVk, result,
                           mImage->stageSubresourceUpdateAndGetData(
                               ...
                               gl::kOffsetZero, &dstData, dstFormat.id, &result));
                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    if (samples > 1 && !mMultisampledImages[renderToTextureIndex].valid() && !hasRenderToTextureEXT)
    {
        ...
        ANGLE_VK_TRY_ALLOC(
            contextVk, result,
            multisampledImage->initImplicitMultisampledRenderToTexture(
                contextVk, mState.hasProtectedContent(), renderer->getMemoryProperties(),
                mState.getType(), samples, *mImage, useRobustInit, &result));
    }
...
angle::Result TextureVk::convertBufferToRGBA(ContextVk *contextVk, size_t &conversionBufferSize)
{
    ...
    if (!conversionBufferHelper->valid())
    {
        ANGLE_VK_TRY_ALLOC(
            contextVk, result,
            conversionBufferHelper->allocateForVertexConversion(
                contextVk, conversionBufferSize, vk::MemoryHostVisibility::NonVisible, &result));
    }
                                            ...
                                            const gl::ImageIndex &imageIndex)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       mImage->stageRobustResourceClearWithFormat(
                           ...
                           format.getActualImageFormat(getRequiredImageAccess()), &result));
    return angle::Result::Continue;
}
                                                     ...
                                                     const gl::ImageIndex &imageIndex)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        ...
        mImage->stageResourceClearWithFormat(
            contextVk, imageIndex, desc.size, format.getIntendedFormat(),
            format.getActualImageFormat(getRequiredImageAccess()), clearValue, &result));
    return angle::Result::Continue;
}
                                   ...
                                   ImageMipLevels mipLevels)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        ...
        mImage->initMemory(contextVk, mState.hasProtectedContent(), renderer->getMemoryProperties(),
                           flags, vk::MemoryAllocationType::TextureImage, &result));
         ...
         ++currentMipLevel)
    {
        ...
        ANGLE_VK_TRY_ALLOC(
            ...
            mImage->stageSubresourceUpdateAndGetData(
                ...
                mipLevelExtents, gl::Offset(), &destData, sourceFormat.id, &result));
                                     ...
                                     void *pixels)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        ...
        mImage->readPixelsForGetImage(contextVk, packState, packBuffer, gl::LevelIndex(level),
                                      layer, layerCount, format, type, pixels, &result));
    return angle::Result::Continue;
}
                                               ...
                                               void *pixels)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, result,
                       mImage->readPixelsForCompressedGetImage(contextVk, packState, packBuffer,
                                                               gl::LevelIndex(level), layer,
                                                               layerCount, pixels, &result));
    return angle::Result::Continue;
}
                                          ...
                                          TextureUpdateResult *updateResultOut)
{
    ...
    if (!mImage->valid())
    {
        ...
        else
        {
            ...
            // CPU.
            ANGLE_VK_TRY_ALLOC(contextVk, result,
                               mImage->reformatStagedBufferUpdates(
                                   contextVk, previousActualFormatID, actualFormatID, &result));
        }
    }
