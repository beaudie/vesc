                                  ...
                                  const uint8_t *pixels)
{
    ...
    if (pixels)
    {
        // glTexImage cannot be called on a texture that is bound to an EGL image or surface, no
        // need to offset the image index
        ASSERT(mImageMipOffset == 0);
                                           ...
                                           egl::Image *image)
{
    ...
    setImageHelper(renderer, imageVk->getImage(), imageVk->getImageBaseMipLevel(), false);
...
{
    gl::ImageIndex result(inputImageIndex);
    result.setLevelIndex(inputImageIndex.getLevelIndex() + mImageMipOffset);
    return result;
}
                               ...
                               bool selfOwned)
{
    ...
    mRenderTarget.init(mImage, &mDrawBaseLevelImageView, mImageMipOffset, 0, this);
...
angle::Result TextureVk::generateMipmapsWithCPU(const gl::Context *context)
{
    ...
    ANGLE_TRY(mImage->recordCommands(contextVk, &commandBuffer));
    return mImage->flushStagedUpdates(contextVk, mImageMipOffset, getLevelCount(), commandBuffer);
}
...
angle::Result TextureVk::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    setImageHelper(renderer, offscreenSurface->getColorBufferImage(), surface->getMipmapLevel(),
                   false);
                                                    ...
                                                    const vk::Format &format)
{
    ...
    return mImage->flushStagedUpdates(context, mImageMipOffset, levelCount, commandBuffer);
}
...
angle::Result TextureVk::initCubeMapRenderTargets(ContextVk *contextVk)
{
    ...
    for (size_t cubeMapFaceIndex = 0; cubeMapFaceIndex < gl::kCubeFaceCount; ++cubeMapFaceIndex)
    {
        ...
        mCubeMapRenderTargets[cubeMapFaceIndex].init(mImage, imageView, cubeMapFaceIndex,
                                                     mImageMipOffset, this);
    }
    return angle::Result::Continue;
}
                                                    ...
                                                    vk::ImageView **imageViewOut)
{
    ...
    *imageViewOut = &mLayerLevelDrawImageViews[layer][level];
    ...
    return mImage->initLayerImageView(context, mState.getType(), VK_IMAGE_ASPECT_COLOR_BIT,
                                      gl::SwizzleState(), *imageViewOut, mImageMipOffset + level, 1,
                                      layer, 1);
}
                                        ...
                                        uint32_t levelCount)
{
    ...
    uint32_t baseLevel = mImageMipOffset;

    ANGLE_TRY(mImage->initImageView(context, mState.getType(), VK_IMAGE_ASPECT_COLOR_BIT,
