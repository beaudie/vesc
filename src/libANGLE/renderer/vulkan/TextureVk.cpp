...
void TextureVk::stageSelfAsSubresourceUpdates(ContextVk *contextVk)
{
    ...
    // Step 1 is to collect the garbage from image views first. We want to track the
    // garbage list from image views with the current image.mUse, but the current image.mUse
    ...
    // prevImage together with image.mUse in stageSelfAsSubresourceUpdates().
    releaseImageViews(contextVk);
    ...
    // Step 2: Release the mImage without collecting garbage from image views - call
    // releaseImage(contextVk) after this function
}
