...
bool IsTextureLevelInAllocatedImage(const vk::ImageHelper &image, uint32_t textureLevelIndex)
{
    uint32_t imageBaseLevel = image.getBaseLevel();
    if (textureLevelIndex < imageBaseLevel)
    {
        return false;
    }

    uint32_t imageLevelIndex = textureLevelIndex - imageBaseLevel;
    return imageLevelIndex < image.getLevelCount();
}
...
bool IsTextureLevelDefinitionCompatibleWithImage(const vk::ImageHelper &image,
                                                 uint32_t textureLevelIndex,
                                                 const gl::Extents &size,
                                                 const vk::Format &format)
{
    ASSERT(IsTextureLevelInAllocatedImage(image, textureLevelIndex));

    uint32_t imageLevelIndex = textureLevelIndex - image.getBaseLevel();
    return size == image.getLevelExtents(imageLevelIndex) && format == image.getFormat();
}
...
bool TextureVk::shouldUpdateBeStaged(uint32_t textureLevelIndex) const
{
    ASSERT(mImage);

    // If update is outside the range of image levels, it must be staged.
    if (!IsTextureLevelInAllocatedImage(*mImage, textureLevelIndex))
    {
        return false;
    }

    uint32_t imageLevelIndex = textureLevelIndex - mImage->getBaseLevel();

    // Can't have more than 32 mips for the foreseeable future.
    ASSERT(imageLevelIndex < 32);

    // Otherwise, it can only be directly applied to the image if the level is not previously
    // incompatibly redefined.
    return mIncompatiblyRedefinedLevels.test(imageLevelIndex);
}
...
void TextureVk::releaseAndDeleteImage(ContextVk *contextVk)
{
    ...
    mIncompatiblyRedefinedLevels.reset();
}
                                       ...
                                       const gl::Extents &size)
{
    ...
    if (mImage != nullptr)
    {
        ...
        if (mImage->valid())
        {
            ...
            // mIncompatiblyRedefinedLevels is used during syncState to support this use-case.
            //
            ...
            bool isInAllocatedImage = IsTextureLevelInAllocatedImage(*mImage, levelIndex);
            bool isCompatibleRedefinition =
                isInAllocatedImage &&
                IsTextureLevelDefinitionCompatibleWithImage(*mImage, levelIndex, size, format);
            bool isUpdateToSingleLevelImage =
                mImage->getLevelCount() == 1 && mImage->getBaseLevel() == levelIndex;
            ...
            {
                mIncompatiblyRedefinedLevels.set(levelIndex - mImage->getBaseLevel(),
    ...
    //
    // TODO: write a test that creates a mip chain, then redefines every mip to another format such
    // that after the redefinition the texture is mip-complete.  The image is not released above,
    // so make sure something is recreating it.
    //
    // TODO: change ensureImageAllocated to do the following:
    //
    // - In ensureImageAllocated, only set mImageUsageFlags if mImage is nullptr.  ASSERT in the
    //   other case that the usage flags are properly set.  Look for "new vk::ImageHelper()", and
    //   update them accordingly.  TODO: write a test that uses image as storage image, then
    //   redefines a mip. Redefine will cause the image flags to be reset currently, which is
    //   probably gonna mess something up, or at least be inefficient.
    if (!size.empty())
    {
        ANGLE_TRY(ensureImageAllocated(contextVk, format));
    }
                                                           ...
                                                           GLuint maxLevel)
{
    ...
    if (mImage->valid() && mImage->hasStagedUpdates())
    {
        ...
        ANGLE_TRY(mImage->flushStagedUpdates(
            ...
            mImage->getLayerCount(), mIncompatiblyRedefinedLevels, commandBuffer));
    }
    ...
    for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
    {
        for (uint32_t level = 0; level < updateCount; level++)
        {
            ...
            // update, and the above test will be true.
            ASSERT(!mIncompatiblyRedefinedLevels.test(level));
            ...
            // TODO: this can make srcLevelVK negative, no?! Add a test for it.
                                                    ...
                                                    const vk::Format &format)
{
    ...
    if (!mImage->valid())
    {
        ASSERT(!mIncompatiblyRedefinedLevels.any());
    ...
    return mImage->flushStagedUpdates(contextVk, getNativeImageLevel(0), mImage->getLevelCount(),
                                      getNativeImageLayer(0), mImage->getLayerCount(),
                                      mIncompatiblyRedefinedLevels, commandBuffer);
}
                                   ...
                                   const gl::Texture::DirtyBits &dirtyBits)
{
    ...
    if (oldUsageFlags != mImageUsageFlags || oldCreateFlags != mImageCreateFlags ||
        mIncompatiblyRedefinedLevels.any())
    {
