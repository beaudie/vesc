...
TextureVk::TextureVk(const gl::TextureState &state) : TextureImpl(state), mRequiredSize(0)
{
}
                              ...
                              const uint8_t *pixels)
{
    ...
    if (!mImage.valid())
    {
        ...
        ANGLE_TRY(
            vk::AllocateImageMemory(contextVk, flags, &mImage, &mDeviceMemory, &mRequiredSize));
