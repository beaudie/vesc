...
angle::Result TextureVk::clearSubImage(const gl::Context *context,
                                       ...
                                       const uint8_t *data)
{
    // If any dimension from the clear area is 0, the clear call would be treated as a no-op. For 2D
    // textures, a depth of 1 is expected.
    //
    ...
    if (area.width == 0 || area.height == 0 || area.depth == 0)
    {
        return angle::Result::Continue;
    }
