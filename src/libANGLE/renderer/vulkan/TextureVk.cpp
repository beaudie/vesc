...
vk::Error TextureVk::getCommandBuffer(RendererVk *renderer, vk::CommandBuffer **outCommandBuffer)
{
                                ...
                                const gl::Extents &size)
{
    ...
    ANGLE_TRY(getCommandBuffer(renderer, &commandBuffer));
    ANGLE_TRY(
        initImageAndView(renderer, format, size, static_cast<uint32_t>(levels), commandBuffer));
    return gl::NoError();
}
...
vk::Error TextureVk::ensureImageInitialized(RendererVk *renderer)
{
    ...
    if (!mImage.valid())
    {
        ...
        ANGLE_TRY(initImageAndView(renderer, format, extents, levelCount, commandBuffer));
    }
...
vk::Error TextureVk::initImageAndView(RendererVk *renderer,
                                      const vk::Format &format,
                                      const gl::Extents &extents,
