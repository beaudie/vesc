...
bool NeedsRGBAEmulation(angle::FormatID formatID, RendererVk *renderer)
{
    if (GetRGBAEmulationDstFormat(formatID) == angle::FormatID::NONE)
        return false;
    return !renderer->hasBufferFormatFeatureBits(formatID,
                                   ...
                                   gl::Command source)
{
    ...
    if (mState.getBuffer().get() != nullptr)
    {
        ...
        if (NeedsRGBAEmulation(getBaseLevelFormat(renderer).getIntendedFormatID(), renderer))
        {
                                                   ...
                                                   const vk::BufferView **viewOut)
{
    ...
    if (NeedsRGBAEmulation(imageUniformFormat->getIntendedFormatID(), renderer))
    {
