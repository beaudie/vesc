...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    if (needRedefineImage)
    {
        ...
        ANGLE_TRY(CopyImageToBufferAndUpdate(contextVk, baseLevelDesc, false,
                                                    ...
                                                    uint32_t stagingDstMipLevel)
{
    ...
    gl_vk::GetExtentsAndLayerCount(mState.getType(), baseLevelExtents, &updatedExtents,
                                   &layerCount);
    // According to Vulkan spec, If the calling command's srcImage (vkCmdCopyImageToBuffer) or
    // dstImage (vkCmdCopyBufferToImage) is of type VK_IMAGE_TYPE_1D or VK_IMAGE_TYPE_2D,
    // then imageOffset.z must be 0 and imageExtent.depth must be 1
    VkImageType imageType = gl_vk::GetImageType(mState.getType());
    if (imageType == VK_IMAGE_TYPE_1D || imageType == VK_IMAGE_TYPE_2D)
    {
        updatedExtents.depth = 1;
        offset.z             = 0;
    }
    gl::Box area(offset.x, offset.y, offset.z, updatedExtents.width, updatedExtents.height,
                 updatedExtents.width);
    if (ignoreLayerCount)
    {
        layerCount = 1;
    }
    ...
    ANGLE_TRY(copyImageDataToBuffer(contextVk, sourceMipLevel, layerCount, currentLayer, area,
...
angle::Result TextureVk::changeLevels(ContextVk *contextVk, GLuint baseLevel, GLuint maxLevel)
{
    ...
    for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
    {
        for (uint32_t level = 0; level < updateCount; level++)
        {
            ...
            ANGLE_TRY(CopyImageToBufferAndUpdate(contextVk, desc, true, layer, srcLevelVK, level));
        }
    }
