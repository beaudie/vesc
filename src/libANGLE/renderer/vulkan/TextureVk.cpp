...
TextureVk::TextureVk(const gl::TextureState &state, RendererVk *renderer)
    : TextureImpl(state),
      ...
      mImmutableSamplerStateDirty(false),
                                                 ...
                                                 const vk::ImageHelper *nextImage)
{
    ...
    bool previousImageHadImmutableSampler =
        (previousImage && previousImage->valid()) ? previousImage->hasImmutableSampler() : false;

    // Does the next image require an immutable sampler?
    bool nextImageRequiresImmutableSampler =
        (nextImage && nextImage->valid()) ? nextImage->hasImmutableSampler() : false;

    // Handle transition of immutable sampler state
    if (previousImageHadImmutableSampler != nextImageRequiresImmutableSampler)
    {
        // Record the transition and invalidate the sampler.
        mImmutableSamplerStateDirty = true;
        mSampler.reset();
    }
}
