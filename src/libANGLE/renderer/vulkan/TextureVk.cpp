...
void TextureVk::releaseImageViews(ContextVk *contextVk)
{
         ...
         i < static_cast<int>(gl::RenderToTextureImageIndex::EnumCount); i++)
    {
        ...
        imageViews.release(renderer, imageViewsGarbageObjects);
    }
    ...
    std::vector<vk::GarbageObject> &garbageListInImage = mImage->getImageAndImageViewGarbage();
    for (vk::GarbageObject &garbage : imageViewsGarbageObjects)
    {
        garbageListInImage.emplace_back(std::move(garbage));
    }
}
