                                   ...
                                   const gl::Texture::DirtyBits &dirtyBits)
{
    ...
    bool isGenerateMipmap = dirtyBits.test(gl::Texture::DIRTY_BIT_GENERATE_MIPMAP);
    if (isGenerateMipmap)
    {
        mImage->removeStagedUpdates(contextVk, mState.getEffectiveBaseLevel() + 1,
                                    mState.getEffectiveMaxLevel() + 1);
    }
    ...
    localBits.reset(gl::Texture::DIRTY_BIT_GENERATE_MIPMAP);
