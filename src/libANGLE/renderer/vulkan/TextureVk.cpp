...
angle::Result TextureVk::changeLevels(ContextVk *contextVk, GLuint baseLevel, GLuint maxLevel)
{
    ...
    // Remeber the highest mip level that we've supported for this image
    if (mImage->getHighestLevel() < mState.getMipmapMaxLevel())
    {
        mImage->setHighestLevel(mState.getMipmapMaxLevel());
    }
    ...
    // Stage updates for all levels in the GL texture
    uint32_t updateCount = mImage->getHighestLevel() + 1;
    ...
    for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
    {
        for (uint32_t level = 0; level < updateCount; level++)
        {
            if (mImage->isUpdateStaged(level, layer))
            {
                ...
                // levels that have never fit into the vkImage due to base/max level, updates that
                // haven't been applied yet, and for live vkImage levels that don't fit into the new
                // image.
                continue;
            }
                                   ...
                                   const gl::Texture::DirtyBits &dirtyBits)
{
    ...
    // Set max level before initializing the image
