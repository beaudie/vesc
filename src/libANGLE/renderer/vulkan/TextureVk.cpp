...
void TextureVk::releaseAndDeleteImageAndViews(ContextVk *contextVk)
{
    ...
    mTempBuffer.release(contextVk->getRenderer());
    mTempBufferViews.release(contextVk);
                                                   ...
                                                   const vk::BufferView **viewOut)
{
    ...
    if (!isImage && imageUniformFormat->getIntendedFormatID() == angle::FormatID::R32G32B32_UINT)
    {
        ...
        const VkDeviceSize offset                                 = bufferBinding.getOffset();
        const VkDeviceSize size       = gl::GetBoundBufferAvailableSize(bufferBinding);
        const VkDeviceSize scaledSize = (size * 4) / 3;
        ...
        mTempBuffer.release(renderer);
        ANGLE_TRY(mTempBuffer.init(context, bufferInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
        ...
        mTempBufferViews.release(contextVk);
        mTempBufferViews.init(renderer, 0, scaledSize);
        return mTempBufferViews.getView(contextVk, mTempBuffer, bufferOffset,
