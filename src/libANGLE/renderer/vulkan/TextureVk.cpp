                                    ...
                                    const gl::Extents &size)
{
    ...
    releaseNonOwnedImage(context, renderer);
    ...
    if (mImage->valid())
    {
        releaseImage(context, renderer, false);
    }
                                           ...
                                           egl::Image *image)
{
    ...
    setImageHelper(renderer, imageVk->getImage(), imageVk->getImageTextureType(),
                   imageVk->getImageBaseMipLevel(), imageVk->getImageBaseLayer(), false);
...
gl::ImageIndex TextureVk::getNativeImageIndex(const gl::ImageIndex &inputImageIndex) const
{
    ...
    return gl::ImageIndex::MakeFromType(mImageNativeType,
                                        getNativeImageMipLevel(inputImageIndex.getLevelIndex()),
                                        resultImageLayer, inputImageIndex.getLayerCount());
}

uint32_t TextureVk::getNativeImageMipLevel(uint32_t frontendMip) const
{
    return mImageMipOffset + frontendMip;
}
...
void TextureVk::destroyImage(const gl::Context *context, RendererVk *renderer)
{
    if (mImage)
    {
        releaseImage(context, renderer, true);
        SafeDelete(mImage);
    }
}

void TextureVk::releaseNonOwnedImage(const gl::Context *context, RendererVk *renderer)
{
                               ...
                               bool selfOwned)
{
    ...
    mImageMipOffset   = imageMipOffset;
    mImageLayerOffset = imageLayerOffset;
    mImage            = imageHelper;
    mImage->initStagingBuffer(renderer);

    mRenderTarget.init(mImage, &mDrawBaseLevelImageView, getNativeImageMipLevel(0),
                                       ...
                                       const gl::Extents &size)
{
    ...
    // If this TextureVk does not own its image, release the image reference. Redefining another
    // object's image is never what we want to do.
    releaseNonOwnedImage(context, renderer);
...
angle::Result TextureVk::generateMipmapsWithCPU(const gl::Context *context)
{
    ...
    ANGLE_TRY(mImage->recordCommands(contextVk, &commandBuffer));
    return mImage->flushStagedUpdates(contextVk, getNativeImageMipLevel(0), getLevelCount(),
                                      commandBuffer);
}
...
angle::Result TextureVk::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    setImageHelper(renderer, offscreenSurface->getColorBufferImage(), mState.getType(),
...
angle::Result TextureVk::ensureImageInitialized(vk::Context *context)
{
    ...
    const vk::Format &format =
        context->getRenderer()->getFormat(baseLevelDesc.format.info->sizedInternalFormat);

    return ensureImageInitializedImpl(context, baseLevelExtents, levelCount, format);
}

angle::Result TextureVk::ensureImageInitializedImpl(vk::Context *context,
                                                    ...
                                                    const vk::Format &format)
{
    ...
    vk::CommandBuffer *commandBuffer = nullptr;
    ANGLE_TRY(mImage->recordCommands(context, &commandBuffer));

    if (!mImage->valid())
    {
        ANGLE_TRY(initImage(context, format, baseLevelExtents, levelCount, commandBuffer));
    }

    return mImage->flushStagedUpdates(context, getNativeImageMipLevel(0), levelCount,
                                      commandBuffer);
}
...
angle::Result TextureVk::initCubeMapRenderTargets(ContextVk *contextVk)
{
    ...
    for (size_t cubeMapFaceIndex = 0; cubeMapFaceIndex < gl::kCubeFaceCount; ++cubeMapFaceIndex)
    {
        ...
        mCubeMapRenderTargets[cubeMapFaceIndex].init(mImage, imageView, getNativeImageMipLevel(0),
...
void TextureVk::releaseOwnershipOfImage()
{
    // No need to change the mImage pointer, this texture is about to be redefined
    ASSERT(mOwnsImage == true);
    mOwnsImage = false;
}
                                                    ...
                                                    vk::ImageView **imageViewOut)
{
    ...
    *imageViewOut = &mLayerLevelDrawImageViews[layer][level];
    ...
    // don't have swizzle.
    return mImage->initLayerImageView(
        context, mState.getType(), VK_IMAGE_ASPECT_COLOR_BIT, gl::SwizzleState(), *imageViewOut,
        getNativeImageMipLevel(level), 1, getNativeImageLayer(layer), 1);
}
...
angle::Result TextureVk::initImage(vk::Context *context,
                                   const vk::Format &format,
                                   ...
                                   vk::CommandBuffer *commandBuffer)
{
    ...
    const RendererVk *renderer = context->getRenderer();
    ...
    ANGLE_TRY(mImage->initMemory(context, renderer->getMemoryProperties(), flags));

    ANGLE_TRY(initImageViews(context, format, levelCount));
...
angle::Result TextureVk::initImageViews(vk::Context *context,
                                        const vk::Format &format,
                                        uint32_t levelCount)
{
    ASSERT(mImage != nullptr);
    ...
    // TODO: Support non-zero base level for ES 3.0 by adding it to mImageMipOffset
    uint32_t baseLevel = getNativeImageMipLevel(0);
    uint32_t baseLayer = getNativeImageLayer(0);

    ANGLE_TRY(mImage->initLayerImageView(context, mState.getType(), VK_IMAGE_ASPECT_COLOR_BIT,
                                         mappedSwizzle, &mReadMipmapImageView, baseLevel,
                                         levelCount, baseLayer, 1));
    ANGLE_TRY(mImage->initLayerImageView(context, mState.getType(), VK_IMAGE_ASPECT_COLOR_BIT,
...
void TextureVk::releaseImage(const gl::Context *context,
                             RendererVk *renderer,
                             bool releaseStagingBuffer)
{
    if (mImage)
    {
        if (mOwnsImage)
        {
            mImage->release(renderer, releaseStagingBuffer);
        }
        else
        {
            mImage = nullptr;
