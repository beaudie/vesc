...
angle::Result TextureVk::ensureImageInitializedImpl(ContextVk *contextVk,
                                                    ...
                                                    const vk::Format &format)
{
    if (mImage->valid() && !mImage->hasStagedUpdates() && mIndicesToInitialize.empty())
    {
        return angle::Result::Continue;
    }
    ...
    {
        initializeContentsImpl(imageIndex, commandBuffer);
    }
    mIndicesToInitialize.clear();
                                            ...
                                            const gl::ImageIndex &imageIndex)
{
    ...
    {
        vk::CommandBuffer *commandBuffer = nullptr;
        ANGLE_TRY(mImage->recordCommands(vk::GetImpl(context), &commandBuffer));
