...
{
    return layerCount > 1
               ? samples > 1 ? gl::TextureType::_2DArray : gl::TextureType::_2DMultisampleArray
               : samples > 1 ? gl::TextureType::_2D : gl::TextureType::_2DMultisample;
}
}  // anonymous namespace
    ...
    auto removeIfStatement = [renderer, levelIndex, layerIndex](SubresourceUpdate &update) {
        const VkImageSubresourceLayers &dstSubresource =
            update.fromBuffer ? update.bufferCopyRegion.imageSubresource
                              : update.imageCopyRegion.dstSubresource;
        bool remove = dstSubresource.mipLevel == levelIndex && dstSubresource.baseArrayLayer == layerIndex;
        if (remove && !update.fromBuffer)
        {
            update.image->release(renderer);
            delete update.image;
        }
                                                  ...
                                                  const gl::Extents &extents)
{
    ...
    copyToImage.srcSubresource.mipLevel       = 0;
    copyToImage.srcSubresource.baseArrayLayer = 0;
    ...
    copyToImage.srcOffset                     = VkOffset3D{0, 0, 0};
                                               ...
                                               vk::CommandBuffer *commandBuffer)
{
    ...
    for (const SubresourceUpdate &update : mSubresourceUpdates)
    {
        ...
        const uint32_t updateMipLevel = update.fromBuffer
                                            ...
                                            : update.imageCopyRegion.dstSubresource.mipLevel;
        ...
        else
        {
            ...
            ANGLE_TRY(update.image->recordCommands(contextVk, &srcLayoutChange));
            update.image->changeLayoutWithStages(VK_IMAGE_ASPECT_COLOR_BIT,
                                                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                                 VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                                 VK_PIPELINE_STAGE_TRANSFER_BIT, srcLayoutChange);
                                          ...
                                          gl::Framebuffer *source)
{
    ...
    if (CanCopyWithDraw(renderer, srcFormat, destFormat))
    {
                                            ...
                                            TextureVk *source)
{
    ...
    // If it's possible to perform the copy with a draw call, do that.
    if (CanCopyWithDraw(renderer, sourceVkFormat, destVkFormat))
    {
                                                  ...
                                                  const vk::ImageView *srcView)
{
    ...
    gl::Offset copyDestOffset = destOffset;
    ...
    if (stageCopy)
    {
        ...
        copyDestOffset = gl::Offset(0, 0, 0);
    }
    else
    {
    ...
    vk::ImageHelper *destImage = stageCopy ? stagingImage.get() : &mImage;
    ...
    params.destOffset[0]       = copyDestOffset.x;
    params.destOffset[1]       = copyDestOffset.y;
    ...
    for (uint32_t layerIndex = 0; layerIndex < layerCount; ++layerIndex)
    {
        ...
        ANGLE_TRY(utilsVk.copyImage(contextVk, destImage, destView, srcImage, srcView, params));
