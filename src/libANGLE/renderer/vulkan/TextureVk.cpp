...
constexpr size_t kMaxImageLayerCount            = 6;
...
gl::Error PixelBuffer::stageSubresourceUpdateFromBuffer(const gl::ImageIndex &index,
                                                        const gl::Extents &extents,
                                                        const gl::Offset &offset,
...
{
    VkBufferImageCopy copy;
    copy.bufferOffset                    = static_cast<VkDeviceSize>(bufferOffset);
    copy.bufferRowLength                 = extents.width;
    copy.bufferImageHeight               = extents.height;
    copy.imageSubresource.aspectMask     = VK_IMAGE_ASPECT_COLOR_BIT;
    copy.imageSubresource.mipLevel       = index.getLevelIndex();
...
gl::Error TextureVk::generateMipmapLevelsCpuOnly(ContextVk *contextVk,
                                                 ...
                                                 uint8_t *sourceData)
{
    ...
    for (GLuint currentMipLevel = firstMipLevel; currentMipLevel <= maxMipLevel; currentMipLevel++)
    {
        ...
        ANGLE_TRY(mMipmapsBuffer.allocate(renderer, mipAllocationSize, &destData, &destBufferHandle,
        ...
        ANGLE_TRY(mPixelBuffer.stageSubresourceUpdateFromBuffer(
            gl::ImageIndex::MakeFromType(mState.getType(), currentMipLevel, layer, 1),
            mipLevelExtents, gl::Offset(), destBufferOffset, destBufferHandle));
...
void TextureVk::generateMipmapWithBlit(RendererVk *renderer)
{
    ...
    // Put back the image in the undefined layout. This will cause the next updates to work no
    // matter what layout each level is at.
    mImage.updateLayout(VK_IMAGE_LAYOUT_UNDEFINED);
}

gl::Error TextureVk::generateMipmapCpuOnly(const gl::Context *context)
{
    ContextVk *contextVk = vk::GetImpl(context);
    RendererVk *renderer = contextVk->getRenderer();
    ...
    ANGLE_TRY(mMipmapsBuffer.allocate(renderer, totalAllocationSize, &baseLevelBuffers,
    ...
    for (GLuint layer = 0; layer < imageLayerCount; layer++)
    {
        ...
        ANGLE_TRY(generateMipmapLevelsCpuOnly(
...
gl::Error TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    {
        ANGLE_TRY(generateMipmapCpuOnly(context));
    }
...
vk::Error TextureVk::ensureImageInitialized(RendererVk *renderer)
{
    ...
    mMipmapsBuffer.flush(renderer->getDevice());
