...
void PixelBuffer::removeStagedUpdates(RendererVk *renderer, const gl::ImageIndex &index)
{
    ...
    auto out = mSubresourceUpdates.begin();
    auto end = mSubresourceUpdates.end();

    // The following up to erase() is effectively inlined remove_if().

    // Skip until first hit.
    for (; out != end; ++out)
    {
        if (out->isUpdateToLayerLevel(layerIndex, levelIndex))
        {
            out->release(renderer);
            break;
        }
    }
    // Shift back and remove all hits.
    if (out != end)
    {
        for (auto cur = out + 1; cur != end; ++cur)
        {
            if (cur->isUpdateToLayerLevel(layerIndex, levelIndex))
            {
                cur->release(renderer);
            }
            else
            {
                *out++ = std::move(*cur);
    ...
    mSubresourceUpdates.erase(out, end);
}
