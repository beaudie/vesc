...
TextureVk::TextureVk(const gl::TextureState &state, RendererVk *renderer) : TextureImpl(state) {}
                                          ...
                                          gl::Framebuffer *source)
{
    ...
    mImage->finishCurrentCommands(renderer);
    framebufferVk->getFramebuffer()->addReadDependency(mImage.get());
    return angle::Result::Continue;
}
                                       ...
                                       const gl::Extents &size)
{
    ...
    if (mImage == nullptr && !size.empty())
    {
        mImage.reset(new vk::ImageHelper());
        mImage->initStagingBuffer(renderer);

        mRenderTarget.init(mImage.get(), &mDrawBaseLevelImageView, 0, this);
...
angle::Result TextureVk::initCubeMapRenderTargets(ContextVk *contextVk)
{
    ...
    for (size_t cubeMapFaceIndex = 0; cubeMapFaceIndex < gl::kCubeFaceCount; ++cubeMapFaceIndex)
    {
        ...
        mCubeMapRenderTargets[cubeMapFaceIndex].init(mImage.get(), imageView, cubeMapFaceIndex,
                                                     this);
    }
    return angle::Result::Continue;
}
