                                  ...
                                  const uint8_t *pixels)
{
    ...
    return setImageImpl(context, index, formatInfo, size, type, unpack, unpackBuffer, pixelsFormatInfo, pixels);
}
                                     ...
                                     const uint8_t *pixels)
{
    ...
    return setSubImageImpl(context, index, area, formatInfo, type, unpack, unpackBuffer, formatInfo, pixels,
                           vkFormat);
}
                                         ...
                                         const vk::Format &vkFormat)
{
    ...
    if (unpackBuffer)
    {
        ...
        ANGLE_TRY(mImage->CalculateBufferInfo(
            contextVk, gl::Extents(area.width, area.height, area.depth), pixelsFormatInfo, unpack, type,
        ...
        else
        {
            ...
            ANGLE_TRY(mImage->stageSubresourceUpdateImpl(
                ...
                gl::Offset(area.x, area.y, area.z), pixelsFormatInfo, unpack, type, source, vkFormat,
