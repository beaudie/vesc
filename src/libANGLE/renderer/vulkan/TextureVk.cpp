                                     ...
                                     angle::FormatID dstImageFormatID) const
{
    ...
    // flush the update.
    if (imageHasMismatchFormat(dstImageFormatID))
    {
        return true;
    }
                                               ...
                                               bool fixedSampleLocations)
{
    ...
    ANGLE_TRY(initImage(contextVk, format, format.getActualImageFormatID(getRequiredImageAccess()),
                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    if (vk::ImageAccess::Renderable != mRequiredImageAccess)
    {
...
angle::Result TextureVk::ensureRenderable(ContextVk *contextVk)
{
    ...
    if (!mImage->valid())
    {
        ...
        if (mImage->hasStagedUpdatesWithMismatchedFormat(
                format.getActualImageFormatID(getRequiredImageAccess())))
        {
            const uint32_t levelCount = getMipLevelCount(ImageMipLevels::EnabledLevels);
            // Immutable texture must already have a valid image
            ...
            ANGLE_TRY(initImage(contextVk, format, sampleOnlyFormatID,
                                baseLevelDesc.format.info->sized, baseLevelExtents,
                                mState.getEffectiveBaseLevel(), levelCount));
            ANGLE_TRY(flushImageStagedUpdates(contextVk));
        }
    ...
    else
    {
        ...
        ASSERT(!mImage->hasStagedUpdatesWithMismatchedFormat(
            format.getActualImageFormatID(getRequiredImageAccess())));
    }
...
bool TextureVk::imageHasMismatchFormat(angle::FormatID actualFormatID) const
{
