                                           ...
                                           const uint8_t *data)
{
    ...
    // the image simply by opening/closing a render pass with LOAD_OP of CLEAR. Otherwise, a buffer
    // can be filled with the given pixel data on the host and staged to the image as a buffer
    // update.
    ContextVk *contextVk                 = vk::GetImpl(context);
    const gl::InternalFormat &formatInfo = gl::GetInternalFormatInfo(format, type);
    const vk::Format &vkFormat =
    ...
    if (getRequiredImageAccess() == vk::ImageAccess::Renderable)
    {
        uint32_t baseLayer  = useLayerAsDepth ? clearArea.z : 0;
        uint32_t layerCount = useLayerAsDepth ? clearArea.depth : 1;
        ANGLE_TRY(mImage->stagePartialClear(contextVk, clearArea, isFullClear, mState.getType(),
    ...
    else
    {
        ...
        if (data != nullptr)
        {
            ...
            {
                memcpy(&clearBuffer[i], constValue.data(), pixelSize);
            }
        }
