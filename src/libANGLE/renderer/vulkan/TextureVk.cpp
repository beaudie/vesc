...
angle::Result TextureVk::respecifyImageStorageIfNecessary(ContextVk *contextVk, gl::Command source)
{
    ...
    bool wasBoundToMSRTTFramebuffer   = mHasBeenBoundToMSRTTFramebuffer;
    ...
    // Create a new image if it is bound to an MSRTT framebuffer (only if it did not already
    // include the MSRTSS bit during initialization).
    if (mState.hasBeenBoundToMSRTTFramebuffer() && !mHasBeenBoundToMSRTTFramebuffer)
    {
        mHasBeenBoundToMSRTTFramebuffer = true;
    }
    ...
    if (contextVk->getFeatures().supportsMultisampledRenderToSingleSampled.enabled &&
        ...
        wasBoundToMSRTTFramebuffer != mHasBeenBoundToMSRTTFramebuffer)
    {
