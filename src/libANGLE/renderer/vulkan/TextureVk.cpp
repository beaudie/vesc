...
bool TextureVk::shouldUpdateBeStaged(uint32_t textureLevelIndexGL) const
{
    ...
    return mIncompatiblyRedefinedLevels.test(imageLevelIndexVK);
}
                                       ...
                                       const gl::Extents &size)
{
    ...
    if (mImage != nullptr)
    {
        ...
        if (mImage->valid())
        {
            ...
            if (isInAllocatedImage && !isCompatibleRedefinition)
            {
                mIncompatiblyRedefinedLevels.set(levelIndexGL - mImage->getBaseLevel(),
                                                           ...
                                                           GLuint maxLevel)
{
    ...
    for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
    {
        for (uint32_t levelVK = 0; levelVK < mImage->getLevelCount(); levelVK++)
        {
            ...
            if (mIncompatiblyRedefinedLevels.test(levelVK))
            {
