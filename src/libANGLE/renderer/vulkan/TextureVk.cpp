                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    vk::ImageHelper &multisampledImage = mMultisampledImages[renderToTextureIndex][requestedLevel];
    if (samples > 1 && !multisampledImage.valid() && !hasRenderToTextureEXT)
    {
        ASSERT(mState.getBaseLevelDesc().samples <= 1);

        // Ensure the view serial is valid.
        vk::Renderer *renderer = contextVk->getRenderer();
        mMultisampledImageViews[renderToTextureIndex][requestedLevel].init(renderer);
        ...
        VkExtent3D extents = {};
        gl_vk::GetExtent(mImage->getLevelExtents(mImage->toVkLevel(gl::LevelIndex(requestedLevel))),
                         &extents);
                                             ...
                                             gl::RenderToTextureImageIndex renderToTextureIndex)
{
    ...
    {
        ASSERT(mMultisampledImages[renderToTextureIndex][requestedLevel].valid());
        ASSERT(!mImage->isYuvResolve());

        resolveImage      = drawImage;
        resolveImageViews = drawImageViews;
        drawImage         = &mMultisampledImages[renderToTextureIndex][requestedLevel];
        drawImageViews    = &mMultisampledImageViews[renderToTextureIndex][requestedLevel];
