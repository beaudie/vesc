                              ...
                              VkClearValue *clearValueOut)
{
    if (!formatInfo.isDepthOrStencil())
    {
        if (formatInfo.isInt())
        {
            memcpy(reinterpret_cast<uint8_t *>(&clearValueOut->color.uint32), constValue,
                   formatInfo.pixelBytes);
        }
        else
        {
            clearValueOut->color.float32[0] = gl::normalizedToFloat(constValue[0]);
            clearValueOut->color.float32[1] =
                formatInfo.componentCount > 1 ? gl::normalizedToFloat(constValue[1]) : 0.0f;
            clearValueOut->color.float32[2] =
                formatInfo.componentCount > 2 ? gl::normalizedToFloat(constValue[2]) : 0.0f;
            clearValueOut->color.float32[3] =
                formatInfo.componentCount > 3 ? gl::normalizedToFloat(constValue[3]) : 0.0f;
        }
    }
    else
    {
        ...
        if (formatInfo.depthBits > 0)
        {
            if (formatInfo.isInt())
            {
                memcpy(reinterpret_cast<uint8_t *>(&clearValueOut->depthStencil.depth), constValue,
                       formatInfo.pixelBytes);
            }
            else
            {
                clearValueOut->depthStencil.depth = *reinterpret_cast<float *>(constValue);
            }
        }
        if (formatInfo.stencilBits > 0)
        {
            ...
            {
                clearValueOut->depthStencil.stencil = *reinterpret_cast<uint32_t *>(constValue);
            }
        }
    }
}
                                       ...
                                       const uint8_t *data)
{
    ...
    GetVkClearValueFromBytes(constValue.data(), formatInfo, &clearValue);
    ...
    ANGLE_TRY(mImage->stageSubresourceClear(
        ...
        useLayerAsDepth ? updateArea.depth : 1, aspectMask, formatInfo, clearValue));
