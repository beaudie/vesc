                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    ASSERT(mState.hasBeenBoundAsAttachment());
    ANGLE_TRY(respecifyImageStorageIfNecessary(contextVk, gl::Command::Draw, false));
...
angle::Result TextureVk::respecifyImageStorageIfNecessary(ContextVk *contextVk,
                                                          gl::Command source,
                                                          bool debug_is_from_texture_sync)
{
    ...
    if (mState.hasBeenBoundAsImage() && debug_is_from_texture_sync)
    {
        mImageUsageFlags |= VK_IMAGE_USAGE_STORAGE_BIT;
    ...
    if (isSRGBOverrideEnabled() && debug_is_from_texture_sync)
    {
        mRequiresMutableStorage = true;
    }

    if (mRequiresMutableStorage && debug_is_from_texture_sync)
    {
    ...
    // on.
    if (debug_is_from_texture_sync && mState.hasBeenBoundAsAttachment())
    {
        ANGLE_TRY(ensureRenderable(contextVk));
    ...
    if (isGenerateMipmap)
    {
        ASSERT(debug_is_from_texture_sync);
    ...
    if (didRespecify)
    {
        ASSERT(debug_is_from_texture_sync || oldCreateFlags == mImageCreateFlags);
        oldCreateFlags = mImageCreateFlags;
    }
    ...
    if (!isGenerateMipmap && mImage && mImage->valid() && debug_is_from_texture_sync)
    {
    ...
    {
        ASSERT(debug_is_from_texture_sync || oldUsageFlags == mImageUsageFlags);
        ASSERT(debug_is_from_texture_sync || oldCreateFlags == mImageCreateFlags);
        ASSERT(debug_is_from_texture_sync || !isMipmapEnabledByMinFilter);
        ANGLE_TRY(respecifyImageStorage(contextVk));
    }
                                   ...
                                   gl::Command source)
{
    ...
    ANGLE_TRY(respecifyImageStorageIfNecessary(contextVk, source, true));
