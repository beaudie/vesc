...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    if (needRedefineImage)
    {
        ...
        ANGLE_TRY(CopyImageToBufferAndUpdate(contextVk, baseLevelDesc, mImage->getLayerCount(), 0,
                                             mImage->getBaseLevel()));
                                                    ...
                                                    uint32_t stagingDstMipLevel)
{
    ...
    gl::Extents updatedExtents(baseLevelExtents);
    gl::Box area(gl::Offset(), updatedExtents);
    ...
    ANGLE_TRY(copyImageDataToBuffer(contextVk, sourceMipLevel, layerCount, 0, area, &stagingBuffer,
    ...
    ANGLE_TRY(mImage->stageSubresourceUpdateFromBuffer(contextVk, bufferSize, stagingDstMipLevel, 0,
                                                       layerCount, updatedExtents, gl::Offset(),
...
angle::Result TextureVk::changeLevels(ContextVk *contextVk, GLuint baseLevel, GLuint maxLevel)
{
    ...
    for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
    {
        for (uint32_t level = 0; level < updateCount; level++)
        {
            ...
            ANGLE_TRY(CopyImageToBufferAndUpdate(contextVk, desc, 1, srcLevelVK, level));
        }
    }
