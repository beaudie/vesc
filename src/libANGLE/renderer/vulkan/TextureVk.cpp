...
angle::Result TextureVk::ensureImageAllocated(ContextVk *context)
{
    if (mImage == nullptr)
    {
        setImageHelper(context, new vk::ImageHelper(), mState.getType(), 0, 0, true);
    }
                               ...
                               bool selfOwned)
{
    ...
    mImage            = imageHelper;
    mImage->initStagingBuffer(context->getRenderer());
