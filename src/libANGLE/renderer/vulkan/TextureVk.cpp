                                          ...
                                          gl::Framebuffer *source)
{
    ...
    mImage.addWriteDependency(framebufferVk->getFramebuffer());
    ...
    LoadImageFunctionInfo loadFunction = destFormat.textureLoadFunctions(internalFormat.type);

    bool canBlit =
        renderer->hasTextureFormatFeatureBits(srcFormat.vkTextureFormat, VK_FORMAT_FEATURE_BLIT_SRC_BIT) &&
        renderer->hasTextureFormatFeatureBits(destFormat.vkTextureFormat, VK_FORMAT_FEATURE_BLIT_DST_BIT);

    // If there is no conversion necessary and source and destination support blit, use that.
    if (!loadFunction.requiresConversion && canBlit)
    {
        if (reallocateImage)
        {
            if (mImage.valid())
            ...
            ANGLE_TRY(setStorageImpl(contextVk, index.getLevelIndex(), destFormat, extents));
        }
        return copySubImageImplWithBlit(contextVk, index, modifiedDestOffset, clippedSourceArea, framebufferVk);
    }
    ...
    ANGLE_TRY(mPixelBuffer.stageSubresourceUpdateFromFramebuffer(
        context, index, clippedSourceArea, modifiedDestOffset, extents, internalFormat, framebufferVk));
                                   ...
                                   FramebufferVk *source)
{
    ...
    vk::ImageHelper *srcImage = source->getColorReadRenderTarget()->getImageForRead(&mImage,
