                                                    ...
                                                    uint32_t stagingDstMipLevel)
{
    ...
    /* vk::BufferHelper *stagingBuffer  = nullptr;
     ...
     ANGLE_TRY(copyImageDataToBuffer(
             contextVk, sourceMipLevel, mImage->getLayerCount(), 0,
             gl::Box(0, 0, 0, baseLevelExtents.width, baseLevelExtents.height,
     baseLevelExtents.depth), &stagingBuffer, &stagingBufferOffset, nullptr));

     // Stage an update to the new image
     size_t bufferSize = baseLevelExtents.width * baseLevelExtents.height * baseLevelExtents.depth *
     baseLevelFormat.pixelBytes; ANGLE_TRY(mImage->stageSubresourceUpdateFromBuffer( contextVk,
     bufferSize, stagingDstMipLevel, 0, mImage->getLayerCount(), baseLevelExtents, gl::Offset(),
     stagingBuffer, stagingBufferOffset));*/
