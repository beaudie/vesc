                                                  ...
                                                  GLuint64 offset)
{
    ...
    vk::ImageHelper *image = nullptr;
    ANGLE_TRY(
        memoryObjectVk->createImage(context, type, levels, internalFormat, size, offset, &image));
    ASSERT(image);

    setImageHelper(renderer, image, mState.getType(), format, 0, 0, true);
