                                                  ...
                                                  GLbitfield usageFlags)
{
    ...
    ANGLE_TRY(initImageViews(contextVk, format, glFormat.info->sized, 0,
                                           ...
                                           egl::Image *image)
{
    ...
    const uint32_t baseLevel = mState.getEffectiveBaseLevel();
    ...
    setImageHelper(contextVk, imageVk->getImage(), imageVk->getImageTextureType(), format,
                   ...
                   gl::LevelIndex(baseLevel), false);

    initImageUsageFlags(contextVk, format);

    ASSERT(type != gl::TextureType::CubeMap);
    ANGLE_TRY(initImageViews(contextVk, format, image->getFormat().info->sized, baseLevel, 1, 1));
                                             ...
                                             bool maxLevelChanged)
{
    ...
    if (!respecifyImage)
    {
        ...
        return initImageViews(contextVk, mImage->getFormat(), baseLevelDesc.format.info->sized,
                              baseLevel.get(), maxLevel - baseLevel + 1, layerCount);
    }
...
angle::Result TextureVk::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    setImageHelper(contextVk, offscreenSurface->getColorAttachmentImage(), mState.getType(), format,
                   surface->getMipmapLevel(), 0, gl::LevelIndex(baseLevel), false);

    ASSERT(mImage->getLayerCount() == 1);
    gl::Format glFormat(internalFormat);
    return initImageViews(contextVk, format, glFormat.info->sized, baseLevel, 1, 1);
}
                                   ...
                                   gl::Command source)
{
          ...
          mImage->getLevelCount() != getMipLevelCount(ImageMipLevels::FullMipChain))))
    {
        ...
        // Flush staged updates to the base level of the image.  Note that updates to the rest
        // of the levels have already been discarded through the |removeStagedUpdates| call
        // above.
        ANGLE_TRY(flushImageStagedUpdates(contextVk));
                                   ...
                                   const uint32_t levelCount)
{
    ...
    ANGLE_TRY(initImageViews(contextVk, format, sized, firstLevel, levelCount, layerCount));
                                        ...
                                        uint32_t layerCount)
{
    ...
    gl::LevelIndex firstLevelGL = getNativeImageLevel(gl::LevelIndex(firstLevel));
    vk::LevelIndex firstLevelVk = mImage->toVkLevel(firstLevelGL);
    ...
    ANGLE_TRY(getImageViews().initReadViews(contextVk, mState.getType(), *mImage, format,
                                            formatSwizzle, readSwizzle, firstLevelVk, levelCount,
...
angle::Result TextureVk::refreshImageViews(ContextVk *contextVk)
{
    ...
    ANGLE_TRY(initImageViews(contextVk, mImage->getFormat(), baseLevelDesc.format.info->sized,
                             baseLevel, mImage->getLevelCount(), layerCount));
