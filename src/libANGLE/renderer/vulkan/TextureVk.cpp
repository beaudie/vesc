...
bool HasAnyRedefinedLevels(const gl::TexLevelArray<gl::CubeFaceMask> &redefinedLevels)
{
    for (gl::CubeFaceMask redefinedLevel : redefinedLevels)
    {
        if (redefinedLevel.any())
        {
            return true;
        }
    }
...
bool IsLevelRedefined(const gl::TexLevelArray<gl::CubeFaceMask> &redefinedLevels,
                      gl::LevelIndex level)
{
    return redefinedLevels[level.get()].any();
}
}  // anonymous namespace
                                     ...
                                     angle::FormatID dstImageFormatID) const
{
    ...
    if (!mOwnsImage)
    {
        ...
        ASSERT(!IsLevelRedefined(mRedefinedLevels, textureLevelIndexGL));
        return false;
    }
    ...
    return IsLevelRedefined(mRedefinedLevels, textureLevelIndexGL);
}
    ...
    if (mImage != nullptr)
    {
        ...
        if (mImage->valid())
        {
            ...
            if (isInAllocatedImage)
            {
                ...
                mRedefinedLevels[levelIndexGL.get()].set(redefinedFace, !isCompatibleRedefinition);
            }
...
angle::Result TextureVk::reinitImageAsRenderable(ContextVk *contextVk, const vk::Format &format)
{
    ...
    if (levelCount == 1 && layerCount == 1 &&
        !IsLevelRedefined(mRedefinedLevels, mImage->getFirstAllocatedLevel()))
    {
    ...
    for (vk::LevelIndex levelVk(0); levelVk < vk::LevelIndex(levelCount); ++levelVk)
    {
        ...
        if (IsLevelRedefined(mRedefinedLevels, levelGL))
        {
            continue;
        }
...
void TextureVk::prepareForGenerateMipmap(ContextVk *contextVk)
{
    ...
    // cleared.
    for (gl::LevelIndex level = firstGeneratedLevel; level <= maxLevel; ++level)
    {
        mRedefinedLevels[level.get()].reset();
    }
    ...
    if (IsLevelRedefined(mRedefinedLevels, baseLevel))
    {
        ASSERT(!mState.getImmutableFormat());
        releaseImage(contextVk);
    }
