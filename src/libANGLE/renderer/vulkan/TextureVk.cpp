                                           ...
                                           egl::Image *image)
{
    ...
    // Need the internalformat from the EGL image
    mOriginalInternalformat = image->getFormat().info->internalFormat;
...
angle::Result TextureVk::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    mOriginalInternalformat  = surface->getConfig()->renderTargetFormat;
    const vk::Format &format = renderer->getFormat(mOriginalInternalformat);
