...
uint32_t TextureVk::getNativeImageLevelLayerCount(uint32_t frontendLevel) const
{
    if (mState.getType() == gl::TextureType::CubeMap)
    {
        return gl::kCubeFaceCount;
    }
    else if (gl::IsArrayTextureType(mState.getType()))
    {
        const gl::ImageIndex &index = gl::ImageIndex::MakeFromType(mState.getType(), frontendLevel);
        const gl::ImageDesc &desc   = mState.getImageDesc(index);
        return desc.size.depth;
    }
    else
    {
        return 1;
                                   ...
                                   vk::CommandBuffer *commandBuffer)
{
    ...
    ANGLE_TRY(
        mImage->init(contextVk, mState.getType(), extents, format, 1, imageUsageFlags, levelCount));
                                        ...
                                        uint32_t levelCount)
{
    ...
    uint32_t layerCount = getNativeImageLevelLayerCount(0);
