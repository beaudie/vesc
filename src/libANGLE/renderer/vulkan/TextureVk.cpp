                                       ...
                                       const gl::Extents &size)
{
    ...
    if (mImage != nullptr)
    {
        ...
        if (mImage->valid())
        {
            ...
            ANGLE_TRY(TextureRedefineLevel(
                ...
                IsTextureLevelDefinitionCompatibleWithImage(
                    *mImage, levelIndexGL, size, format.getIntendedFormatID(),
                    format.getActualImageFormatID(getRequiredImageAccess())),
                mState.getImmutableFormat(), mImage->getLevelCount(), layerIndex, index,
                mImage->getFirstAllocatedLevel(), mRedefinedLevels, shouldReleaseImage));
