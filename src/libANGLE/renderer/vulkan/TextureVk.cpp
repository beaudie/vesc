...
bool TextureVk::isMipImageDescAppropriate(gl::TextureTarget textureTarget, size_t level)
{
    // Image should have defined width, height, and format.
...
bool TextureVk::isTextureConsistentlySpecifiedForFlush()
{
    ...
    // Check if the first two mip levels are present and have appropriate values.
    ...
    if (!isMipImageDescAppropriate(textureTarget, 0) ||
        !isMipImageDescAppropriate(textureTarget, 1))
    {
        return false;
    }

    // Acquire the defined mip levels to check if it is appropriate to flush the texture updates.
    gl::ImageDesc baseImageDesc = mState.getImageDesc(textureTarget, 0);

    for (size_t image = 1; image < mState.getImageDescs().size(); image++)
    {
        gl::ImageDesc mipImageDesc = mState.getImageDesc(textureTarget, image);
        if (!isMipImageDescAppropriate(textureTarget, image))
        {
            continue;
        }
...
void TextureVk::releaseAndDeleteImageAndViews(ContextVk *contextVk)
{
    ...
    contextVk->getShareGroupVk()->releasePrevTexture(this);
