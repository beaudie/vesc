...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    if (!mImage->valid())
    {
        ...
        {
            ANGLE_TRY(ensureImageInitialized(contextVk, vk::ImageMipLevels::FullMipChain));
            ASSERT(mImage->valid());
...
angle::Result TextureVk::ensureImageInitialized(ContextVk *contextVk, vk::ImageMipLevels mipLevels)
{
    const gl::ImageDesc &baseLevelDesc  = mState.getBaseLevelDesc();
...
uint32_t TextureVk::getMipLevelCount(vk::ImageMipLevels mipLevels) const
{
    switch (mipLevels)
    {
        case vk::ImageMipLevels::EnabledLevels:
            return mState.getEnabledLevelCount();
        case vk::ImageMipLevels::FullMipChain:
