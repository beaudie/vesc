                         ...
                         const vk::Format &destFormat)
{
    ...
    return srcFormat.internalFormat == destFormat.internalFormat &&
           renderer->hasTextureFormatFeatureBits(srcFormat.vkImageFormat,
                                                 VK_FORMAT_FEATURE_TRANSFER_SRC_BIT) &&
           renderer->hasTextureFormatFeatureBits(destFormat.vkImageFormat,
...
bool CanCopyWithDraw(RendererVk *renderer,
                     ...
                     const vk::Format &destFormat)
{
    return renderer->hasTextureFormatFeatureBits(srcFormat.vkImageFormat,
...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    // only.
    if (renderer->hasTextureFormatFeatureBits(mImage->getFormat().vkImageFormat, kBlitFeatureFlags))
    {
        ANGLE_TRY(ensureImageInitialized(contextVk));
