                                                    ...
                                                    vk::ImageViewHelper *destViews)
{
         ...
         destBaseLevel += maxGenerateLevels)
    {
        ...
        for (uint32_t layer = 0; layer < mImage->getLayerCount(); ++layer)
        {
            ...
            uint32_t srcLevel             = 0;
            vk::ImageHelper *src          = mImage;
            vk::ImageViewHelper *srcViews = &mImageViews;
            ...
            ANGLE_TRY(
                srcViews->getLevelLayerDrawImageView(contextVk, *src, srcLevel, layer, &srcView));
            ...
            ANGLE_TRY(contextVk->getUtils().generateMipmap(contextVk, src, srcView, dest,
...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
                                     ...
                                     mImage->getSamples()))
    {
        ...
        if ((mImageUsageFlags & VK_IMAGE_USAGE_STORAGE_BIT) == 0)
        {
            ...
            constexpr VkImageUsageFlags kIntermediateImageUsage = VK_IMAGE_USAGE_SAMPLED_BIT |
                                                                  VK_IMAGE_USAGE_STORAGE_BIT |
                                                                  VK_IMAGE_USAGE_TRANSFER_SRC_BIT;

            ANGLE_TRY(intermediate.get().init(contextVk, mState.getType(), mip1Extents,
                                              mImage->getFormat(), 1, kIntermediateImageUsage, 0,
                                              maxLevel - 1, maxLevel, mImage->getLayerCount()));
                                   ...
                                   gl::TextureCommand source)
{
    ...
    if (isGenerateMipmap)
    {
        ...
        if (!renderer->getFeatures().avoidStorageFlagAfterGenerateMipmap.enabled &&
            CanGenerateMipmapWithCompute(renderer, imageType, format, samples))
        {
            mImageUsageFlags |= VK_IMAGE_USAGE_STORAGE_BIT;
        }
    }
