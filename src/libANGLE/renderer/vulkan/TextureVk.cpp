...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    if (!mImage->valid())
    {
        ...
        if (mImage->hasStagedUpdates())
        {
            ...
            ANGLE_TRY(ensureImageInitializedImpl(
                contextVk, baseLevelExtents, getMaxLevelCount() - mState.getEffectiveBaseLevel(),
                format));
            ASSERT(mImage->valid());
    ...
    if (needRedefineImage)
    {
        ...
        ANGLE_TRY(CopyImageToBufferAndUpdate(contextVk, baseLevelDesc, 0, mImage->getBaseLevel()));

        // Release the origin image and recreate it with new mipmap counts.
        releaseImage(contextVk);
        ANGLE_TRY(ensureImageInitializedImpl(contextVk, baseLevelExtents,
...
angle::Result TextureVk::CopyImageToBufferAndUpdate(ContextVk *contextVk,
                                                    ...
                                                    uint32_t stagingDstMipLevel)
{
    ...
    /*
        for (uint32_t layer = 0; layer < mImage->getLayerCount(); layer++)
        {
            for (uint32_t level = 0; level < mImage->getLevelCount(); level++)
            {
                ...
                const gl::ImageDesc &subdesc =
                    mState.getImageDesc(gl::TextureTypeToTarget(mState.getType(), layer), level);
                const gl::Extents &subextents  = subdesc.size;
                const gl::InternalFormat &info = *subdesc.format.info;
                ...
                uint32_t srcLevelVK =
                    mImage->getBaseLevel() != 0 ? level - mImage->getBaseLevel() : level;
                ASSERT(srcLevelVK <= mImage->getLevelCount());

                // Adjust offset and depth based on our knowledge of image type here
                gl::Box area(0, 0, 0, subextents.width, subextents.height, subextents.depth);
       ...
       &stagingBuffer, &stagingBufferOffset, nullptr));
                ...
                size_t bufferSize =
                    subextents.width * subextents.height * subextents.depth * info.pixelBytes;
                ANGLE_TRY(mImage->stageSubresourceUpdateFromBuffer(contextVk, bufferSize, level,
       layer, 1, subextents, gl::Offset(), stagingBuffer, stagingBufferOffset));
            }
        }
        */
    // Adjust offset and depth based on our knowledge of image type here
    gl::Box area(0, 0, 0, baseLevelExtents.width, baseLevelExtents.height, baseLevelExtents.depth);
    if (gl::IsArrayTextureType(mState.getType()))
    {
        area.z     = 0;
        area.depth = 1;
    ...
    vk::BufferHelper *stagingBuffer  = nullptr;
    VkDeviceSize stagingBufferOffset = 0;
    ANGLE_TRY(copyImageDataToBuffer(contextVk, sourceMipLevel, mImage->getLayerCount(), 0, area,
                                    &stagingBuffer, &stagingBufferOffset, nullptr));

    // Stage an update to the new image
    size_t bufferSize = baseLevelExtents.width * baseLevelExtents.height * baseLevelExtents.depth *
                        baseLevelFormat.pixelBytes;
    ANGLE_TRY(mImage->stageSubresourceUpdateFromBuffer(
        contextVk, bufferSize, stagingDstMipLevel, 0, mImage->getLayerCount(), baseLevelExtents,
        gl::Offset(), stagingBuffer, stagingBufferOffset));
...
angle::Result TextureVk::ensureImageInitialized(ContextVk *contextVk)
{
    const gl::ImageDesc &baseLevelDesc  = mState.getBaseLevelDesc();
    const gl::Extents &baseLevelExtents = baseLevelDesc.size;
    const uint32_t levelCount           = getEnabledLevelCount();
...
uint32_t TextureVk::getEnabledLevelCount() const
{
