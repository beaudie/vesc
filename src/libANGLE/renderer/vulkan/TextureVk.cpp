                                         ...
                                         const vk::Format &vkFormat)
{
    ...
    if (unpackBuffer)
    {
        ...
        ANGLE_VK_CHECK_MATH(
            contextVk, formatInfo.computeDepthPitch(area.height, unpack.imageHeight, inputRowPitch,
                                                    &inputDepthPitch));
        ANGLE_VK_CHECK_MATH(
            contextVk,
            formatInfo.computeSkipBytes(type, inputRowPitch, inputDepthPitch, unpack,
                                        getNativeImageIndex(index).usesTex3D(), &inputSkipBytes));
                                                      ...
                                                      vk::ImageHelper *srcImage)
{
    ...
    if (mImage->valid())
    {
        ...
        // Also update the serial of the srcBuffer
                                               ...
                                               size_t offset)
{
    ...
    ASSERT(offset % kBufferOffsetMultiple == 0);
    ...
    region.imageExtent.depth  = index.getType() == gl::TextureType::_2DArray ? 0 : sourceArea.depth;
    ...
    region.imageSubresource.layerCount =
        index.getType() == gl::TextureType::_2DArray ? sourceArea.depth : 1;
