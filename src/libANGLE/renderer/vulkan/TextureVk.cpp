...
void GetVkClearValueFromBytes(uint8_t *constValue,
                              const gl::InternalFormat &formatInfo,
                              GLenum type,
                              VkClearValue *clearValueOut)
{
    *clearValueOut          = {};
    uint32_t componentCount = formatInfo.componentCount;
    ...
    if (!formatInfo.isDepthOrStencil())
    {
        switch (type)
        {
            ...
            {
                uint8_t byteData[4];
                memcpy(byteData, constValue, componentCount * sizeof(uint8_t));
                for (uint32_t i = 0; i < componentCount; i++)
                {
                    clearValueOut->color.float32[i] = static_cast<float>(byteData[i]) / 255.0;
                }

                // TODO: Hack for RGB; generalize using actual format?
                if (componentCount == 3)
                {
                    clearValueOut->color.float32[3] = 1.0;
                }
                break;
            }
            case GL_FLOAT:
            {
                float byteData[4];
                memcpy(byteData, constValue, componentCount * sizeof(float));
                for (uint32_t i = 0; i < componentCount; i++)
                {
                    clearValueOut->color.float32[i] = byteData[i];
                }
                break;
            }
            ...
            {
                uint32_t byteData[4];
                memcpy(byteData, constValue, componentCount * sizeof(uint32_t));
                for (uint32_t i = 0; i < componentCount; i++)
                {
                    clearValueOut->color.uint32[i] = byteData[i];
                }
                break;
            }
            case GL_INT:
            {
                int32_t byteData[4];
                memcpy(byteData, constValue, componentCount * sizeof(int32_t));
                for (uint32_t i = 0; i < componentCount; i++)
                {
                    clearValueOut->color.int32[i] = byteData[i];
                }
                break;
            }
            default:
    ...
    {
        // TODO: Depth/Stencil together?
        bool hasDepth   = formatInfo.depthBits > 0;
        bool hasStencil = formatInfo.stencilBits > 0;
        ...
        if (hasDepth)
        {
            switch (type)
            {
                ...
                {
                    float byteData;
                    memcpy(&byteData, constValue, sizeof(float));
                    clearValueOut->depthStencil.depth = byteData;
                    break;
                }
                case GL_UNSIGNED_INT:
                {
                    uint32_t byteData;
                    memcpy(&byteData, constValue, sizeof(uint32_t));
                    clearValueOut->depthStencil.depth =
                        static_cast<float>(byteData) /
        ...
        //            if (formatInfo.isInt())
        //            {
        //                memcpy(reinterpret_cast<uint8_t *>(&clearValueOut->depthStencil.depth),
        //                constValue,
        //                       formatInfo.pixelBytes);
        //            }
        //            else
        //            {
        //                clearValueOut->depthStencil.depth = *reinterpret_cast<float
        //                *>(constValue);
        //            }
        //        }
        ...
        //            if (formatInfo.isInt())
        //            {
        //                memcpy(reinterpret_cast<uint8_t *>(&clearValueOut->depthStencil.stencil),
        //                       constValue, formatInfo.pixelBytes);
        //            }
        //            else
        //            {
        //                clearValueOut->depthStencil.stencil = *reinterpret_cast<uint32_t
        //                *>(constValue);
        //            }
        //        }
    }
                                       ...
                                       const uint8_t *data)
{
    ...
    // TODO: Add D/S modes for GetVkClearValueFromBytes().
    VkClearValue clearValue = {};

    if (!formatInfo.isDepthOrStencil() &&
        (textureType == gl::TextureType::_2D || textureType == gl::TextureType::_2DMultisample))
    {
        GetVkClearValueFromBytes(constValue.data(), formatInfo, type, &clearValue);
    }
