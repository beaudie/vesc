                                   ...
                                   gl::TextureCommand source)
{
    ...
    if (isGenerateMipmap)
    {
        ...
        uint32_t levelsMask = angle::Bit<uint32_t>(maxLevel + 1 - baseLevel) - 1;
    ...
    {
        // If base level is incompatibly redefined, the image is going to be recreated.  Don't try
        // to preserve the other mips.
        ...
        mImage->getLevelCount() != getMipLevelCount(ImageMipLevels::FullMipChain))
    {
        ...
        ANGLE_TRY(ensureImageInitialized(contextVk, ImageMipLevels::EnabledLevels));
        ANGLE_TRY(copyAndStageImageSubresource(contextVk, false, getNativeImageLayer(0), 0,
                                               mImage->getBaseLevel()));
