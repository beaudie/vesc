                                   ...
                                   gl::Command source)
{
    ...
    // image must already be deleted, and the sampler reset.  Texture buffers are immutable, so it's
    // impossible to get out of this state.
    if (mState.getBuffer().get() != nullptr)
    {
        ...
        return mBufferView.initView(contextVk, buffer, vkFormat, bufferBinding.getOffset(),
                                    bufferBinding.getSize());
    }
