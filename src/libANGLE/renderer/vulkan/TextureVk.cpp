                                            ...
                                            TextureVk *source)
{
    ...
    const vk::Format &sourceVkFormat = source->getImageForCopy().getFormat();
    VkImageTiling srcTilingMode      = source->getImageForCopy().getTilingMode();
        ...
        !forceCPUPath)
    {
        return copySubImageImplWithDraw(
            ...
            unpackFlipY, unpackPremultiplyAlpha, unpackUnmultiplyAlpha, &source->getImageForCopy(),
                                               ...
                                               size_t offset)
{
    ...
    // TODO: should also set rendertarget mContentDefined
...
angle::Result TextureVk::generateMipmap(const gl::Context *context)
{
    ...
    // TODO: should set render targets having defined contents
                                                   ...
                                                   FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    if (samples > 1 && mMultisampledImage == nullptr)
    {
        ASSERT(mState.getBaseLevelDesc().samples <= 1);

        mMultisampledImage = new vk::ImageHelper();
        ...
        constexpr VkImageUsageFlags kMultisampledUsageFlags =
            mImage->getAspectFlags() == VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT |
        ...
        constexpr VkMemoryPropertyFlags kMultisampledMemoryFlags =
                                           ...
                                           GLuint levelIndex)
{
    ...
    for (uint32_t layerIndex = 0; layerIndex < layerCount; ++layerIndex)
    {
        ...
        if (mMultisampledImage)
        {
            ...
            // TODO(syoussefi): If the texture is depth/stencil,
            // GL_EXT_multisampled_render_to_texture2 explicitly indicates that there is no need
            // for the image to be resolved.  However, not specifying a resolve attachment currently
            // would indicate that the multisampled depth/stencil attachment should have STORE as
            // its storeOp, while it should be DONT_CARE.  This information needs to be communicated
            // to the render target when that extension is implemented.  http://anglebug.com/4836
        }
