...
class FlushThreadVk
{
  public:
    ...
    // Waits for the submission thread to become idle.  This is generally called interally, but is
  ...
  private:
    ...
    // Synchronization primitives used for passing parameters to the submission thread and getting
    // the result.  SubmitData::op is used by the main thread to notify the submission thread that a
    // job is available.  It's also used by the submission thread to notify the main thread that the
    ...
    // image.
    //
    // Created on the main thread, and handed off to the submission thread on every flush.
