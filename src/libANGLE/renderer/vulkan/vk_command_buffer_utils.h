...
class CommandBufferCommandTracker
{
  public:
    ...
    uint32_t getCopyCommandCount() const { return mCopyCommandCount; }
    ...
    void resetCopyCommandCount() { mCopyCommandCount = 0; }
  ...
  private:
    ...
    // The number and size of copy commands used from buffers and images to images. Used to submit
    // the command buffer for the outside render pass.
    uint32_t mCopyCommandCount = 0;
