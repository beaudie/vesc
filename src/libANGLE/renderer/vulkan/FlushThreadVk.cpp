...
angle::Result FlushThreadVk::initialize(vk::Context *context)
{
    ...
    if (kEnableThreading)
    {
        // Create the flush thread now that everything else is initialized.
...
VkResult FlushThreadVk::waitForPreviousOperation()
{
    ...
    TRACE_EVENT0(kTraceEventCagetory, "FlushThreadVk::waitForPreviousOperation");
...
bool FlushThreadVk::hasPendingWork()
{
    if (!kEnableThreading)
    {
        return false;
    ...
    std::unique_lock<std::mutex> lock(mSubmitMutex);
    return mSubmitData.op != SubmitOp::Done;
}
