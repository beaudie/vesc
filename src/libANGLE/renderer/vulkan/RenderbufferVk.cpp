                                             ...
                                             gl::MultisamplingMode mode)
{
    ...
    if (isRenderToTexture && !hasRenderToTextureEXT)
    {
        ...
        ANGLE_VK_TRY_ALLOC(contextVk, result,
                           mMultisampledImage.initImplicitMultisampledRenderToTexture(
                               contextVk, false, renderer->getMemoryProperties(),
                               gl::TextureType::_2D, samples, *mImage, robustInit, &result));
                                                   ...
                                                   void *pixels)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        contextVk, result,
        mImage->readPixelsForGetImage(contextVk, packState, packBuffer, gl::LevelIndex(0), 0, 0,
                                      format, type, pixels, &result));
    return angle::Result::Continue;
}
