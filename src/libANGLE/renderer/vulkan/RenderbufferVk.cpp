                                             ...
                                             gl::MultisamplingMode mode)
{
    ...
    ANGLE_VK_TRY_ALLOC(
        ...
        mImage->initMemory(contextVk, false, renderer->getMemoryProperties(), flags,
                           vk::MemoryAllocationType::RenderBufferStorageImage, &isOutOfMemory));
    ...
    if (isRenderToTexture && !hasRenderToTextureEXT)
    {
        ...
        ANGLE_VK_TRY_ALLOC(
            contextVk, mMultisampledImage.initImplicitMultisampledRenderToTexture(
                           ...
                           samples, *mImage, robustInit, &isOutOfMemory));
                                                   ...
                                                   void *pixels)
{
    ...
    ANGLE_VK_TRY_ALLOC(contextVk, mImage->readPixelsForGetImage(contextVk, packState, packBuffer,
                                                                gl::LevelIndex(0), 0, 0, format,
                                                                type, pixels, &isOutOfMemory));
    return angle::Result::Continue;
}
