                                         ...
                                         size_t height)
{
    ...
    if ((mImage == nullptr || !mImage->valid()) && (width != 0 && height != 0))
    {
        ...
        // Clear the renderbuffer if WebGL or has emulated channels.  This could be deferred to
        // first use and optimized by using a renderpass loadOp=Clear, but would require extra
        // tracking in RenderTargetVk.
        if (context->isWebGL() || vkFormat.hasEmulatedChannels())
        {
