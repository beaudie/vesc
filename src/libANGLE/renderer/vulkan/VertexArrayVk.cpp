...
four cases
buffer pass thru
buffer cvt - dirty data only applies here
stream
default
*/

gl::Error VertexArrayVk::syncDirtyAttrib(const gl::Context *context,
                                         ...
                                         size_t attribIndex)
{
    ...
    if (attrib.enabled)
    {
        ...
        {
            BufferVk *bufferVk = vk::GetImpl(bufferGL);
            const angle::Format& angleFormat = mCurrentArrayBufferFormats[attribIndex]->angleFormat();
            size_t componentSize = gl::VariableComponentSize(angleFormat.componentType);
            bool isAligned = binding.getOffset() % componentSize == 0 && binding.getStride() % componentSize == 0;

            if (mCurrentArrayBufferFormats[attribIndex]->vertexLoadRequiresConversion || !isAligned)
            {
                // Make another buffer and put converted data into it.
                unsigned dstVertexSize =
                    mCurrentArrayBufferFormats[attribIndex]->bufferFormat().pixelBytes;
                unsigned numVertices = (bufferVk->getSize() - binding.getOffset() - angleFormat.pixelBytes) /
                                           binding.getStride() +
                                       1;
                ...
                ANGLE_TRY(mCurrentArrayBufferConversion[attribIndex].allocate(
                    contextVk, numVertices * dstVertexSize, &dst,
                    &mCurrentArrayBufferHandles[attribIndex], &offset, nullptr));
                void *src = nullptr;
                ANGLE_TRY(bufferVk->mapRange(context, binding.getOffset(),
                                             bufferVk->getSize() - binding.getOffset(),
                                             0 /*access*/, &src));
                mCurrentArrayBufferFormats[attribIndex]->vertexLoadFunction(
                    static_cast<const uint8_t *>(src), binding.getStride(), numVertices, dst);
                ANGLE_TRY(bufferVk->unmap(context, nullptr));
