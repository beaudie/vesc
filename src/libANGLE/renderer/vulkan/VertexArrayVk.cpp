...
/* TODO
- the whole buffer will be mapped after it is allocated (and unmapped after it's filled)
- map() will change to allocate() which will allocate memory if needed, and anyway return ptr,
handle, offset - unmap() will change to flush() which will flush everything since the last flush
*/
gl::Error VertexArrayVk::streamVertexData(ContextVk *context, GLsizei count)
{
    ...
    for (auto attribIndex : programGL->getActiveAttribLocationsMask())
    {
        ...
        if (b)
            // XXX cast is ugly.  changing the array from BufferVk to ResourceVk was supposed to be
            // an improvement...
        ...
        if (attrib.enabled && !bufferGL)
        {
            ...
            uint8_t *dst;
            ANGLE_TRY(vertexData->allocate(count * dstStride, &dst, &mHandles[attribIndex],
