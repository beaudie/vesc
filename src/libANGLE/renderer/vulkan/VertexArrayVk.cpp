                                             ...
                                             bool bufferOnly)
{
    ...
    if (attrib.enabled)
    {
        ...
        else
        {
            ...
            bool combined = !vertexFormat.getVertexLoadRequiresConversion(compressed) &&
                            binding.getDivisor() == 0 &&
...
std::vector<std::pair<uintptr_t, uintptr_t>> VertexArrayVk::mergeClientAttribsRange(
    RendererVk *renderer,
    ...
    size_t endVertex)
{
    const auto &attribs  = mState.getVertexAttributes();
    const auto &bindings = mState.getVertexBindings();
    std::vector<std::pair<uintptr_t, uintptr_t>> mergeRanges;
    for (size_t attribIndex : activeStreamedAttribs)
    {
        ...
        const gl::VertexBinding &binding = bindings[attrib.bindingIndex];
        const vk::Format &vertexFormat   = renderer->getFormat(attrib.format->id);
        if (vertexFormat.getVertexLoadRequiresConversion(false) || binding.getDivisor() > 0 ||
            !ClientBindingAligned(attrib, binding.getStride(),
                                  vertexFormat.getVertexInputAlignment(false)))
        {
            continue;
        ...
        mergeRanges.emplace_back(
            ...
            (uintptr_t)attrib.pointer + (endVertex - 1) * binding.getStride() + pixelBytes);
    }
    ...
    mergeRanges.resize(cur - mergeRanges.begin() + 1);
    return mergeRanges;
}
                                                   ...
                                                   const void *indices)
{
    ...
    auto mergeRanges = mergeClientAttribsRange(renderer, activeStreamedAttribs, startVertex,
                                               startVertex + vertexCount);
    std::vector<vk::BufferHelper *> attribBufferHelper(mergeRanges.size());
    for (size_t attribIndex : activeStreamedAttribs)
    {
        const gl::VertexAttribute &attrib = attribs[attribIndex];
        ASSERT(attrib.enabled);
        ...
        bool needConversion   = vertexFormat.getVertexLoadRequiresConversion(compressed);
        if (needConversion)
        {
            ANGLE_VK_PERF_WARNING(contextVk, GL_DEBUG_SEVERITY_LOW,
                                  "The Vulkan driver does not support vertex attribute format "
                                  "0x%04X, emulating with 0x%04X",
                                  vertexFormat.getIntendedFormat().glInternalFormat,
                                  vertexFormat.getActualBufferFormat(compressed).glInternalFormat);
        }
        ...
        {
            ASSERT(binding.getBuffer().get() == nullptr);
            size_t bytesToAllocate, destOffset;
            src += startVertex * binding.getStride();
            if (needConversion ||
                !ClientBindingAligned(attrib, binding.getStride(),
                                      vertexFormat.getVertexInputAlignment(compressed)))
            {
                destOffset      = startVertex * pixelBytes;
                bytesToAllocate = destOffset + vertexCount * pixelBytes;
                // Allocate buffer for results
                ANGLE_TRY(contextVk->allocateStreamedVertexBuffer(attribIndex, bytesToAllocate,
                                                                  &vertexDataBuffer));
            ...
            else
            {
                ...
                destOffset = startVertex * stride;
                ASSERT(mergeRanges.size() > 0);
                size_t i = 0;
                while (i < mergeRanges.size())
                {
                    if ((uintptr_t)src < mergeRanges[i].second)
                        break;
                    ++i;
                }
                ASSERT(i < mergeRanges.size() && (uintptr_t)src >= mergeRanges[i].first &&
                       mergeRanges[i].second > mergeRanges[i].first);
                bytesToAllocate = mergeRanges[i].second - mergeRanges[i].first;
                if (attribBufferHelper[i])
                {
                    vertexDataBuffer = attribBufferHelper[i];
                }
                else
                {
                    // Allocate buffer for results
                    ANGLE_TRY(contextVk->allocateStreamedVertexBuffer(attribIndex, bytesToAllocate,
                                                                      &vertexDataBuffer));
                    ANGLE_TRY(StreamVertexData(contextVk, vertexDataBuffer,
                                               (const uint8_t *)mergeRanges[i].first + destOffset,
                                               bytesToAllocate - destOffset, destOffset,
                                               vertexCount, binding.getStride(), nullptr));
                    attribBufferHelper[i] = vertexDataBuffer;
                }
                startOffset = (uintptr_t)attrib.pointer - mergeRanges[i].first;
                ASSERT(startOffset % vertexFormat.getVertexInputAlignment(compressed) == 0);
            }
        }
        ...
        mCurrentArrayBufferHandles[attribIndex] =
            vertexDataBuffer
                ...
                .getHandle();
        ASSERT(bufferOffset % vertexFormat.getVertexInputAlignment(compressed) == 0);
