                                          ...
                                          int lastVertex)
{
    ...
    for (auto attribIndex : programGL->getActiveAttribLocationsMask())
    {
        ...
        if (attrib.enabled && !bufferGL)
        {
            ...
            uint8_t *dst = nullptr;
            ANGLE_TRY(stream->allocate(context, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, lastByte, &dst,
    ...
    DrawType drawType)
{
    ...
    if (drawType == DrawType::Elements || overrideElementArrayBufferReadResource.valid())
    {
