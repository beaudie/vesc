...
size_t GetVertexCount(GLint64 srcBufferBytes, uint32_t srcFormatSize, uint32_t srcVertexStride)
{
...
size_t GetVertexCount(BufferVk *srcBuffer, const gl::VertexBinding &binding, uint32_t srcFormatSize)
{
    ...
    uint32_t stride = binding.getStride() == 0 ? srcFormatSize : binding.getStride();
