                                                    ...
                                                    bool compressed)
{
    ...
    GLint64 srcMaxBytes   = srcBuffer->getSize() - conversion->getCacheKey().offset;
    size_t maxNumVertices = GetVertexCountForRange(srcMaxBytes, srcFormatSize, srcStride);
    if (maxNumVertices == 0)
    {
        return angle::Result::Continue;
    }
                                                    ...
                                                    bool compressed)
{
    ...
    if (conversion->isEntireBufferDirty())
    {
        ...
        size_t dstOffset = 0;
        ANGLE_TRY(StreamVertexData(contextVk, dstBufferHelper, srcBytes, maxNumVertices * dstStride,
                                   dstOffset, maxNumVertices, srcStride,
