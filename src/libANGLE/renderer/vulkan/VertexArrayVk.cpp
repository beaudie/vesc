                                                   ...
                                                   const void *indices)
{
    ...
    vk::BufferHelper *combinedAttribBufferHelper = nullptr;
    for (size_t attribIndex : activeStreamedAttribs)
    {
        const gl::VertexAttribute &attrib = attribs[attribIndex];
        ASSERT(attrib.enabled);
        ...
        if (divisor > 0)
        {
            ...
            if (divisor > renderer->getMaxVertexAttribDivisor())
            {
                ...
                // Allocate buffer for results
                ANGLE_TRY(
                    contextVk->allocateStreamedVertexBuffer(bytesToAllocate, &vertexDataBuffer));
        ...
        {
            ASSERT(binding.getBuffer().get() == nullptr);
            if (combined && combinedAttribBufferHelper != nullptr)
            {
                vertexDataBuffer = combinedAttribBufferHelper;
            }
            else
            {
                ...
                combinedAttribBufferHelper = combined ? vertexDataBuffer : nullptr;
            }
...
angle::Result VertexArrayVk::updateDefaultAttrib(ContextVk *contextVk, size_t attribIndex)
{
    if (!mState.getEnabledAttributesMask().test(attribIndex))
    {
        ...
        ANGLE_TRY(contextVk->allocateStreamedVertexBuffer(kDefaultValueSize, &bufferHelper));
