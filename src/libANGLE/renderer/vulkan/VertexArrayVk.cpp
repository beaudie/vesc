                                             ...
                                             bool bufferOnly)
{
    ...
    if (attrib.enabled)
    {
        ...
        if (!isStreamingVertexAttrib)
        {
            ...
            {
                ConversionBuffer *conversion = bufferVk->getExistingVertexConversionBuffer(
                    intendedFormat.id, binding.getStride(),
                    binding.getOffset() + attrib.relativeOffset);

                // Only compress the first time, don't re-compress non-static data
                if (conversion == nullptr || conversion->conversionCount == 0)
                {
                    compressed = true;
                }
            }
            ...
            if (vertexFormat.getVertexLoadRequiresConversion(compressed) || !bindingIsAligned)
            {
                ...
                stride = vertexFormat.actualBufferFormat(false).pixelBytes;
            }
            else
            {
