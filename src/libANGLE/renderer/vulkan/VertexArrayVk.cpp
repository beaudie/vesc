...
angle::Result VertexArrayVk::streamIndexData(ContextVk *contextVk,
                                             ...
                                             vk::DynamicBuffer *dynamicBuffer)
{
    ASSERT(!mState.getElementArrayBuffer().get() || drawCallParams.type() == GL_UNSIGNED_BYTE);
                                                    ...
                                                    const gl::DrawCallParams &drawCallParams)
{
    ...
    if (!glBuffer)
    {
        ANGLE_TRY(streamIndexData(contextVk, drawCallParams, drawCallParams.indices(),
                                  &mDynamicIndexData));
    }
    else
    {
        ...
        ANGLE_TRY(streamIndexData(contextVk, drawCallParams, srcData, &mTranslatedByteIndexData));
