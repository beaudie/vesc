    ...
    size_t endVertex)
{
    ...
    for (size_t attribIndex : activeStreamedAttribs)
    {
        ...
        mergeRanges.emplace_back(
            ...
            (intptr_t)attrib.pointer + endVertex * binding.getStride() + pixelBytes);
    }
    std::sort(mergeRanges.begin(), mergeRanges.begin());
    ...
    for (size_t attribIndex : activeStreamedAttribs)
    {
        ...
        else
        {
            ...
            if (needConversion)
            {
                ...
                // Only vertexCount vertices will be used by the upcoming draw so that is all we
                // copy.
            ...
            else
            {
                ...
                else
                {
                    ...
                    ANGLE_TRY(StreamVertexData(contextVk, vertexDataBuffer,
                                               (const uint8_t *)mergeRanges[i].first,
                                               bytesToAllocate, 0, vertexCount, binding.getStride(),
                                               vertexFormat.getVertexLoadFunction(compressed)));
                    attribBufferHelper[i] = vertexDataBuffer;
                }
                // this value may negtive
                startOffset =
                    (intptr_t)src - mergeRanges[i].first - startVertex * binding.getStride();
