...
// Handle copying client attribs or expanding attrib buffer in case where attribute
    ...
    for (size_t attribIndex : activeAttribs)
    {
        ...
        if (binding.getDivisor() > 0)
        {
            ...
            if (mEmulateAttribDivisorMask[attribIndex])
            {
                ...
                if (!clientAttribs[attribIndex])
                {
                    // TODO: Map GPU buffer and copy data, replicating
                    // See convertVertexBufferCPU() as a guide
                    UNIMPLEMENTED();
                }
