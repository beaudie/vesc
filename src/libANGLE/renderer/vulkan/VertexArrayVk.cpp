...
gl::Error VertexArrayVk::streamVertexData(const gl::Context *context, GLsizei count)
{
    ...
    for (auto attribIndex : programGL->getActiveAttribLocationsMask())
    {
        ...
        {
            //TODO (count-1)*stride + size of one vertex
            ANGLE_TRY(contextVk->storeVertexData(attrib.pointer, count * binding.getStride(), &mHandles[attribIndex], &mOffsets[attribIndex]));
        }
    }
