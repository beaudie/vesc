    ...
    std::vector<RangeSpan> &mergeRanges) const
{
    ...
    if (attributeMaskCanCombine.any())
    {
        ...
        // Merge range span.
        auto it  = mergeAttribIndexs.begin();
        auto cur = it++;
        while (it != mergeAttribIndexs.end() || (cur != it))
        {
            // cur and it overlaps: merge next into cur and move it next.
            if (it != mergeAttribIndexs.end() && mergeRanges[*cur].end >= mergeRanges[*it].start)
            {
            ...
            else
            {
                ...
                else if (it != mergeAttribIndexs.end())
                {
                    ++it;
                }
            }
                                                   ...
                                                   const void *indices)
{
    ...
    for (size_t attribIndex : activeStreamedAttribs)
    {
        ...
        else
        {
            ...
            {
                size_t srcOffset  = (it->copyStart - it->start);
                size_t destOffset = combined ? srcOffset : startVertex * stride;
                size_t bytesToAllocate =
                    combined ? (it->end - it->start) : destOffset + vertexCount * stride;
                ANGLE_TRY(contextVk->allocateStreamedVertexBuffer(attribIndex, bytesToAllocate,
                                                                  &vertexDataBuffer));
                ANGLE_TRY(StreamVertexData(
                    contextVk, vertexDataBuffer, (const uint8_t *)it->start + srcOffset,
