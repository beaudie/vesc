...
angle::Result StreamVertexDataWithReplication(ContextVk *contextVk,
                                              vk::BufferHelper *dstBufferHelper,
                                              const uint8_t *srcData,
                                              size_t bytesToAllocate,
                                              size_t srcStride,
                                              ...
                                              size_t numSrcVertices)
{
    ...
    // Each src vertex is reused `divisor` times
    size_t clampedSize = std::min(numSrcVertices * dstStride * divisor, bytesToAllocate);

    ASSERT(divisor > 1);
    uint32_t sameSrcRemainingCount = divisor;
    for (size_t dataCopied = 0; dataCopied < clampedSize; dataCopied += dstStride, dst += dstStride)
    {
        ...
        sameSrcRemainingCount--;
        if (sameSrcRemainingCount == 0)
        {
            srcData += srcStride;
            sameSrcRemainingCount = divisor;
        }
    }
        ...
        if (divisor > 0)
        {
            ...
            if (divisor > renderer->getMaxVertexAttribDivisor())
            {
                ...
                if (binding.getBuffer().get() != nullptr)
                {
                    ...
                    ANGLE_TRY(StreamVertexDataWithReplication(
