                                             ...
                                             size_t attribIndex)
{
    if (attrib.enabled)
    {
        // If any conversin happens in syncDirtyAttribBuffer, the stride would be overwrited
        contextVk->onVertexAttributeChange(attribIndex, binding.getStride(), binding.getDivisor(),
                                           attrib.format->id, attrib.relativeOffset);

        ANGLE_TRY(syncDirtyAttribBuffer(contextVk, attrib, binding, attribIndex));
    }
    else
    {
                                                   ...
                                                   size_t attribIndex)
{
    ...
    if (bufferGL)
    {
        ...
        if (vertexFormat.vertexLoadRequiresConversion || !bindingIsAligned)
        {
            ...
            contextVk->onVertexAttributeChange(attribIndex, binding.getStride(),
