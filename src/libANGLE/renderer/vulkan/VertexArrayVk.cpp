                                         ...
                                         size_t attribIndex)
{
    ...
    if (attrib.enabled)
    {
        ...
        if (bufferGL)
        {
            ...
            bool isAligned       = binding.getOffset() % componentSize == 0 &&
                             binding.getStride() % componentSize == 0;
            ...
            if (mCurrentArrayBufferFormats[attribIndex]->vertexLoadRequiresConversion || !isAligned)
            {
                ...
                size_t numVertices =
                    (bufferVk->getSize() - binding.getOffset() - angleFormat.pixelBytes) /
                        binding.getStride() +
                    1;
                // printf( "attrib %d offset %d stride %d src format %d dst format %d buf size %d
                // vert " "size %d num vert %d\n", (int)attribIndex, (int)binding.getOffset(),
                // (int)binding.getStride(),
                // (int)mCurrentArrayBufferFormats[attribIndex]->angleFormatID,
                // (int)mCurrentArrayBufferFormats[attribIndex]->bufferFormatID,
                // (int)bufferVk->getSize(), (int)srcVertexSize, (int)numVertices);
                uint8_t *dst    = nullptr;
                uint32_t offset = 0;
                ...
                ANGLE_TRY(bufferVk->mapRange(context, binding.getOffset(),
                                             static_cast<size_t>(bufferVk->getSize() - binding.getOffset()),
                                             0 /*access*/, &src));
                mCurrentArrayBufferFormats[attribIndex]->vertexLoadFunction(
