    ...
    if (attrib.enabled)
    {
        ...
        if (bufferGL)
        {
            ...
            if (mCurrentArrayBufferFormats[attribIndex]->vertexLoadRequiresConversion)
            {
                ...
                //XXX vertex 0 starts at whatever the offset is for this attrib
                ...
                printf("attrib %d offset %d stride %d format %d buf size %d vert size %d num vert %d\n", (int)attribIndex, (int)binding.getOffset(), (int)binding.getStride(), (int)mCurrentArrayBufferFormats[attribIndex]->angleFormatID, (int)bufferVk->getSize(), (int)srcVertexSize, (int)numVertices);
                uint8_t *dst    = nullptr;
                uint32_t offset = 0;
                VkBuffer handle;
