...
void VertexArrayVk::updateDefaultAttrib(size_t attribIndex, VkBuffer bufferHandle, uint32_t offset)
{
    if (!mState.getEnabledAttributesMask().test(attribIndex))
    {
        ...
        mCurrentArrayBufferResources[attribIndex] = nullptr;
        ASSERT(mCurrentArrayBufferStrides[attribIndex] == 0);
        ASSERT(mCurrentArrayBufferFormats[attribIndex]->angleFormatID ==
