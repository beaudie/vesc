      ...
      mDirtyLineLoopTranslation(true)
{
    ...
    for (vk::DynamicBuffer &dynamicVertexData : mDynamicVertexData)
    {
        dynamicVertexData.init(contextVk->getRenderer(), vk::kVertexBufferUsageFlags,
                               vk::kVertexBufferAlignment, kDynamicVertexDataSize, true,
...
void VertexArrayVk::destroy(const gl::Context *context)
{
    ...
    for (vk::DynamicBuffer &dynamicVertexData : mDynamicVertexData)
    {
        dynamicVertexData.release(renderer);
    }
                                                   ...
                                                   const void *indices)
{
    ...
    {
        mDynamicVertexData[attribIndex].releaseInFlightBuffers(contextVk);
        ...
        if (divisor > 0)
        {
            ...
            if (divisor > renderer->getMaxVertexAttribDivisor())
            {
                ...
                // Allocate buffer for results
                ANGLE_TRY(mDynamicVertexData[attribIndex].allocate(contextVk, bytesToAllocate,
