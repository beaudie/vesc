                                                   ...
                                                   const void *indices)
{
    ...
    for (size_t attribIndex : activeStreamedAttribs)
    {
        ...
        {
            // Instanced attrib
            // Use the same stride as the original vertex attribute. This can waste
            // some space, but expect this to be rare. Otherwise would need to
            // update call GraphicsPipelineDesc::updateVertexInput or similar to
            // update the Vulkan stride for this vertex attribute.
            stride = binding.getStride();
