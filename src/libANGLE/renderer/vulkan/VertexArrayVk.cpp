...
void VertexArrayVk::destroy(const gl::Context *context)
{
    ...
    for (vk::DynamicBuffer &buffer : mCurrentArrayBufferConversion)
    {
        buffer.release(contextVk, getStoredQueueSerials());
    }
    mDynamicVertexData.release(contextVk, getStoredQueueSerials());
    mDynamicIndexData.release(contextVk, getStoredQueueSerials());
    mTranslatedByteIndexData.release(contextVk, getStoredQueueSerials());
    mLineLoopHelper.release(contextVk, getStoredQueueSerials());
}
