...
void ProgramExecutableVk::reset(ContextVk *contextVk)
{
    ...
    mUniformsAndXfbDescriptorsCache.clear();
    mShaderBufferDescriptorsCache.clear();
    ...
    bool *newDescriptorSetAllocated)
{
    ...
    if (mUniformsAndXfbDescriptorsCache.get(
            xfbBufferDesc, &descriptorSet,
            &mPerfCounters.cacheStats[DescriptorSetIndex::UniformsAndXfb]))
    {
    ...
    mUniformsAndXfbDescriptorsCache.insert(
        xfbBufferDesc, mDescriptorSets[DescriptorSetIndex::UniformsAndXfb],
        &mPerfCounters.cacheStats[DescriptorSetIndex::UniformsAndXfb]);
                                                    ...
                                                    vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t imageIndex = 0; imageIndex < imageBindings.size(); ++imageIndex)
    {
        ...
        for (gl::ShaderType shaderType : executable.getLinkedShaderStages())
        {
            ...
            const ShaderInterfaceVariableInfo &info = mVariableInfoMap.getIndexedVariableInfo(
                executable, shaderType, IndexedShaderVariableType::Image, imageIndex);
            if (info.isDuplicate)
            {
                continue;
            }
    ...
    vk::DescriptorSetLayoutDesc *descOut)
{
    ...
    for (uint32_t textureIndex = 0; textureIndex < samplerBindings.size(); ++textureIndex)
    {
        ...
        for (gl::ShaderType shaderType : executable.getLinkedShaderStages())
        {
            ...
            const ShaderInterfaceVariableInfo &info = mVariableInfoMap.getIndexedVariableInfo(
                executable, shaderType, IndexedShaderVariableType::Texture, textureIndex);
            if (info.isDuplicate)
            {
                continue;
            }
    ...
    gl::ActiveTextureArray<vk::TextureUnit> *activeTextures)
{
    ...
    for (gl::ShaderType shaderType : linkedShaderStages)
    {
        addInterfaceBlockDescriptorSetDesc(glExecutable, glExecutable.getUniformBlocks(),
                                           shaderType, IndexedShaderVariableType::UniformBuffer,
        ...
        addInterfaceBlockDescriptorSetDesc(glExecutable, glExecutable.getShaderStorageBlocks(),
                                           shaderType,
                                           IndexedShaderVariableType::ShaderStorageBuffer,
...
void ProgramExecutableVk::resolvePrecisionMismatch(const gl::ProgramMergedVaryings &mergedVaryings)
{
    for (const gl::ProgramVaryingRef &mergedVarying : mergedVaryings)
    {
        ...
        if (frontPrecision > backPrecision)
        {
            ...
            ShaderInterfaceVariableInfo &info = mVariableInfoMap.getVaryingMutable(
    ...
    const vk::DescriptorSetDesc &shaderBuffersDesc)
{
    ...
    for (gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
        ANGLE_TRY(updateBuffersDescriptorSet(
            ...
            IndexedShaderVariableType::UniformBuffer, mUniformBufferDescriptorType,
            limits.maxUniformBufferRange, cacheHit));
        ANGLE_TRY(updateBuffersDescriptorSet(
            contextVk, updateBuilder, emptyBuffer, resourceUseList, shaderType, shaderBuffersDesc,
            storageBuffers, *executable, executable->getShaderStorageBlocks(),
            IndexedShaderVariableType::ShaderStorageBuffer, kStorageBufferDescriptorType,
    ...
    bool emulateSeamfulCubeMapSampling)
{
    ...
    if (mTextureDescriptorsCache.get(texturesDesc, &descriptorSet,
                                     &mPerfCounters.cacheStats[DescriptorSetIndex::Texture]))
    {
    ...
    for (gl::ShaderType shaderType : executable.getLinkedShaderStages())
    {
             ...
             ++textureIndex)
        {
            ...
            if (descriptorSet == VK_NULL_HANDLE)
            {
                ...
                mTextureDescriptorsCache.insert(
                    texturesDesc, descriptorSet,
                    &mPerfCounters.cacheStats[DescriptorSetIndex::Texture]);
            }
            ASSERT(descriptorSet != VK_NULL_HANDLE);
