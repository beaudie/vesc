...
angle::Result ProgramExecutableVk::mergeProgramExecutablePipelineCacheToRenderer(
    vk::Context *context) const
{
    // Merge the cache with Renderer's
    if (context->getFeatures().mergeProgramPipelineCachesToGlobalCache.enabled)
    {
        ANGLE_TRACE_EVENT0("gpu.angle",
                           "ProgramExecutableVk::mergeProgramExecutablePipelineCacheToRenderer");
        ANGLE_TRY(context->getRenderer()->mergeIntoPipelineCache(context, mPipelineCache));
    }
