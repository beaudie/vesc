    ...
    VkDescriptorType descriptorType)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
    {
        ...
        // Delay allocating the descriptor set, since we may not have one if all of the buffers
        // are inactive.
    ...
    vk::CommandBufferHelper *commandBufferHelper)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < atomicCounterBuffers.size(); ++bufferIndex)
    {
        ...
        // Delay allocating the descriptor set, since we may not have one if there are no buffer
        // bindings.
        if (descriptorSet == VK_NULL_HANDLE)
        {
            ANGLE_TRY(allocateDescriptorSet(contextVk, DescriptorSetIndex::ShaderResource));
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
             ...
             ++textureIndex)
        {
            ...
            for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
            {
                ...
                // Delay allocating the descriptor set, since we may not have one if all of the
                // buffers are inactive.
                VkDescriptorSet descriptorSet =
                    mDescriptorSets[ToUnderlying(DescriptorSetIndex::Texture)];
                if (descriptorSet == VK_NULL_HANDLE)
                {
                    bool newPoolAllocated;
