    ...
    VkDescriptorType descriptorType)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
    {
        ...
        // Delay allocating the descriptor set, since we may not need one if all of the buffers are
        // inactive.
    ...
    ContextVk *contextVk)
{
    ...
    for (uint32_t imageIndex = 0; imageIndex < imageBindings.size(); ++imageIndex)
    {
             ...
             ++arrayElement)
        {
            ...
            // Delay allocating the descriptor set, since we may not need one if all of the image
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
             ...
             ++textureIndex)
        {
            ...
            for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
            {
                ...
                // Delay allocating the descriptor set, since we may not need one if all of the
