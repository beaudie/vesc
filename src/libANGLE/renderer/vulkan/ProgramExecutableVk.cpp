    ...
    vk::DescriptorCacheResult *cacheResultOut)
{
    ...
    ANGLE_TRY(mDescriptorPools[DescriptorSetIndex::ShaderResource].get().getOrAllocateDescriptorSet(
        context, resourceUseList, shaderResourcesDesc.getDesc(), layout, binding, descriptorSet,
        cacheResultOut));
    ...
    const vk::DescriptorSetDescBuilder &shaderBuffersDesc)
{
    ...
    ANGLE_TRY(getOrAllocateShaderResourcesDescriptorSet(contextVk, resourceUseList,
                                                        shaderBuffersDesc, &cacheResult));

    if (cacheResult == vk::DescriptorCacheResult::NewAllocation)
    {
        shaderBuffersDesc.updateDescriptorSet(updateBuilder,
