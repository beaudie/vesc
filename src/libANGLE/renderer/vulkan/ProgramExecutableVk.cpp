...
void ProgramExecutableVk::reset(ContextVk *contextVk)
{
    ...
    mTextureDescriptorsCache.reset(rendererVk);
    mUniformsAndXfbDescriptorSetCache.reset(rendererVk);
    ...
    bool *newDescriptorSetAllocated)
{
    ...
    if (newPoolAllocated)
    {
        mUniformsAndXfbDescriptorSetCache.reset(contextVk->getRenderer());
    }
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
             ...
             ++textureIndex)
        {
            ...
            if (descriptorSet == VK_NULL_HANDLE)
            {
                ...
                if (newPoolAllocated)
                {
                    mTextureDescriptorsCache.reset(contextVk->getRenderer());
                }
