...
class WarmUpTaskCommon : public vk::Context, public LinkSubTask
{
  public:
    ...
    virtual vk::PipelineHelper *getWarmUpPipelineHelper()
    {
        UNREACHABLE();
        return nullptr;
    }
...
class WarmUpGraphicsTask : public WarmUpTaskCommon
{
  public:
    ...
    ~WarmUpGraphicsTask() override
    {
        if (mWarmUpPipelineResult->valid())
        {
            // Destroy unused warm up PipelineHelper
            mWarmUpPipelineResult->destroy(mRenderer->getDevice());
        }
        SafeDelete(mWarmUpPipelineResult);
    }
    ...
    vk::PipelineHelper *getWarmUpPipelineHelper() override { return mWarmUpPipelineResult; }
...
void ProgramExecutableVk::waitForPostLinkTasksImpl(ContextVk *contextVk)
{
    ...
    for (const std::shared_ptr<rx::LinkSubTask> &task : postLinkSubTasks)
    {
        ...
        ASSERT(!warmUpPipelineHelper->valid());
