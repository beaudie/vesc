                                                         ...
                                                         gl::BinaryInputStream *stream)
{
    ...
    std::vector<uint8_t> compressedPipelineData;
    stream->readIntVector<uint8_t>(&compressedPipelineData);
    ...
    status = initializePipelineCache(contextVk, compressedPipelineData);
    return std::make_unique<LinkEventDone>(status);
}

void ProgramExecutableVk::save(ContextVk *contextVk, gl::BinaryOutputStream *stream)
{
                                                       ...
                                                       const gl::ProgramExecutable &glExecutable)
{
    ...
    if (isCompute)
    {
        ...
        return getComputePipeline(contextVk, mPipelineCache, &pipeline);
    }
    ...
    for (bool rotation : surfaceRotationVariations)
    {
        for (bool depthCorrection : depthCorrectionVariations)
        {
            ...
            ANGLE_TRY(getGraphicsPipelineImpl(contextVk, transformOptions, mode, shaderOutMask,
                                              mPipelineCache, graphicsPipelineDesc, glExecutable,
                                              &descPtr, &pipeline));
        }
    }
                                                       ...
                                                       vk::PipelineHelper **pipelineOut)
{
    ...
    return getGraphicsPipelineImpl(contextVk, transformOptions, mode, framebufferMask,
                                   pipelineCache, desc, glExecutable, descPtrOut, pipelineOut);
}
