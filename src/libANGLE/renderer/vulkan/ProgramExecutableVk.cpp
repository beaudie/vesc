                                       ...
                                       ProgramExecutableVk *executableVk)
{
    ...
    std::string output;
    spirvTools.Disassemble(originalSpirvBlob, &output, SPV_BINARY_TO_TEXT_OPTION_NONE);
    WARN() << "Original spirv:\n" << output.c_str();
...
std::unique_ptr<rx::LinkEvent> ProgramExecutableVk::load(gl::BinaryInputStream *stream)
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        for (size_t i = 0; i < variableInfoMapSize; ++i)
        {
            ...
            info->fixPrecision       = stream->readInt<uint8_t>() != 0;
            info->varyingIsOutput    = stream->readInt<uint8_t>() != 0;
            info->fixedVaryingId     = stream->readInt<uint32_t>();
            info->fixedVaryingTypeId = stream->readInt<uint32_t>();
        }
    }
...
void ProgramExecutableVk::save(gl::BinaryOutputStream *stream)
{
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        for (const auto &it : mVariableInfoMap[shaderType])
        {
            ...
            stream->writeInt<uint8_t>(it.second.fixPrecision);
            stream->writeInt<uint8_t>(it.second.varyingIsOutput);
            stream->writeInt<uint32_t>(it.second.fixedVaryingId);
            stream->writeInt<uint32_t>(it.second.fixedVaryingTypeId);
        }
    }
}
