    ...
    if (mUniformsAndXfbDescriptorsCache.get(xfbBufferDesc, &descriptorSet))
    {
        *newDescriptorSetAllocated                          = false;
        ...
        mDescriptorPoolBindings[DescriptorSetIndex::UniformsAndXfb].get().retain(
            &contextVk->getResourceUseList(), vk::ResourceUseType::Access);
        return angle::Result::Continue;
    }
    ...
    ContextVk *contextVk)
{
    ...
    if (defaultUniformBlock.uniformData.empty())
    {
        ...
        bufferHelper->retain(&contextVk->getResourceUseList(), vk::ResourceUseType::Read);
    ...
    bool cacheHit)
{
    ...
    emptyBuffer.retain(&contextVk->getResourceUseList(), vk::ResourceUseType::Access);
