                                                       ...
                                                       const gl::ProgramExecutable &glExecutable)
{
    ...
    if (contextVk->getFeatures().disablePipelineCacheWarmUp.enabled)
    {
        return angle::Result::Continue;
    }
    ...
    if (isCompute)
    {
        ...
        return getComputePipeline(contextVk, &pipelineCache, glExecutable, &pipeline);
    }
    ...
    if (contextVk->getFeatures().enablePreRotateSurfaces.enabled &&
        !contextVk->getFeatures().forceDriverUniformOverSpecConst.enabled)
    {
    ...
    for (bool rotation : surfaceRotationVariations)
    {
        for (bool depthCorrection : depthCorrectionVariations)
        {
            ...
            ANGLE_TRY(getGraphicsPipelineImpl(contextVk, transformOptions, mode, shaderOutMask,
                                              &pipelineCache, graphicsPipelineDesc, glExecutable,
                                              &descPtr, &pipeline));
        }
    }
