...
VkDeviceSize GetShaderBufferBindingSize(const gl::OffsetBindingPointer<gl::Buffer> &bufferBinding)
{
    ...
    // backing buffer may be larger than max*BufferRange.  In that case, we use the minimum
    // of the backing buffer size (what's left after offset) and the buffer size as defined
    // by the shader.  That latter is only valid for UBOs, as SSBOs may have variable length
    // arrays.
