...
void ProgramExecutableVk::updateEarlyFragmentTestsOptimizaion(ContextVk *contextVk)
{
    const gl::State &glState = contextVk->getState();

    transformOptionBits[ProgramTransformOption::RemoveEarlyFragmentTestsOptimization] = false;
    if (!glState.isEarlyFragmentTestsOptimizationAllowed())
    {
        ...
        {
            transformOptionBits[ProgramTransformOption::RemoveEarlyFragmentTestsOptimization] =
                true;
        }
    }
}
...
{
    const gl::State &glState = contextVk->getState();
    transformOptionBits[ProgramTransformOption::EnableLineRasterEmulation] =
        contextVk->isBresenhamEmulationEnabled(mode);
    ProgramInfo &programInfo         = getProgramInfo(transformOptionBits);
    RendererVk *renderer             = contextVk->getRenderer();
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
        ...
        if (programVk)
        {
            ANGLE_TRY(programVk->initGraphicsShaderProgram(contextVk, shaderType,
                                                           transformOptionBits, programInfo));
        }
    }
