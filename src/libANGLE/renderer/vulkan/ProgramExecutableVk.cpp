    ...
    const vk::DescriptorSetDescBuilder &shaderResourcesDesc)
{
    ...
    if (numOffsets > 0)
    {
        memcpy(mDynamicShaderBufferDescriptorOffsets.data(),
...
{
    return getOrAllocateDescriptorSet(context, updateBuilder, resourceUseList, texturesDesc,
                                      DescriptorSetIndex::Texture);
}
