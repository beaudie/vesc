...
{
    const gl::ProgramExecutable *glExecutable = glState.getExecutable();
    ASSERT(glExecutable);

    mStorageBlockBindingsOffset =
        static_cast<uint32_t>(glExecutable->getUniqueUniformBlockCount(glState));

    mAtomicCounterBufferBindingsOffset = static_cast<uint32_t>(
        mStorageBlockBindingsOffset + glExecutable->getUniqueStorageBlockCount(glState));

    mImageBindingsOffset =
        mAtomicCounterBufferBindingsOffset +
        static_cast<uint32_t>(glExecutable->getAtomicCounterBuffersCount(glState));
}
...
angle::Result ProgramExecutableVk::createPipelineLayout(const gl::ProgramState &programState,
                                                        const gl::Context *glContext)
{
    const gl::State &glState                 = glContext->getState();
    const gl::Program *glProgram             = glContext->getState().getProgram();
    ...
    reset(contextVk);
    AssignLocations(GlslangWrapperVk::CreateSourceOptions(contextVk->getRenderer()->getFeatures()),
                    glProgram->getState(), glProgram->getResources(), &mVariableInfoMap);
    updateBindingOffsets(glState);
    ...
    for (const gl::ShaderType shaderType : glState.getExecutable()->getLinkedShaderStages())
    {
        uniformsAndXfbSetDesc.update(uniformBindingIndex++,
                                     VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1,
                                     gl_vk::kShaderStageMap[shaderType]);
    }
    size_t xfbBufferCount = executable->getTransformFeedbackBufferCount(glState);
    if (executable->hasLinkedShaderStage(gl::ShaderType::Vertex) && transformFeedback &&
        (xfbBufferCount != 0))
    {
    ...
    ContextVk *contextVk)
{
         ...
         programState.getProgramExecutable().getLinkedShaderStages())
    {
        ...
        ShaderInterfaceVariableInfo &shaderInterfaceVariableInfo =
            mVariableInfoMap[shaderType][uniformBlockName];
    ...
    vk::CommandGraphResource *recorder)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        updateBuffersDescriptorSet(contextVk, shaderType, resourceUseList, commandBufferHelper,
                                   recorder, programState.getUniformBlocks(),
                                   VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
        updateBuffersDescriptorSet(contextVk, shaderType, resourceUseList, commandBufferHelper,
                                   recorder, programState.getShaderStorageBlocks(),
                                   VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        updateAtomicCounterBuffersDescriptorSet(programState, shaderType, contextVk,
                                                resourceUseList, commandBufferHelper, recorder);
        angle::Result status =
            updateImagesDescriptorSet(programState, shaderType, contextVk, recorder);
    ...
    vk::FramebufferHelper *framebuffer)
{
    ...
    if (contextVk->commandGraphEnabled())
    {
        ...
        transformFeedbackVk->addFramebufferDependency(contextVk, programState, framebuffer);
    }
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
             ...
             ++textureIndex)
        {
            ...
            for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
            {
                ...
                ShaderInterfaceVariableInfo &shaderInterfaceVariableInfo =
                    mVariableInfoMap[shaderType][samplerName];
...
angle::Result ProgramExecutableVk::updateDescriptorSets(const gl::ProgramState &programState,
                                                        ContextVk *contextVk,
                                                        vk::CommandBuffer *commandBuffer)
{
    ...
    const VkPipelineBindPoint pipelineBindPoint = programState.getProgramExecutable().isCompute()
