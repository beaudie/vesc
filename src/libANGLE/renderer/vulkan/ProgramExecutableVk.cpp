                                     ...
                                     VkDeviceSize requiredOffsetAlignment)
{
    ...
    contextVk->recordWriteInfoAndBufferInfo(writeDescriptorSet, {bufferInfo});
}
    ...
    for (uint32_t imageIndex = 0; imageIndex < imageBindings.size(); ++imageIndex)
    {
        ...
        if (imageBinding.textureType == gl::TextureType::Buffer)
        {
            ...
            for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
            {
                ...
                contextVk->recordWriteInfoAndBufferView(writeInfo, {view->getHandle()});
            }
            continue;
        }
        ...
        for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
        {
            ...
            contextVk->recordWriteInfoAndImageInfo(writeInfo, {imageInfo});
        }
    }
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
             ...
             ++textureIndex)
        {
            ...
            for (uint32_t arrayElement = 0; arrayElement < arraySize; ++arrayElement)
            {
                ...
                {
                    imageInfo.sampler = textureVk->getSampler().get().getHandle();
                }
                const std::string samplerName = GlslangGetMappedSamplerName(samplerUniform.name);
                const ShaderInterfaceVariableInfo &info =
                    mVariableInfoMap.get(shaderType, samplerName);
