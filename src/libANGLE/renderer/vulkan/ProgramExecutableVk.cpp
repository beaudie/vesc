...
void SchedulePostLinkTasks(
    std::shared_ptr<angle::WorkerThreadPool> workerThreadPool,
    std::vector<std::shared_ptr<rx::WarmUpTaskCommon>> *postLinkTasks,
    std::vector<std::shared_ptr<angle::WaitableEvent>> *postLinkTaskWaitableEvents)
{
    postLinkTaskWaitableEvents->reserve(postLinkTasks->size());
    for (std::shared_ptr<rx::WarmUpTaskCommon> task : *postLinkTasks)
    {
        postLinkTaskWaitableEvents->push_back(workerThreadPool->postWorkerTask(task));
    }
}
}  // namespace
    ...
    std::shared_ptr<angle::WorkerThreadPool> workerThreadPool)
{
    ...
    if (isCompute)
    {
        ...
        SchedulePostLinkTasks(workerThreadPool, &mPostLinkTasks, &mPostLinkTaskWaitableEvents);
