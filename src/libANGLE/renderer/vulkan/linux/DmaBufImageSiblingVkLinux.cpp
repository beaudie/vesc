...
angle::Result GetFormatModifierProperties(RendererVk *renderer,
                                          VkFormat vkFormat,
...
angle::Result DmaBufImageSiblingVkLinux::initImpl(DisplayVk *displayVk)
{
    ...
    for (uint32_t plane = 0; plane < planeCount; ++plane)
    {
        if (planeModifiers[plane] != plane0Modifier)
        {
            return angle::Result::Stop;
        }
    }
    ...
    ANGLE_TRY(
        GetFormatModifierProperties(renderer, vulkanFormat, plane0Modifier, &modifierProperties));
    ...
    if (modifierProperties.drmFormatModifierPlaneCount != planeCount)
    {
        return angle::Result::Stop;
    }
    ...
    if (!isWidthValid || !isHeightValid || !isSampleCountValid || !isMemoryImportable)
    {
        return angle::Result::Stop;
    }
