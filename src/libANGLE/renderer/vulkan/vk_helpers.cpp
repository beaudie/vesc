...
DynamicBuffer::DynamicBuffer(DynamicBuffer &&other)
    : mUsage(other.mUsage),
      ...
      mDisableSubAllocation(other.mDisableSubAllocation),
                                  ...
                                  bool disableSubAllocation)
{
    ...
    mDisableSubAllocation = disableSubAllocation;
                                      ...
                                      bool *newBufferAllocatedOut)
{
    size_t sizeToAllocate =
        (mDisableSubAllocation) ? sizeInBytes : roundUp(sizeInBytes, mAlignment);

    angle::base::CheckedNumeric<size_t> checkedNextWriteOffset = mNextAllocationOffset;
    checkedNextWriteOffset += sizeToAllocate;

    if (!checkedNextWriteOffset.IsValid() || checkedNextWriteOffset.ValueOrDie() >= mSize ||
        mDisableSubAllocation)
    {
        if (mBuffer)
        {
        ...
        bool needToClearFreeList =
            (mDisableSubAllocation) ? (sizeToAllocate != mSize) : (sizeToAllocate > mSize);
...
void DynamicBuffer::releaseInFlightBuffers(ContextVk *contextVk)
{
    for (BufferHelper *toRelease : mInFlightBuffers)
    {
        ...
        bool needToRelease = (mDisableSubAllocation) ? (toRelease->getSize() != mSize)
                                                     : (toRelease->getSize() < mSize);
        if (needToRelease)
        {
