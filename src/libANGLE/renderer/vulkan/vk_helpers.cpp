         ...
         ++updateMipLevelGL)
    {
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            else if (update.updateSource == UpdateSource::Buffer)
            {
                ...
                vk::CommandBufferAccess access;
                access.onBufferTransferRead(currentBuffer);
                ANGLE_TRY(
                    contextVk->getOutsideRenderPassCommandBufferWithAccess(access, &commandBuffer));
            ...
            {
                vk::CommandBufferAccess access;
                access.onImageTransferRead(aspectFlags, update.image.image);
                ANGLE_TRY(
                    contextVk->getOutsideRenderPassCommandBufferWithAccess(access, &commandBuffer));
                                      ...
                                      DynamicBuffer *stagingBuffer)
{
    ...
    if (isMultisampled)
    {
        ...
        ANGLE_TRY(
            contextVk->getOutsideRenderPassCommandBufferWithAccess(readAccess, &commandBuffer));
                                       ...
                                       BufferHelper *buffer)
{
    ...
    readBuffers.emplace_back(buffer, readAccessType, readStage);
}
...
{
    ASSERT(!buffer->isReleasedToExternal());
    writeBuffers.emplace_back(buffer, writeAccessType, writeStage);
}
                                      ...
                                      ImageHelper *image)
{
    ...
    readImages.emplace_back(image, aspectFlags, imageLayout);
}
                                       ...
                                       ImageHelper *image)
{
    ASSERT(!image->isReleasedToExternal());
    ASSERT(image->getImageSerial().valid());
    writeImages.emplace_back(CommandBufferImageAccess{image, aspectFlags, imageLayout}, levelStart,
