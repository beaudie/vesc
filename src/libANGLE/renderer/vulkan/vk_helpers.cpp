                                                      ...
                                                      const GLuint inputSkipBytes)
{
    ...
    allocationSize = roundUp(allocationSize + imageCopyAlignment - 1, imageCopyAlignment);
    ANGLE_TRY(currentBuffer->initStagingBuffer(contextVk, allocationSize, false));
    VkDeviceSize stagingOffset =
        roundUp<VkDeviceSize>(currentBuffer->getOffset(), imageCopyAlignment);
    ...
    for (std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        for (SubresourceUpdate &update : levelUpdates)
        {
                ...
                update.data.buffer.formatID == srcFormatID)
            {
                ...
                // Add extra padding for potential offset alignment
                dstBufferSize = roundUp(dstBufferSize + imageCopyAlignment - 1, imageCopyAlignment);
                ANGLE_TRY(dstBuffer->initStagingBuffer(contextVk, dstBufferSize, false));
                VkDeviceSize dstBufferOffset =
                    roundUp<VkDeviceSize>(dstBuffer->getOffset(), imageCopyAlignment);
                uint8_t *dstData =
    ...
    FramebufferVk *framebufferVk)
{
    ...
    size_t imageCopyAlignment = GetImageCopyBufferAlignment(storageFormat.id);
    // Add extra padding for potential offset alignment
    size_t allocationSize = roundUp(outputDepthPitch + imageCopyAlignment - 1, imageCopyAlignment);
    ANGLE_TRY(currentBuffer->initStagingBuffer(contextVk, allocationSize, false));
    VkDeviceSize stagingOffset =
        roundUp<VkDeviceSize>(currentBuffer->getOffset(), imageCopyAlignment);
    uint8_t *stagingPointer =
