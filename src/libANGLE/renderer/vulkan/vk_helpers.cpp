...
void DynamicBuffer::setAlignment(RendererVk *renderer, size_t alignment)
{
...
void ImageHelper::initStagingBuffer(RendererVk *renderer)
{
    ...
    // of the pixel block size.  Unless specifically necessary (for compressed formats), an
    // alignment of 4 is sufficient.
    // https://www.khronos.org/registry/vulkan/specs/1.0/man/html/VkBufferImageCopy.html
    mStagingBuffer.init(4, renderer);
}

void ImageHelper::realignStagingBuffer(RendererVk *renderer, const gl::InternalFormat &formatInfo)
{
    // vkCmdCopyBufferToImage must have an offset that is a multiple of 4 as well as a multiple
    ...
    //
    size_t blockSize  = formatInfo.pixelBytes;
    size_t multiplier = blockSize % 2 ? 4 : blockSize % 4 ? 2 : 1;
    size_t alignment  = multiplier * blockSize;

    mStagingBuffer.setAlignment(renderer, alignment);
}
