                                                 ...
                                                 const PruneReason reason)
{
    ...
    using SubresourceUpdatesReverseIterator =
        std::vector<ImageHelper::SubresourceUpdate>::reverse_iterator;
    ...
    SubresourceUpdatesReverseIterator update = levelUpdates->rbegin();
    while (update != levelUpdates->rend())
    {
        ...
        // Increment the reverse iterator before possibly erasing current update
        update++;

        // If current update region is invalid, keep the update and move on.
        if (!currentUpdateBox.valid())
        {
            continue;
        }
        ...
        if (boundingBox[aspectIndex].contains(currentUpdateBox))
        {
            ...
            // Release and erase the superseded update
            SubresourceUpdatesIterator supersededUpdate = update.base();
            supersededUpdate->release(rendererVk);
            levelUpdates->erase(supersededUpdate);
