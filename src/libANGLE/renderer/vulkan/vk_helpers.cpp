...
void CommandBufferHelper::endRenderPass()
{
    ...
    if (mDepthStencilAttachmentIndex != kInvalidAttachmentIndex)
    {
        ...
        // buffer is not been used, and the data is also not been stored back into buffer, then just
        // skip the load/clear op.
        if (!mDepthBufferEverEnabled && mAttachmentOps[mDepthStencilAttachmentIndex].storeOp ==
