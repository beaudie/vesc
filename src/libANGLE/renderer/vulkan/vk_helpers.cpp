...
ImageViewHelper::ImageViewHelper(ImageViewHelper &&other)
{
    ...
    std::swap(mDrawImageView, other.mDrawImageView);
...
void ImageViewHelper::release(RendererVk *renderer)
{
    ...
    garbage.emplace_back(GetGarbage(&mDrawImageView));
...
angle::Result ImageViewHelper::initDrawView(Context *context,
                                            ...
                                            uint32_t layerCount)
{
    ASSERT(!format.imageFormat().isBlock);
    const VkImageAspectFlags aspectFlags = GetFormatAspectFlags(format.angleFormat());
    return image.initLayerImageView(context, viewType, aspectFlags, gl::SwizzleState(),
                                    &mDrawImageView, baseLevel, levelCount, baseLayer, layerCount);
}
                                             ...
                                             uint32_t layerCount)
{
        ...
        viewType == gl::TextureType::_2DMultisampleArray)
    {
        ...
        // FIXME: SwizzleState might be incorrect for CopyTextureCHROMIUM.
