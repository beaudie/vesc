                                         ...
                                         BufferUsageType usageType)
{
    ...
    createInfo.usage                 = GetDefaultBufferUsageFlags(context->getRenderer());
    createInfo.sharingMode           = VK_SHARING_MODE_EXCLUSIVE;
    createInfo.queueFamilyIndexCount = 0;
    createInfo.pQueueFamilyIndices   = nullptr;
    ...
    return (init(context, createInfo, memoryPropertyFlags) == angle::Result::Continue)
               ? VK_SUCCESS
               : VK_ERROR_OUT_OF_DEVICE_MEMORY;
}

VkResult BufferHelper::initSuballocationFromPool(Context *context,
                                                 uint32_t memoryTypeIndex,
                                                 size_t size,
                                                 size_t alignment,
                                                 BufferUsageType usageType,
                                                 BufferPool *pool)
{
    if (!pool)
    {
        return initSuballocation(context, memoryTypeIndex, size, alignment, usageType);
    }
