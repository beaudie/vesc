...
{
    uint64_t count =
        static_cast<uint64_t>(getSize()) / static_cast<uint64_t>(internalFormat.pixelBytes);
    ...
    switch (internalFormat.internalFormat)
    {
        case GL_RGB565:
        {
            ...
            std::fill_n<uint16_t *, uint64_t, uint16_t>(pixelPtr, count, pixelColor);
        }
        break;
        case GL_RGBA8:
        {
            ...
            std::fill_n<uint32_t *, uint64_t, uint32_t>(pixelPtr, count, pixelColor);
        }
        break;
