                                         ...
                                         VkSemaphore *semaphoreOut)
{
    ...
    {
        // TODO: If the layout is changing, the per-level updates should be set so there is a
        // barrier.
        clearPerLevelUpdateFlags();
                                              ...
                                              const gl::CubeFaceArray<gl::TexLevelMask> &skipLevels)
{
    ...
    if (updateCount > 0)
    {
        ...
        // TODO: The following condition resolves the issue with some SwS errors.
        // if (renderer->isAsyncCommandQueueEnabled() || isBarrierRequired)
        if (isBarrierRequired)
        {
            if (transCoding)
            {
