                                                          ...
                                                          BufferHelper **bufferOut)
{
    ...
    ANGLE_TRY(AllocateVertexConversionBuffer(contextVk, &mDynamicIndexBuffer, allocateBytes));
                                            ...
                                            uint32_t *indexCountOut)
{
    ...
    *indexCountOut = numOutIndices;
    ...
    ANGLE_TRY(
        AllocateVertexConversionBuffer(contextVk, &mDynamicIndexBuffer, unitSize * numOutIndices));
                                                    ...
                                                    BufferHelper **indirectBufferOut)
{
    ...
    ANGLE_TRY(AllocateVertexConversionBuffer(contextVk, &mDynamicIndexBuffer, allocateBytes));
    ANGLE_TRY(AllocateVertexConversionBuffer(contextVk, &mDynamicIndirectBuffer,
    ...
    ANGLE_TRY(contextVk->getUtils().convertLineLoopIndexIndirectBuffer(
        contextVk, indirectBuffer, &mDynamicIndirectBuffer, &mDynamicIndexBuffer, indexBuffer,
        params));
                                                  ...
                                                  BufferHelper **indexIndirectBufferOut)
{
    ...
    ANGLE_TRY(contextVk->getUtils().convertLineLoopArrayIndirectBuffer(
        contextVk, arrayIndirectBuffer, &mDynamicIndirectBuffer, &mDynamicIndexBuffer, params));
                                                      ...
                                                      const GLuint inputSkipBytes)
{
    ...
    size_t alignment            = static_cast<size_t>(renderer->getStagingBufferAlignment());
    VkDeviceSize stagingOffset  = 0;

    if (stagingBufferOverride != nullptr)
    {
        VkBuffer bufferHandle;
        ANGLE_TRY(stagingBufferOverride->allocateWithAlignment(contextVk, allocationSize, alignment,
                                                               &stagingPointer, &bufferHandle,
                                                               &stagingOffset, nullptr));
        currentBuffer = stagingBufferOverride->getCurrentBuffer();
    }
    else
    {
        stagingBuffer            = std::make_unique<RefCounted<BufferHelper>>();
        currentBuffer            = &stagingBuffer->get();
        uint32_t memoryTypeIndex = renderer->getNonCoherentStagingBufferMemoryTypeIndex();
        ANGLE_TRY(currentBuffer->initSubAllocation(contextVk, memoryTypeIndex, allocationSize,
                                                   alignment));
        stagingPointer = currentBuffer->getMappedMemory();
        stagingOffset  = currentBuffer->getOffset();
    }
    ...
    if (storageFormat.isYUV)
    {
        ...
        for (uint32_t plane = 0; plane < yuvInfo.planeCount; plane++)
        {
            ...
            appendSubresourceUpdate(
                gl::LevelIndex(0),
                SubresourceUpdate(stagingBuffer.release(), currentBuffer, copy, storageFormat.id));
        }
    ...
    if (stencilAllocationSize > 0)
    {
        ...
        appendSubresourceUpdate(
            updateLevelGL, SubresourceUpdate(stagingBuffer.release(), currentBuffer, stencilCopy,
                                             storageFormat.id));
    ...
    for (std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        for (SubresourceUpdate &update : levelUpdates)
        {
                ...
                update.data.buffer.formatID == srcFormatID)
            {
                ...
                uint32_t memoryTypeIndex = renderer->getNonCoherentStagingBufferMemoryTypeIndex();
                size_t alignment         = renderer->getNonCoherentStagingBufferMemoryTypeIndex();
                GLuint dstBufferSize     = dstDataDepthPitch * copy.imageExtent.depth;
                ANGLE_TRY(dstBuffer->initSubAllocation(contextVk, memoryTypeIndex, dstBufferSize,
                                                       alignment));
                ...
                if (update.refCountedObject.buffer)
                {
                    update.refCountedObject.buffer->releaseRef();
                    if (!update.refCountedObject.buffer->isReferenced())
                    {
                        update.refCountedObject.buffer->get().release(renderer);
                        SafeDelete(update.refCountedObject.buffer);
                    }
                }
                update.refCountedObject.buffer = stagingBuffer.release();
                update.refCountedObject.buffer->addRef();
            }
        }
    }
                                                              ...
                                                              const angle::Format &imageFormat)
{
    ...
    if (imageFormat.isBlock)
    {
        ...
        size_t alignment            = static_cast<size_t>(renderer->getStagingBufferAlignment());
        ANGLE_TRY(
            currentBuffer->initSubAllocation(contextVk, memoryTypeIndex, totalSize, alignment));
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ASSERT(update.updateSource == UpdateSource::Clear ||
                   ...
                   (update.updateSource == UpdateSource::Image &&
                    update.refCountedObject.image != nullptr &&
                    update.refCountedObject.image->isReferenced() &&
                    update.refCountedObject.image->get().valid()));
            ...
            else
            {
                ...
                imageAccess.onImageTransferRead(aspectFlags, &update.refCountedObject.image->get());
                ANGLE_TRY(
                    contextVk->getOutsideRenderPassCommandBuffer(imageAccess, &commandBuffer));

                commandBuffer->copyImage(update.refCountedObject.image->get().getImage(),
                                         update.refCountedObject.image->get().getCurrentLayout(),
                                         mImage, getCurrentLayout(), 1,
                                         &update.data.image.copyRegion);
                onWrite(updateMipLevelGL, 1, updateBaseLayer, updateLayerCount,
    ...
    RefCounted<BufferHelper> *buffer) const
{
    ...
    for (const std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        ...
        {
            if (update.updateSource == UpdateSource::Buffer &&
                update.refCountedObject.buffer == buffer)
            {
                ++refs;
            }
        }
...
bool ImageHelper::validateSubresourceUpdateImageRefConsistent(RefCounted<ImageHelper> *image) const
{
    ...
    for (const std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        ...
        {
            if (update.updateSource == UpdateSource::Image &&
                update.refCountedObject.image == image)
            {
                ++refs;
            }
        }
...
bool ImageHelper::validateSubresourceUpdateRefcountsConsistent() const
{
    for (const std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        for (const SubresourceUpdate &update : levelUpdates)
        {
            if (update.updateSource == UpdateSource::Image)
            {
                if (!validateSubresourceUpdateImageRefConsistent(update.refCountedObject.image))
                {
                    return false;
                }
            }
            ...
            {
                if (!validateSubresourceUpdateBufferRefConsistent(update.refCountedObject.buffer))
                {
                    return false;
                }
            }
                                                 ...
                                                 StagingBufferOffsetArray *bufferOffsetsOut)
{
    ...
    uint32_t memoryTypeIndex = renderer->getNonCoherentStagingBufferMemoryTypeIndex();
    size_t alignment         = static_cast<size_t>(renderer->getStagingBufferAlignment());
    ANGLE_TRY(dstBuffer->initSubAllocation(contextVk, memoryTypeIndex, bufferSize, alignment));
    ...
    commandBuffer->copyImageToBuffer(mImage, getCurrentLayout(), dstBuffer->getBuffer().getHandle(),
                                     1, regions);
                                      ...
                                      void *pixels)
{
    ...
    RendererScoped<vk::BufferHelper> stagingBuffer(renderer);
    vk::BufferHelper *bufferHelper = &stagingBuffer.get();
    uint32_t memoryTypeIndex       = renderer->getNonCoherentStagingBufferMemoryTypeIndex();
    size_t allocationSize          = readFormat->pixelBytes * area.width * area.height;
    size_t alignment               = static_cast<size_t>(renderer->getStagingBufferAlignment());
    ANGLE_TRY(
        bufferHelper->initSubAllocation(contextVk, memoryTypeIndex, allocationSize, alignment));

    VkBufferImageCopy region = {};
    region.bufferImageHeight = srcExtent.height;
    region.bufferOffset      = bufferHelper->getOffset();
    ...
    readbackAccess.onBufferTransferWrite(bufferHelper);
    ...
    readbackCommandBuffer->copyImageToBuffer(src->getImage(), src->getCurrentLayout(),
    ...
    ANGLE_TRY(bufferHelper->invalidate(renderer));
    ...
    if (packPixelsParams.packBuffer)
    {
        ...
        PackPixels(packPixelsParams, *readFormat, area.width * readFormat->pixelBytes,
                   bufferHelper->getMappedMemory(), static_cast<uint8_t *>(dst));
        ANGLE_TRY(packBufferVk->unmapImpl(contextVk));
    }
    else
    {
        PackPixels(packPixelsParams, *readFormat, area.width * readFormat->pixelBytes,
                   bufferHelper->getMappedMemory(), static_cast<uint8_t *>(pixels));
    }
...
ImageHelper::SubresourceUpdate::SubresourceUpdate() : updateSource(UpdateSource::Buffer)
{
    ...
    refCountedObject.buffer  = nullptr;
}
...
{
    refCountedObject.buffer = bufferIn;
    if (refCountedObject.buffer != nullptr)
    {
        refCountedObject.buffer->addRef();
    }
...
{
    refCountedObject.image = imageIn;
    refCountedObject.image->addRef();
...
{
    refCountedObject.image = nullptr;
    ...
    : updateSource(other.updateSource)
{
    switch (updateSource)
    {
        ...
        case UpdateSource::Buffer:
            ...
            refCountedObject.buffer       = other.refCountedObject.buffer;
            other.refCountedObject.buffer = nullptr;
            break;
        case UpdateSource::Image:
            data.image                   = other.data.image;
            refCountedObject.image       = other.refCountedObject.image;
            other.refCountedObject.image = nullptr;
            break;
        default:
            UNREACHABLE();
    }
...
void ImageHelper::SubresourceUpdate::release(RendererVk *renderer)
{
    ...
    {
        refCountedObject.image->releaseRef();

        if (!refCountedObject.image->isReferenced())
        {
            // Staging images won't be used in render pass attachments.
            refCountedObject.image->get().releaseImage(renderer);
            refCountedObject.image->get().releaseStagingBuffer(renderer);
            SafeDelete(refCountedObject.image);
        }

        refCountedObject.image = nullptr;
    }
    else if (updateSource == UpdateSource::Buffer && refCountedObject.buffer != nullptr)
    {
        refCountedObject.buffer->releaseRef();

        if (!refCountedObject.buffer->isReferenced())
        {
            // Staging images won't be used in render pass attachments.
            refCountedObject.buffer->get().release(renderer);
            SafeDelete(refCountedObject.buffer);
        }
