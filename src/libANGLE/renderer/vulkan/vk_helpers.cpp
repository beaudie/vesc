    ...
    DescriptorSetPointer *descriptorSetOut)
{
    ...
    if (mDescriptorPools[mCurrentPoolIndex] &&
        !pool1.owner_equal(mDescriptorPools[mCurrentPoolIndex]))
    {
    ...
    for (DescriptorPoolPointer &pool : mDescriptorPools)
    {
        ...
        if (pool1.owner_equal(pool) || pool2.owner_equal(pool))
        {
            continue;
        }
        pool1 = pool;
    ...
    pool1 = mDescriptorPools[mCurrentPoolIndex];
    bool success =
        pool1->allocateDescriptorSet(context, descriptorSetLayout, pool1, descriptorSetOut);
