                                                    ...
                                                    uint32_t layerCount) const
{
    ...
    // barrier should be checked to avoid RAW and WAR hazards. However, if a read barrier is
    ...
    for (uint32_t levelOffset = 0; levelOffset < levelCount; levelOffset++)
    {
        ...
        if (levelSubresourcesWrittenWithinMaskRange(level, layerMask))
        {
            return true;
        }
    }
         ...
         ++updateMipLevelGL)
    {
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            else
            {
                ...
                if (levelSubresourcesWrittenWithinMaskRange(updateMipLevelGL.get(),
                                                            subresourceHash))
                {
