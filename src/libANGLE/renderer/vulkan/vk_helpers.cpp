...
LevelIndex ImageHelper::toImageLevel(gl::LevelIndex levelIndexGL) const
{
    return gl_vk::ToImageLevelIndex(levelIndexGL, mBaseLevel);
}

gl::LevelIndex ImageHelper::toGLLevel(LevelIndex levelIndexVK) const
{
    return vk_gl::ToGLLevelIndex(levelIndexVK, mBaseLevel);
}
                                                              ...
                                                              const Format &format)
{
    ...
    if (imageFormat.isBlock)
    {
        ...
        copyRegion.imageSubresource.mipLevel       = toGLLevel(index.getLevelIndex()).get();
...
void ImageHelper::stageClearIfEmulatedFormat(Context *context)
{
    ...
    // values.
    for (LevelIndex level = 0; level < mLevelCount; ++level)
    {
        gl::ImageIndex index =
                                                               ...
                                                               uint32_t deferredClearIndex)
{
    ...
    LevelIndex levelVK = toImageLevel(levelGL);
                                              ...
                                              CommandBuffer *commandBuffer)
{
    ...
    for (SubresourceUpdate &update : mSubresourceUpdates)
    {
        ...
        LevelIndex updateMipLevelVK =
            isUpdateLevelOutsideRange ? 0 : toImageLevel(updateMipLevelGL);
        ...
        if (update.updateSource == UpdateSource::Clear)
        {
            clear(update.clear.aspectFlags, update.clear.value, update.clear.levelIndex,
...
void ImageHelper::removeSupersededUpdates(gl::TexLevelMask skipLevelsMask)
{
    ...
    auto markLayersAndDropSuperseded = [&, skipLevelsMask](const SubresourceUpdate &update) {
        ...
        const LevelIndex updateMipLevelVK = toImageLevel(updateMipLevelGL);
                                                 ...
                                                 uint8_t **outDataPtr)
{
    ...
    *bufferOut = contextVk->getStagingBuffer()->getCurrentBuffer();
    ...
    LevelIndex sourceLevelVk = toImageLevel(sourceLevelGL);
                                                 ...
                                                 void *pixels)
{
    ...
    const LevelIndex levelVK     = toImageLevel(levelGL);
                                      ...
                                      DynamicBuffer *stagingBuffer)
{
    ...
    srcSubresource.mipLevel                 = toImageLevel(levelGL).get();
