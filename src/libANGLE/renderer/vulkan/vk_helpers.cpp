                                          ...
                                          uint32_t layerCount) const
{
    ...
    // TODO: Fix or file a bug?
    // If the image can read and write at the same time (including self-copy), it is safer to add a
    ...
    for (uint32_t levelOffset = 0; levelOffset < levelCount; levelOffset++)
    {
        ...
        if ((mPerLevelSubresourceUpdateFlags[level].to_ullong() & layerMask) != 0)
        {
            return true;
        }
    }
...
void ImageHelper::updatePerLevelUpdateFlags(gl::LevelIndex levelStart,
                                            uint32_t levelCount,
                                            uint32_t layerStart,
                                            uint32_t layerCount)
{
    for (uint32_t levelOffset = 0; levelOffset < levelCount; levelOffset++)
    {
        ...
        if (layerCount >= kMaxParallelSubresourceUpload)
        {
            mPerLevelSubresourceUpdateFlags[level].set();
        }
        else
        {
            const uint64_t layerMask = ANGLE_ROTL64(angle::BitMask<uint64_t>(layerCount),
                                                    layerStart % kMaxParallelSubresourceUpload);
            mPerLevelSubresourceUpdateFlags[level] |= layerMask;
        }
    }
}
                                     ...
                                     OutsideRenderPassCommandBufferHelper *commands)
{
    ...
    {
        updatePerLevelUpdateFlags(levelStart, levelCount, layerStart, layerCount);
        return;
    }
                                              ...
                                              const gl::CubeFaceArray<gl::TexLevelMask> &skipLevels)
{
    ...
    bool containsOtherThanCECOUpdates = false;
         ...
         ++updateMipLevelGL)
    {
        ...
        if (levelUpdates == nullptr)
        {
            ...
            levelGLLimit = updateMipLevelGL;
            break;
        }

        size_t updatesToBeRemoved = 0;
        for (SubresourceUpdate &update : *levelUpdates)
        {
            // Since ClearEmulatedChannelsOnly is expected in the beginning, we can move on if there
            // is another update type in the list.
            if (update.updateSource != UpdateSource::ClearEmulatedChannelsOnly)
            {
                containsOtherThanCECOUpdates = true;
                break;
            }
            ...
            // These updates will be removed from the list after this loop.
            updatesToBeRemoved++;
        }
        for (size_t i = 0; i < updatesToBeRemoved; i++)
        {
            levelUpdates->erase(levelUpdates->begin());
        }
    }
    ...
    if (containsOtherThanCECOUpdates)
    {
        if (transCoding)
        {
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            switch (update.updateSource)
            {
                ...
                case UpdateSource::Buffer:
                {
                    ...
                    {
                        // TODD: http://anglebug.com/6368, we should handle this in higher level
            ...
            if (updateLayerCount >= kMaxParallelSubresourceUpload)
            {
                ...
                mPerLevelSubresourceUpdateFlags[updateMipLevelGL.get()].set();
            }
            else
            {
                ...
                if ((mPerLevelSubresourceUpdateFlags[updateMipLevelGL.get()].to_ullong() &
                     subresourceHash) != 0)
                {
                    // If there's overlap in subresource upload, issue a barrier.
                    recordWriteBarrier(contextVk, aspectFlags, barrierLayout, updateMipLevelGL, 1,
                                       updateBaseLayer, updateLayerCount, commandBuffer);
                    mPerLevelSubresourceUpdateFlags[updateMipLevelGL.get()].reset();
                }
                mPerLevelSubresourceUpdateFlags[updateMipLevelGL.get()] |= subresourceHash;
            }
