                                                  ...
                                                  vk::PrimaryCommandBuffer *primary)
{
    ...
    if (mIsRenderPassCommandBuffer)
    {
        ...
        if (mValidTransformFeedbackBufferCount != 0)
        {
            ...
            // TODO: Clean this up before we close http://anglebug.com/3206
...
void ImageHelper::removeSupersededUpdates(gl::TexLevelMask skipLevelsMask)
{
    if (mLayerCount > 64)
    {
        // Not implemented for images with more than 64 layers.
        return;
    }

    // Cache extents for each mip level.
    gl::TexLevelArray<VkExtent3D> levelExtents;
    for (uint32_t level = 0; level < mLevelCount; ++level)
    {
        gl_vk::GetExtent(getLevelExtents(level), &levelExtents[level]);
    }
    ...
    // future updates, so it can be dropped.  This tracking is done per level.
    gl::TexLevelArray<uint64_t> levelSupersededLayers = {};
    ...
    auto markLayersAndDropSuperseded = [&, skipLevelsMask](const SubresourceUpdate &update) {
        ...
        uint64_t updateLayersMask = (angle::Bit<uint64_t>(updateLayerCount) - 1) << updateBaseLayer;
        if ((levelSupersededLayers[updateMipLevelVK] & updateLayersMask) == updateLayersMask)
        {
            return true;
        }
        ...
        // subresource.
        VkExtent3D levelExtent  = levelExtents[updateMipLevelVK];
        VkOffset3D updateOffset = {};
        VkExtent3D updateExtent = levelExtent;

        if (update.updateSource == UpdateSource::Buffer)
        {
            updateOffset = update.buffer.copyRegion.imageOffset;
            updateExtent = update.buffer.copyRegion.imageExtent;
        }
        else if (update.updateSource == UpdateSource::Image)
        {
            updateOffset = update.image.copyRegion.dstOffset;
            updateExtent = update.image.copyRegion.extent;
        }

        // Only if the update is to the whole subresource, mark its layers.
        if (updateOffset.x == 0 && updateOffset.y == 0 && updateOffset.z == 0 &&
            updateExtent.width == levelExtent.width && updateExtent.height == levelExtent.height &&
            updateExtent.depth == levelExtent.depth)
        {
            levelSupersededLayers[updateMipLevelVK] |= updateLayersMask;
        }
