...
{
    fprintf(stderr, "Freeing entry from pool %zu (%zu pools total)\n", poolIndex, mPoolStats.size());
    if (poolIndex < mPoolStats.size())
        fprintf(stderr, " -- pool[%zu].freedCount: %u (%u pool size)\n", poolIndex, mPoolStats[poolIndex].freedCount, mPoolSize);
...
angle::Result DynamicSemaphorePool::allocateSemaphore(Context *context, SemaphoreHelper *semaphoreOut)
{
    ASSERT(!semaphoreOut->getSemaphore());

    if (mCurrentFreeEntry >= mPoolSize)
    {
        fprintf(stderr, "Semaphore pool: %zu is full, allocating new pool\n", mCurrentPool);
        // No more queries left in this pool, create another one.
        ANGLE_TRY(allocateNewPool(context));
    }

    fprintf(stderr, "Allocating semaphore from pool: %zu, index: %u\n", mCurrentPool, mCurrentFreeEntry);
    semaphoreOut->init(mCurrentPool, &mPools[mCurrentPool][mCurrentFreeEntry++]);
    fprintf(stderr, "  -- result: %p\n", semaphoreOut->getSemaphore()->getHandle());
...
void DynamicSemaphorePool::freeSemaphore(Context *context, SemaphoreHelper *semaphore)
{
    ...
    {
        fprintf(stderr, "Freeing semaphore: %p\n", semaphore->getSemaphore()->getHandle());
        freedEntry(context, semaphore->getSemaphorePoolIndex());
        semaphore->deinit();
    }
}
