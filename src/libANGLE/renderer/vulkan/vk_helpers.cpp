...
void CommandBufferHelper::depthStencilImageStartNewRenderPass(ResourceUseList *resourceUseList,
                                                              ImageHelper *image,
...
void CommandBufferHelper::finalizeDepthStencilImageLayout()
{
    ...
    if (barrierRequired)
    {
        ...
        if (mDepthStencilResolveImage &&
            mDepthStencilResolveImage->updateLayoutAndBarrier(aspectFlags, imageLayout, barrier))
        {
            mPipelineBarrierMask.set(barrierIndex);
        }
    }
...
{
    ASSERT(mIsRenderPassCommandBuffer);
    if (mDepthStencilImage == image || mDepthStencilResolveImage == image)
    {
        finalizeDepthStencilImageLayout();
        mDepthStencilImage = nullptr;
        // No need to keep resolve image around since it won't be used any more
        mDepthStencilResolveImage = nullptr;
    }
}
...
void CommandBufferHelper::updateStartedRenderPassWithDepthMode(bool depthStencilReadOnly)
{
    ASSERT(mIsRenderPassCommandBuffer);
    ASSERT(mRenderPassStarted);
    mReadOnlyDepthStencilMode = depthStencilReadOnly;
}
...
void ImageHelper::SubresourceUpdate::release(RendererVk *renderer)
{
    ...
    {
        // Staging images cant be used in renderpass for other purposes.
