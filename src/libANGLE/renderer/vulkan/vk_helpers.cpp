...
constexpr VkBufferUsageFlags kLineLoopDynamicIndirectBufferUsage =
    ...
    VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
constexpr int kLineLoopDynamicIndirectBufferInitialSize = sizeof(VkDrawIndirectCommand);
                                                    ...
                                                    VkDeviceSize *indirectBufferOffsetOut)
{
    ...
    auto unitSize = (indexType == VK_INDEX_TYPE_UINT16 ? sizeof(uint16_t) : sizeof(uint32_t));
    size_t allocateBytes = indexBuffer->getSize() + unitSize;
    ...
    if (contextVk->getState().isPrimitiveRestartEnabled())
    {
        ...
        size_t numInputIndices    = indexBuffer->getSize() / unitSize;
        size_t numNewInputIndices = ((numInputIndices * 4) / 3) + 1;
        allocateBytes             = numNewInputIndices * unitSize;
    }
