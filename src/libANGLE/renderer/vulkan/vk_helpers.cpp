                                                  ...
                                                  const vk::Format &vkFormat)
{
    ...
    else
    {
        ...
        if (storageFormat.depthBits > 0 && storageFormat.stencilBits > 0 &&
            formatInfo.type == GL_UNSIGNED_INT_24_8_OES)
        {
    ...
    // If we haven't cleared the ASPECT_STENCIL_BIT yet we are either
    // updating only the stencil component or we have depth data but the HW
    // buffer is packed depth stencil. In the first case we need to keep
    // the stencil aspect, but clear in the second.
    if (((aspectFlags & VK_IMAGE_ASPECT_STENCIL_BIT) != 0) && (formatInfo.format != GL_STENCIL))
    {
        aspectFlags &= ~VK_IMAGE_ASPECT_STENCIL_BIT;
    }
