...
{
    if (!hasStagedUpdatesInRange(levelGLStart, levelGLEnd))
    {
        return angle::Result::Continue;
    }
...
bool ImageHelper::hasStagedUpdatesInBaseMaxRange() const
{
    return hasStagedUpdatesInRange(mBaseLevel, mMaxLevel + 1);
}

bool ImageHelper::hasStagedUpdatesInRange(gl::LevelIndex levelStart, gl::LevelIndex levelEnd) const
{
...
void ImageHelper::removeSupersededUpdates(gl::TexLevelMask skipLevelsMask)
{
    ...
    auto markLayersAndDropSuperseded = [&supersededLayers, &levelExtents,
                                        this](const SubresourceUpdate &update) {
        uint32_t updateBaseLayer, updateLayerCount;
        update.getDestSubresource(mLayerCount, &updateBaseLayer, &updateLayerCount);
