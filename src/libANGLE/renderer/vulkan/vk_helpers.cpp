...
VkPipelineStageFlags GetImageLayoutSrcStageMask(Context *context, const RefCountedEvent &event)
{
    const ImageMemoryBarrierData &barrierData = kImageMemoryBarrierData[event.getImageLayout()];
    return barrierData.srcStageMask & context->getRenderer()->getSupportedVulkanPipelineStageMask();
}
...
void CommandBufferHelperCommon::executeBarriers(RendererVk *renderer, CommandsState *commandsState)
{
    ...
    mPipelineBarriers.execute(renderer, &primary);
    mEventBarriers.execute(renderer, &primary);
}
                                              ...
                                              ImageHelper *image)
{
    ...
    {
        updateImageLayoutAndBarrier(contextVk, image, aspectFlags, imageLayout, barrierType);
    }
}
                                                            ...
                                                            BarrierType barrierType)
{
    ...
    if (image->updateLayoutAndBarrier(context, aspectFlags, imageLayout, barrierType, mQueueSerial,
                                      &mPipelineBarriers, &mEventBarriers, &semaphore,
                                      &mRefCountedEventGarbage))
    {
...
bool CommandBufferHelperCommon::hasEvent(const RefCountedEvent &event) const
{
    ASSERT(event.valid());
...
void CommandBufferHelperCommon::flushEventsImpl(Context *context, CommandBufferT *commandBuffer)
{
    ...
    for (ImageLayout layout : mRefCountedEvents.mask)
    {
        ...
        mRefCountedEventGarbage.collect(&refCountedEvent);
    }
    ...
    SecondaryCommandBufferCollector *commandBufferCollector)
{
    ...
    mRefCountedEventGarbage.release(context->getRenderer(), mQueueSerial);
...
void OutsideRenderPassCommandBufferHelper::trackImagesWithEvent(Context *context,
                                                                ...
                                                                ImageHelper *image2)
{
    image1->setCurrentRefCountedEvent(context, mRefCountedEvents);
    if (image2 != nullptr)
    {
        image2->setCurrentRefCountedEvent(context, mRefCountedEvents);
    }
    flushEventsImpl(context, &mCommandBuffer);
}
...
{
    imageReadImpl(contextVk, aspectFlags, imageLayout, BarrierType::Event, image);
    // If there is no layout change, we may not need to insert a barrier, but we always have to
    // update event.
    ...
    bool isResolveImage)
{
    ...
    // Even if there is no layout change, we always have to update event. In case of feedback loop,
    // the sampler code should already set the event, we will be set it twice here. But since they
    // uses the same layout and event is refCounted, it works just fine.
    image->setCurrentRefCountedEvent(context, mRefCountedEvents);
}
                                                            ...
                                                            const RenderPass *renderPass)
{
    ...
    // Add VkCmdSetEvent here to track the completion of this renderPass.
    flushEventsImpl(context, &primary);
...
void PipelineBarrierArray::execute(RendererVk *renderer, PrimaryCommandBuffer *primary)
{
...
void ImageHelper::releaseEvents(RendererVk *renderer)
{
    if (mWriteEvent.valid())
    {
        mWriteEvent.destroy(renderer->getDevice());
    }

    while (!mReadOnlyEvents.empty())
    {
        mReadOnlyEvents.back().destroy(renderer->getDevice());
        mReadOnlyEvents.pop_back();
    }
}
                                                   ...
                                                   VkDeviceSize size)
{
    ...
    barrierImpl(context, getAspectFlags(), ImageLayout::TransferDst,
                renderer->getQueueFamilyIndex(), BarrierType::Pipeline, &commandBuffer,
...
void ImageHelper::destroy(RendererVk *renderer)
{
    ...
    if (mWriteEvent.valid())
    {
        mWriteEvent.destroy(device);
    }
    while (!mReadOnlyEvents.empty())
    {
        mReadOnlyEvents.back().destroy(device);
        mReadOnlyEvents.pop_back();
    }
...
void ImageHelper::changeLayoutAndQueue(ContextVk *contextVk,
                                       VkImageAspectFlags aspectMask,
                                       ...
                                       OutsideRenderPassCommandBuffer *commandBuffer)
{
    ...
    VkSemaphore acquireNextImageSemaphore;
    barrierImpl(contextVk, aspectMask, newLayout, newQueueFamilyIndex, BarrierType::Pipeline,
                commandBuffer, &acquireNextImageSemaphore);
    // SwapChain image should not get here.
    ASSERT(acquireNextImageSemaphore == VK_NULL_HANDLE);
    releaseEvents(contextVk->getRenderer());
}
                              ...
                              VkSemaphore *acquireNextImageSemaphoreOut)
{
    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
    {
        ...
        if (mWriteEvent.valid())
        {
            events.emplace_back(mWriteEvent.getEvent().getHandle());
            srcStageMask |= GetImageLayoutSrcStageMask(context, mWriteEvent);
        }
        for (RefCountedEvent &readEvent : mReadOnlyEvents)
        {
            events.emplace_back(readEvent.getEvent().getHandle());
            srcStageMask |= GetImageLayoutSrcStageMask(context, readEvent);
        }
        commandBuffer->waitEvents(static_cast<uint32_t>(events.size()), events.data(), srcStageMask,
                                  dstStageMask, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier);
    }
    else
    {
...
{
    ASSERT(!mWriteEvent.valid() || !commands->hasEvent(mWriteEvent));
    VkSemaphore acquireNextImageSemaphore;
    barrierImpl(context, aspectMask, newLayout, context->getRenderer()->getQueueFamilyIndex(),
                BarrierType::Event, &commands->getCommandBuffer(), &acquireNextImageSemaphore);
    commands->getRefCountedEventCollector().collect(this);
...
bool ImageHelper::updateLayoutAndBarrier(Context *context,
                                         ...
                                         RefCountedEventCollector *garbageCollector)
{
    ...
    if (newLayout == mCurrentLayout)
    {
        ...
        if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
        {
            ASSERT(mWriteEvent.valid());
            ASSERT(mReadOnlyEvents.empty());
            eventBarriers->mergeMemoryEvent(
                ...
                layoutData.dstAccessMask, mWriteEvent);
        }
        else
        {
    ...
    else
    {
        ...
        if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
        {
            ...
            {
                ASSERT(mWriteEvent.valid());
                const ImageMemoryBarrierData &srcLayoutData =
                    kImageMemoryBarrierData[mWriteEvent.getImageLayout()];
                // There is no actual VkImageLayout change and we are read only (from different
                // shader stages), then only wait for writeEvent
                eventBarriers->mergeMemoryEvent(
                    transitionTo.barrierIndex, srcLayoutData.srcStageMask, dstStageMask,
                    srcLayoutData.srcAccessMask, transitionTo.dstAccessMask, mWriteEvent);
            }
            else
            {
                // Either there is actual layout change or same layout but there is WAW hazard.
                VkImageMemoryBarrier imageMemoryBarrier = {};
                ...
                if (mWriteEvent.valid())
                {
                    srcStageMask = GetImageLayoutSrcStageMask(context, mWriteEvent);
                    // If there is an event, we use the waitEvent to do layout change.
                    eventBarriers->mergeImageEvent(transitionTo.barrierIndex, srcStageMask,
                                                   dstStageMask, mWriteEvent, imageMemoryBarrier);
                }
                for (RefCountedEvent &event : mReadOnlyEvents)
                {
                    ASSERT(event.valid());
                    srcStageMask = GetImageLayoutSrcStageMask(context, event);
                    eventBarriers->mergeImageEvent(transitionTo.barrierIndex, srcStageMask,
                                                   dstStageMask, event, imageMemoryBarrier);
                }

                if (IsShaderReadOnlyLayout(transitionTo))
                {
                    garbageCollector->collect(&mReadOnlyEvents);
                }
                else
                {
                    garbageCollector->collect(this);
...
void ImageHelper::setCurrentRefCountedEvent(Context *context, ImageLayoutEventMaps &layoutEventMaps)
{
    ...
    {
        layoutEventMaps.map[mCurrentLayout].init(context->getDevice(), mCurrentLayout);
    ...
    if (HasResourceWriteAccess(barrierData.type))
    {
        ASSERT(!mWriteEvent.valid());
        ASSERT(mReadOnlyEvents.empty());
        mWriteEvent = layoutEventMaps.map[mCurrentLayout];
    }
    else
    {
        for (const RefCountedEvent &event : mReadOnlyEvents)
        {
            // Don't insert the same event twice (triggers VVL error).
            if (event == layoutEventMaps.map[mCurrentLayout])
            {
                return;
            }
        }
        mReadOnlyEvents.emplace_back(layoutEventMaps.map[mCurrentLayout]);
    }
}
                                              ...
                                              const gl::CubeFaceArray<gl::TexLevelMask> &skipLevels)
{
    ...
    {
        // Add VkCmdSetEvent here to track the completion of this copy operation.
        commandBuffer->trackImagesWithEvent(contextVk, this, nullptr);
    }
                                                 ...
                                                 uint8_t **outDataPtr)
{
    ...
    commandBuffer->copyImageToBuffer(mImage, getCurrentLayout(contextVk), bufferHandle, regionCount,
                                     &regions);
    contextVk->trackImagesWithOutsideRenderPassEvent(this, nullptr);
                                                    ...
                                                    vk::BufferHelper *bufferHelper)
{
    ...
    barrierImpl(displayVk, getAspectFlags(), ImageLayout::TransferSrc,
                rendererVk->getQueueFamilyIndex(), BarrierType::Pipeline, &primaryCommandBuffer,
                &acquireNextImageSemaphore);
                                                    ...
                                                    vk::BufferHelper *bufferHelper)
{
    ...
    barrierImpl(displayVk, getAspectFlags(), ImageLayout::TransferDst,
                rendererVk->getQueueFamilyIndex(), BarrierType::Pipeline, &commandBuffer,
                &acquireNextImageSemaphore);
