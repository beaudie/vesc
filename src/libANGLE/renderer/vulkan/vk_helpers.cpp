...
void ImageHelper::releaseEvents(Renderer *renderer)
{
        ...
        if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
        {
            ...
            else
            {
                ...
                if (IsShaderReadOnlyLayout(transitionTo))
                {
                    ...
                    // Since we just used mWriteEvent above, it has to be tracked with mUse. Here we
                    // make a copy of mWriteEvent and collect it.
                    RefCountedEvent event = mWriteEvent;
                    garbageCollector->collect(&event);
                    ASSERT(!event.valid());
                }
                else
                {
...
void ImageHelper::setCurrentRefCountedEvent(Context *context, ImageLayoutEventMaps &layoutEventMaps)
{
    ...
    if (HasResourceWriteAccess(barrierData.type))
    {
        ...
        if (mWriteEvent.valid())
        {
            ASSERT(mCurrentLayout == mWriteEvent.getImageLayout());
                                                    ...
                                                    vk::BufferHelper *bufferHelper)
{
    ...
    barrierImpl(displayVk, getAspectFlags(), ImageLayout::TransferSrc,
                renderer->getQueueFamilyIndex(), BarrierType::Pipeline, &primaryCommandBuffer,
                &acquireNextImageSemaphore);
