                                                 ...
                                                 const PruneReason reason)
{
    ...
    //
    // Color, depth and stencil are the only types supported for now. The boundingBox for each of
    // these types are stored at index 0, 1 and 2 respectively.
    std::array<gl::Box, 3> boundingBox = {gl::Box(gl::kOffsetZero, gl::Extents())};
    angle::FastVector<SupersededWindowMarkers, 8> supersededUpdates;
    for (int index = updateCount - 1; index >= 0; --index)
    {
        ...
        int aspectIndex = isColor ? 0 : (isDepth) ? 1 : 2;
        ...
        if (currentUpdateBox.valid() && boundingBox[aspectIndex].contains(currentUpdateBox))
        {
...
void ImageHelper::removeSupersededUpdates(ContextVk *contextVk, gl::TexLevelMask skipLevelsMask)
{
    ...
    for (LevelIndex levelVk(0); levelVk < LevelIndex(mLevelCount); ++levelVk)
    {
        ...
        pruneSupersededUpdatesForLevel(contextVk, levelGL, PruneReason::MinimizeWorkForFlush);
    }
