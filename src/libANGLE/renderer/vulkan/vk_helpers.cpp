...
angle::Result DynamicBuffer::releaseInFlightBuffersToResourceUseList(ContextVk *contextVk)
{
    for (std::unique_ptr<BufferHelper> &bufferHelper : mInFlightBuffers)
    {
        ...
        contextVk->getShareGroupVk()->acquireResourceUseList(
            std::move(commandBufferHelper->getResourceUseList()));
