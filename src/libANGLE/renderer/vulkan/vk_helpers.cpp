    ...
    SecondaryCommandBufferCollector *commandBufferCollector)
{
    ...
    // If NOT ExecutesInline(), Command Buffer might be already ended in the detachCommandPool().
    // After the Pool is detached (in the CommandProcessor), nothing is written to the buffer (the
    // barriers above are written directly to the primary buffer).
    // Note: RenderPass Command Buffers are explicitly ended in the endRenderPass().
    if (ExecutesInline() || !mIsCommandBufferEnded)
    {
...
angle::Result OutsideRenderPassCommandBufferHelper::endCommandBuffer(Context *context)
{
    ...
    ASSERT(ExecutesInline() || mCommandBuffer.valid());
...
{
    ASSERT(!(*commandBuffer)->isCommandPoolAttached());
    ASSERT(!(*commandBuffer)->hasAllocatorLinks());
    ASSERT(CommandBufferHelperT::ExecutesInline() || !(*commandBuffer)->getCommandBuffer().valid());
    ASSERT(!CommandBufferHelperT::ExecutesInline() || (*commandBuffer)->empty());
