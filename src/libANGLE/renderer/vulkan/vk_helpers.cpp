                                         ...
                                         BufferSuballocation *suballocation)
{
    ...
    // TODO: Try to allocate in the buffer with the largest suballocation size? If that buffer had
    // an issue allocating this despite meeting the requirements, that could be grounds for
    // defragmentation later.

    std::reverse_iterator<rx::vk::BufferBlockPointerVector::iterator> iterMax;
    VkDeviceSize maxSuballoc = 0;
    for (auto iter = mBufferBlocks.rbegin(); iter != mBufferBlocks.rend();)
    {
        std::unique_ptr<BufferBlock> &block = *iter;

        if (maxSuballoc < block->getSuballocSize() &&
            alignedSize + block->getSuballocSize() < (1 << 22) - (1 << 13))
        {
            iterMax = iter;
        }
    ...
    if (iterMax != mBufferBlocks.rend())
    {
        std::unique_ptr<BufferBlock> &block = *iterMax;
        if (block->allocate(alignedSize, alignment, &allocation, &offset) == VK_SUCCESS)
        {
            ...
            WARN() << "* Block " << mBufferBlocks.size() - blockCount
                   << " was chosen due to max size possible.";
            return angle::Result::Continue;
        }
    }
                                                         ...
                                                         bool *isSuccess)
{
    ...
    *isSuccess                = false;
    ...
    VkResult result = newBlock->allocate(bufferSize, alignment, &newVirtualAllocation, &newOffset);
                                              ...
                                              BufferUsageType usageType)
{
    ...
    // TODO: Make the pools per context? Or at least keep track of the context that initializes the
    // buffer, so that data transfer only happens within the same context.
...
angle::Result BufferPool::compressBufferBlocks(ContextVk *contextVk, VkDeviceSize alignment)
{
        ...
        minSizeBlock->getSuballocSize() + secondMinSizeBlock->getSuballocSize() < (1 << 22))
    {
        ...
        for (BufferHelper *bufferHelper : mBufferList)
        {
            ...
            if (bufferHelper->getContext() != contextVk)
            {
                WARN() << "Context mismatch 1; will not proceed with transfer.";
                continue;
            }
        ...
        if (sizeBeforeMove == minSizeBlock->getSuballocSize())
        {
            ...
            for (BufferHelper *bufferHelper : mBufferList)
            {
                ...
                if (bufferHelper->getContext() != contextVk)
                {
                    WARN() << "Context mismatch 2; will not proceed with transfer.";
                    continue;
                }
    ...
    //                }
    //                bool isSuccess = false;
    //                ANGLE_TRY(bufferHelper->transferSuballocationToBlock(contextVk, block.get(),
    //                                                                     alignment, &isSuccess));
    //                if (isSuccess)
    //                {
    //                    break;
