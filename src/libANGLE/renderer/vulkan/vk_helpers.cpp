...
bool ImageHelper::clearLevelLayer(uint32_t level, uint32_t layer, vk::CommandBuffer *commandBuffer)
{
                                              ...
                                              vk::CommandBuffer *commandBuffer)
{
    ...
    for (SubresourceUpdate &update : mSubresourceUpdates)
    {
        ...
        // If the staged update covers the whole level, no need to clear the image in the staged
        // layers of that level.
        uint32_t mipWidth        = mExtents.getMipWidth(dstSubresource.mipLevel);
        uint32_t mipHeight       = mExtents.getMipHeight(dstSubresource.mipLevel);
        uint32_t mipDepth        = mExtents.getMipDepth(dstSubresource.mipLevel);
        const VkOffset3D &offset = update.dstOffset();
        const VkExtent3D &extent = update.dstExtent();
            ...
            extent.height == mipHeight && extent.depth == mipDepth)
        {
            ...
            {
                setCleared(dstSubresource.mipLevel, dstSubresource.baseArrayLayer + layer);
            }
        }

        // Clear any layer that has staged updates.  Any update that covers a whole layer is already
        // marked above as not needing a clear.  That means any update done here will only partially
        // cover the layer and thus clearing it before the update is necessary.
        bool cleared = false;
        for (uint32_t layer = 0; layer < updateLayerCount; ++layer)
        {
            cleared =
                clearLevelLayer(updateMipLevel, updateBaseLayer + layer, commandBuffer) || cleared;
        }

        // Add a barrier between every clear and its corresponding update to avoid races.
        if (cleared)
        {
            changeLayout(VK_IMAGE_ASPECT_COLOR_BIT, vk::ImageLayout::TransferDst, commandBuffer);
        }
