...
DynamicDescriptorPool::DynamicDescriptorPool()
    : mMaxSetsPerPool(kDefaultDescriptorPoolMaxSets), mPoolSize{}
{
}
...
{
    ASSERT(mPoolSize.descriptorCount == 0);
    mPoolSize.type            = descriptorType;
    mPoolSize.descriptorCount = descriptorsPerSet * kDefaultDescriptorPoolMaxSets;
                                                  ...
                                                  VkDescriptorSet *descriptorSetsOut)
{
    if (!bindingOut->valid() || !bindingOut->get().hasCapacity(descriptorSetCount))
    {
        ...
        // Make sure the old binding knows the descriptor sets can still be in-use.
        if (bindingOut->valid())
        {
