                              ...
                              VkSemaphore *acquireNextImageSemaphoreOut)
{
    ...
    // Fallback to VkImageMemoryBarrier for
    // 1) imageWaitEvent below
    // 2) when VK_KHR_Synchronization2 is not supported
    VkImageMemoryBarrier imageMemoryBarrier = {};
    imageMemoryBarrier.sType                = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    imageMemoryBarrier.srcAccessMask =
        static_cast<VkAccessFlags>(imageMemoryBarrier2.srcAccessMask);
    imageMemoryBarrier.dstAccessMask =
        static_cast<VkAccessFlags>(imageMemoryBarrier2.dstAccessMask);
    imageMemoryBarrier.oldLayout           = imageMemoryBarrier2.oldLayout;
    imageMemoryBarrier.newLayout           = imageMemoryBarrier2.newLayout;
    imageMemoryBarrier.srcQueueFamilyIndex = imageMemoryBarrier2.srcQueueFamilyIndex;
    imageMemoryBarrier.dstQueueFamilyIndex = imageMemoryBarrier2.dstQueueFamilyIndex;
    imageMemoryBarrier.image               = imageMemoryBarrier2.image;
    imageMemoryBarrier.subresourceRange    = imageMemoryBarrier2.subresourceRange;
                                                   ...
                                                   LevelIndex maxLevel)
{
    ...
    for (LevelIndex mipLevel(1); mipLevel <= LevelIndex(mLevelCount); ++mipLevel)
    {
        ...
        if (mipLevel > baseLevel && mipLevel <= maxLevel)
        {
            ...
            applyImageMemoryBarrier(commandBuffer, barrier2,
                                    renderer->getFeatures().supportsSynchronization2.enabled);
...
void ImageHelper::applyImageMemoryBarrier(OutsideRenderPassCommandBuffer *commandBuffer,
                                          const VkImageMemoryBarrier2 imageBarrier2,
                                          const bool supportsSynchronization2)
{
    if (supportsSynchronization2)
    {
        commandBuffer->imageBarrier2(imageBarrier2);
    }
    else
    {
        VkImageMemoryBarrier barrier = {};
        barrier.sType                = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        barrier.srcAccessMask        = static_cast<VkAccessFlags>(imageBarrier2.srcAccessMask);
        barrier.dstAccessMask        = static_cast<VkAccessFlags>(imageBarrier2.dstAccessMask);
        barrier.oldLayout            = imageBarrier2.oldLayout;
        barrier.newLayout            = imageBarrier2.newLayout;
        barrier.srcQueueFamilyIndex  = imageBarrier2.srcQueueFamilyIndex;
        barrier.dstQueueFamilyIndex  = imageBarrier2.dstQueueFamilyIndex;
        barrier.image                = imageBarrier2.image;
        barrier.subresourceRange     = imageBarrier2.subresourceRange;

        commandBuffer->imageBarrier(static_cast<VkPipelineStageFlags>(imageBarrier2.srcStageMask),
                                    static_cast<VkPipelineStageFlags>(imageBarrier2.dstStageMask),
                                    barrier);
    }
}
