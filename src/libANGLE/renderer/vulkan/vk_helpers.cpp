...
void CommandBufferHelper::finalizeDepthStencilImageLayout(Context *context)
{
    ...
    if (mDepthStencilImage->usedByCurrentRenderPassAsAttachmentAndSampler())
    {
        ...
        {
            ASSERT(imageLayout == ImageLayout::DepthStencilFragmentShaderReadOnly ||
                                                   ...
                                                   LevelIndex maxLevel)
{
    ...
    // Transition all mip level to the same layout so we can declare
    // our whole image layout to be FragmentShaderReadOnly.
