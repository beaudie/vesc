...
{
    ASSERT(areSubresourceUpdateImageRefsConsistent());
                                                            ...
                                                            gl::TexLevelMask skipLevelsMask)
{
    ...
    for (vk::LevelIndex levelVk(0); levelVk < vk::LevelIndex(image->get().getLevelCount());
         ++levelVk)
    {
...
bool ImageHelper::isSubresourceUpdateImageRefConsistent(RefCounted<ImageHelper> *image) const
{
    if (image == nullptr)
    {
        return true;
...
bool ImageHelper::areSubresourceUpdateImageRefsConsistent() const
{
    for (const std::vector<SubresourceUpdate> &levelUpdates : mSubresourceUpdates)
    {
        for (const SubresourceUpdate &update : levelUpdates)
        {
            if (update.updateSource == UpdateSource::Image &&
                !isSubresourceUpdateImageRefConsistent(update.image))
            {
                return false;
            }
        }
