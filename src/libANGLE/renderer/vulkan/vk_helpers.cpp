...
VkPipelineStageFlags GetImageLayoutDstStageMask(Context *context, const RefCountedEvent &event)
{
    ...
    return barrierData.dstStageMask & context->getRenderer()->getSupportedVulkanPipelineStageMask();
}
    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
    {
        ...
        if (mWriteEvent.valid())
        {
            ...
            srcStageMask |= GetImageLayoutDstStageMask(context, mWriteEvent);
        }
        for (RefCountedEvent &readOnlyEvent : mReadOnlyEvents)
        {
            events.emplace_back(readOnlyEvent.getEvent().getHandle());
            srcStageMask |= GetImageLayoutDstStageMask(context, readOnlyEvent);
        }
    ...
    else
    {
        ...
        if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
        {
            if (transitionFrom.layout == transitionTo.layout &&
                IsShaderReadOnlyLayout(transitionTo) && mWriteEvent.valid())
            {
                ...
                eventBarriers->addMemoryEvent(srcLayoutData.srcStageMask, dstStageMask,
                                              srcLayoutData.srcAccessMask,
                                              transitionTo.dstAccessMask, mWriteEvent);
            }
            else
            {
                ...
                initImageMemoryBarrierStruct(context, aspectMask, newLayout,
                                             ...
                                             &imageMemoryBarrier);
                // If there is an event, we use the waitEvent to do layout change.
                eventBarriers->addImageEvent(context, mWriteEvent, mReadOnlyEvents, dstStageMask,
                                             imageMemoryBarrier);
...
void ImageHelper::setCurrentRefCountedEvent(Context *context, ImageLayoutEventMaps &layoutEventMaps)
{
    ...
    {
        ASSERT(mReadOnlyEvents.empty());
        if (mWriteEvent != layoutEventMaps.map[mCurrentLayout])
        {
            if (mWriteEvent.valid())
            {
                mWriteEvent.destroy(context->getDevice());
            }
