...
void DynamicBuffer::release(Context *context, const ContextSerialMap &serials)
{
    ...
    mBuffer.release(context, serials);
    mMemory.release(context, serials);
}
...
void DynamicBuffer::releaseRetainedBuffers(Context *context, const ContextSerialMap &serials)
{
    ...
    {
        toFree.buffer.release(context, serials);
        toFree.memory.release(context, serials);
    }
                                                  ...
                                                  VkDescriptorSet *descriptorSetsOut)
{
    if (mFreeDescriptorSets < descriptorSetCount || mCurrentSetsCount >= mMaxSetsPerPool)
    {
        ...
        // pool for it.
        mCurrentDescriptorPool.release(context, context->getCurrentQueueSerial());
        ANGLE_TRY(allocateNewPool(context));
    }
...
{
    mImage.release(context, serials);
    mDeviceMemory.release(context, serials);
}
