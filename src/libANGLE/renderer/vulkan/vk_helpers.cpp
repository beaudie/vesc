...
angle::Result RenderPassCommandBufferHelper::beginRenderPass(
    ContextVk *contextVk,
    const MaybeImagelessFramebuffer &framebuffer,
    ...
    const RenderPassSerial rpSerial,
    RenderPassCommandBuffer **commandBufferOut)
{
    ...
    mFramebuffer.copyFromFramebuffer(framebuffer);
    ...
    const RenderPassDesc &renderPassDesc)
{
    ...
    mFramebuffer.copyFromFramebuffer(newFramebuffer);
    mRenderPassDesc = renderPassDesc;
}
...
void ImageHelper::resetCachedProperties()
{
    ...
    //    std::fill(mViewFormats.begin(), mViewFormats.end(), VK_FORMAT_UNDEFINED);
