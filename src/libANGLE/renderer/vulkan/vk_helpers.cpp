...
void DynamicBuffer::init(size_t alignment, RendererVk *renderer)
{
    ...
    fprintf(stderr, "DynamicBuffer alignment. Given: %zu, nonCoherentAtomSize: %llu\n",
            alignment, (unsigned long long)renderer->getPhysicalDeviceProperties().limits.nonCoherentAtomSize);
    ASSERT(alignment > 0);
    mAlignment = std::max(
        alignment,
                                      ...
                                      bool *newBufferAllocatedOut)
{
    ...
    fprintf(stderr, "Allocating buffer of size %zu (requested.  Granted: %zu (alignment was: %zu))\n", sizeInBytes,
            sizeToAllocate, mAlignment);
