    ...
    DescriptorSetPointer *descriptorSetOut)
{
    ...
    DescriptorPoolWeakPointer pool1;
    if (*descriptorSetOut)
    {
    ...
    DescriptorPoolWeakPointer pool2;
    if (mDescriptorPools[mCurrentPoolIndex] &&
        mDescriptorPools[mCurrentPoolIndex].get() != pool1.get())
    {
    ...
    DescriptorPoolWeakPointer pool;
    for (size_t poolIndex = 0; poolIndex < mDescriptorPools.size(); ++poolIndex)
    {
        pool = mDescriptorPools[poolIndex];
        if (!pool)
        {
            continue;
        }
        if (pool.get() == pool1.get() || pool.get() == pool2.get())
        {
            continue;
        }
    ...
    pool = mDescriptorPools[mCurrentPoolIndex];
    bool success =
        pool->allocateDescriptorSet(context, descriptorSetLayout, pool, descriptorSetOut);
                                                       ...
                                                       const DescriptorSetDesc &desc)
{
    ...
    if (mDescriptorSetCache.getDescriptorSet(desc, &descriptorSet))
    {
        ...
        {
            const DescriptorPoolWeakPointer &pool = descriptorSet->getPool();
            descriptorSet.reset();
