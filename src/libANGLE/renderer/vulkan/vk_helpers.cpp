...
{
    return mReadOnlyUse.getSerial().valid();
}
...
{
    renderer->collectGarbageAndReinit(&mReadOnlyUse, &mImage, &mDeviceMemory);
    mReadWriteUse.release();
    // Keep "mReadWriteUse" valid.
    mReadWriteUse.init();
...
void ImageViewHelper::release(RendererVk *renderer)
{
    ...
    if (!garbage.empty())
    {
        renderer->collectGarbage(std::move(mReadOnlyUse), std::move(garbage));
