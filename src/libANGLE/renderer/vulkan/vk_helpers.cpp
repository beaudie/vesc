                                        ...
                                        uint32_t mipLevels)
{
    ...
    mLayerCount = getEffectiveLayerCount(textureType, extents);
...
uint32_t ImageHelper::getEffectiveDepth(gl::TextureType textureType, const gl::Extents &extents)
{
    switch (textureType)
    {
                                                  ...
                                                  const vk::Format &vkFormat)
{
    ...
    copy.imageSubresource.layerCount     = getEffectiveLayerCount(index.getType(), extents);

    gl_vk::GetOffset(offset, &copy.imageOffset);

    copy.imageExtent.width  = extents.width;
    copy.imageExtent.height = extents.height;
    copy.imageExtent.depth  = getEffectiveDepth(index.getType(), extents);
    ...
    if (stencilAllocationSize > 0)
    {
        ...
        stencilCopy.imageSubresource.layerCount = getEffectiveLayerCount(index.getType(), extents);
