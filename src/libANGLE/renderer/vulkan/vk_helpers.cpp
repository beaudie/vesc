                              ...
                              VkSemaphore *acquireNextImageSemaphoreOut)
{
    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    else
    {
        ...
        releaseEvents(context->getRenderer());
    }
                                         ...
                                         RefCountedEventGarbageObjects *garbageObjects)
{
    ...
    if (newLayout == mCurrentLayout)
    {
        ...
        if (barrierType == BarrierType::Event && mWriteEvent.valid())
        {
            ...
            eventBarriers->addMemoryEvent(GetImageLayoutSrcStageMask(context, layoutData),
                                          ...
                                          mWriteEvent);
            garbageObjects->addDuplicate(mWriteEvent);
        }
        else
        {
        ...
        if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
        {
            ...
            else
            {
                ...
                if (IsShaderReadOnlyLayout(transitionTo))
                {
                    if (mWriteEvent.valid())
                    {
                        // Since we just used mWriteEvent above, it has to be tracked with mUse.
                        // Here we make a copy of mWriteEvent and collect it.
                        garbageObjects->addDuplicate(mWriteEvent);
                    }
                    garbageObjects->add(&mReadOnlyEvents);
                }
                else
                {
                    if (mWriteEvent.valid())
                                                   ...
                                                   LevelIndex maxLevel)
{
    ...
    if (mWriteEvent.valid())
    {
        mWriteEvent.release(contextVk->getDevice());
    }
    while (!mReadOnlyEvents.empty())
    {
        mReadOnlyEvents.back().release(contextVk->getDevice());
        mReadOnlyEvents.pop_back();
    }
