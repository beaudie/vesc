    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    if (barrierType == BarrierType::Event && (mWriteEvent.valid() || !mReadOnlyEvents.empty()))
    {
        ...
        if (mWriteEvent.valid())
        {
            ...
            commandBuffer->imageWaitEvent(mWriteEvent.getEvent().getHandle(), srcStageMask,
        ...
        for (RefCountedEvent &event : mReadOnlyEvents)
        {
            ...
            commandBuffer->imageWaitEvent(event.getEvent().getHandle(), srcStageMask, dstStageMask,
                                          imageMemoryBarrier);
        }
    }
    else
