                              ...
                              VkSemaphore *acquireNextImageSemaphoreOut)
{
    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    updateNonFragmentAccessHistory(dstStageMask);
...
void ImageHelper::setCurrentRefCountedEvent(Context *context, EventMaps &eventMaps)
{
    ...
    // in the past 32 layout changes. Use of VkEvent will not be beneficial if it is only accessed
    // by fragment stages.
    if (mNonFragmentAccessHistory.any())
    {
        return;
    }
