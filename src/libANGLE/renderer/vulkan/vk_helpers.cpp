...
angle::Result ImageHelper::stageSubresourceClear(ContextVk *contextVk,
                                                 ...
                                                 VkClearValue clearValue)
{
    // TODO: Use Vulkan APIs for full clear?
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            switch (update.updateSource)
            {
                ...
                case UpdateSource::ClearPartial:
                {
                    ...
                    // TODO: Currently clearing MS depth/stencil textures is not supported.
                    if (clearPartialUpdate.aspectFlags == VK_IMAGE_ASPECT_COLOR_BIT &&
                        (clearPartialUpdate.textureType == gl::TextureType::_2DMultisample ||
                         clearPartialUpdate.textureType == gl::TextureType::_2DMultisampleArray))
                    {
                        UtilsVk::ClearImageParameters params = {};
                        params.clearArea      = {static_cast<int>(clearPartialUpdate.offsetX),
                                                 static_cast<int>(clearPartialUpdate.offsetY),
                                                 static_cast<int>(clearPartialUpdate.width),
                                                 static_cast<int>(clearPartialUpdate.height)};
                        params.dstMip         = updateMipLevelVk;
                        params.colorMaskFlags = VK_COLOR_COMPONENT_R_BIT |
                                                VK_COLOR_COMPONENT_G_BIT |
                                                VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;

                        params.colorClearValue = clearPartialUpdate.value.color;

                        for (uint32_t layerIndex = 0; layerIndex < clearPartialUpdate.layerCount;
                             ++layerIndex)
                        {
                            params.dstLayer = layerIndex + clearPartialUpdate.layerIndex;
