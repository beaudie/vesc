                                                     ...
                                                     ImageHelper *image)
{
    ...
    // Track the image with the event
    image->setCurrentRefCountedEvent(contextVk, mRefCountedEvents);
}
    ...
    bool isResolveImage)
{
    ...
    // the sampler code should already set the event, which means we will be set it twice here. But
    // since they uses the same layout and event is refCounted, it should work just fine.
    image->setCurrentRefCountedEvent(context, mRefCountedEvents);
}
