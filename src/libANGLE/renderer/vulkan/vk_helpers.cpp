                                      ...
                                      bool *newBufferAllocatedOut)
{
    ...
    if (ptrOut)
    {
        ...
        ANGLE_TRY(mBuffer->map(&mappedMemory));
...
angle::Result DynamicBuffer::flush(ContextVk *contextVk)
{
    ...
    {
        ASSERT(mBuffer != nullptr);
        ANGLE_TRY(mBuffer->flush(mLastFlushOrInvalidateOffset,
...
angle::Result DynamicBuffer::invalidate(ContextVk *contextVk)
{
    ...
    {
        ASSERT(mBuffer != nullptr);
        ANGLE_TRY(mBuffer->invalidate(mLastFlushOrInvalidateOffset,
                                      mNextAllocationOffset - mLastFlushOrInvalidateOffset));
        mLastFlushOrInvalidateOffset = mNextAllocationOffset;
    }
    return angle::Result::Continue;
                                 ...
                                 VkMemoryPropertyFlags memoryPropertyFlags)
{
    ...
    allocCreateInfo.requiredFlags  = (memoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    allocCreateInfo.preferredFlags = (memoryPropertyFlags & (~VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT));

    mVmaAllocator = &contextVk->getRenderer()->getVmaAllocator();
    ...
    if (renderer->getFeatures().allocateNonZeroMemory.enabled)
    {
        ...
        else if ((mMemoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) != 0)
        {
            ...
            ANGLE_TRY(InitMappableVmaMemory(*mVmaAllocator, &mVmaMemory, mSize, kNonZeroInitValue,
                                            !hostCoherent));
        }
    }
...
void BufferHelper::release(RendererVk *renderer)
{
    ...
    renderer->collectGarbageAndReinit(&mUse, &mBuffer, &mBufferView, &mVmaMemory);
}
...
angle::Result BufferHelper::mapImpl()
{
    mVmaMemory.map(*mVmaAllocator, &mMappedMemory);

    return (mMappedMemory != nullptr) ? angle::Result::Continue : angle::Result::Stop;
}
...
void BufferHelper::unmap()
{
    if (mMappedMemory)
    {
        mVmaMemory.unmap(*mVmaAllocator);
        mMappedMemory = nullptr;
    }
}

angle::Result BufferHelper::flush(VkDeviceSize offset, VkDeviceSize size)
{
    bool hostVisible  = mMemoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
    bool hostCoherent = mMemoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    if (hostVisible && !hostCoherent)
    {
        mVmaMemory.flush(*mVmaAllocator, offset, size);
    }
    return angle::Result::Continue;
}

angle::Result BufferHelper::invalidate(VkDeviceSize offset, VkDeviceSize size)
{
    bool hostVisible  = mMemoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
    bool hostCoherent = mMemoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    if (hostVisible && !hostCoherent)
    {
        mVmaMemory.invalidate(*mVmaAllocator, offset, size);
    }
    return angle::Result::Continue;
}
