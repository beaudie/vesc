    ...
    RenderPassCommandBuffer **commandBufferOut)
{
    ...
    *commandBufferOut = &getCommandBuffer();
    ...
    for (uint32_t i = 0; i < gl::IMPLEMENTATION_MAX_FRAMEBUFFER_ATTACHMENTS; i++)
    {
        PackedAttachmentIndex attachmentIndexVk(i);
        PackedAttachmentOpsDesc attachmentOps = mAttachmentOps[attachmentIndexVk];
        RenderPassLoadOp loadOp               = static_cast<RenderPassLoadOp>(attachmentOps.loadOp);
        RenderPassLoadOp stencilLoadOp = static_cast<RenderPassLoadOp>(attachmentOps.stencilLoadOp);
        if (loadOp == RenderPassLoadOp::Clear || stencilLoadOp == RenderPassLoadOp::Clear)
        {
            mAttachmentClearOps.set(i, 1);
        }
    }
