...
SubresourceLayerMask GetSubresourceLayerMask(uint32_t layerStart, uint32_t layerCount)
{
                                          ...
                                          uint32_t layerCount) const
{
    ...
    for (uint32_t levelOffset = 0; levelOffset < levelCount; levelOffset++)
    {
        ...
        if ((mSubresourcesWrittenSinceBarrier[level] & layerMask) != 0)
        {
            return true;
        }
    }
                                                  ...
                                                  const gl::TexLevelMask &skipLevelsAllFaces)
{
         ...
         ++updateMipLevelGL)
    {
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            {
                SubresourceLayerMask subresourceHash =
                    GetSubresourceLayerMask(updateBaseLayer, updateLayerCount);

                if ((mSubresourcesWrittenSinceBarrier[updateMipLevelGL.get()] & subresourceHash) !=
                    0)
                {
                                              ...
                                              const gl::CubeFaceArray<gl::TexLevelMask> &skipLevels)
{
    ...
    clipToUpdateListUpperLimit(&levelGLEnd);
...
void ImageHelper::clipToUpdateListUpperLimit(gl::LevelIndex *level) const
{
