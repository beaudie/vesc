...
void RenderPassCommandBufferHelper::depthStencilImagesDraw(
    ResourceUseList *resourceUseList,  // Remove arg?
    gl::LevelIndex level,
    uint32_t layerStart,
    uint32_t layerCount,
    ImageHelper *image,
                                                            ...
                                                            vk::ResourceUseList &resourceUseList)
{
    ...
    //    ResourceUseList *resourceUseListRP =
    //    &contextVk->getStartedRenderPassCommands().getResourceUseList();
    // Outside RP? This function is only used in flushAndGetSerial(), where outside RP command
    // buffer is used. In that case, change the output of this function to angle::Result.
...
void CommandBufferAccess::onBufferSemaphoreAcquire(SemaphoreVk *semaphore, BufferHelper *buffer)
{
    mAcquireSemaphoresBuffers.emplace_back(CommandBufferBufferAcquireSemaphore{semaphore, buffer});
}

void CommandBufferAccess::onBufferSemaphoreRelease(SemaphoreVk *semaphore, BufferHelper *buffer)
{
    mReleaseSemaphoresBuffers.emplace_back(CommandBufferBufferReleaseSemaphore{semaphore, buffer});
}
void CommandBufferAccess::onImageSemaphoreAcquire(SemaphoreVk *semaphore, ImageHelper *image)
{
    mAcquireSemaphoresImages.emplace_back(CommandBufferImageAcquireSemaphore{semaphore, image});
}

void CommandBufferAccess::onImageSemaphoreRelease(SemaphoreVk *semaphore, ImageHelper *image)
{
    mReleaseSemaphoresImages.emplace_back(CommandBufferImageReleaseSemaphore{semaphore, image});
}

void CommandBufferAccess::onResourceUpdate(Resource *resource)
{
    mUpdateResources.emplace_back(CommandBufferResourceUpdate{resource});
}
