...
void ImageHelper::releaseEvents(Renderer *renderer)
{
    ...
    {
        mWriteEvent.release(renderer->getDevice());
    }

    while (!mReadOnlyEvents.empty())
    {
        mReadOnlyEvents.back().release(renderer->getDevice());
...
void ImageHelper::destroy(Renderer *renderer)
{
    ...
    if (mWriteEvent.valid())
    {
        mWriteEvent.release(device);
    }
    while (!mReadOnlyEvents.empty())
    {
        mReadOnlyEvents.back().release(device);
        mReadOnlyEvents.pop_back();
    }
...
void ImageHelper::setCurrentRefCountedEvent(Context *context, ImageLayoutEventMaps &layoutEventMaps)
{
    ...
    if (HasResourceWriteAccess(barrierData.type))
    {
        ...
        if (mWriteEvent.valid())
        {
            ...
            mWriteEvent.release(context->getDevice());
        }
