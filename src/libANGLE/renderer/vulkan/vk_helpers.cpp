    ...
    SharedDescriptorSetCacheKey *newSharedCacheKeyOut)
{
    ...
    if (mDescriptorSetCache.getDescriptorSet(desc, &descriptorSetHelper))
    {
        ...
        *poolOut              = descriptorSetHelper->getPool();
        *descriptorSetOut     = std::move(descriptorSetHelper);
        mCacheStats.hit();
        return angle::Result::Continue;
    }

    ANGLE_TRY(allocateDescriptorSet(context, descriptorSetLayout, poolOut, &descriptorSetHelper));
                                                      ...
                                                      DescriptorPoolPointer *pool)
{
    ...
    if (mDescriptorPools.size() < 2 || (*pool)->hasValidDescriptorSet() || !pool->unique())
    {
        return;
    }
...
bool DynamicDescriptorPool::hasPool(const DescriptorPoolHelper *other) const
{
