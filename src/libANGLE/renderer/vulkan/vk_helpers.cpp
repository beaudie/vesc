                                ...
                                uint32_t layerCount)
{
    ...
    mStagingBuffer.init(4, context->getRenderer());

    return angle::Result::Continue;
}

void ImageHelper::release(RendererVk *renderer, bool releaseStagingBufferr)
{
    renderer->releaseObject(getStoredQueueSerial(), &mImage);
    renderer->releaseObject(getStoredQueueSerial(), &mDeviceMemory);
    if (releaseStagingBufferr)
    {
...
void ImageHelper::SubresourceUpdate::release(RendererVk *renderer)
{
    ...
    {
        image.image->release(renderer, true);
