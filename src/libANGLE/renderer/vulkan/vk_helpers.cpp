...
void BufferMemory::collectGarbage(RendererVk *renderer, Serial serial)
{
    if (valid())
    {
        vk::GarbageList garbageList;
        garbageList.emplace_back(isExternalBuffer() ? vk::GetGarbage(&mExternalMemory)
                                                    : vk::GetGarbage(&mAllocation));
...
void BufferHelper::release(RendererVk *renderer)
{
    ...
    mMemory.collectGarbage(renderer, mUse.getSerial());
    renderer->collectGarbageAndReinit(&mUse, &mBuffer, &mBufferView);
}
