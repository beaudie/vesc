                                         ...
                                         CommandBuffer *readbackCommandBuffer)
{
    ...
    VkOffset3D extent = {src->getExtents().width, src->getExtents().height,
                         src->getExtents().depth};
                                      ...
                                      DynamicBuffer *stagingBuffer)
{
    ...
    if (overrideSourceFormat != readFormat->id)
    {
        // It should not be possible for a multisampled image to require colorspace conversion here
        ASSERT(!isMultisampled);

        // We currently only support converting between linear <-> nonlinear colorspaces for
        // otherwise equivilant formats
        ASSERT((ConvertToSRGB(overrideSourceFormat) == readFormat->id) ||
