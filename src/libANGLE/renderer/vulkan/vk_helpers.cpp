                                                 ...
                                                 ImageHelper *resolveImage)
{
    ...
    image->retain(resourceUseList);
    image->onWrite(level, 1, layerStart, layerCount, kDepthStencilAspects);
    mRenderPassUsedImages.insert(image->getImageSerial().getValue());
    ...
    if (resolveImage)
    {
        ...
        resolveImage->onWrite(level, 1, layerStart, layerCount, kDepthStencilAspects);
                                    ...
                                    VkImageAspectFlags aspectFlags)
{
    ...
    uint32_t layerRangeBits =
        layerCount >= kMaxContentDefinedLayerCount ? ~0u : angle::Bit<uint8_t>(layerCount) - 1;
    ...
    for (uint32_t levelOffset = 0; levelOffset < levelCount; ++levelOffset)
    {
        ...
        if ((aspectFlags & ~VK_IMAGE_ASPECT_STENCIL_BIT) != 0)
        {
            mContentDefined[level.get()] |= layerRangeBits;
        }
        if ((aspectFlags & VK_IMAGE_ASPECT_STENCIL_BIT) != 0)
        {
            mStencilContentDefined[level.get()] |= layerRangeBits;
        }
    }
}
...
bool ImageHelper::hasSubresourceDefinedContent(gl::LevelIndex level, uint32_t layerIndex) const
{
    return layerIndex >= kMaxContentDefinedLayerCount
               ? true
               : mContentDefined[level.get()].test(layerIndex);
}
                                                      ...
                                                      uint32_t layerIndex) const
{
    return layerIndex >= kMaxContentDefinedLayerCount
               ? true
               : mStencilContentDefined[level.get()].test(layerIndex);
}

void ImageHelper::invalidateSubresourceContent(gl::LevelIndex level, uint32_t layerIndex)
{
    if (layerIndex < kMaxContentDefinedLayerCount)
    {
        mContentDefined[level.get()].reset(layerIndex);
    }
}

void ImageHelper::invalidateSubresourceStencilContent(gl::LevelIndex level, uint32_t layerIndex)
{
    if (layerIndex < kMaxContentDefinedLayerCount)
    {
        mStencilContentDefined[level.get()].reset(layerIndex);
    }
}
...
void ImageHelper::restoreSubresourceContent(gl::LevelIndex level, uint32_t layerIndex)
{
    if (layerIndex < kMaxContentDefinedLayerCount)
    {
        mContentDefined[level.get()].set(layerIndex);
    }
}
...
void ImageHelper::restoreSubresourceStencilContent(gl::LevelIndex level, uint32_t layerIndex)
{
    if (layerIndex < kMaxContentDefinedLayerCount)
    {
        mStencilContentDefined[level.get()].set(layerIndex);
    }
}
                                              ...
                                              gl::TexLevelMask skipLevelsMask)
{
    ...
    ANGLE_TRY(contextVk->onImageTransferWrite(gl::LevelIndex(0), 1, kMaxContentDefinedLayerCount, 0,
                                              aspectFlags, this));
    CommandBuffer *commandBuffer = &contextVk->getOutsideRenderPassCommandBuffer();
