                                                      ...
                                                      BufferHelper *buffer)
{
        ...
        contextVk->getStartedRenderPassCommands().usesBuffer(*buffer))
    {
        ...
        // mUsedBufferCount may not be accurate when it is being read by both commands, but that is
        // okay given it is only used for information displayed by widgets. We should not run into
        // situation that RP is writing to it while we are reading it here (in this case it should
        // trigger endRP).
