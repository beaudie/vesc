...
// clang-format off
const ImageMemoryBarrierData* ImageMemoryBarrierData::getPrebuiltBarrierData(ImageLayout imageLayout)
{
  const static ImageMemoryBarrierData kImageMemoryBarrierData[static_cast<uint32_t>(ImageLayout::PrebuiltLayoutCount)] =
  {
  ...
  ASSERT(imageLayout != ImageLayout::MultiShadersAccess);

  // These are most often used enums. For performance reasons, these are statically created so
  // that pointer compariosn will tell if they are same or not.
  const ImageMemoryBarrierData *data = &kImageMemoryBarrierData[static_cast<int>(imageLayout)];
...
{
  // The prebuilt layout enums are always statically created in getPrebuiltBarrierData.
  // Only the ones that are dynamically allocated should be freed here.
...
ImageHelper::ImageHelper()
    : mFormat(nullptr),
      ...
      mLevelCount(0)
{
    mBarrierData = ImageMemoryBarrierData::getPrebuiltBarrierData(mCurrentLayout);
}
...
{
    ASSERT(this != &other);
    mBarrierData = other.mBarrierData;
    ...
    other.setPrebuiltLayout(ImageLayout::Undefined);
}
                                        ...
                                        uint32_t layerCount)
{
    ...
    setPrebuiltLayout(initialLayout);
...
angle::Result ImageHelper::initializeNonZeroMemory(Context *context, VkDeviceSize size)
{
    ...
    const ImageMemoryBarrierData *transitionTo =
        ImageMemoryBarrierData::getPrebuiltBarrierData(ImageLayout::TransferDst);
    forceChangeLayoutAndQueue(getAspectFlags(), transitionTo, mCurrentQueueFamilyIndex,
                              &commandBuffer);
...
void ImageHelper::destroy(VkDevice device)
{
    ...
    mLevelCount = 0;
    mSerial     = rx::kZeroSerial;
    setPrebuiltLayout(ImageLayout::Undefined);
}

void ImageHelper::setPrebuiltLayout(ImageLayout layout)
{
    ASSERT(static_cast<uint32_t>(layout) < static_cast<uint32_t>(ImageLayout::PrebuiltLayoutCount));
                            ...
                            CommandBufferT *command)
{
    ...
    if (needBarrier)
    {
        insertBarrier(aspectFlags, newBarrierData, mCurrentQueueFamilyIndex, command);
    }
                                       ...
                                       CommandBuffer *commandBuffer)
{
    ...
    const ImageMemoryBarrierData *transitionTo =
        ImageMemoryBarrierData::getPrebuiltBarrierData(newLayout);
    forceChangeLayoutAndQueue(aspectMask, transitionTo, newQueueFamilyIndex, commandBuffer);
}

void ImageHelper::onExternalLayoutChange(ImageLayout newLayout)
{
    setPrebuiltLayout(newLayout);
...
{
    insertBarrier(aspectMask, transitionTo, newQueueFamilyIndex, commandBuffer);

    if (mBarrierData != transitionTo)
    {
        ImageMemoryBarrierData::destroyBarrierData(mBarrierData);
        mCurrentLayout = transitionTo->imageLayout();
...
// Explicitly instantiate forceChangeLayoutAndQueue with SecondaryCommandBuffer.
template void ImageHelper::forceChangeLayoutAndQueue(VkImageAspectFlags aspectMask,
                                                     const ImageMemoryBarrierData *transitionTo,
                                                     uint32_t newQueueFamilyIndex,
...
template <typename CommandBufferT>
void ImageHelper::insertBarrier(VkImageAspectFlags aspectMask,
                                const ImageMemoryBarrierData *transitionTo,
                                uint32_t newQueueFamilyIndex,
                                CommandBufferT *commandBuffer)
{
...
angle::Result ImageHelper::generateMipmapsWithBlit(ContextVk *contextVk, GLuint maxLevel)
{
    ...
    setPrebuiltLayout(ImageLayout::TransferSrc);
