...
ImageHelper::ImageHelper(ImageHelper &&other)
    : CommandGraphResource(CommandGraphResourceType::Image),
      ...
      mNeedsClear(std::move(other.mNeedsClear)),
...
bool ImageHelper::needsClear(uint32_t level, uint32_t layer) const
{
    size_t index = level * mLayerCount + layer;
    ASSERT(index < mNeedsClear.size());

    return mNeedsClear[index];
}
...
void ImageHelper::setNeedsClear(uint32_t level, uint32_t layer)
{
    ...
    // Needs to be cleared:
    if (!mNeedsClear[index])
    {
        ++mNeedsClearCount;
    }
    mNeedsClear[index] = true;
}
...
void ImageHelper::setCleared(uint32_t level, uint32_t layer)
{
    ...
    mNeedsClear[index] = false;
}

void ImageHelper::resizeClearInfo(bool needsClear)
{
    mNeedsClear.clear();
    mNeedsClear.resize(mLevelCount * mLayerCount, needsClear);
                                            ...
                                            CommandBuffer *commandBuffer)
{
    ...
    for (uint32_t i = 0; i < subresource.layerCount; ++i)
    {
        ...
        if (!needsClear(subresource.mipLevel, layer))
        {
            continue;
        }
                                              ...
                                              vk::CommandBuffer *commandBuffer)
{
    ...
    if (flushPendingClear)
    {
        ...
        {
            for (uint32_t level = levelStart; level < levelEnd; ++level)
            {
                if (ensureCleared(level, layer, !transitionDone, commandBuffer))
                {
                    transitionDone = true;
                }
            }
