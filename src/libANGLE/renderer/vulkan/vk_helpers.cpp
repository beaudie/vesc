                                              ...
                                              gl::TexLevelMask skipLevelsMask)
{
    ...
    if (transCoding)
    {
             ...
             ++updateMipLevelGL)
        {
            ...
            if (!levelUpdates->empty() && !(*levelUpdates)[0].useCompute)
            {
         ...
         ++updateMipLevelGL)
    {
        ...
        for (SubresourceUpdate &update : *levelUpdates)
        {
            ...
            if (transCoding && !update.useCompute && firstDmaCopy)
            {
                CommandBufferAccess dmaAccess;
                dmaAccess.onImageTransferWrite(updateMipLevelGL, (levelGLEnd - updateMipLevelGL),
                                               kMaxContentDefinedLayerCount, 0, aspectFlags, this);
                ANGLE_TRY(
                    contextVk->getOutsideRenderPassCommandBufferHelper(dmaAccess, &commandBuffer));
                firstDmaCopy = false;
            }
