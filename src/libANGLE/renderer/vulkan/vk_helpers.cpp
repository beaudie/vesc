                                                 ...
                                                 uint8_t **outDataPtr)
{
    ...
    // So just make sure if we use these that we have enough bytes per pixel
    ASSERT((mFormat->vkImageFormat != VK_FORMAT_X8_D24_UNORM_PACK32) ||
           (imageFormat.pixelBytes >= 5));
    ASSERT((mFormat->vkImageFormat != VK_FORMAT_D24_UNORM_S8_UINT) ||
           (imageFormat.pixelBytes >= 5));

    *bufferSize = sourceArea.width * sourceArea.height * sourceArea.depth * imageFormat.pixelBytes *
                  layerCount;
