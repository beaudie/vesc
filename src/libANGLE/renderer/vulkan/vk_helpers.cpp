...
angle::Result DynamicDescriptorPool::allocateNewPool(Context *context)
{
    ...
    RefCountedDescriptorPoolBinding tempBinding;
    ...
    for (size_t poolIndex = 0; poolIndex < mDescriptorPools.size(); ++poolIndex)
    {
        ...
        {
            // The pool may have the cache, calling init will destroy all caches which may cause the
            // pool gets destroyed. Temporary bind it to prevent it from getting destroyed.
            tempBinding.set(mDescriptorPools[poolIndex]);
                                                       ...
                                                       const DescriptorSetDesc &desc)
{
    ...
    if (mDescriptorSetCache.getDescriptorSet(desc, &descriptorSet, &poolOut))
    {
        ...
        checkAndEraseUnusedPool(contextVk->getRenderer(), poolOut);
    }
}
                                                       ...
                                                       const DescriptorSetDesc &desc)
{
    ...
    if (mDescriptorSetCache.getDescriptorSet(desc, &descriptorSet, &poolOut))
    {
        ...
        DescriptorSetHelper descriptorSetHelper(descriptorSet);
        poolOut->get().addGarbage(std::move(descriptorSetHelper));
        checkAndEraseUnusedPool(renderer, poolOut);
    }
}

void DynamicDescriptorPool::checkAndEraseUnusedPool(RendererVk *renderer,
                                                    RefCountedDescriptorPoolHelper *pool)
{
    ...
    // Update mCurrentPoolIndex.
    if (mCurrentPoolIndex == poolIndex)
    {
        mCurrentPoolIndex = kInvalidPoolIndex;
    }
    else if (mCurrentPoolIndex != kInvalidPoolIndex && mCurrentPoolIndex > poolIndex)
    {
        mCurrentPoolIndex--;
    }
