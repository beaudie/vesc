                                      ...
                                      void *pixels)
{
    ...
    if (copyAspectFlags == IMAGE_ASPECT_DEPTH_STENCIL)
    {
        ...
        for (int i = 0; i < area.width * area.height; i++)
        {
            ...
            memcpy(readPixel + depthOffset, depthBuffer.get() + i * depthFormat->pixelBytes,
                   depthFormat->depthBits / 8);
            memcpy(readPixel + stencilOffset, stencilBuffer.get() + i * stencilFormat->pixelBytes,
        ...
        // Compressed format path isn't applicable for us, let's make extra sure
        // we won't hit it
        ASSERT(!readFormat->isBlock);
        ANGLE_TRY(packReadPixelBuffer(contextVk, area, packPixelsParams, readFormat, readFormat,
                                      readPixelBuffer.get(), levelGL, pixels));
                                          ...
                                          void *pixels)
{
    ...
    ANGLE_TRY(packReadPixelBuffer(contextVk, area, packPixelsParams, &getActualFormat(), readFormat,
