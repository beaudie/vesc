                              ...
                              VkSemaphore *acquireNextImageSemaphoreOut)
{
    ...
    *acquireNextImageSemaphoreOut = mAcquireNextImageSemaphore.release();
    ...
    if (barrierType == BarrierType::Event && mCurrentEvent.valid())
    {
        ...
        // future.
        events.emplace_back(mCurrentEvent.getEvent().getHandle());
        VkPipelineStageFlags srcStageMask = GetImageLayoutSrcStageMask(context, mCurrentEvent);
                                         ...
                                         RefCountedEventGarbageObjects *garbageObjects)
{
    ...
    if (mCurrentLayout == ImageLayout::Present)
    {
        // This layout has different srcStageMask and dstStageMask which will trigger
        // VUID-vkCmdWaitEvents-srcStageMask-parameter. Force pipelineBarrier for now to avoid the
        // VVL error.
        barrierType = BarrierType::Pipeline;
    }
    ...
    else
    {
            ...
            mBarrierQueueSerial == queueSerial)
        {
            ...
            if (isNewReadStage)
            {
                if (barrierType == BarrierType::Event && mLastNonShaderReadOnlyEvent.valid())
                {
                    const ImageMemoryBarrierData &layoutData =
