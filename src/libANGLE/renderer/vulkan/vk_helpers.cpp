    ...
    *updateAppliedImmediatelyOut = false;
    ...
    else
    {
        ...
        else if (!stencilOnly)
        {
            ...
            //            {
            //                WARN() << "RGB: Intended format: " << (uint32_t)
            //                intendedStorageFormat.id << " | Actual format: " << (uint32_t)
            //                storageFormat.id; loadFunctionInfo.loadFunction =
            //                angle::LoadToNative<GLubyte, 3>;
            //            }
    ...
    //        VK_BUFFER_USAGE_TRANSFER_DST_BIT; bufferInfo.sharingMode           =
    //        VK_SHARING_MODE_EXCLUSIVE; bufferInfo.queueFamilyIndexCount = 0;
    //        bufferInfo.pQueueFamilyIndices   = nullptr;
    //
    //        ANGLE_TRY(srcBuffer.get().init(contextVk, bufferInfo,
    //        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)); uint8_t *dataPtr;
    ...
    //                                      glExtents.height, glExtents.depth, source,
    //                                      inputRowPitch, inputDepthPitch, dataPtr,
    //                                      outputRowPitch2, outputDepthPitch2);
    //
    ...
    //        params.srcStride   = 1;  // TODO: Should be 3 for correct data, but we get
    //        device-lost. params.srcOffset   = 0; params.dstOffset   = stagingOffset;
