...
{
    bool imageUsedInRecordedCommands = isUsedInRecordedCommands();
    // If we do not have any ImageHelper::mUse retained in ResourceUseList, make a cloned copy of
    // ImageHelper::mUse and use it to free any garbage in imageView immediately.
    if (!imageUsedInRecordedCommands)
    {
        std::vector<vk::GarbageObject> viewGarbage;
        imageView->release(renderer, viewGarbage);
        if (!viewGarbage.empty())
        {
            // clone ImageHelper::mUse
            rx::vk::SharedResourceUse clonedmUse;
            clonedmUse.init();
            clonedmUse.clone(mUse);
            renderer->collectGarbage(std::move(clonedmUse), std::move(viewGarbage));
        }
    }
    else
    {
        // Otherwise, stash the garbage in imageView to the mImageAndViewGarbage and send for
        // garbage collection later
        imageView->release(renderer, mImageAndViewGarbage);
        // Notify us if the application pattern causes the views to keep getting reallocated and
...
bool ImageHelper::isUsedInRecordedCommands() const
{
