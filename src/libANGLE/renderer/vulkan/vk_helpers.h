...
class DescriptorPoolHelper final : angle::NonCopyable
{
  public:
    ...
    // Returns true if all descriptorSets have been freed by caller, thus the pool can be released
    // safely if we want.
    bool empty() const { return mDescriptorSetGarbageList.size() == mAllocatedDescriptorSets; }

  private:
    // Track the number of descriptorSets allocated out of this pool.
    uint32_t mAllocatedDescriptorSets;
    // Track the number of remaining descriptorSets in the pool.
...
class DynamicDescriptorPool final : angle::NonCopyable
{
  public:
    ...
    // Erase the given pool from the list.
    void erasePool(const DescriptorPoolHelper *pool);
  ...
  private:
    ...
    std::vector<std::unique_ptr<DescriptorPoolHelper>> mDescriptorPools;
...
class MetaDescriptorPool final : angle::NonCopyable
{
  public:
    ...
    void accumulateDescriptorCacheStats(VulkanCacheType cacheType, Accumulator *accum) const
    {
        ...
        {
            const vk::RefCountedDynamicDescriptorPool &pool = iter.second;
    ...
    void resetDescriptorCacheStats()
    {
        ...
        {
            vk::RefCountedDynamicDescriptorPool &pool = iter.second;
