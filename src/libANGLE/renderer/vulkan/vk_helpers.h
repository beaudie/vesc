...
class PipelineBarrier : angle::NonCopyable
{
  public:
                         ...
                         const VkImageMemoryBarrier &imageMemoryBarrier)
    {
        ...
        ASSERT(imageMemoryBarrier.pNext == nullptr);
        mWaitEvents.emplace_back(srcStageMask, dstStageMask, event, imageMemoryBarrier);
    }
  ...
  private:
    ...
    std::vector<ImageEventBarrier> mWaitEvents;
};
...
class CommandBufferHelperCommon : angle::NonCopyable
{
  public:
    ...
    void collectRefCountedEvent(RefCountedEvent *event)
    {
        if (event->valid())
        {
            mRefCountedEventGarbage.emplace_back(GetGarbage(event));
        }
    }
...
class ImageHelper final : public Resource, public angle::Subject
{
  public:
    ...
    RefCountedEvent *getCurrentEvent() { return &mCurrentEvent; }
