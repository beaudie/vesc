...
class ImageHelper final : public Resource, public angle::Subject
{
  public:
    ...
    const VkExtent3D &getRotatedExtents() const { return mRotatedExtents; }
  ...
  private:
    ...
    // to Android rotation and pre-rotation" in "SurfaceVk.cpp").  Thus, mExtents are the
    // non-rotated.  The rotated extents are also stored along with a bool that indicates if the
    // aspect ration is different between the rotated and non-rotated extents.
    VkExtent3D mExtents;
    VkExtent3D mRotatedExtents;
