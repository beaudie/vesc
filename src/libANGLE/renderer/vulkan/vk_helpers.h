...
class ImageViewHelper : angle::NonCopyable
{
  ...
  private:
    ...
    void ensureIssueSerial(RendererVk *renderer);
    ...
    // Serials are conceptually a triple of { ImageViewSerial, level, layer }.
    // We pack the level/layer in 32 bits and then combine with the Serial.
