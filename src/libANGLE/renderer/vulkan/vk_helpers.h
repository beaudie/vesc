...
class DynamicallyGrowingPool : angle::NonCopyable
{
  ...
  protected:
    ...
    using DestroyPoolFunc = std::function<void(Pool &)>;
    void destroyEntryPool(DestroyPoolFunc destroyPool);
    ...
    const Pool &getPool(size_t index) const { return mPools[index].pool; }
    Pool &getPool(size_t index) { return mPools[index].pool; }

    uint32_t getPoolSize() const { return mPoolSize; }

    using AllocatePoolFunc = std::function<angle::Result(ContextVk *, Pool &, uint32_t)>;
