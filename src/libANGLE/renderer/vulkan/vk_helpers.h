...
class CommandBufferHelper : angle::NonCopyable
{
  ...
  private:
    ...
    // not include VK_ATTACHMENT_LOAD_OP_CLEAR which is tracked separately. This is done this way to
    // allow clear op to be optimized out when we find out that the depth buffer is not been used
    // during the entire renderpass and store op is VK_ATTACHMENT_STORE_OP_DONTCARE.
    ResourceAccess mDepthAccess;
    // State tracking for the maxmium stencil access during the entire renderpass. Note that this
    // does not include VK_ATTACHMENT_LOAD_OP_CLEAR which is tracked separately. This is done this
    // way to allow clear op to be optimized out when we find out that the stencil buffer is not
    // been used during the entire renderpass and store op is VK_ATTACHMENT_STORE_OP_DONTCARE.
