...
class ImageHelper final : public Resource, public angle::Subject
{
  ...
  private:
    ...
    // If the number of copy-to-image commands or their size in the outside command buffer reaches
    // the threshold below, it is flushed.
    static constexpr uint32_t kMaxCopyCommandCount = 1 << 31;
    static constexpr uint32_t kMaxCopySize         = 1 << 19;
