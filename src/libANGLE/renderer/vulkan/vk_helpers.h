...
class BufferMemory : angle::NonCopyable
{
  ...
  private:
    ...
    Allocation mAllocation;        // used Allocation if isExternalBuffer() is false
    DeviceMemory mExternalMemory;  // used DeviceMemory if isExternalBuffer() is true
...
class BufferHelper final : public Resource
{
  public:
    ...
    bool isMapped() const { return mMemory.mMappedMemory != nullptr; }
