...
class DynamicallyGrowingPool
{
  ...
  protected:
    ...
    // returns true.
    bool findFreePool(Context *context);

    // Allocates a new entry and initializes it with the given pool.
    angle::Result allocateNewPool(Context *context, Pool &&pool);

    // Called by the implementation whenever an entry is freed.
    void freedEntry(Context *context, size_t poolIndex);
...
constexpr uint32_t kDefaultTimestampQueryPoolSize = 1;

class QueryHelper;

class DynamicQueryPool final : public DynamicallyGrowingPool<QueryPool>, angle::NonCopyable
{
  public:
    DynamicQueryPool();
    ~DynamicQueryPool();
                                   ...
                                   angle::NonCopyable
{
  public:
    ...
    angle::Result allocateSemaphore(Context *context,
                                    bool autoFree,
                                    const Semaphore **semaphoreOut);
    void freeSemaphore(Context *context, const Semaphore *semaphore);
