...
class QueryHelper final : public CommandGraphResource
{
  public:
    QueryHelper();
    QueryHelper(const QueryPool *queryPool, uint32_t query);
    QueryHelper(QueryHelper &&other);
    ~QueryHelper();

    QueryHelper &operator=(QueryHelper &&other);

    const QueryPool *getQueryPool() const { return mQueryPool; }
...
class DynamicQueryPool final : angle::NonCopyable
{
  ...
  private:
    ...
    struct QueryPoolStats
    {
        ...
        // new allocations are made from the pool until the next frame.
        Serial serial;
    };
