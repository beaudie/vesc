...
class ImageHelper final : public Resource, public angle::Subject
{
  public:
    ...
    gl::LevelIndex getFirstAllocateLevel() const { return mFirstAllocateLevel; }
  ...
  private:
    ...
    // The first level been allocated. For mutable textures, this should be same as mBaseLevel since
    // we always reallocate VkImage based on mBaseLevel change. But for immutable textures, we
    // always allocate from level 0 regardless of mBaseLevel change.
    gl::LevelIndex mFirstAllocateLevel;
