...
class ImageHelper final : public CommandGraphResource
{
  public:
    ...
    // layers may also be updated. If asked to flush pending clears, a subresource that doesn't have
    // any staged updates will be cleared regardless.
    ...
    bool needsClear(uint32_t level, uint32_t layer) const;
    bool needsClearAnySubresource() const { return mNeedsClearCount > 0; }
    // Marks a subresource as requiring to be cleared.
  ...
  private:
    ...
    // Every bit indicates whether each layer/level needs clearing prior to use.
    //
    // Note that vector<bool> is specialized to take 1 bit per entry.
    std::vector<bool> mNeedsClear;
    // A cached value showing how many bits are set in `mNeedsClear`.  This is used to quickly
    // understand whether the image needs clearing at all.
