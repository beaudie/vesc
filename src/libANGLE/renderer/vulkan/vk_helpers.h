...
{
    // The attachment has been invalidated and is currently still invalidated.
    Invalidated,
    // The attachment was previously invalidated, but has since been used for while enabled for
    // drawing, meaning that it has valid contents defined (and therefore this render pass should
    // STORE it, and a future render pass should LOAD it).
    NoLongerInvalidated,
...
class CommandBufferHelper : angle::NonCopyable
{
  public:
    ...
    {
        ASSERT(mIsRenderPassCommandBuffer);
        // Return true if the attachment is currently invalidated and storeOp of this render pass
        // should be DONT_CARE.
        return (mDepthInvalidatedState == Invalidated);
    }

    bool shouldDiscardStencilAttachment()
    {
        ASSERT(mIsRenderPassCommandBuffer);
        // Return true if the attachment is currently invalidated and stencilStoreOp of this render
        // pass should be DONT_CARE.
        return (mStencilInvalidatedState == Invalidated);
    }
    ...
    bool shouldRestoreDepthStencilAttachment()
    {
        ...
        // to true (i.e. use LOAD).  Return true for the following conditions:
        //
