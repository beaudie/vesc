...
void RendererVk::ensureCapsInitialized() const
{
    ...
    // ANGLE internal variables.  ANGLE implements UBOs as dynamic uniform buffers, so the
    // following is the minimum of:
    //
    // - maxDescriptorSetUniformBuffers - stageCount - 1
    // - maxDescriptorSetUniformBuffersDynamic - 1
    uint32_t maxUniformBuffers =
        std::min(mPhysicalDeviceProperties.limits.maxDescriptorSetUniformBuffers - 3,
                 mPhysicalDeviceProperties.limits.maxDescriptorSetUniformBuffersDynamic - 1);
    // There should be no overflow
    ASSERT(maxUniformBuffers < mPhysicalDeviceProperties.limits.maxDescriptorSetUniformBuffers - 3);
