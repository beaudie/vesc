...
constexpr char kEnableExtShaderOverrideVarName[]      = "ANGLE_ENABLE_EXT_SHADER_OVERRIDE";
constexpr char kEnableExtShaderOverridePropertyName[] = "debug.angle.enable.ext_shader_override";
}  // anonymous namespace
...
void RendererVk::ensureCapsInitialized() const
{
    ...
    std::string enableOverride = angle::GetEnvironmentVarOrAndroidProperty(
        kEnableExtShaderOverrideVarName, kEnableExtShaderOverridePropertyName);
    if ((!enableOverride.empty() && enableOverride.compare("0") != 0) ||
        (!(IsARM(mPhysicalDeviceProperties.vendorID) ||
