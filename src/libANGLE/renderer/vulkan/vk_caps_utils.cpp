...
bool CanSupportYuvInternalFormat(const RendererVk *rendererVk)
{
    ...
    const Format &twoPlaneYuvFormat = rendererVk->getFormat(GL_G8_B8R8_2PLANE_420_UNORM_ANGLE);
    bool twoPlaneYuvFormatSupported = rendererVk->hasImageFormatFeatureBits(
        twoPlaneYuvFormat.actualImageFormatID, VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT);

    const Format &threePlaneYuvFormat = rendererVk->getFormat(GL_G8_B8_R8_3PLANE_420_UNORM_ANGLE);
    bool threePlaneYuvFormatSupported = rendererVk->hasImageFormatFeatureBits(
        threePlaneYuvFormat.actualImageFormatID, VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT);

    return twoPlaneYuvFormatSupported && threePlaneYuvFormatSupported;
}
}  // namespace
}  // namespace vk
