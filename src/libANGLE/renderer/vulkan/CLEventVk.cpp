...
CLEventVk::CLEventVk(const cl::Event &event)
    : CLEventImpl(event), mStatus(isUserEvent() ? CL_SUBMITTED : CL_QUEUED), mProfilingTimestamps(0)
{
...
angle::Result CLEventVk::setTimestamp(cl_int status)
{
    ...
    {
        // TODO: Just get current CPU timestamp for now, look into Vulkan GPU device timestamp query
        // instead and later make CPU timestamp a fallback if GPU timestamp cannot be queried
        cl_ulong cpuTS =
