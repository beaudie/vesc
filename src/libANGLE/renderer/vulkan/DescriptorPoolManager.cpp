...
vk::Error DescriptorPoolManager::init(ContextVk *contextVk)
{
...
vk::Error DescriptorPoolManager::allocateDescriptorSets(
    ...
    VkDescriptorSet* descriptorSetsOut)
{
    ...
    ANGLE_TRY(mCurrentDescriptorSetPool.allocateDescriptorSets(mContextVk->getDevice(), allocInfo, descriptorSetsOut));
    mDescriptorSetsCountInPool += allocInfo.descriptorSetCount;
    return vk::NoError();
}

VkDescriptorPool DescriptorPoolManager::getCurrentPoolHandle()
{
    return mCurrentDescriptorSetPool.getHandle();
}

vk::Error DescriptorPoolManager::allocateNewPool()
{
    // TODO(jmadill): Pick non-arbitrary max.
    VkDescriptorPoolSize poolSizes[2];
    poolSizes[UniformBufferPool].type            = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolSizes[UniformBufferPool].descriptorCount = 2 * kMaxSets / 2;
    poolSizes[TexturePool].type                  = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[TexturePool].descriptorCount       = 64 * kMaxSets / 2;
