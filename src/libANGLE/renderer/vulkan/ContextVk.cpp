...
ContextVk::ContextVk(const gl::State &state, gl::ErrorSet *errorSet, RendererVk *renderer)
    : ContextImpl(state, errorSet),
      ...
      mCommandContent(vk::ConvertProtectedBitToCommandContent(state.hasProtectedContent())),
...
angle::Result ContextVk::submitCommands(const vk::Semaphore *signalSemaphore, Submit submission)
{
    ...
    ANGLE_TRY(mRenderer->submitCommands(this, getCommandContent(), mContextPriority,
...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        ANGLE_TRY(mRenderer->getCommandBufferOneOff(this, getCommandContent(), &commandBuffer));
        ...
        // vkGetEventStatus
        ANGLE_TRY(mRenderer->queueSubmitOneOff(this, std::move(commandBuffer), getCommandContent(),
...
angle::Result ContextVk::flushCommandsAndEndRenderPassWithoutSubmit(RenderPassClosureReason reason)
{
    ...
    ANGLE_TRY(mRenderer->flushRenderPassCommands(this, getCommandContent(), *renderPass,
                                                 &mRenderPassCommands));
...
angle::Result ContextVk::flushOutsideRenderPassCommands()
{
    if (!mWaitSemaphores.empty())
    {
        ...
        ANGLE_TRY(mRenderer->flushWaitSemaphores(getCommandContent(), std::move(mWaitSemaphores),
    ...
    ANGLE_TRY(
        mRenderer->flushOutsideRPCommands(this, getCommandContent(), &mOutsideRenderPassCommands));
