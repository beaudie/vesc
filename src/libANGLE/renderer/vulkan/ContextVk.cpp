...
DescriptorSetUpdates::DescriptorSetUpdates(ContextVk *contextVk) : mContextVk(*contextVk)
{
...
VkDescriptorImageInfo &DescriptorSetUpdates::allocImageInfo()
{
    return allocInfos<VkDescriptorImageInfo, &VkWriteDescriptorSet::pImageInfo>(
        mContextVk.mImageInfos, 1);
}

VkWriteDescriptorSet &DescriptorSetUpdates::allocWriteInfo()
{
    mContextVk.mWriteInfos.emplace_back();
    return mContextVk.mWriteInfos.back();
}
...
void DescriptorSetUpdates::growCapacity(std::vector<T> &mInfos, size_t newSize)
{
    ...
    if (oldInfoStart)
    {
        ...
        for (VkWriteDescriptorSet &set : mContextVk.mWriteInfos)
        {
            if (set.*pInfo)
            {
                                   ...
                                   vk::CommandBuffer **commandBufferOut)
{
    ...
    *commandBufferOut = mRenderPassCommandBuffer;
    ...
    // function
    DescriptorSetUpdates descriptorSetUpdates(this);
