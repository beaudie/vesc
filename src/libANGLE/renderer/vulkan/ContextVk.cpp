...
void ContextVk::logEvent(const char *apiString)
{
    mEventLog.push_back(apiString);
}

void ContextVk::writeEventLog(const gl::Context *context, vk::CommandBuffer *commandBuffer)
{
    const std::vector<std::string> *eventLog = &mEventLog;
    if (!eventLog->empty())
    {
        ...
        commandBuffer->beginDebugUtilsLabelEXT(label);
        for (uint32_t i = 0; i < eventLog->size(); ++i)
        {
            label.pLabelName = (*eventLog)[i].c_str();
