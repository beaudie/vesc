...
ContextVk::ContextVk(const gl::State &state, gl::ErrorSet *errorSet, RendererVk *renderer)
    : ContextImpl(state, errorSet),
      ...
      mQueryEventType(QueryEventCmdBuf::NotInQueryCommand),
...
{
    ASSERT(mQueryEventType == QueryEventCmdBuf::InOuterCmdBufQueryCommand ||
           mQueryEventType == QueryEventCmdBuf::InRenderPassCmdBufQueryCommand);
    if (!mRenderer->angleDebuggerMode())
    {
        return;
    }
    ...
    {
        case QueryEventCmdBuf::InOuterCmdBufQueryCommand:
            ASSERT(mRenderPassCommands);
            commandBuffer = &mRenderPassCommands->getCommandBuffer();
            break;
        case QueryEventCmdBuf::InRenderPassCmdBufQueryCommand:
            ASSERT(mOutsideRenderPassCommands);
...
{
    ASSERT(mQueryEventType == QueryEventCmdBuf::NotInQueryCommand);
    if (!mRenderer->angleDebuggerMode())
    {
        return angle::Result::Continue;
    }
