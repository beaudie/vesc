...
angle::Result ContextVk::updateActiveTextures(const gl::Context *context)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        else if (shouldSwitchToDepthReadOnlyMode(context, texture))
        {
            ...
            // mode in the The tracking tells us when we can retroactively go back and change the
            // RenderPass to read-only. If there are any writes we need to break and finish the
            // current RP before starting the read-only one.
