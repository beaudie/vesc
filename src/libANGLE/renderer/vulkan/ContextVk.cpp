                              ...
                              bool appendOffset)
{
    for (size_t bufferIndex : mask)
    {
        ...
        ASSERT(binding.getSize() < static_cast<GLsizeiptr>(std::numeric_limits<uint32_t>::max()));
        desc->append32BitValue(static_cast<uint32_t>(binding.getSize()));
        if (appendOffset)
        {
            ASSERT(binding.getOffset() <
                   static_cast<GLintptr>(std::numeric_limits<uint32_t>::max()));
...
void ContextVk::updateOverlayOnPresent()
{
...

    {
        gl::RunningGraphWidget *shaderBufferHitRate =
            overlay->getRunningGraphWidget(gl::WidgetId::VulkanShaderBufferHitRate);
...
angle::Result ContextVk::invalidateCurrentShaderResources()
{
    ...
    if (hasUniformBuffers || hasStorageBuffers)
    {
        ...
        AppendBufferVectorToDesc(&mShaderBuffersDescriptorDesc, uniformBuffers,
                                 ...
                                 !mExecutable->usesDynamicUniformBufferDescriptors());
