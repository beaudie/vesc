...
void ContextVk::optimizeRenderPassForPresent(VkFramebuffer framebufferHandle)
{
    ...
    if (depthStencilRenderTarget)
    {
        ...
        mRenderPassCommands->invalidateRenderPassStencilAttachment(
            this, mRenderPassCommands->getRenderArea());
...
bool ContextVk::isAnyLoadOpClear(const FramebufferVk *framebufferVk)
{
    ...
    if (mRenderPassCommands->getRenderPassDesc().hasDepthStencilAttachment())
    {
        ...
        if (attachmentOpsArray[colorIndexVk].stencilLoadOp == VK_ATTACHMENT_LOAD_OP_CLEAR)
        {
            return true;
        }
    }
...
angle::Result ContextVk::updateScissor(const gl::State &glState)
{
        ...
        !mRenderPassCommands->getRenderArea().encloses(rotatedScissoredArea))
    {
        ...
        if (isAnyLoadOpClear(framebufferVk))
        {
