                                   ...
                                   DirtyBits dirtyBitMask)
{
    ...
    // If VK_EXT_vertex_input_dynamic_state is enabled then vkCmdSetVertexInputEXT must be called in
    // the current command buffer prior to the draw command, even if there are no active vertex
    // attributes.
    if (getFeatures().supportsVertexInputDynamicState.enabled)
    {
        dirtyBitMask.set(DIRTY_BIT_VERTEX_BUFFERS);
    }
                                                          ...
                                                          DirtyBits dirtyBitMask)
{
        ...
        getFeatures().supportsVertexInputDynamicState.enabled)
    {
        ...
        std::vector<VkVertexInputAttributeDescription2EXT> attributeDescs;
        ...
        for (size_t attribIndex : activeAttribLocations)
        {
            ...
            if (getFeatures().supportsVertexInputDynamicState.enabled)
            {
                ...
                attribDesc.format  = mGraphicsPipelineDesc->GetPipelineVertexInputStateFormat(
