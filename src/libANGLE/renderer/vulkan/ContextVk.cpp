                                              ...
                                              vk::CommandGraphResource *recorder)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        bool isColorAttachmentAndMasked        = false;
        bool isDepthStencilAttachmentAndMasked = false;
        if (mCurrentDrawMode != gl::PrimitiveMode::InvalidEnum)
        {
            ASSERT(mDrawFramebuffer);
            isColorAttachmentAndMasked =
                isImageColorAttachment(image, mDrawFramebuffer) &&
                !isFormatColorWritable(image.getFormat().actualImageFormat(),
                                       getState().getBlendState());
            isDepthStencilAttachmentAndMasked =
        ...
        if (!isDepthStencilAttachmentAndMasked && !isDepthStencilAttachmentAndMasked)
        {
