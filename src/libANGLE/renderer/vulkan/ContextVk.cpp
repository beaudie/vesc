...
void CommandQueue::handleDeviceLost(VkDevice device)
{
    for (CommandBatch &batch : mInFlightCommands)
    {
...
angle::Result ContextVk::initialize()
{
    ...
    {
        // Once the transientCommandBuffer issue is resolved, the commandPool will only
                                     ...
                                     vk::PrimaryCommandBuffer &&commandBuffer)
{
    ...
    ANGLE_TRY(getNextSubmitFence(&sharedFence));

    ANGLE_TRY(mCommandQueue.submitFrame(this, submitInfo, sharedFence, &mCurrentGarbage,
                                        &mCommandPool, std::move(commandBuffer)));

    // we need to explicitly notify every other Contexts using this VkQueue that they their
    // current command buffer is no longer valid.
...
void ContextVk::handleDeviceLost()
{
    ...
    mCommandQueue.handleDeviceLost(device);
    clearAllGarbage();
...
angle::Result ContextVk::getTimestamp(uint64_t *timestampOut)
{
    ...
    Serial serial;
    ANGLE_TRY(mRenderer->queueSubmit(this, submitInfo, fence.get(), &serial));
