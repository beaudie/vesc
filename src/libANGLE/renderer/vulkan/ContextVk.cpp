...
angle::Result ContextVk::updateActiveTextures(const gl::Context *context, gl::Command command)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        if (!isIncompleteTexture && texture->isDepthOrStencil() && command != gl::Command::Dispatch)
        {
            const bool isStencilTexture = IsStencilSamplerBinding(*executable, textureUnit);
            ANGLE_TRY(updateReadOnlyDepthStencilFeedbackLoopMode(texture, getDrawFramebuffer(),
                                                                 isStencilTexture));
        }
                                                                    ...
                                                                    bool isStencilTexture)
{
    ...
    vk::RenderPassUsageFlags oldUsageFlags = mDepthStencilRenderPassUsageFlags;
    if (isStencilTexture)
    {
        ...
        else if (!mDepthStencilRenderPassUsageFlags[vk::RenderPassUsage::StencilFeedbackLoop])
        {
    ...
    else if (!mDepthStencilRenderPassUsageFlags[vk::RenderPassUsage::DepthFeedbackLoop])
    {
    ...
    if (oldUsageFlags == mDepthStencilRenderPassUsageFlags)
    {
        return angle::Result::Continue;
    }
