...
void UpdateRenderPassDepthStencilState(const gl::State &glState,
                                       vk::CommandBufferHelper *renderPassCommands,
                                       FramebufferVk *drawFramebuffer,
                                       bool updateDepth)
{
    if (renderPassCommands->started())
    {
        if (updateDepth)
        {
            renderPassCommands->onDepthStateChanged(glState.getDepthStencilState());
        }
        else
        {
    ...
    for (auto iter = dirtyBits.begin(), endIter = dirtyBits.end(); iter != endIter; ++iter)
    {
        ...
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_DEPTH_TEST_ENABLED:
            {
                ...
                UpdateRenderPassDepthStencilState(mState, mRenderPassCommands, mDrawFramebuffer,
                                                  true);
                break;
            }
                                         ...
                                         vk::CommandBuffer **commandBufferOut)
{
    ...
    mRenderPassCommands->onDepthStateChanged(dsState);
    mRenderPassCommands->onStencilStateChanged(dsState);
