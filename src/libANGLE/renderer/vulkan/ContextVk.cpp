      ...
      mShareGroupVk(vk::GetImpl(state.getShareGroup()))
{
    ...
    mNewGraphicsCommandBufferDirtyBits = DirtyBits{
        DIRTY_BIT_RENDER_PASS,     DIRTY_BIT_PIPELINE_BIND,          DIRTY_BIT_TEXTURES,
    ...
    mNewComputeCommandBufferDirtyBits =
        DirtyBits{DIRTY_BIT_PIPELINE_BIND, DIRTY_BIT_TEXTURES, DIRTY_BIT_SHADER_RESOURCES,
                  DIRTY_BIT_DESCRIPTOR_SETS, DIRTY_BIT_DRIVER_UNIFORMS_BINDING};
    ...
    mGraphicsDirtyBitHandlers[DIRTY_BIT_PIPELINE_BIND] =
        &ContextVk::handleDirtyGraphicsPipelineBind;
    ...
    mComputeDirtyBitHandlers[DIRTY_BIT_PIPELINE_BIND] = &ContextVk::handleDirtyComputePipelineBind;
...
angle::Result ContextVk::handleDirtyGraphicsEventLog()
{
...
angle::Result ContextVk::handleDirtyGraphicsDefaultAttribs()
{
...
angle::Result ContextVk::handleDirtyGraphicsPipeline()
{
    ...
    // command buffer or UtilsVk, it will happen anyway with DIRTY_BIT_PIPELINE_BIND.
...
angle::Result ContextVk::handleDirtyGraphicsPipelineBind()
{
...
angle::Result ContextVk::handleDirtyComputePipeline()
{
    ...
    ASSERT(mComputeDirtyBits.test(DIRTY_BIT_PIPELINE_BIND));

    return angle::Result::Continue;
}

angle::Result ContextVk::handleDirtyComputePipelineBind()
{
...
angle::Result ContextVk::handleDirtyGraphicsTextures()
{
...
angle::Result ContextVk::handleDirtyGraphicsVertexBuffers()
{
...
angle::Result ContextVk::handleDirtyGraphicsIndexBuffer()
{
...
angle::Result ContextVk::handleDirtyGraphicsShaderResources()
{
...
angle::Result ContextVk::handleDirtyGraphicsDescriptorSets()
{
    ...
    for (auto iter = dirtyBits.begin(), endIter = dirtyBits.end(); iter != endIter; ++iter)
    {
        ...
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_DRAW_FRAMEBUFFER_BINDING:
            {
                ...
                updateRenderPassDesc(mDrawFramebuffer);
                break;
            }
...
angle::Result ContextVk::invalidateCurrentTextures(const gl::Context *context)
{
    ...
    if (executable->hasTextures())
    {
        constexpr DirtyBits kDirtyBits{DIRTY_BIT_TEXTURES, DIRTY_BIT_DESCRIPTOR_SETS};
        mGraphicsDirtyBits |= kDirtyBits;
        mComputeDirtyBits |= kDirtyBits;
...
void ContextVk::invalidateCurrentShaderResources()
{
    ...
    if (executable->hasUniformBuffers() || executable->hasStorageBuffers() ||
        executable->hasAtomicCounterBuffers() || executable->hasImages())
    {
        constexpr DirtyBits kDirtyBits{DIRTY_BIT_SHADER_RESOURCES, DIRTY_BIT_DESCRIPTOR_SETS};
        mGraphicsDirtyBits |= kDirtyBits;
        mComputeDirtyBits |= kDirtyBits;
    }
}
...
{
    mGraphicsDirtyBits |= DirtyBits{DIRTY_BIT_DRIVER_UNIFORMS, DIRTY_BIT_DRIVER_UNIFORMS_BINDING};
}

void ContextVk::invalidateDriverUniforms()
{
    constexpr DirtyBits kDirtyBits{DIRTY_BIT_DRIVER_UNIFORMS, DIRTY_BIT_DRIVER_UNIFORMS_BINDING};
    mGraphicsDirtyBits |= kDirtyBits;
    mComputeDirtyBits |= kDirtyBits;
}
...
void ContextVk::onFramebufferChange(FramebufferVk *framebufferVk)
{
    ...
    updateRenderPassDesc(framebufferVk);
    invalidateCurrentGraphicsPipeline();
}

void ContextVk::updateRenderPassDesc(FramebufferVk *framebufferVk)
{
...
{
    updateRenderPassDesc(framebufferVk);
    // Recreate the graphics pipeline.  This function is called from
    // FramebufferVk::startNewRenderPass when the framebuffer's desc changes due to
    // multisampled-render-to-texture.  This happens after setupDraw() has handled dirty pipelines.
    return handleDirtyGraphicsPipeline();
}
...
void ContextVk::invalidateCurrentTransformFeedbackBuffers()
{
    ...
    {
        mGraphicsDirtyBits |=
            DirtyBits{DIRTY_BIT_TRANSFORM_FEEDBACK_BUFFERS, DIRTY_BIT_DESCRIPTOR_SETS};
    }
}
...
angle::Result ContextVk::handleDirtyGraphicsDriverUniforms()
{
