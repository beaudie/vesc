...
angle::Result ContextVk::initialize()
{
    ...
    ANGLE_TRY(mSubmitSemaphorePool.init(this, vk::kDefaultSemaphorePoolSize));
...
void ContextVk::getSubmitWaitSemaphores(
    ...
    angle::FixedVector<VkPipelineStageFlags, kMaxWaitSemaphores> *waitStageMasks)
{
    if (mSubmitLastSignaledSemaphore.getSemaphore())
    {
        ...
        mSubmitSemaphorePool.freeSemaphore(this, &mSubmitLastSignaledSemaphore);
    }
    ...
    for (vk::SemaphoreHelper &semaphore : mSubmitWaitSemaphores)
    {
        ...
        mSubmitSemaphorePool.freeSemaphore(this, &semaphore);
    }
                                     ...
                                     vk::CommandBuffer &&commandBuffer)
{
    ...
    ANGLE_VK_TRY(this, batch.fence.init(device, fenceInfo));

    ANGLE_TRY(getRenderer()->queueSubmit(this, submitInfo, batch.fence));
...
void ContextVk::freeAllInFlightResources()
{
    ...
    for (CommandBatch &batch : mInFlightCommands)
    {
        ...
        {
            VkResult status = batch.fence.wait(device, kMaxFenceWaitTimeNs);
...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        vk::Scoped<vk::CommandBuffer> commandBatch(device);
        ...
        angle::FixedVector<VkSemaphore, kMaxWaitSemaphores> waitSemaphores;
        angle::FixedVector<VkPipelineStageFlags, kMaxWaitSemaphores> waitStageMasks;
        getSubmitWaitSemaphores(&waitSemaphores, &waitStageMasks);
...
angle::Result ContextVk::flush()
{
    ...
    // will be waited on.
    ANGLE_TRY(mSubmitSemaphorePool.allocateSemaphore(this, &mSubmitLastSignaledSemaphore));
...
angle::Result ContextVk::allocateSubmitWaitSemaphore(const vk::Semaphore **outSemaphore)
{
    ...
    ANGLE_TRY(mSubmitSemaphorePool.allocateSemaphore(this, &semaphore));
...
const vk::Semaphore *ContextVk::getSubmitLastSignaledSemaphore()
{
