...
angle::Result ContextVk::initialize()
{
    ...
    if (mRenderer->getQueueFamilyProperties(isProtectedMemory()).timestampValidBits > 0)
    {
    ...
    if (mGpuEventsEnabled)
    {
        ...
        ASSERT(mRenderer->getQueueFamilyProperties(isProtectedMemory()).timestampValidBits > 0);
...
vk::DynamicQueryPool *ContextVk::getQueryPool(gl::QueryType queryType)
{
    ...
    ASSERT(queryType != gl::QueryType::Timestamp && queryType != gl::QueryType::TimeElapsed ||
           mRenderer->getQueueFamilyProperties(isProtectedMemory()).timestampValidBits > 0);
