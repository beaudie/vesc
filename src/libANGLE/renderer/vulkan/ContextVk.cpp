                                   ...
                                   const gl::State::DirtyBits &bitMask)
{
    ...
    // With storage buffers, storage images and atomic counters, read-after-write and
    // write-after-write hazards require a call to |glMemoryBarrier|.  Write-after-read hazards
    // however need an execution barrier that should be implicitly generated by the implementation.
    // This execution barrier is automatically issued when transitioning images or issuing buffer
    // barriers, except the render pass is not broken / outside render pass commmand buffer is not
...
{
    // Noop memory barrier if there were never any outputs from any shader stage.
    if (srcStageMask == 0)
    {
        return angle::Result::Continue;
    }
...
angle::Result ContextVk::endRenderPassIfResourceWriteAfterRead()
{
    // Similar to flushCommandBuffersIfNecessary(), but using buffers and images currently bound and
    // used by the current program.  This is to handle write-after-read hazards only, as
    // *-after-write requires |glMemoryBarrier|, and only closes the render pass.  Write-after-read
    // in the outside render pass command buffer can heppen with dispatch calls, but they always
    // flush the outside render pass command buffer currently.
    ...
    {
        const gl::Texture *texture = glState.getImageUnit(imageUnitIndex).texture.get();
        if (texture == nullptr)
        {
            continue;
        }
    ...
    for (const gl::ShaderType shaderType : executable->getLinkedShaderStages())
    {
        ...
        for (uint32_t bufferIndex = 0; bufferIndex < blocks.size(); ++bufferIndex)
        {
            ...
            const gl::OffsetBindingPointer<gl::Buffer> &bufferBinding =
                glState.getIndexedShaderStorageBuffer(block.binding);
