...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        // ANGLE_TRY(ensureSubmitFenceInitialized());
        ANGLE_TRY(ensureSubmitFenceAvailable());
...
angle::Result ContextVk::ensureSubmitFenceAvailable()
{
    if (mRenderer->hasInFlightCommandBuffers())
    {
        return mRenderer->newSharedFence(this, &mSubmitFence);
    }
    return ensureSubmitFenceInitialized();
}
