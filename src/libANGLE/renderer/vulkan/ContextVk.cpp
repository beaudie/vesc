...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        ANGLE_TRY(timestampQuery.getUint64Result(this, sizeof(uint64_t), &gpuTimestampCycles));
...
angle::Result ContextVk::checkCompletedGpuEvents()
{
    ...
    for (GpuEventQuery &eventQuery : mInFlightGpuEventQueries)
    {
        ...
        ANGLE_TRY(eventQuery.queryHelper.getUint64ResultNonBlocking(
            this, sizeof(uint64_t), &gpuTimestampCycles, &available));
        if (!available)
        {
            break;
        }
...
angle::Result ContextVk::getTimestamp(uint64_t *timestampOut)
{
    ...
    // Get the query results
    ANGLE_TRY(timestampQuery.getUint64Result(this, sizeof(uint64_t), timestampOut));
