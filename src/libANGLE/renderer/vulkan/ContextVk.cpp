                                                      ...
                                                      DirtyBits dirtyBitMask)
{
    ...
    ASSERT(mDeferredMemoryBarriers != 0);
    ...
    VkPipelineStageFlags dstStageMask = 0;  // vk::kAllShadersPipelineStageFlags;
    ...
    else if (mDeferredMemoryBarriers & GL_UNIFORM_BARRIER_BIT)
    {
        dstStageMask |= kAllShadersPipelineStageFlags;
        memoryBarrier.dstAccessMask = VK_ACCESS_UNIFORM_READ_BIT;
    }
    else if (mDeferredMemoryBarriers & GL_TEXTURE_FETCH_BARRIER_BIT)
    {
        dstStageMask |= kAllShadersPipelineStageFlags;
        memoryBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
    }
    else if (mDeferredMemoryBarriers & GL_SHADER_IMAGE_ACCESS_BARRIER_BIT)
    {
        // srcStageMask |=
    ...
    else if (mDeferredMemoryBarriers & GL_COMMAND_BARRIER_BIT)
    {
        dstStageMask |= VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT;
        memoryBarrier.dstAccessMask = VK_ACCESS_INDIRECT_COMMAND_READ_BIT;
    }
    else if (mDeferredMemoryBarriers & (GL_PIXEL_BUFFER_BARRIER_BIT | GL_BUFFER_UPDATE_BARRIER_BIT))
    {
        dstStageMask |= VK_PIPELINE_STAGE_TRANSFER_BIT;
        memoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT;
    }
    else if (mDeferredMemoryBarriers & GL_FRAMEBUFFER_BARRIER_BIT)
    {
        dstStageMask |= kAllShadersPipelineStageFlags;
        memoryBarrier.dstAccessMask = VK_ACCESS_INPUT_ATTACHMENT_READ_BIT;
    }
    else if (mDeferredMemoryBarriers & GL_TRANSFORM_FEEDBACK_BARRIER_BIT)
    {
        srcStageMask |= VK_PIPELINE_STAGE_TRANSFORM_FEEDBACK_BIT_EXT;
    }
    | | | | GL_TEXTURE_UPDATE_BARRIER_BIT | | GL_ATOMIC_COUNTER_BARRIER_BIT |
        GL_SHADER_STORAGE_BARRIER_BIT;

    mOutsideRenderPassCommands->getCommandBuffer().memoryBarrier(vk::kAllShadersPipelineStageFlags,
                                                                 stageMask, memoryBarrier);
