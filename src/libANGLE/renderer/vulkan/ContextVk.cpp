    ...
    PipelineType pipelineType)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        // TODO: Should the image here be added to the layout usage cases?
...
void ContextVk::addImageLayoutUsage(vk::ImageHelper *imageHelper)
{
    ...
    {
        if (usage.usedVkImage != imageLayoutUsage.usedVkImage)
        {
            continue;
        }
        ...
        //        // TODO: For now, it is limited to TransferDst as a test. But technically, it
        //        should be
        ...
        //            WARN() << "WAW hazard warning for image " << imageLayoutUsage.usedVkImage
        //                   << " | Current layout: " << ToUnderlying(usage.usedImageLayout)
        //                   << " | New layout: " << ToUnderlying(imageLayoutUsage.usedImageLayout)
        //                   << " | Barrier cannot be aggregated for execution";
        //        }

        // Update the usage to the most recently used case.
        usage.usedImageLayout = imageLayoutUsage.usedImageLayout;
        return;
    }
...
bool ContextVk::containsImageLayoutUsage(vk::ImageHelper *imageHelper, vk::ImageLayout imageLayout)
{
    ImageLayoutUsage imageLayoutUsage = {};
    imageLayoutUsage.usedVkImage = reinterpret_cast<void *>(imageHelper->getImage().getHandle());
    imageLayoutUsage.usedImageLayout = imageLayout;
    ...
    for (auto &usage : mImageLayoutUsages)
    {
        ...
        {
            //            WARN() << "There should be a separate barrier here (" << usage.usedVkImage
            //            << "; "
            //                   << (uint32_t)usage.usedImageLayout << " -> "
            //                   << (uint32_t)imageLayoutUsage.usedImageLayout << "); temporarily
            //                   disabled";
            return false;
            // return true;
        }
...
angle::Result ContextVk::onResourceAccess(const vk::CommandBufferAccess &access)
{
    ...
    for (const vk::CommandBufferImageAccess &imageAccess : access.getReadImages())
    {
        ...
        if (containsImageLayoutUsage(imageAccess.image, imageAccess.imageLayout))
        {
    ...
    for (const vk::CommandBufferImageWrite &imageWrite : access.getWriteImages())
    {
        ...
        // If there is a currently pending command that is using this image (with a different
        // layout), this should be a separate barrier due to potential dependency problems.
        if (containsImageLayoutUsage(imageWrite.access.image, imageWrite.access.imageLayout))
        {
...
bool ContextVk::shouldFlushDueToImageLayoutTransition(vk::CommandBufferImageAccess access)
{
    ...
    // TODO: Add the WAW hazard conditions as well. (Except RAR cases; currently TransferDst for
    // test)
    if (imageLayout != image->getCurrentImageLayout() ||
        imageLayout == vk::ImageLayout::TransferDst)
    {
        return true;
    }

    // TODO: Test
    //    if (image->getWriteResourceUse().getSerials().size() > 0 &&
    //        image->getWriteResourceUse().getSerials().front() <
    //        mLastFlushedQueueSerial.getSerial())
    //    {
    //        WARN() << "Should be flushed? Does getResourceUse() cover both read and write?";
    //    }
...
angle::Result ContextVk::flushCommandBuffersIfNecessary(const vk::CommandBufferAccess &access)
{
    ...
    for (const vk::CommandBufferImageAccess &imageAccess : access.getReadImages())
    {
        ...
        {
            //            WARN() << "End of necessary flush (read image)";
        ...
        {
            // TODO: Merge with previous condition. (Also check if the image is already in the list
    ...
    for (const vk::CommandBufferImageWrite &imageWrite : access.getWriteImages())
    {
        ...
        {
            //            WARN() << "End of necessary flush (flush all; write image)";
    ...
    for (const vk::CommandBufferBufferAccess &bufferAccess : access.getReadBuffers())
    {
        ...
        {
            //            WARN() << "End of necessary flush (flush all; read buffer)";
            return flushCommandsAndEndRenderPass(
                RenderPassClosureReason::BufferWriteThenOutOfRPRead);
        }
    ...
    for (const vk::CommandBufferBufferAccess &bufferAccess : access.getWriteBuffers())
    {
        ...
        {
            //            WARN() << "End of necessary flush (flush all; write buffer)";
            return flushCommandsAndEndRenderPass(
                RenderPassClosureReason::BufferUseThenOutOfRPWrite);
        }
    ...
    {
        //        WARN() << "End of necessary flush (flush ORPCB)";
        return flushOutsideRenderPassCommands();
    }

    //    WARN() << "End of necessary flush (no action)";
    return angle::Result::Continue;
}
