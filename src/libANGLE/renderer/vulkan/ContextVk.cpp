...
angle::Result ContextVk::commandProcessorSyncErrorsAndQueueCommand(
    vk::CommandProcessorTask *command)
{
    commandProcessorSyncErrors();
    return mRenderer->queueCommand(this, command);
}
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    if (mRenderer->getFeatures().commandProcessor.enabled)
    {
        ...
        ANGLE_TRY(commandProcessorSyncErrorsAndQueueCommand(&flushAndQueueSubmit));
...
angle::Result ContextVk::flushCommandsAndEndRenderPass()
{
    ...
    if (mRenderer->getFeatures().commandProcessor.enabled)
    {
        ...
        ANGLE_TRY(commandProcessorSyncErrorsAndQueueCommand(&flushToPrimary));
        getNextAvailableCommandBuffer(&mRenderPassCommands, true);
    }
    else
    {
