    ...
    vk::CommandBufferHelper *commandBufferHelper)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        vk::ResourceUseType resourceUseType = vk::IsImageLayoutReadOnly(textureLayout)
                                                  ? vk::ResourceUseType::Read
                                                  : vk::ResourceUseType::ReadWrite;
        textureVk->retainImageViews(&mResourceUseList, resourceUseType);
    }
...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        timestampQuery.retain(&scratchResourceUseList, vk::ResourceUseType::Read);
...
angle::Result ContextVk::onResourceAccess(const vk::CommandBufferAccess &access)
{
    ...
    for (const vk::CommandBufferImageWrite &imageWrite : access.getWriteImages())
    {
        ...
        imageWrite.access.image->recordWriteBarrier(
            ...
            &mOutsideRenderPassCommands->getCommandBuffer());
        imageWrite.access.image->retain(&mResourceUseList, vk::ResourceUseType::ReadWrite);
