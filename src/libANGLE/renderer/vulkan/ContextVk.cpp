...
angle::Result ContextVk::updateActiveTextures(const gl::Context *context, gl::Command command)
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        // Determine if a misbehaving application has specified an image format that does not
        // match the specified sampler format and save the corrected format
        textureVk->checkSamplerImageFormatMismatch(
            getRenderer(), executable->getSamplerFormatForTextureUnitIndex(textureUnit), &image);

        if ((image.getActualFormat().isSRGB &&
             samplerState.getSRGBDecode() == GL_SKIP_DECODE_EXT) ||
            textureVk->recreateViewFormatForSampler())
        {
