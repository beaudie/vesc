...
void ContextVk::updateStencilWriteWorkaround()
{
    ...
    // On ARM, having a stencil write mask of 0 in static state enables optimizations that make the
    // interaction of the stencil write mask dynamic state with discard and alpha to coverage
    // broken.  When the program has discard, or when alpha to coverage is enabled, these
