...
// Enable the GLES-to-Vulkan mapping/logging feature by setting:
// "angle_enable_ogl_vk_api_mapping = true" in GN args.
#if defined(ANGLE_ENABLE_OGL_VK_API_MAPPING)
constexpr static bool kEnableOpenGLToVulkanMapping = true;
#else   // if defined(ANGLE_ENABLE_OGL_VK_API_MAPPING)
constexpr static bool kEnableOpenGLToVulkanMapping = false;
...
{
    if (!kEnableOpenGLToVulkanMapping)
    {
        return;
    }
