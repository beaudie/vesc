                                          ...
                                          const void *indices)
{
    ...
    if (!elementArrayBuffer)
    {
        ...
        if (indexCount == 6 && indexType == gl::DrawElementsType::UnsignedShort)
        {
            ...
            {
                if (reinterpret_cast<const uint64_t *>(indices)[0] ==
                        reinterpret_cast<const uint64_t *>(
                            kCachedElementArrayData[cacheIndex])[0] &&
                    reinterpret_cast<const uint32_t *>(indices)[2] ==
                        reinterpret_cast<const uint32_t *>(kCachedElementArrayData[cacheIndex])[2])
                {
                    foundInCache = true;
                    break;
                }
