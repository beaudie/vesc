                                                       ...
                                                       GLsizei stride)
{
    ...
    // vertex attributes.
    const bool supportsMultiDrawIndirect = getFeatures().supportsMultiDrawIndirect.enabled;
    ...
    const bool isDrawCountBeyondLimit =
        (static_cast<uint32_t>(drawcount) >
         mRenderer->getPhysicalDeviceProperties().limits.maxDrawIndirectCount);
    const bool isMultiDrawWithStreamingAttribs =
        (mVertexArray->getStreamingVertexAttribsMask().any() && drawcount > 1);
    if (!supportsMultiDrawIndirect || isMultiDrawLineLoop || isDrawCountBeyondLimit ||
