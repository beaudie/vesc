                               ...
                               bool *newCommandBufferOut)
{
    ...
    if (mTexturesDirty)
    {
        ...
        for (const gl::SamplerBinding &samplerBinding : programGL->getSamplerBindings())
        {
            ...
            // Null textures represent incomplete textures, incomplete textures are managed when we
            // update the descriptor sets.
            if (texture == nullptr)
            {
                mRenderer->getIncompleteTexture(context, samplerBinding.textureType, &texture);
            }
