...
void ContextVk::onDestroy(const gl::Context *context)
{
    ...
    mCommandPool.outsideRenderPassPool.destroy(device);
    mCommandPool.renderPassPool.destroy(device);
...
angle::Result ContextVk::initialize()
{
    ...
    ANGLE_TRY(vk::OutsideRenderPassCommandBuffer::InitializeCommandPool(
        this, &mCommandPool.outsideRenderPassPool, mRenderer->getDeviceQueueIndex(),
        hasProtectedContent()));
    ANGLE_TRY(vk::RenderPassCommandBuffer::InitializeCommandPool(this, &mCommandPool.renderPassPool,
                                                                 mRenderer->getDeviceQueueIndex(),
                                                                 hasProtectedContent()));
    ANGLE_TRY(mRenderer->getOutsideRenderPassCommandBufferHelper(
        this, &mCommandPool.outsideRenderPassPool, &mOutsideRenderPassCommands));
    ANGLE_TRY(mRenderer->getRenderPassCommandBufferHelper(this, &mCommandPool.renderPassPool,
                                                          &mRenderPassCommands));
...
angle::Result ContextVk::handleDirtyShaderResourcesImpl(CommandBufferHelperT *commandBufferHelper)
{
    ...
    if (hasImages)
    {
        ...
        // Record usage of storage images in the command buffer to aid handling of glMemoryBarrier.
        commandBufferHelper->setHasShaderStorageOutput();
    }

    return handleDirtyShaderBufferResourcesImpl(commandBufferHelper);
}

angle::Result ContextVk::handleDirtyShaderBufferResourcesImpl(
    vk::CommandBufferHelperCommon *commandBufferHelper)
{
    const gl::ProgramExecutable *executable = mState.getProgramExecutable();
    ASSERT(executable);
    ...
    if (!hasUniformBuffers && !hasStorageBuffers && !executable->usesFramebufferFetch())
    {
        return angle::Result::Continue;
    }
    ...
    // Record usage of storage buffers in the command buffer to aid handling of glMemoryBarrier.
