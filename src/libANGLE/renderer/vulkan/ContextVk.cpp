...
{
    addWaitSemaphore(vk::GetImpl(semaphore)->getHandle(), VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT);
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    VkSubmitInfo submitInfo = {};
    InitializeSubmitInfo(&submitInfo, commandBatch.get(), mWaitSemaphores, mWaitSemaphoreStageMasks,
                         signalSemaphore);
...
void ContextVk::addWaitSemaphore(VkSemaphore semaphore, VkPipelineStageFlags waitDstStageMask)
{
    mWaitSemaphores.push_back(semaphore);
    mWaitSemaphoreStageMasks.push_back(waitDstStageMask);
}
...
void ContextVk::waitForSwapchainImageIfNecessary()
{
    if (mCurrentWindowSurface)
    {
        ...
        {
            addWaitSemaphore(waitSemaphore.getHandle(), VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT);
