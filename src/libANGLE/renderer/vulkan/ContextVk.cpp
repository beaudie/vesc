...
constexpr size_t kUniformBlockDynamicBufferMinSize = 16384u << 2;
                                                ...
                                                bool *newBufferOut)
{
    ...
    // into context's mResourceUseList which will ensure they gets tagged with queue serial number
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    // mResourceUseList since they never gets reused or freed until context get destroyed, at which
    // time we always wait for GPU to finish before destroy the dynamic buffers.
