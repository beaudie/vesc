...
void ContextVk::updateScissor(const gl::State &glState)
{
    ...
    if (mCommandBuffer && framebuffer->hasStartedRenderPass() &&
        !framebuffer->getRenderPassRenderArea().encloses(scissoredRenderArea))
    {
        framebuffer->finishCurrentCommands(mRenderer);

        // The above command is expected to clear this, so we would start a new render pass on the
        // next draw.
