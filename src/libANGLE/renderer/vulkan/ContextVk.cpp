...
angle::Result ContextVk::relaseCommandPoolToBatch(CommandBatch *batch)
{
...
void ContextVk::recyleCommandPoolFromBatch(CommandBatch *batch)
{
                                     ...
                                     vk::PrimaryCommandBuffer &&commandBuffer)
{
    ...
    ANGLE_TRY(relaseCommandPoolToBatch(&batch));
...
angle::Result ContextVk::checkCompletedCommands()
{
    ...
    for (CommandBatch &batch : mInFlightCommands)
    {
        ...
        recyleCommandPoolFromBatch(&batch);
        mPrimaryCommandPool.collect(std::move(batch.primaryCommands));
        ++finishedCount;
    }
