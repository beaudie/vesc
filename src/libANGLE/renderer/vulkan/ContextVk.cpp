                                                  ...
                                                  vk::CommandBuffer **commandBufferOut)
{
        ...
        mGraphicsDirtyBits[DIRTY_BIT_INDEX_BUFFER])
    {
        ...
        // The count is in the indirectBuffer.
                                              ...
                                              const void *indirect)
{
    ...
    if (!indirectBuffer)
    {
        const gl::DrawElementsIndirectCommand *cmd =
            reinterpret_cast<const gl::DrawElementsIndirectCommand *>(indirect);
        // TODO: Verify that this call will grab index data from GL_ELEMENT_ARRAY_BUFFER buffer.
        return drawElementsInstancedBaseVertexBaseInstance(context, mode, cmd->primCount, type,
                                                           nullptr, cmd->count, cmd->baseVertex,
                                                           cmd->baseInstance);
    }
    ...
    vk::CommandBuffer *commandBuffer = nullptr;
    ANGLE_TRY(
        setupIndexedIndirectDraw(context, mode, type, indirectBuffer, indirect, &commandBuffer));
    vk::BufferHelper &buffer           = vk::GetImpl(indirectBuffer)->getBuffer();
    vk::FramebufferHelper *framebuffer = mDrawFramebuffer->getFramebuffer();
    buffer.onRead(framebuffer, VK_ACCESS_INDIRECT_COMMAND_READ_BIT);

    commandBuffer->drawIndexedIndirect(buffer.getBuffer(), reinterpret_cast<VkDeviceSize>(indirect),
                                       1, 0);
    return angle::Result::Continue;
}
