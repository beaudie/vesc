...
ANGLE_INLINE void ContextVk::flushDescriptorSetUpdates()
{
    ...
    {
        ASSERT(mVkDescriptorImageInfos.empty());
        ASSERT(mVkDescriptorBufferInfos.empty());
        ASSERT(mVkBufferViews.empty());
        return;
    }

    size_t totalImageInfos = 0;
    for (const std::pair<size_t, size_t> &it : mImageInfoIndices)
    {
        size_t index         = it.first;
        size_t numImageInfos = it.second;

        mWriteDescriptorSets[index].pImageInfo = &mVkDescriptorImageInfos[totalImageInfos];
        totalImageInfos += numImageInfos;
    }

    size_t totalBufferInfos = 0;
    for (const std::pair<size_t, size_t> &it : mBufferInfoIndices)
    {
        size_t index          = it.first;
        size_t numBufferInfos = it.second;

        mWriteDescriptorSets[index].pBufferInfo = &mVkDescriptorBufferInfos[totalBufferInfos];
        totalBufferInfos += numBufferInfos;
    }

    size_t totalBufferViews = 0;
    for (const std::pair<size_t, size_t> &it : mBufferViewIndices)
    {
        size_t index          = it.first;
        size_t numBufferViews = it.second;

        mWriteDescriptorSets[index].pTexelBufferView = &mVkBufferViews[totalBufferViews];
        totalBufferViews += numBufferViews;
    }
    ...
    mImageInfoIndices.clear();
    mVkDescriptorBufferInfos.clear();
    mBufferInfoIndices.clear();
    mVkBufferViews.clear();
    mBufferViewIndices.clear();
}
      ...
      mShareGroupVk(vk::GetImpl(state.getShareGroup()))
{
    ...
    mVkDescriptorImageInfos.reserve(kDescriptorWriteInfosInitialSize);
    mImageInfoIndices.reserve(kDescriptorWriteInfosInitialSize);
    mVkDescriptorBufferInfos.reserve(kDescriptorWriteInfosInitialSize);
    mBufferInfoIndices.reserve(kDescriptorWriteInfosInitialSize);
    mVkBufferViews.reserve(kDescriptorWriteInfosInitialSize);
    mBufferViewIndices.reserve(kDescriptorWriteInfosInitialSize);
}
    ...
    DriverUniformsDescriptorSet *driverUniforms)
{
    ...
    VkWriteDescriptorSet *writeDescriptorSet = nullptr;
    getBufferInfosAndWriteInfo(1, &bufferInfos, &writeDescriptorSet);

    // Update the driver uniform descriptor set.
    bufferInfos->buffer = buffer->getBuffer().getHandle();
    bufferInfos->offset = 0;
    bufferInfos->range  = driverUniformsSize;

    writeDescriptorSet->sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeDescriptorSet->dstSet          = driverUniforms->descriptorSet;
...
void ContextVk::getImageInfosAndWriteInfo(size_t numImageInfos,
                                          VkDescriptorImageInfo **imageInfosOut,
                                          VkWriteDescriptorSet **writeInfoOut)
{
    size_t writeDescriptorSetsIndex = mWriteDescriptorSets.size();
    mImageInfoIndices.emplace_back(writeDescriptorSetsIndex, numImageInfos);

    mWriteDescriptorSets.emplace_back();
    *writeInfoOut = &mWriteDescriptorSets.back();

    size_t oldSize = mVkDescriptorImageInfos.size();
    mVkDescriptorImageInfos.resize(oldSize + numImageInfos);
    *imageInfosOut = &mVkDescriptorImageInfos[oldSize];
}

void ContextVk::getBufferInfosAndWriteInfo(size_t numBufferInfos,
                                           VkDescriptorBufferInfo **bufferInfosOut,
                                           VkWriteDescriptorSet **writeInfoOut)
{
    size_t writeDescriptorSetsIndex = mWriteDescriptorSets.size();
    mBufferInfoIndices.emplace_back(writeDescriptorSetsIndex, numBufferInfos);

    mWriteDescriptorSets.emplace_back();
    *writeInfoOut = &mWriteDescriptorSets.back();

    size_t oldSize = mVkDescriptorBufferInfos.size();
    mVkDescriptorBufferInfos.resize(oldSize + numBufferInfos);
    *bufferInfosOut = &mVkDescriptorBufferInfos[oldSize];
}

void ContextVk::getBufferViewsAndWriteInfo(size_t numBufferViews,
                                           VkBufferView **bufferViewsOut,
                                           VkWriteDescriptorSet **writeInfoOut)
{
    size_t writeDescriptorSetsIndex = mWriteDescriptorSets.size();
    mBufferViewIndices.emplace_back(writeDescriptorSetsIndex, numBufferViews);

    mWriteDescriptorSets.emplace_back();
    *writeInfoOut = &mWriteDescriptorSets.back();

    size_t oldSize = mVkBufferViews.size();
    mVkBufferViews.resize(oldSize + numBufferViews);
    *bufferViewsOut = &mVkBufferViews[oldSize];
}
