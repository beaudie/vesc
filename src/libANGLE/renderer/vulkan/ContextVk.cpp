...
gl::Error ContextVk::storeVertexData(const void *data, size_t amount, VkBuffer *handleOut, VkDeviceSize *offsetOut)
{
    StreamBuffer buf;
    buf.setQueueSerial(mRenderer->getCurrentQueueSerial());
    ANGLE_TRY(buf.stuff(this, data, amount, handleOut, offsetOut));
    printf("release\n");
    buf.release(mRenderer);
    return gl::NoError();
}
