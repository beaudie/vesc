...
angle::Result ContextVk::initialize()
{
    ...
    {
        // printf("Initializing cmdBuffer %p w/ SCB %p at index #%u\n", &mCommandBuffers[i],
        //       &mCommandBuffers[i].getCommandBuffer(), i);
        // Push a scope in the pool allocator so we can easily reinitialize on flush.
        mPoolAllocator[i].push();
        mCommandBuffers[i].initialize(&mPoolAllocator[i], false);
        // printf("\tcmdBuffer has internal alloc ptr %p\n",
        //       mCommandBuffers[i].getCommandBuffer().getPagePtr());
        releaseCommandBufferToQueue(&mCommandBuffers[i]);
    }
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    mOutsideRenderPassCommands->reset();
    mRenderPassCommands->reset();
    // printf("Just flushed, outRPcmdBuffer %p has internal ptr %p\n", mOutsideRenderPassCommands,
    // mOutsideRenderPassCommands->getCommandBuffer().getPagePtr()); printf("Just flushed,
    // inRPcmdBuffer %p has internal ptr %p\n", mRenderPassCommands->
    // mRenderPassCommands->getCommandBuffer().getPagePtr());
    // Must reset all cmdBuffers after pop & push of allocator or risk a non-reset cmd buffer
    // pointing at a stale allocation
    // for (uint32_t i = 0; i < mAvailableCommandBuffers.size(); i++)
    //{
    //    vk::CommandBufferHelper *tmp = mAvailableCommandBuffers.front();
    //    // printf("Just flushed, queue %u has cmdBuffer %p w/ internal ptr %p\n", i, tmp,
    //    // tmp->getCommandBuffer().getPagePtr());
    //    tmp->reset();
    //    // printf("\tReset and cmdBuffer %p now has w/ internal ptr %p\n", tmp,
    //    // tmp->getCommandBuffer().getPagePtr());
    //    mAvailableCommandBuffers.pop();
    //    mAvailableCommandBuffers.push(tmp);
    //}
...
#define ANGLE_SERIALIZE_WORKER_THREAD 0
...
#if ANGLE_SERIALIZE_WORKER_THREAD
    mRenderer->workerThreadWaitIdle();
#endif
    getNextAvailableCommandBuffer(&mRenderPassCommands, true);
...
angle::Result ContextVk::flushOutsideRenderPassCommands()
{
    ...
    {
        // printf("Have non-empty cmdBuffer %p that is being submitted to worker thread\n",
        // mOutsideRenderPassCommands);
