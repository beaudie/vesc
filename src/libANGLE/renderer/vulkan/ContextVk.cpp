...
void ContextVk::onDestroy(const gl::Context *context)
{
    ...
    mGraphicsDriverUniforms.destroy(device);
    mComputeDriverUniforms.destroy(device);
...
angle::Result ContextVk::initialize()
{
    ...
    mGraphicsDriverUniforms.init(mRenderer);
    mComputeDriverUniforms.init(mRenderer);

    // Get the descriptor set layout.
    vk::DescriptorSetLayoutDesc desc =
        getDriverUniformsDescriptorSetDesc(VK_SHADER_STAGE_ALL_GRAPHICS);
    ANGLE_TRY(mRenderer->getDescriptorSetLayout(this, desc,
                                                &mGraphicsDriverUniforms.descriptorSetLayout));
    desc = getDriverUniformsDescriptorSetDesc(VK_SHADER_STAGE_COMPUTE_BIT);
    ANGLE_TRY(
        mRenderer->getDescriptorSetLayout(this, desc, &mComputeDriverUniforms.descriptorSetLayout));
                                                             ...
                                                             size_t offsetsSize)
{
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < atomicCounterBufferCount; ++bufferIndex)
    {
        ...
        {
            uint32_t offset        = atomicCounterBuffer->getOffset();
            uint32_t alignedOffset = (offset / offsetAlignment) * offsetAlignment;

            // GL requires the atomic counter buffer offset to be aligned with uint.
            ASSERT((offset - alignedOffset) % sizeof(uint32_t) == 0);
            offsetDiff = (offset - alignedOffset) / static_cast<uint32_t>(sizeof(uint32_t));
                                                           ...
                                                           vk::CommandBuffer *commandBuffer)
{
    ...
    VkBuffer buffer;
    bool newBuffer;
    ANGLE_TRY(allocateDriverUniforms(sizeof(GraphicsDriverUniforms), &mGraphicsDriverUniforms,
                                     &buffer, &ptr, &newBuffer));
                                                          ...
                                                          vk::CommandBuffer *commandBuffer)
{
    ...
    VkBuffer buffer;
    bool newBuffer;
    ANGLE_TRY(allocateDriverUniforms(sizeof(ComputeDriverUniforms), &mComputeDriverUniforms,
                                     &buffer, &ptr, &newBuffer));
