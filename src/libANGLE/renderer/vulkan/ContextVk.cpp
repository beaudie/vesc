...
void ContextVk::onDestroy(const gl::Context *context)
{
    ...
    mRenderer->getQueueSerialIndexAllocator().release(mCurrentQueueSerial.index);
}
...
angle::Result ContextVk::initialize()
{
    ...
    mCurrentQueueSerial.index  = mRenderer->getQueueSerialIndexAllocator().allocate();
    mCurrentQueueSerial.serial = mQueueSerialFactory.generate();
    ...
    else
    {
        ...
        if (shouldConvertUint8VkIndexType(indexType) && mGraphicsDirtyBits[DIRTY_BIT_INDEX_BUFFER])
        {
            ...
            if (bufferHelper.isHostVisible() &&
                !bufferHelper.isCurrentlyInUse(getLastCompletedQueueSerials()))
            {
                uint8_t *src = nullptr;
                ANGLE_TRY(
...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        ANGLE_TRY(mRenderer->queueSubmitOneOff(this, std::move(commandBuffer),
                                               hasProtectedContent(), mContextPriority, nullptr, 0,
        ...
        // ANGLE_TRY(finishToSerials(submitSerial));
...
angle::Result ContextVk::checkCompletedGpuEvents()
{
    ...
    const vk::QueueSerials &lastCompletedQueueSerials = getLastCompletedQueueSerials();

    for (GpuEventQuery &eventQuery : mInFlightGpuEventQueries)
    {
        // Only check the timestamp query if the submission has finished.
        if (eventQuery.queryHelper.usedInRunningCommands(lastCompletedQueueSerials))
        {
            break;
        }
...
angle::Result ContextVk::finishToSerials(const vk::QueueSerials &queueSerials)
{
    return mRenderer->finishToSerials(this, queueSerials);
}
