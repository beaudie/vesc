...
angle::Result ContextVk::initialize()
{
    ...
    generateQueueSerialForCommandBuffer(mOutsideRenderPassCommands);
...
angle::Result ContextVk::submitFrameOutsideCommandBufferOnly()
{
    ...
    vk::RenderPassSerial *renderPassSerialOut)
{
    ...
    *renderPassSerialOut     = mCurrentRenderPassSerial;
    ...
    // Now generate queueSerial for the render pass.
    ASSERT(mOutsideRenderPassCommands->getQueueSerial().valid());
    generateQueueSerialForCommandBuffer(mRenderPassCommands);
                                                           ...
                                                           RenderPassClosureReason reason)
{
    ...
    ANGLE_TRY(mRenderer->flushRenderPassCommands(this, hasProtectedContent(), *renderPass,
                                                 &mRenderPassCommands));
    // Note: queueSerial for renderpass is deferred until renderpass begin time.
...
angle::Result ContextVk::submitOutsideRenderPassCommandsImpl()
{
    ...
    ANGLE_TRY(flushOutsideRenderPassCommands());
    ANGLE_TRY(submitFrameOutsideCommandBufferOnly());
    return angle::Result::Continue;
}
...
angle::Result ContextVk::flushOutsideRenderPassCommands()
{
    ...
    {
        // This call may be followed by renderpass flush and start a new renderpass. In order to
        // keep queue serials in right order, we have to tell context that this empty command buffer
        // has been flushed and a new queueSerial should be generated to reflect the new command.
        ASSERT(
            mLastFlushedQueueSerial.index != mOutsideRenderPassCommands->getQueueSerial().index ||
    ...
    // We just got a new mOutsideRenderPassCommands. generate a new QueueSerial for it.
    generateQueueSerialForCommandBuffer(mOutsideRenderPassCommands);
