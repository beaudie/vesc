...
gl::Error ClientBuffer::copy(ContextImpl *contextImpl, size_t count, size_t size, size_t stride, const void *srcPtr, size_t *offset)
{
    GLvoid *dstPtr;
    ANGLE_TRY(mBuffer.map(contextImpl, 0, &dstPtr));
    ...
    ANGLE_TRY(mBuffer.unmap(contextImpl, nullptr));
    *offset = mOffset;
    mOffset += count * size;
    return gl::Error(GL_NO_ERROR);
}
    ...
    for (auto attribIndex : angle::IterateBitSet(programGL->getActiveAttribLocationsMask()))
    {
        ...
        if (attrib.enabled)
        {
            ...
            else
            {
                ...
                size_t offset;
                ANGLE_TRY(mClientBuffer->copy(this, count, bindingDesc.stride, attrib.stride, attrib.pointer, &offset));
