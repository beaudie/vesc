                                       ...
                                       const VkImageMemoryBarrier &imageMemoryBarrier)
{
    ...
    mPipelineBarrier.mSrcStageMask |= srcStageMask;
    mPipelineBarrier.mDstStageMask |= dstStageMask;
    mPipelineBarrier.mImageMemoryBarriers.push_back(imageMemoryBarrier);
}
...
{
    if (mPipelineBarrier.mImageMemoryBarriers.empty() &&
        mPipelineBarrier.mMemoryBarrierSrcAccess == 0)
    {
        return;
    }
...
void CommandBufferHelper::addCommandDiagnostics(ContextVk *contextVk)
{
    ...
    if (mPipelineBarrier.mMemoryBarrierSrcAccess != 0 ||
        mPipelineBarrier.mMemoryBarrierDstAccess != 0)
    {
        out << "Memory Barrier Src: 0x" << std::hex << mPipelineBarrier.mMemoryBarrierSrcAccess
            << " &rarr; Dst: 0x" << std::hex << mPipelineBarrier.mMemoryBarrierDstAccess << "\\l";
    }
    if (mIsRenderPassCommandBuffer)
    {
