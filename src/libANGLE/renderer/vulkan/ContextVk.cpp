                               ...
                               bool *newCommandBufferOut)
{
    ...
    {
        *newCommandBufferOut = mVertexArrayChange;
        mVertexArrayChange   = false;
...
gl::Error ContextVk::drawArrays(const gl::Context *context, GLenum mode, GLint first, GLsizei count)
{
    ...
    {
        ANGLE_TRY(vertexArrayVk->onLineLoopDraw(context, mRenderer, drawCallParams, drawNode));
        vk::LineLoopHandler::Draw(count, commandBuffer);
    }
    else
    {
        ANGLE_TRY(vertexArrayVk->onDraw(context, mRenderer, drawCallParams, drawNode, newCommands));
                                  ...
                                  const void *indices)
{
    ...
    {
        ANGLE_TRY(
            vertexArrayVk->onIndexedLineLoopDraw(context, mRenderer, drawCallParams, drawNode));
        vk::LineLoopHandler::Draw(count, commandBuffer);
    }
    else
    {
        ANGLE_TRY(vertexArrayVk->onIndexedDraw(context, mRenderer, drawCallParams, drawNode,
                                               newCommands));
