...
{
    growCapacity<VkDescriptorBufferInfo, &VkWriteDescriptorSet::pBufferInfo>(mBufferInfos, newSize);
}

void DescriptorSetUpdates::growImageInfoCapacity(size_t newSize)
{
    growCapacity<VkDescriptorImageInfo, &VkWriteDescriptorSet::pImageInfo>(mImageInfos, newSize);
}
...
void DescriptorSetUpdates::growCapacity(std::vector<T> &mInfos, size_t newSize)
{
    ...
    size_t newCapacity          = std::min(mInfos.capacity() << 1, newSize);
