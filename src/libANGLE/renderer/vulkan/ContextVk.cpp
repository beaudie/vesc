...
void ContextVk::syncAnyErrorAndQueueCommandToProcessorThread(vk::CommandProcessorTask *command)
{
    ...
    mRenderer->queueCommand(command);
}
...
angle::Result ContextVk::synchronizeCpuGpuTime()
{
    ...
    for (uint32_t i = 0; i < kRetries; ++i)
    {
        ...
        {
            ANGLE_TRY(mRenderer->commandProcessorThreadQueueSubmitOneOff(
                this, std::move(commandBuffer), mContextPriority));
            // Wait for the command to be sent to the GPU
...
void ContextVk::clearAllGarbage()
{
    ...
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        syncAnyErrorAndQueueCommandToProcessorThread(&clearAllGarbage);
    }
    else
    {
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        // Make sure a new fence is created for the next submission.
        // TODO: Is it ok to call this while submit fence is active in worker? That is, does this
        //  blast the current submit fence or just create new one? We want adding fence to flushData
        //  above to cause extra ref so that fence is still in use. Need to release at some point
        //  when worker thread is done with it.
        // mRenderer->resetSharedFence(&mSubmitFence);
        // printf("\tReset sharedFence to %p\n", mSubmitFence.get().getHandle());
...
bool ContextVk::isSerialInUse(Serial serial) const
{
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        // Process any completed commands since we are looking for latest serial
        vk::CommandProcessorTask checkCompletedCommands(vk::CustomTask::CheckCompletedCommands);
        mRenderer->queueCommand(&checkCompletedCommands);
    }
...
angle::Result ContextVk::finishToSerial(Serial serial)
{
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        // TODO: Implement RendererVk::finishToSerial that calls mCommandProcessor::finishToSerial?
        // Do we really need to send a finishToSerial command to the worker?
        vk::CommandProcessorTask task(serial);

        syncAnyErrorAndQueueCommandToProcessorThread(&task);
        mRenderer->waitForCommandProcessorIdle();
...
angle::Result ContextVk::getTimestamp(uint64_t *timestampOut)
{
    ...
    if (getFeatures().enableCommandProcessingThread.enabled)
    {
        ANGLE_TRY(mRenderer->commandProcessorThreadQueueSubmitOneOff(this, std::move(commandBuffer),
                                                                     mContextPriority));
        // Wait for the command to be sent to the GPU
        mRenderer->waitForCommandProcessorIdle();
    }
...
angle::Result ContextVk::flushCommandsAndEndRenderPass()
{
    ...
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        vk::CommandProcessorTask task(this, mRenderPassCommands);
...
angle::Result ContextVk::flushOutsideRenderPassCommands()
{
    ...
    if (mRenderer->getFeatures().enableCommandProcessingThread.enabled)
    {
        ...
        vk::CommandProcessorTask task = {this, mOutsideRenderPassCommands};
