...
gl::Error ContextVk::drawArrays(GLenum mode, GLint first, GLsizei count)
{
    ...
    ANGLE_TRY_RESULT(vkFBO->getFramebuffer(device), framebuffer);
    ...
    colorClear.float32[0] = state.getColorClearValue().red;
    colorClear.float32[1] = state.getColorClearValue().green;
    colorClear.float32[2] = state.getColorClearValue().blue;
    colorClear.float32[3] = state.getColorClearValue().alpha;
    ...
    commandBuffer->beginRenderPass(*renderPass, *framebuffer, state.getViewport(),
