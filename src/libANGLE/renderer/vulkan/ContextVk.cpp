                                                     ...
                                                     vk::CommandBuffer *commandBuffer)
{
    ...
    if (!mCurrentGraphicsPipeline)
    {
        ...
        // Creating the pipeline layout also resets some resources we need the programs to rebuild.
                                                              ...
                                                              vk::CommandGraphResource *recorder)
{
    ...
    if (mProgramPipeline->hasTextures(context->getState()))
    {
    ...
    vk::CommandGraphResource *recorder)
{
    ...
    if (mProgramPipeline->hasImages(getState()))
    {
        ANGLE_TRY(updateActiveImages(context, recorder));
    }

    if (mProgramPipeline->hasUniformBuffers(getState()) ||
        mProgramPipeline->hasStorageBuffers(getState()) ||
        mProgramPipeline->hasAtomicCounterBuffers(getState()) ||
        mProgramPipeline->hasImages(getState()))
    {
    ...
    vk::CommandBuffer *commandBuffer)
{
    ...
    size_t bufferCount = mProgramPipeline->getTransformFeedbackBufferCount(getState());
    ...
    {
        const ProgramVk *program = mProgramPipeline->getShaderProgram(getState(), shaderType);
        if (program)
        {
            transformFeedbackVk->addFramebufferDependency(this, program->getState(), framebuffer);
            break;
        }
    }
                                   ...
                                   const gl::State::DirtyBits &bitMask)
{
    ...
    if ((dirtyBits & mPipelineDirtyBitsMask).any() &&
        (glState.getProgramPipeline() == nullptr || !glState.getProgramPipeline()->isCompute()))
    {
