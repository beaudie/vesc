...
{
    angle::Result result = finishToSerialOrTimeout(serial, mRenderer->getMaxFenceWaitTimeNs());

    return result;
}

angle::Result ContextVk::finishToSerialOrTimeout(Serial serial, uint64_t timeout)
{
    return mCommandQueue.finishToSerialOrTimeout(this, serial, timeout);
}
