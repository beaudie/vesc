                               ...
                               vk::CommandBuffer **commandBuffer)
{
    ...
    if (mVertexArrayDirty || overrideReadResource.valid())
    {
...
gl::Error ContextVk::drawArrays(const gl::Context *context, GLenum mode, GLint first, GLsizei count)
{
    ...
    ANGLE_TRY(setupDraw(context, mode, DrawType::Arrays, first, first + count - 1,
                        Optional<ResourceVk *>(), &commandBuffer));
                                  ...
                                  const void *indices)
{
    ...
    if (mode == GL_LINE_LOOP)
    {
        ...
        Optional<ResourceVk *> overrideReadResource = mLineLoopHandler.getLineLoopBufferResource();
        // TODO(fjhenigman): calculate the index range and pass to setupDraw()
        ANGLE_TRY(setupDraw(context, mode, DrawType::Elements, 0, 0, overrideReadResource,
                            &commandBuffer));
    ...
    {
        // TODO(fjhenigman): calculate the index range and pass to setupDraw()
        ANGLE_TRY(setupDraw(context, mode, DrawType::Elements, 0, 0, Optional<ResourceVk *>(),
                            &commandBuffer));
