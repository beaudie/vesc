                               ...
                               bool *newCommandBufferOut)
{
    ...
    if (mTexturesDirty)
    {
		...
		textureVk->onReadResource(graphNode, mRenderer->getCurrentQueueSerial());
            ...
            {
                // TODO(lucferron): Find the right TextureType to use.
                // Just make sure we generate the texture here so its ready to use.
                mRenderer->getIncompleteTexture(context, gl::TextureType::_2D, nullptr);
