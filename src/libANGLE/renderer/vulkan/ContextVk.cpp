      ...
      mGpuEventTimestampOrigin(0)
{
    ...
    mNewGraphicsCommandBufferDirtyBits.set(DIRTY_BIT_BUFFERS_AND_IMAGES);
    ...
    mNewComputeCommandBufferDirtyBits.set(DIRTY_BIT_BUFFERS_AND_IMAGES);
    ...
    mGraphicsDirtyBitHandlers[DIRTY_BIT_BUFFERS_AND_IMAGES] =
        &ContextVk::handleDirtyGraphicsBuffersAndImages;
    ...
    mComputeDirtyBitHandlers[DIRTY_BIT_BUFFERS_AND_IMAGES] =
        &ContextVk::handleDirtyComputeBuffersAndImages;
...
ANGLE_INLINE angle::Result ContextVk::handleDirtyBuffersAndImagesImpl(
    ...
    vk::CommandGraphResource *recorder)
{
    ...
    {
        ANGLE_TRY(mProgram->updateBuffersAndImagesDescriptorSet(this, recorder));
    }
    return angle::Result::Continue;
}

angle::Result ContextVk::handleDirtyGraphicsBuffersAndImages(const gl::Context *context,
                                                             vk::CommandBuffer *commandBuffer)
{
    return handleDirtyBuffersAndImagesImpl(context, commandBuffer,
                                           mDrawFramebuffer->getFramebuffer());
}

angle::Result ContextVk::handleDirtyComputeBuffersAndImages(const gl::Context *context,
                                                            vk::CommandBuffer *commandBuffer)
{
    return handleDirtyBuffersAndImagesImpl(context, commandBuffer, &mDispatcher);
}
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_PROGRAM_EXECUTABLE:
            {
                ...
                invalidateCurrentBuffersAndImages();
...
void ContextVk::invalidateCurrentBuffersAndImages()
{
    ASSERT(mProgram);
    if (mProgram->hasUniformBuffers() || mProgram->hasStorageBuffers() ||
        mProgram->hasAtomicCounterBuffers())
    {
        mGraphicsDirtyBits.set(DIRTY_BIT_BUFFERS_AND_IMAGES);
        mGraphicsDirtyBits.set(DIRTY_BIT_DESCRIPTOR_SETS);
        mComputeDirtyBits.set(DIRTY_BIT_BUFFERS_AND_IMAGES);
        mComputeDirtyBits.set(DIRTY_BIT_DESCRIPTOR_SETS);
    }
}
