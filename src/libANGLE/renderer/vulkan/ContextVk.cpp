    ...
    uint64_t *framebufferSerialOut)
{
    ...
    // here for various reasons, except typically when UtilsVk needs to start one. The last
    // framebuffer serial is also incremented and used to update the render pass command buffer
    // helper and the framebuffer object (via the output argument). Note that UtilsVk ignores the
    // output serial.
