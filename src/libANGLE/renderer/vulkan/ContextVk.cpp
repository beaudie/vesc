...
angle::Result ContextVk::flush(const gl::Context *context)
{
    ...
    //
    // Don't skip flushes for single-bufferd windows with staged updates. It is expected that a
    // flush call on a single-bufferd window ensures any pending updates reach the screen.
    const bool isSingleBufferedWindow =
        mCurrentWindowSurface != nullptr && mCurrentWindowSurface->isSharedPresentMode();
    const bool isSingleBufferdWindowWithStagedUpdates =
