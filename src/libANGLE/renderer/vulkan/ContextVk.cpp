...
bool CommandQueue::inFlightCommandsHaveValidOrdering() const
{
    Serial currentSerial;
    for (const vk::CommandBatch &commands : mInFlightCommands)
    {
                                           ...
                                           uint64_t timeout)
{
    ...
    ASSERT(inFlightCommandsHaveValidOrdering());

    size_t finishedCount = 0;
    while (mInFlightCommands[finishedCount].serial <= finishSerial &&
           finishedCount < mInFlightCommands.size())
    {
        finishedCount++;
    }
