      ...
      mContextPriority(renderer->getDriverPriority(GetContextPriority(state)))
{
    ...
    {
        mPoolAllocator[i].initialize(kDefaultPoolAllocatorPageSize, 1);
    }
}
...
void ContextVk::onDestroy(const gl::Context *context)
{
    ...
    // printf("Main thread destroying primaryCB %p\n", mPrimaryCommands.getHandle());
...
angle::Result ContextVk::flushImpl(const vk::Semaphore *signalSemaphore)
{
    ...
    // Reset active cmd buffers including free secondary cmd pool allocations and start with the new
    // page.
    // mPoolAllocator.pop();
    // mPoolAllocator.push();
    // mOutsideRenderPassCommands->reset();
    ...
    InitializeSubmitInfo(&submitInfo, mPrimaryCommands, mWaitSemaphores, &mWaitSemaphoreStageMasks,
                         signalSemaphore);
    // printf("Main thread moving primaryCB %p to submitFrame()\n", mPrimaryCommands.getHandle());
...
angle::Result ContextVk::endRenderPass()
{
    ...
    {
        // printf("Main thread calling inRP flushToPrimary() w/ primaryCB %p\n",
        // mPrimaryCommands.getHandle());
...
angle::Result ContextVk::flushOutsideRenderPassCommands()
{
    if (!mOutsideRenderPassCommands->empty())
    {
        ...
        {
            // printf("Main thread calling outRP flushToPrimary() w/ primaryCB %p\n",
            // mPrimaryCommands.getHandle());
