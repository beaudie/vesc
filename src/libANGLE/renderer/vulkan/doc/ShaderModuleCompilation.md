...
completed shader source using ANGLE's [GlslangWrapperVk][GlslangWrapperVk.cpp] helper class. At this
time, we use Khronos' [glslang][glslang] to convert the Vulkan-compatible GLSL into SPIR-V, though
we still cannot generate `VkShaderModules` since some ANGLE specializations like
[OpenGL line rasterization](OpenGLLineSegmentRasterization.md) depend on draw-time information. At
link time, the specializations are taken out of the generated SPIR-V and stored as patches, with the
remaining SPIR-V as one that can be used to draw if no specialization is necessary.

1. **Draw-time SPIR-V Generation**: Once the application records a draw call we can specialize the
SPIR-V if necessary based on draw-time information. The SPIR-V is then compiled into `VkShaderModules`.
For details please see [GlslangWrapperVk.cpp][GlslangWrapperVk.cpp]. The `VkShaderModules` are then
used by `VkPipelines`. Note that we currently don't use [SPIRV-Tools][SPIRV-Tools] to perform any
SPIR-V optimization. This could be something to improve on in the future.
