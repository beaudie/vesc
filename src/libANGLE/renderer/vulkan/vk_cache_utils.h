...
static_assert(gl::MAX_VERTEX_ATTRIBS == 16, "Size mismatch");
struct VertexInputAttributes final
{
    PackedAttribDesc attribs[gl::MAX_VERTEX_ATTRIBS];
    // TDEWIP: This is a bitmask flagging which vtx attribs need to be emulated dur to their divisor
    // TODO: When I enable this, I get a static compile error on line 496 below b/c
    // kGraphicsPipelineDescSize != kGraphicsPipelineDescSumOfSizes
    // It seems like both of those should be growing by 2 bytes as the GraphicsPipelineDesc
    // includes a VertexInputAttributes which is growing by 2 bytes
    // uint16_t emulate;
};
