...
CreateSharedDescriptorSetCacheKey(const DescriptorSetDesc &desc, DescriptorPoolHelper *pool)
{
    ...
    return std::make_shared<DescriptorSetAndPoolPointer>(DescriptorAndPoolPointer.release());
}
...
CreateSharedFramebufferCacheKey(const FramebufferDesc &desc)
{
    ...
    return std::make_shared<FramebufferDescPointer>(framebufferDescPointer.release());
}
...
class SharedCacheKeyManager
{
  public:
    ...
    void releaseSharedCacheKey(ContextVk *contextVk);
    void destroy();
