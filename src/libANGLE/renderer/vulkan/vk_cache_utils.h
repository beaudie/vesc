...
class DescriptorSetDesc
{
  public:
    ...
    {
        return (mWriteDescriptors == other.mWriteDescriptors) &&
               (mDescriptorInfos == other.mDescriptorInfos);
    }
    ...
    uint32_t updateWriteDesc(const WriteDescriptorDesc &writeDesc);
    ...
    uint32_t getInfoDescIndex(uint32_t bindingIndex) const
    {
        ...
        ASSERT(hasWriteDescAtIndex(bindingIndex - 1));
        const WriteDescriptorDesc &previous = mWriteDescriptors[bindingIndex - 1];
        return previous.descriptorInfoIndex + previous.descriptorCount;
    }

    void incrementDescriptorCount(uint32_t bindingIndex)
    {
        // Validate we have no subsequent writes.
        ASSERT(hasWriteDescAtIndex(bindingIndex) && !hasWriteDescAtIndex(bindingIndex + 1));
        mWriteDescriptors[bindingIndex].descriptorCount++;
    }
...
class DescriptorSetDescBuilder final
{
  public:
    ...
    void updateTransformFeedbackBuffer(const ShaderInterfaceVariableInfoMap &variableInfoMap,
                                       uint32_t xfbBufferIndex,
                                       const BufferHelper &bufferHelper,
                                       VkDeviceSize bufferOffset,
                                       VkDeviceSize bufferRange);

    // Specific helpers for shader buffer descriptors.
    ...
    angle::Result updateExecutableActiveTextures(
        Context *context,
  ...
  private:
    ...
    angle::FastMap<VkDeviceSize, kFastDescriptorSetDescLimit> mDynamicOffsets;
};
