...
class alignas(4) RenderPassDesc final
{
  public:
    ...
    bool resolveDepth() const { return (mAttachmentFormats.back() & kResolveDepthFlag) != 0; }
    bool resolveStencil() const { return (mAttachmentFormats.back() & kResolveStencilFlag) != 0; }
...
{
    // Note: Currently only 2 subpasses possible, so there is 5 bits in subpass that can be
    // repurposed.
    uint32_t subpass : 6;
