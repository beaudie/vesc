...
using ShaderStageInfo       = gl::ShaderMap<PackedShaderStageInfo>;
...
class PipelineCache : angle::NonCopyable
{
  ...
  protected:
    bool findPipeline(const PipelineDesc &desc, vk::PipelineAndSerial **pipelineOut);
    void putPipeline(const PipelineDesc &desc,
...
template <typename PipelineDesc>
void PipelineCache<PipelineDesc>::putPipeline(const PipelineDesc &desc,
                                              vk::Pipeline &&pipeline,
                                              vk::PipelineAndSerial **pipelineOut)
{
    // The Serial will be updated outside of this insertion.
    auto insertedItem =
...
void PipelineCache<PipelineDesc>::populate(const PipelineDesc &desc, vk::Pipeline &&pipeline)
{
    ...
    vk::PipelineAndSerial *unused = nullptr;
    putPipeline(desc, std::move(pipeline), &unused);
}
