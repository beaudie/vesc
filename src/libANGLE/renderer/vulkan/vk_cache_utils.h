...
class DescriptorSetLayoutDesc final
{
  ...
  private:
    struct PackedDescriptorSetBinding
    {
        ...
        VkSampler immutableSampler;  // immutable sampler or VK_NULL_HANDLE
    };

    // Now 4 32b
    static_assert(sizeof(PackedDescriptorSetBinding) == (sizeof(uint32_t) * 4) ||
                      sizeof(PackedDescriptorSetBinding) == (sizeof(uint32_t) * 2),
                  "Unexpected size");
...
class SamplerDesc final
{
  public:
    ...
    bool hasExternalFormat() const { return mExternalFormat != 0; }
...
// apparently the entire structure must then be a multiple of 8 bytes in size, so this is 32bytes.
