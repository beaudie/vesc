...
class TransformFeedbackDesc
{
  public:
    ...
    void updateDefaultUniformBuffer(UniqueObjectID objectID) { mBufferObjectIDs[0] = objectID; }
    void update(size_t index, UniqueObjectID objectID) { mBufferObjectIDs[index + 1] = objectID; }
    size_t hash() const;
    void reset();
  ...
  private:
    ...
    static constexpr size_t TRANSFORM_FEEDBACK_DESCRIPTOR_MAX_COUNT =
        gl::IMPLEMENTATION_MAX_TRANSFORM_FEEDBACK_BUFFERS + 1;
    std::array<UniqueObjectID, TRANSFORM_FEEDBACK_DESCRIPTOR_MAX_COUNT> mBufferObjectIDs;
};
...
{
    size_t operator()(const rx::Serial &key) const { return (size_t)(key.getValue() & 0xffffffff); }
};
}  // namespace std
