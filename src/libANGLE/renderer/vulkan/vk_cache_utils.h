...
// The following are for caching descriptor set layouts. They are limited to max two descriptor set
// layouts and no push constants. They will be extended in the future to handle other cases.
constexpr size_t kMaxDescriptorSetLayouts = 2;

// Each push constant range defines a block of memory that can be accessed from shaders. We define
// at most one range per shader stage.
...
class PipelineLayoutDesc final
{
  ...
  private:
    ...
    static_assert(sizeof(mDescriptorSetLayouts) ==
                      (sizeof(DescriptorSetLayoutDesc) * kMaxDescriptorSetLayouts),
                  "Unexpected size");
    static_assert(sizeof(mPushConstantRanges) ==
