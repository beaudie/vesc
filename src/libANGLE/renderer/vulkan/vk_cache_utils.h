...
using RefCountedDescriptorSetHelper = RefCounted<DescriptorSetHelper>;
...
class DescriptorSetCache final : angle::NonCopyable
{
  public:
    ...
    // Remove and return the cache entry associated with the desc. nullptr is returned if no cache
    // entry is found.
    vk::RefCountedDescriptorSetHelper *releaseDescriptorSet(const vk::DescriptorSetDesc &desc);
    ...
    ANGLE_INLINE size_t size() const { return mPayload.size(); }
    ANGLE_INLINE bool empty() const { return mPayload.empty(); }

  private:
    angle::HashMap<vk::DescriptorSetDesc, vk::RefCountedDescriptorSetHelper *> mPayload;
};
