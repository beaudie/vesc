...
{
    mNativeFenceFd = kInvalidFenceFd;
}
...
angle::Result SyncHelperNativeFence::initializeWithFd(ContextVk *contextVk, int inFd)
{
    ...
    else
    {
        ...
        */
        ...
        ANGLE_VK_TRY(contextVk, mFenceWithFd.exportFd(device, fenceGetFdInfo, &importFenceFd));
    }
    ...
    // Make a dup of importFenceFd before tranfering ownership to mFenceWithFd
