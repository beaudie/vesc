...
angle::Result SyncHelperNativeFence::initializeWithFd(ContextVk *contextVk, int inFd)
{
    ...
    else
    {
        ...
        */
        ...
        {
            CommandProcessorTask oneOffQueueSubmit(VK_NULL_HANDLE, contextVk->getPriority(),
                                                   &fence.get());
                                                ...
                                                VkResult *outResult)
{
    ...
    // If we are using worker need to wait for the commands to be issued before waiting on the
    // fence.
    if (contextVk->getRenderer()->getFeatures().enableParallelCommandProcessing.enabled)
    {
        contextVk->getRenderer()->waitForCommandProcessorIdle();
    }
