...
// Note: Having mFenceWithFd hold the FD, so that ownership is with ICD
// Any call to clientWait or serverWait will ensure the FD or dup of FD goes to
// application or ICD. At release above its Garbage collected/destroyed.
angle::Result SyncHelperNativeFence::initializeWithFd(ContextVk *contextVk, int inFd)
{
    ...
    if (inFd == kInvalidFenceFd)
    {
        ...
        */
        ...
        if (contextVk->flushImpl(nullptr) == angle::Result::Continue)
        {
            contextVk->getResourceUseList().add(mUse);
            ...
            if (renderer->queueSubmit(contextVk, contextVk->getPriority(), submitInfo,
...
angle::Result SyncHelperNativeFence::serverWait(ContextVk *contextVk)
{
    if (mFenceWithFd.valid())
    {
        ...
        if (mFenceWithFd.exportFd(device, getFdInfo, &fenceFd) == VK_SUCCESS)
        {
            vk::Semaphore waitSemaphore;  // Wait semaphore for next vkQueueSubmit().
            // Create a Semaphore with imported fenceFd.
            if (waitSemaphore.init(device) == VK_SUCCESS)
            {
                VkImportSemaphoreFdInfoKHR importFdInfo = {};
                importFdInfo.sType      = VK_STRUCTURE_TYPE_IMPORT_SEMAPHORE_FD_INFO_KHR;
                importFdInfo.semaphore  = waitSemaphore.getHandle();
                ...
                if (waitSemaphore.importFd(device, importFdInfo) == VK_SUCCESS)
                {
                    // Flush current work, block after current pending commands.
                    if (contextVk->flushImpl(nullptr) == angle::Result::Continue)
                    {
                        // Add semaphore to next submit job.
                        contextVk->insertWaitSemaphore(&waitSemaphore);
                        contextVk->addGarbage(&waitSemaphore);  // This releases the handle.
                        return angle::Result::Continue;
                    }
                }
                // Destroy on failure
                waitSemaphore.destroy(device);
            }
        }
    }
    // Without valid waitSemaphore submitted - return error.
    return angle::Result::Stop;
}
