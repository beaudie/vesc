...
{
    renderer->collectGarbageAndReinit(&mReadOnlyUse, &mEvent);
}
                                     ...
                                     VkResult *outResult)
{
    ...
    {
        if (!mReadOnlyUse.getSerial().valid())
        {
    ...
    ASSERT(mReadOnlyUse.getSerial().valid());

    VkResult status = VK_SUCCESS;
    ANGLE_TRY(
        renderer->waitForSerialWithUserTimeout(context, mReadOnlyUse.getSerial(), timeout, &status));
...
{
    renderer->collectGarbageAndReinit(&mReadOnlyUse, &mFenceWithFd);
}
                                                ...
                                                VkResult *outResult)
{
    ...
    if (mReadOnlyUse.valid())
    {
        ...
        ANGLE_TRY(renderer->waitForSerialWithUserTimeout(context, mReadOnlyUse.getSerial(), timeout,
                                                         &status));
    }
    else
    {
