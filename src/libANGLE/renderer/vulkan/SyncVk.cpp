...
void MapVkResultToGlenum(VkResult vkResult, void *outResult)
{
...
void MapVkResultToEglint(VkResult result, void *outResult)
{
    GLenum *eglIntOut = static_cast<GLenum *>(outResult);
    ASSERT(eglIntOut);
                                     ...
                                     VkResult *resultOut)
{
    ...
    angle::Result prepareForClientWaitStatus =
        prepareForClientWait(context, contextVk, flushCommands, timeout, resultOut);

    if (prepareForClientWaitStatus == angle::Result::Incomplete)
    {
    ...
    return prepareForClientWaitStatus;
}
                                             ...
                                             MapVkResultToApiType mappingFunction)
{
    ...
    angle::Result prepareForClientWaitStatus =
        prepareForClientWait(context, contextVk, flushCommands, timeout, &status);
    ...
    if (prepareForClientWaitStatus == angle::Result::Incomplete)
    {
        ...
        auto clientWaitUnlocked = [renderer, context, mappingFunction, use = mUse,
            ...
            if (!resultOut)
            {
                // Unexpected nullptr, early return
                return;
            }

            VkResult status = VK_INCOMPLETE;
            (void)renderer->waitForResourceUseToFinishWithUserTimeout(context, use, timeout,
                                                                      &status);
            mappingFunction(status, resultOut);
        };
                                                        ...
                                                        MapVkResultToApiType mappingFunction)
{
    ...
    VkResult status      = VK_INCOMPLETE;
    angle::Result result = clientWait(context, contextVk, flushCommands, timeout, &status);
