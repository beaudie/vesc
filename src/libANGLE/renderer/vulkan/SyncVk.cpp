                                     ...
                                     VkResult *outResult)
{
    ...
    else
    {
        ...
        {
            // The sync object wasn't flushed before waiting and without a context we have no way to
            // do it now, so just return to avoid getting stuck.
            WARN() << "clientWaitSync called without flushing sync object and/or valid context "
                      "active.";
            *outResult = VK_TIMEOUT;
            return angle::Result::Continue;
        }
