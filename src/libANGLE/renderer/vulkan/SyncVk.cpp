...
ExternalFence::ExternalFence()
    : mRefCount(1), mFenceFdStatus(VK_INCOMPLETE), mFenceFd(kInvalidFenceFd)
{}
...
SyncHelperNativeFence::SyncHelperNativeFence() : mExternalFence(new ExternalFence()) {}

SyncHelperNativeFence::~SyncHelperNativeFence() {}

void SyncHelperNativeFence::releaseToRenderer(RendererVk *renderer)
{
    if (mExternalFence != nullptr)
    {
        mExternalFence->release(renderer->getDevice());
