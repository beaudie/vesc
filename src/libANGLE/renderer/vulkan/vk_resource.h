...
class SharedGarbageList final : angle::NonCopyable
{
  public:
    ...
    void add(Renderer *renderer, T &&garbage)
    {
        ...
        {
            std::unique_lock<angle::Mutex> enqueueLock(mMutex);
    ...
    {
        std::unique_lock<angle::Mutex> lock(mSubmittedQueueDequeueMutex);
  ...
  private:
    void addGarbageLocked(angle::FixedQueue<T> &queue, T &&garbage)
    {
        ...
        {
            std::unique_lock<angle::Mutex> dequeueLock(mSubmittedQueueDequeueMutex);
    ...
    angle::Mutex mSubmittedQueueDequeueMutex;
