...
void LiteCommandBuffer::updateCurrentCmdData(const ptrType *paramData,
                                           ...
                                           const uint32_t *dynamicOffsets)
{
    ...
    updateCurrentCmdData<VkDescriptorSet>(descriptorSets, &paramStruct->descriptorSets, descSize);
    updateCurrentCmdData<uint32_t>(dynamicOffsets, &paramStruct->dynamicOffsets, offsetSize);
}
                                          ...
                                          const VkDeviceSize *offsets)
{
    ...
    // Copy variable sized data
    updateCurrentCmdData(reinterpret_cast<uint8_t*>(paramStruct) + sizeof(BindVertexBuffersParams), buffSize);
