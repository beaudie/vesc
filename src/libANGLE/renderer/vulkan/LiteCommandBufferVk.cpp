...
void LiteCommandBuffer::updateCurrentCmdData(const uint8_t *paramData, size_t sizeInBytes)
{
    memcpy(mCurrentCmdData, paramData, sizeInBytes);
    mCurrentCmdData += sizeInBytes;
}
                                           ...
                                           const uint32_t *dynamicOffsets)
{
    ...
    updateCurrentCmdData(reinterpret_cast<uint8_t *>(paramStruct) + sizeof(BindDescriptorSetParams),
                         descSize);
    paramStruct->descriptorSets = reinterpret_cast<const VkDescriptorSet *>(mCurrentCmdData);
    updateCurrentCmdData(mCurrentCmdData, offsetSize);
    paramStruct->dynamicOffsets = reinterpret_cast<const uint32_t *>(mCurrentCmdData);
}
