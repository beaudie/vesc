                                                    ...
                                                    bool copyStagingTo)
{
    ...
    size_t srcRowPitch   = (copyStagingTo == true) ? calculateRowPitch() : rowPitch;
    size_t srcSlicePitch = (copyStagingTo == true) ? calculateSlicePitch(srcRowPitch) : slicePitch;
    size_t dstRowPitch   = (copyStagingTo == true) ? rowPitch : calculateRowPitch();
    size_t dstSlicePitch = (copyStagingTo == true) ? slicePitch : calculateSlicePitch(dstRowPitch);
    if (copyStagingTo)
    {
...
angle::Result CLImageVk::create(void *hostPtr)
{
    ...
    if (mMemory.getFlags().intersects(CL_MEM_USE_HOST_PTR | CL_MEM_COPY_HOST_PTR))
    {
        ...
        {
            ANGLE_TRY(copyStagingToFromWithPitch(hostPtr,
                                                 {mExtent.width, mExtent.height, mExtent.depth},
                                                 mDesc.rowPitch, mDesc.slicePitch, false));
        }
        VkBufferImageCopy copyRegion{};
        copyRegion.bufferOffset      = 0;
        copyRegion.bufferRowLength   = 0;
        copyRegion.bufferImageHeight = 0;
        setExtent({mExtent.width, mExtent.height, mExtent.depth}, &copyRegion.imageExtent);
        setOffset({0, 0, 0}, &copyRegion.imageOffset);
        setSubresourceLayers({0, 0, 0}, mDesc.type, &copyRegion.imageSubresource);
...
void CLImageVk::packPixels(const void *fillColor, void *outData)
{
    ...
    switch (mImageFormat.image_channel_data_type)
    {
        ...
        case CL_SIGNED_INT8:
        {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ptr[i] = (char)std::clamp(srcVector[i], -128, 127);
            break;
        }
        case CL_UNSIGNED_INT8:
        {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ptr[i] = (unsigned char)std::clamp(static_cast<unsigned int>(srcVector[i]),
        ...
        case CL_SIGNED_INT16:
        {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ptr[i] = (short)std::clamp(srcVector[i], -32768, 32767);
            break;
        }
        case CL_UNSIGNED_INT16:
        {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ptr[i] = (unsigned short)std::clamp(static_cast<unsigned int>(srcVector[i]),
                                                    static_cast<unsigned int>(0),
...
void CLImageVk::setExtent(const cl::Coordinate &region, VkExtent3D *extent)
{
...
void CLImageVk::setOffset(const cl::MemOffsets &origin, VkOffset3D *offset)
{
...
void CLImageVk::setSubresourceLayers(const cl::MemOffsets &origin,
