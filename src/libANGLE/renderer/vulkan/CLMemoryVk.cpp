...
{
    mContext->mData->mMemories.erase(mMemory.getNative());
}
...
{
    // TODO: For now default is HOST_VISIBLE_BIT. This needs to be adjusted to check different cl
    // flags
...
angle::Result CLBufferVk::create(size_t size)
{
    if (!isSubBuffer())
    {
        auto createInfo = mDefaultBCI;
        createInfo.size = size;
        auto memFlags   = getVkMemPropertyFlags();
        auto res        = mBuffer.init(mContext, createInfo, memFlags);
        ...
        if (mMemory.getFlags().isSet(CL_MEM_USE_HOST_PTR | CL_MEM_COPY_HOST_PTR))
        {
            ...
            ANGLE_CL_IMPL_TRY_ERROR_RET(
                setDataImpl(static_cast<uint8_t *>(mMemory.getHostPtr()), size, 0),
                CL_OUT_OF_RESOURCES);
        }
    }
    return angle::Result::Continue;
...
angle::Result CLBufferVk::map()
{
    ...
    auto res = mBuffer.map(mContext, &mMapPtr);
    if (res == angle::Result::Continue)
    {
        return angle::Result::Continue;
    }
...
angle::Result CLBufferVk::unmap()
{
    if (mParent != nullptr)
    {
        ANGLE_CL_IMPL_TRY_ERROR_RET(mParent->unmap(), CL_OUT_OF_RESOURCES);
        return angle::Result::Continue;
    }
