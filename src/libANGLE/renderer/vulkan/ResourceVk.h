...
class SharedResourceUse final : angle::NonCopyable
{
  public:
    ...
    {
        ASSERT(valid());
        bool validCounter = mUse->counter > 0 && mUse->garbageCounter > 0;
        ASSERT(validCounter);
        // when counter is 1, only decrement counter when garbageCounter is also 1
        if (mUse->counter > 1)
        {
            --mUse->counter;
        }
        ...
        --mUse->garbageCounter;
        if (mUse->counter == 0 && mUse->garbageCounter == 0)
        {
            delete mUse;
        }
        mUse = nullptr;
