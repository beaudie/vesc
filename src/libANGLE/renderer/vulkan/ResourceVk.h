...
class ResourceUse final
{
  public:
    ...
    {
        for (SerialIndex index(0); index < mSerials.size(); ++index)
        {
            if (mSerials[SerialIndex(index)] > lastSubmittedSerials[SerialIndex(index)])
            {
                return true;
            }
        }
    ...
    ANGLE_INLINE bool usedInRunningCommands(const Serials &lastCompletedSerials) const
    {
        for (SerialIndex index(0); index < mSerials.size(); ++index)
        {
            if (mSerials[SerialIndex(index)] > lastCompletedSerials[SerialIndex(index)])
            {
                return true;
            }
        }
    ...
    ANGLE_INLINE void setSerial(SerialIndex index, Serial serial) { mSerials[index] = serial; }
