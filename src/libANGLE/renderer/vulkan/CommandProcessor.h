...
// Main differences:
// - Allows checking/waiting for commands from Context thread, while flushing/submitting in other
// thread.
...
// - getLastSubmittedQueueSerial() will be updated in Context thread, while submitting in other
// thread.
...
#    endif
    ...
    angle::Result doSubmitFrame(Context *context,
                                GarbageList &&currentGarbage,
...
#    endif
  ...
  private:
    ...
    angle::Result checkAndResetCompletedItems(Context *context,
                                              QueueItemIndex *beginIndex,
                                              const QueueItemIndex endIndex,
    ...
    angle::Result getValidPrimaryCommandBuffer(Context *context,
                                               bool hasProtectedContent,
                                               PrimaryCommandBuffer **primaryCommandBufferOut);

    bool checkIfCommandBufferValid(bool hasProtectedContent);

    void collectCurrentGarbage(Context *context, GarbageList &&currentGarbage);
...
class CommandProcessor final : public Context, public CommandQueueInterface
{
  public:
    ...
    std::atomic<Serial> mLastSubmittedQueueSerial2;
...
#endif
    mutable std::mutex mQueueSerialMutex;  // TODO: Is it also for CommandQueue2?
