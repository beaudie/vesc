...
struct CommandBatch final : angle::NonCopyable
{
    ...
    VkResult waitFenceUnlocked(VkDevice device,
                               uint64_t timeout,
                               std::unique_lock<angle::Mutex> *lock) const;
...
class CommandQueue : angle::NonCopyable
{
  ...
  private:
    ...
    angle::Result queueSubmit(Context *context,
                              std::unique_lock<angle::Mutex> &&dequeueLock,
                              egl::ContextPriority contextPriority,
...
class CommandProcessor : public Context
{
  ...
  private:
    ...
    {
        std::lock_guard<angle::Mutex> queueLock(mErrorMutex);
    ...
    mutable angle::Mutex mErrorMutex;
