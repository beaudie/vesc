...
class ProgramPipelineVk : public ProgramPipelineImpl
{
  public:
    ...
    const gl::ShaderBitSet &getShaderStages() const { return mShaderStages; }
    bool hasLinkedShaderStage(gl::ShaderType shaderType) const { return mShaderStages[shaderType]; }
    size_t getLinkedShaderStageCount() const { return mShaderStages.count(); }

    bool relinkShaders(const gl::Context *glContext);
    ...
    {
        vk::ShaderProgramHelper *shaderProgram = &mDefaultShaderInfo.getShaderProgram();
        ProgramVk *program                     = mPrograms[gl::ShaderType::Compute];
        ASSERT(program);
        ANGLE_TRY(program->initComputeShader(contextVk, mDefaultShaderInfo, shaderProgram));
        ASSERT(!shaderProgram->isGraphicsProgram());
        return shaderProgram->getComputePipeline(contextVk, mPipelineLayout.get(), pipelineOut);
    }

    const ProgramVk *getShaderProgram(const gl::State &glState, gl::ShaderType shaderType) const
    {
        gl::ProgramPipeline *pipeline = glState.getProgramPipeline();
        return vk::GetImpl(pipeline->getShaderProgram(shaderType));
    }
