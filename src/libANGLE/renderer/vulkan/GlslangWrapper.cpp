                                     ...
                                     std::string *fragmentSourceOut)
{
    ...
    // See corresponding code in OutputVulkanGLSL.cpp.
    // TODO(syoussefi): Add support for EXT_blend_func_extended.  http://anglebug.com/1085
    ...
    // Assign uniforms to a descriptor set and binding.
    const auto &uniformBlocks = programState.getUniformBlocks();
    for (const gl::InterfaceBlock &uniform : uniformBlocks)
    {
        const std::string setBindingString =
            uniformsDescriptorSet +
            ", binding = " + Str(uniform.binding + kUserUniformBlocksBindingStartIndex);

        InsertSetBindingSpecifierString(&vertexSource, uniform.name, setBindingString);
        InsertQualifierSpecifierString(&vertexSource, uniform.name, kUniformQualifier);
        InsertSetBindingSpecifierString(&fragmentSource, uniform.name, setBindingString);
        InsertQualifierSpecifierString(&fragmentSource, uniform.name, kUniformQualifier);
    }
