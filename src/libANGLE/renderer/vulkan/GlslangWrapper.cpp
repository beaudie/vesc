                                                ...
                                                gl::ShaderMap<std::vector<uint32_t>> *shaderCodeOut)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (shaderType == gl::ShaderType::Fragment && contextVk->emulateSeamfulCubeMapSampling())
        {
            // Enable SPIR-V 1.3 for shaders that use this workaround, as they use subgroup
            // operations.  This is not enabled everywhere simply because renderdoc doesn't yet
            // have support for disassembling shaders in this version, so it could hinder
            // debuggability.
