...
void AssignResourceBinding(uint32_t globalBindingOffset,
                           ...
                           gl::ShaderMap<uint32_t> *stageBindingIndices)
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (shaderSource)
        {
            uint32_t bindingIndex = (stageBindingIndices)[shaderType] + stageBindingOffset;

            const std::string setBindingString = descriptorSet + ", binding = " + Str(bindingIndex);
                                  ...
                                  const gl::ShaderMap<IntermediateShaderSource *> &shaderSources)
{
    ...
    {
        AssignResourceBinding(globalBindingOffset, buffersDescriptorSet, block.name, qualifier,
                          ...
                          const gl::ShaderMap<IntermediateShaderSource *> &shaderSources)
{
...

    {
        ...
        AssignInterfaceBlockBindings(programState.getUniformBlocks(), globalBindingOffset,
                                     kUniformQualifier, activeUniformBlockCounts, shaderSources);
...

    {
        const gl::ShaderMap<uint32_t> &activeStorageBlockCounts =
            programState.getActiveStorageBlockCounts();
        AssignInterfaceBlockBindings(programState.getShaderStorageBlocks(), globalBindingOffset,
                                     kSSBOQualifier, shaderSources);
                           ...
                           const gl::ShaderMap<IntermediateShaderSource *> &shaderSources)
{
    ...
    for (unsigned int uniformIndex : programState.getSamplerUniformRange())
    {
        ...
        AssignResourceBinding(0, textureDescriptorSet, samplerName, kUniformQualifier,
                              activeSamplerCounts, shaderSources, &stageBindingIndices);
    }
}
                           ...
                           const gl::ShaderMap<IntermediateShaderSource *> &shaderSources)
{
    ...
    for (const std::string &unusedInterfaceBlock : resources.unusedInterfaceBlocks)
    {
        for (IntermediateShaderSource *shaderSource : shaderSources)
        {
            if (shaderSource)
            {
                shaderSource->eraseLayoutAndQualifierSpecifiers(unusedInterfaceBlock, "struct");
            }
        }
    }
    ...
    for (const gl::UnusedUniform &unusedUniform : resources.unusedUniforms)
    {
        if (unusedUniform.isSampler)
        {
            ...
            for (IntermediateShaderSource *shaderSource : shaderSources)
            {
                if (shaderSource)
                {
                    shaderSource->insertLayoutSpecifier(uniformName, layoutString);
                    shaderSource->insertQualifierSpecifier(uniformName, kUniformQualifier);
                }
            }
        }
        else
        {
            for (IntermediateShaderSource *shaderSource : shaderSources)
            {
                if (shaderSource)
                {
                    shaderSource->eraseLayoutAndQualifierSpecifiers(unusedUniform.name, "");
                }
            }
        }
    }
