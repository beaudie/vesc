                            ...
                            IntermediateShaderSource *inStageSource)
{
    ...
    for (const gl::PackedVaryingRegister &varyingReg : resources.varyingPacking.getRegisterList())
    {
        ...
        {
            auto found = std::find(programState.getTransformFeedbackVaryingNames().begin(),
                                   programState.getTransformFeedbackVaryingNames().end(), name);
            if (found == programState.getTransformFeedbackVaryingNames().end())
                continue;
        }
