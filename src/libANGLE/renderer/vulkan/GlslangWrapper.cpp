                                       ...
                                       const std::string &fragmentSource)
{
    ...
    std::array<int, 2> lengths          = {static_cast<int>(vertexSource.length()),
                                  static_cast<int>(fragmentSource.length())};

    glslang::TShader vertexShader(EShLangVertex);
    vertexShader.setStringsWithLengths(&strings[0], &lengths[0], 1);
    vertexShader.setEntryPoint("main");
    bool vertexResult = vertexShader.parse(&glslang::DefaultTBuiltInResource, 100, EEsProfile,
                                           false, false, EShMsgVulkanRules);
    ASSERT(vertexResult);

    glslang::TShader fragmentShader(EShLangFragment);
    fragmentShader.setStringsWithLengths(&strings[1], &lengths[1], 1);
    fragmentShader.setEntryPoint("main");
    bool fragmentResult = fragmentShader.parse(&glslang::DefaultTBuiltInResource, 100, EEsProfile,
                                               false, false, EShMsgVulkanRules);
    ...
    bool linkResult = program.link(EShMsgVulkanRules);
    ASSERT(linkResult);
    ...
    glslang::GlslangToSpv(*vertexStage, vertexSpirv);
    glslang::GlslangToSpv(*fragmentStage, fragmentSpirv);

    return LinkResult(true, gl::NoError());
}
