                                                   ...
                                                   const std::string &bufferIndex)
{
    ...
    for (size_t arrayIndex = arrayIndexStart; arrayIndex < arrayIndexEnd; ++arrayIndex)
    {
        for (uint32_t col = 0; col < info.dimensions[0]; ++col)
        {
            ...
            {
                fprintf(stderr, "%s vs %s\n", varying.name.c_str(), varying.mappedName.c_str());
                result += "xfbOut" + bufferIndex + "[gl_VertexIndex * " + Str(stride) + " + " +
                          Str(offset) + "] = " + info.asFloat + "(" + varying.name;

                if (varying.isArray())
                {
                    result += "[" + Str(arrayIndex) + "]";
                }
                                     ...
                                     std::string *fragmentSourceOut)
{
    ...
    if (programState.getLinkedTransformFeedbackVaryings().size() > 0)
    {
