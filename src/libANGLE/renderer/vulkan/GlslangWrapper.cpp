                                           ...
                                           std::vector<uint32_t> *fragmentCodeOut)
{
    ...
    for (const gl::LinkedUniform &linkedUniform : programState.getUnusedUniforms())
    {
        if (!linkedUniform.isSampler())
        {
            ...
            InsertQualifierSpecifierString(&vertexSource, linkedUniform.name, "");
            InsertQualifierSpecifierString(&fragmentSource, linkedUniform.name, "");
        }
        else
        {
            // Sampler types can only be used in uniform variables or function parameters,
            // if they are unused, we must remove the declaration entirely.
            RemoveEntireDeclaration(&vertexSource, linkedUniform.name);
            RemoveEntireDeclaration(&fragmentSource, linkedUniform.name);
        }
    }
