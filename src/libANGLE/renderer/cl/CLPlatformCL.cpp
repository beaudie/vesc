...
const ExtensionSet &GetSupportedExternsions()
{
...
std::unique_ptr<CLPlatformCL> CLPlatformCL::Create(cl_platform_id platform)
{
    ...
    if (!extensions.empty())
    {
        auto &supported                 = GetSupportedExternsions();
        std::string::size_type extStart = 0u;
        do
        {
            const std::string::size_type spacePos = extensions.find(' ', extStart);
            const bool foundSpace                 = spacePos != std::string::npos;
            const std::string::size_type length =
                (foundSpace ? spacePos : extensions.length()) - extStart;
            if (supported.find(extensions.substr(extStart, length)) != supported.cend())
            {
    ...
    if (result == CL_SUCCESS)
    {
        ...
        // Filter out extensions which are not (yet) supported to be passed through
        auto &supported = GetSupportedExternsions();
        auto extIt      = extList.cbegin();
        while (extIt != extList.cend())
        {
