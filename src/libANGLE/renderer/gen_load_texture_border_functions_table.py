...
def get_unknown_load_func(angle_format_func, internal_format):
    assert angle_format_unknown in angle_format_func
    return get_load_func(
        unknown_func_name(internal_format), angle_format_func[angle_format_unknown])


def parse_json(json_data):
    table_data = ''
    load_functions_data = ''
    for internal_format, angle_format_func in sorted(json_data.items()):
        ...
        if angle_format_unknown in angle_format_func:
            table_data += s + '    return ' + unknown_func_name(internal_format) + ';\n'
            load_functions_data += get_unknown_load_func(angle_format_func, internal_format)
        else:
