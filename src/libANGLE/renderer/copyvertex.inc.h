...
inline void CopyTo32FVertexData(const uint8_t *input, size_t stride, size_t count, uint8_t *output)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        for (size_t j = 0; j < inputComponentCount; j++)
        {
            if (normalized)
            {
                priv::CopyNormToFloatClamp<T>(&offsetInput[j], &offsetOutput[j], NL::max());
            }
            else
            {
                                         ...
                                         uint8_t *output)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        priv::CopyPackedRGB<isSigned, normalized, true, false>(
            (packedValue >> redShift) & rgbMask, offsetOutput + (0 * outputComponentSize));
        priv::CopyPackedRGB<isSigned, normalized, true, false>(
            (packedValue >> greenShift) & rgbMask, offsetOutput + (1 * outputComponentSize));
        priv::CopyPackedRGB<isSigned, normalized, true, false>(
            (packedValue >> blueShift) & rgbMask, offsetOutput + (2 * outputComponentSize));
        priv::CopyPackedAlpha<isSigned, normalized, true, false>(
                                         ...
                                         uint8_t *output)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        for (size_t j = 0; j < inputComponentCount; j++)
        {
            if (normalized)
            {
                priv::CopyNormToFloatScale<T>(&offsetInput[j], &offsetOutput[j], NL::max());
            }
            else
            {
                offsetOutput[j] = static_cast<float>(offsetInput[j]);
                                                  ...
                                                  uint8_t *output)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        priv::CopyPackedRGB<isSigned, normalized, true, true>(
            (packedValue >> redShift) & rgbMask, offsetOutput + (0 * outputComponentSize));
        priv::CopyPackedRGB<isSigned, normalized, true, true>(
            (packedValue >> greenShift) & rgbMask, offsetOutput + (1 * outputComponentSize));
        priv::CopyPackedRGB<isSigned, normalized, true, true>(
            (packedValue >> blueShift) & rgbMask, offsetOutput + (2 * outputComponentSize));
        priv::CopyPackedAlpha<isSigned, normalized, true, true>(
            alphaDefaultValueBits, offsetOutput + (3 * outputComponentSize));
    }
}
