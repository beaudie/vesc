...
inline const T *GetAlignedOffsetInput(const T *offsetInput, T *alignedElement)
{
    ...
    {
        // Applications may pass in arbitrarily aligned buffer to be read here.
        // On certain architectures, have restrictions regarding unaligned reads, specifically, we
        // crash on armeabi-v7a devices with a SIGBUS error when performing such operations. arm64
        // and x86-64 devices do not appear to have such issues.
        //
        // The workaround is to detect if the input buffer is unaligned and if so, perform a
        // byte-wise copy of the head and a memcpy of the remaning portion of the unalinged buffer.
