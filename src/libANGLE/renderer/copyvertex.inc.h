...
inline void CopyTo32FVertexData(const uint8_t *input, size_t stride, size_t count, uint8_t *output)
{
    ...
    typedef typename std::conditional<toHalf, GLushort, float>::type outputType;
...
static inline void CopyPackedRGB(uint32_t data, uint8_t *output)
{
    ...
    if (toFloat || toHalf)
    {
        ...
        if (toHalf)
        {
            *reinterpret_cast<GLushort *>(output) = gl::float32ToFloat16(finalValue);
        }
        else
        {
