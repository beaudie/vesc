...
void ShaderInterfaceVariableInfoMap::clear()
{
    ...
    {
        for (VariableNameToInfoArray &typeArray : mData[shaderType])
        {
            typeArray.clear();
        }
...
ShaderInterfaceVariableInfo &ShaderInterfaceVariableInfoMap::addWithIndex(
    gl::ShaderType shaderType,
    ShaderVariableType variableType,
    const std::string &variableName,
...
{
    ASSERT(!hasVariable(shaderType, variableName));
    mNameToTypeAndIndexMap[shaderType][variableName] = {variableType, resourceIndex};
    if (mData[shaderType][variableType].size() <= resourceIndex + 1)
    {
        mData[shaderType][variableType].resize(resourceIndex + 1);
    }
    return mData[shaderType][variableType][resourceIndex];
}
