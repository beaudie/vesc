...
{
    // Some drivers do not orphan on respecification calls such as glTexImage2D with the same size
    // and format.  Force the orphaning by respecifying the source level of the source texture to
    // have zero size temporarily.
    if (sibling == mState.source.get() && egl::IsTextureTarget(mTarget) &&
        GetWorkaroundsGL(context).forceEGLImageTextureOrphaning)
    {
        const FunctionsGL *functionsGL = GetFunctionsGL(context);
        StateManagerGL *stateManager   = GetStateManagerGL(context);

        const TextureGL *textureGL = GetImplAs<TextureGL>(GetAs<gl::Texture>(mState.source.get()));
        stateManager->bindTexture(textureGL->getType(), textureGL->getTextureID());

        GLenum target = ToGLenum(mState.imageIndex.getTarget());
        GLint level   = mState.imageIndex.getLevelIndex();

        if (mState.imageIndex.has3DLayer())
        {
            functionsGL->texImage3D(target, level, GL_RGBA, 0, 0, 0, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                    nullptr);
        }
        else
        {
            functionsGL->texImage2D(target, level, GL_RGBA, 0, 0, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                    nullptr);
        }
    }
