...
{
    mSize.width  = mAttribs.get(EGL_WIDTH, 0);
    mSize.height = mAttribs.get(EGL_HEIGHT, 0);
    mSize.depth  = 1;
...
void DmaBufImageSiblingEGL::getImageCreationAttributes(std::vector<EGLint> *outAttributes)
{
    EGLenum kForwardedAttribs[] = {
        EGL_WIDTH,
        EGL_HEIGHT,
    ...
    for (EGLenum forwardedAttrib : kForwardedAttribs)
    {
        if (mAttribs.contains(forwardedAttrib))
        {
            ...
            outAttributes->push_back(mAttribs.get(forwardedAttrib));
        }
    }
}
