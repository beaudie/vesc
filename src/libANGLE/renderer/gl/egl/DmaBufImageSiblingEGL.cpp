...
DmaBufImageSiblingEGL::DmaBufImageSiblingEGL(const egl::AttributeMap &attribs)
    : mAttribs(attribs), mFormat(GL_NONE), mYUV(false), mIsProtectedMemory(false)
{
    ...
    mSize.depth        = 1;
    mIsProtectedMemory = static_cast<bool>(mAttribs.getAsInt(EGL_PROTECTED_CONTENT_EXT, EGL_FALSE));
...
bool DmaBufImageSiblingEGL::isProtectedMemory() const
{
