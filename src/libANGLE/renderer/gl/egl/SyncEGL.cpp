...
egl::Error SyncEGL::set(const egl::Display *display, EGLenum type)
{
    ...
    mSync = mFunctions->createSyncKHR(type, nullptr);
    if (mSync == nullptr)
    {
                               ...
                               EGLint *outResult)
{
    ...
    EGLint result = mFunctions->clientWaitSyncKHR(mSync, flags, timeout);
...
{
    ASSERT(mSync != EGL_NO_SYNC);
    EGLint result = mFunctions->waitSyncKHR(mSync, flags);
...
egl::Error SyncEGL::getStatus(const egl::Display *display, EGLAttrib *outStatus)
{
    ASSERT(mSync != EGL_NO_SYNC);
    EGLBoolean result = mFunctions->getSyncAttrib(mSync, EGL_SYNC_STATUS, outStatus);
