...
DisplayAndroid::DisplayAndroid(const egl::DisplayState &state)
    : DisplayEGL(state),
      ...
      mCurrentDrawSurface(EGL_NO_SURFACE),
      mCurrentReadSurface(EGL_NO_SURFACE),
      mCurrentContext(EGL_NO_CONTEXT)
{
}
...
ContextImpl *DisplayAndroid::createContext(const gl::ContextState &state)
{
    ...
    return new ContextEGL(state, newRenderer);
}
                                       ...
                                       gl::Context *context)
{
    ...
    if (mCurrentDrawSurface != newDrawSurface || mCurrentReadSurface != newReadSurface ||
        mCurrentContext != newContext)
    {
        ...
        mCurrentDrawSurface = newDrawSurface;
        mCurrentReadSurface = newReadSurface;
        mCurrentContext     = newContext;
    }
    ...
    bool makeNewContextCurrent)
{
    ...
    ANGLE_TRY_RESULT(createContext(displayAttributes));

    if (mEGL->makeCurrent(mDummyPbuffer, mDummyPbuffer, newContext) == EGL_FALSE)
    {
        return egl::Error(mEGL->getError(), "eglMakeCurrent failed");
    }
    ...
    {
        mCurrentDrawSurface = mDummyPbuffer;
        mCurrentReadSurface = mDummyPbuffer;
        mCurrentContext     = context;
    }
    else
    {
        if (mEGL->makeCurrent(mCurrentDrawSurface, mCurrentReadSurface, mCurrentContext) ==
            EGL_FALSE)
        {
            return egl::Error(mEGL->getError(), "eglMakeCurrent failed");
        }
