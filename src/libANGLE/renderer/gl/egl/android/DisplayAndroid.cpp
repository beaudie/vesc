                                       ...
                                       gl::Context *context)
{
    ...
    if (currentContext.isExternalContext || isExternalContext)
    {
        ...
        {
            ERR() << "EEEE makeCurrent newContext=" << newContext;
            // Not call makeCurrent() for now, since chrome doesn't need to change the EGLSurface
            // during compositing.
            // TODO: supporint change EGLSurface. save the current FBO and bind the restored FBO
            // when chrome bind FBO0. if (mEGL->makeCurrent(newSurface, newContext) == EGL_FALSE)
            // {
