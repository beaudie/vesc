                                           ...
                                           const egl::AttributeMap &attribs)
{
    ...
    bool saveState = attribs.get(EGL_EXTERNAL_CONTEXT_SAVE_STATE_ANGLE, EGL_FALSE) == EGL_TRUE;
...
SurfaceImpl *DisplayAndroid::createPbufferFromClientBuffer(const egl::SurfaceState &state,
                                                           ...
                                                           const egl::AttributeMap &attribs)
{
    ASSERT(buftype == EGL_EXTERNAL_SURFACE_ANGLE);
...
{
    if (buftype != EGL_EXTERNAL_SURFACE_ANGLE || clientBuffer != nullptr)
    {
        // TODO
    }
    return egl::NoError();
                                       ...
                                       gl::Context *context)
{
    ...
    if (context)
    {
        ...
        isExternalContext      = contextEGL->isExternal();
    ...
    if (currentContext.isExternalContext || isExternalContext)
    {
        if (!currentContext.isExternalContext)
        {
            ...
            // We only supports using external surface with external context.
