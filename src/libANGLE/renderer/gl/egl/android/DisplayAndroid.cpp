...
static constexpr bool kDefaultEGLVirtualizedContexts = false;
...
egl::Error DisplayAndroid::initialize(egl::Display *display)
{
    mVirtualizedContexts =
        ShouldUseVirtualizedContexts(display->getAttributeMap(), kDefaultEGLVirtualizedContexts);
...
void DisplayAndroid::destroyNativeContext(EGLContext context)
{
    ...
    for (auto &currentContext : mCurrentNativeContext)
    {
        ...
        {
            currentContext.second.surface = nullptr;
            currentContext.second.context = nullptr;
        }
    }
}
                                          ...
                                          std::shared_ptr<RendererEGL> *outRenderer)
{
    ...
    else
    {
        ...
        if (mEGL->makeCurrent(currentContext.context, currentContext.surface) == EGL_FALSE)
        {
            return egl::EglNotInitialized()
                   << "eglMakeCurrent failed with " << egl::Error(mEGL->getError());
        }
