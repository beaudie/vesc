...
egl::SupportedCompositorTimings SurfaceEGL::getSupportedCompositorTimings() const
{
    ...
    for (egl::CompositorTiming name : AllEnums<egl::CompositorTiming>())
    {
        result[name] = mEGL->getCompositorTimingSupportedANDROID(mSurface, name);
    }
    return result;
}
...
egl::SupportedTimestamps SurfaceEGL::getSupportedTimestamps() const
{
    ...
    for (egl::Timestamp timestamp : AllEnums<egl::Timestamp>())
    {
        result[timestamp] = mEGL->getFrameTimestampSupportedANDROID(mSurface, timestamp);
    }
    return result;
}
