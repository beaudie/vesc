...
DisplayEGL::DisplayEGL(const egl::DisplayState &state)
    : DisplayGL(state), mRenderer(nullptr), mEGL(nullptr)
{}
                                         ...
                                         native_egl::AttributeVector *outAttribs) const
{
    ...
    {
        context = mEGL->createContext(EGL_NO_CONFIG_KHR, shareContext, attribList.data());
...
egl::Error DisplayEGL::initialize(egl::Display *display)
{
    ...
    ANGLE_TRY(egl->initialize(display->getNativeDisplayId(), "libEGL.so.1", eglHandle));
...
{
    EGLContext context =
        mEGL->createContext(EGL_NO_CONFIG_KHR, sharedContext, workerAttribs.data());
    if (context == EGL_NO_CONTEXT)
    {
