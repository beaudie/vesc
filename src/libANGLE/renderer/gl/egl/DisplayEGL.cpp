...
egl::Error DisplayEGL::initialize(egl::Display *display)
{
    ...
    {
        baseConfigAttribs.insert(EGL_SURFACE_TYPE, surfaceType);
        configAttribsWithFormat.insert(EGL_SURFACE_TYPE, surfaceType);
        ...
        for (EGLint renderableType : renderableTypes)
        {
            ...
            EGLint numConfig = 0;
            if (mEGL->chooseConfig(attribVector.data(), &configWithFormat, 1, &numConfig) ==
                EGL_TRUE)
            {
                break;
            }
    ...
    // Unset any context made during initialization
    ANGLE_TRY(makeCurrent(display, nullptr, nullptr, nullptr));
                                       ...
                                       const egl::AttributeMap &attribs)
{
    ...
    {
        renderer = mVirtualizationGroups[virtualizationGroup];
        if (!renderer)
        {
