...
#include "libANGLE/renderer/gl/egl/cros/WindowSurfaceCros.h"
...
DisplayDRM::Buffer::Buffer(DisplayDRM *display,
...
static int ChooseCRTC(int fd, drmModeConnectorPtr conn)
{
...
void DisplayDRM::pageFlipHandler(int fd,
...
void DisplayDRM::drawWithBlit(Buffer *buffer)
{
    ...
    mFunctionsGL->blitFramebuffer(0, 0, buffer->mWidth, buffer->mHeight,
                                  buffer->mNative->x,
                                  buffer->mNative->y,
                                  buffer->mNative->x + buffer->mWidth,
                                  buffer->mNative->y + buffer->mHeight,
...
EGLContext DisplayDRM::initializeContext(EGLConfig config, const egl::AttributeMap &eglAttributes)
{
...
egl::Error DisplayDRM::waitNative(EGLint engine,
