...
void DisplayOzone::drawBuffer(Buffer *buffer)
{
    if (!mDrawing)
    {
        ...
        sm->setColorMask(true, true, true, true);
        sm->setDepthMask(true);
        mFunctionsGL->gl.clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    }
...
void DisplayOzone::terminate()
{
    ...
    if (mProgram)
    {
        mFunctionsGL->gl.deleteProgram(mProgram);
        mFunctionsGL->gl.deleteShader(mVertexShader);
        mFunctionsGL->gl.deleteShader(mFragmentShader);
        mFunctionsGL->gl.deleteBuffers(1, &mVertexBuffer);
        mFunctionsGL->gl.deleteBuffers(1, &mIndexBuffer);
        mProgram = 0;
    }
