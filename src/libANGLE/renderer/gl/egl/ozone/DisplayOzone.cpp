...
uint32_t DisplayOzone::Buffer::getDRMFB()
{
    if (!mHasDRMFB)
    {
        ...
        {
            WARN() << "drmModeAddFB2 failed: " << errno << " " << strerror(errno) << std::endl;
        }
        else
        {
            mHasDRMFB = true;
...
void DisplayOzone::presentScreen()
{
    ...
    if (mPending)
    {
        ...
        if (poll(&pfd, 1, 0) < 0)
        {
            WARN() << "poll failed: " << errno << " " << strerror(errno) << std::endl;
        }
    ...
    if (!mPending && mDrawing)
    {
        ...
        if (mSetCRTC)
        {
            ...
            {
                WARN() << "set crtc failed: " << errno << " " << strerror(errno) << std::endl;
            }
            mSetCRTC = false;
        }
        if (drmModePageFlip(fd, mCRTC->crtc_id, mDrawing->getDRMFB(), DRM_MODE_PAGE_FLIP_EVENT,
                            this))
        {
            WARN() << "page flip failed: " << errno << " " << strerror(errno) << std::endl;
        }
        mPending = mDrawing;
        mDrawing = nullptr;
    }
...
GLuint DisplayOzone::makeShader(GLuint type, const char *src)
{
    ...
    if (compiled != GL_TRUE)
    {
        WARN() << "DisplayOzone shader compilation error: " << buf << std::endl;
    }
