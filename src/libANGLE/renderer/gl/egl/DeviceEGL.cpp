...
void DeviceEGL::generateExtensions(egl::DeviceExtensions *outExtensions) const
{
    ...
    if (std::find(extensions.begin(), extensions.end(), "EGL_EXT_device_drm") != extensions.end())
    {
        outExtensions->deviceDrmExt = true;
    }

    if (std::find(extensions.begin(), extensions.end(), "EGL_EXT_device_drm_render_node") !=
        extensions.end())
    {
