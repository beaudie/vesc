...
bool DisplayGbm::Buffer::resize(int32_t width, int32_t height)
{
    ...
    ANGLE_SWALLOW_ERROR(sm->bindRenderbuffer(nullptr, GL_RENDERBUFFER, mColorBuffer));
    gl->eGLImageTargetRenderbufferStorageOES(GL_RENDERBUFFER, mImage);

    if (mDepthBits || mStencilBits)
    {
        ANGLE_SWALLOW_ERROR(sm->bindRenderbuffer(nullptr, GL_RENDERBUFFER, mDSBuffer));
...
GLuint DisplayGbm::Buffer::getTexture()
{
    ...
    ANGLE_SWALLOW_ERROR(sm->bindTexture(nullptr, gl::TextureType::_2D, mTexture));
...
GLuint DisplayGbm::Buffer::createGLFB(const gl::Context *context)
{
    ...
    ANGLE_SWALLOW_ERROR(tateManager->bindFramebuffer(nullptr, GL_FRAMEBUFFER, framebuffer));
...
void DisplayGbm::drawWithTexture(const gl::Context *context, Buffer *buffer)
{
    ...
    if (!mProgram)
    {
        ...
        ANGLE_SWALLOW_ERROR(sm->useProgram(context, mProgram));
        ...
        ANGLE_SWALLOW_ERROR(sm->bindBuffer(context, gl::BufferBinding::Array, mVertexBuffer));
        ...
        ANGLE_SWALLOW_ERROR(sm->bindBuffer(context, gl::BufferBinding::ElementArray, mIndexBuffer));
    ...
    ANGLE_SWALLOW_ERROR(sm->activeTexture(context, 0));
    GLuint tex = buffer->getTexture();
    ANGLE_SWALLOW_ERROR(sm->bindTexture(context, gl::TextureType::_2D, tex));
    gl->vertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
    gl->enableVertexAttribArray(0);
    GLuint fbo = mDrawing->createGLFB(context);
    ANGLE_SWALLOW_ERROR(sm->bindFramebuffer(context, GL_DRAW_FRAMEBUFFER, fbo));
    gl->drawArrays(GL_TRIANGLE_STRIP, 0, 4);
    gl->drawElements(GL_TRIANGLE_STRIP, 10, GL_UNSIGNED_INT, 0);
    ANGLE_SWALLOW_ERROR(sm->deleteTexture(context, tex));
    ANGLE_SWALLOW_ERROR(sm->deleteFramebuffer(context, fbo));
}
...
void DisplayGbm::drawBuffer(const gl::Context *context, Buffer *buffer)
{
    if (!mDrawing)
    {
        ...
        GLuint fbo = mDrawing->createGLFB(context);
        ANGLE_SWALLOW_ERROR(Framebuffer(nullptr, GL_DRAW_FRAMEBUFFER, fbo));
