...
#define ANGLE_GET_PROC_OR_ERROR(MEMBER, NAME)                                           \
    ...
    if (drmRenderNodeDeviceId != 0)
    {
        // If a native device ID was provided, try looking for that specific device first.
        mEGLDisplay = getNativeDisplay(&minorVersion, &majorVersion, drmRenderNodeDeviceId);
    }
...
EGLDisplay FunctionsEGL::getNativeDisplay(int *major, int *minor, dev_t drmRenderNodeDeviceId)
{
    ...
    // - EGL_EXT_device_query for eglQueryDeviceStringEXT if drmRenderNodeDeviceId is non-zero
    ...
    for (EGLint i = 0; i < numDevices; i++)
    {
        if (drmRenderNodeDeviceId != 0)
        {
            ...
            if (stat(renderNodePath, &sb) < 0)
                continue;
            if (sb.st_rdev != drmRenderNodeDeviceId)
                continue;
        }
