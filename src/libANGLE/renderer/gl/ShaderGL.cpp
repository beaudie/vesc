...
class WaitableCompileEventWorkerContext final : public WaitableCompileEvent
{
  public:
    ...
    bool postTranslate(std::string *infoLog) override
    {
        ...
        {
            ShHandle handle = mCompilerInstance.getHandle();
...
{
  public:
    WaitableCompileEventNativeParallel(gl::ShCompilerInstance &&compilerInstance,
                                       PostTranslateFunctor &&postTranslateFunctor,
                                       bool result,
        ...
        : WaitableCompileEvent(std::shared_ptr<angle::WaitableEventDone>(),
                               std::shared_ptr<angle::WorkerThreadPool>(),
                               std::move(compilerInstance)),
          mPostTranslateFunctor(std::move(postTranslateFunctor)),
          mResult(result),
...
{
  public:
    WaitableCompileEventDone(gl::ShCompilerInstance &&compilerInstance,
                             PostTranslateFunctor &&postTranslateFunctor,
                             bool result)
                                                        ...
                                                        ShCompileOptions options)
{
    ...
    if (mRenderer->hasNativeParallelCompile())
    {
        ...
        bool result     = sh::Compile(handle, &srcStrings[0], srcStrings.size(), options);
        if (result)
        {
            ...
            return std::make_shared<WaitableCompileEventNativeParallel>(
                std::move(compilerInstance), std::move(postTranslateFunctor), result,
                std::move(checkShaderFunctor), std::move(peekCompletionFunctor));
        }
        else
        {
            return std::make_shared<WaitableCompileEventDone>(
                std::move(compilerInstance), [](std::string *) { return true; }, result);
        }
    }
    else if (workerThreadPool->isAsync())
    {
        ...
        return std::make_shared<WaitableCompileEventWorkerContext>(
            ...
            std::move(compilerInstance), std::move(compileAndCheckShaderFunctor),
    ...
    else
    {
        ...
        return std::make_shared<WaitableCompileEventDone>(std::move(compilerInstance),
                                                          std::move(postTranslateFunctor), result);
    }
}
