...
class WaitableCompileEventWorkerContext final : public WaitableCompileEvent
{
  public:
                       ...
                       UpdateShaderStateFunctor &&updateShaderStateFunctor) override
    {
        ...
        updateShaderStateFunctor(compiled, mTranslateTask->getHandle());
        if (compiled)
        {
...
class WaitableCompileEventNativeParallel final : public WaitableCompileEvent
{
  public:
    ...
    bool postTranslate(std::string *infoLog,
                       UpdateShaderStateFunctor &&updateShaderStateFunctor) override
    {
        updateShaderStateFunctor(mResult, mHandle);
        return mResult && mPostTranslateFunctor(infoLog);
    }
...
{
  public:
    WaitableCompileEventDone(PostTranslateFunctor &&postTranslateFunctor,
                             bool result,
                             ShHandle handle)
    ...
    if (mRenderer->hasNativeParallelCompile())
    {
        ...
        if (result)
        {
            ...
            return std::make_shared<WaitableCompileEventNativeParallel>(
                std::move(postTranslateFunctor), result, handle, std::move(checkShaderFunctor),
