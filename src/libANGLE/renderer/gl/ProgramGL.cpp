...
void ProgramGL::setPathFragmentInputGen(const std::string &inputName, GLenum genMode,
    GLint components, const GLfloat *coeffs)
{
    for (const auto &input : mPathRenderingFragmentInputs)
    {
        ...
        {
            mFunctions->programPathFragmentInputGenNV(mProgramID, input.location, genMode, components, coeffs);
            return;
        }
    }
    UNREACHABLE();
...
void ProgramGL::postLink()
{
    ...
    mFunctions->getProgramInterfaceiv(mProgramID, GL_FRAGMENT_INPUT_NV, GL_ACTIVE_RESOURCES, &numFragmentInputs);
    if (numFragmentInputs <= 0)
        return;

    GLint maxNameLength = 0;
    mFunctions->getProgramInterfaceiv(mProgramID, GL_FRAGMENT_INPUT_NV, GL_MAX_NAME_LENGTH, &maxNameLength);
