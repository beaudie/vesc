                               ...
                               gl::InfoLog &infoLog)
{
    ...
    {
        const auto &allVaryings = mState.getAttachedVertexShader()->getVaryings(context);

        // Set the transform feedback state
        std::vector<std::string> transformFeedbackVaryingMappedNames;
        for (const auto &tfVarying : mState.getTransformFeedbackVaryingNames())
        {
            auto bracketIndex = tfVarying.find("[");
            if (bracketIndex != std::string::npos)
            {
                auto tfVaryingVariableName = tfVarying.substr(0, bracketIndex);
                for (const auto &varying : allVaryings)
                {
                    if (varying.name == tfVaryingVariableName)
                    {
                        std::string mappedNameWithArrayIndex =
                            varying.mappedName + tfVarying.substr(bracketIndex);
                        transformFeedbackVaryingMappedNames.push_back(mappedNameWithArrayIndex);
                        break;
                    }
                }
            }
            else
            {
                for (const auto &varying : allVaryings)
                {
                    if (varying.name == tfVarying)
                    {
                        transformFeedbackVaryingMappedNames.push_back(varying.mappedName);
                        break;
                    }
                }
            }
