...
void ProgramGL::postLink()
{
    ...
    std::vector<std::vector<GLuint>> uniformToRealLocation;
    uniformToRealLocation.resize(uniforms.size());

    for (size_t uniformId = 0; uniformId < uniforms.size(); ++uniformId)
    {
        const gl::LinkedUniform &linkedUniform = uniforms[uniformId];
        uniformToRealLocation[uniformId].resize(linkedUniform.elementCount(), 0);
    }
    ...
    for (size_t uniformLocation = 0; uniformLocation < uniformLocations.size(); uniformLocation++)
    {
        ...
        uniformToRealLocation[entry.index][entry.element] = realLocation;
    }
    ...
    for (size_t uniformId = 0; uniformId < uniforms.size(); ++uniformId)
    {
        ...
        else if (linkedUniform.isImage())
        {
            ...
            imageBinding.boundImageUnits.resize(linkedUniform.elementCount(), 0);

            for (size_t arrayIndex = 0; arrayIndex < linkedUniform.elementCount(); ++arrayIndex)
            {
                GLuint uniformRealLocation = uniformToRealLocation[uniformId][arrayIndex];
                GLuint imageBindingValue   = 0;
                mFunctions->getUniformuiv(mProgramID, uniformRealLocation, &imageBindingValue);
                imageBinding.boundImageUnits[arrayIndex] = imageBindingValue;
            }
