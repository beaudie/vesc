...
using LinkImplFunctor = std::function<void()>;
class ProgramGL::LinkTask final : public angle::Closure
{
  public:
    LinkTask(LinkImplFunctor &&functor) : mLinkImplFunctor(functor) {}

    void operator()() override { mLinkImplFunctor(); }
                                           ...
                                           gl::InfoLog &infoLog)
{
    ...
    bool isWorkerThread = workerPool->isAsync();
    auto linkTask       = std::make_shared<LinkTask>([this, isWorkerThread]() {
        std::string infoLog;
        bool workerBound = false;
        if (isWorkerThread)
        {
            workerBound = mRenderer->bindWorkerContext(&infoLog);
#if !defined(NDEBUG)
            if (!workerBound)
            {
        ...
        // Make sure it has actually linked.
