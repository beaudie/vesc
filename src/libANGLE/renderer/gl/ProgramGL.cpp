                                       ...
                                       std::vector<gl::LinkedUniform> *uniforms)
{
    ...
    for (size_t i = 0; i < uniformBlocks->size(); i++)
    {
        ...
        mFunctions->getActiveUniformBlockiv(
            mProgramID, blockIndex, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER, &referencedByVS);
        uniformBlock.vertexStaticUse = referencedByVS != GL_FALSE;

        GLint referencedByFS = 0;
        mFunctions->getActiveUniformBlockiv(mProgramID, blockIndex,
    ...
    for (int uniformIdx = 0; uniformIdx < uniforms->size(); uniformIdx++)
    {
        ...
        if (uniformIndex == -1)
        {
            ...
            for (size_t uniformBlockIdx = 0; uniformBlockIdx < uniformBlocks->size(); uniformBlockIdx++)
            {
                auto &uniformBlock = uniformBlocks->at(uniformBlockIdx);
                for (int memberIndex = 0; memberIndex < uniformBlock.memberUniformIndexes.size(); memberIndex++)
                {
                    if (uniformBlock.memberUniformIndexes[memberIndex] == static_cast<unsigned int>(uniformIdx))
                    {
                        uniformBlock.memberUniformIndexes.erase(uniformBlock.memberUniformIndexes.begin() + memberIndex);
                        memberIndex--;
                    }
                    else if (uniformBlock.memberUniformIndexes[memberIndex] > static_cast<unsigned int>(uniformIdx))
                    {
