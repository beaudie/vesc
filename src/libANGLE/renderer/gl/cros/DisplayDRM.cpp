...
bool DisplayDRM::Buffer::initialize(int width, int height)
{
    mWidth = width;
    mHeight = height;

    mBO = gbm_bo_create(mDisplay->mGBM, mWidth, mHeight, GBM_FORMAT_ARGB8888, GBM_BO_USE_RENDERING|GBM_BO_USE_SCANOUT);
    if (!mBO)
    {
        return false;
        //return egl::Error(EGL_NOT_INITIALIZED, "gbm_bo_create scanout failed");
    }

    int dmabuf = gbm_bo_get_fd(mBO);
    ...
    EGLImageKHR image = mDisplay->mEGL.createImage(EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attr);
    ...
    sm->bindRenderbuffer(GL_RENDERBUFFER, renderbuffer);
    gl->eglImageTargetRenderbufferStorageOES(GL_RENDERBUFFER, image);

    gl->genFramebuffers(1, &mGLFB);
    sm->bindFramebuffer(GL_FRAMEBUFFER, mGLFB);

    gl->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0_EXT, GL_RENDERBUFFER, renderbuffer);
    ...
    if (depth || stencil)
    {
        ...
        sm->bindRenderbuffer(GL_RENDERBUFFER, depth_stencil);
        gl->renderbufferStorage(GL_RENDERBUFFER, depth_stencil_format, mWidth, mHeight);
    }
...
uint32_t DisplayDRM::Buffer::getDRMFB()
{
    if (!mHasDRMFB)
    {
        ...
        {
            //XXX return false;
            //return egl::Error(EGL_NOT_INITIALIZED, "drmModeAddFB scanout failed");
        }
        mHasDRMFB = true;
    }
...
bool DisplayDRM::Buffer::present()
{
    return mDisplay->present(this);
}
...
egl::Error DisplayDRM::initialize(egl::Display *display)
{
    ...
    egl::Error error = DisplayGL::initialize(display);

    if (!error.isError())
    {
        mScanout[0].initialize(mWidth, mHeight);
        mScanout[1].initialize(mWidth, mHeight);
    }
...
static void bounce(int *p, int *d, int m)
{
    *p+=*d;
    if(*p<0 || *p>m) {
        *d *= -1;
...
bool DisplayDRM::present(Buffer *buffer)
{
    static double f=0;
    StateManagerGL *sm = getRenderer()->getStateManager();

    sm->bindFramebuffer(GL_DRAW_FRAMEBUFFER, mScanout[mBack].mGLFB);

    sm->setClearColor(wheel(f));
    mFunctionsGL->clear(GL_COLOR_BUFFER_BIT);
    ...
    mFunctionsGL->blitFramebuffer(0, 0, buffer->mWidth, buffer->mHeight,
                                  buffer->x, buffer->y, buffer->mWidth + buffer->x, buffer->mHeight + buffer->y,
                                  GL_COLOR_BUFFER_BIT, GL_NEAREST);

    bounce(&buffer->x, &buffer->dx, mWidth-buffer->mWidth);
    bounce(&buffer->y, &buffer->dy, mHeight-buffer->mHeight);
    ...
    if (mSetCRTC)
    {
        if (drmModeSetCrtc(fd, mCRTC->crtc_id, mScanout[mBack].getDRMFB(), 0, 0, &mConnector->connector_id, 1, mMode))
        {
            return false;
        }
        mSetCRTC = false;
    ...
    {
        if (drmModePageFlip(fd, mCRTC->crtc_id, mScanout[mBack].getDRMFB(), DRM_MODE_PAGE_FLIP_EVENT, this))
        {
            return false;
        }
        mFlipPending = true;
                                             ...
                                             const egl::AttributeMap &attribs)
{
    ...
    buffer->initialize(size[0], size[1]);
    return new WindowSurfaceCros(this->getRenderer(), buffer);
}
