...
class DisplayDRM : public DisplayGL
{
  public:
    ...
    struct Buffer
    {
        Buffer(DisplayDRM *display, const NativeWindow *window, uint32_t useFlags, uint32_t gbmFormat, uint32_t drmFormat, uint32_t drmFormatFB, int depthBits, int stencilBits);
        ~Buffer();
        bool initialize();
        void reset();
        bool resize();
