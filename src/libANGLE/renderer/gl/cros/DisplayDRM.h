...
// DisplayDRM.h: DRM/KMS/GBM implementation of egl::Display
...
class DisplayDRM : public DisplayGL
{
  public:
    ...
    struct Buffer
    {
        Buffer(DisplayDRM *display, const NativeWindow *window);
        ~Buffer();
        bool initialize(uint32_t format, uint32_t flags, bool depthStencil);
        bool resize(uint32_t format, uint32_t flags, bool depthStencil);
        FramebufferGL *framebufferGL(const gl::Framebuffer::Data &data);
        void present();
        void cleanup(bool cleanupGLFB);
        uint32_t getDRMFB();
        GLuint getTexture();
