                                          ...
                                          gl::PrimitiveMode primitiveMode) const
{
    if (mIsActive != active)
    {
        ...
        gl::TransformFeedback *tf  = context->getState().getCurrentTransformFeedback();
        const gl::Program *program = tf ? tf->getBoundProgram() : nullptr;
        GLuint programID           = 0;
        if (program)
        {
            programID = GetImplAs<ProgramGL>(program)->getProgramID();
        }
        ...
        else
        {
            ...
            {
                // Driver bug workaround: If transform feedback is paused and a new program is
