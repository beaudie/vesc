                                       ...
                                       FramebufferGL *source)
{
    ...
    for (size_t colorAttachmentIdx : colorAttachments)
    {
        ...
        {
            uninitializedColorAttachments[colorAttachmentIdx] = true;
        }
    }

    bool depthNeedsInit = depthClear && source->getState().getDepthAttachment()->initState() ==
                                            gl::InitState::MayNeedInit;
    bool stencilNeedsInit =
        stencilClear &&
        source->getState().getStencilAttachment()->initState() == gl::InitState::MayNeedInit;
