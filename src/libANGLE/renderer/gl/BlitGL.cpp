                                     ...
                                     const nativegl::TexImageFormat &format)
{
    ...
    // Make an intermediate texture with 1 less level than the source
    ...
    mStateManager->bindFramebuffer(GL_FRAMEBUFFER, mScratchFBO);
    mStateManager->setFramebufferSRGBEnabled(context, false);
    ...
    {
        {
            ANGLE_GL_TRY(context, mFunctions->framebufferTexture2D(
                                      ...
                                      source->getTextureID(), baseLevel + levelIdx - 1));

            GLubyte px[4] = {0};
            mFunctions->readPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);

            WARN() << "source data " << " (" << int(px[0]) << " " << int(px[1]) << " " << int(px[2])
                   << " " << int(px[3]) << ")";
        }
...

        {
            GLubyte px[4] = {0};
            mFunctions->readPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);
            WARN() << "data in intermediateTexture " << " (" << int(px[0]) << " " << int(px[1])
                   << " " << int(px[2]) << " " << int(px[3]) << ")";
        }
...

        {
            ...
            WARN() << "blit source " << sourceTextureReadLevel << " -> intermediate "
                   << levelIdx - 1 << " [" << levelSize.width << "x" << levelSize.height << "]"
...

        {
            GLubyte px[4] = {0};
            mFunctions->readPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);
            WARN() << "blit intermediate " << levelIdx - 1 << " -> source "
                   << sourceTextureWriteLevel << " (" << int(px[0]) << " " << int(px[1]) << " "
                   << int(px[2]) << " " << int(px[3]) << ")";
        }
    }
...
angle::Result BlitGL::orphanScratchTextures(const gl::Context *context)
{
    ...
    //         ANGLE_GL_TRY(context, mFunctions->texParameteriv(GL_TEXTURE_2D,
    ...
    //         ANGLE_GL_TRY(context,
    //                      mFunctions->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G,
    //                      GL_GREEN));
    //         ANGLE_GL_TRY(context,
    //                      mFunctions->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B,
    //                      GL_BLUE));
    //         ANGLE_GL_TRY(context,
    //                      mFunctions->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A,
    //                      GL_ALPHA));
    //     }

    //     ANGLE_GL_TRY(context, mFunctions->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL,
    //     0)); ANGLE_GL_TRY(context, mFunctions->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL,
    //     1000)); ANGLE_GL_TRY(context, mFunctions->texParameteri(GL_TEXTURE_2D,
