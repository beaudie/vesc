                                            ...
                                            GLenum filter)
{
    ...
    // We'll keep things simple by removing flip from the rectangles. In the end we'll apply the
    // flips to the source texture coordinates if needed. The destination rectangle will be set to
    // the gl viewport, which can't be flipped.
    bool flipX               = sourceAreaIn.isFlippedX() != destAreaIn.isFlippedX();
    bool flipY               = sourceAreaIn.isFlippedY() != destAreaIn.isFlippedY();
    gl::Rectangle sourceArea = sourceAreaIn.removeFlip();
    gl::Rectangle destArea   = destAreaIn.removeFlip();
    ...
    Vector2 sourceAreaOffsetInTexCoords(
    ...
    Vector2 sourceAreaSizeInTexCoords(
    ...
    if (flipX)
    {
        sourceAreaOffsetInTexCoords.x() =
            sourceAreaOffsetInTexCoords.x() + sourceAreaSizeInTexCoords.x();
        sourceAreaSizeInTexCoords.x() = -sourceAreaSizeInTexCoords.x();
    }
    if (flipY)
    {
        sourceAreaOffsetInTexCoords.y() =
            sourceAreaOffsetInTexCoords.y() + sourceAreaSizeInTexCoords.y();
        sourceAreaSizeInTexCoords.y() = -sourceAreaSizeInTexCoords.y();
    }

    ASSERT(sourceAreaSizeInTexCoords.x() != 0.0 && sourceAreaSizeInTexCoords.y() != 0.0);
    Vector2 texCoordScale = Vector2(1.0, 1.0) / sourceAreaSizeInTexCoords;
    ...
    mFunctions->uniform2f(blitProgram->offsetLocation, sourceAreaOffsetInTexCoords.x(),
                          sourceAreaOffsetInTexCoords.y());
    mFunctions->uniform1i(blitProgram->multiplyAlphaLocation, 0);
    mFunctions->uniform1i(blitProgram->unMultiplyAlphaLocation, 0);
