                                                      ...
                                                      const gl::Framebuffer *source)
{
    ...
    ScopedGLState guard(mStateManager, mFunctions,
                                            ...
                                            GLenum filter)
{
    ...
    // rectangle
    ScopedGLState guard(mStateManager, mFunctions, destArea, ScopedGLState::KEEP_SCISSOR);
    guard.willUseTextureUnit(0);
...
gl::Error BlitGL::initializeResources()
{
    if (mBlitProgram == 0)
    {
        ...
        const char *fsSource =
            ...
            "        discard;\n"
            "    }\n"
            "    vec4 color = texture(u_source_texture, v_texcoord);\n"
            "    if (u_multiply_alpha) {color.xyz = color.xyz * color.a;}"
            "    if (u_unmultiply_alpha) {color.xyz = color.xyz / color.a;}"
            "    output_color = color;"
            "}\n";
