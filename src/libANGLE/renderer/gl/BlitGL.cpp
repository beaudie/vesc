...
gl::Error BlitGL::copyImageToLUMAWorkaroundTexture(GLuint texture, GLenum textureType, GLenum target, GLenum lumaFormat, size_t level, const gl::Rectangle &sourceArea, GLenum internalFormat, const gl::Framebuffer *source)
{
    mStateManager->bindTexture(textureType, texture);

    // Allocate the texture memory
    const gl::InternalFormat &internalFormatInfo = gl::GetInternalFormatInfo(internalFormat);
    mFunctions->texImage2D(target, static_cast<GLint>(level), internalFormat, sourceArea.width, sourceArea.height, 0, internalFormatInfo.format, source->getImplementationColorReadType(), nullptr);

    return copySubImageToLUMAWorkaroundTexture(texture, textureType, target, lumaFormat, level, gl::Offset(0, 0, 0), sourceArea, source);
}

gl::Error BlitGL::copySubImageToLUMAWorkaroundTexture(GLuint texture, GLenum textureType, GLenum target, GLenum lumaFormat, size_t level, const gl::Offset &destOffset, const gl::Rectangle &sourceArea, const gl::Framebuffer *source)
{
    ...
    nativegl::CopyTexImageImageFormat copyTexImageFormat = nativegl::GetCopyTexImageImageFormat(
        mFunctions, mWorkarounds, source->getImplementationColorReadFormat(), source->getImplementationColorReadType());

    mStateManager->bindTexture(GL_TEXTURE_2D, mScratchTexture);
    mFunctions->copyTexImage2D(GL_TEXTURE_2D, 0,
        copyTexImageFormat.internalFormat, sourceArea.x, sourceArea.y,
        sourceArea.width, sourceArea.height, 0);

    // Set the swizzle of the scratch texture so that the channels sample into the correct emulated LUMA channels.
    GLint swizzle[4] =
    {
    ...
    mFunctions->framebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, target, texture, static_cast<GLint>(level));

    // Render to the destination texture, sampling from the scratch texture
    mStateManager->useProgram(mBlitProgram);
    mFunctions->uniform1i(mTextureUniform, static_cast<GLint>(mStateManager->getActiveTexture()));

    mStateManager->setViewport(gl::Rectangle(destOffset.x, destOffset.y, sourceArea.width, sourceArea.height));
...
static gl::Error CheckCompileStatus(const FunctionsGL *functions, GLuint shader)
{
    ...
    {
        return gl::Error(GL_OUT_OF_MEMORY, "Failed to compiler internal blit shader.");
    }

    return gl::Error(GL_NO_ERROR);
}

static gl::Error CheckLinkStatus(const FunctionsGL *functions, GLuint program)
{
...
gl::Error BlitGL::initializeResources()
{
    if (mBlitProgram == 0)
    {
        ...
        const char *vsSource =
            ...
            "    );\n"
            "\n"
            "    vec2 coords = quad_positions[gl_VertexID];\n"
        ...
        mTextureUniform = mFunctions->getUniformLocation(mBlitProgram, "u_source_texture");
    }
