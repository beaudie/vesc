    ...
    else
    {
        ...
        else
        {
            ...
            for (GLint layer = 0; layer < layerCount; layer++)
            {
                for (GLenum bindTarget : bindTargets)
                {
                    mFunctions->framebufferTexture3D(
                        GL_FRAMEBUFFER, bindTarget, ToGLenum(imageIndex.target),
                        source->getTextureID(), imageIndex.mipIndex, layer + firstLayer);
                }
                mFunctions->clear(clearMask);
            }
        }
