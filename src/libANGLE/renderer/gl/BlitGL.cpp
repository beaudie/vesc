...
class StateGuard : public angle::NonCopyable
{
  public:
    enum
    {
    ...
    StateGuard(StateManagerGL *stateManager,
                                                      ...
                                                      const gl::Framebuffer *source)
{
    ...
    StateGuard guard(mStateManager, mFunctions,
                     gl::Rectangle(0, 0, sourceArea.width, sourceArea.height));
    guard.willUseTextureUnit(0);
                                            ...
                                            GLenum filter)
{
    ...
    // rectangle
    StateGuard guard(mStateManager, mFunctions, destArea, StateGuard::KEEP_SCISSOR);
    guard.willUseTextureUnit(0);
                              ...
                              bool unpackUnmultiplyAlpha)
{
    ...
    mFunctions->texImage2D(destTarget, 0, destInternalFormat, size.width, size.height, 0,
                           internalFormatInfo.format, internalFormatInfo.type, nullptr);

    return copySubTexture(sourceTexture, sourceTarget, sourceInternalFormat,
                          gl::Rectangle(0, 0, size.width, size.height), destTexture, destTarget,
                          destInternalFormat, gl::Offset(0, 0, 0), unpackFlipY,
