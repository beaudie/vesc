                                            ...
                                            bool unpackUnmultiplyAlpha)
{
    ...
    GLenum readPixelsFormat        = GL_RGBA;
    ColorReadFunction readFunction = angle::ReadColor<angle::R8G8B8A8, GLfloat>;
    ...
    mStateManager->setPixelPackState(gl::PixelPackState(1, 0));
    mFunctions->texSubImage2D(destTarget, static_cast<GLint>(destLevel), destOffset.x, destOffset.y,
                              sourceArea.width, sourceArea.height, destFormat, destType,
                              destMemory);
