...
{
    if (depthStencilWorkaround || lumaWorkaround.enabled)
    {
        workaroundDirtyBits.set(gl::Texture::DIRTY_BIT_SWIZZLE_RED);
        workaroundDirtyBits.set(gl::Texture::DIRTY_BIT_SWIZZLE_GREEN);
        workaroundDirtyBits.set(gl::Texture::DIRTY_BIT_SWIZZLE_BLUE);
        workaroundDirtyBits.set(gl::Texture::DIRTY_BIT_SWIZZLE_ALPHA);
    }
}
...
void TextureGL::setLevelInfo(size_t level, size_t levelCount, const LevelInfoGL &levelInfo)
{
    ...
    // If the base level is modified, local dirty bits may need to be set for workarounds
    GLuint baseLevel = mState.getEffectiveBaseLevel();
    if (level <= baseLevel && level + levelCount >= baseLevel)
    {
        const auto &prevBaseLevelInfo = mLevelInfo[baseLevel];
        mLocalDirtyBits |= prevBaseLevelInfo.workaroundDirtyBits | levelInfo.workaroundDirtyBits;
    }

    std::fill(mLevelInfo.begin() + level, mLevelInfo.begin() + level + levelCount, levelInfo);
}
