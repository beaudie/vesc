                               ...
                               const gl::Framebuffer *source)
{
    ...
    LUMAWorkaround lumaWorkaround = GetLUMAWorkaroundInfo(internalFormat, copyTexImageFormat.internalFormat);
    if (lumaWorkaround.enabled)
    {
        gl::Error error = mBlitter->copyImageToLUMAWorkaroundTexture(mTextureID, mTextureType, target, lumaWorkaround.sourceFormat, level, sourceArea, copyTexImageFormat.internalFormat, source);
        if (error.isError())
        {
            return error;
        }
                                  ...
                                  const gl::Framebuffer *source)
{
    ...
    if (lumaWorkaround.enabled)
    {
        gl::Error error = mBlitter->copySubImageToLUMAWorkaroundTexture(mTextureID, mTextureType, target, lumaWorkaround.sourceFormat, level, destOffset, sourceArea, source);
        if (error.isError())
        {
            return error;
        }
