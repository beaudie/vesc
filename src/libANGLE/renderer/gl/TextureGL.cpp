                               ...
                               const gl::Framebuffer *source)
{
    ...
    // We'll do it in robust mode as well, which implies WebGL mode.
    // We just zero the whole thing before copying into the area that overlaps the framebuffer.
    // TODO(fjhenigman): As the robust resource stuff evolves, see if we can avoid zeroing here.
    if (outside && context->getExtensions().robustResourceInitialization)
    {
        GLuint pixelBytes =
