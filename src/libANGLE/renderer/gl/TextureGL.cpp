                               ...
                               const gl::Framebuffer *source)
{
    ...
    else
    {
        if (UseTexImage2D(getTarget()))
        {
            const auto &formatInfo = readAttachment->getFormat().info;
            void *textureData      = nullptr;
            if (context->getExtensions().webglCompatibility)
            {
                angle::MemoryBuffer *zero;
                context->getScratchBuffer(
                    origSourceArea.width * origSourceArea.height * formatInfo->pixelBytes, &zero);
                zero->fill(0);
            ...
            mFunctions->texImage2D(target, static_cast<GLint>(level),
                                   ...
                                   origSourceArea.height, 0, formatInfo->getReadPixelsType(),
