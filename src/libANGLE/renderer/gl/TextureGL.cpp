                               ...
                               const gl::Framebuffer *source)
{
    ...
    else
    {
        if (UseTexImage2D(getTarget()))
        {
            ...
            {
                auto bpp = gl::GetSizedInternalFormatInfo(copyTexImageFormat.internalFormat).pixelBytes;
                printf("bpp %u\n",bpp);
                angle::MemoryBuffer *zero;
                context->getScratchBuffer(
                    origSourceArea.width * origSourceArea.height * bpp, &zero);
                zero->fill(0);
            ...
            mFunctions->texImage2D(target, static_cast<GLint>(level),
                                   ...
                                   origSourceArea.height, 0, gl::GetUnsizedFormat(copyTexImageFormat.internalFormat),
