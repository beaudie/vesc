                                     ...
                                     const uint8_t *pixels)
{
    ...
    {
        constexpr const size_t kMaxUploadChunkSize = (240 * 1024) - 1;
        return setSubImageRowByRowWorkaround(context, target, level, area, format, type, unpack,
                                                       ...
                                                       const uint8_t *pixels)
{
    ...
    gl::PixelUnpackState directUnpack = context->getState().getUnpackState();
            ...
            for (GLint row = 0; row < area.height; row += rowsPerChunk)
            {
                ...
                ANGLE_GL_TRY(context,
                             functions->texSubImage3D(
                                 ...
                                 image + area.z, area.width, height, 1, format, type, rowPixels));
            }
        }
    }
    else
    {
        ...
        for (GLint row = 0; row < area.height; row += rowsPerChunk)
        {
            ...
            ANGLE_GL_TRY(context, functions->texSubImage2D(
                                      ...
                                      row + area.y, area.width, height, format, type, rowPixels));
        }
    }
    return angle::Result::Continue;
}
