                                          ...
                                          const gl::Texture *source)
{
    ...
    if (source->getType() == gl::TextureType::_2D && !unpackFlipY &&
                                ...
                                const gl::Extents &size)
{
    ...
    if (UseTexImage2D(getType()))
    {
        ...
        {
            mFunctions->texStorage2D(ToGLenum(target), static_cast<GLsizei>(levels),
        ...
        else
        {
            ...
            for (size_t level = 0; level < levels; level++)
            {
                ...
                if (getType() == gl::TextureType::_2D || getType() == gl::TextureType::Rectangle)
                {
                    if (internalFormatInfo.compressed)
                    {
                        ...
                        mFunctions->compressedTexImage2D(
                            ...
                            compressedTexImageFormat.format, levelSize.width, levelSize.height, 0,
                ...
                {
                    for (gl::TextureTarget face : gl::CubeFaceTextureTargets())
                    {
                        if (internalFormatInfo.compressed)
                        {
                            nativegl::CompressedTexSubImageFormat compressedTexImageFormat =
    ...
    else if (UseTexImage3D(getType()))
    {
        ...
        {
            mFunctions->texStorage3D(ToGLenum(target), static_cast<GLsizei>(levels),
                                     texStorageFormat.internalFormat, size.width, size.height,
                                     size.depth);
        }
        else
            ...
            for (GLsizei i = 0; i < static_cast<GLsizei>(levels); i++)
            {
                ...
                if (internalFormatInfo.compressed)
                {
                    ...
                    ANGLE_TRY_RESULT(
                        ...
                        dataSize);
                    mFunctions->compressedTexImage3D(
                        ToGLenum(target), i, compressedTexImageFormat.format, levelSize.width,
                ...
                else
                {
                    ...
                    mFunctions->texImage3D(ToGLenum(target), i, texImageFormat.internalFormat,
                                           ...
                                           bool fixedSampleLocations)
{
    ...
    mFunctions->texStorage2DMultisample(ToGLenum(target), samples, texStorageFormat.internalFormat,
                             ...
                             const LevelInfoGL &levelInfo)
{
    ...
    {
        for (gl::TextureTarget target : gl::CubeFaceTextureTargets())
        {
