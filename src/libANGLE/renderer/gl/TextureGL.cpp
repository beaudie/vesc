...
static LUMAWorkaround GetLUMAWorkaroundInfo(const gl::InternalFormat &originalFormatInfo,
                                            GLenum destinationFormat)
{
    ...
    {
        return LUMAWorkaround(false, GL_NONE);
    }
}
...
static LevelInfo GetLevelInfo(GLenum originalFormat, GLenum destinationFormat)
{
    const gl::InternalFormat &originalFormatInfo = gl::GetInternalFormatInfo(originalFormat);
    return LevelInfo(originalFormat, GetDepthStencilWorkaround(originalFormatInfo),
                     GetLUMAWorkaroundInfo(originalFormatInfo, destinationFormat));
}

LUMAWorkaround::LUMAWorkaround() : LUMAWorkaround(false, GL_NONE)
{
}

LUMAWorkaround::LUMAWorkaround(bool enabled_, GLenum workaroundFormat_)
    : enabled(enabled_), workaroundFormat(workaroundFormat_)
{
}

LevelInfo::LevelInfo() : LevelInfo(GL_NONE, false, LUMAWorkaround())
{
}

LevelInfo::LevelInfo(GLenum sourceFormat_,
                     bool depthStencilWorkaround_,
                     const LUMAWorkaround &lumaWorkaround_)
                               ...
                               const gl::Framebuffer *source)
{
    ...
    LevelInfo levelInfo = GetLevelInfo(internalFormat, copyTexImageFormat.internalFormat);
                                  ...
                                  const gl::Framebuffer *source)
{
    ...
    const LevelInfo &levelInfo = mLevelInfo[level];
    if (levelInfo.lumaWorkaround.enabled)
    {
...
gl::Error TextureGL::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents &size)
{
    ...
    LevelInfo levelInfo = GetLevelInfo(internalFormat, texStorageFormat.internalFormat);
...
void TextureGL::syncSamplerState(const gl::SamplerState &samplerState) const
{
    ...
    const LevelInfo &levelInfo = mLevelInfo[samplerState.baseLevel];
