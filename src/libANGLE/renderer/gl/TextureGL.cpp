...
{
    const FunctionsGL *functions                              = GetFunctionsGL(context);
    const gl::OffsetBindingPointer<gl::Buffer> &bindingBuffer = mState.getBuffer();
    const gl::Buffer *buffer                                  = bindingBuffer.get();
    const GLintptr offset                                     = bindingBuffer.getOffset();
    const GLsizeiptr size                                     = bindingBuffer.getSize();
    ...
    if (buffer == nullptr || size == std::numeric_limits<uint32_t>::max())
    {
