...
static gl::Texture::DirtyBits GetWorkaroundDirtyBits()
{
    return gl::Texture::DIRTY_BIT_SWIZZLE_RED | gl::Texture::DIRTY_BIT_SWIZZLE_GREEN | gl::Texture::DIRTY_BIT_SWIZZLE_BLUE | gl::Texture::DIRTY_BIT_SWIZZLE_ALPHA;
}
...
gl::Error TextureGL::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents &size)
{
    ...
    setLevelInfo(0, mLevelInfo.size(), GetLevelInfo(internalFormat, texStorageFormat.internalFormat));
                                           ...
                                           GLenum value)
{
    ...
    functions->texParameteri(textureType, name, resultSwizzle);
}

void TextureGL::setBaseLevel(GLuint baseLevel)
{
    // Don't know if the previous base level was using any workarounds, always re-sync the workaround dirty bits
    syncState(GetWorkaroundDirtyBits());
}
    ...
    for (auto dirtyBit : angle::IterateBitSet(dirtyBits))
    {
        switch (dirtyBit)
        {
            ...
            case gl::Texture::DIRTY_BIT_SWIZZLE_RED:
                SyncTextureStateSwizzle(mFunctions, levelInfo, mState.mTarget, GL_TEXTURE_SWIZZLE_R,
                                        mState.getSwizzleState().swizzleRed);
                break;
            case gl::Texture::DIRTY_BIT_SWIZZLE_GREEN:
                SyncTextureStateSwizzle(mFunctions, levelInfo, mState.mTarget, GL_TEXTURE_SWIZZLE_G,
                                        mState.getSwizzleState().swizzleGreen);
                break;
            case gl::Texture::DIRTY_BIT_SWIZZLE_BLUE:
                SyncTextureStateSwizzle(mFunctions, levelInfo, mState.mTarget, GL_TEXTURE_SWIZZLE_B,
                                        mState.getSwizzleState().swizzleBlue);
                break;
            case gl::Texture::DIRTY_BIT_SWIZZLE_ALPHA:
                SyncTextureStateSwizzle(mFunctions, levelInfo, mState.mTarget, GL_TEXTURE_SWIZZLE_A,
