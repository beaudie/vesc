...
void StateManagerGL::setSampleMaski(GLuint maskNumber, GLbitfield mask)
{
    ...
    if (mSampleMaskValues[maskNumber] != mask)
    {
        ...
        mLocalDirtyBits.set(gl::State::DIRTY_BIT_SAMPLE_MASK_WORD);
    }
}
    ...
    for (auto dirtyBit : glAndLocalDirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_SAMPLE_MASK_ENABLED:
                ...
                break;
            case gl::State::DIRTY_BIT_SAMPLE_MASK_WORD:
            {
                for (GLuint maskNumber = 0; maskNumber < gl::MAX_SAMPLE_MASK_WORDS; ++maskNumber)
                {
