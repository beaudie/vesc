      ...
      mProgramTexturesAndSamplersDirty(true)
{
    ...
    mIndexedBuffers[gl::BufferTarget::Uniform].resize(rendererCaps.maxUniformBufferBindings);
    mIndexedBuffers[gl::BufferTarget::AtomicCounter].resize(
        rendererCaps.maxAtomicCounterBufferBindings);
    mIndexedBuffers[gl::BufferTarget::ShaderStorage].resize(
...
void StateManagerGL::deleteBuffer(GLuint buffer)
{
    ...
    for (auto target : angle::AllEnums<gl::BufferTarget>())
    {
...
void StateManagerGL::bindVertexArray(GLuint vao, GLuint elementArrayBuffer)
{
    ...
    {
        mVAO                                     = vao;
        mBuffers[gl::BufferTarget::ElementArray] = elementArrayBuffer;
...
void StateManagerGL::bindBuffer(gl::BufferTarget target, GLuint buffer)
{
    if (mBuffers[target] != buffer)
    {
...
void StateManagerGL::bindBufferBase(gl::BufferTarget target, size_t index, GLuint buffer)
{
...
void StateManagerGL::bindBufferRange(gl::BufferTarget target,
                                     size_t index,
                                     GLuint buffer,
                                     size_t offset,
                                     size_t size)
...
void StateManagerGL::setPixelUnpackBuffer(const gl::Buffer *pixelBuffer)
{
    ...
    bindBuffer(gl::BufferTarget::PixelUnpack, bufferID);
}
...
void StateManagerGL::setPixelPackBuffer(const gl::Buffer *pixelBuffer)
{
    ...
    bindBuffer(gl::BufferTarget::PixelPack, bufferID);
}
...
gl::Error StateManagerGL::setDrawIndirectState(const gl::Context *context, GLenum type)
{
    ...
    bindBuffer(gl::BufferTarget::DrawIndirect, bufferGL->getBufferID());
...
void StateManagerGL::setGenericShaderState(const gl::Context *context)
{
         ...
         uniformBlockIndex++)
    {
        ...
        if (uniformBuffer.get() != nullptr)
        {
            ...
            {
                bindBufferBase(gl::BufferTarget::Uniform, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferTarget::Uniform, binding, bufferGL->getBufferID(),
    ...
    for (const auto &atomicCounterBuffer : program->getState().getAtomicCounterBuffers())
    {
        ...
        if (buffer.get() != nullptr)
        {
            ...
            if (buffer.getSize() == 0)
            {
                bindBufferBase(gl::BufferTarget::AtomicCounter, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferTarget::AtomicCounter, binding, bufferGL->getBufferID(),
...
void StateManagerGL::updateProgramTextureAndSamplerBindings(const gl::Context *context)
{
         ...
         blockIndex++)
    {
        ...
        if (shaderStorageBuffer.get() != nullptr)
        {
            ...
            {
                bindBufferBase(gl::BufferTarget::ShaderStorage, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferTarget::ShaderStorage, binding, bufferGL->getBufferID(),
