...
StateManagerGL::StateManagerGL(const FunctionsGL *functions,
    ...
    : mFunctions(functions),
      ...
      mDispatchIndirectBufferDirty(false),
      mAppliedDispatchIndirectBuffer(),
      mDrawIndirectBufferDirty(false),
      mAppliedDrawIndirectBuffer()
{
    ASSERT(mFunctions);
...
gl::Error StateManagerGL::setDrawIndirectState(const gl::Context *context, GLenum type)
{
    ...
    if (mDrawIndirectBufferDirty)
    {
        gl::Buffer *drawIndirectBuffer = glState.getTargetBuffer(gl::BufferBinding::DrawIndirect);
        ASSERT(drawIndirectBuffer);
        if (drawIndirectBuffer != mAppliedDrawIndirectBuffer.get())
        {
            ...
            mAppliedDrawIndirectBuffer.set(context, drawIndirectBuffer);
        }
    }
    return setGenericDrawState(context);
}
...
void StateManagerGL::updateDispatchIndirectBufferBinding(const gl::Context *context)
{
    ...
    if (dispatchIndirectBuffer != mAppliedDispatchIndirectBuffer.get())
    {
        ...
        mAppliedDispatchIndirectBuffer.set(context, dispatchIndirectBuffer);
    }
}
...
gl::Error StateManagerGL::setDispatchComputeState(const gl::Context *context, bool isIndirect)
{
    ...
    if (isIndirect && mDispatchIndirectBufferDirty)
    {
    ...
    for (auto dirtyBit : glAndLocalDirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_DRAW_INDIRECT_BUFFER_BINDING:
                mDrawIndirectBufferDirty = true;
                break;
            case gl::State::DIRTY_BIT_DISPATCH_INDIRECT_BUFFER_BINDING:
                mDispatchIndirectBufferDirty = true;
                break;
