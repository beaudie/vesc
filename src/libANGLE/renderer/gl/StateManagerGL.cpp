      ...
      mProgramTexturesAndSamplersDirty(true)
{
    ...
    mIndexedBuffers[gl::BufferType::Uniform].resize(rendererCaps.maxUniformBufferBindings);
    mIndexedBuffers[gl::BufferType::AtomicCounter].resize(
        rendererCaps.maxAtomicCounterBufferBindings);
    mIndexedBuffers[gl::BufferType::ShaderStorage].resize(
...
void StateManagerGL::deleteBuffer(GLuint buffer)
{
    ...
    for (auto target : angle::AllEnums<gl::BufferType>())
    {
...
void StateManagerGL::bindVertexArray(GLuint vao, GLuint elementArrayBuffer)
{
    ...
    {
        mVAO                                     = vao;
        mBuffers[gl::BufferType::ElementArray] = elementArrayBuffer;
...
void StateManagerGL::bindBuffer(gl::BufferType target, GLuint buffer)
{
    if (mBuffers[target] != buffer)
    {
...
void StateManagerGL::bindBufferBase(gl::BufferType target, size_t index, GLuint buffer)
{
...
void StateManagerGL::bindBufferRange(gl::BufferType target,
                                     size_t index,
                                     GLuint buffer,
                                     size_t offset,
                                     size_t size)
...
void StateManagerGL::setPixelUnpackBuffer(const gl::Buffer *pixelBuffer)
{
    ...
    bindBuffer(gl::BufferType::PixelUnpack, bufferID);
}
...
void StateManagerGL::setPixelPackBuffer(const gl::Buffer *pixelBuffer)
{
    ...
    bindBuffer(gl::BufferType::PixelPack, bufferID);
}
...
gl::Error StateManagerGL::setDrawIndirectState(const gl::Context *context, GLenum type)
{
    ...
    gl::Buffer *drawIndirectBuffer = glState.getTargetBuffer(gl::BufferType::DrawIndirect);
    ASSERT(drawIndirectBuffer);
    const BufferGL *bufferGL = GetImplAs<BufferGL>(drawIndirectBuffer);
    bindBuffer(gl::BufferType::DrawIndirect, bufferGL->getBufferID());
...
void StateManagerGL::setGenericShaderState(const gl::Context *context)
{
         ...
         uniformBlockIndex++)
    {
        ...
        if (uniformBuffer.get() != nullptr)
        {
            ...
            {
                bindBufferBase(gl::BufferType::Uniform, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferType::Uniform, binding, bufferGL->getBufferID(),
    ...
    for (const auto &atomicCounterBuffer : program->getState().getAtomicCounterBuffers())
    {
        ...
        if (buffer.get() != nullptr)
        {
            ...
            if (buffer.getSize() == 0)
            {
                bindBufferBase(gl::BufferType::AtomicCounter, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferType::AtomicCounter, binding, bufferGL->getBufferID(),
...
void StateManagerGL::updateProgramTextureAndSamplerBindings(const gl::Context *context)
{
         ...
         blockIndex++)
    {
        ...
        if (shaderStorageBuffer.get() != nullptr)
        {
            ...
            {
                bindBufferBase(gl::BufferType::ShaderStorage, binding, bufferGL->getBufferID());
            }
            else
            {
                bindBufferRange(gl::BufferType::ShaderStorage, binding, bufferGL->getBufferID(),
    ...
    for (auto dirtyBit : glAndLocalDirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_UNPACK_BUFFER_BINDING:
                setPixelUnpackBuffer(state.getTargetBuffer(gl::BufferType::PixelUnpack));
                break;
            case gl::State::DIRTY_BIT_PACK_STATE:
                setPixelPackState(state.getPackState());
                break;
            case gl::State::DIRTY_BIT_PACK_BUFFER_BINDING:
                setPixelPackBuffer(state.getTargetBuffer(gl::BufferType::PixelPack));
                break;
