...
void StateManagerGL::bindIndexedBuffer(GLenum type, size_t index, GLuint buffer, size_t offset, size_t size)
{
    auto &binding = mIndexedBuffers[type][index];
...
gl::Error StateManagerGL::setGenericDrawState(const gl::Data &data)
{
    ...
    for (size_t uniformBlockIndex = 0; uniformBlockIndex < gl::IMPLEMENTATION_MAX_COMBINED_SHADER_UNIFORM_BUFFERS; uniformBlockIndex++)
    {
        ...
        if (uniformBuffer.get() != nullptr)
        {
            ...
            {
                bindIndexedBuffer(GL_UNIFORM_BUFFER, binding, bufferGL->getBufferID(), uniformBuffer.getOffset(), uniformBuffer.getSize());
            }
        }
    }
