...
StateManagerGL::StateManagerGL(const FunctionsGL *functions, const gl::Caps &rendererCaps)
    : mFunctions(functions),
      ...
      mPackSkipRows(0),
      mPackSkipPixels(0),
      mFramebuffers(2, 0), // Bindings for read and draw
      mRenderbuffer(0),
...
void StateManagerGL::deleteFramebuffer(GLuint fbo)
{
    if (fbo != 0)
    {
        // Use 0 and 1 for read and draw framebuffers
        for (unsigned int binding = 0; binding < 2; ++binding)
        {
            if (mFramebuffers[binding] == fbo)
            {
                GLenum enumValue = angle::FramebufferBindingToEnum(static_cast<angle::FramebufferBinding>(binding));
