...
gl::Error StateManagerGL::setGenericDrawState(const gl::Data &data)
{
    ...
    {
        const gl::Texture *texture = state.getSamplerTexture(samplerUniform.boundTextureUnit, samplerUniform.textureType);
        if (texture != nullptr)
        {
            const TextureGL *textureGL = GetImplAs<TextureGL>(texture);
            textureGL->syncSamplerState(texture->getSamplerState());

            if (mTextures[samplerUniform.textureType][samplerUniform.boundTextureUnit] != textureGL->getTextureID())
            {
                activeTexture(samplerUniform.boundTextureUnit);
                bindTexture(samplerUniform.textureType, textureGL->getTextureID());
            }
        ...
        {
            if (mTextures[samplerUniform.textureType][samplerUniform.boundTextureUnit] != 0)
            {
                activeTexture(samplerUniform.boundTextureUnit);
                bindTexture(samplerUniform.textureType, 0);
            }
        }
    }
