...
gl::Error StateManagerGL::resumeQuery(GLenum type)
{
    ...
    auto iter = mTemporaryPausedQueries.find(type);
    if (iter != mTemporaryPausedQueries.end())
    {
        ANGLE_TRY(iter->second->resume());
        mTemporaryPausedQueries.erase(iter);
    }
...
gl::Error StateManagerGL::onMakeCurrent(const gl::Context *context)
{
    ...
    ASSERT(mTemporaryPausedQueries.empty());
