...
gl::Error StateManagerGL::setGenericDrawState(const gl::Data &data)
{
    ...
    if (mFunctions->standard == STANDARD_GL_DESKTOP)
    {
        // Enable SRGB blending for all framebuffers except the default framebuffer on Desktop OpenGL.
        // When SRGB blending is enabled, only SRGB capable formats will use it but the default framebuffer will always use it if it is enabled.
