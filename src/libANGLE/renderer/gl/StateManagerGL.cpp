...
void StateManagerGL::setScissor(const gl::Rectangle &scissor, bool forceUpdate)
{
    if (scissor != mScissor || forceUpdate)
    {
        mScissor = scissor;
        if (!mIsDrawbufferSideBySide)
        {
        ...
        else
        {
            ...
            mFunctions->scissorArrayv(0u, static_cast<GLsizei>(mViewportOffsets.size()), scissorData.data());
        }
        mLocalDirtyBits.set(gl::State::DIRTY_BIT_SCISSOR);
    }
}

void StateManagerGL::setViewport(const gl::Rectangle &viewport, bool forceUpdate)
{
    if (viewport != mViewport || forceUpdate)
    {
        mViewport = viewport;
        if (!mIsDrawbufferSideBySide)
        {
...
void StateManagerGL::syncState(const gl::Context *context, const gl::State::DirtyBits &glDirtyBits)
{
    ...
    for (auto dirtyBit : glAndLocalDirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_SIDE_BY_SIDE_FRAMEBUFFER_TRANSITION:
                handleFramebufferSideBySideTransition(state.isDrawframebufferSideBySide(), state.getViewport(), state.getScissor(), state.isScissorTestEnabled(), state.getViewportOffsets());
                break;
...
void StateManagerGL::handleFramebufferSideBySideTransition(bool sideBySide, const gl::Rectangle &viewport, const gl::Rectangle &scissor, bool scissorTest, const std::vector<gl::Offset> *viewportOffsets)
{
    if (sideBySide != mIsDrawbufferSideBySide)
    {
        mIsDrawbufferSideBySide = sideBySide;
        if (!mIsDrawbufferSideBySide)
        {
            // If the new draw framebuffer doesn't have a side-by-side layout, we have to restore the viewport, scissor and scissor test state.
            setViewport(viewport, true);
            setScissor(scissor, true);
            setScissorTestEnabled(scissorTest);
        }
        else
        {
            ASSERT(viewportOffsets);
            bool viewportOffsetsDiffer = (mViewportOffsets != *viewportOffsets);
            if (viewportOffsetsDiffer)
            {
                // We always have to propagate the calls because the viewport offsets can differ.
