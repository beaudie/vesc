...
void StateManagerGL::setPixelUnpackState(const gl::PixelUnpackState &unpack)
{
    ...
    setPixelUnpackState(unpack.alignment, unpack.rowLength, unpack.skipRows,
...
{
    for (size_t dirtyBit : angle::IterateBitSet(state.getDirtyBits()))
    {
        switch (dirtyBit)
        {
            ...
            default:
            {
                ...
                setAttributeCurrentData(
                    attribIndex, state.getVertexAttribCurrentValue(static_cast<unsigned int>(attribIndex)));
                break;
            }
        }
    }
