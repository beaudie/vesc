...
VertexArrayGL::~VertexArrayGL()
{
    ...
    for (size_t idx = 0; idx < mAppliedBindings.size(); idx++)
    {
        mAppliedBindings[idx].buffer.set(nullptr);
    }
}
...
{
    const AttribFormat &format = mData.getVertexAttribute(attribIndex).getAttribFormat();
...
void VertexArrayGL::syncState(const VertexArray::DirtyBits &dirtyBits)
{
    for (unsigned long dirtyBit : angle::IterateBitSet(dirtyBits))
    {
                 ...
                 dirtyBit < VertexArray::DIRTY_BIT_BINDING_MAX_DIVISOR)
        {
            ...
            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
            const AttribBinding &binding = mData.getAttribBinding(bindingIndex);
