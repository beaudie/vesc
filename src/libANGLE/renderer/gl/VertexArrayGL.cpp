    ...
    else
    {
        ...
        if (features.shiftInstancedArrayDataWithExtraOffset.enabled && first > 0)
        {
            ...
            for (GLuint attribIndex = 0; attribIndex < gl::MAX_VERTEX_ATTRIBS; attribIndex++)
            {
                ...
                // const gl::VertexAttribute &attrib = mAppliedAttributes[attribIndex];
                // const gl::VertexBinding &binding  = mAppliedBindings[attrib.bindingIndex];

                if (binding.getDivisor() > 0 &&
                    !mForcedStreamingAttributesForDrawArraysInstancedMask.test(attribIndex))
                {
        ...
        for (auto idx : attribsToStream)
        {
            ...
            if (destStride == sourceStride)
            {
                if (features.shiftInstancedArrayDataWithExtraOffset.enabled && adjustedDivisor > 0)
                {
                    ...
                    // mStateManager->setAttributeCurrentDat
                    const size_t copySize =
                        destStride *
                        streamedVertexCount;  // the real data in the buffer we are streaming
                    const size_t offset =
                        destStride * indexRange.start;  // the extra offset at the begining we add
                    const size_t inputBuffersize = copySize + offset;
                    ...
                    mStateManager->bindBuffer(gl::BufferBinding::Array, buffer->getBufferID());

                    // The workaround is only for Mac Intel so getBufferSubData function should
                    // exist
                    ASSERT(mFunctions->getBufferSubData != nullptr);
                    mFunctions->getBufferSubData(
                        GL_ARRAY_BUFFER, binding.getOffset(), copySize,
                    ...
                    // auto ctx = const_cast<Context*>(context);
                    // ctx->getStateCache().updateActiveAttribsMask(ctx);
                    // mState.mClientMemoryAttribsMask.set(idx);
                }
                else
                {
            ...
            else
            {
                ...
                // TODO(crbug.com/1144207): workaround for this case as well
