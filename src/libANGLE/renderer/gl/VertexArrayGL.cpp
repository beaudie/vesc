...
bool SameVertexBuffer(const VertexBindingGL &a, const VertexBinding &b)
{
    return a.stride == b.getStride() && a.offset == b.getOffset() &&
           a.frontendBuffer == reinterpret_cast<uintptr_t>(b.getBuffer().get()) &&
           a.buffer == GetNativeBufferID(b.getBuffer().get());
}

bool SameIndexBuffer(const VertexArrayStateGL *a, const gl::Buffer *frontendBuffer)
{
    return a->frontendElementArrayBuffer == reinterpret_cast<uintptr_t>(frontendBuffer) &&
           a->elementArrayBuffer == GetNativeBufferID(frontendBuffer);
}
...
void VertexArrayGL::updateElementArrayBufferBinding(const gl::Context *context) const
{
    ...
    if (!SameIndexBuffer(mNativeState, elementArrayBuffer))
    {
        ...
        mNativeState->frontendElementArrayBuffer = reinterpret_cast<uintptr_t>(elementArrayBuffer);
                                           ...
                                           const void **outIndices) const
{
    ...
    else
    {
        ...
        mNativeState->frontendElementArrayBuffer = 0;
    ...
    bool applyExtraOffsetWorkaroundForInstancedAttributes) const
{
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
        ...
        for (auto idx : attribsToStream)
        {
            ...
            mNativeState->bindings[idx].frontendBuffer = 0;
    ...
    gl::AttributesMask *attributeMask) const
{
    ...
    for (auto idx : *attributeMask)
    {
        ...
        mNativeState->bindings[idx].frontendBuffer = reinterpret_cast<uintptr_t>(buffer);
...
void VertexArrayGL::updateAttribPointer(const gl::Context *context, size_t attribIndex)
{
    ...
    if (arrayBuffer == nullptr)
    {
        ...
        mNativeState->bindings[attribIndex].frontendBuffer = 0;
    ...
    mNativeState->bindings[attribIndex].frontendBuffer = reinterpret_cast<uintptr_t>(arrayBuffer);
...
void VertexArrayGL::updateBindingBuffer(const gl::Context *context, size_t bindingIndex)
{
    ...
    mNativeState->bindings[bindingIndex].frontendBuffer = reinterpret_cast<uintptr_t>(arrayBuffer);
