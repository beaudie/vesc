...
bool IsVertexAttribPointerValid(size_t attribIndex, const VertexAttribute &attrib)
{
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    // e.g. We should use VertexAttribFormat when relativeOffset > 0.
    ...
    if (SameVertexAttribFormat(attrib, mAppliedAttributes[attribIndex]) &&
        attrib.bindingIndex == mAppliedAttributes[attribIndex].bindingIndex &&
        SameBindVertexBuffer(binding, mAppliedBindings[attribIndex]))
    {
        return;
    }
...
inline void VertexArrayGL::callVertexAttribPointer(GLuint attribIndex,
                                                   GLuint size,
                                                   GLenum type,
