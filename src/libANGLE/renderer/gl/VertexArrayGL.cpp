...
bool VertexArrayGL::updateAttribEnabled(size_t attribIndex)
{
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex, bool streamingUpdated)
{
    ASSERT(isVertexAttribPointerValid(attribIndex));
    ...
    // We don't need to update mAttributesNeedStreaming if it has been updated in
    // updateAttribEnabled.
    if (!streamingUpdated)
    {
        updateNeedsStreaming(attribIndex);
...
void VertexArrayGL::syncState(ContextImpl *contextImpl, const VertexArray::DirtyBits &dirtyBits)
{
    ...
    AttributesMask needStreamingUpdated;
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        ...
        {
            needStreamingUpdated.set(index, updateAttribEnabled(index));
        }
        else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_POINTER &&
                 dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_POINTER)
        {
            updateAttribPointer(index, needStreamingUpdated[index]);
        }
