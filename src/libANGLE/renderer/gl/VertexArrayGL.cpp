...
void VertexArrayGL::updateAttribPointer(const gl::Context *context, size_t attribIndex)
{
    ...
    const GLuint bufferID         = arrayBufferGL->getBufferID();

    // Implement VertexAttribPointer by Vertex Attrib Binding APIs when driver supports this
    // feature.
    // 1. We can check and update the format, binding and attribBinding separately instead of always
    //    setting them together into the driver.
    // 2. We can avoid setting format, binding or attribBinding twice when both VertexAttribPointer
    //    and Vertex Attrib Binding APIs are called.
    //    e.g. Because VertexAttribPointer cannot set attrib.relativeOffset to non-zero values, we
    //    cannot avoid calling VertexAttribFormat when attrib.relativeOffset isn't zero. We can just
    //    set everything once when we update format, binding and attribBinding separately.
    if (supportVertexAttribBinding())
    {
        updateAttribFormat(attribIndex);
        updateAttribBinding(attribIndex);
        updateBindingBuffer(context, attribIndex);
    }
    else
    {
        ...
        ASSERT(mAppliedAttributes[attribIndex].relativeOffset == 0 &&
