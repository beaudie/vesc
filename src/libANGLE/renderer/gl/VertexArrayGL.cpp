    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
        ...
        for (auto idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask))
        {
            ...
            const uint8_t *inputPointer =
                reinterpret_cast<const uint8_t *>(binding.bindingPointer) +
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    // inputPointer = VertexBufferBinding.bindingPointer + VertexAttribute.relativeOffset
    const uint8_t *inputPointer = reinterpret_cast<const uint8_t *>(binding.bindingPointer) +
                                  static_cast<const uint8_t>(attrib.relativeOffset);
    if (attrib.pureInteger)
    {
...
void VertexArrayGL::syncState(const VertexArray::DirtyBits &dirtyBits)
{
    for (unsigned long dirtyBit : angle::IterateBitSet(dirtyBits))
    {
        ...
        {
            // TODO(jiawei.shao@intel.com): Vertex Buffer Bindings
            size_t attribIndex =
                static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_BINDING;
                 ...
                 dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_DIVISOR)
        {
            ...
            if (mAppliedVertexBufferBindings[bindingIndex].divisor != binding.divisor)
            {
                mStateManager->bindVertexArray(mVertexArrayID, getAppliedElementArrayBufferID());
                // TODO(jiawei.shao@intel.com): Vertex Buffer Bindings
                for (size_t attribIndex = 0; attribIndex < mAppliedAttributes.size(); attribIndex++)
                {
                    if (mAppliedAttributes[attribIndex].bindingIndex == bindingIndex)
                    {
                        mFunctions->vertexAttribDivisor(static_cast<GLuint>(attribIndex),
                                                        binding.divisor);
                    }
                }
