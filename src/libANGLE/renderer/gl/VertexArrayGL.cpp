...
void VertexArrayGL::updateAttribPointer(const gl::Context *context, size_t attribIndex)
{
    ...
    // Update needsStreaming when the buffer has been changed because only the changes in attrib
    // enabled/disabled and buffer can affect if an attribute needs to be streamed. The changes in
    // format and another non-nullptr attribPointer won't affect the value of
    // mAttributesNeedStreaming[attribIndex] (it keeps the same value as it was in the last draw).
    ...
    // We update vertex attributes that doesn't contain client memory pointers by Vertex Attrib
    // Binding APIs instead of VertexAttribPointer on platforms that support this feature.
    // The dirty bits for format, binding and attribBinding have already been set together with that
    // for attribPointer when VertexAttribPointer is called.
...
void VertexArrayGL::updateAttribFormat(size_t attribIndex)
{
    ...
    // If an attribute is using a client memory pointer, mAttributesNeedStreaming[attribIndex]
    // should be updated if necessary in updateAttribPointer() and its format and binding should
    // be updated in streamAttributes().
    const VertexBinding &binding = mData.getVertexBinding(attrib.bindingIndex);
...
void VertexArrayGL::syncState(const gl::Context *context, const VertexArray::DirtyBits &dirtyBits)
{
    ...
    for (size_t dirtyBit : dirtyBits)
    {
        ...
        // format, binding, attribBinding, and attribPointer. So these 4 dirty bits are set dirty
        // when VertexAttribPointer is called.
        // Skip updating format, binding, and attribBinding separately on platforms that don't
        // support
        /// Vertex Attrib Binding because they should be updated together in updateAttribPointer.
        ...
        // On platforms that don't support Vertex Attrib Binding, we can only update the
        // binding divisor by VertexAttribDivisor.
