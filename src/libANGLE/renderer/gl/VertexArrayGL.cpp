                                          ...
                                          const gl::IndexRange &indexRange) const
{
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
        ...
        for (auto idx : attribsToStream)
        {
            ...
            updateWithVertexAttribPointer(static_cast<GLuint>(idx), attrib.size, attrib.type,
...
{
    // Skip using VertexAttribPointer if it is not needed.
    // e.g. We cannot skip using VertexAttribFormat when relativeOffset > 0.
    const VertexAttribute &attrib = mData.getVertexAttribute(attribIndex);
    if (!IsVertexAttribPointerValid(attribIndex, attrib))
    {
        return;
    }

    const VertexBinding &binding  = mData.getVertexBinding(attribIndex);
    VertexBinding &appliedBinding = mAppliedBindings[attribIndex];
    // Here we need to update needStreaming only when the binding buffer has been changed.
    bool sameBindingBuffer = (binding.buffer.get() == appliedBinding.buffer.get());
    if (!sameBindingBuffer)
    {
        updateNeedsStreaming(attribIndex);
    }
    ...
    {
        // Set the flag on the binding that current attribute is using a client memory pointer as we
        // can only use client memory pointer on default VAO, where attrib.bindingIndex won't be
        // changed. If it doesn't use client memory pointer later, sameBindVertexBuffer will return
        // false and the caching will not skip it.
        mAppliedBindings[attribIndex].buffer.set(nullptr);
        return;
    }

    VertexAttribute &appliedAttrib = mAppliedAttributes[attribIndex];
    // We do not need to compare attrib.pointer because when we use a different client memory
    // pointer, we won't update attribPointer in this function.
    if ((sameBindingBuffer && SameBindingStrideAndOffset(binding, appliedBinding)) &&
        (SameVertexAttribFormat(attrib, appliedAttrib)) &&
        (attrib.bindingIndex == appliedAttrib.bindingIndex))
    {
        return;
    }
    ...
    updateWithVertexAttribPointer(static_cast<GLuint>(attribIndex), attrib.size, attrib.type,
                                  attrib.normalized, binding.stride, binding.offset,
                                  attrib.pureInteger);

    appliedAttrib.size           = attrib.size;
    appliedAttrib.type           = attrib.type;
    appliedAttrib.normalized     = attrib.normalized;
    appliedAttrib.pureInteger    = attrib.pureInteger;
    appliedAttrib.relativeOffset = attrib.relativeOffset;

    appliedAttrib.bindingIndex = attrib.bindingIndex;

    appliedBinding.stride = binding.stride;
    appliedBinding.offset = binding.offset;
    appliedBinding.buffer = binding.buffer;
}

void VertexArrayGL::updateWithVertexAttribPointer(GLuint attribIndex,
                                                  GLuint size,
                                                  GLenum type,
