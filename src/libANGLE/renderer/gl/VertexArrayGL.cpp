...
{
    return (attrib.enabled == true && binding.buffer.get() == nullptr);
}
}  // anonymous namespace
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    // Skip the attribute that is disabled and use a client memory pointer.
    if (binding.buffer.get() == nullptr)
    {
        ASSERT(attrib.enabled == false);
    ...
    {
        // Attributes with a pointer to client memory should be updated by streamAttributes().
