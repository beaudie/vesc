...
angle::Result VertexArrayGL::streamAttributes(const gl::Context *context,
                                              ...
                                              GLsizei instanceCount,
                                              const gl::IndexRange &indexRange,
                                              bool applyWorkaroundForInstancedAttributes) const
{
    const FunctionsGL *functions = GetFunctionsGL(context);
    StateManagerGL *stateManager = GetStateManagerGL(context);
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
        ...
        for (auto idx : attribsToStream)
        {
            ...
            const size_t firstIndex =
                (adjustedDivisor == 0 || applyWorkaroundForInstancedAttributes) ? indexRange.start
                                                                                : 0;
            ...
            if (applyWorkaroundForInstancedAttributes && adjustedDivisor > 0)
            {
