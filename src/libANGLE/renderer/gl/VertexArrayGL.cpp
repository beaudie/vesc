...
{
    return (attribute.enabled && attribute.getBinding().buffer.get() == nullptr);
}
}  // anonymous namespace
      ...
      mStreamingArrayBuffer(0)
{
    ...
    {
        mAppliedAttributes.emplace_back(i, &mAppliedBindings);
    }
}
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    if (mAppliedAttributes[attribIndex] == attrib &&
        mAppliedBindings[attrib.format.bindingIndex] == binding)
    {
        return;
    }
    ...
    size_t bindingIndex                   = format.bindingIndex;
    ...
    mAppliedAttributes[attribIndex].format.bindingIndex   = format.bindingIndex;
...
void VertexArrayGL::syncState(const VertexArray::DirtyBits &dirtyBits)
{
    for (unsigned long dirtyBit : angle::IterateBitSet(dirtyBits))
    {
                 ...
                 dirtyBit < VertexArray::DIRTY_BIT_BINDING_MAX_BUFFER)
        {
            ...
            ASSERT(mData.getVertexAttribute(index).format.bindingIndex == index);
        }
