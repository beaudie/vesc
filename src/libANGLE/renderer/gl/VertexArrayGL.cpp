...
namespace
{
bool AttributeNeedsStreaming(const VertexAttribute &attribute, const VertexBufferBinding &binding)
{
    return (attribute.enabled && binding.buffer.get() == nullptr);
}
...
VertexArrayGL::VertexArrayGL(const VertexArrayState &state,
    ...
    : VertexArrayImpl(state),
      ...
      mVertexAttribBindingAvailable(false)
{
    ASSERT(mFunctions);
    ...
    GLint maxVertexBufferBindings = maxVertexAttribs;
    if (mFunctions->vertexAttribBinding)
    {
        mFunctions->getIntegerv(GL_MAX_VERTEX_ATTRIB_BINDINGS, &maxVertexBufferBindings);
        ASSERT(maxVertexBufferBindings >= maxVertexAttribs);
        mVertexAttribBindingAvailable = true;
    }
    mAppliedVertexBufferBindings.resize(maxVertexBufferBindings);
...
VertexArrayGL::~VertexArrayGL()
{
    ...
    for (size_t idx = 0; idx < mAppliedVertexBufferBindings.size(); idx++)
    {
        mAppliedVertexBufferBindings[idx].buffer.set(nullptr);
    }
}
                                                   ...
                                                   size_t *outMaxAttributeDataSize) const
{
    ...
    *outMaxAttributeDataSize = 0;
    ...
    const auto &bindings = mData.getVertexBufferBindings();
    for (auto idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask))
    {
        ...
        *outStreamingDataSize +=
            typeSize * ComputeVertexAttributeElementCount(attrib, binding, indexRange.vertexCount(),
                                                          instanceCount);
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
        ...
        for (auto idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask))
        {
            ...
            const size_t streamedVertexCount = ComputeVertexAttributeElementCount(
                attrib, binding, indexRange.vertexCount(), instanceCount);
            ...
            // inputPointer = VertexBufferBinding.bindingPointer + VertexAttribute.relativeOffset
            const uint8_t *inputPointer =
                reinterpret_cast<const uint8_t *>(binding.bindingOffset + attrib.relativeOffset);
...
void VertexArrayGL::updateNeedsStreaming(size_t attribIndex)
{
    ...
    const VertexBufferBinding &binding = mData.getVertexBufferBinding(attrib.bindingIndex);
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    if (mAppliedAttributes[attribIndex] == attrib &&
        mAppliedVertexBufferBindings[attrib.bindingIndex] == binding)
    {
        return;
    }
    ...
    mAppliedVertexBufferBindings[bindingIndex].buffer = binding.buffer;
    ...
    mAppliedVertexBufferBindings[bindingIndex].stride         = binding.stride;
    mAppliedVertexBufferBindings[bindingIndex].bindingPointer = binding.bindingPointer;
}
...
void VertexArrayGL::syncState(const VertexArray::DirtyBits &dirtyBits)
{
    for (unsigned long dirtyBit : angle::IterateBitSet(dirtyBits))
    {
        ...
        {
            size_t attribIndex =
                static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_FORMAT;
            updateAttribPointer(attribIndex);
        }
        else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_ATTRIB_BINDING &&
                 dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_ATTRIB_BINDING)
        {
            size_t attribIndex =
                static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_ATTRIB_BINDING;

            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
            ASSERT(mData.getVertexAttribute(attribIndex).bindingIndex == attribIndex);
        }
        else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_BINDING_POINT &&
                 dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_BINDING_POINT)
        {
            size_t bindingIndex =
                static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_BINDING_POINT;

            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
            ASSERT(mData.getVertexAttribute(bindingIndex).bindingIndex == bindingIndex);
        }
                 ...
                 dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_DIVISOR)
        {
            ...
            const VertexBufferBinding &binding = mData.getVertexBufferBinding(bindingIndex);

            if (mAppliedVertexBufferBindings[bindingIndex].divisor != binding.divisor)
            {
                mStateManager->bindVertexArray(mVertexArrayID, getAppliedElementArrayBufferID());
                ...
                mAppliedVertexBufferBindings[bindingIndex].divisor = binding.divisor;
            }
        }
        else
            UNREACHABLE();
