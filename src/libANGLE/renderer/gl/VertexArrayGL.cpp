...
{
    return (attrib.enabled && binding.getBufferRawPointer() == nullptr);
}
...
bool SameVertexBuffer(const VertexBinding &a, const VertexBinding &b)
{
    return a.getStride() == b.getStride() && a.getOffset() == b.getOffset() &&
           a.getBufferRawPointer() == b.getBufferRawPointer();
}
...
VertexArrayGL::~VertexArrayGL()
{
    ...
    {
        binding.setBufferRawPointer(nullptr);
    }
}
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    const Buffer *arrayBuffer = binding.getBufferRawPointer();
    if (arrayBuffer == nullptr)
    {
        ...
        mAppliedBindings[bindingIndex].setBufferRawPointer(nullptr);
        return;
    }
    ...
    mAppliedBindings[bindingIndex].setBuffer(binding.getBuffer());
}
