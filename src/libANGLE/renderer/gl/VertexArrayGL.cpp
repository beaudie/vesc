...
gl::Error VertexArrayGL::syncDrawArraysState(const gl::AttributesMask &activeAttributesMask,
                                             ...
                                             GLsizei count) const
{
    return syncDrawState(activeAttributesMask, first, count, GL_NONE, nullptr, instanceCount, nullptr);
}
                                                   ...
                                                   size_t *outMaxAttributeDataSize) const
{
    ...
    *outMaxAttributeDataSize = 0;
    ...
    for (unsigned int idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask))
    {
        ...
        *outStreamingDataSize += typeSize * ComputeVertexAttributeElementCount(attrib, vertexCount, instanceCount);
                                          ...
                                          const gl::RangeUI &indexRange) const
{
    ...
    computeStreamingAttributeSizes(activeAttributesMask, instanceCount, indexRange, &streamingDataSize,
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0)
    {
             ...
             angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask))
        {
            ...
            const size_t streamedVertexCount = ComputeVertexAttributeElementCount(attrib, vertexCount, instanceCount);
...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    mStateManager->bindVertexArray(mVertexArrayID, getAppliedElementArrayBufferID());
    Buffer *arrayBuffer = attrib.buffer.get();
    ...
    mAppliedAttributes[attribIndex].buffer.set(arrayBuffer);
