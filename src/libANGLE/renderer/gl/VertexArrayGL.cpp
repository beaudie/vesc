                                       ...
                                       gl::RangeUI *outIndexRange, const GLvoid **outIndices) const
{
    ...
    if (mData.getElementArrayBuffer() != nullptr)
    {
        const BufferGL *bufferGL = GetImplAs<BufferGL>(mData.getElementArrayBuffer());
        GLuint elementArrayBufferID = bufferGL->getBufferID();
        if (elementArrayBufferID != mAppliedElementArrayBuffer.id())
        {
            mStateManager->bindBuffer(GL_ELEMENT_ARRAY_BUFFER, elementArrayBufferID);
            mAppliedElementArrayBuffer.set(mData.getElementArrayBuffer());
        }
