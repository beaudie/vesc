...
VertexArrayGL::~VertexArrayGL()
{
    ...
    if (mStreamingElementArrayBufferSize != 0)
    {
...
void VertexArrayGL::syncDrawArraysState(GLint first, GLsizei count) const
{
    mStateManager->bindVertexArray(mVertexArrayID);

    syncDrawState(first, count, GL_NONE, nullptr, nullptr);
}

void VertexArrayGL::syncDrawElementsState(GLsizei count, GLenum type, const GLvoid *indices, const GLvoid **outIndices) const
{
    mStateManager->bindVertexArray(mVertexArrayID);
    ...
    syncDrawState(0, count, type, indices, outIndices);
}

void VertexArrayGL::syncDrawState(GLint first, GLsizei count, GLenum type, const GLvoid *indices, const GLvoid **outIndices) const
{
    // Check if any attributes need to be streamed, determines if the index range needs to be computed
    bool attributesNeedStreaming = false;
    for (size_t idx = 0; idx < mAttributes.size(); idx++)
    {
        if (mAttributes[idx].enabled && mAttributes[idx].buffer.get() == nullptr)
        {
            attributesNeedStreaming = true;
            break;
        }
    }
    ...
    if (type != GL_NONE)
    {
        ...
        // Need to check the range of indices
        if (mElementArrayBuffer.get() != nullptr)
        {
            // Only compute the index range if the attributes also need to be streamed
            if (attributesNeedStreaming)
            {
                size_t elementArrayBufferOffset = reinterpret_cast<size_t>(indices);

                // Find the index range in the buffer
                const IndexRangeCache* rangeCache = mElementArrayBuffer.get()->getIndexRangeCache();

                unsigned int streamOffset = 0;
                if (!rangeCache->findRange(type, elementArrayBufferOffset, count, &indexRange, &streamOffset))
                {
                    // Need to compute the index range.
                    const BufferGL *bufferGL = GetImplAs<BufferGL>(mElementArrayBuffer.get());

                    mStateManager->bindBuffer(GL_ELEMENT_ARRAY_BUFFER, bufferGL->getBufferID());
                    uint8_t *elementArrayBufferPointer = reinterpret_cast<uint8_t*>(mFunctions->mapBuffer(GL_ARRAY_BUFFER, GL_READ_ONLY));

                    indexRange = IndexRangeCache::ComputeRange(type, elementArrayBufferPointer + elementArrayBufferOffset, count);

                    // TODO: Store the range cache at the impl level since the gl::Buffer object is supposed to remain constant
                    const_cast<IndexRangeCache*>(rangeCache)->addRange(type, elementArrayBufferOffset, count, indexRange, 0);
        ...
        else
        {
            ...
            if (attributesNeedStreaming)
            {
                indexRange = IndexRangeCache::ComputeRange(type, indices, count);
            }

            // Allocate the streaming element array buffer
            if (mStreamingElementArrayBuffer == 0)
            {
                mFunctions->genVertexArrays(1, &mStreamingElementArrayBuffer);
                mStreamingElementArrayBufferSize = 0;
            }
    ...
    for (size_t idx = 0; idx < mAttributes.size(); idx++)
    {
        ...
        if (mAttributes[idx].enabled && mAttributes[idx].buffer.get() == nullptr)
        {
            ...
            streamingDataSize += typeSize * streamedVertexCount;
    ...
    if (streamingDataSize > 0)
    {
        ...
        if (mStreamingArrayBuffer == 0)
        {
            mFunctions->genVertexArrays(1, &mStreamingArrayBuffer);
            mStreamingArrayBufferSize = 0;
        }
