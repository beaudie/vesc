...
void VertexArrayGL::setElementArrayBuffer(const gl::Buffer *buffer)
{
    ...
    {
        mStateManager->setVertexArray(mVertexArrayID);
        mStateManager->setBuffer(GL_ELEMENT_ARRAY_BUFFER, elementArrayBufferID);
        mStateManager->setVertexArray(0);
...
void VertexArrayGL::setAttribute(size_t idx, const gl::VertexAttribute &attr)
{
        ...
        mAppliedAttributes[idx].buffer.get() != attr.buffer.get())
    {
        ...
        if (arrayBuffer != nullptr)
        {
            ...
            mStateManager->setBuffer(GL_ARRAY_BUFFER, arrayBufferGL->getBufferID());
        }
        else
        {
