...
void VertexArrayGL::updateAttribPointer(size_t attribIndex)
{
    ...
    if (UseClientMemoryPointer(binding))
    {
        ...
        // can only use client memory pointer on a default VAO, where attrib.bindingIndex won't be
        // changed.
        // If it doesn't need to be streamed later, sameBindVertexBuffer will return false and the
        // caching will not skip it.
