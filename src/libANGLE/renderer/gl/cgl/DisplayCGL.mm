...
egl::Error DisplayCGL::initialize(egl::Display *display)
{
    ...
    {
        auto gpuID = info.getPreferredDeviceSystemDeviceId();
        if (gpuID)
        {
...
egl::Error DisplayCGL::setGPU(EGLint gpuIDHigh, EGLint gpuIDLow)
{
    if (mSupportsGPUSwitching)
    {
...
void DisplayCGL::setGPU(uint64_t gpuID, GLint virtualScreen)
{
    CGLError error = CGLSetVirtualScreen(mContext, virtualScreen);
    ASSERT(error == kCGLNoError);
    if (error == kCGLNoError)
