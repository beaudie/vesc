...
WindowSurfaceCGL::WindowSurfaceCGL(RendererGL *renderer,
    ...
    : SurfaceGL(renderer),
      ...
      mFramebuffer(0)
{
    for (size_t i = 0; i < ArraySize(mSurfaces); i++)
    {
...
egl::Error WindowSurfaceCGL::initialize()
{
    ...
    CFDictionaryRef ioSurfaceOptions = nil;
    {
        ...
        size_t bytesPerRow =
            IOSurfaceAlignProperty(kIOSurfaceBytesPerRow, width * kBytesPerElement);
        size_t totalBytes = IOSurfaceAlignProperty(kIOSurfaceAllocSize, height * bytesPerRow);

        NSDictionary *options = @{
            (id) kIOSurfaceWidth : @(width), (id)
            kIOSurfaceHeight : @(height), (id)
            kIOSurfacePixelFormat : @(pixelFormat), (id)
            kIOSurfaceBytesPerElement : @(kBytesPerElement), (id)
            kIOSurfaceBytesPerRow : @(bytesPerRow), (id)
    ...
    for (size_t i = 0; i < ArraySize(mSurfaces); i++)
    {
        ...
        CGLError error = CGLTexImageIOSurface2D(CGLGetCurrentContext(), GL_TEXTURE_RECTANGLE_ARB,
                                                GL_RGBA, width, height, GL_BGRA,
                                                GL_UNSIGNED_INT_8_8_8_8_REV, surface.ioSurface, 0);
...
egl::Error WindowSurfaceCGL::swap()
{
    ...
    mStateManager->bindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);
    mFunctions->framebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_RECTANGLE_ARB,
                                     mSurfaces[mCurrentSurface].texture, 0);
