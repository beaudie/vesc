...
egl::Error DisplayWGL::initialize(egl::Display *display)
{
    ...
    EGLint requestedDisplayType = displayAttributes.get(EGL_PLATFORM_ANGLE_TYPE_ANGLE,
    ...
    {
        return egl::Error(EGL_NOT_INITIALIZED, "Cannot create an OpenGL ES platform on Windows without "
                                               "the WGL_EXT_create_context_es_profile extension.");
    }
    ...
    if (mFunctionsWGL->createContextAttribsARB)
    {
        ...
        {
            // the ES profile will give us ES version 1.1 unless a higher version is requested. Requesting
            // version 2.0 will give us the highest compatible version available (2.0, 3.0, 3.1, etc).
            if (requestedDisplayType == EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE)
            {
                contextCreationAttibutes.push_back(WGL_CONTEXT_MAJOR_VERSION_ARB);
                contextCreationAttibutes.push_back(2);
