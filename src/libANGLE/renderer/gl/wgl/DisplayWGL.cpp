...
egl::Error DisplayWGL::initialize(egl::Display *display)
{
    ...
    mHasARBCreateContextRobustness = mFunctionsWGL->hasExtension("WGL_ARB_create_context_robustness");
    ...
    if (mFunctionsWGL->createContextAttribsARB)
    {
        ...
        {
            contextCreationAttributes.push_back(GLX_CONTEXT_RESET_NOTIFICATION_STRATEGY_ARB);
            contextCreationAttributes.push_back(GLX_LOSE_CONTEXT_ON_RESET_ARB);
        }
