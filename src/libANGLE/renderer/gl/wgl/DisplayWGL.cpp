...
HGLRC DisplayWGL::initializeContextAttribs(const egl::AttributeMap &eglAttributes) const
{
    ...
    // Try all the GL version in order. Assume we are not on Mesa since there is no way to detect
    // it.
    for (const auto &info : GenerateContextCreationToTry(false))
    {
        ...
        if (info.displayType == EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE)
        {
