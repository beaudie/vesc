...
void DisplayWGL::destroy()
{
    ...
    if (mFunctionsWGL)
    {
        if (mSharedContext)
        {
            ...
            mSharedContext = nullptr;
            // Release the free worker windows.
...
WorkerContext *DisplayWGL::createWorkerContext(std::string *infoLog)
{
    if (!mSharedContext)
    {
        *infoLog += "Unable to create shared contexts.";
        return nullptr;
    }
...
#define CLEANUP_ON_ERROR()                            \
    ...
    {
        *infoLog += "Unable to create worker windows.";
        return nullptr;
    }
    ...
    {
        workerContext =
            mFunctionsWGL->createContextAttribsARB(mDeviceContext, mSharedContext, &mAttribs[0]);
    }
    else
    {
