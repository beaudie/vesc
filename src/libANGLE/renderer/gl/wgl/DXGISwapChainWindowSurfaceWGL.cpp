...
egl::Error DXGISwapChainWindowSurfaceWGL::initialize()
{
    ...
    if (dxgiFactory2)
    {
        ...
        HRESULT result = dxgiFactory2->CreateSwapChainForHwnd(mDevice, mWindow, &swapChainDesc,
                                                              nullptr, nullptr, &mSwapChain1);
    ...
    {
        DWORD error = GetLastError();
        return egl::Error(EGL_BAD_ALLOC, "Failed to lock D3D object, error: 0x%X.",
                          HRESULT_CODE(error));
    }
                                                        ...
                                                        EGLint height)
{
    ...
    {
        RECT rect = {static_cast<LONG>(x), static_cast<LONG>(y), static_cast<LONG>(x + width),
...
egl::Error DXGISwapChainWindowSurfaceWGL::checkForResize()
{
    ...
    if (newWidth != mWidth || newHeight != mHeight)
    {
        ...
        HRESULT result =
            mSwapChain->ResizeBuffers(0, static_cast<UINT>(newWidth), static_cast<UINT>(newHeight),
                                      mSwapChainFormat, mSwapChainFlags);
        if (FAILED(result))
        {
        ...
        if (FAILED(result))
        {
            return egl::Error(EGL_BAD_ALLOC,
                              "Failed to query texture from swap chain, result: 0x%X", result);
        }
        ...
        if (hadBoundSurface)
        {
            mTextureHandle = mFunctionsWGL->dxRegisterObjectNV(
                mDeviceHandle, colorBuffer, mTextureID, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
            if (mTextureHandle == nullptr)
            {
                DWORD error = GetLastError();
                return egl::Error(EGL_BAD_ALLOC, "Failed to register D3D object, error: 0x%X.",
