...
egl::Error DXGISwapChainWindowSurfaceWGL::initialize()
{
    ...
    IDXGIFactory2 *dxgiFactory2 = DynamicCastComObject<IDXGIFactory2>(dxgiFactory);
    if (dxgiFactory2)
    {
        ...
        HRESULT result = dxgiFactory2->CreateSwapChainForHwnd(mDevice, mWindow, &swapChainDesc, nullptr, nullptr, &mSwapChain1);
        ...
        mSwapChain = DynamicCastComObject<IDXGISwapChain>(mSwapChain1);
    }
    else
    {
    ...
    HRESULT result = mSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&colorBuffer));
    if (FAILED(result))
    {
    ...
    mRenderbufferBufferHandle = mFunctionsWGL->dxRegisterObjectNV(mDeviceHandle, colorBuffer, mColorRenderbufferID, GL_RENDERBUFFER, WGL_ACCESS_READ_WRITE_NV);
    ...
    mFunctionsGL->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, mColorRenderbufferID);
    ...
    if (mDepthBufferFormat != GL_NONE)
    {
        ...
        mFunctionsGL->renderbufferStorage(GL_RENDERBUFFER, mDepthBufferFormat, static_cast<GLsizei>(mWidth), static_cast<GLsizei>(mHeight));
        ...
        {
            mFunctionsGL->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, mDepthRenderbufferID);
        }
        if (depthStencilFormatInfo.stencilBits > 0)
        {
            mFunctionsGL->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, mDepthRenderbufferID);
        }
    }
...
egl::Error DXGISwapChainWindowSurfaceWGL::postSubBuffer(EGLint x, EGLint y, EGLint width, EGLint height)
{
    ...
    else
    {
        ...
        {
            static_cast<LONG>(x),
            static_cast<LONG>(mHeight - y - height),
            static_cast<LONG>(x + width),
            static_cast<LONG>(mHeight - y)
        };
...
egl::Error DXGISwapChainWindowSurfaceWGL::bindTexImage(gl::Texture *texture, EGLint buffer)
{
    ...
    mTextureHandle = mFunctionsWGL->dxRegisterObjectNV(mDeviceHandle, colorBuffer, textureID, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
    SafeRelease(colorBuffer);
    ...
    if (!mFunctionsWGL->dxLockObjectsNV(mDeviceHandle, 1, &mTextureHandle))
    {
        ...
        DWORD error = GetLastError();
        return egl::Error(EGL_BAD_ALLOC, "Failed to lock D3D object, error: 0x%08x.", HRESULT_CODE(error));
    }
...
egl::Error DXGISwapChainWindowSurfaceWGL::releaseTexImage(EGLint buffer)
{
    ...
    {
        DWORD error = GetLastError();
        return egl::Error(EGL_BAD_ALLOC, "Failed to unlock D3D object, error: 0x%08x.", HRESULT_CODE(error));
    }
...
FramebufferImpl *DXGISwapChainWindowSurfaceWGL::createDefaultFramebuffer(const gl::Framebuffer::Data &data)
{
...
egl::Error DXGISwapChainWindowSurfaceWGL::setObjectsLocked(bool locked)
{
    ...
    GLint count = mTextureHandle ? 2 : 1;
    ...
    if (locked)
    {
        ...
        {
            DWORD error = GetLastError();
            return egl::Error(EGL_BAD_ALLOC, "Failed to lock object, error: 0x%08x.", HRESULT_CODE(error));
        }
    }
    else
    {
...
egl::Error DXGISwapChainWindowSurfaceWGL::checkForResize()
{
    ...
    if (newWidth != mWidth || newHeight != mHeight)
    {
        ...
        HRESULT result = mSwapChain->ResizeBuffers(0, static_cast<UINT>(newWidth), static_cast<UINT>(newHeight), mSwapChainFormat, mSwapChainFlags);
        if (FAILED(result))
        {
            return egl::Error(EGL_BAD_ALLOC, "Failed to resize swap chain, result: 0x%X", result);
        }
        ...
        if (hadBoundSurface)
        {
            mTextureHandle = mFunctionsWGL->dxRegisterObjectNV(mDeviceHandle, colorBuffer, mTextureID, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
            if (mTextureHandle == nullptr)
            {
                DWORD error = GetLastError();
                return egl::Error(EGL_BAD_ALLOC, "Failed to register D3D object, error: 0x%X.", HRESULT_CODE(error));
        ...
        // Rebind the renderbuffer to the framebuffer.
        mStateManager->bindFramebuffer(GL_FRAMEBUFFER, mFramebufferID);
        mFunctionsGL->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, mColorRenderbufferID);
