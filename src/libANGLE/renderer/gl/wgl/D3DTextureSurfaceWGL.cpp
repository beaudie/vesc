...
egl::Error D3DTextureSurfaceWGL::initialize()
{
    ...
    if (SUCCEEDED(buffer->QueryInterface<IDirect3DSurface9>(&surface9)))
    {
    ...
    else if (SUCCEEDED(buffer->QueryInterface<ID3D11Texture2D>(&texture11)))
    {
    ...
    {
        return egl::Error(EGL_BAD_PARAMETER,
                          "Provided buffer is not a IDirect3DDevice9 or ID3D11Texture2D.");
    }
