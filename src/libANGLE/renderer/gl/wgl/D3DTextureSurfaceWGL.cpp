...
egl::Error D3DTextureSurfaceWGL::initialize(const DisplayImpl *displayImpl)
{
    ...
    GLenum dsFormat = mState.config->depthStencilFormat;
    if (dsFormat != GL_NONE)
    {
        ...
        mFunctionsGL->renderbufferStorage(GL_RENDERBUFFER, dsFormat, static_cast<GLsizei>(mWidth),
    ...
    mFunctionsGL->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                          mDepthStencilRenderbufferID);
