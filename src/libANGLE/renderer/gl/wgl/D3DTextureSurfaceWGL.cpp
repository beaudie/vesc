...
D3DTextureSurfaceWGL::D3DTextureSurfaceWGL(RendererGL *renderer,
...
egl::Error D3DTextureSurfaceWGL::initialize()
{
    ...
    mBoundObjectRenderbufferHandle = mFunctionsWGL->dxRegisterObjectNV(mDeviceHandle, mObject, mRenderbufferID, GL_RENDERBUFFER, WGL_ACCESS_READ_WRITE_NV);
...
egl::Error D3DTextureSurfaceWGL::bindTexImage(gl::Texture *texture, EGLint buffer)
{
    ...
    mBoundObjectTextureHandle = mFunctionsWGL->dxRegisterObjectNV(mDeviceHandle, mObject, textureID, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
    ...
    if (mObjectsLocked && !mFunctionsWGL->dxLockObjectsNV(mDeviceHandle, 1, &mBoundObjectTextureHandle))
    {
        DWORD error = GetLastError();
...
egl::Error D3DTextureSurfaceWGL::lock()
{
    return setObjectsLocked(false);
}

egl::Error D3DTextureSurfaceWGL::unlock()
{
    return setObjectsLocked(true);
}

FramebufferImpl *D3DTextureSurfaceWGL::createDefaultFramebuffer(const gl::Framebuffer::Data &data)
{
    return new FramebufferGL(mFramebufferID, data, mFunctionsGL, mWorkarounds, mStateManager);
}

egl::Error D3DTextureSurfaceWGL::setObjectsLocked(bool locked)
{
    HANDLE resources[] =
    {
    ...
    GLint count = mBoundObjectTextureHandle ? 2 : 1;
