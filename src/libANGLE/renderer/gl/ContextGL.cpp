...
VertexArrayImpl *ContextGL::createVertexArray(const gl::VertexArrayState &data)
{
    ...
    // (syncVertexArraysToDefault) or for the frontend default vertex array so that client data can
    // be used directly
    if (features.syncVertexArraysToDefault.enabled ||
        (data.isDefault() && mState.areClientArraysEnabled() &&
         nativegl::CanUseDefaultVertexArrayObject(functions)))
    {
        StateManagerGL *stateManager = getStateManager();
