...
ContextGL::ContextGL(const gl::ContextState &state, RendererGL *renderer) : ContextImpl(state), mRenderer(renderer)
{
}
...
TextureImpl *ContextGL::createTexture(GLenum target)
{
    return new TextureGL(target, getFunctions(), getWorkaroundsGL(), getStateManager(), mRenderer->getBlitter());
}

RenderbufferImpl *ContextGL::createRenderbuffer()
{
    return new RenderbufferGL(getFunctions(), getWorkaroundsGL(), getStateManager(), getNativeTextureCaps());
}
...
gl::Error ContextGL::drawArrays(GLenum mode,
                                GLint first,
                                GLsizei count)
{
...
gl::Error ContextGL::drawElementsInstanced(GLenum mode,
                                           ...
                                           const gl::IndexRange &indexRange)
{
    return mRenderer->drawElementsInstanced(mState, mode, count, type, indices, instances, indexRange);
}
