                                                         ...
                                                         GLuint baseInstance)
{
    ...
    else
    {
        ...
        if (baseInstance != 0)
        {
            ...
            for (size_t attribIndex = 0; attribIndex < gl::MAX_VERTEX_ATTRIBS; attribIndex++)
            {
                ...
                if (program->isAttribLocationActive(attribIndex) && binding.getDivisor() != 0)
                {
                    ...
                    // mState.getVertexArray()->setVertexAttribPointerBaseInstance(context,
                    // attribIndex,
                    //                                                             baseInstance,
                    //                                                             attrib.pointer);

                    // char *p = static_cast<char *>(const_cast<void *>(attrib.pointer));
                    const char *p          = static_cast<const char *>(attrib.pointer);
                    const void *newPointer = p + attrib.format->pixelBytes * baseInstance;
                    functions->vertexAttribPointer((GLuint)attribIndex, attrib.format->channelCount,
                                                   gl::ToGLenum(attrib.format->vertexAttribType),
                                                   attrib.format->isNorm(),
                                                   attrib.vertexAttribArrayStride, newPointer);

                    // mState.getVertexArray()->callVertexAttribPointer(static_cast<GLuint>(attribIndex),
                    // attrib, binding.getStride(),
                    //         binding.getOffset());

                    // bindVertexBufferImpl(context, attribIndex, boundBuffer, offset,
                    // effectiveStride);

                    // mState.setVertexAttribPointer(
                    //     context,
                    //     attribIndex,
                    //     binding.getBuffer(),
                    //     attrib.format->pixelBytes,
                    //     attrib.format->componentType,
                    //     false, // normalized (temp)
                    //     0,      // stride
                    //     reinterpret_cast<const void*>(baseInstance)
                    // );

                    // attribIdsNeedReset.push_back(attribIndex);
                }
            }
        }
        ...
        {
            // mState.getVertexArray()->setVertexAttribPointerBaseInstance(context, p.first,
            //                                                                     0, p.second);
            ...
            functions->vertexAttribPointer((GLuint)p.first, attrib.format->channelCount,
                                           gl::ToGLenum(attrib.format->vertexAttribType),
                                           attrib.format->isNorm(), attrib.vertexAttribArrayStride,
                                           p.second);
        }
    }
