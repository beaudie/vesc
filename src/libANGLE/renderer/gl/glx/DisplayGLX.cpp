...
glx::Context DisplayGLX::initializeContext(glx::FBConfig config, const egl::AttributeMap &eglAttributes)
{
    // Create a context of the requested version, if any.
    EGLint requestedMajorVersion = eglAttributes.get(EGL_PLATFORM_ANGLE_MAX_VERSION_MAJOR_ANGLE, EGL_DONT_CARE);
    EGLint requestedMinorVersion = eglAttributes.get(EGL_PLATFORM_ANGLE_MAX_VERSION_MINOR_ANGLE, EGL_DONT_CARE);
    ...
    {
        auto& version = desktopVersions[i];
...
egl::ConfigSet DisplayGLX::generateConfigs() const
{
    ...
    for (int i = 0; i < glxConfigCount; i++)
    {
        ...
        // configuration.
        if (!(config.depthSize == 24 && config.stencilSize == 8) &&
            (mIsMesa || !(config.depthSize == 0 && config.stencilSize == 0)))
        {
            continue;
        }
