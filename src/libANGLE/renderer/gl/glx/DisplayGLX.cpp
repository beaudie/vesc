...
egl::Error DisplayGLX::initialize(egl::Display *display)
{
    ...
    // Check we have a supported version of GLX
    {
        ...
        ASSERT(extensions);
        mGLXExtensions = TokenizeExtensionsString(extensions);
    ...
    mDummyPbuffer = glXCreatePbuffer(mXDisplay, contextConfig, nullptr);
    glXMakeCurrent(mXDisplay, mDummyPbuffer, mContext);
...
void DisplayGLX::terminate()
{
    ...
    if (mDummyPbuffer)
    {
        glXDestroyPbuffer(mXDisplay, mDummyPbuffer);
        mDummyPbuffer = 0;
    }
