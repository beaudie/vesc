...
egl::Error DisplayGLX::initialize(egl::Display *display)
{
    ...
    {
        GLenum status =
            functionsGL->getGraphicsResetStatus() if (status != GL_NO_ERROR &&
                                                      status != GL_PURGED_CONTEXT_RESET_NV)
        {
            return egl::EglNotInitialize() << "Context lost for unknown reason.";
        }
    }
                                       ...
                                       const egl::AttributeMap &attribs)
{
    ...
    return new ContextGL(state, errorSet, mRenderer, robustnessVideoMemoryPurgeRequested);
}
