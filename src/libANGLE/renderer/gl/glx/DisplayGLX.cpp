...
egl::Error DisplayGLX::waitNative() const
{
    // eglWaitNative is used to notice the driver of changes in X11 for the current surface, such as changes of the window size. We use this event to update the child window of WindowSurfaceGLX to match its parent window's size.
    // Handling eglWaitNative this way helps the application control when resize happens. This is important because drivers have a tendency to clobber the back buffer when the windows are resized. See Chromium bug #326995.
