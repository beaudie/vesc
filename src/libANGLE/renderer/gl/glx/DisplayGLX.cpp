...
{
    mEGLDisplay = display;
    mDisplay = display->getNativeDisplayId();
    ...
    // Check we have a supported version of GLX
    {
        int major;
        int minor;
        if (!glXQueryVersion(mDisplay, &major, &minor))
        {
            return egl::Error(EGL_NOT_INITIALIZED, "Could not query the GLX version.");
        }
        if (major != 1 || minor < 3)
        {
            return egl::Error(EGL_NOT_INITIALIZED, "Unsupported GLX version.");
        }

        const char *extensions = glXQueryExtensionsString(mDisplay, DefaultScreen(mDisplay));
        ASSERT(extensions);
        mGLXExtensions = ParseExtensions(extensions);
        ...
        // TODO(cwallez) is this too restrictive? IIRC Chrome requires it too.
    ...
    // when it queries EGL.
    {
        ...
        GLXFBConfig* candidates = glXChooseFBConfig(mDisplay, DefaultScreen(mDisplay), attribList, &nConfigs);
        if (nConfigs == 0)
        {
            XFree(candidates);
    ...
    mContext = glXCreateContextAttribsARB(mDisplay, contextConfig, nullptr, True, nullptr);
    if (!mContext)
    {
    ...
    GLXPbuffer pbuffer = glXCreatePbuffer(mDisplay, contextConfig, nullptr);
    glXMakeCurrent(mDisplay, pbuffer, mContext);
...
egl::Error DisplayGLX::createWindowSurface(const egl::Config *configuration, EGLNativeWindowType window,
                                           const egl::AttributeMap &attribs, SurfaceImpl **outSurface)
{
    ...
    WindowSurfaceGLX *surface = new WindowSurfaceGLX(window, mDisplay, mContext, fbConfig);
...
egl::Error DisplayGLX::createPbufferSurface(const egl::Config *configuration, const egl::AttributeMap &attribs,
                                            SurfaceImpl **outSurface)
{
    //TODO WGL implements it
    UNIMPLEMENTED();
    return egl::Error(EGL_SUCCESS);
}

egl::Error DisplayGLX::createPbufferFromClientBuffer(const egl::Config *configuration, EGLClientBuffer shareHandle,
                                                     const egl::AttributeMap &attribs, SurfaceImpl **outSurface)
{
    UNIMPLEMENTED();
...
egl::Error DisplayGLX::createPixmapSurface(const egl::Config *configuration, NativePixmapType nativePixmap,
                                           const egl::AttributeMap &attribs, SurfaceImpl **outSurface)
{
    UNIMPLEMENTED();
    return egl::Error(EGL_BAD_DISPLAY);
...
egl::ConfigSet DisplayGLX::generateConfigs() const
{
    ...
    int glxConfigCount;
    GLXFBConfig *glxConfigs = glXGetFBConfigs(mDisplay, DefaultScreen(mDisplay), &glxConfigCount);
    ...
    for (int i = 0; i < glxConfigCount; i++)
    {
        ...
        // The spec isn't super clear, assume the buffer size is with alpha included.
        ...
        {
            config.configCaveat = EGL_NON_CONFORMANT_CONFIG;
        }
        ...
        // TODO(cwallez) for now I have no idea what the right values are for these
        config.maxSwapInterval = 8;
...
bool DisplayGLX::isValidNativeWindow(EGLNativeWindowType window) const
{
    ...
    unsigned nChildren;
    int status = XQueryTree(mDisplay, window, &root, &parent, &children, &nChildren);
    if (children)
    {
        XFree(children);
    }
...
{
    int result;
    glXGetFBConfigAttrib(mDisplay, config, attrib, &result);
    return result;
}
