...
void DisplayGLX::setSwapInterval(glx::Drawable drawable, SwapControlData *data)
{
    ...
    if (mSwapControl == SwapControl::EXT)
    {
        ...
        if (data->currentSwapInterval != data->targetSwapInterval)
        {
            ...
            // Don't set to realInterval as it would make it seem like the current swap interval
            // is always dirty. Instead set to the pre-clamping interval that will stay the same
