...
// GLXLoader.h: Implements the GLXLoader class.

#include "libANGLE/renderer/gl/glx/GLXLoader.h"
...
template<typename T>
bool GetProc(PFNGLXGETPROCADDRESSPROC getProc, T* member, const char *name)
{
...
egl::Error GLXLoader::initialize(Display *xDisplay)
{
    terminate();

    mLibHandle = dlopen("libGL.so", RTLD_NOW);
    if (!mLibHandle)
    {
        return egl::Error(EGL_NOT_INITIALIZED, "Could not dlopen libGL.so: %s", dlerror());
    }
...
bool GLXLoader::hasExtension(const char *extension) const
{
