                                      ...
                                      const gl::InternalFormat &internalFormat)
{
    ...
    else if (functions->isAtLeastGLES(gl::Version(3, 0)))
    {
        if (internalFormat.componentType == GL_FLOAT)
        {
            ...
            else if (internalFormat.isLuma() && internalFormat.sized &&
                     !functions->hasGLESExtension("GL_EXT_texture_storage"))
            {
                result = EmulateLUMA(internalFormat).sizedInternalFormat;
            }
                                     ...
                                     GLenum internalFormat)
{
    ...
    if (features.emulateSizedLuminanceAlphaFormats.enabled && sizedFormatInfo.sized &&
        sizedFormatInfo.isLUMA())
    {
        sizedFormatInfo = EmulateLUMA(sizedFormatInfo);
    }
