...
// supported = (requiredExt || altRequiredExt)
static inline SupportRequirement ExtsOrAltExts(const std::string &requiredExt,
                                               const std::string &altRequiredExt)
{
    SupportRequirement requirement;
    angle::SplitStringAlongWhitespace(requiredExt, &requirement.requiredExtensions);
    angle::SplitStringAlongWhitespace(altRequiredExt, &requirement.altRequiredExtensions);
    return requirement;
}
...
// sized formats, due to D3D supporting only sized formats. Thus, sized formats support in this map
// is an OR of the actual sized format support and the corresponding unsized format support.
// For example, GL_EXT_texture_rg provides support for RED_EXT format with UNSIGNED_BYTE type.
// Therefore, DetermineRGTextureSupport checks for GL_R8 support. Therefore, in this map,
// GL_R8 support values combine those of GL_R8 and RED_EXT format with UNSIGNED_BYTE type.
// Note 2: Texture Attachment support is checked also by SupportsNativeRendering().
// The assumption is that it will not check sized formats in ES2 frontend and the information
// in unsized formats is correct. BTW, this is the reason for unsized formats appearing in this map.
// In ES3 frontend, it could happen that SupportsNativeRendering() would be wrong,
// but this will be mitigated by data upload fallback.
// TODO(ynovikov): Check if there are indeed scenarios where SupportsNativeRendering() is wrong.
...
// Note 4: "AlwaysSupported" in "Filter" actually means - on the condition of "Texture supported".
// This is enforced by ANDing the two values in GenerateTextureFormatCaps().
// Texture attachment, however, also included the conditions for texture creation to be supported.
// GL_EXT_color_buffer_float implies ES3.0 for that purpose.
// TODO(ynovikov): verify support of BGRA, depth, stencil and compressed formats, and Desktop GL.
static InternalFormatInfoMap BuildInternalFormatInfoMap()
{
    ...
    InsertFormatMapping(&map, GL_ALPHA16F_EXT,           VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_half_float"), ExtsOnly("GL_OES_texture_half_float_linear"), NeverSupported(),                      NeverSupported()                );
    InsertFormatMapping(&map, GL_LUMINANCE16F_EXT,       VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_half_float"), ExtsOnly("GL_OES_texture_half_float_linear"), NeverSupported(),                      NeverSupported()                );
    InsertFormatMapping(&map, GL_LUMINANCE_ALPHA16F_EXT, VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_half_float"), ExtsOnly("GL_OES_texture_half_float_linear"), NeverSupported(),                      NeverSupported()                );
    InsertFormatMapping(&map, GL_ALPHA32F_EXT,           VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_float"),      ExtsOnly("GL_OES_texture_float_linear"),      NeverSupported(),                      NeverSupported()                );
    InsertFormatMapping(&map, GL_LUMINANCE32F_EXT,       VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_float"),      ExtsOnly("GL_OES_texture_float_linear"),      NeverSupported(),                      NeverSupported()                );
    InsertFormatMapping(&map, GL_LUMINANCE_ALPHA32F_EXT, VersionOrExts(3, 0, "GL_ARB_texture_float"), AlwaysSupported(), NeverSupported(), ExtsOnly("OES_texture_float"),      ExtsOnly("GL_OES_texture_float_linear"),      NeverSupported(),                      NeverSupported()                );
