                              ...
                              GLenum type)
{
    ...
    else if (functions->isAtLeastGLES(gl::Version(3, 0)))
    {
                 ...
                 !functions->hasGLExtension("GL_EXT_texture_storage"))
        {
            ...
            // from the exposed GL_OES_texture_float extension must be emulated with R/RG textures.
            if (IsEmulatedLUMAFormat(format))
            {
                result = EmulateLUMAFormat(format);
            }
    ...
    else if (functions->isAtLeastGLES(gl::Version(3, 0)))
    {
        if (type == GL_HALF_FLOAT_OES)
        {
            if (!IsEmulatedLUMAFormat(format) ||
                !functions->hasGLExtension("GL_EXT_texture_storage"))
            {
                // In ES3, the luminance formats come from EXT_texture_storage, which uses
