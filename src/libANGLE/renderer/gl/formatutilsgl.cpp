...
static bool IsEmulatedLUMAFormat(GLenum format)
{
...
{
    ASSERT(internalFormat.format == GL_LUMINANCE || internalFormat.format == GL_ALPHA ||
    ...
    {
        return gl::GetInternalFormatInfo(GL_RED, internalFormat.type);
    }

    return gl::GetInternalFormatInfo(GL_RG, internalFormat.type);
}

static GLenum EmulateLUMAFormat(const GLenum format)
{
    // This is needed separately from the above helper because some format/type combinations that
    // come in to GetNativeFormat don't have entries in the internal format map.

    ASSERT(format == GL_LUMINANCE || format == GL_ALPHA || format == GL_LUMINANCE_ALPHA);
                                      ...
                                      const gl::InternalFormat &internalFormat)
{
    ...
    if (functions->standard == STANDARD_GL_DESKTOP)
    {
        ...
        {
            if (IsEmulatedLUMAFormat(internalFormat.format))
            {
    ...
    else if (functions->isAtLeastGLES(gl::Version(3, 0)))
    {
        if (internalFormat.componentType == GL_FLOAT)
        {
            ...
            else if (!functions->hasGLExtension("GL_OES_texture_float") ||
                     !functions->hasGLExtension("GL_OES_texture_half_float"))
            {
                              ...
                              GLenum type)
{
    ...
    if (functions->standard == STANDARD_GL_DESKTOP)
    {
        ...
        if ((functions->profile & GL_CONTEXT_CORE_PROFILE_BIT) != 0)
        {
            ...
            if (IsEmulatedLUMAFormat(format))
            {
    ...
    else if (functions->isAtLeastGLES(gl::Version(3, 0)))
    {
        ...
        if ((type == GL_FLOAT && !functions->hasGLExtension("GL_OES_texture_float")) ||
            (type == GL_HALF_FLOAT_OES && !functions->hasGLExtension("GL_OES_texture_half_float")))
        {
