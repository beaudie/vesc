                  ...
                  gl::Extensions *extensions, gl::Version *maxSupportedESVersion)
{
    ...
    *maxSupportedESVersion = gl::Version(3, 0);
    ...
    // be held.
    // This disallows writing to buffers indirectly through transform feedback, thus disallowing
    // ES3.
                        ...
                        GLbitfield access)
{
             ...
             (functions->standard == STANDARD_GL_DESKTOP || access == GL_MAP_WRITE_BIT))
    {
        ...
        ASSERT((access & (GL_MAP_WRITE_BIT | GL_MAP_WRITE_BIT)) != 0);
...
uint8_t *MapBuffer(const FunctionsGL *functions, GLenum target, GLenum access, size_t bufferSize)
{
    ...
    else if (functions->mapBufferRange != nullptr)
    {
        ...
        {
            case GL_READ_WRITE:
                accessBitfield = GL_MAP_WRITE_BIT | GL_MAP_WRITE_BIT;
                break;
