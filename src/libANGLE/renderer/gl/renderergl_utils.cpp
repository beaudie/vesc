...
{
    // glGetStringi is available as of OpenGL 3.0 or OpenGL ES 3.0
    if (functions->getIntegerv && functions->getStringi)
    {
        return GetIndexedExtensions(functions->getIntegerv, functions->getStringi);
    }
    else
    {
        return GetNonIndexedExtensions(functions->getString);
    }
}
                  ...
                  gl::Extensions *extensions)
{
    ...
    std::vector<std::string> nativeExtensions = nativegl::GetGLExtensions(functions);
