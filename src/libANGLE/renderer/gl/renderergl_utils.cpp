                  ...
                  gl::Extensions *extensions, gl::Version *maxSupportedESVersion)
{
    ...
    *maxSupportedESVersion = gl::Version(3, 0);
    ...
    else
    {
        ...
        caps->maxRenderbufferSize = 0;
        caps->maxColorAttachments = 0;
        LimitVersion(maxSupportedESVersion, gl::Version(0, 0));
    }

    if (functions->isAtLeastGL(gl::Version(2, 0)) || functions->hasGLExtension("GL_EXT_draw_buffers2") ||
    ...
    else
    {
        ...
        caps->maxVertexUniformComponents = 0;
    ...
    else
    {
        ...
        caps->maxVertexOutputComponents = 0;
        LimitVersion(maxSupportedESVersion, gl::Version(2, 0));
    }
    ...
    {
        // Can't support ES2 version without these caps
        caps->maxFragmentUniformComponents = 0;
        caps->maxTextureImageUnits = 0;
        LimitVersion(maxSupportedESVersion, gl::Version(0, 0));
    }
    ...
    else
    {
        // There doesn't seem, to be a desktop extension to add this cap, maybe it could be given a safe limit
        // instead of limiting the supported ES version.
        caps->maxFragmentInputComponents = 0;
        LimitVersion(maxSupportedESVersion, gl::Version(2, 0));
    }
    ...
    extensions->mapBuffer = functions->isAtLeastGL(gl::Version(1, 5)) || functions->hasGLExtension("GL_ARB_map_buffer_range") ||
