...
gl::Error QueryGL::timestamp()
{
    ...
    // Directly create a query for the timestamp and add it to the
    // pending query queue, as timestamp queries do not have the
    // traditional begin/end block and never need to be paused/resumed
    GLuint query;
...
gl::Error QueryGL::flush(bool force)
{
    while (!mPendingQueries.empty())
    {
        ...
        if (mFunctions->getQueryObjectui64v == nullptr)
        {
            ...
            mResultSum = MergeQueryResults(mType, mResultSum, (GLuint64)result);
        }
        else
        {
            GLuint64 result = 0;
