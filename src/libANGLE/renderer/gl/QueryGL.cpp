...
{
  public:
    SyncProviderGLQuery(const gl::Context *context,
                        const FunctionsGL *functions,
                        StateManagerGL *stateManager,
                        gl::QueryType type)
        : mFunctions(functions), mQuery(0)
    {
        ...
        (void)(stateManager->pauseQuery(context, type));
        mFunctions->beginQuery(ToGLenum(type), mQuery);
        mFunctions->endQuery(ToGLenum(type));
        (void)(stateManager->resumeQuery(context, type));
    }
...
gl::Error SyncQueryGL::end(const gl::Context *context)
{
    ...
    {
        mSyncProvider.reset(
            new SyncProviderGLQuery(context, mFunctions, mStateManager, gl::QueryType::AnySamples));
    }
    else
    {
        ASSERT(false);
