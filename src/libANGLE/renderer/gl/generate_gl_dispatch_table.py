...
for comment, entry_points in json_data.iteritems():
    for entry_point_no_prefix in entry_points:
        ...
        for ep in [entry_point, entry_point + "EXT"]:
            if ep in gl_extension_commands:
                found = True
                for extension in gl_extension_commands[ep]:
                    safe_append(gl_extension_requirements, extension, entry_point)

            if ep in gles2_extension_commands:
                found = True
                for extension in gles2_extension_commands[ep]:
                    safe_append(gles2_extension_requirements, extension, entry_point)
...
struct DispatchTableGL final : private angle::NonCopyable
{{
{table_data}
}};

void InitializeTableDesktopGL(DispatchTableGL *table, FunctionsGL *functionsGL, const std::set<std::string> &extensions);
void InitializeTableGLES(DispatchTableGL *table, FunctionsGL *functionsGL, const std::set<std::string> &extensions);

#if defined(ANGLE_ENABLE_OPENGL_NULL)
void InitializeTableDesktopGLNULL(DispatchTableGL *table, FunctionsGL *functionsGL, const std::set<std::string> &extensions);
void InitializeTableGLESNULL(DispatchTableGL *table, FunctionsGL *functionsGL, const std::set<std::string> &extensions);
...
table_data = []
for comment, entry_points in json_data.iteritems():
    formatted = ["    // " + comment] + [format_ep_decl(entry_point) for entry_point in entry_points]
    table_data.append("\n".join(formatted))
...
#define ASSIGN(NAME, FP) if (!table->FP) *reinterpret_cast<void **>(&table->FP) = functionsGL->loadProcAddress(NAME)
...
def format_assign_ep(entry_point):
    return '        ASSIGN("' + entry_point + '", ' + first_lower(entry_point[2:]) + ');'

# KHR suffixes are mandated for GLES extensions, unlike desktop GL. (See gl.xml)
def format_assign_ext_ep(entry_point, extension, api):
    suffix = ""
    if extension.startswith("GL_KHR_") and api == "gles2":
        suffix = "KHR"
    elif extension.startswith("GL_EXT_") and not entry_point.endswith("EXT"):
        suffix = "EXT"
    return '        ASSIGN("' + entry_point  + suffix + '", ' + first_lower(entry_point[2:]) + ');'

def format_requirements_lines(required, entry_points):
    major, minor = required
    lines = ['    if (version >= gl::Version(' + major + ', ' + minor + '))', '    {']
    lines += [format_assign_ep(entry_point) for entry_point in entry_points]
    lines += ['    }']
    return '\n'.join(lines)

def format_extension_requirements_lines(extension, entry_points, api):
    lines = ['    if (extensions.count("' + extension + '") != 0)', '    {']
    lines += [format_assign_ext_ep(entry_point, extension, api) for entry_point in entry_points]
    lines += ['    }']
    return '\n'.join(lines)
