                                              ...
                                              GLubyte *tmpPixels)
{
    ...
    // We don't handle the behavior when pack.rowLength < area.width
    // as it doesn't seem to be pratically meaningful.
    // It's also undefined based on the spec.
    // The observed behavior on OpenGL and OpenGL ES is weird.
    // (The first row reads n=width pixels
    // while subequent rows read n=pack.rowLength pixels)
    ASSERT(pack.rowLength == 0 || pack.rowLength >= area.width);
                                        ...
                                        void *pixels)
{
        ...
        readType == GL_UNSIGNED_SHORT)
    {
        ...
        if (format != readFormat && (readFormat == GL_RED || readFormat == GL_RG))
        {
            if (packState.rowLength != 0 && packState.rowLength < area.width)
            {
                // We didn't handle this case in the workaround implementation
                packState.rowLength = 0;
            }
        }
    }
                                                ...
                                                GLubyte *pixels) const
{
    ...
    angle::Result result = workaround.Initialize(context, area, originalReadFormat, format, type,
                                                 skipBytes, rowBytes, pixels);
    if (result != angle::Result::Continue)
    {
        return result;
    }
