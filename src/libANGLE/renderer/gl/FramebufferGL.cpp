                                        ...
                                        void *pixels)
{
    ...
    int leftClip    = clippedArea.x - clippedArea.x;
    int topClip     = clippedArea.y - clippedArea.y;
    if (leftClip || topClip)
    {
        ...
        GLuint rowBytes = 0;
        ANGLE_CHECK_GL_MATH(
            contextGL, glFormat.computeRowPitch(readType, clippedArea.width, packState.alignment,
                                                packState.rowLength, &rowBytes));
        outPtr += leftClip * glFormat.pixelBytes + topClip * rowBytes;
    }

    if (packState.rowLength == 0 && clippedArea.width != clippedArea.width)
    {
