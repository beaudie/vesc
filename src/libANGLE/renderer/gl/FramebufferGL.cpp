                                ...
                                void *ptrOrOffset) const
{
    ...
    bool cannotSetDesiredRowLength = packState.rowLength && !context->getExtensions().packSubimage;

    if (cannotSetDesiredRowLength || useOverlappingRowsWorkaround)
    {
        return readPixelsRowByRow(area, readFormat, readType, packState, pixels);
    }
    ...
    {
        ANGLE_TRY_RESULT(
            ShouldApplyLastRowPaddingWorkaround(gl::Extents(area.width, area.height, 1), packState,
                                                readFormat, readType, false, pixels),
            useLastRowPaddingWorkaround);
    }

    return readPixelsAllAtOnce(area, readFormat, readType, packState, pixels,
                               useLastRowPaddingWorkaround);
}
...
gl::Error FramebufferGL::readPixelsRowByRow(const gl::Rectangle &area,
                                            GLenum format,
                                            GLenum type,
...
gl::Error FramebufferGL::readPixelsAllAtOnce(const gl::Rectangle &area,
                                             GLenum format,
                                             GLenum type,
                                             const gl::PixelPackState &pack,
                                             GLubyte *pixels,
