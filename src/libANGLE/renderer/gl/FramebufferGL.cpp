                                                ...
                                                gl::Rectangle *newDestArea)
{
    ...
    if (!destBounds.encloses(destRegion))
    {
        ...
        else
        {
            ...
            destOriginX = destX + destClippedWidth / 2;
            offsetXHalvings++;
        }

        GLuint offsetYHalvings = 0;
        ...
        {
            // Region completely spans bounds
            destOriginY = destY + destClippedHeight / 2;
            offsetYHalvings++;
        }
        ...
        CheckedNumeric<GLuint> checkedXOffset(destRegion.x - destX);
        CheckedNumeric<GLuint> checkedYOffset(destRegion.y - destY);
        ...
        sourceRegion = gl::Rectangle(sourceX + (xOffset >> (destXHalvings + offsetXHalvings)),
                                     sourceY + (yOffset >> (destYHalvings + offsetYHalvings)),
                                     sourceRegion.width >> destXHalvings,
                                     sourceRegion.height >> destYHalvings);
    ...
    if (!sourceBounds.encloses(sourceRegion))
    {
        ...
        {
            // Region completely spans bounds
            sourceOriginX = sourceX + sourceClippedHeight / 2;
            offsetXHalvings++;
        }
        ...
        {
            // Region completely spans bounds
            sourceOriginY = sourceY + sourceClippedHeight / 2;
            offsetYHalvings++;
        }
        ...
        CheckedNumeric<GLuint> checkedXOffset(sourceRegion.x - sourceX);
        CheckedNumeric<GLuint> checkedYOffset(sourceRegion.y - sourceY);

        // if X/Y is reversed, use the top/right out-of-bounds region to compute
        // the origin offset instead of the left/bottom out-of-bounds region
        if (xFlipped)
        {
            checkedXOffset = (sourceX + sourceWidth - sourceRegion.x1());
        }
        if (yFlipped)
        {
            checkedYOffset = (sourceY + sourceHeight - sourceRegion.y1());
        }
        ...
        destRegion = gl::Rectangle(destX + (xOffset >> (sourceXHalvings + offsetXHalvings)),
                                   destY + (yOffset >> (sourceYHalvings + offsetYHalvings)),
                                   destRegion.width >> sourceXHalvings,
                                   destRegion.height >> sourceYHalvings);
    }
