...
void RetrieveMultiviewFieldsFromAttachment(const gl::FramebufferAttachment *attachment,
                                           const std::vector<gl::Offset> **offset,
                                           GLenum *multiviewLayout)
{
    if (attachment)
    {
        *offset          = &attachment->getMultiviewViewportOffsets();
...
Error FramebufferGL::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    genericSideBySideClear(ClearCommandType::Clear, mask, GL_NONE, 0, nullptr, GL_NONE, GL_NONE);
                                   ...
                                   const GLfloat *values)
{
    ...
    genericSideBySideClear(ClearCommandType::ClearBufferfv, static_cast<GLbitfield>(0u), buffer,
                           drawbuffer, reinterpret_cast<const uint8_t *>(values), GL_NONE, GL_NONE);
...
void FramebufferGL::syncState(const gl::Context *context, const Framebuffer::DirtyBits &dirtyBits)
{
    ...
    for (auto dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            case Framebuffer::DIRTY_BIT_DEPTH_ATTACHMENT:
                ...
                RetrieveMultiviewFieldsFromAttachment(mState.getDepthAttachment(), &viewportOffsets,
                                                      &multiviewLayout);
            ...
            case Framebuffer::DIRTY_BIT_STENCIL_ATTACHMENT:
                ...
                RetrieveMultiviewFieldsFromAttachment(mState.getStencilAttachment(),
                                                      &viewportOffsets, &multiviewLayout);
                isAttachmentModified = true;
                break;
    ...
    if (isAttachmentModified)
    {
        ...
        mStateManager->setViewportOffsets(*viewportOffsets);
    }
}
