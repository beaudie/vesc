                          ...
                          GLenum filter)
{
    ...
    //TODO all that only for GL_LINEAR and not NEAREST?

    // Prior to OpenGL 4.4 BlitFramebuffer (section 18.3.1 of GL 4.3 core profile) reads:
    //      When values are taken from the read buffer, no linearization is performed, even if the
    //      format of the buffer is SRGB.
    ...
    //      converted from the non-linear sRGB color space according [...].
    {
        ...
        bool sourceSRGB = readAttachment != nullptr && readAttachment->getColorEncoding() == GL_SRGB;
        needManualColorBlit = needManualColorBlit || (sourceSRGB && !mFunctions->isAtLeastGL(gl::Version(4, 4)));
    }

    // Prior to OpenGL 4.2 BlitFramebuffer (section 4.3.2 of GL 4.1 core profile) reads:
    //      Blit operations bypass the fragment pipeline. The only fragment operations which affect
    //      a blit are the pixel ownership test and scissor test.
    ...
    if (!needManualColorBlit) {
        ...
        needManualColorBlit = needManualColorBlit || (destSRGB && !mFunctions->isAtLeastGL(gl::Version(4, 2)));
    }
    ...
    {
        ANGLE_TRY(mBlitter->blitColorBufferWithSRGBWorkaround(sourceFramebuffer, destFramebuffer, sourceArea, destArea, filter));
    ...
    mFunctions->blitFramebuffer(sourceArea.x, sourceArea.y, sourceArea.x1(), sourceArea.y1(),
                                destArea.x, destArea.y, destArea.x1(), destArea.y1(), blitMask, filter);
