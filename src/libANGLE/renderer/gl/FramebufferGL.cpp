                          ...
                          GLenum filter)
{
    ...
    if (mWorkarounds.limitBlitFramebufferDimensions)
    {
        bool widthOutOfRange =
            std::abs(sourceArea.width) > static_cast<int64_t>(std::numeric_limits<int>::max()) ||
            std::abs(destArea.width) > static_cast<int64_t>(std::numeric_limits<int>::max());
        bool heightOutOfRange =
            std::abs(sourceArea.height) > static_cast<int64_t>(std::numeric_limits<int>::max()) ||
            std::abs(destArea.height) > static_cast<int64_t>(std::numeric_limits<int>::max());
