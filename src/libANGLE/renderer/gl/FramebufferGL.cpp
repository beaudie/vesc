    ...
    for (auto dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case Framebuffer::DIRTY_BIT_READ_BUFFER:
                ...
                break;
            case Framebuffer::DIRTY_BIT_DRAW_BUFFERS_INDEXED:
            {
                if (!mIsDefault)
                {
                    break;
                ...
                if (mState.getMultiviewViewCount() == 2 &&
                    mState.getEnabledDrawBuffers().size() == 1)
                {
                    if (mState.getDrawBufferStateIndices()[0] == 0)
                    {
            ...
            case Framebuffer::DIRTY_BIT_READ_BUFFER_INDEXED:
                if (!mIsDefault)
                {
                    break;
                }
