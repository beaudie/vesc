...
bool KeepBufferShadowCopy(const gl::Context *context, gl::BufferBinding target)
{
    ...
    if (context->isWebGL() && context->isBufferAccessValidationEnabled() &&
        target == gl::BufferBinding::ElementArray)
    {
        return true;
    }
                                ...
                                gl::BufferUsage usage)
{
    ...
    if (KeepBufferShadowCopy(context, target))
    {
    ...
    // Don't delete existing shadow data. WebGL allows users to bind as an element array buffer
    // first and then copy source/dest later (but not the other way around).
                                   ...
                                   size_t offset)
{
    ...
    {
        memcpy(mShadowCopydata() + offset, data, size);
    }
