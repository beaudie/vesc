...
#define GET_FNPTR_OR_ERROR(MEMBER, NAME) *MEMBER = NAME;
    ...
    GET_FNPTR_OR_ERROR(&bindAPIPtr, eglBindAPI);
    ...
    GET_FNPTR_OR_ERROR(&mFnPtrs->destroyContextPtr, eglDestroyContext);
    GET_FNPTR_OR_ERROR(&mFnPtrs->makeCurrentPtr, eglMakeCurrent);
    GET_FNPTR_OR_ERROR(&mFnPtrs->queryStringPtr, eglQueryString);
    ...
    std::cout << extensions << std::endl; //XXX
...
EGLDisplay FunctionsDRM::getDisplay() const
{
    return mEGLDisplay;
}
...
EGLBoolean FunctionsDRM::destroyContext(EGLContext context) const
{
...
char const * FunctionsDRM::queryString(EGLint name) const
{
    return mFnPtrs->queryStringPtr(mEGLDisplay, name);
}

EGLImageKHR FunctionsDRM::createImage(EGLContext context, EGLenum target, EGLClientBuffer buffer, const EGLint *attrib_list) const
{
    return mFnPtrs->createImagePtr(mEGLDisplay, context, target, buffer, attrib_list);
}

EGLBoolean FunctionsDRM::destroyImage(EGLImageKHR image) const
{
