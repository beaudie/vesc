...
egl::Error WindowSurfaceDRM::initialize()
{
    ...
    // extension.
    {
        XWindowAttributes windowAttributes; windowAttributes.visual = nullptr;
        //XXX XGetWindowAttributes(mDisplay, mParent, &windowAttributes);
    ...
    //XXX Visual* visual = visualInfo->visual;
    ...
    //XXX XSetWindowAttributes attributes;
    //XXX unsigned long attributeMask = CWColormap | CWBorderPixel;

    Colormap colormap = 0; //XXX XCreateColormap(mDisplay, mParent, visual, AllocNone);
    if(!colormap)
    {
        //XXX XFree(visualInfo);
        return egl::Error(EGL_BAD_NATIVE_WINDOW, "Failed to create the Colormap for the child window.");
    }
    //XXX attributes.colormap = colormap;
    //XXX attributes.border_pixel = 0;

    //TODO(cwallez) set up our own error handler to see if the call failed
    mWindow = 0; //XXX XCreateWindow(mDisplay, mParent, 0, 0, mParentWidth, mParentHeight, 0, visualInfo->depth, InputOutput, visual, attributeMask, &attributes);
