...
egl::Error DisplayDRM::initialize(egl::Display *display)
{
    ...
    //XXX const auto &attribMap = display->getAttributeMap();
    ...
    /*XXX
    EGLint major, minor;
    EGLDisplay eglDisplay = mDRM.getDisplay(nativeDisplay);
    if (!mDRM.initialize(eglDisplay, &major, &minor))
    {
        return egl::Error(EGL_NOT_INITIALIZED, "failed to init system egl");
    }
    */

#define EGL_NO_CONFIG_MESA 0 //XXX
    const EGLint attrib[] = { EGL_NONE };
    EGLContext context = mDRM.createContext(EGL_NO_CONFIG_MESA, EGL_NO_CONTEXT, attrib);
    if (context == EGL_NO_CONTEXT)
    {
        return egl::Error(EGL_NOT_INITIALIZED, "failed to create system egl context");
    }
    if (!mDRM.makeCurrent(EGL_NO_SURFACE, context))
    {
        return egl::Error(EGL_NOT_INITIALIZED, "failed to make current system egl context");
    }
    ...
    if (mDRM.hasExtension("DRM_EXT_swap_control"))
    {
        mSwapControl = SwapControl::EXT;

        // In DRM_EXT_swap_control querying these is done on a DRMWindow so we just
        // set default values.
        mMinSwapInterval = 0;
        mMaxSwapInterval = 4;
    }
    else if (mDRM.hasExtension("DRM_MESA_swap_control"))
    {
    ...
    else if (mDRM.hasExtension("DRM_SGI_swap_control"))
    {
    ...
    if (!mDRM.makeCurrent(mDummyPbuffer, mContext))
    {
...
void DisplayDRM::terminate()
{
    ...
    if (mContext)
    {
        //XXX mDRM.destroyContext(mContext);
        mContext = nullptr;
    }
                                           ...
                                           const egl::AttributeMap &eglAttributes)
{
    ...
    if (requestedMajorVersion != EGL_DONT_CARE && requestedMinorVersion != EGL_DONT_CARE)
    {
        ...
        contextAttributes.push_back(DRM_CONTEXT_MAJOR_VERSION_ARB);
        contextAttributes.push_back(requestedMajorVersion);

        contextAttributes.push_back(DRM_CONTEXT_MINOR_VERSION_ARB);
    ...
    bool useProfile = mDRM.hasExtension("DRM_ARB_create_context_profile");
    for (size_t i = 0; i < ArraySize(desktopVersions); ++i)
    {
        ...
        {
            contextAttributes.push_back(DRM_CONTEXT_PROFILE_MASK_ARB);
            contextAttributes.push_back(DRM_CONTEXT_CORE_PROFILE_BIT_ARB);
        }
