...
{
    /*XXX don't have native config, just pass egl::Config ?
...
egl::ConfigSet DisplayDRM::generateConfigs() const
{
...
#else
    egl::Config config;
    //XXX guess we need to figure out all configs which we can do in surfaceless
    config.redSize = 8;
    config.greenSize = 8;
    config.blueSize = 8;
    config.alphaSize = 8;
    ...
    int id = configs.add(config);
    configIdToDRMConfig[id] = 0; //XXX don't have native configs, get rid of this map
#endif
    return configs;
}
...
bool DisplayDRM::isValidNativeWindow(EGLNativeWindowType window) const
{
    ...
    int status = XQueryTree(mDRM.getDisplay(), window, &root, &parent, &children, &nChildren);
    if (children)
    {
        XFree(children);
    }
    return status != 0;
#else
    //XXX don't have native windows so return true.  in the long run we may have something that takes the place of native windows, if that's the path of least resistance
    return true;
#endif
}
