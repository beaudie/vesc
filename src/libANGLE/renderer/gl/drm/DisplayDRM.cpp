...
egl::Error DisplayDRM::initialize(egl::Display *display)
{
    ...
    if (attribMap.contains(EGL_X11_VISUAL_ID_ANGLE))
    {
        ...
        EGLConfig *allConfigs = mDRM.chooseFBConfig(attribList, &nConfigs);
    ...
    else
    {
        ...
        EGLConfig *candidates = mDRM.chooseFBConfig(attribList, &nConfigs);
        if (nConfigs == 0)
        {
            XFree(candidates);
                                             ...
                                             const egl::AttributeMap &attribs)
{
    ...
    EGLImageKHR image = mDRM.createImage(EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attr);

    return new WindowSurfaceDRM(mDRM, this, this->getRenderer(), size[0], size[1], image, mFunctionsGL, mDRM.getDisplay(), mContext, 0/*config*/);
    //return new WindowSurfaceDRM(mDRM, this, this->getRenderer(), window, mDRM.getDisplay(), mContext, fbConfig);
}
...
egl::ConfigSet DisplayDRM::generateConfigs() const
{
    ...
    int drmConfigCount;
    EGLConfig *drmConfigs = mDRM.chooseFBConfig(attribList, &drmConfigCount);
