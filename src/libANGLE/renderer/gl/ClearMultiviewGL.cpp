    ...
    for (auto dirtyAttachment : mDirtyAttachments)
    {
        switch (dirtyAttachment)
        {
            case DIRTY_BIT_DEPTH_ATTACHMENT:
                mFunctions->framebufferTextureLayer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, 0, 0,
                                                    0);
                break;
            case DIRTY_BIT_STENCIL_ATTACHMENT:
                mFunctions->framebufferTextureLayer(GL_DRAW_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, 0,
                                                    0, 0);
                break;
            case DIRTY_BIT_DEPTH_STENCIL_ATTACHMENT:
                mFunctions->framebufferTextureLayer(GL_DRAW_FRAMEBUFFER,
                                                    GL_DEPTH_STENCIL_ATTACHMENT, 0, 0, 0);
                break;
            default:
                ...
                mFunctions->framebufferTextureLayer(
                    GL_DRAW_FRAMEBUFFER,
                    static_cast<GLenum>(GL_COLOR_ATTACHMENT0 + dirtyAttachment), 0, 0, 0);
        }
    }
