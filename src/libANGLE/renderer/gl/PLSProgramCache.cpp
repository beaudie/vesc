...
namespace rx
{
static GLuint CreateVAO(const FunctionsGL *gl)
{
    GLuint vao;
...
static bool IsShaderCompiled(const FunctionsGL *gl, GLuint shaderID)
{
    GLint isCompiled = 0;
...
static bool IsProgramLinked(const FunctionsGL *gl, GLuint programID)
{
    GLint isLinked = 0;
...
{
    // Draws a fullscreen quad with GL_TRIANGLE_STRIP.
    static const char *LoadPLSVertexShader =
        "#version 310 es\n"
        "void main()"
        "{"
        "gl_Position=vec4("
        ...
        "1);"
        "}";
    mGL->shaderSource(mVertexShaderID, 1, &LoadPLSVertexShader, nullptr);
...
static PLSFormatKey GetPLSFormatKey(GLenum internalformat)
{
...
constexpr static uint64_t BitRepeat(uint64_t bits, int interval)
{
...
static const char *GetPLSQualifier(PLSProgramType type)
{
    switch (type)
    {
...
static const char *GetFormatQualifier(PLSFormatKey formatKey)
{
    switch (formatKey)
    {
...
static const char *GetFormatPrecision(PLSFormatKey formatKey)
{
    switch (formatKey)
    {
        case PLSFormatKey::RGBA8:
...
static const char *GetFormatRawType(PLSFormatKey formatKey)
{
    switch (formatKey)
    {
        case PLSFormatKey::RGBA8:
...
static const char *GetFormatPrefix(PLSFormatKey formatKey)
{
    switch (formatKey)
    {
        case PLSFormatKey::RGBA8:
...
static const char *GetFormatSwizzle(PLSFormatKey formatKey)
{
    switch (formatKey)
    {
        case PLSFormatKey::RGBA8:
...
static void ExpandPLSVar(std::ostringstream &out, int binding, PLSFormatKey formatKey)
{
    switch (formatKey)
    {
        case PLSFormatKey::RGBA8:
