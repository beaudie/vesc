      ...
      mNativeParallelCompileEnabled(false)
{
    ...
    if (mWorkarounds.initializeCurrentVertexAttributes.applied)
    {
...
{
    if (mWorkarounds.finishDoesNotCauseQueriesToBeAvailable.applied && mUseDebugOutput)
    {
        mFunctions->enable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
    }
...
{
    if (mWorkarounds.disableWorkerContexts.applied)
    {
        return false;
    }
