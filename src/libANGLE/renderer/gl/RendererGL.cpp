...
{
    std::string navtiveVendorString(reinterpret_cast<const char*>(mFunctions->getString(GL_VENDOR)));
    std::string navtiveRendererString(reinterpret_cast<const char*>(mFunctions->getString(GL_RENDERER)));
    ...
    rendererString << navtiveVendorString << " " << navtiveRendererString << " OpenGL";
    if (isES)
    {
        rendererString << " ES";
    }
