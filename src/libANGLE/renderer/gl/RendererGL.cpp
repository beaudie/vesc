...
bool RendererGL::bindWorkerContext(std::string *infoLog)
{
    ...
    {
        // Choose a worker context based on thread affinity.
    ...
    {
        if (!mWorkerContextFactory)
        {
            *infoLog += "WorkerContextFactory unimplemented.";
            return false;
        }
...
void RendererGL::unbindWorkerContext()
{
    ...
    mCurrentWorkerContexts[threadID]->unmakeCurrent();
    mWorkerContextPool.push_back(std::move(mCurrentWorkerContexts[threadID]));
