                                              ...
                                              ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    {
        AddResourceInfo(variableInfoMapOut, nullptr, shaderType, GetXfbBufferName(bufferIndex),
                           ...
                           ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    {
        AddResourceInfo(variableInfoMapOut, nullptr, shaderType, kDefaultUniformNames[shaderType],
                        programInterfaceInfo->uniformsAndXfbDescriptorSetIndex,
                        programInterfaceInfo->currentUniformBindingIndex);
        ++programInterfaceInfo->currentUniformBindingIndex;
                    ...
                    uint32_t bindingIndex)
{
    if (uniformBindingIndexMapOut->count(name) == 0)
    {
        (*uniformBindingIndexMapOut)[name] =
            std::tuple<uint32_t, gl::ShaderBitSet>(bindingIndex, gl::ShaderBitSet());
        return true;
    }
    ASSERT(uniformBindingIndexMapOut->count(name) != 0);
    return false;
}
                                   ...
                                   ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    for (unsigned int uniformIndex : inputAttachmentUniformRange)
    {
            ...
            inputAttachmentUniform.isActive(shaderType))
        {
            ...
            insertIfAbsent(uniformBindingIndexMapOut, mappedInputAttachmentName,
                           inputAttachmentBindingIndex);

            AddResourceInfo(variableInfoMapOut, nullptr, shaderType, mappedInputAttachmentName,
    ...
    for (const gl::InterfaceBlock &block : blocks)
    {
        if (!block.isArray || block.arrayElement == 0)
        {
            ...
            if (programExecutable.hasLinkedShaderStage(shaderType) && block.isActive(shaderType))
            {
                ...
                AddResourceInfo(variableInfoMapOut,
                                &std::get<1>((*uniformBindingIndexMapOut)[block.mappedName]),
                                shaderType, block.mappedName,
                                programInterfaceInfo->shaderResourceDescriptorSetIndex,
                                       ...
                                       ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    if (programExecutable.hasLinkedShaderStage(shaderType))
    {
        ...
        AddResourceInfo(
            ...
            shaderType, sh::vk::kAtomicCountersBlockName,
            programInterfaceInfo->shaderResourceDescriptorSetIndex,
    ...
    for (unsigned int uniformIndex : imageUniformRange)
    {
        ...
        if (GetImageNameWithoutIndices(&name))
        {
            if (programExecutable.hasLinkedShaderStage(shaderType))
            {
                ...
                AddResourceInfo(variableInfoMapOut,
                                &std::get<1>((*uniformBindingIndexMapOut)[name]), shaderType, name,
                                programInterfaceInfo->shaderResourceDescriptorSetIndex,
                                std::get<0>((*uniformBindingIndexMapOut)[name]));
            }
        }
    }
}
    ...
    for (unsigned int uniformIndex : programExecutable.getSamplerUniformRange())
    {
        ...
        if (UniformNameIsIndexZero(samplerUniform.name))
        {
                ...
                samplerUniform.isActive(shaderType))
            {
                ...
                AddResourceInfo(variableInfoMapOut,
                                &std::get<1>((*uniformBindingIndexMapOut)[samplerName]), shaderType,
                                samplerName, programInterfaceInfo->textureDescriptorSetIndex,
