...
void AddLocationInfo(ShaderInterfaceVariableInfoMap &infoMap,
                            ...
                            ShaderMapInterfaceVariableInfoMap *variableInfoMapOut)
{
         ...
         programExecutable.getResources().varyingPacking.getRegisterList())
    {
        ...
        if (varying.frontVarying.varying && (varying.frontVarying.stage == shaderType))
        {
            ...
            AddLocationInfo(infoMap, varying.frontVarying, varying.isStructField(), location,
                            component);
        }
        ...
        if (varying.backVarying.varying && (varying.backVarying.stage == shaderType))
        {
            ...
            AddLocationInfo(infoMap, varying.backVarying, varying.isStructField(), location,
                            component);
        }
    }
...
void SpirvTransformer::visitVariable(const uint32_t *instruction)
{
    ...
    // the empty string is generated for unnamed input or output interface block. it also use the
    // block name.
