...
std::string RemoveVertexShaderDepthCorrection(const std::string &originalSource)
{
    const char depthCorrectionCode[] = "(gl_Position.z = ((gl_Position.z + gl_Position.w) * 0.5));";

    const size_t depthCorrectionStart = originalSource.find(depthCorrectionCode);
    const size_t depthCorrectionEnd   = depthCorrectionStart + ConstStrLen(depthCorrectionCode);

    std::string result;

    result.append(&originalSource[0], &originalSource[depthCorrectionStart]);
    result.append(&originalSource[depthCorrectionEnd], &originalSource[originalSource.size()]);
    return result;
}
                            ...
                            ShaderMapInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    if (!vertexSource->empty())
    {
        ...
        // Tess evaluation shader has a depth correction code so remove it from vertex shader
        if (programState.getAttachedShader(gl::ShaderType::TessEvaluation))
        {
            *vertexSource = RemoveVertexShaderDepthCorrection(*vertexSource);
        }
    }
