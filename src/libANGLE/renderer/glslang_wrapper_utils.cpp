                            ...
                            ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    for (const std::string &varyingName : inactiveVaryingMappedNames[shaderType])
    {
        ...
        // inactive.
        if (variableInfoMapOut->contains(shaderType, ShaderVariableType::Varying, varyingName))
        {
            continue;
        }
                           ...
                           ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    if (programExecutable.hasLinkedShaderStage(shaderType))
    {
        ...
        AddResourceInfoToAllStages(variableInfoMapOut, shaderType,
                                   ShaderVariableType::DefaultUniform,
    ...
    spv::StorageClass storageClass)
{
    ...
    if (shaderType == gl::ShaderType::Vertex && storageClass == spv::StorageClassUniform)
    {
        ...
        ASSERT(&info == &variableInfoMap.get(shaderType, ShaderVariableType::TransformFeedback,
                                             GetXfbBufferName(0)) ||
               &info == &variableInfoMap.get(shaderType, ShaderVariableType::TransformFeedback,
                                             GetXfbBufferName(1)) ||
               &info == &variableInfoMap.get(shaderType, ShaderVariableType::TransformFeedback,
                                             GetXfbBufferName(2)) ||
               &info == &variableInfoMap.get(shaderType, ShaderVariableType::TransformFeedback,
...
void SpirvTransformer::visitDecorate(const uint32_t *instruction)
{
    ...
    if (mIds.isIOBlock(id))
    {
        ...
        ShaderVariableType variableType =
            mVariableInfoMap.getVariableType(mOptions.shaderType, name);
        const ShaderInterfaceVariableInfo &info =
            mVariableInfoMap.get(mOptions.shaderType, variableType, name);
...
void SpirvTransformer::visitMemberName(const uint32_t *instruction)
{
    ...
    if (!mVariableInfoMap.hasVariableType(mOptions.shaderType, name))
    {
        return;
    }

    ShaderVariableType variableType = mVariableInfoMap.getVariableType(mOptions.shaderType, name);
    const ShaderInterfaceVariableInfo &info =
        mVariableInfoMap.get(mOptions.shaderType, variableType, name);
...
void SpirvTransformer::visitVariable(const uint32_t *instruction)
{
    ...
    ShaderVariableType shaderVariableType =
        mVariableInfoMap.getVariableType(mOptions.shaderType, name);
    const ShaderInterfaceVariableInfo &info =
        mVariableInfoMap.get(mOptions.shaderType, shaderVariableType, name);
