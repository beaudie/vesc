...
void SpirvTransformer::visitTypePointer(const uint32_t *instruction)
{
    ...
    if (typeId == mOutputPerVertexTypeId || typeId == mInputPerVertexTypeId)
    {
        ...
        // two ids.
        if ((typeId == mOutputPerVertexTypeId && storageClass == spv::StorageClassInput) ||
            (typeId == mInputPerVertexTypeId && storageClass == spv::StorageClassOutput))
        {
            std::swap(mOutputPerVertexTypeId, mInputPerVertexTypeId);
        }
    }
}
