...
bool SpirvTransformer::transformDecorate(const uint32_t *instruction, size_t wordCount)
{
    ...
    {
        // Create a patch hunk for inactive output varyings.  Note that every other inactive
        // resource is already removed by the translator, including inactive input varyings.
        mSpirvBlobOut->patches[SpirvPatchType::InactiveOutputVaryings].hunks.emplace_back(
...
bool SpirvTransformer::transformEntryPoint(const uint32_t *instruction, size_t wordCount)
{
    ...
    for (size_t index = 0; index < interfaceCount; ++index)
    {
        ...
        if (!info->activeStages[mShaderType])
        {
            // Remember this id as part of the inactive output varyings patch.
            mSpirvBlobOut->patches[SpirvPatchType::InactiveOutputVaryings]
                .entryPointAdditions.push_back(id);
            continue;
        }
...
bool SpirvTransformer::transformVariable(const uint32_t *instruction, size_t wordCount)
{
    ...
    // Create a patch hunk for inactive output varyings.
    mSpirvBlobOut->patches[SpirvPatchType::InactiveOutputVaryings].hunks.emplace_back(
        createPatchHunk(instruction, instructionOffset,
                        getCurrentOutputOffset() - instructionOffset));
...
// applied.
using PatchesToApply =
    angle::FixedVector<const SpirvPatch *, static_cast<size_t>(SpirvPatchType::EnumCount)>;

void ApplySpirvPatches(const SpirvBlob &spirvBlob,
                       const PatchesToApply &patchesToApply,
                       std::vector<uint32_t> *patchedSpirvBlob)
{
    ...
    std::array<size_t, static_cast<size_t>(SpirvPatchType::EnumCount)> nextHunks = {};
...
bool ValidateSpirvBlob(const SpirvBlob &spirvBlob)
{
    ...
    static_assert(static_cast<size_t>(SpirvPatchType::EnumCount) == 1,
                  "Generalize the following code to validate all permutations");

    PatchesToApply patchesToApply;
    patchesToApply.push_back(&spirvBlob.patches[SpirvPatchType::InactiveOutputVaryings]);

    std::vector<uint32_t> patchedSpirvBlob;
    ApplySpirvPatches(spirvBlob, patchesToApply, &patchedSpirvBlob);
    ValidateSpirv(patchedSpirvBlob, " with InactiveOutputVaryings patch");
                              ...
                              gl::ShaderMap<std::vector<uint32_t>> *patchedSpirvBlobsOut)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        {
            if (patches[patchType])
            {
                patchesToApply.push_back(&spirvBlob.patches[patchType]);
            }
        }

        // At least one patch must be requested.
        ASSERT(patchesToApply.size() > 0);
