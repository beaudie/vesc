...
class SpirvTransformer final : angle::NonCopyable
{
  ...
  private:
    ...
    bool transformSpecConstantFalse(const uint32_t *instruction, size_t wordCount);
...
void SpirvTransformer::transformInstruction()
{
    ...
    switch (opCode)
    {
        ...
        case kOpTypePointer:
            ...
            break;
        case kOpSpecConstantFalse:
            transformed = transformSpecConstantFalse(instruction, wordCount);
            break;
        case kOpVariable:
...
bool SpirvTransformer::transformSpecConstantFalse(const uint32_t *instruction, size_t wordCount)
{
    // SPIR-V 1.0 Section 3.32 Instructions, OpSpecConstantFalse
    ...
    if (id == mLineRasterSpecConstVarId)
    {
        ...
        // Note: The layout of OpSpecConstantFalse and OpConstantFalse is otherwise identical.
        copyInstruction(instruction, wordCount);
        ...
        // Replace OpSpecConstantFalse with OpConstantTrue for line raster emulation.
...
bool SpirvTransformer::transformVariable(const uint32_t *instruction, size_t wordCount)
{
    ...
    if (id == mANGLEPositionVarId)
    {
        // Encountered: %ANGLEPosition = OpVariable %type Input/Output
    ...
    std::vector<uint32_t> *specializedSpirvShaderOut)
{
    ...
    specializedSpirvShaderOut->assign(spirvShader.code.begin(),
                                      spirvShader.code.begin() + currentUnpatchedOffset);
    ...
    while (true)
    {
        ...
        for (size_t patchIndex = 0; patchIndex < patchesToApply.size(); ++patchIndex)
        {
            ...
            ASSERT(hunkOffset > spirvShader.entryPointOffset);
        ...
        specializedSpirvShaderOut->insert(specializedSpirvShaderOut->end(),
                                          spirvShader.code.begin() + currentUnpatchedOffset,
                                          spirvShader.code.begin() + nextUnpatchedHunkOffset);
