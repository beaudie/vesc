...
class SpirvMultiSampleTransformer final : angle::NonCopyable
{
  public:
    SpirvMultiSampleTransformer(const GlslangSpirvOptions &options)
        : mOptions(options), mIsSampleRateShadingCapabilityEnabled(false)
    {}
  ...
  private:
    GlslangSpirvOptions mOptions;
    spirv::IdRef mPreTransformBuiltInGLSampleID;
    spirv::IdRef mAfterTransformBuiltInGLSampleID;
                                                                    ...
                                                                    spirv::Blob *blobOut)
{
    ...
    ASSERT(mAfterTransformBuiltInGLSampleID.valid());
    ...
    spirv::WriteLoad(blobOut, ids.intId(), builtInSampleIDOpLoad, mAfterTransformBuiltInGLSampleID,
                     nullptr);
    ...
    spirv::Blob *blobOut)
{
    ...
    if (mPreTransformBuiltInGLSampleID.valid())
    {
        return;
    }
    ...
    spirv::WriteVariable(blobOut, mIntInputPointerId, mAfterTransformBuiltInGLSampleID,
...
{
    size_t interfaceListSize = interfaceList.size();
    for (size_t idx = 0; idx < interfaceListSize; ++idx)
    {
        if (interfaceList[idx] == id)
        {
            return true;
        }
    }
                                                                ...
                                                                spirv::Blob *blobOut)
{
    ...
    if (mPreTransformBuiltInGLSampleID.valid())
    {
        ASSERT(verifyEntryPointsContainsID(*interfaceList, mPreTransformBuiltInGLSampleID));
        return;
    }
    ...
    mAfterTransformBuiltInGLSampleID = SpirvTransformerBase::GetNewId(blobOut);

    // Increment the interfaceList size by 1 to hold the id of gl_sampleID
    size_t curInterfaceListSize = interfaceList->size();
    interfaceList->resize(curInterfaceListSize + 1);

    // Add the generated id to the interfaceList
    (*interfaceList)[curInterfaceListSize] = mAfterTransformBuiltInGLSampleID;
    return;
}
    ...
    spirv::Blob *blobOut)
{
    ...
    spirv::WriteDecorate(blobOut, mAfterTransformBuiltInGLSampleID, spv::DecorationRelaxedPrecision,
                         {});
    spirv::WriteDecorate(blobOut, mAfterTransformBuiltInGLSampleID, spv::DecorationFlat, {});
    spirv::WriteDecorate(blobOut, mAfterTransformBuiltInGLSampleID, spv::DecorationBuiltIn,
                         {spirv::LiteralInteger(spv::BuiltIn::BuiltInSampleId)});
    return TransformationState::Unchanged;
}

// const spirv::IdRef &SpirvMultiSampleTransformer::getPreTransformBuiltInGLSampleID() const
//{
//     return mPreTransformBuiltInGLSampleID;
// }
//
                                                ...
                                                const spirv::LiteralIntegerList &valueList)
{
    ...
    if (decoration == spv::DecorationBuiltIn)
    {
        ...
        {
            mPreTransformBuiltInGLSampleID = id;
        }
    }
    return;
}
