                                             ...
                                             uint8_t attributeRowCount)
{
    ...
    info->attributeRowCount       = attributeRowCount;
                              ...
                              ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    for (const sh::ShaderVariable &attribute : programExecutable.getProgramInputs())
    {
        ...
        AddLocationInfo(variableInfoMapOut, attribute.mappedName, attribute.location,
                        ...
                        static_cast<uint8_t>(gl::VariableRowCount(attribute.type)));
    }
}
...
void ValidateShaderInterfaceVariableIsAttribute(const ShaderInterfaceVariableInfo *info)
{
    ...
    ASSERT(info->attributeRowCount > 0);
    ASSERT(info->location != ShaderInterfaceVariableInfo::kInvalid);
}
...
void SpirvVertexAttributeAliasingTransformer::preprocessAliasingAttributes()
{
    ...
    for (size_t id = 0; id < indexBound; ++id)
    {
        ...
        const bool isMatrixAttribute = info->attributeRowCount > 1;

        for (uint32_t row = 0; row < info->attributeRowCount; ++row)
        {
            uint32_t location = info->location + row;
            ...
            if (isMatrixAttribute)
            {
                ...
                // Get a new id for this row and associate it with the matrix.
                ...
                ASSERT(attributeId == mExpandedMatrixFirstVectorIdById[matrixId] + row);
    ...
    uint32_t row) const
{
    ...
    const AliasingAttributeMap *aliasingMap = &mAliasingAttributeMap[aliasingInfo->location + row];
                                                                  ...
                                                                  size_t wordCount)
{
    ...
    for (size_t index = interfaceStart; index < instructionLength; ++index)
    {
        ...
        for (uint32_t row = 1; row < info->attributeRowCount; ++row)
        {
            uint32_t vecId = vec0Id + row;
                                                                ...
                                                                size_t wordCount)
{
    ...
    if (isMatrixAttribute(id))
    {
        ...
        for (uint32_t row = 0; row < info->attributeRowCount; ++row)
        {
            ...
            (*mSpirvBlobOut)[instOffset + kDecorationValueIndex] = info->location + row;
        }
    }
    else
    {
                                                            ...
                                                            size_t wordCount)
{
    ...
    if (isMatrixAttribute(pointerId))
    {
        ...
        const uint32_t replacementTypeId = mMatrixTypes[info->attributeRowCount];
...
void SpirvVertexAttributeAliasingTransformer::declareExpandedMatrixVectors()
{
    ...
    for (size_t matrixId = 0; matrixId < mExpandedMatrixFirstVectorIdById.size(); ++matrixId)
    {
        ...
        //     %vecN-1 = OpVariable %vecType Input
        const uint32_t componentCount = info->attributeComponentCount;
        const uint32_t rowCount       = info->attributeRowCount;
        ASSERT(componentCount == rowCount);
        ASSERT(mMatrixTypes[rowCount] != 0);

        // OpTypePointer Private %matrixType
        uint32_t privateType = mPrivateMatrixTypePointers[rowCount];
        if (privateType == 0)
        {
            privateType                          = getNewId();
            mPrivateMatrixTypePointers[rowCount] = privateType;
            writeTypePointer(privateType, spv::StorageClassPrivate, mMatrixTypes[rowCount]);
        }

        // OpVariable %privateType Private
        writeVariable(matrixId, privateType, spv::StorageClassPrivate);

        // If the OpTypePointer is not declared for the vector type corresponding to each row,
        // declare it now.
        //
        //     %vecType = OpTypePointer %vecType Input
        ...
        // Declare a vector for each row of the matrix.
        for (uint32_t row = 0; row < info->attributeRowCount; ++row)
        {
            uint32_t vecId = vec0Id + row;
            if (!mIsAliasingAttributeById[vecId])
...
void SpirvVertexAttributeAliasingTransformer::writeExpandedMatrixInitialization()
{
    ...
    for (size_t matrixId = 0; matrixId < mExpandedMatrixFirstVectorIdById.size(); ++matrixId)
    {
        ...
        for (uint32_t row = 0; row < rowCount; ++row)
        {
            ...
            if (mIsAliasingAttributeById[vecId])
            {
                replacementId = getAliasingAttributeReplacementId(vecId, row);
            }
        ...
        writeCompositeConstruct(compositeId, mMatrixTypes[rowCount], vecLoadIds);
...
bool HasAliasingAttributes(const ShaderInterfaceVariableInfoMap &variableInfoMap)
{
    ...
    for (const auto &infoIter : variableInfoMap)
    {
        ...
        for (uint8_t row = 0; row < info.attributeRowCount; ++row)
        {
            uint32_t location = info.location + row;
