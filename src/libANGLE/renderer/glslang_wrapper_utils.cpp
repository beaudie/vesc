...
class SpirvTransformFeedbackCodeGenerator final : angle::NonCopyable
{
  ...
  private:
    ...
    // defined due to the uniquity of usage.
    ...
    // This allows the output generation code to always use offset += 1, thus relying only one
    // constant (1).
    struct XfbVarying
    {
    ...
    SpirvBlob *blobOut)
{
    ...
    for (const std::vector<XfbVarying> &varyings : mXfbVaryings)
    {
        ...
        const ShaderInterfaceVariableXfbInfo *info = varyings[0].info;

        // Define the buffer stride constant
        ASSERT(info->stride % sizeof(float) == 0);
        uint32_t stride = info->stride / sizeof(float);

        mBufferStrides[info->buffer] = SpirvTransformerBase::GetNewId(blobOut);
        spirv::WriteConstant(blobOut, ids.intId(), mBufferStrides[info->buffer],
                             spirv::LiteralContextDependentNumber(stride));

        compositeIds.push_back(mBufferStrides[info->buffer]);
        ...
        for (const XfbVarying &varying : varyings)
        {
            ...
            if (info->arrayElements.empty())
            {
                ASSERT(info->arraySize != ShaderInterfaceVariableXfbInfo::kInvalid);
                for (uint32_t arrayIndex = 0; arrayIndex < info->arraySize; ++arrayIndex)
                {
            ...
            {
                for (const ShaderInterfaceVariableXfbInfo &element : info->arrayElements)
                {
                    writeIntConstant(ids, element.arrayIndex, ids.intId(), blobOut);
                }
            }
        }
    }
...
void SpirvTransformFeedbackCodeGenerator::writeTransformFeedbackEmulationOutput(
    ...
    SpirvBlob *blobOut)
{
    if (!mIsEmulated || currentFunctionId != mXfbCaptureFuncId)
    {
        return;
    }
    ...
    for (uint32_t bufferIndex = 0; bufferIndex < mXfbVaryings.size(); ++bufferIndex)
    {
        ...
        for (size_t varyingIndex = 0; varyingIndex < varyings.size(); ++varyingIndex)
        {
            ...
            // component starting from the base variablestarting from the base variable.  For
            // example:
            //
            //     var.member[3][2].x
            //
            ...
            //     field 4
            //
            // The following tracks the access chain as the field, array elements, rows and columns
            // are looped over.
            spirv::IdRefList indexList;
            ...
            const bool isArray = arrayIndexEnd != ShaderInterfaceVariableXfbInfo::kInvalid;
            if (!isArray)
            {
                // Not an array.
                arrayIndexEnd = 1;
...
TransformationState SpirvTransformer::transformReturn(const uint32_t *instruction)
{
    ...
    {
        // Transform feedback emulation is written to a designated function.  Allow its code to be
        // generated if this is the right function.
        mXfbCodeGenerator.writeTransformFeedbackEmulationOutput(mIds, mVaryingPrecisionFixer,
                                                                mCurrentFunctionId, mSpirvBlobOut);
