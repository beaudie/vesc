                                               ...
                                               const std::string &xfbOut)
{
    ...
    result.append(&originalSource[xfbOutMarkerEnd], std::string::npos);
                            ...
                            ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    // emulation.
    if (options.emulateLineRasterization)
    {
    ...
    for (const gl::PackedVaryingRegister &varyingReg : resources.varyingPacking.getRegisterList())
    {
        ...
        //     out S varStruct;
        //
        // "_uvarStruct" is found through |parentStructMappedName|, with |varying->mappedNameame|
...
class SpirvTransformer final : angle::NonCopyable
{
  ...
  private:
    ...
    // Pointer to OpTypePointer instructions that define a type id.  Used to remove inactive
    // varyings by replacing them with a Private (i.e. global) variable of the same type.
...
bool SpirvTransformer::transform()
{
    ...
    mSpirvBlob->code.assign(mOriginalSpirvBlob.begin(),
...
{
    size_t indexBound = mOriginalSpirvBlob[kHeaderIndexIndexBound];
    ...
    mTypePointerInstructions.resize(indexBound + 1, nullptr);
...
void SpirvTransformer::visitTypePointer(const uint32_t *instruction)
{
    ...
    // Keep a pointer to the instruction for future reference.
    const uint32_t id = instruction[kIdIndex];
...
bool SpirvTransformer::transformDecorate(const uint32_t *instruction, size_t wordCount)
{
    ...
    if (!info->activeStages[mShaderType])
    {
        ...
        mSpirvBlob->inactiveVaryingsPatch.hunks.emplace_back(
    ...
    mSpirvBlob->code[instructionOffset + kDecorationValueIndex] = newDecorationValue;

    // If there are decorations to be added, add them right after the Location decoration is
    // encountered.
    if (decoration != spv::DecorationLocation || !info->activeStages[mShaderType])
    {
        return true;
    }
    ...
    if (info->component != ShaderInterfaceVariableInfo::kInvalid)
    {
        ...
        mSpirvBlob->code[instOffset + kDecorationIndex] = spv::DecorationComponent;
        mSpirvBlob->code[instOffset + kDecorationValueIndex] = info->component;
    }
        ...
        info->xfbBuffer != ShaderInterfaceVariableInfo::kInvalid)
    {
        ...
        for (size_t i = 0; i < kXfbDecorationCount; ++i)
        {
            ...
            mSpirvBlob->code[xfbInstructionOffset + kDecorationIndex]      = xfbDecorations[i];
            mSpirvBlob->code[xfbInstructionOffset + kDecorationValueIndex] = xfbDecorationValues[i];
        }
    }
...
bool SpirvTransformer::transformEntryPoint(const uint32_t *instruction, size_t wordCount)
{
    ...
    for (size_t index = 0; index < interfaceCount; ++index)
    {
        ...
        if (!info->activeStages[mShaderType])
        {
            ...
            mSpirvBlob->inactiveVaryingsPatch.entryPointAdditions.push_back(id);
            continue;
        }
    ...
    ASSERT(mSpirvBlob->entryPointOffset == 0);
    mSpirvBlob->entryPointOffset = entryPointOffset;
...
bool SpirvTransformer::transformVariable(const uint32_t *instruction, size_t wordCount)
{
    ...
    // Encountered: %varying = OpVariable %type Output
    //
    // This should be patched to:
    //
    // %newType = OpTypePointer Private %same_as_in_type's_instruction
    // %varying = OpVariable %newType Private
    //
    // Which effectively turns the varying into a global variable.  This allows the rest of the
    // code to reference it (and eventually be deleted through dead-code elimination) without
    // contributing to the shader interface.
    ...
    newTypeDeclaration[kTypePointerStorageClassIndex] = spv::StorageClassPrivate;

    // Copy the new type declaration.
    const size_t startOutputOffset =
        copyInstruction(newTypeDeclaration.data(), newTypeDeclaration.size());

    // Copy the variable declaration for modification.
    const size_t instructionOffset = copyInstruction(instruction, wordCount);

    mSpirvBlob->code[instructionOffset + kTypeIdIndex]       = newTypeId;
    mSpirvBlob->code[instructionOffset + kStorageClassIndex] = spv::StorageClassPrivate;

    // Create a patch hunk for inactive varyings.
    mSpirvBlob->inactiveVaryingsPatch.hunks.emplace_back(createPatchHunk(
...
size_t SpirvTransformer::copyInstruction(const uint32_t *instruction, size_t wordCount)
{
    ...
    mSpirvBlob->code.insert(mSpirvBlob->code.end(), instruction, instruction + wordCount);
    return instructionOffset;
}

size_t SpirvTransformer::getCurrentOutputOffset() const
{
    return mSpirvBlob->code.size();
}

uint32_t SpirvTransformer::getNewId()
{
    return mSpirvBlob->code[kHeaderIndexIndexBound]++;
}
                            ...
                            ShaderInterfaceVariableInfoMap *variableInfoMapOut)
{
    ...
    if (!vertexSource->empty())
    {
        ...
        {
            SubstituteTransformFeedbackMarkers(*vertexSource, "", "");
        }
        else
        {
                                        ...
                                        gl::ShaderMap<SpirvBlob> *spirvBlobsOut)
{
    ...
    for (const gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        ASSERT(ValidateSpirv(spirvBlob->code));
    }
