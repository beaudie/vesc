...
class TextureWgpu : public TextureImpl
{
  ...
  private:
    ...
    angle::Result initializeImage(ContextWgpu *contextWgpu, gl::Texture::ImageMipLevels mipLevels);

    angle::Result redefineLevel(const gl::Context *context,
                                const gl::ImageIndex &index,
                                const gl::Extents &size);

    uint32_t getMipLevelCount(gl::Texture::ImageMipLevels mipLevels) const;
