                                     ...
                                     gl::Command source)
{
    ...
    ANGLE_TRY(initializeImage(
        contextWgpu, isGenerateMipmap ? gl::Texture::ImageMipLevels::FullMipChainForGenerateMipmap
                                      : gl::Texture::ImageMipLevels::EnabledLevels));
                                                     ...
                                                     FramebufferAttachmentRenderTarget **rtOut)
{
    ...
    {
        ANGLE_TRY(initializeImage(contextWgpu, gl::Texture::ImageMipLevels::EnabledLevels));
    }
...
angle::Result TextureWgpu::initializeImage(ContextWgpu *contextWgpu,
                                           gl::Texture::ImageMipLevels mipLevels)
{
    if (mImage->isInitialized())
    {
        return angle::Result::Continue;
                                         ...
                                         const gl::Extents &size)
{
    if (mImage != nullptr)
    {
        ...
        if (mImage->isInitialized())
        {
            ...
            const bool isInAllocatedImage = mImage->isTextureLevelInAllocatedImage(levelIndexGL);
            const bool isCompatibleRedefinition =
                isInAllocatedImage && (size == wgpu_gl::getExtents(mImage->getSize()));
            ...
            // If incompatible, and redefining the single-level image, create a new image
            // immediately.
            //
                                                            ...
                                                            gl::Command source)
{
    ...
    if (!isGenerateMipmap && mImage && mImage->isInitialized())
    {
        isMipmapEnabledByMinFilter =
            mImage->getLevelCount() < getMipLevelCount(gl::Texture::ImageMipLevels::EnabledLevels);
    }
    ...
    if (isGenerateMipmap && mImage && mImage->isInitialized() &&
        (!mState.getImmutableFormat() &&
         mImage->getLevelCount() !=
             getMipLevelCount(gl::Texture::ImageMipLevels::FullMipChainForGenerateMipmap)))
    {
        mImage->flushStagedUpdates(contextWgpu);
    ...
    if (rx::HasAnyRedefinedLevels(mRedefinedLevels) || isMipmapEnabledByMinFilter)
    {
        mImage->flushStagedUpdates(contextWgpu);
...
void TextureWgpu::prepareForGenerateMipmap(ContextWgpu *contextWgpu)
{
    ...
    if (rx::IsLevelRedefined(mRedefinedLevels, mState.getType(), baseLevel))
    {
