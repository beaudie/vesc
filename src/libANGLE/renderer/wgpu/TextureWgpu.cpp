...
bool IsTextureFormatCompatibleWithImage(webgpu::ImageHelper *image, const webgpu::Format &format)
{
    return image->getIntendedFormatID() == format.getIntendedFormatID() &&
                                       ...
                                       const uint8_t *pixels)
{
    ...
    return setSubImageImpl(context, contextWgpu->getFormat(format), type, index, area, unpack,
                           pixels);
}
    ...
    if (mImage != nullptr)
    {
        ...
        if (mImage->isInitialized())
        {
            ...
            if (TextureRedefineLevel(levelAllocation, levelDefinition, mState.getImmutableFormat(),
                ...
                !IsTextureFormatCompatibleWithImage(mImage, webgpuFormat))
            {
                mImage->resetImage();
            }
        }
