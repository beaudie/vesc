                                         ...
                                         const gl::Extents &size)
{
    if (mImage != nullptr)
    {
        ...
        if (mImage->isInitialized())
        {
            bool shouldResetImage = false;
            bool isTextureLevelDefinitionCompatibleWithImage =
                size == wgpu_gl::getExtents(mImage->getSize());
            ANGLE_TRY(TextureRedefineLevel(mImage->isTextureLevelInAllocatedImage(levelIndexGL),
                                           isTextureLevelDefinitionCompatibleWithImage,
                                           mState.getImmutableFormat(), mImage->getLevelCount(),
                                           layerIndex, index, mImage->getFirstAllocatedLevel(),
                                           mRedefinedLevels, shouldResetImage));
            if (shouldResetImage)
            {
                mImage->resetImage();
            }
...
void TextureWgpu::prepareForGenerateMipmap(ContextWgpu *contextWgpu)
{
    ...
    maybeRemoveLevels(baseLevel, maxLevel, firstGeneratedLevel, mRedefinedLevels);
