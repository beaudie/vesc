...
angle::Result OffscreenSurfaceWgpu::initializeImpl(const egl::Display *display)
{
    ...
    if (config->renderTargetFormat != GL_NONE)
    {
        ...
        mColorAttachment.renderTarget.setTextureView(view);
    }

    ASSERT(config->depthStencilFormat == GL_NONE);
...
{
    mSurface.GetCurrentTexture(&mSurfaceTexture);
    ASSERT(mSurfaceTexture.status == wgpu::SurfaceGetCurrentTextureStatus::Success);

    wgpu::TextureViewDescriptor textureViewDesc;
    textureViewDesc.format          = mSurfaceTexture.texture.GetFormat();
    textureViewDesc.dimension       = wgpu::TextureViewDimension::e2D;
    textureViewDesc.baseMipLevel    = 0;
    textureViewDesc.mipLevelCount   = 1;
    textureViewDesc.baseArrayLayer  = 0;
    ...
    wgpu::TextureView view = mSurfaceTexture.texture.CreateView(&textureViewDesc);
    mRenderTarget.setTextureView(view);
...
angle::Result WindowSurfaceWgpu::swapImpl(const egl::Display *display)
{
    ...
    if (currentSize != mCurrentSurfaceSize || mSurfaceTexture.suboptimal)
    {
