...
{
    uint8_t formatPacked = static_cast<uint8_t>(format);
    if (mColorTargetStates[colorIndex].format == formatPacked)
    {
        return false;
    }

    mColorTargetStates[colorIndex].format = formatPacked;
    return true;
}
...
{
    uint8_t formatPacked = static_cast<uint8_t>(format);
    if (mDepthStencilState.format == formatPacked)
    {
        return false;
    }

    mDepthStencilState.format = formatPacked;
    return true;
}
