    ...
    *adjustedIndicesPtr = indices;
    ...
    if (clientAttributesToSync.any())
    {
        if (drawElementsTypeOrInvalid != gl::DrawElementsType::InvalidEnum)
        {
            ...
            {
                // Index data needs to be read back from the index buffer to stream vertex data
                ptrdiff_t elementArrayBufferOffset = reinterpret_cast<ptrdiff_t>(indices);
                gl::IndexRange localIndexRange;
                ANGLE_TRY(elementArrayBuffer->getIndexRange(
                    context, drawElementsTypeOrInvalid, elementArrayBufferOffset, count,
                    primitiveRestartEnabled, &localIndexRange));
                indexRange = localIndexRange;
            }
            else
            {
                // Index data is in the `indices` pointer.
                indexRange = gl::ComputeIndexRange(drawElementsTypeOrInvalid, indices, count,
    ...
    // All buffer copies must be a multiple of 4 bytes for alignment and size
    constexpr size_t kCopyBufferAlignment = 4;
    ...
    if (indexedDrawCallWithNoIndexBuffer)
    {
        ...
        stagingBufferSize += rx::roundUpPow2(indexDataSize.value(), kCopyBufferAlignment);
    }
    ...
    if (clientAttributesToSync.any())
    {
        for (size_t attribIndex : clientAttributesToSync)
        {
            ...
            stagingBufferSize += rx::roundUpPow2(attribSize, kCopyBufferAlignment);
        }
    }
    ...
    if (indexedDrawCallWithNoIndexBuffer)
    {
        ...
        size_t copySize = rx::roundUpPow2(indexDataSize.value(), kCopyBufferAlignment);
        ANGLE_TRY(
            ensureStreamingBufferCreated(mStreamingIndexBuffer, copySize,
    ...
    for (size_t attribIndex : clientAttributesToSync)
    {
        ...
        size_t copySize = rx::roundUpPow2(streamedVertexCount * typeSize, kCopyBufferAlignment);
        ANGLE_TRY(
            ensureStreamingBufferCreated(mStreamingArrayBuffers[attribIndex], copySize,
...
angle::Result VertexArrayWgpu::syncDirtyElementArrayBuffer(ContextWgpu *contextWgpu)
{
    ...
    BufferWgpu *buffer   = bufferGl ? webgpu::GetImpl(bufferGl) : nullptr;
    if (buffer)
    {
