                                               ...
                                               size_t attribIndex)
{
    if (attrib.enabled)
    {
        ...
        SetBitField(mCurrentAttribs[attribIndex].format, webgpuFormat.getActualWgpuTextureFormat());

        gl::Buffer *bufferGl = binding.getBuffer().get();
        if (bufferGl && bufferGl->getSize() > 0)
        {
            SetBitField(mCurrentAttribs[attribIndex].offset, attrib.pointer);
            BufferWgpu *bufferWgpu = GetImplAs<BufferWgpu>(bufferGl);
            size_t numVertices = GetVertices(bufferWgpu->getSize() - binding.getOffset(), binding,
                                             webgpuFormat.getIntendedFormat().pixelBytes);
            mCurrentBuffers[attribIndex] = bufferWgpu->getBuffer();
        }
        else
        {
            SetBitField(mCurrentAttribs[attribIndex].offset, binding.getOffset());
            mCurrentBuffers[attribIndex] = nullptr;
        }
    }
    return angle::Result::Continue;
}
