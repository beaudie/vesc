...
angle::Result ImageHelper::flushStagedUpdates(ContextWgpu *contextWgpu)
{
    ...
    for (const SubresourceUpdate &srcUpdate : mSubresourceQueue)
    {
        ...
        // TODO(liza) : add support for flushing clears.
        if (srcUpdate.updateSource == UpdateSource::Texture)
        {
            LevelIndex targetLevelWgpu = toWgpuLevel(srcUpdate.targetLevel);
            dst.mipLevel               = targetLevelWgpu.get();
            encoder.CopyBufferToTexture(&srcUpdate.textureData.buffer, &dst,
                                        &mTextureDescriptor.size);
        }
        else
        {
            ...
            colorAttachment.depthSlice = srcUpdate.clearData.depthSlice;
            colorAttachment.loadOp     = wgpu::LoadOp::Clear;
            colorAttachment.storeOp    = wgpu::StoreOp::Store;
            colorAttachment.clearValue = srcUpdate.clearData.clearColor;
    ...
    if (!colorAttachments.empty())
    {
        ...
        ANGLE_TRY(frameBuffer->startNewRenderPass(colorAttachments, contextWgpu));
    }
                                              ...
                                              const uint8_t *pixels)
{
    ...
    TextureUpdate dataUpload = {imageCopyBuffer};
