...
bool RenderPassColorAttachmentsEqual(
    ...
    const std::vector<wgpu::RenderPassColorAttachment> &attachments2)
{
    ...
    {
        if (!RenderPassColorAttachmentEqual(attachments1[i], attachments2[i]))
        {
            return false;
        }
    }
...
angle::Result FramebufferWgpu::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    for (size_t enabledDrawBuffer : mState.getEnabledDrawBuffers())
    {
        ...
        // TODO(liza): reset color attachments in syncState.
        colorAttachments.push_back(colorAttachment);
    }

    // Attempt to end a render pass if one has already been started.
    ANGLE_UNUSED_VARIABLE(RenderPassDepthStencilAttachmentEqual);
    if (!RenderPassColorAttachmentsEqual(mCurrentColorAttachments, colorAttachments))
    {
