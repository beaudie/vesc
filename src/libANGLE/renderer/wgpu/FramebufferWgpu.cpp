                                          ...
                                          void *ptrOrOffset)
{
    ...
    uint8_t *readPixelBuffer =
        renderTarget->getImage()->getReadPixelBuffer(contextWgpu, allocationSize);
    if (readPixelBuffer == nullptr)
    {
        return angle::Result::Stop;
    }
    ANGLE_TRY(renderTarget->getImage()->readPackPixelBuffer(params.area, params, angleFormat,
                                                            readPixelBuffer, pixels));
