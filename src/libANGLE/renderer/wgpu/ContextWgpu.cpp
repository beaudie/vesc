                                        ...
                                        const void *indices)
{
    ...
    ANGLE_TRY(setupDraw(context, mode, count, 0, 1, type, indices, &firstVertex));
                                                 ...
                                                 GLsizei instances)
{
    ...
    uint32_t firstVertex = 0;
    ANGLE_TRY(setupDraw(context, mode, count, 0, instances, type, indices, &firstVertex));
                                     ...
                                     uint32_t *outFirstIndex)
{
    ...
    if (mState.areClientArraysEnabled())
    {
        ...
        ANGLE_TRY(vertexArrayWgpu->syncClientArrays(context, mCurrentRenderPipelineAllAttributes,
                                                    vertexOrIndexCount, indexTypeOrInvalid, indices,
                                                    instanceCount, &adjustedIndicesPtr));
    }

    bool reAddDirtyIndexBufferBit = false;
    if (indexTypeOrInvalid != gl::DrawElementsType::InvalidEnum)
    {
        *outFirstIndex = gl_wgpu::GetFirstIndexForDrawCall(indexTypeOrInvalid, indices);
    ...
    {
        // If this is not an indexed draw call, don't sync the index buffer. Save it for a future
        // indexed draw call when we know what index type to use
        reAddDirtyIndexBufferBit = mDirtyBits.test(DIRTY_BIT_INDEX_BUFFER);
