...
AllocationTrackerWgpu::AllocationTrackerWgpu(size_t maxTotalAllocationSize)
    : mAllocatedBytes(0), mMaxBytes(maxTotalAllocationSize)
{}

AllocationTrackerWgpu::~AllocationTrackerWgpu()
{
    // ASSERT that all objects with the Wgpu renderer clean up after themselves
    ASSERT(mAllocatedBytes == 0);
}

bool AllocationTrackerWgpu::updateMemoryAllocation(size_t oldSize, size_t newSize)
{
...
ContextWgpu::ContextWgpu(const gl::State &state,
...
{
    return new BufferWgpu(state, mAllocationTracker);
}
