                                     ...
                                     uint32_t *outFirstIndex)
{
    ...
    {
        mDirtyBits.set(DIRTY_BIT_UNIFORMS);
    }
             ...
             dirtyBitIter != mDirtyBits.end(); ++dirtyBitIter)
        {
            ...
            switch (dirtyBit)
            {
                ...
                case DIRTY_BIT_INDEX_BUFFER:
                    ...
                    break;
                case DIRTY_BIT_UNIFORMS:
                    ANGLE_TRY(handleDirtyUniforms(&dirtyBitIter));
                    break;
                default:
                    UNREACHABLE();
                    break;
...
angle::Result ContextWgpu::handleDirtyUniforms(DirtyBits::Iterator *dirtyBitsIterator)
{
    ProgramExecutableWgpu *executableWgpu = webgpu::GetImpl(mState.getProgramExecutable());
    wgpu::BindGroup bindGroup;
    ANGLE_TRY(executableWgpu->updateUniformsAndGetBindGroup(this, &bindGroup));
    // TODO(anglebug.com/376553328): need to set up the whole bind group here.
