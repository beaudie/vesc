...
void CLExtensions::initializeVersionedExtensions(NameVersionVector &&versionedExtList)
{
    ...
    auto createExtensionString = [](NameVersionVector &extList) {
        std::string extStr = "";
        for (cl_name_version ext : extList)
        {
            extStr += ext.name;
            extStr += " ";
        }
    ...
    return initializeExtensions(createExtensionString(extensionsWithVersion));
}
