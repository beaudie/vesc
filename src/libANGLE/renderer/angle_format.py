...
def get_vertex_copy_function(src_format, dst_format):
    ...
    if 'FIXED' in src_format:
        assert 'FLOAT' in dst_format, 'get_vertex_copy_function: can only convert fixed to float, not to ' + dst_format
    ...
    assert 'FLOAT' in dst_format, 'get_vertex_copy_function: can only convert to float, not to ' + dst_format
