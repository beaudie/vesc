...
def get_vertex_copy_function(src_format, dst_format):
    ...
    if src_format.endswith('_VERTEX'):
        ...
        if 'A2' in src_format:
            return 'CopyW2XYZ10ToXYZW32FVertexData<%s, %s, true>' % (is_signed, is_normal)
        else:
            return 'CopyXYZ10ToXYZW32FVertexData<%s, %s, true>' % (is_signed, is_normal)
    ...
    return "CopyTo32FVertexData<%s, %d, %d, %s, %s>" % (src_gl_type, num_channel, num_channel,
