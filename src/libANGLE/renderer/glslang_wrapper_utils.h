...
struct SpirvPatch
{
    ...
    // Note: Currently, there is no patch that can remove interface variables.
    std::vector<uint32_t> entryPointAdditions;
    // Hunks to be replaced.  They must be sorted by |offset|.
    std::vector<SpirvPatchHunk> hunks;
};

enum class SpirvPatchType
{
    // SPIR-V patch to bring back removed inactive output varyings.
    InactiveOutputVaryings = 0,
...
template <typename T>
using SpirvPatchMap = angle::PackedEnumMap<SpirvPatchType, T>;
...
struct SpirvBlob
{
    ...
    // Extracted patches.
    SpirvPatchMap<SpirvPatch> patches;
};
...
void GlslangApplySpirvPatches(SpirvPatchMap<bool> patches,
