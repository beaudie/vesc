...
{
    static constexpr uint32_t kInvalid = -1u;

    // Used for interface blocks and opaque uniforms.
    uint32_t set     = kInvalid;
    ...
    // Used for varyings.
    angle::ShaderBitSet activeStages;
...
using ShaderInterfaceVariableInfoMap = std::unordered_map<std::string, ShaderInterfaceVariableMap>;
