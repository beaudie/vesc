...
class StreamProducerImpl : angle::NonCopyable
{
  public:
    ...
    // data. Validation on the frame will be performed if needed and an error will be returned if
    // the frame cannot be accepted. If validation succeeds, the current internal frame will be
    // replaced with the new frame.
    virtual egl::Error produceFrameFromPointer(void *pointer,
                                               const egl::AttributeMap &attributes) = 0;
