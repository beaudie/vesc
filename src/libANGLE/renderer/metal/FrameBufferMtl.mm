                                           ...
                                           const mtl::BlitParams &baseParams)
{
    ...
    if (blitDepthBuffer || blitStencilBuffer)
    {
        ...
        if (blitDepthBuffer)
        {
            ...
            dsBlitParams.srcLayer = depthRt->getLayerIndex();
        }

        if (blitStencilBuffer && stencilRt->getTexture())
        {
            dsBlitParams.srcStencil = stencilRt->getTexture()->getStencilView();
            dsBlitParams.srcLevel   = stencilRt->getLevelIndex();
            dsBlitParams.srcLayer   = stencilRt->getLayerIndex();
        }

        if (contextMtl->getDisplay()->getFeatures().reuseRenderPassForDSClearWithDraw.enabled)
        {
            // We can use the same render pass as the color blit.
            renderEncoder = ensureRenderPassStarted(context);
        }
        else
        {
            renderEncoder = ensureDepthStencilOnlyRenderPassStarted(context);
        }
                                                                   ...
                                                                   const mtl::RenderPassDesc &desc)
{
    ...
    if (contextMtl->hasStartedRenderPass(desc))
    {
    ...
    // Only support ensureRenderPassStarted() with different load & store options and/or with
    // depth/stencil only. The texture, level, slice must be the same.
    ASSERT(desc.equalIgnoreLoadStoreOptions(mRenderPassDesc) ||
           (desc.numColorAttachments == 0 && desc.sampleCount == mRenderPassDesc.sampleCount &&
            desc.depthAttachment.equalIgnoreLoadStoreOptions(mRenderPassDesc.depthAttachment) &&
            desc.stencilAttachment.equalIgnoreLoadStoreOptions(mRenderPassDesc.stencilAttachment)));
...
mtl::RenderCommandEncoder *FramebufferMtl::ensureDepthStencilOnlyRenderPassStarted(
    const gl::Context *context)
{
    // This method is used on platforms that don't allow depth stencil clear without output anything
    // to color attachemnt. In this case, we need to create a separate render pass with depth &
    // stencil attachments only.
    mtl::RenderPassDesc dpOnlyRp;
    dpOnlyRp.sampleCount       = mRenderPassDesc.sampleCount;
    dpOnlyRp.depthAttachment   = mRenderPassDesc.depthAttachment;
    dpOnlyRp.stencilAttachment = mRenderPassDesc.stencilAttachment;

    // Since we create a separate render pass with depth & stencil only, we need to save their
    // content so that when the real render pass is resumed, the depth & stencil content can be
    // re-loaded.
    setDefaultStoreAction(&dpOnlyRp.depthAttachment);
    setDefaultStoreAction(&dpOnlyRp.stencilAttachment);

    ContextMtl *contextMtl = mtl::GetImpl(context);
    // If real render pass already started, we need to save its content:
    if (renderPassHasStarted(contextMtl))
    {
        contextMtl->endEncoding(true);
    }

    return ensureRenderPassStarted(context, dpOnlyRp);
}
    ...
    mtl::RenderPassAttachmentDesc *attachmentOut)
{
    ...
    setDefaultStoreAction(attachmentOut);
}

void FramebufferMtl::setDefaultStoreAction(mtl::RenderPassAttachmentDesc *attachmentOut)
{
    mtl::RenderPassAttachmentDesc &attachment = *attachmentOut;
