...
angle::Result TextureImageSiblingMtl::initImpl(DisplayMtl *displayMtl)
{
    ...
    {
        // Hold the original texture so that the view data stays valid.
        mBaseTexture = std::move(mNativeTexture);
        unsigned textureArraySlice =
            static_cast<unsigned>(mAttribs.getAsInt(EGL_METAL_TEXTURE_ARRAY_SLICE_ANGLE, 0));
        mNativeTexture =
            mBaseTexture->createSliceMipView(textureArraySlice, mtl::kZeroNativeMipLevel);
    }
