...
macos_metal_shader_binary_output_file = 'mtl_default_shaders_compiled_macos_autogen.h'
ios_metal_shader_binary_output_file = 'mtl_default_shaders_compiled_ios_autogen.h'
...
def metal_to_air(metal_src, metal_standard, min_os_version, dest_air_file):
    temp_fname = 'temp_metal_src.metal'
    with open(temp_fname, 'wt') as out_file:
        out_file.write(metal_src.decode("utf-8"))

    result = subprocess.run([
        "xcrun", "-sdk", "macosx", "metal", "-std=" + metal_standard,
        "-mmacosx-version-min=" + min_os_version, "-c", temp_fname, "-o", dest_air_file
    ],
...
def metal_to_metallib(metal_src, metal_standard, min_os_version):
    intermediate_air_file = "temp_metal.air"
    metal_to_air(metal_src, metal_standard, min_os_version, intermediate_air_file)

    intermediate_metallib_file = "temp_metal.metallib"
    air_to_mtllib(intermediate_air_file, intermediate_metallib_file)
...
def generate_metallib_header(metllib_data, global_variable_name, dest_header_file):
    ...
    with open(dest_header_file, 'wt') as out_file:
        ...
        out_file.write('\n\nstatic constexpr uint8_t ' + global_variable_name + '[] = {\n')
        for byte in metllib_data:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        outputs = [macos_metal_shader_binary_output_file, ios_metal_shader_binary_output_file]
    ...
    macos_metallib = metal_to_metallib(combined_metal_src, "macos-metal2.1", "10.13")
    generate_metallib_header(macos_metallib, "gDefaultMetallib",
                             macos_metal_shader_binary_output_file)

    ios_metallib = metal_to_metallib(combined_metal_src, "ios-metal2.1", "11")
    generate_metallib_header(ios_metallib, "gDefaultMetallib", ios_metal_shader_binary_output_file)
