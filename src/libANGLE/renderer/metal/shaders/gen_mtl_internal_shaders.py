...
metal_shader_output_file_ext = "mtl_ext_internal_shaders_autogen.metal"
...
def main():
    ...
    # src_files_new contains code that can only be compiled with metal 2.4+
    src_files_ext = ['vertex_pulling.metal']

    # auto_script parameters.
    if len(sys.argv) > 1:
        inputs = angle_format_script_files + src_files + src_files_ext + [
            'common.h', 'constants.h'
        ]
        outputs = [
            metal_source_output_header, metal_shader_output_file, metal_shader_output_file_ext
        ]
    ...
    generate_combined_metal_src_header(
        generate_combined_metal_src(src_files + src_files_ext), metal_source_output_header)
    ...
    generate_combined_metal_shader_file(
        generate_combined_metal_src(src_files), metal_shader_output_file)
    generate_combined_metal_shader_file(
        generate_combined_metal_src(src_files_ext), metal_shader_output_file_ext)
