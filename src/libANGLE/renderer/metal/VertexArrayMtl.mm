...
uint32_t GetOffsetForVertexAttribDesc(const angle::Format &angleFormat, size_t bindingOffset)
{
    return static_cast<uint32_t>(bindingOffset % angleFormat.pixelBytes);
}
    ...
    if (dirty)
    {
        ...
        for (uint32_t v = 0; v < mtl::kMaxVertexAttribs; ++v)
        {
            ...
            else
            {
                ...
                uint32_t offsetForDesc =
                    GetOffsetForVertexAttribDesc(angleFormat, mCurrentArrayBufferOffsets[v]);
                                              ...
                                              size_t attribIndex)
{
    ...
    if (attrib.enabled)
    {
        ...
        if (bufferGL)
        {
            ...
            // 1. setVertexBuffer's offset which must be multiple of format's bytes size.
            // 1. MTLVertexAttributeDescriptor's offset which must be multiple of 4.
            uint32_t offsetForDesc =
                GetOffsetForVertexAttribDesc(format.actualAngleFormat(), binding.getOffset());
            size_t offsetForSetBuffer = binding.getOffset() - offsetForDesc;

            bool needConversion =
                format.actualFormatId != format.intendedFormatId ||
                (offsetForSetBuffer % format.actualAngleFormat().pixelBytes) != 0 ||
