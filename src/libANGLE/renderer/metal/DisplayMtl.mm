...
angle::Result DisplayMtl::initializeShaderLibrary()
{
    ...
    // Remove '@@' tokens used for preventing clang from expanding the preprocessor directives
    for (char &c : gDefaultMetallibSrc)
    {
        if (c == '@')
        {
            c = ' ';
...
id<MTLLibrary> DisplayMtl::getDefaultShadersLib()
{
    ...
    mDefaultShadersAsyncInfo->cv.wait(lg, [this] { return mDefaultShadersAsyncInfo->compiled; });
