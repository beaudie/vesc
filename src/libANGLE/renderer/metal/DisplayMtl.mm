...
void DisplayMtl::initializeTextureCaps() const
{
    ...
    // emulated.
    if (supportsIOSGPUFamily(1) && gl::DetermineCompressedTextureETCSupport(mNativeTextureCaps))
    {
    ...
    if (supportsIOSGPUFamily(3) && mNativeExtensions.textureCompressionASTCLDRKHR)
    {
        mNativeExtensions.textureCompressionSliced3dASTCKHR = true;
    }

    // Enable ASTC HDR, requires MTLGPUFamilyApple6
    if (supportsIOSGPUFamily(6) && mNativeExtensions.textureCompressionASTCLDRKHR)
    {
