...
egl::ConfigSet DisplayMtl::generateConfigs()
{
    ...
    egl::Error DisplayMtl::validateClientBuffer(const egl::Config *configuration, EGLenum buftype,
                                                EGLClientBuffer clientBuffer,
    ...
    egl::Error DisplayMtl::validateImageClientBuffer(const gl::Context *context, EGLenum target,
                                                     EGLClientBuffer clientBuffer,
                                                     const egl::AttributeMap &attribs) const
    {
        switch (target)
    ...
    void DisplayMtl::ensureCapsInitialized() const
    {
        ...
        mNativeCaps.maxShaderUniformComponents[gl::ShaderType::Vertex] =
            maxDefaultUniformComponents;
        mNativeCaps.maxFragmentUniformVectors = maxDefaultUniformVectors;
        mNativeCaps.maxShaderUniformComponents[gl::ShaderType::Fragment] =
            maxDefaultUniformComponents;
        ...
        {
            mNativeCaps.maxCombinedShaderUniformComponents[shaderType] =
    ...
    void DisplayMtl::initializeFeatures()
    {
        ...
        ANGLE_FEATURE_CONDITION((&mFeatures), allowSamplerCompareGradient,
                                supportsEitherGPUFamily(3, 1));
        ANGLE_FEATURE_CONDITION((&mFeatures), allowSamplerCompareLod,
                                supportsEitherGPUFamily(3, 1));
    ...
    angle::Result DisplayMtl::initializeShaderLibrary()
    {
        ...
        mDefaultShadersAsyncInfo->defaultShaders =
            [getMetalDevice() newDefaultLibraryWithBundle:[NSBundle bundleWithPath:path]
                                                    error:&error];
    ...
    id<MTLLibrary> DisplayMtl::getDefaultShadersLib()
    {
        ...
        if (!mDefaultShadersAsyncInfo->compiled)
        {
            ...
            mDefaultShadersAsyncInfo->cv.wait(
                lg, [this] { return mDefaultShadersAsyncInfo->compiled; });
        }
...
#    endif  // TARGET_OS_MACCATALYST
    ...
    bool DisplayMtl::isAMD() const { return angle::IsAMD(mMetalDeviceVendorId); }

    bool DisplayMtl::isIntel() const { return angle::IsIntel(mMetalDeviceVendorId); }

    bool DisplayMtl::isNVIDIA() const { return angle::IsNVIDIA(mMetalDeviceVendorId); }
        ...
        if (!mSharedEventListener)
        {
            ANGLE_MTL_OBJC_SCOPE
            {
                mSharedEventListener =
                    [[[MTLSharedEventListener alloc] init] ANGLE_MTL_AUTORELEASE];
            }
        }
