    ...
    for (auto &attribute : programAttributes)
    {
        ...
        if (regs > 1)
        {
            for (int i = 0; i < regs; i++)
            {
                ...
                stream.str("");
                // Any renamed attributes are marked as Angle Internal (see RewriteKeywords.cpp line
                // 62)
                stream << " ANGLE_" << attribute.mappedName << "_" << std::to_string(i)
                       << sh::kUnassignedAttributeString;
                attributeBindings.insert({std::string(stream.str()), i + attribute.location});
            }
        }
        else
        {
            ...
            stream.str("");
            // Any renamed attributes are marked as Angle Internal (see RewriteKeywords.cpp line 62)
            stream << " ANGLE_" << attribute.mappedName << sh::kUnassignedAttributeString;
            attributeBindings.insert({std::string(stream.str()), attribute.location});
        }
    }
    // Rewrite attributes
