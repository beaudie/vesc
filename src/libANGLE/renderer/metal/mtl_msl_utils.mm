...
constexpr char kVertexPullingHelpFuncPrefix[] = "ANGLE_pullVertexAttrib";
...
std::string RewriteShaderAttributesWithVertexPulling(
    Context *context,
    const std::string &shaderSourceIn,
    const gl::ProgramExecutable &executable,
    const std::map<std::string, AttributeBindingInfo> &attributeBindingInfos)
{
    std::ostringstream helperFunctionsSs;
    std::ostringstream bufferBindingsSs;

    std::string outputSource = shaderSourceIn;

    // Sometimes built-in attribute is also listed, so we make sure to only process
    // user defined attributes.
    std::set<std::string> userAttributes;

    for (const auto &info : attributeBindingInfos)
    {
        std::size_t attribFound = outputSource.find(info.first);
        if (attribFound != std::string::npos)
        {
            // Remove kUnassignedAttributeString marker.
            outputSource = outputSource.replace(
                attribFound + info.first.length() - strlen(sh::kUnassignedAttributeString),
                strlen(sh::kUnassignedAttributeString), "");

            userAttributes.insert(info.first);
        }
    }

    for (const auto &userAttribute : userAttributes)
    {
        const auto &info = attributeBindingInfos.at(userAttribute);

        // Declare vertex pulling function which will be stitched by mtl_pipeline_cache.cpp when
        // constructing a pipeline.
        helperFunctionsSs << "[[visible]] metal::uint4 "
                          << MslGetVertexPullingHelperFunctionName(info.location);
        helperFunctionsSs << R"(
    (
        const device uchar *inputUnaligned,
        ...
        // Declare a buffer entry in main function.
        bufferBindingsSs << "const device metal::uchar* ANGLE_vbuffer" << info.location
                         << "[[buffer(" << (mtl::kVboBindingIndexStart + info.location) << ")]],\n";
    }

    bufferBindingsSs << "const device metal::uchar* ANGLE_defaultAttribs[[buffer("
                     << kDefaultAttribsBindingIndex << ")]]";
    std::string bufferBindingsStr = bufferBindingsSs.str();

    helperFunctionsSs << R"(
template <typename T>
void reinterpret_cast_uint4_to(thread T& output, metal::uint4 src) {
    output = as_type<T>(src.x);
}

template <typename T>
void reinterpret_cast_uint4_to(thread metal::vec<T,2>& output, metal::uint4 src) {
    output.xy = as_type<metal::vec<T,2>>(src.xy);
}

template <typename T>
void reinterpret_cast_uint4_to(thread metal::vec<T,3>& output, metal::uint4 src) {
    output.xyz = as_type<metal::vec<T,3>>(src.xyz);
}

template <typename T>
void reinterpret_cast_uint4_to(thread metal::vec<T,4>& output, metal::uint4 src) {
    output.xyzw = as_type<metal::vec<T,4>>(src.xyzw);
}
)";

    //
    // Generate ANGLE_fillVertexInput function that:
    // - calls vertex pulling helper function for each attribute.
    // e.g:
    // void ANGLE_fillVertexInput(VertexIn& input, buffer1, buffer2, ...) {
    //      reinterpret_cast_uint4_to(input.attribute1, ANGLE_pullVertexAttrib1(buffer1, ...);
    //      reinterpret_cast_uint4_to(input.attribute2, ANGLE_pullVertexAttrib2(buffer2, ...);
    //      reinterpret_cast_uint4_to(input.attribute3, ANGLE_pullVertexAttrib3(buffer3, ...);
    // }
    // Note in the above example, ANGLE_pullVertexAttrib1, ... ANGLE_pullVertexAttrib3 are not
    // defined, they will be generated by mtl_pipeline_cache.mm by stitching precompiled functions.
    //
    helperFunctionsSs << "template <typename T> void ANGLE_fillVertexInput(";
    helperFunctionsSs << "thread T& input, " << bufferBindingsStr << ",";
    helperFunctionsSs << "int vertexIndex, int instanceIndex, int baseInstance) {\n";
    for (const auto &userAttribute : userAttributes)
    {
        const auto &info = attributeBindingInfos.at(userAttribute);

        helperFunctionsSs << "  reinterpret_cast_uint4_to(input." << info.name << ", "
                          << MslGetVertexPullingHelperFunctionName(info.location) << "(";
        helperFunctionsSs << "ANGLE_vbuffer" << info.location << ", ";
        helperFunctionsSs << "ANGLE_defaultAttribs, ";
        helperFunctionsSs << "vertexIndex, ";
        helperFunctionsSs << "instanceIndex, ";
        helperFunctionsSs << "baseInstance";
        helperFunctionsSs << "));\n";
    }
    helperFunctionsSs << "}\n\n";

    //
    // Define ANGLE_VERTEX_PULLING macro:
    // #define ANGLE_VERTEX_PULLING(input) \
    //   ANGLE_fillVertexInput(input, buffer1, buffer2, ...)
    //
    helperFunctionsSs << "#define ANGLE_VERTEX_PULLING(input) ";
    helperFunctionsSs << "ANGLE_fillVertexInput(input, ";
    for (const auto &userAttribute : userAttributes)
    {
        const auto &info = attributeBindingInfos.at(userAttribute);
        helperFunctionsSs << "ANGLE_vbuffer" << info.location << ", ";
    }
    helperFunctionsSs << "ANGLE_defaultAttribs, gl_VertexID, ANGLE_instanceIdMod - "
                         "ANGLE_baseInstance, ANGLE_baseInstance)\n\n\n";

    // Replace kAttributeBuffersMarker with actual buffers' bindings.
    size_t vboBindingsMarkerPos = outputSource.find(sh::kAttributeBuffersMarker);
    if (vboBindingsMarkerPos != std::string::npos)
    {
        outputSource = outputSource.replace(
            vboBindingsMarkerPos, std::size(sh::kAttributeBuffersMarker) - 1, bufferBindingsStr);
    }

    // Replace kVertexPullingFuncsMarker with generated helper functions and prototypes.
    size_t helperFuncsMarkerPos = outputSource.find(sh::kVertexPullingFuncsMarker);
    if (helperFuncsMarkerPos != std::string::npos)
    {
        outputSource =
            outputSource.replace(helperFuncsMarkerPos, std::size(sh::kVertexPullingFuncsMarker) - 1,
                                 helperFunctionsSs.str());
    }

    // Replace kVertexPullingMarker with ANGLE_VERTEX_PULLING macro call.
    size_t vertexPulingCallMarkerPos = outputSource.find(sh::kVertexPullingMarker);
    if (vertexPulingCallMarkerPos != std::string::npos)
    {
        outputSource =
            outputSource.replace(vertexPulingCallMarkerPos, std::size(sh::kVertexPullingMarker) - 1,
                                 "ANGLE_VERTEX_PULLING");
    }

    return outputSource;
}

std::string RewriteShaderAttributes(
    Context *context,
    const std::string &shaderSourceIn,
    const gl::ProgramExecutable &executable,
    const std::map<std::string, AttributeBindingInfo> &attributeBindingInfos)
{
    ...
    for (auto it = attributeBindingInfos.begin(); it != attributeBindingInfos.end(); ++it)
    {
        ...
        if (attribFound != std::string::npos)
        {
            ...
            stream << "[[attribute(" << it->second.location << ")]]";
            outputSource = outputSource.replace(
...
std::string updateShaderAttributes(Context *context,
                                   const std::string &shaderSourceIn,
                                   const gl::ProgramExecutable &executable)
{
    ...
    std::map<std::string, AttributeBindingInfo> attributeBindings;
    for (auto &attribute : programAttributes)
    {
        ...
        if (regs > 1)
        {
            ...
            {
                int binding = i + attribute.getLocation();

                stream.str("");
                stream << " " << kUserDefinedNamePrefix << attribute.name << "_"
                       << std::to_string(i);

                AttributeBindingInfo info;
                info.location = binding;
                info.name     = stream.str();

                attributeBindings.insert({info.name + sh::kUnassignedAttributeString, info});
            }
        }
        else
        {
            ...
            AttributeBindingInfo info;
            info.location = attribute.getLocation();
            info.name     = stream.str();
    ...
    // Rewrite attributes
    if (context->getDisplay()->getFeatures().useVertexPulling.enabled)
    {
        return RewriteShaderAttributesWithVertexPulling(context, shaderSourceIn, executable,
                                                        attributeBindings);
    }
    return RewriteShaderAttributes(context, shaderSourceIn, executable, attributeBindings);
}
                        ...
                        gl::ShaderMap<TranslatedShaderInfo> *mslShaderInfoOut)
{
    ...
    for (gl::ShaderType type : {gl::ShaderType::Vertex, gl::ShaderType::Fragment})
    {
        ...
        {
            source = updateShaderAttributes(context, shaderSources[type], executable);
...
std::string MslGetVertexPullingHelperFunctionName(uint32_t attributeIdx)
{
    ...
    ss << kVertexPullingHelpFuncPrefix << attributeIdx << "AsUInt4";
    return ss.str();
}
