...
{
    ANGLE_MTL_OBJC_SCOPE { return [get() newSamplerStateWithDescriptor:descriptor]; }
}
    ...
    NSUInteger plane) const
{
    ANGLE_MTL_OBJC_SCOPE
    {
        // No need to set ownership as the iosurface's ownership should already be managed
...
AutoObjCPtr<id<MTLLibrary>> ContextDevice::newLibraryWithSource(
    ...
    __autoreleasing NSError **error) const
{
    ANGLE_MTL_OBJC_SCOPE { return [get() newLibraryWithSource:source options:options error:error]; }
}
...
{
    ANGLE_MTL_OBJC_SCOPE { return [get() newDepthStencilStateWithDescriptor:descriptor]; }
}

AutoObjCPtr<id<MTLSharedEvent>> ContextDevice::newSharedEvent() const
{
    ANGLE_MTL_OBJC_SCOPE { return [get() newSharedEvent]; }
}
