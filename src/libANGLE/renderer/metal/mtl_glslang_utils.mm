...
{
    std::string source = translatedSource;
...
{
  public:
    SpirvToMslCompiler(rx::mtl::Context *context, angle::spirv::Blob &&spriv)
  ...
  private:
    ...
    std::string to_function_args(const TextureFunctionArguments &args, bool *pForward) override
    {
        ...
        std::string argsString = to_expression(args.base.img);
        argsString += ", ";
        argsString += argsWithoutDref;
        argsString += ", ";
        ...
        if (args.base.is_proj)
            drefExpr =
                spirv_cross::join(to_enclosed_expression(args.dref), " / ",
        ...
        argsString += drefExpr;
        argsString += ", ";
        argsString += toShadowCompareModeExpression(args.base.img);
        ...
        for (auto &arg : func.arguments)
        {
            ...
            if (arg_type.basetype == SPIRType::SampledImage && !is_dynamic_img_sampler)
            {
                ...
                if (arg_type.image.dim != DimBuffer)
                    decl += join(", thread const ", sampler_type(arg_type, name_id), " ", to_sampler_expression(arg.id));
            }
