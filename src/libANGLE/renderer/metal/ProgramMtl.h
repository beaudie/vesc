...
class ProgramMtl : public ProgramImpl, public mtl::RenderPipelineCacheSpecializeShaderFactory
{
  ...
  private:
    ...
    std::unique_ptr<LinkEvent> linkImplDirect(const gl::Context *glContext,
                                              const gl::ProgramLinkedResources &resources,
                                              gl::InfoLog &infoLog);

    void linkResources(const gl::Context *context, const gl::ProgramLinkedResources &resources);
    std::unique_ptr<LinkEvent> linkImpl(const gl::Context *glContext,
                                        const gl::ProgramLinkedResources &resources,
                                        gl::InfoLog &infoLog);

    std::unique_ptr<LinkEvent> linkTranslatedShaders(const gl::Context *glContext,
                                                     gl::BinaryInputStream *stream,
                                                     gl::InfoLog &infoLog);
