                                                             ...
                                                             const gl::Extents &size)
{
    ...
    if (mDepthFormat.valid() && (!mDepthTexture || mDepthTexture->sizeAt0() != size))
    {
        ...
        mDepthStencilTexturesInitialized =
            false;  // Robust resource case should initialize depth to 1.0.
    }
    ...
    if (mStencilFormat.valid() && (!mStencilTexture || mStencilTexture->sizeAt0() != size))
    {
        ...
        if (!mDepthTexture)
        {
            // Robust resource case should initialize stencil to zero. New textures start as zero,
            // so ensure that a draw after swap + resize does not initialize redundantly.
            mDepthStencilTexturesInitialized = true;
        }
    }
...
angle::Result WindowSurfaceMtl::swapImpl(const gl::Context *context)
{
    ...
    // Robust resource case should initialize stencil zero and depth to 1.0 after swap.
