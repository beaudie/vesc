...
angle::Result WindowSurfaceMtl::ensureColorTextureReadyForReadPixels(const gl::Context *context)
{
    ...
    {
        if (mRetainedColorTexture->isCPUReadMemNeedSync())
        {
            CopyTextureNoScale(mtl::GetImpl(context), mRetainedColorTexture, &mColorTexture);
            mRetainedColorTexture->resetCPUReadMemNeedSync();
        }
    }
