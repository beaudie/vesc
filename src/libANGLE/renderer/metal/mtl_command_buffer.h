...
class CommandBuffer final : public WrappedObject<id<MTLCommandBuffer>>, angle::NonCopyable
{
  ...
  private:
    ...
    // Note: due to render command encoder being a deferred
    // encoder, it can coexist with blit/compute encoder. When submitting,
    // blit/compute encoder will be executed before the render encoder.
