...
class IOSurfaceBaseMtl : public SurfaceMtlProtocol
{
  public:
    ~IOSurfaceBaseMtl() override;
  ...
  protected:
    IOSurfaceBaseMtl(const egl::SurfaceState &state);

    void initializeWithIOSurface(DisplayMtl *display,
    ...
    bool mIOSurfaceTextureCreated = false;
    int mWidth                    = 0;
    int mHeight                   = 0;
    int mPlane                    = 0;
    int mFormatIndex              = 0;
    int mRowStrideInPixels        = 0;

#if defined(ANGLE_PLATFORM_IOS_SIMULATOR)
    GLuint mBoundTextureID    = 0;
    bool mUploadFromIOSurface = false;
    bool mReadbackToIOSurface = false;
#endif
};

class IOSurfaceClientSurfaceMtl : public IOSurfaceBaseMtl
{
  public:
    // For eglCreatePbufferFromClientBuffer
    IOSurfaceClientSurfaceMtl(DisplayMtl *display,
                              const egl::SurfaceState &state,
                              EGLClientBuffer buffer,
                              const egl::AttributeMap &attribs);
};

class IOSurfacePbufferSurfaceMtl : public IOSurfaceBaseMtl
{
  public:
    // For eglCreatePbufferSurface
    IOSurfacePbufferSurfaceMtl(DisplayMtl *display,
                               const egl::SurfaceState &state,
                               const egl::AttributeMap &attribs);
