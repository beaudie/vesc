...
{
    uint64_t serial                 = 0;
    MTLCommandBuffer metalCmdBuffer = mCmdQueue.makeMetalCommandBuffer(&serial);
...
{
    if (mStateCache.viewport.ready() && mStateCache.viewport.value() == viewport)
    {
        return *this;
    }
...
RenderCommandEncoder &RenderCommandEncoder::setScissorRect(const MTLScissorRect &rect)
{
    ...
    if (mStateCache.scissorRect.ready() && mStateCache.scissorRect.value() == clampedRect)
    {
        return *this;
    }
                                                            ...
                                                            uint32_t index)
{
    ...
    RenderCommandEncoderShaderStates &shaderStates = mStateCache.perShaderStates[shaderType];
    if (shaderStates.samplers[index] == state && shaderStates.samplerLodClamps[index].ready())
    {
...
BlitCommandEncoder &BlitCommandEncoder::synchronizeResource(BufferRef *buffer)
{
    if (!buffer)
    {
        return *this;
