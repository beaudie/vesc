...
bool TextureMtl::needsFormatViewForPixelLocalStorage(const gl::Context *context) const
{
    ...
    // the texture if you set MTLTextureUsagePixelFormatView. This shouldn't be a problem though
    ...
    {
        // Only the pixel local storage formats that require a different format view need
                                            ...
                                            GLenum format)
{
    ...
    gl::TexLevelArray<ImageDefinitionMtl> &levelsForFormat = mShaderImageViews[format];
    GLuint imageMipLevel =
        mtl::GetGLMipLevel(mtl::kZeroNativeMipLevel + level, mState.getEffectiveBaseLevel());
    ImageDefinitionMtl &imageDef = levelsForFormat[imageMipLevel];

    if (imageDef.image == nullptr)
    {
        imageDef.image = mNativeTexture->createShaderImageView(
            mtl::kZeroNativeMipLevel + level, layer, imageAccessFormat.metalFormat);
        imageDef.formatID = angleFormatId;
    }

    cmdEncoder->setTexture(shaderType, imageDef.image, textureSlotIndex);
    return angle::Result::Continue;
}
