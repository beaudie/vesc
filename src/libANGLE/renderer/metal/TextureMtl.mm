...
angle::Result TextureMtl::onBaseMaxLevelsChanged(const gl::Context *context)
{
    ...
    const GLuint newBaseLevel      = mState.getEffectiveBaseLevel();
    const gl::Extents &newBaseSize = mState.getBaseLevelDesc().size;
    const GLuint newNumLevels      = mState.getMipmapMaxLevel() - newBaseLevel + 1;
    if (newBaseLevel == mNativeTextureStorage->getBaseGLLevel() &&
        newBaseSize == mNativeTextureStorage->sizeAt0() &&
        newNumLevels == mNativeTextureStorage->mipmapLevels())
    {
