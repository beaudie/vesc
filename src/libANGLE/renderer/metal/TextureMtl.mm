...
angle::Result TextureMtl::onBaseMaxLevelsChanged(const gl::Context *context)
{
        ...
        mState.getMipmapMaxLevel() == mNativeTextureStorage->getMaxSupportedGLLevel())
    {
        ...
        // If level range remain the same, don't recreate the texture.
        ...
        // before base/max level dirty bit is passed to Metal backend. Example:
        ...
        // 3. The same texture is then used for sampling; this operation fully syncs the texture state. Base/max
        // level dirty bits may lead to recreating the texture storage thus invalidating native
        // render target references created in step 2.
        // 4. If the framebuffer created in step 2 is used again, its native render target references will not be updated to point to the new
        // storage because everything is in sync from the frontend point of view.
        // 5. The issue is bypassed by not recreating the texture storage in step 3 if the new
        // effective level range is the same as the existing one. If the new range is different, it
        // is expected that native render target references will be updated during draw framebuffer
        // sync.
        return angle::Result::Continue;
    }
