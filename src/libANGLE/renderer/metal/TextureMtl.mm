...
angle::Result TextureMtl::onBaseMaxLevelsChanged(const gl::Context *context)
{
    ...
    const gl::Extents &newBaseSize = mState.getBaseLevelDesc().size;
    const GLuint newLevels = mState.getMipmapMaxLevel() - mState.getEffectiveBaseLevel() + 1;
    if (newBaseSize == mNativeTextureStorage->sizeAt0() &&
        newLevels == mNativeTextureStorage->mipmapLevels())
    {
        // If size & level range remain the same, don't recreate the texture.
        return angle::Result::Continue;
    }
    ...
    // lazily recreated later via ensureNativeStorageCreated(). Note: We release the native texture
    // storage but keep old image definitions. So that when the storage is recreated, its levels can
    // be recreated with data from the respectively old image definitions.
