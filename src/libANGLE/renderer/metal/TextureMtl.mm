...
// It seemlessly translates GL level to native level based on the
// base GL information passed in the constructor. The base GL level
// is unchanged thoughout the lifetime of this object.
// Note that NativeTextureWrapper's base GL level doesn't necessarily mean
// it's the same as a GL texture's real base level.
...
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createCubeFaceView(uint32_t face)
{
    return mNativeTexture->createCubeFaceView(face);
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createSliceMipView(uint32_t slice,
                                                                                    GLuint glLevel)
{
    return mNativeTexture->createSliceMipView(slice, getNativeLevel(glLevel));
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createMipsView(GLuint glLevel,
                                                                                uint32_t levels)
{
    return mNativeTexture->createMipsView(getNativeLevel(glLevel), levels);
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createMipView(GLuint glLevel)
{
    return mNativeTexture->createMipView(getNativeLevel(glLevel));
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createViewWithDifferentFormat(
    MTLPixelFormat format)
{
    return mNativeTexture->createViewWithDifferentFormat(format);
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createShaderImageView(
    GLuint glLevel,
    int layer,
    MTLPixelFormat format)
{
...
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createViewWithCompatibleFormat(
    MTLPixelFormat format)
{
    return mNativeTexture->createViewWithCompatibleFormat(format);
}
mtl::TextureRef TextureMtl::NativeTextureWrapperWithViewSupport::createMipsSwizzleView(
    GLuint glLevel,
    uint32_t levels,
    MTLPixelFormat format,
    const mtl::TextureSwizzleChannels &swizzle)
{
    return mNativeTexture->createMipsSwizzleView(getNativeLevel(glLevel), levels, format, swizzle);
}
