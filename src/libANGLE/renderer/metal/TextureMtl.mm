                                              ...
                                              const gl::Extents &size)
{
    ...
    {
        for (mtl::MipmapNativeLevel actualMip; actualMip < mips; ++actualMip)
        {
            GLuint imageMipLevel = actualMip.getWithBase(mState.getEffectiveBaseLevel());
...
angle::Result TextureMtl::ensureNativeLevelViewsCreated()
{
    ...
    for (mtl::MipmapNativeLevel mip; mip < mNativeTexture->mipmapLevels(); ++mip)
    {
        ...
        if (mNativeTexture->textureType() != MTLTextureTypeCube &&
            mTexImageDefs[0][mip.getWithBase(baseLevel)].image)
        {
            // Reuse texture image view.
            mNativeLevelViews[mip] = mTexImageDefs[0][mip.getWithBase(baseLevel)].image;
        }
        else
        {
...
void TextureMtl::retainImageDefinitions()
{
    ...
    for (int face = 0; face < numCubeFaces; ++face)
    {
        for (mtl::MipmapNativeLevel mip; mip < mips; ++mip)
        {
            GLuint imageMipLevel         = mip.getWithBase(mState.getEffectiveBaseLevel());
...
mtl::MipmapNativeLevel TextureMtl::getNativeLevel(const gl::ImageIndex &imageIndex) const
{
    ...
    return mtl::MipmapNativeLevel(imageIndex.getLevelIndex(), baseLevel);
}
...
angle::Result TextureMtl::generateMipmapCPU(const gl::Context *context)
{
    ...
    {
        mtl::MipmapNativeLevel maxMipLevel(mNativeTexture->mipmapLevels() - 1, 0);
                                                   ...
                                                   const mtl::TextureRef &texture)
{
    ...
    if (emulatedChannels)
    {
        ...
        for (uint32_t layer = 0; layer < layers; ++layer)
        {
            ...
            {
                auto index = mtl::ImageIndexLegalizer::FromNativeIndex(
                                             ...
                                             const gl::ImageIndex &index)
{
    ...
    return mtl::InitializeTextureContents(
        ...
        mtl::ImageIndexLegalizer::FromNativeIndex(GetZeroLevelIndex(image)));
}
                                                 ...
                                                 const mtl::TextureRef &sourceTexture)
{
    ...
    mtl::RenderCommandEncoder *cmdEncoder = contextMtl->getTextureRenderCommandEncoder(
        image, mtl::ImageIndexLegalizer::FromNativeIndex(GetZeroLevelIndex(image)));
    mtl::ColorBlitParams blitParams;
