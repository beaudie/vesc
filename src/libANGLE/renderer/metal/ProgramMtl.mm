...
{
#if ANGLE_ENABLE_METAL_SPIRV
    if (angle::GetBoolEnvironmentVar("ANGLE_GEN_MTL_WITH_SPIRV"))
    {
        return @{};
    }
    else
...
ProgramMtl::ProgramMtl(const gl::ProgramState &state)
    : ProgramImpl(state), mMetalRenderPipelineCache(this), mAuxBufferPool(nullptr)
{
...
#if ANGLE_ENABLE_METAL_SPIRV
    static const bool useSpirv = angle::GetBoolEnvironmentVar("ANGLE_GEN_MTL_WITH_SPIRV");
#endif
...
angle::Result ProgramMtl::commitUniforms(ContextMtl *context, mtl::RenderCommandEncoder *cmdEncoder)
{
    for (gl::ShaderType shaderType : gl::kAllGLES2ShaderTypes)
    {
        ...
        if (uniformBlock.uniformData.size() > mtl::kDefaultUniformsMaxSize)
        {
            ...
            ASSERT(pool);  // Ensure we allocated a buffer pool to hold a large uniform group.
            else
            {
                ...
                size_t offsetOut;
                uint8_t *ptrOut;
                ANGLE_TRY(pool->allocate(context, uniformBlock.uniformData.size(), &ptrOut, &refOut,
                                         &offsetOut));
                memcpy(ptrOut, uniformBlock.uniformData.data(), uniformBlock.uniformData.size());
                // Send data to the shader.
                cmdEncoder->setBuffer(shaderType, refOut, (uint32_t)offsetOut,
