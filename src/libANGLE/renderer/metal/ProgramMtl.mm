                                                ...
                                                gl::InfoLog &infoLog)
{
    ...
    return linkTranslatedShaders(context, stream, infoLog);
}
                                            ...
                                            const gl::ProgramMergedVaryings &mergedVaryings)
{
    ...
    return linkImpl(context, resources, infoLog);
}

std::unique_ptr<LinkEvent> ProgramMtl::linkImplDirect(const gl::Context *glContext,
                                                      const gl::ProgramLinkedResources &resources,
                                                      gl::InfoLog &infoLog)
{
    ...
    reset(contextMtl);
    ANGLE_PARALLEL_LINK_TRY(initDefaultUniformBlocks(glContext));
    ...
    ANGLE_PARALLEL_LINK_TRY(mtl::MTLGetMSL(
        glContext, mState, contextMtl->getCaps(), shaderSources, &mMslShaderTranslateInfo,
    ...
    return compileMslShaderLibs(glContext, infoLog);
}
...
std::unique_ptr<LinkEvent> ProgramMtl::linkImpl(const gl::Context *glContext,
                                                const gl::ProgramLinkedResources &resources,
                                                gl::InfoLog &infoLog)
{
...
std::unique_ptr<LinkEvent> ProgramMtl::linkTranslatedShaders(const gl::Context *glContext,
                                                             gl::BinaryInputStream *stream,
                                                             gl::InfoLog &infoLog)
{
    ContextMtl *contextMtl = mtl::GetImpl(glContext);
    ...
    ANGLE_PARALLEL_LINK_TRY(loadDefaultUniformBlocksInfo(glContext, stream));
    ANGLE_PARALLEL_LINK_TRY(loadInterfaceBlockInfo(glContext, stream));
...
class ProgramMtl::CompileMslTask final : public angle::Closure
{
  public:
    ...
    {
        mResult = createMslShaderLib(mContext, mShaderType, mInfoLog, mTranslatedMslInfo,
                                     mSubstitutionMacros);
    }
                                                            ...
                                                            gl::InfoLog &infoLog)
{
    ...
    for (gl::ShaderType shaderType : gl::kAllGLES2ShaderTypes)
    {
        ...
        {
            auto task = std::make_shared<ProgramMtl::CompileMslTask>(contextMtl, shaderType,
                                                                     translateInfo, macros);
