...
void BufferMtl::destroy(const gl::Context *context)
{
    ...
    if (mBuffer)
    {
        contextMtl->getDisplay()->getBufferManager().returnBuffer(contextMtl, mBuffer);
        mBuffer = nullptr;
    }
                                     ...
                                     gl::BufferUsage usage)
{
    ...
    mtl::BufferManager &bufferManager = contextMtl->getDisplay()->getBufferManager();
    if (mBuffer)
    {
    ...
    // Get a new buffer
    ANGLE_TRY(bufferManager.getBuffer(contextMtl, adjustedSize, mBuffer));
...
// TODO: The Metal backend doesn't have this info AFAICT
// For now just use this to choose either
//   true:
//      * break current renderpass
//      * copy data to staging buffer
//      * blit new data to existing buffer
//  false:
//      * create a new buffer
//      * copy data to new buffer
//      * copy unchanged portions of old buffer to new buffer via blit
//      * subsequent draws will use new buffer
                                        ...
                                        size_t offset)
{
    ...
    if (mBuffer->isBeingUsedByGPU(contextMtl))
    {
        if (isBeingWrittenToDuringRenderPass())
        {
            // fprintf(stderr, " written by GPU: use staging buffer\n");
            //  we can't use the buffer now so copy the data
            //  to a staging buffer and blit it in
            mtl::BufferManager &bufferManager = contextMtl->getDisplay()->getBufferManager();
            ANGLE_TRY(bufferManager.queueBlitCopyDataToBuffer(contextMtl, srcPtr, sizeToCopy,
                                                              offset, mBuffer));
        }
        else
        {
            ...
            // * copy any old data to the new buffer
            // * start using the new buffer
            // mBufferPool.releaseInFlightBuffers(contextMtl);
            mtl::BufferManager &bufferManager = contextMtl->getDisplay()->getBufferManager();
            mtl::BufferRef oldBuffer          = mBuffer;
            // fprintf(stderr, "--oldBuffer: %p\n", oldBuffer.get());
            ANGLE_TRY(bufferManager.getBuffer(contextMtl, mGLSize, mBuffer));
            mBuffer->get().label =
                [NSString stringWithFormat:@"BufferMtl=%p(%lu)", this, ++mRevisionCount];
            ...
            if (offset > 0 || offset + sizeToCopy < mGLSize)
            {
                mtl::BlitCommandEncoder *blitEncoder =
                    contextMtl->getBlitCommandEncoderWithoutEndingRenderPass();
                if (offset > 0)
                {
                ...
                if (offset + sizeToCopy < mGLSize)
                {
                    ...
                    blitEncoder->copyBuffer(oldBuffer, endOffset, mBuffer, endOffset,
                                            endSizeToCopy);
                }
            }
    ...
    {
        // fprintf(stderr, " not used so just map and update\n");
