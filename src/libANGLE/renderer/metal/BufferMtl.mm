...
bool isOffsetAndSizeMetalCompatible(size_t offset, size_t size)
{
...
angle::Result BufferMtl::updateExistingBufferVisBlitFromStagingBuffer(ContextMtl *contextMtl,
                                                                      const uint8_t *srcPtr,
                                                                      size_t sizeToCopy,
                                                                      size_t offset)
{
    ASSERT(isOffsetAndSizeMetalCompatible(offset, sizeToCopy));
...
angle::Result BufferMtl::copyDataToExistingBuffer(ContextMtl *contextMtl,
                                                  const uint8_t *srcPtr,
                                                  size_t sizeToCopy,
                                                  size_t offset)
{
                                        ...
                                        size_t offset)
{
    ...
    if (isOffsetAndSizeMetalCompatible(offset, size) &&
        (alwaysUseStagedBufferUpdates || mBuffer->isBeingUsedByGPU(contextMtl)))
    {
        if (alwaysUseStagedBufferUpdates || isBeingWrittenToDuringRenderPass())
        {
            ...
            return updateExistingBufferVisBlitFromStagingBuffer(contextMtl, srcPtr, sizeToCopy,
                                                                offset);
        }
        else
        {
    ...
    {
        return copyDataToExistingBuffer(contextMtl, srcPtr, sizeToCopy, offset);
    }
}
