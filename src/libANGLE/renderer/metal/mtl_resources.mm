...
gl::Extents Texture::size(const ImageIndexLegalizer &index) const
{
    gl::Extents extents = size(index.getNativeLevel());

    if (index.getNativeIndex().hasLayer())
    {
        extents.depth = 1;
    }
...
void Buffer::flush(ContextMtl *context, size_t offsetWritten, size_t sizeWritten)
{
    ...
    if (!mMapReadOnly)
    {
        if (get().storageMode == MTLStorageModeManaged)
        {
            ...
            [get() didModifyRange:NSMakeRange(startOffset, endOffset - startOffset)];
        }
    }
#endif
}
