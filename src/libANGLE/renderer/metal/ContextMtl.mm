                                                        ...
                                                        GLsizei instances)
{
    ...
    mtl::BufferRef drawIdxBuffer = mProvokingVertexHelper.generateIndexBuffer(
        mtl::GetImpl(context), first, count, mode, convertedType, outIndexCount, outIndexOffset, outIndexMode);
...
#define DRAW_PROVOKING_VERTEX_ARRAY(xfbPass)                                                      \
    ...
    {                                                                                             \
        ANGLE_TRY(setupDraw(context, outIndexMode, 0, outIndexCounti32, instances, convertedType, \
                            drawIdxBuffer->mapReadOnly(this) + outIndexOffset, xfbPass));                          \
                                                                                                  \
        MTLPrimitiveType mtlType = mtl::GetPrimitiveType(outIndexMode);                           \
        MTLIndexType mtlIdxType  = mtl::GetIndexType(convertedType);                              \
        if (instances == 0)                                                                       \
        {                                                                                         \
            mRenderEncoder.drawIndexed(mtlType, outIndexCounti32, mtlIdxType, drawIdxBuffer, outIndexOffset);  \
        }                                                                                         \
        else                                                                                      \
        {                                                                                         \
                                           ...
                                           GLsizei instances)
{
    ...
    if (requiresIndexRewrite(context->getState()))
    {
        ...
        drawIdxBuffer             = mProvokingVertexHelper.preconditionIndexBuffer(
            ...
            mState.isPrimitiveRestartEnabled(), mode, convertedType, outIndexCount, provokingVertexAdditionalOffset, newMode);
        if (!drawIdxBuffer)
        {
            return angle::Result::Stop;
        }
...
void ContextMtl::updateProgramExecutable(const gl::Context *context)
{
    ...
    invalidateDefaultAttributes(context->getStateCache().getActiveDefaultAttribsMask());
    // Render pipeline need to be re-applied594
    invalidateRenderPipeline();
}
