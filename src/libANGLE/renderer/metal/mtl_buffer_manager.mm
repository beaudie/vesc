                                       ...
                                       BufferRef &bufferRef)
{
    ...
    {
        // Flush the current command buffer so that the in-flight buffers are
        // scheduled and will eventually become unreferenced. They won't be
        // collected during this GC cycle but hopefully will in the next one.
        contextMtl->flushCommandBuffer(mtl::NoWait);
        if (mInUseBuffers.size() > 0 && lotOfMemAllocedRecently)
        {
            // A considerable amount of memory has been allocated since the last
            // GC. Wait for buffers to become unused so they can be freed.
            contextMtl->cmdQueue().ensureResourceReadyForCPU(mInUseBuffers.back());
        }
        freeUnusedBuffers(contextMtl);
