...
class EventSyncImpl : public SyncImpl
{
  ...
  public:
    ...
    angle::Result set(ContextMtl *contextMtl)
    {
        ...
        contextMtl->queueEventSignal(mMetalEvent, kEventSignalValue);
        mEncodedCommandBufferSerial = contextMtl->getCurrentEncoderSerial();
        return angle::Result::Continue;
    }
...
EGLSyncMtl::EGLSyncMtl(const egl::AttributeMap &attribs) : EGLSyncImpl()
{
    ...
    {
        mSignalValue = mtl::UnpackSignalValue(
    ...
    mCondition = attribs.getAsInt(EGL_SYNC_CONDITION, 0);
}
                                  ...
                                  EGLenum type)
{
    ...
    switch (type)
    {
        ...
        case EGL_SYNC_METAL_SHARED_EVENT_ANGLE:
        {
            ...
            uint64_t signalValue = mSignalValue.has_value() ? mSignalValue.value()
                                                            : mSharedEvent.get().signaledValue + 1;
            ...
            bool enqueEvent = mCondition != EGL_SYNC_METAL_SHARED_EVENT_SIGNALED_ANGLE;

            std::unique_ptr<mtl::SharedEventSyncImpl> impl =
                std::make_unique<mtl::SharedEventSyncImpl>();
            if (IsError(impl->set(contextMtl, mSharedEvent, signalValue, enqueEvent)))
            {
                return egl::Error(EGL_BAD_ALLOC, "eglCreateSyncKHR failed to create sync object");
            }
            break;
...
{
    ASSERT(mType == EGL_SYNC_METAL_SHARED_EVENT_ANGLE);
