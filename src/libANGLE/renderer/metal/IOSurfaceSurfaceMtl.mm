...
IOSurfaceBaseMtl::IOSurfaceBaseMtl(const egl::SurfaceState &state) : SurfaceMtlProtocol(state) {}

IOSurfaceBaseMtl::~IOSurfaceBaseMtl()
{
    StopFrameCapture();
...
void IOSurfaceBaseMtl::destroy(const egl::Display *display) {}

egl::Error IOSurfaceBaseMtl::initialize(const egl::Display *display)
{
...
FramebufferImpl *IOSurfaceBaseMtl::createDefaultFramebuffer(const gl::Context *context,
...
egl::Error IOSurfaceBaseMtl::makeCurrent(const gl::Context *context)
{
...
egl::Error IOSurfaceBaseMtl::unMakeCurrent(const gl::Context *context)
{
    StopFrameCapture();
    return egl::NoError();
}

egl::Error IOSurfaceBaseMtl::swap(const gl::Context *context)
{
    StopFrameCapture();
    ContextMtl *contextMtl = mtl::GetImpl(context);
    StartFrameCapture(contextMtl);
...
egl::Error IOSurfaceBaseMtl::postSubBuffer(const gl::Context *context,
                                           EGLint x,
                                           EGLint y,
                                           EGLint width,
                                           EGLint height)
...
egl::Error IOSurfaceBaseMtl::querySurfacePointerANGLE(EGLint attribute, void **value)
{
    UNIMPLEMENTED();
    return egl::EglBadAccess();
}

egl::Error IOSurfaceBaseMtl::bindTexImage(const gl::Context *context,
                                          gl::Texture *texture,
                                          EGLint buffer)
{
...
egl::Error IOSurfaceBaseMtl::releaseTexImage(const gl::Context *context, EGLint buffer)
{
...
egl::Error IOSurfaceBaseMtl::getSyncValues(EGLuint64KHR *ust, EGLuint64KHR *msc, EGLuint64KHR *sbc)
{
    UNIMPLEMENTED();
    return egl::EglBadAccess();
}

egl::Error IOSurfaceBaseMtl::getMscRate(EGLint *numerator, EGLint *denominator)
{
    UNIMPLEMENTED();
    return egl::EglBadAccess();
}

void IOSurfaceBaseMtl::setSwapInterval(EGLint interval)
{
    UNIMPLEMENTED();
}

void IOSurfaceBaseMtl::setFixedWidth(EGLint width)
{
    UNIMPLEMENTED();
}

void IOSurfaceBaseMtl::setFixedHeight(EGLint height)
{
    UNIMPLEMENTED();
}

// width and height can change with client window resizing
EGLint IOSurfaceBaseMtl::getWidth() const
{
    return static_cast<EGLint>(mWidth);
}

EGLint IOSurfaceBaseMtl::getHeight() const
{
    return static_cast<EGLint>(mHeight);
}

EGLint IOSurfaceBaseMtl::isPostSubBufferSupported() const
{
    return EGL_FALSE;
}

EGLint IOSurfaceBaseMtl::getSwapBehavior() const
{
    return EGL_BUFFER_DESTROYED;
}

angle::Result IOSurfaceBaseMtl::getAttachmentRenderTarget(const gl::Context *context,
                                                          GLenum binding,
...
angle::Result IOSurfaceBaseMtl::ensureCurrentDrawableObtained(const gl::Context *context,
                                                              bool *newDrawableOut)
{
    return angle::Result::Continue;
}
...
angle::Result IOSurfaceBaseMtl::createBackingTexture(const gl::Context *context)
{
...
void IOSurfaceBaseMtl::initializeWithIOSurface(DisplayMtl *display,
                                               const egl::SurfaceState &state,
                                               IOSurfaceRef ioSurface,
                                               int width,
                                               int height,
                                               int ioSurfaceFormatIndex)
{
    ...
    mFormatIndex = ioSurfaceFormatIndex;
...
angle::Result IOSurfaceBaseMtl::ensureTextureCreated(const gl::Context *context)
{
...
mtl::TextureRef IOSurfaceBaseMtl::getTexture(const gl::Context *context)
{
...
angle::Result IOSurfaceBaseMtl::createTexture(const gl::Context *context,
...
//
// IOSurfaceClientSurfaceMtl
//

IOSurfaceClientSurfaceMtl::IOSurfaceClientSurfaceMtl(DisplayMtl *display,
                                                     const egl::SurfaceState &state,
                                                     EGLClientBuffer buffer,
                                                     const egl::AttributeMap &attribs)
    : IOSurfaceBaseMtl(state)
{
    // Extract attributes specifically for the use of the client's IOSurface.
    int width  = static_cast<int>(attribs.get(EGL_WIDTH));
    int height = static_cast<int>(attribs.get(EGL_HEIGHT));
    ...
    initializeWithIOSurface(display, state, reinterpret_cast<IOSurfaceRef>(buffer), width, height,
                            formatIndex);
}

//
// IOSurfacePbufferSurfaceMtl
//

IOSurfacePbufferSurfaceMtl::IOSurfacePbufferSurfaceMtl(DisplayMtl *display,
                                                       const egl::SurfaceState &state,
                                                       const egl::AttributeMap &attribs)
    : IOSurfaceBaseMtl(state)
{
    // Only support RGBA8 pbuffer creation at the moment.
    int formatIndex = FindIOSurfaceFormatIndex(GL_BGRA_EXT, GL_UNSIGNED_BYTE);
    ASSERT(formatIndex >= 0);
    ASSERT(kIOSurfaceFormats[formatIndex].internalPixelFormat == angle::FormatID::B8G8R8A8_UNORM);

    int width  = static_cast<int>(attribs.get(EGL_WIDTH));
    int height = static_cast<int>(attribs.get(EGL_HEIGHT));

    unsigned int bytesPerElement = 4;
    unsigned int bytesPerPixel   = 4;
    unsigned int pixelFormat     = 'BGRA';
    ...
    IOSurfaceRef surface = IOSurfaceCreate(static_cast<CFDictionaryRef>(options));
    mPlane               = 0;
    initializeWithIOSurface(display, state, surface, width, height, formatIndex);
    // initializeWithIOSurface takes a reference to the IOSurface.
    CFRelease(surface);
}

egl::Error IOSurfacePbufferSurfaceMtl::makeCurrent(const gl::Context *context)
{
    egl::Error result = IOSurfaceBaseMtl::makeCurrent(context);
    if (result.isError())
        return result;
    if (ensureTextureCreated(context) != angle::Result::Continue)
        return egl::EglBadSurface() << "Could not allocate backing texture for pbuffer";
    return egl::NoError();
}
