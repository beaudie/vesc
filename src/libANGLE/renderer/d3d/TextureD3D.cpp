                                                 ...
                                                 ImageD3D **outImage) const
{
    ...
    if (mTexStorage)
    {
        ANGLE_TRY(image->copyFromTexStorage(context, index, mTexStorage.get()));
    }
    *outImage = image;
    return gl::NoError();
}
...
gl::Error TextureD3D::generateMipmap(const gl::Context *context)
{
    ...
    if (mTexStorage && mTexStorage->supportsNativeMipmapFunction())
    {
        ...
        ANGLE_TRY(mRenderer->generateMipmapUsingD3D(context, mTexStorage.get(), mState));
    }
    else
    {
    ...
    if (mRenderer->getWorkarounds().setDataFasterThanImageUpload && mTexStorage)
    {
        if (!mTexStorage->isRenderTarget())
        {
            ...
            for (GLint layer = 0; layer < layerCount; ++layer)
            {
                ...
                ANGLE_TRY(image->copyFromTexStorage(context, srcIndex, mTexStorage.get()));
            }
        }
        else
        {
...
gl::Error TextureD3D::ensureRenderTarget(const gl::Context *context)
{
    ...
    if (!isBaseImageZeroSize())
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            std::unique_ptr<TextureStorage> newRenderTargetStorage;
                                   ...
                                   const gl::Box &region)
{
    if (mTexStorage)
    {
        ...
        ImageD3D *image = getImage(index);
        ANGLE_TRY(image->copyToStorage(context, mTexStorage.get(), index, region));
        image->markClean();
    }
...
gl::Error TextureD3D_2DMultisample::createCompleteStorage(
    bool renderTarget,
    std::unique_ptr<TextureStorage> *outStorage) const
{
    UNIMPLEMENTED();
    return gl::InternalError();
}
    ...
    else
    {
        ...
        if (sourceArea.width != 0 && sourceArea.height != 0 && isValidLevel(level))
        {
            ANGLE_TRY(mRenderer->copyImage2D(context, source, sourceArea, internalFormat,
                                             destOffset, mTexStorage.get(), level));
        }
    }
                                     ...
                                     const gl::Texture *source)
{
    ...
    if (canCreateRenderTargetForImage(gl::ImageIndex::Make2D(destLevel)))
    {
        ...
        ANGLE_TRY(mRenderer->copyTexture(context, source, static_cast<GLint>(sourceLevel),
                                         ...
                                         mTexStorage.get(), target, destLevel, unpackFlipY,
                                        ...
                                        const gl::Texture *source)
{
    ...
    if (canCreateRenderTargetForImage(gl::ImageIndex::Make2D(destLevel)))
    {
        ...
        ANGLE_TRY(mRenderer->copyTexture(
            ...
            gl::GetUnsizedFormat(getInternalFormat(destLevel)), destOffset, mTexStorage.get(),
                                               ...
                                               const gl::Texture *source)
{
    ...
    ANGLE_TRY(mRenderer->copyCompressedTexture(context, source, sourceLevel, mTexStorage.get(),
                                               destLevel));
...
gl::Error TextureD3D_2D::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    mTexStorage.reset(mRenderer->createTextureStorage2D(surfaceD3D->getSwapChain()));
    mEGLImageTarget = false;
                                           ...
                                           egl::Image *image)
{
    ...
    mTexStorage.reset(mRenderer->createTextureStorageEGLImage(eglImaged3d, renderTargetD3D));
                                               ...
                                               TextureStorage *newCompleteTexStorage)
{
    ...
    mTexStorage.reset(newCompleteTexStorage);
                                       ...
                                       bool forceRelease)
{
    ...
    if (mTexStorage)
    {
        ...
        if (level != 0 && mEGLImageTarget)
        {
            ...
            mImageArray[0]->copyFromTexStorage(context, gl::ImageIndex::Make2D(0),
                                               mTexStorage.get());
        }
    ...
    else
    {
        ...
        if (size.width > 0 && isValidFaceLevel(faceIndex, level))
        {
            ANGLE_TRY(mRenderer->copyImageCube(context, source, sourceArea, internalFormat,
                                               destOffset, mTexStorage.get(), target, level));
        }
    }
                                          ...
                                          const gl::Texture *source)
{
    ...
    if (canCreateRenderTargetForImage(gl::ImageIndex::MakeCube(target, destLevel)))
    {
        ...
        ANGLE_TRY(
            mRenderer->copyTexture(context, source, static_cast<GLint>(sourceLevel), sourceArea,
                                   gl::GetUnsizedFormat(getInternalFormat(destLevel, faceIndex)),
                                   destOffset, mTexStorage.get(), target, destLevel, unpackFlipY,
                                   unpackPremultiplyAlpha, unpackUnmultiplyAlpha));
    }
    else
    {
...
gl::Error TextureD3D_2DArray::createCompleteStorage(
    bool renderTarget,
    std::unique_ptr<TextureStorage> *outStorage) const
{
    GLsizei width         = getLevelZeroWidth();
                                                ...
                                                const egl::Stream::GLTextureDescription &desc)
{
    ...
    if (stream != nullptr)
    {
        mTexStorage.reset(mRenderer->createTextureStorageExternal(stream, desc));
    }
...
gl::Error TextureD3D_External::createCompleteStorage(
    bool renderTarget,
    std::unique_ptr<TextureStorage> *outStorage) const
{
    UNREACHABLE();
