...
angle::Result TextureD3D::setBaseLevel(const gl::Context *context, GLuint baseLevel)
{
         ...
         newStorageDepth != oldStorageDepth || newStorageFormat != oldStorageFormat))
    {
        ...
        ANGLE_TRY(releaseTexStorage(context, false));
    }
...
angle::Result TextureD3D::releaseTexStorage(const gl::Context *context, bool copyStorageToImages)
{
    if (!mTexStorage)
    {
        return angle::Result::Continue;
    ...
    if (copyStorageToImages && mTexStorage->isRenderTarget())
    {
...
{
    (void)releaseTexStorage(context, false);
}
...
angle::Result TextureD3D_2D::bindTexImage(const gl::Context *context, egl::Surface *surface)
{
    ...
    ANGLE_TRY(releaseTexStorage(context, true));
