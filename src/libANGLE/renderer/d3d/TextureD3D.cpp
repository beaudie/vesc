...
gl::Error TextureD3D::getCleanImage(const gl::ImageIndex &index, ImageD3D **outImage) const
{
                                     ...
                                     const gl::Texture *source)
{
    ...
    else
    {
        ...
        ANGLE_TRY(sourceD3D->getCleanImage(sourceImageIndex, &sourceImage));

        gl::ImageIndex destImageIndex = gl::ImageIndex::Make2D(static_cast<GLint>(destLevel));
        ImageD3D *destImage           = nullptr;
        ANGLE_TRY(getCleanImage(destImageIndex, &destImage));
