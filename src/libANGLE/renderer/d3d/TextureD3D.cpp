...
gl::Error TextureD3D::releaseTexStorage(const gl::Context *context)
{
    ...
    // Keep a local RAII pointer to handle the error case of onDestroy.
    std::unique_ptr<TextureStorage> releasedStorage(mTexStorage);
    mTexStorage = nullptr;
    ANGLE_TRY(releasedStorage->onDestroy(context));
    return gl::NoError();
}
