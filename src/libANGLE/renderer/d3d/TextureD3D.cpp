...
gl::Error TextureD3D_2D::setEGLImageTarget(GLenum target, egl::Image *image)
{
    ...
    RenderTargetD3D *renderTargetD3D = eglImaged3d->getRenderTarget();

    GLenum internalformat = renderTargetD3D->getInternalFormat();
    gl::Extents size(renderTargetD3D->getWidth(), renderTargetD3D->getHeight(), 1);
    mImageArray[0]->redefine(GL_TEXTURE_2D, internalformat, size, true);
