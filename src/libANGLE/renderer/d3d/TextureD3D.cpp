...
gl::Error TextureD3D::setImage(const gl::ImageIndex &index, GLenum type, const gl::PixelUnpackState &unpack, const void *pixels)
{
...
gl::Error TextureD3D::setCompressedImage(const gl::ImageIndex &index, const gl::PixelUnpackState &unpack, const void *pixels)
{
    // We no longer need the "GLenum format" parameter to TexImage to determine what data format "pixels" contains.
    // From our image internal format we know how many channels to expect, and "type" gives the format of pixel's components.
    const uint8_t *pixelData = NULL;
                                   ...
                                   const gl::Framebuffer *source)
{
    ...
    {
        gl::Error error = mImageArray[level]->copy(gl::Extents(0, 0, 0), sourceArea, source);
        if (error.isError())
        {
            return error;
        }
    ...
    else
    {
        ...
        {
            gl::Error error = mRenderer->copyImage2D(source, sourceArea, internalFormat, 0, 0, mTexStorage, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_2D::copySubImage(GLenum target, size_t level, const gl::Extents &destOffset, const gl::Rectangle &sourceArea,
                                      const gl::Framebuffer *source)
{
    ASSERT(target == GL_TEXTURE_2D && destOffset.depth == 0);
    ...
    else
    {
        ...
        if (isValidLevel(level))
        {
            ...
            error = mRenderer->copyImage2D(source, sourceArea,
                                           ...
                                           destOffset.width, destOffset.height, mTexStorage, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_2D::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents &size)
{
    ...
    for (int level = 0; level < levels; level++)
    {
        gl::Extents levelSize(std::max(1, size.width >> level),
                              ...
                              1);
        mImageArray[level]->redefine(mRenderer, GL_TEXTURE_2D, internalFormat, levelSize, true);
    }

    for (int level = levels; level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; level++)
    {
        mImageArray[level]->redefine(mRenderer, GL_TEXTURE_2D, GL_NONE, gl::Extents(0, 0, 0), true);
    }
...
void TextureD3D_2D::bindTexImage(egl::Surface *surface)
{
    ...
    mImageArray[0]->redefine(mRenderer, GL_TEXTURE_2D, internalformat, size, true);
...
void TextureD3D_2D::releaseTexImage()
{
    ...
    for (int i = 0; i < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; i++)
    {
        mImageArray[i]->redefine(mRenderer, GL_TEXTURE_2D, GL_NONE, gl::Extents(0, 0, 0), true);
    }
}
...
void TextureD3D_2D::redefineImage(GLint level, GLenum internalformat, const gl::Extents &size)
{
    ...
    mImageArray[level]->redefine(mRenderer, GL_TEXTURE_2D, internalformat, size, false);
                                     ...
                                     const gl::Framebuffer *source)
{
    ...
    if (!canCreateRenderTargetForImage(index))
    {
        gl::Error error = mImageArray[faceIndex][level]->copy(gl::Extents(0, 0, 0), sourceArea, source);
        if (error.isError())
        {
            return error;
        }
    ...
    else
    {
        ...
        {
            error = mRenderer->copyImageCube(source, sourceArea, internalFormat, 0, 0, mTexStorage, target, level);
            if (error.isError())
            {
                return error;
            }
    ...
    else
    {
        ...
        if (isValidFaceLevel(faceIndex, level))
        {
            ...
            error = mRenderer->copyImageCube(source, sourceArea, gl::GetInternalFormatInfo(getBaseLevelInternalFormat()).format,
                                             destOffset.width, destOffset.height, mTexStorage, target, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_Cube::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents &size)
{
    ...
    for (int level = 0; level < levels; level++)
    {
        ...
        {
            mImageArray[faceIndex][level]->redefine(mRenderer, GL_TEXTURE_CUBE_MAP, internalFormat, gl::Extents(mipSize, mipSize, 1), true);
        }
    }
    ...
    for (int level = levels; level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; level++)
    {
        for (int faceIndex = 0; faceIndex < 6; faceIndex++)
        {
            mImageArray[faceIndex][level]->redefine(mRenderer, GL_TEXTURE_CUBE_MAP, GL_NONE, gl::Extents(0, 0, 0), true);
        }
    }
...
void TextureD3D_Cube::redefineImage(int faceIndex, GLint level, GLenum internalformat, const gl::Extents &size)
{
    ...
    mImageArray[faceIndex][level]->redefine(mRenderer, GL_TEXTURE_CUBE_MAP, internalformat, size, false);
...
gl::Error TextureD3D_3D::copySubImage(GLenum target, size_t level, const gl::Extents &destOffset, const gl::Rectangle &sourceArea,
                                      const gl::Framebuffer *source)
{
    ASSERT(target == GL_TEXTURE_3D);
    ...
    else
    {
        ...
        if (isValidLevel(level))
        {
            ...
            error = mRenderer->copyImage3D(source, sourceArea,
                                           gl::GetInternalFormatInfo(getBaseLevelInternalFormat()).format,
                                           destOffset.width, destOffset.height, destOffset.depth, mTexStorage, level);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureD3D_3D::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents &size)
{
    ...
    for (int level = 0; level < levels; level++)
    {
        ...
        mImageArray[level]->redefine(mRenderer, GL_TEXTURE_3D, internalFormat, levelSize, true);
    }

    for (int level = levels; level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; level++)
    {
        mImageArray[level]->redefine(mRenderer, GL_TEXTURE_3D, GL_NONE, gl::Extents(0, 0, 0), true);
    }
...
void TextureD3D_3D::redefineImage(GLint level, GLenum internalformat, const gl::Extents &size)
{
    ...
    mImageArray[level]->redefine(mRenderer, GL_TEXTURE_3D, internalformat, size, false);
...
gl::Error TextureD3D_2DArray::copySubImage(GLenum target, size_t level, const gl::Extents &destOffset, const gl::Rectangle &sourceArea,
                                           const gl::Framebuffer *source)
{
    ASSERT(target == GL_TEXTURE_2D_ARRAY);

    gl::ImageIndex index = gl::ImageIndex::Make2DArray(level, destOffset.depth);

    if (canCreateRenderTargetForImage(index))
    {
        gl::Extents destLayerOffset(destOffset.width, destOffset.height, 0);
        gl::Error error = mImageArray[level][destOffset.depth]->copy(destLayerOffset, sourceArea, source);
        if (error.isError())
        {
            return error;
        }
    ...
    for (int level = 0; level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; level++)
    {
        ...
        if (mLayerCounts[level] > 0)
        {
            ...
            for (int layer = 0; layer < mLayerCounts[level]; layer++)
            {
                ...
                mImageArray[level][layer]->redefine(mRenderer, GL_TEXTURE_2D_ARRAY, internalFormat, levelLayerSize, true);
            }
        }
    }
...
void TextureD3D_2DArray::redefineImage(GLint level, GLenum internalformat, const gl::Extents &size)
{
    ...
    if (size.depth > 0)
    {
        ...
        {
            mImageArray[level][layer] = mRenderer->createImage();
            mImageArray[level][layer]->redefine(mRenderer, GL_TEXTURE_2D_ARRAY, internalformat,
