...
{
    ASSERT(level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS);
    ASSERT(layer >= 0 && layer < gl::kCubeFaceCount);
                                      ...
                                      const gl::Extents &size)
{
    ...
    for (size_t level = 0; level < levels; level++)
    {
        ...
        for (int faceIndex = 0; faceIndex < gl::kCubeFaceCount; faceIndex++)
        {
...
bool TextureD3D_Cube::isCubeComplete() const
{
    ...
    for (int faceIndex = 1; faceIndex < gl::kCubeFaceCount; faceIndex++)
    {
                                                     ...
                                                     TexStoragePointer *outStorage) const
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround)
    {
        ...
        // Otherwise, it should use the level-zero-only texture.
        hintLevelZeroOnly = true;
        for (int faceIndex = 0; faceIndex < gl::kCubeFaceCount && hintLevelZeroOnly; faceIndex++)
        {
            for (int level = 1; level < levels && hintLevelZeroOnly; level++)
            {
...
angle::Result TextureD3D_Cube::updateStorage(const gl::Context *context)
{
    ...
    ASSERT(mTexStorage != nullptr);
    GLint storageLevels = mTexStorage->getLevelCount();
    for (int face = 0; face < gl::kCubeFaceCount; face++)
    {
        for (int level = 0; level < storageLevels; level++)
        {
...
bool TextureD3D_Cube::isFaceLevelComplete(int faceIndex, int level) const
{
    ...
    ASSERT(level >= 0 && faceIndex < gl::kCubeFaceCount &&
...
void TextureD3D_Cube::markAllImagesDirty()
{
    ...
    {
        for (int dirtyFace = 0; dirtyFace < gl::kCubeFaceCount; dirtyFace++)
        {
