...
angle::Result TextureD3D::ensureRenderTarget(const gl::Context *context)
{
    ...
    if (!isBaseImageZeroSize())
    {
        ...
        if (!mTexStorage->isRenderTarget())
        {
            ...
            ANGLE_TRY(
                createCompleteStorage(context, BindFlags::RenderTarget(), &newRenderTargetStorage));
...
angle::Result TextureD3D::ensureStorageTexture(const gl::Context *context)
{
    ANGLE_TRY(initializeStorage(context, BindFlags::StorageTexture()));
    ...
    {
        ASSERT(mTexStorage);
        if (!mTexStorage->isStorageTexture())
        {
            TexStoragePointer newRenderTargetStorage;
            ANGLE_TRY(createCompleteStorage(context, BindFlags::StorageTexture(),
