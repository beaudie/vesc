...
{
    if (!mRenderer->getWorkarounds().setDataFasterThanImageUpload.applied)
    {
        return false;
    }
...
angle::Result TextureD3D::generateMipmap(const gl::Context *context)
{
    ...
    if (mTexStorage && mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied)
    {
                                                    ...
                                                    const GLuint maxLevel)
{
    ...
    // an image layer.
    if (mRenderer->getWorkarounds().setDataFasterThanImageUpload.applied && mTexStorage)
    {
    ...
    bool renderableStorage = (mTexStorage && mTexStorage->isRenderTarget() &&
                              !(mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied));
                                                   ...
                                                   TexStoragePointer *outStorage) const
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied)
    {
