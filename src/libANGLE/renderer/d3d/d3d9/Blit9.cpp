...
gl::Error Blit9::boxFilter(IDirect3DSurface9 *source, IDirect3DSurface9 *dest)
{
    ...
    setViewport(getSurfaceRect(dest), getSurfaceSize(source), gl::Offset(0, 0, 0));
                               ...
                               bool unmultiplyAlpha)
{
    ...
    setViewport(sourceRect, sourceSize, destOffset);
...
gl::Error Blit9::setFormatConvertShaders(GLenum destFormat,
                                         ...
                                         bool unmultiplyAlpha)
{
    ANGLE_TRY(setVertexShader(flipY ? SHADER_VS_FLIPY : SHADER_VS_STANDARD));
...
void Blit9::setViewport(const RECT &sourceRect,
                        const gl::Extents &sourceSize,
                        const gl::Offset &offset)
{
    ...
    float vertexConstants[8] = {
        ...
        static_cast<float>(sourceRect.top) / sourceSize.height,
        static_cast<float>(sourceRect.right - sourceRect.left) / sourceSize.width,
        static_cast<float>(sourceRect.bottom - sourceRect.top) / sourceSize.height,
    };
