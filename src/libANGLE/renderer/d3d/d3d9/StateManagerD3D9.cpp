...
gl::Error StateManagerD3D9::setRasterizerState(const gl::RasterizerState &rasterizerState,
                                               const gl::State::DirtyBits &dirtyBits)
{
    if (dirtyBits.test(gl::State::DIRTY_BIT_CULL_FACE_ENABLED) ||
        dirtyBits.test(gl::State::DIRTY_BIT_FRONT_FACE) ||
        dirtyBits.test(gl::State::DIRTY_BIT_CULL_FACE) || mForceSetRasterizerState)
    {
        setRasterizerMode(rasterizerState.cullFace, rasterizerState.cullMode,
                          rasterizerState.frontFace);
    }

    if (dirtyBits.test(gl::State::DIRTY_BIT_POLYGON_OFFSET_FILL_ENABLED) ||
        dirtyBits.test(gl::State::DIRTY_BIT_POLYGON_OFFSET) || mForceSetRasterizerState)
    {
...
void StateManagerD3D9::setRasterizerMode(bool cullFace, GLenum cullMode, GLenum frontFace)
{
    if (mRasterizerState.cullFace != cullFace || mRasterizerState.cullMode != cullMode ||
        mRasterizerState.frontFace != frontFace)
    {
        mDevice->SetRenderState(D3DRS_CULLMODE, cullFace
                                                    ? gl_d3d9::ConvertCullMode(cullMode, frontFace)
                                                    : D3DCULL_NONE);

        mRasterizerState.cullFace  = cullFace;
        mRasterizerState.cullMode  = cullMode;
        mRasterizerState.frontFace = frontFace;
    }
}

void StateManagerD3D9::setRasterizerPolygonOffset(bool polygonOffsetFill,
                                                  GLfloat polygonOffsetFactor,
                                                  GLfloat polygonOffsetUnits)
{
    if (mRasterizerState.polygonOffsetFill != polygonOffsetFill ||
        mRasterizerState.polygonOffsetFactor != polygonOffsetFactor ||
        mRasterizerState.polygonOffsetUnits != polygonOffsetUnits)
    {
        ...
        mRasterizerState.polygonOffsetFill   = polygonOffsetFill;
        mRasterizerState.polygonOffsetFactor = polygonOffsetFactor;
        mRasterizerState.polygonOffsetUnits  = polygonOffsetUnits;
    }
}
