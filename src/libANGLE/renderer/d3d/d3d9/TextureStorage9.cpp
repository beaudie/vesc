...
gl::Error TextureStorage9_2D::getSurfaceLevel(GLenum target, int level, bool dirty, IDirect3DSurface9 **outSurface)
{
...
gl::Error TextureStorage9_2D::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (!mRenderTargets[index.mipIndex] && isRenderTarget())
    {
        ...
        baseTexture->AddRef();
        mRenderTargets[index.mipIndex] = new TextureRenderTarget9(baseTexture, textureMipLevel, surface,
                                                                  mInternalFormat, mipWidth, mipHeight, 1, 0);
    }
      ...
      mImage(image)
{
    ...
    mTopLevel = renderTarget9->getTextureLevel();
...
gl::Error TextureStorage9_EGLImage::getSurfaceLevel(GLenum target, int level, bool dirty, IDirect3DSurface9 **outSurface)
{
    ASSERT(target == GL_TEXTURE_2D);
    ASSERT(level == 0);
...
gl::Error TextureStorage9_EGLImage::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
...
gl::Error TextureStorage9_EGLImage::generateMipmap(const gl::ImageIndex &sourceIndex, const gl::ImageIndex &destIndex)
{
    UNREACHABLE();
    return gl::Error(GL_INVALID_OPERATION);
}
...
// Increments refcount on surface.
// caller must Release() the returned surface
gl::Error TextureStorage9_Cube::getSurfaceLevel(GLenum target, int level, bool dirty, IDirect3DSurface9 **outSurface)
{
    IDirect3DBaseTexture9 *baseTexture = NULL;
    gl::Error error = getBaseTexture(&baseTexture);
    if (error.isError())
...
gl::Error TextureStorage9_Cube::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (mRenderTarget[index.layerIndex] == NULL && isRenderTarget())
    {
        ...
        IDirect3DSurface9 *surface = NULL;
        error = getSurfaceLevel(GL_TEXTURE_CUBE_MAP_POSITIVE_X + index.layerIndex, mTopLevel + index.mipIndex,
                                false, &surface);
        if (error.isError())
        {
            return error;
        ...
        baseTexture->AddRef();
        mRenderTarget[index.layerIndex] = new TextureRenderTarget9(baseTexture, mTopLevel + index.mipIndex,
                                                                   surface, mInternalFormat, mTextureWidth,
                                                                   mTextureHeight, 1, 0);
    }
