...
gl::Error TextureStorage9_2D::getRenderTarget(const gl::ImageIndex &/*index*/, RenderTargetD3D **outRT)
{
    if (!mRenderTarget && isRenderTarget())
    {
        ...
        error = getSurfaceLevel(GL_TEXTURE_2D, 0, false, &surface);
        if (error.isError())
        {
            return error;
        }
        ...
        baseTexture->AddRef();
        mRenderTarget = new TextureRenderTarget9(baseTexture, surface, mInternalFormat, mTextureWidth, mTextureHeight, 1, 0);
    }
...
gl::Error TextureStorage9_Cube::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (mRenderTarget[index.layerIndex] == NULL && isRenderTarget())
    {
        ...
        IDirect3DSurface9 *surface = NULL;
        error = getSurfaceLevel(GL_TEXTURE_CUBE_MAP_POSITIVE_X + index.layerIndex, 0, false, &surface);
        if (error.isError())
        {
            return error;
        }
        ...
        baseTexture->AddRef();
        mRenderTarget[index.layerIndex] = new TextureRenderTarget9(baseTexture, surface, mInternalFormat, mTextureWidth, mTextureHeight, 1, 0);
    }
