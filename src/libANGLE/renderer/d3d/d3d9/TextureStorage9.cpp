...
gl::Error TextureStorage9_2D::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (!mRenderTargets[index.mipIndex] && isRenderTarget())
    {
        ...
        mRenderTargets[index.mipIndex] = new TextureRenderTarget9(
            baseTexture, textureMipLevel, surface, mInternalFormat, static_cast<GLsizei>(mipWidth), static_cast<GLsizei>(mipHeight), 1, 0);
    }
...
gl::Error TextureStorage9_EGLImage::generateMipmap(const gl::ImageIndex &sourceIndex,
...
gl::Error TextureStorage9_Cube::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (mRenderTarget[index.layerIndex] == NULL && isRenderTarget())
    {
        ...
        mRenderTarget[index.layerIndex] =
            new TextureRenderTarget9(baseTexture, mTopLevel + index.mipIndex, surface,
            mInternalFormat, static_cast<GLsizei>(mTextureWidth), static_cast<GLsizei>(mTextureHeight), 1, 0);
    }
