            ...
            switch (getD3DFormat())
            {
                ...
                case D3DFMT_A8R8G8B8:
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            float r = gl::float16ToFloat32(((uint16_t *)sourcePixels)[x * 4 + 0]);
                            float g = gl::float16ToFloat32(((uint16_t *)sourcePixels)[x * 4 + 1]);
                            float b = gl::float16ToFloat32(((uint16_t *)sourcePixels)[x * 4 + 2]);
                            float a = gl::float16ToFloat32(((uint16_t *)sourcePixels)[x * 4 + 3]);
                ...
                case D3DFMT_L8:
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            float r       = gl::float16ToFloat32(((uint16_t *)sourcePixels)[x * 4]);
            ...
            switch (getD3DFormat())
            {
                ...
                case D3DFMT_A8R8G8B8:
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            float r               = ((float *)sourcePixels)[x * 4 + 0];
                            float g               = ((float *)sourcePixels)[x * 4 + 1];
                            float b               = ((float *)sourcePixels)[x * 4 + 2];
                            float a               = ((float *)sourcePixels)[x * 4 + 3];
                            destPixels[x * 4 + 0] = gl::floatToNormalized<uint8_t>(b);
                            destPixels[x * 4 + 1] = gl::floatToNormalized<uint8_t>(g);
                            destPixels[x * 4 + 2] = gl::floatToNormalized<uint8_t>(r);
                            destPixels[x * 4 + 3] = gl::floatToNormalized<uint8_t>(a);
                ...
                case D3DFMT_L8:
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            float r       = ((float *)sourcePixels)[x * 4];
                            destPixels[x] = gl::floatToNormalized<uint8_t>(r);
                        }
                        sourcePixels += sourceLock.Pitch;
                        destPixels += destLock.Pitch;
