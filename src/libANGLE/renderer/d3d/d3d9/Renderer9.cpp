                           ...
                           const gl::FramebufferAttachment *depthStencilBuffer)
{
    ...
    if (clearColor)
    {
        ...
        const gl::Format &format             = colorBuffer->getFormat();
        const d3d9::D3DFormat &d3dFormatInfo = d3d9::GetD3DFormatInfo(colorRenderTarget9->getD3DFormat());

        color =
            D3DCOLOR_ARGB(gl::unorm<8>((format.info->alphaBits == 0 && d3dFormatInfo.alphaBits > 0)
                                           ? 1.0f
                                           : clearParams.colorFClearValue.alpha),
                          gl::unorm<8>((format.info->redBits == 0 && d3dFormatInfo.redBits > 0)
                                           ? 0.0f
                                           : clearParams.colorFClearValue.red),
                          gl::unorm<8>((format.info->greenBits == 0 && d3dFormatInfo.greenBits > 0)
                                           ? 0.0f
                                           : clearParams.colorFClearValue.green),
                          gl::unorm<8>((format.info->blueBits == 0 && d3dFormatInfo.blueBits > 0)
                                           ? 0.0f
