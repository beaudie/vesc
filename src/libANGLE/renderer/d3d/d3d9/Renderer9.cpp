...
{
    std::vector<CurSamplerState> &appliedSamplers = (type == gl::SAMPLER_PIXEL) ? mCurPixelSamplerStates: mCurVertexSamplerStates;
    ...
    if (appliedSamplers[index].forceSet || appliedSamplers[index].baseLevel != baseLevel ||
        memcmp(&samplerState, &appliedSamplers[index], sizeof(gl::SamplerState)) != 0)
    {
    ...
    appliedSamplers[index].forceSet = false;
    appliedSamplers[index].samplerState = samplerState;
    appliedSamplers[index].baseLevel = baseLevel;
...
gl::Error Renderer9::createRenderTarget(int width, int height, GLenum format, GLsizei samples, RenderTargetD3D **outRT)
{
        ...
        else
        {
            ...
            else
            {
                result = mDevice->CreateTexture(width, height, 1, D3DUSAGE_RENDERTARGET,
                                                d3d9FormatInfo.texFormat, D3DPOOL_DEFAULT,
                                                &texture, nullptr);
                if (!FAILED(result))
                {
    ...
    *outRT = new TextureRenderTarget9(texture, 0, renderTarget, format, width, height, 1, supportedSamples);
    return gl::Error(GL_NO_ERROR);
}
...
gl::Error Renderer9::createRenderTargetCopy(RenderTargetD3D *source, RenderTargetD3D **outRT)
{
    ...
    HRESULT result = mDevice->StretchRect(source9->getSurface(), nullptr, dest9->getSurface(), nullptr, D3DTEXF_NONE);
    if (FAILED(result))
    {
        ASSERT(result == D3DERR_OUTOFVIDEOMEMORY || result == E_OUTOFMEMORY);
