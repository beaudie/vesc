...
bool RendererD3D::shouldClearDirtyBitsAfterSync() const
{
    return false;
}
...
gl::Error RendererD3D::applyState(const gl::Data &data, GLenum drawMode)
{
    ...
    error = mStateManager->setDepthStencilState(
        data.state->getDepthStencilState(), data.state->getStencilRef(),
        data.state->getStencilBackRef(), data.state->getRasterizerState().frontFace == GL_CCW,
        data.state->getDirtyBits());

    mStateManager->syncState(data, data.state->getDirtyBits());
