                                        ...
                                        const VaryingPacking &packing)
{
    ...
    {
        // We have to pass gl_ViewID_OVR separately because we cannot easily rely to retrieve the
        // information from SV_ViewportArrayIndex or SV_RenderTargetArrayIndex. We would need access
        // to the constant buffer for that.
        builtins->glViewIDOVR.enable(userSemantic, reservedSemanticIndex++);

        // gl_Layer and gl_ViewportIndex are necessary so that we can select either in the geometry
        // stage.
