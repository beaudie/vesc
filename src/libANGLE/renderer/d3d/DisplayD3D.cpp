...
egl::Error DisplayD3D::restoreLostDevice(const egl::Thread *thread)
{
    ...
    for (auto &surface : mState.surfaceSet)
    {
        ...
        {
            ANGLE_TRY(surface->releaseTexImage(thread->getContext(), EGL_BACK_BUFFER));
        }
    ...
    for (const auto &surface : mState.surfaceSet)
    {
        ...
        ANGLE_TRY(surfaceD3D->resetSwapChain(thread));
    }
...
egl::Error DisplayD3D::waitClient(const egl::Thread *thread) const
{
    for (auto &surface : mState.surfaceSet)
    {
        SurfaceD3D *surfaceD3D = GetImplAs<SurfaceD3D>(surface);
        surfaceD3D->checkForOutOfDateSwapChain(thread);
    }

    return egl::NoError();
}

egl::Error DisplayD3D::waitNative(const egl::Thread *thread, EGLint engine) const
{
    egl::Surface *drawSurface = thread->getDrawSurface();
    egl::Surface *readSurface = thread->getReadSurface();

    if (drawSurface != nullptr)
    {
        SurfaceD3D *drawSurfaceD3D = GetImplAs<SurfaceD3D>(drawSurface);
        drawSurfaceD3D->checkForOutOfDateSwapChain(thread);
    }

    if (readSurface != nullptr)
    {
        SurfaceD3D *readurfaceD3D = GetImplAs<SurfaceD3D>(readSurface);
        readurfaceD3D->checkForOutOfDateSwapChain(thread);
    }
