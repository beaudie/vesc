...
// ETCDecoder.cpp: Decodes ETC and EAC encoded textures.
...
signed char ClampSByte(int value)
{
...
int Extend_4to8bits(int x)
{
    return (x << 4) | x;
}

int Extend_5to8bits(int x)
{
    return (x << 3) | (x >> 2);
}

int Extend_6to8bits(int x)
{
    return (x << 2) | (x >> 4);
}

int Extend_7to8bits(int x)
{
    return (x << 1) | (x >> 6);
}
...
struct ETC2
{
    ...
    static void DecodeBlock(const ETC2 **sources,
                            unsigned char *dest,
                            int nbChannels,
                            int x,
                            int y,
        ...
        if (isSigned)
        {
            ...
            for (int j = 0; j < 4 && (y + j) < h; j++)
            {
                for (int i = 0; i < 4 && (x + i) < w; i++)
                {
                    ...
                    {
                        sDst[i * nbChannels + c] =
                            ClampSByte(sources[c]->getSingleChannel(i, j, isSigned));
                    }
                }
                sDst += pitch;
            }
        ...
        else
        {
            for (int j = 0; j < 4 && (y + j) < h; j++)
            {
                for (int i = 0; i < 4 && (x + i) < w; i++)
                {
                    ...
                    {
                        dest[i * nbChannels + c] =
                            ClampByte(sources[c]->getSingleChannel(i, j, isSigned));
                    }
                }
                dest += pitch;
            }
    ...
    void decodeBlock(unsigned char *dest,
                     int x,
                     int y,
                     int w,
                     int h,
                     ...
                     bool punchThroughAlpha) const
    {
        ...
        if (u.idht.mode.idm.diffbit || punchThroughAlpha)
        {
            ...
            {
                decodeDifferentialBlock(dest, x, y, w, h, pitch, alphaValues,
  ...
  private:
    ...
    void decodeIndividualBlock(unsigned char *dest,
                               int x,
                               int y,
                               int w,
                               int h,
                               ...
                               bool nonOpaquePunchThroughAlpha) const
    {
        ...
        decodeIndividualOrDifferentialBlock(dest, x, y, w, h, pitch, r1, g1, b1, r2, g2, b2,
                                            alphaValues, nonOpaquePunchThroughAlpha);
    }

    void decodeDifferentialBlock(unsigned char *dest,
                                 int x,
                                 int y,
                                 int w,
                                 int h,
    ...
    void decodeIndividualOrDifferentialBlock(unsigned char *dest,
                                             int x,
                                             int y,
                                             int w,
                                             int h,
                                             ...
                                             bool nonOpaquePunchThroughAlpha) const
    {
        ...
        static const int intensityModifierNonOpaque[8][4] = {
            {0, 8, 0, -8},   {0, 17, 0, -17}, {0, 29, 0, -29},   {0, 42, 0, -42},
            {0, 60, 0, -60}, {0, 80, 0, -80}, {0, 106, 0, -106}, {0, 183, 0, -183}};
        ...
        const int i10 = intensityModifier[u.idht.mode.idm.cw1][0];
        const int i11 = intensityModifier[u.idht.mode.idm.cw1][1];
        const int i12 = intensityModifier[u.idht.mode.idm.cw1][2];
        const int i13 = intensityModifier[u.idht.mode.idm.cw1][3];
        ...
        const int i20 = intensityModifier[u.idht.mode.idm.cw2][0];
        const int i21 = intensityModifier[u.idht.mode.idm.cw2][1];
        const int i22 = intensityModifier[u.idht.mode.idm.cw2][2];
        const int i23 = intensityModifier[u.idht.mode.idm.cw2][3];
        ...
        if (u.idht.mode.idm.flipbit)
        {
            for (int j = 0; j < 2 && (y + j) < h; j++)
            {
                ...
                if ((x + 0) < w)
                    color[0] = subblockColors0[getIndex(0, j)].addA(alphaValues[j][0]);
                if ((x + 1) < w)
                    color[1] = subblockColors0[getIndex(1, j)].addA(alphaValues[j][1]);
                if ((x + 2) < w)
                    color[2] = subblockColors0[getIndex(2, j)].addA(alphaValues[j][2]);
                if ((x + 3) < w)
                    color[3] = subblockColors0[getIndex(3, j)].addA(alphaValues[j][3]);
                dest += pitch;
            }
            for (int j = 2; j < 4 && (y + j) < h; j++)
            {
                bgra8 *color = (bgra8 *)dest;
                if ((x + 0) < w)
                    color[0] = subblockColors1[getIndex(0, j)].addA(alphaValues[j][0]);
                if ((x + 1) < w)
                    color[1] = subblockColors1[getIndex(1, j)].addA(alphaValues[j][1]);
                if ((x + 2) < w)
                    color[2] = subblockColors1[getIndex(2, j)].addA(alphaValues[j][2]);
                if ((x + 3) < w)
                    color[3] = subblockColors1[getIndex(3, j)].addA(alphaValues[j][3]);
                dest += pitch;
            }
        }
        else
    ...
    void decodeTBlock(unsigned char *dest,
                      int x,
                      int y,
                      int w,
                      int h,
                      ...
                      bool nonOpaquePunchThroughAlpha) const
    {
        ...
        for (int j = 0; j < 4 && (y + j) < h; j++)
        {
            bgra8 *color = (bgra8 *)dest;
            if ((x + 0) < w)
                color[0] = paintColors[getIndex(0, j)].addA(alphaValues[j][0]);
            if ((x + 1) < w)
                color[1] = paintColors[getIndex(1, j)].addA(alphaValues[j][1]);
            if ((x + 2) < w)
                color[2] = paintColors[getIndex(2, j)].addA(alphaValues[j][2]);
            if ((x + 3) < w)
                color[3] = paintColors[getIndex(3, j)].addA(alphaValues[j][3]);
            dest += pitch;
        }
        if (nonOpaquePunchThroughAlpha)
        {
    ...
    void decodeHBlock(unsigned char *dest,
                      int x,
                      int y,
                      int w,
                      int h,
    ...
    void decodePlanarBlock(unsigned char *dest,
                           int x,
                           int y,
                           int w,
                           int h,
                           int pitch,
                           unsigned char alphaValues[4][4]) const
    {
        ...
        for (int j = 0; j < 4 && (y + j) < h; j++)
        {
            ...
            for (int i = 0; i < 4 && (x + i) < w; i++)
            {
                ((bgra8 *)(dest))[i].set(((i * (rh - ro) + ry) >> 2) + ro,
                                         ((i * (gh - go) + gy) >> 2) + go,
                                         ((i * (bh - bo) + by) >> 2) + bo, alphaValues[j][i]);
            }
            dest += pitch;
        }
    }
    ...
    int getIndex(int x, int y) const
    {
        ...
        int lsb       = (u.idht.pixelIndexLSB[1 - (bitIndex >> 3)] >> bitOffset) & 1;
        int msb = (u.idht.pixelIndexMSB[1 - (bitIndex >> 3)] >> bitOffset) & 1;
        return (msb << 1) | lsb;
    }

    void decodePunchThroughAlphaBlock(unsigned char *dest, int x, int y, int w, int h, int pitch)
        const
    {
        for (int j = 0; j < 4 && (y + j) < h; j++)
        {
    ...
    int getSingleChannel(int x, int y, bool isSigned) const
    {
    ...
    int getSingleChannelIndex(int x, int y) const
    {
        switch (x * 4 + y)
        {
            case 0:
            ...
            case 1:
                return u.scblk.mb;
            case 2:
                return u.scblk.mc1 << 1 | u.scblk.mc2;
            case 3:
                return u.scblk.md;
            case 4:
                return u.scblk.me;
            case 5:
                return u.scblk.mf1 << 2 | u.scblk.mf2;
            case 6:
                return u.scblk.mg;
            case 7:
                return u.scblk.mh;
            ...
            case 9:
                return u.scblk.mj;
            case 10:
                return u.scblk.mk1 << 1 | u.scblk.mk2;
            case 11:
                return u.scblk.ml;
            case 12:
                return u.scblk.mm;
            case 13:
                return u.scblk.mn1 << 2 | u.scblk.mn2;
            case 14:
                return u.scblk.mo;
            default:
                return u.scblk.mp;  // 15
        }
    }

    int getSingleChannelModifier(int x, int y) const
    {
