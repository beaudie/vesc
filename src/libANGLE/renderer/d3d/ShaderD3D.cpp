...
class WaitableCompileEventD3D final : public WaitableCompileEvent
{
  public:
    ...
    bool postTranslate(std::string *infoLog,
                       UpdateShaderStateFunctor &&UpdateShaderStateFunctor) override
    {
        bool compiled = mTranslateTask->getResult();
        UpdateShaderStateFunctor(compiled, mCompilerInstance->getHandle());
        return compiled && mPostTranslateFunctor(mCompilerInstance, infoLog);
    }
