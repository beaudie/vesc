...
struct D3DUniformBlock
{
    ...
    bool activeInProgram() const
    {
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            if (activeInShader(shaderType))
            {
                return true;
            }
        }
...
class ProgramD3D : public ProgramImpl
{
  public:
    ...
    UniformStorageD3D &getVertexUniformStorage() const
    {
        return *mShaderUniformStorages[gl::ShaderType::Vertex].get();
    }
    UniformStorageD3D &getFragmentUniformStorage() const
    {
        return *mShaderUniformStorages[gl::ShaderType::Fragment].get();
    }
    UniformStorageD3D &getComputeUniformStorage() const
    {
        return *mShaderUniformStorages[gl::ShaderType::Geometry].get();
    }
  ...
  private:
    ...
    void assignSamplerRegistersPerShader(const ShaderD3D *shaderD3D,
    ...
    gl::ShaderMap<std::string> mShaderHLSLs;
    ...
    gl::ShaderMap<std::vector<GLint>> mShaderUBOCaches;
