      ...
      mSerial(issueSerial())
{
    ...
    mGeometryExecutables.resize(gl::PRIMITIVE_TYPE_MAX);
    for (size_t geometryExecutableIndex = 0; geometryExecutableIndex < gl::PRIMITIVE_TYPE_MAX;
         geometryExecutableIndex++)
    {
        mGeometryExecutables[geometryExecutableIndex].reset(nullptr);
    }
}
                            ...
                            gl::BinaryInputStream *stream)
{
         ...
         ++geometryExeIndex)
    {
        ...
        {
            mGeometryExecutables[geometryExeIndex].reset(nullptr);
            continue;
        }
...
gl::Error ProgramD3D::save(gl::BinaryOutputStream *stream)
{
    ...
    for (size_t geometryExecutableIndex = 0; geometryExecutableIndex < mGeometryExecutables.size();
         geometryExecutableIndex++)
    {
        ShaderExecutableD3D *geometryExe = mGeometryExecutables[geometryExecutableIndex].get();
...
LinkResult ProgramD3D::compileComputeExecutable(const gl::ContextState &data,
                                                ...
                                                gl::InfoLog &infoLog)
{
    ...
    std::string computeShader =
        mDynamicHLSL->generateComputeShaderLinkHLSL(mState, programMetadata);
                            ...
                            gl::InfoLog &infoLog)
{
    ...
    if (computeShader)
    {
        ...
        LinkResult result = compileComputeExecutable(data, metadata, infoLog);
        if (result.isError())
        {
