...
ProgramD3D::UniformBlockInfoGathererD3D::UniformBlockInfoGathererD3D()
{
}
...
bool ProgramD3D::UniformBlockInfoGathererD3D::getBlockSize(const std::string &blockName,
...
bool ProgramD3D::UniformBlockInfoGathererD3D::getBlockMemberInfo(
    const std::string &memberUniformName,
...
void ProgramD3D::UniformBlockInfoGathererD3D::initFromData(const gl::Program::Data &data)
{
    const gl::Shader *vertexShader = data.getAttachedVertexShader();
    ...
    for (const sh::InterfaceBlock &vertexBlock : vertexShader->getInterfaceBlocks())
    {
        ...
        size_t dataSize                   = getBlockInfo(vertexBlock);
        mBlockDataSizes[vertexBlock.name] = dataSize;
    }

    const gl::Shader *fragmentShader = data.getAttachedFragmentShader();
    ...
    for (const sh::InterfaceBlock &fragmentBlock : fragmentShader->getInterfaceBlocks())
    {
        ...
        size_t dataSize                     = getBlockInfo(fragmentBlock);
        mBlockDataSizes[fragmentBlock.name] = dataSize;
    }
}

size_t ProgramD3D::UniformBlockInfoGathererD3D::getBlockInfo(
...
LinkResult ProgramD3D::link(const gl::Data &data, gl::InfoLog &infoLog)
{
    ...
    mUniformBlockInfoGatherer.initFromData(mData);
...
const ProgramImpl::UniformBlockInfoGatherer &ProgramD3D::getUniformBlockInfoGatherer()
{
    return mUniformBlockInfoGatherer;
}
