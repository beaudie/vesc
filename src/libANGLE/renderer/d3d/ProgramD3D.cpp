                                ...
                                gl::BinaryInputStream *stream)
{
    ...
    for (unsigned int uniformIndex = 0; uniformIndex < uniformCount; uniformIndex++)
    {
        ...
        D3DUniform *d3dUniform =
            new D3DUniform(linkedUniform.type, RegisterType::None, linkedUniform.name,
                                   ...
                                   D3DUniformMap *uniformMap)
{
    ...
    if (gl::IsSamplerType(uniform.type))
    {
        defineUniform(shader->getType(), uniform, uniform.name,
                      RegisterType::TextureAndTextureBuffer, nullptr, uniformMap);
        return;
    }
    else if (gl::IsImageType(uniform.type))
    {
        ...
        {
            defineUniform(shader->getType(), uniform, uniform.name,
                          RegisterType::UnorderedAccessView, nullptr, uniformMap);
        }
    ...
    {
        defineUniform(shader->getType(), uniform, uniform.name, RegisterType::None, nullptr,
                      uniformMap);
        return;
    }
    ...
    defineUniform(shader->getType(), uniform, uniform.name, RegisterType::None, &encoder,
                  uniformMap);
}
...
void ProgramD3D::assignImageRegisters(size_t uniformIndex)
{
    ...
    if (computeShader)
    {
        ...
        if (d3dUniform->regType == RegisterType::TextureAndTextureBuffer)
        {
        ...
        else if (d3dUniform->regType == RegisterType::UnorderedAccessView)
        {
            AssignImages(d3dUniform->csRegisterIndex, bindingIter->second,
