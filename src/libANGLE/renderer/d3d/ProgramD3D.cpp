...
void GetDefaultImage2DBindLayoutFromComputeShader(const std::vector<sh::Uniform> &image2DUniforms,
                                                  gl::ImageUnitTextureTypeMap *image2DBoundLayout)
{
    image2DBoundLayout->clear();

    for (const auto &image2D : image2DUniforms)
    {
        if (gl::IsImage2DType(image2D.type))
        {
            if (image2D.binding == -1)
            {
                image2DBoundLayout->insert(std::make_pair(0, gl::TextureType::_2D));
            }
            else
            {
...
void ProgramD3D::save(const gl::Context *context, gl::BinaryOutputStream *stream)
{
    ...
    for (auto &image2DBoundLayout : mComputeShaderImage2DBindLayoutCache)
    {
