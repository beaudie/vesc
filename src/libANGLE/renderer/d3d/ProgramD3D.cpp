...
void GetPixelOutputLayoutFromFramebuffer(const gl::Framebuffer *framebuffer,
                                         std::vector<GLenum> *signature)
{
    signature->clear();

    const FramebufferD3D *fboD3D           = GetImplAs<FramebufferD3D>(framebuffer);
...
void ProgramD3D::initAttribLocationsToD3DSemantic(const gl::Context *context)
{
    ...
    for (const sh::Attribute &attribute : vertexShader->getActiveAttributes(context))
    {
        ...
        {
            mAttribLocationToD3DSemantic[attribute.location + reg] = semanticIndex++;
        }
    }
}
...
void ProgramD3D::updateCachedOutputLayout(const gl::Framebuffer *framebuffer)
{
    GetPixelOutputLayoutFromFramebuffer(framebuffer, &mPixelShaderOutputLayoutCache);
}
