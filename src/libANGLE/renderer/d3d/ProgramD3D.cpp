...
class BlockEncodingVisitorD3D : public sh::VariableNameBlockVisitor
{
  public:
    BlockEncodingVisitorD3D(gl::ShaderType shaderType,
                            HLSLRegisterType registerType,
                            D3DUniformMap *uniformMapOut)
        : VariableNameBlockVisitor(""),
    ...
    void visitVariable(const sh::ShaderVariable &variable,
                       const sh::BlockMemberInfo &variableInfo) override
    {
        std::string fullName = collapseNameStack(variable.name);

        auto uniformMapEntry   = mUniformMapOut->find(fullName);
        ...
        {
            d3dUniform =
                new D3DUniform(variable.type, mRegisterType, fullName, variable.arraySizes, true);
            (*mUniformMapOut)[fullName] = d3dUniform;
        }
                                   ...
                                   D3DUniformMap *uniformMap)
{
    ...
    {
        BlockEncodingVisitorD3D visitor(shader->getType(), HLSLRegisterType::Texture, uniformMap);
        sh::EncodeShaderVariable(uniform, false, &dummyEncoder, &visitor);
        return;
    }
    ...
    if (gl::IsImageType(uniform.type))
    {
        if (uniform.readonly)
        {
            BlockEncodingVisitorD3D visitor(shader->getType(), HLSLRegisterType::Texture,
                                            uniformMap);
            sh::EncodeShaderVariable(uniform, false, &dummyEncoder, &visitor);
        }
        else
        {
            BlockEncodingVisitorD3D visitor(shader->getType(),
                                            HLSLRegisterType::UnorderedAccessView, uniformMap);
            sh::EncodeShaderVariable(uniform, false, &dummyEncoder, &visitor);
        }
    ...
    {
        BlockEncodingVisitorD3D visitor(shader->getType(), HLSLRegisterType::None, uniformMap);
        sh::EncodeShaderVariable(uniform, false, &dummyEncoder, &visitor);
        return;
    }
    ...
    BlockEncodingVisitorD3D visitor(shader->getType(), HLSLRegisterType::None, uniformMap);
    sh::EncodeShaderVariable(uniform, false, &encoder, &visitor);
}
