...
{
    std::vector<gl::PackedVarying> &vertexVaryings   = vertexShaderD3D->getVaryings();
    std::vector<gl::PackedVarying> &fragmentVaryings = fragmentShaderD3D->getVaryings();

    for (size_t fragVaryingIndex = 0; fragVaryingIndex < fragmentVaryings.size();
         fragVaryingIndex++)
    {
        gl::PackedVarying *input = &fragmentVaryings[fragVaryingIndex];
        bool matched             = false;
        ...
        for (size_t vertVaryingIndex = 0; vertVaryingIndex < vertexVaryings.size();
             vertVaryingIndex++)
        {
            gl::PackedVarying *output = &vertexVaryings[vertVaryingIndex];
