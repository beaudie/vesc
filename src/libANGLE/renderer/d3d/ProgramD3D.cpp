...
void ProgramD3D::initializeShaderStorageBlocks()
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        for (const sh::InterfaceBlock &ssbo : shader->getShaderStorageBlocks())
        {
            ...
            D3DShaderStorageBlock block;
            block.mName      = !ssbo.instanceName.empty() ? ssbo.instanceName : ssbo.name;
            block.mArraySize = ssbo.isArray() ? ssbo.arraySize : 0;
            block.mShaderRegisterIndexes[shaderType] =
