  ...
  public:
    GetExecutableTask(ProgramD3D *program)
        : mProgram(program),
          ...
          mInfoLog(),
          mExecutable(nullptr),
          mDeviceWasLost(false),
          mStoredErrors(nullptr)
    {
    }
    ...
    {
        HandleD3DError(hr, message, file, function, line, &mStoredErrors, &mDeviceWasLost);
    }

    bool deviceWasLost() const { return mDeviceWasLost; }
    gl::ErrorSet *getStoredErrors() { return &mStoredErrors; }
...
class ProgramD3D::GraphicsProgramLinkEvent final : public LinkEvent
{
  ...
  private:
    angle::Result checkTask(const gl::Context *context, ProgramD3D::GetExecutableTask *task)
    {
        ...
        if (task->deviceWasLost())
        {
            ContextD3D *contextD3D = GetImplAs<ContextD3D>(context);
            contextD3D->getRendererD3D()->notifyDeviceLost();
        }

        context->handleError(gl::Error(task->getStoredErrors()->popError()));
