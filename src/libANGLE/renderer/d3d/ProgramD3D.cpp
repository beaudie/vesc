...
{
    ASSERT(context);
    ASSERT(shaders[gl::ShaderType::Vertex] && shaders[gl::ShaderType::Fragment]);

    // TODO(jiawei.shao@intel.com): find flat interplation varyings when a geometry shader is
    // attached. Tracking bug: Tracking bug: http://anglebug.com/1941
    return HasFlatInterpolationVarying(
               shaders[gl::ShaderType::Vertex]->getOutputVaryings(context)) ||
           HasFlatInterpolationVarying(
               shaders[gl::ShaderType::Fragment]->getInputVaryings(context));
}
...
bool D3DUniform::hasNonNullData() const
{
...
ProgramD3DMetadata::ProgramD3DMetadata(RendererD3D *renderer,
                                       const gl::ShaderMap<const ShaderD3D *> &shaders)
...
{
    return (mShaders[gl::ShaderType::Fragment]->usesFragColor() &&
            mShaders[gl::ShaderType::Fragment]->usesMultipleRenderTargets() &&
            data.getClientMajorVersion() < 3);
}

bool ProgramD3DMetadata::usesFragDepth() const
{
    return mShaders[gl::ShaderType::Fragment]->usesFragDepth();
}

bool ProgramD3DMetadata::usesPointCoord() const
{
    return mShaders[gl::ShaderType::Fragment]->usesPointCoord();
}

bool ProgramD3DMetadata::usesFragCoord() const
{
    return mShaders[gl::ShaderType::Fragment]->usesFragCoord();
}

bool ProgramD3DMetadata::usesPointSize() const
{
    return mShaders[gl::ShaderType::Vertex]->usesPointSize();
}
...
{
    return mShaders[gl::ShaderType::Vertex]->hasANGLEMultiviewEnabled();
}

bool ProgramD3DMetadata::usesViewID() const
{
    return mShaders[gl::ShaderType::Fragment]->usesViewID();
}
...
{
    return mShaders[gl::ShaderType::Fragment]->usesMultipleRenderTargets();
}

GLint ProgramD3DMetadata::getMajorShaderVersion() const
{
    return mShaders[gl::ShaderType::Vertex]->getData().getShaderVersion();
}

const ShaderD3D *ProgramD3DMetadata::getFragmentShader() const
{
    return mShaders[gl::ShaderType::Fragment];
}
                                    ...
                                    const gl::Caps &caps) const
{
    ...
    if (samplerIndex < mShaderSamplers[type].size() && mShaderSamplers[type][samplerIndex].active)
    {
        logicalTextureUnit = mShaderSamplers[type][samplerIndex].logicalTextureUnit;
    }
...
{
    ASSERT(type != gl::ShaderType::InvalidEnum);
    ASSERT(samplerIndex < mShaderSamplers[type].size());
    ASSERT(mShaderSamplers[type][samplerIndex].active);

    return mShaderSamplers[type][samplerIndex].textureType;
}
    ...
    for (const D3DUniform *d3dUniform : mD3DUniforms)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (samplerIndex < mShaderSamplers[shaderType].size())
                {
                    ASSERT(mShaderSamplers[shaderType][samplerIndex].active);
                    mShaderSamplers[shaderType][samplerIndex].logicalTextureUnit =
                        d3dUniform->mSamplerData[i];
                }
            }
        }
    }
                                ...
                                gl::BinaryInputStream *stream)
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        stream->readString(&mShaderHLSLs[shaderType]);
...
void ProgramD3D::save(const gl::Context *context, gl::BinaryOutputStream *stream)
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        stream->writeString(mShaderHLSLs[shaderType]);
                                                              ...
                                                              gl::InfoLog *infoLog)
{
    ...
    std::string finalPixelHLSL = mDynamicHLSL->generatePixelShaderForOutputSignature(
        mShaderHLSLs[gl::ShaderType::Fragment], mPixelShaderKey, mUsesFragDepth,
                                                              ...
                                                              gl::InfoLog *infoLog)
{
    ...
    std::string finalVertexHLSL = mDynamicHLSL->generateVertexShaderForInputLayout(
        mShaderHLSLs[gl::ShaderType::Vertex], mCachedInputLayout, mState.getAttributes());
                                ...
                                gl::InfoLog &infoLog)
{
    ...
    {
        gl::ShaderMap<const ShaderD3D *> graphicsShadersD3D = {};
        for (gl::ShaderType shaderType : gl::kAllGraphicsShaderTypes)
        {
            if (mState.getAttachedShader(shaderType))
            {
                graphicsShadersD3D[shaderType] =
                    GetImplAs<ShaderD3D>(mState.getAttachedShader(shaderType));
                graphicsShadersD3D[shaderType]->generateWorkarounds(
                    &mShaderWorkarounds[shaderType]);
        ...
        {
            if (graphicsShadersD3D[gl::ShaderType::Fragment]->usesFrontFacing())
            {
        ...
        ProgramD3DMetadata metadata(mRenderer, graphicsShadersD3D);
        ...
        mDynamicHLSL->generateShaderLinkHLSL(context, mState, metadata, resources.varyingPacking,
                                             builtins, &mShaderHLSLs);

        mUsesPointSize = graphicsShadersD3D[gl::ShaderType::Vertex]->usesPointSize();
...
void ProgramD3D::initializeUniformBlocks()
{
    ...
    gl::ShaderMap<const ShaderD3D *> shaderD3Ds = {};
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        shaderD3Ds[shaderType] = SafeGetImplAs<ShaderD3D>(mState.getAttachedShader(shaderType));
    }
    ...
    for (const gl::InterfaceBlock &uniformBlock : mState.getUniformBlocks())
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            ...
            {
                ASSERT(shaderD3Ds[shaderType]);
                unsigned int baseRegister =
                    shaderD3Ds[shaderType]->getUniformBlockRegister(uniformBlock.name);
...
void ProgramD3D::initializeUniformStorage()
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        mShaderUniformStorages[shaderType] = std::unique_ptr<UniformStorageD3D>(
...
void ProgramD3D::updateUniformBufferCache(const gl::Caps &caps,
                                          const gl::ShaderMap<unsigned int> &shaderReservedUBOs)
{
    if (mState.getUniformBlocks().empty())
    {
        return;
         ...
         uniformBlockIndex++)
    {
        ...
        if (!uniformBlock.activeInProgram())
        {
            continue;
        }
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            ...
            unsigned int registerIndex =
                uniformBlock.mShaderRegisterIndexes[shaderType] - shaderReservedUBOs[shaderType];
            ASSERT(registerIndex < GetMaximumShaderUniformBlocksPerShader(shaderType, caps));

            if (mShaderUBOCaches[shaderType].size() <= registerIndex)
            {
                mShaderUBOCaches[shaderType].resize(registerIndex + 1, -1);
            }

            ASSERT(mShaderUBOCaches[shaderType][registerIndex] == -1);
            mShaderUBOCaches[shaderType][registerIndex] = blockBinding;
        }
    }
}
...
void ProgramD3D::assignSamplerRegisters(size_t uniformIndex)
{
    ...
    if (computeShader)
    {
        ...
        assignSamplerRegistersPerShader(computeShaderD3D, baseName, registerOffset, d3dUniform);
    }
    else
    {
        bool hasUniformInGraphicsShaders = false;
        for (gl::ShaderType shaderType : gl::kAllGraphicsShaderTypes)
        {
            if (!mState.getAttachedShader(shaderType))
            {
            ...
            {
                assignSamplerRegistersPerShader(shaderD3D, baseName, registerOffset, d3dUniform);
                hasUniformInGraphicsShaders = true;
            }
        }

        ASSERT(hasUniformInGraphicsShaders);
    }
}

void ProgramD3D::assignSamplerRegistersPerShader(const ShaderD3D *shaderD3D,
                                                 const std::string &baseName,
                                                 unsigned int registerOffset,
                                                 D3DUniform *d3dUniform)
{
    ASSERT(shaderD3D && shaderD3D->hasUniform(baseName));

    gl::ShaderType shaderType = shaderD3D->getData().getShaderType();
...
void ProgramD3D::reset()
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        mShaderHLSLs[shaderType].clear();
        mShaderWorkarounds[shaderType].reset();
    }
