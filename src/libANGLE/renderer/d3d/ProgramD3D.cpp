...
bool IsSamplerIndexExceedsLimit(gl::ShaderType shaderType,
                                ...
                                const gl::Caps &caps)
{
    switch (shaderType)
    {
        case gl::ShaderType::Fragment:
            return samplerIndex >= caps.maxTextureImageUnits;
        case gl::ShaderType::Vertex:
            return samplerIndex >= caps.maxVertexTextureImageUnits;
        case gl::ShaderType::Compute:
            return samplerIndex >= caps.maxComputeTextureImageUnits;
        case gl::ShaderType::Geometry:
            return samplerIndex >= caps.maxGeometryTextureImageUnits;
        default:
            UNREACHABLE();
            return true;
    }
...
bool IsUniformBlockRegisterIndexExceedsLimit(gl::ShaderType shaderType,
                                             ...
                                             const gl::Caps &caps)
{
    ...
    {
        case gl::ShaderType::Vertex:
            return registerIndex >= caps.maxVertexUniformBlocks;
        case gl::ShaderType::Fragment:
            return registerIndex >= caps.maxFragmentUniformBlocks;
        case gl::ShaderType::Compute:
            return registerIndex >= caps.maxComputeUniformBlocks;
        case gl::ShaderType::Geometry:
            return registerIndex >= caps.maxGeometryUniformBlocks;
        default:
            UNREACHABLE();
            return true;
    }
                                    ...
                                    const gl::Caps &caps) const
{
    ...
    ASSERT(!IsSamplerIndexExceedsLimit(type, samplerIndex, caps));
                                ...
                                gl::InfoLog &infoLog)
{
    ...
    else
    {
        ...
        mShaderSamplers[gl::ShaderType::Vertex].resize(data.getCaps().maxVertexTextureImageUnits);
        mShaderSamplers[gl::ShaderType::Fragment].resize(data.getCaps().maxTextureImageUnits);

        vertexShaderD3D->generateWorkarounds(&mShaderWorkarounds[gl::ShaderType::Vertex]);
        fragmentShaderD3D->generateWorkarounds(&mShaderWorkarounds[gl::ShaderType::Fragment]);
        ...
        mDynamicHLSL->generateShaderLinkHLSL(context, mState, metadata, resources.varyingPacking,
                                             builtins, &mShaderHLSLs[gl::ShaderType::Fragment],
                                             &mShaderHLSLs[gl::ShaderType::Vertex]);
                                          ...
                                          unsigned int reservedFragment)
{
    ...
    gl::ShaderMap<unsigned int> reservedUBOs = {};
    reservedUBOs[gl::ShaderType::Vertex]     = reservedVertex;
    reservedUBOs[gl::ShaderType::Fragment]   = reservedFragment;
         ...
         uniformBlockIndex++)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            ...
            unsigned int registerIndex =
                uniformBlock.mShaderRegisterIndexes[shaderType] - reservedUBOs[shaderType];
            ASSERT(!IsUniformBlockRegisterIndexExceedsLimit(shaderType, registerIndex, caps));
