...
bool SamplerMetadataD3D11::initBuffer(ID3D11Device *device, ID3D11DeviceContext *deviceContext)
{
    ...
    d3d11::InitConstantBufferDesc(
        ...
        sizeof(dx_SamplerMetadata) * mSamplerCount + sizeof(dx_ShaderConstants11));
    HRESULT result =
                                      ...
                                      const dx_ShaderConstants11 &extraConstants)
{
    ...
    if (mAppliedConstants.empty())
    {
        mAppliedConstants.resize(sizeof(dx_SamplerMetadata) * mSamplerCount +
                                 sizeof(dx_ShaderConstants11));
        mDirty = true;
    }
    if (mDirty ||
        memcmp(&extraConstants, mAppliedConstants.data(), sizeof(dx_ShaderConstants11)) != 0)
    {
        memcpy(mAppliedConstants.data(), &extraConstants, sizeof(dx_ShaderConstants11));
        if (mSamplerCount > 0)
        {
            memcpy(&mAppliedConstants.data()[sizeof(dx_ShaderConstants11)], mSamplerMetadata.data(),
