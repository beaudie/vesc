...
size_t ProgramD3D::getUniformBlockInfo(const sh::InterfaceBlock &interfaceBlock)
{
    ...
    // Use a lambda with a capture to process the incoming uniform block info.
    auto *blockInfoMap = &mBlockInfo;
    auto writeToMap    = [blockInfoMap](const std::string &fieldName,
                                     const sh::BlockMemberInfo &info) {
        (*blockInfoMap)[fieldName] = info;
    };

    sh::GetUniformBlockInfo(interfaceBlock.fields, interfaceBlock.fieldPrefix(), encoder,
                            interfaceBlock.isRowMajorLayout, &writeToMap);
