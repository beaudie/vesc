...
const uint8_t *D3DUniform::firstNonNullData() const
{
    ...
    return vsData
               ...
               : (psData
                      ? psData
                      : (csData ? csData : reinterpret_cast<const uint8_t *>(mSamplerData.data())));
}
...
void ProgramD3D::getUniformInternal(GLint location, DestT *dataOut) const
{
    ...
    ASSERT(gl::VariableComponentType(uniform.type) == gl::GLTypeToGLenum<DestT>::value);
    if (gl::IsMatrixType(uniform.type))
    {
        GetMatrixUniform(gl::VariableColumnCount(uniform.type),
                            gl::VariableRowCount(uniform.type), dataOut,
                            reinterpret_cast<const DestT *>(srcPointer));
    }
    else
    {
