...
D3DUniform::D3DUniform(GLenum type,
    ...
    : typeInfo(gl::GetUniformTypeInfo(type)),
      ...
      registerElement(0)
{
    shaderRegisterIndexes.fill(GL_INVALID_INDEX);
...
{
    return shaderRegisterIndexes[shaderType] != GL_INVALID_INDEX;
}
...
const uint8_t *D3DUniform::firstNonNullData() const
{
    ...
    {
        if (shaderData[shaderType])
        {
            return shaderData[shaderType];
        }
    }
...
ProgramD3D::SamplerMapping ProgramD3D::updateSamplerMapping()
{
    ...
    for (const D3DUniform *d3dUniform : mD3DUniforms)
    {
        ...
        {
            unsigned int firstIndex = d3dUniform->shaderRegisterIndexes[gl::ShaderType::Fragment];
        ...
        {
            unsigned int firstIndex = d3dUniform->shaderRegisterIndexes[gl::ShaderType::Vertex];
        ...
        {
            unsigned int firstIndex = d3dUniform->shaderRegisterIndexes[gl::ShaderType::Compute];
                                ...
                                gl::BinaryInputStream *stream)
{
    ...
    for (unsigned int uniformIndex = 0; uniformIndex < uniformCount; uniformIndex++)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            stream->readInt(&d3dUniform->shaderRegisterIndexes[shaderType]);
        }
...
void ProgramD3D::save(const gl::Context *context, gl::BinaryOutputStream *stream)
{
    ...
    for (const D3DUniform *uniform : mD3DUniforms)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            stream->writeIntOrNegOne(uniform->shaderRegisterIndexes[shaderType]);
        }
...
void ProgramD3D::initializeUniformStorage(const gl::ShaderBitSet &availableShaderStages)
{
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            if (d3dUniform->isReferencedByShader(shaderType))
            {
                ...
                shaderRegisters[shaderType] = std::max(
                    ...
                    d3dUniform->shaderRegisterIndexes[shaderType] + d3dUniform->registerCount);
            }
        }
    }
    ...
    for (D3DUniform *d3dUniform : mD3DUniforms)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            if (d3dUniform->isReferencedByShader(shaderType))
            {
                d3dUniform->shaderData[shaderType] =
                    mShaderUniformStorages[shaderType]->getDataPointer(
                        d3dUniform->shaderRegisterIndexes[shaderType], d3dUniform->registerElement);
            }
        }
    }
}
...
void ProgramD3D::updateUniformBufferCache(
    const gl::Caps &caps,
    const gl::ShaderMap<unsigned int> &shaderReservedUniformBuffers)
{
    if (mState.getUniformBlocks().empty())
    {
        return;
         ...
         uniformBlockIndex++)
    {
        ...
        for (gl::ShaderType shaderType : gl::AllShaderTypes())
        {
            ...
            unsigned int registerIndex = uniformBlock.mShaderRegisterIndexes[shaderType] -
                                         shaderReservedUniformBuffers[shaderType];
                               ...
                               D3DUniformMap *uniformMap)
{
    ...
    if (encoder)
    {
        ...
        d3dUniform->shaderRegisterIndexes[shaderType] = reg;
...
void ProgramD3D::setUniformInternal(GLint location, GLsizei count, const T *v, GLenum uniformType)
{
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        if (targetUniform->shaderData[shaderType])
        {
            setUniformImpl(locationInfo, count, v, targetUniform->shaderData[shaderType],
                           uniformType);
    ...
    for (gl::ShaderType shaderType : gl::AllShaderTypes())
    {
        if (targetUniform->shaderData[shaderType])
        {
            if (setUniformMatrixfvImpl<cols, rows>(location, countIn, transpose, value,
                                                   targetUniform->shaderData[shaderType],
                                                   targetUniformType))
            {
                mShaderUniformsDirty.set(shaderType);
            }
...
void ProgramD3D::assignSamplerRegisters(size_t uniformIndex)
{
    ...
    if (computeShader)
    {
        ...
        d3dUniform->shaderRegisterIndexes[gl::ShaderType::Compute] =
            computeShaderD3D->getUniformRegister(baseName) + registerOffset;
        ASSERT(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Compute] != GL_INVALID_INDEX);
        AssignSamplers(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Compute],
    ...
    else
    {
        ...
        {
            d3dUniform->shaderRegisterIndexes[gl::ShaderType::Vertex] =
                vertexShaderD3D->getUniformRegister(baseName) + registerOffset;
            ASSERT(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Vertex] != GL_INVALID_INDEX);
            AssignSamplers(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Vertex],
                           d3dUniform->typeInfo, d3dUniform->getArraySizeProduct(), mSamplersVS,
                           &mUsedVertexSamplerRange);
        }
        if (fragmentShaderD3D->hasUniform(baseName))
        {
            d3dUniform->shaderRegisterIndexes[gl::ShaderType::Fragment] =
                fragmentShaderD3D->getUniformRegister(baseName) + registerOffset;
            ASSERT(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Fragment] != GL_INVALID_INDEX);
            AssignSamplers(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Fragment],
...
void ProgramD3D::assignImageRegisters(size_t uniformIndex)
{
    ...
    if (computeShader)
    {
        ...
        {
            AssignImages(d3dUniform->shaderRegisterIndexes[gl::ShaderType::Compute],
