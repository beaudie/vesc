...
class UniformEncodingVisitorD3D : public sh::BlockEncoderVisitor
{
  public:
    ...
    void visitSampler(const sh::ShaderVariable &sampler) override
    {
        std::string fullName = collapseNameStack(sampler.name);
        auto uniformMapEntry = mUniformMapOut->find(fullName);
        if (uniformMapEntry == mUniformMapOut->end())
        {
            (*mUniformMapOut)[fullName] =
                new D3DUniform(sampler.type, mRegisterType, fullName, sampler.arraySizes, true);
        }
    }
