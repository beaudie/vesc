...
SamplerMetadataD3D11::SamplerMetadataD3D11()
    : mSamplerCount(0),
      mType(gl::SAMPLER_VERTEX),
      mDirty(false),
      mSamplerMetadataBuffer(nullptr),
      mAppliedConstants(nullptr)
{
}
...
void SamplerMetadataD3D11::reset()
{
    ...
    SafeDeleteArray(mAppliedConstants);
}
...
bool SamplerMetadataD3D11::initBuffer(ID3D11Device *device, ID3D11DeviceContext *deviceContext)
{
    ...
    d3d11::InitConstantBufferDesc(
        ...
        sizeof(dx_SamplerMetadata) * mSamplerCount + sizeof(dx_ShaderConstants));
    HRESULT result =
...
gl::Error SamplerMetadataD3D11::apply(ID3D11Device *device,
                                      ID3D11DeviceContext *deviceContext,
                                      const dx_ShaderConstants &extraConstants)
{
    ...
    if (!mAppliedConstants)
    {
        mAppliedConstants =
            new char[sizeof(dx_SamplerMetadata) * mSamplerCount + sizeof(dx_ShaderConstants)];
        mDirty = true;
    }
    if (mDirty || memcmp(&extraConstants, mAppliedConstants, sizeof(dx_ShaderConstants)) != 0)
    {
        memcpy(mAppliedConstants, &extraConstants, sizeof(dx_ShaderConstants));
        if (mSamplerCount > 0)
        {
            memcpy(&mAppliedConstants[sizeof(dx_ShaderConstants)], mSamplerMetadata.data(),
                   sizeof(dx_SamplerMetadata) * mSamplerCount);
        }
        ASSERT(mSamplerMetadataBuffer);
        if (mSamplerMetadataBuffer)
        {
            deviceContext->UpdateSubresource(mSamplerMetadataBuffer, 0, nullptr, mAppliedConstants,
                                             16, 0);
            mDirty = false;
        }
    }
