                                      ...
                                      const dx_ShaderConstants11 &extraConstants)
{
    ...
    if (mDirty || memcmp(&mAppliedConstants, &extraConstants, sizeof(dx_ShaderConstants11)) != 0)
    {
        D3D11_MAPPED_SUBRESOURCE mapping = {0};
        HRESULT result =
            deviceContext->Map(mSamplerMetadataBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapping);
        ASSERT(SUCCEEDED(result));
        UNUSED_ASSERTION_VARIABLE(result);
        memcpy(mapping.pData, &extraConstants, sizeof(dx_ShaderConstants11));
        memcpy(&mAppliedConstants, &extraConstants, sizeof(dx_ShaderConstants11));
        if (mSamplerCount > 0)
        {
            memcpy(&reinterpret_cast<uint8_t *>(mapping.pData)[sizeof(dx_ShaderConstants11)],
                   mSamplerMetadata.data(), sizeof(dx_SamplerMetadata) * mSamplerCount);
        }
        deviceContext->Unmap(mSamplerMetadataBuffer, 0);
        mDirty = false;
    }
