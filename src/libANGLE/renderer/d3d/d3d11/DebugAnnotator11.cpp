...
{
    // ID3DUserDefinedAnnotation.GetStatus only works reliably on Windows10 or greater.
    // Returning true unconditionally means writing out all compiled shaders to temporary
    // files even if debugging tools are not attached. See
    // rx::ShaderD3D::prepareSourceAndReturnOptions. If you want debug annotations, you must
    // use Windows 10.
