          ...
          case GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN:
            ...
            case GL_TIME_ELAPSED_EXT:
            {
                ...
                if (result == S_OK)
                {
                    ...
                    ASSERT(sizeof(UINT64) == sizeof(GLuint64));
                    ASSERT(sizeof(UINT64) == sizeof(unsigned long long));
                    ...
                    else
                    {
                        ...
                        // If an overflow does somehow occur, there is no way the elapsed time is
                        // accurate, so we generate a disjoint event
                        mRenderer->setGPUDisjoint();
                    }
                }
            }
            ...
            {
                // D3D11 doesn't support timestamps as timestamps are not guaranteed to have any
                // sort of continuity outside of a disjoint timestamp query block
                mResult = 0;
            }
            break;
