          ...
          case GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN:
            ...
            case GL_TIME_ELAPSED_EXT:
            {
                ...
                if (result == S_OK)
                {
                    ...
                    if (FAILED(beginRes) || FAILED(endRes))
                    {
                        return gl::Error(
                            GL_OUT_OF_MEMORY,
                            "Failed to get the data of an internal query, result: 0x%X.", result);
                    }
            ...
            case GL_TIMESTAMP_EXT:
            {
                ...
                // to have any
                // sort of continuity outside of a disjoint timestamp query block, which GL depends
                // on
                mResult = 0;
            }
            break;
