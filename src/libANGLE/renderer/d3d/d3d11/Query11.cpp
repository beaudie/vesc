...
gl::Error Query11::flush(bool force)
{
    ...
    {
        Query11Info query = mPendingQueries.front();
...
gl::Error Query11::testQuery(Query11Info query)
{
    if (!mQueryFinished)
    {
        ASSERT(query.query);
        ...
        switch (getType())
        {
          ...
          case GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT:
            {
                UINT64 numPixels = 0;
                HRESULT result = context->GetData(query.query, &numPixels, sizeof(numPixels), 0);
                if (FAILED(result))
                {
          ...
          case GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN:
            {
                ...
                HRESULT result = context->GetData(query.query, &soStats, sizeof(soStats), 0);
                if (FAILED(result))
                {
                    return gl::Error(GL_OUT_OF_MEMORY, "Failed to get the data of an internal query, result: 0x%X.", result);
                }
            ...
            case GL_TIME_ELAPSED_EXT:
            {
                ...
                HRESULT result = context->GetData(query.query, &timeStats, sizeof(timeStats), 0);
                if (FAILED(result))
                {
                ...
                if (result == S_OK)
                {
                    UINT64 beginTime = 0;
                    HRESULT beginRes =
                        context->GetData(query.beginTimestamp, &beginTime, sizeof(UINT64), 0);
                    if (FAILED(beginRes))
                    {
                        return gl::Error(
                            GL_OUT_OF_MEMORY,
                    ...
                    UINT64 endTime = 0;
                    HRESULT endRes =
                        context->GetData(query.endTimestamp, &endTime, sizeof(UINT64), 0);
                    if (FAILED(endRes))
                    {
                        return gl::Error(
                            GL_OUT_OF_MEMORY,
