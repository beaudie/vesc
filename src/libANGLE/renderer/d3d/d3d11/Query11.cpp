...
angle::Result Query11::queryCounter(const gl::Context *glContext)
{
    ...
    ANGLE_TRY(mRenderer->beginTimestamp(context11));
        ...
        switch (getType())
        {
            ...
            case gl::QueryType::Timestamp:
            {
                ...
                else
                {
                    ...
                    // call, this is false.
                    const bool timestampValid = true;  // mRenderer->isTimestamp();

                    if (timestampValid)
                    {
                        D3D11_QUERY_DATA_TIMESTAMP_DISJOINT timeStats = {};

                        ANGLE_TRY(mRenderer->getDisjointStatus(context11, &timeStats));
                        UINT64 timestamp     = 0;
