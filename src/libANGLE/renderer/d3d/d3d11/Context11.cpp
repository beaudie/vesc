...
gl::Error Context11::initialize(gl::Caps *caps)
{
    ASSERT(caps);
    ...
    // version:
    // - If the upper context is ES 3.0 and below, we use D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT(8)
    //   as the value of max draw buffers because UAVs are not used.
    // - If the upper context is ES 3.1 and the feature level is 11_0, the RTVs and UAVs share 8
    //   resource slots. As ES 3.1 requires at least 1 atomic counter buffer in compute shaders, the
    //   value of max combined shader output resources is limited to 7, thus only 7 RTV slots can be
    //   used simultaneously.
    // - If the upper context is ES 3.1 and the feature level is 11_1, there are 64 UAV slots
    //   available, currently we allocate 60 UAV slots for combined shader output resources, so we
    //   can use at most D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT(8) RTVs.
