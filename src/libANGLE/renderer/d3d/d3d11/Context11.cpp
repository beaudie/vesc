...
Context11::Context11(const gl::ContextState &state, Renderer11 *renderer) : ContextImpl(state), mRenderer(renderer)
{
}
...
TextureImpl *Context11::createTexture(GLenum target)
{
    switch (target)
    {
        case GL_TEXTURE_2D: return new TextureD3D_2D(mRenderer);
        case GL_TEXTURE_CUBE_MAP: return new TextureD3D_Cube(mRenderer);
        case GL_TEXTURE_3D: return new TextureD3D_3D(mRenderer);
        case GL_TEXTURE_2D_ARRAY: return new TextureD3D_2DArray(mRenderer);
        case GL_TEXTURE_EXTERNAL_OES:
            return new TextureD3D_External(mRenderer);
        default:
            UNREACHABLE();
    }
...
gl::Error Context11::drawArrays(GLenum mode,
                     GLint first,
                     GLsizei count)
{
