                                      ...
                                      const gl::Framebuffer *sourceFramebuffer)
{
    if (blitRenderTarget)
    {
        ...
        UINT readLayer =
            readBuffer->getTexture()->getType() == gl::TextureType::_3D ? readBuffer->layer() : 0;
             ...
             colorAttachment++)
        {
            ...
            if (drawBuffer.isAttached() && drawBufferStates[colorAttachment] != GL_NONE)
            {
                ...
                UINT drawLayer = drawBuffer.getTexture()->getType() == gl::TextureType::_3D
                                     ? drawBuffer.layer()
                                     : 0;
    ...
    if (blitDepth || blitStencil)
    {
        ...
        UINT readLayer =
            readBuffer->getTexture()->getType() == gl::TextureType::_3D ? readBuffer->layer() : 0;
        UINT drawLayer =
            drawBuffer->getTexture()->getType() == gl::TextureType::_3D ? drawBuffer->layer() : 0;

        ANGLE_TRY(mRenderer->blitRenderbufferRect(
            context, actualSourceArea, actualDestArea, readLayer, drawLayer, readRenderTarget,
            drawRenderTarget, filter, scissor, false, blitDepth, blitStencil));
    }
