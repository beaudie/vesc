        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            switch (attachments[i])
            {
              ...
              case GL_COLOR_ATTACHMENT0:
                  if (mData.mColorAttachments[0])
                  {
                      error = d3d11::GetAttachmentRenderTarget(mData.mColorAttachments[0], &renderTarget);
                      if (error.isError())
                      {
                          return error;
                      }
