...
gl::Error Framebuffer11::clear(const gl::Data &data, const ClearParameters &clearParams)
{
    ...
    if (clearParams.scissorEnabled == true && mRenderer->directRenderingEnabled() &&
        colorAttachment != nullptr && colorAttachment->type() == GL_FRAMEBUFFER_DEFAULT)
    {
        ClearParameters directRenderingClearParams = clearParams;
        gl::Extents framebufferSize                = colorAttachment->getSize();
        directRenderingClearParams.scissor.y       = framebufferSize.height -
                                               directRenderingClearParams.scissor.y -
                                               directRenderingClearParams.scissor.height;
        error = clearer->clearFramebuffer(directRenderingClearParams, mData);
    }
    else
    {
                                        ...
                                        uint8_t *pixels) const
{
    ...
    else
    {
        ...
        const bool invertSourceTexture =
            (colorbuffer->type() == GL_FRAMEBUFFER_DEFAULT && mRenderer->directRenderingEnabled());
        ...
        for (size_t colorAttachment = 0; colorAttachment < colorAttachments.size(); colorAttachment++)
        {
                ...
                drawBufferStates[colorAttachment] != GL_NONE)
            {
                ...
                const bool invertColorSource = (readBuffer->type() == GL_FRAMEBUFFER_DEFAULT &&
                                                mRenderer->directRenderingEnabled());
