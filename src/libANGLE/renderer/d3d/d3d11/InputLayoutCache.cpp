                                               ...
                                               GLenum mode, gl::Program *program, TranslatedIndexData *indexInfo)
{
    ...
    bool indexedPointSpriteEmulationActive = instancedPointSpritesActive && (indexInfo != nullptr);
    ...
    for (unsigned int i = 0; i < gl::MAX_VERTEX_ATTRIBS; i++)
    {
        ...
        if (attributes[i].active)
        {
            ...
            {
                buffer = bufferStorage ? bufferStorage->getEmulatedIndexedBuffer(BUFFER_USAGE_VERTEX_OR_TRANSFORM_FEEDBACK, indexInfo, &attributes[i])
