    ...
    bool instancedRenderingUsed)
{
    ...
    for (unsigned int i = 0; i < unsortedAttributes.size(); i++)
    {
        if (sortedAttributes[i]->active)
        {
            ...
            // If instanced PointSprite emulation is active and instanced rendering is used,
            ...
            // per instance.
            if (instancedPointSpritesActive && instancedRenderingUsed && sortedAttributes[i]->divisor > 0 &&
    ...
    for (unsigned int i = 0; i < gl::MAX_VERTEX_ATTRIBS; i++)
    {
        ...
        if (i < unsortedAttributes.size() && sortedAttributes[i]->active)
        {
            ...
            // If instanced PointSprite emulation is active and instanced rendering is used, then
            ...
            // with DrawIndexedInstanced on each iteration.  The offset into the instance data must
