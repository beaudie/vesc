                                   ...
                                   ID3D11ShaderResourceView **outSRV)
{
    ...
    gl::Error error = getCachedOrCreateSRV(key, outSRV);
    if (error.isError())
    {
        return error;
    }
...
gl::Error TextureStorage11::getCachedOrCreateSRV(const SRVKey &key, ID3D11ShaderResourceView **outSRV)
{
    ...
    gl::Error error = createSRV(key.baseLevel, key.mipLevels, format, texture, &srv);
    if (error.isError())
    {
        return error;
    }
