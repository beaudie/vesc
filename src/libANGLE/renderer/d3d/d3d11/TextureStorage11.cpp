...
#if !defined(NDEBUG)
    // Make sure that all the swap chain DXGI formats are consistent with the ANGLE format we got by
    // using the swapchain's GL internal format.
      ...
      mSwizzleRenderTargets(gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS, nullptr)
{
    ...
    ASSERT(d3d11::GetANGLEFormat(mTextureFormatSet.texFormat, srvDesc.Format, rtvFormat,
                                 dsvFormat) == mTextureFormatSet.format);
#endif
}
