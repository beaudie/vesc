                                                 ...
                                                 const d3d11::SharedSRV **outSRV)
{
    ...
    const bool workaround = mRenderer->getWorkarounds().emulateTinyStencilTextures.applied;
                                                       ...
                                                       const gl::Box &copyArea)
{
    ...
    if (level > 0 && mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied)
    {
                                                     ...
                                                     const gl::Box &region)
{
    ...
    // If the zero-LOD workaround is active and we want to update a level greater than zero, then we
    // should update the mipmapped texture, even if mapmaps are currently disabled.
    if (index.getLevelIndex() > 0 && mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied)
    {
      ...
      mSwizzleTexture()
{
    ...
    ASSERT(!mUseLevelZeroTexture || mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied);
}
                                                     ...
                                                     const TextureHelper11 **outResource)
{
    ...
    ASSERT(mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied);
...
angle::Result TextureStorage11_2D::ensureTextureExists(const gl::Context *context, int mipLevels)
{
    ...
    bool useLevelZeroTexture = mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied
                                                         ...
                                                         UINT *outSubresourceIndex) const
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround.applied && mUseLevelZeroTexture &&
