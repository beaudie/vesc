...
gl::Error TextureStorage11_2DMultisample::getResource(const TextureHelper11 **outResource)
{
    ANGLE_TRY(ensureTextureExists(1));
...
{
    // For Multisampled textures, mMipLevels always equals 1.
    ASSERT(mipLevels == 1);
    ...
    if (!outputTexture->valid() && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        ANGLE_TRY(mRenderer->CheckMultisampleQualityLevels(desc.Format, mSamples, &qualityLevels));
        ASSERT(qualityLevels > 0);
        desc.SampleDesc.Count   = mSamples;
        desc.SampleDesc.Quality = qualityLevels - 1;

        ANGLE_TRY(mRenderer->allocateTexture(desc, mFormatInfo, outputTexture));
        outputTexture->setDebugName("TexStorage2DMS.Texture");
    }
                                                                 ...
                                                                 Image11 *incomingImage)
{
    ...
    if (mAssociatedImage != nullptr && mAssociatedImage != incomingImage)
    {
        ...
        // Force the image to recover from storage before its data is overwritten.
        // This will reset mAssociatedImages to nullptr too.
        ANGLE_TRY(mAssociatedImage->recoverFromAssociatedStorage());
    }
                                                          ...
                                                          RenderTargetD3D **outRT)
{
    ...
    if (mFormatInfo.rtvFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        mRenderTarget = new TextureRenderTarget11(
            std::move(rtv), *texture, *srv, *blitSRV, mFormatInfo.internalFormat, getFormatSet(),
