                                        ...
                                        ID3D11ShaderResourceView **outSRV)
{
    ...
    ID3D11ShaderResourceView **otherLevelSRVs = (blitSRV) ? mLevelSRVs : mLevelBlitSRVs;

    if (!levelSRVs[mipLevel])
    {
        // Only create a different SRV for blit if blit format is different from regular srv format
        if (otherLevelSRVs[mipLevel] && mTextureFormatSet.srvFormat == mTextureFormatSet.blitSRVFormat)
        {
