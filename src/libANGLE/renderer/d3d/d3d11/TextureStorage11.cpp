                                        ...
                                        angle::ComPtr<ID3D11ShaderResourceView> *outSRV)
{
    ...
    if (!levelSRVs[mipLevel])
    {
        ...
        else
        {
            ...
            ANGLE_TRY(createSRV(mipLevel, 1, resourceFormat, resource.Get(), &levelSRVs[mipLevel]));
        }
    }
                                         ...
                                         angle::ComPtr<ID3D11ShaderResourceView> *outSRV) const
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround)
    {
        ...
        if (mipLevels == 1 && mMipLevels > 1)
        {
            ...
            ASSERT(mLevelZeroTexture != nullptr && texture == mLevelZeroTexture.Get());
            srvTexture = mLevelZeroTexture.Get();
        }
        else
        {
            ASSERT(mipLevels == static_cast<int>(mMipLevels));
            ASSERT(mTexture != nullptr && texture == mTexture.Get());
