...
TextureStorage11_2DMultisample::~TextureStorage11_2DMultisample()
{
    ...
    SafeRelease(mLevelZeroTexture);
    SafeDelete(mLevelZeroRenderTarget);
    SafeRelease(mSwizzleRenderTarget);
}
...
gl::Error TextureStorage11_2DMultisample::copyToStorage(TextureStorage *destStorage)
{
    ...
    TextureStorage11_2DMultisample *dest11 = GetAs<TextureStorage11_2DMultisample>(destStorage);
    ID3D11DeviceContext *immediateContext  = mRenderer->getDeviceContext();
                                                          ...
                                                          RenderTargetD3D **outRT)
{
    ...
    if (mUseLevelZeroTexture)
    {
        if (!mLevelZeroRenderTarget)
        {
            ...
            mLevelZeroRenderTarget = new TextureRenderTarget11(
                rtv, mLevelZeroTexture, srv, nullptr, mFormatInfo.internalFormat,
                getFormatSet(), getLevelWidth(level), getLevelHeight(level), 1, 0);
...
gl::Error TextureStorage11_2DMultisample::getSwizzleTexture(ID3D11Resource **outTexture)
{
    ...
    if (!mSwizzleTexture)
    {
        ...
        d3d11::SetDebugName(mSwizzleTexture, "TexStorage2DMultisample.SwizzleTexture");
    }
                                                                 ...
                                                                 ID3D11RenderTargetView **outRTV)
{
    ...
    if (!mSwizzleRenderTarget)
    {
        ...
        HRESULT result =
            device->CreateRenderTargetView(mSwizzleTexture, &rtvDesc, &mSwizzleRenderTarget);
