                                            ...
                                            BindFlags flags)
{
    ...
    if (formatInfo.uavFormat != DXGI_FORMAT_UNKNOWN && flags.storageTexture)
    {
...
angle::Result TextureStorage11_2D::ensureTextureExists(const gl::Context *context, int mipLevels)
{
    ...
    if (!outputTexture->valid() && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        desc.ArraySize = 1;
        desc.Format    = getBindFlags() & D3D11_BIND_UNORDERED_ACCESS ? mFormatInfo.typelessFormat
                                                                      : mFormatInfo.texFormat;
...
angle::Result TextureStorage11_Cube::ensureTextureExists(const gl::Context *context, int mipLevels)
{
    ...
    if (!outputTexture->valid() && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        desc.Format    = isStorageTexture() ? mFormatInfo.typelessFormat : mFormatInfo.texFormat;
        desc.SampleDesc.Count   = 1;
        desc.SampleDesc.Quality = 0;
        desc.Usage              = D3D11_USAGE_DEFAULT;
        desc.BindFlags          = getBindFlags();
