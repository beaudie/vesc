...
angle::Result TextureStorage11_2D::resolveTexture(const gl::Context *context)
{
    if (mMSTexInfo && mMSTexInfo->msTex && mMSTexInfo->msTextureNeedsResolve)
    {
        ...
        mRenderTarget[mMSTexInfo->indexSS.getLevelIndex()].reset(nullptr);
        onStateChange(angle::SubjectMessage::ContentsChanged);
    }
    return angle::Result::Continue;
}
...
angle::Result TextureStorage11_Cube::resolveTexture(const gl::Context *context)
{
    ...
    {
        ANGLE_TRY(resolveTextureHelper(context, mTexture));
        const int faceIndex = mMSTexInfo->indexSS.cubeMapFaceIndex();
...
angle::Result TextureStorage11_2DArray::clearRenderTargetsAfterResolve()
{
    // Get list of layers and layer count
    // Miplevel will be the same for all cases
    std::vector<int> flatLayers;
    const int layerIndex = mMSTexInfo->indexSS.getLayerIndex();
    const int layerCount = mMSTexInfo->indexSS.getLayerCount();
    for (int i = 0; i < layerCount; i++)
    {
        flatLayers.push_back(layerIndex + i);
    }

    // keys to erase
    std::vector<LevelLayerRangeKey> keysToDel;
    // Loop through mRenderTargets
    for (auto it = mRenderTargets.begin(); it != mRenderTargets.end(); ++it)
    {
        // get flat list of RenderTarget key
        LevelLayerRangeKey key = it->first;
        std::vector<int> rtLayers;
        for (int i = 0; i < key.numLayers; i++)
        {
            if (std::find(flatLayers.begin(), flatLayers.end(), key.layer + i) != flatLayers.end())
            {
                keysToDel.push_back(key);
                break;
            }
        }
    }
...
angle::Result TextureStorage11_2DArray::resolveTexture(const gl::Context *context)
{
    if (mMSTexInfo && mMSTexInfo->msTex && mMSTexInfo->msTextureNeedsResolve)
    {
        ...
        ASSERT(color != (depth || stencil));
        if (depth)
        {
            RenderTargetD3D *rtDepth;
            gl::ImageIndex indexMS =
                gl::ImageIndex::Make2DMultisampleArrayRange(0, mMSTexInfo->indexSS.getLayerCount());
            ANGLE_TRY(mMSTexInfo->msTex->getRenderTarget(context, indexMS, mMSTexInfo->samples,
                                                         &rtDepth));
            ANGLE_TRY(mRenderer->resolveMultisampledTexture(context, GetAs<RenderTarget11>(rtDepth),
                                                            depth, stencil, &mTexture));
        }
        if (color)
        {
            ANGLE_TRY(resolveTextureHelper(context, mTexture));
        }
        ANGLE_TRY(clearRenderTargetsAfterResolve());
        onStateChange(angle::SubjectMessage::ContentsChanged);
    }
    return angle::Result::Continue;
}
