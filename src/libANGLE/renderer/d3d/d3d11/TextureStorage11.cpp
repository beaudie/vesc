...
gl::Error TextureStorage11::getSRV(const gl::SamplerState &samplerState, ID3D11ShaderResourceView **outSRV)
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround)
    {
        ...
        gl::Error error = pickLevelZeroWorkaroundTexture(mipLevels == 1);
        if (error.isError())
        {
            return error;
        }
...
gl::Error TextureStorage11_2D::copyToStorage(TextureStorage *destStorage)
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround)
    {
        ...
        if (mTexture)
        {
            gl::Error error = dest11->pickLevelZeroWorkaroundTexture(false);
            if (error.isError())
            {
                return error;
            }
        ...
        {
            gl::Error error = dest11->pickLevelZeroWorkaroundTexture(true);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureStorage11_2D::pickLevelZeroWorkaroundTexture(bool useLevelZeroTexture)
{
