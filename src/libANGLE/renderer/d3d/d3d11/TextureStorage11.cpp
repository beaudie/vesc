...
TextureStorage11::SwizzleCacheValue::SwizzleCacheValue(GLenum red, GLenum green, GLenum blue,
                                                       GLenum alpha)
...
gl::Error TextureStorage11::getSRV(const gl::SamplerState &samplerState,
                                   ID3D11ShaderResourceView **outSRV)
{
    ...
    if (swizzleRequired)
    {
        verifySwizzleExists(samplerState.swizzleRed, samplerState.swizzleGreen,
                            samplerState.swizzleBlue, samplerState.swizzleAlpha);
    }
...
gl::Error TextureStorage11::getSRVLevels(GLint baseLevel, GLint maxLevel,
                                         ID3D11ShaderResourceView **outSRV)
{
...
gl::Error TextureStorage11::generateSwizzles(GLenum swizzleRed, GLenum swizzleGreen,
                                             GLenum swizzleBlue, GLenum swizzleAlpha)
{
                                                   ...
                                                   const gl::Box &copyArea)
{
    ...
    if (!fullCopy && (dxgiFormatInfo.depthBits > 0 || dxgiFormatInfo.stencilBits > 0))
    {
        ...
        return blitter->copyDepthStencil(srcTexture, sourceSubresource, copyArea, texSize,
                                         dstTexture, dstSubresource, copyArea, texSize, NULL);
    }
    else
    {
        D3D11_BOX srcBox;
...
void TextureStorage11::verifySwizzleExists(GLenum swizzleRed, GLenum swizzleGreen,
                                           GLenum swizzleBlue, GLenum swizzleAlpha)
{
    SwizzleCacheValue swizzleTarget(swizzleRed, swizzleGreen, swizzleBlue, swizzleAlpha);
...
gl::Error TextureStorage11::setData(const gl::ImageIndex &index, ImageD3D *image,
                                    ...
                                    const gl::PixelUnpackState &unpack, const uint8_t *pixelData)
{
    ...
    loadFunction(width, height, depth, pixelData, srcRowPitch, srcDepthPitch,
    ...
    if (!fullUpdate)
    {
        ...
        immediateContext->UpdateSubresource(resource, destSubresource, &destD3DBox,
                                            conversionBuffer->data(), bufferRowPitch,
                                            bufferDepthPitch);
    }
    else
    {
        immediateContext->UpdateSubresource(resource, destSubresource, NULL,
                                            conversionBuffer->data(), bufferRowPitch,
                                            bufferDepthPitch);
    }
...
TextureStorage11_2D::TextureStorage11_2D(Renderer11 *renderer, GLenum internalformat,
                                         bool renderTarget, GLsizei width, GLsizei height,
                                         int levels, bool hintLevelZeroOnly)
    : TextureStorage11(
          renderer,
          GetTextureBindFlags(internalformat, renderer->getRenderer11DeviceCaps(), renderTarget),
          GetTextureMiscFlags(internalformat, renderer->getRenderer11DeviceCaps(), renderTarget,
                              levels)),
      mTexture(NULL),
      mLevelZeroTexture(NULL),
      mLevelZeroRenderTarget(NULL),
...
gl::Error TextureStorage11_2D::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    // the GLES 2.0 spec, page 113 of version 2.0.25).
    // Other parts of TextureStorage11_2D could create RTVs on non-zero levels of the texture (e.g.
    // generateMipmap).
    // On Feature Level 9_3, this is unlikely to be useful. The renderer can't create SRVs on the
    // individual levels of the texture,
        ...
        if (mUseLevelZeroTexture)
        {
            if (!mLevelZeroRenderTarget)
            {
                ...
                mLevelZeroRenderTarget =
                    new TextureRenderTarget11(rtv, mLevelZeroTexture, NULL, mInternalFormat,
...
gl::Error TextureStorage11_2D::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format,
...
TextureStorage11_Cube::TextureStorage11_Cube(Renderer11 *renderer, GLenum internalformat,
                                             bool renderTarget, int size, int levels,
        ...
        if (!mUseLevelZeroTexture && mTexture)
        {
            ...
            for (int face = 0; face < 6; face++)
            {
                context->CopySubresourceRegion(mLevelZeroTexture, D3D11CalcSubresource(0, face, 1),
                                               0, 0, 0, mTexture, face * mMipLevels, NULL);
            }
        }
        ...
        if (mUseLevelZeroTexture && mLevelZeroTexture)
        {
            ...
            for (int face = 0; face < 6; face++)
            {
                context->CopySubresourceRegion(mTexture, D3D11CalcSubresource(0, face, mMipLevels),
                                               0, 0, 0, mLevelZeroTexture, face, NULL);
            }
        }
...
gl::Error TextureStorage11_Cube::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format,
                                           ID3D11Resource *texture,
                                           ID3D11ShaderResourceView **outSRV) const
{
    ASSERT(outSRV);
...
TextureStorage11_3D::TextureStorage11_3D(Renderer11 *renderer, GLenum internalformat,
                                         bool renderTarget, GLsizei width, GLsizei height,
                                         GLsizei depth, int levels)
    : TextureStorage11(
          renderer,
          GetTextureBindFlags(internalformat, renderer->getRenderer11DeviceCaps(), renderTarget),
          GetTextureMiscFlags(internalformat, renderer->getRenderer11DeviceCaps(), renderTarget,
...
gl::Error TextureStorage11_3D::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format,
                                         ID3D11Resource *texture,
                                         ID3D11ShaderResourceView **outSRV) const
{
    ASSERT(outSRV);
...
TextureStorage11_2DArray::TextureStorage11_2DArray(Renderer11 *renderer, GLenum internalformat,
                                                   bool renderTarget, GLsizei width, GLsizei height,
                                                   GLsizei depth, int levels)
    : TextureStorage11(
          renderer,
...
gl::Error TextureStorage11_2DArray::createSRV(int baseLevel, int mipLevels, DXGI_FORMAT format,
                                              ID3D11Resource *texture,
                                              ID3D11ShaderResourceView **outSRV) const
{
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
