    ...
    if (*outputTexture == nullptr && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        if (d3d11::isDeviceLostError(result))
        {
            ...
            return gl::OutOfMemory()
                   << "Failed to create 2D texture storage, " << gl::FmtHR(result);
        }
                                         ...
                                         ID3D11ShaderResourceView **outSRV) const
{
    ...
    if (FAILED(result))
    {
        return gl::OutOfMemory() << "Failed to create internal texture storage SRV, "
                                 << gl::FmtHR(result);
    }
...
gl::Error TextureStorage11_2D::getSwizzleTexture(ID3D11Resource **outTexture)
{
    ...
    if (!mSwizzleTexture)
    {
        ...
        {
            return gl::OutOfMemory()
                   << "Failed to create internal swizzle texture, " << gl::FmtHR(result);
        }
...
gl::Error TextureStorage11_2D::getSwizzleRenderTarget(int mipLevel, ID3D11RenderTargetView **outRTV)
{
    ...
    if (!mSwizzleRenderTargets[mipLevel])
    {
        ...
        {
            return gl::OutOfMemory()
                   << "Failed to create internal swizzle render target view, " << gl::FmtHR(result);
        }
    }
                                               ...
                                               ID3D11ShaderResourceView **outSRV) const
{
    ...
    if (texture == mSwizzleTexture)
    {
        ...
        {
            return gl::OutOfMemory()
                   << "Failed to create internal texture storage SRV, " << gl::FmtHR(result);
        }
...
gl::Error TextureStorage11_Cube::ensureTextureExists(int mipLevels)
{
    ...
    if (*outputTexture == nullptr && mTextureWidth > 0 && mTextureHeight > 0)
    {
        ...
        if (d3d11::isDeviceLostError(result))
        {
            mRenderer->notifyDeviceLost();
            return gl::OutOfMemory()
                   << "Failed to create cube texture storage, " << gl::FmtHR(result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
                                                       ...
                                                       ID3D11ShaderResourceView **srv) const
{
    ...
    {
        return gl::OutOfMemory()
               << "Failed to create internal shader resource view for texture storage, "
               << gl::FmtHR(result);
    }
    return gl::NoError();
}
...
gl::Error TextureStorage11_3D::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == nullptr && mTextureWidth > 0 && mTextureHeight > 0 && mTextureDepth > 0)
    {
        ...
        if (d3d11::isDeviceLostError(result))
        {
            mRenderer->notifyDeviceLost();
            return gl::OutOfMemory()
                   << "Failed to create 3D texture storage, " << gl::FmtHR(result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
...
gl::Error TextureStorage11_2DArray::getResource(ID3D11Resource **outResource)
{
    ...
    if (mTexture == nullptr && mTextureWidth > 0 && mTextureHeight > 0 && mTextureDepth > 0)
    {
        ...
        if (d3d11::isDeviceLostError(result))
        {
            mRenderer->notifyDeviceLost();
            return gl::OutOfMemory()
                   << "Failed to create 2D array texture storage, " << gl::FmtHR(result);
        }
        else if (FAILED(result))
        {
            ASSERT(result == E_OUTOFMEMORY);
    ...
    if (mRenderTargets.find(key) == mRenderTargets.end())
    {
        ...
        else
        {
            ...
            if (FAILED(result))
            {
                ...
                SafeRelease(blitSRV);
                return gl::OutOfMemory()
                       << "Failed to create TexStorage2DArray DSV, " << gl::FmtHR(result);
            }
