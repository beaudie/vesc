...
gl::Error TextureStorage11_Cube::copySubresourceLevel(ID3D11Resource* dstTexture, unsigned int dstSubresource,
    const gl::ImageIndex &index, const gl::Box &region)
{
    ASSERT(dstTexture);
    ...
    if (mRenderer->getFeatureLevel() <= D3D_FEATURE_LEVEL_9_3)
    {
        // D3D10Level9 doesn't fully support CopySubresourceRegion'ing from a mipmapped TextureCube to a non-mipmapped Texture2D
        // To work around this, we create an intermediary mipmapped non-cube Texture2D
        ID3D11Texture2D *intermediaryTexture = NULL;
        D3D11_TEXTURE2D_DESC intermediaryDesc;

        ID3D11Texture2D *srcTexture2D = NULL;
        srcTexture->QueryInterface(__uuidof(ID3D11Texture2D), (void**)(&srcTexture2D));
        srcTexture2D->GetDesc(&intermediaryDesc);

        // Use the src texture's desc to create the intermediary texture, except not as a texture cube
        ASSERT(intermediaryDesc.MiscFlags & D3D11_RESOURCE_MISC_TEXTURECUBE);
        intermediaryDesc.MiscFlags &= ~D3D11_RESOURCE_MISC_TEXTURECUBE;
        intermediaryDesc.ArraySize = 1;

        HRESULT result = device->CreateTexture2D(&intermediaryDesc, NULL, &intermediaryTexture);

        // this can happen from windows TDR
        if (d3d11::isDeviceLostError(result))
        {
            mRenderer->notifyDeviceLost();
            SafeRelease(srcTexture2D);
            SafeRelease(intermediaryTexture);
        ...
        context->CopySubresourceRegion(intermediaryTexture, index.mipIndex, 0, 0, 0,
                                       srcTexture, srcSubresource, NULL);

        context->CopySubresourceRegion(dstTexture, dstSubresource, region.x, region.y, region.z,
                                       intermediaryTexture, index.mipIndex, NULL);
