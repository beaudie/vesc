...
TextureStorage11::TextureStorage11(Renderer11 *renderer, UINT bindFlags, UINT miscFlags)
    : mRenderer(renderer),
      ...
      mBlitShaderResourceFormat(DXGI_FORMAT_UNKNOWN),
...
gl::Error TextureStorage11::getSRVLevel(int mipLevel, bool blitSRV, ID3D11ShaderResourceView **outSRV)
{
    ASSERT(mipLevel >= 0 && mipLevel < getLevelCount());

    // TODO: Only create a different SRV if (mBlitShaderResourceFormat != mShaderResourceFormat)
    // (difficult because double release must be avoided).
    ID3D11ShaderResourceView **levelSRVs = (blitSRV) ? mLevelBlitSRVs : mLevelSRVs;
    ...
    if (!levelSRVs[mipLevel])
    {
        ...
        DXGI_FORMAT resourceFormat = blitSRV ? mShaderResourceFormat : mBlitShaderResourceFormat;
...
gl::Error TextureStorage11::generateSwizzles(GLenum swizzleRed, GLenum swizzleGreen, GLenum swizzleBlue, GLenum swizzleAlpha)
{
    ...
    for (int level = 0; level < getLevelCount(); level++)
    {
        ...
        if (mSwizzleCache[level] != swizzleTarget)
        {
            ...
            // TODO: Make sure that using the blitSRV is appropriate here.
...
gl::Error TextureStorage11::generateMipmap(const gl::ImageIndex &sourceIndex, const gl::ImageIndex &destIndex)
{
    ...
    ID3D11ShaderResourceView *sourceSRV = GetAs<RenderTarget11>(source)->getBlitShaderResourceView();
      ...
      mSwizzleTexture(NULL)
{
    ...
    // TODO: mBlitShaderResourceFormat = 
      ...
      mSwizzleTexture(NULL)
{
    ...
    mBlitShaderResourceFormat = formatInfo.blitSRVFormat;
    ...
    if (!mRenderTarget[level])
    {
        ...
        if (mUseLevelZeroTexture)
        {
            if (!mLevelZeroRenderTarget)
            {
                ...
                mLevelZeroRenderTarget = new TextureRenderTarget11(rtv, mLevelZeroTexture, nullptr, nullptr, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1, 0);
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[level] = new TextureRenderTarget11(rtv, texture, srv, blitSRV, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1, 0);
      ...
      mSwizzleRenderTargets(gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS, nullptr)
{
    ...
    ID3D11ShaderResourceView *blitSRV = renderTarget11->getBlitShaderResourceView();
    D3D11_SHADER_RESOURCE_VIEW_DESC blitSRVDesc;
    blitSRV->GetDesc(&blitSRVDesc);
    mShaderResourceFormat = blitSRVDesc.Format;
...
gl::Error TextureStorage11_Cube::createRenderTargetSRV(ID3D11Resource *texture, const gl::ImageIndex &index, DXGI_FORMAT resourceFormat, ID3D11ShaderResourceView **srv) const
{
    ID3D11Device *device = mRenderer->getDevice();
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
    ...
    if (!mRenderTarget[faceIndex][level])
    {
        ...
        if (mUseLevelZeroTexture)
        {
            if (!mLevelZeroRenderTarget[faceIndex])
            {
                ...
                mLevelZeroRenderTarget[faceIndex] = new TextureRenderTarget11(rtv, mLevelZeroTexture, nullptr, nullptr, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1, 0);
        ...
        ID3D11ShaderResourceView *srv = nullptr;
        error = createRenderTargetSRV(texture, index, mShaderResourceFormat, &srv);
        if (error.isError())
        {
            return error;
        }
        ID3D11ShaderResourceView *blitSRV = nullptr;
        error = createRenderTargetSRV(texture, index, mBlitShaderResourceFormat, &blitSRV);
        if (error.isError())
        {
            return error;
        }
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[faceIndex][level] = new TextureRenderTarget11(rtv, texture, srv, blitSRV, mInternalFormat, getLevelWidth(level), getLevelHeight(level), 1, 0);
...
gl::Error TextureStorage11_3D::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (!index.hasLayer())
    {
        if (!mLevelRenderTargets[mipLevel])
        {
            ...
            mLevelRenderTargets[mipLevel] = new TextureRenderTarget11(rtv, texture, srv, blitSRV, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), getLevelDepth(mipLevel), 0);
    ...
    else
    {
        ...
        if (mLevelLayerRenderTargets.find(key) == mLevelLayerRenderTargets.end())
        {
            ...
            mLevelLayerRenderTargets[key] = new TextureRenderTarget11(rtv, texture, srv, blitSRV, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1, 0);
...
gl::Error TextureStorage11_2DArray::createRenderTargetSRV(ID3D11Resource *texture, const gl::ImageIndex &index, DXGI_FORMAT resourceFormat, ID3D11ShaderResourceView **srv) const
{
    ID3D11Device *device = mRenderer->getDevice();
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
...
gl::Error TextureStorage11_2DArray::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (mRenderTargets.find(key) == mRenderTargets.end())
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTargets[key] = new TextureRenderTarget11(rtv, texture, srv, blitSRV, mInternalFormat, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1, 0);
