      ...
      mSwizzleTexture(nullptr)
{
    ...
    mTextureFormat =
        d3d11::GetANGLEFormat(texDesc.Format, srvDesc.Format, rtvDesc.Format, DXGI_FORMAT_UNKNOWN);
    ASSERT(mTextureFormat != d3d11::ANGLE_FORMAT_NONE);
    const d3d11::ANGLEFormatInfo &angleFormatInfo = d3d11::GetANGLEFormatInfo(mTextureFormat);
    mTextureFormatSet                             = angleFormatInfo.dxgiFormatSet;
    mSwizzleFormat                                = angleFormatInfo.swizzleFormat;
    mSwizzleFormatSet                             = d3d11::GetANGLEFormatInfo(mSwizzleFormat).dxgiFormatSet;

    /*const d3d11::DXGIFormat &dxgiFormatInfo =
    d3d11::GetDXGIFormatInfo(mTextureFormatSet.texFormat);
      ...
      mSwizzleTexture(nullptr)
{
    ...
    mTextureFormat    = formatInfo.angleFormat;
...
gl::Error TextureStorage11_2D::getRenderTarget(const gl::ImageIndex &index, RenderTargetD3D **outRT)
{
    ...
    if (mUseLevelZeroTexture)
    {
        if (!mLevelZeroRenderTarget)
        {
            ...
            mLevelZeroRenderTarget = new TextureRenderTarget11(
                rtv, mLevelZeroTexture, nullptr, mInternalFormat, mTextureFormat,
    ...
    if (mTextureFormatSet.rtvFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        mRenderTarget[level] =
            new TextureRenderTarget11(rtv, texture, srv, mInternalFormat, mTextureFormat,
                                      getLevelWidth(level), getLevelHeight(level), 1, 0);
    ...
    mRenderTarget[level] =
        new TextureRenderTarget11(dsv, texture, srv, mInternalFormat, mTextureFormat,
                                  getLevelWidth(level), getLevelHeight(level), 1, 0);
      ...
      mSwizzleRenderTargets(gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS, nullptr)
{
    ...
    mMipLevels      = 1;
    mTextureFormat                                = renderTarget11->getANGLEFormat();
    const d3d11::ANGLEFormatInfo &angleFormatInfo = d3d11::GetANGLEFormatInfo(mTextureFormat);
    mTextureFormatSet                             = angleFormatInfo.dxgiFormatSet;
    mSwizzleFormat                                = angleFormatInfo.swizzleFormat;
    mSwizzleFormatSet                             = d3d11::GetANGLEFormatInfo(mSwizzleFormat).dxgiFormatSet;
    ...
    ASSERT(d3d11::GetANGLEFormat(mTextureFormatSet.texFormat, srvDesc.Format, rtvFormat,
                                 dsvFormat) == mTextureFormat);
#endif
}
    ...
    if (!index.hasLayer())
    {
        if (!mLevelRenderTargets[mipLevel])
        {
            ...
            mLevelRenderTargets[mipLevel] = new TextureRenderTarget11(
                rtv, texture, srv, mInternalFormat, mTextureFormat, getLevelWidth(mipLevel),
