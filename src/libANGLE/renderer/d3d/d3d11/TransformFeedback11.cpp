...
TransformFeedback11::TransformFeedback11(size_t maxBufferBindings)
    : mIsDirty(true),
      mBindings(maxBufferBindings),
      mBuffers(maxBufferBindings, nullptr),
      mBufferOffsets(maxBufferBindings, 0)
{
}

TransformFeedback11::~TransformFeedback11()
{
    for (size_t bindingIdx = 0; bindingIdx < mBindings.size(); bindingIdx++)
    {
        mBindings[bindingIdx].set(nullptr);
    }
}
...
gl::ErrorOrResult<const std::vector<ID3D11Buffer *> *> TransformFeedback11::getSOBuffers()
{
    ...
    {
        const auto &binding = mBindings[bindingIdx];
