...
void StateManager11::syncViewport(const gl::Context *context)
{
    ...
    for (UINT i = 0u; i < numRectangles; ++i)
    {
        ...
        {
            dxViewport.Width =
                static_cast<GLfloat>(std::min(dxViewportWidth, framebuffer->getDefaultWidth()));
            dxViewport.Height =
                static_cast<GLfloat>(std::min(dxViewportHeight, framebuffer->getDefaultHeight()));
        }
        else
        {
