...
void StateManager11::syncScissorRectangle(const gl::Rectangle &scissor, bool enabled)
{
    ...
    if (enabled)
    {
        ...
        rectangles.reserve(mViewportOffsets.size());
        ...
        mRenderer->getDeviceContext()->RSSetScissorRects(static_cast<UINT>(rectangles.size()),
                                                         rectangles.data());
    }
                                  ...
                                  float zFar)
{
    ...
    dxViewports.reserve(mViewportOffsets.size());
