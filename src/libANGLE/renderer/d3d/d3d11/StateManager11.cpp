                                                    ...
                                                    GLuint numGroupsZ)
{
    ...
    // TODO(jmadill): Use dirty bits.
    ANGLE_TRY_EXPR(generateSwizzlesForShader(context, gl::ShaderType::Compute));
                                          ...
                                          const gl::DrawCallParams &drawCallParams)
{
    ...
    ANGLE_TRY_EXPR(mFramebuffer11->markAttachmentsDirty(context));
    ...
    ANGLE_TRY_EXPR(mVertexArray11->syncStateForDraw(context, drawCallParams));
    ...
    for (auto dirtyBit : dirtyBitsCopy)
    {
        switch (dirtyBit)
        {
            case DIRTY_BIT_RENDER_TARGET:
                ANGLE_TRY_ERR(syncFramebuffer(context), context);
                break;
            ...
            case DIRTY_BIT_BLEND_STATE:
                ANGLE_TRY_ERR(syncBlendState(context, glState.getBlendState(),
...
{
    ANGLE_TRY_EXPR(generateSwizzles(context));
                                                        ...
                                                        gl::ShaderType type)
{
    ...
    for (unsigned int i = 0; i < samplerRange; i++)
    {
        ...
        if (textureUnit != -1)
        {
            ...
            {
                ANGLE_TRY_EXPR(generateSwizzle(context, texture));
            }
        }
    }
...
{
    ANGLE_TRY_EXPR(generateSwizzlesForShader(context, gl::ShaderType::Vertex));
