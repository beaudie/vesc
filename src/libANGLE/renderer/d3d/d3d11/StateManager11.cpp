...
StateManager11::StateManager11()
    : mBlendStateIsDirty(false),
      ...
      mCurFar(0.0f),
      mViewportBounds(),
      mCurDirectRenderingEnabled(false),
      mCurDirectRenderingColorBufferHeight(0),
...
void StateManager11::updateDirectRendering(bool directRenderingActive,
                                           const gl::Framebuffer *framebufferObject)
{
    const int colorBufferHeight = framebufferObject->getFirstColorbuffer()->getSize().height;

    if ((mCurDirectRenderingEnabled != directRenderingActive) ||
        (directRenderingActive && (colorBufferHeight != mCurDirectRenderingColorBufferHeight)))
    {
        mCurDirectRenderingEnabled           = directRenderingActive;
        mCurDirectRenderingColorBufferHeight = colorBufferHeight;
...
void StateManager11::setScissorRectangle(const gl::Rectangle &scissor, bool enabled)
{
    ...
    if (mCurDirectRenderingEnabled)
    {
        modifiedScissorY = mCurDirectRenderingColorBufferHeight - scissor.height - scissor.y;
    }
                                 ...
                                 float zFar)
{
    ...
    if (mCurDirectRenderingEnabled)
    {
        ...
        dxViewport.TopLeftY = static_cast<float>(mCurDirectRenderingColorBufferHeight -
    ...
    mPixelConstants.viewScale[1] = mCurDirectRenderingEnabled ? 1.0f : -1.0f;
