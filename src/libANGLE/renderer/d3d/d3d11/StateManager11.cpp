...
void StateManager11::syncViewport(const gl::Context *context, gl::Framebuffer *framebuffer)
{
    ...
    // When rendering to fbo with no attachments, the rendered pixels are discarded. Function
    // OMSetRenderTargets is called to set number of render target to 0.  In addition,  d3d
    // doesn't have parameters to set size of framebuffer, so we set the viewport size instead.
    // In order to avoid viewport set conflict, we have to judge if fbo has no color attachment
    // and default width or height is smaller than viewport params, then set viewport to the
    // smaller size. https://bugs.chromium.org/p/angleproject/issues/detail?id=1594&desc=2
...
gl::Error StateManager11::syncFramebuffer(const gl::Context *context, gl::Framebuffer *framebuffer)
{
    ...
    // Set default framebuffer width or height
    if (appliedRTIndex == 0)
    {
        syncViewport(context, framebuffer);
    }
