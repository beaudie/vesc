    ...
    const gl::FramebufferState &drawFramebufferState)
{
    switch (drawFramebufferState.getMultiviewLayout())
    {
        case GL_FRAMEBUFFER_MULTIVIEW_SIDE_BY_SIDE_ANGLE:
            mRenderer->updateMultiviewConstantBufferData(true);
            break;
        case GL_FRAMEBUFFER_MULTIVIEW_LAYERED_ANGLE:
            // Because the base view index is applied as an offset to the 2D texture array when the
            // RTV is created, we just have pass a boolean to select which code path is to be
            // selected.
            mRenderer->updateMultiviewConstantBufferData(false);
            break;
        default:
            break;
    }
