                                                   ...
                                                   gl::ShaderType shaderType)
{
    ...
    for (unsigned int readonlyImageRegister = readonlyImageRange.low();
         readonlyImageRegister < readonlyImageRange.high(); readonlyImageRegister++)
    {
        GLint imageUnitIndex =
            mProgramD3D->getImageUnitMapping(shaderType, readonlyImageRegister, true, caps);
        ASSERT(imageUnitIndex != -1);
        const gl::ImageUnit &imageUnit = glState.getImageUnit(imageUnitIndex);
        ANGLE_TRY(setImageState(context, shaderType, readonlyImageRegister, imageUnit));
        ANGLE_TRY(setTextureForImage(context, shaderType, readonlyImageRegister, true, imageUnit));
    }
                                                   ...
                                                   gl::ShaderType shaderType)
{
    ...
    for (unsigned int imageRegister = imageRange.low(); imageRegister < imageRange.high();
         imageRegister++)
    {
        GLint imageUnitIndex =
            mProgramD3D->getImageUnitMapping(shaderType, imageRegister, false, caps);
        ASSERT(imageUnitIndex != -1);
        const gl::ImageUnit &imageUnit = glState.getImageUnit(imageUnitIndex);
        ANGLE_TRY(setImageState(context, shaderType, imageRegister, imageUnit));
        ANGLE_TRY(setTextureForImage(context, shaderType, imageRegister, false, imageUnit));
    }
