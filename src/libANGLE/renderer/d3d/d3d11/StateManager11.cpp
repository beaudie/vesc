...
{
    mDirtyVertexBufferRange = gl::RangeUI(0, mRenderer->getNativeCaps().maxVertexAttributes);
}
...
void StateManager11::applyVertexBufferChanges()
{
    ...
    ASSERT(mDirtyVertexBufferRange.end < gl::MAX_VERTEX_ATTRIBS);

    UINT start = static_cast<UINT>(mDirtyVertexBufferRange.start);

    ID3D11DeviceContext *deviceContext = mRenderer->getDeviceContext();
    deviceContext->IASetVertexBuffers(start,
                                      static_cast<UINT>(mDirtyVertexBufferRange.length() + 1),
