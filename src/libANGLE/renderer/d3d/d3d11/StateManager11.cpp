    ...
    for (auto dirtyBit : dirtyBits)
    {
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_DRAW_FRAMEBUFFER_BINDING:
                ...
                {
                    // If ANGLE_multiview is enabled and a new draw framebuffer is bound, we have to
                    // update the state information before we go over the other dirty bits.
...
void StateManager11::handleDrawFramebufferChange(const gl::Context *context)
{
    ...
    // Update side-by-side boolean field.
    mIsSideBySideDrawFramebuffer =
        (drawFramebuffer->getMultiviewLayout() == GL_FRAMEBUFFER_MULTIVIEW_SIDE_BY_SIDE_ANGLE);
    ...
    mNumActiveViews = static_cast<int>(viewportOffsets.size());
...
void StateManager11::syncScissorRectangle(const gl::Rectangle &scissor, bool enabled)
{
    ...
    if (enabled)
    {
        ...
        mRenderer->getDeviceContext()->RSSetScissorRects(mNumActiveViews, rectangles.data());
    }
