...
StateManager11::StateManager11(Renderer11 *renderer)
    : mRenderer(renderer),
      ...
      mCurNear(0.0f),
      mCurFar(0.0f),
      mFramebufferDefaultWidth(0),
      mFramebufferDefaultHeight(0),
      mViewportBounds(),
        ...
        switch (dirtyBit)
        {
            ...
            case gl::State::DIRTY_BIT_VIEWPORT:
                if (state.getViewport() != mCurViewport ||
                    mFramebufferDefaultWidth != framebuffer->getDefaultWidth() ||
                    mFramebufferDefaultHeight != framebuffer->getDefaultHeight())
                {
                    mInternalDirtyBits.set(DIRTY_BIT_VIEWPORT_STATE);
                }
                break;
...
void StateManager11::syncViewport(const gl::Context *context)
{
    ...
    if (!framebuffer11->getActiveRenderTargetCount() &&
        !framebuffer11->getCachedDepthStencilRenderTarget() &&
