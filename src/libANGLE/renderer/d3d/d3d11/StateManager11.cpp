...
void StateManager11::syncScissorRectangle(const gl::Context *context)
{
    ...
    gl::Offset drawOffset;
    if (framebuffer->isDefault() && framebuffer->getFirstColorAttachment()->getSurface())
    {
        drawOffset = framebuffer->getFirstColorAttachment()->getSurface()->getDrawOffset();
    }

    int scissorX = scissor.x + drawOffset.x;
    int scissorY = scissor.y + drawOffset.y;
...
void StateManager11::syncViewport(const gl::Context *context)
{
    ...
    dxViewportTopLeftX =
        gl::clamp(viewport.x + drawOffset.x, dxMinViewportBoundsX, dxMaxViewportBoundsX);
    dxViewportTopLeftY =
        gl::clamp(viewport.y + drawOffset.y, dxMinViewportBoundsY, dxMaxViewportBoundsY);
...
void StateManager11::processFramebufferInvalidation(const gl::Context *context)
{
    ...
    // Dirty scissor and viewport because surface draw offset might have changed.
    mInternalDirtyBits.set(DIRTY_BIT_SCISSOR_STATE);
