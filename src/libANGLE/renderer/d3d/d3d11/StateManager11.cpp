                                              ...
                                              const gl::SamplerState &samplerState)
{
    ...
    const bool clampToBorder = samplerState.getWrapS() == GL_CLAMP_TO_BORDER ||
                               samplerState.getWrapT() == GL_CLAMP_TO_BORDER ||
                               samplerState.getWrapR() == GL_CLAMP_TO_BORDER;

    if (mForceSetShaderSamplerStates[type][index] || clampToBorder ||
        memcmp(&samplerState, &mCurShaderSamplerStates[type][index], sizeof(gl::SamplerState)) != 0)
    {
        ...
        if (clampToBorder && samplerState.getBorderColor().type == angle::ColorGeneric::Type::Float)
        {
            const uint32_t baseLevel = texture->getTextureState().getEffectiveBaseLevel();
            const gl::TextureTarget target =
                (texture->getType() == gl::TextureType::CubeMap)
                    ? gl::kCubeMapTextureTargetMin
                    : gl::NonCubeTextureTypeToTarget(texture->getType());
            ...
            borderColor =
                AdjustBorderColor<false>(samplerState.getBorderColor(), format, false).colorF;
        }
        gl::SamplerState adjustedSamplerState(samplerState);
        adjustedSamplerState.setBorderColor(borderColor);
