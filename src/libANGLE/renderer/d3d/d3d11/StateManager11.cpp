...
gl::Error StateManager11::syncFramebuffer(gl::Framebuffer *framebuffer)
{
    ...
    for (size_t rtIndex = 0; rtIndex < colorRTs.size(); ++rtIndex)
    {
        ...
        if (renderTarget)
        {
            ...
            maxExistingRT = static_cast<UINT>(appliedRTIndex);
    ...
    ASSERT(maxExistingRT < mRenderer->getNativeCaps().maxDrawBuffers);

    // Apply the render target and depth stencil
    mRenderer->getDeviceContext()->OMSetRenderTargets(maxExistingRT + 1, framebufferRTVs.data(),
                                                      framebufferDSV);
