      ...
      mFramebuffer11(nullptr)
{
    ...
    mInternalDirtyBits.reset(DIRTY_BIT_RENDER_SRVUAV_STATE);
        ...
        switch (shaderType)
        {
            case gl::ShaderType::Vertex:
                if (srvPtr)
                {
                    ...
                    unsetConflictingUAVs(gl::PipelineType::RenderPipeline, gl::ShaderType::Compute,
                                         resource, nullptr);
                }
                                                    ...
                                                    GLuint numGroupsZ)
{
    ...
    if (dirtyBitsCopy.test(DIRTY_BIT_TEXTURE_AND_SAMPLER_STATE) &&
        dirtyBitsCopy.test(DIRTY_BIT_COMPUTE_SRVUAV_STATE))
    {
        dirtyBitsCopy.reset(DIRTY_BIT_COMPUTE_SRVUAV_STATE);
    }
...
void StateManager11::unsetConflictingView(gl::PipelineType pipeline, ID3D11View *view)
{
    uintptr_t resource = reinterpret_cast<uintptr_t>(GetViewResource(view));

    unsetConflictingSRVs(pipeline, gl::ShaderType::Vertex, resource, nullptr);
    unsetConflictingSRVs(pipeline, gl::ShaderType::Fragment, resource, nullptr);
    unsetConflictingSRVs(pipeline, gl::ShaderType::Compute, resource, nullptr);
                                          ...
                                          const gl::ImageIndex *index)
{
    ...
    if (foundOne && pipeline != conflictPipeline)
    {
        switch (shaderType)
        {
            case gl::ShaderType::Vertex:
            case gl::ShaderType::Fragment:
                mInternalDirtyBits.set(DIRTY_BIT_RENDER_SRVUAV_STATE);
                break;
            case gl::ShaderType::Compute:
                                          ...
                                          const gl::ImageIndex *index)
{
    ...
    if (foundOne && pipeline == gl::PipelineType::RenderPipeline)
    {
        mInternalDirtyBits.set(DIRTY_BIT_COMPUTE_SRVUAV_STATE);
    }
}
    ...
    ID3D11Resource *resource)
{
    ...
    if (attachment.type() == GL_TEXTURE)
    {
        ...
        unsetConflictingSRVs(gl::PipelineType::RenderPipeline, gl::ShaderType::Fragment,
                             resourcePtr, &index);
        unsetConflictingSRVs(gl::PipelineType::RenderPipeline, gl::ShaderType::Compute, resourcePtr,
                             &index);
                                          ...
                                          GLint baseVertex)
{
    ...
    if (dirtyBitsCopy.test(DIRTY_BIT_COMPUTE_SRVUAV_STATE))
    {
        dirtyBitsCopy.reset(DIRTY_BIT_COMPUTE_SRVUAV_STATE);
        mInternalDirtyBits.set(DIRTY_BIT_COMPUTE_SRVUAV_STATE);
    }
    ...
    if (dirtyBitsCopy.test(DIRTY_BIT_TEXTURE_AND_SAMPLER_STATE) &&
        dirtyBitsCopy.test(DIRTY_BIT_RENDER_SRVUAV_STATE))
    {
        dirtyBitsCopy.reset(DIRTY_BIT_RENDER_SRVUAV_STATE);
    }
    ...
    for (auto dirtyBit : dirtyBitsCopy)
    {
        switch (dirtyBit)
        {
            ...
            case DIRTY_BIT_DEPTH_STENCIL_STATE:
                ...
                break;
            case DIRTY_BIT_TEXTURE_AND_SAMPLER_STATE:
            case DIRTY_BIT_RENDER_SRVUAV_STATE:
    ...
    ASSERT(mInternalDirtyBits.none() || mInternalDirtyBits.test(DIRTY_BIT_COMPUTE_SRVUAV_STATE) &&
