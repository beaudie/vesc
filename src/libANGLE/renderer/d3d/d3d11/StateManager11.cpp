...
void StateManager11::syncViewport(const gl::Context *context)
{
    ...
    ASSERT(dxViewport.Width == dxViewportWidth && dxViewport.Height == dxViewportHeight);
    ASSERT(dxViewportTopLeftX == dxViewport.TopLeftX && dxViewportTopLeftY == dxViewport.TopLeftY);
    mShaderConstants.onViewportChange(viewport, dxViewport, actualZFar, actualZNear, is9_3,
...
gl::Error StateManager11::applyComputeUniforms(ProgramD3D *programD3D)
{
    ...
    {
        ID3D11Buffer *computeConstantBuffer = constantBuffer->get();
