...
void StateManager11::invalidateRenderTarget(const gl::Context *context)
{
    ...
    // The context can be null if called from EGL initialization code.
    if (!context)
    {
        return;
    }
    ...
    mCurFramebufferSamples            = 0;
    Framebuffer11 *fbo11              = GetImplAs<Framebuffer11>(fbo);
    RenderTarget11 *firstRenderTarget = fbo11->getFirstRenderTarget();
    if (firstRenderTarget)
    {
        mCurFramebufferSamples = firstRenderTarget->getSamples();
    }

    bool multiSample = (mCurFramebufferSamples.value() != 0);
...
gl::Error StateManager11::updateState(const gl::Context *context, GLenum drawMode)
{
    ...
    ASSERT(mCurFramebufferSamples.valid());
    unsigned int sampleMask = GetBlendSampleMask(data, mCurFramebufferSamples.value());
    ...
    for (auto dirtyBit : dirtyBitsCopy)
    {
        switch (dirtyBit)
        {
            ...
            case DIRTY_BIT_BLEND_STATE:
                ANGLE_TRY(syncBlendState(framebuffer, glState.getBlendState(),
...
gl::Error StateManager11::syncProgram(const gl::Context *context, GLenum drawMode)
{
    ...
    const auto *drawFramebuffer = glState.getDrawFramebuffer();
