...
{
    const gl::ShaderBitSet &linkedShaderStages = mProgramD3D->getLinkedShaderStages();
    for (gl::ShaderType shaderType : gl::kAllGraphicsShaderTypes)
    {
        if (linkedShaderStages[shaderType])
        {
            ANGLE_TRY(applyTextures(context, shaderType));
        }
    }
...
gl::Error StateManager11::applyDriverUniforms()
{
    ...
    for (gl::ShaderType shaderType : gl::kAllGraphicsShaderTypes)
    {
        ...
        if (!shaderDriverConstantBuffer.valid())
        {
            ...
            ANGLE_TRY(mRenderer->allocateResource(constantBufferDescription,
            ...
            switch (shaderType)
            {
                case gl::ShaderType::Vertex:
                    ...
                    break;
                case gl::ShaderType::Fragment:
                    deviceContext->PSSetConstantBuffers(d3d11::RESERVED_CONSTANT_BUFFER_SLOT_DRIVER,
                                                        1, &driverConstants);
                    break;
                case gl::ShaderType::Geometry:
                    deviceContext->GSSetConstantBuffers(d3d11::RESERVED_CONSTANT_BUFFER_SLOT_DRIVER,
                                                        1, &driverConstants);
                    break;
                default:
                    UNREACHABLE();
