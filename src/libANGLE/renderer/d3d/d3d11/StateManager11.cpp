                                                   ...
                                                   gl::ShaderType shaderType)
{
         ...
         readonlyImageIndex < readonlyImageRange.high(); readonlyImageIndex++)
    {
        ...
        ANGLE_TRY(setTextureForReadonlyImage(context, shaderType, readonlyImageIndex, imageUnit));
    }

    return angle::Result::Continue;
}

angle::Result StateManager11::setUAVsForRWImages(const gl::Context *context,
                                                 gl::ShaderType shaderType,
                                                 UAVList *uavList)
{
    const auto &glState = context->getState();
    ...
    for (unsigned int imageIndex = imageRange.low(); imageIndex < imageRange.high(); imageIndex++)
    {
        ...
        ANGLE_TRY(setUAVForRWImage(context, shaderType, imageIndex, imageUnit, uavList));
    }
...
angle::Result StateManager11::setTextureForReadonlyImage(const gl::Context *context,
                                                         gl::ShaderType type,
                                                         int index,
                                                         const gl::ImageUnit &imageUnit)
{
...
angle::Result StateManager11::setUAVForRWImage(const gl::Context *context,
                                               gl::ShaderType type,
                                               int index,
                                               const gl::ImageUnit &imageUnit,
                                               UAVList *uavList)
{
    ...
    if (!imageUnit.texture.get())
    {
        setUnorderedAccessViewInternal<d3d11::UnorderedAccessView>(type, static_cast<UINT>(index),
                                                                   nullptr, nullptr);
        return angle::Result::Continue;
    }
    ...
    ASSERT((index < mRenderer->getNativeCaps().maxImageUnits));
    setUnorderedAccessViewInternal(type, index, textureUAV, uavList);
                                                                ...
                                                                UAVList *uavList)
{
         ...
         blockIndex++)
    {
        ...
        switch (shaderType)
        {
            ...
            case gl::ShaderType::Compute:
            {
                setUnorderedAccessViewInternal(shaderType, registerIndex, uavPtr, uavList);
                break;
            }
...
angle::Result StateManager11::syncUAVsForGraphics(const gl::Context *context)
{
    ...
    ANGLE_TRY(syncShaderStorageBuffersForShader(context, gl::ShaderType::Fragment, &uavList));
    ANGLE_TRY(syncShaderStorageBuffersForShader(context, gl::ShaderType::Vertex, &uavList));
    ANGLE_TRY(setUAVsForRWImages(context, gl::ShaderType::Fragment, &uavList));
    ANGLE_TRY(setUAVsForRWImages(context, gl::ShaderType::Vertex, &uavList));
    ...
    if (uavList.highestUsed >= 0)
    {
        ...
        deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(
            ...
            uavList.highestUsed + 1, uavList.data(), nullptr);
    }
...
angle::Result StateManager11::syncUAVsForCompute(const gl::Context *context)
{
    ...
    ANGLE_TRY(syncShaderStorageBuffersForShader(context, gl::ShaderType::Compute, &uavList));
    ANGLE_TRY(setUAVsForRWImages(context, gl::ShaderType::Compute, &uavList));
    ANGLE_TRY(syncAtomicCounterBuffersForShader(context, gl::ShaderType::Compute, &uavList));

    if (uavList.highestUsed >= 0)
    {
        ID3D11DeviceContext *deviceContext = mRenderer->getDeviceContext();
        deviceContext->CSSetUnorderedAccessViews(0, uavList.highestUsed + 1, uavList.data(),
                                                 nullptr);
    }
