...
gl::Error StateManager11::applyTexturesForCompute(const gl::Context *context)
{
    ...
    unsigned int readonlyImageRange = programD3D->getUsedReadonlyImageRange(SHADER_COMPUTE);
    for (unsigned int readonlyImageIndex = 0; readonlyImageIndex < readonlyImageRange;
         readonlyImageIndex++)
    {
        GLint imageUnitIndex =
            programD3D->getReadonlyImageMapping(SHADER_COMPUTE, readonlyImageIndex, caps);
        if (imageUnitIndex != -1)
        {
            const gl::ImageUnit &imageUnit = glState.getImageUnit(imageUnitIndex);
            ANGLE_TRY(setTextureForImage(context, SHADER_COMPUTE, readonlyImageIndex, imageUnit));
        }
        else
        {
            ANGLE_TRY(
                setTextureForImage(context, SHADER_COMPUTE, readonlyImageIndex, gl::ImageUnit()));
        }
    }

    unsigned int imageRange = programD3D->getUsedImageRange(SHADER_COMPUTE);
    for (unsigned int imageIndex = 0; imageIndex < imageRange; imageIndex++)
    {
        GLint imageUnitIndex = programD3D->getImageMapping(SHADER_COMPUTE, imageIndex, caps);
        if (imageUnitIndex != -1)
        {
            const gl::ImageUnit &imageUnit = glState.getImageUnit(imageUnitIndex);
            ANGLE_TRY(setRWTextureForImage(context, SHADER_COMPUTE, imageIndex, imageUnit));
        }
        else
        {
            ANGLE_TRY(setRWTextureForImage(context, SHADER_COMPUTE, imageIndex, gl::ImageUnit()));
        }
    }
                                             ...
                                             const gl::ImageUnit &imageUnit)
{
    ...
    else
    {
        const auto &glState    = context->getGLState();
        ProgramD3D *programD3D = GetImplAs<ProgramD3D>(glState.getProgram());
        GLenum textureType     = programD3D->getImageTextureType(type, index);
...
gl::Error StateManager11::setRWTextureForImage(const gl::Context *context,
                                               ShaderType type,
                                               int index,
                                               const gl::ImageUnit &imageUnit)
{
