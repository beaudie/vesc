                                           ...
                                           TranslatedIndexData *indexInfo)
{
    ...
    if (!vao11->updateElementArrayStorage(context, type, destElementType, indices) && !mIndexBufferIsDirty)
    {
...
void DrawCallVertexParams::ensureResolved() const
{
    if (mHasIndexRange)
    {
        // Resolve the index range now if we need to. Note that in some cases the first vertex may
        // be valid, but we need to compute the full index range for a "vertexCount" query.
