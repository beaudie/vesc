...
void StateManager11::SRVCache::update(size_t resourceIndex, ID3D11ShaderResourceView *srv)
{
    ...
    if (srv)
    {
        record->resource = reinterpret_cast<uintptr_t>(d3d11::GetViewResource(srv));
...
void StateManager11::unsetConflictingAttachmentResources(
    ...
    ID3D11View *resourceView)
{
    // Unbind render target SRVs from the shader here to prevent D3D11 warnings.
    if (attachment->type() == GL_TEXTURE)
    {
        uintptr_t resourcePtr       = reinterpret_cast<uintptr_t>(d3d11::GetViewResource(resourceView));
...
gl::Error StateManager11::syncFramebuffer(const gl::Framebuffer *framebuffer)
{
    ...
    for (size_t rtIndex = 0; rtIndex < colorRTs.size(); ++rtIndex)
    {
        ...
        ASSERT(attachment);
        unsetConflictingAttachmentResources(attachment, framebufferRTVs[appliedRTIndex]);
    ...
    if (depthStencilRenderTarget)
    {
        ...
        ASSERT(attachment);
        unsetConflictingAttachmentResources(attachment, framebufferDSV);
    }
