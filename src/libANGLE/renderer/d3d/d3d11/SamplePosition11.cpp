...
gl::Error SamplePosition11::getSample(const gl::Context *context,
                                      ...
                                      GLfloat *xy)
{
    // samplePositionsMap preserve queried sample positions, translate D3D's sample coordinate to
    // OpenGL
    int sampleCount = attachmentRenderTarget->getSamples();
    auto iter       = mSamplePositionsMap.find(sampleCount);
    if (iter != mSamplePositionsMap.end())
    {
        xy[0] = iter->second.get()[index].x + 0.5f;
        xy[1] = iter->second.get()[index].y + 0.5f;
        return gl::NoError();
    }
    ...
    D3D11_BUFFER_DESC samplePositionsBufferDesc   = {0};
    samplePositionsBufferDesc.BindFlags           = D3D11_BIND_UNORDERED_ACCESS;
    samplePositionsBufferDesc.ByteWidth           = sizeof(SamplePosition) * sampleCount;
    samplePositionsBufferDesc.StructureByteStride = sizeof(SamplePosition);
    samplePositionsBufferDesc.MiscFlags           = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
    ANGLE_TRY(mRenderer->allocateResource(samplePositionsBufferDesc, &mSamplePositionsBuffer));
    mSamplePositionsBuffer.setDebugName("SamplePosition read buffer");
    ...
    mRenderer->allocateResource(uav_desc, mSamplePositionsBuffer.get(), &mSamplePositionUAV);
    mSamplePositionUAV.setDebugName("SamplePosition buffer result uav");
    ID3D11UnorderedAccessView *uav = mSamplePositionUAV.get();
    ...
    D3D11_MAPPED_SUBRESOURCE MappedResource;
    d3d11::Buffer resultBuffer;
    D3D11_BUFFER_DESC result_desc = {0};
    mSamplePositionsBuffer.get()->GetDesc(&result_desc);
    ...
    mRenderer->allocateResource(result_desc, &resultBuffer);
    resultBuffer.setDebugName("SamplePosition result buffer");

    deviceContext->CopyResource(resultBuffer.get(), mSamplePositionsBuffer.get());

    result = deviceContext->Map(resultBuffer.get(), 0, D3D11_MAP_READ, 0, &MappedResource);
    if (FAILED(result))
    {
    ...
    SamplePosition *pData = (SamplePosition *)(MappedResource.pData);
    // Translate D3D's sample coordinate to OpenGL
    xy[0] = pData[index].x + 0.5f;
    xy[1] = pData[index].y + 0.5f;

    mSamplePositionsMap[sampleCount].reset((new SamplePosition[sampleCount]));
    std::memcpy(mSamplePositionsMap[sampleCount].get(), pData,
                sizeof(SamplePosition) * sampleCount);
    deviceContext->Unmap(mSamplePositionsBuffer.get(), 0);
