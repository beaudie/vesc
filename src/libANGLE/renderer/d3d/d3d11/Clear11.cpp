                                    ...
                                    const gl::FramebufferState &fboData)
{
    ...
    if (!(needScissoredClear && isSideBySideFBO))
    {
        ...
        // framebuffers because: - For layered clears we only select the layer in the GS, but the
        // viewport remains the same. - For side-by-side clears we draw one quad which covers the
        // whole framebuffer. For non-multiview framebuffers we always have one view, and thus we
        // set only one viewport.
    ...
    if (hasMultiviewLayout)
    {
        ...
        // shader stage enabled where it selected either the viewport index or the RT array index.
