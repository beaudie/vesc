            ...
            drawBufferStates[colorAttachmentIndex] != GL_NONE)
        {
                  ...
                  formatInfo.componentType == GL_SIGNED_NORMALIZED))
            {
                ERR() << "It is undefined behaviour to clear a render buffer which is not "
                         ...
                         "attachment "
                      << colorAttachmentIndex << " has internal format 0x" << std::hex
                      << std::uppercase << attachment.getFormat().asSized() << ").";
            }
...
ID3D11BlendState *Clear11::getBlendState(const std::vector<MaskedRenderTarget> &rts)
{
    ...
    else
    {
        ...
        {
            ERR() << "Unable to create a ID3D11BlendState, HRESULT: 0x" << std::hex
...
ID3D11DepthStencilState *Clear11::getDepthStencilState(const ClearParameters &clearParams)
{
    ...
    else
    {
        ...
        {
            ERR() << "Unable to create a ID3D11DepthStencilState, HRESULT: 0x" << std::hex
                  << std::uppercase << result << ".";
            return nullptr;
        }
