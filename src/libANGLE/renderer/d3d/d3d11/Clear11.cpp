...
template <typename T>
bool UpdateShaderDataCache(void *buffer,
                           const gl::Color<T> &color,
                           const float *zValue,
                           const uint32_t numRtvs,
                           const uint8_t writeMask)
{
    RtvDsvClearInfo<T> *dataCache = reinterpret_cast<RtvDsvClearInfo<T> *>(buffer);
      ...
      mShaderDataCache({})
{
    ...
    initialData.pSysMem          = &mShaderDataCache;
                                    ...
                                    const gl::FramebufferState &fboData)
{
    ...
    {
        case GL_FLOAT:
            dirtyCb = UpdateShaderDataCache(&mShaderDataCache, clearParams.colorF, zValue, numRtvs,
                                            colorMask);
            break;
        case GL_UNSIGNED_INT:
            dirtyCb = UpdateShaderDataCache(&mShaderDataCache, clearParams.colorUI, zValue, numRtvs,
                                            colorMask);
            break;
        case GL_INT:
            dirtyCb = UpdateShaderDataCache(&mShaderDataCache, clearParams.colorI, zValue, numRtvs,
                                            colorMask);
            break;
        default:
            UNREACHABLE();
    ...
    if (dirtyCb)
    {
        ...
        memcpy(mappedResource.pData, &mShaderDataCache, g_ConstantBufferSize);
