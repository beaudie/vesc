                                    ...
                                    const gl::FramebufferState &fboData)
{
    ...
    if (!needScissoredClear)
    {
        // If a scissored clear is not necessary, then we have need of only one viewport.
    ...
    else
    {
        ...
        int numViews = fboData.getNumViews();
        std::vector<D3D11_VIEWPORT> viewports(fboData.getNumViews());
