...
ID3D11Buffer *Buffer11::getEmulatedIndexedBuffer(BufferUsage usage, TranslatedIndexData *indexInfo, TranslatedAttribute *attribute)
{
    ...
    if (!mEmulatedIndexedBuffer)
    {
        ...
        uint8_t *expandedData = new uint8_t[expandedDataSize];
        if (!expandedData)
        {
            // Storage out-of-memory
            SafeDelete(mEmulatedIndexedBuffer);
        ...
        ZeroMemory(expandedData, expandedDataSize);

        uint8_t *curr = expandedData;
        ...
        error = mEmulatedIndexedBuffer->setData(expandedData, expandedDataSize, BUFFER_USAGE_SYSTEM_MEMORY);
        SafeDeleteArray(expandedData);
        if (error.isError())
        {
            // Storage out-of-memory
            SafeDelete(mEmulatedIndexedBuffer);
