                                                    ...
                                                    const d3d11::ShaderResourceView **srvOut)
{
    ...
    if (newStorage->getSize() < static_cast<size_t>(mSize))
    {
        size_t maximumAllowedAdditionalSize = 2 * getSize();

        size_t sizeDelta = mSize - newStorage->getSize();
        ...
        ANGLE_TRY(structuredBufferStorage->resizeStructuredBuffer(
            context, static_cast<unsigned int>(mSize), false, structureByteStride));
    ...
    ANGLE_TRY(updateBufferStorage(context, newStorage, offset, mSize));
    ...
    const d3d11::ShaderResourceView **srvOut)
{
    ...
    D3D11_SHADER_RESOURCE_VIEW_DESC bufferSRVDesc;
    bufferSRVDesc.BufferEx.NumElements  = size / structureByteStride;
