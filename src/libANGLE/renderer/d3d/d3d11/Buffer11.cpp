...
{
  public:
    PackStorage(Renderer11 *renderer);
...
class Buffer11::SystemMemoryStorage : public Buffer11::BufferStorage
{
  ...
  protected:
    DISALLOW_COPY_AND_ASSIGN(Buffer11::SystemMemoryStorage);
...
gl::Error Buffer11::getSystemMemoryStorage(SystemMemoryStorage **storageOut)
{
    ...
    ASSERT(HAS_DYNAMIC_TYPE(SystemMemoryStorage*, memStorageUntyped));
    SystemMemoryStorage *systemMemoryStorage = static_cast<SystemMemoryStorage*>(memStorageUntyped);

    if (systemMemoryStorage == nullptr)
    {
        // TODO(jmadill): convert all to errors
        return gl::Error(GL_OUT_OF_MEMORY);
    }

    *storageOut = systemMemoryStorage;
    return gl::Error(GL_NO_ERROR);
}
...
{
    ASSERT(!mSystemCopy.empty());
    return mSystemCopy.data();
}
