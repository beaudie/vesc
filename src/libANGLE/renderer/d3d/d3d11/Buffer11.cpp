...
ID3D11Buffer *Buffer11::EmulatedIndexedStorage::getNativeStorage()
{
    if (!mNativeStorage)
    {
        ...
        unsigned int indexStride = sizeof(GLushort);
        switch (mIndexInfo.srcIndexType)
        {
          case GL_UNSIGNED_INT: indexStride = sizeof(GLuint); break;
          case GL_UNSIGNED_SHORT: indexStride = sizeof(GLushort); break;
          case GL_UNSIGNED_BYTE: indexStride = sizeof(GLubyte); break;
        }
        ...
        {
            GLuint idx = 0;
            memcpy(&idx, ptr + (i * indexStride), indexStride);
