...
class Buffer11::TranslatedIndirectStorage : public Buffer11::BufferStorage
{
  ...
  private:
    ID3D11Buffer *mNativeStorage;  // contains the translated indirect data for use by d3d11
...
Buffer11::TranslatedIndirectStorage::TranslatedIndirectStorage(Renderer11 *renderer)
    : BufferStorage(renderer, BUFFER_USAGE_TRANSLATED_INDIRECT), mNativeStorage(nullptr)
{
}
    ...
    unsigned int size)
{
    ...
    if (!mNativeStorage)
    {
        ...
        if (FAILED(result))
        {
            return gl::Error(GL_OUT_OF_MEMORY, "Could not create translated indirect buffer: %08lX",
                             result);
        }
...
gl::Error Buffer11::TranslatedIndirectStorage::resize(size_t size, bool preserveData)
{
    if (mMemoryBuffer.size() < size)
    {
        if (!mMemoryBuffer.resize(size))
        {
            return gl::Error(GL_OUT_OF_MEMORY, "Failed to resize TranslatedIndirectStorage");
        }
        mBufferSize = size;
    }
