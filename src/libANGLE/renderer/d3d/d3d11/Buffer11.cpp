                               ...
                               size_t offset)
{
    ...
    if (data && size > 0)
    {
        ...
        makeLatestStorage(writeBuffer, writeBuffer);
                                ...
                                GLsizeiptr size)
{
    ...
    ANGLE_TRY_RESULT(copyDest->copyFromStorage(context, copySource, sourceOffset, size, destOffset),
                     copyResult);
    makeLatestStorage(copyDest, copyDest);
                             ...
                             void **mapPtr)
{
    ...
    if ((access & GL_MAP_WRITE_BIT) > 0)
    {
        ...
        makeLatestStorage(mMappedStorage, mMappedStorage);
        invalidateStaticData(context);
    }
...
gl::Error Buffer11::markTransformFeedbackUsage(const gl::Context *context)
{
    ...
    {
        makeLatestStorage(transformFeedbackStorage, transformFeedbackStorage);
    }
                               ...
                               const PackPixelsParams &params)
{
    ...
    makeLatestStorage(packStorage, latestStorage);
    ...
    if (latestBuffer && latestBuffer->getDataRevision() > storage->getDataRevision())
    {
             ...
             !storage->isCPUAccessible(GL_MAP_WRITE_BIT)))
        {
            ...
            ANGLE_TRY_RESULT(stagingBuffer->copyFromStorage(context, latestBuffer, 0,
                             ...
                             copyResult);
            makeLatestStorage(stagingBuffer, latestBuffer);
        ...
        makeLatestStorage(storage, latestBuffer);
    }
...
void Buffer11::makeLatestStorage(BufferStorage *newLatest, BufferStorage *previousLatest)
{
    newLatest->setDataRevision(previousLatest ? (previousLatest->getDataRevision() + 1) : 1);
    mLatestBufferStorage = newLatest;
}
