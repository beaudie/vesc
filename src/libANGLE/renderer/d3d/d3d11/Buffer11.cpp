...
class Buffer11::NativeStorage : public Buffer11::BufferStorage
{
  public:
    NativeStorage(Renderer11 *renderer,
                  BufferUsage usage,
                  OnBufferDataDirtyChannel *onStorageChanged);
  ...
  private:
    ...
    OnBufferDataDirtyChannel *mOnStorageChanged;
...
gl::Error Buffer11::setData(const gl::Context *context,
                            ...
                            GLenum usage)
{
    ANGLE_TRY(updateD3DBufferUsage(context, usage));
                               ...
                               size_t offset)
{
    ...
    if (data && size > 0)
    {
        ...
        ANGLE_TRY(mDirectBroadcastChannel.signal(context));
    }

    mSize = std::max(mSize, requiredSize);
    ANGLE_TRY(invalidateStaticData(context));
...
gl::Error Buffer11::initializeStaticData(const gl::Context *context)
{
    ANGLE_TRY(BufferD3D::initializeStaticData(context));

    // Notify when static data changes.
    ANGLE_TRY(mStaticBroadcastChannel.signal(context));
    return gl::NoError();
}

gl::Error Buffer11::invalidateStaticData(const gl::Context *context)
{
    ANGLE_TRY(BufferD3D::invalidateStaticData(context));
                                                                       ...
                                                                       size_t destOffset)
{
        ...
        source->getUsage() == BUFFER_USAGE_SYSTEM_MEMORY)
    {
        ...
        // Swallow the error so we call unmap.
        ANGLE_SWALLOW_ERR(setData(sourcePointer, destOffset, clampedSize));
                                          ...
                                          bool preserveData)
{
    ...
    {
        ANGLE_TRY(mOnStorageChanged->signal(context));
    }
