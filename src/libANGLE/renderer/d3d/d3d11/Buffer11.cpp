...
D3D11_MAP GetD3DMapTypeFromBits(BufferUsage usage, GLbitfield access)
{
    ...
    else if (writeBit && !readBit)
    {
        ...
        return usage == BUFFER_USAGE_UNIFORM || usage == BUFFER_USAGE_STRUCURED
                                                    ...
                                                    const d3d11::ShaderResourceView **srvOut)
{
...

    {
        // Keep the cacheEntry in a limited scope because it may be invalidated later in the code if
        // we need to reclaim some space.
        StructuredBufferCacheEntry *cacheEntry = &mStructuredBufferRangeStoragesCache[offset];

        if (!cacheEntry->storage || cacheEntry->structureByteStride != structureByteStride)
        {
            cacheEntry->storage             = allocateStorage(BUFFER_USAGE_STRUCURED);
            cacheEntry->lruCount            = ++mMaxStructuredBufferLruCount;
            cacheEntry->structureByteStride = structureByteStride;
        }
    ...
    markBufferUsage(BUFFER_USAGE_STRUCURED);

    // If size is zero, it means should bind complete buffer.
    size_t bufferSize = size == 0u ? mSize : static_cast<size_t>(size);
    if (newStorage->getSize() < bufferSize)
    {
        size_t maximumAllowedAdditionalSize = 2 * getSize();

        size_t sizeDelta = bufferSize - newStorage->getSize();

        while (mStructuredBufferStorageAdditionalSize + sizeDelta > maximumAllowedAdditionalSize)
        {
            auto iter = std::min_element(
                std::begin(mStructuredBufferRangeStoragesCache),
        ...
        ANGLE_TRY(structuredBufferStorage->resizeStructuredBuffer(
            context, static_cast<unsigned int>(bufferSize), false, structureByteStride));
    ...
    ANGLE_TRY(updateBufferStorage(context, newStorage, offset, bufferSize));
    ANGLE_TRY(garbageCollection(context, BUFFER_USAGE_STRUCURED));
    ANGLE_TRY(structuredBufferStorage->getStructuredBufferRangeSRV(
        context, offset, static_cast<unsigned int>(mSize), structureByteStride, srvOut));
    return angle::Result::Continue;
}
    ...
    const d3d11::ShaderResourceView **srvOut)
{
    ...
    D3D11_SHADER_RESOURCE_VIEW_DESC bufferSRVDesc = {};
    bufferSRVDesc.Buffer.NumElements              = size / structureByteStride;
