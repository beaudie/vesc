...
EGLint SwapChain11::resetOffscreenColorBuffer(int backbufferWidth, int backbufferHeight)
{
    ...
    std::cout << "-->resetOffscreenColorBuffer()\n";
...
{        std::cout << "-->resetOffScreenDepthBuffer()\n";
...
{std::cout << "-->initPassThroughResources()\n";
    ...
    std::cout << "----CreateRasterizerState()\n";
...
EGLint SwapChain11::copyOffscreenToBackbuffer(EGLint x, EGLint y, EGLint width, EGLint height)
{
    ...
    // Before that, we perform any cleanup on the D3D /. We do this before Present() to make sure the
