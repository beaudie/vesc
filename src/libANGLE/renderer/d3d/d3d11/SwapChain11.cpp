...
EGLint SwapChain11::resize(EGLint backbufferWidth, EGLint backbufferHeight)
{
    ...
    HRESULT result = mSwapChain->ResizeBuffers(desc.BufferCount, backbufferWidth, backbufferHeight, getValidSwapChainFormat(), 0);
...
DXGI_FORMAT SwapChain11::getValidSwapChainFormat()
{
    // The render target format used for offscreen rendering may not be compatible to use for swapchain creation.
    // A compatible 32-bit format must be chosen.
...
EGLint SwapChain11::reset(int backbufferWidth, int backbufferHeight, EGLint swapInterval)
{
    ...
    {
        HRESULT result = mNativeWindow.createSwapChain(device, mRenderer->getDxgiFactory(), getValidSwapChainFormat(), backbufferWidth, backbufferHeight, &mSwapChain);
