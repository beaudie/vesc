...
class StateManager11 final : angle::NonCopyable
{
  ...
  private:
    ...
    // A cache of current UAVs that also tracks the highest 'used' (non-NULL) UAV
    // We might want to investigate a more robust approach that is also fast when there's
    // a large gap between used UAVs (e.g. if UAV 0 and 7 are non-NULL, this approach will
    // waste time on UAVs 1-6.)
