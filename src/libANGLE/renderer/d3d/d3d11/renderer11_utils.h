...
{
  public:
    LazyInputLayout(const D3D11_INPUT_ELEMENT_DESC inputDesc[N],
                    const BYTE *byteCode,
                    size_t byteCodeLen,
...
template <size_t N>
LazyInputLayout<N>::LazyInputLayout(
    const D3D11_INPUT_ELEMENT_DESC inputDesc[N],
    const BYTE *byteCode,
    size_t byteCodeLen,
    const char *debugName)
...
template <size_t N>
inline ID3D11InputLayout *LazyInputLayout<N>::resolve(ID3D11Device *device)
{
    checkAssociatedDevice(device);

    if (mResource == nullptr)
    {
        HRESULT result = device->CreateInputLayout(mInputDesc, N, mByteCode, mByteCodeLen, &mResource);
        ASSERT(SUCCEEDED(result));
