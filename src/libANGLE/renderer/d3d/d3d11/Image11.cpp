...
bool Image11::redefine(RendererD3D *renderer, GLenum target, GLenum internalformat, const gl::Extents &size, bool forceRelease)
{
...
gl::Error Image11::loadData(const gl::Box &area, GLint unpackAlignment, GLenum type, const void *input)
{
    ...
    uint8_t* offsetMappedData = (reinterpret_cast<uint8_t*>(mappedImage.pData) + (area.y * mappedImage.RowPitch + area.x * outputPixelSize + zoffset * mappedImage.DepthPitch));
...
gl::Error Image11::copy(const gl::Extents &destOffset, const gl::Rectangle &sourceArea, RenderTarget *source)
{
...
gl::Error Image11::copy(const gl::Extents &destOffset, const gl::Rectangle &sourceArea, const gl::ImageIndex &sourceIndex, TextureStorage *source)
{
...
gl::Error Image11::copy(const gl::Extents &destOffset, const gl::Rectangle &sourceArea, ID3D11Texture2D *source, UINT sourceSubResource)
{
    ...
    if (textureDesc.Format == mDXGIFormat)
    {
        ...
        deviceContext->CopySubresourceRegion(stagingTexture, stagingSubresourceIndex, destOffset.width, destOffset.height,
                                             destOffset.depth, srcTex, subresourceAfterResolve, &srcBox);
