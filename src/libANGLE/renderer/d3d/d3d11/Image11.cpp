...
gl::Error Image11::createTemporaryStagingImage2D(DXGI_FORMAT format, ID3D11Texture2D **output)
{
...
gl::Error Image11::createTemporaryStagingImage3D(DXGI_FORMAT format, ID3D11Texture3D **output)
{
                                                     ...
                                                     bool intoStorage)
{
    ...
    if (dim == D3D11_RESOURCE_DIMENSION_TEXTURE2D)
    {
        ...
        createTemporaryStagingImage2D(d3dRenderableFormatInfo.texFormat, &tex2d);
        tempRenderableFormatTexture = d3d11::DynamicCastComObject<ID3D11Resource>(tex2d);
    }
    else if (dim == D3D11_RESOURCE_DIMENSION_TEXTURE3D)
    {
        ID3D11Texture3D *tex3d = nullptr;
        createTemporaryStagingImage3D(d3dRenderableFormatInfo.texFormat, &tex3d);
