...
{
    gl::Error error = mBuffer->getRenderTarget(getImageIndex(mTarget, mMipLevel, mLayer), &mRenderTarget);
    if (error.isError())
    {
...
RenderTargetD3D *EGLImage11::getRenderTarget() const
{
    RenderTargetD3D *rt = nullptr;
    gl::Error error = mBuffer->getRenderTarget(getImageIndex(mTarget, mMipLevel, mLayer), &rt);
    return rt;
