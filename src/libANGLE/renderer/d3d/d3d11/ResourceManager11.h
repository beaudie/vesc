...
#define ANGLE_DESC_TO_RESOURCE_TYPE(NAME, RESTYPE, D3D11TYPE, DESCTYPE, INITDATATYPE) \
    \
template<> struct NAME<DESCTYPE>                                                      \
    {                                                                                 \
        static constexpr ResourceType Value = ResourceType::RESTYPE;                  \
    };
...
ANGLE_TYPE_TO_RESOURCE_TYPE(ResourceTypeFromDesc, ANGLE_DESC_TO_RESOURCE_TYPE)
...
template <typename DescT>
using GetInitDataFromDesc = GetInitDataType<ResourceTypeFromDesc<DescT>::Value>;
...
class ResourceManager11 final : angle::NonCopyable
{
  ...
  private:
    ...
    template <typename DescT>
    GetInitDataFromDesc<DescT> *createInitDataIfNeeded(const DescT *desc);
