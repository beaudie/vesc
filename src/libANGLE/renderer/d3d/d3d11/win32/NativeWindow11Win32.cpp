                                             ...
                                             IDXGISwapChain **swapChain)
{
    ...
    if (mDirectComposition)
    {
        ...
        {
            HRESULT result = mDevice->CreateTargetForHwnd(static_cast<HWND>(getNativeWindow()),
                                                          TRUE, &mCompositionTarget);
            if (FAILED(result))
            {
                return result;
            }
    ...
    if (factory2 != nullptr)
    {
        ...
        IDXGISwapChain1 *swapChain1 = nullptr;
        HRESULT result =
            factory2->CreateSwapChainForHwnd(device, static_cast<HWND>(getNativeWindow()),
                                             &swapChainDesc, nullptr, nullptr, &swapChain1);
        if (SUCCEEDED(result))
        {
            factory2->MakeWindowAssociation(static_cast<HWND>(getNativeWindow()),
