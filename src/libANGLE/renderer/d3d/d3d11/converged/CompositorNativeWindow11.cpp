...
{
    auto inspPtr = static_cast<ABI::Windows::UI::Composition::ISpriteVisual *>(window);
...
RoHelper::RoHelper() : mWinRtAvailable(false)
{
    ...
    if (!AssignProcAddress<WindowsCreateStringReference_>(
            mComBaseModule, "WindowsCreateStringReference", mFpWindowsCreateStringReference))
    {
        return;
    }

    if (!AssignProcAddress<GetActivationFactory_>(mComBaseModule, "RoGetActivationFactory",
                                                  mFpGetActivationFactory))
    {
        return;
    }

    if (!AssignProcAddress<WindowsCompareStringOrginal_>(
            mComBaseModule, "WindowsCompareStringOrdinal", mFpWindowsCompareStringOrdinal))
    {
        return;
    }

    if (!AssignProcAddress<WindowsDeleteString_>(mComBaseModule, "WindowsDeleteString",
                                                 mFpWindowsDeleteString))
    {
        return;
    }

    if (!AssignProcAddress<RoInitialize_>(mComBaseModule, "RoInitialize", mFpRoInitialize))
    {
        return;
    }

    if (!AssignProcAddress<RoUninitialize_>(mComBaseModule, "RoUninitialize", mFpRoUninitialize))
    {
        return;
    }
    ...
    if (!AssignProcAddress<CreateDispatcherQueueController_>(mCoreMessagingModule,
