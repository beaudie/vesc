                                                        ...
                                                        bool sampleAlphaToCoverage)
{
    ...
    for (size_t keyBlendIndex = 0, sourceIndex = 0; keyBlendIndex < key.blendStateArray.size();
         keyBlendIndex++)
    {
        ...
        // Find the original blend state index
        while (sourceIndex < blendStateArray.size() && !colorAttachmentsForRenderMask[sourceIndex])
        {
            sourceIndex++;
        }
        if (sourceIndex == blendStateArray.size())
        {
            continue;  // no more usable source states
        }
