...
def get_internal_format_initializer(internal_format, angle_format):
    ...
    if gl_format_no_alpha and angle_format['channels'] == 'rgba':
        if 'texFormat' in angle_format and angle_format['texFormat'] == 'DXGI_FORMAT_BC1_UNORM':
            pass
        elif 'componentType' not in angle_format:
            print 'warning: internal format initializer could not be generated and may be needed for ' + internal_format
        elif angle_format['componentType'] == 'GLubyte':
            if internal_format == 'GL_RGB8UI':
...
def get_texture_format_item(idx, internal_format, gl_format_mapping, angle_format):
    table_data = '';
...
def parse_json_into_switch_string(json_map, json_data):
    ...
    for internal_format_item in sorted(json_map.iteritems()):
        ...
        for idx, gl_format_mapping in enumerate(sorted(json_map[internal_format], key=gl_format_key)):
            table_data += get_texture_format_item(idx, internal_format, gl_format_mapping, json_data[gl_format_mapping['implementationFormat']])
            if 'requirementsFcn' in gl_format_mapping:
...
with open('texture_format_map.json') as texture_format_map_file:
    with open('texture_format_data.json') as texture_format_json_file:
        ...
        json_map = json.loads(texture_format_map)
        json_data = json.loads(texture_format_data)

        table_data = parse_json_into_switch_string(json_map, json_data)
        output_cpp = template_texture_format_table_autogen_cpp.format(data=table_data)
        with open('texture_format_table_autogen.cpp', 'wt') as out_file:
            out_file.write(output_cpp)
            out_file.close()

        enum_data = parse_json_into_angle_format_enum_string(json_data)
        output_h = template_texture_format_table_autogen_h.format(data=enum_data)
