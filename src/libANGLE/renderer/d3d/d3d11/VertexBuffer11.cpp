...
VertexBuffer11::VertexBuffer11(Renderer11 *const renderer) : mRenderer(renderer)
{
    ...
    mBufferSize = 0;
    mDynamicUsage = false;
    mHasMappedResource = false;
}

VertexBuffer11::~VertexBuffer11()
{
    ASSERT(mHasMappedResource == false);
    SafeRelease(mBuffer);
}
...
{
    if (!mHasMappedResource)
    {
        ID3D11DeviceContext *dxContext = mRenderer->getDeviceContext();

        HRESULT result = dxContext->Map(mBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mMappedResource);
        if (FAILED(result))
        {
            return gl::Error(GL_OUT_OF_MEMORY, "Failed to map internal vertex buffer, HRESULT: 0x%08x.", result);
        }

        mHasMappedResource = true;
    }
                                                ...
                                                GLint start, GLsizei count, GLsizei instances, unsigned int offset)
{
    ...
    uint8_t *output = reinterpret_cast<uint8_t*>(mMappedResource.pData) + offset;
