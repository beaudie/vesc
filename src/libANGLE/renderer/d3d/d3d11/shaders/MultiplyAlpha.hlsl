...
Texture3D<float4> Texture3DF  : register(t0);
Texture2DArray<float4> Texture2DArrayF : register(t0);
...
// Texture3D
// Float to float LUMA
float4 PS_FtoF_PM_LUMA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb = color.r * color.a;
    color.a = 1.0f;
    return color;
}

float4 PS_FtoF_UM_LUMA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to float LUMAALPHA
float4 PS_FtoF_PM_LUMAALPHA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb = color.r * color.a;
    return color;
...
float4 PS_FtoF_UM_LUMAALPHA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to float RGBA
float4 PS_FtoF_PM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    return color;
...
float4 PS_FtoF_UM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to float RGB
float4 PS_FtoF_PM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color.a = 1.0f;
...
float4 PS_FtoF_UM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
float4 PS_FtoF_PM_RGBA_4444_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color = round(color * float4(15, 15, 15, 15)) / float4(15, 15, 15, 15);
...
float4 PS_FtoF_UM_RGBA_4444_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
float4 PS_FtoF_PM_RGB_565_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color.rgb = round(color.rgb * float3(31, 63, 31)) / float3(31, 63, 31);
...
float4 PS_FtoF_UM_RGB_565_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
float4 PS_FtoF_PM_RGBA_5551_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color = round(color * float4(31, 31, 31, 1)) / float4(31, 31, 31, 1);
...
float4 PS_FtoF_UM_RGBA_5551_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to uint RGBA
uint4 PS_FtoU_PT_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    return uint4(color * 255);
}

uint4 PS_FtoU_PM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    return uint4(color * 255);
...
uint4 PS_FtoU_UM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to uint RGB
uint4 PS_FtoU_PT_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    return uint4(color.rgb * 255, 1);
}

uint4 PS_FtoU_PM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    return uint4(color.rgb * 255, 1);
...
uint4 PS_FtoU_UM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to int RGBA
int4 PS_FtoI_PT_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
...
int4 PS_FtoI_PM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color = round(color * float4(127, 127, 127, 127)) / float4(127, 127, 127, 127);
...
int4 PS_FtoI_UM_RGBA_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
// Float to int RGB
int4 PS_FtoI_PT_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color = round(color * float4(127, 127, 127, 127)) / float4(127, 127, 127, 127);
...
int4 PS_FtoI_PM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    color.rgb *= color.a;
    color = round(color * float4(127, 127, 127, 127)) / float4(127, 127, 127, 127);
...
int4 PS_FtoI_UM_RGB_3D(in float4 inPosition : SV_POSITION, in uint layer : SV_RENDERTARGETARRAYINDEX, in float3 inTexCoord : TEXCOORD) : SV_TARGET0
{
    float4 color = Texture3DF.Sample(Sampler, inTexCoord).rgba;
    if (color.a > 0.0f)
    {
...
{
    float4 color = Texture2DArrayF.Sample(Sampler, float3(inTexCoord.xy, inLayer)).rgba;
    color.rgb = color.r * color.a;
    color.a = 1.0f;
    return color;
}
