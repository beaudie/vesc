...
float4 PS_PassthroughLumPremultiply2D(in float4 inPosition : SV_POSITION, in float2 inTexCoord : TEXCOORD0) : SV_TARGET0
{
    float4 color = TextureF.Sample(Sampler, inTexCoord).rrra;
    color.rgb *= color.a;
    color.a = 1.0f;
    return color;
}

float4 PS_PassthroughLumUnmultiply2D(in float4 inPosition : SV_POSITION, in float2 inTexCoord : TEXCOORD0) : SV_TARGET0
{
    float4 color = TextureF.Sample(Sampler, inTexCoord).rrra;
    if (color.a > 0.0f)
    {
...
float4 PS_PassthroughLumAlphaPremultiply2D(in float4 inPosition : SV_POSITION, in float2 inTexCoord : TEXCOORD0) : SV_TARGET0
{
    float4 color = TextureF.Sample(Sampler, inTexCoord).rrra;
    color.rgb *= color.a;
    return color;
...
float4 PS_PassthroughLumAlphaUnmultiply2D(in float4 inPosition : SV_POSITION, in float2 inTexCoord : TEXCOORD0) : SV_TARGET0
{
    float4 color = TextureF.Sample(Sampler, inTexCoord).rrra;
    if (color.a > 0.0f)
    {
