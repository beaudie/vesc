...
static inline bool RequiresFeatureLevel(D3D_FEATURE_LEVEL featureLevel)
{
    return featureLevel >= requiredFeatureLevel;
}

static inline bool AnyDevice(const Renderer11DeviceCaps* deviceCaps)
{
    return true;
}

static inline bool OnlyFL10Plus(const Renderer11DeviceCaps* deviceCaps)
{
    return (deviceCaps->featureLevel >= D3D_FEATURE_LEVEL_10_0);
}

static inline bool OnlyFL9_3(const Renderer11DeviceCaps* deviceCaps)
{
    return (deviceCaps->featureLevel == D3D_FEATURE_LEVEL_9_3);
}

static inline bool OnlyDXGI1dot2(const Renderer11DeviceCaps* deviceCaps)
{
    return (deviceCaps->supportsDXGI1dot2 == true);
}

static inline bool OnlyDXGI1dot1(const Renderer11DeviceCaps* deviceCaps)
{
    return (deviceCaps->supportsDXGI1dot2 == false);
}
                                         ...
                                         FormatSupportFunction formatSupportFunction)
{
    ...
    if (loadFunctionIter != loadFunctions.end())
    {
        ...
        for (size_t i = 0; i < loadFunctionVector.size(); i++)
        {
            ...
            if (loadFunctionVector[i].second.first == texFormat || loadFunctionVector[i].second.first == DXGI_FORMAT_UNKNOWN)
            {
                LoadImageFunction function = loadFunctionVector[i].second.second;
...
static D3D11ES3FormatMap BuildD3D11FormatMap()
{
    ...
    InsertD3D11FormatInfo(&map, GL_RGB565,            DXGI_FORMAT_R8G8B8A8_UNORM,       DXGI_FORMAT_R8G8B8A8_UNORM,      DXGI_FORMAT_R8G8B8A8_UNORM,      DXGI_FORMAT_UNKNOWN,   OnlyDXGI1dot1);
    InsertD3D11FormatInfo(&map, GL_RGB565,            DXGI_FORMAT_B5G6R5_UNORM,         DXGI_FORMAT_B5G6R5_UNORM,        DXGI_FORMAT_B5G6R5_UNORM,        DXGI_FORMAT_UNKNOWN,   OnlyDXGI1dot2);
...
const TextureFormat &GetTextureFormatInfo(GLenum internalFormat, const Renderer11DeviceCaps *renderer11DeviceCaps)
{
    ...
    if (iter != formatMap.end())
    {
        ...
        {
            if (formatVector[i].first(renderer11DeviceCaps))
            {
                return formatVector[i].second;
            }
        }
    }
