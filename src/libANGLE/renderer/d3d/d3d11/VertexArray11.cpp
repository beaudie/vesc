...
void VertexArray11::syncState(const gl::VertexArray::DirtyBits &dirtyBits)
{
    for (auto dirtyBit : angle::IterateBitSet(dirtyBits))
    {
        ...
        else if (dirtyBit >= gl::VertexArray::DIRTY_BIT_ATTRIB_0_ATTRIB_BINDING &&
                 dirtyBit < gl::VertexArray::DIRTY_BIT_ATTRIB_MAX_ATTRIB_BINDING)
        {
            size_t attribIndex = dirtyBit - gl::VertexArray::DIRTY_BIT_ATTRIB_0_ATTRIB_BINDING;
            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
            ASSERT(mData.getVertexAttribute(attribIndex).bindingIndex == attribIndex);
        }
        else if (dirtyBit >= gl::VertexArray::DIRTY_BIT_ATTRIB_0_BINDING_POINT &&
                 dirtyBit < gl::VertexArray::DIRTY_BIT_ATTRIB_MAX_BINDING_POINT)
        {
            size_t bindingIndex = dirtyBit - gl::VertexArray::DIRTY_BIT_ATTRIB_0_BINDING_POINT;
            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
        ...
        else
        {
            ...
            size_t bindingIndex =
                static_cast<size_t>(dirtyBit) - gl::VertexArray::DIRTY_BIT_ATTRIB_0_DIVISOR;
            // TODO(jiawei.shao@intel.com): Vertex Attrib Bindings
            ASSERT(mData.getVertexAttribute(bindingIndex).bindingIndex == bindingIndex);
            mAttribsToUpdate.set(bindingIndex);
        }
    }
}
...
void VertexArray11::updateVertexAttribStorage(size_t attribIndex)
{
    ...
    const auto &binding = mData.getVertexBufferBinding(attrib.bindingIndex);
