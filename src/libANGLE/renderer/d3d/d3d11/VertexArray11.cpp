...
namespace
{
// Hard-code the initial relevant dirty bits.
constexpr size_t kInitialRelevantDirtyBits(
    gl::VertexArray::DirtyBits::Mask(gl::VertexArray::DIRTY_BIT_BINDING_MAX));
}  // anonymous namespace
...
VertexArray11::VertexArray11(const gl::VertexArrayState &data)
    : VertexArrayImpl(data),
      ...
      mRelevantDirtyBitsMask(kInitialRelevantDirtyBits),
                              ...
                              const gl::VertexArray::DirtyBindingBitsArray &bindingBits)
{
    ...
    gl::VertexArray::DirtyBits relevantDirtyBits = (dirtyBits & mRelevantDirtyBitsMask);

    // Make sure we trigger re-translation for static index or vertex data.
    for (size_t dirtyBit : relevantDirtyBits)
    {
        switch (dirtyBit)
        {
