...
size_t GetAttribIndex(unsigned long dirtyBit)
{
    static_assert(gl::MAX_VERTEX_ATTRIBS == gl::MAX_VERTEX_ATTRIB_BINDINGS,
                  "The stride of vertex attributes should equal to that of vertex bindings in D3D");
...
VertexArray11::VertexArray11(const gl::VertexArrayState &data)
    : VertexArrayImpl(data),
      mAttributeStorageTypes(data.getMaxVertexAttribs(), VertexStorageType::CURRENT_VALUE),
      mTranslatedAttribs(data.getMaxVertexAttribs()),
      mCurrentBuffers(data.getMaxVertexAttribs())
{
...
void VertexArray11::syncState(const gl::VertexArray::DirtyBits &dirtyBits)
{
    for (auto dirtyBit : angle::IterateBitSet(dirtyBits))
    {
        ...
        ASSERT(mData.getVertexAttribute(index).getAttribFormat().bindingIndex == index);
...
void VertexArray11::updateVertexAttribStorage(size_t attribIndex)
{
    ...
    gl::Buffer *newBufferGL = attrib.getAttribBinding().buffer.get();
    ...
    if (oldBuffer11 != newBuffer11 || oldStorageType != newStorageType)
    {
        ...
        mCurrentBuffers[attribIndex] = attrib.getAttribBinding().buffer;
    }
}
                                                      ...
                                                      GLsizei instances)
{
    ...
    if (mAttribsToTranslate.any())
    {
        ...
        for (auto dirtyAttribIndex : angle::IterateBitSet(dirtyActiveAttribs))
        {
            ...
            translatedAttrib->divisor = translatedAttrib->attribute->getAttribBinding().divisor;
    ...
    if (mDynamicAttribsMask.any())
    {
        ...
        for (auto dynamicAttribIndex : angle::IterateBitSet(activeDynamicAttribs))
        {
            ...
            dynamicAttrib->divisor          = dynamicAttrib->attribute->getAttribBinding().divisor;
        }
