...
void VertexArray11::unlinkBuffer(size_t attribIndex)
{
    Buffer11 *buffer                    = GetImplAs<Buffer11>(mCurrentBuffers[attribIndex].get());
    VertexStorageType vertexStorageType = mAttributeStorageTypes[attribIndex];
    if (vertexStorageType == VertexStorageType::DIRECT)
    {
        buffer->removeDirectBufferDirtyCallback(&mOnBufferDataDirty[attribIndex]);
    }
    else if (vertexStorageType == VertexStorageType::STATIC ||
             vertexStorageType == VertexStorageType::DYNAMIC)
    {
