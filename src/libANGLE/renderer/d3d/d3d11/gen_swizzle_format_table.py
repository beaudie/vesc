...
def parse_json_into_switch_string(json_data):
    table_data = ''
    for component_type in json_data:
        table_data += '        case ' + component_type + ':\n'
        table_data += '        {\n'

        table_data += '            switch (' + max_bits_param + ')\n'
        table_data += '            {\n'

        for max_width in json_data[component_type]:
            table_data += '                case ' + max_width + ':\n'
            table_data += '                {\n'
            table_data += '                    static const SwizzleFormatInfo formatInfo(' + json_data[component_type][max_width][tex_format_key] + ',\n'
            table_data += '                                                              ' + json_data[component_type][max_width][srv_format_key] + ',\n'
            table_data += '                                                              ' + json_data[component_type][max_width][rtv_format_key] + ');\n'
...
with open('swizzle_format_data.json') as functions_json_file:
    ...
    with open('swizzle_format_info.cpp', 'wt') as out_file:
