...
gl::Error Renderer11::applyVertexBuffer(const gl::State &state, GLenum mode, GLint first, GLsizei count, GLsizei instances, SourceIndexData *sourceInfo)
{
    ...
    // Emulated instance id is set to 0 because Renderer11::applyVertexBuffer is always called to render
    // the first instance.  If non-emulated points rendering is performed, that parameter will be ignored.
                                     ...
                                     GLsizei instances)
{
    ...
    if (instances > 0)
    {
        ...
        {
            // If pointsprite emulation is used with glDrawArraysInstanced then we
            // need to take a less efficent code path.
            //
            // Instanced rendering of emulated pointsprites requires a loop to
            // draw each batch of points. An offset into the instanced data buffer is
            // calculated and applied on each iteration to ensure all instances are
            // rendered correctly.
            //
                                       ...
                                       GLsizei instances)
{
    ...
    if (instances > 0)
    {
        ...
        {
            // If pointsprite emulation is used with glDrawElementsInstanced then we
            // need to take a less efficent code path.
            //
            // Instanced rendering of emulated pointsprites requires a loop to
            // draw each batch of points. An offset into the instanced data buffer is
