...
bool Renderer11::supportsFastIndirectDraw(const gl::State &state, GLenum mode, GLenum type)
{
    ...
    auto *vertexArray11     = GetImplAs<VertexArray11>(vertexArray);
    bool result =
        !vertexArray11->hasDynamicAttrib(state) && mode != GL_LINE_LOOP && mode != GL_TRIANGLE_FAN;
    gl::Buffer *elementArrayBuffer = vertexArray->getElementArrayBuffer().get();
    if (result && elementArrayBuffer)
    {
        // Only none-streaming index data can be directly used to do indirect draw since they don't
