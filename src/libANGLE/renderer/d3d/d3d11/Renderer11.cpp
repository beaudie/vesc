...
void Renderer11::SamplerMetadataD3D11::initData(unsigned int samplerCount)
{
    ...
    mAppliedSamplerMetadata.resize(samplerCount);
}
...
const Renderer11::SamplerMetadataD3D11::dx_SamplerMetadata *
Renderer11::SamplerMetadataD3D11::appliedData() const
{
    return mAppliedSamplerMetadata.data();
}

void Renderer11::SamplerMetadataD3D11::apply(size_t appliedBytes)
{
    memcpy(mAppliedSamplerMetadata.data(), mSamplerMetadata.data(), appliedBytes);
}

size_t Renderer11::SamplerMetadataD3D11::sizeBytes() const
{
    return sizeof(SamplerMetadataD3D11::dx_SamplerMetadata) * mAppliedSamplerMetadata.size();
}

bool Renderer11::SamplerMetadataD3D11::referencedBytesApplied(size_t referencedBytes) const
{
    return memcmp(mAppliedSamplerMetadata.data(), mSamplerMetadata.data(), referencedBytes) != 0;
}
                                              ...
                                              ID3D11Buffer *driverConstantBuffer)
{
    ...
    if (memcmp(appliedConstants, &constants, sizeof(TShaderConstants)) != 0 ||
        !samplerMetadata->referencedBytesApplied(samplerMetadataReferencedBytes))
    {
        memcpy(appliedConstants, &constants, sizeof(TShaderConstants));
        samplerMetadata->apply(samplerMetadataReferencedBytes);
        ...
        memcpy(&reinterpret_cast<uint8_t *>(mapping.pData)[sizeof(TShaderConstants)],
               samplerMetadata->appliedData(), samplerMetadata->sizeBytes());
    ...
    if (width > 0 && height > 0)
    {
        ...
        if (bindSRV)
        {
            ...
            else
            {
                ...
                // for regular shaders, but ANGLE shaders for blitting from render targets require
                // the
                                         ...
                                         uint8_t *pixelsOut)
{
    ...
    ASSERT(d3d11::GetTextureFormatInfo(srcAttachment.getInternalFormat(), mRenderer11DeviceCaps)
               .semanticFormat == rt11->getSemanticDXGIFormat());
    auto errorOrResult = CreateStagingTexture(textureHelper.getTextureType(),
                                              rt11->getSemanticDXGIFormat(), safeSize, mDevice);
                                           ...
                                           bool stencilBlit)
{
    ...
    const auto &dxgiFormatInfo =
        d3d11::GetDXGIFormatInfo(drawRenderTarget11->getSemanticDXGIFormat());
    ...
    if (readRenderTarget11->getSemanticDXGIFormat() ==
            drawRenderTarget11->getSemanticDXGIFormat() &&
