                                    ...
                                    const std::vector<D3DUniform *> &uniformArray)
{
    ...
    if (!mDriverConstantBufferVS)
    {
        ...
        if (FAILED(result))
        {
            return gl::OutOfMemory()
                   << "Failed to create vertex shader constant buffer, " << gl::FmtHR(result);
        }
    ...
    if (!mDriverConstantBufferPS)
    {
        ...
        {
            return gl::OutOfMemory()
                   << "Failed to create pixel shader constant buffer, " << gl::FmtHR(result);
        }
                                         ...
                                         RenderTargetD3D **outRT)
{
    ...
    if (width > 0 && height > 0)
    {
        ...
        if (FAILED(result))
        {
            ...
            return gl::OutOfMemory()
                   << "Failed to create render target texture, " << gl::FmtHR(result);
        }
        ...
        if (bindSRV)
        {
            ...
            if (FAILED(result))
            {
                ASSERT(result == E_OUTOFMEMORY);
                SafeRelease(texture);
                return gl::OutOfMemory() << "Failed to create render target shader resource view, "
                                         << gl::FmtHR(result);
            }
            ...
            if (formatInfo.blitSRVFormat != formatInfo.srvFormat)
            {
                ...
                if (FAILED(result))
                {
                    ...
                    SafeRelease(srv);
                    return gl::OutOfMemory()
                           << "Failed to create render target shader resource view for blits, "
                           << gl::FmtHR(result);
                }
            }
            else
        ...
        if (bindDSV)
        {
            ...
            if (FAILED(result))
            {
                ...
                SafeRelease(srv);
                SafeRelease(blitSRV);
                return gl::OutOfMemory() << "Failed to create render target depth stencil view, "
                                         << gl::FmtHR(result);
            }
        ...
        else if (bindRTV)
        {
            ...
            if (FAILED(result))
            {
                ...
                SafeRelease(srv);
                SafeRelease(blitSRV);
                return gl::OutOfMemory() << "Failed to create render target render target view, "
                                         << gl::FmtHR(result);
            }
                                     ...
                                     ShaderExecutableD3D **outExecutable)
{
    switch (type)
    {
        case SHADER_VERTEX:
        {
            ...
            if (FAILED(result))
            {
                return gl::OutOfMemory() << "Failed to create vertex shader, " << gl::FmtHR(result);
            }
            ...
            if (!streamOutVaryings.empty())
            {
                ...
                {
                    return gl::OutOfMemory()
                           << "Failed to create steam output shader, " << gl::FmtHR(result);
                }
            }
        ...
        case SHADER_PIXEL:
        {
            ...
            if (FAILED(result))
            {
                return gl::OutOfMemory() << "Failed to create pixel shader, " << gl::FmtHR(result);
            }
        ...
        case SHADER_GEOMETRY:
        {
            ...
            {
                return gl::OutOfMemory()
                       << "Failed to create geometry shader, " << gl::FmtHR(result);
            }
        ...
        case SHADER_COMPUTE:
        {
            ...
            {
                return gl::OutOfMemory()
                       << "Failed to create compute shader, " << gl::FmtHR(result);
            }
                                         ...
                                         uint8_t *pixelsOut)
{
    ...
    if (textureHelper.getTextureType() == GL_TEXTURE_2D && textureHelper.getSampleCount() > 1)
    {
        ...
        if (FAILED(result))
        {
            return gl::OutOfMemory() << "Renderer11::readTextureData failed to create internal "
                                        "resolve texture for ReadPixels, "
                                     << gl::FmtHR(result);
        }
...
Renderer11::resolveMultisampledTexture(RenderTarget11 *renderTarget, bool depth, bool stencil)
{
    ...
    if (FAILED(result))
    {
        return gl::OutOfMemory() << "Failed to create a multisample resolve texture, "
                                 << gl::FmtHR(result);
    }
