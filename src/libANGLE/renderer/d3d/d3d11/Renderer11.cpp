...
gl::Error Renderer11::setUniformBuffers(const NonOwningOffsetBindingPointer<gl::Buffer> vertexUniformBuffers[], const NonOwningOffsetBindingPointer<gl::Buffer> fragmentUniformBuffers[])
{
    for (unsigned int uniformBufferIndex = 0; uniformBufferIndex < gl::IMPLEMENTATION_MAX_VERTEX_SHADER_UNIFORM_BUFFERS; uniformBufferIndex++)
    {
        ...
        if (uniformBuffer.get())
        {
                ...
                mCurrentConstantBufferVSSize[uniformBufferIndex] != uniformBuffer.getSize())
            {
                if (mSupportsConstantBufferOffsets)
                {
                    UINT firstConstant = uniformBuffer.getOffset()/16;
                    UINT numConstants = uniformBuffer.getSize()/16;
