                                               ...
                                               const GLvoid *indirect)
{
    ...
    if (type != GL_UNSIGNED_SHORT && !vertexArray11->hasDynamicAttrib(glState) &&
        mode != GL_LINE_LOOP && mode != GL_TRIANGLE_FAN)
    {
        // If type is not GL_UNSIGNED_SHORT, primitiveRestartWorkaround will be false. In that case,
        // we don't need to consider primitive restart issues. Even type equals GL_UNSIGNED_BYTE,
        // applyIndexBuffer can correctly handle translating GL_UNSIGNED_BYTE index buffer and
        // doesn't need to know the first index and count information since it will convert the
        // whole
        // buffer.
    ...
    ANGLE_TRY(applyIndexBuffer(data, nullptr, count, mode, type, &indexInfo));
    ...
    mDeviceContext->DrawIndexedInstanced(count, instances, firstIndex, baseVertexLocation, 0);
    return gl::NoError();
}
