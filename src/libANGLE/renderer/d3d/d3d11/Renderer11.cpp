...
egl::Error Renderer11::initializeD3DDevice()
{
    ...
    if (!mCreatedWithDeviceEXT)
    {
        ...
        PFN_D3D11ON12_CREATE_DEVICE D3D11on12CreateDevice = nullptr;
        {
            ...
            const auto &attributes = mDisplay->getAttributeMap();
            createD3D11on12Device  = static_cast<EGLint>(
                attributes.get(EGL_PLATFORM_ANGLE_ENABLE_D3D11ON12_ANGLE, EGL_FALSE));
        ...
        if (mCreateDebugDevice)
        {
                ...
                mAvailableFeatureLevels[0] == D3D_FEATURE_LEVEL_11_1)
            {
                ...
                result = createD3D11on12Device ? callD3D11On12CreateDevice(
                                                     D3D12CreateDevice, D3D11on12CreateDevice, true)
                                               : callD3D11CreateDevice(D3D11CreateDevice, true);
            }
        ...
        if (!mDevice || FAILED(result))
        {
            ...
            result = createD3D11on12Device ? callD3D11On12CreateDevice(D3D12CreateDevice,
                                                                       D3D11on12CreateDevice, false)
