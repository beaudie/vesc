...
HRESULT Renderer11::callD3D11CreateDevice(PFN_D3D11_CREATE_DEVICE createDevice, bool debug)
{
    ...
    // Check EGL_ANGLE_platform_angle_gpu_selection
    if (high == 0 && low == 0)
    {
        high = static_cast<long>(attributes.get(EGL_PLATFORM_ANGLE_GPU_UNIQUE_ID_HIGH_ANGLE, 0));
        low  = static_cast<unsigned long>(
            attributes.get(EGL_PLATFORM_ANGLE_GPU_UNIQUE_ID_LOW_ANGLE, 0));
    }
    if (high != 0 || low != 0)
    {
