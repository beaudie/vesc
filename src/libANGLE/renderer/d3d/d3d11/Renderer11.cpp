                                      ...
                                      const gl::SamplerState &samplerState)
{
    ...
    // Sampler metadata that's passed to shaders in uniforms is stored separately from rest of
                                              ...
                                              ID3D11Buffer *driverConstantBuffer)
{
    ...
    if (memcmp(appliedConstants, &constants, sizeof(TShaderConstants)) != 0 ||
        memcmp(samplerMetadata->appliedData(), samplerMetadata->getData(),
               samplerMetadataReferencedBytes) != 0)
    {
