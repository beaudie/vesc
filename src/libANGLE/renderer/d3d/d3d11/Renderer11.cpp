...
gl::Error Renderer11::applyRenderTarget(const gl::Framebuffer *framebuffer)
{
    ...
    if (depthStencil)
    {
        ...
        if (!mDepthStencilInitialized || stencilSize != mStateManager->getCurStencilSize())
        {
            mStateManager->setCurStencilSize(stencilSize);
...
gl::Error Renderer11::applyShadersImpl(const gl::Data &data)
{
    ...
    StateManager11 *stateManager11 = GetAs<rx::StateManager11>(mStateManager);

    if (transformFeedbackActive)
    {
        geometryShader = (vertexExe ? GetAs<ShaderExecutable11>(vertexExe)->getStreamOutShader() : NULL);
    }
    else if (stateManager11->getCurRasterizerState().pointDrawMode)
    {
