        ...
        if (colorBufferFormatCaps.renderable)
        {
            for (size_t depthStencilIndex = 0; depthStencilIndex < ArraySize(depthStencilBufferFormats); depthStencilIndex++)
            {
                ...
                if (depthStencilBufferFormatCaps.renderable || depthStencilBufferInternalFormat == GL_NONE)
                {
                    ...
                    config.conformant = (!mDirectRenderingEnabled && mRenderer11DeviceCaps.featureLevel >= D3D_FEATURE_LEVEL_10_0) ? (EGL_OPENGL_ES2_BIT | EGL_OPENGL_ES3_BIT_KHR) : 0;
                                      ...
                                      uint8_t *pixels)
{
    ...
    if (invertTexture)
    {
        ...
        // Note that we can't just assign 'actualPack' to be 'pack' (or memcpy) since that breaks the
        // ref counting/object tracking in the 'pixelBuffer' members, causing leaks.
        // Instead we must use pixelBuffer.set() twice, which performs the correct addRef/release calls.
