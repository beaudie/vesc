...
HRESULT Renderer11::callD3D11On12CreateDevice(PFN_D3D12_CREATE_DEVICE createDevice12,
                                              ...
                                              bool debug)
{
    HRESULT result = createDevice12(nullptr, D3D_FEATURE_LEVEL_11_1, IID_PPV_ARGS(&mDevice12));
    if (SUCCEEDED(result))
    {
    ...
    if (SUCCEEDED(result))
    {
        result = createDevice11on12(
            mDevice12, debug ? D3D11_CREATE_DEVICE_DEBUG : 0, mAvailableFeatureLevels.data(),
            static_cast<unsigned int>(mAvailableFeatureLevels.size()),
            reinterpret_cast<IUnknown **>(&mCommandQueue), 1 /* NumQueues */, 0 /* NodeMask */,
            &mDevice, &mDeviceContext, &(mRenderer11DeviceCaps.featureLevel));
    }

    if (FAILED(result))
    {
        SafeRelease(mCommandQueue);
        SafeRelease(mDevice12);
    }
    ...
    if (!mCreatedWithDeviceEXT)
    {
        ...
        PFN_D3D11ON12_CREATE_DEVICE D3D11on12CreateDevice = nullptr;
        {
            ...
            if (mCreateD3D11on12Device)
            {
                ...
                {
                    return egl::EglNotInitialized(D3D12_INIT_MISSING_DEP)
