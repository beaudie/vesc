...
void Renderer11::releaseDeviceResources()
{
    ...
    mMultiviewConstantBuffer.reset();
    mSyncQuery.reset();
}
...
gl::Error Renderer11::updateMultiviewConstantBufferData(bool writeToViewportIndex)
{
    const UINT kMultiviewCBufferSizeInBytes = 16u;
    const bool bufferData[4]                = {writeToViewportIndex, false, false, false};
    ...
    if (!mMultiviewConstantBuffer.valid())
    {
        ...
        mDeviceContext->GSSetConstantBuffers(d3d11::RESERVED_CONSTANT_BUFFER_SLOT_MULTIVIEW_DRIVER,
                                             1, &buffer);
    }
    else if (mWriteToViewportIndex != writeToViewportIndex)
    {
    ...
    mWriteToViewportIndex = writeToViewportIndex;
    return gl::NoError();
}
