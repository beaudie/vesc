...
void Renderer11::populateRenderer11DeviceCaps()
{
    ...
    // Some Intel cards (e.g. Intel HD Graphics 2000 found in Dell Optiplex 790) claim to support 16-bit DXGI textures,
    // but some ANGLE end2end 16-bit texture tests fail on them. Init data doesn't appear to work correctly.
    // As a result, we shouldn't try to use 16-bit textures on Intel cards.
    const UINT vendorIntel = 0x8086;
    if (!(mAdapterDescription.VendorId == vendorIntel))
    {
