...
void Renderer11::populateRenderer11DeviceCaps()
{
    ...
    if (mDeviceContext3)
    {
        ...
        if (SUCCEEDED(result))
        {
            ...
            {
                // TypedUAVLoadAdditionalFormats is true. Now check if we can both load and store
                // the common additional formats. The common formats are supported in a set, so we
                ...
                result = mDevice->CheckFeatureSupport(D3D11_FEATURE_FORMAT_SUPPORT2, &d3d11Format2,
                                                      sizeof(d3d11Format2));
                if (SUCCEEDED(result))
                {
