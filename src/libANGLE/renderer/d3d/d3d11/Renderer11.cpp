                        ...
                        std::vector<GLuint> *bufferOut)
{
    ...
    {
        size_t indexCount = GetLineLoopIndexCountWithRestart(indexType, count, indices);
        bufferOut->resize(indexCount);
        GLuint d3dRestartIndex = d3d11::GetPrimitiveRestartIndex();
        switch (indexType)
        {
            case gl::DrawElementsType::UnsignedByte:
                CopyLineLoopIndicesWithRestart<GLubyte, GLuint>(count, indices, indexCount,
                                                                bufferOut->data(), d3dRestartIndex);
                break;
            case gl::DrawElementsType::UnsignedShort:
                CopyLineLoopIndicesWithRestart<GLushort, GLuint>(
                    count, indices, indexCount, bufferOut->data(), d3dRestartIndex);
                break;
            case gl::DrawElementsType::UnsignedInt:
                CopyLineLoopIndicesWithRestart<GLuint, GLuint>(count, indices, indexCount,
                                                               bufferOut->data(), d3dRestartIndex);
                break;
            default:
                UNREACHABLE();
