      ...
      mDebug(nullptr)
{
    ...
    else if (display->getPlatform() == EGL_PLATFORM_DEVICE_EXT)
    {
        ...
        mRequestedDriverType = (D3D_DRIVER_TYPE)0;
    }
...
egl::Error Renderer11::initializeD3DDevice()
{
    ...
    if (!mCreatedWithDeviceEXT)
    {
...
#ifdef _DEBUG
        {
            ...
            result = D3D11CreateDevice(NULL, mRequestedDriverType, NULL, D3D11_CREATE_DEVICE_DEBUG,
...
#endif
        {
            ...
            result = D3D11CreateDevice(
                NULL, mRequestedDriverType, NULL, 0, mAvailableFeatureLevels.data(),
...
bool Renderer11::getShareHandleSupport() const
{
    ...
    // Also disable if the caller specifically requested a non-hardware driver, since they
    // won't be able to share resources with hardware drivers.
    // Don't disable if the caller requested a hardware driver but was given a non-hardware one,
    // or if an external EGLDevice was passed in. We want to allow resource sharing if the whole OS
    // is running with non-hardware drivers.
    if (!mCreatedWithDeviceEXT && mRequestedDriverType != D3D_DRIVER_TYPE_HARDWARE)
    {
        return false;
    }
