                                     ...
                                     GLsizei instances)
{
    ...
    if (instances > 0)
    {
        if (mode == GL_POINTS && useInstancedPointSpriteEmulation)
        {
            ...
            {
                gl::Error error = mInputLayoutCache.updateVertexOffsetsForPointSpritesEmulation(
                    data.state->getProgram(), i, count);
                if (error.isError())
                {
                    return error;
                }
                                       ...
                                       GLsizei instances)
{
    ...
    if (instances > 0)
    {
        if (mode == GL_POINTS && programD3D->usesInstancedPointSpriteEmulation())
        {
            ...
            {
                gl::Error error = mInputLayoutCache.updateVertexOffsetsForPointSpritesEmulation(
                    data.state->getProgram(), i, elementsToRender);
                if (error.isError())
                {
                    return error;
                }
