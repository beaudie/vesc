...
egl::Error Renderer11::initializeEGLDevice()
{
    if (mEGLDevice == nullptr)
    {
        if (mDevice == nullptr)
        {
            return egl::Error(EGL_BAD_DEVICE_EXT, "D3D11 device hasn't been initialized yet.");
        }

        mEGLDevice = new DeviceD3D(reinterpret_cast<EGLAttrib>(mDevice), EGL_D3D11_DEVICE_ANGLE);
    }
