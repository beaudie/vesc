...
bool Renderer11::getShareHandleSupport() const
{
    ...
    bool isSoftwareOrRefOrNullDevice = false;
    bool isWARPDevice                = false;

    if (mCreatedWithDeviceEXT)
    {
        HRESULT hr = d3d11::GetDeviceType(mDevice, &isSoftwareOrRefOrNullDevice, &isWARPDevice);
        ASSERT(SUCCEEDED(hr));
        if (FAILED(hr))
        {
            return false;
    ...
    {
        isSoftwareOrRefOrNullDevice = (mRequestedDriverType == D3D_DRIVER_TYPE_SOFTWARE) ||
                                      (mRequestedDriverType == D3D_DRIVER_TYPE_REFERENCE) ||
                                      (mRequestedDriverType == D3D_DRIVER_TYPE_NULL);
        isWARPDevice = (mRequestedDriverType == D3D_DRIVER_TYPE_WARP);
    }

    if (isSoftwareOrRefOrNullDevice)
    {
