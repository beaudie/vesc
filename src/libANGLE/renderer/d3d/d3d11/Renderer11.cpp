                                   ...
                                   UINT *outNumConstants)
{
    ...
    *outNumConstants = static_cast<UINT>(rx::roundUp(size, static_cast<GLsizeiptr>(256)) / 16);
    ...
    // of the buffer.
    // This behaviour is explictly allowed according to the documentation on
...
Renderer11::Renderer11(egl::Display *display)
    : RendererD3D(display), mStateCache(this), mDebug(nullptr)
{
...
egl::Error Renderer11::initialize()
{
...
#ifdef _DEBUG
    {
        ...
        result = D3D11CreateDevice(NULL, mDriverType, NULL, D3D11_CREATE_DEVICE_DEBUG,
                                   mAvailableFeatureLevels.data(), mAvailableFeatureLevels.size(),
...
#endif
    {
        ...
        result = D3D11CreateDevice(NULL, mDriverType, NULL, 0, mAvailableFeatureLevels.data(),
                                   mAvailableFeatureLevels.size(), D3D11_SDK_VERSION, &mDevice,
                                   &(mRenderer11DeviceCaps.featureLevel), &mDeviceContext);
...
gl::Error Renderer11::generateSwizzle(gl::Texture *texture)
{
    if (texture)
    {
        ...
        if (texStorage)
        {
            ...
            error = storage11->generateSwizzles(
                texture->getSamplerState().swizzleRed, texture->getSamplerState().swizzleGreen,
                texture->getSamplerState().swizzleBlue, texture->getSamplerState().swizzleAlpha);
            if (error.isError())
            {
                return error;
            }
...
gl::Error Renderer11::setSamplerState(gl::SamplerType type,
                                      int index,
                                      gl::Texture *texture,
                                      const gl::SamplerState &samplerStateParam)
{
    ...
    if (type == gl::SAMPLER_PIXEL)
    {
        ...
        if (mForceSetPixelSamplerStates[index] ||
            memcmp(&samplerStateInternal, &mCurPixelSamplerStates[index],
    ...
    else if (type == gl::SAMPLER_VERTEX)
    {
        ...
        if (mForceSetVertexSamplerStates[index] ||
            memcmp(&samplerStateInternal, &mCurVertexSamplerStates[index],
                   sizeof(gl::SamplerState)) != 0)
        {
            ID3D11SamplerState *dxSamplerState = NULL;
            error = mStateCache.getSamplerState(samplerStateInternal, &dxSamplerState);
...
gl::Error Renderer11::setTexture(gl::SamplerType type, int index, gl::Texture *texture)
{
    ...
    ASSERT((type == gl::SAMPLER_PIXEL &&
            static_cast<unsigned int>(index) < getRendererCaps().maxTextureImageUnits) ||
           (type == gl::SAMPLER_VERTEX &&
            static_cast<unsigned int>(index) < getRendererCaps().maxVertexTextureImageUnits));
                ...
                mCurrentConstantBufferVSSize[uniformBufferIndex] != uniformBufferSize)
            {
                if (mRenderer11DeviceCaps.supportsConstantBufferOffsets && uniformBufferSize != 0)
                {
                    ...
                    mDeviceContext1->VSSetConstantBuffers1(
                        getReservedVertexUniformBuffers() + uniformBufferIndex, 1, &constantBuffer,
                        &firstConstant, &numConstants);
                }
                else
                {
                ...
                mCurrentConstantBufferPSSize[uniformBufferIndex] != uniformBufferSize)
            {
                if (mRenderer11DeviceCaps.supportsConstantBufferOffsets && uniformBufferSize != 0)
                {
                    ...
                    mDeviceContext1->PSSetConstantBuffers1(
                        getReservedFragmentUniformBuffers() + uniformBufferIndex, 1,
                        &constantBuffer, &firstConstant, &numConstants);
                }
                else
                {
...
{
    if (mForceSetBlendState || memcmp(&blendState, &mCurBlendState, sizeof(gl::BlendState)) != 0 ||
...
gl::Error Renderer11::setDepthStencilState(const gl::DepthStencilState &depthStencilState,
                                           int stencilRef,
                                           int stencilBackRef,
                                           bool frontFaceCCW)
{
...
void Renderer11::setViewport(const gl::Rectangle &viewport,
                             float zNear,
                             float zFar,
                             GLenum drawMode,
                             GLenum frontFace,
                             bool ignoreViewport)
{
    ...
    bool viewportChanged = mForceSetViewport ||
                           memcmp(&actualViewport, &mCurViewport, sizeof(gl::Rectangle)) != 0 ||
    ...
    if (viewportChanged)
    {
        ...
        int dxViewportTopLeftX =
            gl::clamp(actualViewport.x, dxMinViewportBoundsX, dxMaxViewportBoundsX);
        int dxViewportTopLeftY =
            gl::clamp(actualViewport.y, dxMinViewportBoundsY, dxMaxViewportBoundsY);
        int dxViewportWidth =
            gl::clamp(actualViewport.width, 0, dxMaxViewportBoundsX - dxViewportTopLeftX);
        int dxViewportHeight =
            gl::clamp(actualViewport.height, 0, dxMaxViewportBoundsY - dxViewportTopLeftY);
        ...
        if (mRenderer11DeviceCaps.featureLevel <= D3D_FEATURE_LEVEL_9_3)
        {
            mVertexConstants.viewAdjust[0] =
                static_cast<float>((actualViewport.width - dxViewportWidth) +
                                   2 * (actualViewport.x - dxViewportTopLeftX)) /
                dxViewport.Width;
            mVertexConstants.viewAdjust[1] =
                static_cast<float>((actualViewport.height - dxViewportHeight) +
                                   2 * (actualViewport.y - dxViewportTopLeftY)) /
                dxViewport.Height;
            mVertexConstants.viewAdjust[2] =
                static_cast<float>(actualViewport.width) / dxViewport.Width;
            mVertexConstants.viewAdjust[3] =
                static_cast<float>(actualViewport.height) / dxViewport.Height;
        }
...
void Renderer11::unsetConflictingSRVs(gl::SamplerType samplerType,
                                      uintptr_t resource,
...
gl::Error Renderer11::applyRenderTarget(const gl::Framebuffer *framebuffer)
{
    ...
    const gl::AttachmentList &colorbuffers =
        framebufferD3D->getColorAttachmentsForRender(getWorkarounds());
    ...
    {
        mDeviceContext->OMSetRenderTargets(getRendererCaps().maxDrawBuffers, framebufferRTVs,
                                           framebufferDSV);
        ...
        for (size_t i = 0; i < numXFBBindings; ++i)
        {
            ...
            if (binding.get() != nullptr)
            {
                ...
                ID3D11Buffer *d3dBuffer =
                    storage->getBuffer(BUFFER_USAGE_VERTEX_OR_TRANSFORM_FEEDBACK);

                mCurrentD3DOffsets[i] = (mAppliedTFBuffers[i] != d3dBuffer ||
                                         mAppliedTFOffsets[i] != binding.getOffset())
                                            ? static_cast<UINT>(binding.getOffset())
                                            : -1;
...
gl::Error Renderer11::drawArrays(const gl::Data &data,
                                 GLenum mode,
                                 GLsizei count,
                                 GLsizei instances,
                                 bool usesPointSize)
{
    bool useInstancedPointSpriteEmulation =
        usesPointSize && getWorkarounds().useInstancedPointSpriteEmulation;
    if (mode == GL_POINTS && data.state->isTransformFeedbackActiveUnpaused())
    {
        ...
        gl::Error error = programD3D->getPixelExecutableForFramebuffer(
            data.state->getDrawFramebuffer(), &pixelExe);
        if (error.isError())
        {
            return error;
        }
...
gl::Error Renderer11::drawElements(GLenum mode,
                                   GLsizei count,
                                   GLenum type,
...
gl::Error Renderer11::drawLineLoop(GLsizei count,
                                   GLenum type,
                                   const GLvoid *indices,
                                   int minIndex,
                                   gl::Buffer *elementArrayBuffer)
...
gl::Error Renderer11::drawTriangleFan(GLsizei count,
                                      GLenum type,
                                      const GLvoid *indices,
                                      int minIndex,
                                      gl::Buffer *elementArrayBuffer,
                                   ...
                                   bool transformFeedbackActive)
{
    ...
    gl::Error error =
        programD3D->getVertexExecutableForInputLayout(inputLayout, &vertexExe, nullptr);
    if (error.isError())
    {
        return error;
    }
    ...
    ID3D11VertexShader *vertexShader =
        (vertexExe ? GetAs<ShaderExecutable11>(vertexExe)->getVertexShader() : NULL);
    ...
    {
        geometryShader =
            (vertexExe ? GetAs<ShaderExecutable11>(vertexExe)->getStreamOutShader() : NULL);
    }
...
gl::Error Renderer11::applyUniforms(const ProgramImpl &program,
                                    const std::vector<gl::LinkedUniform *> &uniformArray)
{
    ...
    for (size_t uniformIndex = 0; uniformIndex < uniformArray.size(); uniformIndex++)
    {
        ...
        if (!uniform->isSampler())
        {
            ...
            // we assume that uniforms from structs are arranged in struct order in our uniforms
            // list. otherwise we would
    ...
    if (!mDriverConstantBufferVS)
    {
        ...
        HRESULT result =
            mDevice->CreateBuffer(&constantBufferDescription, NULL, &mDriverConstantBufferVS);
        ASSERT(SUCCEEDED(result));
        if (FAILED(result))
        {
            return gl::Error(GL_OUT_OF_MEMORY,
    ...
    if (!mDriverConstantBufferPS)
    {
        ...
        HRESULT result =
            mDevice->CreateBuffer(&constantBufferDescription, NULL, &mDriverConstantBufferPS);
        ASSERT(SUCCEEDED(result));
        if (FAILED(result))
        {
            return gl::Error(GL_OUT_OF_MEMORY,
    ...
    if (memcmp(&mVertexConstants, &mAppliedVertexConstants, sizeof(dx_VertexConstants)) != 0)
    {
        ...
        if (mDriverConstantBufferVS)
        {
            mDeviceContext->UpdateSubresource(mDriverConstantBufferVS, 0, NULL, &mVertexConstants,
                                              16, 0);
    ...
    if (memcmp(&mPixelConstants, &mAppliedPixelConstants, sizeof(dx_PixelConstants)) != 0)
    {
        ...
        if (mDriverConstantBufferPS)
        {
            mDeviceContext->UpdateSubresource(mDriverConstantBufferPS, 0, NULL, &mPixelConstants,
                                              16, 0);
...
bool Renderer11::testDeviceResettable()
{
    ...
    HRESULT result =
        D3D11CreateDevice(NULL, mDriverType, NULL,
#if defined(_DEBUG)
                                         ...
                                         RenderTargetD3D **outRT)
{
    const d3d11::TextureFormat &formatInfo =
        d3d11::GetTextureFormatInfo(format, mRenderer11DeviceCaps);
    ...
    if (width > 0 && height > 0)
    {
        ...
        if (bindDSV)
        {
            ...
            *outRT = new TextureRenderTarget11(dsv, texture, srv, format, width, height, 1,
                                               supportedSamples);
        ...
        else if (bindRTV)
        {
            ...
            *outRT = new TextureRenderTarget11(rtv, texture, srv, format, width, height, 1,
                                               supportedSamples);
    ...
    {
        *outRT = new TextureRenderTarget11(reinterpret_cast<ID3D11RenderTargetView *>(NULL), NULL,
                                           NULL, format, width, height, 1, supportedSamples);
    }
...
gl::Error Renderer11::loadExecutable(
    const void *function,
    size_t length,
    ShaderType type,
        ...
        case SHADER_VERTEX:
        {
            ...
            if (transformFeedbackVaryings.size() > 0)
            {
                ...
                for (size_t i = 0; i < transformFeedbackVaryings.size(); i++)
                {
                    ...
                    for (size_t j = 0; j < varying.semanticIndexCount; j++)
                    {
                        ...
                        entry.ComponentCount =
                            static_cast<BYTE>(gl::VariableColumnCount(transposedType));
                ...
                result = mDevice->CreateGeometryShaderWithStreamOutput(
                    function, length, soDeclaration.data(), soDeclaration.size(), NULL, 0, 0, NULL,
                    &streamOutShader);
                ASSERT(SUCCEEDED(result));
                if (FAILED(result))
                {
...
gl::Error Renderer11::compileToExecutable(
    gl::InfoLog &infoLog,
    ...
    ShaderExecutableD3D **outExectuable)
{
    ...
    gl::Error error = mCompiler.compileToBinary(infoLog, shaderHLSL, profile, configs, loopMacros,
                                                &binary, &debugInfo);
    if (error.isError())
    {
        return error;
    ...
    // executable to NULL
    // and return GL_NO_ERROR to signify that there was a link error but the internal state is still
    // OK.
    if (!binary)
    {
        *outExectuable = NULL;
...
bool Renderer11::supportsFastCopyBufferToTexture(GLenum internalFormat) const
{
    ...
    const d3d11::TextureFormat &d3d11FormatInfo =
        d3d11::GetTextureFormatInfo(internalFormat, mRenderer11DeviceCaps);
                                      ...
                                      uint8_t *pixels)
{
    ...
    {
        return gl::Error(GL_OUT_OF_MEMORY,
                         "Failed to create internal staging texture for ReadPixels, HRESULT: 0x%X.",
                         result);
    }
    ...
    if (textureDesc.SampleDesc.Count > 1)
    {
        ...
        if (FAILED(result))
        {
            ...
            return gl::Error(
                GL_OUT_OF_MEMORY,
                "Failed to create internal resolve texture for ReadPixels, HRESULT: 0x%X.", result);
        }
...
gl::Error Renderer11::packPixels(ID3D11Texture2D *readTexture,
                                 ...
                                 uint8_t *pixelsOut)
{
    ...
    {
        source =
            static_cast<uint8_t *>(mapping.pData) + mapping.RowPitch * (params.area.height - 1);
    ...
    else
    {
        ...
        if (fastCopyFunc)
        {
            ...
            for (int y = 0; y < params.area.height; y++)
            {
                ...
                {
                    uint8_t *dest = pixelsOut + params.offset + y * params.outputPitch +
                                    x * destFormatInfo.pixelBytes;
        ...
        else
        {
            ...
            ColorWriteFunction colorWriteFunction =
                GetColorWriteFunction(params.format, params.type);
...
gl::Error Renderer11::blitRenderbufferRect(const gl::Rectangle &readRect,
                                           ...
                                           bool stencilBlit)
{
    ...
    if (readRenderTarget->getSamples() > 0)
    {
        ...
        ID3D11Texture2D *unresolvedTexture =
            d3d11::DynamicCastComObject<ID3D11Texture2D>(unresolvedResource);
    ...
    if (!readTexture || !readSRV)
    {
        ...
        return gl::Error(
            GL_OUT_OF_MEMORY,
            "Failed to retrieve the internal read render target view from the read render target.");
    }
    ...
    bool wholeBufferCopy = !scissorNeeded && readRect.x == 0 && readRect.width == readSize.width &&
                           readRect.y == 0 && readRect.height == readSize.height &&
                           drawRect.x == 0 && drawRect.width == drawSize.width && drawRect.y == 0 &&
    ...
    const d3d11::DXGIFormat &dxgiFormatInfo =
        d3d11::GetDXGIFormatInfo(drawRenderTarget11->getDXGIFormat());
    bool partialDSBlit = (dxgiFormatInfo.depthBits > 0 && depthBlit) !=
                         (dxgiFormatInfo.stencilBits > 0 && stencilBlit);
    ...
    else
    {
        ...
        {
            result =
                mBlit->copyDepthStencil(readTexture, readSubresource, readArea, readSize,
                                        drawTexture, drawSubresource, drawArea, drawSize, scissor);
        }
        else if (depthBlit)
        {
            result =
                mBlit->copyDepth(readSRV, readArea, readSize, drawDSV, drawArea, drawSize, scissor);
        }
        else if (stencilBlit)
        {
                                                        ...
                                                        unsigned int subresource)
{
    ...
    if (textureDesc.SampleDesc.Count > 1)
    {
        ...
        mDeviceContext->ResolveSubresource(resolveTexture, 0, source, subresource,
                                           textureDesc.Format);
...
{
    return d3d11::GetDXGIFormatInfo(
...
void Renderer11::setShaderResource(gl::SamplerType shaderType,
                                   UINT resourceSlot,
                                   ID3D11ShaderResourceView *srv)
{
