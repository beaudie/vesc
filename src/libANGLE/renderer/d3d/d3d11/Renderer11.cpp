                                         ...
                                         GLenum *fboFormat) const
{
    ...
    {
        UINT qualityLevel = 0;
        mDevice->CheckMultisampleQualityLevels(desc.Format, desc.SampleDesc.Count, &qualityLevel);

        if (qualityLevel == 0 || desc.SampleDesc.Quality == 0)
        {
            return egl::Error(EGL_BAD_PARAMETER,
                              "Texture's multisampling quality level is invalid");
        }
    }
