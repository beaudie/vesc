...
class SwapChain11 final : public SwapChainD3D
{
  public:
    ...
    EGLint resize(const egl::Thread *thread,
                  EGLint backbufferWidth,
                  EGLint backbufferHeight) override;
    EGLint reset(const egl::Thread *thread,
                 EGLint backbufferWidth,
                 EGLint backbufferHeight,
                 EGLint swapInterval) override;
    EGLint swapRect(const egl::Thread *thread,
                    EGLint x,
                    EGLint y,
                    EGLint width,
  ...
  private:
    ...
    EGLint resetOffscreenBuffers(const egl::Thread *thread,
                                 int backbufferWidth,
                                 int backbufferHeight);
    EGLint resetOffscreenColorBuffer(const egl::Thread *thread,
                                     int backbufferWidth,
                                     int backbufferHeight);
    ...
    EGLint copyOffscreenToBackbuffer(const egl::Thread *thread,
                                     EGLint x,
                                     EGLint y,
                                     EGLint width,
                                     EGLint height);
    EGLint present(const egl::Thread *thread, EGLint x, EGLint y, EGLint width, EGLint height);
