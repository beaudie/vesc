...
bool UsingWorkaroundVersion(Optional<LARGE_INTEGER> driverVersion,
                            ...
                            WORD fixedVersionPart2)
{
    if (driverVersion.valid())
    {
        ...
        if (part1 <= fixedVersionPart1 && part2 < fixedVersionPart2)
            return true;
        else
            return false;
    }
                                          ...
                                          const DXGI_ADAPTER_DESC &adapterDesc)
{
    ...
    if (IsNvidia(adapterDesc.VendorId))
    {
        ...
        workarounds.depthStencilBlitExtraCopy =
            UsingWorkaroundVersion(deviceCaps.driverVersion, 13u, 6881);
    }
    ...
    if (IsIntel(adapterDesc.VendorId))
    {
        workarounds.disableB5G6R5Support =
            UsingWorkaroundVersion(deviceCaps.driverVersion, 15u, 4539u);
    }
    ...
    if (IsIntel(adapterDesc.VendorId) && IsSkylake(adapterDesc.DeviceId))
    {
        ...
        workarounds.emulateIsnanFloat =
            UsingWorkaroundVersion(deviceCaps.driverVersion, 16u, 4542u);
    }
