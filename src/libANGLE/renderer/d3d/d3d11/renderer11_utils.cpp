    ...
    gl::TexLevelArray<D3D11_SUBRESOURCE_DATA> *outSubresourceData)
{
    ...
    CheckedSize pixelBytes(dxgiFormatInfo.pixelBytes);
    CheckedSize rowPitch     = pixelBytes * CheckedSize(width);
    ...
    for (unsigned int i = 0; i < mipLevels; i++)
    {
        ...
        CheckedSize mipRowPitch   = pixelBytes * CheckedSize(mipWidth);
        CheckedSize mipDepthPitch = mipRowPitch * CheckedSize(mipHeight);
