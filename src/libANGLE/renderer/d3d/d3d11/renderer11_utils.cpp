...
#include "libANGLE/renderer/drivers/Intel.h"
                                   ...
                                   const DXGI_ADAPTER_DESC &adapterDesc)
{
    ...
    if (IS_NVIDIA(adapterDesc.VendorId))
    {
    ...
    workarounds.flushAfterEndingTransformFeedback = IS_NVIDIA(adapterDesc.VendorId);
    workarounds.getDimensionsIgnoresBaseLevel     = IS_NVIDIA(adapterDesc.VendorId);

    workarounds.preAddTexelFetchOffsets = IS_INTEL(adapterDesc.VendorId);
    workarounds.disableB5G6R5Support    = IS_INTEL(adapterDesc.VendorId);
    workarounds.rewriteUnaryMinusOperator =
        IS_INTEL(adapterDesc.VendorId) &&
        (IsBroadwell(adapterDesc.DeviceId) || IsHaswell(adapterDesc.DeviceId));
    workarounds.emulateIsnanFloat =
        IS_INTEL(adapterDesc.VendorId) && IsSkylake(adapterDesc.DeviceId);

    // TODO(jmadill): Disable when we have a fixed driver version.
    workarounds.emulateTinyStencilTextures = IS_AMD(adapterDesc.VendorId);
