...
IntelDriverVersion GetIntelDriverVersion(const Optional<LARGE_INTEGER> driverVersion)
{
    ...
    uint64_t intVersion        = driverVersion.value().QuadPart;
    constexpr uint64_t kMask16 = std::numeric_limits<uint16_t>::max();
    uint32_t buildNumber       = intVersion & kMask16;
    uint16_t thirdPart         = (intVersion >> 16) & kMask16;
    if (thirdPart >= 100)
    {
        buildNumber += thirdPart * 10000;
    }
