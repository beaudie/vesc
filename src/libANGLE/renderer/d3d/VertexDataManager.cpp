...
int ElementsInBuffer(const gl::VertexAttribute &attrib, unsigned int size)
{
    ...
    GLsizei offset = static_cast<GLsizei>(ComputeVertexAttributeOffset(attrib));
...
{
    const gl::AttribFormat &format = attrib.getAttribFormat();
    ...
    const gl::AttribBinding &binding = attrib.getAttribBinding();
    ...
    GLintptr offset = ComputeVertexAttributeOffset(attrib);
                                               ...
                                               GLsizei instances)
{
    ...
    for (size_t attribIndex = 0; attribIndex < vertexAttributes.size(); ++attribIndex)
    {
        ...
        translated->divisor          = attrib.getAttribBinding().divisor;
...
void VertexDataManager::StoreDirectAttrib(TranslatedAttribute *directAttrib)
{
    ...
    directAttrib->baseOffset = static_cast<unsigned int>(ComputeVertexAttributeOffset(attrib));
                                               ...
                                               GLsizei instances)
{
    ...
    gl::Buffer *buffer = binding.buffer.get();
    ASSERT(buffer && format.enabled && !DirectStoragePossible(attrib));
    BufferD3D *bufferD3D = GetImplAs<BufferD3D>(buffer);

    // Compute source data pointer
    const uint8_t *sourceData = nullptr;
    const int offset          = static_cast<int>(ComputeVertexAttributeOffset(attrib));
    ...
    if (staticBuffer->empty())
    {
        ...
        int startIndex = offset / static_cast<int>(ComputeVertexAttributeStride(attrib));
    ...
    GLsizei count)
{
    ...
    {
        const auto &dynamicAttrib = translatedAttribs[attribIndex];
        const auto &binding       = dynamicAttrib.attribute->getAttribBinding();
        gl::Buffer *buffer        = binding.buffer.get();
        if (buffer)
        {
            BufferD3D *bufferD3D = GetImplAs<BufferD3D>(buffer);
                                                   ...
                                                   GLsizei instances) const
{
    ...
    gl::Buffer *buffer   = attrib.getAttribBinding().buffer.get();
    BufferD3D *bufferD3D = buffer ? GetImplAs<BufferD3D>(buffer) : nullptr;
                                                ...
                                                GLsizei instances)
{
    ...
    if (buffer)
    {
        ...
        sourceData += static_cast<int>(ComputeVertexAttributeOffset(attrib));
    }
    else
    {
        sourceData = static_cast<const uint8_t *>(format.pointer) +
                     static_cast<const uint8_t>(format.relativeOffset);
    }
