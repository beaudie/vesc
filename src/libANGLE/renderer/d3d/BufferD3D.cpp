...
StaticVertexBufferInterface *BufferD3D::getStaticVertexBuffer(const gl::VertexAttribute &attribute)
{
    ...
    if (mStaticVertexBuffer)
    {
        ...
        {
            if (IsUnsignedAdditionSafe(mStaticVertexBuffer->getBufferSize(),
                                       mStaticBufferCacheTotalSize))
            {
                // Ensure that the total size of the static buffer cache remains less than 2x the
                // size of the original buffer
                unsigned int maxStaticCacheSize =
                    IsUnsignedMultiplicationSafe(static_cast<unsigned int>(getSize()), 2u)
                        ? 2u * static_cast<unsigned int>(getSize())
                        : std::numeric_limits<unsigned int>::max();

                // We can't reuse the default static vertex buffer, so we add it to the cache
                if (mStaticVertexBuffer->getBufferSize() + mStaticBufferCacheTotalSize <=
                    maxStaticCacheSize)
                {
                    mStaticBufferCacheTotalSize += mStaticVertexBuffer->getBufferSize();
