                                                              ...
                                                              bool createIfNecessary)
{
    ...
    {
        // In this case, the attribute can be added to mStaticVertexBuffer
    ...
    // We therefore mStaticVertexBuffer for deletion at the next possible time.
...
void BufferD3D::reinitOutOfDateStaticData()
{
    if (mStaticVertexBufferOutOfDate)
    {
        ...
        invalidateStaticData(false);
    }
}

void BufferD3D::invalidateStaticData(bool invalidateWholeCache)
{
    if (invalidateWholeCache && mStaticBufferCache != nullptr)
    {
        // Empty the cache of static vertex buffers too
        for (StaticVertexBufferInterface *staticBuffer : *mStaticBufferCache)
        {
