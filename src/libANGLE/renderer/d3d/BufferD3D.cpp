...
StaticVertexBufferInterface *BufferD3D::getStaticVertexBuffer(const gl::VertexAttribute &attribute)
{
    ...
    // If we're past the threshold, delete as much as we can. We can only delete buffers not
    // currently in-use. Hence the need for checking the data revision.
    if (currentTotalSize > sizeThreshold)
    {
        auto iter = mStaticVertexBuffers.begin();
        while (iter != mStaticVertexBuffers.end())
        {
            auto *staticBuffer = iter->get();
            if (!staticBuffer->isBoundToD3D())
            {
                iter = mStaticVertexBuffers.erase(iter);
            }
            else
            {
                iter++;
