...
std::string Image2DHLSLGroupOffsetSuffix(Image2DHLSLGroup group)
{
    switch (group)
    {
        case IMAGE2D_R_FLOAT4:
...
std::string Image2DHLSLGroupDeclarationSuffix(Image2DHLSLGroup group)
{
    switch (group)
    {
        case IMAGE2D_R_FLOAT4:
...
void OutputImage2DSizeFunction(std::ostringstream &out,
                               ...
                               const std::string &offsetSuffix,
                               const std::string &declarationSuffix)
{
    ...
    if (texture2DCount > 0)
    {
        out << "    if (imageIndex >= " << offsetSuffix << "2D && imageIndex < " << offsetSuffix
            << "2D + " << texture2DCount << ")\n";
        out << "    {\n";
        out << "        const uint index = imageIndex -  " << offsetSuffix << "2D;\n";
        out << "        " << declarationSuffix << "2D[index].GetDimensions(width, height);\n";
        out << "    }\n";
    }

    if (texture3DCount > 0)
    {
        out << "    if (imageIndex >= " << offsetSuffix << "3D && imageIndex < " << offsetSuffix
            << "3D + " << texture3DCount << ")\n";
        out << "    {\n";
        out << "        const uint index = imageIndex -  " << offsetSuffix << "3D;\n";
        out << "        uint depth;\n";
        out << "        " << declarationSuffix
    ...
    {
        out << "    if (imageIndex >= " << offsetSuffix << "2DArray && imageIndex < "
            << offsetSuffix << "2DArray + " << texture2DArrayCount << ")\n";
        out << "    {\n";
        out << "        const uint index = imageIndex -  " << offsetSuffix << "2DArray;\n";
        out << "        uint depth;\n";
        out << "        " << declarationSuffix
                               ...
                               const std::string &declarationSuffix)
{
    ...
    if (texture2DCount > 0)
    {
        ...
        out << "    {\n";
        out << "        const uint index = imageIndex -  " << offsetSuffix << "2D;\n";
        out << "        result = " << declarationSuffix << "2D[index][uint2(p.x, p.y)];\n";
        out << "    }\n";
    }
                                ...
                                const std::string &declarationSuffix)
{
    ...
    if (texture2DCount > 0)
    {
        ...
        out << "    {\n";
        out << "        const uint index = imageIndex -  " << offsetSuffix << "2D;\n";
        out << "        " << declarationSuffix << "2D[index][uint2(p.x, p.y)] = data;\n";
        out << "    }\n";
    }
    ...
    std::map<unsigned int, unsigned int> &image2DImageIndexCS)
{
    ...
    std::string offsetSuffix =
        Image2DHLSLGroupOffsetSuffix(textureGroup) + Image2DHLSLGroupSuffix(textureGroup);
    std::string declarationSuffix =
        Image2DHLSLGroupDeclarationSuffix(textureGroup) + Image2DHLSLGroupSuffix(textureGroup);
    std::string registerSuffix = Image2DHLSLGroupRegisterSuffix(textureGroup);
    if (texture2DCount > 0)
    {
        out << "static const uint " << offsetSuffix << "2D = " << *image2DTexture2D << ";\n";
        out << "uniform " << Image2DHLSLTextureString(textureGroup, gl::TextureType::_2D) << " "
            << declarationSuffix << "2D[" << texture2DCount << "]"
            << " : register(" << registerSuffix << *image2DRegisterIndex << ");\n";
        *image2DRegisterIndex += texture2DCount;
    }
    if (texture3DCount > 0)
    {
        out << "static const uint " << offsetSuffix << "3D = " << *image2DTexture3D << ";\n";
        out << "uniform " << Image2DHLSLTextureString(textureGroup, gl::TextureType::_3D) << " "
            << declarationSuffix << "3D[" << texture3DCount << "]"
            << " : register(" << registerSuffix << *image2DRegisterIndex << ");\n";
        *image2DRegisterIndex += texture3DCount;
    }
    if (texture2DArrayCount > 0)
    {
        out << "static const uint " << offsetSuffix << "2DArray = " << *image2DTexture2DArray
            << ";\n";
        out << "uniform " << Image2DHLSLTextureString(textureGroup, gl::TextureType::_2DArray)
            << " " << declarationSuffix << "2DArray[" << texture2DArrayCount << "]"
            << " : register(" << registerSuffix << *image2DRegisterIndex << ");\n";
    ...
    if (computeShader->useImage2DFunction(Image2DHLSLGroupFunctionName(textureGroup, IMAGE2DSIZE)))
    {
        OutputImage2DSizeFunction(out, textureGroup, texture2DCount, texture3DCount,
                                  texture2DArrayCount, offsetSuffix, declarationSuffix);
    }
