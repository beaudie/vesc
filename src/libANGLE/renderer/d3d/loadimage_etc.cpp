...
struct ETC2Block
{
                        ...
                        bool punchThroughAlpha) const
    {
        ...
        if (u.idht.mode.idm.diffbit || punchThroughAlpha)
        {
            ...
            if (r < 0 || r > 31)
            {
                transcodeTBlockToBC1(dest, x, y, w, h, alphaValues,
                                  nonOpaquePunchThroughAlpha);
            }
            else if (g < 0 || g > 31)
            {
                transcodeHBlockToBC1(dest, x, y, w, h, alphaValues,
                                  nonOpaquePunchThroughAlpha);
            }
            else if (b < 0 || b > 31)
            {
  ...
  private:
    ...
    uint32_t matchBC1Bits(const R8G8B8A8* rgba,
                          const R8G8B8A8& minClr,
                          const R8G8B8A8& maxClr,
                          bool opaque) const
    {
        uint8_t baseClr[4][3] = 
        {
            { maxClr.R, maxClr.G, maxClr.B },
            { minClr.R, minClr.G, minClr.B }
        };
        if (opaque)
        {
            for (int ch = 0; ch < 3; ch++)
            {
                baseClr[2][ch] = (baseClr[0][ch] * 2 + baseClr[1][ch]) / 3;
                baseClr[3][ch] = (baseClr[0][ch] + baseClr[1][ch] * 2) / 3;
            }
        }
        ...
        for (int ch = 0; ch < 3; ch++)
        {
            dir[ch] = baseClr[0][ch] - baseClr[1][ch];
        }
        ...
        if (opaque)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                stops[i] = baseClr[i][0] * dir[0] + 
                           baseClr[i][1] * dir[1] + 
                           baseClr[i][2] * dir[2];
            }

            int c0Point   = (stops[1] + stops[3]) / 2;
            int halfPoint = (stops[3] + stops[2]) / 2;
            int c3Point   = (stops[2] + stops[0]) / 2;
            for (int i = 15; i >= 0; i--)
            {
                bits <<= 2;
                const int dot = rgba[i].R * dir[0] + 
                                rgba[i].G * dir[1] +
                                rgba[i].B * dir[2];
                if (dot < halfPoint)
                {
                    bits |= (dot < c0Point) ? 1 : 3;
                }
                else
                {
                    bits |= (dot < c3Point) ? 2 : 0;
                }
            }
        }
        else
                 ...
                 bool opaque) const
    {
        ...
        max16 = RGB8ToRGB565(maxClr);
        min16 = RGB8ToRGB565(minClr);
        if (max16 != min16)
        {
            bits = matchBC1Bits(rgba, minClr, maxClr, opaque);
        }
        else
        {
            bits = 0;
                                                     ...
                                                     bool nonOpaquePunchThroughAlpha) const
    {
        ...
        // clang-format off
        static const int valueMappingTable[4] =
        {
            2, 3, 1, 0
        };
        // clang-format on
        ...
        if (u.idht.mode.idm.flipbit)
        {
            for (size_t j = 0; j < 2 && (y + j) < h; j++)
            {
                ...
                for (size_t i = 0; i < 4 && (x + i) < w; i++)
                {
                    ...
                    if (valueMappingTable[pixelIndex] < valueMappingTable[pixelRange[0][0]])
                    {
                        pixelRange[0][0] = pixelIndex;
                    }
                    if (valueMappingTable[pixelIndex] > valueMappingTable[pixelRange[0][1]])
                    {
                        pixelRange[0][1] = pixelIndex;
                    }

                    row[i]   = subblockColors0[pixelIndex];
                    row[i].A = alphaValues[j][i];
                }
            }
            for (size_t j = 2; j < 4 && (y + j) < h; j++)
            {
                ...
                {
                    const size_t pixelIndex = getIndex(i, j);
                    if (valueMappingTable[pixelIndex] < valueMappingTable[pixelRange[1][0]])
                    {
                        pixelRange[1][0] = pixelIndex;
                    }
                    if (valueMappingTable[pixelIndex] > valueMappingTable[pixelRange[1][1]])
                    {
                        pixelRange[1][1] = pixelIndex;
                    }

                    row[i]   = subblockColors1[pixelIndex];
                    row[i].A = alphaValues[j][i];
                }
            }
        }
        ...
        if (nonOpaquePunchThroughAlpha)
        {
            decodePunchThroughAlphaBlock(reinterpret_cast<uint8_t*>(rgbaBlock), x, y, w, h, sizeof(R8G8B8A8) * 4);
        }

        R8G8B8A8 minClr;
        const R8G8B8A8& minClr0 = subblockColors0[pixelRange[0][0]];
        const R8G8B8A8& minClr1 = subblockColors1[pixelRange[1][0]];
        if (minClr0.R + minClr0.G + minClr0.B < minClr1.R + minClr1.G + minClr1.B)
        {
            minClr = minClr0;
        }
        else
        ...
        R8G8B8A8 maxClr;
        const R8G8B8A8& maxClr0 = subblockColors0[pixelRange[0][1]];
        const R8G8B8A8& maxClr1 = subblockColors1[pixelRange[1][1]];
        if (maxClr0.R + maxClr0.G + maxClr0.B < maxClr1.R + maxClr1.G + maxClr1.B)
        {
            maxClr = maxClr1;
        }
        else
        ...
        packBC1(dest, rgbaBlock, minClr, maxClr, !nonOpaquePunchThroughAlpha);
    }
    ...
    for (size_t z = 0; z < depth; z++)
    {
        for (size_t y = 0; y < height; y += 4)
        {
            ...
            for (size_t x = 0; x < width; x += 4)
            {
                ...
                sourceBlock->transcodeAsBC1(destPixels, x, y, width, height,
                                            DefaultETCAlphaValues, punchthroughAlpha);
            }
        }
    }
