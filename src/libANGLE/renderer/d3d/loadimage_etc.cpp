...
struct ETC2Block
{
  ...
  private:
    ...
    uint32_t matchBC1Bits(const R8G8B8A8 *rgba,
                          ...
                          bool opaque) const
    {
        uint8_t baseClr[4][3] = {{maxClr.R, maxClr.G, maxClr.B}, {minClr.R, minClr.G, minClr.B}};
        if (opaque)
        {
        ...
        if (opaque)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                stops[i] = baseClr[i][0] * dir[0] + baseClr[i][1] * dir[1] + baseClr[i][2] * dir[2];
            }
            ...
            {
                bits <<= 2;
                const int dot = rgba[i].R * dir[0] + rgba[i].G * dir[1] + rgba[i].B * dir[2];
                if (dot < halfPoint)
                {
