...
struct ETC2Block
{
  ...
  private:
    ...
    static inline signed char clampSByte(int value)
    {
        return static_cast<signed char>(gl::clamp(value, -128, 127));
    }

    static inline R8G8B8A8 createRGBA(int red, int green, int blue, int alpha)
    {
        R8G8B8A8 rgba;
    ...
    static inline R8G8B8A8 createRGBA(int red, int green, int blue)
    {
        return createRGBA(red, green, blue, 255);
    }

    static inline int extend_4to8bits(int x) { return (x << 4) | x; }

    static inline int extend_5to8bits(int x) { return (x << 3) | (x >> 2); }

    static inline int extend_6to8bits(int x) { return (x << 2) | (x >> 4); }

    static inline int extend_7to8bits(int x) { return (x << 1) | (x >> 6); }
...
bool ETCDecoder::Decode(const unsigned char *src,
                        unsigned char *dst,
                        int w,
                        int h,
                        int dstW,
    ...
    switch (inputType)
    {
        ...
        case ETC_R_UNSIGNED:
            for (int y = 0; y < h; y += 4)
            {
                ...
                {
                    ETC2::DecodeBlock(sources, dstRow + (x * dstBpp), 1, x, y, dstW, dstH, dstPitch,
                                      inputType == ETC_R_SIGNED);
                }
            }
            break;
        case ETC_RG_SIGNED:
        case ETC_RG_UNSIGNED:
            ...
            for (int y = 0; y < h; y += 4)
            {
                ...
                {
                    ETC2::DecodeBlock(sources, dstRow + (x * dstBpp), 2, x, y, dstW, dstH, dstPitch,
                                      inputType == ETC_RG_SIGNED);
                }
            }
            break;
        case ETC_RGB:
        case ETC_RGB_PUNCHTHROUGH_ALPHA:
            for (int y = 0; y < h; y += 4)
            {
                ...
                for (int x = 0; x < w; x += 4, sources[0]++)
                {
                    sources[0]->decodeBlock(dstRow + (x * dstBpp), x, y, dstW, dstH, dstPitch,
                                            alphaValues, inputType == ETC_RGB_PUNCHTHROUGH_ALPHA);
                }
            }
            break;
        case ETC_RGBA:
            for (int y = 0; y < h; y += 4)
            {
                ...
                {
                    // Decode Alpha
                    ETC2::DecodeBlock(&sources[0], &(alphaValues[0][0]), 1, x, y, dstW, dstH, 4,
                                      false);
                    sources[0]++;  // RGBA packets are 128 bits, so move on to the next 64 bit
                                   // packet to decode the RGB color
                                   // Decode RGB
                    sources[0]->decodeBlock(dstRow + (x * dstBpp), x, y, dstW, dstH, dstPitch,
                                            alphaValues, false);
                    sources[0]++;
