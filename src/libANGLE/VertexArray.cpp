                                       ...
                                       GLsizei stride)
{
    ...
    bool sameBuffer = oldBuffer == boundBuffer;
    bool sameStride = static_cast<GLuint>(stride) == binding->getStride();
    bool sameOffset = offset == binding->getOffset();
                                                          ...
                                                          const void *pointer)
{
    ...
    if (bindVertexBufferImpl(context, attribIndex, boundBuffer, offset, effectiveStride) &&
        !attribDirty)
    {
        setDirtyBindingBit(attribIndex, DIRTY_BINDING_ONLY_BUFFER_AND_OFFSET);
    }
    else if (attribDirty)
    {
