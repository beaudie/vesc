...
void VertexArray::setElementArrayBuffer(const Context *context, Buffer *buffer)
{
    ...
    mElementArrayBufferObserverBinding.bindGeneric(buffer);
...
void VertexArray::setDependentDirtyBit(size_t dirtyBit)
{
                                       ...
                                       angle::SubjectMessage message)
{
    ...
    {
        setDependentDirtyBit(contentsChanged ? DIRTY_BIT_ELEMENT_ARRAY_BUFFER_DATA
    ...
    else
    {
        ...
        setDependentDirtyBit(DIRTY_BIT_BINDING_0 + index);
        mDirtyBindingBits[index].set(contentsChanged ? DIRTY_BINDING_BUFFER_DATA
...
void VertexArray::updateObserverBinding(size_t bindingIndex)
{
    ...
    mArrayBufferObserverBindings[bindingIndex].bindGeneric(boundBuffer);
}
