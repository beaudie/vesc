                                                          ...
                                                          const void *pointer)
{
    ...
    // Whether binding data change from "memory to buffer" or "buffer to memory"
    // i.e. one of oldBuffer and boundBuffer is zero
    const VertexBinding &binding     = mState.mVertexBindings[attribIndex];
    bool bindingChangeMemoryToBuffer = (boundBuffer == 0) ^ (binding.getBuffer().get() == 0);
    ...
    if (attribDirty || bindingChangeMemoryToBuffer)
    {
