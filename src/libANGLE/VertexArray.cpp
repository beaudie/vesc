    ...
    : mLabel(), mAttributeBindings(maxAttribBindings), mMaxEnabledAttribute(0)
{
    ...
    {
        mVertexAttributes.emplace_back(static_cast<GLuint>(i), &mAttributeBindings);
    }
}
...
{
    ASSERT(attribIndex < getMaxAttribs());
    mState.mVertexAttributes[attribIndex].format.enabled = enabledState;
    ...
    else if (mState.mMaxEnabledAttribute == attribIndex + 1)
    {
        while (mState.mMaxEnabledAttribute > 0 &&
               !mState.mVertexAttributes[mState.mMaxEnabledAttribute - 1].format.enabled)
        {
