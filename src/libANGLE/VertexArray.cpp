        ...
        if (bufferBinding.id() == bufferID)
        {
            if (isBound)
            {
                if (bufferBinding.get())
                {
                    ...
                    bufferBinding->getVertexArrayBufferBindingMask().reset(bindingIndex);
                }
            }
                                                                ...
                                                                GLsizei stride)
{
    ...
    if (oldBuffer)
    {
        ...
        oldBuffer->getVertexArrayBufferBindingMask().reset(bindingIndex);
    ...
    if (boundBuffer)
    {
        ...
        boundBuffer->getVertexArrayBufferBindingMask().set(bindingIndex);
        if (context->isWebGL())
        {
...
void VertexArray::onBind(const Context *context)
{
    ...
    for (size_t bindingIndex : mState.getBufferBindingMask())
    {
        ...
        bufferGL->getVertexArrayBufferBindingMask().set(bindingIndex);
...
void VertexArray::onUnbind(const Context *context)
{
    ...
    for (size_t bindingIndex : mState.mBufferBindingMask)
    {
        ...
        Buffer *bufferGL             = binding.getBuffer().get();
        ASSERT(bufferGL != nullptr);
        bufferGL->getVertexArrayBufferBindingMask().reset(bindingIndex);
    }
}
...
void VertexArray::onBufferStateChange(angle::SubjectMessage message,
                                      VertexArrayBufferBindingMask bindingMask)
{
    switch (message)
    {
        ...
        case angle::SubjectMessage::InternalMemoryAllocationChanged:
            ASSERT(!bindingMask.test(kElementArrayBufferIndex));
            {
                DirtyBits dirtyBits = DirtyBits(bindingMask.to_ulong() << DIRTY_BIT_BINDING_0);
                ASSERT(!mDirtyBitsGuard.valid() ||
                       (mDirtyBitsGuard.value() & dirtyBits) == dirtyBits);
