    ...
    : mLabel(), mVertexBindings(maxAttribBindings)
{
    ...
    for (size_t i = 0; i < maxAttribs; i++)
    {
        ...
        mBoundAttributesMaskOnBindings[i].set(i);
    }
}
...
AttributesMask VertexArrayState::getBoundAttributesMaskOnBinding(GLuint bindingIndex) const
{
    ASSERT(bindingIndex < MAX_VERTEX_ATTRIB_BINDINGS);
    return mBoundAttributesMaskOnBindings[bindingIndex];
}
