        ...
        if (bufferBinding.id() == bufferID)
        {
            if (isBound)
            {
                if (bufferBinding.get())
                {
                    ...
                    bufferBinding->onVertexBufferUnbound(bindingIndex);
                }
            }
                                                                ...
                                                                GLsizei stride)
{
    ...
    if (oldBuffer)
    {
        ...
        boundBuffer->onVertexBufferUnbound(bindingIndex);
    ...
    if (boundBuffer)
    {
        ...
        boundBuffer->onVertexBufferBound(bindingIndex);
        if (context->isWebGL())
        {
...
void VertexArray::onBind(const Context *context)
{
    ...
    for (size_t bindingIndex : mState.getBufferBindingMask())
    {
        ...
        bufferGL->onVertexBufferBound(bindingIndex);
...
void VertexArray::onUnbind(const Context *context)
{
    ...
    for (size_t bindingIndex : mState.mBufferBindingMask)
    {
        ...
        Buffer *bufferGL             = binding.getBuffer().get();
        ASSERT(bufferGL != nullptr);
        bufferGL->onVertexBufferUnbound(bindingIndex);
    }
}
                                      ...
                                      VertexArrayBufferBindingMask bindingMask)
{
    switch (message)
    {
        ...
        case angle::SubjectMessage::InternalMemoryAllocationChanged:
            if (bindingMask.test(kElementArrayBufferIndex))
            {
                mDirtyBits.set(DIRTY_BIT_ELEMENT_ARRAY_BUFFER);
                bindingMask.reset(kElementArrayBufferIndex);
            }
            if (bindingMask.any())
            {
