...
struct LinkedUniform : public sh::Uniform, public ActiveVariable
{
    ...
    inline bool isSampler() const { return typeInfo->isSampler; }
    inline bool isImage() const { return typeInfo->isImageType; }
    inline bool isAtomicCounter() const { return IsAtomicCounterType(type); }
    inline bool isInDefaultBlock() const { return bufferIndex == -1; }
    inline bool isField() const { return name.find('.') != std::string::npos; }
    inline size_t getElementSize() const { return typeInfo->externalSize; }
    inline size_t getElementComponents() const { return typeInfo->componentCount; }
