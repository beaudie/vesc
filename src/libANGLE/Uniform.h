...
struct BufferVariable
{
    ...
    uint16_t getBasicTypeElementCount() const { return pod.basicTypeElementCount; }
...
{
    ShaderVariableBuffer() { memset(&pod, 0, sizeof(pod)); }
...
{
    InterfaceBlock() { memset(&pod, 0, sizeof(pod)); }
    ...
    struct PODStruct
    {
        uint16_t arrayElement;
        uint16_t firstFieldArraySize;
