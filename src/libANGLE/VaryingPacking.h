...
struct VaryingInShaderRef : angle::NonCopyable
{
    ...
    void copyNamesFrom(const VaryingInShaderRef &other);
...
struct PackedVarying : angle::NonCopyable
{
    ...
    std::string fullName(ShaderType stage) const
    {
        ...
        if (isStructField())
        {
            ...
            if (isNestedField() && parentArrayIndex != GL_INVALID_INDEX)
            {
                fullNameStr << "[" << parentArrayIndex << "]";
            }
            fullNameStr << ".";
        }
