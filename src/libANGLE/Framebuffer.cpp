...
bool CheckRenderToTextureCreationMatches(const FramebufferAttachment &attachment,
                                         Optional<bool> *renderToTextureEXT)
{
    bool renderToTexture = attachment.getRenderToTextureSamples() !=
                           FramebufferAttachment::kDefaultRenderToTextureSamples;

    if (renderToTextureEXT->valid())
    {
        if (renderToTextureEXT->value() != renderToTexture)
        {
            return false;
        }
    }
    ...
    {
        *renderToTextureEXT = renderToTexture;
    }
    return true;
}
...
GLenum Framebuffer::checkStatusWithGLFrontEnd(const Context *context)
{
    ...
    for (const FramebufferAttachment &colorAttachment : mState.mColorAttachments)
    {
        if (colorAttachment.isAttached())
        {
            ...
            if (!CheckRenderToTextureCreationMatches(colorAttachment, &renderToTextureEXT))
            {
    ...
    if (depthAttachment.isAttached())
    {
        ...
        if (!CheckRenderToTextureCreationMatches(depthAttachment, &renderToTextureEXT))
        {
            return GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT;
        }
    ...
    if (stencilAttachment.isAttached())
    {
        ...
        if (!CheckRenderToTextureCreationMatches(stencilAttachment, &renderToTextureEXT))
        {
            return GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT;
        }
