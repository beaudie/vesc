...
GLenum Framebuffer::checkStatusWithGLFrontEnd(const Context *context)
{
    ...
    TextureTarget colorAttachmentsTextureTarget = TextureTarget::InvalidEnum;
    ...
    for (const FramebufferAttachment &colorAttachment : mState.mColorAttachments)
    {
        if (colorAttachment.isAttached())
        {
            ...
            if (!hasAttachments)
            {
                ...
                if (isLayered)
                {
                    colorAttachmentsTextureTarget =
                        colorAttachment.getTextureImageIndex().getTarget();
                }
                hasAttachments = true;
            }
            else
            {
                ...
                if (isLayered != colorAttachment.isLayered())
                {
                    return GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_EXT;
                }
                else if (isLayered && colorAttachmentsTextureTarget !=
                                          colorAttachment.getTextureImageIndex().getTarget())
                {
                    return GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_EXT;
                }
    ...
    if (depthAttachment.isAttached())
    {
        ...
        else
        {
            ...
            if (isLayered != depthAttachment.isLayered())
            {
                return GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_EXT;
            }
        }
    ...
    if (stencilAttachment.isAttached())
    {
        ...
        else
        {
            ...
            if (isLayered != stencilAttachment.isLayered())
            {
                return GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_EXT;
            }
        }
