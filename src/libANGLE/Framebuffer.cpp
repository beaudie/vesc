      ...
      mDirtyStencilAttachmentBinding(this, DIRTY_BIT_STENCIL_ATTACHMENT)
{
    ...
    setAttachmentImpl(context, GL_FRAMEBUFFER_DEFAULT, GL_BACK, ImageIndex(), surface,
                      ...
                      FramebufferAttachment::kDefaultRenderToTextureLevel);
...
GLenum Framebuffer::checkStatusWithGLFrontEnd(const Context *context)
{
    ...
    // #TODO check for multisample incomplete framebuffer
                                           ...
                                           GLint level)
{
    setAttachment(context, type, binding, textureIndex, resource,
                  FramebufferAttachment::kDefaultNumViews,
                  FramebufferAttachment::kDefaultBaseViewIndex, false, samples, level);
}
    ...
    switch (binding)
    {
        ...
        default:
            setAttachmentImpl(context, type, binding, textureIndex, resource, numViews,
                              baseViewIndex, isMultiview, samples, level);
            return;
    }
                                                       ...
                                                       GLint level)
{
    ...
    else if (mState.mWebGLStencilAttachment.isAttached())
    {
        ...
        setAttachmentImpl(context, stencil.type(), GL_STENCIL_ATTACHMENT,
                          ...
                          numViews, baseViewIndex, isMultiview, samples, level);
    }
    else if (mState.mWebGLDepthStencilAttachment.isAttached())
    {
        ...
        setAttachmentImpl(context, depthStencil.type(), GL_DEPTH_ATTACHMENT,
                          ...
                          depthStencil.getResource(), numViews, baseViewIndex, isMultiview, samples,
                          level);
    ...
    switch (binding)
    {
        ...
        case GL_DEPTH_STENCIL_ATTACHMENT:
        {
            ...
            updateAttachment(context, &mState.mDepthAttachment, DIRTY_BIT_DEPTH_ATTACHMENT,
                             ...
                             attachmentObj, numViews, baseViewIndex, isMultiview, samples, level);
        ...
        case GL_BACK:
            updateAttachment(context, &mState.mColorAttachments[0], DIRTY_BIT_COLOR_ATTACHMENT_0,
                             ...
                             resource, numViews, baseViewIndex, isMultiview, samples, level);
            break;
...
angle::Result Framebuffer::resolveMultisampledAttachments(const Context *context)
{
    // color0 only color0 can be attached wiht framebuffertexture2dMS
    if (mState.mColorAttachments.size() > 0)
    {
        if (mState.mColorAttachments[0].isAttached())
        {
            return mState.mColorAttachments[0].resolveAttachment(context);
        }
        // depth/stencil later? renderbufferstorage
    }
    return angle::Result::Continue;
}
