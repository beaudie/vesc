...
bool Framebuffer::formsRenderingFeedbackLoopWith(const Context *context) const
{
    ...
    {
        const Texture *texture = glState.getSamplerTexture(static_cast<unsigned int>(textureIndex),
                                                           textureTypes[textureIndex]);
        if (texture && texture->isBoundToFramebuffer(mState.mFramebufferSerial))
        {
