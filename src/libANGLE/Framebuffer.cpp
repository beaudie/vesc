      ...
      mDirtyStencilAttachmentBinding(this, DIRTY_BIT_STENCIL_ATTACHMENT)
{
    ...
    setAttachmentImpl(proxyContext, GL_FRAMEBUFFER_DEFAULT, GL_BACK, gl::ImageIndex::MakeInvalid(),
                      surface, FramebufferAttachment::DefaultNumViews,
                      FramebufferAttachment::DefaultBaseViewIndex,
                      FramebufferAttachment::DefaultMultiviewLayout,
                      FramebufferAttachment::DefaultViewportOffsets);

    if (surface->getConfig()->depthSize > 0)
    {
        setAttachmentImpl(
            proxyContext, GL_FRAMEBUFFER_DEFAULT, GL_DEPTH, gl::ImageIndex::MakeInvalid(), surface,
            FramebufferAttachment::DefaultNumViews, FramebufferAttachment::DefaultBaseViewIndex,
            FramebufferAttachment::DefaultMultiviewLayout,
            FramebufferAttachment::DefaultViewportOffsets);
    }
    ...
    if (surface->getConfig()->stencilSize > 0)
    {
        setAttachmentImpl(proxyContext, GL_FRAMEBUFFER_DEFAULT, GL_STENCIL,
                          ...
                          FramebufferAttachment::DefaultNumViews,
                          FramebufferAttachment::DefaultBaseViewIndex,
                          FramebufferAttachment::DefaultMultiviewLayout,
                          FramebufferAttachment::DefaultViewportOffsets);
    }
...
{
    setAttachment(
        context, type, binding, textureIndex, resource, FramebufferAttachment::DefaultNumViews,
        FramebufferAttachment::DefaultBaseViewIndex, FramebufferAttachment::DefaultMultiviewLayout,
        FramebufferAttachment::DefaultViewportOffsets);
}
...
void Framebuffer::setAttachmentMultiviewLayered(const Context *context,
                                                ...
                                                GLsizei numViews)
{
    std::vector<GLint> viewportOffsets(static_cast<size_t>(numViews) * 2u);
    setAttachment(context, type, binding, textureIndex, resource, numViews, baseViewIndex,
                  GL_FRAMEBUFFER_MULTIVIEW_LAYERED_ANGLE, viewportOffsets.data());
}
