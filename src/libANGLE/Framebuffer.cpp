...
static bool IsValidTextureAttachmentIndex(const FramebufferAttachment *attachment)
{
...
static bool IsValidTextureAttachmentIndex_(const FramebufferAttachment *attachment)
{
    Texture *texture = attachment->getTexture();
    const ImageIndex &imageIndex = *attachment->getTextureImageIndex();

    return texture->isValidImageIndex(imageIndex);
}
...
GLenum Framebuffer::checkStatus(const gl::Data &data) const
{
    ...
    for (unsigned int colorAttachment = 0; colorAttachment < IMPLEMENTATION_MAX_DRAW_BUFFERS; colorAttachment++)
    {
        ...
        if (colorbuffer)
        {
            ...
            if (colorbuffer->type() == GL_TEXTURE)
            {
                ...
                if (!IsValidTextureAttachmentIndex(colorbuffer))
                {
                    return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
                }
    ...
    if (mDepthbuffer)
    {
        ...
        if (mDepthbuffer->type() == GL_TEXTURE)
        {
            ...
            if (!IsValidTextureAttachmentIndex(mDepthbuffer))
            {
                return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
            }
    ...
    if (mStencilbuffer)
    {
        ...
        if (mStencilbuffer->type() == GL_TEXTURE)
        {
            ...
            if (!IsValidTextureAttachmentIndex(mStencilbuffer))
            {
                return GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
            }
