...
Error Framebuffer::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    bool depth   = (mask & GL_DEPTH_BUFFER_BIT) != 0 && depthStencil.depthMask;
    ...
    if (glState.isRobustResourceInitEnabled())
    {
        ...
        if (color)
        {
            ...
            partialMask = !(blend.colorMaskRed && blend.colorMaskGreen && blend.colorMaskBlue &&
        ...
        if (stencil)
        {
            // If entire stencil is masked out, disable stencil clear. If partially masked,
            // pre-clear. Otherwise mark clean after the clear impl method.
            if ((depthStencil.stencilMask & depthStencil.stencilWritemask) == 0)
            {
                stencil = false;
            }
            else if (depthStencil.stencilMask != depthStencil.stencilWritemask)
            {
                partialMask = true;
            }
        }
