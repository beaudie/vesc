...
bool Framebuffer::formsRenderingFeedbackLoopWith(const Context *context) const
{
    ...
    for (size_t textureUnit : activeTextures)
    {
        ...
        GLenum programTextureID = state.getSamplerTextureId(textureUnit, texture->getType());
        ...
        if (texture->isSamplerComplete(context, state.getSampler(textureUnit)))
        {
            for (size_t drawIndex : mColorAttachmentBits)
            {
                ...
                if (attachment.type() == GL_TEXTURE && programTextureID == attachment.id())
                {
