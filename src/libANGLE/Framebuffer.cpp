...
{
    // Mark attachments as uninitialized.
    markAttachmentsAsInitNeeded(context, count, attachments);
...
{
    // Mark attachments as uninitialized.
    // TODO(jmadill): Use area parameter somehow.
    markAttachmentsAsInitNeeded(context, count, attachments);
...
Error Framebuffer::clear(const gl::Context *context, GLbitfield mask)
{
    ...
    // If color is masked out, disable color clear.
    ...
    const auto &depthStencil = glState.getDepthStencilState();
    if (depth && !depthStencil.depthMask)
    {
        depth = false;
    }

    // If entire stencil is masked out, disable stencil clear.
    if (stencil && (depthStencil.stencilMask & depthStencil.stencilWritemask) == 0)
    {
        stencil = false;
    }
...
void Framebuffer::markAttachmentsAsInitNeeded(const Context *context,
                                              size_t count,
                                              const GLenum *attachments)
{
    if (context->isRobustResourceInitEnabled())
    {
        for (size_t index = 0; index < count; ++index)
        {
            const FramebufferAttachment *attachment = getAttachment(attachments[index]);
            if (attachment)
            {
