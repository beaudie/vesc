                                                                   ...
                                                                   GLint drawbuffer)
{
    ...
    if (partialBufferClearNeedsInit(context, buffer))
    {
        ...
        switch (buffer)
        {
            case GL_COLOR:
            {
                ...
                if (mState.mResourceNeedsInit[drawbuffer] &&
                    (mState.mColorAttachments[drawbuffer].initState() == InitState::MayNeedInit))
                {
                    clearColorAttachments.set(drawbuffer);
                    mState.mResourceNeedsInit[drawbuffer] = false;
                }
                break;
            }
            case GL_DEPTH:
            {
                if (mState.mResourceNeedsInit[DIRTY_BIT_DEPTH_ATTACHMENT] &&
                    (mState.mDepthAttachment.initState() == InitState::MayNeedInit))
                {
                    clearDepth                                            = true;
                    mState.mResourceNeedsInit[DIRTY_BIT_DEPTH_ATTACHMENT] = false;
                }
                break;
            }
            case GL_STENCIL:
            {
                if (mState.mResourceNeedsInit[DIRTY_BIT_STENCIL_ATTACHMENT] &&
                    (mState.mStencilAttachment.initState() == InitState::MayNeedInit))
                {
                    clearStencil                                            = true;
                    mState.mResourceNeedsInit[DIRTY_BIT_STENCIL_ATTACHMENT] = false;
                }
                break;
            }
            case GL_DEPTH_STENCIL:
        ...
        {
            ANGLE_TRY(mImpl->ensureAttachmentsInitialized(context, clearColorAttachments,
...
angle::Result Framebuffer::ensureDrawAttachmentsInitialized(const Context *context)
{
    ...
    for (size_t bit : mState.mResourceNeedsInit)
    {
        ...
        {
            case DIRTY_BIT_DEPTH_ATTACHMENT:
                if (mState.mDepthAttachment.initState() == InitState::MayNeedInit)
                {
                    clearDepth = true;
                }
                break;
            case DIRTY_BIT_STENCIL_ATTACHMENT:
                if (mState.mStencilAttachment.initState() == InitState::MayNeedInit)
                {
                    clearStencil = true;
                }
                break;
            default:
                if (mState.mColorAttachments[bit].initState() == InitState::MayNeedInit)
                {
    ...
    if (mState.mReadBufferState != GL_NONE)
    {
        ...
        else
        {
            ...
            if (mState.mResourceNeedsInit[readIndex] &&
                mState.mColorAttachments[readIndex].initState() == InitState::MayNeedInit)
            {
                clearColorAttachments[readIndex]     = true;
                mState.mResourceNeedsInit[readIndex] = false;
            }
        }
    }
