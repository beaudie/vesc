...
const Extents FramebufferState::getCommonIntersectExtent() const
{
    ...
    if (mStencilAttachment.isAttached())
    {
        if (!intersectExtent.valid())
        {
            intersectExtent = mStencilAttachment.getSize();
        }
        else
        {
            intersectExtent.value().width =
                std::min(intersectExtent.value().width, mStencilAttachment.getSize().width);
            intersectExtent.value().height =
                std::min(intersectExtent.value().height, mStencilAttachment.getSize().height);
        }
    }

    ASSERT(intersectExtent.valid());
