...
ResourceMap<ResourceType>::ResourceMap()
    : mFlatResourcesSize(kInitialFlatResourcesSize),
      ...
      mHashedResources()
{
    memset(mFlatResources, kInvalidPointer, mFlatResourcesSize * sizeof(mFlatResources));
}
...
void ResourceMap<ResourceType>::assign(GLuint handle, ResourceType *resource)
{
    if (handle < kFlatResourcesLimit)
    {
        if (handle >= mFlatResourcesSize)
        {
            ...
            size_t halfNewSizeInBytes = mFlatResourcesSize * sizeof(mFlatResources);

            mFlatResources = new ResourceType *[newSize];
            memset(&mFlatResources[mFlatResourcesSize], kInvalidPointer, halfNewSizeInBytes);
            memcpy(mFlatResources, oldResources, halfNewSizeInBytes);
...
{
    memset(mFlatResources, kInvalidPointer, kInitialFlatResourcesSize * sizeof(mFlatResources));
    mFlatResourcesSize = kInitialFlatResourcesSize;
