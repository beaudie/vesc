...
void ResourceMap<ResourceType>::assign(GLuint handle, ResourceType *resource)
{
    if (handle < kFlatResourcesLimit)
    {
        if (handle >= mFlatResourcesSize)
        {
            ...
            size_t oldSizeInBytes = mFlatResourcesSize * sizeof(ResourceType *);

            mFlatResources = new ResourceType *[newSize];
            memset(&mFlatResources[mFlatResourcesSize], kInvalidPointer, oldSizeInBytes);
            memcpy(mFlatResources, oldResources, oldSizeInBytes);
