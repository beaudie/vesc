...
ProgramExecutable::ProgramExecutable()
    : mBasicDataTypeStruct{.maxActiveAttribLocation   = 0,
                           .defaultUniformRange       = RangeUI(0, 0),
                           .samplerUniformRange       = RangeUI(0, 0),
                           .imageUniformRange         = RangeUI(0, 0),
                           .atomicCounterUniformRange = RangeUI(0, 0),
                           .fragmentInoutRange        = RangeUI(0, 0),
                           .hasClipDistance           = false,
                           .hasDiscard                = false,
                           .hasYUVOutput              = false,
                           .enablesPerSampleShading   = false,
                           // [GL_EXT_geometry_shader] Table 20.22
                           .geometryShaderInputPrimitiveType  = PrimitiveMode::Triangles,
                           .geometryShaderOutputPrimitiveType = PrimitiveMode::TriangleStrip,
                           .geometryShaderInvocations         = 1,
                           .geometryShaderMaxVertices         = 0,
                           .tessControlShaderVertices         = 0,
                           .tessGenMode                       = GL_NONE,
                           .tessGenSpacing                    = GL_NONE,
                           .tessGenVertexOrder                = GL_NONE,
                           .tessGenPointMode                  = GL_NONE,
...
ProgramExecutable::ProgramExecutable(const ProgramExecutable &other)
    : mBasicDataTypeStruct(other.mBasicDataTypeStruct),
...
void ProgramExecutable::reset(bool clearInfoLog)
{
    ...
    mBasicDataTypeStruct.activeAttribLocationsMask.reset();
    mBasicDataTypeStruct.attributesTypeMask.reset();
    mBasicDataTypeStruct.attributesMask.reset();
    mBasicDataTypeStruct.maxActiveAttribLocation = 0;
    mBasicDataTypeStruct.activeOutputVariablesMask.reset();
    mBasicDataTypeStruct.hasYUVOutput = false;

    mBasicDataTypeStruct.defaultUniformRange       = RangeUI(0, 0);
    mBasicDataTypeStruct.samplerUniformRange       = RangeUI(0, 0);
    mBasicDataTypeStruct.imageUniformRange         = RangeUI(0, 0);
    mBasicDataTypeStruct.atomicCounterUniformRange = RangeUI(0, 0);

    mBasicDataTypeStruct.fragmentInoutRange      = RangeUI(0, 0);
    mBasicDataTypeStruct.hasClipDistance         = false;
    mBasicDataTypeStruct.hasDiscard              = false;
    mBasicDataTypeStruct.enablesPerSampleShading = false;
    mBasicDataTypeStruct.advancedBlendEquations.reset();

    mBasicDataTypeStruct.geometryShaderInputPrimitiveType  = PrimitiveMode::Triangles;
    mBasicDataTypeStruct.geometryShaderOutputPrimitiveType = PrimitiveMode::TriangleStrip;
    mBasicDataTypeStruct.geometryShaderInvocations         = 1;
    mBasicDataTypeStruct.geometryShaderMaxVertices         = 0;

    mBasicDataTypeStruct.tessControlShaderVertices = 0;
    mBasicDataTypeStruct.tessGenMode               = GL_NONE;
    mBasicDataTypeStruct.tessGenSpacing            = GL_NONE;
...
void ProgramExecutable::load(bool isSeparable, gl::BinaryInputStream *stream)
{
    ...
    stream->readBytes(reinterpret_cast<unsigned char *>(&mBasicDataTypeStruct),
                      sizeof(mBasicDataTypeStruct));
...
void ProgramExecutable::save(bool isSeparable, gl::BinaryOutputStream *stream) const
{
    ...
    ASSERT(mBasicDataTypeStruct.geometryShaderInvocations >= 1 &&
           mBasicDataTypeStruct.geometryShaderMaxVertices >= 0);
    stream->writeBytes(reinterpret_cast<const unsigned char *>(&mBasicDataTypeStruct),
                       sizeof(mBasicDataTypeStruct));
    ...
    const std::vector<std::string> &transformFeedbackVaryingNames)
{
    ...
    for (const std::string &tfVaryingName : transformFeedbackVaryingNames)
    {
        ...
        if (mBasicDataTypeStruct.transformFeedbackBufferMode == GL_SEPARATE_ATTRIBS &&
        ...
        if (mBasicDataTypeStruct.transformFeedbackBufferMode == GL_INTERLEAVED_ATTRIBS &&
    ...
    const ProgramAliasedBindings &fragmentOutputIndices)
{
    ...
    ASSERT(mBasicDataTypeStruct.activeOutputVariablesMask.none());
    ASSERT(mBasicDataTypeStruct.drawBufferTypeMask.none());
    ASSERT(!mBasicDataTypeStruct.hasYUVOutput);
    ...
    for (const sh::ShaderVariable &outputVariable : outputVariables)
    {
        ...
        for (unsigned int elementIndex = 0; elementIndex < elementCount; elementIndex++)
        {
            ...
            ASSERT(location < mBasicDataTypeStruct.activeOutputVariablesMask.size());
            mBasicDataTypeStruct.activeOutputVariablesMask.set(location);
            ...
            SetComponentTypeMask(componentType, location, &mBasicDataTypeStruct.drawBufferTypeMask);
        }

        if (outputVariable.yuv)
        {
            ASSERT(outputVariables.size() == 1);
            mBasicDataTypeStruct.hasYUVOutput = true;
        }
    }
    ...
    if (version >= ES_3_1)
    {
        ...
        if (combinedImageUniformsCount + combinedShaderStorageBlocksCount +
                mBasicDataTypeStruct.activeOutputVariablesMask.count() >
...
{
    UniformLinker linker(mBasicDataTypeStruct.linkedShaderStages, shaderUniforms);
...
void ProgramExecutable::linkSamplerAndImageBindings(GLuint *combinedImageUniforms)
{
    ...
    mBasicDataTypeStruct.fragmentInoutRange = RangeUI(low, high);
    ...
    mBasicDataTypeStruct.atomicCounterUniformRange = RangeUI(low, high);
    ...
    mBasicDataTypeStruct.imageUniformRange = RangeUI(low, high);
    *combinedImageUniforms                 = 0u;
    // If uniform is a image type, insert it into the mImageBindings array.
    for (unsigned int imageIndex : mBasicDataTypeStruct.imageUniformRange)
    {
    ...
    mBasicDataTypeStruct.samplerUniformRange = RangeUI(low, high);

    // If uniform is a sampler type, insert it into the mSamplerBindings array.
    for (unsigned int samplerIndex : mBasicDataTypeStruct.samplerUniformRange)
    {
    ...
    mBasicDataTypeStruct.defaultUniformRange = RangeUI(0, low);
}

bool ProgramExecutable::linkAtomicCounterBuffers(const Context *context, InfoLog &infoLog)
{
    for (unsigned int index : mBasicDataTypeStruct.atomicCounterUniformRange)
    {
...
void ProgramExecutable::copyUniformsFromProgramMap(const ShaderMap<Program *> &programs)
{
    ...
    mBasicDataTypeStruct.defaultUniformRange =
        AddUniforms(programs, mBasicDataTypeStruct.linkedShaderStages, &mUniforms, &mUniformNames,
    ...
    mBasicDataTypeStruct.samplerUniformRange =
        AddUniforms(programs, mBasicDataTypeStruct.linkedShaderStages, &mUniforms, &mUniformNames,
    ...
    mBasicDataTypeStruct.imageUniformRange =
        AddUniforms(programs, mBasicDataTypeStruct.linkedShaderStages, &mUniforms, &mUniformNames,
    ...
    mBasicDataTypeStruct.atomicCounterUniformRange =
        AddUniforms(programs, mBasicDataTypeStruct.linkedShaderStages, &mUniforms, &mUniformNames,
    ...
    mBasicDataTypeStruct.fragmentInoutRange =
        AddUniforms(programs, mBasicDataTypeStruct.linkedShaderStages, &mUniforms, &mUniformNames,
