...
ProgramExecutable::ProgramExecutable(const ProgramExecutable &other)
    : mLinkedShaderStages(other.mLinkedShaderStages),
      ...
      mActiveTextureUnitToSamplerIndexSetMap(other.mActiveTextureUnitToSamplerIndexSetMap),
...
void ProgramExecutable::reset(bool clearInfoLog)
{
    ...
    for (SamplerIndexSet &set : mActiveTextureUnitToSamplerIndexSetMap)
    {
        set.clear();
    }
                                  ...
                                  const gl::LinkedUniform &samplerUniform)
{
    ...
    mActiveTextureUnitToSamplerIndexSetMap[textureUnit].insert(samplerIndex);
}
...
void ProgramExecutable::setInactive(size_t textureUnit)
{
    ...
    mActiveTextureUnitToSamplerIndexSetMap[textureUnit].clear();
}
    ...
    for (uint32_t samplerIndex = 0; samplerIndex < samplerBindings.size(); ++samplerIndex)
    {
        ...
        for (GLint textureUnit : samplerBinding.boundTextureUnits)
        {
            if (++mActiveSamplerRefCounts[textureUnit] == 1)
            {
                ...
                setActive(textureUnit, samplerBinding, samplerIndex,
                          programState.getUniforms()[uniformIndex]);
            }
            else
            {
            ...
            // Irrespective of conflicts mark textureUnit as active and add samplerIndex to map
            mActiveSamplersMask.set(textureUnit);
            mActiveTextureUnitToSamplerIndexSetMap[textureUnit].insert(samplerIndex);
        }
    }
    ...
    for (uint32_t samplerIndex = 0; samplerIndex < samplerBindings.size(); ++samplerIndex)
    {
        ...
        for (GLuint textureUnit : binding.boundTextureUnits)
        {
            ...
            if (!foundBinding)
            {
                ...
                setActive(textureUnit, binding, samplerIndex, mUniforms[uniformIndex]);
            }
            else
            {
