...
ProgramExecutable::ProgramExecutable()
    : mMaxActiveAttribLocation(0),
      ...
      mIsCompute(false),
      mIsExecutableForPipeline(false),
...
ProgramExecutable::ProgramExecutable(const ProgramExecutable &other)
    : mLinkedGraphicsShaderStages(other.mLinkedGraphicsShaderStages),
      ...
      mIsExecutableForPipeline(other.mIsExecutableForPipeline)
{
    reset();
}
...
void ProgramExecutable::load(bool isSeparable, gl::BinaryInputStream *stream)
{
    ...
    mIsExecutableForPipeline    = stream->readBool();
...
void ProgramExecutable::save(bool isSeparable, gl::BinaryOutputStream *stream) const
{
    ...
    stream->writeBool(mIsExecutableForPipeline);
...
{
    if (hasLinkedShaderStage(ShaderType::Geometry))
    {
        return ShaderType::Geometry;
    }
    if (hasLinkedShaderStage(ShaderType::TessEvaluation))
    {
