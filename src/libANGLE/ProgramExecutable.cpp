...
bool ProgramExecutable::hasDefaultUniforms(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasDefaultUniforms())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasTextures(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasTextures())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasUniformBuffers(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasUniformBuffers())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasStorageBuffers(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasStorageBuffers())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasAtomicCounterBuffers(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasAtomicCounterBuffers())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasImages(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasImages())
            {
                return true;
            }
        }
...
bool ProgramExecutable::hasTransformFeedbackOutput(const gl::State &glState) const
{
    ...
    if (pipeline)
    {
        ...
        {
            const Program *program = pipeline->getShaderProgram(shaderType);
            if (program && program->getState().hasTransformFeedbackOutput())
            {
                return true;
            }
        }
