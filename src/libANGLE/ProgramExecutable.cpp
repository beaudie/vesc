...
bool ProgramExecutable::linkAtomicCounterBuffers(const Context *context, InfoLog &infoLog)
{
    for (unsigned int index : mAtomicCounterUniformRange)
    {
        ...
        uniform.mFixedSizeData.blockInfo.offset           = uniform.getOffset();
        uniform.mFixedSizeData.blockInfo.arrayStride      = (uniform.isArray() ? 4 : 0);
        uniform.mFixedSizeData.blockInfo.matrixStride     = 0;
        uniform.mFixedSizeData.blockInfo.isRowMajorMatrix = false;
             ...
             ++bufferIndex)
        {
            ...
            if (buffer.binding == uniform.getBinding())
            {
                ...
                uniform.mFixedSizeData.bufferIndex = bufferIndex;
                found                              = true;
        ...
        if (!found)
        {
            ...
            uniform.mFixedSizeData.bufferIndex =
                static_cast<int>(getActiveAtomicCounterBufferCount() - 1);
        }
    }
