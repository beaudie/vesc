...
void ProgramExecutable::reset()
{
    ...
    mShaderUniforms.fill(std::vector<sh::ShaderVariable>());
}
...
bool ProgramExecutable::linkUniforms(const Caps &caps,
                                     const Version &version,
                                     InfoLog &infoLog,
...
{
    UniformLinker linker(mLinkedShaderStages, mShaderUniforms);
    if (!linker.link(caps, infoLog, uniformLocationBindings))
    {
        return false;
    }

    linker.getResults(&mUniforms, unusedUniforms, uniformLocationsOutOrNull);
...
void ProgramExecutable::addShaderUniforms(ShaderType shaderType,
                                          const std::vector<sh::ShaderVariable> &uniforms)
{
    mShaderUniforms[shaderType] = uniforms;
}
