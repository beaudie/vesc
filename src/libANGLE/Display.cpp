...
{
    mContexts.insert(std::pair(context->id(), context));
...
Error Display::destroyInvalidEglObjects()
{
    ...
    {
        destroyImageImpl(mInvalidImageMap.begin()->first, &mInvalidImageMap);
    }
    ...
    {
        destroySyncImpl(mInvalidSyncMap.begin()->first, &mInvalidSyncMap);
    }
                                   ...
                                   Surface **outSurface)
{
    ...
    *outSurface = surface.release();
    mState.surfaceMap.insert(std::pair((*outSurface)->id(), *outSurface));
                           ...
                           Image **outImage)
{
    ...
    *outImage = image;
    ...
    image->addRef();
    mImageMap.insert(std::pair(image->id(), image));
                             ...
                             gl::Context **outContext)
{
    ...
    mState.contextMap.insert(std::pair(context->id(), context));
                          ...
                          Sync **outSync)
{
    ...
    sync->addRef();
    mSyncMap.insert(std::pair(sync->id(), sync));
...
Error Display::destroySurfaceImpl(Surface *surface, SurfaceMap *surfaces)
{
    ...
    auto iter = surfaces->find(surface->id());
...
void Display::destroyImageImpl(ImageID id, ImageMap *images)
{
    auto iter = images->find(id);
    ASSERT(iter != images->end());
    mImageHandleAllocator.release(id.value);
...
Error Display::releaseContextImpl(gl::Context *context, ContextMap *contexts)
{
    ...
    ASSERT(contexts->find(context->id()) != contexts->end());
...
void Display::destroySyncImpl(SyncID id, SyncMap *syncs)
{
    auto iter = syncs->find(id);
    ASSERT(iter != syncs->end());
    mSyncHandleAllocator.release(iter->first.value);
    iter->second->release(this);
    syncs->erase(iter);
}
...
{
    return destroyImageImpl(image->id(), &mImageMap);
}
