...
Error Display::releaseContext(gl::Context *context, Thread *thread)
{
    ...
    // If eglTerminate() has previously been called and this is the last Context in the share group,
    // we can now fully terminate the display and release all of its resources.
    for (const gl::Context *shareContext : mContextSet)
    {
        if (shareContext->getRefCount() > 0)
        {
            return NoError();
        }
    }
