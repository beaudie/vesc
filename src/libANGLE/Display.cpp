...
Error Display::terminate(Thread *thread, TerminateReason terminateReason)
{
    ...
    // Cache size of all contexts before invalidation. This is used as a check to verify that
    ...
    // All contexts in "mState.contextSet" must end up in either
    // "contextsStillCurrent" or "mInvalidContextSet".
    size_t contextSetSizeAfterInvalidation =
        contextsStillCurrent.size() + mInvalidContextSet.size();
    ASSERT(contextSetSizeBeforeInvalidation == contextSetSizeAfterInvalidation);
