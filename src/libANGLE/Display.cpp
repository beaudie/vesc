...
Error Display::initialize()
{
    ...
    mState.mSingleThreadPool = angle::WorkerThreadPool::Create(1, ANGLEPlatformCurrent());
    mState.mMultiThreadPool  = angle::WorkerThreadPool::Create(0, ANGLEPlatformCurrent());
...
Error Display::terminate(Thread *thread, TerminateReason terminateReason)
{
    ...
    mState.mSingleThreadPool.reset();
    mState.mMultiThreadPool.reset();

    mState.mDeviceLost = false;
...
bool Display::testDeviceLost()
{
    ...
    if (!mState.mDeviceLost && mImplementation->testDeviceLost())
    {
        notifyDeviceLost();
    }
