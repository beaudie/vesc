...
Error Display::terminate(Thread *thread, TerminateReason terminateReason)
{
    ...
    // "mState.contextSet" will contain only those contexts that are still current on some thread.
    mState.contextSet = contextsStillCurrent;
    ...
    ASSERT(mState.contextSet.empty());
