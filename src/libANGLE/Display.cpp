                                              ...
                                              const AttributeMap &attribMap)
{
    ...
    // Don't return a Display from the cache if it's terminated.
    if (iter != displays->end() && !iter->second->mIsTerminated)
    {
        display = iter->second;
    }
...
Display::~Display()
{
    removeFromPlatformDisplayMap();

    SafeDelete(mDevice);
    SafeDelete(mImplementation);
}

void Display::removeFromPlatformDisplayMap()
{
...
Error Display::terminate(Thread *thread)
{
    ...
    // Remove this Display from the cache, so it's no longer returned by calls like
