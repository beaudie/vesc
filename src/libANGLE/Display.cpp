...
#if defined(ANGLE_ENABLE_VULKAN)
    const std::string vulkanString = "vulkan";
    // If environment variable starts with 'vulkan'
    if ((vulkanString.length() <= angleDefaultEnv.length() &&
         angleDefaultEnv.compare(0, vulkanString.length(), vulkanString) == 0) ||
                                              ...
                                              const AttributeMap &attribMap)
{
    ...
    if (!display->isInitialized())
    {
        ...
        display->updateAttribsFromEnvironment(attribMap, &displayType);
...
void Display::updateAttribsFromEnvironment(const AttributeMap &attribMap, EGLAttrib *outDisplayType)
{
    ASSERT(outDisplayType != nullptr);
