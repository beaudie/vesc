...
AttributeBinding::AttributeBinding() : stride(16u), divisor(0), offset(0)
{
}
...
AttributeBinding &AttributeBinding::operator=(AttributeBinding &&binding)
{
    if (this != &binding)
    {
...
AttributeFormat::AttributeFormat(GLuint formatIndex)
    : type(GL_FLOAT),
      size(4u),
      normalized(false),
      pureInteger(false),
...
AttributeFormat::AttributeFormat(AttributeFormat &&format)
{
    *this = std::move(format);
}

AttributeFormat &AttributeFormat::operator=(AttributeFormat &&format)
{
    if (this != &format)
    {
        type           = format.type;
        ...
        bindingIndex   = format.bindingIndex;
        relativeOffset = format.relativeOffset;
    }
    return *this;
}

VertexAttribute::VertexAttribute(GLuint index, const std::vector<AttributeBinding> *bindings)
    : enabled(false), pointer(nullptr), stride(0), format(index), bindings(bindings)
{
    ASSERT(bindings);
}
...
VertexAttribute::VertexAttribute(VertexAttribute &&attrib)
    : enabled(attrib.enabled),
      ...
      format(std::move(attrib.format)),
      bindings(attrib.bindings)
{
    ASSERT(bindings);
}
...
{
    return attrib.getBinding().offset + attrib.format.relativeOffset;
}
...
size_t ComputeVertexAttributeStride(const VertexAttribute &attrib)
{
    ...
    return attrib.enabled ? attrib.getBinding().stride : 16u;
}
