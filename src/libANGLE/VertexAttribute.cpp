...
VertexAttribute::VertexAttribute(GLuint index, const std::vector<AttributeBinding> *bindings)
    : pointer(nullptr), stride(0), format(index), bindings(bindings)
{
    ASSERT(bindings);
}
...
{
    return attrib.getBinding().offset + attrib.getFormat().relativeOffset;
}
...
size_t ComputeVertexAttributeStride(const VertexAttribute &attrib)
{
    ...
    return attrib.getFormat().enabled ? attrib.getBinding().stride : 16u;
}
