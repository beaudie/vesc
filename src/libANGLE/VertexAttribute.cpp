...
VertexBufferBinding::VertexBufferBinding() : stride(16), divisor(0), bindingOffset(0)
{
}

VertexAttribute::VertexAttribute(size_t index)
    : enabled(false),
      type(GL_FLOAT),
      size(4),
      normalized(false),
...
{
    return binding.bindingOffset + attrib.relativeOffset;
}
                                    ...
                                    const VertexBufferBinding &binding)
{
    ...
    // (https://www.opengl.org/registry/specs/ARB/vertex_attrib_binding.txt Issues(3))
    return attrib.enabled ? binding.stride : 16;
}
