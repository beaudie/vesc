                             ...
                             const sh::BlockMemberInfo &blockInfoIn)
{
    ...
    if (bufferIndexIn == -1)
    {
        flagBits.isDefaultBlock = true;
        SetBitField(bufferIndex, 0);
    }
    else
    {
        flagBits.isDefaultBlock = false;
    ...
    SetBitField(flagBits.blockIsRowMajorMatrix, blockInfoIn.isRowMajorMatrix);
}
...
LinkedUniform::LinkedUniform(const UsedUniform &usedUniform)
{
    ...
    if (usedUniform.bufferIndex == -1)
    {
        flagBits.isDefaultBlock = true;
        SetBitField(bufferIndex, 0);
        SetBitField(blockOffset, 0);
        SetBitField(blockArrayStride, 0);
        SetBitField(blockMatrixStride, 0);
        SetBitField(flagBits.blockIsRowMajorMatrix, false);
    }
    else
    {
        flagBits.isDefaultBlock = false;
        SetBitField(bufferIndex, usedUniform.bufferIndex);
        SetBitField(blockOffset, usedUniform.blockInfo.offset);
        SetBitField(blockArrayStride, usedUniform.blockInfo.arrayStride);
        SetBitField(blockMatrixStride, usedUniform.blockInfo.matrixStride);
        SetBitField(flagBits.blockIsRowMajorMatrix, usedUniform.blockInfo.isRowMajorMatrix);
    }
