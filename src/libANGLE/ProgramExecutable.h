...
struct ProgramInput
{
    ...
    sh::InterpolationType getInterpolation() const { return basicDataTypeStruct.interpolation; }
    ...
    // The struct bellow must only contain data of basic type so that entire struct can memcpy.
    struct
    {
        ...
        union
        {
            struct
            {
                uint16_t active : 1;
                uint16_t isPatch : 1;
                uint16_t hasImplicitLocation : 1;
                uint16_t isArray : 1;
                uint16_t isBuiltIn : 1;
                uint16_t padding : 11;
            } flagBits;
