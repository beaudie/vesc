...
{
    ANGLE_TRY(mImpl->getSupportedImageFormats(flags, imageType, numEntries, imageFormats,
                                              numImageFormats));
    return angle::Result::Continue;
}
...
{
    ANGLE_TRY(mImpl->waitForEvents(Event::Cast(numEvents, eventList)));
    return angle::Result::Continue;
}
...
Context::Context(Platform &platform,
    ...
    : mPlatform(platform),
      ...
      mNotify(notify),
      mUserData(userData),
      mImpl(std::invoke([&]() -> rx::CLContextImpl::Ptr {
          rx::CLContextImpl::Ptr implPtr = nullptr;
          return IsError(platform.getImpl().createContext(*this, devices, userSync, implPtr))
                     ? nullptr
                     : std::move(implPtr);
      })),
...
Context::Context(Platform &platform,
    ...
    : mPlatform(platform),
      ...
      mImpl(std::invoke([&]() -> rx::CLContextImpl::Ptr {
          rx::CLContextImpl::Ptr implPtr = nullptr;
          return IsError(
                     platform.getImpl().createContextFromType(*this, deviceType, userSync, implPtr))
                     ? nullptr
                     : std::move(implPtr);
      })),
      mDevices(std::invoke([&]() -> cl::DevicePtrs {
          cl::DevicePtrs devPtrs;
          return mImpl && !IsError(mImpl->getDevices(devPtrs)) ? std::move(devPtrs)
                                                               : cl::DevicePtrs{};
      }))
{}
