...
Error TransformFeedback::onDestroy(const Context *context)
{
    ...
    {
        if (mIsBound && mState.mIndexedBuffers[i].get())
        {
            mState.mIndexedBuffers[i]->onBindingChanged(false, BufferBinding::TransformFeedback);
        }
                                          ...
                                          size_t size)
{
    ...
    if (mIsBound && mState.mIndexedBuffers[index].get())
    {
...
void TransformFeedback::setIsBound(bool bound)
{
    if (bound != mIsBound)
    {
        mIsBound = bound;
