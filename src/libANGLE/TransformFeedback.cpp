...
bool TransformFeedback::checkBufferSpaceForDraw(GLsizei count, GLsizei primcount) const
{
    ...
    for (size_t index = 0; index < vertexSizes.size(); index++)
    {
        ...
        if (!size.IsValid() || !mState.mIndexedBuffers[index].get() ||
            size.ValueOrDie() > mState.mIndexedBuffers[index].getSizeClamped())
            return false;
    }
    return true;
}
...
{
    if (mState.mActive && !mState.mPaused)
    {
        // All draws should be validated with checkBufferSpaceForDraw so ValueOrDie should never
        // fail.
