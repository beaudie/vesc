...
void FrameCaptureShared::onEndFrame(const gl::Context *context)
{
    ...
    // The shared+aux setup needs to be performed exactly once:
    // - During the first frame if capture starts immediately.
    //   - mFrameIndex = 2 && mCaptureStartFrame == 1
    // - Before the first frame for mid-execution capture.
    //   - mFrameIndex == mCaptureStartFrame, because mFrameIndex was just incremented
    if (enabled() &&
        ((mFrameIndex == 2 && mCaptureStartFrame == 1) || (mFrameIndex == mCaptureStartFrame)))
    {
