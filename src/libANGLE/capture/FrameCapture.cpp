                         ...
                         bool &anyResourceReset)
{
    ...
    switch (resourceIDType)
    {
        ...
        case ResourceIDType::ShaderProgram:
        {
            ...
            // Do the same for shaders/programs to be regenned
            for (const GLuint &shaderProgramToRegen : shaderProgramsToRegen)
            {
                if (resourceTracker->getShaderProgramType({shaderProgramToRegen}) ==
                    ShaderProgramType::ShaderType)
                {
                    out << "    glDeleteShader(gShaderProgramMap[" << shaderProgramToRegen
                        << "]);\n";
                }
                else
                {
                    ASSERT(resourceTracker->getShaderProgramType({shaderProgramToRegen}) ==
                           ShaderProgramType::ProgramType);
                    out << "    glDeleteProgram(gShaderProgramMap[" << shaderProgramToRegen
                        << "]);\n";
                }
            }
    ...
    const PackedEnumMap<ResourceIDType, uint32_t> &maxAccessedResourceIDs)
{
    ...
    for (const auto &programIter : programs)
    {
        ...
        // Create two lists of calls for program setup
        TrackedResource &trackedShaderPrograms =
    ...
    for (const auto &shaderIter : shaders)
    {
        ...
        // Create two lists of calls for shader setup
        TrackedResource &trackedShaderPrograms =
            resourceTracker->getTrackedResource(context->id(), ResourceIDType::ShaderProgram);
