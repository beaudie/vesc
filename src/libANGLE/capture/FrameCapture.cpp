    ...
    for (const auto &textureIter : textures)
    {
        ...
        while (imageIter.hasNext())
        {
            ...
            if (texture->getType() == gl::TextureType::External)
            {
                ...
                gl::TextureID stagingTexId = {mMaxAccessedResourceIDs[ResourceIDType::Texture] + 1};
                ...
                Capture(stagingTexCall, CaptureDeleteTextures(replayState, true, 1, &stagingTexId));
                continue;
            }
    ...
    ResourceIDToSetupCallsMap *resourceIDToSetupCalls)
{
    switch (call.entryPoint)
    {
        ...
        case EntryPoint::EGLCreateImageKHR:
        {
            ...
            std::vector<GLint64> reconstructedAttribs(static_cast<uint>(attribs[0].size() / 8));

            uint bytesPerAttrib = 8;
            uint numAttribs     = static_cast<uint>(attribs[0].size() / bytesPerAttrib);
            for (uint i = 0; i < numAttribs; i++)
            {
                GLint64 compiledAttrib = 0;
                for (uint b = 0; b < bytesPerAttrib; b++)
                {
            ...
            if (reconstructedAttribs[0] != EGL_IMAGE_PRESERVED ||
