                                        ...
                                        gl::State &replayState)
{
    ...
    for (const auto &textureIter : textures)
    {
        ...
        for (std::vector<CallCapture> *calls : texSetupCalls)
        {
            ...
            {
                Capture(calls, CaptureBindTexture(replayState, true, gl::TextureType::_2D, id));
            }
            else
            {
        ...
        auto capTexParam = [&replayState, texture, &texSetupCalls](GLenum pname, GLint param) {
            for (std::vector<CallCapture> *calls : texSetupCalls)
            {
                ...
                {
                    Capture(calls, CaptureTexParameteri(replayState, true, texture->getType(),
                                                        pname, param));
                }
            }
        };
        ...
        while (imageIter.hasNext())
        {
            ...
            if (texture->getType() == gl::TextureType::External)
            {
                ...
                Capture(setupCalls, CaptureEGLCreateImage(context, EGL_GL_TEXTURE_2D_KHR, texture,
                                                          attrib_listPacked, image));

                // Then create the texture
                Capture(setupCalls, CaptureEGLImageTargetTexture2DOES(
                                        replayState, true, gl::TextureType::_2D, &image));
                continue;
            }
