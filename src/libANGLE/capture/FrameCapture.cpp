                             ...
                             gl::State *replayState)
{
    ...
    std::set<GLuint> vertexPointerBindings;
    ...
    for (GLuint attribIndex = 0; attribIndex < gl::MAX_VERTEX_ATTRIBS; ++attribIndex)
    {
            ...
            binding.getBuffer().get() != nullptr)
        {
                     ...
                     (!buffer || binding.getOffset() == reinterpret_cast<GLintptr>(attrib.pointer)))
            {
                ...
                vertexPointerBindings.insert(attribIndex);
    ...
    {
        if (vertexPointerBindings.find(bindingIndex) != vertexPointerBindings.end())
        {
            // This index used VertexPointer, so skip additional ES31 bindings
            continue;
        }
        ...
        {
            Capture(setupCalls, CaptureBindVertexBuffer(*replayState, true, bindingIndex,
                                                        binding.getBuffer().id(),
