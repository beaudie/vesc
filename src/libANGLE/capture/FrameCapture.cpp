...
{
    const auto iter = resourceIDToSetupCallsMap->find(resourceID);
    if (iter == resourceIDToSetupCallsMap->end())
    {
        return;
    }
    ...
    for (size_t index = calls.low(); index < calls.high(); index++)
    {
                                        ...
                                        gl::State &replayState)
{
    ...
    for (const auto &programIter : programs)
    {
        ...
        // Start tracking program setup calls for this resource
        ...
        // Finish tracking program setup calls
        size_t programSetupEnd = setupCalls->size();

        frameCaptureShared->markResourceSetupCalls(
            setupCalls, ResourceID(ResourceIDType::ShaderProgram, id.value),
    ...
    for (const auto &shaderIter : shaders)
    {
        ...
        // Start tracking shader setup calls for this resource
        ...
        // Finish tracking shader setup calls
    ...
    if (!context->isGLES1())
    {
        ...
        if (apiState.getProgram())
        {
            ...
            MarkResourceIDActive(
                ...
                context->getShareGroup()->getFrameCaptureShared()->getResourceIDToSetupCalls());
        }
...
void FrameCaptureShared::markResourceSetupCalls(std::vector<CallCapture> *setupCalls,
                                                ResourceID id,
                                                gl::Range<size_t> range)
{
    ASSERT(mResourceIDToSetupCalls.find(id) == mResourceIDToSetupCalls.end());

    // Mark all of the calls that were used to initialize this resource as INACTIVE
    for (size_t index = range.low(); index < range.high(); index++)
    {
        (*setupCalls)[index].isActive = false;
    }

    mResourceIDToSetupCalls[id] = std::move(range);
}
