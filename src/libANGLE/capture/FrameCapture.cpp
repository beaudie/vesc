                               ...
                               size_t *maxResourceIDBufferSize)
{
    ...
    for (const auto &uniformIter : defaultUniformsToReset)
    {
        ...
        if (ProgramNeedsReset(context->id(), resourceTracker, programID) ||
            frameCaptureShared->midExecutionSetupComplete())
        {
            // Skip programs marked for reset or unlinked programs as they will update their own
            // uniforms
            return;
        }
    ...
    const PackedEnumMap<ResourceIDType, uint32_t> &maxAccessedResourceIDs)
{
    ...
    for (const auto &programIter : programs)
    {
        ...
        // Unlinked programs can be processed during framecapture, so ensure program/shader
        ...
        // If program has not been linked leave setupcalls marked as active
        if (program->isLinked())
        {
