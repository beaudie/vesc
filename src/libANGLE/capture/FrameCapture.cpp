                                         ...
                                         const FrameCaptureShared &frameCaptureShared)
{
...

    {
        ...
        replayWriter.addPublicFunction(proto, headerStream, bodyStream);
    }

    // Write out the source and header files.
    replayWriter.saveFrame(frameIndex);
    replayWriter.saveHeader();
}
                                   ...
                                   const FrameCaptureShared &frameCaptureShared)
{
...

    {
        ...
        protoStream << "void " << FmtReplayFunction(kSharedContextId, frameIndex);
        std::string proto = protoStream.str();
    ...
    // Write out the source file. We don't write out the header for now.
    replayWriter.saveFrame(frameIndex);
}
                                                  ...
                                                  bool writeResetContextCall)
{
...

    {
        ...
        std::string includes = source.str();
        mReplayWriter.setSourcePrologue(includes);
    }
                                                   ...
                                                   const std::vector<CallCapture> &setupCalls)
{
    ...
    if (frameIndex == 1)
    {
...

        {
            ...
            mReplayWriter.addPublicFunction("void SetupReplay()", std::stringstream(), out);
        }
    }
...
void ReplayWriter::saveIndexFiles()
{
