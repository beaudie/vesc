...
bool SkipCall(EntryPoint entryPoint)
{
    switch (entryPoint)
    {
        ...
        case EntryPoint::EGLGetProcAddress:
            ...
            // - Has a complex type that isn't handled in the auto-generation, and has no side
            // effects.
            return true;

        case EntryPoint::EGLGetConfigAttrib:
            // Skip this call because:
            // - Uses an output pointer that isn't handled in EGL capture, and has no side effects.
            return true;
...
{
    return angle::CallCapture("Invalid", angle::ParamBuffer());
}
}  // namespace egl
