...
void FrameCaptureShared::onMakeCurrent(const gl::Context *context, const egl::Surface *drawSurface)
{
    ...
    mDrawSurfaceParams[context->id()] = SurfaceParams();
    mDrawSurfaceParams[context->id()].extents =
        gl::Extents(drawSurface->getWidth(), drawSurface->getHeight(), 1),
    mDrawSurfaceParams[context->id()].colorSpace =
        egl::FromEGLenum<egl::ColorSpace>(drawSurface->getGLColorspace());
}
