...
void FrameCaptureShared::determineMemoryProtectionSupport(gl::Context *context)
{
    ...
    HashMap<std::string, std::vector<std::string>> blackList = {
        {"Google", {"Pixel 6", "Pixel 6 Pro", "Pixel 6a", "Pixel 7", "Pixel 7 Pro"}},
    };

    angle::SystemInfo info;
    angle::GetSystemInfo(&info);
    bool isDeviceBlackListed = false;

    if (blackList.find(info.machineManufacturer) != blackList.end())
    {
        const std::vector<std::string> &models = blackList[info.machineManufacturer];
        isDeviceBlackListed =
    ...
    {
        WARN() << "Direct memory protection not possible on blacklisted device '"
    ...
    {
        // Device is not on blacklisted. Run a test if we actually can protect directly. Do this
