    ...
    : mPageSize(pageSize), mUseShadowMemory(useShadowMemory), mRealStart(start)
{
    ...
    if (mUseShadowMemory)
    {
        ...
        shadowMemory          = AlignedAlloc(numShadowPages * pageSize, pageSize);
...
void CoherentBufferTracker::markShadowDirty()
{
    for (const auto &pair : mBuffers)
    {
        std::shared_ptr<CoherentBuffer> cb = pair.second;
...
void FrameCaptureShared::maybeCapturePostCallUpdates(const gl::Context *context)
{
    ...
    switch (lastCall.entryPoint)
    {
        ...
        case EntryPoint::GLDispatchCompute:
        {
            ...
            if (mCoherentBufferTracker.useShadowMemory())
            {
                mCoherentBufferTracker.markShadowDirty();
            }
            break;
        }
        default:
...
void FrameCaptureShared::onMakeCurrent(const gl::Context *context, const egl::Surface *drawSurface)
{
    if (!mCoherentBufferTracker.useShadowMemory())
    {
        if (!context->getLimitations().canProtectVulkanMemory)
        {
            WARN() << "Vulkan memory protection not possible, enabling shadow memory for coherent "
                      "buffer tracking.";
            mCoherentBufferTracker.enableShadowMemory();
        }
    }
