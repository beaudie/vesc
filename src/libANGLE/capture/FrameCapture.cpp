...
{
    ASSERT(frameCaptureShared.getMainContextID() != context->id());
...
void WriteMainContextCppReplay(bool compression,
                               const std::string &outDir,
                               const gl::Context *context,
                               const std::string &captureLabel,
                               uint32_t frameIndex,
...
void FrameCaptureShared::onEndFrame(const gl::Context *context)
{
    ...
    if (mFrameIndex >= mCaptureStartFrame)
    {
        ...
        WriteMainContextCppReplay(
            mCompression, mOutDirectory, context, mCaptureLabel, getReplayFrameIndex(),
            getFrameCount(), mFrameCalls, frameCapture->getSetupCalls(), &mResourceTracker,
            &mBinaryData, mClientArraySizes, mReadBufferSize, mSerializeStateEnabled, *this);
    ...
    if (enabled() && mFrameIndex == mCaptureStartFrame)
    {
        ...
        // WriteMainContextCppReplay() output.
        ...
        for (gl::Context *shareContext : *shareContextSet)
        {
            ...
            if (!frameCapture->getSetupCalls().empty())
            {
                WriteAuxiliaryContextCppSetupReplay(
                    mCompression, mOutDirectory, shareContext, mCaptureLabel, getReplayFrameIndex(),
                    frameCapture->getSetupCalls(), &mBinaryData, mClientArraySizes, mReadBufferSize,
