                         ...
                         std::vector<uint8_t> *binaryData)
{
    switch (resourceIDType)
    {
        ...
        case ResourceIDType::ShaderProgram:
        {
            ...
            {
                ProgramSet::iterator programIter = newPrograms.begin();

                for (size_t i = 0; programIter != newPrograms.end(); ++i, ++programIter)
                {
                    out << "    glDeleteProgram(gShaderProgramMap[" << (*programIter).value
                        << "]);\n";
                }
            }

            // TODO (http://anglebug.com/5968): Handle programs that need regen or restore
            // This would only happen if a starting program was deleted or modified during the run
            ASSERT(resourceTracker->getProgramsToRegen().empty() &&
                   resourceTracker->getProgramsToRestore().empty());
            break;
        }
        default:
