...
std::vector<std::pair<uintptr_t, size_t>> CoherentBuffer::getDirtyRanges()
{
    std::vector<std::pair<uintptr_t, size_t>> dirtyAddressRanges;
    ...
    for (auto &dirtyPageRange : dirtyPageRanges)
    {
        uintptr_t start = 0;
        ...
        dirtyAddressRanges.push_back(dirtyAddressRange);
    }

    return dirtyAddressRanges;
}
...
void CoherentBuffer::protect()
{
    ...
    mDirtyPages.assign(mPageCount, false);
}
...
void CoherentBufferTracker::protectBuffer(gl::BufferID id)
{
    mBuffers.at(id)->protect();
}
...
void FrameCaptureShared::captureCoherentBufferSnapshot(const gl::Context *context, gl::BufferID id)
{
    ...
    mCoherentBufferTracker.protectBuffer(id);
}
