...
GLint GetActualCachedTextureLevel(gl::TextureTarget target, GLint level)
{
    GLint cachedLevel = level;

    // If target is a cube, we need to maintain 6 images per level
    if (IsCubeMapFaceTarget(target))
    {
        cachedLevel *= 6;
        cachedLevel += CubeMaptTextureTargetToFaceIndex(target);
    }
                                                                           ...
                                                                           GLint level)
{
    ...
    GLint cachedLevel = GetActualCachedTextureLevel(target, level);

    const auto &foundTextureLevel = foundTextureLevels->second.find(cachedLevel);
                                                                    ...
                                                                    EntryPoint entryPoint)
{
    ...
    // For this texture, look up the cached level, which may not match 1:1 due to cubes
    GLint cachedLevel = GetActualCachedTextureLevel(target, textureLevel);

    TextureLevels &foundLevels         = foundTextureLevels->second;
    TextureLevels::iterator foundLevel = foundLevels.find(cachedLevel);
    ...
    auto emplaceResult = foundLevels.emplace(cachedLevel, std::move(newPixelData));
    ASSERT(emplaceResult.second);
