                           ...
                           std::vector<uint8_t> *binaryData)
{
    ...
    if (call.entryPoint == EntryPoint::EGLCreateNativeClientBufferANDROID)
    {
        ...
        callOut << "ggClientBufferMap[" << reinterpret_cast<EGLClientBuffer>(buffer) << "] = ";
    }
...
void CaptureCreateImage(gl::Context *context,
                        EGLenum target,
                        ...
                        egl::Image *image)
{
    ...
    if (frameCaptureShared->isCapturing())
    {
        ...
        /* The EGL display should be queried directly in th emitted code */
        ...
        angle::ParamCapture paramsContext("context", angle::ParamType::TEGLContext);
        angle::SetParamVal<angle::ParamType::TEGLContext>(static_cast<EGLContext>(context),
                                                          &paramsContext.value);
        paramBuffer.addParam(std::move(paramsContext));
...
void CaptureDestroyImage(gl::Context *context, egl::Display *display, egl::Image *image)
{
    angle::FrameCaptureShared *frameCaptureShared =
        context->getShareGroup()->getFrameCaptureShared();
    if (frameCaptureShared->isCapturing())
