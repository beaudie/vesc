...
{
    // A map to store the gShaderProgram map lookup index of the temp shaders we
    // attached below. We need this map to retrieve the lookup index to pass to
    // CaptureDetachShader calls at the end of GenerateLinkedProgram.
    ...
    {
        // Bump the max shader program id for each new tempIDStart we use to
        // create, compile, and attach the temp shader object.
        resourceTracker->onShaderProgramAccess(tempIDStart);
        // Store the tempIDStart in the tempShaderIDTracker to retrieve for
        // CaptureDetachShader calls later.
        ...
        // Increment tempIDStart to get a new gShaderProgram map index for the
        // next linked stage shader object. We can't reuse the same tempIDStart
        // as we need to retrieve the index of each attached shader object later
        // to pass to CaptureDetachShader calls.
        tempIDStart.value += 1;
    }
    ...
    // ResourceManagerBase::mHandleAllocator can release the
    // ShaderProgramID handle assigned to the shader object when glDeleteShader
    // is called. This ensures the ShaderProgramID handles used in
    // SetupReplayContextShared() are consistent with the ShaderProgramID handles
    // used by the app.
    for (gl::ShaderType shaderType : program->getExecutable().getLinkedShaderStages())
    {
