                                       ...
                                       const FrameCaptureShared &frameCaptureShared)
{
    ...
    // TIMTIM: need to move this block to writeCppReplayIndexFiles()
    if (frameIndex == 1)
    {
        std::stringstream setupCallStream;
                                                  ...
                                                  bool writeResetContextCall)
{
...

    {
        ...
        setupCallStream << "void " << FmtSetupFunction(kNoPartId, contextId) << "\n";
        setupCallStream << "{\n";

        WriteInitReplayCall(mCompression, setupCallStream, contextId, mCaptureLabel,
                            MaxClientArraySize(mClientArraySizes), mReadBufferSize);

        WriteCppReplayFunctionWithParts(contextId, ReplayFunc::Setup, &dataTracker, mFrameIndex,
