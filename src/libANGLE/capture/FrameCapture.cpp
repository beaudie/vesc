...
{
    size_t protectionStartPages = mProtectionRange.start / mPageSize;
...
{
    if (mDirtyPages.at(relativePage) == dirty)
    {
    ...
    if (!ret)
    {
        ERR() << "Could not set protection for buffer page " << relativePage << " at " << pageStart
              << " with size " << mPageSize;
    }
    mDirtyPages.at(relativePage) = dirty;
}
...
void CoherentBufferTracker::removeProtection()
{
    for (const auto &pair : mBuffers)
    {
        std::shared_ptr<CoherentBuffer> buffer = pair.second;
...
void FrameCaptureShared::onEndFrame(const gl::Context *context)
{
    ...
    {
        setCaptureInactive();
        mCoherentBufferTracker.removeProtection();
