...
void FrameCaptureShared::onEndFrame(const gl::Context *context)
{
    ...
    WritePresentationContextCppReplay(
        mCompression, mOutDirectory, context, mCaptureLabel, getReplayFrameIndex(), getFrameCount(),
        mFrameCalls, frameCapture->getSetupCalls(), &mResourceTracker, &mBinaryData,
        mClientArraySizes, mReadBufferSize, mSerializeStateEnabled, *this);
