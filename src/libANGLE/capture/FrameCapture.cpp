...
{
    ASSERT(frameCaptureShared.getPresentationContextID() != context->id());
...
void WritePresentationContextCppReplay(bool compression,
                                       const std::string &outDir,
                                       const gl::Context *context,
                                       const std::string &captureLabel,
                                       uint32_t frameIndex,
                                 ...
                                 const FrameCaptureShared &frameCaptureShared)
{
    ...
    WriteInitReplayCall(compression, setupCallStream, kSharedContextId, captureLabel,
                              ...
                              std::vector<uint8_t> &binaryData)
{
...

    {
        ...
        saveIndex << GetCaptureFileName(kSharedContextId, captureLabel, 1, ".cpp") << "\n";
    }
}
    ...
    if (isCallValid)
    {
        ...
        if (mLastContextId != context->id().value)
        {
            ...
            paramBuffer.addValueParam("display", ParamType::TeglDisplayPointer,
                                      context->getDisplay());
            paramBuffer.addValueParam("draw", ParamType::TSurfacePointer,
                                      context->getCurrentDrawSurface());
            paramBuffer.addValueParam("read", ParamType::TSurfacePointer,
                                      context->getCurrentReadSurface());
            paramBuffer.addValueParam("context", ParamType::TglContextPointer,
...
void FrameCaptureShared::onEndFrame(const gl::Context *context)
{
    ...
    if (isCaptureActive())
    {
        ...
        // swap every frame.
        ASSERT(mPresentationContextID == context->id());
        ...
        WritePresentationContextCppReplay(
        ...
        if (mFrameIndex == mCaptureEndFrame)
        {
            ...
            {
                SaveBinaryData(mCompression, mOutDirectory, kSharedContextId, mCaptureLabel,
                               mBinaryData);
                mBinaryData.clear();
            }
    ...
    if (enabled() && mFrameIndex == mCaptureStartFrame)
    {
        ...
        mPresentationContextID = context->id();
        ...
        {
            shareContext->finish();
            shareContext->getFrameCapture()->reset();
        }

        // The presentation context's setup functions will be written later with as part of the
        // WritePresentationContextCppReplay() output.
        gl::State presentationReplayState(nullptr, nullptr, nullptr, nullptr, nullptr,
                                          EGL_OPENGL_ES_API, context->getState().getClientVersion(),
                                          false, true, true, true, false,
                                          EGL_CONTEXT_PRIORITY_MEDIUM_IMG);
        CaptureSharedContextMidExecutionSetup(context, &mSetupCalls, &mResourceTracker,
                                              presentationReplayState);
        WriteSharedContextCppReplay(mCompression, mOutDirectory, context, mCaptureLabel,
                                    getReplayFrameIndex(), getFrameCount(), mFrameCalls,
                                    mSetupCalls, &mResourceTracker, &mBinaryData, mClientArraySizes,
                                    mReadBufferSize, mSerializeStateEnabled, *this);
        ...
        for (gl::Context *shareContext : *shareContextSet)
        {
            ...
            gl::State shareReplayState(nullptr, nullptr, nullptr, nullptr, nullptr,
                                       EGL_OPENGL_ES_API,
                                       shareContext->getState().getClientVersion(), false, true,
                                       true, true, false, EGL_CONTEXT_PRIORITY_MEDIUM_IMG);
            CaptureMidExecutionSetup(shareContext, &frameCapture->getSetupCalls(),
                                     &mResourceTracker, shareReplayState);
...
#if 1
template <>
void WriteParamValueReplay<ParamType::TglContextPointer>(std::ostream &os,
                                                         const CallCapture &call,
                                                         uint32_t value)
{
