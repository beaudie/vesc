...
{
    const auto iter = (*resourceIDToSetupCallsMap)[resourceType].find(id);
    if (iter == (*resourceIDToSetupCallsMap)[resourceType].end())
    {
        return;
    }
                                                ...
                                                gl::Range<size_t> range)
{
    ...
    mResourceIDToSetupCalls[type][id] = std::move(range);
}
