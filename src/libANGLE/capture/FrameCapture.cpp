                                 ...
                                 const FrameCaptureShared &frameCaptureShared)
{
    ...
    // Only perform shared context setup if this is a Mid-Execution Capture. Otherwise, the setup
    // would be repeated by SetupReplayContextShared() and the presentation context's SetupReplay().
    if (!isMidExecutionCapture)
    {
        WriteCppReplayFunctionWithParts(kSharedContextId, ReplayFunc::Setup, &dataTracker,
                                        frameIndex, binaryData, setupCalls, include,
                                        setupCallStream, out);
    }
...
void FrameCaptureShared::firstFrame(const gl::Context *context)
{
    reset();

    {
        gl::State replayState(nullptr, nullptr, nullptr, nullptr, nullptr, EGL_OPENGL_ES_API,
                              context->getState().getClientVersion(), false, true, true, true,
                              false, EGL_CONTEXT_PRIORITY_MEDIUM_IMG);
        CaptureSharedContextMidExecutionSetup(context, &mSetupCalls, &mResourceTracker,
                                              replayState);
        WriteSharedContextCppReplay(
            mCompression, mOutDirectory, mCaptureLabel, getReplayFrameIndex(), getFrameCount(),
            mFrameCalls, mSetupCalls, &mResourceTracker, &mBinaryData, mClientArraySizes,
            mReadBufferSize, mSerializeStateEnabled, mIsMidExecutionCapture, *this);
    ...
    for (const gl::Context *shareContext : *shareContextSet)
    {
        ...
        gl::State shareReplayState(nullptr, nullptr, nullptr, nullptr, nullptr, EGL_OPENGL_ES_API,
                                   shareContext->getState().getClientVersion(), false, true, true,
