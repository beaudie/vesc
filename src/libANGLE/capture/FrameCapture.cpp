...
void CaptureCustomCreateEGLImage(const gl::Context *context,
                                 const char *name,
                                 CallCapture &imageCall,
                                 std::vector<CallCapture> &callsOut)
{
    ParamBuffer &&imageParams = std::move(imageCall.params);
    EGLImage returnVal        = imageParams.getReturnValue().value.EGLImageVal;
    egl::ImageID imageID      = egl::PackParam<egl::ImageID>(returnVal);
    imageParams.addValueParam("image", ParamType::TGLuint, imageID.value);
    imageCall.customFunctionName = name;

    // Create a backing resouce during replay if EGLImage image came from an AHB
    EGLenum target = imageParams.getParam("target", ParamType::TEGLenum, 2).value.EGLenumVal;
    if (target == EGL_NATIVE_BUFFER_ANDROID)
    {
        ParamBuffer resourceParams;
        resourceParams.addValueParam("eglImage", ParamType::TGLuint, imageID.value);
        GLsizei width                    = 1;
        GLsizei height                   = 1;
        ...
        resourceParams.addValueParam("width", ParamType::TGLsizei, width);
        resourceParams.addValueParam("height", ParamType::TGLsizei, height);
        CallCapture resourceCall("CreateEGLImageResource", std::move(resourceParams));

        callsOut.emplace_back(std::move(resourceCall));
    }

    callsOut.emplace_back(std::move(imageCall));
}
