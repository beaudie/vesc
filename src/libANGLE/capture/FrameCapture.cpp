                            ...
                            std::vector<uint8_t> *binaryData)
{
    ...
    if (str.size() > kMaxInlineStringLength)
    {
        ...
        binaryData->resize(offset + str.size());
        memcpy(binaryData->data() + offset, str.data(), str.size());
                                                 ...
                                                 bool isMidExecutionCapture)
{
    ...
    gl::State sharedContextReplayState(nullptr, nullptr, nullptr, nullptr, nullptr,
                                       EGL_OPENGL_ES_API, contextState.getClientVersion(), false,
    ...
    {
        sharedContextReplayState.initializeForCapture(context);
        CaptureSharedContextMidExecutionSetup(context, &mSetupCalls, &mResourceTracker,
                                              sharedContextReplayState);
    }
        ...
        if (isMidExecutionCapture)
        {
            if (shareContext->id() == context->id())
            {
                CaptureMidExecutionSetup(shareContext, &frameCapture->getSetupCalls(),
                                         &mResourceTracker, sharedContextReplayState);
            }
            else
            {
                gl::State auxContextReplayState(
                    ...
                    shareContext->getState().hasProtectedContent());
