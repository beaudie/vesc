                            ...
                            size_t *maxResourceIDBufferSize)
{
    ...
    for (size_t textureUnit : stateResetHelper.getDirtyTextureBindings())
    {
        TextureResetMap::const_iterator it = resetBindings.find(textureUnit);
        if (it != resetBindings.end())
        {
            ...
            if (textureUnit != activeTexture)
            {
                ...
                WriteCppReplayForCall(
                    CaptureActiveTexture(context->getState(), true,
                                         GL_TEXTURE0 + static_cast<GLenum>(textureUnit)),
                    replayWriter, out, header, binaryData, maxResourceIDBufferSize);
                out << ";\n";
                activeTexture = textureUnit;
            }

            // Bind all of the textures for this texture unit
            for (const auto &[target, id] : it->second)
            {
                out << "    ";
                WriteCppReplayForCall(CaptureBindTexture(context->getState(), true, target, id),
    ...
    for (gl::TextureType textureType : angle::AllEnums<gl::TextureType>())
    {
        ...
        for (size_t bindingIndex = 0; bindingIndex < apiBindings.size(); ++bindingIndex)
        {
            ...
            if (apiTextureID.value)
            {
                ...
                // Save bound textures in for reset
                resetHelper.getResetTextureBindings()[bindingIndex].emplace_back(textureType,
                                                                                 apiTextureID);
            }
        }
    }
        ...
        case EntryPoint::GLBindTexture:
            if (isCaptureActive())
            {
                context->getFrameCapture()->getStateResetHelper().setTextureBindingDirty(
                    context->getState().getActiveSampler());
...
StateResetHelper::StateResetHelper() : mResetActiveTexture(0) {}
