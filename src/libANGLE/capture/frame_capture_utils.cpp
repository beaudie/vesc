...
std::string sstr(const ArgT &arg)
{
...
void SerializeImageIndex(JsonSerializer *json, const gl::ImageIndex &imageIndex)
{
    ...
    json->addString("ImageType", sstr(imageIndex.getType()));
...
void SerializeRasterizerState(JsonSerializer *json, const gl::RasterizerState &rasterizerState)
{
    ...
    json->addString("CullMode", sstr(rasterizerState.cullMode));
...
void SerializeContextState(JsonSerializer *json, const gl::State &state)
{
    ...
    {
        json->addScalar("ReadFramebufferID", state.getReadFramebuffer()->id().value);
    }
    else
    {
        json->addScalar("ReadFramebufferID", 0);
    }
    if (state.getDrawFramebuffer())
    {
        json->addScalar("DrawFramebufferID", state.getDrawFramebuffer()->id().value);
    }
    else
    {
        json->addScalar("DrawFramebufferID", 0);
    }
    json->addScalar("RenderbufferID", state.getRenderbufferId().value);
    if (state.getProgram())
    {
        json->addScalar("CurrentProgramID", state.getProgram()->id().value);
    }
    else
    {
        json->addScalar("CurrentProgramID", 0);
    }
    if (state.getProgramPipeline())
    {
        json->addScalar("CurrentProgramPipelineID", state.getProgramPipeline()->id().value);
    }
    else
    {
        json->addScalar("CurrentProgramPipelineID", 0);
    }
    json->addString("ProvokingVertex", sstr(state.getProvokingVertex()));
    ...
    json->addScalar("ActiveSampler", state.getActiveSampler());
    {
        ...
        for (gl::TextureType textureType : AllEnums<gl::TextureType>())
        {
            ...
            GroupScope texturesGroup(json, sstr(textureType));
...
void SerializeBufferState(JsonSerializer *json, const gl::BufferState &bufferState)
{
    ...
    json->addString("Usage", sstr(bufferState.getUsage()));
...
void SerializeShaderState(JsonSerializer *json, const gl::ShaderState &shaderState)
{
    ...
    if (shaderState.getGeometryShaderInputPrimitiveType().valid())
    {
        json->addString("GeometryShaderInputPrimitiveType",
                        sstr(shaderState.getGeometryShaderInputPrimitiveType().value()));
    }
    if (shaderState.getGeometryShaderOutputPrimitiveType().valid())
    {
        json->addString("GeometryShaderOutputPrimitiveType",
                        sstr(shaderState.getGeometryShaderOutputPrimitiveType().value()));
    }
...
{
    json->addString("Type", sstr(textureState.getType()));
...
Result SerializeTexture(const gl::Context *context,
                        ...
                        gl::Texture *texture)
{
    // Force texture synchronization.
    if (texture->hasAnyDirtyBit() && texture->getState().getEnabledLevelCount() > 0)
    {
