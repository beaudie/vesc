...
ImageIndex GetImageIndexFromDescIndex(GLenum target, size_t descIndex)
{
    if (target == GL_TEXTURE_CUBE_MAP)
    {
        ...
        return ImageIndex::MakeCube(LayerIndexToCubeMapTextureTarget(faceIndex), static_cast<GLint>(mipIndex));
    }
                        ...
                        const uint8_t *pixels)
{
    ...
    auto initState = (pixels == nullptr) ? InitState::NeedsInit : InitState::Clean;
...
Error Texture::generateMipmap(const Context *context)
{
    ...
    auto initState = mNeedsInit ? InitState::NeedsInit : InitState::Clean;
...
Error Texture::ensureInitialized(const Context *context)
{
    ...
    for (size_t descIndex = 0; descIndex < mState.mImageDescs.size(); ++descIndex)
    {
        ...
        if (imageDesc.initState == InitState::NeedsInit)
        {
            ...
            ANGLE_TRY(initialize(context, imageIndex));
