                                 ...
                                 GLenum internalFormat)
{
    ...
    return setBufferRange(context, buffer, internalFormat, 0, std::numeric_limits<uint32_t>::max());
}
                                      ...
                                      GLsizeiptr size)
{
    ...
    size = std::min<GLsizeiptr>(size, buffer->getSize());
    ...
    switch (message)
    {
        ...
        case angle::SubjectMessage::SubjectChanged:
            ...
            else if (index == kBufferSubjectIndex)
            {
                ...
                GLsizeiptr size = std::min<GLsizeiptr>(mState.mBuffer.getSize(), buffer->getSize());
