                                    ...
                                    Framebuffer *source)
{
    ...
    // Most if not all renderers clip these copies to the size of the source
    // framebuffer, leaving other pixels untouched. For safety in robust
    // resource initialization, assume that that clipping is going to occur when
    // computing the region for which to ensure initialization. If the copy lies
    // entirely off the source framebuffer, initialize as though a zero-size box
    // is going to be set during the copy operation. Note that this assumes that
    // ensureSubImageInitialized ensures initialization of the entire
    // destination texture, and not just a sub-region.
    Box destBox;
                                                 ...
                                                 const gl::Box &area)
{
    ...
    if (desc.initState == InitState::MayNeedInit)
    {
        ...
        if (!coversWholeImage)
        {
            // NOTE: do not optimize this to only initialize the passed area of
            // the texture, or the initialization logic in copySubImage will be
            // incorrect.
