                                ...
                                const uint8_t *pixels)
{
    ...
    // context can be null for tests.
    if (context)
    {
        isRenderable = imageFormat.info->textureAttachmentSupport(context->getClientVersion(),
                                                                  context->getExtensions());
    }
    mState.setImageDesc(target, level, ImageDesc(size, imageFormat, initState, isRenderable));
...
// Note that we don't use |Context| param here as used in SetImage().
angle::Result Texture::setImageForTesting(const PixelUnpackState &unpackState,
                                          Buffer *unpackBuffer,
                                          TextureTarget target,
                                          GLint level,
                                          GLenum internalFormat,
                                          ...
                                          const uint8_t *pixels)
{
    return setImage(nullptr, unpackState, unpackBuffer, target, level, internalFormat, size, format,
                    type, pixels);
}
