                                      ...
                                      Texture *source)
{
    ...
    if (context->isRobustResourceInitEnabled())
    {
        Extents fbSize          = source->getExtents(target, level);
        sourceAreaIsOutOfBounds = (sourceBox.x < 0) || (sourceBox.y < 0) ||
                                  ((sourceBox.x + sourceBox.width) > fbSize.width) ||
                                  ((sourceBox.y + sourceBox.height) > fbSize.height);
    }
    ANGLE_TRY(ensureSubImageInitialized(context, index, destBox, sourceAreaIsOutOfBounds));
