...
angle::Result Texture::generateMipmap(Context *context)
{
    ...
    ANGLE_TRY(syncState(context, SyncStateSource::GenerateMipmap));
...
angle::Result Texture::syncState(const Context *context, SyncStateSource source)
{
    ASSERT(hasAnyDirtyBit() || source == SyncStateSource::GenerateMipmap);
                                   ...
                                   void *pixels)
{
    if (hasAnyDirtyBit())
    {
        ANGLE_TRY(syncState(context, SyncStateSource::Other));
    }
