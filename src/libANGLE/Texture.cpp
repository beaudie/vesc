...
bool TextureState::isCubeComplete() const
{
    ...
    for (GLenum face = FirstCubeMapTextureTarget + 1; face <= LastCubeMapTextureTarget; face++)
    {
        ...
        if (faceImageDesc.size.width != baseImageDesc.size.width ||
            ...
            faceImageDesc.format != baseImageDesc.format)
        {
            return false;
        }
    }
...
bool TextureState::computeLevelCompleteness(GLenum target, size_t level) const
{
    ...
    if (levelImageDesc.format != baseImageDesc.format)
    {
        return false;
    }
                         ...
                         const Framebuffer *source)
{
    ...
    GLenum effectiveFormat = GL_NONE;
    bool result = GetEffectiveInternalFormat(GetInternalFormatInfo(internalFormat), *source->getReadColorbuffer()->getFormat().info, &effectiveFormat);
    ASSERT(result && effectiveFormat != GL_NONE);
