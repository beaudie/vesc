                                ...
                                const uint8_t *pixels)
{
    ...
    const auto image_format = Format(internalFormat, type);
    mState.setImageDesc(target, level,
                        ImageDesc(size, image_format, initState,
                                  image_format.info->textureAttachmentSupport(
                                          ...
                                          const uint8_t *pixels)
{
    ...
    InitState initState     = DetermineInitState(context, unpackBuffer, pixels);
    const auto image_format = Format(internalFormat);
    mState.setImageDesc(target, level,
                        ImageDesc(size, image_format, initState,
                                  image_format.info->textureAttachmentSupport(
                                      context->getClientVersion(), context->getExtensions())));
...
angle::Result Texture::bindTexImageFromSurface(Context *context, egl::Surface *surface)
{
    ...
    const auto image_format = surface->getBindTexImageFormat();
    ImageDesc desc(size, surface->getBindTexImageFormat(), InitState::Initialized,
                   image_format.info->textureAttachmentSupport(context->getClientVersion(),
                                                               context->getExtensions()));
                                              ...
                                              const egl::Stream::GLTextureDescription &desc)
{
    ...
    const auto image_format = Format(desc.internalFormat);
    mState.setImageDesc(NonCubeTextureTypeToTarget(mState.mType), 0,
                        ImageDesc(size, image_format, InitState::Initialized,
                                  image_format.info->textureAttachmentSupport(
                                      context->getClientVersion(), context->getExtensions())));
    signalDirtyStorage(InitState::Initialized);
    return angle::Result::Continue;
