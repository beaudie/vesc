...
std::unique_ptr<CompilerInstance> Compiler::getInstance(ShaderType type)
{
    ...
    if (pool.empty())
    {
        ...
        ASSERT(handle);
        return std::make_unique<CompilerInstance>(handle, this, type);
    }
    else
    {
        auto instance = std::move(pool.front());
        pool.pop_front();
        return instance;
    }
}

void Compiler::putInstance(std::unique_ptr<CompilerInstance> instance)
{
...
CompilerInstance::CompilerInstance(ShHandle handle, Compiler *compiler, ShaderType shaderType)
    : mHandle(handle), mCompiler(compiler), mShaderType(shaderType)
{
}
...
ShaderType CompilerInstance::getShaderType() const
{
    return mShaderType;
}
