...
namespace
{
cl_mem_flags InheritFlags(cl_mem_flags parent, cl_mem_flags flags)
{
    if ((flags & (CL_MEM_READ_WRITE | CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY)) == 0u)
    {
        flags |= parent & (CL_MEM_READ_WRITE | CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY);
    }
    if ((flags & (CL_MEM_HOST_WRITE_ONLY | CL_MEM_HOST_READ_ONLY | CL_MEM_HOST_NO_ACCESS)) == 0u)
    {
        flags |= parent & (CL_MEM_HOST_WRITE_ONLY | CL_MEM_HOST_READ_ONLY | CL_MEM_HOST_NO_ACCESS);
    }
    flags |= parent & (CL_MEM_USE_HOST_PTR | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR);
    return flags;
}
}  // namespace
...
cl_int Memory::getInfo(MemInfo name, size_t valueSize, void *value, size_t *valueSizeRet)
{
    ...
    switch (name)
    {
        ...
        case MemInfo::UsesSVM_Pointer:
            valUInt   = CL_FALSE;  // TODO(jplate) Check if mHostPtr is SVM pointer
            copyValue = &valUInt;
            copySize  = sizeof(valUInt);
            break;
...
Memory::Memory(const Buffer &buffer,
    ...
    : _cl_mem(parent.getDispatch()),
      ...
      mFlags(InheritFlags(parent.mFlags, flags)),
      mHostPtr(parent.mHostPtr != nullptr ? static_cast<char *>(parent.mHostPtr) + offset
                                          : nullptr),
      mParent(&parent),
      mOffset(offset),
      mImpl(parent.mImpl->createSubBuffer(buffer, parent, flags, size, errcodeRet)),
      mSize(size)
{}
