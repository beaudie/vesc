    ...
    : mResource(nullptr)
{
    attach(context, type, binding, textureIndex, resource, kDefaultNumViews, kDefaultBaseViewIndex,
           false, kDefaultRenderToTextureSamples, framebuferSerial);
}
...
void FramebufferAttachment::detach(const Context *context, rx::Serial framebuferSerial)
{
    mType = GL_NONE;
    if (mResource != nullptr)
    {
        mResource->onDetach(context, framebuferSerial);
        mResource = nullptr;
    }
    mNumViews      = kDefaultNumViews;
    mIsMultiview   = false;
                                   ...
                                   rx::Serial framebuferSerial)
{
    if (resource == nullptr)
    {
        detach(context, framebuferSerial);
        return;
    }
    ...
    resource->onAttach(context, framebuferSerial);
