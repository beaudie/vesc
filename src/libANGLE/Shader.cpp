...
Shader::Shader(ShaderProgramManager *manager,
    ...
    : mState(type),
      ...
      mMaxComputeWorkGroupInvocations(0u)
{
...
void Shader::compile(const Context *context)
{
    ...
    mMaxComputeWorkGroupInvocations = context->getCaps().maxComputeWorkGroupInvocations;
}
    ...
    switch (mState.mShaderType)
    {
        case ShaderType::Compute:
        {
            ...
            if (mState.mLocalSize.isDeclared())
            {
                ...
                if (checked_local_size_product.ValueOrDie() > mMaxComputeWorkGroupInvocations)
                {
                    WARN() << std::endl
