...
void Shader::resolveCompile()
{
    ...
    auto updateShaderStateFunctor = [this](bool compiled, ShHandle compilerHandle) {
        if (!compiled)
        {
    ...
    bool success = mCompilingState->compileEvent->postTranslate(
        &mInfoLog, std::move(updateShaderStateFunctor));
