...
ShaderState::ShaderState(ShaderType shaderType)
    : mShaderType(shaderType), mLabel(), mCompileStatus(CompileStatus::NOT_COMPILED)
{}
...
void Shader::compile(const Context *context)
{
    ...
    mState.mCompiledShaderState.hasValidGeometryShaderInputPrimitiveType  = false;
    mState.mCompiledShaderState.hasValidGeometryShaderOutputPrimitiveType = false;
    mState.mCompiledShaderState.hasValidGeometryShaderMaxVertices         = false;
    ...
    mState.mCompiledShaderState.advancedBlendEquationBits                 = 0;
    mState.mCompiledShaderState.hasDiscard                                = false;
    mState.mCompiledShaderState.enablesPerSampleShading                   = false;
    mState.mCompiledShaderState.specConstUsageBits                        = 0;
...
void Shader::resolveCompile(const Context *context)
{
    ...
    sh::GetCompiledShaderState(compilerHandle, &mState.mCompiledShaderState);
    ...
    if (mState.mShaderType == ShaderType::Compute &&
...
{
    resolveCompile(context);
    if (mState.mCompiledShaderState.hasValidGeometryShaderInputPrimitiveType)
    {
        return static_cast<PrimitiveMode>(
            mState.mCompiledShaderState.geometryShaderInputPrimitiveType);
    }
    return Optional<PrimitiveMode>::Invalid();
}

Optional<PrimitiveMode> Shader::getGeometryShaderOutputPrimitiveType(const Context *context)
{
    resolveCompile(context);
    if (mState.mCompiledShaderState.hasValidGeometryShaderOutputPrimitiveType)
    {
        return static_cast<PrimitiveMode>(
            mState.mCompiledShaderState.geometryShaderOutputPrimitiveType);
    }
    return Optional<PrimitiveMode>::Invalid();
}
...
{
    resolveCompile(context);
    if (mState.mCompiledShaderState.hasValidGeometryShaderMaxVertices)
    {
        return static_cast<GLint>(mState.mCompiledShaderState.geometryShaderMaxVertices);
    }
    return Optional<GLint>::Invalid();
}
...
angle::Result Shader::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    mState.mCompiledShaderState.serialize(stream, ToGLenum(mState.mShaderType));
...
angle::Result Shader::deserialize(const Context *context, BinaryInputStream &stream)
{
    ...
    mState.mCompiledShaderState.deserialize(stream, ToGLenum(mState.mShaderType));
