...
SharedCompileJob Shader::getCompileJob()
{
    if (mCompileJob)
    {
    ...
    CompileJobDone *doneJob = new CompileJobDone;
    doneJob->compiled       = mState.mCompileStatus == CompileStatus::COMPILED;

    return std::shared_ptr<CompileJob>(doneJob);
}
