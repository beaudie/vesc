...
void Shader::compile(const Context *context)
{
    ...
    if (display && context->getFrontendFeatures().cacheCompiledShader.enabled)
    {
        mMemoryShaderCache = display->getMemoryShaderCache();
    }
    ...
    if (mMemoryShaderCache)
    {
        std::lock_guard<std::mutex> cacheLock(mMemoryShaderCache->getMutex());
        angle::Result cacheResult = mMemoryShaderCache->getShader(context, this, &shaderHash);
...
void Shader::resolveCompile()
{
    ...
    if (success && mMemoryShaderCache)
    {
        // Save to the shader cache.
        std::lock_guard<std::mutex> cacheLock(mMemoryShaderCache->getMutex());
        if (mMemoryShaderCache->putShader(mCompilingState->shaderHash, this) !=
            angle::Result::Continue)
        {
            WARN() << "Failed to save compiled shader to memory shader cache.";
        }
    }
}
...
angle::Result Shader::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    stream.writeInt((uint32_t)SHADER_CACHE_IDENTIFIER);
    ...
    switch (mType)
    {
        case ShaderType::Compute:
        {
            ...
            stream.writeInt(mCurrentMaxComputeWorkGroupInvocations);
            stream.writeInt(mMaxComputeSharedMemory);
            break;
        }
    ...
    {
        WARN() << "Failed to allocate enough memory to serialize a shader. (" << stream.length()
               << " bytes )";
...
angle::Result Shader::deserialize(const Context *context, BinaryInputStream &stream)
{
    ...
    if (stream.readInt<uint32_t>() != SHADER_CACHE_IDENTIFIER)
    {
    ...
    switch (mType)
    {
        case ShaderType::Compute:
        {
            ...
            stream.readInt(&mCurrentMaxComputeWorkGroupInvocations);
            stream.readInt(&mMaxComputeSharedMemory);
            break;
        }
        case ShaderType::Vertex:
        {
