...
class Shader::CompileTask final : public angle::Closure
{
public:
    ...
    bool isCompilingInternally()
    {
        return !mTranslateTask->isCompilingInternally();
    }
...
class Shader::CompileEvent final
{
  public:
    ...
    {}
    ~CompileEvent() override {}

    angle::Result wait() override
    {
        ANGLE_TRACE_EVENT0("gpu.angle", "Shader::CompileEvent::wait");
    ...
    // TODO: give info log to resolveCompile
...
{
    // TODO: key like in program
    std::unique_ptr<ComileEvent> compileEvent;
    ShCompilerInstance shCompilerInstance;
};

angle::Result Shader::CompileTask::compileImpl()
{
    // Call the translator and get the info log
    angle::Result result = mTranslateTask->translate(mCompilerHandle, mOptions, mSource);
    ...
    // being overriden, etc.
    ANGLE_TRY(postTranslate());

    // For the GL backend, ensure the driver starts compilation now that translation has succeeded.
    mTranslateTask->compileTranslated(mCompilerHandle);
    return angle::Result::Continue;
}

angle::Result Shader::CompileTask::postTranslate()
{
    ...
    ASSERT(!mCompiledState->translatedSource.empty() ||
           !mCompiledState->compiledBinary.empty());

    // Validation checks for compute shaders
    if (mCompiledState->shaderType == ShaderType::Compute &&
        mCompiledState->localSize.isDeclared())
    {
        angle::CheckedNumeric<size_t> checked_local_size_product(
            mCompiledState->localSize[0]);
        checked_local_size_product *= mCompiledState->localSize[1];
        checked_local_size_product *= mCompiledState->localSize[2];

        if (!checked_local_size_product.IsValid() || checked_local_size_product.ValueOrDie() >
...
void Shader::compile(const Context *context)
{
    ...
    std::shared_ptr<rx::ShaderTranslateTask> translateTask = mImplementation->compile(context, &options);

    // Prepare the complete compile task
    std::shared_ptr<CompileTask> compileTask(new CompileTask(this, std::move(translateTask)));
    ...
    std::shared_ptr<angle::WaitableEvent> compileEvent =
        compilePool->postWorkerTask(compileTask);
...
void Shader::resolveCompile(const Context *context)
{
    ...
    // TODO: Resolve basically should be so empty, it can be called from another thread
