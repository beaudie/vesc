...
#define ANGLE_MACRO_STATIC_ASSERT_SIZE(className, classSize, methodName) \
    static_assert(                                                       \
        sizeof(className) == classSize,                                  \
        ANGLE_MACRO_STRINGIZE(className) " class has changed. Please "   \
                                         "update " ANGLE_MACRO_STRINGIZE(methodName) " method");
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(Framebuffer, 744, SerializeFramebuffer)
    ...
    bos->writeInt<unsigned long>(framebuffer->mDirtyBits.to_ulong());
    bos->writeInt<unsigned long>(framebuffer->mFloat32ColorAttachmentBits.to_ulong());
    if (framebuffer->mDirtyBitsGuard.valid())
    {
        bos->writeInt<unsigned long>(framebuffer->mDirtyBitsGuard.value().to_ulong());
    }
    SerializeObserverBinding(bos, &framebuffer->mDirtyDepthAttachmentBinding);
    for (angle::ObserverBinding &binding : framebuffer->mDirtyColorAttachmentBindings)
    {
        SerializeObserverBinding(bos, &binding);
    }
    SerializeObserverBinding(bos, &framebuffer->mDirtyStencilAttachmentBinding);
}
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(FramebufferState, 488, SerializeFramebufferState)
    ...
    bos->writeInt<unsigned long>(framebufferState->mResourceNeedsInit.to_ulong());
    bos->writeInt<bool>(framebufferState->mDefaultFramebufferReadAttachmentInitialized);
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(Offset, 12, SerializeOffset)
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(FramebufferAttachment, 48, SerializeFramebufferAttachment)
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(ImageIndex, 16, SerializeImageIndex)
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(angle::ObserverBinding, 32, SerializeObserverBinding)
    if (observerBinding->mSubject != nullptr)
    {
        SerializeSubject(bos, observerBinding->mSubject);
    }
}

void SerializeSubject(gl::BinaryOutputStream *bos, angle::Subject *subject)
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(angle::Subject, 96, SerializeSubject)
    for (size_t i = 0; i < subject->mObservers.size(); i++)
    {
        SerializeObserverBindingBase(bos, subject->mObservers[i]);
    }
}
...
{
    ANGLE_MACRO_STATIC_ASSERT_SIZE(angle::ObserverBindingBase, 24, SerializeObserverBindingBase)
    bos->writeInt<angle::SubjectIndex>(observerBindingBase->mIndex);
}
