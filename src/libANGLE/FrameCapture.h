...
using BufferMapStatus = std::map<gl::BufferID, bool>;
...
class ResourceTracker final : angle::NonCopyable
{
  public:
    ...
    bool &startingBuffersMappedCurrent(gl::BufferID id)
    {
        return mStartingBuffersMappedCurrent[id];
    }
    bool &startingBuffersMappedInitial(gl::BufferID id)
    {
  ...
  private:
    ...
    BufferMapStatus mStartingBuffersMappedInitial;
    // The status of buffer mapping throughout the trace, modified with each Map/Unmap call
    BufferMapStatus mStartingBuffersMappedCurrent;
};
