...
{
    Framebuffer *pFB = context->getFramebuffer(PackParam<FramebufferID>(framebuffer));
    ...
    if (focalPointsPerLayer > FoveationState::kMaxNumFocalPoints)
    {
    ...
    if (pFB->getState().getFoveationState().isConfigured())
    {
                                                ...
                                                GLfloat foveaArea)
{
    ...
    // FramebufferFoveationConfigQCOM. GFXSW-19887 update this condition when we support multiple
    // layers
    if (layer >= FoveationState::kMaxNumLayers)
    {
        ANGLE_VALIDATION_ERROR(GL_INVALID_VALUE, kFramebufferFoveationLayersExceedMaxArrayLayers);
        return false;
    }
    ...
    if (focalPoint >= FoveationState::kMaxNumFocalPoints)
    {
        ANGLE_VALIDATION_ERROR(GL_INVALID_VALUE, kFocalPointsExceedMaxFocalPoints);
        return false;
    }
...
bool ValidateTextureFoveationParametersQCOM(const Context *context,
                                            ...
                                            GLfloat foveaArea)
{
    Texture *pTex = context->getTexture(PackParam<TextureID>(texture));
    ...
    if (focalPoint > pTex->getNumFocalPoints() - 1)
    {
        ANGLE_VALIDATION_ERROR(GL_INVALID_VALUE, kFocalPointsExceedMaxFocalPoints);
        return false;
    }
