...
{
    Framebuffer *frameBuffer = context->getFramebuffer(PackParam<FramebufferID>(framebuffer));
...
bool ValidateTextureFoveationParametersQCOM(const Context *context,
                                            ...
                                            GLfloat foveaArea)
{
    Texture *textureObj = context->getTexture(PackParam<TextureID>(texture));
    ...
    if (!textureObj->isFoveationEnabled())
    {
    ...
    if (focalPoint > textureObj->getNumFocalPoints() - 1)
    {
        ANGLE_VALIDATION_ERROR(GL_INVALID_VALUE, kFocalPointsExceedMaxFocalPoints);
        return false;
    }
