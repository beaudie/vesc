...
bool ValidatePLSCommon(const Context *context, angle::EntryPoint entryPoint, GLint plane)
{
    ...
    if (plane >= context->getCaps().maxPixelLocalStoragePlanes)
    {
                                                      ...
                                                      GLint layer)
{
    ...
    if (backingtexture.value != 0)
    {
        ...
        if ((GLuint)level >= tex->getImmutableLevels())
        {
        ...
        ImageIndex index      = ImageIndex::MakeFromType(tex->getType(), level, layer);
        GLenum internalformat = tex->getFormat(index.getTarget(), level).info->internalFormat;
        if (!ValidatePLSInternalformat(context, entryPoint, internalformat))
        {
            return false;
        }
...
bool ValidateFramebufferPixelLocalClearValuefvANGLE(const Context *context,
                                                    ...
                                                    const GLfloat *value)
{
    return ValidatePLSCommon(context, entryPoint, plane);
}
                                         ...
                                         const GLenum *loadops)
{
    ...
    if (planes > context->getCaps().maxPixelLocalStoragePlanes)
    {
    ...
    size_t firstReservedDrawBuffer = std::min(caps.maxCombinedDrawBuffersAndPLSPlanes - planes,
                                              caps.maxColorAttachmentsWithActivePLS);
    for (size_t i = firstReservedDrawBuffer; i < framebuffer->getDrawbufferStateCount(); ++i)
    {
        if (framebuffer->getDrawBufferState(i) != GL_NONE)
        {
            context->validationError(entryPoint, GL_INVALID_OPERATION, kPLSReservedDrawBufferInUse);
            return false;
        }
    }
    ...
    //
    // (Whichever is lesser.)
    for (size_t i = firstReservedDrawBuffer; i < framebuffer->getNumColorAttachments(); ++i)
    {
        if (framebuffer->getColorAttachment(i))
        {
            context->validationError(entryPoint, GL_INVALID_OPERATION,
                                     kPLSReservedColorAttachmentInUse);
            return false;
        }
    }

    PixelLocalStorage *pls         = framebuffer->peekPixelLocalStorage();
    ...
    for (int i = 0; i < planes; ++i)
    {
        ...
        if (!pls || pls->isPlaneDeinitialized(i))
        {
            context->validationError(entryPoint, GL_INVALID_OPERATION,
        ...
        // internalformat.
        PixelLocalStoragePlane &plane = pls->getPlane(i);
        plane.convertToMemorylessIfTextureIDDeleted(context);
        ...
        if (loadops[i] == GL_KEEP && plane.isMemoryless())
        {
            context->validationError(entryPoint, GL_INVALID_OPERATION, kPLSKeepingMemorylessPlane);
            return false;
        }
    ...
    // Register the PLS extents now that we've validated them.
    if (pls)
    {
        pls->setPLSExtents(hasTextureBackedPLSPlanes ? textureBackedPLSExtents
                                                     : firstAttachment->getSize());
    }
