                              ...
                              const void *pixels)
{
    ...
    {
        context->validationError(GL_INVALID_OPERATION, kGetImageCompressed);
        return false;
    }
    ...
    switch (target)
    {
        ...
        case GL_TEXTURE_CUBE_MAP:
        {
            context->validationError(GL_INVALID_OPERATION,
                                     "There is no EGL image of Cube_map type");
            return false;
        }
        case GL_TEXTURE_CUBE_MAP_ARRAY:
        {
            context->validationError(GL_INVALID_OPERATION,
                                     "There is no EGL image of Cube_map_array type");
            return false;
        }
        default:
            context->validationError(GL_INVALID_ENUM, "Invalid texture target");
            return false;
    }

    // attrib list validation
    if (attrib_list != NULL && attrib_list[0] != GL_NONE)
    {
        context->validationError(GL_INVALID_VALUE, "Attrib list must be Null or GL_NONE");
        return false;
    }
        ...
        case GL_TEXTURE_EXTERNAL_OES:
        {
            if (imageObject->getSourceType() != EGL_GL_TEXTURE_2D_KHR)
            {
                context->validationError(
                    ...
                    "The source texture of image and target texture must match");
                return false;
            }
