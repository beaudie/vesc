...
class PixelLocalStoragePlane
{
  public:
    ...
    // Called when the context is being torn down. Causes this class to release its texture object
    // reference, and just let go the GL texture ID for the memoryless plane (if any) -- GL objects
    // get cleaned up automatically as part of teardown.
    void onContextTeardown(const Context *);
...
class PixelLocalStorage
{
  public:
    ...
    // Called when the context is being torn down. Causes this class to release its object
    // references, and just let go of GL objects -- GL objects get cleaned up automatically as part
    // of teardown.
    virtual void onContextTeardown(const Context *);
    ...
    {
        mPlanes[plane].setClearValuef(value);
    }
    void setClearValuei(GLint plane, const GLint value[]) { mPlanes[plane].setClearValuei(value); }
    void setClearValueui(GLint plane, const GLuint value[])
    {
        mPlanes[plane].setClearValueui(value);
    }
  ...
  private:
    PixelLocalStoragePlane mPlanes[IMPLEMENTATION_MAX_PIXEL_LOCAL_STORAGE_PLANES];
    Extents mPLSExtents;
