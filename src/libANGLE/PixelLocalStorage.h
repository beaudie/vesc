...
class PixelLocalStorage
{
  public:
    ...
    PixelLocalStorage(bool memorylessNeedsBackingTexture);
  ...
  private:
    // In some implementations we need a backing texture for every plane, even if the plane is
    // memoryless; glInvalidateFramebuffer() will ideally prevent memory transactions with this
    // texture where possible.
    const bool mMemorylessNeedsBackingTexture;
