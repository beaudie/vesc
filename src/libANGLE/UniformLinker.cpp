                                  ...
                                  const Program::Bindings &uniformLocationBindings)
{
    ...
    for (size_t uniformIndex = 0; uniformIndex < mUniforms.size(); uniformIndex++)
    {
        ...
        for (unsigned int arrayIndex = 0; arrayIndex < uniform.getArraySizeProduct(); arrayIndex++)
        {
    ...
    int *maxUniformLocation)
{
    for (const LinkedUniform &uniform : mUniforms)
    {
        ...
        if (shaderLocation != -1)
        {
            for (unsigned int arrayIndex = 0; arrayIndex < uniform.getArraySizeProduct();
                 arrayIndex++)
            {
    ...
    int *location)
{
    ...
    if (uniform.isArray())
    {
        // We're following the spec and including [0] at the end of the uniform name. This makes
        // implementation of uniform queries simpler.
        fullName += "[0]";
        fullMappedName += "[0]";
    }
    ...
    else
    {
        ...
        linkedUniform.flattenedOffsetInParentArray = uniform.flattenedOffsetInParentArray;
        if (markStaticUse)
        {
