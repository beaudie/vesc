    ...
    int *location)
{
    ...
    else
    {
        ...
        // ESSL 3.10, section 4.4.5:
        // Any uniform sampler, image or atomic counter variable declared without a binding
        // qualifier is initially bound to unit zero.
        if (IsOpaqueType(uniform.type) && linkedUniform.binding == -1)
        {
            linkedUniform.binding = 0;
        }
