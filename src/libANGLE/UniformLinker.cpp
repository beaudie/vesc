...
void UniformLinker::indexUniformArray(size_t uniformIndex,
                                      int preSetLocation,
                                      int shaderLocation,
                                      ...
                                      std::map<GLuint, VariableLocation> *preLocatedUniforms)
{
         ...
         arrayIndex++)
    {
        ...
        {
            VariableLocation location(*arrayIndices, static_cast<unsigned int>(uniformIndex));

            if ((location.areAllArrayIndicesZero() && preSetLocation != -1) || shaderLocation != -1)
            {
                int elementLocation =
                    preSetLocation + mState.flattenUniformArrayElementOffset(location);
         ...
         ++arrayElement)
    {
        ...
        if (arrayNestingIndex + 1u < uniform.arraySizes.size())
        {
            shaderUniformCount += flattenStructArrayUniform(
                ...
                imageUniforms, atomicCounterUniforms, markStaticUse, binding, offset, location);
        }
        else
        {
            shaderUniformCount += flattenStructUniform(
                uniform.fields, elementName, elementMappedName, samplerUniforms, imageUniforms,
                atomicCounterUniforms, markStaticUse, binding, offset, location);
        }
    }
    return shaderUniformCount;
}
    ...
    int *location)
{
    ...
    if (uniform.isStruct())
    {
        if (uniform.isArray())
        {
            flattenStructArrayUniform(uniform, 0u, fullName, fullMappedName, samplerUniforms,
                                      imageUniforms, atomicCounterUniforms, markStaticUse, binding,
                                      offset, location);
        }
        else
        {
