    ...
    InfoLog &infoLog)
{
    ...
    for (const sh::Uniform &uniform : shader->getUniforms(context))
    {
        shaderUniformCount += flattenUniform(uniform, &samplerUniforms, &imageUniforms,
                                             &atomicCounterUniforms, shaderType);
    }
...
bool UniformLinker::flattenUniformsAndCheckCaps(const Context *context, InfoLog &infoLog)
{
    ...
    if (mState.getAttachedComputeShader())
    {
        ...
        if (!flattenUniformsAndCheckCapsForShader(
                context, computeShader, GL_COMPUTE_SHADER, caps.maxComputeUniformComponents / 4,
    ...
    else
    {
        ...
        if (!flattenUniformsAndCheckCapsForShader(
                context, vertexShader, GL_VERTEX_SHADER, caps.maxVertexUniformVectors,
        ...
        if (!flattenUniformsAndCheckCapsForShader(
                context, fragmentShader, GL_FRAGMENT_SHADER, caps.maxFragmentUniformVectors,
                caps.maxTextureImageUnits, caps.maxFragmentImageUniforms,
    ...
    GLenum shaderType)
{
    ...
    ShaderUniformCount shaderUniformCount = flattenUniformImpl(
        uniform, uniform.name, samplerUniforms, imageUniforms, atomicCounterUniforms, shaderType,
    ...
    int *location)
{
        ...
        for (unsigned int elementIndex = 0; elementIndex < uniform.elementCount(); elementIndex++)
        {
            ...
            for (size_t fieldIndex = 0; fieldIndex < uniform.fields.size(); fieldIndex++)
            {
                ...
                shaderUniformCount += flattenUniformImpl(
                    field, fieldFullName, samplerUniforms, imageUniforms, atomicCounterUniforms,
                    shaderType, markStaticUse, -1, -1, location);
            }
        }
    ...
    else if (isAtomicCounter)
    {
        ...
        // GLES 3.10 P83:
        // For atomic counter uniforms and uniforms in uniform blocks,the value -1 is written to
        // params.
        *location = -1;
    }
    ...
    if (existingUniform)
    {
        ...
        if (markStaticUse)
        {
            ...
            existingUniform->setRef(shaderType);
        }
    }
    else
    {
        ...
        if (markStaticUse)
        {
            linkedUniform.setRef(shaderType);
        }
