         ...
         ++arrayElement)
    {
        ...
        if (arrayNestingIndex + 1u < uniform.arraySizes.size())
        {
            shaderUniformCount += flattenStructArrayUniform(
                ...
                imageUniforms, atomicCounterUniforms, shaderType, markStaticUse, binding, offset, location);
        }
        else
        {
    ...
    if (uniform.isStruct())
    {
        if (uniform.isArray())
        {
            flattenStructArrayUniform(uniform, 0u, fullName, fullMappedName, samplerUniforms,
                                      imageUniforms, atomicCounterUniforms, shaderType, markStaticUse, binding,
                                      offset, location);
        }
        else
        {
