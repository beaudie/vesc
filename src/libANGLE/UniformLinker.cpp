...
bool UniformLinker::flattenUniformsAndCheckCapsForShader(
    ...
    InfoLog &infoLog)
{
    VectorAndOpaque vectorAndOpaqueCount;
    for (const sh::Uniform &uniform : shader.getUniforms())
    {
        vectorAndOpaqueCount += flattenUniform(uniform, &samplerUniforms);
    }

    if (vectorAndOpaqueCount.vectorCount > maxUniformComponents)
    {
        infoLog << componentsErrorMessage << maxUniformComponents << ").";
        return false;
    }

    if (vectorAndOpaqueCount.samplerCount > maxTextureImageUnits)
    {
        infoLog << samplerErrorMessage << maxTextureImageUnits << ").";
        return false;
    }

    if (vectorAndOpaqueCount.atomicCounterCount > maxAtomicCounters)
    {
...
UniformLinker::VectorAndOpaque UniformLinker::flattenUniform(
    ...
    std::vector<LinkedUniform> *samplerUniforms)
{
    ...
    return VectorAndOpaque();
}

UniformLinker::VectorAndOpaque UniformLinker::flattenUniformImpl(
