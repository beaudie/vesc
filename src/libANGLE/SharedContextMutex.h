...
class [[nodiscard]] ScopedContextMutexAddRefLock final : angle::NonCopyable
{
  public:
    ...
    ANGLE_INLINE ScopedContextMutexAddRefLock(ContextMutex *mutex) { lock(mutex); }
...
// Use when need to lock ContextMutex that is nullptr most of the time.
class [[nodiscard]] ScopedOptionalContextMutexLock final : angle::NonCopyable
{
  public:
    ANGLE_INLINE explicit ScopedOptionalContextMutexLock(egl::ContextMutex *mutex) : mMutex(mutex)
    {
        if (ANGLE_UNLIKELY(mMutex != nullptr))
        {
            mMutex->lock();
        }
    }
    ANGLE_INLINE ~ScopedOptionalContextMutexLock()
    {
        if (ANGLE_UNLIKELY(mMutex != nullptr))
        {
            mMutex->unlock();
