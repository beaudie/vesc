...
size_t Buffer::getContentsObserverIndex(void *observingObject, uint32_t bufferIndex) const
{
    ContentsObserver observer{bufferIndex, observingObject};
    for (size_t observerIndex = 0; observerIndex < mContentsObservers.size(); ++observerIndex)
    {
        if (mContentsObservers[observerIndex] == observer)
        {
...
void Buffer::removeContentsObserverImpl(void *observingObject, uint32_t bufferIndex)
{
    size_t foundObserver = getContentsObserverIndex(observingObject, bufferIndex);
...
void Buffer::onContentsChange()
{
    ...
    {
        if (observer.bufferIndex != ContentsObserver::kBufferTextureIndex)
        {
            static_cast<VertexArray *>(observer.observingObject)
                ->onBufferContentsChange(observer.bufferIndex);
        }
        else
        {
            static_cast<Texture *>(observer.observingObject)->onBufferContentsChange();
        }
    }
}
}  // namespace gl
