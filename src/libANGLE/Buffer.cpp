                                     ...
                                     GLbitfield flags)
{
    ...
    if (usage == BufferUsage::InvalidEnum)  // called from glBufferStorage.
    {
        result = mImpl->setStorageData(context, target, dataForImpl, size, flags);
    }
    else  // called from glBufferData
    {
        result = mImpl->setData(context, target, dataForImpl, size, usage);
    }
    ...
    mState.mUsage = usage;
    mState.mSize  = size;
    mState.setImmutable(usage == BufferUsage::InvalidEnum);
    mState.setImmutableFlags(flags);
