                        ...
                        GLvoid *pixels)
{
    ...
    {
        // ..  the data would be packed to the buffer object such that the memory writes required would exceed the data store size. 
        ...
        if (checkedEndByte.ValueOrDie() > static_cast<size_t>(pixelPackBuffer->getSize()))
        {
            ...
            context->handleError(Error(GL_INVALID_OPERATION, "Writes would overflow from the pixel pack buffer."));
            return false;
        }
