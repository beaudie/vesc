...
bool ValidImageDataSize(ValidationContext *context,
                        ...
                        GLsizei imageSize)
{
    gl::Buffer *pixelUnpackBuffer = context->getGLState().getTargetBuffer(BufferType::PixelUnpack);
...
bool ValidateDrawBase(ValidationContext *context, GLenum mode, GLsizei count)
{
    ...
    if (state.hasMappedBuffer(BufferType::Array))
    {
        context->handleError(InvalidOperation());
        return false;
    }
                                ...
                                GLsizei primcount)
{
    ...
    // Check for mapped buffers
    if (state.hasMappedBuffer(BufferType::ElementArray))
    {
                            ...
                            void *pixels)
{
    ...
    gl::Buffer *pixelPackBuffer = context->getGLState().getTargetBuffer(BufferType::PixelPack);
