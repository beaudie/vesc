...
Program *GetValidProgramNoResolve(Context *context, ProgramID id)
{
    ...
    if (!validProgram)
    {
        if (context->getShader(ShaderID{id.value}))  // FIXME
        {
...
Shader *GetValidShader(Context *context, ShaderID id)
{
    ...
    if (!validShader)
    {
        if (context->getProgramNoResolveLink(ProgramID{id.value}))  // FIXME
        {
...
bool ValidateGetProgramivBase(Context *context, ProgramID program, GLenum pname, GLsizei *numParams)
{
