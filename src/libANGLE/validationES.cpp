...
bool ProgramOutputsMatchFramebuffer(const Program *program, const Framebuffer *framebuffer)
{
    ...
    const uint16_t outputTypes = static_cast<uint16_t>(program->getDrawBufferTypeMask().to_ulong());
    const uint16_t inputTypes =
        static_cast<uint16_t>(framebuffer->getDrawBufferTypeMask().to_ulong());

    uint16_t outputMask =
        static_cast<uint16_t>(program->getState().getActiveOutputVariables().to_ulong());
    uint16_t inputMask = static_cast<uint16_t>(framebuffer->getDrawBufferMask().to_ulong());
