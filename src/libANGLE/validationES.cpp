...
bool ValidBufferTarget(const Context *context, GLenum target)
{
    switch (target)
    {
      ...
      case GL_PIXEL_UNPACK_BUFFER:
          return (context->getExtensions().pixelBufferObject || context->getClientMajorVersion() >= 3);
...
bool ValidBufferParameter(const Context *context, GLenum pname)
{
    ...
    switch (pname)
    {
      ...
      case GL_BUFFER_MAPPED:
        ...
        return (context->getClientMajorVersion() >= 3) || extensions.mapBuffer || extensions.mapBufferRange;
...
bool ValidateUniform(gl::Context *context, GLenum uniformType, GLint location, GLsizei count)
{
    ...
    if (VariableComponentType(uniformType) == GL_UNSIGNED_INT && context->getClientMajorVersion() < 3)
    {
        context->handleError(Error(GL_INVALID_OPERATION));
        return false;
    }
                                  ...
                                  GLenum textarget, GLuint texture, GLint level)
{
    ...
    if (context->getClientMajorVersion() < 3 && !context->getExtensions().fboRenderMipmap && level != 0)
    {
        context->handleError(Error(GL_INVALID_VALUE));
        return false;
    }
