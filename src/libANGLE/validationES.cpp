...
bool ValidateVertexShaderAttributeTypeMatch(ValidationContext *context)
{
    ...
    unsigned long currentValuesTypeMask = glState.getCurrentValuesTypeMask().to_ulong();
    unsigned long vaoTypeMask           = vao->getAttributesTypeMask().to_ulong();
    unsigned long vaoEnabledMask        = vao->getAttributesMask().to_ulong();

    vaoEnabledMask |= vaoEnabledMask << MAX_COMPONENT_TYPE_MASK_INDEX;
    vaoTypeMask = (vaoEnabledMask & vaoTypeMask);
    vaoTypeMask |= (~vaoEnabledMask & currentValuesTypeMask);

    if (!ComponentTypeMask::Validate(program->getAttributesTypeMask().to_ulong(), vaoTypeMask,
