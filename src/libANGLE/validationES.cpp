                                               ...
                                               GLsizei height)
{
    ...
    fprintf(stderr, "ValidateRenderbufferStorageParametersBase: %d (internalformat: %u, converted: %u)\n", __LINE__, internalformat, convertedInternalFormat);

    const TextureCaps &formatCaps = context->getTextureCaps().get(convertedInternalFormat);
    fprintf(stderr, "ValidateRenderbufferStorageParametersBase: %d (Caps: %d, %d, %d, %d)\n", __LINE__, formatCaps.texturable,
            formatCaps.filterable, formatCaps.textureAttachment, formatCaps.renderbuffer);
    if (!formatCaps.renderbuffer)
    {
        fprintf(stderr, "ValidateRenderbufferStorageParametersBase: %d (format not suitable for renderbuffer)\n", __LINE__);
