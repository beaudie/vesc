...
{
    ASSERT(context->hasAnyEnabledClientAttrib());
...
bool ValidateDrawAttribs(Context *context, GLint primcount, GLint maxVertex)
{
    ...
    if (maxVertex <= context->getNonInstancedVertexElementLimit() &&
        primcount < context->getInstancedVertexElementLimit())
    {
        return true;
    }

    // An overflow can happen when adding the offset. Negative indicates overflow.
    if (context->getNonInstancedVertexElementLimit() < 0 ||
        context->getInstancedVertexElementLimit() < 0)
    {
...
bool ValidateDrawBase(Context *context, PrimitiveMode mode, GLsizei count)
{
    ...
    if (!extensions.webglCompatibility && state.getVertexArray()->hasMappedArrayBuffer())
    {
        context->handleError(InvalidOperation());
        return false;
    }
