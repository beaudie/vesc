...
bool ValidateDrawAttribs(Context *context, GLint primcount, GLint maxVertex, GLint vertexCount)
{
    ...
    if ((webglCompatibility || !state.areClientArraysEnabled()) &&
        vao->getEnabledClientMemoryAttribsMask().any())
    {
        // [WebGL 1.0] Section 6.5 Enabled Vertex Attributes and Range Checking
        // If a vertex attribute is enabled as an array via enableVertexAttribArray but no buffer is
        // bound to that attribute via bindBuffer and vertexAttribPointer, then calls to drawArrays
        // or drawElements will generate an INVALID_OPERATION error.
