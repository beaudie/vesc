                     ...
                     GLsizei count)
{
    ...
    if (!programObject)
    {
        ProgramPipeline *programPipelineObject = context->getState().getProgramPipeline();
        if (programPipelineObject)
        {
            programObject = programPipelineObject->getLinkedActiveShaderProgram(context);
        }
    }
...
const char *ValidateDrawStates(const Context *context)
{
    ...
    if (context->getClientVersion() >= Version(2, 0))
    {
        ...
        if (!executable || !executable->hasLinkedShaderStage(ShaderType::Vertex) ||
