                               ...
                               GLenum valueType)
{
    ...
    // check that value type is legal for this uniform
    bool match = valueType == uniform.type ||
                 (valueType == GL_INT && IsSamplerType(uniform.type)) ||
                 VariableAsBool(valueType) == uniform.type;
    if (!match)
    {
                            ...
                            GLsizei count)
{
    ...
    return ValidateUniformCommonBase(context, GetValidProgram(context, program), location, count,
                                     valueType);
}
...
bool ValidateUniform(gl::Context *context, GLenum valueType, GLint location, GLsizei count)
{
    ...
    return ValidateUniformCommonBase(context, context->getGLState().getProgram(), location, count,
                                     valueType);
}
