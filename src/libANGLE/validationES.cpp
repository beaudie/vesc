...
bool ValidateVertexShaderAttributeTypeMatch(const Context *context, angle::EntryPoint entryPoint)
{
                                              ...
                                              ProgramPipeline *programPipeline)
{
    for (const ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        {
            const char *errorMsg =
                ValidateProgramDrawStates(context, entryPoint, extensions, program);
            if (errorMsg)
            {
                return errorMsg;
            }
...
const char *ValidateDrawStates(const Context *context, angle::EntryPoint entryPoint)
{
    ...
    if (context->getClientVersion() >= Version(2, 0))
    {
        ...
        else if (programPipeline)
        {
            ...
            errorMsg =
                ValidateProgramPipelineDrawStates(context, entryPoint, extensions, programPipeline);
            if (errorMsg)
            {
                return errorMsg;
            }
        ...
        if (extensions.webglCompatibilityANGLE)
        {
            ...
            if (!ValidateVertexShaderAttributeTypeMatch(context, entryPoint))
            {
                ...
                !context->getState().allActiveDrawBufferChannelsMasked())
            {
                ...
                if (!ValidateFragmentShaderColorBufferMaskMatch(context, entryPoint))
                {
                    return kDrawBufferMaskMismatch;
                }

                // Detect that the color buffer types match the fragment shader output types
                if (!ValidateFragmentShaderColorBufferTypeMatch(context, entryPoint))
                {
