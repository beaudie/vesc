...
bool ValidateProgramSamplers(const Context *context, Program *program)
{
    ...
    Optional<bool> cachedValidateSamplersResult = program->getCachedValidateSamplersResult();
    if (cachedValidateSamplersResult.valid())
    {
        return cachedValidateSamplersResult.value();
    }

    return ValidateProgramSamplersImpl(context, program);
}
...
bool ValidateProgramSamplersImpl(const Context *context, Program *program)
{
    ...
    const ActiveTextureMask &activeSamplersMask               = executable.getActiveSamplersMask();
    const ActiveTextureArray<TextureType> &activeSamplerTypes = executable.getActiveSamplerTypes();

    // if any two active samplers in a program are of different types, but refer to the same
    // texture image unit, and this is the current program, then ValidateProgram will fail, and
    // DrawArrays and DrawElements will issue the INVALID_OPERATION error.
    for (size_t textureUnit : activeSamplersMask)
    {
        if (activeSamplerTypes[textureUnit] == TextureType::InvalidEnum)
        {
            program->setCachedValidateSamplersResult(false);
            return false;
        }
    }

    program->setCachedValidateSamplersResult(true);
    return true;
}
...
bool ValidateProgramPipelineSamplers(const Context *context, ProgramPipeline *programPipeline)
{
    for (const ShaderType shaderType : gl::AllShaderTypes())
    {
        ...
        if (program && !ValidateProgramSamplers(context, program))
        {
            return false;
        }
    }
...
const char *ValidateDrawStates(const Context *context)
{
    ...
    if (context->getClientVersion() >= Version(2, 0))
    {
        ...
        else if (programPipeline)
        {
            ...
            if (!ValidateProgramPipelineSamplers(context, programPipeline))
            {
                return kTextureTypeConflict;
            }
