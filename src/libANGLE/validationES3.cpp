                                              ...
                                              GLenum type)
{
    ...
    if (!formatInfo.textureSupport(context->getClientVersion(), context->getExtensions()))
    {
        if (formatInfo.internalFormat == GL_NONE || formatInfo.sized)
        {
            ...
            // as a
            // GLint instead of a GLenum. Therefor an invalid internal format gives a
            // GL_INVALID_VALUE
            // error instead of a GL_INVALID_ENUM error. As this validation function is only called
            // in
