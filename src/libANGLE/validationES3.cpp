                                              ...
                                              GLenum type)
{
    ...
    else
    {
        ...
        {
            if (!context->getExtensions().textureExternalYuvSamplingANGLE)
            {
                context->validationError(GL_INVALID_ENUM, kInvalidFormat);
                return false;
            }
                                       ...
                                       const void *pixels)
{
    ...
    if (gl::IsYuvFormat(format))
    {
        // According to ANGLE_texture_external_yuv_sampling, the texture needs to be an immutable
        // texture, texture target can only be TEXTURE_2D and there is no mipmap support
        if (!context->getExtensions().textureExternalYuvSamplingANGLE || !isSubImage)
        {
            context->validationError(GL_INVALID_ENUM, kInvalidFormat);
            return false;
        }
