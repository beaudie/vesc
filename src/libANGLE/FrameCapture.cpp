...
void FrameCapture::checkForCaptureTrigger()
{
    ...
    if (captureTrigger != mCaptureTrigger)
    {
        ...
        INFO() << "Capture triggered at frame " << mFrameIndex << " for " << mCaptureTrigger
               << " frames";
...
void FrameCapture::onEndFrame(const gl::Context *context)
{
    ...
    if (!mFrameCalls.empty() && mFrameIndex >= mCaptureStartFrame)
    {
        ...
        WriteCppReplay(mCompression, mOutDirectory, context, mCaptureLabel,
                       mFrameIndex - mCaptureStartFrame + 1, getFrameCount(), mFrameCalls,
                       mSetupCalls, &mResourceTracker, &mBinaryData, mSerializeStateEnabled);
