...
#ifdef __ANDROID__
#    define CAPTURE_FILE_FOLDER ("/sdcard/Android/data/" + current_apk_name() + "/")

#    include <sys/system_properties.h>

inline std::string current_apk_name()
{
    constexpr application_id_size = 64;
    static char *application_id   = new char[application_id_size];
    // static char application_id[64] = {0};
    if (!application_id[0])
    {
        FILE *cmdline = fopen("/proc/self/cmdline", "r");
        if (cmdline)
        {
            fread(application_id, application_id_size, 1, cmdline);
            fclose(cmdline);
        }
