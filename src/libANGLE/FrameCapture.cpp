            ...
            if (context->getExtensions().getImageANGLE)
            {
                ...
                if (captureCompressedImage)
                {
                    ...
                    cap(CaptureCompressedTexImage2D(
                        replayState, true, index.getTarget(), index.getLevelIndex(), internalFormat,
...
void FrameCapture::maybeCaptureClientData(const gl::Context *context, const CallCapture &call)
{
    switch (call.entryPoint)
    {
        ...
        case gl::EntryPoint::CompressedTexImage2D:
        {
            ...
            // If data is coming from a buffer, determine how to create a shadow copy
            ...
            // If no data was provided, nothing to capture
            if (!data)
            {
                return;
            }
