    ...
    for (const auto &bufferIter : buffers)
    {
        ...
        if (bufferMapped)
        {
            ...
            frameCapture->trackBufferMapping(&setupCalls->back(), buffer->id(),
                                             static_cast<GLsizeiptr>(buffer->getMapOffset()),
                                             static_cast<GLsizeiptr>(buffer->getMapLength()),
                                             buffer->getAccessFlags() & GL_MAP_WRITE_BIT);
        }
        else
        {
...
void FrameCapture::captureCompressedTextureData(const gl::Context *context, const CallCapture &call)
{
    ...
    ASSERT(gl::isPow2(format.compressedBlockWidth));
    ASSERT(gl::isPow2(format.compressedBlockHeight));
    int widthScale  = gl::log2(static_cast<int>(format.compressedBlockWidth));
    int heightScale = gl::log2(static_cast<int>(format.compressedBlockHeight));
