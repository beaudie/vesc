                              ...
                              const ProgramSourceMap &cachedProgramSources)
{
    ...
    for (const auto &bufferIter : buffers)
    {
        ...
        (void)buffer->mapRange(context, 0, buffer->getSize(), GL_MAP_READ_BIT);
    ...
    for (const auto &textureIter : textures)
    {
        ...
        {
            capTexParam(GL_TEXTURE_WRAP_R, textureSamplerState.getWrapT());
        }

        // Iterate texture levels and layers.
        gl::ImageIndexIterator imageIter = gl::ImageIndexIterator::MakeGeneric(
            texture->getType(), 0, texture->getMipmapMaxLevel(), gl::ImageIndex::kEntireLevel,
        ...
        while (imageIter.hasNext())
        {
            ...
            if (context->getExtensions().getImageANGLE)
            {
                ...
                uint32_t dataSize = internalFormat.pixelBytes * desc.size.width * desc.size.width;
                ...
                (void)texture->getTexImage(context, packState, nullptr, index.getTarget(),
                                           index.getLevelIndex(), readFormat, readType,
                                           data.data());

                cap(CaptureTexImage2D(context, true, index.getTarget(), index.getLevelIndex(),
                                      readFormat, desc.size.width, desc.size.height, 0, readFormat,
                                      readType, data.data()));
            }
            else
            {
