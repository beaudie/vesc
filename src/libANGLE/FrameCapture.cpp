    ...
    for (const auto &textureIter : textures)
    {
        ...
        while (imageIter.hasNext())
        {
            ...
            if (format.compressed)
            {
                ...
                const auto &foundTextureLevels = cachedTextureLevelData.find(texture->id().value);
...
void FrameCapture::maybeCaptureClientData(const gl::Context *context, const CallCapture &call)
{
    switch (call.entryPoint)
    {
        ...
        case gl::EntryPoint::CompressedTexImage2D:
        {
            ...
            const auto &foundTextureLevels = mCachedTextureLevelData.find(texture->id().value);
            ...
            else
            {
                ...
                mCachedTextureLevelData[texture->id().value] = std::move(textureLevels);
            }
            break;
        }
        ...
        case gl::EntryPoint::DeleteTextures:
        {
            ...
            for (int32_t i = 0; i < n; ++i)
            {
                ...
                const auto &foundTextureLevels = mCachedTextureLevelData.find(textureIDs[i].value);
                if (foundTextureLevels != mCachedTextureLevelData.end())
                {
