...
ParamBuffer::ParamBuffer(const char *callName) {}
                                              ...
                                              size_t instanceCount)
{
    ...
    for (size_t attribIndex : context->getStateCache().getActiveClientAttribsMask())
    {
        ...
        if (callIndex != -1)
        {
            ...
            GLuint stride = format.pixelBytes;
            ...
            size_t bytesToCapture = count * stride;
...
void FrameCapture::onEndFrame()
{
    if (!mCalls.empty())
    {
        ...
        if (!binaryData.empty())
        {
            std::string fname = GetCaptureFileName(mFrameIndex, ".data");
...
void WriteParamValueToStream<ParamType::TvoidConstPointer>(std::ostream &os, const void *value)
{
    ...
    {
        os << "reinterpret_cast<const void *>(" << reinterpret_cast<int>(value) << ")";
    }
}
}  // namespace angle
