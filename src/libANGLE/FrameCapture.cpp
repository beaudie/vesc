...
void FrameCapture::serializeContext(gl::Context *context)
{
    ...
    if (mFrameIndex == mFrameEnd && mSerializeContextEnabled)
    {
        ...
        // copy the offset of the serialization binary representation into the reserve block
        memcpy(mBinaryData.data(), reinterpret_cast<const uint8_t *>(&kBinaryAlignment),
               sizeof(size_t));
        // copy the length of the serialization binary representation into the reserve block
