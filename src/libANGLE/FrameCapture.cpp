                         ...
                         ParamCapture *paramCapture)
{
    ...
    if (pname == GL_COMPRESSED_TEXTURE_FORMATS || pname == GL_PROGRAM_BINARY_FORMATS ||
        pname == GL_SHADER_BINARY_FORMATS)
    {
        // This value needs to be updated if any new extensions are introduced that would allow for
        // more compressed texture formats The current value is taken from the max reported from
        // those capabilities on any driver:
        ...
        numParams                                       = kMaxReportedCapabilities;
    }
    else if (!gl::GetQueryParameterInfo(glState, pname, &nativeType, &numParams))
    {
        numParams = 1;
    }
