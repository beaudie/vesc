...
void FrameCapture::onEndFrame(const gl::Context *context)
{
    ...
    if (!mFrameCalls.empty() && mFrameIndex >= mFrameStart)
    {
        ...
        if (mFrameIndex == mFrameEnd)
        {
            ...
            {
                offset = rx::roundUp(mBinaryData.size(), kBinaryAlignment);
                mBinaryData.resize(offset + serializedContextData.length());
                memcpy(mBinaryData.data() + offset, serializedContextData.data(),
                       serializedContextData.length());
            }
            ...
            WriteCppReplayIndexFiles(mCompression, mOutDirectory, context->id(), mCaptureLabel,
                                     ...
                                     mReadBufferSize, mClientArraySizes, mHasResourceType, offset,
                                     serializedContextData.length());
        }
    }

    if (mFrameIndex == mFrameEnd && !mBinaryData.empty())
    {
