...
void FrameCapture::saveCapturedFrameAsCpp()
{
    ...
    if (!binaryData.empty())
    {
        ...
        if (!fp)
        {
            FATAL() << "file " << binaryDataFilename << " can not be created!: " << strerror(errno);
        }
    ...
    {
        header << "size_t " << binaryDataSizeVarname << " = 0;\n";
    ...
    if (!fp)
    {
        FATAL() << "file " << cppFileName << " can not be created!: " << strerror(errno);
    }
