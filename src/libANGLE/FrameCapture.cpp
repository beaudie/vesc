                              ...
                              const HasResourceTypeMap &hasResourceType)
{
    ...
    FILE *fp               = fopen(headerPath.c_str(), "w");
    if (!fp)
    {
        FATAL() << "file " << headerPath << " can not be created!: " << strerror(errno);
    }
    ...
    fp                     = fopen(sourcePath.c_str(), "w");
    if (!fp)
    {
        FATAL() << "file " << sourcePath << " can not be created!: " << strerror(errno);
    }
    fprintf(fp, "%s\n", sourceContents.c_str());
    fclose(fp);
    printf("Saved '%s'.\n", sourcePath.c_str());
}  // namespace
}  // namespace
