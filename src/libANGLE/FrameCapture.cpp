                              ...
                              const ProgramSourceMap &cachedProgramSources)
{
    ...
    for (const auto &textureIter : textures)
    {
        ...
        while (imageIter.hasNext())
        {
            ...
            if (context->getExtensions().getImageANGLE && !format.compressed)
            {
                uint32_t dataSize = format.pixelBytes * desc.size.width * desc.size.width;
                ...
                (void)texture->getTexImage(context, packState, nullptr, index.getTarget(),
                                           index.getLevelIndex(), format.format, format.type,
                                           data.data());
