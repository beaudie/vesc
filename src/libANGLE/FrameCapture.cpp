                              ...
                              const TextureLevelDataMap &cachedTextureLevelData)
{
    ...
    for (const auto &xfbIter : xfbMap)
    {
        ...
        {
            const auto &xfbBuffer = xfb->getIndexedBuffer(i);
            gl::BufferID bufferID = xfbBuffer.id();

            // Note: Buffers bound with BindBufferBase can be used with BindBuffer
            cap(CaptureBindBufferBase(replayState, true, gl::BufferBinding::TransformFeedback, 0,
                                      bufferID));
        }

        if (xfb->isActive() || xfb->isPaused())
        {
            // We don't support active queries in MEC yet
            UNIMPLEMENTED();
        }
    }
