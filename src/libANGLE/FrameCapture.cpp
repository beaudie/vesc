    ...
    switch (resourceIDType)
    {
        case ResourceIDType::Buffer:
        {
            ...
            for (const gl::BufferID id : startingBuffers)
            {
                ...
                if (resourceTracker->startingBuffersMappedInitial(id) &&
                    !resourceTracker->startingBuffersMappedCurrent(id))
                {
    ...
    for (size_t i = 0; i < uniforms.size(); i++)
    {
        ...
        switch (typeInfo->componentType)
        {
            case GL_FLOAT:
            {
                ...
                for (int i = 0; i < uniformCount; i++, readLoc.value++)
                {
                    program->getUniformfv(
                        context, readLoc,
                        static_cast<GLfloat *>(uniformBuffer.data() + i * sizeof(GLfloat)));
                }
            ...
            case GL_INT:
            {
                ...
                for (int i = 0; i < uniformCount; i++, readLoc.value++)
                {
                    program->getUniformiv(
                        context, readLoc,
                        static_cast<GLint *>(uniformBuffer.data() + i * sizeof(GLint)));
                }
            ...
            case GL_UNSIGNED_INT:
            {
                ...
                for (int i = 0; i < uniformCount; i++, readLoc.value++)
                {
                    program->getUniformuiv(
                        context, readLoc,
                        static_cast<GLuint *>(uniformBuffer.data() + i * sizeof(GLuint)));
                }
                switch (typeInfo->componentCount)
                {
                    case 4:
                              ...
                              FrameCapture *frameCapture)
{
    ...
    for (const auto &bufferIter : buffers)
    {
        ...
        bool bufferMapped = buffer->isMapped();
...
{
    // If this was a starting buffer that was unmapped, we need to ensure it gets unmapped again
    // during Reset
    if (mStartingBuffers.find(id) != mStartingBuffers.end())
    {
...
{
    // If this was a starting buffer that was mapped, we need to ensure it gets mapped again
    // during Reset
    if (mStartingBuffers.find(id) != mStartingBuffers.end())
    {
        // Check the current state
