                         ...
                         std::vector<uint8_t> *binaryData)
{
    switch (resourceIDType)
    {
        case ResourceIDType::Buffer:
        {
            ...
            for (BufferSet::iterator iter = buffersToRegen->begin(); iter != buffersToRegen->end();
                 ++iter)
            {
                // Emit their regen calls
                for (CallCapture &call : (*bufferRegenCalls)[(*iter)])
                {
                    out << "    ";
            ...
            for (BufferSet::iterator iter = buffersToRestore->begin();
                 iter != buffersToRestore->end(); ++iter)
            {
                // Emit their restore calls
                for (CallCapture &call : (*bufferRestoreCalls)[(*iter)])
                {
                    out << "    ";
                    WriteCppReplayForCall(call, counters, out, header, binaryData);
                    out << ";\n";
