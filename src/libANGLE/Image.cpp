...
ImageState::ImageState(EGLenum target,
                       ImageSibling *buffer,
                       const AttributeMap &attribs,
                       gl::TextureType glTextureType)
    : label(nullptr),
      target(target),
      imageIndex(GetImageIndex(target, attribs, glTextureType)),
      source(buffer),
      targets(),
      format(GL_NONE),
      size(),
...
Image::Image(rx::EGLImplFactory *factory,
    ...
    : mState(target,
             buffer,
             attribs,
             IsTextureTarget(target)
                 ? context->getTextureType(gl::FromGL<gl::TextureID>(buffer->getId()))
                 : gl::TextureType::InvalidEnum),
