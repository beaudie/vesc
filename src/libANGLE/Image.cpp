...
ImageSibling::ImageSibling(GLuint id)
    : gl::FramebufferAttachmentObject(id),
      mSources(),
      mTarget()
{
}
...
void ImageSibling::addImageSource(egl::Image *imageSource)
{
    ...
    mSources.insert(imageSource);
}

void ImageSibling::setTargetImage(egl::Image *imageTarget)
{
    ASSERT(imageTarget != nullptr);
    mTarget.set(imageTarget);
    imageTarget->addTargetSibling(this);
}

void ImageSibling::orphanImages()
{
    if (mTarget.get() != nullptr)
    {
        mTarget->orphanSibling(this);
        mTarget.set(nullptr);
    }

    for (auto &sourceImage : mSources)
    {
        sourceImage->orphanSibling(this);
    }
    mSources.clear();
}
      ...
      mTargets()
{
    ...
    {
        gl::Texture *texture = rx::GetAs<gl::Texture>(buffer);
    ...
    {
        gl::Renderbuffer *renderbuffer = rx::GetAs<gl::Renderbuffer>(buffer);
...
void Image::orphanSibling(ImageSibling *sibling)
{
    // notify impl
    mImplementation->orphan(sibling);
