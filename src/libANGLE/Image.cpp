...
namespace egl
{
ImageSibling::ImageSibling(GLuint id) : gl::FramebufferAttachmentObject(id), mSources(), mTarget()
{
}
...
gl::Error ImageSibling::orphanImages()
{
    if (mTarget.get() != nullptr)
    {
        ...
        gl::Error error = mTarget->orphanSibling(this);
        if (error.isError())
        {
            return error;
        }
...
{
    ASSERT(mSources.find(imageSource) != mSources.end());
    mSources.erase(imageSource);
}
