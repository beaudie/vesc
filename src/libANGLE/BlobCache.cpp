                    ...
                    size_t valueSize)
{
    ...
    else if (display && display->getState().setBlobFunc)
    {
                    ...
                    std::vector<uint8_t> *valueOut)
{
    ...
    else if (display && display->getState().getBlobFunc)
    {
        ...
        if (valueSize > 0)
        {
            ...
            valueSize = display->getState().getBlobFunc(key, keySize, valueOut->data(), valueSize);
            result    = static_cast<size_t>(valueSize) == valueOut->size();
        }
    }
