                    ...
                    BlobCache::Value *valueOut)
{
    ...
    if (areBlobCacheFuncsSet() || (context && context->areBlobCacheFuncsSet()))
    {
        auto callBlobGetter = [eglGetBlob = mGetBlobFunc, context](const void *key, size_t keySize,
                                                                   void *value,
                                                                   size_t valueSize) -> size_t {
            if (context && context->areBlobCacheFuncsSet())
            {
                const gl::BlobCacheCallbacks &contextCallbacks =
                    context->getState().getBlobCacheCallbacks();
            ...
            {
                ASSERT(eglGetBlob);
                return eglGetBlob(key, keySize, value, valueSize);
            }
        };

        std::scoped_lock<angle::SimpleMutex> lock(mBlobCacheMutex);
        EGLsizeiANDROID valueSize = callBlobGetter(key.data(), key.size(), nullptr, 0);
        if (valueSize <= 0)
        {
            return false;
        }
        ...
        valueSize = callBlobGetter(key.data(), key.size(), scratchMemory->data(), valueSize);
