...
void BlobCache::put(egl::Display *display, const void *key, size_t keySize, const void *value, size_t valueSize)
{
    ...
    // It is assumed that display is always provided, except for during application startup, where they are
    // read from disk.  The `display` pointer is used to test this as during application startup this pointer
    // would be nullptr to avoid generating callbacks.
...
bool BlobCache::get(egl::Display *display, const void *key, size_t keySize, std::vector<uint8_t> *valueOut)
{
    std::vector<uint8_t> keyBuf;
    makeKey(key, keySize, &keyBuf);
...
bool BlobCache::getAt(size_t index, const std::vector<uint8_t> **keyOut, const std::vector<uint8_t> **valueOut)
{
    const CacheEntry *valueBuf;
