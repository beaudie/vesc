...
const char *GetShaderResourcesKey(const Shader *shader)
{
    return shader ? shader->getCompilerResourcesString().c_str() : "";
}
...
ProgramHash MemoryProgramCache::ComputeHash(const Context *context, const Program *program)
{
    ...
    hashStream << GetShaderHashKey(vertexShader) << ":" << GetShaderHashKey(fragmentShader) << ":"
               << GetShaderHashKey(computeShader) << ":" << GetShaderResourcesKey(vertexShader)
               << ":" << GetShaderResourcesKey(fragmentShader) << ":"
               << GetShaderResourcesKey(computeShader) << ":";
