                                          ...
                                          BlobCache::Key *hashOut)
{
    ...
    const angle::MemoryBuffer *binaryProgram;
    LinkResult result(false);
    if (get(*hashOut, context, &binaryProgram))
    {
        InfoLog infoLog;
...
bool MemoryProgramCache::get(const BlobCache::Key &programHash,
                             const Context *context,
                             const angle::MemoryBuffer **programOut)
{
    return mBlobCache.get(context->getCurrentDisplay(), programHash, programOut);
}
...
void MemoryProgramCache::remove(const BlobCache::Key &programHash)
{
    mBlobCache.remove(programHash);
}

void MemoryProgramCache::putProgram(const BlobCache::Key &programHash,
                                    const Context *context,
                                    const Program *program)
{
    ...
    mBlobCache.put(context->getCurrentDisplay(), programHash, std::move(serializedProgram));
}
...
void MemoryProgramCache::putBinary(const BlobCache::Key &programHash,
                                   const uint8_t *binary,
                                   size_t length)
{
    // Copy the binary.
    ...
    // Store the binary.
    mBlobCache.put(nullptr, programHash, std::move(newEntry));
}
