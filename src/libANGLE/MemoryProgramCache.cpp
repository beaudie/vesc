...
{
    CacheEntry newEntry;
    newEntry.first  = std::move(binaryProgram);
    newEntry.second = CacheSource::PutProgram;

    const CacheEntry *result =
        mProgramBinaryCache.put(program, std::move(newEntry), newEntry.first.size());
    if (!result)
    {
    ...
    else
    {
        ...
        platform->cacheProgram(platform, program, result->first.size(), result->first.data());
    }
}
