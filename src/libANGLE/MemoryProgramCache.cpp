                                           ...
                                           InfoLog &infoLog)
{
    ...
    state->mAttributesTypeMask.from_ulong(stream.readInt<unsigned long>());
    state->mAttributesMask = stream.readInt<unsigned long>();
    ...
    static_assert(IMPLEMENTATION_MAX_DRAW_BUFFERS * 2 <= 8 * sizeof(uint32_t),
                  "All bits of mDrawBufferTypeMask and mActiveOutputVariables types and mask fit into 32 bits each");
                                   ...
                                   angle::MemoryBuffer *binaryOut)
{
    ...
    static_assert(MAX_VERTEX_ATTRIBS * 2 <= sizeof(uint32_t) * 8,
                  "All bits of mAttributesTypeMask types and mask fit into 32 bits each");
    stream.writeInt(static_cast<uint32_t>(state.mAttributesTypeMask.to_ulong()));
    stream.writeInt(static_cast<uint32_t>(state.mAttributesMask.to_ulong()));
    ...
    static_assert(IMPLEMENTATION_MAX_DRAW_BUFFERS * 2 <= 8 * sizeof(uint32_t),
                  "All bits of mDrawBufferTypeMask and mActiveOutputVariables can be contained in an uint32_t");
