...
{
    ANGLE_TRY(mImpl->enqueueWaitForEvents(Event::Cast(numEvents, eventList)));
    return angle::Result::Continue;
}
...
{
    ANGLE_TRY(mImpl->enqueueBarrier());
    return angle::Result::Continue;
}
...
CommandQueue::CommandQueue(Context &context,
    ...
    : mContext(&context),
      ...
      mSize(size),
      mImpl(std::invoke([&]() -> rx::CLCommandQueueImpl::Ptr {
          rx::CLCommandQueueImpl::Ptr implPtr = nullptr;
          return IsError(context.getImpl().createCommandQueue(*this, implPtr)) ? nullptr
                                                                               : std::move(implPtr);
      }))
{
