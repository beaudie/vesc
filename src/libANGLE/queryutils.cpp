                                ...
                                ParamType *params)
{
    ...
    switch (pname)
    {
        ...
        case GL_TEXTURE_BUFFER_DATA_STORE_BINDING:
            *params = CastFromStateValue<ParamType>(
                pname, static_cast<uint32_t>(texture->getBufferDataStoreBinding(target, level)));
            break;
        case GL_TEXTURE_BUFFER_OFFSET:
            *params = CastFromStateValue<ParamType>(
                pname, static_cast<uint32_t>(texture->getBufferOffset(target, level)));
            break;
        case GL_TEXTURE_BUFFER_SIZE:
            *params = CastFromStateValue<ParamType>(
                pname, static_cast<uint32_t>(texture->getBufferSize(target, level)));
            break;
        default:
            UNREACHABLE();
            break;
