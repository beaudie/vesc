...

{
    switch (pname)
    {
        ...
        case GL_ACTIVE_VARIABLES:
            ...
            break;
        case GL_REFERENCED_BY_VERTEX_SHADER:
            params[(*outputPosition)++] = static_cast<GLint>(buffer.shaderRef.vertexStaticUse);
            break;
        case GL_REFERENCED_BY_FRAGMENT_SHADER:
            params[(*outputPosition)++] = static_cast<GLint>(buffer.shaderRef.fragmentStaticUse);
            break;
        case GL_REFERENCED_BY_COMPUTE_SHADER:
            params[(*outputPosition)++] = static_cast<GLint>(buffer.shaderRef.computeStaticUse);
            break;
        default:
            UNREACHABLE();
            break;
...
GLint GetUniformResourceProperty(const Program *program, GLuint index, const GLenum prop)
{
    ...
    switch (resourceProp)
    {
        ...
        case GL_REFERENCED_BY_VERTEX_SHADER:
            return uniform.shaderRef.vertexStaticUse;

        case GL_REFERENCED_BY_FRAGMENT_SHADER:
            return uniform.shaderRef.fragmentStaticUse;

        case GL_REFERENCED_BY_COMPUTE_SHADER:
            return uniform.shaderRef.computeStaticUse;
