...
GLint GetUniformResourceProperty(const Program *program, GLuint index, const GLenum prop)
{
    ...
    switch (resourceProp)
    {
        ...
        case GL_OFFSET:
            return uniform.isInDefaultBlock() ? -1 : uniform.blockOffset;

        case GL_ARRAY_STRIDE:
            return uniform.isInDefaultBlock() ? -1 : uniform.blockArrayStride;

        case GL_MATRIX_STRIDE:
            return uniform.isInDefaultBlock() ? -1 : uniform.blockMatrixStride;
