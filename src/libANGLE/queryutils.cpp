...
void QueryTexParameterBase(const Texture *texture, GLenum pname, ParamType *params)
{
    ...
    switch (pname)
    {
        ...
        {
            auto rect = texture->getCrop();
            params[0] = (ParamType)rect.x;
            params[1] = (ParamType)rect.y;
            params[2] = (ParamType)rect.width;
            params[3] = (ParamType)rect.height;
            break;
        }
        case GL_GENERATE_MIPMAP:
            *params = (ParamType)texture->getGenerateMipmapHint();
            break;
        default:
            UNREACHABLE();
            break;
...
void SetTexParameterBase(Context *context, Texture *texture, GLenum pname, const ParamType *params)
{
    ...
    switch (pname)
    {
        ...
        case GL_TEXTURE_CROP_RECT_OES:
        {
            ...
            texture->setCrop(gl::Rectangle(crop[0], crop[1], crop[2], crop[3]));
            break;
        }
        case GL_GENERATE_MIPMAP:
