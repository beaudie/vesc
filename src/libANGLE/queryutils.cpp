...
GLint GetInputResourceProperty(const Program *program, GLuint index, GLenum prop)
{
    ...
    switch (prop)
    {
        ...
        case GL_REFERENCED_BY_GEOMETRY_SHADER_EXT:
        {
            ...
            ShaderType shaderType = ShaderType::InvalidEnum;
            for (Shader *shader : program->getState().getAttachedShaders())
            {
                if (shader)
                {
...
GLint QueryProgramInterfaceActiveResources(const Program *program, GLenum programInterface)
{
    switch (programInterface)
    {
        case GL_PROGRAM_INPUT:
            return clampCast<GLint>(program->getState().getProgramInterfaceInputs().size());
...
GLint FindMaxNameSize(const ProgramInterfaceList &resources)
{
    GLint max = 0;
    for (const ProgramInterfacePair &resource : resources)
    {
        if (resource.second->isArray())
        {
            max = std::max(max, clampCast<GLint>((resource.first + "[0]").size()));
        }
        else
        {
            max = std::max(max, clampCast<GLint>((resource.first).size()));
        }
    }
    return max;
}
...
GLint QueryProgramInterfaceMaxNameLength(const Program *program, GLenum programInterface)
{
    ...
    {
        case GL_PROGRAM_INPUT:
            maxNameLength = FindMaxNameSize(program->getState().getProgramInterfaceInputs());
            break;
