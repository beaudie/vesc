...
template <typename T>
static void GetObjectID(const T *object)
{
...
std::vector<QueryT> QueryState(const Context *context, GLenum pname)
{
    ...
    switch (pname)
    {
      ...
      case GL_ARRAY_BUFFER_BINDING:                     InsertResult<GLint>(result, pname, GetObjectID(state.getArrayBuffer())));                            break;
      case GL_ELEMENT_ARRAY_BUFFER_BINDING:             InsertResult<GLint>(result, pname, GetObjectID(state.getVertexArray()->getElementArrayBuffer()));    break;
      //case GL_FRAMEBUFFER_BINDING:                    // now equivalent to GL_DRAW_FRAMEBUFFER_BINDING_ANGLE
      case GL_DRAW_FRAMEBUFFER_BINDING_ANGLE:           InsertResult<GLint>(result, pname, GetObjectID(state.getDrawFramebuffer()));                         break;
      case GL_READ_FRAMEBUFFER_BINDING_ANGLE:           InsertResult<GLint>(result, pname, GetObjectID(state.getReadFramebuffer()));                         break;
      case GL_RENDERBUFFER_BINDING:                     InsertResult<GLint>(result, pname, GetObjectID(state.getCurrentRenderbuffer()));                     break;
      case GL_VERTEX_ARRAY_BINDING:                     InsertResult<GLint>(result, pname, GetObjectID(state.getVertexArray()));                             break;
      case GL_CURRENT_PROGRAM:                          InsertResult<GLint>(result, pname, GetObjectID(state.getProgram()));                                 break;
      ...
      case GL_STENCIL_CLEAR_VALUE:                      InsertResult<GLint>(result, pname, state.getStencilClearValue());                                    break;
            ...
            if (framebuffer->checkStatus(data) == GL_FRAMEBUFFER_COMPLETE)
            {
                switch (pname)
                {
                  case GL_SAMPLE_BUFFERS:
                    ...
                    {
                        InsertResult<GLint>(result, pname, 1));
                    }
                    else
                    {
                    ...
                    break;
                  case GL_SAMPLES:
                    InsertResult<GLint>(result, pname, framebuffer->getSamples(data)));
                    break;
                }
            }
            else
      ...
      case GL_COPY_READ_BUFFER_BINDING:
        InsertResult<GLint>(result, pname, GetObjectID(state.getCopyReadBuffer())));
        break;
      case GL_COPY_WRITE_BUFFER_BINDING:
        InsertResult<GLint>(result, pname, GetObjectID(state.getCopyWriteBuffer())));
        break;
      case GL_PIXEL_PACK_BUFFER_BINDING:
        InsertResult<GLint>(result, pname, GetObjectID(state.getPixelPackBuffer())));
        break;
      case GL_PIXEL_UNPACK_BUFFER_BINDING:
        InsertResult<GLint>(result, pname, GetObjectID(state.getPixelUnpackuffer())));
        break;
