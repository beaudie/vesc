...
QueryT CastFromSpeicalValue(GLenum pname, const ParamType param)
{
    if (isGLfixed)
    {
        return ConvertFloatToFixed(CastFromStateValue<GLfloat>(pname, param));
    }
    else
    {
                           ...
                           ParamType *params)
{
    ...
    switch (pname)
    {
        ...
        case GL_TEXTURE_MAX_ANISOTROPY_EXT:
            *params =
                CastFromSpeicalValue<isGLfixed, ParamType>(pname, texture->getMaxAnisotropy());
            break;
        ...
        case GL_TEXTURE_MIN_LOD:
            *params = CastFromSpeicalValue<isGLfixed, ParamType>(pname, texture->getMinLod());
            break;
        case GL_TEXTURE_MAX_LOD:
            *params = CastFromSpeicalValue<isGLfixed, ParamType>(pname, texture->getMaxLod());
            break;
        ...
        case GL_TEXTURE_CROP_RECT_OES:
        {
            ...
            params[0]                 = CastFromSpeicalValue<isGLfixed, ParamType>(pname, crop.x);
            params[1]                 = CastFromSpeicalValue<isGLfixed, ParamType>(pname, crop.y);
            params[2] = CastFromSpeicalValue<isGLfixed, ParamType>(pname, crop.width);
            params[3] = CastFromSpeicalValue<isGLfixed, ParamType>(pname, crop.height);
            break;
        }
        case GL_GENERATE_MIPMAP:
            *params = CastFromGLintStateValue<ParamType>(pname, texture->getGenerateMipmapHint());
            break;
        case GL_MEMORY_SIZE_ANGLE:
            *params = CastFromSpeicalValue<isGLfixed, ParamType>(pname, texture->getMemorySize());
            break;
        case GL_TEXTURE_BORDER_COLOR:
            ConvertFromColor<isPureInteger>(texture->getBorderColor(), params);
            break;
        case GL_TEXTURE_NATIVE_ID_ANGLE:
            *params = CastFromSpeicalValue<isGLfixed, ParamType>(pname, texture->getNativeID());
            break;
