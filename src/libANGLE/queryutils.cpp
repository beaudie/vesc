...
GLint QueryProgramInterfaceActiveResources(const Program *program, GLenum programInterface)
{
    switch (programInterface)
    {
        case GL_PROGRAM_INPUT:
            ...
            {
                return clampCast<GLint>(program->getState().getAttributes().size());
            }
            else
            {
                return clampCast<GLint>(program->getState().getInputVaryings().size());
            }
...
template <typename T>
GLint FindMaxNameSize(const std::vector<T> &resources)
{
    GLint max = 0;
...
GLint QueryProgramInterfaceMaxNameLength(const Program *program, GLenum programInterface)
{
    ...
    switch (programInterface)
    {
        case GL_PROGRAM_INPUT:
            if (program->hasLinkedShaderStage(ShaderType::Vertex))
            {
                maxNameLength = FindMaxNameSize(program->getState().getAttributes());
            }
            else
            {
                maxNameLength = FindMaxNameSize(program->getState().getInputVaryings());
            }
            break;
