...
ColorVariant ConvertToColor(const GLfloat *params)
{
    if (isPureInteger)
    {
        UNREACHABLE();
        return ColorVariant(ColorI());
    }
    else
    {
        return ColorVariant(ColorF::fromData(params));
    }
}

template <bool isPureInteger>
ColorVariant ConvertToColor(const GLint *params)
{
    if (isPureInteger)
    {
        return ColorVariant(ColorI(params[0], params[1], params[2], params[3]));
    }
    else
    {
        return ColorVariant(ColorF(normalizedToFloat(params[0]), normalizedToFloat(params[1]),
                                   normalizedToFloat(params[2]), normalizedToFloat(params[3])));
    }
}

template <bool isPureInteger>
ColorVariant ConvertToColor(const GLuint *params)
{
    if (isPureInteger)
    {
        return ColorVariant(ColorUI(params[0], params[1], params[2], params[3]));
    }
    else
    {
        UNREACHABLE();
        return ColorVariant(ColorF());
    }
}

template <bool isPureInteger>
void ConvertFromColor(const ColorVariant &color, GLfloat *outParams)
{
    if (isPureInteger)
    {
        UNREACHABLE();
    ...
    {
        ASSERT(color.type == ColorVariant::Type::Float);
        color.colorF.writeData(outParams);
    }
}

template <bool isPureInteger>
void ConvertFromColor(const ColorVariant &color, GLint *outParams)
{
    if (isPureInteger)
    {
        ASSERT(color.type == ColorVariant::Type::Int);
...
template <bool isPureInteger>
void ConvertFromColor(const ColorVariant &color, GLuint *outParams)
{
    if (isPureInteger)
    {
        ASSERT(color.type == ColorVariant::Type::UInt);
