...
void Context::fogx(GLenum pname, GLfixed param)
{
    ...
    {
        case GL_FOG_MODE:
            fogf(pname, static_cast<GLfloat>(static_cast<GLenum>(param)));
            break;
        case GL_FOG_DENSITY:
        case GL_FOG_START:
        case GL_FOG_END:
...
void Context::fogxv(GLenum pname, const GLfixed *params)
{
    ...
    {
        case GL_FOG_MODE:
            fogf(pname, static_cast<GLfloat>(static_cast<GLenum>(params[0])));
            break;
        case GL_FOG_DENSITY:
        case GL_FOG_START:
        case GL_FOG_END:
            paramf[0] = FixedToFloat(params[0]);
            fogf(pname, paramf[0]);
            break;
        case GL_FOG_COLOR:
            for (int i = 0; i < 4; i++)
            {
                paramf[i] = FixedToFloat(params[i]);
            }
            fogfv(pname, paramf);
            break;
        default:
