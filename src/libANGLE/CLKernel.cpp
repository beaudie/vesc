                                       ...
                                       size_t *valueSizeRet) const
{
    ...
    if (device != nullptr)
    {
        ...
        {
            ANGLE_CL_ERROR(CL_INVALID_DEVICE);
        }
    }
...
Kernel::Kernel(Program &program, const char *name)
    : mProgram(&program),
      mImpl(std::invoke([&]() -> rx::CLKernelImpl::Ptr {
          rx::CLKernelImpl::Ptr implPtr = nullptr;
          return IsError(program.getImpl().createKernel(*this, name, implPtr)) ? nullptr
                                                                               : std::move(implPtr);
      })),
      mInfo(std::invoke([&]() -> rx::CLKernelImpl::Info {
