...
bool ComponentTypeMask::Validate(unsigned long outputTypeBits,
                                 unsigned long inputTypeBits,
                                 unsigned long outputMaskBits,
                                 unsigned long inputMaskBits)
{
    static_assert(IMPLEMENTATION_MAX_DRAW_BUFFERS <= MAX_COMPONENT_TYPE_MASK_INDEX,
                  "Output/input masks should fit into 16 bits - 1 bit per draw buffer. The corresponding type masks should fit into 32 bits - 2 bits per draw buffer.");
    static_assert(MAX_VERTEX_ATTRIBS <= MAX_COMPONENT_TYPE_MASK_INDEX, "Output/input masks should fit into 16 bits - 1 bit per attrib. The corresponding type masks should fit into 32 bits - 2 bits per attrib.");

    // For performance reasons, draw buffer and attribute type validation is done using bit masks. We store two
    ...
    // elswewhere used DrawBufferMask or AttributeMask.

    // OR the masks with themselves, shifted 16 bits. This is to match our split type bits.
    outputMaskBits |= (outputMaskBits << MAX_COMPONENT_TYPE_MASK_INDEX);
    inputMaskBits |= (inputMaskBits << MAX_COMPONENT_TYPE_MASK_INDEX);
