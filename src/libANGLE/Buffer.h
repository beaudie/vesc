...
struct ContentsObserver
{
    ...
    void *observingObject = nullptr;
};

ANGLE_INLINE bool operator==(const ContentsObserver &lhs, const ContentsObserver &rhs)
{
    return lhs.bufferIndex == rhs.bufferIndex && lhs.observingObject == rhs.observingObject;
}
                     ...
                     public angle::Subject
{
  ...
  private:
    ...
    size_t getContentsObserverIndex(void *observingObject, uint32_t bufferIndex) const;
    void removeContentsObserverImpl(void *observingObject, uint32_t bufferIndex);
