...
class SizedMRUCache final : angle::NonCopyable
{
  public:
    ...
    void put(const Key &key, const Value &value, size_t size)
    {
        ...
        mStore.Put(key, ValueAndSize(value, size));
        mCurrentSize += size;
  ...
  private:
    struct ValueAndSize
    {
        ...
        ValueAndSize(const Value &value, size_t size) : value(value), size(size) {}
        ValueAndSize(const ValueAndSize &other) = default;
        ValueAndSize &operator=(const ValueAndSize &other) = default;
