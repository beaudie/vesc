...
class VertexBinding final : private angle::NonCopyable
{
  public:
    ...
    GLuint getBufferId() const { return buffer.id(); }
    Buffer *getBufferRawPointer() const { return buffer.get(); }
    const BindingPointer<Buffer> &getBuffer() const { return buffer; }

    void setBufferRawPointer(Buffer *bufferIn) { buffer.set(bufferIn); }
