...
class State : public OnAttachmentDirtyReceiver, angle::NonCopyable
{
  public:
    ...
    const TexEnv &getTexEnv(unsigned int activeSampler) const
    {
    ...
    {
        return mEnabledClientStates.find(GL_TEXTURE_COORD_ARRAY + unit) !=
    ...
    float *getMultiTexCoord(int unit) { return mMultiTexCoords[unit].data(); }
    ...
    GLenum getAlphaFunc() const { return mAlphaFunc; }
    GLfloat getAlphaTestRef() const { return mAlphaTestRef; }
    ...
    GLfloat getPointSize() const { return mPointSize; }
    ...
    GLenum getLogicOp() const { return mLogicOp; }
    ...
    angle::Mat4 textureMatrix(int i) { return mTextureMatrices[i].back(); }
