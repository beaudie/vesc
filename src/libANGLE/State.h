...
class State : public OnAttachmentDirtyReceiver, angle::NonCopyable
{
  public:
    ...
    void setBufferBinding(const Context *context, BufferType target, Buffer *buffer);
    Buffer *getTargetBuffer(BufferType target) const;
    ...
    bool hasMappedBuffer(BufferType target) const;
  ...
  private:
    ...
    using BoundBufferMap = angle::PackedEnumMap<BufferType, BindingPointer<Buffer>>;
