...
class State : angle::NonCopyable
{
  public:
    ...
    Program *getActiveProgram()
    {
        ...
        if (!program)
        {
            ProgramPipeline *programPipelineObject = getProgramPipeline();
            if (programPipelineObject)
            {
#if 0  // TIMTIM - don't have the context here
                program = programPipelineObject->getLinkedActiveShaderProgram();
#else
