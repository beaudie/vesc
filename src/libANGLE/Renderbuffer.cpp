...
const GLsizei RenderbufferState::getSamples() const
{
    return mSamples;
}
                               ...
                               size_t height)
{
    ...
    mState.mWidth     = static_cast<GLsizei>(width);
    mState.mHeight    = static_cast<GLsizei>(height);
    mState.mFormat    = Format(internalformat);
    mState.mSamples   = 0;
                                          ...
                                          size_t height)
{
    ...
    mState.mHeight    = static_cast<GLsizei>(height);
    mState.mFormat    = Format(internalformat);
    mState.mSamples   = static_cast<GLsizei>(samples);
    mState.mInitState = InitState::MayNeedInit;
...
Error Renderbuffer::setStorageEGLImageTarget(const Context *context, egl::Image *image)
{
    ...
    mState.mHeight    = static_cast<GLsizei>(image->getHeight());
    mState.mFormat    = Format(image->getFormat());
    mState.mSamples   = 0;
    mState.mInitState = image->sourceInitState();
