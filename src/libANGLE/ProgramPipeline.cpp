...
void ProgramPipeline::updateHasBooleans()
{
    ...
    for (const gl::ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        if (shaderProgram)
        {
            ...
            {
                mState.mExecutable->mHasGraphicsUniformBuffers = true;
            }
            if (executable.hasStorageBuffers())
            {
                mState.mExecutable->mHasGraphicsStorageBuffers = true;
            }
            if (executable.hasAtomicCounterBuffers())
            {
                mState.mExecutable->mHasGraphicsAtomicCounterBuffers = true;
            }
            if (executable.hasDefaultUniforms())
            {
                mState.mExecutable->mHasGraphicsDefaultUniforms = true;
            }
        }
    }
    ...
    if (computeProgram)
    {
        ...
        if (executable.hasUniformBuffers())
        {
            mState.mExecutable->mHasComputeUniformBuffers = true;
        }
        if (executable.hasStorageBuffers())
        {
            mState.mExecutable->mHasComputeStorageBuffers = true;
        }
        if (executable.hasAtomicCounterBuffers())
        {
            mState.mExecutable->mHasComputeAtomicCounterBuffers = true;
        }
        if (executable.hasDefaultUniforms())
        {
            mState.mExecutable->mHasComputeDefaultUniforms = true;
        }
    }
}
