...
// GLES specification 3.2, sec 11.1.3.11
// An INVALID_OPERATION error is generated by any command that transfers vertices to the GL or
// launches compute work if the current set of active program objects cannot be executed, for
// reasons including:
...
//    * A program object is active for at least one, but not all of the shader stages that were
//    present when the program was linked.
bool ProgramPipelineState::verifyProgramStagesUsage()
{
    // Check if there is executable code installed.
    if (!mExecutable->getLinkedShaderStages().any())
    {
        return false;
    }

    // Check to make sure each program's shader stages are active. Each program's linked stages must
    // have been used.
    for (const ShaderType shaderType : mExecutable->getLinkedShaderStages())
    {
        Program *program = mPrograms[shaderType];
        ASSERT(program != nullptr);
        if ((program->getExecutable().getLinkedShaderStages() &
             *mProgramUsedStagesMap.query(program->id())) !=
            program->getExecutable().getLinkedShaderStages())
        {
            return false;
        }
    }
                                           ...
                                           angle::ObserverBinding *programObserverBindings)
{
    ...
    if (oldProgram)
    {
        mProgramUsedStagesMap.query(oldProgram->id())->set(shaderType, false);
    ...
    {
        if (!mProgramUsedStagesMap.contains(shaderProgram->id()))
        {
            mProgramUsedStagesMap.assign(shaderProgram->id(), new gl::ShaderBitSet());
        }
        mProgramUsedStagesMap.query(shaderProgram->id())->set(shaderType, true);
...
bool ProgramPipeline::linkVaryings(InfoLog &infoLog) const
{
    ...
    int vertexVersion                                           = kInvalidShaderVersion;
    int fragmentVersion                                         = kInvalidShaderVersion;
...
void ProgramPipeline::validate(const gl::Context *context)
{
    ...
    if (!linkVaryings(infoLog) || !mState.verifyProgramStagesUsage())
    {
        mState.mValid = false;
