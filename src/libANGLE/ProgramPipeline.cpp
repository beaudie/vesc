...
void ProgramPipelineState::destroyDiscardedExecutables(const Context *context)
{
    for (SharedProgramExecutable &executable : mProgramExecutablesToDiscard)
    {
        ...
        // The following reset is necessary to ensure proper clean up if the same executable is
        // found twice in the above list, for example because the same executable was installed for
        // two different shader stages.
        executable.reset();
    }
