...
void ProgramPipelineState::useProgramStage(
    ...
    std::vector<angle::ObserverBinding> *programObserverBindings)
{
    ...
    programObserverBindings->at(index).bind(program);
}
    ...
    std::vector<angle::ObserverBinding> *programObserverBindings)
{
    if (stages == GL_ALL_SHADER_BITS)
    {
        for (const ShaderType shaderType : gl::AllShaderTypes())
        {
            useProgramStage(context, shaderType, shaderProgram, programObserverBindings);
        }
    }
    else
    {
        ...
        {
            useProgramStage(context, ShaderType::Vertex, shaderProgram, programObserverBindings);
        }

        if (stages & GL_FRAGMENT_SHADER_BIT)
        {
            useProgramStage(context, ShaderType::Fragment, shaderProgram, programObserverBindings);
        }

        if (stages & GL_COMPUTE_SHADER_BIT)
        {
            useProgramStage(context, ShaderType::Compute, shaderProgram, programObserverBindings);
        }
    }
}
