    ...
    if (!getExecutable().isCompute())
    {
        ...
        for (ShaderType shaderType : kAllGraphicsShaderTypes)
        {
            ...
            if (program)
            {
                ...
                ANGLE_TRY(program->linkMergedVaryings(
                    context, program->getResources().varyingPacking, mergedVaryings));
            }
        }
    }
...
bool ProgramPipeline::linkVaryings(InfoLog &infoLog) const
{
    ...
    return fragmentProgram->linkValidateFragmentInputBindings(infoLog);
}
