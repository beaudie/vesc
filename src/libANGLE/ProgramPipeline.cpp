...
{
    // Check if there's executable code installed
    if (!mExecutable->getLinkedShaderStages().any())
    {
        return false;
    }

    // Check to make sure each program's shader stages are active
    for (const ShaderType shaderType : mExecutable->getLinkedShaderStages())
    {
        Program *program = mPrograms[shaderType];
        ASSERT(program != nullptr);
        for (const ShaderType usedShaderType : program->getExecutable().getLinkedShaderStages())
        {
            if (!mProgramUsedStagesMap[program->id().value][usedShaderType])
            {
                return false;
            }
        }
                                           ...
                                           angle::ObserverBinding *programObserverBindings)
{
    ...
    {
        if (mProgramUsedStagesMap.count(shaderProgram->id().value) == 0)
        {
