...
bool ProgramPipelineState::verifyProgramStagesUsage()
{
    ...
    for (const ShaderType shaderType : mExecutable->getLinkedShaderStages())
    {
        ...
        if ((linkedShaderStages & mProgramUsedStagesMap[program->id().value]) != linkedShaderStages)
        {
            return false;
        }
    }
                                           ...
                                           angle::ObserverBinding *programObserverBindings)
{
    ...
    if (oldProgram)
    {
        mProgramUsedStagesMap[oldProgram->id().value].set(shaderType, false);
    ...
    {
        if (!mProgramUsedStagesMap.count(shaderProgram->id().value))
        {
            mProgramUsedStagesMap[shaderProgram->id().value] = gl::ShaderBitSet();
        }
        mProgramUsedStagesMap[shaderProgram->id().value].set(shaderType, true);
