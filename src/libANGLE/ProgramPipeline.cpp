...
angle::Result ProgramPipeline::link(const Context *context)
{
    ...
    if (!getExecutable().isCompute())
    {
        ...
        // Store temporary variables necessary for linking.
        linkingVariables.savePendingLinkInfo(mState, getExecutable());
        if (linkingVariables.mIsShaderStageUsedBitset[ShaderType::Vertex])
        {
