...
void ProcessCleanupCallback(void *ptr)
{
    ...
    if (gActiveThreadCount == 0)
    {
        ...
        for (egl::Display *display : displays)
        {
            ...
            {
                (void)display->destroyInvalidEglObjects(thread);
            }
        }
    }
...
void Thread::onDisplayFullyTerminated(Display *display) const
{
    // Display was fully terminated, reset active thread count to 0
    angle::gActiveThreadCount = 0;
}
