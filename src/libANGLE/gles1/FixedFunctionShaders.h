...
vec4 getPointTexture(int unit) {
    vec4 res;

    switch (unit) {
    case 0:
        if (enable_texture_2d[0]) {
            res = texture(tex_sampler0, gl_PointCoord);
        }
        break;
    case 1:
        if (enable_texture_2d[1]) {
            res = texture(tex_sampler1, gl_PointCoord);
        }
        break;
    case 2:
        if (enable_texture_2d[2]) {
            res = texture(tex_sampler2, gl_PointCoord);
        }
        break;
    case 3:
        if (enable_texture_2d[3]) {
            res = texture(tex_sampler3, gl_PointCoord);
        }
        break;
    default:
        break;
...
void main() {
    ...
    if (point_rasterization && point_sprite_enabled) {
        for (int i = 0; i < kMaxTexUnits; i++) {
            vec4 pointTexture = texture_env_point_sprite_coord_replace[i] ?
                                getPointTexture(i) : getTextureColor(i);
            currentFragment =
                textureFunction(i,
                        texture_format[i],
                        texture_env_mode[i],
