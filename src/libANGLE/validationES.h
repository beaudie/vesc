                                   ...
                                   PrimitiveMode mode)
{
    ...
    if (drawStatesError)
    {
        ...
        ANGLE_VALIDATION_ERROR(context, entryPoint, errorCode, errorMessage);
        return false;
    }
                                       ...
                                       VertexAttribTypeCase validation)
{
    ...
    switch (validation)
    {
        case VertexAttribTypeCase::Invalid:
            ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_ENUM, err::kInvalidType);
            return false;
        case VertexAttribTypeCase::Valid:
            if (size < 1 || size > 4)
            {
                ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_VALUE,
                                       err::kInvalidVertexAttrSize);
                return false;
            }
            break;
                                              ...
                                              const Framebuffer *framebuffer)
{
    ...
    if (!framebufferStatus.isComplete())
    {
        ...
        ANGLE_VALIDATION_ERROR(context, entryPoint, ErrorCode, framebufferStatus.reason);
        return false;
    }
                                            ...
                                            GLsizei count)
{
    ...
    {
        ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_OPERATION, err::kIntegerOverflow);
        return false;
    }
                                           ...
                                           GLsizei primcount)
{
    if (first < 0)
    {
        ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_VALUE, err::kNegativeStart);
        return false;
    }
    ...
    if (count <= 0)
    {
        if (count < 0)
        {
            ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_VALUE, err::kNegativeCount);
            return false;
        }
                                           ...
                                           DrawElementsType type)
{
    if (!context->getStateCache().isValidDrawElementsType(type))
    {
        if (type == DrawElementsType::UnsignedInt)
        {
            ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_ENUM,
                                   err::kTypeNotUnsignedShortByte);
            return false;
        }

        ASSERT(type == DrawElementsType::InvalidEnum);
        ANGLE_VALIDATION_ERRORF(context, entryPoint, GL_INVALID_ENUM, err::kEnumInvalid);
        return false;
    }
    ...
    if (drawElementsError)
    {
        ...
        ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_OPERATION, errorMessage);
        return false;
    }
                                             ...
                                             GLsizei primcount)
{
    ...
    if (context->isWebGL())
    {
        ...
        {
            ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_VALUE, err::kNegativeOffset);
            return false;
        }
    }
                                              ...
                                              VertexArrayID array)
{
    if (!context->isVertexArrayGenerated(array))
    {
        ...
        ANGLE_VALIDATION_ERROR(context, entryPoint, GL_INVALID_OPERATION, err::kInvalidVertexArray);
        return false;
    }
