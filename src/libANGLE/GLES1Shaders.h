...
bool isUnitTextureEnabled(int unit) {
...
vec4 textureCombine(
    ...
    if (combineRgb == kInterpolate) {
        arg2Rgb = textureCombineSrcnOpnRgb(src2Alpha, op2Alpha, textureEnvColor, vertexColor, texturePrevColor, textureColor);
    }
...
void main() {
    ...
    for (int i = 0; i < kMaxTexUnits; i++) {
        if (isUnitTextureEnabled(i)) {
            currentFragment =
                textureFunction(texture_format[i],
                                texture_env_mode[i],
