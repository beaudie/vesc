...
void main() {
    ...
    vec4 texturePrevColor = vertex_color;

    if (enable_texture_2d[0] ||
        enable_texture_cube_map[0]) {
        vec4 textureColor;
        if (enable_reflection_map) {
            textureColor = texture(tex_cube_sampler0, reflect(pos_varying.xyz, normalize(normal_varying)));
            currentFragment = textureColor;
        } else {

            if (enable_texture_2d[0]) {
                textureColor = texture(tex_sampler0, texcoord0_varying.xy);
            } else {
                textureColor = texture(tex_cube_sampler0, texcoord0_varying.xyz);
            }

            switch (texture_env_mode[0]) {
            case kReplace:
                switch (texture_format[0]) {
                case kAlpha:
                    currentFragment.rgb = texturePrevColor.rgb;
                ...
                default:
                    ...
                    currentFragment.a = texturePrevColor.a;
                    break;
                }
                break;
            case kModulate:
                ...
                case kAlpha:
                    currentFragment.rgb = texturePrevColor.rgb;
                    currentFragment.a = texturePrevColor.a * textureColor.a;
                    break;
                case kRGBA:
                case kLuminanceAlpha:
                    currentFragment.rgba = texturePrevColor.rgba * textureColor.rgba;
                    break;
                case kRGB:
                case kLuminance:
                default:
                    currentFragment.rgb = texturePrevColor.rgb * textureColor.rgb;
                    currentFragment.a = texturePrevColor.a;
                    break;
                }
                break;
            case kDecal:
                ...
                case kRGB:
                    ...
                    break;
                case kRGBA:
                    currentFragment.rgb = texturePrevColor.rgb * (1.0 - textureColor.a) + textureColor.rgb * textureColor.a;
                    currentFragment.a = texturePrevColor.a;
                    break;
                case kAlpha:
                case kLuminance:
            ...
            case kBlend:
                ...
                case kAlpha:
                    currentFragment.rgb = texturePrevColor.rgb;
                    currentFragment.a = textureColor.a * texturePrevColor.a;
                    break;
                case kLuminance:
                case kRGB:
                    currentFragment.rgb = texturePrevColor.rgb * (1.0 - textureColor.rgb) + texture_env_color[0].rgb * textureColor.rgb;
                    currentFragment.a = texturePrevColor.a;
                    break;
                case kLuminanceAlpha:
                case kRGBA:
