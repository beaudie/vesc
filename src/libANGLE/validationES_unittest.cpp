...
TEST(ValidationESTest, DrawElementsWithMaxIndexGivesError)
{
    ...
    EXPECT_CALL(*framebufferImpl, checkStatus()).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE));
    ...
    // Call with excessive index.
    GLuint indexData[] = {0, 1, 2, 3, 4, static_cast<GLuint>(caps.maxElementIndex)};
