                      ...
                      GLuint index)
{
    ...
    if (numPLSPlanes != 0)
    {
        // INVALID_OPERATION is generated by Enablei*(), Disablei*() if <cap> is not BLEND.
        if (target != GL_BLEND)
        {
            context->validationErrorF(entryPoint, GL_INVALID_OPERATION, kPLSCapNotAllowed, target);
            return false;
        }
