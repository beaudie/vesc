...
bool ValidateMatrix(Context* context, GLenum matrixMode, const GLfloat* matrix)
{
    ...
    if (matrixMode != GL_PATH_MODELVIEW_CHROMIUM &&
        matrixMode != GL_PATH_PROJECTION_CHROMIUM)
    {
...
bool ValidateGenPaths(Context* context, GLsizei range)
{
    ...
    if (!rx::IsIntegerCastSafe<std::uint32_t>(range))
    {
        context->handleError(Error(GL_INVALID_OPERATION));
        return false;
    }
...
bool ValidateDeletePaths(Context* context, GLuint path, GLsizei range)
{
    ...
    if (!rx::IsIntegerCastSafe<std::uint32_t>(range) ||
        !rx::IsUnsignedAdditionSafe<std::uint32_t>(path, range))
    {
        context->handleError(Error(GL_INVALID_OPERATION));
        return false;
    }
...
bool ValidatePathCommands(Context* context, GLuint path,
                          GLsizei numCommands,
                          ...
                          const void *coords)
{
    if (!context->getExtensions().pathRendering)
    {
        context->handleError(Error(GL_INVALID_OPERATION, "GL_CHROMIUM_path_rendering is not available."));
        return false;
    }
    ...
    if (!rx::IsUnsignedAdditionSafe<std::uint32_t>(numCommands, coordTypeSize * numCoords))
    {
        context->handleError(Error(GL_INVALID_OPERATION));
        return false;
    }
...
bool ValidateSetPathParameter(Context* context, GLuint path, GLenum pname, GLfloat value, GLfloat *actual)
{
    if (!context->getExtensions().pathRendering)
    {
        context->handleError(Error(GL_INVALID_OPERATION, "GL_CHROMIUM_path_rendering is not available."));
