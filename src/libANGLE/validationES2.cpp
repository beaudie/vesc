...
static bool ValidateObjectIdentifierAndName(Context *context, GLenum identifier, GLuint name)
{
    switch (identifier)
    {
        ...
        case GL_PROGRAM:
            if (context->getMaybeLinkedProgram(name) == nullptr)
            {
                                               ...
                                               const GLchar *name)
{
    ...
    const auto *programObject = context->getMaybeLinkedProgram(program);
    if (!programObject)
    {
                                                 ...
                                                 const GLfloat *coeffs)
{
    ...
    const auto *programObject = context->getLinkedProgram(program);
...
bool ValidateDeleteProgram(Context *context, GLuint program)
{
    ...
    if (!context->getLinkedProgram(program))
    {
...
bool ValidateDeleteShader(Context *context, GLuint shader)
{
    ...
    {
        if (context->getLinkedProgram(shader))
        {
...
bool ValidateUseProgram(Context *context, GLuint program)
{
    if (program != 0)
    {
        Program *programObject = context->getLinkedProgram(program);
        if (!programObject)
        {
