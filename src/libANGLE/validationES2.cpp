...
bool IsValidCopyTextureFormat(Context *context, GLenum internalFormat)
{
    ...
    switch (internalFormatInfo.format)
    {
        ...
        case GL_RED:
            return context->getClientVersion() >= 3 || context->getExtensions().textureRG;
...
bool IsValidCopyTextureDestinationTarget(Context *context, GLenum target)
{
    switch (target)
    {
        ...
        // TODO: accept GL_TEXTURE_RECTANGLE_ARB if the texture_rectangle extension is supported
...
bool IsValidCopyTextureSourceTarget(Context *context, GLenum target)
{
    ...
    // TODO: accept GL_TEXTURE_EXTERNAL_OES if the texture_external extension is supported
                                 ...
                                 GLboolean unpackUnmultiplyAlpha)
{
    ...
    {
        context->recordError(Error(GL_INVALID_VALUE, "Source texture a valid texture type."));
        return false;
    }
    ...
    GLenum sourceInternalFormat = source->getInternalFormat(sourceTarget, 0);
    if (!IsValidCopyTextureFormat(context, sourceInternalFormat))
    {
        context->recordError(
    ...
    {
        context->recordError(Error(GL_INVALID_VALUE, "Destination texture a valid texture type."));
        return false;
    }
    ...
    {
        context->recordError(Error(GL_INVALID_OPERATION, "Destination texture is immutable."));
        return false;
    }
                                    ...
                                    GLboolean unpackUnmultiplyAlpha)
{
    ...
    if (x < 0 || y < 0)
    {
        context->recordError(Error(GL_INVALID_VALUE, "x and y cannot be negative."));
        return false;
    }

    if (x + width > source->getWidth(sourceTarget, 0) ||
        y + height > source->getHeight(sourceTarget, 0))
    {
        context->recordError(
    ...
    GLenum destInternalFormat            = dest->getInternalFormat(destTarget, 0);
    const InternalFormat &destFormatInfo = GetInternalFormatInfo(destInternalFormat);
    if (!IsValidCopyTextureDestinationFormatType(context, destFormatInfo.format,
                                                 destFormatInfo.type))
    {
        context->recordError(
            Error(GL_INVALID_OPERATION,
    ...
    {
        context->recordError(Error(GL_INVALID_VALUE, "xoffset and yoffset cannot be negative."));
        return false;
    }

    if (xoffset + width > dest->getWidth(destTarget, 0) ||
        yoffset + height > dest->getHeight(destTarget, 0))
    {
        context->recordError(
...
bool ValidateCompressedCopyTextureCHROMIUM(Context *context, GLuint sourceId, GLuint destId)
{
    ...
    {
        context->recordError(Error(GL_INVALID_VALUE, "Source texture must level 0 defined."));
        return false;
    }
    ...
    if (dest->getImmutableFormat())
    {
        context->recordError(Error(GL_INVALID_OPERATION, "Destination cannot be immutable."));
        return false;
    }
