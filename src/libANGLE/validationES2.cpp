                                   ...
                                   const void *pixels)
{
    ...
    if (isSubImage)
    {
        ...
        if (width > 0 && height > 0 && pixels == nullptr &&
            context->getGLState().getTargetBuffer(gl::BufferType::PixelUnpack) == nullptr)
        {
...
bool ValidateGetBufferPointervOES(Context *context, BufferType target, GLenum pname, void **params)
{
    return ValidateGetBufferPointervBase(context, target, pname, nullptr, params);
}

bool ValidateMapBufferOES(Context *context, BufferType target, GLenum access)
{
...
bool ValidateUnmapBufferOES(Context *context, BufferType target)
{
    if (!context->getExtensions().mapBuffer)
    {
        context->handleError(InvalidOperation() << "Map buffer extension not available.");
...
bool ValidateMapBufferBase(Context *context, BufferType target)
{
    Buffer *buffer = context->getGLState().getTargetBuffer(target);
...
bool ValidateBindBuffer(ValidationContext *context, BufferType target, GLuint buffer)
{
    if (!ValidBufferType(context, target))
    {
        ANGLE_VALIDATION_ERR(context, InvalidEnum(), InvalidBufferTypes);
                                 ...
                                 const void *ptr)
{
    ...
    if (!nullBufferAllowed && context->getGLState().getTargetBuffer(BufferType::Array) == 0 &&
        ptr != nullptr)
    {
        context
            ->handleError(InvalidOperation()
