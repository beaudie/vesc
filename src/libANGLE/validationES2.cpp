...
bool ValidBlendFuncExtended(const Context *context, GLenum val)
{
    // these are always valid for src and dst.
    switch (val)
    {
...
bool ValidateSrcBlendFunc(const Context *context, angle::EntryPoint entryPoint, GLenum val)
{
    ...
    // INVALID_OPERATION is generated by BlendFunci*() and BlendFuncSeparatei*() if <srcRGB>,
    // <dstRGB>, <srcAlpha>, or <dstAlpha> is a blend function requiring the secondary color
    // input, as specified in EXT_blend_func_extended.
    if (ValidBlendFuncExtended(context, val))
    {
