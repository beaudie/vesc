                                     ...
                                     const GLfloat *transformValues)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorIntegerOverflow);
        return false;
    }
                                       ...
                                       GLint border)
{
    ...
    {
        context->handleError(InvalidEnum() << g_kErrorTextureTarget);
        return false;
    }
    ...
    if (isSubImage)
    {
        switch (formatInfo.format)
        {
            case GL_ALPHA:
                ...
                {
                    context->handleError(InvalidOperation() << g_kErrorInvalidFormat);
                    return false;
                }
                break;
            case GL_LUMINANCE:
    ...
    else
    {
        switch (internalformat)
        {
            ...
            case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
                ...
                {
                    context->handleError(InvalidEnum() << g_kErrorEnumNotSupported);
                    return false;
                }
                break;
            case GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE:
                                   ...
                                   const void *pixels)
{
    ...
    {
        context->handleError(InvalidValue() << g_kErrorInvalidMipLevel);
        return false;
    }
    ...
    {
        context->handleError(InvalidValue() << g_kErrorResourceMaxTextureSize);
        return false;
    }
    ...
    else if (IsCubeMapTextureTarget(target))
    {
        ...
        {
            context->handleError(InvalidValue() << g_kErrorCubemapFacesEqualDimensions);
            return false;
        }
    ...
    if (!texture)
    {
        context->handleError(InvalidOperation() << g_kErrorBufferNotBound);
        return false;
    }
    ...
    if (isSubImage)
    {
        ...
        if (format != GL_NONE)
        {
            ...
            {
                context->handleError(InvalidOperation() << g_kErrorTypeMustMatchOriginalType);
                return false;
            }
        }
    ...
    if (border != 0)
    {
        context->handleError(InvalidValue() << g_kErrorInvalidBorder);
        return false;
    }
    ...
    if (isCompressed)
    {
        ...
        switch (actualInternalFormat)
        {
            ...
            case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
                if (!context->getExtensions().textureCompressionDXT1)
                {
                    context->handleError(InvalidEnum() << g_kErrorInvalidInternalFormat);
                    return false;
                }
                break;
            case GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE:
    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case GL_FLOAT:
                break;
            default:
                context->handleError(InvalidEnum() << g_kErrorInvalidType);
                return false;
        }
            ...
            case GL_LUMINANCE_ALPHA:
                switch (type)
                {
                    ...
                    case GL_HALF_FLOAT_OES:
                        break;
                    default:
                        context->handleError(InvalidOperation() << g_kErrorMismatchedTypeAndFormat);
                        return false;
                }
                break;
            case GL_RED:
        ...
        if (!isSubImage)
        {
            switch (internalformat)
            {
                case GL_RGBA32F:
                    ...
                    {
                        context->handleError(InvalidOperation()
                                             << g_kErrorMismatchedTypeAndFormat);
                        return false;
                    }
                    if (format != GL_RGBA)
                    {
                                   ...
                                   const GLenum *attachments)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorExtensionNotEnabled);
        return false;
    }
    ...
    switch (target)
    {
        case GL_FRAMEBUFFER:
            ...
            break;
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidFramebufferTarget);
            return false;
    }
                                   ...
                                   GLchar *messageLog)
{
    ...
    {
        context->handleError(InvalidValue()
                             << g_kErrorInsufficientBufferSize);
        return false;
    }
                               ...
                               GLchar *label)
{
    ...
    if (bufSize < 0)
    {
        context->handleError(InvalidValue() << g_kErrorInsufficientBufferSize);
        return false;
    }
...
bool ValidateGetPointervKHR(Context *context, GLenum pname, void **params)
{
    ...
    switch (pname)
    {
        ...
        default:
            context->handleError(InvalidEnum() << g_kErrorUnknownPname);
            return false;
    }
...
bool ValidateClear(ValidationContext *context, GLbitfield mask)
{
    ...
    {
        context->handleError(InvalidValue() << g_kErrorClearInvalidMask);
        return false;
    }
...
bool ValidateMapBufferOES(Context *context, GLenum target, GLenum access)
{
    ...
    {
        context->handleError(InvalidEnum() << g_kErrorInvalidBufferTypes);
        return false;
    }
...
bool ValidateBindTexture(Context *context, GLenum target, GLuint texture)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorSubtextureFormatType);
        return false;
    }
                                         ...
                                         const GLchar *name)
{
    ...
    {
        context->handleError(InvalidValue() << g_kErrorNameInvalidCharacters);
        return false;
    }

    if (strncmp(name, "gl_", 3) == 0)
    {
        context->handleError(InvalidOperation() << g_kErrorNameBeginsWithGL);
        return false;
    }
...
bool ValidatePathStencilFunc(Context *context, GLenum func, GLint ref, GLuint mask)
{
    ...
    switch (func)
    {
        ...
        case GL_NOTEQUAL:
            break;
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidStencil);
            return false;
    }
                                       ...
                                       const GLchar *name)
{
    ...
    if (!programObject)
    {
        context->handleError(InvalidOperation() << g_kErrorInvalidProgramName);
        return false;
    }
                                         ...
                                         const GLfloat *coeffs)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorProgramNotLinked);
        return false;
    }
                                 ...
                                 GLboolean unpackUnmultiplyAlpha)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorInvalidInternalFormat);
        return false;
    }
                                    ...
                                    GLboolean unpackUnmultiplyAlpha)
{
    ...
    if (width < 0 || height < 0)
    {
        context->handleError(InvalidValue() << g_kErrorNegativeSize);
        return false;
    }
    ...
    {
        context->handleError(InvalidValue() << g_kErrorSubtextureDimensions);
        return false;
    }
    ...
    {
        context->handleError(InvalidValue() << g_kErrorNegativeOffset);
        return false;
    }
...
bool ValidateCreateShader(Context *context, GLenum type)
{
    switch (type)
    {
        ...
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidShaderName);
            return false;
    }
                        ...
                        GLenum usage)
{
    ...
    switch (usage)
    {
        ...
        case GL_DYNAMIC_COPY:
            if (context->getClientMajorVersion() < 3)
            {
                context->handleError(InvalidEnum() << g_kErrorInvalidBufferUsage);
                return false;
            }
            break;
    ...
    if (!buffer)
    {
        context->handleError(InvalidOperation() << g_kErrorBufferDataNull);
        return false;
    }
                           ...
                           const void *data)
{
    ...
    if (!buffer)
    {
        context->handleError(InvalidOperation() << g_kErrorBufferSubDataNull);
        return false;
    }
    ...
    {
        context->handleError(InvalidValue() << g_kErrorParamOverflow);
        return false;
    }
...
bool ValidateAttachShader(ValidationContext *context, GLuint program, GLuint shader)
{
    ...
    switch (shaderObject->getType())
    {
        case GL_VERTEX_SHADER:
        {
            ...
            {
                context->handleError(InvalidOperation() << g_kErrorShaderAttachmentHasShader);
                return false;
            }
            break;
        }
                                ...
                                const GLchar *name)
{
    ...
    {
        context->handleError(InvalidValue() << g_kErrorIndexExceedsMax);
        return false;
    }
...
static bool ValidateWebGLNamePrefix(ValidationContext *context, const GLchar *name)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorWebglBindAttribLocationReservedPrefix);
        return false;
    }
...
static bool ValidateWebGLNameLength(ValidationContext *context, const GLchar *name)
{
    ...
    if (strlen(name) > 256)
    {
        context->handleError(InvalidValue() << g_kErrorWebglNameLengthLimitExceeded);
        return false;
    }
...
bool ValidateBindBuffer(ValidationContext *context, GLenum target, GLuint buffer)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorObjectNotGenerated);
        return false;
    }
...
bool ValidateBindRenderbuffer(ValidationContext *context, GLenum target, GLuint renderbuffer)
{
    ...
    {
        context->handleError(InvalidEnum() << g_kErrorInvalidRenderbufferTarget);
        return false;
    }
...
bool ValidateBlendEquation(ValidationContext *context, GLenum mode)
{
    ...
    {
        context->handleError(InvalidEnum() << g_kErrorInvalidBlendEquation);
        return false;
    }
                               ...
                               GLenum dstAlpha)
{
    ...
    {
        context->handleError(InvalidEnum() << g_kErrorInvalidBlendFunction);
        return false;
    }
        ...
        context->getExtensions().webglCompatibility)
    {
        ...
        if (constantColorUsed && constantAlphaUsed)
        {
            ...
            if (context->getExtensions().webglCompatibility)
            {
                msg = g_kErrorInvalidConstantColor;
            }
            else
            {
                msg =
                                 ...
                                 const void *ptr)
{
    ...
    if (stride < 0)
    {
        context->handleError(InvalidValue() << g_kErrorNegativeStride);
        return false;
    }
...
bool ValidateDepthRangef(ValidationContext *context, GLfloat zNear, GLfloat zFar)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorDepthRange);
        return false;
    }
...
bool ValidateCullFace(ValidationContext *context, GLenum mode)
{
    switch (mode)
    {
        ...
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidCullMode);
            return false;
    }
...
bool ValidateDeleteProgram(ValidationContext *context, GLuint program)
{
    ...
    if (!context->getProgram(program))
    {
        ...
        {
            context->handleError(InvalidValue() << g_kErrorInvalidProgramName);
            return false;
        }
    }
...
bool ValidateDeleteShader(ValidationContext *context, GLuint shader)
{
    ...
    if (!context->getShader(shader))
    {
        ...
        {
            context->handleError(InvalidOperation() << g_kErrorInvalidShaderName);
            return false;
        }
        else
        {
            context->handleError(InvalidValue() << g_kErrorInvalidShaderName);
            return false;
        }
    }
...
bool ValidateDetachShader(ValidationContext *context, GLuint program, GLuint shader)
{
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorShaderToDetachMustBeAttached);
        return false;
    }
...
bool ValidateGetAttribLocation(ValidationContext *context, GLuint program, const GLchar *name)
{
    ...
    if (context->getExtensions().webglCompatibility && !ValidateWebGLNameLength(context, name))
    {
        return false;
    }
                                      ...
                                      GLint *precision)
{
    switch (shadertype)
    {
        ...
        case GL_COMPUTE_SHADER:
            ...
            return false;
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidShaderType);
            return false;
    }
    ...
    switch (precisiontype)
    {
        ...
        default:
            context->handleError(InvalidEnum() << g_kErrorInvalidPrecision);
            return false;
    }
...
bool ValidateGetUniformLocation(ValidationContext *context, GLuint program, const GLchar *name)
{
    ...
    if (context->getExtensions().webglCompatibility)
    {
        // The WebGL spec (section 6.20) disallows strings containing invalid ESSL characters for
        // shader-related entry points
        if (!IsValidESSLString(name, strlen(name)))
        {
            context->handleError(InvalidValue() << g_kErrorNameInvalidCharacters);
            return false;
        }
        ...
        // WebGL 1.0 [Section 6.21] Maxmimum Uniform and Attribute Location Lengths
        // WebGL imposes a limit of 256 characters on the lengths of uniform and attribute locations. 
        if (!ValidateWebGLNameLength(context, name))
        {
            return false;
        }
    }
...
bool ValidatePixelStorei(ValidationContext *context, GLenum pname, GLint param)
{
    ...
    switch (pname)
    {
        case GL_UNPACK_ALIGNMENT:
            ...
            {
                context->handleError(InvalidValue() << g_kErrorInvalidUnpackAlignment);
                return false;
            }
            break;
                          ...
                          const GLint *length)
{
    if (count < 0)
    {
        context->handleError(InvalidValue() << g_kErrorNegativeCount);
        return false;
    }
    ...
    if (context->getExtensions().webglCompatibility)
    {
        for (GLsizei i = 0; i < count; i++)
        {
            ...
            {
                context->handleError(InvalidValue() << g_kErrorShaderSourceInvalidCharacters);
                return false;
            }
        }
    }
...
bool ValidateViewport(ValidationContext *context, GLint x, GLint y, GLsizei width, GLsizei height)
{
    if (width < 0 || height < 0)
    {
        context->handleError(InvalidValue() << g_kErrorViewportNegativeSize);
        return false;
    }
                                  ...
                                  GLint level)
{
    ...
    if (context->getClientMajorVersion() < 3 && !context->getExtensions().fboRenderMipmap &&
        level != 0)
    {
        context->handleError(InvalidValue() << g_kErrorInvalidFramebufferTextureLevel);
        return false;
    }
    ...
    if (texture != 0)
    {
        ...
        switch (textarget)
        {
            case GL_TEXTURE_2D:
            {
                ...
                {
                    context->handleError(InvalidOperation() << g_kErrorTextureTarget);
                    return false;
                }
            }
            break;
...
bool ValidateGenerateMipmap(Context *context, GLenum target)
{
    ...
    if (texture == nullptr)
    {
        context->handleError(InvalidOperation() << g_kErrorTextureNotBound);
        return false;
    }
    ...
    if (format.info->compressed)
    {
        context->handleError(InvalidOperation() << g_kErrorGenerateMipmapNotAllowed);
        return false;
    }
         ...
         !isPow2(static_cast<int>(texture->getHeight(baseTarget, 0)))))
    {
        ...
        context->handleError(InvalidOperation() << g_kErrorTextureNotPow2);
        return false;
    }
    ...
    {
        context->handleError(InvalidOperation() << g_kErrorCubemapComplete);
        return false;
    }
