...
bool ValidCap(const Context *context, GLenum cap, bool queryOnly)
{
    switch (cap)
    {
        ...
        case GL_TEXTURE_COORD_ARRAY:
            return queryOnly && context->getClientMajorVersion() < 2;
        default:
            return false;
    }
}
...
{
    unsigned int maxUnits = context->getCaps().maxCombinedTextureImageUnits;

    if (context->getClientVersion() < ES_2_0) {
        maxUnits = context->getCaps().maxMultitextureUnits;
    }

    if (texture < GL_TEXTURE0 ||
        texture > GL_TEXTURE0 + maxUnits - 1)
    {
        context->handleError(InvalidEnum());
        return false;
    }
