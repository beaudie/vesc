                           ...
                           GLuint pathBase)
{
    ...
    for (GLsizei i = 0; i < numPaths; ++i)
    {
        ...
        if (context->hasPath(pathName) && !context->isPath(pathName))
        {
...
bool ValidateGetPointervRobustANGLERobustANGLE(Context *context, GLenum pname,
    GLsizei bufSize,
    GLsizei *length,
    void **params)
{
...
bool ValidateStencilFillPathCHROMIUM(Context *context, GLuint path, GLenum fillMode, GLuint mask)
{
    ...
    if (context->hasPath(path) && !context->isPath(path))
    {
        ANGLE_VALIDATION_ERR(context, InvalidOperation(), NoSuchPath);
        return false;
    }
