                                                   ...
                                                   ShaderMap<const ProgramState *> &programStates)
{
    ...
    for (ShaderType backShaderType : kAllGraphicsShaderTypes)
    {
        ...
        bool hasLinkedShaderStage = programStates[backShaderType] != nullptr;

        if (!backShader && !hasLinkedShaderStage)
        {
            continue;
        }
        ...
        const std::vector<sh::ShaderVariable> &backShaderOutputVaryings =
            backShader ? backShader->getOutputVaryings()
                       : programStates[backShaderType]->getExecutable().getLinkedOutputVaryings(
                             backShaderType);
        const std::vector<sh::ShaderVariable> &backShaderInputVaryings =
            backShader ? backShader->getInputVaryings()
                       : programStates[backShaderType]->getExecutable().getLinkedInputVaryings(
                             backShaderType);
