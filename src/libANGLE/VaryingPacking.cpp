                                                ...
                                                const bool isSeparableProgram)
{
    ...
    for (const ProgramVaryingRef &ref : mergedVaryings)
    {
        ...
        // When separable programs are linked, all varyings are treated as active. See section 7.4.1
        // in https://www.khronos.org/registry/OpenGL/specs/es/3.2/es_spec_3.2.pdf
        if ((input && output && output->staticUse) || isActiveBuiltInInput ||
            isActiveBuiltInOutput || isSeparableProgram)
        {
