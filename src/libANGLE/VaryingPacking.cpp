...
// Implementation of PackedVaryingSingleStage
PackedVaryingSingleStage::PackedVaryingSingleStage(ShaderType stageIn,
                                                   const sh::ShaderVariable *varyingIn)
    : varying(varyingIn), stage(stageIn)
{}

PackedVaryingSingleStage::~PackedVaryingSingleStage() = default;

PackedVaryingSingleStage::PackedVaryingSingleStage(PackedVaryingSingleStage &&other)
{
    *this = std::move(other);
}

PackedVaryingSingleStage &PackedVaryingSingleStage::operator=(PackedVaryingSingleStage &&other)
{
...
PackedVarying::PackedVarying(PackedVaryingSingleStage &&frontVaryingIn,
...
{
    ShaderMap<std::set<std::string>> uniqueFullNames;
    ...
    for (const ProgramVaryingRef &ref : mergedVaryings)
    {
            ...
            (output && output->isBuiltIn() && output->active))
        {
                  ...
                  mPackMode == PackMode::ANGLE_NON_CONFORMANT_D3D9))
            {
                ...
                auto interpolation = input ? input->interpolation : output->interpolation;
                ...
                if (varying->isStruct())
                {
                    ...
                    {
                        const sh::ShaderVariable *frontField =
                            input ? &input->fields[fieldIndex] : nullptr;
                        const sh::ShaderVariable *backField =
                            output ? &output->fields[fieldIndex] : nullptr;

                        PackedVaryingSingleStage frontVarying(ref.frontShaderStage, frontField);
                        PackedVaryingSingleStage backVarying(ref.backShaderStage, backField);
                        ...
                        mPackedVaryings.emplace_back(std::move(frontVarying),
                                                     std::move(backVarying), interpolation,
                                                     fieldIndex);
                ...
                {
                    PackedVaryingSingleStage frontVarying(ref.frontShaderStage, input);
                    PackedVaryingSingleStage backVarying(ref.backShaderStage, output);
        ...
        for (const std::string &tfVarying : tfVaryings)
        {
            ...
            if (input->isStruct())
            {
                ...
                if (field != nullptr)
                {
                    ...
                    PackedVaryingSingleStage frontVarying(ref.frontShaderStage, field);
                    PackedVaryingSingleStage backVarying(ref.backShaderStage, nullptr);
