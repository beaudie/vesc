    ...
    const std::vector<std::string> &tfVaryings) const
{
    ...
    for (const auto &ref : mergedVaryings)
    {
        ...
        if (((input && output) || (output && output->isBuiltIn())) && output->staticUse)
        {
            ...
            if (output->isStruct())
            {
                ...
                for (const auto &field : output->fields)
                {
                    ...
                    uniqueFullNames.insert(packedVaryings.back().nameWithArrayIndex());
                }
            }
            else
            {
        ...
        for (const std::string &tfVarying : tfVaryings)
        {
            ...
            // already specified by fragment shader.
            if ((uniqueFullNames.count(tfVarying) > 0) || (uniqueFullNames.count(baseName) > 0))
            {
                continue;
            }
            // linkValdateTransformFeedback has already filtered out array and array elment
            // conflict.
