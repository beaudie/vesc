                                                ...
                                                const bool isSeparableProgram)
{
    ...
    for (const ProgramVaryingRef &ref : mergedVaryings)
    {
        ...
        bool validInput                 = (input && (input->active || !output));
        bool validOutput                = (output && (output->active || !input));
        bool separableProgramConditions = (isSeparableProgram && (validInput || validOutput));

        if (matchedInputOutputStaticUse || activeBuiltIn || separableProgramConditions)
        {
