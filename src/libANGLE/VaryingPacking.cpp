                                                     ...
                                                     ShaderType backShaderStage)
{
    ...
    if (frontVarying && frontVarying->isArray() && !frontVarying->isPatch &&
        !frontVarying->isShaderIOBlock && frontShaderStage == ShaderType::TessControl)
    {
...
unsigned int PackedVarying::getBasicTypeElementCount() const
{
    ...
    // and outputs. This outer array level is removed from the type before considering how many
    // locations the type consumes."
    std::vector<unsigned int> arr = StripVaryingArrayDimension(
        frontVarying.varying, frontVarying.stage, backVarying.varying, backVarying.stage);
                                    ...
                                    VaryingUniqueFullNames *uniqueFullNames)
{
    ...
    if (varying.isStruct())
    {
        std::vector<unsigned int> arraySizes = StripVaryingArrayDimension(
            ref.frontShader, ref.frontShaderStage, ref.backShader, ref.backShaderStage);
