...
Error::Error(GLenum errorCode, const std::string &message)
    : mCode(errorCode), mID(errorCode), mMessage(new std::string(message))
{
}
...
template <GLenum EnumT>
ErrorStream<EnumT> &ErrorStream<EnumT>::operator<<(const char *cString)
{
    mErrorStream << cString;
    return *this;
}

#if defined(ANGLE_PLATFORM_WINDOWS)
template <GLenum EnumT>
ErrorStream<EnumT> &ErrorStream<EnumT>::operator<<(HRESULT hresult)
{
