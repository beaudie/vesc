...
ProgramID ShaderProgramManager::createProgram(rx::GLImplFactory *factory)
{
    ProgramID handle = ProgramID{mHandleAllocator.allocate()};
    mPrograms.assign(handle, new Program(factory, this, handle));
    return handle;
}

void ShaderProgramManager::deleteProgram(const gl::Context *context, ProgramID program)
{
