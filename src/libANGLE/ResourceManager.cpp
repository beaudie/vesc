...
Path *ResourceManager::getPath(GLuint handle) const
{
...
bool ResourceManager::isPath(GLuint path) const
{
    auto it = mPathMap.find(path);
    if (it == std::end(mPathMap))
        return false;
...
Error ResourceManager::setPathCommands(GLuint path, GLsizei numCommands,
                                      ...
                                      const void *coords)
{
    ...
    return p->setCommands(numCommands, commands,
        numCoords, coordType, coords);
}

void ResourceManager::setPathParameter(GLuint path, GLenum pname, GLfloat value)
{
    auto* p = getPath(path);

    p->setParameter(pname, value);
}

void ResourceManager::getPathParameter(GLuint path, GLenum pname, GLfloat *value) const
{
    const auto* p = getPath(path);

    p->getParameter(pname, value);
}
