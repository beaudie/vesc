...
void ValidateLocalSizeState(const std::array<int, 3> &localSize)
{
    ASSERT((localSize[0] < 1 && localSize[1] < 1 && localSize[2] < 1) ||
           (localSize[0] > 0 && localSize[1] > 0 && localSize[2] > 0));
}

bool IsLocalSizeDeclared(const std::array<int, 3> &localSize)
{
    bool localSizeDeclared = localSize[0] > 0;
    UNUSED_ASSERTION_VARIABLE(ValidateLocalSizeState(localSize));
...
bool Program::doVertexAndFragmentUniformsMatch(InfoLog &infoLog) const
{
...
bool Program::linkUniformBlocks(InfoLog &infoLog, const Caps &caps)
{
    ...
    else
    {
        ...
        if (!matchInterfaceBlocksBetweenVertexAndFragmentShader(vertexInterfaceBlocks,
