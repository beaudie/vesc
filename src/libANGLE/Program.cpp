...
void Program::resolveLinkImpl()
{
    ...
    auto *cache = mLinkingState->context->getMemoryProgramCache();
    if (cache &&
        (mState.mLinkedTransformFeedbackVaryings.empty() ||
         !mLinkingState->context->getWorkarounds().disableProgramCachingForTransformFeedback))
    {
        cache->putProgram(mLinkingState->programHash, mLinkingState->context, this);
    }
    mLinkingState.reset(nullptr);
}
