...
bool Program::linkVaryings(const Context *context, InfoLog &infoLog) const
{
    ...
    for (const sh::Varying &inputVarying : fragmentVaryings)
    {
        bool matched = false;
        ...
        for (const sh::Varying &outputVarying : vertexVaryings)
        {
            if (inputVarying.name == outputVarying.name)
            {
                ASSERT(!outputVarying.isBuiltIn());

                std::string mismatchedStructFieldName;
                LinkMismatchError linkError = LinkValidateVaryings(
                    outputVarying, inputVarying, vertexShader->getShaderVersion(context),
                    &mismatchedStructFieldName);
                if (linkError != LinkMismatchError::NO_MISMATCH)
                {
                    LogLinkMismatch(infoLog, inputVarying.name, "varying", linkError,
        ...
        if (!matched && inputVarying.staticUse)
        {
            infoLog << GetShaderTypeString(fragmentShader->getType()) << " varying "
                    << inputVarying.name << " does not match any "
                    << GetShaderTypeString(vertexShader->getType()) << " varying";
            return false;
        }
        ...
        if (!inputVarying.staticUse)
            continue;

        const auto inputBinding = mFragmentInputBindings.getBinding(inputVarying.name);
        if (inputBinding == -1)
            continue;
        ...
        {
            staticFragmentInputLocations.insert(std::make_pair(inputBinding, inputVarying.name));
        }
        else
        {
            infoLog << "Binding for fragment input " << inputVarying.name << " conflicts with "
                    << it->second;
            return false;
        }
    }
