...
bool Program::linkValidateGlobalNames(InfoLog &infoLog) const
{
    ...
    std::unordered_map<std::string, std::vector<const sh::InterfaceBlock *>> uniformBlockFieldMap;
    ...
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        for (const auto &uniformBlock : uniformBlocks)
        {
            ...
            for (const auto &field : uniformBlock.fields)
            {
                ...
                {
                    // First time we've seen this uniform block field name, so add it immediately
                    // since there can't be a conflict yet
                    std::vector<const sh::InterfaceBlock *> newUniformBlockList;
                    newUniformBlockList.push_back(&uniformBlock);
                ...
                std::vector<const sh::InterfaceBlock *> prevUniformBlocks =
                    uniformBlockFieldMap[field.name];
                for (const auto &prevUniformBlock : prevUniformBlocks)
                {
                    ...
                    // The uniform blocks don't match, so find the field with the name conflict and
                    // check if the necessary properties also match
                    for (const auto &prevField : prevUniformBlock->fields)
                    {
                        if ((field.name == prevField.name) && (field.type == prevField.type) &&
                            (field.precision == prevField.precision))
                        {
                ...
                uniformBlockFieldMap[field.name].push_back(&uniformBlock);
            }
        }
    }
