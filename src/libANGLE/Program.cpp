...
bool Program::linkValidateGlobalNames(InfoLog &infoLog) const
{
    ...
    std::unordered_map<std::string, sh::Uniform> uniformMap;
    std::unordered_map<std::string, sh::InterfaceBlock> uniformBlockFieldMap;
        ...
        for (const auto &uniformBlock : uniformBlocks)
        {
            ...
            if (uniformBlock.instanceName.empty())
            {
                for (const auto &field : uniformBlock.fields)
                {
                    ...
                    {
                        // We've seen this name before, so there may be a conflict between uniform
                        // block field names
                        sh::InterfaceBlock prevUniformBlock = uniformBlockFieldMap[field.name];
                        if (!uniformBlock.isSameInterfaceBlockAtLinkTime(prevUniformBlock))
                        {
                            // The uniform blocks don't match, so find the field with the name
                            // conflict and check if the necessary properties also match
                            for (const auto &fieldIn : prevUniformBlock.fields)
                            {
                    ...
                    {
                        uniformBlockFieldMap[field.name] = uniformBlock;
                    }
                }
            }
        }
    ...
    {
        if (uniformMap.count(uniformBlockField.first))
        {
            infoLog << "Name conflicts between a uniform and a uniform block field: "
                    << uniformBlockField.first;
            return false;
        }
    }
