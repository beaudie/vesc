...
VariableLocation::VariableLocation()
    : name(), element(0), index(0), staticallyUsedIndex(0), used(false), ignored(false)
{
}
...
VariableLocation::VariableLocation(const std::string &name,
    ...
    : name(name),
      element(element),
      index(index),
      staticallyUsedIndex(staticallyUsedIndex),
      used(true),
      ignored(false)
{
}
    ...
    for (size_t uniformIndex = 0; uniformIndex < mState.mUniforms.size(); uniformIndex++)
    {
        ...
        for (unsigned int arrayIndex = 0; arrayIndex < uniform.elementCount(); arrayIndex++)
        {
            VariableLocation location(uniform.name, arrayIndex,
                                      ...
                                      static_cast<unsigned int>(usedUniformIndex));
            ...
            if ((arrayIndex == 0 && preSetLocation != -1) || shaderLocation != -1)
            {
                ...
                if (preLocatedUniforms.find(elementLocation) != preLocatedUniforms.end())
                {
...
void Program::linkOutputVariables()
{
         ...
         outputVariableIndex++)
    {
             ...
             elementIndex++)
        {
            ...
            mState.mOutputVariables[location] = VariableLocation(
                outputVariable.name, element, outputVariableIndex, outputVariableIndex);
        }
    }
}
                                                       ...
                                                       int *location)
{
    ...
    LinkedUniform *uniformInList = nullptr;
    if (isSampler)
    {
        uniformInList = UniformInList(*samplerUniforms, fullName);
    }
    else
    {
        uniformInList = UniformInList(mState.mUniforms, fullName);
    }
    if (uniformInList)
    {
        if (binding != -1)
