...
void Program::onDestroy(const Context *context)
{
    ...
    for (ShaderType shaderType : AllShaderTypes())
    {
        Shader *shader = getAttachedShader(context, shaderType);
        if (shader != nullptr)
        {
            shader->release(context);
        }
        mState.mAttachedShaders[shaderType].reset();
        mAttachedShaderHandles[shaderType] = {};
    }
...
void Program::attachShader(Shader *shader)
{
    ...
    shader->addRef();
    mAttachedShaderHandles[shaderType] = shader->getHandle();
}
...
void Program::detachShader(const Context *context, Shader *shader)
{
    ...
    ASSERT(mAttachedShaderHandles[shaderType] == shader->getHandle());
    shader->release(context);
    mAttachedShaderHandles[shaderType] = {};
    mState.mAttachedShaders            = {};
}
...
int Program::getAttachedShadersCount() const
{
    ...
    for (const ShaderProgramID shader : mAttachedShaderHandles)
    {
        if (GetIDValue(shader) != 0)
        {
            ++numAttachedShaders;
        }
    }
...
Shader *Program::getAttachedShader(const Context *context, ShaderType shaderType) const
{
    ASSERT(!mLinkingState);
    return context->getShaderNoResolveCompile(mAttachedShaderHandles[shaderType]);
}
...
angle::Result Program::link(const Context *context)
{
    ...
    for (ShaderType shaderType : AllShaderTypes())
    {
        Shader *shader = context->getShaderResolveCompile(mAttachedShaderHandles[shaderType]);
...
angle::Result Program::linkImpl(const Context *context)
{
    ...
    else
    {
        ...
        if (fragmentShader)
        {
            ...
            mState.mExecutable->mEnablesPerSampleShading = fragmentShader->enablesPerSampleShading;
            mState.mExecutable->mAdvancedBlendEquations  = fragmentShader->advancedBlendEquations;
...
bool Program::linkValidateShaders(const Context *context, InfoLog &infoLog)
{
    ...
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        Shader *shaderObj = getAttachedShader(context, shaderType);
        ASSERT(shaderObj->getType() == shaderType);
    ...
    else
    {
        ...
        if (tessEvaluationShader)
        {
            ...
            mState.mExecutable->mTessGenSpacing     = tessEvaluationShader->tessGenSpacing;
            mState.mExecutable->mTessGenVertexOrder = tessEvaluationShader->tessGenVertexOrder;
            mState.mExecutable->mTessGenPointMode   = tessEvaluationShader->tessGenPointMode;
        }
    }
...
angle::Result Program::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    if (context->getShareGroup()->getFrameCaptureShared()->enabled())
    {
        ...
        {
            gl::Shader *shader = getAttachedShader(context, shaderType);
            if (shader)
            {
