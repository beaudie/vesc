...
class LinkEvent : angle::NonCopyable
{
  public:
    ...
    // See MainLinkLoadTask::hasPostLinkTasks
    virtual bool hasPostLinkTasks() const { return false; }
};
...
class Program::MainLinkLoadTask : public angle::Closure
{
  public:
    ...
    bool hasPostLinkTasks() const { return mHasPostLinkTasks; }
  ...
  protected:
    ...
    // If true, the tasks are not waited on in |resolveLink|, but instead they are free to
...
class Program::MainLinkLoadEvent final : public LinkEvent
{
  public:
    ...
    bool hasPostLinkTasks() const override { return mLinkTask->hasPostLinkTasks(); }
...
angle::Result Program::MainLinkTask::linkImpl()
{
    ...
    std::vector<std::shared_ptr<rx::LinkSubTask>> subTasks =
        mLinkTask->link(*mResources, mergedVaryings, &mHasPostLinkTasks);
    ...
    // Schedule post-link tasks, if any
    if (mHasPostLinkTasks)
    {
        mLinkTask->schedulePostLinkTasks(mSubTaskWorkerPool);
    }
...
{
    std::vector<std::shared_ptr<rx::LinkSubTask>> subTasks = mLinkTask->load(&mHasPostLinkTasks);
...
void Program::resolveLinkImpl(const Context *context)
{
    ...
    mPostLinkTasksPending = mLinkingState->linkEvent->hasPostLinkTasks();
    ...
    //
    if (!linkingState->linkingFromBinary && !mPostLinkTasksPending)
    {
...
{
    if (!mPostLinkTasksPending)
    {
        return;
    }

    mProgram->waitForPostLinkTasks(context);
    mPostLinkTasksPending = false;
