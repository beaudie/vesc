...
bool Program::indexUniforms(InfoLog &infoLog,
                            ...
                            const Bindings &uniformLocationBindings)
{
    std::vector<VariableLocation> uniformsAwaitingLocation;
    std::map<GLuint, VariableLocation> uniformsWithPreSetLocation;
    ...
    for (size_t uniformIndex = 0; uniformIndex < mState.mUniforms.size(); uniformIndex++)
    {
        ...
        if (preSetLocation != -1 &&
            uniformsWithPreSetLocation.find(preSetLocation) != uniformsWithPreSetLocation.end())
        {
        ...
        for (unsigned int arrayIndex = 0; arrayIndex < uniform.elementCount(); arrayIndex++)
        {
            ...
            {
                uniformsWithPreSetLocation[preSetLocation] = location;
                maxUniformLocation = std::max(maxUniformLocation, preSetLocation);
            }
            else
            {
                uniformsAwaitingLocation.push_back(location);
            }
        }
    }
    ...
    for (const auto &locationBinding : uniformLocationBindings)
    {
        ...
        if (uniformsWithPreSetLocation.find(location) == uniformsWithPreSetLocation.end())
        {
    ...
    mState.mUniformLocations.resize(std::max(uniformsAwaitingLocation.size() +
                                                 uniformsWithPreSetLocation.size() +
                                                 reservedLocations.size(),
                                             static_cast<size_t>(maxUniformLocation + 1)));

    // Assign uniforms with pre-set locations
    for (const auto &uniform : uniformsWithPreSetLocation)
    {
    ...
    for (const auto &uniformAwaitingLocation : uniformsAwaitingLocation)
    {
        ...
        mState.mUniformLocations[nextUniformLocation] = uniformAwaitingLocation;
        nextUniformLocation++;
    }
