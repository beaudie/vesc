                                  ...
                                  ShaderStorageBlockLinker *blockLinker)
{
    if (state.getAttachedVertexShader())
    {
        blockLinker->addShaderBlocks(
            ShaderType::Vertex, &state.getAttachedVertexShader()->getShaderStorageBlocks(context));
    }
    ...
    if (state.getAttachedComputeShader())
    {
        blockLinker->addShaderBlocks(
            ShaderType::Compute,
            &state.getAttachedComputeShader()->getShaderStorageBlocks(context));
    }
}
...
void Program::updateLinkedShaderStages()
{
    ...
    {
        mState.mLinkedShaderStages.set(static_cast<size_t>(ShaderType::Vertex));
    }

    if (mState.mAttachedFragmentShader)
    {
        mState.mLinkedShaderStages.set(static_cast<size_t>(ShaderType::Fragment));
    }

    if (mState.mAttachedComputeShader)
    {
        mState.mLinkedShaderStages.set(static_cast<size_t>(ShaderType::Compute));
    }

    if (mState.mAttachedGeometryShader)
    {
        mState.mLinkedShaderStages.set(static_cast<size_t>(ShaderType::Geometry));
    }
}
