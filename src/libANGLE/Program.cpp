...
bool Program::linkVaryings(const Context *context, InfoLog &infoLog) const
{
    ...
    const std::vector<sh::Varying> &inputVaryings  = generatingShader->getOutputVaryings(context);
    const std::vector<sh::Varying> &outputVaryings = consumingShader->getInputVaryings(context);
    ...
    for (const sh::Varying &output : outputVaryings)
    {
        ...
        for (const sh::Varying &input : inputVaryings)
        {
            if (output.name == input.name)
            {
                ...
                LinkMismatchError linkError =
                    LinkValidateVaryings(input, output, generatingShader->getShaderVersion(context),
                                         &mismatchedStructFieldName);
                if (linkError != LinkMismatchError::NO_MISMATCH)
                {
                    LogLinkMismatch(infoLog, output.name, "varying", linkError,
        ...
        {
            infoLog << GetShaderTypeString(consumingShader->getType()) << " varying " << output.name
    ...
    // TODO(jmadill): verify no unmatched input varyings?
...
LinkMismatchError Program::LinkValidateVaryings(const sh::Varying &inputVarying,
                                                const sh::Varying &outputVarying,
                                                int shaderVersion,
                                                std::string *mismatchedStructFieldName)
{
    LinkMismatchError linkError =
        LinkValidateVariablesBase(inputVarying, outputVarying, false, mismatchedStructFieldName);
    if (linkError != LinkMismatchError::NO_MISMATCH)
    {
        return linkError;
    }

    if (!sh::InterpolationTypesMatch(inputVarying.interpolation, outputVarying.interpolation))
    {
        return LinkMismatchError::INTERPOLATION_TYPE_MISMATCH;
    }

    if (shaderVersion == 100 && inputVarying.isInvariant != outputVarying.isInvariant)
    {
