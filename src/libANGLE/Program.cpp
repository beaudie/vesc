...
{
    bool localSizeDeclared = localSize[0] > 0;
#ifndef NDEBUG
...
bool Program::doVertexAndFragmentUniformsMatch(InfoLog &infoLog)
{
                           ...
                           const Bindings &uniformBindings)
{
    ...
    {
        if (!doVertexAndFragmentUniformsMatch(infoLog))
        {
            return false;
        }
    }
...
bool Program::matchInterfaceBlocksBetweenVertexAndFragmentShader(
...
bool Program::linkUniformBlocks(InfoLog &infoLog, const Caps &caps)
{
    ...
    if (mState.mAttachedComputeShader)
    {
        ...
        bool uniformBlockValidationStatus = validateUniformBlocksCount(
            caps.maxComputeUniformBlocks, computeInterfaceBlocks,
            "Compute shader uniform block count exceeds GL_MAX_COMPUTE_UNIFORM_BLOCKS (", infoLog);
        if (!uniformBlockValidationStatus)
            return false;
    }
    else
    {
        ...
        uniformBlockValidationStatus = validateUniformBlocksCount(
        ...
        bool matchIntefaceBlocksValidationStatus =
            matchInterfaceBlocksBetweenVertexAndFragmentShader(vertexInterfaceBlocks,
                                                               fragmentInterfaceBlocks, infoLog);
        if (!matchIntefaceBlocksValidationStatus)
        {
            return false;
        }
    }
...
bool Program::flattenUniformsAndCheckCaps(const Caps &caps, InfoLog &infoLog)
{
    ...
    if (mState.mAttachedComputeShader)
    {
        ...
        bool countStatus = flattenUniformsAndCheckCapsForShader(
            *computeShader, caps.maxComputeUniformComponents / 4, caps.maxComputeTextureImageUnits,
