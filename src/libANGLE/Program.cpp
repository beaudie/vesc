                          ...
                          GLsizei length)
{
         ...
         shaderStorageBlockIndex < shaderStorageBlockCount; ++shaderStorageBlockIndex)
    {
        ...
        stream.readInt(&shaderStorageBlock.binding);
        stream.readBool(&shaderStorageBlock.isArray);
        stream.readInt(&shaderStorageBlock.arrayElement);
        stream.readInt(&shaderStorageBlock.dataSize);
        stream.readBool(&shaderStorageBlock.vertexStaticUse);
        stream.readBool(&shaderStorageBlock.fragmentStaticUse);

        unsigned int numMembers = stream.readInt<unsigned int>();
        for (unsigned int blockMemberIndex = 0; blockMemberIndex < numMembers; blockMemberIndex++)
        {
            shaderStorageBlock.memberIndexes.push_back(stream.readInt<unsigned int>());
        }
                          ...
                          GLsizei *length) const
{
    ...
    for (const InterfaceBlock &shaderStorageBlock : mState.mShaderStorageBlocks)
    {
        stream.writeString(shaderStorageBlock.name);
        stream.writeInt(shaderStorageBlock.binding);
        stream.writeInt(shaderStorageBlock.isArray);
        stream.writeInt(shaderStorageBlock.arrayElement);
        stream.writeInt(shaderStorageBlock.dataSize);

        stream.writeInt(shaderStorageBlock.vertexStaticUse);
        stream.writeInt(shaderStorageBlock.fragmentStaticUse);

        stream.writeInt(shaderStorageBlock.memberIndexes.size());
        for (unsigned int memberShaderStorageIndex : shaderStorageBlock.memberIndexes)
        {
            stream.writeInt(memberShaderStorageIndex);
        }
    }
...
void Program::gatherInterfaceBlockInfo()
{
    ...
    std::set<std::string> visitedShaderStorageBlockList;
    ...
    for (const sh::InterfaceBlock &vertexBlock : vertexShader->getInterfaceBlocks())
    {
        ...
        {
            if (visitedShaderStorageBlockList.count(vertexBlock.name) > 0)
                continue;

            defineShaderStorageBlock(vertexBlock, GL_VERTEX_SHADER);
            visitedShaderStorageBlockList.insert(vertexBlock.name);
        }
    }
    ...
    for (const sh::InterfaceBlock &fragmentBlock : fragmentShader->getInterfaceBlocks())
    {
        ...
        {
            if (visitedShaderStorageBlockList.count(fragmentBlock.name) > 0)
            {
                for (InterfaceBlock &block : mState.mShaderStorageBlocks)
                {
                    if (block.name == fragmentBlock.name)
            ...
            defineShaderStorageBlock(fragmentBlock, GL_FRAGMENT_SHADER);
            visitedShaderStorageBlockList.insert(fragmentBlock.name);
        }
    }
}
