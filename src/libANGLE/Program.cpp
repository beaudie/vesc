...
{
    ShaderStorageBlockLinker blockLinker(&mState.mShaderStorageBlocks, &mState.mBufferVariables);
    InitShaderStorageBlockLinker(context, mState, &blockLinker);

    auto getImplBlockSize = [this](const std::string &name, const std::string &mappedName,
                                   size_t *sizeOut) {
        return this->mProgram->getShaderStorageBlockSize(name, mappedName, sizeOut);
    };

    auto getImplMemberInfo = [this](const std::string &name, const std::string &mappedName,
                                    sh::BlockMemberInfo *infoOut) {
        return this->mProgram->getShaderStorageBlockMemberInfo(name, mappedName, infoOut);
    };
