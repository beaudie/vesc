...
template <>
GLint GetVariableLocation<LinkedUniform>(const std::vector<LinkedUniform> &list,
                                         const std::vector<VariableLocation> &locationList,
                                         const std::string &name)
{
    size_t nameLengthWithoutArrayIndex;
    ...
    for (size_t location = 0u; location < locationList.size(); ++location)
    {
        ...
        if (angle::BeginsWith(variable.name, name) &&
            (!std::is_base_of<sh::ShaderVariable, LinkedUniform>::value ||
...
// ProgramUniformBindings implementation.
ProgramUniformBindings::ProgramUniformBindings() {}

ProgramUniformBindings::~ProgramUniformBindings() {}

void ProgramUniformBindings::bindLocation(GLuint index, const std::string &name)
{
...
int ProgramUniformBindings::getBindingByName(const std::string &name) const
{
    auto iter = mBindings.find(name);
    return (iter != mBindings.end()) ? iter->second.location : -1;
}

int ProgramUniformBindings::getBinding(const sh::ShaderVariable &variable) const
{
    const std::string &name = variable.name;
...
ProgramUniformBindings::const_iterator ProgramUniformBindings::begin() const
{
    return mBindings.begin();
}

ProgramUniformBindings::const_iterator ProgramUniformBindings::end() const
{
    return mBindings.end();
}
...
GLuint Program::getResourceLocation(const GLchar *name, const sh::ShaderVariable &variable) const
{
    ...
    if (variable.isArray())
    {
        ...
        location += ParseArrayIndex(name, &nameLengthWithoutArrayIndexOut);
        ASSERT(nameLengthWithoutArrayIndexOut != std::string(name).length());
    }
...
const ProgramUniformBindings &Program::getUniformLocationBindings() const
{
    ASSERT(mLinkResolved);
                                  ...
                                  GLuint combinedShaderStorageBlocksCount)
{
    ...
    for (const auto &binding : mFragmentOutputLocations)
    {
             ...
             outputVariableIndex < mState.mOutputVariables.size(); outputVariableIndex++)
        {
            ...
            unsigned int location = binding.second;
