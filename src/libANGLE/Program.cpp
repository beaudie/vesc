                                   ...
                                   const GLint *v)
{
    ...
    for (GLsizei arrayIndex = 0; arrayIndex < clampedCount; ++arrayIndex)
    {
        ...
        boundTextureUnits[arrayIndex + locationInfo.arrayIndex] = samplerIndex;

        mState.mActiveSamplersMask.set(newSamplerIndex);
        mState.mActiveSamplerRefCounts[newSamplerIndex]++;

        // Check for binding type change.
        TextureType &newSamplerType = mState.mActiveSamplerTypes[newSamplerIndex];

        if (newSamplerType == TextureType::InvalidEnum)
        {
        ...
        if (--mState.mActiveSamplerRefCounts[oldSamplerIndex] == 0)
        {
        ...
        else if (oldSamplerType == TextureType::InvalidEnum)
        {
            ...
            newSamplerType = mState.getSamplerUniformTextureType(oldSamplerIndex);
        }
    }
