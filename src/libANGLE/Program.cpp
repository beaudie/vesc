...
{
    Shader *generatingShader = nullptr;
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        Shader *currentShader = mState.mAttachedShaders[shaderType];
        if (currentShader)
        {
            if (generatingShader != nullptr)
            {
                if (!linkValidateShaderInterfaceMatching(context, generatingShader, currentShader,
                                                         infoLog))
                {
                    return false;
                }
            }
            generatingShader = currentShader;
        }
    }
