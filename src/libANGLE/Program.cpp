...
// a list of uniforms
Error Program::link(const gl::ContextState &data)
{
    unlink(false);
    ...
    for (size_t uniformIndex = 0; uniformIndex < mState.mUniforms.size(); uniformIndex++)
    {
        ...
        for (unsigned int arrayIndex = 0; arrayIndex < uniform.elementCount(); arrayIndex++)
        {
            if (!uniform.isBuiltIn())
            {
                ...
                mState.mUniformLocations.push_back(gl::VariableLocation(
...
bool Program::linkAttributes(const gl::ContextState &data,
                             InfoLog &infoLog,
