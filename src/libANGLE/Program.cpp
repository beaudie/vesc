...
VariableLocation::VariableLocation() : index(kUnused), ignored(false)
{
}
...
{
    arrayElementIndices.push_back(element);
}

bool VariableLocation::areAllElementIndicesZero() const
{
    for (unsigned int elementIndex : arrayElementIndices)
    {
        if (elementIndex != 0)
        {
            return false;
...
GLint ProgramState::getUniformLocation(const std::string &name) const
{
    ...
    for (size_t location = 0; location < mUniformLocations.size(); ++location)
    {
        ...
        if (uniform.name == baseName)
        {
            if (uniform.isArray())
            {
                if (uniformLocation.arrayElementIndices == subscripts ||
                    (uniformLocation.areAllElementIndicesZero() && subscripts.empty()))
                {
...
unsigned int ProgramState::flattenUniformArrayElementOffset(
    const VariableLocation &locationInfo) const
{
    unsigned int elementOffset = 0u;
    for (unsigned int arrayNestingIndex = 0;
         arrayNestingIndex < locationInfo.arrayElementIndices.size(); ++arrayNestingIndex)
    {
        unsigned int arrayElementIndex = locationInfo.arrayElementIndices[arrayNestingIndex];
...
GLint Program::getFragDataLocation(const std::string &name) const
{
    ...
    for (auto outputPair : mState.mOutputLocations)
    {
        ...
        ASSERT(locationInfo.arrayElementIndices.size() <= 1);
        if (outputVariable.name == baseName &&
            (arrayIndex == GL_INVALID_INDEX ||
             (!locationInfo.arrayElementIndices.empty() &&
              arrayIndex == locationInfo.arrayElementIndices.back())))
        {
                                   ...
                                   const GLint *v)
{
    ...
    unsigned int destinationElementOffset = mState.flattenUniformArrayElementOffset(locationInfo);
    std::copy(v, v + clampedCount, boundTextureUnits->begin() + destinationElementOffset);
