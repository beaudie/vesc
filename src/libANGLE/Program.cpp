...
size_t Program::getInputResourceListSize() const
{
    ASSERT(mLinkResolved);

    if (hasLinkedShaderStage(ShaderType::Vertex))
    {
        return mState.mAttributes.size();
    }
    else
    {
        return mState.mInputVaryings.size();
    }
}

const sh::VariableWithLocation &Program::getInputResource(size_t index) const
{
    ASSERT(mLinkResolved);
...
GLuint Program::getInputResourceIndex(const GLchar *name) const
{
    ...
    size_t listSize              = getInputResourceListSize();

    for (size_t index = 0; index < listSize; index++)
    {
        sh::VariableWithLocation resource = getInputResource(index);
