...
bool Program::linkVaryings(const Context *context, InfoLog &infoLog) const
{
    ...
    if (geometryShader)
    {
        return linkValidateShaderVaryings(context, vertexShader, geometryShader, infoLog) &&
               linkValidateShaderVaryings(context, geometryShader, fragmentShader, infoLog);
    }
    else
    {
        return linkValidateShaderVaryings(context, vertexShader, fragmentShader, infoLog);
    }
}

bool Program::linkValidateShaderVaryings(const Context *context,
                                         ...
                                         InfoLog &infoLog) const
{
    ...
    if (consumingShader->getType() == GL_FRAGMENT_SHADER)
    {
    ...
    // TODO(jiawei.shao@intel.com): validate shader input/output blocks matching
    for (const sh::Varying &input : inputVaryings)
    {
        ...
        if (staticFragmentInputLocations &&
            !linkValidateFragmentInputBindings(input, staticFragmentInputLocations, infoLog))
        {
            return false;
        }
    }
            ...
            if (entry != linkedInterfaceBlocks.end())
            {
                ...
                if (linkError != LinkMismatchError::NO_MISMATCH)
                {
                    ...
                    GLenum mismatchedShaderType = InfoLog::GetShaderTypeFromUniformName(
                        geometryInterfaceBlock.name, vertexInterfaceBlocks,
                        fragmentInterfaceBlocks);
                    LogLinkMismatch(infoLog, geometryInterfaceBlock.name, "interface block",
                                    linkError, mismatchedBlockFieldName, mismatchedShaderType,
                                    GL_GEOMETRY_SHADER_EXT);
                    return false;
                }
            }
            else if (geometryInterfaceBlock.staticUse ||
                     geometryInterfaceBlock.layout != sh::BLOCKLAYOUT_PACKED)
            {
                blockCount += std::max(geometryInterfaceBlock.arraySize, 1u);
            }
        }
    }
...
bool Program::linkInterfaceBlocks(const Context *context, InfoLog &infoLog)
{
    ...
    Shader *geometryShader                                       = mState.mAttachedGeometryShader;
    const std::vector<sh::InterfaceBlock> *geometryUniformBlocks = nullptr;
    if (geometryShader)
    {
        geometryUniformBlocks = &geometryShader->getUniformBlocks(context);
        if (!validateInterfaceBlocksCount(
    ...
    if (!ValidateGraphicsInterfaceBlocks(
            vertexUniformBlocks, fragmentUniformBlocks, geometryUniformBlocks, infoLog,
            webglCompatibility, sh::BlockType::BLOCK_UNIFORM, caps.maxCombinedUniformBlocks))
    {
        return false;
    }
    ...
    if (context->getClientVersion() >= Version(3, 1))
    {
        ...
        const std::vector<sh::InterfaceBlock> *geometryShaderStorageBlocks = nullptr;
        if (geometryShader)
        {
            geometryShaderStorageBlocks = &geometryShader->getShaderStorageBlocks(context);
        ...
        if (!ValidateGraphicsInterfaceBlocks(vertexShaderStorageBlocks, fragmentShaderStorageBlocks,
                                             geometryShaderStorageBlocks, infoLog,
