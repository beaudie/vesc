...
angle::Result Program::link(const Context *context)
{
    ...
    InfoLog &infoLog = mState.getProgramExecutable().getInfoLog();

    auto *platform   = ANGLEPlatformCurrent();
    double startTime = platform->currentTime(platform);

    unlink();
    mState.getProgramExecutable().resetInfoLog();
...
{
    mState.getProgramExecutable().getLinkedShaderStages().reset();

    for (const Shader *shader : mState.mAttachedShaders)
    {
        if (shader)
        {
            mState.getProgramExecutable().getLinkedShaderStages().set(shader->getType());
        }
    }
}
...
angle::Result Program::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    stream.writeInt(mState.getProgramExecutable().getLinkedShaderStages().to_ulong());
                                   ...
                                   InfoLog &infoLog)
{
    ...
    mState.getProgramExecutable().getLinkedShaderStages() = ShaderBitSet(stream.readInt<uint8_t>());
