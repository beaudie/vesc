...
bool Program::linkAttributes(const Context *context, InfoLog &infoLog)
{
    ...
    if ((shaderVersion >= 300 && mState.mAttributes.size() > maxAttribs) ||
        vertexShader->getActiveAttributes(context).size() > maxAttribs)
    {
    ...
    // Assign locations for attributes that have a binding location and check for attribute
    // aliasing.
    for (sh::Attribute &attribute : mState.mAttributes)
    {
        ...
        // ES 3.00.6 section 12.46. In GLSL ES 1.00.17 we only do aliasing checks for active
        // attributes.
        if (shaderVersion < 300 && !attribute.active)
        {
            continue;
        }
    ...
    // Assign locations for attributes that don't have a binding location.
    for (sh::Attribute &attribute : mState.mAttributes)
    {
        if (shaderVersion < 300 && !attribute.active)
        {
