...
LinkMismatchError LinkValidateInterfaceBlocks(const sh::InterfaceBlock &interfaceBlock1,
...
using ShaderInterfaceBlocks = std::pair<GLenum, const std::vector<sh::InterfaceBlock> *>;
using ShaderInterfaceBlock  = std::pair<GLenum, const sh::InterfaceBlock *>;
using InterfaceBlockMap     = std::map<const std::string, ShaderInterfaceBlock>;

bool ValidateGraphicsInterfaceBlocksPerShader(const ShaderInterfaceBlocks &shaderInterfaceBlocks,
                                              bool extendLinkedBlocks,
                                              bool webglCompatibility,
                                              ...
                                              InfoLog &infoLog)
{
    ...
    GLenum shaderType                                            = shaderInterfaceBlocks.first;
    const std::vector<sh::InterfaceBlock> &interfaceBlocksToLink = *(shaderInterfaceBlocks.second);
    for (const sh::InterfaceBlock &block : interfaceBlocksToLink)
    {
        ...
        if (entry != linkedBlocks->end())
        {
            ...
            LinkMismatchError linkError = LinkValidateInterfaceBlocks(
                block, linkedBlock, webglCompatibility, &mismatchedStructFieldName);
            if (linkError != LinkMismatchError::NO_MISMATCH)
            {
                LogLinkMismatch(infoLog, block.name, "interface block", linkError,
                                mismatchedStructFieldName, entry->second.first, shaderType);
                return false;
            }
        }
        else if (extendLinkedBlocks)
        {
...
bool ValidateGraphicsInterfaceBlocks(
    const std::vector<ShaderInterfaceBlocks> &activeGraphicsInterfaceBlocks,
    InfoLog &infoLog,
    bool webglCompatibility,
    ...
    GLuint maxCombinedInterfaceBlocks)
{
    ...
    InterfaceBlockMap linkedBlocks;
    GLuint combinedInterfaceBlockCount = 0u;

    ASSERT(activeGraphicsInterfaceBlocks.size() >= 2u);

    const auto &vertexShaderInterfaceBlocks = *activeGraphicsInterfaceBlocks.front().second;
    for (const sh::InterfaceBlock &linkedBlock : vertexShaderInterfaceBlocks)
    {
        linkedBlocks[linkedBlock.name] =
            std::make_pair(activeGraphicsInterfaceBlocks.front().first, &linkedBlock);
        if (IsActiveInterfaceBlock(linkedBlock))
        {
            combinedInterfaceBlockCount += std::max(linkedBlock.arraySize, 1u);
        }
    }

    const size_t numActiveShadersToLink = activeGraphicsInterfaceBlocks.size();
    for (size_t shaderIndex = 1; shaderIndex < numActiveShadersToLink; ++shaderIndex)
    {
        bool isLastShader = (shaderIndex == numActiveShadersToLink - 1);
        if (!ValidateGraphicsInterfaceBlocksPerShader(
                activeGraphicsInterfaceBlocks[shaderIndex], !isLastShader, webglCompatibility,
                &linkedBlocks, &combinedInterfaceBlockCount, infoLog))
        {
            return false;
        }
    }

    if (combinedInterfaceBlockCount > maxCombinedInterfaceBlocks)
    {
        switch (blockType)
        {
...
bool Program::linkInterfaceBlocks(const Context *context, InfoLog &infoLog)
{
    ...
    std::vector<ShaderInterfaceBlocks> activeGraphicsUniformBlocks;
    activeGraphicsUniformBlocks.emplace_back(GL_VERTEX_SHADER, &vertexUniformBlocks);
    activeGraphicsUniformBlocks.emplace_back(GL_FRAGMENT_SHADER, &fragmentUniformBlocks);
    ...
    if (geometryShader)
    {
        ...
        activeGraphicsUniformBlocks.emplace_back(GL_GEOMETRY_SHADER_EXT, &geometryUniformBlocks);
    }

    if (!ValidateGraphicsInterfaceBlocks(
            activeGraphicsUniformBlocks, infoLog, context->getExtensions().webglCompatibility,
            sh::BlockType::BLOCK_UNIFORM, caps.maxCombinedUniformBlocks))
    {
        return false;
    }
    ...
    if (context->getClientVersion() >= Version(3, 1))
    {
        ...
        std::vector<ShaderInterfaceBlocks> activeGraphicsShaderStorageBlocks;
        activeGraphicsShaderStorageBlocks.emplace_back(GL_VERTEX_SHADER,
                                                       &vertexShaderStorageBlocks);
        activeGraphicsShaderStorageBlocks.emplace_back(GL_FRAGMENT_SHADER,
                                                       &fragmentShaderStorageBlocks);
        ...
        if (geometryShader)
        {
            ...
            activeGraphicsShaderStorageBlocks.emplace_back(GL_GEOMETRY_SHADER_EXT,
                                                           &geometryShaderStorageBlocks);
        }

        if (!ValidateGraphicsInterfaceBlocks(activeGraphicsShaderStorageBlocks, infoLog,
