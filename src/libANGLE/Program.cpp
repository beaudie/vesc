...
angle::Result Program::link(const Context *context)
{
    ...
    if ((context->getState().getProgram() == this) &&
        context->getState().getProgramExecutable() != mState.mExecutable)
    {
...
angle::Result Program::linkImpl(const Context *context)
{
    ...
    if (mState.mAttachedShaders[ShaderType::Compute])
    {
        ...
        if (!linkUniforms(context->getCaps(), context->getClientVersion(), infoLog,
                          ...
                          &mState.mExecutable->mResources->unusedUniforms))
        {
            return angle::Result::Continue;
        }
        ...
        InitUniformBlockLinker(mState, &mState.mExecutable->mResources->uniformBlockLinker);
        InitShaderStorageBlockLinker(mState,
                                     &mState.mExecutable->mResources->shaderStorageBlockLinker);
    }
    else
    {
    ...
    mLinkingState->linkEvent = mProgram->link(context, *mState.mExecutable->mResources, infoLog);
    mLinkResolved            = false;
                                 ...
                                 GLchar *name) const
{
    ...
    ASSERT(index < mState.mExecutable->mProgramInputs.size());
    const sh::ShaderVariable &attrib = mState.mExecutable->mProgramInputs[index];
...
GLint Program::getActiveAttributeCount() const
{
    ...
    return static_cast<GLint>(mState.mExecutable->mProgramInputs.size());
}
...
GLint Program::getActiveAttributeMaxLength() const
{
    ...
    for (const sh::ShaderVariable &attrib : mState.mExecutable->mProgramInputs)
    {
...
{
    ASSERT(mLinkResolved);
    return mState.mExecutable->mProgramInputs;
}
...
const sh::ShaderVariable &Program::getInputResource(size_t index) const
{
    ASSERT(mLinkResolved);
    ASSERT(index < mState.mExecutable->mProgramInputs.size());
    return mState.mExecutable->mProgramInputs[index];
}
...
GLuint Program::getInputResourceIndex(const GLchar *name) const
{
    ...
    for (size_t index = 0; index < mState.mExecutable->mProgramInputs.size(); index++)
    {
...
GLuint Program::getInputResourceMaxNameSize() const
{
    ...
    for (const sh::ShaderVariable &resource : mState.mExecutable->mProgramInputs)
    {
...
GLuint Program::getOutputResourceMaxNameSize() const
{
    ...
    for (const sh::ShaderVariable &resource : mState.mExecutable->mOutputVariables)
    {
        max = getResourceMaxNameSize(resource, max);
    }
...
GLuint Program::getOutputResourceIndex(const GLchar *name) const
{
    ...
    for (size_t index = 0; index < mState.mExecutable->mOutputVariables.size(); index++)
    {
...
{
    ASSERT(mLinkResolved);
    return (mLinked ? mState.mExecutable->mOutputVariables.size() : 0);
}
...
{
    ASSERT(mLinkResolved);
    ASSERT(index < mState.mExecutable->mUniforms.size());
    getResourceName(mState.mExecutable->mUniforms[index].name, bufSize, length, name);
}
...
{
    ASSERT(mLinkResolved);
    ASSERT(index < mState.mExecutable->mOutputVariables.size());
    return mState.mExecutable->mOutputVariables[index];
}
...
{
    ASSERT(mLinkResolved);
    return mState.mExecutable->mTransformFeedbackStrides;
}

GLint Program::getFragDataLocation(const std::string &name) const
{
    ASSERT(mLinkResolved);
    GLint primaryLocation = GetVariableLocation(mState.mExecutable->mOutputVariables,
                                                mState.mExecutable->mOutputLocations, name);
    if (primaryLocation != -1)
    {
        return primaryLocation;
    }
    return GetVariableLocation(mState.mExecutable->mOutputVariables,
...
{
    ASSERT(mLinkResolved);
    if (GetVariableLocation(mState.mExecutable->mOutputVariables,
                            mState.mExecutable->mOutputLocations, name) != -1)
    {
        return 0;
    }
    if (GetVariableLocation(mState.mExecutable->mOutputVariables, mState.mSecondaryOutputLocations,
                            name) != -1)
    {
        return 1;
    }
                               ...
                               GLchar *name) const
{
    ...
    if (mLinked)
    {
        ...
        ASSERT(index < mState.mExecutable->mUniforms.size());
        const LinkedUniform &uniform = mState.mExecutable->mUniforms[index];
...
GLint Program::getActiveUniformCount() const
{
    ...
    if (mLinked)
    {
        return static_cast<GLint>(mState.mExecutable->mUniforms.size());
    }
    else
    {
        return 0;
...
GLint Program::getActiveUniformMaxLength() const
{
    ...
    if (mLinked)
    {
        for (const LinkedUniform &uniform : mState.mExecutable->mUniforms)
        {
...
const LinkedUniform &Program::getUniformByLocation(UniformLocation location) const
{
    ...
    ASSERT(location.value >= 0 &&
           static_cast<size_t>(location.value) < mState.mUniformLocations.size());
    return mState.mExecutable->mUniforms[mState.getUniformIndexFromLocation(location)];
}
...
{
    ASSERT(mLinkResolved);
    return {GetVariableLocation(mState.mExecutable->mUniforms, mState.mUniformLocations, name)};
}
...
{
    ASSERT(mLinkResolved);
    GetInterfaceBlockName(blockIndex, mState.mExecutable->mUniformBlocks, bufSize, length,
                          blockName);
}
...
{
    ASSERT(mLinkResolved);
    GetInterfaceBlockName(blockIndex, mState.mExecutable->mShaderStorageBlocks, bufSize, length,
                          blockName);
}
...
{
    ASSERT(mLinkResolved);
    return getActiveInterfaceBlockMaxNameLength(mState.mExecutable->mUniformBlocks);
}

GLint Program::getActiveShaderStorageBlockMaxNameLength() const
{
    ASSERT(mLinkResolved);
    return getActiveInterfaceBlockMaxNameLength(mState.mExecutable->mShaderStorageBlocks);
}

GLuint Program::getUniformBlockIndex(const std::string &name) const
{
    ASSERT(mLinkResolved);
    return GetInterfaceBlockIndex(mState.mExecutable->mUniformBlocks, name);
}

GLuint Program::getShaderStorageBlockIndex(const std::string &name) const
{
    ASSERT(mLinkResolved);
    return GetInterfaceBlockIndex(mState.mExecutable->mShaderStorageBlocks, name);
}

const InterfaceBlock &Program::getUniformBlockByIndex(GLuint index) const
{
    ASSERT(mLinkResolved);
    ASSERT(index < static_cast<GLuint>(mState.mExecutable->mUniformBlocks.size()));
    return mState.mExecutable->mUniformBlocks[index];
}

const InterfaceBlock &Program::getShaderStorageBlockByIndex(GLuint index) const
{
    ASSERT(mLinkResolved);
    ASSERT(index < static_cast<GLuint>(mState.mExecutable->mShaderStorageBlocks.size()));
    return mState.mExecutable->mShaderStorageBlocks[index];
}
...
{
    ASSERT(mLinkResolved);
    return mState.mExecutable->mTransformFeedbackBufferMode;
}
...
bool Program::linkVaryings(InfoLog &infoLog) const
{
    ...
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        if (previousShader)
        {
            if (!linkValidateShaderInterfaceMatching(
                    previousShader->getState(), currentShader->getState(), isSeparable(), infoLog))
            {
                return false;
            }
        }
    ...
    Shader *fragmentShader = mState.mAttachedShaders[ShaderType::Fragment];
    if (vertexShader && fragmentShader &&
        !linkValidateBuiltInVaryings(vertexShader->getState(), fragmentShader->getState(), infoLog))
    {
        return false;
    }
...
bool Program::doShaderVariablesMatch(const ShaderState &generatingShaderState,
                                     ...
                                     gl::InfoLog &infoLog)
{
    ...
    if (namesMatch || locationsMatch)
    {
        ...
        LinkMismatchError linkError = LinkValidateVaryings(
            output, input, generatingShaderState.getShaderVersion(), validateGeometryShaderInputs,
            isSeparable, &mismatchedStructFieldName);
        if (linkError != LinkMismatchError::NO_MISMATCH)
        {
            LogLinkMismatch(infoLog, input.name, "varying", linkError, mismatchedStructFieldName,
                            generatingShaderState.getShaderType(),
                            consumingShaderState.getShaderType());
            return false;
        }
...
bool Program::linkValidateShaderInterfaceMatching(const ShaderState &generatingShaderState,
                                                  const ShaderState &consumingShaderState,
                                                  bool isSeparable,
                                                  gl::InfoLog &infoLog)
{
    ASSERT(generatingShaderState.getShaderVersion() == consumingShaderState.getShaderVersion());

    const std::vector<sh::ShaderVariable> &outputVaryings =
        generatingShaderState.getOutputVaryings();
    const std::vector<sh::ShaderVariable> &inputVaryings = consumingShaderState.getInputVaryings();
    std::vector<const sh::ShaderVariable *> filteredInputVaryings;
    std::vector<const sh::ShaderVariable *> filteredOutputVaryings;
    bool validateGeometryShaderInputs =
        consumingShaderState.getShaderType() == ShaderType::Geometry;
    ...
    {
        infoLog << GetShaderTypeString(consumingShaderState.getShaderType())
                << " does not consume all varyings generated by "
                << GetShaderTypeString(generatingShaderState.getShaderType());
        return false;
    }
    if (isSeparable && filteredInputVaryings.size() > filteredOutputVaryings.size())
    {
        infoLog << GetShaderTypeString(generatingShaderState.getShaderType())
                << " does not generate all varyings consumed by "
                << GetShaderTypeString(consumingShaderState.getShaderType());
        return false;
    }
    ...
    for (const sh::ShaderVariable *input : filteredInputVaryings)
    {
        ...
        {
            if (doShaderVariablesMatch(generatingShaderState, consumingShaderState, *input, *output,
        ...
        {
            infoLog << GetShaderTypeString(consumingShaderState.getShaderType()) << " varying "
                    << input->name << " does not match any "
                    << GetShaderTypeString(generatingShaderState.getShaderType()) << " varying";
            return false;
        }
    }
...
void Program::linkSamplerAndImageBindings(GLuint *combinedImageUniforms)
{
    ...
    auto highIter = mState.mExecutable->mUniforms.rbegin();
    auto lowIter  = highIter;

    unsigned int high = static_cast<unsigned int>(mState.mExecutable->mUniforms.size());
    unsigned int low  = high;

    // Note that uniform block uniforms are not yet appended to this list.
    ASSERT(mState.mExecutable->mUniforms.size() == 0 || highIter->isAtomicCounter() ||
           highIter->isImage() || highIter->isSampler() || highIter->isInDefaultBlock());

    for (; lowIter != mState.mExecutable->mUniforms.rend() && lowIter->isAtomicCounter(); ++lowIter)
    {
        --low;
    }
    ...
    for (; lowIter != mState.mExecutable->mUniforms.rend() && lowIter->isImage(); ++lowIter)
    {
        --low;
    }
    ...
    *combinedImageUniforms                 = 0u;
    ...
    for (unsigned int imageIndex : mState.mExecutable->mImageUniformRange)
    {
        ...
        auto &imageUniform = mState.mExecutable->mUniforms[imageIndex];
    ...
    for (; lowIter != mState.mExecutable->mUniforms.rend() && lowIter->isSampler(); ++lowIter)
    {
        --low;
    }
    ...
    for (unsigned int samplerIndex : mState.mExecutable->mSamplerUniformRange)
    {
        const auto &samplerUniform = mState.mExecutable->mUniforms[samplerIndex];
...
bool Program::linkAtomicCounterBuffers()
{
    for (unsigned int index : mState.mAtomicCounterUniformRange)
    {
        ...
        for (unsigned int bufferIndex = 0;
             bufferIndex < mState.mExecutable->mAtomicCounterBuffers.size(); ++bufferIndex)
        {
        ...
        if (!found)
        {
            ...
            uniform.bufferIndex =
                static_cast<int>(mState.mExecutable->mAtomicCounterBuffers.size() - 1);
        }
    }
...
bool Program::linkAttributes(const Context *context, InfoLog &infoLog)
{
    ...
    if (shaderVersion >= 300)
    {
        for (auto attributeIter = mState.mExecutable->mProgramInputs.begin();
             attributeIter != mState.mExecutable->mProgramInputs.end();)
        {
    ...
    for (const sh::ShaderVariable &attribute : mState.mExecutable->mProgramInputs)
    {
...
bool Program::linkValidateBuiltInVaryings(const ShaderState &vertexShaderState,
                                          const ShaderState &fragmentShaderState,
                                          InfoLog &infoLog)
{
    const auto &vertexVaryings   = vertexShaderState.getOutputVaryings();
    const auto &fragmentVaryings = fragmentShaderState.getInputVaryings();
    int shaderVersion            = vertexShaderState.getShaderVersion();
                                            ...
                                            const Caps &caps) const
{
    ...
    for (const std::string &tfVaryingName : mState.mTransformFeedbackVaryingNames)
    {
        ...
        if (mState.mExecutable->mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS &&
        ...
        if (mState.mExecutable->mTransformFeedbackBufferMode == GL_INTERLEAVED_ATTRIBS &&
                                  ...
                                  GLuint combinedShaderStorageBlocksCount)
{
    ...
    for (const auto &binding : mFragmentOutputLocations)
    {
        ...
        for (unsigned int outputVariableIndex = 0;
             outputVariableIndex < mState.mExecutable->mOutputVariables.size();
             outputVariableIndex++)
        {
    ...
    for (unsigned int outputVariableIndex = 0;
         outputVariableIndex < mState.mExecutable->mOutputVariables.size(); outputVariableIndex++)
    {
        const sh::ShaderVariable &outputVariable =
            mState.mExecutable->mOutputVariables[outputVariableIndex];
...
void Program::initInterfaceBlockBindings()
{
    ...
    for (unsigned int blockIndex = 0; blockIndex < mState.mExecutable->mUniformBlocks.size();
         blockIndex++)
    {
    ...
    for (GLsizei arrayIndex = 0; arrayIndex < clampedCount; ++arrayIndex)
    {
        ...
        if (newRefCount == 1)
        {
            ...
            mState.mExecutable->mActiveSamplerShaderBits[newTextureUnit] =
                mState.mExecutable->mUniforms[locationInfo.index].activeShaders();
        }
        else
        {
                                   ...
                                   const T *v)
{
    ...
    const LinkedUniform &linkedUniform = mState.mExecutable->mUniforms[locationInfo.index];
...
angle::Result Program::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    for (const auto &atomicCounterBuffer : mState.mExecutable->mAtomicCounterBuffers)
    {
                                   ...
                                   InfoLog &infoLog)
{
    ...
    ASSERT(mState.mExecutable->mUniforms.empty());
    ...
    ASSERT(mState.mExecutable->mUniformBlocks.empty());
    ...
    ASSERT(mState.mExecutable->mShaderStorageBlocks.empty());
    ...
    ASSERT(mState.mExecutable->mAtomicCounterBuffers.empty());
    ...
    ASSERT(mState.mExecutable->mOutputLocations.empty());
