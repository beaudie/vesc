...
struct Program::LinkingState
{
    ...
    std::unique_ptr<rx::ParallelEvent> linkingEvent;
};

struct Program::LoadingState
{
    std::unique_ptr<rx::ParallelEvent> loadingEvent;
};
...
Error Program::link(const gl::Context *context)
{
    ...
    mLinkingState->linkingEvent = mProgram->link(context, *resources, mInfoLog);
...
{
    return ((mLinkingState.get() && mLinkingState->linkingEvent->isOngoing()) ||
            (mLoadingState.get() && mLoadingState->loadingEvent->isOngoing()));
}
...
void Program::resolveLinkImpl(const Context *context)
{
    ...
    angle::Result result = mLinkingState->linkingEvent->wait(context);
...
class LoadEvent final : public rx::ParallelEvent
{
  public:
    LoadEvent(const Context *context,
    ...
    {
        return MemoryProgramCache::Deserialize(mContext, mProgram, mState, mBinary, mLength,
                                               mInfoLog);
    }
    bool isOngoing() override { return true; }

  private:
    const Context *mContext;
    const Program *mProgram;
    ProgramState *mState;
    const uint8_t *mBinary;
    size_t mLength;
    InfoLog &mInfoLog;
};
                          ...
                          GLsizei length)
{
    ...
    mLoadingState.reset(new LoadingState());
    mLoadingState->loadingEvent =
        std::make_unique<LoadEvent>(context, this, &mState, bytes, length, mInfoLog);
    mResolved = false;
    return NoError();
#endif
}

void Program::resolveLoadImpl(const gl::Context *context)
{
    ASSERT(mLoadingState.get());

    angle::Result result = mLoadingState->loadingEvent->wait(nullptr);
    mLoadingState.reset();
