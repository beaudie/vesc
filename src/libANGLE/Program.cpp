...
void Program::detachShader(const Context *context, Shader *shader)
{
    ...
    // Don't actually detach the shader yet, since we may need to recompile with it at draw time.
}
...
angle::Result Program::link(const Context *context)
{
    ...
    else
    {
        ...
        if (programPipeline->usesShaderProgram(id()))
        {
            const ProgramMergedVaryings &mergedVaryings =
            ...
            ANGLE_TRY(linkMergedVaryings(context, varyingPacking, mergedVaryings, infoLog));
        }
        else
        {
            const ProgramMergedVaryings &mergedVaryings = getMergedVaryings();
            ANGLE_TRY(
                linkMergedVaryings(context, mResources->varyingPacking, mergedVaryings, infoLog));
        }
    }
...
void Program::unlink()
{
    ...
    mState.mExecutable.resetInfoLog();
    mState.mExecutable.mActiveAttribLocationsMask.reset();
...
{
    ASSERT(mLinkResolved);
    return mState.mExecutable.mAttributesTypeMask;
}
AttributesMask Program::getAttributesMask() const
{
    ASSERT(mLinkResolved);
    return mState.mExecutable.mAttributesMask;
}
    ...
    for (const sh::ShaderVariable *input : filteredInputVaryings)
    {
        ...
        for (const sh::ShaderVariable *output : filteredOutputVaryings)
        {
            if (doShaderVariablesMatch(generatingShader, consumingShader, *input, *output,
                                       validateGeometryShaderInputs, infoLog))
            {
                match = true;
                break;
            }
                                                     ...
                                                     std::string *mismatchedStructOrBlockMemberName)
{
    ...
    if ((variable1.name == variable2.name) && (variable1.location != variable2.location))
    {
                                                ...
                                                std::string *mismatchedStructFieldName)
{
    ...
    bool validatePrecision = shaderVersion > 100;
    LinkMismatchError linkError =
