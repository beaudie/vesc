...
Error Program::link(const ContextState &data)
{
    ...
    else
    {
        ...
        auto packMode =
            data.getExtensions().webglCompatibility ? PackMode::WEBGL_STRICT : ANGLE_RELAXED;
    ...
    const Program::MergedVaryings &mergedVaryings) const
{
    ...
    for (const auto &ref : mergedVaryings)
    {
        ...
        if ((input && output) || (output && output->isBuiltIn()))
        {
            ...
            ASSERT(!input || input->interpolation == output->interpolation);
            if (output->isStruct())
            {
                ...
                for (const auto &field : output->fields)
                {
                    ...
                    packedVaryings.push_back(
                        PackedVarying(field, output->interpolation, output->name));
                }
            }
            else
            {
