...
struct Program::LinkingState
{
    ...
    std::unique_ptr<ProgramLinkedResources> mResources;
    ProgramHash mProgramHash;
    MemoryProgramCache *mCache;
    std::unique_ptr<rx::LinkEvent> mLinkEvent;
};
...
Error Program::link(const gl::Context *context, angle::WorkerThreadPool *threadPool)
{
    ...
    mLinkingState->mContext     = context;
    mLinkingState->mProgramHash = programHash;
    mLinkingState->mCache       = cache;
    mLinkingState->mLinkEvent.reset(mProgram->link(context, threadPool, *resources, mInfoLog));
    mLinkingState->mResources = std::move(resources);
...
{
    return (mLinking && mLinkingState->mLinkEvent->isLinking());
}
...
void Program::resolveLink()
{
    ...
    mLinked  = mLinkingState->mLinkEvent->wait();
    mLinking = false;

    if (!mLinked)
    {
        mLinkingState.reset(nullptr);
        return;
    }
    ...
    if (mLinkingState->mCache &&
        (mState.mLinkedTransformFeedbackVaryings.empty() ||
         !mLinkingState->mContext->getWorkarounds().disableProgramCachingForTransformFeedback))
    {
        mLinkingState->mCache->putProgram(mLinkingState->mProgramHash, mLinkingState->mContext,
                                          this);
    }
    mLinkingState.reset(nullptr);
}
