...
bool Program::linkInterfaceBlocks(const Context *context, InfoLog &infoLog)
{
    ...
    if (!ValidateGraphicsInterfaceBlocks(vertexUniformBlocks, fragmentUniformBlocks,
                                         geometryUniformBlocks, infoLog, webglCompatibility))
    {
        return false;
    }
...
LinkMismatchError Program::LinkValidateVaryings(const sh::Varying &outputVarying,
                                                ...
                                                std::string *mismatchedStructFieldName)
{
    // Geometry shader cannot accept arrays of arrays as its input, so the generating shader outputs
    // cannot be arrays.
    if (isGeometryShaderInput && outputVarying.isArray())
    {
        return LinkMismatchError::ARRAY_SIZE_MISMATCH;
    }

    LinkMismatchError linkError = LinkValidateVariablesBase(
        outputVarying, inputVarying, false, !isGeometryShaderInput, mismatchedStructFieldName);
    if (linkError != LinkMismatchError::NO_MISMATCH)
    {
        return linkError;
    }
