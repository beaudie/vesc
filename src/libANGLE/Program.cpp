...
angle::Result Program::serialize(const Context *context, angle::MemoryBuffer *binaryOut) const
{
    ...
    if (context->getShareGroup()->getFrameCaptureShared()->enabled())
    {
        ...
        for (ShaderType shaderType : mState.mExecutable->getLinkedShaderStages())
        {
            ...
            {
                // If we dont have an attached shader, pull from our cached copy
                ...
                {
                    ERR() << "No source availabe for program=" << id().value
                          << " stage=" << shaderType;
                    UNREACHABLE();
                }
                                   ...
                                   InfoLog &infoLog)
{
    ...
    if (context->getShareGroup()->getFrameCaptureShared()->enabled())
    {
        ...
        for (ShaderType shaderType : mState.mExecutable->getLinkedShaderStages())
        {
            ...
            {
                ERR() << "No source for stage=" << shaderType
                      << " was read in program=" << id().value;
                UNREACHABLE();
            }
        }
