                          ...
                          const std::string &name)
{
    ...
    for (size_t location = 0u; location < locationList.size(); ++location)
    {
        ...
        if (variable.isArray() && variableLocation.arrayIndex == arrayIndex &&
            strncmp(variable.name.c_str(), name.c_str(), nameLengthWithoutArrayIndex) == 0 &&
    ...
    for (const auto &binding : mFragmentInputBindings)
    {
        ...
        for (const auto &in : inputs)
        {
            if (in.name.length() == nameLengthWithoutArrayIndex &&
                strncmp(in.name.c_str(), binding.first.c_str(), nameLengthWithoutArrayIndex) == 0)
            {
                if (in.isArray())
                {
                                        ...
                                        int blockIndex)
{
    for (const VarT &field : fields)
    {
        ...
        else
        {
            ...
            // TODO: We might need to split array of array fields into the innermost arrays.
