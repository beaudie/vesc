...
class Program::MainLinkLoadTask : public angle::Closure
{
  public:
    ...
    angle::Result getResult(const Context *context)
    {
        ...
        angle::Result result = getResultImpl(context);

        // Make sure post-link is called no matter what.
        ANGLE_TRY(mLinkTask->postLink(context, infoLog));
  ...
  protected:
    ...
    angle::Result getResultImpl(const Context *context)
    {
        InfoLog &infoLog = mState.getExecutable().getInfoLog();
