...
void UpdateMismatchedVariableFieldName(const std::string &parentName,
...
const char *GetLinkMismatchErrorString(LinkMismatchError linkError)
{
    switch (linkError)
    {
        case LINK_TYPE_MISMATCH:
            return "Type";
        case LINK_ARRAY_SIZE_MISMATCH:
            return "Array size";
        case LINK_PRECISION_MISMATCH:
            return "Precision";
        case LINK_STRUCT_NAME_MISMATCH:
            return "Structure name";
        case LINK_STRUCT_LENGTH_MISMATCH:
            return "Structure length";
        case LINK_FIELD_NAME_MISMATCH:
            return "Field name";

        case LINK_INTERPOLATION_TYPE_MISMATCH:
            return "Interpolation type";
        case LINK_INVARIANCE_MISMATCH:
            return "Invariance";

        case LINK_BINDING_MISMATCH:
            return "Binding layout qualifier";
        case LINK_LOCATION_MISMATCH:
            return "Location layout qualifier";
        case LINK_OFFSET_MISMATCH:
            return "Offset layout qualilfier";

        case LINK_LAYOUT_QUALIFIER_MISMATCH:
            return "Layout qualifier";
        case LINK_MATRIX_PACKING_MISMATCH:
    ...
    std::string *mismatchedBlockFieldName)
{
    ...
    {
        return LINK_FIELD_NAME_MISMATCH;
    }
    ...
    if (linkError != LINK_NO_ERROR)
    {
        UpdateMismatchedVariableFieldName(blockField1.name, mismatchedBlockFieldName);
        return linkError;
    }
    ...
    {
        UpdateMismatchedVariableFieldName(blockField1.name, mismatchedBlockFieldName);
        return LINK_MATRIX_PACKING_MISMATCH;
    }
                                                     ...
                                                     std::string *mismatchedStructOrBlockMemberName)
{
    ...
    {
        return LINK_PRECISION_MISMATCH;
    }
    if (variable1.structName != variable2.structName)
    {
        return LINK_STRUCT_NAME_MISMATCH;
    }

    if (variable1.fields.size() != variable2.fields.size())
    {
        return LINK_STRUCT_LENGTH_MISMATCH;
    }
    ...
    for (unsigned int memberIndex = 0; memberIndex < numMembers; memberIndex++)
    {
        ...
        if (linkErrorOnField != LINK_NO_ERROR)
        {
            UpdateMismatchedVariableFieldName(member1.name, mismatchedStructOrBlockMemberName);
