                                  ...
                                  GLuint combinedShaderStorageBlocksCount)
{
    ...
    // Here we assign locations for the output variables that don't yet have them. The variables are
    // processed in the order of descending array size. Each output variable is put to the first
    // slot where it fits. This does not guarantee the best end result but is close enough for our
    // purposes.
         ...
         outputVariableIndex++)
    {
        ...
        while (!elementsFit)
        {
            ...
            if (baseLocation + elementCount > maxLocation)
            {
                ...
                mInfoLog << "Could not fit variable into outputs: " << outputVariable.name;
                return false;
            }
            for (unsigned int elementIndex = 0; elementIndex < elementCount; elementIndex++)
            {
