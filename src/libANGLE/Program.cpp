...
bool Program::linkVaryings(const Context *context, InfoLog &infoLog) const
{
    ...
    for (const sh::Varying &inputVarying : inputVaryings)
    {
        bool matched = false;
        ...
        for (const sh::Varying &outputVarying : outputVaryings)
        {
            if (inputVarying.name == outputVarying.name)
            {
                ...
                LinkMismatchError linkError = LinkValidateVaryings(
                    outputVarying, inputVarying, generatingShader->getShaderVersion(context),
        ...
        {
            infoLog << GetShaderTypeString(consumingShader->getType()) << " varying "
                    << inputVarying.name << " does not match any "
                    << GetShaderTypeString(generatingShader->getType()) << " varying";
            return false;
        }
