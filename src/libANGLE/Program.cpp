...
Optional<GLuint> ProgramState::getSamplerIndex(GLint location) const
{
    ...
    return index - mSamplerUniformRange.start;
}
...
void Program::setUniform1iv(GLint location, GLsizei count, const GLint *v)
{
    ...
    const auto &uniformLoc     = mState.mUniformLocations[location];
    const auto &linkedUniform  = mState.mUniforms[uniformLoc.index];
    const uint8_t *destPointer = linkedUniform.getDataPtrToElement(uniformLoc.element);

    // Invalidate the validation cache only if we modify the sampler data.
    if (mState.isSamplerUniformIndex(uniformLoc.index) &&
        memcmp(destPointer, v, sizeof(GLint) * clampedCount) != 0)
    {
        ASSERT(linkedUniform.staticUse);
        GLuint samplerIndex = uniformLoc.index;
        std::vector<GLuint> &boundTextureUnits =
            mState.mSamplerBindings[samplerIndex].boundTextureUnits;

        std::copy(v, v + clampedCount, boundTextureUnits.begin() + uniformLoc.element);
