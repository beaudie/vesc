...
{
  public:
    ScopedBindDrawFramebuffer(Context *context, FramebufferID id)
        : mANGLE(context), mSavedFramebuffer(mANGLE->getState().getDrawFramebuffer())
    {
...
class ScopedDisableScissor
{
  public:
    ScopedDisableScissor(Context *context)
        : mANGLE(context), mScissorTestEnabled(mANGLE->getState().isScissorTestEnabled())
    {
...
class PixelLocalStorageImageLoadStore : public PixelLocalStorage
{
  public:
                 ...
                 Extents plsExtents) override
    {
        ...
        ScopedBindDrawFramebuffer scopedBindDrawFramebuffer(context,
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            {
                plane.attachToDrawFramebuffer(context, plsExtents, GL_COLOR_ATTACHMENT0);
                plane.performLoadOperationClear(context, 0, loadop, cleardata + i * 4 * 4);
                didClear = true;
            }
        ...
        // ALL_BARRIER_BITS because the textures could have been used in any way before this.
    ...
    void onEnd(Context *context, GLsizei numActivePLSPlanes) override
    {
        ...
        // ALL_BARRIER_BITS because the textures can be used in any way after this.
        context->memoryBarrier(GL_ALL_BARRIER_BITS);
    }
...
class PixelLocalStorageFramebufferFetch : public PixelLocalStorage
{
  public:
                 ...
                 Extents plsExtents) override
    {
        ...
        int numAppDrawBuffers  = std::min((int)appDrawBuffers.size(), firstPLSDrawBuffer);
    ...
    void onEnd(Context *context, GLint numActivePLSPlanes) override
    {
        ...
        {
            context->invalidateFramebuffer(GL_DRAW_FRAMEBUFFER, (GLsizei)mInvalidateList.size(),
        ...
        context->drawBuffers((GLsizei)mSavedDrawBuffers.size(), mSavedDrawBuffers.data());
