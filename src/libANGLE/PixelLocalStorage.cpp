...
void PixelLocalStoragePlane::onFramebufferDestroyed(const Context *angle)
{
    if (mTextureRef)
    {
...
void PixelLocalStorage::onFramebufferDestroyed(const Context *angle)
{
    if (angle->getRefCount() == 0)
    {
    ...
    for (PixelLocalStoragePlane &plane : mPlanes)
    {
        plane.onFramebufferDestroyed(angle);
    }
}
...
void PixelLocalStorage::barrier(Context *angle)
{
...
{
  public:
    PixelLocalStorageImageLoadStore(Context *angle, bool needsR32Packing)
                 ...
                 Extents plsExtents) override
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            plane.bindToImage(angle, plsExtents, i, mNeedsR32Packing);
        }
        ...
        angle->memoryBarrier(GL_ALL_BARRIER_BITS);
    }

    void onEnd(Context *angle, GLsizei numActivePLSPlanes) override
    {
    ...
    void onBarrier(Context *angle) override
    {
...
class PixelLocalStorageFramebufferFetch : public PixelLocalStorage
{
  public:
    ...
    void onBarrier(Context *angle) override { angle->framebufferFetchBarrier(); }
