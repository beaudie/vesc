...
PixelLocalStoragePlane::PixelLocalStoragePlane(PixelLocalStoragePlane &&plane)
{
    ...
    memset(&plane, 0, sizeof(PixelLocalStoragePlane));
}
...
void PixelLocalStoragePlane::deinitialize(Context *angle)
{
    ...
    if (mMemorylessTextureID.value)
    {
        ...
        // calling glDeleteTextures, but it seems unnecessary to worry about that case.
        // This also shouldn't be a problem in WebGL.
...
void PixelLocalStoragePlane::convertToMemorylessIfTextureIDDeleted(const Context *angle)
{
    if (!isDeinitialized() && isTextureIDDeleted(angle))
    {
        ASSERT(mTextureRef);
        mMemoryless = true;
        // mMemorylessTextureID could be nonzero here if the app deleted it by guessing the ID and
        // calling glDeleteTextures! In that case, we can just drop it on the floor.
        mMemorylessTextureID = TextureID();
        mTextureImageIndex   = ImageIndex::MakeFromType(TextureType::_2D, 0, 0);
        mTextureRef->release(angle);
        mTextureRef = nullptr;
...
void PixelLocalStoragePlane::ensureBackingIfMemoryless(Context *angle, Extents plsExtents)
{
        ...
        (GLsizei)mTextureRef->getHeight(TextureTarget::_2D, 0) != plsExtents.height)
    {
        ...
        GLenum internalformat = mInternalformat;
        deinitialize(angle);
        setMemoryless(angle, internalformat);
        ASSERT(!mTextureRef);
        ASSERT(!mMemorylessTextureID.value);
