...
{
    ASSERT(!isDeinitialized());
    ASSERT(mTextureRef != nullptr);      // Call ensureBackingIfMemoryless() first!
...
PixelLocalStorage::PixelLocalStorage(bool memorylessNeedsBackingTexture)
    : mMemorylessNeedsBackingTexture(memorylessNeedsBackingTexture)
{}
...
void PixelLocalStorage::begin(Context *context, GLsizei n, const GLenum loadops[])
{
    ...
    for (GLsizei i = 0; i < n; ++i)
    {
        ...
        PixelLocalStoragePlane &plane = mPlanes[i];
        if (mMemorylessNeedsBackingTexture)
        {
            plane.ensureBackingIfMemoryless(context, plsExtents);
        }
...
class PixelLocalStorageImageLoadStore : public PixelLocalStorage
{
  public:
    PixelLocalStorageImageLoadStore(bool needsR32Packing)
        : PixelLocalStorage(true /*memorylessNeedsBackingTexture*/),
...
{
  public:
    PixelLocalStorageFramebufferFetch() : PixelLocalStorage(true /*memorylessNeedsBackingTexture*/)
    {}
...
{
  public:
    PixelLocalStorageEXT() : PixelLocalStorage(false /*memorylessNeedsBackingTexture*/) {}
