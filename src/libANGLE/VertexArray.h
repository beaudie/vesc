...
class VertexArrayState final : public angle::NonCopyable
{
  public:
    ...
    size_t getMaxBindings() const { return mAttributeBindings.size(); }
    ...
    const std::vector<AttributeBinding> &getBindings() const { return mAttributeBindings; }
    const AttributeBinding &getBinding(size_t bindingIndex) const
    {
        return mAttributeBindings[bindingIndex];
    }
    size_t getBindingIndexByAttribIndex(size_t attribIndex) const
    {
        size_t bindingIndex = mVertexAttributes[attribIndex].binding - &mAttributeBindings[0];
        ASSERT(bindingIndex < mAttributeBindings.size());
        return bindingIndex;
    };
...
class VertexArray final : public LabeledObject
{
  public:
    ...
    const std::vector<AttributeBinding> &getBindings() const { return mState.getBindings(); }
