...
class VertexArrayBufferContentsObservers final : angle::NonCopyable
{
  public:
    ...
    bool any() const { return mNeedsObserveBufferContentChange.any(); }

  private:
    VertexArray *mVertexArray;
    // Bit is set when it is observing the buffer content change
    gl::AttributesMask mNeedsObserveBufferContentChange;
};
                          ...
                          public angle::Subject
{
  public:
    ...
    enum DirtyBitType
    {
        // This vertex array were not observing changes inside buffer while becomes not current.
        DIRTY_BIT_OBSERVER,
    ...
    static_assert(DIRTY_BIT_OBSERVER < DIRTY_BIT_BINDING_0,
                  "OBSERVER dirty bits should come before BINDING.");
