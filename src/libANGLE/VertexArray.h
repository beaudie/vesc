...
class VertexArrayState final : public angle::NonCopyable
{
  public:
    ...
    size_t getMaxVertexAttribs() const { return mVertexAttribs.size(); }
    size_t getMaxAttribBindings() const { return mAttribBindings.size(); }
    size_t getMaxEnabledAttribute() const { return mMaxEnabledAttribute; }
    const std::vector<VertexAttribute> &getVertexAttributes() const { return mVertexAttribs; }
    const VertexAttribute &getVertexAttribute(size_t attribIndex) const
    {
        return mVertexAttribs[attribIndex];
    }
    const std::vector<AttributeBinding> &getAttribBindings() const { return mAttribBindings; }
    const AttributeBinding &getAttribBinding(size_t bindingIndex) const
    {
        return mAttribBindings[bindingIndex];
    }
  ...
  private:
    ...
    std::vector<AttributeBinding> mAttribBindings;
...
class VertexArray final : public LabeledObject
{
  public:
    ...
    size_t getMaxVertexAttribs() const { return mState.getMaxVertexAttribs(); }
    size_t getMaxAttribBindings() const { return mState.getMaxAttribBindings(); }

    const std::vector<VertexAttribute> &getVertexAttributes() const
    {
        return mState.getVertexAttributes();
    }
    const std::vector<AttributeBinding> &getAttribBindings() const
    {
        return mState.getAttribBindings();
    }
