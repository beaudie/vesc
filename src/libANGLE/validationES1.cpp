...
bool ValidateAlphaFuncCommon(Context *context, GLenum func)
{
    switch (func) {
    case GL_NEVER:
    case GL_ALWAYS:
    ...
    case GL_NOTEQUAL:
        return true;
    default:
        context->handleError(gl::InvalidEnum() << "Invalid glAlphaFunc function.");
        return false;
    }
}
...
bool ValidateClipPlaneCommon(Context *context, GLenum plane)
{
    if (plane < GL_CLIP_PLANE0 ||
        plane > GL_CLIP_PLANE0 + context->getCaps().maxClipPlanes - 1)
    {
...
bool ValidateGLES1PointerBase(Context* context, GLenum arrayType,
                              GLint size, GLenum type, GLsizei stride, const void* pointer) {
    if (!context->gles1Emu) {
        context->handleError(gl::InvalidOperation() << "GLES1 only");
        return false;
    }

    return gl::ValidateVertexAttribPointer(
            context, context->gles1Emu->vertexArrayIndex(arrayType),
...
bool ValidateClientActiveTexture(Context *context, GLenum texture)
{
    ...
    {
        context->handleError(InvalidEnum() << "Invalid |texture| unit specified.");
        return false;
    }
...
bool ValidateColorPointer(Context *context,
                          ...
                          const void *pointer)
{
    return ValidateGLES1PointerBase(context, GL_COLOR_ARRAY, size, type, stride, pointer);
}
...
{
    return ValidateGLES1PointerBase(context, GL_NORMAL_ARRAY, 3, type, stride, pointer);
}
...
bool ValidateTexCoordPointer(Context *context,
                             ...
                             const void *pointer)
{
    return ValidateGLES1PointerBase(context, GL_TEXTURE_COORD_ARRAY, size, type, stride, pointer);
}
...
bool ValidateVertexPointer(Context *context,
                           ...
                           const void *pointer)
{
    return ValidateGLES1PointerBase(context, GL_VERTEX_ARRAY, size, type, stride, pointer);
}
