...
void JsonSerializer::addEntry(const std::string &name, bool value)
{
    rapidjson::Value tag(name.c_str(), mAllocator);
    mGroupValueStack.top()->AddMember(tag, value, mAllocator);
}
...
void JsonSerializer::addEntry(const std::string &name, const uint8_t *blob, size_t length)
{
    rapidjson::Value tag(name.c_str(), mAllocator);
