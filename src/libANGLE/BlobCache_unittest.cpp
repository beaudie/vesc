...
TEST(BlobCacheTest, MaxSizedValue)
{
    ...
    Blob blob;
    EXPECT_FALSE(blobCache.get(MakeKey(0), &blob, nullptr));
...
TEST(BlobCacheTest, ManySmallValues)
{
    ...
    for (size_t value = 0; value < kSize; ++value)
    {
        ...
        Blob qvalue;
        EXPECT_TRUE(blobCache.get(MakeKey(value), &qvalue, nullptr));
        if (qvalue.size() > 0)
        {
    ...
    Blob qvalue;
    EXPECT_FALSE(blobCache.get(MakeKey(0), &qvalue, nullptr));
    ...
    for (size_t value = 0; value <= kSize; ++value)
    {
        EXPECT_FALSE(blobCache.get(MakeKey(value), &qvalue, nullptr));
    }
    EXPECT_TRUE(blobCache.get(MakeKey(kSize + 1), &qvalue, nullptr));
    if (qvalue.size() > 0)
    {
...
TEST(BlobCacheTest, OversizeValue)
{
    ...
    Blob qvalue;
    EXPECT_FALSE(blobCache.get(MakeKey(5), &qvalue, nullptr));
}
