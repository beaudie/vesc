...
TEST(BlobCacheTest, MaxSizedValue)
{
    ...
    blobCache.put(nullptr, MakeKey(0), MakeBlob(kSize));
    EXPECT_EQ(32u, blobCache.size());
    EXPECT_FALSE(blobCache.empty());

    blobCache.put(nullptr, MakeKey(1), MakeBlob(kSize));
    EXPECT_EQ(32u, blobCache.size());
    EXPECT_FALSE(blobCache.empty());
...
TEST(BlobCacheTest, ManySmallValues)
{
    ...
    {
        blobCache.put(nullptr, MakeKey(value), MakeBlob(1, value));
    ...
    blobCache.put(nullptr, MakeKey(kSize), MakeBlob(1, kSize));

    const Blob *qvalue;
    EXPECT_FALSE(blobCache.get(nullptr, MakeKey(0), &qvalue));

    // Putting one large element cleans out the whole stack.
    blobCache.put(nullptr, MakeKey(kSize + 1), MakeBlob(kSize, kSize + 1));
    EXPECT_EQ(32u, blobCache.size());
    EXPECT_FALSE(blobCache.empty());
...
TEST(BlobCacheTest, OversizeValue)
{
    ...
    blobCache.put(nullptr, MakeKey(5), MakeBlob(100));
