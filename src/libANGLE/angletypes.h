...
class BlendStateExt final
{
    ...
    struct StorageType final
    {
        ...
        static constexpr Type Mask(const size_t drawBuffers)
        {
            ASSERT(drawBuffers <= IMPLEMENTATION_MAX_DRAW_BUFFERS);
            return static_cast<Type>(0xFFFFFFFFFFFFFFFFull >> (64 - drawBuffers * Bits));
        }
        ...
        // packed values.
        static constexpr ElementType GetValueIndexed(const Type values, const size_t index)
        {
            ASSERT(index < IMPLEMENTATION_MAX_DRAW_BUFFERS);

            return static_cast<ElementType>((values >> (index * Bits)) & MaxValueMask);
        }
        ...
        static constexpr void SetValueIndexed(Type &target,
