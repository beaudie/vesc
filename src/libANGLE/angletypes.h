...
// ShaderArray can contain one item per shader.  It differs from ShaderMap in that the values are
// not indexed by ShaderType.
template <typename T>
using ShaderArray = std::array<T, static_cast<size_t>(ShaderType::EnumCount)>;
