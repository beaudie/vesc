    ...
    for (const sh::Uniform &fragmentUniform : fragmentUniforms)
    {
        ...
        if (entry != linkedUniforms.end())
        {
            LinkMismatchError linkError =
                LinkValidateUniforms(*(entry->second), fragmentUniform, &mismatchedStructFieldName);
            if (linkError != LinkMismatchError::LINK_NO_ERROR)
            {
                infoLog.logLinkMismatch(fragmentUniform.name, "uniform", linkError,
                                        mismatchedStructFieldName, GL_VERTEX_SHADER,
                                        GL_FRAGMENT_SHADER);
                return false;
            }
        }
    ...
    for (const sh::Uniform &geometryUniform : *geometryUniforms)
    {
        ...
        if (entry != linkedUniforms.end())
        {
            ...
            if (linkError != LinkMismatchError::LINK_NO_ERROR)
            {
                ...
                infoLog.logLinkMismatch(geometryUniform.name, "uniform", linkError,
                                                      ...
                                                      std::string *mismatchedStructFieldName)
{
    ...
    LinkMismatchError linkError = Program::LinkValidateVariablesBase(
        uniform1, uniform2, validatePrecision, false, mismatchedStructFieldName);
    if (linkError != LinkMismatchError::LINK_NO_ERROR)
    {
        return linkError;
    }
    ...
    {
        return LinkMismatchError::LINK_BINDING_MISMATCH;
    }
    ...
    {
        return LinkMismatchError::LINK_LOCATION_MISMATCH;
    }
    if (uniform1.offset != uniform2.offset)
    {
        return LinkMismatchError::LINK_OFFSET_MISMATCH;
    }

    return LinkMismatchError::LINK_NO_ERROR;
}
