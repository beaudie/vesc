...
UniformLinker::UniformLinker(const ShaderBitSet &linkedShaderStages,
                             const ShaderMap<std::vector<sh::ShaderVariable>> &shaderUniforms)
    : mLinkedShaderStages(linkedShaderStages), mShaderUniforms(shaderUniforms)
{}
...
{
    if (mLinkedShaderStages[ShaderType::Vertex] && mLinkedShaderStages[ShaderType::Fragment])
    {
        ASSERT(!mLinkedShaderStages[ShaderType::Compute]);
...
bool UniformLinker::validateGraphicsUniforms(InfoLog &infoLog) const
{
    ...
    for (const ShaderType shaderType : mLinkedShaderStages)
    {
...
{
    ASSERT(mLinkedShaderStages[shaderToLink] && linkedUniforms);
