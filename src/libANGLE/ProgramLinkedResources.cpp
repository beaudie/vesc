...
class ShaderStorageBlockVisitor : public sh::VariableNameVisitor
{
  public:
    ...
    void enterArray(const sh::ShaderVariable &arrayVar) override
    {
        if (mStructStackSize == 0 && !arrayVar.hasParentArrayIndex())
        {
            mTopLevelArraySize = arrayVar.getOutermostArraySize();
        }
        sh::VariableNameVisitor::enterArray(arrayVar);
    }

    void exitArray(const sh::ShaderVariable &arrayVar) override
    {
        if (mStructStackSize == 0 && !arrayVar.hasParentArrayIndex())
        {
            mTopLevelArraySize = 1;
        }
        sh::VariableNameVisitor::exitArray(arrayVar);
    }
