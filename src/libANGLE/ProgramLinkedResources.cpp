...
LinkingVariables::LinkingVariables(const ProgramState &state)
{
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        if (shader)
        {
            linkingOutputVaryings[shaderType] = shader->getOutputVaryings();
            linkingInputVaryings[shaderType]  = shader->getInputVaryings();
            linkingUniforms[shaderType]       = shader->getUniforms();
            linkingUniformBlocks[shaderType]  = shader->getUniformBlocks();
                                    ...
                                    const LinkingVariables &linkingVariables)
{
    ...
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        const std::vector<sh::ShaderVariable> &uniforms =
            linkingVariables.linkingUniforms[shaderType];
        ...
        const std::vector<sh::InterfaceBlock> &uniformBlocks =
            linkingVariables.linkingUniformBlocks[shaderType];
    ...
    if (linkingVariables.isShaderStageUsedBitset[ShaderType::Vertex])
    {
        ...
        for (const sh::ShaderVariable &uniform :
             linkingVariables.linkingUniforms[ShaderType::Vertex])
        {
