...
void LinkingVariables::savePendingLinkInfo(const ProgramState &state)
{
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        if (shader)
        {
            ...
            mIsShaderStageUsedBitset.set(shaderType);
        }
    }
}

void LinkingVariables::savePendingLinkInfo(const ProgramPipelineState &state,
                                           const ProgramExecutable &programExecutable)
{
    for (ShaderType shaderType : programExecutable.getLinkedShaderStages())
    {
        const Program *program = state.getShaderProgram(shaderType);
        ASSERT(program);
        mPendingLinkOutputVaryings[shaderType] =
            program->getExecutable().getLinkedOutputVaryings(shaderType);
        mPendingLinkInputVaryings[shaderType] =
            program->getExecutable().getLinkedInputVaryings(shaderType);
                                    ...
                                    const LinkingVariables &linkingVariables)
{
    ...
    for (ShaderType shaderType : kAllGraphicsShaderTypes)
    {
        ...
        const std::vector<sh::ShaderVariable> &uniforms =
            linkingVariables.mPendingLinkUniforms[shaderType];
        ...
        const std::vector<sh::InterfaceBlock> &uniformBlocks =
            linkingVariables.mPendingLinkUniformBlocks[shaderType];
    ...
    if (linkingVariables.mIsShaderStageUsedBitset[ShaderType::Vertex])
    {
        ...
        for (const sh::ShaderVariable &uniform :
             linkingVariables.mPendingLinkUniforms[ShaderType::Vertex])
        {
