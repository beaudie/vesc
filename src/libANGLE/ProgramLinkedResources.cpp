    ...
    for (const sh::Uniform &fragmentUniform : fragmentUniforms)
    {
        ...
        if (entry != linkedUniforms.end())
        {
            if (!LinkValidateUniforms(*(entry->second), fragmentUniform, &mismatchedItem,
                                      &mismatchedFieldName))
            {
                infoLog.logLinkMismatch(fragmentUniform.name, "uniform", mismatchedItem,
                                        mismatchedFieldName, GL_VERTEX_SHADER, GL_FRAGMENT_SHADER);
                return false;
            }
        }
    ...
    for (const sh::Uniform &geometryUniform : *geometryUniforms)
    {
        ...
        if (entry != linkedUniforms.end())
        {
                                      ...
                                      &mismatchedFieldName))
            {
                ...
                infoLog.logLinkMismatch(geometryUniform.name, "uniform", mismatchedItem,
                                        mismatchedFieldName, mismatchedShaderType,
                                         ...
                                         std::string *mismatchedFieldName)
{
    ...
    {
        *mismatchedItem = "Binding layout qualifiers";
        return false;
    }
    ...
    {
        *mismatchedItem = "Location layout qualifiers";
        return false;
    }
    if (uniform1.offset != uniform2.offset)
    {
        *mismatchedItem = "Offset layout qualifiers";
        return false;
    }
