...
{
    for (auto &uniform : *mUniformsOut)
    {
        if (uniform.name == fullName)
        {
            uniform.setStaticUse(shaderType, staticUse);
            return;
        }
    }
}
...
void ShaderStorageBlockLinker::updateBlockMemberStaticUsedImpl(const std::string &fullName,
                                                               ...
                                                               bool staticUse) const
{
    for (auto &bufferVariable : *mBufferVariablesOut)
    {
        if (bufferVariable.name == fullName)
        {
            bufferVariable.setStaticUse(shaderType, staticUse);
            return;
        }
    }
}
