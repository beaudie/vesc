...
bool UniformLinker::validateGraphicsUniforms(const Context *context, InfoLog &infoLog) const
{
    ...
    for (const sh::Uniform &fragmentUniform : fragmentUniforms)
    {
        ...
        {
            if (!LinkValidateUniforms(*(entry->second), fragmentUniform, mismatchedItem,
                                      mismatchedFields))
            {
                infoLog.logLinkMismatch(fragmentUniform.name, "uniform", mismatchedItem.get(),
                                        mismatchedFields.get(), GL_VERTEX_SHADER,
                                        GL_FRAGMENT_SHADER);
                return false;
            }
        }
    ...
    for (const sh::Uniform &geometryUniform : *geometryUniforms)
    {
        ...
        if (entry != linkedUniforms.end())
        {
            if (!LinkValidateUniforms(*(entry->second), geometryUniform, mismatchedItem,
                                      mismatchedFields))
            {
                ...
                infoLog.logLinkMismatch(geometryUniform.name, "uniform", mismatchedItem.get(),
                                        mismatchedFields.get(), mismatchedShaderType,
...
bool UniformLinker::LinkValidateUniforms(const sh::Uniform &uniform1,
                                         ...
                                         std::unique_ptr<std::string> &mismatchedFields)
{
    ...
    if (!Program::LinkValidateVariablesBase(uniform1, uniform2, validatePrecision, false,
                                            mismatchedItem, mismatchedFields))
    {
        return false;
    }
    ...
    {
        mismatchedItem.reset(new std::string("Binding layout qualifiers"));
        return false;
    }
    ...
    {
        mismatchedItem.reset(new std::string("Location layout qualifiers"));
        return false;
    }
    if (uniform1.offset != uniform2.offset)
    {
        mismatchedItem.reset(new std::string("Offset layout qualifiers"));
        return false;
    }
