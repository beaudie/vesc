...
SurfaceState::SurfaceState(const egl::Config *configIn, const AttributeMap &attributesIn)
    : label(nullptr),
      config(new egl::Config(*configIn)),
...
{
    // Create a temp config to account for colorspace override of render target format
    egl::Config overrideConfig = *(mState.config);

    // To account for color space differences, override the config with the
    // non-linear format. If no suitable non-linear format was found, return
    // EGL_BAD_MATCH error
    if (!gl::ColorspaceFormatOverride(mGLColorspace, overrideConfig.renderTargetFormat))
        return egl::EglBadMatch();

    // If an override is required update mState.config as well
    if (mState.config->renderTargetFormat != overrideConfig.renderTargetFormat)
    {
        delete mState.config;
        mState.config = new egl::Config(overrideConfig);
