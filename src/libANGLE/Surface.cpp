...
bool Surface::lockedSurface() const
{
...
egl::Error Surface::lockSurfaceKHR(const egl::Display *display, const AttributeMap &attributes)
{
    ...
    if ((getConfig()->surfaceType & EGL_LOCK_SURFACE_BIT_KHR) == false)
    {
        return egl::EglBadAccess();
    }

    if (currentToAContext())
    {
        return egl::EglBadAccess() << "Surface cannot be current to a context for eglLockSurface()";
    }

    if (hasProtectedContent())
    {
        return egl::EglBadAccess() << "Surface cannot be protected content for eglLockSurface()";
    }
    ...
    egl::Error error = mImplementation->lockSurface(display, lockBufferUsageHint, preservePixels,
...
egl::Error Surface::unlockSurfaceKHR(const egl::Display *display)
{
    if (!lockedSurface())
    {
        return egl::EglBadAccess() << "Surface is not locked.";
    }

    bool preservePixels = true;
    egl::Error error    = mImplementation->unlockSurface(display, preservePixels);
    mLockBufferPtr      = nullptr;
    mLockBufferPitch    = 0;
