...
{
    if (mRobustResourceInitialization && mState.mSwapBehavior != EGL_BUFFER_PRESERVED)
    {
        mInitState = gl::InitState::MayNeedInit;
...
Error Surface::initialize(const Display *display)
{
    ...
    mState.mSwapBehavior = mImplementation->getSwapBehavior();
...
Error Surface::getBufferAge(const gl::Context *context, EGLint *age) const
{
    ...
    if (mState.mSwapBehavior == EGL_BUFFER_PRESERVED)
    {
        if (age != nullptr)
        {
            *age = 1;
    ...
    else if (mState.mSwapBehavior == EGL_BUFFER_DESTROYED)
    {
        if (age != nullptr)
        {
            *age = 0;
...
egl::Error Surface::lockSurfaceKHR(const egl::Display *display, const AttributeMap &attributes)
{
    ...
    bool preservePixels = ((attributes.getAsInt(EGL_MAP_PRESERVE_PIXELS_KHR, false) == EGL_TRUE) ||
                           (mState.mSwapBehavior == EGL_BUFFER_PRESERVED));
