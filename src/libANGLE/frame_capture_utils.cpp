...
{
    SerializeGLGlobalStates(bos, context->getState());
...
void SerializeGLGlobalStates(gl::BinaryOutputStream *bos, const gl::State &state)
{
    ...
    {
        SerializeTransformFeedback(bos, state.getCurrentTransformFeedback());
    }
    ...
    const gl::VertexAttribCurrentValueData &vertexAttribCurrentValueData)
{
    ...
    else if (vertexAttribCurrentValueData.Type == gl::VertexAttribType::UnsignedInt)
    {
...
{
    bos->writeString(transformFeedback->getLabel());
    bos->writeInt(transformFeedback->isActive());
    bos->writeEnum(transformFeedback->getPrimitiveMode());
    bos->writeInt(transformFeedback->isPaused());
    bos->writeInt(transformFeedback->getVerticesDrawn());
    bos->writeInt(transformFeedback->getVertexCapacity());
    if (transformFeedback->getBoundProgram())
    {
        bos->writeInt(transformFeedback->getBoundProgram()->id().value);
    }
    SerializeOffsetBindingPointerVector<gl::Buffer>(bos, transformFeedback->getIndexedBuffers());
}
