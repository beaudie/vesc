...
template void SerializeRange<unsigned int>(gl::BinaryOutputStream *bos,
                                           const gl::Range<unsigned int> &range);
...
void SerializeShaderVariable(gl::BinaryOutputStream *bos, const sh::ShaderVariable &shaderVariable)
{
    ...
    for (unsigned int arraySize : shaderVariable.arraySizes)
    {
        bos->writeInt(arraySize);
    }
