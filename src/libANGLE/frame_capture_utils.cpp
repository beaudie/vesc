                                ...
                                MemoryBuffer **pixels)
{
    ...
    if (binding == GL_BACK || binding == GL_NONE ||
        (binding >= GL_COLOR_ATTACHMENT0 ||
         (binding - GL_COLOR_ATTACHMENT0) < framebuffer->getState().getColorAttachments().size()))
    {
        format =
            framebuffer->getImplementation()->getImplementationColorReadFormat(context);
    }
    ANGLE_CHECK_GL_ALLOC(
        context, scratchBuffer->getInitialized(
    ...
    ANGLE_TRY(framebuffer->readPixels(context, gl::Rectangle{0, 0, extents.width, extents.height},
                                      ...
                                      gl::PixelPackState{}, nullptr, (*pixels)->data()));
