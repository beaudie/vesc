...
Caps GenerateMinimumCaps(const Version &clientVersion, const Extensions &extensions)
{
    ...
    if (clientVersion >= Version(3, 0))
    {
        ...
        for (ShaderType shaderType : AllGLES2ShaderTypes())
        {
            caps.maxCombinedShaderUniformComponents[shaderType] =
                caps.maxShaderUniformBlocks[shaderType] * (caps.maxUniformBlockSize / 4) +
    ...
    if (extensions.geometryShader)
    {
        ...
        caps.maxCombinedShaderUniformComponents[ShaderType::Geometry] =
            caps.maxShaderUniformBlocks[ShaderType::Geometry] *
                static_cast<GLuint>(caps.maxUniformBlockSize / 4) +
            caps.maxShaderUniformComponents[ShaderType::Geometry];
