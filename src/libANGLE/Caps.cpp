                                       ...
                                       const Extensions &extensions)
{
    ...
    caps.renderbufferAttachment =
        internalFormatInfo.renderbufferAttachmentSupport(clientVersion, extensions);
...
static bool GetFormatSupportBase(const TextureCapsMap &textureCaps,
                                 ...
                                 bool requiresRenderbufferAttaching)
{
    for (size_t i = 0; i < requiredFormatsSize; i++)
    {
        ...
        if (requiresFramebufferAttaching && !cap.textureAttachment)
        {
            return false;
        }

        if (requiresRenderbufferAttaching && !cap.renderbufferAttachment)
        {
            return false;
        }
    }
