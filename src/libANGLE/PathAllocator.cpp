...
const GLuint PathAllocator::kInvalidPath = 0;

PathAllocator::PathAllocator()
{
    // Simplify the code by making sure that lower_bound(id) never
    // returns the beginning of the map, if id is valid (eg != kInvalidPath).
    mUsed.insert(std::make_pair(0u, 0u));
}

GLuint PathAllocator::allocate()
{
    return allocateRange(1u);
}

GLuint PathAllocator::allocateAtOrAbove(GLuint wanted)
{
...
GLuint PathAllocator::allocateRange(GLuint range)
{
    ASSERT(range != 0);
...
bool PathAllocator::markAsUsed(GLuint handle)
{
    ASSERT(handle);
...
void PathAllocator::release(GLuint handle)
{
   releaseRange(handle, 1u);
}

void PathAllocator::releaseRange(GLuint first, GLuint range)
{
...
bool PathAllocator::isUsed(GLuint handle) const
{
    if (handle == kInvalidPath)
        return false;
