        ...
        threads[i] = std::thread([&, i]() {
            ...
            for (size_t j = 0; j < iterations; ++j)
            {
                ...
                if (expectContains)
                {
                    ...
                    const LockedType queryResultInt = static_cast<LockedType>(
                        reinterpret_cast<size_t>(queryResult) & 0xFFFF'FFFF);
                    const size_t queryResultIteration = reinterpret_cast<size_t>(queryResult) >> 32;
