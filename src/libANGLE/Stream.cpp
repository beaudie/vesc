...
Error Stream::consumerAcquire()
{
    ...
    if (mPlanes[0].texture != nullptr)
    {
        mPlanes[0].texture->acquireImageFromStream(
            mProducerImplementation->getGLFrameDescription(0));
    }
    if (mPlanes[1].texture != nullptr)
    {
        mPlanes[1].texture->acquireImageFromStream(
            mProducerImplementation->getGLFrameDescription(1));
    }
...
Error Stream::validateD3D11NV12Texture(void *texture) const
{
    ...
    return mProducerImplementation->validateFramePointer(texture);
}
