...
{
    if (name.empty() || mInfo.builtInKernels.empty())
    {
        return false;
    }
    ...
    do
    {
        std::string::size_type end = mInfo.builtInKernels.find(';', start);
        if (end == std::string::npos)
        {
            end = mInfo.builtInKernels.length();
        }
        const std::string::size_type length = end - start;
        if (length == name.length() && mInfo.builtInKernels.compare(start, length, name) == 0)
        {
            return true;
        }
        start = end + 1u;
    } while (start < mInfo.builtInKernels.size());
    return false;
}
