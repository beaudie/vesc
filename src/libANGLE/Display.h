...
using ContextMap = angle::HashMap<gl::ContextID, gl::Context *>;
using SurfaceMap = angle::HashMap<SurfaceID, Surface *>;
                      ...
                      public angle::NonCopyable
{
  ...
  private:
    ...
    typedef angle::HashMap<ImageID, Image *> ImageMap;
    ImageMap mImageMap;

    typedef angle::HashSet<Stream *> StreamSet;
    StreamSet mStreamSet;

    typedef angle::HashMap<SyncID, Sync *> SyncMap;
    SyncMap mSyncMap;

    void destroyImageImpl(ImageID id, ImageMap *images);
    void destroyStreamImpl(Stream *stream, StreamSet *streams);
    Error destroySurfaceImpl(Surface *surface, SurfaceMap *surfaces);
    void destroySyncImpl(SyncID id, SyncMap *syncs);
