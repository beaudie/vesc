...
class ImageSibling : public gl::FramebufferAttachmentObject
{
  ...
  private:
    ...
    std::set<Image*> mSources;
    BindingPointer<Image> mTarget;
};
...
class Image final : public RefCountObject
{
  ...
  private:
    ...
    // Called from ImageSibling only notify the image that a new target sibling exists for state tracking.
    ...
    void orphanSibling(ImageSibling *sibling);
