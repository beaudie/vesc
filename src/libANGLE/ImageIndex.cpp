...
GLint ImageIndex::cubeMapFaceIndex() const
{
    ...
    ASSERT(mLayerIndex == kEntireLevel || mLayerIndex < 6u);
    return static_cast<GLint>(CubeFaceIndexToTextureTarget(mLayerIndex));
}
...
ImageIndexIterator ImageIndexIterator::MakeGeneric(TextureType type, GLint minMip, GLint maxMip)
{
    ...
    return ImageIndexIterator(type, Range<GLint>(minMip, maxMip),
                              Range<GLint>(ImageIndex::kEntireLevel, ImageIndex::kEntireLevel),
                              nullptr);
}
...
ImageIndex ImageIndexIterator::next()
{
    ...
    angle::EnumIterator<TextureTarget> currentTarget = mCurrentIndex.getTarget();
