...
{
    return std::tie(a.type, a.target, a.level, a.layer, a.layerCount) <
           std::tie(b.type, b.target, b.level, b.layer, b.layerCount);
}
...
ImageIndex::ImageIndex(GLenum textureType,
                       GLenum textureTarget,
                       GLint level,
                       GLint layer,
                       GLint layerCount)
    : type(textureType), target(textureTarget), level(level), layer(layer), layerCount(layerCount)
{}
...
ImageIndexIterator::ImageIndexIterator(GLenum textureType,
                                       const Range<GLenum> &textureTargetRange,
                                       const Range<GLint> &mipRange,
                                       const Range<GLint> &layerRange,
                                       const GLsizei *layerCounts)
    : mTextureTargetRange(textureTargetRange),
      mMipRange(mipRange),
      mLayerRange(layerRange),
      mLayerCounts(layerCounts),
      mCurrentIndex(textureType, textureTargetRange.low(), mipRange.low(), layerRange.low(), 1)
{}
...
GLint ImageIndexIterator::maxLayer() const
{
    if (mLayerCounts)
    {
        ...
        return (mCurrentIndex.level < mMipRange.high()) ? mLayerCounts[mCurrentIndex.level] : 0;
    }
...
ImageIndex ImageIndexIterator::next()
{
    ...
    if (mCurrentIndex.target < mTextureTargetRange.high())
    {
        mCurrentIndex.target++;
    }
    else if (mCurrentIndex.hasLayer() && mCurrentIndex.layer < maxLayer() - 1)
    {
        mCurrentIndex.target = mTextureTargetRange.low();
        mCurrentIndex.layer++;
    }
    else if (mCurrentIndex.level < mMipRange.high() - 1)
    {
        mCurrentIndex.target = mTextureTargetRange.low();
        mCurrentIndex.layer  = mLayerRange.low();
        mCurrentIndex.level++;
    }
    else
    {
