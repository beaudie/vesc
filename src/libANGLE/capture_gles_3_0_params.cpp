                              ...
                              ParamCapture *paramCapture)
{
    ...
    bool rowPitch        = internalFormatInfo.computeRowPitch(type, width, unpack.alignment,
                                                       unpack.rowLength, &srcRowPitch);
    GLuint srcDepthPitch = 0;
    bool depthPitch = internalFormatInfo.computeDepthPitch(height, unpack.imageHeight, srcRowPitch,
                                                           &srcDepthPitch);
    GLuint srcSkipBytes = 0;
    bool skipBytes = internalFormatInfo.computeSkipBytes(type, srcRowPitch, srcDepthPitch, unpack,
                                                         true /* is3D */, &srcSkipBytes);

    ASSERT(rowPitch && depthPitch && skipBytes);

    size_t captureSize = (srcRowPitch * height) * (srcDepthPitch * depth) + srcSkipBytes;
