...
bool ValidateGetBooleani_v(Context *context, GLenum target, GLuint index, GLboolean *data)
{
    ...
    {
        context->handleError(gl::InvalidOperation() << "Context does not support GLES3.1");
        return false;
    }
...
bool ValidateDrawIndirectBase(Context *context, GLenum mode, const void *indirect)
{
    ...
    {
        context->handleError(gl::InvalidOperation() << "zero is bound to VERTEX_ARRAY_BINDING");
        return false;
    }
    ...
    {
        context->handleError(gl::InvalidOperation() << "zero is bound to DRAW_INDIRECT_BUFFER");
        return false;
    }
...
bool ValidateDrawElementsIndirect(Context *context, GLenum mode, GLenum type, const void *indirect)
{
    ...
    {
        context->handleError(gl::InvalidOperation() << "zero is bound to ELEMENT_ARRAY_BUFFER");
        return false;
    }
                                      ...
                                      GLsizei *length)
{
    ...
    if (context->getTargetTexture(IsCubeMapTextureTarget(target) ? GL_TEXTURE_CUBE_MAP : target) ==
        nullptr)
    {
        context->handleError(gl::InvalidEnum() << "No texture bound.");
        return false;
    }
    ...
    switch (pname)
    {
        ...
        case GL_TEXTURE_COMPRESSED:
            break;
        default:
            context->handleError(gl::InvalidEnum() << "Unknown pname.");
            return false;
    }
                                     ...
                                     GLboolean fixedSampleLocations)
{
    ...
    {
        context->handleError(gl::InvalidEnum() << "Target must be TEXTURE_2D_MULTISAMPLE.");
        return false;
    }

    if (width < 1 || height < 1)
    {
        context->handleError(gl::InvalidValue() << "Width and height must be positive.");
        return false;
    }
    ...
    if (samples == 0)
    {
        context->handleError(gl::InvalidValue() << "Samples may not be zero.");
        return false;
    }
    ...
    {
        context->handleError(gl::InvalidEnum() << "SizedInternalformat must be color-renderable, "
    ...
    {
        context->handleError(gl::InvalidOperation() << "Zero is bound to target.");
        return false;
    }

    if (texture->getImmutableFormat())
    {
        context->handleError(gl::InvalidOperation() << "The value of TEXTURE_IMMUTABLE_FORMAT for "
...
bool ValidateGetMultisamplefv(Context *context, GLenum pname, GLuint index, GLfloat *val)
{
    ...
    {
        context->handleError(gl::InvalidEnum() << "Pname must be SAMPLE_POSITION.");
        return false;
    }
    ...
    {
        context->handleError(gl::InvalidValue() << "Index must be less than the value of SAMPLES.");
        return false;
    }
...
bool ValidationFramebufferParameteri(Context *context, GLenum target, GLenum pname, GLint param)
{
    ...
    if (framebuffer->id() == 0)
    {
        context->handleError(gl::InvalidOperation() << "Default framebuffer is bound to target.");
        return false;
    }
    return true;
}
                                     ...
                                     const GLchar *name)
{
    if (context->getClientVersion() < ES_3_1)
    {
        context->handleError(gl::InvalidOperation() << "Context does not support GLES 3.1.");
        return false;
    }
    ...
    {
        context->handleError(gl::InvalidEnum() << "Invalid program interface: 0x" << std::hex
                              ...
                              GLsizei stride)
{
    ...
    {
        context->handleError(gl::InvalidOperation() << "Buffer is not generated.");
        return false;
    }
    ...
    if (offset < 0)
    {
        context->handleError(gl::InvalidValue() << "offset cannot be negative.");
        return false;
    }
    ...
    {
        context->handleError(gl::InvalidOperation() << "Default vertex array buffer is bound.");
        return false;
    }
...
bool ValidateVertexBindingDivisor(ValidationContext *context, GLuint bindingIndex, GLuint divisor)
{
    ...
    if (context->getGLState().getVertexArrayId() == 0)
    {
        context->handleError(gl::InvalidOperation() << "Default vertex array object is bound.");
        return false;
    }
                                    ...
                                    GLchar *name)
{
    ...
    {
        context->handleError(gl::InvalidValue() << "Invalid index: " << index);
        return false;
    }

    if (bufSize < 0)
    {
        context->handleError(gl::InvalidValue() << "Invalid bufSize: " << bufSize);
        return false;
    }
