...
bool ValidateDispatchComputeIndirect(Context *context, GLintptr indirect)
{
    ...
    GLint64 offset = reinterpret_cast<GLint64>(indirect);
    if (offset < 0 || (static_cast<GLuint>(offset) % sizeof(GLuint)) != 0)
    {
    ...
    auto checkedSum = offset + static_cast<GLint64>(3 * sizeof(GLuint));
    if (checkedSum > dispatchIndirectBuffer->getSize())
    {
        context->handleError(
            InvalidOperation()
            << "the  command  would source data beyond the end of the buffer object.");
