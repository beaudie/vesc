...
bool ValidationFramebufferParameteri(Context *context, GLenum target, GLenum pname, GLint param)
{
    ...
    {
        GLint defaultWidth = context->getCaps().maxFramebufferWidth;
        if (param < 0 || param > defaultWidth)
        {
    ...
    {
        GLint defaultHeight = context->getCaps().maxFramebufferHeight;
        if (param < 0 || param > defaultHeight)
        {
            context->handleError(Error(
