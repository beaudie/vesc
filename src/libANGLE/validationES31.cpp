...
bool ValidateDrawArraysIndirect(Context *context, GLenum mode, const GLvoid *indirect)
{
    ...
    gl::Buffer *drawIndirectBuffer = state.getDrawIndirectBuffer();
    GLint64 offset                 = reinterpret_cast<GLint64>(indirect);
    if ((offset + static_cast<GLint64>(sizeof(DrawArraysIndirectCommand))) >
        drawIndirectBuffer->getSize())
    {
        context->handleError(
                                  ...
                                  const GLvoid *indirect)
{
    ...
    gl::Buffer *drawIndirectBuffer = state.getDrawIndirectBuffer();
    GLint64 offset                 = reinterpret_cast<GLint64>(indirect);
    if ((offset + static_cast<GLint64>(sizeof(DrawElementsIndirectCommand))) >
        drawIndirectBuffer->getSize())
    {
        context->handleError(
            Error(GL_INVALID_OPERATION,
