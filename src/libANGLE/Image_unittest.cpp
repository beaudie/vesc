...
TEST(ImageTest, RefCounting)
{
    ...
    // Verify that the image added a ref to the texture and the texture has not added a ref to the image
    ...
    EXPECT_CALL(*renderbufferImpl, setStorageEGLImage(_))
        .WillOnce(Return(gl::Error(GL_NO_ERROR)))
        .RetiresOnSaturation();
    renderbuffer->setStorageEGLImage(image);
    ...
    // Simulate deletion of the texture and verify that it still exists because the image holds a ref
    texture->release();
    EXPECT_EQ(texture->getRefCount(), 1u);
    EXPECT_EQ(image->getRefCount(), 2u);
    EXPECT_EQ(renderbuffer->getRefCount(), 1u);

    // Simulate deletion of the image and verify that it still exists because the renderbuffer holds a ref
    image->release();
    EXPECT_EQ(texture->getRefCount(), 1u);
    EXPECT_EQ(image->getRefCount(), 1u);
    EXPECT_EQ(renderbuffer->getRefCount(), 1u);
...
TEST(ImageTest, RespecificationReleasesReferences)
{
    ...
    EXPECT_CALL(*textureImpl, setImage(_, _, _, _, _, _, _, _))
        .WillOnce(Return(gl::Error(GL_NO_ERROR)))
        .RetiresOnSaturation();
    texture->setImage(GL_TEXTURE_2D, 0, GL_RGBA8, gl::Extents(1, 1, 1), GL_RGBA, GL_UNSIGNED_BYTE, gl::PixelUnpackState(), nullptr);
