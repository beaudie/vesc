...
def get_text_description(target_name):
    try:
        desc = subprocess.check_output(
            ['gn', 'desc', '--format=json', 'out/Default', target_name])
...
def get_json_description(target_name):
    try:
...
def load_deps(desc, target_name, indent="  "):
...
def load_json_deps(desc, target_name, all_desc, indent="  "):
    ...
    for dep in target.get('deps', []):
        if dep not in all_desc:
            logging.debug("dep: %s%s" % (indent,dep))
            new_desc = get_json_description(dep)
            all_desc[dep] = new_desc[dep]
            load_json_deps(new_desc, dep, all_desc, indent+"  ")
        else:
            logging.debug("dup: %s%s" % (indent,dep))

def create_build_description(targets):
...
def main():
    ...
    if args.desc:
        ...
        desc = create_build_description(args.targets)
        if args.desc_output:
            fh = open(args.desc_output,"w")
            fh.write(json.dumps(desc, indent=4, sort_keys=True))
            fh.close()
            print("Wrote json description to %s" % args.desc_output)
