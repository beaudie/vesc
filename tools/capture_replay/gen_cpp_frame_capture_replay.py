...
def main(capture_dir, context_id, last_frame):
    ...
    content = template_capture_replay.format(
        includes='\n'.join([
            '#include "%s/angle_capture_context%d_frame%s.cpp"' %
            (capture_dir_include_path, context_id, str(frame_idx).zfill(3))
            for frame_idx in range(0, last_frame + 1)
        ]),
        draw_calls='\n'.join(
            ['ReplayFrame%d;\nswap();\n' % frame_idx for frame_idx in range(0, last_frame + 1)]),
    )
...
if __name__ == '__main__':
    ...
    context_id = int(arguments[0]) if len(arguments) > 0 else 1
    frame_end = int(arguments[1]) if len(arguments) > 1 else 99
    capture_dir = arguments[2] if len(arguments) > 2 else '.'
    main(capture_dir, context_id, frame_end)
