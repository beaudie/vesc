...
def _CheckCommitMessageFormatting(input_api, output_api):
    ...
    if len(commit_msg_lines) > 0:
        for whitelist_string in whitelist_strings:
            if commit_msg_lines[0].startswith(whitelist_string):
                return []
    errors = []
    # get rid of newlines after the last paragraph but not the whitespace at the end of the last
    # line of the last paragraph
    while len(commit_msg_lines) > 0 and _IsLineBlank(commit_msg_lines[-1]):
    ...
    # get rid of newlines before description summary, but not the whitespace at the start of
    # the description summary
    while len(commit_msg_lines) > 0 and _IsLineBlank(commit_msg_lines[0]):
    ...
    if not _IsLineBlank(commit_msg_lines[0]):
        ...
        if len(commit_msg_lines) == 0:
            errors.append(
                output_api.PresubmitError(
                    "Please ensure that your description body is not blank."))
            return errors
