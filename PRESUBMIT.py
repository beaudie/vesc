...
def _CheckShaderVersionInShaderLangHeader(input_api, output_api):
    ...
    VERSION_DEFINE = '#define ANGLE_SH_VERSION'
    ...
    # Skip this check for reverts
    git_output = input_api.change.DescriptionText()
    multiple_commits = _SplitIntoMultipleCommits(git_output)
    for commit in multiple_commits:
        if commit.startswith('revert') or commit.startswith('Revert'):
            return []

    version_changed = False
    old_version = 0
    new_version = 0
    for f in headers_changed:
        version_diff_lines = [
            line for line in f.GenerateScmDiff().splitlines()
            if line.startswith(VERSION_DEFINE, 1)
        ]

        for line in version_diff_lines:
            if line[0] == '-':
                version_changed = True
                old_version = int(line[len(VERSION_DEFINE) + 1:].split()[0])
            elif line[0] == '+':
                new_version = int(line[len(VERSION_DEFINE) + 1:].split()[0])

    if not version_changed or new_version <= old_version:
        return [
            output_api.PresubmitError(
