...
def _CheckWildcardInTestExpectationFiles(input_api, output_api):
    ...
    for f in input_api.AffectedSourceFiles(expectation_files):
        ...
        for line in diff.splitlines():
            ...
            #
            #     FRONTENDAPI_BACKENDAPI[_FEATURE1]*
            #
            # Any part of the above may be a wildcard.  Warn about usage of FRONTEND_BACKENDAPI as
            # the tag.  Instead, the backend should be specified before the : and `*` used.  If any
            # additional tags are present, it's a specific expectation that should remain specific
            # (and not wildcarded).  NoFixture is an exception as X_Y_NoFixture is the generic form
            # of the tags of tests that don't use the fixture.
            ...
            if len(sections) == 2:
                expectations_without_wildcard.append(line[1:] + '=====>' + tag)
