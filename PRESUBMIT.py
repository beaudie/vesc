...
def _CheckCommitMessageFormatting(input_api, output_api, is_committing):
    ...
    if input_api.change.issue:
        print("ABCCCCCCCCCCCCCCCCC--------------------")
        print(input_api.gerrit.GetChangeDescription(input_api.change.issue))
        print("ABCCCCCCCCCCCCCCCCC--------------------")
    else:
        print("Issue is 0------------------------")
    ...
    #     ["git", "log", "-n", "1", sha.strip(), "--pretty=format:%B"])
    if is_committing:
        git_output = subprocess.check_output(["git", "log", "-n", "1", "--pretty=format:%B"])
    else:
        git_output = input_api.change.FullDescriptionText()
    print(git_output)
    commit_msg_lines = git_output.splitlines()
    print(commit_msg_lines)
    ...
    if len(commit_msg_lines) == 0:
        ...
        return errors
    if _IsLineBlank(commit_msg_lines[-1]):
        errors.append(
...
def CheckChangeOnUpload(input_api, output_api):
    ...
    print("Uploading")
    results.extend(_CheckCommitMessageFormatting(input_api, output_api, False))
    return results


def CheckChangeOnCommit(input_api, output_api):
    print("Commiting")
    return _CheckCommitMessageFormatting(input_api, output_api, True)
