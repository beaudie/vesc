...
target_blockist = [
    ...
    '//third_party/android_ndk:cpu_features',  # only needed by zlib and we're using Android's cpu_features
...
def gn_deps_to_blueprint_deps(abi, target, build_info):
    ...
    for dep in target_info['deps']:
        if dep not in target_blockist and (not dep.startswith('//third_party') or any(
            ...
            static_libs.extend(['zlib_google_compression_utils_portable', 'libz', 'cpufeatures'])
