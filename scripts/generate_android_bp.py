...
def main():
    ...
    frameworkless_infos = {}
    for abi in ABI_TARGETS:
        with open(args['gn_json_' + gn_abi(abi) + '_frameworkless'], 'r') as f:
            frameworkless_infos[abi] = json.load(f)
    frameworkless_build_info = BuildInfo(frameworkless_infos)

    generated_targets, flagged_module_types = merge_targets_with_flag(
        default=get_blueprint_targets_from_build_info(build_info),
        flagged=get_blueprint_targets_from_build_info(frameworkless_build_info),
