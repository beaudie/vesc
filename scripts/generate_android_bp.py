...
// See: http://go/android-license-faq"""
    ...
    output.append('')
    if blueprint_comment := values.get(BLUEPRINT_COMMENT_PROPERTY):
...
def get_blueprint_targets_from_build_info(build_info: BuildInfo) -> List[Tuple[str, dict]]:
    ...
    for target in reversed(targets_to_write.keys()):
        ...
        module_type, bp = gn_target_to_blueprint(target, build_info)
        if target in DMA_BUF_TARGETS:
            bp[BLUEPRINT_COMMENT_PROPERTY] = '''\
...
products that use it.'''
        ...
        generated_targets.append((module_type, bp))
