...
def remap_gn_path(path):
    # TODO: pass the gn gen folder as an arg so it is future proof
    remap_folders = [
...
def gn_sources_to_blueprint_sources(sources):
    # blueprints fail to compile if a header is included in a source list
...
target_blackist = [
    ...
    '//third_party/spirv-tools/src:spvtools_val',
...
dep_remaps = {
    '//third_party/glslang/src:glslang_sources': ['deqp_SPIRV', 'deqp_glslang', 'deqp_OSDependent', 'deqp_OGLCompiler', 'deqp_HLSL' ],
    '//third_party/glslang/src:glslang_default_resource_limits_sources': [ 'deqp_glslang_default_resource_limits', ],
}
...
def gn_deps_to_blueprint_deps(target_info, build_info):
    ...
    for dep in target_info['deps']:
        if dep in dep_remaps:
            for dep_remap in dep_remaps[dep]:
                static_libs.append(dep_remap)
        elif not dep in target_blackist:
            ...
            (child_static_libs, _, _, _) = gn_deps_to_blueprint_deps(dep_info, build_info)
            static_libs += child_static_libs

    return (static_libs, shared_libs, defaults, generated_headers)

def gn_libs_to_blueprint_shared_libraries(libs):
    lib_blackist = [
        'android_support',
    ]
...
def gn_include_dirs_to_blueprint_include_dirs(dirs):
    result = []
...
def gn_cflags_to_blueprint_cflags(cflags, cflags_c, cflagcs_cc, defines):
    result = []
    ...
    for cflag in cflags + cflags_c + cflagcs_cc:
...
def library_target_to_blueprint(target, build_info):
    ...
    (bp['static_libs'], bp['shared_libs'], bp['defaults'], bp['generated_headers']) = gn_deps_to_blueprint_deps(target_info, build_info)
    bp['shared_libs'] += gn_libs_to_blueprint_shared_libraries(target_info['libs'])

    bp['local_include_dirs'] = gn_include_dirs_to_blueprint_include_dirs(target_info['include_dirs'])

    bp['cflags'] =  gn_cflags_to_blueprint_cflags(target_info['cflags'], target_info['cflags_c'], target_info['cflags_cc'], target_info['defines'])
...
def gn_action_args_to_blueprint_args(blueprint_inputs, blueprint_outputs, args):
    # TODO: pass the gn gen folder as an arg so we know how to get from the gen path to the root path
    remap_folders = [
        ('../../', ''),
        ('gen/', ''),
    ]
...
blueprint_group_types = {
    "group": "cc_defaults",
}
def group_target_to_blueprint(target, build_info):
    target_info = build_info[target]
    blueprint_type = blueprint_group_types[target_info['type']]
...
def action_target_to_blueprint(target, build_info):
    ...
    cmd = [ '$(location)' ] + gn_action_args_to_blueprint_args(bp_srcs, bp_outputs, target_info['args'])
    bp['cmd'] = ' '.join(cmd)
...
def gn_target_to_blueprint(target, build_info):
    ...
    elif gn_type in blueprint_group_types:
        return group_target_to_blueprint(target, build_info)
...
def get_gn_target_dependencies(output_dependencies, build_info, target):
    ...
    for dep in build_info[target]['deps']:
        if dep in target_blackist or dep in dep_remaps:
            # Blacklisted or renamed dep
            continue
...
def main(args):
    ...
    blueprint_targets.append(('android_app', {
        'name': 'GoogleANGLE',
        'sdk_version': 'system_' + sdk_version,
        ...
        'jni_libs': [ gn_target_to_blueprint_target(target, build_info[target]) for target in root_targets ],
        'product_specific': True,
        'privileged': True,
        'certificate': ':com_google_android_angle_container',
        'owner': 'google',
    }))
