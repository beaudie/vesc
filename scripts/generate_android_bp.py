...
root_targets = [
    ...
    # "//third_party/angle:libGLESv1_CM",
...
def action_target_to_blueprint(target, build_info):
    ...
    gn_inputs = []
    if 'inputs' not in inputs_blacklist and 'inputs' in target_info:
    ...
    cmd = ['$(location)'] + gn_action_args_to_blueprint_args(bp_srcs, bp_outputs,
                                                             target_info['args'])
    if bp['name'] == 'third_party_glslang_src_glslang_build_info':
        cmd.insert(0, 'python')
    bp['cmd'] = ' '.join(cmd)
...
def main():
    ...
    parser.add_argument(
        'gn_json_arm',
        help='ARM 32b gn desc in json format. Generated with \'gn desc <out_dir> --format=json "*"\'.'
    )
    parser.add_argument(
        'gn_json_arm64',
        help='ARM 64b gn desc in json format. Generated with \'gn desc <out_dir> --format=json "*"\'.'
    )
    parser.add_argument(
        'gn_json_x86',
        help='x86 32b gn desc in json format. Generated with \'gn desc <out_dir> --format=json "*"\'.'
    )
    parser.add_argument(
        'gn_json_x64',
        help='x86 64b gn desc in json format. Generated with \'gn desc <out_dir> --format=json "*"\'.'
    )
    args = parser.parse_args()

    build_info = {}
    with open(args.gn_json_arm, 'r') as f:
        build_info[abi_arm] = json.load(f)
    with open(args.gn_json_arm64, 'r') as f:
        build_info[abi_arm64] = json.load(f)
    with open(args.gn_json_x86, 'r') as f:
        build_info[abi_x86] = json.load(f)
    with open(args.gn_json_x64, 'r') as f:
        build_info[abi_x64] = json.load(f)
...
""" % (sys.argv[0], date.today().year)
    ...
    # Cleanup the output to handle generatng the gn description in Chromium's src/
    cleaned_output = [line.replace("third_party/angle/", "") for line in output]
