...
def write_blueprint(output, target_type, values, blueprint_comment=None):
...
def get_angle_android_dma_buf_flag_config(build_info):
    ...
    }, ('Note: this is a no-op for most builds, only applies to products that ' +
        'explicitly enable the angle_android_dma_buf config flag. See b/353262025 ' +
        'for details of the products that use it.')))
...
def main():
    ...
    # A list of either
    # - (blueprint type (str), properties (dict)), or
    # - (blueprint type (str), properties (dict), comment (str))
    # The comment, if present, will be generated as-is before the blueprint
    # module.
    blueprint_targets = []
...
//""" % sys.argv[0]
    ...
    for (blueprint_type, blueprint_data, *rest) in blueprint_targets:
        write_blueprint(
            output, blueprint_type, blueprint_data, blueprint_comment=rest[0] if rest else None)
