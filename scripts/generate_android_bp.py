...
# Example usage:
#   gn desc out/Android --format=json "*" > esc.json
...
def gn_deps_to_blueprint_deps(target_info, build_info):
    ...
    for dep in target_info['deps']:
        if not dep in target_blackist:
            ...
            # Each blueprint target runs genrules in a different output directory unlike GN. If a target depends on
            # another's genrule, it wont find the outputs. Propogate generated headers up the dependency stack.
