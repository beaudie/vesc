...
# Libraries listed here should be "vendor" libraries. Which means
# they show up in /vendor/lib[64] rather than /system/lib[64]
# Android EGL wrapper expects all driver libraries to be in /vendor
vendor_targets = [
]

# The Android library names need to be different than the GN
# names. This table defines what gets renamed to what.
rename_targets = {
    'angle_third_party_angle_libEGL': 'libEGL_angle',
    'angle_third_party_angle_libGLESv2': 'libGLESv2_angle',
}

# version_script indicates the library version script to use for
# a specific target library.
version_scripts = {
    'libEGL_angle': 'libEGL_angle.txt',
}
...
class Module(object):
    ...
    def __init__(self, mod_type, name):
        ...
        if name in rename_targets:
            self.name = rename_targets[name]
        else:
            self.name = name
        if self.name in vendor_targets:
            self.vendor = True
        if self.name in version_scripts:
            self.version_script = version_scripts[self.name]
        self.srcs = []
        self.comment = None
        self.shared_libs = []
        self.static_libs = []
    ...
    def to_string(self, output):
        ...
        self._output_field(output, 'vendor')
...
def label_to_module_name(label):
    ...
    if not module.startswith(module_prefix) and label not in default_targets:
        name = module_prefix + module
    else:
        name = module
    if name in rename_targets:
        name = rename_targets[name]
    return name
