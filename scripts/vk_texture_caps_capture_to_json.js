  ...
  $("#" + allTableIds[i] + " td").each(function() {
    ...
    $this.find("code").each(function() {
      if (!$(this).text().startsWith("VK_FORMAT_FEATURE") &&
        ...
        var squareIndex = 0;
        var result = {format: vkFormat, optimalTilingFeatures: [], bufferFeatures: []};
        var skipEntry = false;
        ...
        $(this).closest("tr").find("td.halign-center").each(function() {
          ...
          if ($(this).text() === "✓") {
            if (squareIndex >= 0 && squareIndex <= 8) {
                result.optimalTilingFeatures.push(indexToFeatureMap[squareIndex]);
            } else {
              result.bufferFeatures.push(indexToFeatureMap[squareIndex]);
            }
          }
          if ($(this).text() === "†") {
            skipEntry = true;
        ...
        if (!skipEntry &&
              (result.optimalTilingFeatures.length > 0 || result.bufferFeatures.length > 0)) {
          outputJson[objectIndex] = result;
          objectIndex++;
        }
      }
    });
