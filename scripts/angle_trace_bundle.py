...
#    - includes list_traces.sh and run_test.sh (see --trace-name)
...
LIST_TESTS_TEMPLATE = r'''#!/bin/bash

{gn_dir}/angle_trace_tests --list-tests
'''

RUN_TESTS_TEMPLATE = r'''#!/bin/bash

{gn_dir}/angle_trace_tests --filter='TraceTest.{trace_name}' --verbose --fixed-test-time-with-warmup 10
'''
...
def main():
    ...
    mb = os.path.join('tools', 'mb', 'mb.py')
    subprocess.check_call([
        'python3', mb, 'isolate', gn_dir, 'angle_trace_perf_tests', '-i',
        os.path.join('infra', 'specs', 'gn_isolate_map.pyl')
    ])
    ...
    if not args.include_unstripped_libs:
        skipped_prefixes.append(gn_dir + 'lib.unstripped/')
    ...
    with zipfile.ZipFile(args.output_zip_file, 'w', zipfile.ZIP_DEFLATED, allowZip64=True) as fzip:
        ...
        addScript(fzip, 'list_traces.sh', LIST_TESTS_TEMPLATE.format(gn_dir=gn_dir))

        if args.trace_name:
            addScript(fzip, 'run_trace.sh',
                      RUN_TESTS_TEMPLATE.format(gn_dir=gn_dir, trace_name=args.trace_name))
