...
def FindAddedDeps(angle_deps, new_cr_deps):
  ...
  Args:
    angle_deps: dict of deps as defined in the ANGLE DEPS file.
  ...
  """
    all_added_deps = _FindNewDeps(angle_deps, new_cr_deps)
...
def FindRemovedDeps(angle_deps, new_cr_deps):
  ...
  Since it's hard to compute:
   ...
   2/ We rely on existing behavior of CalculateChangeDeps.
      I.e. Assumes non-CIPD dependencies are ANGLE-only, don't remove them.
  ...
  """
    all_removed_deps = _FindNewDeps(new_cr_deps, angle_deps)
    generated_android_deps = [
        path for path in all_removed_deps if path.startswith(ANDROID_DEPS_PATH)
    ]
    # ANGLE-only dependencies are handled in CalculateChangedDeps.
    other_deps = [
        path for path in all_removed_deps
        if path not in generated_android_deps and path not in ANGLE_CHROMIUM_DEPS
    ]
    return generated_android_deps, other_deps
  ...
  """
    ...
    for path, angle_deps_entry in angle_entries.iteritems():
        ...
        if cr_deps_entry:
            ...
            assert angle_deps_entry.url == cr_deps_entry.url, (
                'ANGLE DEPS entry %s has a different URL (%s) than Chromium (%s).' %
                (path, angle_deps_entry.url, cr_deps_entry.url))
        else:
            if isinstance(angle_deps_entry, DepsEntry):
                # Use the HEAD of the deps repo.
                stdout, _ = _RunCommand(['git', 'ls-remote', angle_deps_entry.url, 'HEAD'])
        ...
        if angle_deps_entry.revision != new_rev:
            ...
            result.append(
                ChangedDep(path, angle_deps_entry.url, angle_deps_entry.revision, new_rev))
...
def UpdateDepsFile(deps_filename, rev_update, changed_deps, new_cr_content):
    ...
    for dep in changed_deps:
        ...
        _RunCommand(['gclient', 'setdep', '--revision', update], working_dir=CHECKOUT_SRC_DIR)
...
def main():
    ...
    new_generated_android_deps, _ = FindAddedDeps(angle_deps, new_cr_deps)
    removed_generated_android_deps, other_deps = FindRemovedDeps(angle_deps, new_cr_deps)
    if other_deps:
        raise RollError('ANGLE DEPS entries are missing from Chromium: %s. '
