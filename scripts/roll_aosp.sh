...
# This script is executed in _AOSP_ checkout (with new ANGLE changes merged into it by git merge).
# It is executed by the Skia roller which merges ANGLE changes and also has code to remove submodules and extra files.
# For local setup including the additional Skia roller actions see:
# https://g3doc.corp.google.com/company/users/yuxinhu/ANGLE/Wrangler.md?cl=head#how-to-resolve-angle-android-roller-merge-conflict
#
# Some caveats due to the way this is set up:
# * If a file is deleted from AOSP by a manual commit to AOSP *and* is changed upstream,
#   git merge will fail in Skia roller before even reaching this script
#
...
#   * If this file comes from a third_party dep it might be:
#      1. copied to aosp by this script (copy_to_aosp_deps below)
#      2. pulled in by gclient but then deleted after the codegen so it is *not* in AOSP (delete_after_codegen_deps below)
#
...
#         In cases like third_party/jdk/BUILD.gn, we need to make sure it is *not* part of delete_after_codegen_deps as
#         this script would delete this file from AOSP after one roll and the next roll would fail gn gen.

This case occasionally hapens when .gitmodules files are updated.

# Deps copied to AOSP by having .git removed from them then git add (via git_add_deps)
copy_to_aosp_deps=(
    "build"
...
# Dirs added to the commit with `git add -f`. Applies to both copy_to_aosp_deps and delete_after_codegen_deps.
git_add_deps=(
  "build"
  "third_party"
)
...
for dep in "${copy_to_aosp_deps[@]}"; do
    rm -rf "$dep"
done
...
for dep in "${delete_after_codegen_deps[@]}"; do
   rm -rf "$dep"
done
...
# Add all changes under $git_add_deps to sync all changes in those dirs to AOSP.
for git_add_dep in "${git_add_deps[@]}"; do
    git add -f $git_add_deps
done
