...
#      1. copied to AOSP by this script (copy_to_aosp_paths)
#      2. pulled in by gclient but then ignored due it being part of a sub-repo not this repo
#      3. pulled in by gclient but then deleted after the codegen so it is *not* in AOSP (delete_after_codegen_paths)
#
...
# Delete files that we do not want in AOSP. Some of them are needed to generate Android.bp.
