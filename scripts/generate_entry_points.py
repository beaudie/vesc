...
with open(script_relative('entry_point_internal_enums.json')) as f:
    cmd_internal_enums = json.loads(f.read())
...
def just_the_name_internal(param, reserved_set):
...
def format_entry_point_def(cmd_name, proto, params):
    internal_enums = cmd_internal_enums.get(cmd_name, {})
    internal_params = [just_the_name_internal(param, internal_enums) for param in params]
    internal_enum_conversions = []
    for param in params:
        name = just_the_name(param)
        if name in internal_enums:
            internal_name = name + "Internal"
            internal_type = internal_enums[name]
    ...
    return template_entry_point_def.format(
        ...
        internal_enum_conversions = "".join(internal_enum_conversions),
