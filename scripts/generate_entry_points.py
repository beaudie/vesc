...
template_capture_replay_sourece = """// GENERATED FILE - DO NOT EDIT.
// Generated by {script_name} using data from {data_source_name}.
//
// Copyright {year} The ANGLE Project Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
...
inline void GetParamVal<ParamType::T{enum}, {type}>(const ParamValue &value, {type} *valueOut)
{{
    *valueOut = value.{union_name};
}}"""

template_access_param_value_case = """        case ParamType::T{enum}:
    GetParamVal<ParamType::T{enum}>(value, valueOut);break;"""
...
def get_command_param_type_list(command_node, cmd_name, cmd_packed_gl_enums):
    param_type_list = list()
    packed_enums = get_packed_enums(cmd_packed_gl_enums, cmd_name)
    for param_node in command_node.findall('param'):
        param_text = "".join(param_node.itertext())
        param_type = just_the_type_packed(param_text, packed_enums)
        param_type_list.append(param_type)
    return param_type_list


def format_capture_replay_param_access(param_type_list):
    param_access_strs = list()
    for i, param_type in enumerate(param_type_list):
        pointer_count = param_type.count('*')
        if pointer_count == 0:
            param_template = "paramCaptures[{index}].getAs<{type}>()"
        elif pointer_count == 1:
            param_template = "paramCaptures[{index}].getAsPointer<{type}>()"
        elif pointer_count == 2:
            param_template = "paramCaptures[{index}].getAsPointerPointer<{type}>(&paramBuffer->pointersBuffer)"
        else:
...
def write_capture_replay_source(all_commands_nodes, gles_command_names, cmd_packed_gl_enums):
    ...
    for command_node in all_commands_nodes:
        ...
        entry_point_to_param_types_mapping[entry_point_name] = get_command_param_type_list(
            command_node, command_name, cmd_packed_gl_enums)

    call_replay_cases = '\n'.join([
        template_capture_replay_call_case.format(
            entry_point=entry_point_name,
            param_value_access=format_capture_replay_param_access(cmd_param_types),
    ...
    source_content = template_capture_replay_sourece.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name="gl.xml and gl_angle_ext.xml",
        year=date.today().year,
