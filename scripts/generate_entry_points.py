...
template_capture_replay_call_case_with_update = """case gl::EntryPoint::{entry_point}:
    ...
    replayContext->Update_{context_call}({param_value_access});
    break;"""
...
def format_capture_replay_param_access(command_name, param_text_list, cmd_packed_gl_enums):
    ...
    for i, param_text in enumerate(param_text_list):
        ...
        if pointer_count == 0:
            ...
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getAsConstPointer<{type}>()'
        elif pointer_count == 2 and is_const:
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getAsPointerConstPointer<{type}>(&replayContext->pointersBuffer)'
        elif pointer_count == 1 or (pointer_count == 2 and is_get_pointer_command(command_name)):
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getReadBufferPointer<{type}>(&replayContext->readBuffer)'
        else:
