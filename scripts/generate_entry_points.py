...
def format_context(cmd_name, proto, params, template):
...
def get_entry_points(all_commands, commands, is_explicit_context):
    decls = []
    defs = []
    export_defs = []
    validation_protos = []
...
def get_decls(formatter, all_commands, gles_commands, already_included, overloaded):
    ...
    for command in all_commands:
        ...
        param_text = ["".join(param.itertext()) for param in command.findall('param')]
        proto_text = "".join(proto.itertext())
        decls.append(format_context(cmd_name, proto_text, param_text, formatter))
...
def main():
    ...
    for major_version, minor_version in [[2, 0], [3, 0], [3, 1], [1, 0]]:
        ...
        included_commands.extend(xml.commands[version])
        included_commands_suffix.extend(xml.commands[version])
    ...
    for extension_name, ext_cmd_names in sorted(xml.ext_data.iteritems()):
        ...
        if (extension_name in registry_xml.gles1_extensions and
            ...
            glesdecls['exts']['GLES1 Extensions'][extension_name] = get_decls(
                context_decl_format, all_commands, ext_cmd_names, included_commands,
                gles1_overloaded)
    ...
    for name in extension_commands:
        included_commands_suffix.append(name)
        name_no_suffix = name
        for suffix in strip_suffixes:
            if name_no_suffix.endswith(suffix):
                name_no_suffix = name_no_suffix[0:-len(suffix)]
        included_commands.append(name_no_suffix)
    ...
    for major_version, minor_version in [[1, 0], [1, 1], [1, 2], [1, 3], [1, 4], [1, 5], [2, 0],
        ...
        just_opengl32_commands = [
            cmd for cmd in glxml.commands[version] if cmd not in included_commands
        ]
        just_opengl32_commands_suffix = [
            cmd for cmd in glxml.commands[version] if cmd not in included_commands_suffix
        ]
        all_commands32 = glxml.all_commands

        # might need to change formats of get_entry_points, are they for gles
        trash1, trash2, trash3, validation_protos32 = get_entry_points(
            all_commands32, just_opengl32_commands_suffix, False)
        decls32, defs32, opengl32_defs, trash4 = get_entry_points(all_commands32,
                                                                  all_opengl32_commands, False)
        ...
        gldecls['core'][(major_version, minor_version)] = get_decls(
            context_decl_format, all_commands32, just_opengl32_commands, included_commands, [])
    ...
    # might need to change formats of get_entry_points, they are gles
    decls_wgl, defs_wgl, wgl_defs, validation_protos_wgl = get_entry_points(
        all_commands32, wgl_commands, False)
    ...
    entry_points_enum_header_path = path_to("openGL32", "entry_points_enum_autogen.h")
    with open(entry_points_enum_header_path, "w") as out:
        out.write(entry_points_enum)
        out.close()
