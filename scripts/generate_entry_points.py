...
// Generated by {script_name} using data from {data_source_name}.
//
// Copyright (c) 2002-{year} The ANGLE Project Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
//
...
def get_libgles_entry_points(commands, gles_commands):
    defs = []
    exports = []
    ordinal = 1
    for command in all_commands:
        ...
        param_text = ["".join(param.itertext()) for param in command.findall('param')]
        proto_text = "".join(proto.itertext())
        defs.append(format_libgles_entry_point_def(cmd_name, proto_text, param_text))
...
def write_export_files(entry_points, includes, exports):
    ...
    path = path_to("libGLESv2", "libGLESv2.cpp")
    ...
    path = path_to("libGLESv2", "libGLESv2.def")
...
libgles_defs, libgles_exports = get_libgles_entry_points(all_commands, all_cmd_names)
...
write_export_files("\n".join([item for item in libgles_defs]), source_includes, "\n".join([item for item in libgles_exports]))