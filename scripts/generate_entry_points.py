...
def write_windows_def_file(data_source_name, lib, libexport, folder, exports):
    ...
    for export in exports:
        if "#" in export:
            ...
            export = "    " + export
        cleaned_exports.append(export)

    content = TEMPLATE_WINDOWS_DEF_FILE.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name=data_source_name,
        exports="\n".join(cleaned_exports),
        lib=libexport)
...
def write_mac_exports_file(data_source_name, lib, folder, exports):
    cleaned_exports = []
    for export in exports:
        if "#" in export:
            export = "\n" + export
    ...
    content = TEMPLATE_MAC_EXPORTS_FILE.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name=data_source_name,
        exports="\n".join(cleaned_exports))

    path = path_to(folder, "%s_autogen.exports" % lib)
...
def write_linux_lds_file(data_source_name, lib, folder, exports):
    cleaned_exports = []
    for export in exports:
        if "#" in export:
            export = "\n    " + export
    ...
    content = TEMPLATE_LINUX_LDS_FILE.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name=data_source_name,
        exports="\n".join(cleaned_exports))

    path = path_to(folder, "%s_autogen.lds" % lib)
...
def get_egl_exports():
    ...
    for major, minor in registry_xml.EGL_VERSIONS:
        ...
        exports.append("# EGL %d.%d" % (major, minor))
    ...
    for extension_name, ext_cmd_names in sorted(egl.ext_data.iteritems()):
        ...
        exports.append("# %s" % extension_name)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        outputs = [
            ...
            '../src/libGLESv2/libGLESv2_autogen.exports',
            '../src/libGLESv2/libGLESv2_autogen.lds',
            '../src/libGLESv2/libGLESv2_no_capture_autogen.def',
            '../src/libGLESv2/libGLESv2_no_capture_autogen.exports',
            '../src/libGLESv2/libGLESv2_no_capture_autogen.lds',
            '../src/libGLESv2/libGLESv2_with_capture_autogen.def',
            '../src/libGLESv2/libGLESv2_with_capture_autogen.exports',
            '../src/libGLESv2/libGLESv2_with_capture_autogen.lds',
    ...
    for major_version, minor_version in registry_xml.GLES_VERSIONS:
        ...
        libgles_ep_exports.append("# OpenGL ES %s" % comment)
    ...
    for extension_name, ext_cmd_names in sorted(xml.ext_data.iteritems()):
        ...
        libgles_ep_exports.append("# %s" % extension_name)
    ...
    if registry_xml.support_EGL_ANGLE_explicit_context:
        ...
        libgles_ep_exports.append("# EGL_ANGLE_explicit_context")
    ...
    for major_version, minor_version in registry_xml.EGL_VERSIONS:
        ...
        win_def_comment = "# EGL %d.%d" % (major_version, minor_version)
    ...
    for extension_name, ext_cmd_names in sorted(eglxml.ext_data.iteritems()):
        ...
        comment = "\n// %s" % extension_name
        win_def_comment = "# %s" % (extension_name)
    ...
    # Write the version as a comment before the first EP.
    libgl_ep_exports.append("# WGL %s" % comment)
    ...
    for lib in ["libGLESv2" + suffix for suffix in ["", "_no_capture", "_with_capture"]]:
        ...
        write_mac_exports_file(everything, lib, "libGLESv2", libgles_ep_exports)
        write_linux_lds_file(everything, lib, "libGLESv2", libgles_ep_exports)
    ...
    write_windows_def_file("egl.xml and egl_angle_ext.xml", "libEGL", "libEGL", "libEGL",
                           libegl_windows_def_exports)
    write_mac_exports_file("egl.xml and egl_angle_ext.xml", "libEGL", "libEGL",
                           libegl_windows_def_exports)
    write_linux_lds_file("egl.xml and egl_angle_ext.xml", "libEGL", "libEGL",
                         libegl_windows_def_exports)
