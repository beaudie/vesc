...
TEMPLATE_REPLAY_CALL_CASE = """\
        case angle::EntryPoint::GL{entry_point}:
            {call}({params});
            break;
"""
...
def resource_id_ns(id):
    return 'egl' if id in EGL_PACKED_TYPES.values() else 'gl'
...
def format_replay_params(api, command_name, param_text_list, gl_packed_enums, resource_id_types):
    ...
    for i, param_text in enumerate(param_text_list):
        ...
        if command_name == 'glCreateShaderProgramvEXT' and i == 2:
            ...
            if cmd_no_suffix in gl_packed_enums and param_name in gl_packed_enums[cmd_no_suffix]:
                packed_type = remove_id_suffix(gl_packed_enums[cmd_no_suffix][param_name])
...
def write_capture_replay_source(api, all_commands_nodes, gles_command_names, gl_packed_enums,
                                resource_id_types):
    ...
    call_replay_cases = format_capture_replay_call_case(api, command_to_param_types_mapping,
                                                        gl_packed_enums, resource_id_types)
...
def main():
    ...
    write_capture_replay_source(apis.GLES, xml.all_commands, all_commands_with_suffix,
                                GLEntryPoints.get_packed_enums(), resource_id_types)
