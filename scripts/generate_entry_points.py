...
template_capture_replay_call_case_with_update = """case gl::EntryPoint::{entry_point}:
    context->{context_call}({param_value_access});
    paramBuffer->Update_{context_call}({param_value_access});
    break;"""
...
def format_capture_replay_param_access(param_text_list, cmd_packed_enums):
    ...
    for i, param_text in enumerate(param_text_list):
        ...
        pointer_count = param_type.count('*')
        if pointer_count == 0:
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getAs<{type}>()'
        elif pointer_count == 1:
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getAsPointer<{type}>()'
        elif pointer_count == 2:
            param_template = 'params.getParam("{name}", ParamType::T{enum_type}, {index}).getAsPointerPointer<{type}>(&paramBuffer->pointersBuffer)'
        else:
            assert False, "No support for pointers more than two levels"
...
def format_capture_replay_call_case(command_to_param_types_mapping, cmd_packed_gl_enums):
    ...
    for command_name, cmd_param_texts in sorted(command_to_param_types_mapping.items()):
        ...
        if command_name in ('glVertexAttribPointer', 'glVertexAttribIPointer',
                            'glVertexAttribLPointer'):
            call_case_template = template_capture_replay_call_case_with_update
        else:
            call_case_template = template_capture_replay_call_case

        call_str_list.append(
            call_case_template.format(
                entry_point=entry_point_name,
                param_value_access=format_capture_replay_param_access(
                    cmd_param_texts, get_packed_enums(cmd_packed_gl_enums, command_name)),
