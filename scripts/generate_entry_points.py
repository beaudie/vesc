...
# List of ANGLE extensions
angle_supported_extensions = sorted([
...
angle_ext_tree = etree.parse(script_relative('gl_ext.xml'))
angle_ext_root = angle_ext_tree.getroot()
angle_ext_commands = angle_ext_root.find(".//command[@namespace='GL']")
...
all_angle_ext_commands = angle_ext_root.findall('commands/command')
all_angle_ext_cmd_names = []
...
write_file("ext", "extension", template_entry_point_header,
           "\n".join([item for item in extension_decls]), "h", header_includes, "gl.xml")
write_file("ext", "extension", template_entry_point_source,
           "\n".join([item for item in extension_defs]), "cpp", source_includes, "gl.xml")

angle_extension_defs = []
angle_extension_decls = []
angle_ext_data = {}

for angle_extension in angle_ext_root.findall("extensions/extension"):
    angle_ext_cmd_names = []

    angle_extension_name = angle_extension.attrib['name']
    if not angle_extension_name in angle_supported_extensions:
        continue

    for require in angle_extension.findall('require'):
        angle_extension_commands = require.findall('command')
        angle_ext_cmd_names += [command.attrib['name'] for command in angle_extension_commands]

    angle_ext_data[angle_extension_name] = sorted(angle_ext_cmd_names)

for angle_extension_name, angle_ext_cmd_names in sorted(angle_ext_data.iteritems()):

    all_cmd_names += angle_ext_cmd_names
    decls, defs = get_entry_points(all_angle_ext_commands, angle_ext_cmd_names)

    comment = "\n// {}".format(angle_extension_name)
    defs.insert(0, comment)
    decls.insert(0, comment)

    angle_extension_defs += defs
    angle_extension_decls += decls
...
source_includes = template_sources_includes.format("2_0_ext", "", "")
source_includes += """
#include "libGLESv2/global_state.h"
...
write_file("2_0_ext", "ANGLE extension", template_entry_point_header,
           "\n".join([item for item in angle_extension_decls]), "h", header_includes, "gl_ext.xml")
write_file("2_0_ext", "ANGLE extension", template_entry_point_source,
           "\n".join([item for item in angle_extension_defs]), "cpp", source_includes, "gl_ext.xml")
...
entry_points_enum = template_entry_points_enum_header.format(
    script_name = os.path.basename(sys.argv[0]),
    data_source_name = "gl.xml and gl_ext.xml",
    year = date.today().year,
