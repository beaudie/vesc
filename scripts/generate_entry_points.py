...
# egl entry points whose code path that touches both sync and non-sync objects
def is_egl_entry_point_accessing_both_sync_and_non_sync_objects(cmd_name):
    if cmd_name in ["eglTerminate", "eglLabelObjectKHR", "eglReleaseThread"]:
        return True
    return False
...
def get_egl_lock(cmd_name):
    ...
    if is_egl_entry_point_accessing_both_sync_and_non_sync_objects(cmd_name):
