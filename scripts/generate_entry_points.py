...
{{
    ...
    g_push_debug_group(GL_DEBUG_SOURCE_THIRD_PARTY_KHR, static_cast<GLuint>(angle::EntryPoint::GL{name}), -1, entryPointMessage);

    //ALOGI("%s%s", "LLLLL Function is gl{name}  pushed message is ", entryPointMessage);
...
{{
    ...
    //ALOGI("%s", "LLLLL Function is gl{name}");

    // Call down to the next layer
    void *entry = funcMap["gl{name}"];
    PFNGL{name_upper}PROC next = reinterpret_cast<PFNGL{name_upper}PROC>(entry);
    {return_type} ret = next({non_packed_internal_params});
...
{{
    ...
    g_push_debug_group(GL_DEBUG_SOURCE_THIRD_PARTY_KHR, static_cast<GLuint>(angle::EntryPoint::EGL{name}), -1, entryPointMessage);

    //ALOGI("%s", "LLLLL Function is egl{name}");
...
{{
    ...
    g_push_debug_group(GL_DEBUG_SOURCE_THIRD_PARTY_KHR, static_cast<GLuint>(angle::EntryPoint::CL{name}), -1, entryPointMessage);

    //ALOGI("%s", "LLLLL Function is cl{name}");
...
{{
    ...
    void *entry = funcMap["cl{name}"];
    PFNCL{name} next = reinterpret_cast<PFNCL{name}>(entry);
    cl_int retval = next({non_packed_internal_params});
...
{{
    ...
    void *entry = funcMap["cl{name}"];
    PFNCL{name} next = reinterpret_cast<PFNCL{name}>(entry);
    {return_type} retval = next({non_packed_internal_params});
...
def main():
    ...
    for extension_name, ext_cmd_names in sorted(clxml.ext_data.items()):
        ...
        #### LUGMAL -- cl extensions have a differnet format for their PFN typedefs --
        #### normal is cl_api_clXxx
        #### extension is clIcdXxx_fn
        cl_layer_defs += eps.layer_defs
