...
def format_capture_replay_param_access(param_text_list, cmd_packed_enums):
    ...
    for i, param_text in enumerate(param_text_list):
        ...
        pointer_count = param_type.count('*')
        if pointer_count == 0:
            param_template = 'paramBuf.getParam("{name}", ParamType::T{enum_type}, {index}).getAs<{type}>()'
        elif pointer_count == 1:
            param_template = 'paramBuf.getParam("{name}", ParamType::T{enum_type}, {index}).getAsPointer<{type}>()'
        elif pointer_count == 2:
            param_template = 'paramBuf.getParam("{name}", ParamType::T{enum_type}, {index}).getAsPointerPointer<{type}>(&paramBuffer->pointersBuffer)'
        else:
...
def write_capture_replay_source(all_commands_nodes, gles_command_names, cmd_packed_gl_enums):
    ...
    call_replay_cases = '\n'.join([
        template_capture_replay_call_case.format(
            entry_point=command_name[2:],  # strip the 'gl' prefix 
            param_value_access=format_capture_replay_param_access(
                cmd_param_texts, get_packed_enums(cmd_packed_gl_enums, command_name)),
            context_call=command_name[2].lower() +
            command_name[3:],  # strip the 'gl' prefix and upper case first char
