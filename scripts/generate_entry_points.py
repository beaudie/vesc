...
def get_gles_decls(all_commands, gles_commands, already_included, is_gles1):
    decls = []
    for command in all_commands:
        proto = command.find('proto')
        cmd_name = proto.find('name').text
        ...
        if is_gles1 and cmd_name in gles1_overloaded:
            continue
    ...
    for major_version, minor_version in [[2, 0], [3, 0], [3, 1], [1, 0]]:
        ...
        if is_gles1:
            glesdecls['core'][(major_version, minor_version)] = get_gles_decls(
                all_commands, gles_commands, [], True)
        else:
            glesdecls['core'][(major_version, minor_version)] = get_gles_decls(
                all_commands, gles_commands, [], False)
