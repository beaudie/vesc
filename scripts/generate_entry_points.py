...
void GL_APIENTRY GL_{name}({params})
{{
    {event_comment}EVENT(GL{name}{comma_if_needed}{pass_params});

    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    gl{name}({non_packed_internal_params});
}}
"""
...
{{
    ...
    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    return gl{name}({non_packed_internal_params});
}}
"""
...
{{
    ...
    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    egl{name}({non_packed_internal_params});
}}
"""
...
{{
    ...
    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    return egl{name}({non_packed_internal_params});
}}
"""
...
{{
    ...
    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    cl{name}({non_packed_internal_params});
}}
"""
...
{{{initialization}
    ...
    // Call down to the next layer.  TODO/FIXME: DO THIS CORRECTLY!!!
    return cl{name}({non_packed_internal_params});
}}
"""
...
// found in the LICENSE file.
//
// layer_gles_{annotation_lower}_autogen.h:
//   Layer functions for the OpenGL ES {comment} entry points.

#ifndef LIBANGLE_LAYER_GLES_{annotation_upper}_AUTOGEN_H_
#define LIBANGLE_LAYER_GLES_{annotation_upper}_AUTOGEN_H_
...
#endif  // LIBANGLE_LAYER_GLES_{annotation_upper}_AUTOGEN_H_
"""
...
// found in the LICENSE file.
//
// layer_gles_{annotation_with_dash}_autogen.cpp:
//   Layer functions for the OpenGL ES {comment} entry points.

#include "libGLESv2/layer_gles_{annotation_with_dash}_autogen.h"
...
def write_layer_header(annotation, comment, protos):
    ...
    path = path_to("libGLESv2", "layer_gles_%s_autogen.h".format(annotation.lower()))
...
def write_layer_source(annotation_with_dash, annotation_no_dash, comment, defs):
    ...
    path = path_to("libGLESv2", "layer_gles_%s_autogen.cpp".format(annotation_with_dash))
...
def main():
    ...
    for major_version, minor_version in registry_xml.GLES_VERSIONS:
        ...
        write_layer_source(version, validation_annotation, "GLES " + comment, "\n".join(eps.defs))
    ...
    for major_version in sorted(
        ...
        for _, minor_version in filter(is_major, registry_xml.DESKTOP_GL_VERSIONS):
            ...
            eps_suffix = GLEntryPoints(apis.GL, glxml, just_libgl_commands_suffix, for_layer)
            eps = GLEntryPoints(apis.GL, glxml, all_libgl_commands, for_layer)
