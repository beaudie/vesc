...
def write_glext_explicit_context_inl(version, ptrs, protos):
    ...
    path = path_to("..\include\GLES{}".format(folder_version), 
        "gl{}ext_explicit_context.inl".format(version))
...
if(support_EGL_ANGLE_explicit_context):
    ...
    for major, minor in [[2, 0], [3, 0], [3, 1], [1, 0]]:
        ...
        glext_ptrs, glext_protos = get_glext_decls(all_commands,
            all_cmd_names.get_commands(annotation), version)

        glext_ext_ptrs = []
        glext_ext_protos = []

        # Append extensions for 1.0 and 2.0
        if(annotation is "1_0"):
            glext_ext_ptrs, glext_ext_protos = get_glext_decls(all_commands,
                all_cmd_names.get_commands("glext"), version)
        elif(annotation is "2_0"):
            glext_ext_ptrs, glext_ext_protos = get_glext_decls(all_commands,
                all_cmd_names.get_commands("gl2ext"), version)
