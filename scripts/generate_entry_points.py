...
{{
    ...
    {{
        ...
        ANGLE_EGL_CONTEXT_LOCK({name}, thread{comma_if_needed_context_mutex}{internal_context_mutex_params});
...
TEMPLATE_EGL_CONTEXT_MUTEX_HEADER = """\
// GENERATED FILE - DO NOT EDIT.
// Generated by {script_name} using data from {data_source_name}.
//
// Copyright 2020 The ANGLE Project Authors. All rights reserved.
...
// found in the LICENSE file.
//
// {annotation_lower}_context_mutex_autogen.h:
//   Context Lock functions for the {comment} entry points.

#ifndef LIBGLESV2_{annotation_upper}_CONTEXT_MUTEX_AUTOGEN_H_
#define LIBGLESV2_{annotation_upper}_CONTEXT_MUTEX_AUTOGEN_H_
...
#endif  // LIBGLESV2_{annotation_upper}_CONTEXT_MUTEX_AUTOGEN_H_
"""
...
TEMPLATE_CONTEXT_MUTEX_PROTO = "ANGLE_INLINE ContextMutex *GetContextMutex_%s(%s);"
...
def format_entry_point_def(api, command_node, cmd_name, proto, params, cmd_packed_enums,
    ...
    format_params = {
        ...
        "internal_context_mutex_params":
            ", ".join(internal_context_mutex_params),
        "initialization":
            initialization,
        "packed_gl_enum_conversions":
        ...
        "comma_if_needed_context_mutex":
            ", " if len(internal_context_mutex_params) > 0 else "",
        "gl_capture_params":
...
def get_context_mutex_params(api, cmd_name, params, cmd_packed_gl_enums, packed_param_types):
    packed_gl_enums = get_packed_enums(api, cmd_packed_gl_enums, cmd_name, packed_param_types,
                                       params)
    return ", ".join([
        make_param(
        ...
        for param in params
        if just_the_type_packed(param, packed_gl_enums) in
        ["Thread *", "egl::Display *", "gl::ContextID"]
    ])
...
def format_context_mutex_proto(api, cmd_name, params, cmd_packed_gl_enums, packed_param_types):
    with_extra_params = ["Thread *thread"] + params
    internal_params = get_context_mutex_params(api, cmd_name, with_extra_params,
                                               cmd_packed_gl_enums, packed_param_types)
    return TEMPLATE_CONTEXT_MUTEX_PROTO % (strip_api_prefix(cmd_name), internal_params)
...
class ANGLEEntryPoints(registry_xml.EntryPoints):
    ...
    def __init__(self,
        ...
        for (cmd_name, command_node, param_text, proto_text) in self.get_infos():
            ...
            if api == apis.EGL:
                self.context_mutex_protos.append(
                    format_context_mutex_proto(api, cmd_name, param_text, cmd_packed_enums,
                                               packed_param_types))
...
def write_context_mutex_header(annotation, comment, protos, source, template):
    content = template.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name=source,
        annotation_lower=annotation.lower(),
    ...
    path = path_to("libGLESv2", "%s_context_mutex_autogen.h" % annotation.lower())
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        outputs = [
            ...
            '../src/libGLESv2/egl_context_mutex_autogen.h',
            '../util/capture/frame_capture_replay_autogen.cpp',
        ]
    ...
    for major_version, minor_version in registry_xml.EGL_VERSIONS:
        ...
        egl_context_mutex_protos += [comment] + eps.context_mutex_protos
    ...
    write_validation_header("EGL", "EGL", egl_validation_protos, "egl.xml and egl_angle_ext.xml",
                            TEMPLATE_EGL_VALIDATION_HEADER)
    write_context_mutex_header("EGL", "EGL", egl_context_mutex_protos,
                               "egl.xml and egl_angle_ext.xml", TEMPLATE_EGL_CONTEXT_MUTEX_HEADER)
