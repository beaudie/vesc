...
def truncated_list(data, n):
    ...
    data_copy = list(data)
    for x in range(1, n):
      data_copy.remove(max(data_copy))
      data_copy.remove(min(data_copy))
    return data_copy
...
if len(sys.argv) < 2:
    print("Append test name to command line!")
    sys.exit(1)
...
while True:
    ...
    if (len(scores) > 7):
        ...
        sys.stdout.write("trunctated mean: " + str(truncated_mean(scores, trucation_n)))
        sys.stdout.write("trunctated stddev: " + str(truncated_stddev(scores, trucation_n)))
