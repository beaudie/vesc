...
def main(output_base_path):
    ...
    # parse find groups of GLenum and build a grouped
    # name to value mapping. note that one GLenum may be in multiple groups.
    ...
    # for enums do not have a group, add them to a default group
    default_group = registry_xml.default_enum_group_name
    gl_enum_in_groups[default_group] = dict()
    default_group = gl_enum_in_groups[default_group]
    for enum_name in exporting_enums:
        if enum_name not in enums_has_group:
            default_group[enum_name] = all_gl_enums[enum_name]

    # dump all GLenum groups into cpp enum and put in header file
    ...
    # dump GLenum value to string mapping to cpp switch-case code
