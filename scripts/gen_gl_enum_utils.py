...
def main(output_base_path):
    ...
    enums_has_group = set()
    for enums_group_dom in xml.root.findall('groups/group'):
        group_name = enums_group_dom.attrib['name']
        if group_name not in gl_enum_in_groups:
            gl_enum_in_groups[group_name] = dict()

        for enum_dom in enums_group_dom.findall('enum'):
            enum_name = enum_dom.attrib['name']
    ...
    header_content = template_gl_enums_header.format(
        ...
        gl_enum_groups=',\n'.join(gl_enum_in_groups.iterkeys()))
