...
# to the enum values
template_gl_enum_to_string_flattened_source = """// GENERATED FILE - DO NOT EDIT.
// Generated by {script_name} using data from {data_source_name}.
//
// Copyright {year} The ANGLE Project Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
//
// gl_enum_to_string_flattened_autogen.cpp:
//   mapping of GLenum value to string.

#include "tests/gl_tests/gl_enum_to_string_flattened.h"
...
{{
    {gl_enums_value_to_string_table_flattened}
}}
...
template_enum_to_string_flattened = """
    switch (value) {{
        {inner_group_cases}
        default:
            return kUnknownGLenumString;
...
def dump_value_to_string_mapping(gl_enum_in_groups, exporting_enums):
    ...
    for group_name, inner_mapping in gl_enum_in_groups.iteritems():
        ...
        exporting_string_value_pairs = removeDuplicatesFromSortedList(string_value_pairs)
...
def removeDuplicatesFromSortedList(string_value_pairs):
    '''
    remove all duplicate values from the pairs list
    The pairs list should be sorted on 2 attributes,
    the enum's unsigned ints and the enum string names
    which should be sorted lexicographically
    ...
    choose the lexicographically lesser value which comes first
    in the sorted list
    '''
    ...
    return exporting_string_value_pairs


def dump_value_to_string_mapping_flattened(all_gl_enums):
    string_value_pairs = list()
    string_value_pairs = list(all_gl_enums.iteritems())
    string_value_pairs.sort(key=lambda x: (x[1], x[0]))
    string_value_pairs = removeDuplicatesFromSortedList(string_value_pairs)

    # remove the excluded enums
    pairs_to_add = list()
    for name, value in string_value_pairs:
        if value > 0 and name not in exclude_gl_enums:
            pairs_to_add.append((name, value))
    string_value_pairs = pairs_to_add
    ...
    return template_enum_to_string_flattened.format(inner_group_cases=inner_code_block)


def main(header_output_path, source_output_path, gl_enums_to_string_flattened_source_output_path):
    ...
    # Write mapping to source file with flattened switch case layout
    gl_enums_value_to_string_table_flattened = dump_value_to_string_mapping_flattened(all_gl_enums)

    source_content_flattened = template_gl_enum_to_string_flattened_source.format(
        script_name=os.path.basename(sys.argv[0]),
        data_source_name="gl.xml and gl_angle_ext.xml",
        year=date.today().year,
        gl_enums_value_to_string_table_flattened=gl_enums_value_to_string_table_flattened,
    )

    gl_enums_to_string_flattened_source_output_path = registry_xml.script_relative(
        gl_enums_to_string_flattened_source_output_path)
    with open(gl_enums_to_string_flattened_source_output_path, 'w') as f:
        f.write(source_content_flattened)
...
if __name__ == '__main__':
    ...
    gl_enum_to_string_flattened_autogen_base_path = '../src/tests/gl_tests/gl_enum_to_string_flattened_autogen'
    outputs = [
        gl_enum_utils_autogen_base_path + '.h',
        gl_enum_utils_autogen_base_path + '.cpp',
        gl_enum_to_string_flattened_autogen_base_path + '.cpp',
    ]
    ...
    if len(sys.argv) > 1:
        ...
        sys.exit(
            main(
                registry_xml.script_relative(outputs[0]), registry_xml.script_relative(outputs[1]),
                registry_xml.script_relative(outputs[2])))
