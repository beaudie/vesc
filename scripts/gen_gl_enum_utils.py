...
std::string GLbitfieldToString(GLenumGroup enumGroup, unsigned int value) {{
    ...
    {{
        ...
        const char *enumString = GLenumToString(enumGroup, mask);
        if (enumString)
        {{
            st << enumString;
        }}
...
trivial_gl_enums = {
    'GL_FALSE', 'GL_TRUE', 'GL_ZERO', 'GL_ONE', 'GL_NO_ERROR', 'GL_TIMEOUT_IGNORED',
    'GL_INVALID_INDEX'
}
...
def main(header_output_path, source_output_path):
    ...
    for enums_group_node in xml.root.findall('groups/group'):
        group_name = registry_xml.enum_group_name_normalize(enums_group_node.attrib['name'])
    ...
    default_group_name = registry_xml.enum_group_name_normalize(
        registry_xml.default_enum_group_name)
