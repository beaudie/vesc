...
def main():
    ...
    ran_generators = generators
    running_selected_generators = False
    if (args.specified_generators):
        ran_generators = {k: v for k, v in generators.items() if k in args.specified_generators[0]}
        running_selected_generators = True

    if not ran_generators:
        print("No valid generators specified.")
        return 1

    for name, script in sorted(ran_generators.items()):
    ...
    if not running_selected_generators and any_old_hash_missing(all_new_hashes, all_old_hashes):
        any_dirty = True
