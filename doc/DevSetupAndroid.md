...
The following command will open a text editor to populate GN args for an Android Release ARM64 build:
```
gn args out/Android
```
...
[ANGLEChrome]: ANGLEChromeBuildingAngleForChromiumDevelopment.md

## Using ANGLE as the Android OpenGL ES driver

Starting with Android 10 (Q), you can load ANGLE as your phone's OpenGL ES driver.

`== Important Note ==` You can only run this ANGLE with *DEBUGGABLE APPS* or when you have
*ROOT ACCESS*. Debuggable apps are [marked debuggable][Debuggable] in the manifest. You get Root
Access from [userdebug][UserDebug] builds.
...
### Other relevant Android GN args:
