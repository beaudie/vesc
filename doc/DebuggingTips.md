...
Since right now we still build ANGLE as apk, it needs a little tweak to get ANGLE symbols into lldb. 
...
```
* Start your lldbclient.py from `/your_path_to_chromium_src/out/Debug` folder. This makes the angle source file paths matches to allow lldb find the source files. Refer to https://source.android.com/devices/tech/debug/gdb for how to attach the app for debugging.
* If you are debugging angle_perftests, you can use `--shard-timeout 100000000` to disable the timeout so that the test won't get killed while you are debugging. If the test runs too fast that you dont have time to attach, use `--delay-test-start=60` to give you extra time to attach.
