...
```
Change cmdline arguments to modify the default behavior. Change `--use_goma` to turn on/off 
building with goma, `--gtest_filter` to run only specific tests, `--keep_temp_files` to keep the
trace files, and `--output_to_file` to write the log to results.txt at 
[src/tests/capture_replay_tests][capture_replay_test_folder] folder. 
Change `--batch_count` to set the number of tests in a batch. More tests in a batch means that
...
[angle_folder]:https://chromium.googlesource.com/angle/angle/+/refs/heads/master
[capture_replay_test_folder]:https://chromium.googlesource.com/angle/angle/+/refs/heads/master/src/tests/capture_replay_tests/
