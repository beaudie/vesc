...
## Running a sample replay (desktop only)
...
Place all the trace output files into it.  For example, if our label was `desktop_test`:
```
...
+    "desktop_test 1",
     "disney_tsum_tsum 5",
```
Next run the script that generates the glue bringing your trace into the build:
```
# Must be run from the restricted_traces directory
vpython3 gen_restricted_traces.py
```
You should see a diff like this:
```
       'dep_type': 'cipd',
       'condition': 'checkout_angle_restricted_traces',
   },
+  'src/tests/restricted_traces/desktop_test': {
+      'packages': [
+        {
+            'package': 'angle/traces/desktop_test',
+            'version': 'version:1',
+        },
+      ],
+      'dep_type': 'cipd',
...
+  },
   'src/tests/restricted_traces/disney_tsum_tsum': {
       'packages': [
         {
```
From there you should be able to compile and run the perf test including your trace:
```
