...
```
is_debug = false (for a release build)
target_cpu = "x64"  (or "x86")
is_clang = false  (to use compiler other than clang)
```
You can create and name build directories as you please, for example `out/Release` with arg `is_debug = false` for a release build.
Each build directory has its own set of gn args.
