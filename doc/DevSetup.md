...
Note: Running the tests not using the runner is tricky, but is necessary in order to get TestResults.qpa from dEQP tests (since runner shards the tests, only the results of last shard will be available with it). First, use the runner to install APK and tests data and expectations on the device. After tests start running, runner can be stopped with Ctrl+C. Then, run
```
