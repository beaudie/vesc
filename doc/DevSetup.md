...
### Build Tools
GN is the supported build system.  GYP is deprecated and support will be removed in the future.
Report problems with GN in the [issue tracker](http://code.google.com/p/angleproject/issues/list).
There should be no need to use GYP but [instructions are available](gyp.md).

Clang is the default compiler and will be set up by the build system.
...
GN will generate ninja files by default.  If you make a change that requires it the ninja files should by regenerated automatically.  Ninja can be used to compile on all platforms.
```
ninja -C out/Debug
  or
ninja -C out/Release
```
Ninja is provided by `depot_tools` so make sure it is in your path.

GN can also generate a Visual Studio 2015 solution file as out/Debug/ANGLE.sln.
```
buildtools/linux64/gn gen out/Debug --ide=vs
```
For generating a Windows Store version of ANGLE view the [Windows Store instructions](BuildingAngleForWindowsStore.md).
...
Now that ANGLE has converted to GN it should be possible to add a standalone build for Android but at present it can be built only within a Chromium checkout.
It can only be built on Linux, following the steps in [Checking out and building Chromium for Android](https://chromium.googlesource.com/chromium/src/+/master/docs/android_build_instructions.md).
