...
Building ANGLE for Android heavily depends on Chromium toolchain, thus it is not possible to build ANGLE for Android within a standalone ANGLE checkout, but it should be built within a Chromium checkout instead. See anglebug.com/2344 for details.
...
```
Additional flags to build Vulkan backend, enable only if running on Android N or higher:
```
