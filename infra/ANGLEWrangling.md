...
- For Auto-Rollers,
  1. Open the [autoroll dashboard](https://autoroll.skia.org/) and put **angle** in "Filter".  This provides the status of ANGLE related rollers.  Monitor their modes and numbers.
  2. Filter out the non-critical emails by a filter: “**from:(*-autoroll (Gerrit))**”.  This improves the signal to noise ratio of inbox, so the important emails like **The roll is failing consistently. Time to investigate.** can stand out.
