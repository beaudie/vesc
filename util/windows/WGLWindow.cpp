...
WINAPI angle::GenericProc GetProcAddressWithFallback(const char *name)
{
...
bool WGLWindow::initializeGL(OSWindow *osWindow, angle::Library *glLibrary)
{
    glLibrary->getAs("wglGetProcAddress", &gCurrentWGLGetProcAddress);

    if (!gCurrentWGLGetProcAddress)
        return false;

    gCurrentModule = reinterpret_cast<HMODULE>(glLibrary->getNative());
