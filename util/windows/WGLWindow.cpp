...
std::vector<int> GetPixelFormatAttributes(const ConfigParameters &configParams, bool preservedSwap)
{
                             ...
                             const ConfigParameters &configParams)
{
    ...
    {
        std::cerr << "Driver does not expose wglChoosePixelFormatARB." << std::endl;
        return false;
    }

    std::vector<int> pixelFormatAttribs = GetPixelFormatAttributes(configParams, false);
    int pixelFormat                     = 0;
