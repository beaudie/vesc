...
bool ReadFromFile(bool blocking, HANDLE handle, std::string *out)
{
    char buffer[8192];
    DWORD bytesRead = 0;
...
void ReadEntireFile(HANDLE handle, std::string *out)
{
    ...
    while (true)
    {
        if (!ReadFromFile(true, handle, out))
            break;
    }
}
...
class WindowsProcess : public Process
{
  public:
    ...
    void getStdout(std::string *stdoutOut) override { *stdoutOut = mStdoutStream; }

    void getStderr(std::string *stderrOut) override { *stderrOut = mStderrStream; }

    bool finish() override
    {
        if (mStdoutPipe.valid())
        {
            ReadEntireFile(mStdoutPipe.readHandle, &mStdoutStream);
        }

        if (mStderrPipe.valid())
        {
            ReadEntireFile(mStderrPipe.readHandle, &mStderrStream);
        }
    ...
    bool finished() override
    {
        ...
        if (mStdoutPipe.valid())
        {
            ReadFromFile(false, mStdoutPipe.readHandle, &mStdoutStream);
        }

        if (mStderrPipe.valid())
        {
            ReadFromFile(false, mStderrPipe.readHandle, &mStderrStream);
        }
  ...
  private:
    ...
    Timer mTimer;
    std::string mStdoutStream;
    std::string mStderrStream;
};
}  // anonymous namespace
