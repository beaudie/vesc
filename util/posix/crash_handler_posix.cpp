...
#        if defined(ANGLE_HAS_ADDR2LINE)
        ...
        // Use fork/execv/wait, rather than system(), since system() has to parse the command line
        // arguments which:
        // 1) it doesn't have to, we have them split already
        // 2) can get into all sorts of trouble with quotations.
        pid_t pid = fork();
        if (pid < 0)
        {
        ...
        else
        {
            ...
            const std::array<const char *, 8> &commandLineArgs = {
