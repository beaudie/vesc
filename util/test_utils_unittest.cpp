...
// Just tests that the function doesn't crash.
TEST(TestUtils, SetLowPriorityProcess)
{
    SetLowPriorityProcess();
}

// Test creating and deleting temporary file.
TEST(TestUtils, CreateAndDeleteTemporaryFile)
{
    std::string path;
    ASSERT_TRUE(CreateTemporaryFile(&path));
    ASSERT_TRUE(!path.empty());

    const std::string kOutputString = "test output";

    FILE *fp = fopen(path.c_str(), "wt");
    ASSERT_NE(fp, nullptr);
    int retval = fputs(kOutputString.c_str(), fp);
    fclose(fp);
    ...
    EXPECT_EQ(actualString, kOutputString);

    // Delete the temporary file.
    EXPECT_TRUE(angle::DeleteFile(path.c_str()));
}

// Tests creating and deleting a file in the system temp dir.
TEST(TestUtils, CreateAndDeleteFileInTempDir)
{
    std::string tempDir;
    ASSERT_TRUE(GetTempDir(&tempDir));

    std::string path;
    ASSERT_TRUE(CreateTemporaryFileInDir(tempDir, &path));
    ASSERT_TRUE(!path.empty());
