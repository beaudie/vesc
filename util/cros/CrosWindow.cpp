...
static Key CodeToKey(Display *display, unsigned int scancode)
{
    int temp;
    KeySym *keySymbols;
...
static void AddKeyStateToEvent(Event *event, unsigned int state)
{
...
#ifdef XXX
void CrosWindow::processEvent(const XEvent &xEvent)
{
      ...
      case KeyPress:
        {
            ...
            event.Key.Code = CodeToKey(mDisplay, xEvent.xkey.keycode);
            AddKeyStateToEvent(&event, xEvent.xkey.state);
            pushEvent(event);
        }
        break;
