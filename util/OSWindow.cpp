...
bool FindTestDataPath(const char *searchPath, char *dataPathOut, size_t maxDataPathOutLen)
{
    ...
    for (const std::string &path : searchPaths)
    {
        ...
        if (angle::IsDirectory(candidatePath.c_str()))
        {
            ...
            strncpy_s(dataPathOut, candidatePath.c_str(), maxDataPathOutLen - 1);
            dataPathOut[maxDataPathOutLen - 1] = 0;
            return true;
        }
        ...
        {
            fprintf(stderr, "found %s\n", candidatePath.c_str());
            strncpy(dataPathOut, candidatePath.c_str(), maxDataPathOutLen - 1);
            dataPathOut[maxDataPathOutLen - 1] = 0;
            return true;
        }
    }
