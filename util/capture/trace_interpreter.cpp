...
{
  public:
    Parser(const std::string &stream) : mStream(stream), mIndex(0) {}
...
void PackIntParameter(ParamBuffer &params, ParamType paramType, const Token &token)
{
    ...
    if (token[0] == 'G')
    {
        ...
        if (strchr(token, '|') == 0)
        {
            value = gl::StringToGLenum(token);
        }
        else
        {
            value = gl::StringToGLbitfield(token);
        }
    }
    else
    {
...
void PackConstPointerParameter(ParamBuffer &params, ParamType paramType, const Token &token)
{
    ...
    else
    {
        ...
        params.addUnnamedParam(paramType, reinterpret_cast<const T *>(offset));
    }
}
}  // anonymous namespace
