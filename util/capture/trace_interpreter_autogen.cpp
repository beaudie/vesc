                             ...
                             const TraceStringMap &strings)
{
    ...
    if (strcmp(nameToken, "SetContextIDToCurrent") == 0)
    {
        ParamBuffer params = ParseParameters<decltype(SetContextIDToCurrent)>(paramTokens, strings);
        return CallCapture("SetContextIDToCurrent", std::move(params));
    }
...
void ReplayCustomFunctionCall(const CallCapture &call, const TraceFunctionMap &customFunctions)
{
    ...
    if (call.customFunctionName == "SetContextIDToCurrent")
    {
        DispatchCallCapture(SetContextIDToCurrent, captures);
        return;
    }
