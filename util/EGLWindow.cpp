...
bool EGLWindow::initializeDisplayAndSurface(OSWindow *osWindow)
{
    ...
    if (mMultiviewWindowViewCount.value() > 0)
    {
        displayAttributes.push_back(EGL_MULTIVIEW_VIEW_COUNT_EXT);
        displayAttributes.push_back(mMultiviewWindowViewCount.value());
    }
    ...
    if (hasMultiviewWindow && (mMultiviewWindowViewCount.value() > 0))
    {
        surfaceAttributes.push_back(EGL_MULTIVIEW_VIEW_COUNT_EXT);
        surfaceAttributes.push_back(mMultiviewWindowViewCount.value());
    }
