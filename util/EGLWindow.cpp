...
{
    debugLayersEnabled.reset();
    contextVirtualization.reset();
    platformMethods = nullptr;
    ...
    stencilBits  = -1;
    swapInterval = -1;
    webGLCompatibility.reset();
    robustResourceInit.reset();

    componentType = EGL_COLOR_COMPONENT_TYPE_FIXED_EXT;
    multisample   = false;
    debug         = false;
    noError       = false;
    extensionsEnabled.reset();
    bindGeneratesResource = true;
    clientArraysEnabled   = true;
    robustAccess          = false;
    samples               = -1;
    contextProgramCacheEnabled.reset();
}
