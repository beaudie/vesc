...
ScenicWindow::ScenicWindow()
    : loop_(GetDefaultLoop()),
      ...
      mNativeWindow(std::make_unique<FuchsiaEGLWindow>())
{}
...
bool ScenicWindow::initialize(const std::string &name, size_t width, size_t height)
{
    ...
    // Set up FuchsiaEGLWindow.
    mNativeWindow->magic           = FUCHSIA_EGL_WINDOW_MAGIC;
    mNativeWindow->size            = sizeof(*mNativeWindow);
    mNativeWindow->imagePipeHandle = ZX_HANDLE_INVALID;
    mNativeWindow->width           = width;
    mNativeWindow->height          = height;
...
{
    setNativeWindowImagePipe(ZX_HANDLE_INVALID);
}
...
void ScenicWindow::resetNativeWindow()
{
    ...
    setNativeWindowImagePipe(imagePipe.Unbind().TakeChannel().release());
}
...
{
    loop_->Run(zx::deadline_after({}), true /* once */);
}
...
void ScenicWindow::setNativeWindowImagePipe(zx_handle_t imagePipeHandle)
{
    if (mNativeWindow->imagePipeHandle != ZX_HANDLE_INVALID)
    {
        zx_handle_close(mNativeWindow->imagePipeHandle);
    }
    mNativeWindow->imagePipeHandle = imagePipeHandle;
}
