...
{
    return reinterpret_cast<EGLNativeWindowType>(&mImagePipeHandle);
}
...
{
    if (mImagePipeHandle != ZX_HANDLE_INVALID)
    {
        zx_handle_close(mImagePipeHandle);
    }
    mImagePipeHandle = imagePipeHandle;
}
